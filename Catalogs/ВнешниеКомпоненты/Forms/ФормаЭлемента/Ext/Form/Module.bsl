...
Процедура ЗагрузитьКомпонентуПослеПредупрежденияБезопасности(Ответ, Контекст) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Внешняя компонента (*.zip)|*.zip|Все файлы (*.*)|*.*';uk='Зовнішня компонента (*.zip)|*.zip|Всі файли (*.*)|*.*'");
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл внешней компоненты';uk='Виберіть файл зовнішньої компоненти'");
...
Процедура ЗагрузитьКомпонентуПослеПомещенияФайла(ПомещенныйФайл, Контекст) Экспорт
	...
	Если ПомещенныйФайл = Неопределено Тогда
		ЗагрузитьКомпонентуПриОтображенииОшибки(НСтр("ru='Не удалось загрузить файл компоненты.';uk='Не вдалося завантажити файл компоненти.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьКомпонентуПриОтображенииОшибки(ОписаниеОшибки = "")
	...
	Иначе 
		...
		СтрокаСПредупреждением = НСтр("ru='%1
            ...
            |Подробнее см. <a href = ""https://its.bas-soft.eu/db/metoddevu#content:8021071:hdoc"">Технология создания внешних компонент</a>'
            |;uk='%1
            |Необхідно вказати zip-архів з зовнішньої компонентою.
            |Детальніше див. <a href = ""https://its.bas-soft.eu/db/metoddevu#content:8021071:hdoc"">Технологія створення зовнішніх компонент</a>'");
