	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Справочник ""%1"" не может использоваться
                       ...
                       |
                       |Для восстановления связи с главным узлом запустите клиентское приложение BAF и
                       |нажмите кнопку Восстановить или программно установите главный узел,
                       |сохраненный в константе Главный узел.
                       |
                       |Для подтверждения отмены связи с главным узлом запустите клиентское приложение BAF и
                       ...
                       |
                       |Для відновлення зв''язку з головним вузлом запустіть клієнтську прикладну програму BAF і
                       |натисніть кнопку Відновити або програмно встановіть головний вузол,
                       |збережений у константі Головний вузол.
                       |
                       |Для підтвердження скасування зв''язку з головним вузлом запустіть клієнтську прикладну програму BAF і
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Некорректное значение параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных.
                           ...
                           |;uk='Деякі значення параметра ОпісаніеОб''ОписаниеОбъектаМетаданных в функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных.
                           |Вказано неіснуючий об''єкт метаданих: ""%1"".'"),
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректный тип параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных:
                       |""%1"".'
                       |;uk='Некоректний тип параметра ОписаниеОбъектаМетаданных в функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных:
                       |""%1"".'"),
			...
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Некорректное значение параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
                               |Указан несуществующий объект метаданных: ""%1"".'
                               |;uk='Некоректне значення параметра ОписаниеОбъектовМетаданных функції ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
                               |Вказано неіснуючий об''єкт метаданих: ""%1"".'"),
					ОписаниеОбъектаМетаданных);
			КонецЕсли;
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Некорректный тип параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
                           |""%1""'
                           |;uk='Некоректний тип параметра ОписаниеОбъектовМетаданных функції ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
                           |""%1""'"),
				ТипОписанияОбъектаМетаданных);
		КонецЕсли;
	...
	Попытка
		...
		Если Не ТолькоПроверка
		   ...
		   И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ИмяСобытия = НСтр("ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';uk='Ідентифікатори об''єктів метаданих.Потрібно завантажити критичні зміни'",ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если Не ТолькоПроверка Тогда
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ru='Идентификаторы объектов расширений.Выполнены критичные изменения';uk='Ідентифікатори об''єктів розширень.Виконані критичні зміни'",ОбщегоНазначения.КодОсновногоЯзыка()),
						НСтр("ru='Идентификаторы объектов метаданных.Выполнены критичные изменения';uk='Ідентифікатори об''єктів метаданих.Виконано критичні зміни'",ОбщегоНазначения.КодОсновногоЯзыка())),
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Если КоличествоОшибок > 0 Тогда
		...
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().'");
		Иначе
			ЗаголовокОшибки = НСтр("ru='Ошибки при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().';uk='Помилки при виконанні функції ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().'");
		КонецЕсли;
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				...
				ОписаниеОшибки = "... " + СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru=';И еще %1 ошибка;;И еще %1 ошибки;И еще %1 ошибок;И еще %1 ошибки';uk=';І ще %1 помилка;;І ще %1 помилки;І ще %1 помилок;І ще %1 помилки'"),
...
Функция ПредставлениеИдентификатора(Ссылка) Экспорт
	...
	Если ПолноеИмя = Неопределено Тогда
		Возврат НСтр("ru='Объект не найден.';uk='Об''єкт не знайдений.'")
	КонецЕсли;
...
Функция ВыполнитьЗаменуЭлементов(Знач Заменяемые, Знач ТаблицаСсылок, Знач ОтключатьКонтрольЗаписи = Ложь, Знач ОбъектыРасширений = Ложь)
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Значения не заменяются в данных типа: %1';uk='Значення не замінюються в даних типу: %1'"),
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru='Идентификаторы объектов расширений.Замена идентификатора';uk='Ідентифікатори об''єктів розширень.Заміна ідентифікатора'",ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru='Идентификаторы объектов метаданных.Замена идентификатора';uk='Ідентифікатори об''єктів метаданих.Заміна ідентифікатора'",ОбщегоНазначения.КодОсновногоЯзыка())),
