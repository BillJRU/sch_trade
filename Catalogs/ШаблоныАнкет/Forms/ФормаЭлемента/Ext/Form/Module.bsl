...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ДеревоШаблонаАнкеты.Строки[0].Строки.Найти("","Формулировка",Истина) <> Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не все формулировки или имена разделов заполнены.';uk='Не всі формулювання або імена розділів заповнені.'"),,"ДеревоАнкеты");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НайденныеСтроки.Количество() <> 0 Тогда
		Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.ТипВопроса <> Перечисления.ТипыВопросовШаблонаАнкеты.Табличный
				...
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не все вопросы заполнены.';uk='Не всі питання заповнені.'"),,"ДеревоАнкеты");
				Отказ = Истина;
				Прервать;
...
Процедура ФормаДереваАнкетыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	СписокВыбора.Добавить(НСтр("ru='Комплексный вопрос';uk='Комплексне питання'"));
...
Процедура ВыборТипаДобавляемогоЭлементаПриЗавершении(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранныйЭлемент = Неопределено Тогда
		...
		ИначеЕсли ВыбранныйЭлемент.Значение = НСтр("ru='Комплексный вопрос';uk='Комплексне питання'") Тогда
