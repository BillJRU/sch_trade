...
Процедура ПеренестиВШаблон(Команда)
	...
	Если Не ЗначениеЗаполнено(Формулировка) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнена формулировка';uk='Не заповнене формулювання'"),,"Формулировка");
	КонецЕсли;
	
	Если ТипСтрокиДерева = "Вопрос" И (Не ЗначениеЗаполнено(ЭлементарныйВопрос)) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не указан элементарный вопрос';uk='Не вказано елементарне питання'"),,"ЭлементарныйВопрос");
	КонецЕсли; 
