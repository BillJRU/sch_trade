...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если Элементы.ПериодСравнения.АвтоОтметкаНезаполненного
		...
		ТекстСообщения = НСтр("ru='Поле ""Период сравнения"" не заполнено';uk='Поле ""Період порівняння"" не заповнено'");
	...
	Если Элементы.ОбъектАнализа.АвтоОтметкаНезаполненного
		...
		ТекстСообщения = НСтр("ru='Поле ""Объект анализа"" не заполнено';uk='Поле ""Об''єкт аналізу"" не заповнено'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
	...
	Если Элементы.ЗначениеАнализаДополнительное.АвтоОтметкаНезаполненного
		...
		ТекстСообщения = НСтр("ru='Поле ""Доп. значение"" не заполнено';uk='Поле ""Дод. значення"" не заповнено'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
	...
	Если Не Объект.Ответственный.Пустая() Тогда
		Если Объект.НастройкиДоступности.НайтиСтроки(ОтборПоПользователю).Количество() = 0 Тогда
			СтрокаВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Сделать доступным вариант анализа для %1?';uk='Зробити доступним варіант аналізу для %1?'"),
...
| Продовжити з втратою демонстраційних даних?'");
...
Процедура УстановитьНадписьЗнаковПослеЗапятой(ЗнаковПослеЗапятой, Элементы)
	ЕдЧислоИменительный = НСтр("ru='знак';uk='знак'");
	ЕдЧислоРодительный = НСтр("ru='знака';uk='знака'");
	МнЧислоРодительный = НСтр("ru='знаков';uk='знаків'");
	...
	Элементы.НадписьЗнаковПослеЗапятой.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 после запятой';uk=' %1 після коми'"),
		ЧастьЗаголовка);
КонецПроцедуры
...
Процедура УстановитьНадписиНастроекДоступностиИОтчетов(Объект, Элементы)
	Если Объект.НастройкиДоступности.Количество() = 0 Тогда
		Элементы.НастроитьДоступность.Заголовок = НСтр("ru='настроить доступность';uk='настроїти доступність'");
	Иначе
		Элементы.НастроитьДоступность.Заголовок = НСтр("ru='изменить доступность';uk='змінити доступність'");
	КонецЕсли;
	
	Если Объект.ОтчетыДляРасшифровки.Количество() = 0 Тогда
		Элементы.НастроитьОтчеты.Заголовок = НСтр("ru='настроить отчеты';uk='настроїти звіти'");
	Иначе
		Элементы.НастроитьОтчеты.Заголовок = НСтр("ru='изменить отчеты';uk='змінити звіти'");
	КонецЕсли;
КонецПроцедуры
