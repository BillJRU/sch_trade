...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Заголовок = НСтр("ru='Помощник регистрации карты лояльности';uk='Помічник реєстрації карти лояльності'");
	КонецЕсли;
...
|;uk='Зчитайте нову карту лояльності за допомогою сканера штрихкоду
...
|;uk='Дані були змінені. 
|Перед завершенням роботи рекомендується зберегти зміни, 
|інакше змінені дані будуть загублені.'");
	...
	Если НоваяКартаПерсонизирована Тогда
		Если ЗаполнитьИнформациюОВладельцеКарты = "Сейчас" Тогда
			Если Не ЗначениеЗаполнено(Партнер) Тогда
				...
				ТекстСообщения = НСтр("ru='Поле ""Клиент"" не заполнено.';uk='Поле ""Клієнт"" не заповнено.'");
		...
		Иначе
			Если Элементы.ГруппаДоступа.Видимость и Не ЗначениеЗаполнено(ГруппаДоступа) Тогда
				...
				ТекстСообщения = НСтр("ru='Поле ""Группа доступа"" не заполнено.';uk='Поле ""Група доступу"" не заповнено.'");
...
Процедура УстановитьЗаголовок(Страница)
	Если Страница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВыборРежимаРегистрацииКартыЛояльности Тогда
		Заголовок = НСтр("ru='Помощник регистрации карт лояльности';uk='Помічник реєстрації карт лояльності'");
	...
	Если НайденныеКартыЛояльностиСчитываниеНовойКартыЛояльности.Количество() = 0 Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрихкод ""%1"" не відповідає виду карт лояльності ""%2""'"), ПервыйКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
...
Функция ОбработатьВторойКодНовойКартыНаСервере(Данные, Предобработка)
	...
	Если Не Результат Тогда
		...
		Если ТипВторогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрихкод ""%1"" не відповідає виду карт лояльності ""%2""'"), ВторойКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
...
Функция ОбработатьПервыйКодЗаменяемойКартыНаСервере(Данные, ТипКода, Предобработка)
	...
	Если НайденныеКартыЛояльностиСчитываниеЗаменяемойКартыЛояльности.Количество() > 0 Тогда
		...
		Если ТипКодаЗаменяемойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрихкод ""%1"" не відповідає виду карт лояльності ""%2""'"), КодЗаменяемойКарты, ТребуемыйВидНовойКарты);
			Иначе
