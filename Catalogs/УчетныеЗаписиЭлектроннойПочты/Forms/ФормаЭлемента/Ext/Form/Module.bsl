...
Процедура ПротоколПриИзменении(Элемент)
	...
	Элементы.СерверВходящейПочты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Сервер %1';uk='Сервер %1'"), Объект.ПротоколВходящейПочты);
...
Процедура УстановитьВидГруппыТребуетсяАвторизация(Форма, ИспользуетсяПротоколPOP)
	...
	Если ИспользуетсяПротоколPOP Тогда
		Форма.Элементы.ТребуетсяАвторизацияПриОтправкеПисем.Заголовок = НСтр("ru='При отправке писем требуется авторизация';uk='При відправці листів необхідна авторизація'");
	Иначе
		Форма.Элементы.ТребуетсяАвторизацияПриОтправкеПисем.Заголовок = НСтр("ru='При отправке писем требуется авторизация на сервере исходящей почты (SMTP)';uk='При відправці листів необхідна авторизація на сервері вихідної пошти (SMTP)'");
	КонецЕсли;
