...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Обновление персональных учетных записей';uk='Оновлення персональних облікових записів'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Наименование                     = НСтр("ru='Системная учетная запись';uk='Системний обліковий запис'",ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.ИмяПользователя                  = НСтр("ru='BAS';uk='BAS'");
...
Функция ПроверитьПодключениеКСерверуВходящейПочты(Профиль, Протокол)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Проверка подключения к почтовому серверу';uk='Перевірка підключення до поштового сервера'",ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	Тема = НСтр("ru='Тестовое сообщение BAS';uk='Тестове повідомлення BAS'");
	Тело = НСтр("ru='Это сообщение отправлено подсистемой электронной почты BAS';uk='Це повідомлення надіслано підсистемою електронної пошти BAS'");
	ИмяОтправителяПисем = НСтр("ru='BAS';uk='BAS'");
...
Функция ВариантыНастройкиПодключенияКСерверуIMAP(АдресЭлектроннойПочты) Экспорт
	...
	// Стандартные настройки, подходящие для ящиков gmail и ukr.net
...
Функция ПроверитьНастройкиПрофилей(ПрофильИсходящейПочты, ПрофильВходящейПочты, АдресЭлектроннойПочты) Экспорт
	...
	Если ПрофильИсходящейПочты <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Если ОшибкаАутентификации(ТекстОшибки) Тогда
				ТекстСообщения.Добавить(НСтр("ru='Отправка тестового сообщения не выполнена: не удалось авторизоваться.';uk='Відправка тестового повідомлення не виконана: не вдалося авторизуватися.'"));
			Иначе
				ТекстСообщения.Добавить(НСтр("ru='Отправка тестового сообщения не выполнена.';uk='Відправка тестового повідомлення не виконана.'"));
			...
			КонецЕсли;
		Иначе
			ВыполненныеПроверки.Добавить("- " + НСтр("ru='Выполнена отправка тестового сообщения.';uk='Виконана відправка тестового повідомлення.'"));
		КонецЕсли;
	...
	Если ПрофильВходящейПочты <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ОшибкаАутентификации = ОшибкаАутентификации(ТекстОшибки);
			Если ОшибкаАутентификации(ТекстОшибки) Тогда
				ТекстСообщения.Добавить(НСтр("ru='Подключение к серверу входящей почты не выполнено: не удалось авторизоваться.';uk='Підключення до сервера вхідної пошти не виконано: не вдалося авторизуватися.'"));
			Иначе
				ТекстСообщения.Добавить(НСтр("ru='Подключение к серверу входящей почты не выполнено.';uk='Підключення до сервера вхідної пошти не виконано.'"));
			...
			КонецЕсли;
		Иначе
			ВыполненныеПроверки.Добавить("- " + НСтр("ru='Выполнено подключение к серверу входящей почты.';uk='Ви підключені до сервера вхідної пошти.'"));
		КонецЕсли;
		
	КонецЕсли;
	
	ТехническиеПодробности.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Адрес почты: %1';uk='Адреса пошти: %1'"), АдресЭлектроннойПочты));
	...
	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		...
		Если ОшибкаАутентификации Тогда
			Рекомендации.Добавить(НСтр("ru='Проверьте правильность ввода логина и пароля, а также выбранный способ авторизации.';uk='Перевірте правильність введення логіна і пароля, а також обраний спосіб авторизації.'"));
		КонецЕсли;
		Если ТребуетсяПроверкаНастроекИсходящейПочты Тогда
			Рекомендации.Добавить(НСтр("ru='Проверьте настройки подключения к серверу исходящей почты.';uk='Перевірте настройки підключення до сервера вихідної пошти.'"));
		КонецЕсли;
		Если ТребуетсяПроверкаНастроекВходящейПочты Тогда
			Рекомендации.Добавить(НСтр("ru='Проверьте настройки подключения к серверу входящей почты.';uk='Перевірте настройки підключення до сервера вхідної пошти.'"));
		КонецЕсли;
		Если ТребуетсяПроверкаНастроекИсходящейПочты Или ТребуетсяПроверкаНастроекВходящейПочты Тогда
			Рекомендации.Добавить(НСтр("ru='Обратитесь к администратору локальной сети.';uk='Зверніться до адміністратора локальної мережі.'"));
		КонецЕсли;
		...
		ОшибкиПодключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1
            ...
            |%2
            |
            |Зверніться до адміністратора поштового сервера ""%3"".
            |
            |============================
            |
            |Інформація для техпідтримки:
            |
	...
	Если ЗначениеЗаполнено(Профиль.АдресСервераSMTP) Тогда
		...
		Для Каждого ИмяСвойства Из СтрРазделить(СписокСвойствSMTP, ",", Ложь) Цикл
			Настройки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1=""%2""';uk='%1=""%2""'"), ИмяСвойства, Профиль[ИмяСвойства]));
		КонецЦикла;
...
Функция ИнформацияОПрограмме()
	...
	Результат = Новый Массив;
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Операционная система: %1';uk='Операційна система: %1'"), СистемнаяИнформация.ВерсияОС));
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Платформа: %1 (%2)';uk='Платформа: %1 (%2)'"),
		СистемнаяИнформация.ВерсияПриложения, СистемнаяИнформация.ТипПлатформы));
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Конфигурация: %1 (%2)';uk='Конфігурація: %1 (%2)'"), 
		?(ЗначениеЗаполнено(Метаданные.Синоним), Метаданные.Синоним, Метаданные.Имя), Метаданные.Версия));
		
	Результат.Добавить(Символы.ПС + НСтр("ru='Расширения:';uk='Розширення:'"));
	
	УстановитьПривилегированныйРежим(Истина);
	Для Каждого Расширение Из РасширенияКонфигурации.Получить() Цикл
		Если Расширение.Активно Тогда
			Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2)';uk='%1 (%2)'"),
