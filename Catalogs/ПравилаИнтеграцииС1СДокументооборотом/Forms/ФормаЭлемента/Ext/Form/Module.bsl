...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ПредставлениеОбъектаДО) Тогда 
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru='Объект BAS Документооборота';uk='Об''єкт BAS Документообігу'"));
		...
		Для Каждого Строка из ПравилаЗаполненияРеквизитовДО.ПолучитьЭлементы() Цикл
			Если Строка.ОбязательноеЗаполнение 
				...
				Сообщение.Текст = СтрШаблон(
					НСтр("ru='Не установлено правило для обязательного %1 ""%2""';uk='Не встановлено правило для обов''язкового %1 ""%2""'"),
					?(Строка.ЭтоТаблица, НСтр("ru='списка';uk='списку'"), НСтр("ru='реквизита';uk='реквізиту'")),
...
|;uk='Таблиця заповнюється результатом виразу на вбудованій мові, 
|тому правила для окремих реквізитів не потрібні.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПравилаЗаполненияРеквизитовДОПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru='Очистить правило заполнения?';uk='Очистити правило заповнення?'");
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузке(Команда)
	...
	Если НужноЗадатьВопрос Тогда
		...
		ТекстВопроса = НСтр("ru='Правила будут заполнены по умолчанию. Заполнить?';uk='Правила будуть заповнені по умовчанню. Заповнити?'");
...
Функция СписокОбъектовДокументооборота(Контрагенты)
	...
	Если Контрагенты Тогда
		...
		Иначе
			Список.Добавить("DMCorrespondent",    НСтр("ru='Контрагент';uk='Контрагент'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокПечатныхФорм()
	...
	ЗаголовокЗакладки = НСтр("ru='Добавление печатных форм';uk='Додавання друкованих форм'");
	...
	Если Не ЗаполненВШаблоне
		...
		Для Каждого ВидДокумента Из ОбъектыXDTO.objects Цикл
			Если ВидДокумента.Свойства().Получить("templateRequired") <> Неопределено
				...
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = СтрШаблон(
					НСтр("ru='В настройках вида документа ""%1"" установлен запрет на создание документов не по шаблону, но шаблон документа не выбран.';uk='В настройках виду документа ""%1"" встановлено заборону на створення документів не за шаблоном, але шаблон документа не вибрано.'"),
