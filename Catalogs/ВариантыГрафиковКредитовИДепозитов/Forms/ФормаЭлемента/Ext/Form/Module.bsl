	...
	Если ЗначениеЗаполнено(СуммаЛимита) Тогда
		...
		Если СуммаТраншей > СуммаЛимита Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Сумма траншей превосходит заданный лимит в %1 %2 по договору!';uk='Сума траншів перевершує заданий ліміт в %1 %2 за договором!'"), СуммаЛимита, ВалютаДоговора),
				,,, Отказ);
		КонецЕсли;
	КонецЕсли;
...
|;uk='Рядки з нульовими сумами не можуть бути записані! 
|Вилучіть весь рядок цілком.'");
		КонецЕсли;
...
Процедура НастроитьФормуПоДоговору()
	...
	Элементы.ГруппаТранши.Заголовок       = СтрШаблон(НСтр("ru='Всего транши (%1)';uk='Всього транші (%1)'"), СтрокаВалюта);
	...
	Элементы.ТраншиСумма.Заголовок        = СтрШаблон(НСтр("ru='Сумма (%1)';uk='Сума (%1)'"), СтрокаВалюта);
	Элементы.ОплатыСумма.Заголовок        = СтрШаблон(НСтр("ru='Сумма основного долга (%1)';uk='Сума основного боргу (%1)'"), СтрокаВалюта);
	Элементы.ОплатыПроценты.Заголовок     = СтрШаблон(НСтр("ru='Сумма процентов (%1)';uk='Сума відсотків (%1)'"), СтрокаВалюта);
	Элементы.ОплатыКомиссия.Заголовок     = СтрШаблон(НСтр("ru='Сумма комиссии (%1)';uk='Сума комісії (%1)'"), СтрокаВалюта);
	Элементы.НачисленияПроценты.Заголовок = СтрШаблон(НСтр("ru='Сумма процентов (%1)';uk='Сума відсотків (%1)'"), СтрокаВалюта);
	Элементы.НачисленияКомиссия.Заголовок = СтрШаблон(НСтр("ru='Сумма комиссии (%1)';uk='Сума комісії (%1)'"), СтрокаВалюта);
...
Функция НаборГрафика(ТипГрафика, Ссылка)
	...
	Набор = РеквизитФормыВЗначение(ТипГрафика, Тип("РегистрСведенийНаборЗаписей." + ТипГрафика + "КредитовИДепозитов")); // РегистрСведенийНаборЗаписей
...
Функция СуммыГрафиковСходятся()
	...
	Если Не СуммыДолгаРавны Тогда
		Текст  = НСтр("ru='Различаются итоговые суммы основного долга на закладках ""Транши"" и ""Оплаты"".';uk='Розрізняються підсумкові суми основного боргу на закладках ""Транші"" і ""Оплати"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
	
	Если Не ПроцентыРавны Тогда
		Текст  = НСтр("ru='Различаются итоговые суммы процентов на закладках ""Оплаты"" и ""Начисления"".';uk='Розрізняються підсумкові суми відсотків на закладках ""Оплати"" і ""Нарахування"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
	
	Если ЕстьКомиссия И Не КомиссияРавна Тогда
		Текст  = НСтр("ru='Различаются итоговые суммы комиссии на закладках ""Оплаты"" и ""Начисления"".';uk='Розрізняються підсумкові суми комісії на закладках ""Оплати"" і ""Нарахування"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
