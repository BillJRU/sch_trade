...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	ПО ВнешниеПользователиКонтактныеЛица.ОбъектАвторизации = ЭтотСписок.Ссылка
	|;
	|РазрешитьЧтениеИзменение
	|ГДЕ
...
Функция РезультатСозданияКонтактногоЛица(ДанныеЗаполнения) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.Наименование) Тогда
		...
		ТекстОшибки = НСтр("ru='Не заполнено наименование';uk='Не заповнено найменування'");
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.Партнер) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровИКонтрагентов") Тогда
			ИмяПартнера = НСтр("ru='партнер';uk='партнер'");
		Иначе
			ИмяПартнера = НСтр("ru='контрагент';uk='контрагент'");
		КонецЕсли;
		
		ТекстОшибки = СтрШаблон(НСтр("ru='Не заполнен %1';uk='Не заповнений %1'",ИмяПартнера));
		РезультатСоздания.Ошибки.Добавить(ТекстОшибки);
		Отказ = Истина;
