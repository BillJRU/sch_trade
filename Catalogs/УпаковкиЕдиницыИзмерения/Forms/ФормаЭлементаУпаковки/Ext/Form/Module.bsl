...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Знаменатель <> 0 
		...
		Если Не ЕстьЕдиничнаяУпаковкаСЕдиницейИзмерения(Объект.Ссылка, Объект.Владелец, Объект.ЕдиницаИзмерения) 
			...
			Иначе
				ТекстСообщения = НСтр("ru='Для набора упаковок ""%Владелец%"" уже создана упаковка с коэффициентом 1 и единицей измерения ""%ЕдиницаИзмерения%"".?';uk='Для набору упаковок ""%Владелец%"" вже створена упаковка з коефіцієнтом 1 і одиницею виміру ""%ЕдиницаИзмерения%"".?'");
			КонецЕсли;
...
Процедура НастроитьФорму()
	...
	Иначе
		ПредставлениеЕдиницыИзмерения =  НСтр("ru='<ед. изм. по классификатору>';uk='<од. вим. за класифікатором>'");
	КонецЕсли;
	...
	Если ИспользоватьОрдерныеСклады Тогда
		...
		ТекстПредставления = НСтр("ru='Входит в состав %ЕдиницаИзмеренияВладельца% в количестве';uk='Входить до складу %ЕдиницаИзмеренияВладельца% в кількості'");
		ТекстПредставления = СтрЗаменить(ТекстПредставления, "%ЕдиницаИзмеренияВладельца%", ЕдиницаИзмеренияВладельца);
...
Функция СформироватьСтрокуВариантаПодбораТары(НомерВарианта)
	...
	Если НомерВарианта = 0 Тогда
		Если ЗначениеЗаполнено(ЕдиницаИзмеренияВладельца) Тогда
			СтрВарианта = СтрШаблон(НСтр("ru=' полностью (%1 %2)';uk='Повністю (%1 %2)'"),КоличествоЕдиницВУпаковке,ЕдиницаИзмерения);	
		Иначе
			СтрВарианта = СтрШаблон(НСтр("ru=' полностью (%1)';uk='Повністю (%1)'"),КоличествоЕдиницВУпаковке);
		КонецЕсли;
	ИначеЕсли НомерВарианта = 1 Тогда
		СтрВарианта = НСтр("ru=' начиная с';uk='починаючи з'");		
	КонецЕсли;
	Элементы.НадписьЕдиницаИзмерения.Заголовок = СтрШаблон(НСтр("ru='%1';uk='%1'"),ЕдиницаИзмерения);
