		...
		Если ДанныеЗаполнения.Свойство("ТипИзмеряемойВеличины",ТипИзмеряемойВеличины)
			...
			Если ТипЗнч(ДанныеЗаполнения.Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда
				...
				Если Не ЗначениеРеквизитов.ИспользоватьУпаковки Тогда
					ТекстИсключения = НСтр("ru='В карточке номенклатуры %Номенклатура% не включено использование упаковок. Создание упаковки невозможно.';uk='У картці номенклатури %Номенклатура% не включене використання упаковок. Створення упаковки неможливо.'");
...
Процедура ПроверитьКратностьЧисловыхРеквизитов(Отказ)
	...
	СообщениеКратностьБолее = НСтр("ru='Кратность более 10 000 000 / 1';uk='Кратність більш 10 000 000/1'");
	СообщениеКратностьМенее = НСтр("ru='Кратность менее 1 / 99 999 999';uk='Кратність менше 1/99 999 999'");
	...
	СообщениеКратностьБолее = НСтр("ru='Кратность к %ПредставлениеБазовойЕдиницыИзмерения% более 99 999 999 / 1';uk='Кратність до %ПредставлениеБазовойЕдиницыИзмерения% більше 99 999 999/1'");
	СообщениеКратностьМенее = НСтр("ru='Кратность к %ПредставлениеБазовойЕдиницыИзмерения% менее 1 / 10 000 000';uk='Кратність до %ПредставлениеБазовойЕдиницыИзмерения% менше 1/10 000 000'");
	Если ТипИзмеряемойВеличины = Перечисления.ТипыИзмеряемыхВеличин.Упаковка Тогда
