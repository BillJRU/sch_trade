...
Функция ТекстЗапросаПлощадьУпаковки(ИсточникУпаковки, ИсточникНоменклатуры, ПлощадьУпаковкиВЕдиницеКонстанты = Истина) Экспорт 
	...
	Если ПлощадьУпаковкиВЕдиницеКонстанты Тогда
		...
		Если Не ЗначениеЗаполнено(ЕдиницаИзмеренияПлощади) Тогда
			ВызватьИсключение НСтр("ru='Не задано значение константы ""Единица измерения площади"". Обратитесь к администратору.';uk='Не задано значення константи ""Одиниця вимірювання площі"". Зверніться до адміністратора.'");
		КонецЕсли;
...
Функция ТекстЗапросаДлинаУпаковки(ИсточникУпаковки, ИсточникНоменклатуры, ДлинаУпаковкиВЕдиницеКонстанты = Истина) Экспорт 
	...
	Если ДлинаУпаковкиВЕдиницеКонстанты Тогда
		...
		Если Не ЗначениеЗаполнено(ЕдиницаИзмеренияДлины) Тогда
			ВызватьИсключение НСтр("ru='Не задано значение константы ""Единица измерения Длины"". Обратитесь к администратору.';uk='Не задано значення константи ""Одиниця вимірювання Довжини"". Зверніться до адміністратора.'");
		КонецЕсли;
...
//  * Ключ - Код единицы измерения по классификатору
//  * Значение - ссылка на единицу измерения или Неопределено, если такого кода нет в классификаторе.
//
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("194275fb-cb2a-4eb0-87b1-ea2d55ada0b3");
	Обработчик.Процедура = "Справочники.УпаковкиЕдиницыИзмерения.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.ЧитаемыеОбъекты = "Справочник.УпаковкиЕдиницыИзмерения";
	Обработчик.ИзменяемыеОбъекты = "Справочник.УпаковкиЕдиницыИзмерения";
	Обработчик.Комментарий = НСтр("ru='Заполняет типы измеряемых величин и коэффициент пересчета единиц измерения в соответствии с поставляемым классификатором. Заполняет владельцев у тех единиц измерений, у которых они не заполнены.';uk='Заповнює типи вимірюваних величин і коефіцієнт перерахунку одиниць вимірювання відповідно до класифікатора, що поставляється. Заповнює власників тих одиниць вимірювань, у яких вони не заповнені.'");
...
Процедура ЗаполнитьВладельцевУНекорректныхЕдиницИзмерений()
	...
	Пока Выборка.Следующий() Цикл
		...
		// Если тип измеряемой величины не заполнен, но есть код по СОЕИ, тогда попробуем восстановить тип из классификатора
...
// Функция возвращает дерево значений с данными классификатора.
// Возвращаемое значение:
// 	ДеревоЗначений - содержит в том числе:
// 		* Наименование - Строка
//
