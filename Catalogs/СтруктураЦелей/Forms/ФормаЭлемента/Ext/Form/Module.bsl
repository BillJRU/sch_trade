...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредставлениеСхемы = НСтр("ru='Произвольный';uk='Довільний'");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЦельИзмеримая Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущийОбъект.СхемаКомпоновкиДанных) 
			...
			Иначе
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Необходимо настроить шаблон расчета целевого показателя.';uk='Необхідно настроїти шаблон розрахунку цільового показника.'");
