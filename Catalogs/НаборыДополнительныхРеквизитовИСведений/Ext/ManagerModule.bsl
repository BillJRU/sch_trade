...
Процедура ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обработать набор свойств: %1 по причине:
                    |%2'
                    |;uk='Не вдалося обробити набір властивостей: %1 через:
                    |%2'"), 
	...
	Если ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("ru='Процедура ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию завершилась с ошибкой. Не все наборы свойств удалось обновить.';uk='Процедура ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию завершилася з помилкою. Не всі набори властивостей вдалося оновити.'");
...
Функция СвойстваНабора(НаборыСвойств, Набор, Родитель = Неопределено, Наименования = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ПриСозданииПредопределенныхНаборовСвойств
                   ...
                   |;uk='Помилка в процедурі ПриСозданииПредопределенныхНаборовСвойств
                   |загального модуля УправлениеСвойствамиПереопределяемый.'")
		+ Символы.ПС
		+ Символы.ПС;
	
	Если Не ЗначениеЗаполнено(Набор.Имя) Тогда
		ВызватьИсключение ЗаголовокОшибки + НСтр("ru='Имя набора свойств не заполнено.';uk='Ім''я набору властивостей не заповнено.'");
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(Набор.Имя) <> Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Имя набора свойств ""%1"" уже определено.';uk='Ім''я набору властивостей ""%1"" вже визначено.'"),
			Набор.Имя);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Набор.Идентификатор) Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Идентификатор набора свойств ""%1"" не заполнен.';uk='Ідентифікатор набору властивостей ""%1"" не заповнений.'"),
			Набор.Имя);
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(НаборСсылка) <> Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Идентификатор ""%1"" набора свойств
                       ...
                       |;uk='Ідентифікатор ""%1"" набору властивостей 
                       |""%2"" вже використовується для набору ""%3"".'"),
