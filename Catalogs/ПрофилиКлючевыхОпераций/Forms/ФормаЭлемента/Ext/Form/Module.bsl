...
Процедура ЗагрузитьПрофильКлючевыхОпераций(Команда)
	...
	Если ЕстьБазоваяФункциональность Тогда
		...
		Если ТипЗнч(МодульФайловаяСистемаКлиент) = Тип("ОбщийМодуль") Тогда
			...
			ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл профиля ключевых операций';uk='Виберіть файл профілю ключових операцій'");
	...
	Иначе          		
		...
		НСтр("ru='Выберите файл профиля ключевых операций';uk='Виберіть файл профілю ключових операцій'"),
...
Процедура СохранитьПрофильКлючевыхОпераций(Команда)
	...
	Если ЕстьБазоваяФункциональность Тогда
		...
		Если ТипЗнч(МодульФайловаяСистемаКлиент) = Тип("ОбщийМодуль") Тогда
			...
			ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Сохранить профиль ключевых операций в файл';uk='Зберегти профіль ключових операцій у файл'");
	...
	Иначе              		
		ДополнительныеПараметры = Новый Структура("Режим, Заголовок, ОповещениеОЗакрытии",  
			...
			НСтр("ru='Сохранить профиль ключевых операций в файл';uk='Зберегти профіль ключових операцій у файл'"),
...
Процедура ДиалогВыбораФайлаПоказать(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СообщитьПользователю(НСтр("ru='Без расширения работы с файлами невозможно работать с файлами.';uk='Без розширення роботи з файлами неможливо працювати з файлами.'"), "Объект");
	КонецЕсли;
КонецПроцедуры
