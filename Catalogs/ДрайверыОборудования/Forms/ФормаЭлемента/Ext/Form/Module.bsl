...
Функция ПолучитьОписаниеРевизииИнтерфейса(РевизияИнтерфейса)
	...
	Результат = Символы.НПП + НСтр("ru='(Версия требований к разработке драйверов %Версия%)';uk='(Версія вимог до розробки драйверів %Версия%)'");
...
Процедура ОбновитьТекущийСтатусДрайвера()
	...
	Если НоваяАрхитектура И ИнтеграционныйКомпонент Тогда
		ТекущийСтатусДрайвера = НСтр("ru='Установлен интеграционный компонент.';uk='Встановлено інтеграційний компонент.'") + Символы.НПП;
...
Процедура ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, Параметры) Экспорт
	...
	Если ПустаяСтрока(Объект.ИдентификаторОбъекта) И ПоставляемыйВСоставеКонфигурации Тогда
		ТекущийСтатусДрайвера = НСтр("ru='Установка драйвера не требуется.';uk='Встановлення драйверів не потрібне.'");
...
Процедура ОбновитьСостояниеЭлементов();
	...
	Если НЕ ПустаяСтрока(ИмяФайлДрайвераНаФорме) Или ПоставляемыйВСоставеКонфигурации Тогда
		...
		Иначе
			ДополнительнаяИнформация = НСтр("ru='Драйвер поставляется в виде архива.';uk='Драйвер поставляється у виді архіву.'")
				+ ?(ПустаяСтрока(Объект.ВерсияДрайвера), "", Символы.ПС + НСтр("ru='Версия драйвера в архиве:';uk='Версія драйвера в архіві:'")
