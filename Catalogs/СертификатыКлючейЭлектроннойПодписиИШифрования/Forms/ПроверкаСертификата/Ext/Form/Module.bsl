...
Процедура ИнформацияДляПоддержкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ТекстОшибок = НСтр("ru='Результат поверки на клиенте';uk='Результат перевірки на клієнті'") + ":" + Символы.ПС;
	ДополнитьТекстОшибками(ТекстОшибок, СоставПроверок);
	
	ТекстОшибок = ТекстОшибок + Символы.ПС
		+ НСтр("ru='Результат поверки на сервере';uk='Результат перевірки на сервері'") + ":" + Символы.ПС;
...
Процедура ПроверитьНаСторонеКлиента(Оповещение)
	...
	Если СтандартныеПроверки Тогда
		ЭлектроннаяПодписьКлиент.УстановитьРасширение(Ложь, Новый ОписаниеОповещения(
			...
			НСтр("ru='Для продолжения необходимо установить расширение для работы с криптографией.';uk='Для продовження необхідно встановити розширення для роботи з криптографією.'"));
	Иначе
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Результат.Сертификат) Тогда
		ОписаниеОшибки = НСтр("ru='Сертификат отсутствует в облачном сервисе (возможно удален).';uk='Сертифікат відсутній в хмарному сервісі (можливо вилучений).'");
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			ЭлементКартинка.Подсказка = ?(ОписаниеОшибки = Неопределено,
				НСтр("ru='Проверка не выполнялась.';uk='Перевірка не виконувалася.'"),
				НСтр("ru='Проверка выполнена успешно.';uk='Перевірка виконана успішно.'"));
...
Функция СтандартныйЗаголовокЭлемента(ИмяЭлемента)
	...
	Если ИмяЭлемента = "ЗаконныйСертификатНадпись" Тогда
		Возврат НСтр("ru='Соответствие законодательству';uk='Відповідність законодавству'");
	ИначеЕсли ИмяЭлемента = "НаличиеСертификатаНадпись" Тогда
		Возврат НСтр("ru='Наличие сертификата в личном списке';uk='Наявність сертифіката в особистому списку'");
	ИначеЕсли ИмяЭлемента = "ДанныеСертификатаНадпись" Тогда
		Возврат НСтр("ru='Корректность данных сертификата';uk='Коректність даних сертифіката'");
	ИначеЕсли ИмяЭлемента = "НаличиеПрограммыНадпись" Тогда
		Возврат НСтр("ru='Наличие программы для подписания и расшифровки';uk='Наявність програми для підписання і розшифровки'");
	ИначеЕсли ИмяЭлемента = "ПодписаниеНадпись" Тогда
		Возврат НСтр("ru='Подписание данных';uk='Підписання даних'");
	ИначеЕсли ИмяЭлемента = "ПроверкаПодписиНадпись" Тогда
		Возврат НСтр("ru='Проверка созданной подписи';uk='Перевірка створеної підпису'");
	ИначеЕсли ИмяЭлемента = "ШифрованиеНадпись" Тогда
		Возврат НСтр("ru='Шифрование данных';uk='Шифрування даних'");
	ИначеЕсли ИмяЭлемента = "РасшифровкаНадпись" Тогда
		Возврат НСтр("ru='Расшифровка данных';uk='Розшифровка даних'");
	Иначе
		Возврат "";
	КонецЕсли;
	...
	Для Каждого Проверка Из Проверки Цикл
		...
		Если СоставПроверок = Неопределено Тогда
			ТекстОшибок = ТекстОшибок + Проверка + ": "
				+ НСтр("ru='Не запускалась';uk='Не запускалася'") + Символы.ПС;
		Иначе
			...
			ТекстОшибок = ТекстОшибок + Проверка + ": "
				+ ?(РезультатПроверки = Неопределено, НСтр("ru='Не выполнялась';uk='Не виконувалася'"),
				Формат(РезультатПроверки, НСтр("ru='БЛ=Ошибка; БИ=Успех';uk='БЛ=Помилка; БИ=Успіх'")))
