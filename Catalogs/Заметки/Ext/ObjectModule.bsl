...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(Родитель) И Родитель.Автор <> Автор Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Нельзя указывать группу другого пользователя.';uk='Не можна вказувати групу іншого користувача.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не ЭтоГруппа Тогда 
		...
		Если ПустаяСтрока(Тема) Тогда 
			Тема = "<" + НСтр("ru='Пустая заметка';uk='Порожня нотатка'") + ">";
		КонецЕсли;
