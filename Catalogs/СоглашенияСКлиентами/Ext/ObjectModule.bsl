...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ИспользоватьГрафикиОплаты
		...
		Если ТребуетсяЗалогЗаТару Тогда
			...
			Если ЭтапыГрафикаОплаты.Итог("ПроцентЗалогаЗаТару") <> 100 Тогда	
				ТекстОшибки = НСтр("ru='Процент залога за тару по всем этапам оплаты должен быть равен 100%';uk='Відсоток застави за тару по всіх етапах оплати має дорівнювати 100%'");
	...
	Если Регулярное Тогда
		...
		Если Не ЗначениеЗаполнено(СуммаДокумента) Тогда
			...
			ТекстОшибки = НСтр("ru='В регулярном соглашении необходимо заполнить поле ""Сумма""';uk='У регулярній оферті необхідно заповнити поле ""Сума""'");
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала Тогда
		Если Не ИспользуютсяДоговорыКонтрагентов Тогда
			...
			ТекстОшибки = НСтр("ru='При использовании ""Поставки под принципала"" необходимо использовать договор с контрагентами.';uk='При використанні ""Поставки під принципала"" необхідно використовувати договір з контрагентами.'");
		...
		Если Не ОбеспечиватьЗаказыОбособленно Тогда
			ТекстОшибки = НСтр("ru='При использовании ""Поставки под принципала"" необходимо использовать обособленное обеспечение.';uk='При використанні ""Поставки під принципала"" необхідно використовувати відокремлений забезпечення.'");
...
Процедура ИнициализироватьСправочник(ЗаполнятьВсеРеквизиты = Истина)
	...
	Если ЗаполнятьВсеРеквизиты Тогда
    	...
    	ПараметрыЗаполнения = Справочники.СоглашенияСКлиентами.ПараметрыЗаполненияНалогообложенияНДСПродажи(ЭтотОбъект);
