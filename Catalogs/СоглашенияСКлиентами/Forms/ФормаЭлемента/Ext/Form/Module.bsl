	...
	Если Объект.ДоступноВнешнимПользователям
		...
		Если Элементы.ИспользованиеПрайсЛиста.Доступность Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Для соглашений, доступных внешним пользователям должны использоваться цены из Прайс-листа.';uk='Для оферт, які доступні зовнішнім користувачам повинні використовуватися ціни з Прайс-листа.'"), ,
...
|;uk='Для оферт, які доступні зовнішнім користувачам
|повинні використовуватися ціни з Прайс-листа.
|Можливість зазначення ціни з Прайс-листа буде доступна тільки при включеному прапорі ""Ціна включає ПДВ""'"), Объект.Ссылка,
					"ЦенаВключаетНДС",
					"Объект",Отказ);
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
|;uk='Для оферт, які доступні зовнішнім користувачам
|повинні використовуватися ціни з Прайс-листа. 
|Можливість зазначення ціни з Прайс-листа буде доступна тільки при вимкненому прапорі ""Ціна включає ПДВ""'"), Объект.Ссылка,
					"ЦенаВключаетНДС",
					"Объект",Отказ);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОплатаВВалютеПриИзменении(Элемент)
	...
	ЗаполнитьГруппуФинансовогоУчета(Объект.ВалютаВзаиморасчетов, Объект.ХозяйственнаяОперация, Объект.ГруппаФинансовогоУчета);
...
Процедура ЗаполнитьВидЦенВыделенныхСтрокТовары(Команда)
	...
	Если Объект.Товары.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru='В документе не заполнен список ""Товары"". Вид цен не может быть заполнен';uk='У документі не заповнений список ""Товари"". Вид цін не може бути заповнений'"));
		Возврат;
...
Процедура ЗаполнитьВидЦенВыделенныхСтрокЦеновыеГруппы(Команда)
	...
	Если Объект.ЦеновыеГруппы.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru='В документе не заполнен список ""Ценовые группы"". Вид цен не может быть заполнен';uk='У документі не заповнений список ""Цінові групи"". Вид цін не може бути заповнений'"));
		Возврат;
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
		...
		ЗаполнитьОплатуВВалютеГруппуФинансовогоУчета(Объект.ВалютаВзаиморасчетов,
													Объект.ОплатаВВалюте,
													Объект.ГруппаФинансовогоУчета,
													Объект.ИспользуютсяДоговорыКонтрагентов,
													Объект.ХозяйственнаяОперация);
...
Процедура ЗаполнитьГруппуФинансовогоУчета(ВалютаВзаиморасчетов, ХозяйственнаяОперация, ГруппаФинансовогоУчета)
	
	ГруппаФинансовогоУчета = Справочники.ГруппыФинансовогоУчетаРасчетов.ПолучитьГруппуФинансовогоУчетаПоУмолчанию(ВалютаВзаиморасчетов, ХозяйственнаяОперация);
...
Процедура НастроитьЭлементыДоговорыКонтрагентовИПорядокРасчетов()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
		...
		ТекстИспользованияПередачиНаХранение = НСтр("ru='Договоры не используются, порядок расчетов по накладным';uk='Договори не використовуються, порядок розрахунків за накладними'")
		...
		ТекстИспользуютсяДоговорыКонтрагентов = ?(Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию,
												НСтр("ru='Договоры не используются, порядок расчетов по отчетам по комиссии';uk='Договори не використовуються, порядок розрахунків за звітами по комісії'"),
	...
	ДоступностьИспользованиеДоговоров = Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи
		И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала;
	...
	Элементы.ГруппаФинансовогоУчета.Доступность = НЕ Объект.ИспользуютсяДоговорыКонтрагентов;
	Элементы.СтатьяДвиженияДенежныхСредств.Доступность = НЕ Объект.ИспользуютсяДоговорыКонтрагентов;
	Элементы.НаправлениеДеятельности.Доступность = НЕ Объект.ИспользуютсяДоговорыКонтрагентов;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьОплатуВВалютеГруппуФинансовогоУчета(Валюта, ОплатаВВалюте, ГруппаФинансовогоУчета, ИспользуютсяДоговорыКонтрагентов, ХозяйственнаяОперация)
	
	Если НЕ ИспользуютсяДоговорыКонтрагентов Тогда
		ОплатаВВалюте = ВзаиморасчетыСервер.ПолучитьОплатуВВалютеПоУмолчанию();
	КонецЕсли;
	
	ЗаполнитьГруппуФинансовогоУчета(Валюта, ХозяйственнаяОперация, ГруппаФинансовогоУчета);
...
Процедура УстановитьНалогообложениеНДСПоУмолчанию(ПриОткрытииФормы = Ложь)
	...
	ПараметрыЗаполнения = Справочники.СоглашенияСКлиентами.ПараметрыЗаполненияНалогообложенияНДСПродажи(Объект);
	УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(НовоеНалогообложениеНДСПоУмолчанию, ПараметрыЗаполнения);
	УчетНДСУП.ЗаполнитьСписокВыбораНалогообложенияНДСПродажи(Элементы.НалогообложениеНДСПоУмолчанию, НовоеНалогообложениеНДСПоУмолчанию, ПараметрыЗаполнения); 
...
Процедура УстановитьЗаголовокФлагаДоступноВнешнимПользователям()
	...
	Если ВнешниеПользователиИспользуются И ОбменССайтомИспользуется Тогда
		Элементы.ДоступноВнешнимПользователям.Заголовок = НСтр("ru='Доступно внешним пользователям и для обмена с сайтом';uk='Доступна зовнішнім користувачам та для обміну з сайтом'");
