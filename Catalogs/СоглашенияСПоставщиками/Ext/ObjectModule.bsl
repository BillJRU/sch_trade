...
Процедура ИнициализироватьСправочник(ЗаполнятьВсеРеквизиты = Истина)
	...
	Если ЗаполнятьВсеРеквизиты Тогда
    	...
    	ПараметрыЗаполнения = Справочники.СоглашенияСПоставщиками.ПараметрыЗаполненияНалогообложенияНДСЗакупки(ЭтотОбъект);
...
Процедура ПроверитьАгентскиеУслуги(Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для данной хозяйственной операции необходимо указание организации';uk='Для даної господарської операції необхідно зазначення організації'"),ЭтотОбъект,"Организация",,Отказ);
	КонецЕсли;
	...
	ШаблонСообщения = НСтр("ru='Обнаружено существующее соглашение %Наименование%%ДляУслуги%';uk='Виявлено існуючу оферту %Наименование%%ДляУслуги%'");
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%Наименование%", Строка(Выборка.Наименование));
		Если ЗначениеЗаполнено(Выборка.Номенклатура) Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДляУслуги%", " " + НСтр("ru='для услуги';uk='для послуги'") + " " 
