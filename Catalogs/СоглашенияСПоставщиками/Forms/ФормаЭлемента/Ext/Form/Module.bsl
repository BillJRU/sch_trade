...
|;uk='При закритті оферти необхідно, щоб дата закінчення дії відповідала даті закриття.
|Скорегувати дату закінчення дії?'");
		Иначе
			ТекстВопроса = НСтр("ru='При закрытии соглашения необходимо, чтобы дата окончания действия была заполнена.
|Скорректировать дату окончания действия?'
|;uk='При закритті оферти необхідно, щоб дата закінчення дії була заповнена.
|Скорегувати дату закінчення дії?'");
		КонецЕсли;
...
Процедура УстановитьНалогообложениеНДСПоУмолчанию(ПриОткрытииФормы = Ложь)
	...
	Иначе
    	...
    	ПараметрыЗаполнения = Справочники.СоглашенияСПоставщиками.ПараметрыЗаполненияНалогообложенияНДСЗакупки(Объект);
        УчетНДСУП.ЗаполнитьНалогообложениеНДСЗакупки(НовоеНалогообложениеНДСПоУмолчанию, ПараметрыЗаполнения);
    	УчетНДСУП.ЗаполнитьСписокВыбораНалогообложенияНДСЗакупки(Элементы.НалогообложениеНДСПоУмолчанию, НовоеНалогообложениеНДСПоУмолчанию, ПараметрыЗаполнения); 
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи Тогда
		...
		Для Каждого ЭтапГрафикаОплаты Из Объект.ЭтапыГрафикаОплаты Цикл
			...
			Если ЭтапГрафикаОплаты.ВариантОплаты <> Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления Тогда
				...
				ТекстОшибки = ТекстОшибки + НСтр("ru='при установленной хозяйственной операции ""Прием на хранение""';uk='при встановленій господарської операції ""Приймання на зберігання""'");
...
Процедура РассчитатьКоличествоАгентскихУслуг()
	...
	ТекстЗаголовка = НСтр("ru='Перейти (%Количество% шт.)';uk='Перейти (%Количество% шт.)'");
