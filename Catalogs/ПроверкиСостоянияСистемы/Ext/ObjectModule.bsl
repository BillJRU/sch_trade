	...
	Если РегламентноеЗаданиеОбъект <> Неопределено Тогда
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при удалении регламентного задания: %1';uk='Помилка при вилученні регламентного завдання: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Удаление регламентного задания';uk='Вилучення регламентного завдання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбновитьДанныеРегламентногоЗадания(Отказ, РасписаниеРегламентногоЗадания, ТекущийОбъект) Экспорт
	...
	Если РегламентноеЗаданиеОбъект <> Неопределено Тогда
		...
		НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка состояния системы: %1';uk='Перевірка стану системи: %1'"), СокрЛП(ТекущийОбъект.Наименование));
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Произошла ошибка при сохранении расписания выполнения проверки. Подробное описание ошибки: %1';uk='Сталася помилка при збереженні розкладу виконання перевірки. Детальний опис помилки: %1'"), 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Изменение регламентного задания';uk='Зміна регламентного завдання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.РегламентныеЗадания.ПроверкаСостоянияСистемы,
				,
				ТекстСообщения);
