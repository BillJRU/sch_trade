...
Процедура ЗаписатьСвойстваДопРеквизитов(НазначениеДопРеквзитов, Отказ)
	...
	|	
	|	ВЫБРАТЬ
	|		ВТРеквизитыДляКонтроля.Свойство,
	|		ВТРеквизитыДляКонтроля.ЗаполнятьОбязательно,
	|		ВТРеквизитыДляКонтроля.ЭтоОбщийРеквизит,
	|		-1
	|	ИЗ
	...
	Запрос.УстановитьПараметр("РеквизитыДляКонтроля", РеквизитыДляКонтроля.Выгрузить(Новый Структура("ЭтоДопРеквизит", Истина)));
	...
	НаборыСвойств      = Результаты[2].Выгрузить();
		...
		Иначе
			...
			Если ВыборкаПоСвойствам.ЗаполнятьОбязательно Тогда
				...
				Если Не СвойствоОбъект.ЗаполнятьОбязательно Тогда
					...
					СвойствоОбъект.ЗависимостиДополнительныхРеквизитов.Очистить();
					
					НайденныеСтроки = НаборыСвойств.НайтиСтроки(Новый Структура("Свойство", ВыборкаПоСвойствам.Свойство));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ТекстСообщенияНеЗаполненаПолитика = НСтр("ru='Не заполнена колонка ""Политика учета серий"" для склада ""%1"" списка ""Политики учета серий"".';uk='Не заповнена колонка ""Політика обліку серій"" для складу ""%1"" списку ""Політики обліку серій"".'");
	ТекстСообщенияНеЗаполненСклад = НСтр("ru='Не заполнена колонка ""Склад"" в списке ""Политики учета серий"".';uk='Не заповнена колонка ""Склад"" в списку ""Політики обліку серій"".'");
...
Процедура ЕстьИндивидуальныеНастройкиПолитикиУчетаСерийПриИзменении(Элемент)
	...
	Если Объект.ПолитикиУчетаСерий.Количество() > 0
		...
		Если ВТЧУказываютсяСкладыИПодразделения Тогда 
			ТекстВопроса = НСтр("ru='Для всех складов и производственных подразделений будет установлена общая политика учета серий.';uk='Для всіх складів і виробничих підрозділів буде встановлена загальна політика обліку серій.'");
		ИначеЕсли ИспользоватьНесколькоСкладов Тогда
			ТекстВопроса = НСтр("ru='Для всех складов будет установлена общая политика учета серий.';uk='Для всіх складів буде встановлена загальна політика обліку серій.'");
		Иначе
			ТекстВопроса = НСтр("ru='Для всех производственных подразделений будет установлена общая политика учета серий.';uk='Для всіх виробничих підрозділів буде встановлена загальна політика обліку серій.'");
		КонецЕсли;
...
|;uk='Змінилися параметри, що впливають на можливість вибору політик зазначення серій. 
|Політики, які не задовольняють новим параметрам будуть вилучені. Продовжити?'");
...
Процедура РеквизитыДляКонтроляНоменклатурыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	РеквизитыДляКонтроляВыбор("Номенклатуры", ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура РеквизитыДляКонтроляХарактеристикВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	РеквизитыДляКонтроляВыбор("Характеристик", ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура РеквизитыДляКонтроляСерийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	РеквизитыДляКонтроляВыбор("Серий", ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура НастроитьФорму()
	...
	Если ЗначениеЗаполнено(ОтборПоПолитике) Тогда
		...
		Если ЗначениеЗаполнено(Объект.ОсобенностьУчета)
			...
			ОтборПоОсобенностиУчетаОписание = Новый ФорматированнаяСтрока(НСтр("ru='Для вида номенклатуры с особенностью учета ""%ОсобенностьУчета%"" возможно использование';uk='Для виду номенклатури з особливістю обліку ""%ОсобенностьУчета%"" можливе використання'") + Символы.НПП,
											,
		...
		ИначеЕсли Объект.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
			ЕстьОтборПоТипуПолитикиОписание.Добавить(Новый ФорматированнаяСтрока(
											НСтр("ru='Для серий, идентифицирующих экземпляр товара, возможно использование';uk='Для серій, які ідентифікують примірник товару, можливе використання'") + Символы.НПП,
											,
											ЦветаСтиля.ПоясняющийТекст));
		Иначе
			ЕстьОтборПоТипуПолитикиОписание.Добавить(Новый ФорматированнаяСтрока(
											НСтр("ru='Для этого вида номенклатуры возможно использование';uk='Для цього виду номенклатури можливе використання'") + Символы.НПП,
											,
											ЦветаСтиля.ПоясняющийТекст));
		КонецЕсли;
		
		ЕстьОтборПоТипуПолитикиОписание.Добавить(Новый ФорматированнаяСтрока(
										НСтр("ru='только';uk='тільки'") + Символы.НПП,
										Новый Шрифт(, , Истина),
										ЦветаСтиля.ПоясняющийОшибкуТекст));
		ОтборПоТипуПолитикиОписание     = НСтр("ru='политики учета серий с типом ""%ТипПолитикиУчетаСерий%""';uk='політики обліку серій з типом ""%ТипПолитикиУчетаСерий%""'");
		...
		Если ИспользоватьПередачуНаХранение Тогда
			ОписанияОпераций.Добавить(НСтр("ru='""Учет серий в переданных на хранение товарах""';uk='""Облік серій в переданих на зберігання товарах""'"));
		КонецЕсли;
		
		Если ИспользоватьТоварыВПутиОтПоставщиков Тогда
			ОписанияОпераций.Добавить(НСтр("ru='""Учет товаров в пути от поставщика по сериям""';uk='""Облік товарів в дорозі від постачальника по серіях""'"));
		КонецЕсли;
		
		Если ИспользоватьНеотфактурованныеПоставки Тогда
			ОписанияОпераций.Добавить(НСтр("ru='""Учет серий в неотфактурованных поставках товаров""';uk='""Облік серій в невідфактурованих поставках товарів""'"));
		КонецЕсли;
		
		ТекстВключенныхОпций = ?(ОписанияОпераций.Количество() = 0, "",
								?(ОписанияОпераций.Количество() > 1, НСтр("ru='со снятыми флагами';uk='зі знятими прапорами'"),
								НСтр("ru='со снятым флагом';uk='зі знятим прапором'")));
	...
	Если ВТЧУказываютсяСкладыИПодразделения Тогда
		ПолитикиУчетаСерийСкладЗаголовок       = НСтр("ru='Склад или подразделение';uk='Склад або підрозділ'");
		ПолитикиУчетаСерийСкладПодсказка       = НСтр("ru='Склад или подразделение, в котором ведется учет по сериям';uk='Склад чи підрозділ, в якому ведеться облік по серіях'");
		...
		ПолитикаУчетаСерийПодсказка
			= НСтр("ru='Будет использована как общая политика для всех складов и производственных подразделений: будет устанавливаться по умолчанию для новых складов и производственных подразделений.';uk='Буде використана як загальна політика для всіх складів і виробничих підрозділів: буде встановлюватися по умовчанню для нових складів і виробничих підрозділів.'");
	ИначеЕсли ИспользоватьНесколькоСкладов Тогда
		ПолитикиУчетаСерийСкладЗаголовок       = НСтр("ru='Склад';uk='Склад'");
		ПолитикиУчетаСерийСкладПодсказка       = НСтр("ru='Склад, в котором ведется учет по сериям';uk='Склад, в якому ведеться облік по серіях'");
		...
		ПолитикаУчетаСерийПодсказка
			= НСтр("ru='Будет использована как общая политика для всех складов; будет устанавливаться по умолчанию для новых складов.';uk='Буде використана як загальна політика для всіх складів; буде встановлюватися по умовчанню для нових складів.'");
	Иначе
		ПолитикиУчетаСерийСкладЗаголовок       = НСтр("ru='Подразделение';uk='Підрозділ'");
		ПолитикиУчетаСерийСкладПодсказка       = НСтр("ru='Подразделение, в котором ведется учет по сериям';uk='Підрозділ, в якому ведеться облік по серіях'");
		...
		ПолитикаУчетаСерийПодсказка
			= НСтр("ru='Будет использована как общая политика для всех производственных подразделений; будет устанавливаться по умолчанию для новых производственных подразделений.';uk='Буде використана як загальна політика для всіх виробничих підрозділів; буде встановлюватися по умовчанню для нових виробничих підрозділів.'");
	КонецЕсли;
		...
		Для Каждого Описание Из НастройкиИспользованияСерий.ОписанияИспользованияРеквизитовСерии Цикл
			Если Описание.ИмяНастройки = "ИспользоватьRFIDМеткиСерии" Тогда
				...
				Если Описание.ИмяРеквизита = "RFIDTID"
					...
					МассивОписанияРеквизитов.Добавить(НСтр("ru='RFID-метка';uk='RFID-мітка'"));
				КонецЕсли;
			...
			ИначеЕсли Описание.Использование Тогда
				...
				Если Не ПустаяСтрока(Описание.ПредставлениеФорматнойСтрокиВЗаголовке) Тогда
					...
					ПредставлениеРеквизита = НСтр("ru='%ПредставлениеРеквизита% (%ТочностьУказания%)';uk='%ПредставлениеРеквизита% (%ТочностьУказания%)'");
	...
	Если ЭтоУслуга
	 ...
	 Или ЭтоНабор Тогда
		...
		ЗаголовокГруппыЕдиницаИзмерения = НСтр("ru='Единицы измерения';uk='Одиниці вимірювання'");
	Иначе
		ЗаголовокЕдиницыИзмерения = НСтр("ru='Ед. хранения';uk='Од. зберігання'");
		ЗаголовокГруппыЕдиницаИзмерения = НСтр("ru='Единицы измерения и условия хранения';uk='Одиниці вимірювання та умови зберігання'");
	КонецЕсли;
...
Процедура НастроитьПоШаблонуНаименования(ЗначащаяЧастьИмениШаблона)
	...
	Если СтрНайти(ЗначащаяЧастьИмениШаблона, "Серии") = 0 Тогда
		ПолеФормы = Элементы["ЗапретРедактирования" + ЗначащаяЧастьИмениШаблона]; // ПолеФормы
		ПолеФормы.Доступность = Не ПустаяСтрока(Объект["Шаблон" + ЗначащаяЧастьИмениШаблона]);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеРеквизитыДляКонтроля(ЗначащаяЧастьИмени)
	...
	//Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ГиперссылкаЦвет);
 	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru='БЛ=''всегда''; БИ=''с условием''';uk='БЛ=''завжди''; БИ=''з умовою'''"));
	...
	Если ЗначащаяЧастьИмени <> "Серий" Тогда 
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<для доп. реквизитов>';uk='<для дод. реквізитів>'"));
		...
		//Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ГиперссылкаЦвет);
		Элемент.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru='БЛ=''доступен всегда''; БИ=''доступен с условием''';uk='БЛ=''доступний завжди''; БИ=''доступний з умовою'''"));
		...
		//Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ГиперссылкаЦвет);
		Элемент.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru='БЛ=''виден всегда''; БИ=''виден с условием''';uk='БЛ=''видимий завжди''; БИ=''видимий з умовою'''"));
...
Процедура ВыбратьВидВладелец(ЗначащаяЧастьИмениНадписи)
	...
	ОтборФормыВыбора = ЭтотОбъект["ОтборВладельца" + ЗначащаяЧастьИмениНадписи];
	...
	Если Особенности.НуженТекстВНадписи Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(ВидВладелец) Тогда
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='<вид не выбран>';uk='<вид не вибрано>'"),Новый Шрифт(,,Истина),ЦветаСтиля.ЦветОсобогоТекста));
				МассивСтрок.Добавить(" ");
		...
		КонецЕсли;
	КонецЕсли;
	ЭлементФормы = Элементы["Владелец" + ЗначащаяЧастьИмениНадписи]; // ДекорацияФормы
...
Процедура ЗаполнитьРеквизитыФормыПоРеквизитамОбъекта()
	...
	ОтборВладельцаТоварныхКатегорий = НоменклатураСервер.ОтборВладельцаПодчиненныхСправочников(Объект, "ТоварныхКатегорий");
	ОтборВладельцаХарактеристик     = НоменклатураСервер.ОтборВладельцаПодчиненныхСправочников(Объект, "Характеристик");
	ОтборВладельцаСерий             = НоменклатураСервер.ОтборВладельцаПодчиненныхСправочников(Объект, "Серий");
...
&НаКлиенте
Процедура РеквизитыДляКонтроляВыбор(ЗначащаяЧастьИмени, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыОбработки = Новый Структура;
	ПараметрыОбработки.Вставить("ВыбраннаяСтрока", ВыбраннаяСтрока);
	ПараметрыОбработки.Вставить("ЗначащаяЧастьИмени", ЗначащаяЧастьИмени);
	ПараметрыОбработки.Вставить("Поле", Поле);
	
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru='Перед открытием формы настройки необходимо записать элемент. Продолжить?';uk='Перед відкриттям форми настройки необхідно записати елемент. Продовжити?'"); 	
...
Процедура РеквизитыДляКонтроляВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекущаяСтрока         = Объект["РеквизитыДляКонтроля" + ЗначащаяЧастьИмени].НайтиПоИдентификатору(ВыбраннаяСтрока);
	РедактируемоеСвойство = ТекущаяСтрока.Свойство;
		
	Если Не ЗначениеЗаполнено(РедактируемоеСвойство) Тогда
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			ТекущийНаборСвойств = Объект["НаборСвойств" + ЗначащаяЧастьИмени];
		КонецЕсли;
...
Процедура НастроитьФормуЛокализация()
	...
	#Область ТТН
	ВидимостьНаименованийДляТТН = Ложь;
    ...
    ПеречислениеСтавкаНДС = НДСКлиентСерверПовтИсп.ПеречислениеСтавкаНДСПоСтавкеНДС(Объект.СтавкаНДС);
	ЭтоСтавкаБезНДСИлиНеНДС = (ПеречислениеСтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС")) ИЛИ (ПеречислениеСтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НеНДС"));
    ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
        Элементы,
        "ЛьготаНДС",
        "Доступность",
