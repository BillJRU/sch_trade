...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ИспользоватьСерии
		...
		Если Не Отказ
			...
			ТекстСообщения = НСтр("ru='Включено использование серий, но политика учета серий не назначена ни одному складу.';uk='Включено використання серій, але політика обліку серій не призначена жодному складу.'");
		...
		Если Не ЕстьВключеннаяНастройка Тогда
			ТекстСообщения = НСтр("ru='Включено использование серий, но не настроено, какие реквизиты серии будут использоваться. Необходимо выключить использование хотя бы одного реквизита.';uk='Включено використання серій, але не налаштованО, які реквізити серії будуть використовуватися. Необхідно виключити використання хоча б одного реквізиту.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,Ссылка,,,Отказ);
		КонецЕсли;
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВладелецСерий) Тогда
			ТекстСообщения = НСтр("ru='Сделана настройка, что серии используются также, как в другом виде номенклатуры, но при этом не выбран этот вид.';uk='Зроблена настройка, що серії використовуються так само, як в іншому виді номенклатури, але при цьому не обраний цей вид.'");
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВладелецХарактеристик) Тогда
			ТекстСообщения = НСтр("ru='Сделана настройка, характеристики общие с другим видом номенклатуры, но при этом не выбран этот вид.';uk='Зроблена настройка, характеристики спільні з іншим видом номенклатури, але при цьому не обраний цей вид.'");
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВладелецТоварныхКатегорий) Тогда
			ТекстСообщения = НСтр("ru='Сделана настройка, товарные категории общие с другим видом номенклатуры, но при этом не выбран этот вид.';uk='Зроблена настройка, товарні категорії спільні з іншим видом номенклатури, але при цьому не обраний цей вид.'");
