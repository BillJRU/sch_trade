...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Открытие формы элемента';uk='Розсилка звітів. Відкриття форми елемента'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбран тип получателей';uk='Не обрано тип одержувачів'");
	...
	Если Объект.Подготовлена Тогда
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не выбрана учетная запись для отправки';uk='Не обрано обліковий запис для відправки'");
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Поле ""Получатели"" не заполнено';uk='Поле ""Одержувачі"" не заповнене'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	ПараметрыПредупреждения.Заголовок = НСтр("ru='Результат проверки';uk='Результат перевірки'");
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Проверка способа доставки';uk='Розсилка звітів. Перевірка способу доставки'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПараметрыЖурнала.МассивОшибок.Количество() = 0 Тогда 
		РезультатВыполнения.Текст = НСтр("ru='Проверка возможности доставки успешно пройдена.';uk='Перевірка можливості доставки успішно пройдена.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ru='Проверка возможности доставки не пройдена.';uk='Перевірка можливості доставки не пройдена.'");
...
Функция ПолучитьКэш()
	...
	Шаблоны = Новый ФиксированнаяСтруктура("Тема, Текст, ИмяАрхива",
		РассылкаОтчетов.ШаблонТемы(ЯзыкШаблона),
		РассылкаОтчетов.ШаблонТекста(ЯзыкШаблона),
		РассылкаОтчетов.ШаблонИмениАрхива(ЯзыкШаблона));
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Инициализация отчета';uk='Розсилка звітів. Ініціалізація звіту'",ОбщегоНазначения.КодОсновногоЯзыка()));
