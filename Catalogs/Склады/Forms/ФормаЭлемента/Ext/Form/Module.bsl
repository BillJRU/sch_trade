...
Процедура НастроитьФормуЛокализация()
	...
	Элементы.ГруппаПодразделение.РасширеннаяПодсказка.Заголовок = НСтр("ru='Используется при синхронизации данных о товародвижении.';uk='Використовується при синхронізації даних про рух товарів.'");
	
	//++ Локализация
	Элементы.ИсточникИнформации.РасширеннаяПодсказка.Заголовок	= НСтр("ru='Источник информации о ценах по умолчанию в регламентированных печатных формах.';uk='Джерело інформації про ціни по умовчанню в регламентованих друкованих формах.'");
	Элементы.ВидЦены.РасширеннаяПодсказка.Заголовок				= НСтр("ru='Этот вид цен будет использоваться при выводе в регламентированных печатных формах документов, в которых цены пользователем не указываются. Например, при печати формы М-11 из документа ""Внутреннее потребление"".';uk='Цей вид цін буде використовуватися при виведенні в регламентованих друкованих формах документів, в яких ціни користувачем не зазначаються. Наприклад, при друку форми М-11 з документа ""Внутрішнє споживання"".'");
	//-- Локализация
	Возврат;
