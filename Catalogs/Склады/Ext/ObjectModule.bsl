...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоГруппа Тогда
		...
		Если Не ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач
			...
			ТекстСообщения = НСтр("ru='Нельзя отключить использование ордерной схемы при отражении излишков, недостач и порчи, т.к. есть вид номенклатуры, для которого настроено указание серий при отражении недостач и/или излишков.';uk='Неможна відключити використання ордерної схеми при відображенні надлишків, нестач і псування, тому що є вид номенклатури, для якого налаштовано зазначення серій при відображенні нестач та/або надлишків.'");
...
Процедура ОбновитьФлагКонтроляОперативныхОстатков()
	...
	Если Выборка.КонтролироватьОперативныеОстатки = Истина Тогда
