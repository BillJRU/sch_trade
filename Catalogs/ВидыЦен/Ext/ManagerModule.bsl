...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3f198b0a-3840-49aa-99e5-32b645820ebc");
	...
	Обработчик.ЧитаемыеОбъекты = "Справочник.ВидыЦен";
	Обработчик.ИзменяемыеОбъекты = "Справочник.ВидыЦен";
	Обработчик.БлокируемыеОбъекты = "Справочник.ВидыЦен";
	Обработчик.Комментарий = НСтр("ru='Заполняет статус ""Действует"" в справочник ""Виды цен""';uk='Заповнює статус ""Діє"" у довідник ""Види цін""'");
	
КонецПроцедуры

// Заполняет реквизит Статус статусом Действует.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.ВидыЦен КАК ВидыЦен
	|ГДЕ
	|	ВидыЦен.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДействияВидовЦен.ПустаяСсылка)
	|	И НЕ ВидыЦен.ЭтоГруппа
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ВидЦенОбъект = Выборка.Ссылка.ПолучитьОбъект();
		
		Если ВидЦенОбъект = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВидЦенОбъект.Статус) Тогда
		...
		ВидЦенОбъект.Статус = Перечисления.СтатусыДействияВидовЦен.Действует;
		
		Попытка
			
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидЦенОбъект);
			ЗафиксироватьТранзакцию();
