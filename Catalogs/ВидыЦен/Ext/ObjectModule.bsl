...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Статус = Перечисления.СтатусыДействияВидовЦен.НеДействует И ВидЦенИспользуется() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Статус ""Не действует"" не может быть установлен, т.к. вид цен используется в других видах цен';uk='Статус ""Не діє"" не може бути встановлений, тому що вид цін використовується в інших видах цін'"),
	...
	Если Статус = Перечисления.СтатусыДействияВидовЦен.Действует 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Статус ""Действует"" не может быть установлен, т.к. данный вид цен зависит от недействующих видов цен';uk='Статус ""Діє"" не може бути встановлений, тому що даний вид цін залежить від недіючих видів цін'"),
			ЭтотОбъект,,, Отказ);
