...
|;uk='Розсилки та сповіщення будуть відправлятися згідно настройок підписок. Види контактної інформації, 
|зазначені в цій формі, будуть використані як значення по умовчанню і можуть бути 
|змінені при оформленні підписок'");
	Иначе
...
|;uk='Розсилки і сповіщення будуть відправлятися примусово. Настройка підписок неможлива. В даній формі
...
Процедура ЗаполнитьДоступныеУчетныеЗаписи()
	...
	Иначе
		...
		Элементы.ДекорацияОшибкаУчетнаяЗапись.Подсказка = НСтр("ru='У вас нет прав на чтение учетных записей электронной почты, обратитесь к администратору.';uk='У вас немає прав на читання облікових записів електронної пошти, зверніться до адміністратора.'");
	КонецЕсли;
