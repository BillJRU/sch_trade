...
//
Функция СформироватьПечатнуюФормуРеестрСертификатовНоменклатуры(МассивОбъектов, ОбъектыПечати, ПараметрыВывода = Неопределено) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Справочник.СертификатыНоменклатуры.ПФ_MXL_РеестрСертификатов", КодЯзыкаПечать);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		СтруктураПараметров.Вставить("ТекстЗаголовка", НСтр("ru='Реестр сертификатов номенклатуры';uk='Реєстр сертифікатів номенклатури'",КодЯзыкаПечать));
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("ПредставлениеДокумента", ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати, , КодЯзыкаПечать));
		Область.Параметры.Заполнить(СтруктураПараметров);
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерСтраницы", НСтр("ru='Страница ';uk='Сторінка '",КодЯзыкаПечать) + 1);
								...
								Пока Выборка.Следующий() Цикл
									...
									ИначеЕсли Не Выборка.СертификатДействует Тогда
										...
										Если ЗначениеЗаполнено(Выборка.Серия) Тогда
											ТекстОшибки = НСтр("ru='<%ТипСертификата%: нет действующих сертификатов для серии %Серия%>';uk='<%ТипСертификата%: немає діючих сертифікатів для серії %Серия%>'");
										...
										Иначе 
											ТекстОшибки = НСтр("ru='<%ТипСертификата%: нет действующих сертификатов>';uk='<%ТипСертификата%: немає діючих сертифікатів>'");
											Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Для товара %1 нет действующих сертификатов (%2). Требуется зарегистрировать действующий сертификат.';uk='Для товару %1 немає діючих сертифікатів (%2). Потрібно зареєструвати чинний сертифікат.'"),
												ПредставлениеНоменклатуры,
												Выборка.ТипСертификата);
...
Функция ТекстЗапросаСертификатовНоменклатурыДокумента()
	...
	|		ИЛИ СтрокиТоваров.Ссылка ССЫЛКА Документ.ПередачаТоваровХранителю
	|		ИЛИ СтрокиТоваров.Ссылка ССЫЛКА Документ.ПеремещениеТоваров)
