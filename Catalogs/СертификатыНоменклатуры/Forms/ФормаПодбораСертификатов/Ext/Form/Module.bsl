...
|Робота по підбору сертифікатів для номенклатури буде перервана.'");
			Возврат;
		КонецЕсли;
		
		ОповещениеЗакрытия = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ru='Подобранные сертификаты не будут действовать для номенклатуры. Сопоставить сертификаты с номенклатурой?';uk='Підібрані сертифікати не будуть діяти для номенклатури. Зіставити сертифікати з номенклатурою?'");
