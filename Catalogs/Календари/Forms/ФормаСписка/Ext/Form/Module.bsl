	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПроизводственныйКалендарь) Тогда
			Если Не ЗначениеЗаполнено(Выборка.ДатаЗаполненностиПроизводственногоКалендаря) Тогда
				ВозможнаяПричина = НСтр("ru='Производственный календарь не заполнен';uk='Виробничий календар не заповнений'");
			Иначе
				Если Выборка.ДатаЗаполненностиПроизводственногоКалендаря < ДатаНеобходимойЗаполненности Тогда
					ВозможнаяПричина = НСтр("ru='Производственный календарь не заполнен на очередной календарный год';uk='Виробничий календар не заповнений на черговий календарний рік'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если Не ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
				ВозможнаяПричина = НСтр("ru='График не был заполнен на очередной календарный год';uk='Графік не був заповнений на черговий календарний рік'");
			Иначе
				Если Выборка.ДатаОкончания < ДатаНеобходимойЗаполненности Тогда
					ВозможнаяПричина = НСтр("ru='Период заполнения графика ограничен (см. поле «Дата окончания»)';uk='Період заповнення графіка обмежений (див. поле «Дата закінчення»)'")
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
