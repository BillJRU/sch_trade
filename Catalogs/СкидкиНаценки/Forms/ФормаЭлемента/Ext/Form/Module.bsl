...
Процедура ЗначениеСкидкиНаценкиКоличествоОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если СтрНайти(Текст, "-") > 0 Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для типа скидки ""Скидка количеством"", поле ""Бесплатно"" не может быть меньше 0';uk='Для типу знижки ""Знижка кількістю"", поле ""Безкоштовно"" не може бути менше 0'"),,,,);	
	КонецЕсли;  
КонецПроцедуры
...
Процедура ЗначениеСкидкиНаценкиПодарокОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если СтрНайти(Текст, "-") > 0 Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для типа скидки ""Подарок"", поле ""Стоимость"" не может быть меньше 0';uk='Для типу знижки ""Подарунок"", поле ""Вартість"" не може бути менше 0'"),,,,);	
	КонецЕсли;
