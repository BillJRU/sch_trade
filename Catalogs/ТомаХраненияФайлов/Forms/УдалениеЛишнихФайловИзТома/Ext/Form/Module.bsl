...
Процедура ПодключениеРасширенияРаботыСФайламиЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не установлено расширение для работы с BAF. Действие не доступно.';uk='Не встановлено розширення для роботи з BAF. Дія не доступна.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьОшибкуПравДоступа(ИнформацияОбОшибке, ИсходноеОповещение)
    ...
    |сервер BAF, не имеет прав доступа к указанному каталогу.
    |
    |%1'
    |;uk='Шлях каталогу для копіювання некоректний.
    |Можливо обліковий запис, від імені якої працює
    |сервер BAF, не має прав доступу до зазначеного каталогу.
    |
    |%1'");
...
Процедура ОбработатьСообщениеОбОшибке(ИмяФайла, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Ошибка удаления лишних файлов';uk='Файли.Помилка вилучення зайвих файлів'",ОбщегоНазначения.КодОсновногоЯзыка()),
