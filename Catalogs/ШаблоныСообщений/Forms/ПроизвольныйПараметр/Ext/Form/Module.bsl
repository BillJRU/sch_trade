...
Процедура ОК(Команда)
	...
	Для Каждого ПараметрИзФормы Из СписокПараметров Цикл
		Если СтрСравнить(ПараметрИзФормы.Значение, ИмяПараметра) = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Некорректное имя параметра. Параметр с таким именем уже был добавлен ранее.';uk='Некоректне ім''я параметра. Параметр з таким ім''ям вже був доданий раніше.'"));
			Возврат;
		КонецЕсли;
		Если СтрСравнить(ПараметрИзФормы.Представление, ПредставлениеПараметра) = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Некорректное представление параметра. Параметр с таким представлением уже был добавлен ранее.';uk='Некоректне представлення параметра. Параметр з таким представленням вже був доданий раніше.'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	Если НедопустимоеИмяПараметра(ИмяПараметра) ИЛИ ПустаяСтрока(ИмяПараметра) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Некорректное имя параметра. Нельзя использовать пробелы, знаки пунктуации и другие спец. символы.';uk='Некоректне ім''я параметра. Не можна використовувати пробіли, знаки пунктуації та інші спец. символи.'"));
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(ПредставлениеПараметра) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Некорректное представление параметра.';uk='Некоректне представлення параметра.'"));
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(ТипСтрокой) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Некорректный тип параметра.';uk='Некоректний тип параметра.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораВводНаОсновании(ТипПараметра)
	...
	Если ТипПараметра = Тип("Строка") Тогда
		ПредставлениеТипа = НСтр("ru='Строка';uk='Рядок'");
	ИначеЕсли ТипПараметра = Тип("Дата") Тогда
		ПредставлениеТипа = НСтр("ru='Дата';uk='Дата'");
	КонецЕсли;
	
	Элементы.ТипСтрокой.СписокВыбора.Вставить(0, "Дата", НСтр("ru='Дата';uk='Дата'"));
	Элементы.ТипСтрокой.СписокВыбора.Вставить(0, "Строка", НСтр("ru='Строка';uk='Рядок'"));
