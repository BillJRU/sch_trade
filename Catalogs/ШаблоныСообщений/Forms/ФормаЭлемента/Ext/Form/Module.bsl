...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(РезультатПроверкиШаблоны) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Шаблон сообщения не может быть записан.';uk='Шаблон повідомлення не може бути записаний.'")
...
Процедура ПроверитьШаблон(Команда)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Шаблон заполнен корректно';uk='Шаблон заповнений коректно'"));
	КонецЕсли;
...
Процедура СкопироватьВложение(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
		...
		ТекстВопроса = НСтр("ru='Файл возможно скопировать только после записи шаблона сообщения. Записать?';uk='Файл можливо скопіювати тільки після запису шаблону повідомлення. Записати?'");
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ОтображениеЭлементовФормы(ФорматаПисьма = "")
	...
	Если Объект.ПредназначенДляSMS Тогда
		СуффиксЗаголовка = НСтр("ru='Шаблон сообщения SMS';uk='Шаблон повідомлення SMS'");
	...
	Иначе
		СуффиксЗаголовка = НСтр("ru='Шаблон сообщения электронного письма';uk='Шаблон повідомлення електронного листа'");
	...
	Иначе
		Заголовок = СуффиксЗаголовка + " (" + НСтр("ru='создание';uk='створення'")+ ")";
	КонецЕсли;
...
Функция РезультатПроверкиШаблоны()
	...
	Если ОшибочныеРеквизиты.Количество() > 0 Тогда
		ТекстОшибки = ?(ОшибочныеРеквизиты.Количество() = 1,
			НСтр("ru='Некорректный реквизит в шаблоне сообщения:';uk='Некоректний реквізит у шаблоні повідомлення:'"),
			НСтр("ru='Некорректные реквизиты в шаблоне сообщения:';uk='Некоректні реквізити в шаблоні повідомлення:'")) + " ";
		Разделитель = "";
...
Процедура УстановитьЭлектронноеПисьмоОбычныйТекст(ТребуетсяПереносТекста = Ложь)
	Элементы.ФормаВидТекстаЭлектронногоПисьма.Заголовок = НСтр("ru='Обычный текст';uk='Звичайний текст'");
...
Функция СкопироватьВложенияИзИсточника()
	...
	ОписаниеОшибки = НСтр("ru='Не удалось скопировать вложение по причине: %1';uk='Не вдалося скопіювати вкладення з причини: %1'");
		...
		Для каждого Вложение Из СписокФайлов Цикл
			Если ПустаяСтрока(Вложение.ИДФайлаЭлектронногоПисьма) Тогда
				...
				Исключение
					...
					ЗаписатьОшибкуВЖурналРегистрации(ИмяСобытияШаблоныСообщений(), ИнформацияОбОшибке, НСтр("ru='Не удалось извлечь и записать присоединенный файл для копирования';uk='Не вдалося витягти і записати приєднаний файл для копіювання'"));
...
Функция ИмяСобытияШаблоныСообщений()
	...
	Возврат НСтр("ru='Шаблоны сообщений';uk='Шаблони повідомлень'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьШаблонПоВнешнейОбработке()
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru='Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.';uk='Предмет %1 визначений у зовнішній обробці не знайдений. Зовнішня обробка не може бути підключена.'");
