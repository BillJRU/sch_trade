...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		ВызватьИсключение НСтр("ru='Создание нового элемента копированием запрещено.';uk='Створення нового елемента копіюванням заборонено.'");
	КонецЕсли;
	...
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Работа с печатными формами не поддерживается.';uk='Робота з друкованими формами не підтримується.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИзФайлаПослеПодтверждения(Ответ, ПараметрыРегистрации) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл внешнего отчета или обработки';uk='Виберіть файл зовнішнього звіту або обробки'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 1;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл внешнего отчета';uk='Виберіть файл зовнішнього звіту'");
	Иначе
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 2;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл внешней обработки';uk='Виберіть файл зовнішньої обробки'");
	КонецЕсли;
...
Процедура ПоказатьКонфликты(ПараметрыРегистрации)
	...
	Иначе
		...
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ru='Отменить и открыть';uk='Скасувати та відкрити'");
	КонецЕсли;
