...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПлановаяДатаОкончания < ПлановаяДатаНачала
		...
		ТекстСообщения = НСтр("ru='Плановая дата окончания не должна быть меньше чем плановая дата начала.';uk='Планова дата закінчення не повинна бути менше ніж планова дата початку.'");
	...
	Если ДатаОкончания < ДатаНачала
		...
		ТекстСообщения = НСтр("ru='Фактическая дата окончания не должна быть меньше чем фактическая дата начала.';uk='Фактична дата закінчення не повинна бути менше ніж фактична дата початку.'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		...
		Для Каждого СтрокаПартнер Из ПартнерыИКонтактныеЛица Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаПартнер.Партнер) Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru='Не заполнена колонка ""Контрагент"" в строке %1 табличной части ""Контрагенты и контактные лица""';uk='Не заповнена колонка ""Контрагент"" в рядку %1 табличної частини ""Контрагенти і контактні особи""'"), СтрокаПартнер.НомерСтроки);
