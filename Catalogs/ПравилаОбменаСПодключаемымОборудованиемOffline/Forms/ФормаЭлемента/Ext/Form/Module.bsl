...
Процедура ПрименитьИзмененияНаСервере(ТекущийОбъект)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Редактирование правила обмена с ККМ Offline';uk='Редагування правила обміну з ККМ Offline'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			СтрШаблон(НСтр("ru='Во время применения изменений произошла ошибка:
|%1'
|;uk='Під час застосування змін сталася помилка:
...
Процедура УдалитьСвободныеКодыТоваровНаСервере()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Удаление свободных кодов ККМ Offline';uk='Вилучення вільних кодів ККМ Offline'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			СтрШаблон(НСтр("ru='Во время удаления свободных кодов ККМ Offline произошла ошибка:
|%1'
|;uk='Під час вилучення вільних кодів ККМ Offline сталася помилка: 
|%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
...
Процедура ОбновитьОтборыКомпоновщикаНастроек()
	...
	Для Каждого СтрокаОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если СтрокаОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Склад") Тогда
			...
			СтрокаОтбора.ПравоеЗначение = Объект.Склад;
			СтрокаОтбора.Использование = ЗначениеЗаполнено(Объект.Склад);
		КонецЕсли;
		
		Если СтрокаОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОстатокНаСкладе") Тогда
			СтрокаОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Больше;
			СтрокаОтбора.ПравоеЗначение = 0;
			СтрокаОтбора.Использование = ЗначениеЗаполнено(Объект.Склад);
		КонецЕсли;
	КонецЦикла;
