...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(РольИсполнителя) Тогда
		...
		КонецЕсли;
	Иначе
		Наименование = НСтр("ru='Без ролевой адресации';uk='Без рольової адресації'");
	КонецЕсли;
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Уже есть группа исполнителей задач, для которой заданы:
                       ...
                       |;uk='Вже є група виконавців задач, для якої задані:
                       |роль виконавця ""%1"",
                       |основний об''єкт адресації ""%2""
                       |і додатковий об''єкт адресації ""%3""'"),
