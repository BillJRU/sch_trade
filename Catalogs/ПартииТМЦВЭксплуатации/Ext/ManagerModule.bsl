...
Процедура ЗаполнитьПартии(Объект, Отказ) Экспорт
	...
	ЭтоВводОстатковТМЦВЭксплуатации = (ТипЗнч(Объект) = Тип("ДокументОбъект.ВводОстатковТМЦВЭксплуатации"));
	
	ЭтоОперацияПередачи = (
		(ЭтоВнутреннееПотребление И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию)
		Или ((ЭтоВводОстатков Или ЭтоВводОстатковТМЦВЭксплуатации) 
			И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковТМЦВЭксплуатации));
	
	ПометкаУдаленияПартий = Объект.ПометкаУдаления Или Не ЭтоОперацияПередачи;
	
	Если СвойстваДокумента.РежимЗаписи = РежимЗаписиДокумента.Запись И Не ПометкаУдаленияПартий Тогда
		Возврат;
	КонецЕсли;
	...
	Если СвойстваДокумента.ЭтоНовый Тогда
		...
		ИначеЕсли ЭтоВводОстатковТМЦВЭксплуатации Тогда
			СсылкаНаДокумент = Документы.ВводОстатковТМЦВЭксплуатации.ПолучитьСсылку();
		КонецЕсли;
		Объект.УстановитьСсылкуНового(СсылкаНаДокумент);
	КонецЕсли;
	
	ТабличнаяЧасть = ?(ЭтоВводОстатков Или ЭтоВводОстатковТМЦВЭксплуатации, 
		Объект.ТМЦВЭксплуатации.Выгрузить(), Объект.Товары.Выгрузить());
	...
	ТабличнаяЧасть = ?(ЭтоВводОстатков Или ЭтоВводОстатковТМЦВЭксплуатации, Объект.ТМЦВЭксплуатации, Объект.Товары);
