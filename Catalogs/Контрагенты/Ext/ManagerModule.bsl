...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Генерирует элементы справочника Объекты расчетов.';uk='Генерує елементи довідника Об''єкти розрахунків.'");
	...
	НоваяСтрока.ИмяОбъекта = "Справочники.Контрагенты";
...
Процедура СгенерироватьОбъектыРасчетов(Параметры) Экспорт
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Если ВыборкаДетальныеЗаписи.ТребуетсяОбработать Тогда
					...
					Если Не ЗначениеЗаполнено(НовыйОбъектРасчетов) Тогда
						Шаблон = НСтр("ru='Не удалось создать объект расчетов по аналитике учета по партнерам: ""%1""';uk='Не вдалося створити об''єкт розрахунків по аналітиці обліку по партнерам: ""%1""'");
							ТекстСообщения = 
							СтрШаблон(Шаблон,
	...
	Исключение
		...
		Шаблон = НСтр("ru='Не удалось сгенерировать объект расчетов для пустого значения по причине: %1';uk='Не вдалося згенерувати об''єкт розрахунків для порожнього значення по причині: %1'");
...
Функция ТекстЗапросаКонтрагентыДляГенерацииОбъектовРасчетов()
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА НЕ КлючиАналитикиУчетаПоПартнерам.Контрагент В (ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.организации.ПустаяСсылка))
	...
	//|			И (ВЫБОР 
	//|           	КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	//|					И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	//|					И ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаПоПартнерам.Контрагент) = ТИП(Справочник.Организации))
	//|					ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыМеждуОрганизациями.ПустаяСсылка)
	//|           	КОГДА КлючиАналитикиУчетаПоПартнерам.Договор = НЕОПРЕДЕЛЕНО
	//|					ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	//|				ИНАЧЕ КлючиАналитикиУчетаПоПартнерам.Договор
	//|           КОНЕЦ)
	|			И (ВЫБОР 
	|           	КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|					И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	...
	|			И
	|				ДвиженияДенежныеСредстваКонтрагент.НаправлениеДеятельностиКонтрагента = КлючиАналитикиУчетаПоПартнерам.НаправлениеДеятельности
	...
	|			И ДвиженияДенежныеСредстваКонтрагент.Договор = (ВЫБОР 
	|           		КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|						И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|						И ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаПоПартнерам.Контрагент) = ТИП(Справочник.Организации))
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыМеждуОрганизациями.ПустаяСсылка)
	...
	|			И
	|				ДвиженияКонтрагентДоходыРасходы.НаправлениеДеятельностиКонтрагента = КлючиАналитикиУчетаПоПартнерам.НаправлениеДеятельности
	...
	|			И ДвиженияКонтрагентДоходыРасходы.Договор = (ВЫБОР 
	|           		КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|						И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|						И ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаПоПартнерам.Контрагент) = ТИП(Справочник.Организации))
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыМеждуОрганизациями.ПустаяСсылка)
	...
	|			И ДвиженияКонтрагентКонтрагент.Договор = (ВЫБОР 
	|           		КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|						И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|						И ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаПоПартнерам.Контрагент) = ТИП(Справочник.Организации))
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыМеждуОрганизациями.ПустаяСсылка)
	...
	|			И КлючиАналитикиУчетаПоПартнерам.Партнер = ОбъектыРасчетов.Партнер
	|			И КлючиАналитикиУчетаПоПартнерам.Контрагент = ОбъектыРасчетов.Контрагент
	|			И ОбъектыРасчетов.Договор = (ВЫБОР 
	|           		КОГДА (КлючиАналитикиУчетаПоПартнерам.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|						И КлючиАналитикиУчетаПоПартнерам.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|						И ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаПоПартнерам.Контрагент) = ТИП(Справочник.Организации))
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.ДоговорыМеждуОрганизациями.ПустаяСсылка)
	...
	ИсточникиПустыхОбъектовРасчетов.Добавить("СписаниеЗадолженности");
