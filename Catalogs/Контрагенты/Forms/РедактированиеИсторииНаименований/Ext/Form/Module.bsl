	...
	Для Каждого Запись Из ИсторияНаименований Цикл
		...
		Если НЕ ЗначениеЗаполнено(Запись.НаименованиеПолное) Тогда
			Если ЮрФизлицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо")
				...
				СообщениеОбОшибке = НСтр("ru='Поле ""Фамилия, имя, отчество"" не заполнено';uk='Поле ""Прізвище, ім''я, по батькові"" не заповнено'");
			Иначе
				СообщениеОбОшибке = НСтр("ru='Поле ""Сокр. юр наименование"" не заполнено';uk='Поле ""Скор. юр найменування"" не заповнено'");
			КонецЕсли;
...
Процедура СформироватьЗаголовки()
	...
	Если ЮрФизлицо = Перечисления.ЮрФизЛицо.ФизЛицо
		...
		Заголовок = НСтр("ru='История изменения ФИО';uk='Історія зміни ПІБ'");
		Элементы.ИсторияНаименованийНаименованиеПолное.Заголовок = НСтр("ru='Фамилия, имя, отчество';uk='Прізвище, ім''я, по батькові'");
