...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы И НЕ ВыполняетсяЗакрытие И Модифицированность И НЕ СохранитьПараметры Тогда
		...
		Отказ = Истина;
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ru='Запрет отгрузки клиенту не будет изменен. Закрыть форму без сохранения результата?';uk='Заборона відвантаження клієнтові не буде змінена. Закрити форму без збереження результату?'"), СписокКнопок);
