...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокМонохромныйШрифт.Добавить(Истина, НСтр("ru='Монохромный';uk='Монохромний'"));
	СписокМонохромныйШрифт.Добавить(Ложь, НСтр("ru='Градации серого';uk='Градації сірого'"));

	СписокУголПоворота.Добавить(0, НСтр("ru='0°';uk='0°'"));
	СписокУголПоворота.Добавить(90, НСтр("ru='90°';uk='90°'"));
	СписокУголПоворота.Добавить(180, НСтр("ru='180°';uk='180°'"));
	СписокУголПоворота.Добавить(270, НСтр("ru='270°';uk='270°'"));

	СписокGS1DatabarКоличествоСтрок.Добавить(100, НСтр("ru='Приоритет модуля и числа строк (max)';uk='Пріоритет модуля і числа рядків (max)'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(101, НСтр("ru='Приоритет модуля и числа строк (min)';uk='Пріоритет модуля і числа рядків (min)'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(102, НСтр("ru='Приоритет соотношения сторон';uk='Пріоритет співвідношення сторін'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(1, НСтр("ru='1 строка';uk='1 рядок'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(2, НСтр("ru='2 строки';uk='2 рядка'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(3, НСтр("ru='3 строки';uk='3 рядка'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(4, НСтр("ru='4 строки';uk='4 рядка'"));
	СписокGS1DatabarКоличествоСтрок.Добавить(5, НСтр("ru='5 строк';uk='5 рядків'"));

	СписокВариантМасштабирования.Добавить("РеальныйРазмер", НСтр("ru='Реальный размер';uk='Реальний розмір'"));
	СписокВариантМасштабирования.Добавить("МасштабироватьСохранятьПропорции", НСтр("ru='Масштабировать (сохранять пропорции)';uk='Масштабувати (зберігати пропорції)'"));
	СписокВариантМасштабирования.Добавить("МасштабироватьНеСохранятьПропорции", НСтр("ru='Масштабировать (не сохранять пропорции)';uk='Масштабувати (не зберігати пропорції)'"));

	СписокВертикальноеВыравнивание.Добавить(1, НСтр("ru='По верхнему краю';uk='По верхньому краю'"));
	СписокВертикальноеВыравнивание.Добавить(2, НСтр("ru='По центру';uk='По центру'"));
	СписокВертикальноеВыравнивание.Добавить(3, НСтр("ru='По нижнему краю';uk='По нижньому краю'"));

	СписокУровеньЧеткости.Добавить(1, НСтр("ru='Стандартный';uk='Стандартний'"));
	СписокУровеньЧеткости.Добавить(2, НСтр("ru='Повышенный';uk='Підвищений'"));
...
Процедура НапечататьОбразец(Команда)
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляДоставки") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьЭтикетокИЦенников", "ЭтикеткаДоставки", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДоставки());
...
Процедура Назначить(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ДляЧего) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Для чего"" не заполнено';uk='Поле ""Для чого"" не заповнено'"), , "Объект.ДляЧего");
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьШаблонЗавершение(ИмяШаблона, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяШаблона) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВыбратьШаблонВопросЗавершение", ЭтотОбъект, Новый Структура("ИмяШаблона", ИмяШаблона)), НСтр("ru='Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?';uk='Редагований шаблон буде замінений на шаблон по умовчанню, продовжити?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<Не доступно>';uk='<Не доступно>'"));
...
Процедура РассчитатьМаксимальноеКоличествоНаСтранице()
	...
	ОписаниеРазмещенияДляЦенников = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На странице %1 помещается по вертикали: %2, по горизонтали: %3';uk='На сторінці %1 вміщається по вертикалі: %2, по горизонталі: %3'"), РазмерСтраницы(ПолеТабличногоДокумента), КоличествоПоВертикали, КоличествоПоГоризонтали);
...
Процедура ДобавитьПоле(Коллекция, СтруктураПоля, Дерево)
	...
	Для Каждого ЭлементИерархии Из Иерархия Цикл
		...
		ИмяРодителяОписание = ?(ЗначениеЗаполнено(ИмяЭлементаОписание), ИмяЭлементаОписание
			+ ".", "");
		ИмяЭлементаОписание = ИмяРодителяОписание
			+ НайденныйЭлементПолеКомпоновки.Заголовок;
	...
	Если Действия.Найти("Свойства") <> Неопределено Тогда
		...
		Для Каждого НаборСсылка Из НаборыРеквизитовИСведений Цикл
			...
			Для Каждого ДополнительныйРеквизит Из Набор.ДополнительныеРеквизиты Цикл
				...
				НоваяСтрока.ПолеКомпоновкиДанных = Новый ПолеКомпоновкиДанных(ИмяПоля
					+ ".[" + Строка(ДополнительныйРеквизит.Свойство) + "]");
			...
			Для Каждого ДополнительноеСведение Из Набор.ДополнительныеСведения Цикл
				...
				НоваяСтрока.ПолеКомпоновкиДанных = Новый ПолеКомпоновкиДанных(ИмяПоля
					+ ".[" + Строка(ДополнительноеСведение.Свойство) + "]");
...
Процедура ЗаполнитьТаблицуСвойств()
	...
	ИначеЕсли ТипЗнч(ВыделеннаяОбласть) = Тип("РисунокТабличногоДокумента") Тогда
		...
		Если СтрНайти(ВыделеннаяОбласть.Имя, "Штрихкод") > 0 Тогда
			...
			НоваяСтрока.Представление = НСтр("ru='Тип штрихкода';uk='Тип штрихкоду'");
			...
			НоваяСтрока.Представление = НСтр("ru='Тип шрифта';uk='Тип шрифту'");
			НоваяСтрока.ТолькоПросмотр = Не ОтображатьТекст;
			...
			НоваяСтрока.Представление = НСтр("ru='Уровень четкости';uk='Рівень чіткості'");
			...
			Если ТипКода = 17 Тогда
				...
				НоваяСтрока.Представление = НСтр("ru='Количество строк';uk='Кількість рядків'");
			...
			Если ТипКода = 17 Или ТипКода = 6 Или ТипКода = 18 Или ТипКода = 24 Тогда
				...
				НоваяСтрока.Представление = НСтр("ru='Вертикальное выравнивание';uk='Вертикальне вирівнювання'");
			...
			НоваяСтрока.Представление = НСтр("ru='Угол поворота';uk='Кут повороту'");
			...
			НоваяСтрока.Представление = НСтр("ru='Масштабирование';uk='Масштабування'");
...
Процедура НастроитьЛинию(ВыделеннаяОбласть, ТекущиеДанные)
	...
	ОткрытьФорму("Справочник.ШаблоныЭтикетокИЦенников.Форма.ФормаВыбораТипаЛинии", ПараметрыФормы, , , , , Новый ОписаниеОповещения("НастроитьЛиниюЗавершение", ЭтотОбъект, Новый Структура("ВыделеннаяОбласть, ТекущиеДанные", ВыделеннаяОбласть, ТекущиеДанные)), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
// Перед началом добавления необходимо выделить область в табличном документе.
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод()
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПодставитьПолеШаблонаЗавершение", ЭтотОбъект, Новый Структура("ИмяПоляВШаблоне, ТекущаяОбласть", ИмяПоляВШаблоне, ТекущаяОбласть)), НСтр("ru='Добавить штрихкод, как картинку?';uk='Додати штрихкод, як картинку?'"), РежимДиалогаВопрос.ДаНет);
	...
	ИначеЕсли ИмяПоляВШаблоне = ИмяПараметраВалютаЦены() Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПодставитьПолеШаблонаЗавершение", ЭтотОбъект, Новый Структура("ИмяПоляВШаблоне, ТекущаяОбласть", ИмяПоляВШаблоне, ТекущаяОбласть)), НСтр("ru='Добавить валюту как...';uk='Додати валюту як...'"), Кнопки);
