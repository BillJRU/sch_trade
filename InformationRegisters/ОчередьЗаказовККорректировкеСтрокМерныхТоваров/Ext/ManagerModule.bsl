...
Процедура ДобавитьЗаказВОчередь(ЗаказСсылка, Регистратор = Неопределено) Экспорт
	...
	Иначе
		...
		Исключение
			...
			Иначе
				ТекстСообщения = НСтр("ru='Не удалось добавить в очередь корректировки строк мерных товаров заказ %Заказ%';uk='Не вдалося додати в чергу коригування рядків мірних товарів замовлення %Заказ%'");
