...
Процедура ЗаписатьНастройкиНаКлиенте()
	...
	Если ОтложенныйСтартПроцесса <> ОтложенныйСтартПроцессаПриОткрытии Тогда 
		...
		ТекстОповещения = ?(ОтложенныйСтартПроцесса, НСтр("ru='Отложенный старт:';uk='Відкладений старт:'"), НСтр("ru='Отложенный старт отменен:';uk='Відкладений старт скасований:'"));
...
Процедура ОбновитьСписокВыбораВремени()
	...
	Для Инд = 1 По 48 Цикл
		Элементы.ДатаОтложенногоСтартаВремя.СписокВыбора.Добавить(ПустаяДата, Формат(ПустаяДата, НСтр("ru='ДФ=ЧЧ:мм';uk='ДФ=ЧЧ:мм'")));
...
Функция ТекстИнтервала(ДатаНачала, ДатаОкончания)
	...
	Если ДатаНачала > ДатаОкончания Тогда
		Возврат НСтр("ru='Дата запуска задания находится в прошлом.';uk='Дата запуску завдання знаходиться в минулому.'");
	КонецЕсли;	
	...
	РазностьДат = "";
	Приставка = НСтр("ru='Задание будет запущено';uk='Завдання буде запущено'") + " ";
	Корень = НСтр("ru='через';uk='через'") + " ";
	Если ИспользоватьДатуИВремяВСрокахЗадач Тогда
		Если ЧислоДней > 0 И ЧислоЧасов > 0 Тогда
			РазностьДат = Корень + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 дн. и %2 ч.';uk='%1 дн. і %2 г.'"),
				Строка(ЧислоДней),
				Строка(ЧислоЧасов));
		...
		Иначе
			РазностьДат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='менее чем через час.';uk='менш ніж через годину.'"), Строка(ЧислоЧасов));
		КонецЕсли;
	Иначе
		...
		Иначе
			РазностьДат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='менее чем через день.';uk='менш ніж через день.'"), Строка(ЧислоДней));
		КонецЕсли;
	КонецЕсли;
...
Функция ФормаЗаполненаКорректно()
	...
	Если ОтложенныйСтартПроцесса И ДатаОтложенногоСтарта < ТекущаяДатаСервера Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Дата и время отложенного старта должны быть больше текущей даты.';uk='Дата й час відкладеного старту повинні бути більше поточної дати.'"),,
			"ДатаОтложенногоСтарта");
		ЗаполненаКорректно = Ложь;
	КонецЕсли;
		
	Если ОтложенныйСтартПроцесса И ДатаОтложенногоСтарта > СрокИсполнения Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Дата и время отложенного старта должны быть меньше срока исполнения задания.';uk='Дата і час відкладеного старту повинні бути менше строку виконання завдання.'"),,
			"ДатаОтложенногоСтарта");
		ЗаполненаКорректно = Ложь;
	КонецЕсли;
