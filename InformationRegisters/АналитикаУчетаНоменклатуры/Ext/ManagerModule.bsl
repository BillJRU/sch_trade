...
Функция ПараметрыЗаполненияКлючейАналитики() Экспорт
	Возврат Новый Структура("ИзмененаАналитика,СоздаватьВКлючи,ДополнитьКлюч", Ложь, Истина, Ложь);
КонецФункции
...
Процедура ЗаполнитьВКоллекции(Коллекция, МестаУчета, ИменаПолей = Неопределено, ПараметрыЗаполнения = Неопределено) Экспорт
    ...
    ДополнитьКлюч = ПараметрыЗаполнения.ДополнитьКлюч;
	...
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.АналитикаУчетаНоменклатуры) Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru='Ошибка при заполнении ключей в коллекции: есть аналитики, по которым ключи еще не созданы.';uk='Помилка при заповненні ключів в колекції: є аналітики, за якими ключі ще не створені.'");
        ...
        ОбрабатываемаяСтрока = Коллекция.Найти(Выборка.Индекс+1,"НомерСтроки");
		Если ДополнитьКлюч Тогда
			ОбрабатываемаяСтрока[ИменаПолей.ЗаполняемаяАналитикаУчетаНоменклатуры] = КлючАналитики;
		Иначе
			ОбрабатываемаяСтрока[ИменаПолей.АналитикаУчетаНоменклатуры] = КлючАналитики;
		КонецЕсли;
...
//
// Возвращаемое значение:
//	Структура - содержит два ключа:
//	 - "Произвольный" - место учета по-умолчанию.
//	 - "Работа" - место учета для работ.
//
Функция МестаУчета(Операция, Склад, Подразделение, Партнер, Договор = Неопределено) Экспорт
	...
	ИспользоватьДоговор = 
        (Операция = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути)
...
Функция ПолучитьНаборЗаписей(ПараметрыАналитики)
	...
	Если Не ЗначениеЗаполнено(СтруктураАналитики.Номенклатура) Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: не указана номенклатура.';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номенклатури: не вказана номенклатура.'");
	...
	Иначе
		...
		Если ТипыНоменклатурыБезКонтроляМестаХранения.Найти(РеквизитыНоменклатуры.ТипНоменклатуры) = Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(СтруктураАналитики.МестоХранения) Тогда
				ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана номенклатура с типом, отличном от ""Услуга"" или ""Работа"", а место хранения не заполнено.';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номеклатури: вказана номенклатура з типом, відмінним від ""Послуга"" або ""Робота"", а місце зберігання не заповнено.'");
			КонецЕсли;
		...
		Если РеквизитыНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда 
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Назначение)
				...
				ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана номенклатура с типом ""Многооборотная тара"", и указано назначение.';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номеклатури: вказана номенклатура з типом ""Багатооборотна тара"", і вказано призначення.'")
			КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(РеквизитыНоменклатуры.ИспользованиеХарактеристик)
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана для номенклатуры не настроено ведение характеристики, но характеристика указана.';uk='Помилка при отриманні набору регістра відомостей АналитикаУчетаНоменклатуры: вказана для номенклатури не налаштоване ведення характеристики, але характеристика вказана.'")
			КонецЕсли;
			
		Иначе
			
			Если Не ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана для номенклатуры настроено ведение характеристики, но характеристика не указана.';uk='Помилка при отриманні набору регістра відомостей АналітікаУчетаНоменклатури: вказана для номенклатури налаштоване ведення характеристики, але характеристика не вказана.'")
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.Серия) Тогда
		...
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.Склады")
			...
			ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: серия указана, но место хранения не того типа.';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номеклатури: серія вказана, але місце зберігання не того типу.'");
		КонецЕсли;
		
		Если РеквизитыНоменклатуры.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Товар Тогда
			
			ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: серия указана, но номенклатура не с типом ""Товар"".';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номеклатури: серія вказана, але номенклатура не з типом ""Товар"".'");
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.СтатьяКалькуляции) Тогда
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.СтруктураПредприятия") Тогда
			ВызватьИсключение НСтр("ru='Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: СтатьяКалькуляции указана, но место хранения не того типа.';uk='Помилка при отриманні набору регістра відомостей Аналітика обліку номеклатури: Стаття калькуляціі вказана, але місце зберігання не того типу.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстДополнениеКлючейАналитикиВКоллекции(ИменаПолей)
	ТекстЗапроса = "
		...
		|		И Ключи.МестоХранения = Коллекция.МестоХранения
