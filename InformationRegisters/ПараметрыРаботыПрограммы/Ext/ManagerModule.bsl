...
Функция ЗагрузитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru='Фоновая загрузка параметров работы программы';uk='Фонове завантаження параметрів роботи програми'");
	   ...
	   И Не ДоступноВыполнениеФоновыхЗаданий() Тогда
		...
		ВызватьИсключение
			НСтр("ru='Обновление параметров работы программы, когда подключены расширения конфигурации,
                       ...
                       |через запуск платформы BAF, либо временно отключать расширения конфигурации.'
                       |;uk='Оновлення параметрів роботи програми, коли підключені розширення конфігурації,
                       |може бути виконано тільки в фоновому завданні без розширень конфігурації.
                       |
                       |В файловій інформаційній базі фонове завдання неможливо запустити
                       |з іншого фонового завдання, а також з COM-З''єднання.
                       |
                       |Для виконання оновлення необхідно, або робити оновлення інтерактивно
                       |через запуск платформи BAF, або тимчасово відключати розширення конфігурації.'");
	КонецЕсли;
...
Функция ОбновитьПараметрыРаботыВерсийРасширенийВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru='Фоновое обновление параметров работы версий расширений';uk='Фонове оновлення параметрів роботи версій розширень'");
	ПараметрыОперации.ОжидатьЗавершение = ОжидатьЗавершение;
		...
		Если Операция = "ЗагрузкаПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки =
				НСтр("ru='Не удалось загрузить параметры работы программы по причине:
                           ...
                           |;uk='Не вдалося завантажити параметри роботи програми через:
                           |Фонове завдання, яке виконує завантаження скасовано.'");
		...
		Иначе // ОбновлениеПараметровРаботыВерсийРасширений.
			КраткоеПредставлениеОшибки =
				НСтр("ru='Не удалось обновить параметры работы версий расширений по причине:
                           |Фоновое задание, выполняющее обновление отменено.'
                           |;uk='Не вдалося оновити параметри роботи версій розширень через:
                           |Фонове завдання, яке виконує оновлення скасовано.'");
		КонецЕсли;
		...
		ИначеЕсли Операция = "ЗагрузкаПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки =
				НСтр("ru='Не удалось загрузить параметры работы программы по причине:
                           ...
                           |;uk='Не вдалося завантажити параметри роботи програми через:
                           |Фонове завдання, яке виконує завантаження не повернуло результат.'");
		...
		Иначе // ОбновлениеПараметровРаботыВерсийРасширений.
			КраткоеПредставлениеОшибки =
				НСтр("ru='Не удалось обновить параметры работы версий расширений по причине:
                           |Фоновое задание, выполняющее обновление не вернуло результат.'
                           |;uk='Не вдалося оновити параметри роботи версій розширень через:
                           |Фонове завдання, яке виконує оновлення не повернуло результат.'");
		КонецЕсли;
	   ...
	   И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение
			НСтр("ru='Не удалось загрузить параметры работы программы по причине:
                       ...
                       |;uk='Не вдалося завантажити параметри роботи програми через:
                       |Завантаження неможливо виконати в області даних.'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьОбновленияВМоделиСервиса(ИмяПараметра, НовоеЗначение, Операция)
	...
	ИмяСобытия = НСтр("ru='Параметры работы программы.Не выполнено обновление в неразделенном режиме';uk='Параметри роботи програми.Не виконано оновлення в нерозділеному режимі'",ОбщегоНазначения.КодОсновногоЯзыка());
	
	Комментарий =
		НСтр("ru='1. Перешлите сообщение в техническую поддержку.
                   |2. Попытайтесь устранить проблему самостоятельно. Для этого
                   |запустите программу с параметром командной строки платформы BAF 8
                   ...
                   |;uk='1. Перешліть повідомлення в технічну підтримку.
                   |2. Спробуйте усунути проблему самостійно. Для цього
                   |запустіть програму з параметром командного рядка платформи BAF 8
                   |""/С ЗапуститьОбновлениеИнформационнойБазы"" від імені користувача
                   |з правами адміністратора сервісу, тобто в нерозділеному режимі.
                   |
                   |Відомості про проблемному параметрі:'");
