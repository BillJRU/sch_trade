...
|;uk='Перід, що закривається, міжнародного обліку не можеть бути пізніше, 
|ніж період, що закривається, регламентованого обліку.'"));
			Возврат;
...
|;uk='Період, що закривається, регламентованого обліку не можеть бути пізніше, 
|ніж період, що закривається, оперативного обліку.'"));
			Возврат;
...
|;uk='Період, що закривається, регламентованого обліку не можеть бути раніше, 
|ніж період, що закривається, міжнародного обліку.'"));
			Возврат;
...
|чем закрываемый период регламентированного учета.'
|;uk='Період, що закривається, оперативного обліку не можеть бути раніше, 
|ніж період, що закривається, регламентованого обліку.'"));
			Возврат;
...
Процедура СохранитьРеквизитыРегламентногоЗадания()
	...
	Если ОписаниеЗадания.Задание = Неопределено Тогда
		...
		СтрокаНаименования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Закрытие месяца по организации: %1';uk='Закриття місяця по організації: %1'"),
			?(ЗначениеЗаполнено(ОрганизацияПредыдущая), СокрЛП(ОрганизацияПредыдущая), НСтр("ru='По всем организациям.';uk='По всіх організаціях.'")));
...
Процедура ОбновитьПредставлениеРасписанияИСостояния(ОписаниеЗадания = Неопределено)
	...
	Если ОписаниеЗадания <> Неопределено И ОписаниеЗадания.СостояниеЗадания <> Неопределено Тогда
		...
		Если ОписаниеЗадания.СостояниеЗадания.Состояние = Неопределено Тогда
			СостояниеЗадания = НСтр("ru='Не выполнялось';uk='Не виконувалось'");
		Иначе
			СостояниеЗадания =
	...
	ИначеЕсли ОписаниеЗадания <> Неопределено Тогда
		...
		СостояниеЗадания = НСтр("ru='Не настроено';uk='Не налаштоване'");
