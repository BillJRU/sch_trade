...
Процедура ДобавитьПоддержаниеЗапасаМинМакс(КлючиЗаписи, ПараметрыМинМакс = Неопределено) Экспорт
	...
	Для каждого Ключ Из КлючиЗаписи Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru='Недопустимый тип ключа записи!';uk='Неприпустимий тип ключа запису!'");;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьПоддержаниеЗапасаГруппировкой(КлючиЗаписи, КлючГруппировки) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Недопустимый тип ключа записи группировки!';uk='Неприпустимий тип ключа запису групування!'");
	КонецЕсли;
...
Процедура УстановитьПоддержаниеЗапасаКакДляНоменклатуры(КлючиЗаписиХарактеристик) Экспорт
	...
	Для каждого Ключ Из КлючиЗаписиХарактеристик Цикл
		Если Ключ.ТипКлюча = "ДетальнаяНастройка" Тогда
			...
			ЗаписьТаблицы.Склад				= Ключ.Склад;
		Иначе
			ВызватьИсключение НСтр("ru='Недопустимый тип ключа записи характеристики!';uk='Неприпустимий тип ключа запису характеристики!'");;
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьПараметрПоддержанияЗапаса(КлючиЗаписи, ИмяПараметра, Значение) Экспорт
	...
	ИначеЕсли ИмяПараметра = "ОбеспечениеЗаказовПриПоддержанииЗапаса" Тогда
		...
		МетодыДляОтбора.Добавить(Перечисления.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаНаСрокПоСтатистике);
	Иначе
		ВызватьИсключение НСтр("ru='Некорректно задано имя параметра!';uk='Некоректно задано ім''я параметра!'");
	КонецЕсли;
...
Процедура ЗадатьУпаковкуТовара(Номенклатура, Характеристика, Склад, УпаковкаЗаказа) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Задание упаковки заказа для товара на складе';uk='Завдання упаковки замовлення для товару на складі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура РассчитатьСтатистикуПотребленияТоваров(МенеджерВременныхТаблиц) Экспорт
	...
	Пока Истина Цикл
		...
		Попытка
			...
			Для каждого ЗаписьГруппировки Из Группировки Цикл
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					ВызватьИсключение НСтр("ru='Несоответствие в транзакции заблокированных и изменяемых группировок';uk='Невідповідність в транзакції заблокованих і змінюваних групувань'");
				КонецЕсли;
			КонецЦикла;
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Расчет статистики потребления товаров';uk='Розрахунок статистики споживання товарів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение;
		КонецПопытки;
...
Функция Записать(ТаблицаТоваров, ТаблицаГруппировок, ПараметрыДляЗаписи, МетодыДляОтбора = Неопределено)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Запись в регистр ""Товарные ограничения""';uk='Запис в регістр ""Товарні обмеження""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура Удалить(ТаблицаТоваров, ТаблицаГруппировок, УдалятьДетальные = Ложь)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Удаление записей в регистре ""Товарные ограничения""';uk='Вилучення записів в регістрі ""Товарні обмеження""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РегистрыСведений.ТоварныеОграничения.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5c59d6f0-34a3-46ae-8744-47d04085dc28");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "РегистрыСведений.ТоварныеОграничения.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.ТоварныеОграничения";
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.ТоварныеОграничения";
	Обработчик.БлокируемыеОбъекты = "РегистрСведений.ТоварныеОграничения";
	Обработчик.Комментарий = НСтр("ru='Обновляет записи в регистре ""Товарные ограничения"". Удаляет записи с методом обеспечения ""Заказ под заказ"", создает записи со служебными группировками, переносит упаковку заказа из группировок в детальные записи, заполняет служебные реквизиты.';uk='Оновлює записи у регістрі ""Товарні обмеження"". Видаляє записи з методом забезпечення ""Замовлення під замовлення"", створює записи зі службовими групованнями, переносить упаковку замовлення із груповань до детальних записів, заповнює службові реквізити.'");
...
#Область Обработка1
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			Шаблон = НСтр("ru='Не удалось обработать запись. Группировка: %1, Номенклатура: %2, Характеристика: %3, Склад: %4 по причине: %5';uk='Не вдалося опрацювати запис. Груповання: %1, Номенклатура: %2, Характеристика: %3, Склад: %4 по причині: %5'");
