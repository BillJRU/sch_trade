...
Процедура ЗаписатьНаСервере(Отказ = Ложь)
	...
	Для каждого СтрокаТЧ Из СезонныеКоэффициенты Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Запись сезонных коэффициентов';uk='Запис сезонних коефіцієнтів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция РассчитатьПоСтатистикеПродажНаСервере(ПараметрыЗадания)
	...
	НаименованиеЗадания = НСтр("ru='Расчет сезонных коэффициентов по статистике продаж';uk='Розрахунок сезонних коефіцієнтів за статистикою продажів'");
...
Функция ПрефиксРеквизитаКолонкиКоэффициент()
	...
	Возврат НСтр("ru='Коэффициент_';uk='Коефіцієнт_'");
