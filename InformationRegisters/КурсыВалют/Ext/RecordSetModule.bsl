...
Функция КурсВалютыПоФормуле(Знач Валюта, Знач Формула, Знач Период)
	...
	Исключение
		...
		Если ОшибкаРасчетаКурсаПоФормуле[Валюта] = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Расчет курса валюты ""%1"" по формуле ""%2"" не выполнен:';uk='Розрахунок курсу валюти ""%1"" за формулою ""%2"" не виконано:'",ОбщегоНазначения.КодОсновногоЯзыка()), Валюта, Формула);
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Валюты.Загрузка курсов валют';uk='Валюти.Завантаження курсів валют'",ОбщегоНазначения.КодОсновногоЯзыка()),
