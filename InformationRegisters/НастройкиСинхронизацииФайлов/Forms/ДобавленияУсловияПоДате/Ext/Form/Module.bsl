...
Процедура ОК(Команда)
	...
	Если ИнтервалИсключение = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Количество дней не может быть равно 0.';uk='Кількість днів не може дорівнювати 0.'"),,, "ИнтервалИсключение");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РеквизитСТипомДата) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Необходимо заполнить условия очистки файлов.';uk='Необхідно заповнити умови очищення файлів.'"),,, "РеквизитСТипомДата");
		Возврат;
	КонецЕсли;
