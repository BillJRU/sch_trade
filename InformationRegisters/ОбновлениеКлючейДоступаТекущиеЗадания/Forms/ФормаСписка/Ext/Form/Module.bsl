...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.Список.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Запись не выделена.';uk='Запис не виділений.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьФоновоеЗаданиеНаСервере(ИдентификаторЗадания, ТекстРезультата)
	...
	Если ФоновоеЗадание = Неопределено Тогда
		ТекстРезультата = НСтр("ru='Не удалость найти фоновое задание по идентификатору.';uk='Не вдалося знайти фонове завдання за ідентифікатором.'");
		Возврат;
	КонецЕсли;
	
	Попытка
		ФоновоеЗадание.Отменить();
		ТекстРезультата = НСтр("ru='Фоновое задание отменено.';uk='Фонове завдання скасовано.'");
	Исключение
		...
		ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось отменить фоновое задание по причине:
                       |%1'
                       |;uk='Не вдалося скасувати фонове завдання через:
