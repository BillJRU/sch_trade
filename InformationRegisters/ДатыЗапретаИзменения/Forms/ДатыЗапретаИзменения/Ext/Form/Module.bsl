...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.ПользователиПолноеПредставление.Заголовок = 
			...
			НСтр("ru='Пользователь, группа пользователей';uk='Користувач, група користувачів'"), НСтр("ru='Пользователь';uk='Користувач'"));
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям" И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		Если ЕстьНедоступныеОбъекты Тогда
			ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав для изменения дат запрета.';uk='Недостатньо прав для зміни дат заборони.'"));
			Возврат;	
		КонецЕсли;	
...
Процедура ДатыЗапретаПередУдалением(Элемент, Отказ)
	...
	Если ВыбраноНесколькоРазделов Тогда
		ТекстВопроса = НСтр("ru='Отключить даты запрета для выбранных разделов?';uk='Відключити дати заборони для обраних розділів?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Во всех остальных случаях, когда ниже нет уточняющих настроек.';uk='У всіх інших випадках, коли нижче немає уточнюючих настройок.'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	...
	ИначеЕсли Не Данные.ЭтоРаздел Тогда	
		...
		ИначеЕсли Не ЗначениеЗаполнено(ДанныеРаздела.ДатаЗапрета) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Общая дата для раздела ""%1""';uk='Спільна дата для розділу ""%1""'"), ДанныеРаздела.Представление);
		Иначе	
