...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПредставлениеОтбора   = НСтр("ru='Календарь = ""%1""';uk='Календар = ""%1""'");
	...
	Если Обработано = 0 И Проблемных <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре %1 не удалось обработать некоторые записи (пропущены): %2';uk='Процедурі %1 не вдалося обробити деякі записи (пропущені): %2'"), 
			ИмяПроцедуры,
			Проблемных);
	...
	ЗаписьЖурналаРегистрации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедура %1 обработала очередную порцию записей: %2';uk='Процедура %1 опрацювала чергову порцію записів: %2'"),
			ИмяПроцедуры,
			Обработано));
