...
Функция WSОпределения(Знач АдресWSDL, Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 10, Знач ЗащищенноеСоединение = Неопределено)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение WSDL';uk='Одержання WSDL'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если Не ОписаниеФайла.Статус Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл описания web-сервиса %1 по причине:
                |%2'
                |;uk='Не вдалося отримати файл опису web-сервісу %1 через:
                |%2'"),
	...
	Исключение
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1
                       ...
                       |Таймаут: %3'"),
			ТекстОшибки,
			Формат(ЗащищенноеСоединение, НСтр("ru='БЛ=Нет; БИ=Да';uk='БЛ=Ні; БИ=Так'")),
			Формат(Таймаут, "ЧГ=0"));
	...
	Если Определения.Сервисы.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл описания web-сервиса по причине:
                       ...
                       |%1'
                       |;uk='Не вдалося отримати файл опису web-сервісу через:
                       |В отриманому файлі не міститься жодного опису сервісу
                       | Можливо, адреса файлу опису вказана невірно:
                       |%1'"),
			Адрес);
	КонецЕсли;
