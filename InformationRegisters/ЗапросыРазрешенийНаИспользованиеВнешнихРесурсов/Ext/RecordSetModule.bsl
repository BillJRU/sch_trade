		...
		Для Каждого Запись Из ЭтотОбъект Цикл
			...
			Если Запись.БезопасныйРежим <> ТекущийБезопасныйРежим Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Безопасный режим %1 отличается от текущего %2';uk='Безпечний режим %1 відрізняється від поточного %2'"),
			...
			Если ПрограммныйМодуль <> Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
				...
				Если Запись.БезопасныйРежим <> БезопасныйРежимПрограммногоМодуля Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Для программного модуля %1 не может быть выполнен запрос разрешений из безопасного режима %2';uk='Для програмного модуля %1 не може бути виконаний запит дозволів з безпечного режиму %2'"),
