...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = НСтр("ru='через';uk='через'") + " " + НапоминанияПользователяКлиент.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		...
		Иначе
			Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Интервал времени не определен.';uk='Інтервал часу не визначений.'"), , "СпособУстановкиВремениНапоминания");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Если Объект.ИнтервалВремениНапоминания > 0 Тогда
		ИнтервалВремениСтрокой = НапоминанияПользователяКлиент.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Интервал времени не определен';uk='Інтервал часу не визначений'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Функция РасчетноеВремяСтрокой()
	...
	ВремяСтрокой = Формат(РасчетноеВремяНапоминания, НСтр("ru='ДФ=H:mm';uk='ДФ=H:mm'"));
