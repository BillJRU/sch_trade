...
Функция ЗаполнитьСпособОбеспечения(Таблица, СпособОбеспечения) Экспорт
	...
	Если ЗначениеЗаполнено(СпособОбеспечения) Тогда
		...
		Для Каждого СтрокаТаблицы Из Таблица Цикл
			...
			Исключение
				...
				ИмяСобытияЖурнала = НСтр("ru='Заполнение способа обеспечения товаров';uk='Заповнення способу забезпечення товарів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ТекстДанныеНеЗаписаны = НСтр("ru='Не удалось записать изменения способа обеспечения товарами.';uk='Не вдалося записати зміни способу забезпечення товарами.'");
	ТекстДанныеЗаписаныЧастично = НСтр("ru='Не все изменения способа обеспечения товарами удалось записать.';uk='Не всі зміни способу забезпечення товарами вдалося записати.'");
