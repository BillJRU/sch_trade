...
Процедура ОповеститьПользователейВариантаОтчета(Записи) Экспорт 
	...
	ЗаголовокОбсуждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Доступ к отчету ""%1""';uk='Доступ до звіту ""%1""'"), ПредставлениеВариантаОтчета);
	...
	Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Предоставлен отчет %1';uk='Надано звіт %1'"), ПолучитьНавигационнуюСсылку(ВариантОтчета));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Варианты отчетов.Оповещение о предоставлении доступа к варианту отчета';uk='Варіанти звітів.Сповіщення про надання доступу до варіанту звіту'",КодОсновногоЯзыка),
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт 
	...
	Пока Вариант.Следующий() Цикл
		...
		Исключение
			...
			ШаблонКомментария = НСтр("ru='Не удалось перенести настройки доступности варианта отчета ""%1"" в регистр ""%2""
                |по причине: %3'
                |;uk='Не вдалося перенести настройки доступності варіанту звіту ""%1"" в регістр ""%2""
                |через: %3'");
	...
	Если Обработано = 0 И Отказано <> 0 Тогда
		...
		ШаблонСообщения = НСтр("ru='Процедуре ПеренестиНастройкиДоступностиВариантаОтчета не удалось обработать некоторые данные: %1';uk='Процедурі ПеренестиНастройкиДоступностиВариантаОтчета не вдалося обробити деякі дані: %1'");
	...
	Иначе
		...
		ШаблонКомментария = НСтр("ru='Процедура ПеренестиНастройкиДоступностиВариантаОтчета обработала очередной пакет данных: %1';uk='Процедура ПеренестиНастройкиДоступностиВариантаОтчета опрацювала черговий пакет даних: %1'");
