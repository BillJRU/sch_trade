...
Процедура СкинутьГраницыНаСервере(ДатаГраницы, Организация)
	...
	Попытка
		...
		Если НЕ ЕстьГраницыВПрошлом Тогда
			ТекстИсключения = НСтр("ru='Нельзя сдвигать границы в будущее, относительно старых границ.';uk='Не можна зсувати межі в майбутнє, відносно старих меж.'");
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось перенести границы заданий по причине: %Причина%';uk='Не вдалося перенести кордону завдань по причині: %Причина%'");
