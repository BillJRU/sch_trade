...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаБалансировкаНагрузкиНаДискПодсказка1.Подсказка =
		НСтр("ru='Если при обновлении доступа скорость передачи данных
                   ...
                   |;uk='Якщо при оновленні доступу швидкість передачі даних
                   |з диска HDD зменшується до 2-10 Мб/сек замість
                   |40-150 мб/сек і диск зайнятий на 100% протягом
                   |5-10 хвилин або більше, то диск повільний.
                   |Прим: SSD диск вважається швидким.'");
...
Процедура ПоследнееЗавершениеОбновленияДоступаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	#Иначе
		...
		ТекстовыйДокумент.Показать(НСтр("ru='Ошибка обновления доступа';uk='Помилка оновлення доступу'"));
	#КонецЕсли
	КонецЕсли;
	...
	Элементы.ГруппаКоличествоПотоковОбновленияДоступа1.Заголовок =
		Формат(КоличествоПотоковОбновленияДоступа, "ЧГ=") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПотоковОбновленияДоступа,
				"", НСтр("ru='поток,потока,потоков,,,,,,0';uk='потік,потоки,потоків,,,,,,0'"));
	...
	Если ЗначениеЗаполнено(Состояние.ПоследнееЗавершениеОбновленияДоступа) Тогда
		...
		Если Состояние.ОбновлениеОтмененоНештатно Тогда
			Если Состояние.ПоследнееЗавершениеСегодня Тогда
				Шаблон = НСтр("ru='<1>отменено нештатно</1> после запуска в %1';uk='<1>скасовано нештатно</1> після запуску в %1'");
			Иначе
				Шаблон = НСтр("ru='<1>отменено нештатно</1> после запуска %1';uk='<1>скасовано нештатно</1> після запуску %1'");
			КонецЕсли;
		...
		ИначеЕсли ЗначениеЗаполнено(ТекстОшибкиОбновленияДоступа) Тогда
			Если Состояние.ОбновлениеОтменено Тогда
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("ru='<1>отменено</1> <2>с ошибкой</2> в %1 через %2';uk='<1>скасовано</1> <2>з помилкою</2> в %1 через %2'");
				Иначе
					Шаблон = НСтр("ru='<1>отменено</1> <2>с ошибкой</2> %1 через %2';uk='<1>скасовано</1> <2>з помилкою</2> %1 через %2'");
				КонецЕсли;
			Иначе
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("ru='<1>завершено</1> <2>с ошибкой</2> в %1 через %2';uk='<1>завершено</1> <2>з помилкою</2> в %1 через %2'");
				Иначе
					Шаблон = НСтр("ru='<1>завершено</1> <2>с ошибкой</2> %1 через %2';uk='<1>завершено</1> <2>з помилкою</2> %1 через %2'");
				КонецЕсли;
			КонецЕсли;
			ФорматЧастей.Вставить(1, Новый Структура("Шрифт, ЦветТекста", ПолужирныйШрифт, КрасныйЦвет));
		...
		Иначе
			Если Состояние.ОбновлениеОтменено Тогда
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("ru='отменено в %1 через %2';uk='скасовано в %1 через %2'");
				Иначе
					Шаблон = НСтр("ru='отменено %1 через %2';uk='скасовано %1 через %2'");
				КонецЕсли;
			Иначе
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
...
Процедура ПоказатьОшибкуОбновленияПрогрессаОбработчикОжидания()
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось обновить прогресс по причине:
                   |
                   |%1'
                   |;uk='Не вдалося оновити прогрес через:
                   |
...
Процедура СортироватьСписок(ПоУбыванию = Ложь)
	...
	ПоказатьОповещениеПользователя(
		?(ПоУбыванию, НСтр("ru='Сортировка по убыванию';uk='Сортування за спаданням'"),
			НСтр("ru='Сортировка по возрастанию';uk='Сортування по зростанню'")),,
