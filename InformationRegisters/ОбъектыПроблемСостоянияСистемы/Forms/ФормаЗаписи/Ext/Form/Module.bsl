...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ПредставлениеПроблемы = НСтр("ru='не удалось нийти описание проблемы';uk='не вдалося знаійти опис проблеми'");
		КонецЕсли;
					...
					Если КлючИЗначение.Ключ = "ИмяРегистраДляРасшифровки" Тогда
						...
						Если МетаданныеРегистраДляРасшифровки <> Неопределено Тогда
							...
							ПредставлениеРегистраДляРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Открыть форму списка регистра ""%1"".';uk='Відкрити форму списку регістра ""%1"".'"),
	...
	Иначе
		ДатаПроверки = НСтр("ru='не удалось определить дату проверки';uk='не вдалося визначити дату перевірки'");
	КонецЕсли;
