...
Процедура НазначитьИсполнителей(Команда)
	...
	Если Назначение = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Необходимо выбрать роль в списке.';uk='Необхідно вибрати роль у списку.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДанныеЭлементов()
	...
	Для каждого СтрокаСписка Из СписокОбъект Цикл
		Если СтрокаСписка.Исполнители = 0 Тогда
			СтрокаСписка.ИсполнителиСтрока = ?(СтрокаСписка.ВнешняяРоль, НСтр("ru='заданы в другой программе';uk='задані в іншій програмі'"), НСтр("ru='не заданы';uk='не задані'"));
		...
		Иначе
			СтрокаСписка.ИсполнителиСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 чел';uk='%1 люд'"), Строка(СтрокаСписка.Исполнители) );
			СтрокаСписка.Картинка = -1;
		КонецЕсли;
	КонецЦикла;
