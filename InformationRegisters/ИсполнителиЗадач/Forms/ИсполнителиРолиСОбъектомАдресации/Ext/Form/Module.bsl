...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Исполнители роли ""%1""';uk='Виконавці ролі ""%1""'"), Роль);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Для каждого СтрокаИсполнитель Из НаборЗаписей Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаИсполнитель.Исполнитель) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Необходимо указать исполнителей.';uk='Необхідно вказати виконавців.'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура Подобрать(Команда)
	...
	Если РольДоступнаДляВнешнихПользователей И РольДоступнаДляПользователей Тогда
		...
		Выбор.Добавить("ВнешнийПользователь", НСтр("ru='Внешний пользователь';uk='Зовнішній користувач'"));
		Выбор.Добавить("Пользователь", НСтр("ru='Пользователь';uk='Користувач'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораТипаПользователя", ЭтотОбъект);
		Выбор.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru='Выберите тип пользователя';uk='Виберіть тип користувача'"));
