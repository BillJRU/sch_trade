...
Процедура ПровестиДокументы(Знач ВыделенныеСтроки, СообщениеОбОшибке = "")
		...
		Исключение
			...
			Если Не БлокировкаУстановлена Тогда
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |%2.'
                    |;uk='Не вдалося провести документ ""%1"" через:
                    |%2.'"),
	...
	Если СчетчикНеПроведенные > 0 Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не проведено документов: %1 из %2.';uk='Не проведено документів: %1 з %2.'"),
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Нет версии для сравнения.';uk='Немає версії для порівняння.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаОшибкиXDTO,             НСтр("ru='Ошибки проверки конвертируемых объектов';uk='Помилки перевірки об''єктів, що конверуються'"), КоличествоОшибокXDTO());	
...
Процедура ПринятьОтклонитьВерсиюНаСервере(Знач ВыделенныеСтроки, ИмяЭлемента, СообщениеОбОшибке = "")
		...
		Иначе
			...
			Исключение
				...
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |%2.'
                    |;uk='Не вдалося прийняти версію об''єкта ""%1"" через:
                    |%2.'"),
	...
	Если СчетчикНеПринятые > 0 Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не принято версий объектов: %1 из %2.';uk='Не прийнято версій об''єктів: %1 з %2.'"),
