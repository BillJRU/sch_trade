...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ДвижениеТоваров);
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ГрафикОтгрузкиТоваров);
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.СвободныеОстатки);
		
		ДвиженияПоОбеспечениюЗаказов = Истина;
		Если ДвиженияПоОбеспечениюЗаказов Тогда
			Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ОбеспечениеЗаказов);
		КонецЕсли;
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ОбеспечениеЗаказовРаботами);
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Документ.ДополнительныеСвойства.Свойство("КонтролироватьОбеспечение")
			...
			Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
					...
					Тогда
				Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ГрафикОтгрузкиТоваров);
			КонецЕсли;
			
			Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
					И (ТипДокумента = Тип("ДокументОбъект.АктОРасхожденияхПослеОтгрузки")
					Или ТипДокумента = Тип("ДокументОбъект.АктОРасхожденияхПослеПеремещения")
					Или ТипДокумента = Тип("ДокументОбъект.АктОРасхожденияхПослеПриемки")
					...
					Или ТипДокумента = Тип("ДокументОбъект.ОтчетОРозничныхПродажах")
						И Не ЗапасыСервер.ПроверитьНеобходимостьПерезаполненияВидовЗапасовДокумента(Документ)
				...
				Или ТипДокумента = Тип("ДокументОбъект.КорректировкаНазначенияТоваров")
					И Не Документ.ДополнительныеСвойства.Свойство("ПрограммноеСозданиеДокумента")Тогда
					Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.СвободныеОстатки);
			КонецЕсли;
			
		КонецЕсли;
		
		Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
				И (ТипДокумента = Тип("ДокументОбъект.ВнутреннееПотреблениеТоваров")
				Или ТипДокумента = Тип("ДокументОбъект.ВозвратТоваровПоставщику")
				Или ТипДокумента = Тип("ДокументОбъект.РеализацияТоваровУслуг")
					И Не ЗапасыСервер.ПроверитьНеобходимостьПерезаполненияВидовЗапасовДокумента(Документ)
				Или ТипДокумента = Тип("ДокументОбъект.ОтгрузкаТоваровСХранения")
			...
			Или ТипДокумента = Тип("ДокументОбъект.АктОРасхожденияхПослеОтгрузки")
			Или ТипДокумента = Тип("ДокументОбъект.АктОРасхожденияхПослеПеремещения")
			Или ТипДокумента = Тип("ДокументОбъект.ЗаказКлиента")
			Или ТипДокумента = Тип("ДокументОбъект.ЗаказНаВнутреннееПотребление")
			Или ТипДокумента = Тип("ДокументОбъект.ЗаказНаПеремещение")
			...
			Или ТипДокумента = Тип("ДокументОбъект.КорректировкаРеализации")
			Или ТипДокумента = Тип("ДокументОбъект.ОприходованиеИзлишковТоваров")
			Или ТипДокумента = Тип("ДокументОбъект.ПеремещениеТоваров")
			...
			Или ТипДокумента = Тип("ДокументОбъект.ПриходныйОрдерНаТовары")
				И Документ.СкладскаяОперация <> Перечисления.СкладскиеОперации.ВозвратНепринятыхТоваров
			...
			Тогда
			Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ОбеспечениеЗаказов);
		...
		Если Не Свойства.ЭтоНовый
			...
			Или ТипДокумента = Тип("ДокументОбъект.ПриобретениеТоваровУслуг")
			Тогда
			Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ОбеспечениеЗаказовРаботами);
		КонецЕсли;
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ДвижениеТоваров");
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ГрафикОтгрузкиТоваров");
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "СвободныеОстатки");
	
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ОбеспечениеЗаказов");
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ОбеспечениеЗаказовРаботами");
...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	ЕстьИзмененияСвободныеОстатки = ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ,
		"ДвиженияСвободныеОстаткиИзменение");
	ЕстьИзмененияГрафикОтгрузкиТоваров = ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ,
		"ДвиженияГрафикОтгрузкиТоваровИзменение");
	
	Если ЕстьИзмененияСвободныеОстатки Или ЕстьИзмененияГрафикОтгрузкиТоваров Тогда
		...
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ОшибкиОбеспечение");
		
	КонецЕсли;
	
	#КонецОбласти
	
	#Область ОбеспечениеЗаказов
	
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ОбеспечениеЗаказовИзменение") Тогда
		...
		Иначе
			...
			Если Не Запрос.Параметры.Свойство("ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров") Тогда
		...
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ОшибкиОбеспечениеЗаказов");
		
	КонецЕсли;
	
	#КонецОбласти
	
	#Область ОбеспечениеЗаказовРаботами
	
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ОбеспечениеЗаказовРаботамиИзменение") Тогда
		...
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ОшибкиОбеспечениеЗаказовРаботами");
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияСвободныеОстаткиИзменение")
		Или ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияГрафикОтгрузкиТоваровИзменение") Тогда
		
		ШаблонСообщенияВНаличииМеньшеНоля = НСтр("ru='Номенклатура %1
|Недостаточно товара в наличии на складе %2 на %3 %4'
|;uk='Номенклатура %1 
|Недостатньо товару в наявності на складі %2 на %3 %4'");
		
		ШаблонСообщенияВНаличииМеньшеРезерваИзЗаказовНаДату = НСтр("ru='Номенклатура %1
|Превышен доступный на %Период% остаток товара на складе %2 на %3 %4'
|;uk='Номенклатура %1
|Перевищено доступний на %Период% залишок товару на складі %2 на %3 %4'");
		
		ШаблонСообщенияВНаличииМеньшеРезерваИзЗаказов = НСтр("ru='Номенклатура %1
|Превышен доступный остаток товара на складе %2 на %3 %4'
|;uk='Номенклатура %1 
|Перевищено доступний залишок товару на складі %2 на %3 %4'");
		
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиОбеспечение Цикл
			...
			Если ЗначениеЗаполнено(СтрокаОшибки.Период)
				И СтрокаОшибки.ВНаличии - СтрокаОшибки.ВРезервеПодЗаказ
					- СтрокаОшибки.ВРезервеСоСклада - СтрокаОшибки.ВРезервеИзЗаказов < 0 Тогда
				
				ШаблонСообщения = СтрЗаменить(ШаблонСообщенияВНаличииМеньшеРезерваИзЗаказовНаДату, "%Период%", СтрокаОшибки.Период);
				Количество      = - (СтрокаОшибки.ВНаличии - СтрокаОшибки.ВРезервеПодЗаказ
					- СтрокаОшибки.ВРезервеСоСклада - СтрокаОшибки.ВРезервеИзЗаказов);
				
			ИначеЕсли СтрокаОшибки.ВНаличии < 0 Тогда
				
				ШаблонСообщения = ШаблонСообщенияВНаличииМеньшеНоля;
				Количество      = - СтрокаОшибки.ВНаличии;
				
			ИначеЕсли СтрокаОшибки.ВНаличии - СтрокаОшибки.ВРезервеПодЗаказ
				- СтрокаОшибки.ВРезервеСоСклада - СтрокаОшибки.ВРезервеИзЗаказов < 0 Тогда
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПредставлениеНоменклатуры,
				СтрокаОшибки.Склад, Количество, СтрокаОшибки.ЕдиницаИзмерения);
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ОбеспечениеЗаказовИзменение") Тогда
		...
		ШаблонСообщенияЗаказатьМеньшеНуля = НСтр("ru='Номенклатура %1 %2 назначение %3:
|	Заказано для обеспечения больше чем требуется на %4 %5.'
|;uk='Номенклатура %1 %2 призначення %3: 
|Замовлено для забезпечення більше ніж потрібно на %4 %5.'");
			
		ШаблонСообщенияНаличиеБольшеПотребности = НСтр("ru='Номенклатура %1 %2 назначение %3:
|	Приходуется для обеспечения больше чем требуется на %4 %5.'
|;uk='Номенклатура %1 %2 призначення %3: 
|Оприбутковується для забезпечення більше ніж потрібно на %4 %5.'");
			
		ШаблонСообщенияНаличиеВМинусе = НСтр("ru='Номенклатура %1 %2 назначение %3:
|	Недостаточное количество товара в наличии на %4 %5.'
|;uk='Номенклатура %1 %2 призначення %3: 
|Недостатня кількість товару в наявності на %4 %5.'");
		
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиОбеспечениеЗаказов Цикл
			
			ПредставлениеНоменклатуры = НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
				СтрокаОшибки.Номенклатура, СтрокаОшибки.Характеристика);
			
			Если СтрокаОшибки.ОшибкаЗаказатьМеньшеНуля Тогда
				
				ТекстСообщения = СтрШаблон(ШаблонСообщенияЗаказатьМеньшеНуля,
					ПредставлениеНоменклатуры, СтрокаОшибки.Склад, СтрокаОшибки.Назначение,
					-СтрокаОшибки.ВеличинаЗаказатьМеньшеНуля, СтрокаОшибки.ЕдиницаИзмерения);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Документ,,, Отказ);
				
			КонецЕсли;
			
			Если СтрокаОшибки.ОшибкаНаличиеБольшеПотребности Тогда
				
				ТекстСообщения = СтрШаблон(ШаблонСообщенияНаличиеБольшеПотребности,
					ПредставлениеНоменклатуры, СтрокаОшибки.Склад, СтрокаОшибки.Назначение,
					СтрокаОшибки.ВеличинаНаличиеБольшеПотребности, СтрокаОшибки.ЕдиницаИзмерения);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Документ,,, Отказ);
				
			КонецЕсли;
			
			Если СтрокаОшибки.ОшибкаНаличиеВМинусе Тогда
				
				ТекстСообщения = СтрШаблон(ШаблонСообщенияНаличиеВМинусе,
					ПредставлениеНоменклатуры, СтрокаОшибки.Склад, СтрокаОшибки.Назначение,
					-СтрокаОшибки.Наличие, СтрокаОшибки.ЕдиницаИзмерения);
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ОбеспечениеЗаказовРаботамиИзменение") Тогда
		...
		ШаблонСообщения = НСтр("ru='Номенклатура %1 подразделение %2 назначение %3
|	Заказано для обеспечения больше чем требуется на %4 %5.'
|;uk='Номенклатура %1 підрозділ %2 призначення %3 
|Замовлено для забезпечення більше ніж потрібно на %4 %5.'");
		
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиОбеспечениеЗаказовРаботами Цикл
			
			ПредставлениеНоменклатуры = НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
				СтрокаОшибки.Номенклатура, СтрокаОшибки.Характеристика);
			
			ТекстСообщения = СтрШаблон(ШаблонСообщения,
				ПредставлениеНоменклатуры, СтрокаОшибки.Подразделение, СтрокаОшибки.Назначение,
...
Функция СостояниеОбеспеченияНаличием(ТаблицаПотребностей) Экспорт
	...
	ОстаткиИОбороты = ОстаткиИОбороты(Отбор, Неопределено, Ложь);
...
// 	Объект - ДанныеФормыСтруктура - Реквизит формы редактируемого документа,
// 	ТабЧасть - ДанныеФормыКоллекция - Табличная часть, реквизит формы редактируемого документа.
// 	ПараметрыЗаполнения - Структура - структура параметров заполнения таблицы:
// * ПереченьВариантов - Массив - Массив действий обеспечения
// * ИзменятьОбособление - Булево - Признак возможности изменения обеспечения
// * ЗаполнятьЦелымиУпаковками - Булево - Признак необходимости контроля целостности упаковок.
// 	ПараметрыДокумента - Структура, Неопределено - Содержит параметры, специфичные для документа:
// * СтатусКВыполнению - ПеречислениеСсылка.СтатусыЗаказовКлиента - Значение статуса заказа в состоянии "к выполнению".
// * ГруппаСкладов - СправочникСсылка.Склады - Группа складов, которые будут использоваться при получении вариантов обеспечения
// * МенеджерРегистра - РегистрНакопленияМенеджер - Менеджер регистра накопления
// * ИмяМенеджераРегистра - Строка - Имя регистра сведений для получения менеджера регистра
...
Функция ТаблицаЗаполнениеОбеспеченияУниверсальный(ТаблицаВыделенныеСтроки,
	...
	ЗначенияПараметровЗаполнения = Новый Структура;
	ЗначенияПараметровЗаполнения.Вставить("ПереченьВариантов", Новый Массив);
	ЗначенияПараметровЗаполнения.Вставить("ИзменятьОбособление", Истина);
	ЗначенияПараметровЗаполнения.Вставить("ЗаполнятьЦелымиУпаковками", Ложь);
	ЗначенияПараметровЗаполнения.Вставить("УчитыватьГрафикОтгрузки", Ложь);
	...
	ВариантыОбеспечения = ВариантыОбеспечения(Отбор, Корректировка, ЗначенияПараметровЗаполнения.УчитыватьГрафикОтгрузки);
...
//   УчитыватьГрафикОтгрузки - Булево - необходимость учитывать график отгрузки
//
// Возвращаемое значение:
//   Структура - Структура следующего состава:
...
//
Функция ДоступныеОстатки(Отбор, Корректировка, УчитыватьГрафикОтгрузки = Ложь) Экспорт

	ОстаткиИОбороты = ОстаткиИОбороты(Отбор, Корректировка, УчитыватьГрафикОтгрузки);
	Результат = Новый Структура();
...
Функция СостояниеОбеспеченияЗаказов(МенеджерВременныхТаблиц, Объект, Фильтры, Режим) Экспорт
	...
	ОстаткиИОбороты = ОстаткиИОбороты(Отбор, Корректировка, Ложь);
...
Процедура ДобавитьВременнуюТаблицуСтатистикиПотребления(Запрос, ИмяТаблицыТовары) Экспорт
	...
	Иначе
		...
		Запрос.УстановитьПараметр("ПериодКлассификацииДатаНачала",    ДобавитьМесяц(Запрос.Параметры.ПериодКлассификацииДатаНачала, - 12));
		Запрос.УстановитьПараметр("ПериодКлассификацииДатаОкончания", ДобавитьМесяц(Запрос.Параметры.ПериодКлассификацииДатаОкончания, - 12));
...
Процедура РассчитатьДатуОтгрузкиВСтрокахТабличнойЧасти(СтрокиТабличнойЧасти, Объект, ТабличнаяЧасть, ИмяПоля = "ДатаОтгрузки") Экспорт
	...
	Для Каждого Идентификатор Из СтрокиТабличнойЧасти Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары[ИмяПоля]) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаТовары.ВариантОбеспечения <> ВариантОбеспеченияТребуется
			...
			СтрокаТовары[ИмяПоля] = Макс(НачалоТекущегоДня, Объект.ЖелаемаяДатаОтгрузки);
	...
	Для Каждого Строка Из Таблица Цикл
		ТабличнаяЧасть[Строка.НомерСтроки - 1][ИмяПоля] = Строка.ДатаОтгрузки;
	КонецЦикла;
...
Функция ТекстЗапросаКоличестваДнейМеждуДатамиГрафика() Экспорт
	...
	ТекстЗапроса =
		...
		|		+ ВЫБОР
		|			КОГДА График2.ДеньВключенВГрафик ТОГДА
		|				0
		|			ИНАЧЕ
		|				1
		|		КОНЕЦ
		...
		|			- ВЫБОР
		|				КОГДА График1.ДеньВключенВГрафик ТОГДА
		|					0
		|				ИНАЧЕ
		|					1
		|			КОНЕЦ
...
//                               отличное от имени по умолчанию.
// Возвращаемое значение:
// 	ТаблицаЗначений - таблица содержащая данные из выделенных строк с именами колонок по умолчанию:
// * Идентификатор - Число -
// * Упаковка - СправочникСсылка.УпаковкиЕдиницыИзмерения -
// * СтрокаНеВыделена - Булево -
// * ПриоритетТипаОбеспечения 
// * СкладНаименование 
// * Упаковка - СправочникСсылка.УпаковкиЕдиницыИзмерения -
...
Функция ВариантыОбеспечения(Отбор, Корректировка, УчитыватьГрафикОтгрузки)

	ДоступныеОстатки = ДоступныеОстатки(Отбор, Корректировка, УчитыватьГрафикОтгрузки);
...
Функция ОстаткиИОбороты(Отбор, Корректировка, УчитыватьГрафикОтгрузки)
	...
	Запрос.Текст = РегистрыНакопления.ГрафикПоступленияТоваров.ТекстЗапросаОстатковИОборотов(
	               ...
	               + РегистрыНакопления.ГрафикПоступленияТоваров.ТекстЗапросаОстатковИОборотовОбособленных(
	                 Корректировка <> Неопределено, Разделы, УчитыватьГрафикОтгрузки)
...
Процедура ЗаписатьДокумент(ДокументОбъект, СообщенияОбОшибках)
	...
	Иначе //нет ошибок, проводим документ
		...
		Исключение
			...
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
			ИмяСобытияЖурнала = НСтр("ru='Обеспечение потребностей.Создание заказов';uk='Забезпечення потреб. Створення замовлень'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
