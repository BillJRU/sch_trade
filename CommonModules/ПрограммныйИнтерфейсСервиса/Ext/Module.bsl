...
Функция СтраницаВыбораТарифаОбслуживающейОрганизации(КодОО,
		ВызыватьИсключениеПриОшибке = Истина, КодСостояния = 0, КодОтвета = 0,
		Сообщение = "") Экспорт
...
Функция УстановитьРольПользователяАбонента(Логин, Роль,
	...
	Результат = ОтправитьДанныеВМенеджерСервиса(ДанныеЗапроса);
	РезультатВыполнения(Результат, ВызыватьИсключениеПриОшибке, КодСостояния, КодОтвета, Сообщение);
...
Процедура ЗавершитьСеансы(Знач НомераСеансов,
...
Функция ЕстьПравоАдминистрированияПриложения(Знач КодПриложения,
		Знач АдресДанных = Неопределено) Экспорт
...
// 	Переименования - Соответствие - переименования полей:
// 	* Ключ - Строка - имя исходного поля.
// 	* Значение  - см. ОписаниеКолонки - структура переименования
// 				- Строка - имя колонки.
// Возвращаемое значение:
...
Функция ПереименоватьСвойства(ДанныеОтвета, Знач Переименования) Экспорт
	...
	Для Каждого Элемент Из Переименования Цикл
		Если ДанныеОтвета.Свойство(Элемент.Ключ) Тогда
			ЗначениеПереименования = Элемент.Значение; // Строка, см. ОписаниеКолонки
			Если ТипЗнч(ЗначениеПереименования) = Тип("Структура") Тогда 
...
// 	ВызыватьИсключениеПриОшибке - Булево - признак вызова исключения при ошибке. Ошибка, когда код состояния <> 10200 или 10202.
// 	КодСостояния - Число - возвращаемый параметр - код состояния HTTP-ответа.
// 	КодОтвета - Число - возвращаемый параметр - заполняется из ответа значением свойства "general.response".
//  Сообщение - Строка - возвращаемый параметр - заполняется из ответа значением свойства "general.message".
// Возвращаемое значение:
// 	Неопределено, Структура - ответ на запрос.
...
// Возвращает данные для переименования и восстановления типа значения поля.
// 
// Параметры:
