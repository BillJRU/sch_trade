...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ВыводитьПолностью Тогда
		...
		Исключение
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Отчет не сформирован! %1';uk='Звіт не сформований!  %1'");
...
Процедура СохранитьТипФайлаТабличногоДокументаПоУмолчанию(ТипФайла) Экспорт
	...
	ОписаниеНастройки.Представление = НСтр("ru='Значение по-умолчанию для формата в диалоге выбора файла при сохранении отчета';uk='Значення по умовчанню для формату в діалозі вибору файлу при збереженні звіту'");
...
Функция ДвоичныеДанныеИзТабличногоДокумента(СохраняемыйДокумент, ТипФайла) Экспорт
	...
	Если ПреобразоватьЧерезПоток = Неопределено Тогда
		...
		ТекстИсключения = НСтр("ru='Сохранение результата отчета в табличный документ формата %1 не предусмотрено';uk='Збереження результату звіту в табличний документ формату %1 не передбачено'");
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, ЗагружатьТолькоРеквизиты = Ложь) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета, "ИдентификаторЗаданияАктуализации")
		...
		БухгалтерскиеОтчеты.УдалитьСсылкуНаСправкуИзОписанияФорматированнойСтроки(ФормаОтчета);
...
Процедура ИнициализироватьКомпоновщикНастроекСКД(КомпоновщикНастроек, СхемаКомпоновкиДанных, ИмяОтчета)
    ...
    Исключение
        ИмяСобытияЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            НСтр("ru='Отчеты.%1';uk='Звіти.%1'",ОбщегоНазначения.КодОсновногоЯзыка()),
            ИмяОтчета);
