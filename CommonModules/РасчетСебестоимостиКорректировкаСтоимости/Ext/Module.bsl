...
Процедура ДобавитьЭтап_ОчисткаДвиженийПриОтключенномУчетеСебестоимости(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ТекстВыполнить     = НСтр("ru='Очистить';uk='Очистити'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура Использование_ОчисткаДвиженийПриОтключенномУчетеСебестоимости(ПараметрыОбработчика) Экспорт
	...
	Если КоличествоДокументов = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru='При отключенном учете себестоимости отсутствуют документы с движениями по регистрам учета себестоимости.';uk='При відключеному обліку собівартості відсутні документи з рухами по регістрах обліку собівартості.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		
	Иначе
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При отключенном учете себестоимости найдены документы с движениями по регистрам учета себестоимости: %1';uk='При відключеному обліку собівартості знайдені документи з рухами по регістрах обліку собівартості: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ДобавитьЭтап_РасчетПартийИСебестоимости(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ТекстВыполнить = НСтр("ru='Рассчитать';uk='Розрахувати'",ОбщегоНазначения.КодОсновногоЯзыка());
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура Использование_РасчетПартийИСебестоимости(ПараметрыОбработчика) Экспорт
	...
	Если МассивДополнительныеОрганизации.Количество() > 0 Тогда
		...
		ТекстДополнительныеОрганизации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Также будет выполнен расчет для организаций, связанных по схеме Интеркампани: %1';uk='Також буде виконано розрахунок для організацій, пов''язаних за схемою Інтеркампані: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|;uk='Виявлено витрати, які можна списати на вибуття минулих періодів. 
|Рекомендується включити настройку ""Розподіл дод. витрат по вибулим товарам"".'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если НачалоПериодаРасчета <= ПериодРасчета Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Требуется пересчет начиная с периода %1.';uk='Потрібний перерахунок починаючи з періоду %1.'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если РасчетСебестоимостиПрикладныеАлгоритмы.ЕстьНекорректныеДвиженияСебестоимости(ПериодРасчета, МассивОрганизаций) Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
        	НСтр("ru='Период еще не был пересчитан в партионном учете версии 2.5.';uk='Період ще не було перераховано в партіоному обліку версії 2.5.'",ОбщегоНазначения.КодОсновногоЯзыка()));
        Возврат; // период не рассчитывался в партионном учете версии 2.5
	...
	Если (ЗначениеЗаполнено(ОрганизацииСДвижениямиПоСебестоимости)
	 ...
	 ИЛИ (ЗначениеЗаполнено(ОрганизацииСОстаткамиПоСебестоимости)
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
			НСтр("ru='Расчет себестоимости в данном периоде еще не выполнялся.';uk='Розрахунок собівартості в цьому періоді ще не виконувався.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
...
Процедура Выполнить_РасчетПартийИСебестоимости(ПараметрыОбработчика) Экспорт
	...
	ПараметрыЗапуска.Вставить("РежимОтладки", 				ПараметрыОбработчика.РежимОтладки);
...
Процедура ИнициализироватьОбщиеПараметрыРасчетаСебестоимости(ПараметрыЗапуска, ПараметрыРасчета, ПараметрыОтладки)
	...
	Если ПараметрыЗапуска.Свойство("РежимОтладки") И ПараметрыЗапуска.РежимОтладки Тогда
	...
	Если ПараметрыРасчета.ЗапущенРасчетПартий Тогда
		...
		Если РасчетСебестоимостиПрикладныеАлгоритмы.РазмерВременнойТаблицы(ПараметрыРасчета, "ВТСтоимостьПартийТоваров") > 0 Тогда
			...
			РасчетСебестоимостиПротоколРасчета.ОчисткаСформированныхДвижений(
				ПараметрыРасчета,
				РасчетСебестоимостиПовтИсп.ИмяРегистраСведенийСтоимостьТоваров());
...
Процедура ИнициализироватьПараметрыРасчетаСебестоимостиПоГруппеОрганизаций(ПараметрыЗапуска, ПараметрыРасчета, ПараметрыОтладки)
	...
	ПараметрыРасчета.Вставить("КоличествоУзловУпр", 0);
	
	// Добавим информацию о текущей итерации расчета себестоимости в протокол расчета
	СтрокаПротокола = НСтр("ru='Вид расчета себестоимости';uk='Вид розрахунку собівартості'",ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
		+ ?(ПараметрыРасчета.ПредварительныйРасчет, НСтр("ru='Предварительный';uk='Попередній'",ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru='Фактический';uk='Фактичний'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПараметрыРасчета.ПредварительныйРасчет Тогда
		...
		ПредставлениеМетодаОценки = НСтр("ru='Не влияет на результат расчета';uk='Не впливає на результат розрахунку'",ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли ПараметрыРасчета.ПартионныйУчетВерсии22 И НЕ ЗначениеЗаполнено(ПараметрыРасчета.МетодОценки) Тогда
		...
		ПредставлениеМетодаОценки = НСтр("ru='Отличный от ""ФИФО (взвешенная оценка)""';uk='Відмінний від ""ФІФО (зважена оцінка)""'",ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ПредставлениеМетодаОценки = ПараметрыРасчета.МетодОценки;
	КонецЕсли;
	
	СтрокаПротокола = НСтр("ru='Метод оценки стоимости';uk='Метод оцінки вартості'",ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
		+ РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(ПредставлениеМетодаОценки);
	РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(ПараметрыРасчета, СтрокаПротокола);
	
	СтрокаПротокола = НСтр("ru='Организации';uk='Організації'",ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
...
Процедура РасчетСебестоимостиТоваров(ПараметрыРасчета, СтрокаСхемыРасчета, ПредварительныйРасчет, УчитыватьКорректировкиСтоимости = Истина) Экспорт
	...
	Если ПараметрыРасчета.Отладка.НеВыполнятьРасчетСебестоимости Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			НСтр("ru='Расчет себестоимости пропущен для целей отладки.';uk='Розрахунок собівартості пропущений для цілей налагодження.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыЗапускаРасчетаСебестоимости.Вставить("РежимОтладки", 	 				 ПараметрыРасчета.РежимОтладки);
...
Процедура РассчитатьСебестоимостьПоГруппеОрганизаций(ПараметрыЗапуска, ПараметрыРасчета, ПараметрыОтладки)
	...
	Если ПараметрыРасчета.КоличествоУзлов <> 0 Тогда
        ...
        РассчитатьСтоимость(ПараметрыРасчета, Ложь, Ложь);
		...
		// Формирует движения по регистрам:
		// - СтоимостьТоваров
        ЗарегистрироватьСтоимость(ПараметрыРасчета, Ложь, Ложь);
	...
	// Этап 3.10
	// Подготовка узлов корректировки для расчета регламентированных стоимостей:
	//	- СтоимостьРегл;
	//	- СтоимостьРеглБезНДС.
	// Формирует временные таблицы:
	// - ВтУзлыКорректировки
	...
	Если ПараметрыРасчета.КоличествоУзловРегл <> 0 Тогда
		...
		// - ВтТаблицаРешений
		// - ВтПеремещенияСписания
        РассчитатьСтоимость(ПараметрыРасчета, Истина, Ложь);
		...
		// Формирует движения по регистрам:
		// - СтоимостьТоваров
        ЗарегистрироватьСтоимость(ПараметрыРасчета, Истина, Ложь);
    ...
    // Этап 3.10у
    // Подготовка узлов корректировки для расчета управленческих стоимостей по правилам МСФО:
    //	- СтоимостьУпр;
    //	- СтоимостьУпрБезНДС.
    // Формирует временные таблицы:
    // - ВтУзлыКорректировки
    СформироватьУзлыКорректировкиСписанияСтоимостиУпр(ПараметрыРасчета);
    
    Если ПараметрыРасчета.КоличествоУзловУпр <> 0 Тогда
    	
    	// Этап 3.11у
    	// Формирует временные таблицы:
    	// - ВтТаблицаРешений
    	// - ВтПеремещенияСписания
        РассчитатьСтоимость(ПараметрыРасчета, Ложь, Истина);
    	...
    	// Формирует движения по регистрам:
    	// - СтоимостьТоваров
        ЗарегистрироватьСтоимость(ПараметрыРасчета, Ложь, Истина);
...
Процедура ПодготовкаДанныхДляПрочихРасходов(ПараметрыРасчета)
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеРасходы();
	
	КопируемыеПоля =
		"Период, ВидДвижения, Организация, Подразделение, НалоговоеНазначение, ХозяйственнаяОперация,
		|СтатьяРасходов, АналитикаРасходов, Сумма, СуммаБезНДС, СуммаРегл, СуммаУпр, ПостояннаяРазница, ВременнаяРазница,
        |НДСРегл, СуммаРеглБезНДС, СуммаУпрБезНДС,
...
Процедура РаспределитьРасходыНаСебестоимость(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.ПредварительныйРасчет Тогда 
        ...
        |	,Таблица.ДопРасходыРеглБезНДС             КАК ДопРасходыРеглБезНДС,
        |	Таблица.ДопРасходыУпрБезНДС              КАК ДопРасходыУпрБезНДС
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		ВТДопРасходов = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТДопРасходов"); 
	КонецЕсли;
...
Процедура СкорректироватьСтоимостьВозвратовПрошлыхПериодов(ПараметрыРасчета)
    ...
    |		КОГДА Себестоимость.КорНалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
    |			ТОГДА Себестоимость.НалоговоеНазначение
    |		ИНАЧЕ Себестоимость.КорНалоговоеНазначение
    ...
    |	СУММА(Себестоимость.ДопРасходыРеглБезНДС)               КАК ДопРасходыРеглБезНДС,
    |	СУММА(Себестоимость.ПостатейныеПостоянныеРеглБезНДС)    КАК ПостатейныеПостоянныеРеглБезНДС,
    |	СУММА(Себестоимость.ПостатейныеПеременныеРеглБезНДС)    КАК ПостатейныеПеременныеРеглБезНДС,
    ...
    |	,СУММА(Себестоимость.СтоимостьУпрБезНДС)    			  КАК СтоимостьУпрБезНДС,
    |	СУММА(Себестоимость.ДопРасходыУпрБезНДС)    			  КАК ДопРасходыУпрБезНДС,
    |	СУММА(Себестоимость.ПостатейныеПостоянныеУпрБезНДС)     КАК ПостатейныеПостоянныеУпрБезНДС,
    |	СУММА(Себестоимость.ПостатейныеПеременныеУпрБезНДС)     КАК ПостатейныеПеременныеУпрБезНДС
    ...
    |	СебестоимостьПродаж.НалоговоеНазначение               КАК НалоговоеНазначение,
	|
    ...
    |	,СУММА(СебестоимостьПродаж.ДопРасходыРеглБезНДС)             КАК ДопРасходыРеглБезНДС,
    |	СУММА(СебестоимостьПродаж.ДопРасходыУпрБезНДС)    		  КАК ДопРасходыУпрБезНДС
	|
    ...
    |		СебестоимостьПродаж.НалоговоеНазначение,
	|
    ...
    |		,СебестоимостьПродаж.ДопРасходыРеглБезНДС,
    |		СебестоимостьПродаж.ДопРасходыУпрБезНДС
	|	ИЗ
    ...
    |	Себестоимость.НДСРеглРеквизит                     КАК НДСРеглРеквизит,
    ...
    |	ТаблицаВозвратов.НДСРеглРеквизит               КАК НДСРеглРеквизит,
    ...
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.НДСРегл / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.НДСРегл / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.НДСРегл
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.НДСРегл КАК ЧИСЛО(31,2)) 						    КАК НДСРегл,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    ...
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.СтоимостьРеглБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.СтоимостьРеглБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.СтоимостьРеглБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.СтоимостьРеглБезНДС КАК ЧИСЛО(31,2)) 						КАК СтоимостьРеглБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ДопРасходыРеглБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА (СтоимостиПродаж22.ДопРасходыРеглБезНДС + ЕСТЬNULL(СуммыДопРасходов.ДопРасходыРеглБезНДС, 0)) / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ДопРасходыРеглБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ДопРасходыРеглБезНДС КАК ЧИСЛО(31,2)) 					КАК ДопРасходыРеглБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ПостатейныеПостоянныеРеглБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.ПостатейныеПостоянныеРеглБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ПостатейныеПостоянныеРеглБезНДС КАК ЧИСЛО(31,2)) 			КАК ПостатейныеПостоянныеРеглБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ПостатейныеПеременныеРеглБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.ПостатейныеПеременныеРеглБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ПостатейныеПеременныеРеглБезНДС КАК ЧИСЛО(31,2)) 			КАК ПостатейныеПеременныеРеглБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    ...
    |	,ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.СтоимостьУпрБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.СтоимостьУпрБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.СтоимостьУпрБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.СтоимостьУпрБезНДС КАК ЧИСЛО(31,2)) 						КАК СтоимостьУпрБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ДопРасходыУпрБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА (СтоимостиПродаж22.ДопРасходыУпрБезНДС + ЕСТЬNULL(СуммыДопРасходов.ДопРасходыУпрБезНДС, 0)) / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ДопРасходыУпрБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ДопРасходыУпрБезНДС КАК ЧИСЛО(31,2)) 						КАК ДопРасходыУпрБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ПостатейныеПостоянныеУпрБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.ПостатейныеПостоянныеУпрБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ПостатейныеПостоянныеУпрБезНДС КАК ЧИСЛО(31,2)) 			КАК ПостатейныеПостоянныеУпрБезНДС,
    |	ВЫРАЗИТЬ(
    |		ВЫБОР КОГДА ТаблицаВозвратов.ПоЦенеПродажи И НЕ СтоимостиПродаж.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж.ПостатейныеПеременныеУпрБезНДС / СтоимостиПродаж.Количество
    |		КОГДА ТаблицаВозвратов.ПоЦенеПродажи22 И НЕ СтоимостиПродаж22.Ссылка ЕСТЬ NULL
    |			ТОГДА СтоимостиПродаж22.ПостатейныеПеременныеУпрБезНДС / СтоимостиПродаж22.Количество
    |			ИНАЧЕ СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС
    |		КОНЕЦ * ТаблицаВозвратов.Количество
    |		- ТаблицаВозвратов.ПостатейныеПеременныеУпрБезНДС КАК ЧИСЛО(31,2)) 			КАК ПостатейныеПеременныеУпрБезНДС
	|ИЗ
    ...
    |			И ТаблицаВозвратов.НалоговоеНазначение          = СтоимостиПродаж22.НалоговоеНазначение
    ...
    |			И ТаблицаВозвратов.НалоговоеНазначение          = СуммыДопРасходов.НалоговоеНазначение
	...
	ОписаниеРегистраСебестоимость = ПараметрыРасчета.Движения[РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияСебестоимостьТоваров()];
	
	ПоляТаблицыВозвратов =
		"ВидДвижения, СлужебноеВидДвиженияПриход, Организация, АналитикаУчетаНоменклатуры, РазделУчета, ВидЗапасов,
        |Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение, "
	...
	Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
		...
		Пока Выборка.Следующий() Цикл
            ...
            СуммаРеглБезНДС   = Выборка.СтоимостьРеглБезНДС + Выборка.ДопРасходыРеглБезНДС + Выборка.ТрудозатратыРегл + Выборка.ПостатейныеПостоянныеРеглБезНДС + Выборка.ПостатейныеПеременныеРеглБезНДС;
			НДСРегл     = Выборка.НДСРегл;
            ...
            СуммаУпрБезНДС    = Выборка.СтоимостьУпрБезНДС + Выборка.ДопРасходыУпрБезНДС + Выборка.ТрудозатратыУпр + Выборка.ПостатейныеПостоянныеУпрБезНДС + Выборка.ПостатейныеПеременныеУпрБезНДС;
			...
			Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
				...
				Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СторноСписанияНаРасходы Тогда
					...
					СуммовыеПоказатели.Вставить("СуммаРеглБезНДС", СуммаРеглБезНДС);
					СуммовыеПоказатели.Вставить("НДСРегл", НДСРегл);
                    СуммовыеПоказатели.Вставить("СуммаУпр", СуммаУпр);
                    СуммовыеПоказатели.Вставить("СуммаУпрБезНДС", СуммаУпрБезНДС);
				...
				ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТарыОтКлиентаПрошлыхПериодов Тогда
					...
					СуммовыеПоказатели.Вставить("СуммаРеглБезНДС", ?(СуммаРеглБезНДС < 0, -СуммаРеглБезНДС, 0));
					СуммовыеПоказатели.Вставить("НДСРегл", ?(НДСРегл < 0, -НДСРегл, 0));
                    СуммовыеПоказатели.Вставить("СуммаУпр", ?(СуммаУпр < 0, -СуммаУпр, 0));
                    СуммовыеПоказатели.Вставить("СуммаУпрБезНДС", ?(СуммаУпрБезНДС < 0, -СуммаУпрБезНДС, 0));
					...
					СформироватьДвиженияПрочиеДоходы(
						...
						?(СуммаРеглБезНДС > 0, СуммаРеглБезНДС, 0),
						?(СуммаУпрБезНДС > 0, СуммаУпрБезНДС, 0)
					);
	...
	Иначе // предварительный расчет
		...
		Если ПараметрыРасчета.РегламентноеЗадание Тогда
			...
			ТаблицаВозвратов.ЗаполнитьЗначения(Неопределено,
                "Партия,  АналитикаФинансовогоУчета, АналитикаУчетаПартий, НалоговоеНазначение");
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимости(ПараметрыРасчета)
    ...
    |	ВЫБОР КОГДА &РегламентноеЗадание
    |		ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
    |		ИНАЧЕ ВложенныйЗапрос.НалоговоеНазначение 
    |	КОНЕЦ										КАК НалоговоеНазначение,
	...
	|	ВЫРАЗИТЬ(0 КАК ЧИСЛО(28,10))               КАК НДСРегл,
    ...
    |		УчетСебестоимости.КорНалоговоеНазначение        КАК НалоговоеНазначение,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)	КАК НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)КАК СтатьяРасходов,
	|		НЕОПРЕДЕЛЕНО                       							КАК АналитикаРасходов,
    ...
    |		 И УчетСебестоимости.КорНалоговоеНазначение = УчетСебестоимости.НалоговоеНазначение
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	Если ПараметрыРасчета.РежимОтладки Тогда	
		...
		ВТПромежуточнаяСебестоимостьТоваров       = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТПромежуточнаяСебестоимостьТоваров");
		ЕстьВозвратныеОтходы                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ЕстьВозвратныеОтходы");
		ВТВозвраты                                = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТВозвраты");
		ВТДопРасходов                             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТДопРасходов");
		ВтУзлыКорректировки                       = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтУзлыКорректировки");
	КонецЕсли;
	...
	ПараметрыНумерации = РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьПараметрыНумерацииСтрокВременнойТаблицы(
		...
		, // без группировки
		"АналитикаУчетаНоменклатуры,
            |ВидЗапасов, Организация, Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение", // порядок
		"НомерУзла", // поле номера
        "АналитикаУчетаНоменклатуры, РазделУчета, ВидЗапасов, Организация, Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение, НомерУзла"); // индекс
    ...
    ПодготовитьДанныеДляРаспределенияРезервов(ПараметрыРасчета, Ложь, Ложь);
	
КонецПроцедуры

// Этап 3.7, 3.11, 3.11у
Процедура РассчитатьСтоимость(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр)
	
    РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "РассчитатьСтоимость" + ?(РасчетСтоимостиРегл, "(Регл)", "") + ?(РасчетСтоимостиУпр, "(Упр)", ""));
    ...
    Запрос.Текст = ?(РасчетСтоимостиРегл,
    	...
    	?(РасчетСтоимостиУпр,
    	    ТекстЗапросаДвиженияСтоимостиУпр(ПараметрыРасчета),
	...
	Если ПараметрыРасчета.РежимОтладки Тогда	
		ВтУзлыКорректировки                                          = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтУзлыКорректировки");
		ВтТаблицаРешений                                             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтТаблицаРешений");
		ВТКэшРасчетныеОборотыСебестоимостьТоваров                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТКэшРасчетныеОборотыСебестоимостьТоваров");
		ЕстьВозвратныеОтходы                                         = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ЕстьВозвратныеОтходы");
		ВтПеремещенияСписания                                        = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтПеремещенияСписания");
	КонецЕсли;
	
	
	Если ПараметрыРасчета.ИспользоватьПлатформуДляРешенияСЛУ Тогда
        РешитьСЛУПлатформой(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр);
	Иначе
        РешитьСЛУ(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр);
	КонецЕсли;
	
КонецПроцедуры

// Этап 3.8, 3.12, 3.12у
Процедура ЗарегистрироватьСтоимость(ПараметрыРасчета, РегистрацияСтоимостиРегл, РегистрацияСтоимостиУпр)
	
    РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "ЗарегистрироватьСтоимость" + ?(РегистрацияСтоимостиРегл, "(Регл)", "") + ?(РегистрацияСтоимостиУпр, "(Упр)", ""));
	
	Запрос = Новый Запрос;
	ИнициализироватьСвойстваЗапроса(Запрос, ПараметрыРасчета);
	
    Запрос.УстановитьПараметр("РегистрацияСтоимостиРегл", РегистрацияСтоимостиРегл);
    Запрос.УстановитьПараметр("РегистрацияСтоимостиУпр", РегистрацияСтоимостиУпр);
    ...
    |	ДанныеПоКоличеству.НалоговоеНазначение,
    ...
    |	УзлыКорректировки.НалоговоеНазначение 		 КАК НалоговоеНазначение,
	...
	|	СУММА(ТаблицаРешений.НДСРегл)                       КАК НДСРегл,
    ...
    |	УзлыКорректировки.НалоговоеНазначение 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	УзлыКорректировки.Организация
	|;
    ...
    |	СтоимостьТоваров.НалоговоеНазначение 		 	  					КАК НалоговоеНазначение,
	|
	// упр.
    |	ВЫБОР КОГДА &РегистрацияСтоимостиРегл ИЛИ &РегистрацияСтоимостиУпр
	|		ТОГДА 0
    ...
    |	ВЫБОР КОГДА &РегистрацияСтоимостиРегл
    |		ТОГДА СтоимостьТоваров.НДСРегл
    |		ИНАЧЕ 0
    |	КОНЕЦ 																КАК НДСРегл,
    |	ВЫБОР КОГДА &РегистрацияСтоимостиРегл
    ...
    |		ИНАЧЕ 0
    |	КОНЕЦ 																КАК ПостатейныеПостоянныеРеглБезНДС,
    |	ВЫБОР КОГДА &РегистрацияСтоимостиРегл
    |		ТОГДА СтоимостьТоваров.ПостатейныеПеременныеГраф_2
    |		ИНАЧЕ 0
    |	КОНЕЦ 																КАК ПостатейныеПеременныеРеглБезНДС,
	|
    // упр. по МСФО
    |	ВЫБОР КОГДА &РегистрацияСтоимостиУпр
    |		ТОГДА СтоимостьТоваров.СтоимостьГраф_1
    |		ИНАЧЕ 0
    ...
    |		ТОГДА СтоимостьТоваров.ПостатейныеПостоянныеГраф_2
    |		ИНАЧЕ 0
    |	КОНЕЦ 																КАК ПостатейныеПостоянныеУпрБезНДС,
    |	ВЫБОР КОГДА &РегистрацияСтоимостиУпр
    |		ТОГДА СтоимостьТоваров.ПостатейныеПеременныеГраф_2
    |		ИНАЧЕ 0
    |	КОНЕЦ 																КАК ПостатейныеПеременныеУпрБезНДС,
	|
	// разницы
    ...
    |			И СтоимостьТоваров.НалоговоеНазначение 			= Обороты.НалоговоеНазначение
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
    ...
    |	СУММА(СтоимостьТоваров.НДСРегл)		 	  					        КАК НДСРегл,
    ...
    |	СУММА(СтоимостьТоваров.СтоимостьРеглБезНДС)		 	  				КАК СтоимостьРеглБезНДС,
    |	СУММА(СтоимостьТоваров.ДопРасходыРеглБезНДС)		 	  			КАК ДопРасходыРеглБезНДС,
    |	СУММА(СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС) 			КАК ПостатейныеПостоянныеРеглБезНДС,
    |	СУММА(СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС) 			КАК ПостатейныеПеременныеРеглБезНДС,
	|
    ...
    |	,СУММА(СтоимостьТоваров.СтоимостьУпрБезНДС)                         КАК СтоимостьУпрБезНДС,
    |	СУММА(СтоимостьТоваров.ДопРасходыУпрБезНДС)                      	КАК ДопРасходыУпрБезНДС,
    |	СУММА(СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС)              КАК ПостатейныеПостоянныеУпрБезНДС,
    |	СУММА(СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС)              КАК ПостатейныеПеременныеУпрБезНДС
	|
    ...
    |		СтоимостьТоваров.ДопРасходыРеглБезНДС		 	  						КАК ДопРасходыРеглБезНДС,
    |		СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС		 	  			КАК ПостатейныеПостоянныеРеглБезНДС,
    |		СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС                   	КАК ПостатейныеПеременныеРеглБезНДС,
	|
    ...
    |		,СтоимостьТоваров.СтоимостьУпрБезНДС                         	КАК СтоимостьУпрБезНДС,
    |		СтоимостьТоваров.ДопРасходыУпрБезНДС		 	  				КАК ДопРасходыУпрБезНДС,
    |		СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС                 КАК ПостатейныеПостоянныеУпрБезНДС,
    |		СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС                 КАК ПостатейныеПеременныеУпрБезНДС
	|	ИЗ
    ...
    |	СтоимостьТоваров.НалоговоеНазначение
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
    ...
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.НДСРегл * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК НДСРегл,
    |	СУММА(
    ...
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.СтоимостьРеглБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК СтоимостьРеглБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ДопРасходыРеглБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ДопРасходыРеглБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ПостатейныеПостоянныеРеглБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ПостатейныеПеременныеРеглБезНДС,
    |	СУММА(
    ...
    |	,СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.СтоимостьУпрБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК СтоимостьУпрБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ДопРасходыУпрБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ДопРасходыУпрБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ПостатейныеПостоянныеУпрБезНДС,
    |	СУММА(
    |		ВЫРАЗИТЬ(СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС * СтоимостьТоваров.Количество
    |				 / Обороты.Количество КАК ЧИСЛО(31,10)))									КАК ПостатейныеПеременныеУпрБезНДС
	|ИЗ
	|	ВТСтоимостьТоваров КАК СтоимостьТоваров
	...
	Если ПараметрыРасчета.РежимОтладки Тогда	
		ВТКэшРасчетныеОборотыСебестоимостьТоваров = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТКэшРасчетныеОборотыСебестоимостьТоваров");
		ВТКэшРасчетныеОстаткиСебестоимостьТоваров = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТКэшРасчетныеОстаткиСебестоимостьТоваров");
		ЕстьВозвратныеОтходы                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ЕстьВозвратныеОтходы");
		ВТОборотыКоличествоСПартиями              = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТОборотыКоличествоСПартиями");
		ВТОборотыКоличествоБезПартий              = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТОборотыКоличествоБезПартий");
		ВтТаблицаРешений                          = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтТаблицаРешений");
		ВтУзлыКорректировки                       = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтУзлыКорректировки");
		ВТСтоимостьТоваровБезВидаУчета            = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТСтоимостьТоваровБезВидаУчета");
		ВТДокументыРасчетаСебестоимости           = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТДокументыРасчетаСебестоимости");
		ВТСтоимостьПартийТоваровВременная         = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТСтоимостьПартийТоваровВременная");
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимостиРегл(ПараметрыРасчета)
	...
	|	ВЫРАЗИТЬ(
	|		ВЫБОР КОГДА СУММА(ВложенныйЗапрос.ВесГраф_1) <> 0 ТОГДА
	|			ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.НДСРегл) КАК ЧИСЛО(28,10)) / СУММА(ВложенныйЗапрос.ВесГраф_1)
	|		ИНАЧЕ
	|			0
	|		КОНЕЦ КАК ЧИСЛО(28,10))   КАК НДСРегл,
	|	ВЫРАЗИТЬ(
	|		ВЫБОР
	...
	|			И УчетСебестоимости.ТипЗаписи <> ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление)
	|			И УчетСебестоимости.СлужебноеВидДвиженияПриход
	|			И (УчетСебестоимости.Количество <> 0 ИЛИ &СредняяЗаМесяц ИЛИ &ФИФОВзвешеннаяОценка ИЛИ &ПартионныйУчетВерсии22)
	|		ТОГДА
	|			УчетСебестоимости.НДСРегл
	|		ИНАЧЕ
	|			0
    ...
    // Показатели узла 2го графа. Данные для расчета ресурса СтоимостьРеглБезНДС.
	|		ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход
	|			 И УчетСебестоимости.ТипЗаписи <> ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление)
	|				ТОГДА УчетСебестоимости.Количество
	...
	|		ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход И &ПартионныйУчетВерсии22
    |				ТОГДА УчетСебестоимости.ДопРасходыРеглБезНДС
	|			КОГДА УчетСебестоимости.ХозяйственнаяОперация В (&МассивОперацийПоступление)
	|				И УчетСебестоимости.СлужебноеВидДвиженияПриход
	|				И УчетСебестоимости.Количество = 0
	|				И НЕ &СредняяЗаМесяц
	...
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода))
	|			ТОГДА
    |				-УчетСебестоимости.ДопРасходыРеглБезНДС
	|		ИНАЧЕ
	|			0
	|		КОНЕЦ КАК ДопРасходыГраф_2,
	
	|		ВЫБОР КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА
    |				УчетСебестоимости.ПостатейныеПостоянныеРеглБезНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ                                        КАК ПостатейныеПостоянныеГраф_2,
	
	|		ВЫБОР КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА
    |				УчетСебестоимости.ПостатейныеПеременныеРеглБезНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ                                        КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК УчетСебестоимости
	...
	|		УчетСебестоимости.ВременнаяРазница 				КАК ВременнаяРазница,
	        // Показатели граф 2.
	|		УчетСебестоимости.Количество                 	КАК ВесГраф_2,
    |		УчетСебестоимости.СтоимостьРеглБезНДС           	КАК СтоимостьГраф_2,
    |		УчетСебестоимости.ДопРасходыРеглБезНДС              КАК ДопРасходыГраф_2,
    |		УчетСебестоимости.ПостатейныеПостоянныеРеглБезНДС  	КАК ПостатейныеПостоянныеГраф_2,
    |		УчетСебестоимости.ПостатейныеПеременныеРеглБезНДС   КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		ВТВозвраты КАК УчетСебестоимости
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		0                                  							КАК ВесГраф_2,
	|		0                                  							КАК СтоимостьГраф_2,
    |		Таблица.ДопРасходыРеглБезНДС             					КАК ДопРасходыГраф_2,
	|		0                                  							КАК ПостатейныеПостоянныеГраф_2,
	|		0                                  							КАК ПостатейныеПеременныеГраф_2
	|
	|	ИЗ
	...
	|		УчетСебестоимости.НДСРеглОстаток 				    КАК НДСРеглОстаток,
	...
	|		УчетСебестоимости.ВременнаяРазницаОстаток 			КАК ВременнаяРазница,
	        // Показатели граф 2.
	|		УчетСебестоимости.КоличествоОстаток					КАК ВесГраф_2,
    |		УчетСебестоимости.СтоимостьРеглБезНДСОстаток 				КАК СтоимостьГраф_2,
    |		УчетСебестоимости.ДопРасходыРеглБезНДСОстаток 				КАК ДопРасходыГраф_2,
    |		УчетСебестоимости.ПостатейныеПостоянныеРеглБезНДСОстаток 	КАК ПостатейныеПостоянныеГраф_2,
    |		УчетСебестоимости.ПостатейныеПеременныеРеглБезНДСОстаток 	КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		РегистрНакопления.СебестоимостьТоваров.Остатки(
	|			&ГраницаНачалоПериода,
	|			Организация В (&МассивОрганизаций)
	...
	ПараметрыНумерации = РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьПараметрыНумерацииСтрокВременнойТаблицы(
		...
		"АналитикаУчетаНоменклатуры, РазделУчета, ВидЗапасов, Организация,
            |Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение", // порядок
		"НомерУзла", // поле номера
		"АналитикаУчетаНоменклатуры, РазделУчета, ВидЗапасов, Организация,
            |Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение, НомерУзла"); // индекс
    ...
    ПодготовитьДанныеДляРаспределенияРезервов(ПараметрыРасчета, Истина, Ложь);
...
// Формирует временную таблицу узлов корректировки - ВтУзлыКорректировки.
//
Процедура СформироватьУзлыКорректировкиСписанияСтоимостиУпр(ПараметрыРасчета)
	
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "СформироватьУзлыКорректировкиСписанияСтоимостиУпр");
	...
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)КАК СтатьяРасходов,
	|		НЕОПРЕДЕЛЕНО                       							КАК АналитикаРасходов,
	// Показатели узла 1го графа. Данные для расчета ресурса СтоимостьУпр.
	|		ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход
	|			 И УчетСебестоимости.ТипЗаписи <> ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление)
	|				ТОГДА УчетСебестоимости.Количество
    ...
    // Показатели узла 2го графа. Данные для расчета ресурса СтоимостьУпрБезНДС.
	|		ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход
	|			 И УчетСебестоимости.ТипЗаписи <> ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление)
	|				ТОГДА УчетСебестоимости.Количество
	...
	|		 И (НЕ &ПредварительныйРасчет
	|				ИЛИ УчетСебестоимости.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)) ТОГДА
    |			УчетСебестоимости.СтоимостьУпрБезНДС
	|
	|		КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход
	|			И УчетСебестоимости.РасчетСебестоимости
	|		ТОГДА
	...
	|		ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход И &ПартионныйУчетВерсии22
    |				ТОГДА УчетСебестоимости.ДопРасходыУпрБезНДС
	|			КОГДА УчетСебестоимости.ХозяйственнаяОперация В (&МассивОперацийПоступление)
	|				И УчетСебестоимости.СлужебноеВидДвиженияПриход
	|				И УчетСебестоимости.Количество = 0
	|				И НЕ &СредняяЗаМесяц
	...
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода))
	|			ТОГДА
    |				-УчетСебестоимости.ДопРасходыУпрБезНДС
	|		ИНАЧЕ
	|			0
	|		КОНЕЦ КАК ДопРасходыГраф_2,
	
	|		ВЫБОР КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА
    |				УчетСебестоимости.ПостатейныеПостоянныеУпрБезНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ                                        КАК ПостатейныеПостоянныеГраф_2,
	
	|		ВЫБОР КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА
    |				УчетСебестоимости.ПостатейныеПеременныеУпрБезНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ                                        КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК УчетСебестоимости
	...
	|		НЕОПРЕДЕЛЕНО                       							КАК АналитикаРасходов,
	        // Показатели граф 1.
	|		УчетСебестоимости.Количество 					КАК ВесГраф_1,
	|		УчетСебестоимости.СтоимостьУпр 				КАК СтоимостьГраф_1,
	|		УчетСебестоимости.ДопРасходыУпр 				КАК ДопРасходыГраф_1,
	|		УчетСебестоимости.ТрудозатратыУпр 				КАК ТрудозатратыГраф_1,
	|		УчетСебестоимости.ПостатейныеПостоянныеУпр 	КАК ПостатейныеПостоянныеГраф_1,
	|		УчетСебестоимости.ПостатейныеПеременныеУпр 	КАК ПостатейныеПеременныеГраф_1,
	        // Показатели граф 2.
	|		УчетСебестоимости.Количество                 	КАК ВесГраф_2,
    |		УчетСебестоимости.СтоимостьУпрБезНДС           КАК СтоимостьГраф_2,
    |		УчетСебестоимости.ДопРасходыУпрБезНДС              КАК ДопРасходыГраф_2,
    |		УчетСебестоимости.ПостатейныеПостоянныеУпрБезНДС  	КАК ПостатейныеПостоянныеГраф_2,
    |		УчетСебестоимости.ПостатейныеПеременныеУпрБезНДС   КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		ВТВозвраты КАК УчетСебестоимости
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		0                                  							КАК ВесГраф_1,
	|		0                                  							КАК СтоимостьГраф_1,
	|		Таблица.ДопРасходыУпр             							КАК ДопРасходыГраф_1,
	|		0                                  							КАК ТрудозатратыГраф_1,
	|		0                                  							КАК ПостатейныеПостоянныеГраф_1,
	|		0                                  							КАК ПостатейныеПеременныеГраф_1,
	        // Показатели граф 2.
	|		0                                  							КАК ВесГраф_2,
	|		0                                  							КАК СтоимостьГраф_2,
    |		Таблица.ДопРасходыУпрБезНДС             					КАК ДопРасходыГраф_2,
	|		0                                  							КАК ПостатейныеПостоянныеГраф_2,
	|		0                                  							КАК ПостатейныеПеременныеГраф_2
	|
	|	ИЗ
	...
	|		НЕОПРЕДЕЛЕНО                       							КАК АналитикаРасходов,
	        // Показатели граф 1.
	|		УчетСебестоимости.КоличествоОстаток 				КАК ВесГраф_1,
	|		УчетСебестоимости.СтоимостьУпрОстаток 				КАК СтоимостьГраф_1,
	|		УчетСебестоимости.ДопРасходыУпрОстаток 			КАК ДопРасходыГраф_1,
	|		УчетСебестоимости.ТрудозатратыУпрОстаток 			КАК ТрудозатратыГраф_1,
	|		УчетСебестоимости.ПостатейныеПостоянныеУпрОстаток 	КАК ПостатейныеПостоянныеГраф_1,
	|		УчетСебестоимости.ПостатейныеПеременныеУпрОстаток 	КАК ПостатейныеПеременныеГраф_1,
	        // Показатели граф 2.
	|		УчетСебестоимости.КоличествоОстаток					КАК ВесГраф_2,
    |		УчетСебестоимости.СтоимостьУпрБезНДСОстаток 				КАК СтоимостьГраф_2,
    |		УчетСебестоимости.ДопРасходыУпрБезНДСОстаток 				КАК ДопРасходыГраф_2,
    |		УчетСебестоимости.ПостатейныеПостоянныеУпрБезНДСОстаток 	КАК ПостатейныеПостоянныеГраф_2,
    |		УчетСебестоимости.ПостатейныеПеременныеУпрБезНДСОстаток 	КАК ПостатейныеПеременныеГраф_2
	|	ИЗ
	|		РегистрНакопления.СебестоимостьТоваров.Остатки(
	|			&ГраницаНачалоПериода,
	|			Организация В (&МассивОрганизаций)
	...
	Если НЕ ПараметрыРасчета.ПредварительныйРасчет
	 И ПараметрыРасчета.МетодОценки = Перечисления.МетодыОценкиСтоимостиТоваров.ФИФОВзвешеннаяОценка Тогда
		ПодготовитьДанныеДляРасчетаСтоимостиУпрПоФИФО(ПараметрыРасчета, Запрос); // корректировка ВтУзлыКорректировки
	КонецЕсли;
	...
	ПараметрыРасчета.КоличествоУзловУпр =
		РасчетСебестоимостиПрикладныеАлгоритмы.РазмерВременнойТаблицы(ПараметрыРасчета, "ВтУзлыКорректировки");

    ПодготовитьДанныеДляРаспределенияРезервов(ПараметрыРасчета, Ложь, Истина);
...
Процедура СкорректироватьСтоимостьСписанияЗапасов(ПараметрыРасчета)
    ...
    |	СУММА(УчетСебестоимости.НДСРеглРеквизит)            КАК НДСРеглРеквизит,
    |	СУММА(УчетСебестоимости.СтоимостьУпр)              	КАК СтоимостьУпр,
    |	СУММА(УчетСебестоимости.СтоимостьУпрБезНДС)         КАК СтоимостьУпрБезНДС,
    ...
    |	(ВЫБОР КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ДопРасходыРеглБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ДопРасходыРеглБезНДС,
	|
	|	(ВЫБОР КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
	|	ИНАЧЕ
    ...
    |	(ВЫБОР КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ПостатейныеПостоянныеРеглБезНДС,
	|
    |	(ВЫБОР КОГДА ТаблицаКорректировки.ДокументДвижения ССЫЛКА Документ.ВозвратТоваровМеждуОрганизациями
    |		И ТаблицаКорректировки.Количество < 0
    ...
    |	КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ПостатейныеПеременныеРеглБезНДС,
	|
    |	(ВЫБОР
    ...
    |	(ВЫБОР
    |		КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА - ТаблицаКорректировки.СтоимостьРеглБезНДС
    |		ИНАЧЕ ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.СтоимостьРеглБезНДС, 0) КАК ЧИСЛО(31,2))
    |		 	- ТаблицаКорректировки.СтоимостьРеглБезНДС КОНЕЦ) КАК СтоимостьРеглБезНДС,
    |	(ВЫБОР
    |		КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА - ТаблицаКорректировки.НДСРегл
    |		ИНАЧЕ ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.НДСРегл, 0) КАК ЧИСЛО(31,2))
    |		 	- ТаблицаКорректировки.НДСРегл КОНЕЦ) КАК НДСРегл,
	|	(ВЫБОР
    ...
    |	(ВЫБОР
    |		КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА - ТаблицаКорректировки.СтоимостьУпрБезНДС
    |		ИНАЧЕ ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.СтоимостьУпрБезНДС, 0) КАК ЧИСЛО(31,2))
    |		 	- ТаблицаКорректировки.СтоимостьУпрБезНДС КОНЕЦ) КАК СтоимостьУпрБезНДС,
    |	(ВЫБОР КОГДА ТаблицаКорректировки.ДокументДвижения ССЫЛКА Документ.ВозвратТоваровМеждуОрганизациями
    |		И ТаблицаКорректировки.Количество < 0
    |		И НЕ &ПартионныйУчетВерсии22
    |	ТОГДА
    ...
    |	КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ДопРасходыУпрБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ДопРасходыУпрБезНДС,
	|	(ВЫБОР КОГДА ТаблицаКорректировки.ДокументДвижения ССЫЛКА Документ.ВозвратТоваровМеждуОрганизациями
	|		И ТаблицаКорректировки.Количество < 0
	|		И НЕ &ПартионныйУчетВерсии22
    ...
    |	КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ПостатейныеПостоянныеУпрБезНДС,
    ...
    |	КОГДА ТаблицаКорректировки.ПродукцияДавальца ТОГДА 0
    |	ИНАЧЕ
    |		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество * ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС, 0) КАК ЧИСЛО(31,2))
    |	КОНЕЦ) КАК ПостатейныеПеременныеУпрБезНДС,
	|
    |	ТаблицаКорректировки.НДСРеглРеквизит КАК НДСРеглРеквизит,
    ...
    |	ТаблицаКорректировки.СтоимостьУпр КАК ИсходнаяСтоимостьУпр,
    |	ТаблицаКорректировки.СтоимостьУпрБезНДС КАК ИсходнаяСтоимостьУпрБезНДС,
    ...
    |		И СтоимостьТоваров.НалоговоеНазначение 			= ТаблицаКорректировки.НалоговоеНазначение
    ...
    |	ТаблицаКорректировки.ДопРасходыРеглБезНДС КАК ДопРасходыРеглБезНДС,
    |	ТаблицаКорректировки.ПостатейныеПостоянныеРеглБезНДС КАК ПостатейныеПостоянныеРеглБезНДС,
    |	ТаблицаКорректировки.ПостатейныеПеременныеРеглБезНДС КАК ПостатейныеПеременныеРеглБезНДС,
    ...
    |	ТаблицаКорректировки.СтоимостьУпрБезНДС КАК СтоимостьУпрБезНДС,
    |	ТаблицаКорректировки.ДопРасходыУпрБезНДС КАК ДопРасходыУпрБезНДС,
    |	ТаблицаКорректировки.ПостатейныеПостоянныеУпрБезНДС КАК ПостатейныеПостоянныеУпрБезНДС,
    |	ТаблицаКорректировки.ПостатейныеПеременныеУпрБезНДС КАК ПостатейныеПеременныеУпрБезНДС,
	|
    |	ТаблицаКорректировки.НДСРеглРеквизит КАК НДСРеглРеквизит,
	|	ТаблицаКорректировки.НДСУпр КАК НДСУпр,
	...
	|	ТаблицаКорректировки.ИсходнаяСтоимостьРеглБезНДС КАК ИсходнаяСтоимостьРеглБезНДС,
	|	ТаблицаКорректировки.ИсходнаяНДСРегл КАК ИсходнаяНДСРегл,
    |	ТаблицаКорректировки.ИсходнаяСтоимостьУпр КАК ИсходнаяСтоимостьУпр,
    |	ТаблицаКорректировки.ИсходнаяСтоимостьУпрБезНДС КАК ИсходнаяСтоимостьУпрБезНДС,
	|	ТаблицаКорректировки.Количество КАК Количество,
	|
	|	ВЫБОР
    ...
    |	СтоимостьРеглБезНДС + ДопРасходыРеглБезНДС + ТрудозатратыРегл + ПостатейныеПостоянныеРеглБезНДС + ПостатейныеПеременныеРеглБезНДС КАК СуммаРеглБезНДС,
    |	СтоимостьУпр + ДопРасходыУпр + ТрудозатратыУпр + ПостатейныеПостоянныеУпр + ПостатейныеПеременныеУпр КАК СуммаУпр,
    |	СтоимостьУпрБезНДС + ДопРасходыУпрБезНДС + ТрудозатратыУпр + ПостатейныеПостоянныеУпрБезНДС + ПостатейныеПеременныеУпрБезНДС КАК СуммаУпрБезНДС,
	|
	|	ВЫБОР
    ...
    |
    |			ИЛИ ТаблицаКорректировки.СтоимостьРеглБезНДС + ТаблицаКорректировки.ДопРасходыРеглБезНДС + ТаблицаКорректировки.ТрудозатратыРегл
    |			+ ТаблицаКорректировки.ПостатейныеПостоянныеРеглБезНДС + ТаблицаКорректировки.ПостатейныеПеременныеРеглБезНДС  <> 0
    |
    |			ИЛИ ТаблицаКорректировки.СтоимостьУпрБезНДС + ТаблицаКорректировки.ДопРасходыУпрБезНДС + ТаблицаКорректировки.ТрудозатратыУпр
    |			+ ТаблицаКорректировки.ПостатейныеПостоянныеУпрБезНДС + ТаблицаКорректировки.ПостатейныеПеременныеУпрБезНДС  <> 0 
	|		
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		ЕстьВозвратныеОтходы                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ЕстьВозвратныеОтходы");
		ВТКэшРасчетныеОборотыСебестоимостьТоваров = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТКэшРасчетныеОборотыСебестоимостьТоваров");
		РаботыДляДавальца                         = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "РаботыДляДавальца");
		СторноОтчетыКомиссионера                  = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "СторноОтчетыКомиссионера");
		ВтТаблицаКорректировки                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтТаблицаКорректировки");
		ВТСтоимостьПартийТоваров                  = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТСтоимостьПартийТоваров");
		ДвиженияТаблицаКорректировки              = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ДвиженияТаблицаКорректировки");
	КонецЕсли;
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
		...
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаРеглБезНДС", "(СуммаРеглБезНДС + ВЫБОР КОГДА ЭтоВыпускПродукции ТОГДА ИсходнаяСтоимостьРеглБезНДС ИНАЧЕ 0 КОНЕЦ)");
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстНДСРегл", "(НДСРегл + ВЫБОР КОГДА ЭтоВыпускПродукции ТОГДА ИсходнаяНДСРегл ИНАЧЕ 0 КОНЕЦ)");
        ...
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпрБезНДС", "(СуммаУпрБезНДС + ВЫБОР КОГДА ЭтоВыпускПродукции ТОГДА ИсходнаяСтоимостьУпрБезНДС ИНАЧЕ 0 КОНЕЦ)");
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
		...
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаРеглБезНДС", "СуммаРеглБезНДС");
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстНДСРегл", "0");
        
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпр", "СуммаУпр");
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпрБезНДС", "СуммаУпрБезНДС");
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстПостояннаяРазница", "ПостояннаяРазница");
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстВременнаяРазница", "ВременнаяРазница");
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
		...
		СтруктураТекстовСуммовыхПоказателей = Новый Структура;
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСумма", "ВЫБОР КОГДА ЭтоВозвратМеждуОрганизациями ТОГДА 0 ИНАЧЕ СуммаСНДС КОНЕЦ");
		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаБезНДС", "ВЫБОР КОГДА ЭтоВозвратМеждуОрганизациями ТОГДА 0 ИНАЧЕ СуммаБезНДС КОНЕЦ");
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпр", "СуммаУпр");
        СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпрБезНДС", "СуммаУпрБезНДС");
		...
		СформироватьДвиженияПрочиеДоходыВУпрУчетеЗапросом(ПараметрыРасчета, "ВтПрочиеДоходыВозвратПоставщикуУпр", СоответствиеВременныхТаблицДвижений, 
			УсловиеГДЕ + "
				...
				|	)"
				,
			"ВЫБОР КОГДА ЭтоВозвратМеждуОрганизациями ТОГДА 0 ИНАЧЕ -СуммаСНДС КОНЕЦ",
			"- СуммаУпр",
			"- СуммаУпрБезНДС"
		);
		...
		Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходовРегл Тогда
            ...
            СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаУпрБезНДС", "0");
			СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаРегл", "СуммаРегл");
            
            СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстСуммаРеглБезНДС", "СуммаРеглБезНДС");
    		СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстНДСРегл", "НДСРегл");
			СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстПостояннаяРазница", "ПостояннаяРазница");
			СтруктураТекстовСуммовыхПоказателей.Вставить("ТекстВременнаяРазница", "ВременнаяРазница");
...
Процедура СформироватьДвиженияСебестоимостьТоваровЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияСебестоимостьТоваров();
    ...
    |НДСРеглРеквизит, НДСУпр, СтатьяКалькуляции, ТипЗаписи, ДокументИсточник
	|";
	...
	Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
        ...
        |ДопРасходыРеглБезНДС, ПостатейныеПостоянныеРеглБезНДС, ПостатейныеПеременныеРеглБезНДС,
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
	    ВременнаяТаблица = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, ИмяВременнойТаблицы);
	КонецЕсли;
...
Процедура СформироватьКорДвиженияСебестоимостьТоваровЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
    ...
    |СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, СтоимостьЗабалансоваяРегл, ПостояннаяРазница, ВременнаяРазница";
	Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
		КопируемыеПоля = КопируемыеПоля + ",
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ВидЗапасов", "
		|ВЫБОР 
		|	КОГДА ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию) 
		|		И АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) ТОГДА
	...
	Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "НалоговоеНазначение", "
		|ВЫБОР 
		|	КОГДА ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию) ТОГДА
		|		КорВидЗапасов.НалоговоеНазначение 
		|	ИНАЧЕ
		|		КорНалоговоеНазначение 
		|КОНЕЦ");
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьРеглБезНДС",
		"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ СтоимостьРеглБезНДС КОНЕЦ");

	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "НДСРегл",
		"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ НДСРегл КОНЕЦ");
	...
	Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
        ...
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ДопРасходыРеглБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ДопРасходыРеглБезНДС КОНЕЦ");
        
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ПостатейныеПостоянныеРеглБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ПостатейныеПостоянныеРеглБезНДС КОНЕЦ");
        
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ПостатейныеПеременныеРеглБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ПостатейныеПеременныеРеглБезНДС КОНЕЦ");
        ...
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьУпрБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ СтоимостьУпрБезНДС КОНЕЦ");
        
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ДопРасходыУпрБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ДопРасходыУпрБезНДС КОНЕЦ");
        
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ПостатейныеПостоянныеУпрБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ПостатейныеПостоянныеУпрБезНДС КОНЕЦ");
        
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "ПостатейныеПеременныеУпрБезНДС",
        	"ВЫБОР КОГДА ЭтоПередачаМеждуОрганизациями ТОГДА 0 ИНАЧЕ ПостатейныеПеременныеУпрБезНДС КОНЕЦ");
...
Процедура СформироватьКорДвиженияСебестоимостьТоваровНеПередачаЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
	...
	Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "АналитикаФинансовогоУчета", "КорАналитикаФинансовогоУчета");
        РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "НалоговоеНазначение", "КорНалоговоеНазначение");
    ...
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьРегл", "НДСРеглРеквизит");
    ...
    |	И (НДСРеглРеквизит <> 0 ИЛИ НДСУпр <> 0)
	|";
...
Процедура СформироватьДвиженияСебестоимостьТоваровСторноВУпрУчетеЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьРеглБезНДС", "0");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "НДСРегл", "0");
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьУпр", "0");
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СтоимостьУпрБезНДС", "0");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "Количество", "0");
...
Процедура СформироватьДвиженияНоменклатураДоходыРасходыЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияДвиженияНоменклатураДоходыРасходы();
    ...
    |СтоимостьРеглБезНДС, ДопРасходыРеглБезНДС, ПостатейныеПостоянныеРеглБезНДС, ПостатейныеПеременныеРеглБезНДС,
    |СтоимостьУпр, ДопРасходыУпр, ТрудозатратыУпр, ПостатейныеПостоянныеУпр, ПостатейныеПеременныеУпр,
    |СтоимостьУпрБезНДС, ДопРасходыУпрБезНДС, ПостатейныеПостоянныеУпрБезНДС, ПостатейныеПеременныеУпрБезНДС,
    ...
    |ПолнаяСуммаРеглБезНДС, ПолнаяСуммаУпрБезНДС,
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		ДвиженияТаблицаКорректировки = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ДвиженияТаблицаКорректировки", УсловиеГДЕ);
	    ВременнаяТаблица = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, ИмяВременнойТаблицы);
	КонецЕсли;
	СоответствиеВременныхТаблицДвижений.Вставить(ИмяВременнойТаблицы, ИмяРегистра);
...
Процедура СформироватьДвиженияНоменклатураНоменклатураЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ, ТекстХозяйственнаяОперация)
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияДвиженияНоменклатураНоменклатура();
...
Процедура СформироватьДвиженияПоОборотнымРегистрамУпрУчетаЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
	...
	|	ВЫБОР 
    |		КОГДА СуммаСНДС <> 0 ИЛИ СуммаБезНДС  <> 0 ИЛИ СуммаРегл  <> 0 ИЛИ СуммаУпр <> 0 ИЛИ СуммаРеглБезНДС  <> 0 ИЛИ СуммаУпрБезНДС <> 0 ТОГДА
	|			ИСТИНА
	|		ИНАЧЕ
	|			ЛОЖЬ
	|	КОНЕЦ
	...
	Иначе
        ...
        СтруктураКорректировкиСумм.Вставить("КоэффициентСуммыРеглБезНДС", -1);
        СтруктураКорректировкиСумм.Вставить("КоэффициентСуммыУпр", -1);
        СтруктураКорректировкиСумм.Вставить("КоэффициентСуммыУпрБезНДС", -1);
	...
	Пока ДанныеДвижения.Следующий() Цикл
		...
		Если ДанныеДвижения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями Тогда
            СкорректированныеСуммы = СкорректироватьСуммовыеПоказателиДвиженияНоменклатуры(ДанныеСумм, 0, 0, 1, 1, 1, 1, 0);
		Иначе
            СкорректированныеСуммы = СкорректироватьСуммовыеПоказателиДвиженияНоменклатуры(ДанныеСумм, 1, 1, 1, 1, 1, 1, 0);
		КонецЕсли;
		...
		ИначеЕсли СкорректированныеСуммы.ПолнаяСуммаСНДС < 0
		 ...
		 ИЛИ СкорректированныеСуммы.ПолнаяСуммаУпр < 0 Тогда 
            ...
            СкорректированныеСуммы = СкорректироватьСуммовыеПоказателиДвиженияНоменклатуры(СкорректированныеСуммы, -1, -1, -1, -1, -1, -1, 0);
...
Функция СкорректироватьСуммовоеПолеДвиженияНоменклатурыЗапросом(Поле, СтруктураКорректировкиСумм)
    ...
    ИменаПолейРеглБезНДС   = Новый Структура("СтоимостьРеглБезНДС, НДСРегл, ДопРасходыРеглБезНДС, ПостатейныеПостоянныеРеглБезНДС, ПостатейныеПеременныеРеглБезНДС");
    ...
    ИменаПолейУпрБезНДС = Новый Структура("СтоимостьУпрБезНДС, ДопРасходыУпрБезНДС, ПостатейныеПостоянныеУпрБезНДС, ПостатейныеПеременныеУпрБезНДС");
    ...
    ИначеЕсли ИменаПолейРеглБезНДС.Свойство(Поле) Тогда
    	
    	НовоеПоле = Поле + " * " + Формат(СтруктураКорректировкиСумм.КоэффициентСуммыРеглБезНДС, "ЧРД=.; ЧН=0; ЧГ=0");
    ...
    ИначеЕсли ИменаПолейУпрБезНДС.Свойство(Поле) Тогда
    	
    	НовоеПоле = Поле + " * " + Формат(СтруктураКорректировкиСумм.КоэффициентСуммыУпрБезНДС, "ЧРД=.; ЧН=0; ЧГ=0");
	КонецЕсли;
...
Процедура СформироватьДвиженияПрочиеАктивыПассивыЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений, УсловиеГде)
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеАктивыПассивы();
...
Процедура СформироватьДвиженияПрочиеРасходыВУпрУчетеЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
    ...
    ТекстСуммаУпрБезНДС = СтруктураТекстовСуммовыхПоказателей.ТекстСуммаУпрБезНДС;
	...
	КопируемыеПоля = КопируемыеПоля + ",НалоговоеНазначение";
    ...
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаУпрБезНДС", "
    |ВЫБОР
    |	КОГДА НЕ ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПрочиеАктивы) ТОГДА
    |		ВЫБОР
    |			КОГДА НЕ &УправленческийУчетОрганизаций ИЛИ ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию) ТОГДА
    ...
    |				0
    |			ИНАЧЕ
    |				Знак * (" + ТекстСуммаУпрБезНДС + ")
    |		КОНЕЦ
    |	ИНАЧЕ
    |		" + РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьЗначениеПоляЗапроса(ТаблицаОписанияПолей, "СуммаУпрБезНДС") + " 
    |КОНЕЦ");
	...
	|		ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
	|	ИНАЧЕ
	|		" + РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьЗначениеПоляЗапроса(ТаблицаОписанияПолей, "НалоговоеНазначение") + " 
	|КОНЕЦ");
	...
	СформироватьДвиженияПрочиеРасходыКорЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
            ТекстСумма, ТекстСуммаБезНДС, ТекстСуммаУпр, ТекстСуммаУпрБезНДС);
...
Процедура СформироватьДвиженияПрочиеРасходыЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
	...
	ТекстСуммаРеглБезНДС = СтруктураТекстовСуммовыхПоказателей.ТекстСуммаРеглБезНДС;
	ТекстНДСРегл = СтруктураТекстовСуммовыхПоказателей.ТекстНДСРегл;
    ТекстСуммаУпр = СтруктураТекстовСуммовыхПоказателей.ТекстСуммаУпр;
    ТекстСуммаУпрБезНДС = СтруктураТекстовСуммовыхПоказателей.ТекстСуммаУпрБезНДС;
	...
	// СуммаРеглБезНДС
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаРеглБезНДС", "
	|ВЫБОР
	|	КОГДА НЕ &ИспользоватьУчетПрочихДоходовРасходовРегл
	| 		ИЛИ ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию)
	| 		ИЛИ ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПрочиеАктивы)
...
Процедура СформироватьДвиженияПрочиеРасходыКорЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаБезНДС", "-(" + ТекстСуммаБезНДС + ")");
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаУпр", "-(" + ТекстСуммаУпр + ")");
    РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаУпрБезНДС", "-(" + ТекстСуммаУпрБезНДС + ")");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаРегл", "0");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаРеглБезНДС", "0");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "НДСРегл", "0");
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
    	ВременнаяТаблица = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, ИмяВременнойТаблицы + "КорСторнированиеРасходовУУ");
	КонецЕсли;
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		ДвиженияТаблицаКорректировки = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ДвиженияТаблицаКорректировки");
		ВременнаяТаблица             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, ИмяВременнойТаблицы + "КорРегистрацияРасходовУУ");
	КонецЕсли;
...
Процедура СформироватьДвиженияПрочиеДоходыВУпрУчетеЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
			ТекстСумма, ТекстСуммаУпр, ТекстСуммаУпрБезНДС)
	
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеДоходы();
...
Процедура СформироватьДвиженияПрочиеДоходыЗапросом(ПараметрыРасчета, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений, УсловиеГДЕ,
			ТекстСумма, ТекстСуммаРегл, ТекстСуммаУпр, ТекстСуммаРеглБезНДС, ТекстСуммаУпрБезНДС)
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходовРегл Тогда
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей, "СуммаРеглБезНДС", ТекстСуммаРеглБезНДС);
	КонецЕсли;
...
Процедура СкорректироватьСтоимостьПродаж(ПараметрыРасчета)
	...
	|	(ВЫБОР
	|		КОГДА ЛОЖЬ ТОГДА НЕОПРЕДЕЛЕНО // для удобства расстановки тегов
	|		ИНАЧЕ Себестоимость.ВидЗапасов КОНЕЦ),
	|	Себестоимость.Организация,
	|	Себестоимость.ЗаказКлиента,
	|	Себестоимость.АналитикаУчетаПоПартнерам
	|;
	...
	|	ТаблицаКорректировки.ХозяйственнаяОперация      КАК ХозяйственнаяОперация,
    |	ТаблицаКорректировки.НалоговоеНазначениеСписания КАК НалоговоеНазначениеСписания,
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.СтоимостьРеглБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.СтоимостьРеглБезНДС, 0) КАК ЧИСЛО(31,2))) КАК СтоимостьРеглБезНДС,
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.НДСРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.НДСРегл, 0) КАК ЧИСЛО(31,2))) КАК НДСРегл,
	|
	|	ЕСТЬNULL(
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.СтоимостьЗабалансовая / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.Трудозатраты / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.ПостатейныеПостоянныеСНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.ПостатейныеПеременныеСНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.ПостатейныеПостоянныеБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.ПостатейныеПеременныеБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	...
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
	|			* Себестоимость.СтоимостьЗабалансоваяРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
	|		ВЫРАЗИТЬ(
	|			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ДопРасходыРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ТрудозатратыРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПостоянныеРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПеременныеРегл / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ДопРасходыРеглБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ДопРасходыРеглБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ДопРасходыРеглБезНДС,
    |
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПостоянныеРеглБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеРеглБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ПостатейныеПостоянныеРеглБезНДС,
    |
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПеременныеРеглБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеРеглБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ПостатейныеПеременныеРеглБезНДС,
	|
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.СтоимостьУпр / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ДопРасходыУпр / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ТрудозатратыУпр / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПостоянныеУпр / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПеременныеУпр / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    ...
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.СтоимостьУпрБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.СтоимостьУпрБезНДС, 0) КАК ЧИСЛО(31,2))) КАК СтоимостьУпрБезНДС,
    |
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ДопРасходыУпрБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ДопРасходыУпрБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ДопРасходыУпрБезНДС,
    |
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПостоянныеУпрБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеУпрБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ПостатейныеПостоянныеУпрБезНДС,
    |
    |	ЕСТЬNULL(
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* Себестоимость.ПостатейныеПеременныеУпрБезНДС / Себестоимость.Количество КАК ЧИСЛО(31,2)),
    |		ВЫРАЗИТЬ(
    |			ТаблицаКорректировки.Количество
    |			* ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеУпрБезНДС, 0) КАК ЧИСЛО(31,2))) КАК ПостатейныеПеременныеУпрБезНДС,
	|
    ...
    |		И Себестоимость.НалоговоеНазначение = ТаблицаКорректировки.НалоговоеНазначение
...
Процедура СписатьОшибкиОкругленияРасчетаСебестоимости(ПараметрыРасчета)
    ...
    |	Себестоимость.ДопРасходыРеглБезНДС            КАК ДопРасходыРеглБезНДС,
    |	Себестоимость.ПостатейныеПостоянныеРеглБезНДС КАК ПостатейныеПостоянныеРеглБезНДС,
    |	Себестоимость.ПостатейныеПеременныеРеглБезНДС КАК ПостатейныеПеременныеРеглБезНДС,
    ...
    |	,Себестоимость.СтоимостьУпрБезНДС     			КАК СтоимостьУпрБезНДС,
    |	Себестоимость.ДопРасходыУпрБезНДС     			КАК ДопРасходыУпрБезНДС,
    |	Себестоимость.ПостатейныеПостоянныеУпрБезНДС    КАК ПостатейныеПостоянныеУпрБезНДС,
    |	Себестоимость.ПостатейныеПеременныеУпрБезНДС    КАК ПостатейныеПеременныеУпрБезНДС
	...
	|		И Себестоимость.СтоимостьРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И &ЗначениеПогрешностиСебестоимостьРегл
	|		И Себестоимость.НДСРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И &ЗначениеПогрешностиСебестоимостьРегл
    ...
    |		И Себестоимость.ДопРасходыРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И &ЗначениеПогрешностиСебестоимостьРегл
    |		И Себестоимость.ПостатейныеПостоянныеРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И &ЗначениеПогрешностиСебестоимостьРегл
    |		И Себестоимость.ПостатейныеПеременныеРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И &ЗначениеПогрешностиСебестоимостьРегл
    |		И Себестоимость.СтоимостьУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И &ЗначениеПогрешностиСебестоимостьУпр
    |		И Себестоимость.ДопРасходыУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И &ЗначениеПогрешностиСебестоимостьУпр
    |		И Себестоимость.ПостатейныеПостоянныеУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И &ЗначениеПогрешностиСебестоимостьУпр
    |		И Себестоимость.ПостатейныеПеременныеУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И &ЗначениеПогрешностиСебестоимостьУпр
    |		И Себестоимость.СтоимостьУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И &ЗначениеПогрешностиСебестоимостьУпр
	...
	|ГДЕ
	|	Себестоимость.Количество > 0
	|	И (Себестоимость.Стоимость МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.СтоимостьБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ДопРасходы МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ДопРасходыБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.СтоимостьРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|		И Себестоимость.СтоимостьРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|		И Себестоимость.НДСРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|		И Себестоимость.СтоимостьУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.СтоимостьЗабалансовая МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.Трудозатраты МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ПостатейныеПостоянныеСНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ПостатейныеПостоянныеБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ПостатейныеПеременныеСНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.ПостатейныеПеременныеБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|		И Себестоимость.СтоимостьЗабалансоваяРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ДопРасходыРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ТрудозатратыРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ПостатейныеПостоянныеРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ПостатейныеПеременныеРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ДопРасходыРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ПостатейныеПостоянныеРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.ПостатейныеПеременныеРеглБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
    |		И Себестоимость.СтоимостьУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ДопРасходыУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ПостатейныеПостоянныеУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ПостатейныеПеременныеУпрБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.СтоимостьУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ДопРасходыУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ТрудозатратыУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ПостатейныеПостоянныеУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
    |		И Себестоимость.ПостатейныеПеременныеУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ ПЕРВЫЕ 0
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьДвиженияПоРегиструПоДаннымЗапроса(
		ПараметрыРасчета,
		РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияСебестоимостьТоваров(),
		Запрос,
		РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияСебестоимостьТоваров());
    ...
    |	СУММА(Т.СуммаРеглБезНДС) 														КАК СуммаРеглБезНДС,
    |	СУММА(Т.НДСРегл) 																КАК НДСРегл,
    |	СУММА(Т.СуммаУпр) 																КАК СуммаУпр
    |	,СУММА(Т.СуммаУпрБезНДС) 														КАК СуммаУпрБезНДС
    ...
    |			+ Т.ПостатейныеПеременныеРеглБезНДС КАК СуммаРеглБезНДС,
	|		Т.НДСРегл КАК НДСРегл,
    |		Т.СтоимостьУпр
    |			+ Т.ДопРасходыУпр
    |			+ Т.ТрудозатратыУпр
    ...
    |			+ Т.ПостатейныеПеременныеУпрБезНДС КАК СуммаУпрБезНДС
	|	ИЗ
	|		ВТОкруглениеСебестоимостьТоваров КАК Т
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
    ...
    |	ИЛИ СУММА(Т.СуммаРеглБезНДС) <> 0
    |	ИЛИ СУММА(Т.НДСРегл) <> 0
    |	ИЛИ СУММА(Т.СуммаУпр) <> 0
    |	ИЛИ СУММА(Т.СуммаУпрБезНДС) <> 0
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьДвиженияПоРегиструПоДаннымЗапроса(
		ПараметрыРасчета,
		РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеРасходы(),
		Запрос,
		РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеРасходы());
    ...
    |		-Т.СуммаРеглБезНДС 											КАК СтоимостьРеглБезНДС,
    |		-Т.НДСРегл 												    КАК НДСРегл,
    |		-Т.СуммаУпр 												КАК СтоимостьУпр
    |		,-Т.СуммаУпрБезНДС 											КАК СтоимостьУпрБезНДС
	|	ИЗ
	|		ВТОкруглениеПрочиеРасходы КАК Т
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
    ...
    |		ИЛИ СУММА(Т.СтоимостьРеглБезНДС) <> 0
    |		ИЛИ СУММА(Т.НДСРегл) <> 0
    |		ИЛИ СУММА(Т.СтоимостьУпрБезНДС) <> 0
	|		ИЛИ СУММА(Т.СтоимостьУпр) <> 0)";
	
	Выборка = РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина,,, НСтр("ru='Регистры упр. учета';uk='Регістри упр. обліку'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура РаспределитьВыручкуПоНаправлениямДеятельности(ПараметрыРасчета) Экспорт
	...
	|	СУММА(ВыручкаИСебестоимость.СуммаВыручкиРегл)		     КАК СуммаВыручкиРеглБезНДС,
	|	СУММА(ВыручкаИСебестоимость.СуммаВыручкиБезНДС)			 КАК СуммаВыручкиУпрБезНДС,
	|
	...
	|
	|
	|	СУММА(ВыручкаИСебестоимость.СтоимостьРеглБезНДС
	|		+ ВыручкаИСебестоимость.ДопРасходыРеглБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПостоянныеРеглБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПеременныеРеглБезНДС
	|		+ ВыручкаИСебестоимость.ТрудозатратыРегл) КАК СебестоимостьРеглБезНДС,
	|
	|
	|	СУММА(ВыручкаИСебестоимость.СтоимостьУпрБезНДС
	|		+ ВыручкаИСебестоимость.ДопРасходыУпрБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПостоянныеУпрБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПеременныеУпрБезНДС
	|		+ ВыручкаИСебестоимость.ТрудозатратыУпр) КАК СебестоимостьУпрБезНДС
	|
	...
	|	ИЛИ СУММА(ВыручкаИСебестоимость.СуммаВыручкиРегл) <> 0
	|	ИЛИ СУММА(ВыручкаИСебестоимость.СуммаВыручкиБезНДС)	<> 0
	|	ИЛИ СУММА(ВыручкаИСебестоимость.Стоимость
	|		+ ВыручкаИСебестоимость.ДопРасходы
	|		+ ВыручкаИСебестоимость.ПостатейныеПостоянныеСНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПеременныеСНДС
	|		+ ВыручкаИСебестоимость.Трудозатраты) <> 0
	|	ИЛИ СУММА(ВыручкаИСебестоимость.СтоимостьРеглБезНДС
	|		+ ВыручкаИСебестоимость.ДопРасходыРеглБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПостоянныеРеглБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПеременныеРеглБезНДС
	|		+ ВыручкаИСебестоимость.ТрудозатратыРегл) <> 0
	|	ИЛИ СУММА(ВыручкаИСебестоимость.СтоимостьУпрБезНДС
	|		+ ВыручкаИСебестоимость.ДопРасходыУпрБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПостоянныеУпрБезНДС
	|		+ ВыручкаИСебестоимость.ПостатейныеПеременныеУпрБезНДС
	|		+ ВыручкаИСебестоимость.ТрудозатратыУпр) <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	СУММА(ПрочаяВыручка.ВыручкаБезНДСРегл)		     			 КАК СуммаВыручкиРеглБезНДС,
	|	СУММА(ПрочаяВыручка.ВыручкаБезНДСУпр)			             КАК СуммаВыручкиУпрБезНДС,
	|	0                                                            КАК Себестоимость,
	|	0                                                            КАК СебестоимостьРеглБезНДС,
	|	0                                                            КАК СебестоимостьУпрБезНДС
	|ИЗ
	...
	|	ИЛИ СУММА(ПрочаяВыручка.ВыручкаБезНДСРегл) <> 0
	|	ИЛИ СУММА(ПрочаяВыручка.ВыручкаБезНДСУпр) <> 0
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
	...
	|	ТаблицаВыручки.СуммаВыручкиРеглБезНДС      КАК СуммаВыручкиРеглБезНДС,
	|	ТаблицаВыручки.СуммаВыручкиУпрБезНДС       КАК СуммаВыручкиУпрБезНДС,
	|	ТаблицаВыручки.Себестоимость               КАК Себестоимость,
	|	ТаблицаВыручки.СебестоимостьРеглБезНДС     КАК СебестоимостьРеглБезНДС,
	|	ТаблицаВыручки.СебестоимостьУпрБезНДС      КАК СебестоимостьУпрБезНДС,
	...
	|	СУММА(ТаблицаФинансовыхРезультатов.СуммаВыручкиРеглБезНДС)           КАК СуммаВыручкиРеглБезНДС,
	|	СУММА(ТаблицаФинансовыхРезультатов.СуммаВыручкиУпрБезНДС)            КАК СуммаВыручкиУпрБезНДС,
	|	СУММА(ТаблицаФинансовыхРезультатов.Себестоимость)                    КАК Себестоимость,
	|	СУММА(ТаблицаФинансовыхРезультатов.СебестоимостьРеглБезНДС)          КАК СебестоимостьРеглБезНДС,
	|	СУММА(ТаблицаФинансовыхРезультатов.СебестоимостьУпрБезНДС)           КАК СебестоимостьУпрБезНДС,
	...
	|		СУММА(ТаблицаРаспределения.СуммаВыручкиРеглБезНДС) КАК СуммаВыручкиРеглБезНДС,
	|		СУММА(ТаблицаРаспределения.СуммаВыручкиУпрБезНДС)  КАК СуммаВыручкиУпрБезНДС,
	|		СУММА(ТаблицаРаспределения.Себестоимость)          КАК Себестоимость,
	|		СУММА(ТаблицаРаспределения.СебестоимостьРеглБезНДС) КАК СебестоимостьРеглБезНДС,
	|		СУММА(ТаблицаРаспределения.СебестоимостьУпрБезНДС) КАК СебестоимостьУпрБезНДС,
	...
	|	МАКСИМУМ(СуммаВыручкиРеглБезНДС),
	|	МАКСИМУМ(СуммаВыручкиУпрБезНДС),
	|	МАКСИМУМ(Себестоимость),
	|	МАКСИМУМ(СебестоимостьРеглБезНДС),
	|	МАКСИМУМ(СебестоимостьУпрБезНДС),
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияФинансовыеРезультаты();
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			Пока ВыборкаСпособыРаспределения.Следующий() Цикл
				...
				РасходыКРаспределениюРеглБезНДС	= ВыборкаСпособыРаспределения.СебестоимостьРеглБезНДС;
				РасходыКРаспределениюУпрБезНДС 	= ВыборкаСпособыРаспределения.СебестоимостьУпрБезНДС;
				ДоходыКРаспределению  = ВыборкаСпособыРаспределения.СуммаВыручки;
				ДоходыКРаспределениюРеглБезНДС	= ВыборкаСпособыРаспределения.СуммаВыручкиРеглБезНДС;
				ДоходыКРаспределениюУпрБезНДС  	= ВыборкаСпособыРаспределения.СуммаВыручкиУпрБезНДС;
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если ВсегоДолей <> 0 Тогда
						...
						РасходыРеглБезНДС	= Окр(РасходыКРаспределениюРеглБезНДС * ВыборкаДетали.КоэффициентРаспределения / ВсегоДолей,
										2, РежимОкругления.Окр15как20);
						РасходыУпрБезНДС 	= Окр(РасходыКРаспределениюУпрБезНДС * ВыборкаДетали.КоэффициентРаспределения / ВсегоДолей,
										2, РежимОкругления.Окр15как20);
					Иначе
						Расходы = 0;
						РасходыРеглБезНДС	= 0;
					...
					Если ВсегоДолей <> 0 Тогда
						Доходы = Окр(ДоходыКРаспределению * ВыборкаДетали.КоэффициентРаспределения / ВсегоДолей,
										2, РежимОкругления.Окр15как20);
						ДоходыРеглБезНДС	= Окр(ДоходыКРаспределениюРеглБезНДС * ВыборкаДетали.КоэффициентРаспределения / ВсегоДолей,
										2, РежимОкругления.Окр15как20);
						ДоходыУпрБезНДС 	= Окр(ДоходыКРаспределениюУпрБезНДС * ВыборкаДетали.КоэффициентРаспределения / ВсегоДолей,
										2, РежимОкругления.Окр15как20);
					Иначе
						Доходы = 0;
						ДоходыРеглБезНДС	= 0;
					...
					РасходыКРаспределениюРеглБезНДС	= РасходыКРаспределениюРеглБезНДС - РасходыРеглБезНДС;
					РасходыКРаспределениюУпрБезНДС 	= РасходыКРаспределениюУпрБезНДС - РасходыУпрБезНДС;
					ДоходыКРаспределению  = ДоходыКРаспределению  - Доходы;
					ДоходыКРаспределениюРеглБезНДС	= ДоходыКРаспределениюРеглБезНДС  - ДоходыРеглБезНДС;
					ДоходыКРаспределениюУпрБезНДС  	= ДоходыКРаспределениюУпрБезНДС  - ДоходыУпрБезНДС;
					...
					Если Доходы <> 0 ИЛИ ДоходыРеглБезНДС <> 0 ИЛИ ДоходыУпрБезНДС <> 0 Тогда
						ДанныеДвижения.Вставить("Доходы", 		 Доходы);
						ДанныеДвижения.Вставить("ДоходыРеглБезНДС",	ДоходыРеглБезНДС);
						ДанныеДвижения.Вставить("ДоходыУпрБезНДС", 	ДоходыУпрБезНДС);
						ДанныеДвижения.Вставить("СтатьяДоходов", ВыборкаДетали.СтатьяДоходов);
					КонецЕсли;

					Если Расходы <> 0 ИЛИ РасходыРеглБезНДС <> 0 ИЛИ РасходыУпрБезНДС <> 0 Тогда
						ДанныеДвижения.Вставить("Расходы", 		  Расходы);
						ДанныеДвижения.Вставить("РасходыРеглБезНДС",	РасходыРеглБезНДС);
						ДанныеДвижения.Вставить("РасходыУпрБезНДС", 	РасходыУпрБезНДС);
...
Процедура ОтразитьПрибылиИУбытки(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьДвиженияПоРегиструПоДаннымЗапроса(
		ПараметрыРасчета,
		РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияПрочиеАктивыПассивы(),
		Запрос);
	
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		"ВТПромежуточнаяТаблица1, ВТПромежуточнаяТаблица2, ВТПромежуточнаяТаблица3, ВТПромежуточнаяТаблица4, ВТПромежуточнаяТаблица5");
...
Функция ТекстЗапросаТаблицаРасходов()
    ...
    |	СУММА(ТаблицаРасходы.СуммаУпрБезНДС) КАК СуммаУпрБезНДС,
...
Функция ТекстЗапросаДвиженияСтоимости(ПараметрыРасчета)
	...
	|	ВЫРАЗИТЬ(УзлыКорректировки.НДСРегл КАК ЧИСЛО(28,10))                        КАК НДСРегл,
    ...
    |			И УчетСебестоимости.НалоговоеНазначение         = УзлыКорректировкиИсточник.НалоговоеНазначение
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ
    ...
    |			И (УчетСебестоимости.КорНалоговоеНазначение        = УзлыКорректировкиПриемник.НалоговоеНазначение
    |				ИЛИ НЕ &ПартионныйУчетВерсии22)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|           	ИЛИ (УчетСебестоимости.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) 
	|					И УзлыКорректировкиПриемникПередачи.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
	|           )
	...
	|				ИЛИ НЕ &ПартионныйУчетВерсии22)
	|			И (
	|			    (УчетСебестоимости.КорНалоговоеНазначение = УзлыКорректировкиПриемникПередачи.НалоговоеНазначение И УчетСебестоимости.КорОрганизация = УчетСебестоимости.Организация)
	|			    ИЛИ (УчетСебестоимости.КорВидЗапасов.НалоговоеНазначение = УзлыКорректировкиПриемникПередачи.НалоговоеНазначение И УчетСебестоимости.КорОрганизация <> УчетСебестоимости.Организация)
    |				ИЛИ НЕ &ПартионныйУчетВерсии22)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|				ИЛИ НЕ &ПартионныйУчетВерсии22)
    |			И (УчетСебестоимости.КорНалоговоеНазначение 		 = УзлыКорректировкиПриемникРеглУчет.НалоговоеНазначение
    |				ИЛИ НЕ &ПартионныйУчетВерсии22)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЕстьВозвратныеОтходы КАК ЕстьВозвратныеОтходы
	|			ПО ЕстьВозвратныеОтходы.Организация = УчетСебестоимости.Организация
...
// Служебная, этап 3.11у
Функция ТекстЗапросаДвиженияСтоимостиУпр(ПараметрыРасчета)
...
Процедура СформироватьДвиженияПоОборотнымРегистрамУпрУчета(ПараметрыРасчета, ДанныеДвижения) Экспорт
	...
	Если ДанныеДвижения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПересортицаТоваровСПереоценкой Тогда
		...
		Если ДанныеСумм.ПолнаяСуммаПереоценки > 0 Тогда 
            ...
            СкорректированныеСуммы = СкорректироватьСуммовыеПоказателиДвиженияНоменклатуры(ДанныеСумм, 1, 1, 1, 1, 1, 1, 1);
...
Функция ПолучитьСуммовыеПоказателиДвиженияНоменклатуры(ДанныеЗаполнения)
    ...
    ДанныеСумм.Вставить("СтоимостьРеглБезНДС", 		  	0);
    ...
    ДанныеСумм.Вставить("ДопРасходыРеглБезНДС", 		0);
    ДанныеСумм.Вставить("ПостатейныеПостоянныеРеглБезНДС",0);
    ДанныеСумм.Вставить("ПостатейныеПеременныеРеглБезНДС",0);
    ...
    ДанныеСумм.Вставить("СтоимостьУпрБезНДС", 		  	0);
    ДанныеСумм.Вставить("ДопРасходыУпрБезНДС", 		  	0);
    ДанныеСумм.Вставить("ПостатейныеПостоянныеУпрБезНДС",0);
    ДанныеСумм.Вставить("ПостатейныеПеременныеУпрБезНДС",0);
    ...
    ДанныеСумм.Вставить("НДСРеглРеквизит", 			    0);
...
Функция СкорректироватьСуммовыеПоказателиДвиженияНоменклатуры(ДанныеСумм,
            КоэффициентСуммыСНДС, КоэффициентСуммыБезНДС, КоэффициентСуммыРегл, КоэффициентСуммыРеглБезНДС, КоэффициентСуммыУпр, КоэффициентСуммыУпрБезНДС, КоэффициентКорСуммы)
	...
	Для Каждого КлючИЗначение Из ДанныеСумм Цикл
        ...
        ИначеЕсли ИменаПолейРеглБезНДС.Свойство(КлючИЗначение.Ключ) Тогда
        	
        	СкорректированныеСуммы.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение * КоэффициентСуммыРеглБезНДС);
        ...
        ИначеЕсли ИменаПолейУпрБезНДС.Свойство(КлючИЗначение.Ключ) Тогда
        	
        	СкорректированныеСуммы.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение * КоэффициентСуммыУпрБезНДС);
...
Процедура ЗаполнитьИтоговыеСуммовыеПоказателиДвижения(ДанныеСумм)
    ...
    ДанныеСумм.Вставить("ПолнаяСуммаРеглБезНДС",
    	ДанныеСумм.СтоимостьРеглБезНДС + ДанныеСумм.ДопРасходыРеглБезНДС + ДанныеСумм.ТрудозатратыРегл
    	+ ДанныеСумм.ПостатейныеПостоянныеРеглБезНДС + ДанныеСумм.ПостатейныеПеременныеРеглБезНДС);
    ...
    ДанныеСумм.Вставить("ПолнаяСуммаУпрБезНДС",
    	ДанныеСумм.СтоимостьУпрБезНДС + ДанныеСумм.ДопРасходыУпрБезНДС + ДанныеСумм.ТрудозатратыУпр
    	+ ДанныеСумм.ПостатейныеПостоянныеУпрБезНДС + ДанныеСумм.ПостатейныеПеременныеУпрБезНДС);
	...
	ДанныеСумм.Вставить("ЕстьНенулеваяПолнаяСумма",
        ...
        ИЛИ ДанныеСумм.ПолнаяСуммаРеглБезНДС <> 0
        ИЛИ ДанныеСумм.ПолнаяСуммаУпрБезНДС <> 0
...
Процедура СформироватьДвиженияСебестоимостьТоваров(ПараметрыРасчета, ДанныеДвижения, ВидДвижения)
    ...
    |Партия, АналитикаУчетаПартий, АналитикаФинансовогоУчета, НалоговоеНазначение,
    |КорПартия, КорАналитикаУчетаПартий, КорАналитикаФинансовогоУчета, КорНалоговоеНазначение,
    |НДСРеглРеквизит, СтатьяКалькуляции, ТипЗаписи, ДокументИсточник,
	|Трудозатраты, ПостатейныеПостоянныеСНДС, ПостатейныеПостоянныеБезНДС,
    |ДопРасходыРегл, ТрудозатратыРегл, ПостатейныеПостоянныеРегл,
    |ДопРасходыРеглБезНДС, ПостатейныеПостоянныеРеглБезНДС, ПостатейныеПеременныеРеглБезНДС,
	|ПостатейныеПеременныеСНДС, ПостатейныеПеременныеБезНДС, ПостатейныеПеременныеРегл,
...
Процедура СформироватьКорДвиженияСебестоимостьТоваров(ПараметрыРасчета, ДанныеДвижения, ПромежуточнаяСебестоимость = Ложь) Экспорт
	...
	Иначе
		ТаблицаПриемник = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияСебестоимостьТоваров();
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Запись.ХозяйственнаяОперация) Тогда
	...
	Если ДанныеДвижения.ЭтоПередачаМеждуОрганизациями Тогда
        ...
        Запись.ДопРасходыРеглБезНДС = 0;
        Запись.ПостатейныеПостоянныеРеглБезНДС = 0;
        Запись.ПостатейныеПеременныеРеглБезНДС = 0;
        ...
        Запись.СтоимостьУпрБезНДС = 0;
        Запись.ДопРасходыУпрБезНДС = 0;
        Запись.ПостатейныеПостоянныеУпрБезНДС = 0;
        Запись.ПостатейныеПеременныеУпрБезНДС = 0;
		
    ИначеЕсли ДанныеДвижения.НДСРеглРеквизит <> 0 ИЛИ ДанныеДвижения.НДСУпр <> 0 Тогда // это не передача
        ...
        Запись.СтоимостьРегл 			  	 = ДанныеДвижения.НДСРеглРеквизит;
...
Процедура СформироватьДвиженияПрочиеРасходы(ПараметрыРасчета, ДанныеДвижения, СуммовыеПоказатели) Экспорт
    ...
    СуммаРеглБезНДС = СуммовыеПоказатели.СуммаРеглБезНДС;
    НДСРегл = СуммовыеПоказатели.НДСРегл;
    СуммаУпр = СуммовыеПоказатели.СуммаУпр;
    СуммаУпрБезНДС = СуммовыеПоказатели.СуммаУпрБезНДС;
    ...
    Если ДанныеДвижения.ВариантРаспределенияРасходовРегл = Перечисления.ВариантыРаспределенияРасходов.НаПрочиеАктивы
     	ИЛИ ДанныеДвижения.ВариантРаспределенияРасходовРегл = Перечисления.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы Тогда
	...
	Если НЕ ДанныеДвижения.ВариантРаспределенияРасходовУпр = Перечисления.ВариантыРаспределенияРасходов.НаПрочиеАктивы Тогда
		...
		Иначе
            ...
            Запись.СуммаУпрБезНДС = Знак * СуммаУпрБезНДС;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
            ...
            Запись.СуммаРеглБезНДС = Знак * СуммаРеглБезНДС;
            Запись.НДСРегл = Знак * НДСРегл;
            Запись.ПостояннаяРазница = Знак * ПостояннаяРазница;
			Запись.ВременнаяРазница  = Знак * ВременнаяРазница;
		КонецЕсли;
    КонецЕсли;
    
    Если Запись.НДСРегл <> 0 И ЗначениеЗаполнено(Запись.НалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая Тогда
	...
	Если ЗначениеЗаполнено(ДанныеДвижения.КорОрганизация)
	 ...
	 И ДанныеДвижения.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию Тогда
        ...
        ЗаписьУпр.СуммаУпрБезНДС = -Запись.СуммаУпрБезНДС;
...
Процедура СформироватьДвиженияПрочиеДоходы(ПараметрыРасчета, ДанныеДвижения,
			Сумма, СуммаРегл, СуммаУпр, СуммаРеглБезНДС, СуммаУпрБезНДС) Экспорт
	...
	Запись.СуммаУпрБезНДС = СуммаУпрБезНДС;
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходовРегл Тогда
...
Процедура СформироватьДвиженияВыручкаИСебестоимостьПродаж(ПараметрыРасчета, ДанныеДвижения) Экспорт
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияВыручкаИСебестоимостьПродаж();
    ...
    Если ДанныеДвижения.НДСРеглРеквизит <> 0 Тогда
...
Процедура СформироватьДвиженияЗакупки(ПараметрыРасчета, ДанныеДвижения)
	...
	ИмяРегистра = РасчетСебестоимостиПовтИсп.ИмяРегистраНакопленияЗакупки();
...
Функция ТаблицаРаспределенияРасходовНаСебестоимостьТоваров(ПараметрыРасчета)
	...
	|			(ВЫБОР
    |				КОГДА ПрочиеРасходы.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
    ...
    |		КОГДА ПрочиеРасходы.СтатьяРасходов.ВариантРаспределенияРасходовУпр
    |				= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
    |		 И ПрочиеРасходы.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
	...
	// МассивРезультатов[0] - временная таблица "ТаблицаСпособовРаспределения"
	// МассивРезультатов[1] - временная таблица "ТаблицаДокументов"
	// МассивРезультатов[2] - временная таблица "ВтПоступленияТоваров"
	// МассивРезультатов[3] - временная таблица "ТаблицаПоступленияТоваров"
	РезультатЗапросаПоБазе = МассивРезультатов[4]; // РезультатЗапроса
	ТаблицаРасходов = МассивРезультатов[5].Выгрузить();
    ...
    Колонки.Добавить("ДопРасходыРеглБезНДС", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
    Колонки.Добавить("ДопРасходыУпрБезНДС",  ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	Колонки.Добавить("НДСРегл",              ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	...
	Пока ВыборкаПоИндексу.Следующий() Цикл
		...
		Для Каждого СтрокаРасходов Из МассивРасходов Цикл
            ...
            СуммаУпрБезНДСКРаспределению	= СтрокаРасходов.СуммаУпрБезНДС;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если БазаВсего <> 0 Тогда
                    ...
                    СтоимостьУпрБезНДС 	= Окр(СуммаУпрБезНДСКРаспределению * Выборка.База / БазаВсего, 2, 1);
				...
				СуммаУпрКРаспределению = СуммаУпрКРаспределению - СтоимостьУпр;
                СуммаУпрБезНДСКРаспределению = СуммаУпрБезНДСКРаспределению - СтоимостьУпрБезНДС;
                ...
                НоваяСтрока.ДопРасходыРеглБезНДС       = СтоимостьРеглБезНДС;
                НоваяСтрока.ДопРасходыУпрБезНДС        = СтоимостьУпрБезНДС;
...
Процедура ПодготовитьДанныеДляРасчетаСтоимостиПоФИФО(ПараметрыРасчета, Запрос)
    ...
    |	КонечныеОстатки.НалоговоеНазначение,
	|
	|	(ВЫБОР
    ...
    |		И КонечныеОстатки.НалоговоеНазначение = НачальныеОстатки.НалоговоеНазначение
    ...
    |		И КонечныеОстатки.НалоговоеНазначение = Приходы.НалоговоеНазначение
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
    ...
    |	СебестоимостьТоваров.НалоговоеНазначение  				   КАК НалоговоеНазначение,
    ...
    |				И СебестоимостьТоваров.НалоговоеНазначение  		 = ТаблицаОстатков.НалоговоеНазначение
    ...
    |	СебестоимостьТоваров.НалоговоеНазначение 
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
    ...
    |		ПериодыПартий.НалоговоеНазначение  				    КАК НалоговоеНазначение,
    ...
    |				И ПериодыПартий.НалоговоеНазначение  		  = Себестоимость.НалоговоеНазначение
    ...
    |			И ТаблицаОстатков.НалоговоеНазначение  		= ТаблицаВнешнихПоступлений.НалоговоеНазначение
	|			И ТаблицаОстатков.КоличествоОстаток <> 0
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
    ...
    |	ТаблицаНовыхОстатков.НалоговоеНазначение,
	|
    ...
    |	ТаблицаУзлыКорректировки.НалоговоеНазначение  		КАК НалоговоеНазначение,
    ...
    |			И ТаблицаУзлыКорректировки.НалоговоеНазначение  		 = ТаблицаОстатков.НалоговоеНазначение
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
...
Процедура ПодготовитьДанныеДляРасчетаСтоимостиРеглПоФИФО(ПараметрыРасчета, Запрос)
    ...
    |	Себестоимость.ДопРасходыРеглБезНДС КАК ДопРасходыРеглБезНДСОстаток,
    |	Себестоимость.НДСРегл КАК НДСРеглОстаток,
    |	Себестоимость.ПостатейныеПостоянныеРеглБезНДС КАК ПостатейныеПостоянныеРеглБезНДСОстаток,
    |	Себестоимость.ПостатейныеПеременныеРеглБезНДС КАК ПостатейныеПеременныеРеглБезНДСОстаток
	|ПОМЕСТИТЬ ВТКонечныеОстатки
	|ИЗ
	|	ВТКэшРасчетныеОстаткиСебестоимостьТоваров КАК Себестоимость
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикаНоменклатуры КАК Таблица
    ...
    |		ИНАЧЕ 0
    |	КОНЕЦ) КАК СтоимостьДопРасходыРегл,
    |	СУММА(ВЫБОР
    |		КОГДА 
    |		 НЕ Себестоимость.ХозяйственнаяОперация В (&МассивОперацийВозвратыПрошлыхПериодов)
    |		 И Себестоимость.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции) ТОГДА
    ...
    |		 И Себестоимость.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции) ТОГДА
    |			ВЫБОР
    |				КОГДА &ПартионныйУчетВерсии22 ТОГДА Себестоимость.ДопРасходыРеглБезНДС
    |				ИНАЧЕ Себестоимость.СтоимостьРеглБезНДС КОНЕЦ
    |		ИНАЧЕ 0
    |	КОНЕЦ) КАК СтоимостьДопРасходыРеглБезНДС
	|ПОМЕСТИТЬ ВТПриходы
	|ИЗ
	|	ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК Себестоимость
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикаНоменклатуры КАК Таблица
	...
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * ЕСТЬNULL(Приходы.СтоимостьДопРасходыРегл, 0) /
	|					(ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК СтоимостьДопРасходыРеглОстаток,
	|	(ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22 
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
	|			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ДопРасходыРеглБезНДСОстаток /
	|					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
	|		КОГДА НЕ &ПартионныйУчетВерсии22 
	|		 И ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * ЕСТЬNULL(Приходы.СтоимостьДопРасходыРеглБезНДС, 0) /
	|					(ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ДопРасходыРеглБезНДСОстаток,
	|	(ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22 
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
	|			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.НДСРеглОстаток /
	|					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
	|		КОГДА НЕ &ПартионныйУчетВерсии22 
	|		 И ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * ЕСТЬNULL(Приходы.НДСРегл, 0) /
	|					(ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0))
    ...
    |		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
    |		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ПостатейныеПостоянныеРеглБезНДСОстаток /
    |					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ПостатейныеПостоянныеРеглБезНДСОстаток,
    |	(ВЫБОР
    |		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
    |		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ПостатейныеПеременныеРеглБезНДСОстаток /
    |					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ПостатейныеПеременныеРеглБезНДСОстаток
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ
	|	ВТКонечныеОстатки КАК КонечныеОстатки
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНачальныеОстатки КАК НачальныеОстатки
    ...
    |	И (СебестоимостьТоваров.СтоимостьРегл <> 0 ИЛИ СебестоимостьТоваров.СтоимостьРеглБезНДС <> 0)
	|	И СебестоимостьТоваров.Период < ТаблицаОстатков.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	СебестоимостьТоваров.АналитикаУчетаНоменклатуры,
    ...
    |	ПериодыПартий.НалоговоеНазначение,
    ...
    |
	|	ТаблицаОстатков.НДСРеглОстаток КАК НоваяНДСРеглОстаток,
    |
	...
	|
    |	ТаблицаОстатков.СтоимостьДопРасходыРеглОстаток КАК НоваяСтоимостьДопРасходыРеглОстаток,
	...
	|
    |	ТаблицаОстатков.ДопРасходыРеглБезНДСОстаток КАК НоваяДопРасходыРеглБезНДСОстаток,
    |	ТаблицаОстатков.ПостатейныеПостоянныеРеглБезНДСОстаток КАК НоваяПостатейныеПостоянныеРеглБезНДСОстаток,
    |	ТаблицаОстатков.ПостатейныеПеременныеРеглБезНДСОстаток КАК НоваяПостатейныеПеременныеРеглБезНДСОстаток
	|
	|ПОМЕСТИТЬ ТаблицаНовыхОстатков
	|
	|ИЗ
    ...
    |	ТаблицаНовыхОстатков.НоваяСтоимостьДопРасходыРеглОстаток 		КАК СтоимостьДопРасходыРеглОстаток,
    ...
    |	ТаблицаНовыхОстатков.НоваяДопРасходыРеглБезНДСОстаток 			 КАК ДопРасходыРеглБезНДСОстаток,
    |	ТаблицаНовыхОстатков.НоваяПостатейныеПостоянныеРеглБезНДСОстаток КАК ПостатейныеПостоянныеРеглБезНДСОстаток,
    |	ТаблицаНовыхОстатков.НоваяПостатейныеПеременныеРеглБезНДСОстаток КАК ПостатейныеПеременныеРеглБезНДСОстаток
	|
	|ПОМЕСТИТЬ ТаблицаОстатков
	|
	|ИЗ
    ...
    |		- ЕСТЬNULL(ТаблицаОстатков.СтоимостьДопРасходыРеглОстаток, 0))
	|		/(ТаблицаУзлыКорректировки.ВесГраф_1
	|		- ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ) КАК СтоимостьГраф_1,
	|
	...
	|	ТОГДА
	|		0
    |	КОГДА (ТаблицаУзлыКорректировки.СтоимостьГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2 - ЕСТЬNULL(ТаблицаОстатков.СтоимостьРеглБезНДСОстаток, 0)) = 0
	|		И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|		ТОГДА 0	
	|	ИНАЧЕ
    ...
    |		- ЕСТЬNULL(ТаблицаОстатков.ДопРасходыРеглБезНДСОстаток, 0))
	|		/(ТаблицаУзлыКорректировки.ВесГраф_2
	|		- ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ) КАК СтоимостьГраф_2,
	...
	|			ТОГДА 0
	|		КОГДА (ВЫРАЗИТЬ(ТаблицаУзлыКорректировки.СтоимостьГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2 КАК ЧИСЛО(15,3))
    |				- ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаОстатков.СтоимостьРеглБезНДСОстаток, 0) КАК ЧИСЛО(15,3))) = 0
	|			И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ДопРасходыГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ДопРасходыРеглБезНДСОстаток, 0))
	...
	|			И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.НДСРегл * ТаблицаУзлыКорректировки.ВесГраф_1
	...
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ПостатейныеПостоянныеГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ПостатейныеПостоянныеРеглБезНДСОстаток, 0))
	|			 	 /(ТаблицаУзлыКорректировки.ВесГраф_2
	|			 	 	 - ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ                                 КАК ПостатейныеПостоянныеГраф_2,
	...
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ПостатейныеПеременныеГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ПостатейныеПеременныеРеглБезНДСОстаток, 0))
	|			 	 /(ТаблицаУзлыКорректировки.ВесГраф_2
	|			 	 	 - ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ                                 КАК ПостатейныеПеременныеГраф_2
	|ПОМЕСТИТЬ ВтУзлыКорректировкиВременная
...
// Служебная, этап 3.10у
Процедура ПодготовитьДанныеДляРасчетаСтоимостиУпрПоФИФО(ПараметрыРасчета, Запрос)
    ...
    |
    |	Себестоимость.ДопРасходыУпрБезНДС КАК ДопРасходыУпрБезНДСОстаток,
    |	Себестоимость.ПостатейныеПостоянныеУпрБезНДС КАК ПостатейныеПостоянныеУпрБезНДСОстаток,
    |	Себестоимость.ПостатейныеПеременныеУпрБезНДС КАК ПостатейныеПеременныеУпрБезНДСОстаток
	|ПОМЕСТИТЬ ВТКонечныеОстатки
	|ИЗ
	|	ВТКэшРасчетныеОстаткиСебестоимостьТоваров КАК Себестоимость
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикаНоменклатуры КАК Таблица
    ...
    |		ИНАЧЕ 0
    |	КОНЕЦ) КАК СтоимостьДопРасходыУпр,
    |	СУММА(ВЫБОР
    |		КОГДА Себестоимость.Количество = 0
    |		 И НЕ Себестоимость.ХозяйственнаяОперация В (&МассивОперацийВозвратыПрошлыхПериодов)
    |		 И Себестоимость.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции) ТОГДА
    |			ВЫБОР
    |				КОГДА &ПартионныйУчетВерсии22 ТОГДА Себестоимость.ДопРасходыУпрБезНДС
    |				ИНАЧЕ Себестоимость.СтоимостьУпрБезНДС КОНЕЦ
    |		ИНАЧЕ 0
    |	КОНЕЦ) КАК СтоимостьДопРасходыУпрБезНДС
	|ПОМЕСТИТЬ ВТПриходы
	|ИЗ
	|	ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК Себестоимость
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикаНоменклатуры КАК Таблица
    ...
    |	0 КАК СтоимостьУпрБезНДСОстаток,
	|	(ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22 
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
	...
	|		 И ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
	|			ТОГДА КонечныеОстатки.КоличествоОстаток * ЕСТЬNULL(Приходы.СтоимостьДопРасходыУпр, 0) /
	|					(ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК СтоимостьДопРасходыУпрОстаток,
    |	(ВЫБОР
    |		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
    |		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ДопРасходыУпрБезНДСОстаток /
    |					(ЕСТЬNULL(Приходы.КоличествоПриход, 0) + ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ДопРасходыУпрБезНДСОстаток,
	|	(ВЫБОР
	|		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
	|		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    ...
    |		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
    |		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ПостатейныеПостоянныеУпрБезНДСОстаток /
    |					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ПостатейныеПостоянныеУпрБезНДСОстаток,
    |	(ВЫБОР
    |		КОГДА ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > КонечныеОстатки.КоличествоОстаток
    |		 И ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0) > 0
    |			ТОГДА КонечныеОстатки.КоличествоОстаток * КонечныеОстатки.ПостатейныеПеременныеУпрБезНДСОстаток /
    |					(ЕСТЬNULL(НачальныеОстатки.КоличествоОстаток, 0) + ЕСТЬNULL(Приходы.КоличествоПриход, 0))
    |		ИНАЧЕ 0 КОНЕЦ) КАК ПостатейныеПеременныеУпрБезНДСОстаток
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ
	|	ВТКонечныеОстатки КАК КонечныеОстатки
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНачальныеОстатки КАК НачальныеОстатки
	...
	|	СебестоимостьТоваров.СлужебноеВидДвиженияПриход
	|	И НЕ СебестоимостьТоваров.РасчетСебестоимости
    |	И (СебестоимостьТоваров.СтоимостьУпр <> 0 ИЛИ СебестоимостьТоваров.СтоимостьУпрБезНДС <> 0)
	|	И СебестоимостьТоваров.Период < ТаблицаОстатков.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	СебестоимостьТоваров.АналитикаУчетаНоменклатуры,
	...
	|	) КАК СтоимостьУпр,
    |	СУММА(
    |		ВЫБОР КОГДА ЕСТЬNULL(Себестоимость.Количество, 0) <> 0 ТОГДА ЕСТЬNULL(Себестоимость.СтоимостьУпрБезНДС, 0)
    |		ИНАЧЕ 0 КОНЕЦ
    ...
    |
    |
    |	ТаблицаОстатков.СтоимостьУпрБезНДСОстаток +
    |	ВЫБОР КОГДА ТаблицаОстатков.КоличествоОстаток >= ЕСТЬNULL(ТаблицаВнешнихПоступлений.Количество, 0) ТОГДА
    |		ЕСТЬNULL(ТаблицаВнешнихПоступлений.СтоимостьУпрБезНДС, 0)
    |	ИНАЧЕ
    |		ЕСТЬNULL(ТаблицаВнешнихПоступлений.СтоимостьУпрБезНДС, 0) * ТаблицаОстатков.КоличествоОстаток / ЕСТЬNULL(ТаблицаВнешнихПоступлений.Количество, 1)
    |	КОНЕЦ КАК НоваяСтоимостьУпрБезНДСОстаток,
	|
    |	ТаблицаОстатков.СтоимостьДопРасходыУпрОстаток КАК НоваяСтоимостьДопРасходыУпрОстаток,
	...
	|
    |	ТаблицаОстатков.ДопРасходыУпрБезНДСОстаток КАК НоваяДопРасходыУпрБезНДСОстаток,
    |	ТаблицаОстатков.ПостатейныеПостоянныеУпрБезНДСОстаток КАК НоваяПостатейныеПостоянныеУпрБезНДСОстаток,
    |	ТаблицаОстатков.ПостатейныеПеременныеУпрБезНДСОстаток КАК НоваяПостатейныеПеременныеУпрБезНДСОстаток
	|
	|ПОМЕСТИТЬ ТаблицаНовыхОстатков
	|
	|ИЗ
    ...
    |	ТаблицаНовыхОстатков.НоваяСтоимостьУпрБезНДСОстаток        КАК СтоимостьУпрБезНДСОстаток,
    |	ТаблицаНовыхОстатков.НоваяСтоимостьДопРасходыУпрОстаток 		КАК СтоимостьДопРасходыУпрОстаток,
    ...
    |	ТаблицаНовыхОстатков.НоваяДопРасходыУпрБезНДСОстаток 			 КАК ДопРасходыУпрБезНДСОстаток,
    |	ТаблицаНовыхОстатков.НоваяПостатейныеПостоянныеУпрБезНДСОстаток КАК ПостатейныеПостоянныеУпрБезНДСОстаток,
    |	ТаблицаНовыхОстатков.НоваяПостатейныеПеременныеУпрБезНДСОстаток КАК ПостатейныеПеременныеУпрБезНДСОстаток
	|
	|ПОМЕСТИТЬ ТаблицаОстатков
	|
	|ИЗ
	...
	|	ТОГДА
	|		0
	|	КОГДА (ТаблицаУзлыКорректировки.СтоимостьГраф_1 * ТаблицаУзлыКорректировки.ВесГраф_1 - ЕСТЬNULL(ТаблицаОстатков.СтоимостьУпрОстаток, 0)) = 0
	|		И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|		ТОГДА 0	
	|	ИНАЧЕ
	|		(ТаблицаУзлыКорректировки.СтоимостьГраф_1 * ТаблицаУзлыКорректировки.ВесГраф_1
	...
	|	ИНАЧЕ
	|		(ТаблицаУзлыКорректировки.ДопРасходыГраф_1 * ТаблицаУзлыКорректировки.ВесГраф_1
    |		- ЕСТЬNULL(ТаблицаОстатков.СтоимостьДопРасходыУпрОстаток, 0))
	|		/(ТаблицаУзлыКорректировки.ВесГраф_1
	|		- ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ) КАК СтоимостьГраф_1,
	|
	...
	|	ТОГДА
	|		0
    |	КОГДА (ТаблицаУзлыКорректировки.СтоимостьГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2 - ЕСТЬNULL(ТаблицаОстатков.СтоимостьУпрБезНДСОстаток, 0)) = 0
	|		И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|		ТОГДА 0	
	|	ИНАЧЕ
	|		(ТаблицаУзлыКорректировки.СтоимостьГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |		- ЕСТЬNULL(ТаблицаОстатков.СтоимостьУпрБезНДСОстаток, 0))
	|		/(ТаблицаУзлыКорректировки.ВесГраф_2
	|		- ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0)
	|		+ ЕСТЬNULL(ТаблицаОстатков.КоличествоОстаток, 0))
	|	КОНЕЦ)
	...
	|	ИНАЧЕ
	|		(ТаблицаУзлыКорректировки.ДопРасходыГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |		- ЕСТЬNULL(ТаблицаОстатков.ДопРасходыУпрБезНДСОстаток, 0))
	|		/(ТаблицаУзлыКорректировки.ВесГраф_2
	|		- ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ) КАК СтоимостьГраф_2,
	
	|	ТаблицаУзлыКорректировки.СтоимостьЗабалансовая КАК СтоимостьЗабалансовая,
	|	
	|	ВЫБОР
	|		КОГДА НЕ &ПартионныйУчетВерсии22 ТОГДА 0
	|		КОГДА ТаблицаУзлыКорректировки.ВесГраф_1
	...
	|			ТОГДА 0
	|		КОГДА (ВЫРАЗИТЬ(ТаблицаУзлыКорректировки.СтоимостьГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2 КАК ЧИСЛО(15,3))
    |				- ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаОстатков.СтоимостьУпрБезНДСОстаток, 0) КАК ЧИСЛО(15,3))) = 0
	|			И ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0) <> 0
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ДопРасходыГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ДопРасходыУпрБезНДСОстаток, 0))
	...
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ПостатейныеПостоянныеГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ПостатейныеПостоянныеУпрБезНДСОстаток, 0))
	|			 	 /(ТаблицаУзлыКорректировки.ВесГраф_2
	|			 	 	 - ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ                                 КАК ПостатейныеПостоянныеГраф_2,
	...
	|			ТОГДА 0	
	|		ИНАЧЕ (ТаблицаУзлыКорректировки.ПостатейныеПеременныеГраф_2 * ТаблицаУзлыКорректировки.ВесГраф_2
    |			 - ЕСТЬNULL(ТаблицаОстатков.ПостатейныеПеременныеУпрБезНДСОстаток, 0))
	|			 	 /(ТаблицаУзлыКорректировки.ВесГраф_2
	|			 	 	 - ЕСТЬNULL(ТаблицаОстатков.КоличествоОстатокНаКонецПериода, 0))
	|	КОНЕЦ                                 КАК ПостатейныеПеременныеГраф_2
	|ПОМЕСТИТЬ ВтУзлыКорректировкиВременная
...
Процедура ПодготовитьДанныеДляРаспределенияРезервов(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр)
...
//
Процедура РешитьСЛУ(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр)
	...
	|	УзлыКорректировки.НомерУзла                                      КАК НомерУзла,
        ...
        // Показатели графа 1. Стоимость, Стоимость регл., Стоимость упр.
	|	ВЫРАЗИТЬ(СУММА(
	|		ВЫБОР
	...
	|			КОГДА ЕСТЬNULL(ПеремещенияСписания.ПродукцияДавальца, ЛОЖЬ) ТОГДА 0
	|			КОГДА ЕСТЬNULL(ПеремещенияСписания.ПередачаВЭксплуатацию, ЛОЖЬ) ТОГДА 0
	|			КОГДА &ЗащитаОтПереполненияПоля > 0 И ЕСТЬNULL(ВтТаблицаРешений.НДСРегл, 0)
	|				* ЕСТЬNULL(ПеремещенияСписания.ВесДугиГраф_1, 0) > &ЗащитаОтПереполненияПоля 
	|				ТОГДА 0
	|			КОГДА &ЗащитаОтПереполненияПоля > 0 И ЕСТЬNULL(ВтТаблицаРешений.НДСРегл, 0)
	|				* ЕСТЬNULL(ПеремещенияСписания.ВесДугиГраф_1, 0) < -&ЗащитаОтПереполненияПоля 
	|				ТОГДА 0
	|			ИНАЧЕ ВЫРАЗИТЬ(ЕСТЬNULL(ВтТаблицаРешений.НДСРегл, 0) КАК ЧИСЛО(28,10))
	|				* ВЫРАЗИТЬ(ЕСТЬNULL(ПеремещенияСписания.ВесДугиГраф_1, 0) КАК ЧИСЛО(28,10))
	|		КОНЕЦ) КАК ЧИСЛО(28,10)) КАК НДСРегл,
    |
	|	ВЫРАЗИТЬ(СУММА(
	...
	|		КОНЕЦ) КАК ЧИСЛО(28,10)) КАК СтоимостьЗабалансовая,
	|
        // Показатели графа 2. Стоимость без НДС, Стоимость регл. без НДС, Стоимость упр. без НДС
	|	ВЫРАЗИТЬ(СУММА(
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(ПеремещенияСписания.ПродукцияДавальца, ЛОЖЬ) ТОГДА 0
	...
	|				КОГДА УзлыКорректировки.ВесГраф_1 = 0 ТОГДА
	|					0
	|				КОГДА ЕСТЬNULL(ТаблицаРешений.НДСРегл, 0) - УзлыКорректировки.НДСРегл
	|					- ВЫРАЗИТЬ(ВременнаяТаблицаРешений.НДСРегл / УзлыКорректировки.ВесГраф_1 КАК ЧИСЛО(28,10)) > 0 ТОГДА
	|
	|					ЕСТЬNULL(ТаблицаРешений.НДСРегл, 0) - УзлыКорректировки.НДСРегл
	|						- ВЫРАЗИТЬ(ВременнаяТаблицаРешений.НДСРегл / УзлыКорректировки.ВесГраф_1 КАК ЧИСЛО(28,10))
	|
	|				ИНАЧЕ
	|					-(
	|						ЕСТЬNULL(ТаблицаРешений.НДСРегл, 0) - УзлыКорректировки.НДСРегл
	...
	Пока ТекущееОтклонение > ПараметрыРасчета.РешениеСЛУ.ТребуемаяТочность
	 И СчетчикИтераций <= ПараметрыРасчета.РешениеСЛУ.МаксимальноеКоличествоИтераций Цикл
		...
		СтрокаПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отклонение на текущей итерации: %1';uk='Відхилення на поточній ітерації: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	// Добавим запись в протокол расчета
	СтрокаПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполнено решение СЛУ (итерации / итоговое отклонение): %1 / %2';uk='Виконано рішення СЛР (ітерації / підсумкове відхилення): %1 / %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
// в каждом узле корректировки.
//
Процедура РешитьСЛУПлатформой(ПараметрыРасчета, РасчетСтоимостиРегл, РасчетСтоимостиУпр)
...
//		СтоимостьГраф_1, ДопРасходыГраф_1, ТрудозатратыГраф_1, НДСРегл, СтоимостьЗабалансовая
...
Процедура РешитьСЛУПлатформойБазовыеЗначенияГраф_1(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru='Решение для базовых значений (граф 1)';uk='Рішення для базових значень (граф 1)'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	|	ВЫРАЗИТЬ(УзлыКорректировки.НДСРегл * УзлыКорректировки.ВесГраф_1 КАК ЧИСЛО(28,10))	                КАК НДСРегл,
	...
	|		КОГДА ЕСТЬNULL(ПеремещенияСписания.ПередачаВЭксплуатацию, ЛОЖЬ) ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(-ПеремещенияСписания.ВесДугиГраф_1, 0)
	|	КОНЕЦ КАК ЧИСЛО(28,10)) КАК ВесДугиНДСРегл,
    |
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ЕСТЬNULL(ПеремещенияСписания.РаботаДляДавальца, ЛОЖЬ) ТОГДА 0
	|		КОГДА ЕСТЬNULL(ПеремещенияСписания.ПередачаВЭксплуатацию, ЛОЖЬ) ТОГДА 0
    ...
    |	ВЫРАЗИТЬ(УзлыКорректировки.ВесГраф_1 КАК ЧИСЛО(28,10)) КАК ВесДугиНДСРегл,
	...
	// источники данных
	МатрицаУзлов  = РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, ЗапросУзлы,,, Истина, ПояснениеДляЗамера + ", " + НСтр("ru='узлы';uk='вузли'",ОбщегоНазначения.КодОсновногоЯзыка()));
	МатрицаСвязей = РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, ЗапросСвязи,,, Истина, ПояснениеДляЗамера + ", " + НСтр("ru='связи';uk='зв''язки'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ОписаниеСистемы.КолонкаКоэффициентовВСвязях = "ВесДугиНДСРегл";
    ...
    |	ВЫРАЗИТЬ(ТаблицаРешений.Решение4 КАК ЧИСЛО(28,10)) КАК НДСРегл,
    |	ВЫРАЗИТЬ(ТаблицаРешений.Решение5 КАК ЧИСЛО(28,10)) КАК СтоимостьЗабалансовая,
	|	1 КАК СтоимостьЗнакГраф_1
	|
	...
	|	ВЫРАЗИТЬ(ВЫБОР КОГДА УзлыКорректировки.ВесГраф_1 = 0 ТОГДА 0 ИНАЧЕ ЕСТЬNULL(ТаблицаРешений.НДСРегл, 0) КОНЕЦ КАК ЧИСЛО(28,10)) КАК НДСРегл,
...
Процедура РешитьСЛУПлатформойБазовыеЗначенияГраф_2(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru='Решение для базовых значений (граф 2)';uk='Рішення для базових значень (граф 2)'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура РешитьСЛУПлатформойПостатейныеГраф_1(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru='Решение для постатейных (граф 1)';uk='Рішення для постатейних (граф 1)'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура РешитьСЛУПлатформойПостатейныеГраф_2(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru='Решение для постатейных (граф 2)';uk='Рішення для постатейних (граф 2)'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура РешитьСЛУПлатформойРазницыШаг(ПараметрыРасчета, РасчетСтоимостиРегл, НомерШага)
	...
	ШаблонПояснениеДляЗамера = НСтр("ru='Решение для %1 (шаг %2)';uk='Рішення для %1 (крок %2)'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	|	ВЫРАЗИТЬ(СУММА
	|		(
    |		    0 * ЕСТЬNULL(ПеремещенияСписания.ВесДугиГраф_1, 0)
	|		) 
	...
	ПояснениеДляЗамера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПояснениеДляЗамера, НСтр("ru='ПР';uk='ПР'",ОбщегоНазначения.КодОсновногоЯзыка()), НомерШага);
	...
	ПояснениеДляЗамера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПояснениеДляЗамера, НСтр("ru='ВР';uk='ТР'",ОбщегоНазначения.КодОсновногоЯзыка()), НомерШага);
...
Процедура РешитьСЛУПлатформойОбъединитьРешения(ПараметрыРасчета, РасчетСтоимостиРегл)
	...
	ПояснениеДляЗамера = НСтр("ru='Объединить решения СЛАУ';uk='Об''єднати рішення СЛАР'",ОбщегоНазначения.КодОсновногоЯзыка());
    ...
    |
	|	ВЫРАЗИТЬ(ЕСТЬNULL(ВтТаблицаБазовыхРешенийГраф_1.НДСРегл, 0) КАК ЧИСЛО(28,10)) КАК НДСРегл,
	|
...
//
// Возвращаемое значение:
//	РасчетСистемЛинейныхУравнений - компонента решения системы линейныъ уравнений
//
...
|%2'
|;uk='Не вдалося знайти каталог для збереження даних СЛР: ""%1"" 
...
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Иначе
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный формат выгрузки СЛУ: %1';uk='Невірний формат вивантаження СЛР: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
			ФорматВыгрузки);
...
Процедура ИнициализироватьСвойстваЗапроса(Запрос, ПараметрыРасчета, ИспользоватьОбщийМенеджерВТ = Истина) Экспорт
	...
	ФИФОВзвешеннаяОценка = (ПараметрыРасчета.МетодОценки = Перечисления.МетодыОценкиСтоимостиТоваров.ФИФОВзвешеннаяОценка);
	...
	Запрос.УстановитьПараметр("ФИФОВзвешеннаяОценка", 					ФИФОВзвешеннаяОценка);
...
|який не може мати рухів з даного регістру.'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОписаниеПриемника.ЭтоОписаниеРегистра И НЕ ЗначениеЗаполнено(Запись.Регистратор) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДанныеДвижения.ДокументДвижения) Тогда
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не заполнено свойство ДокументДвижения для движения по регистру ""%1""';uk='Не заповнена властивість ДокументДвижения для руху по регістру ""%1""'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ТаблицаПриемник);
		Иначе
			// Ошибка в метаданных - документ не является регистратором для данного регистра.
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Документ ""%1"" не может иметь движений по регистру ""%2""';uk='Документ ""%1"" не може мати рухів по регістру ""%2""'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ЗарегистрироватьПроблемуВыполненияРасчета(
			...
			НСтр("ru='При формировании движений диагностированы ошибки';uk='При формуванні рухів діагностовано помилки'",ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола,
			ДанныеДвижения.ДокументДвижения);
...
Функция ИспользуемыеКэшиРегистровПартионногоУчета() Экспорт
    ...
    // - рассчитываются механизмом партионного учета версии 2.5
	...
	//
    // В случае, если выполняется отдельный запуск расчета себестоимости (не из партионного учета версии 2.5),
...
Функция ТекущаяВерсияМеханизма() Экспорт
    Возврат НСтр("ru='Расчет себестоимости, версия 2.5';uk='Розрахунок собівартості, версія 2.5'",ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
