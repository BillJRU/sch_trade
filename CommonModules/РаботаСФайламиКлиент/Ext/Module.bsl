...
Процедура ДобавитьФайлы(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач Фильтр = "", ГруппаФайлов = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайлы.';uk='Не задано значення параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайлы.'");
	КонецЕсли;
...
Процедура ДобавитьФайл(ОбработчикРезультата, ВладелецФайла, ФормаВладелец, РежимСоздания = Неопределено, 
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайл.';uk='Не задано значення параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайл.'");
	КонецЕсли;
...
Процедура СкопироватьФайл(ВладелецФайла, ФайлОснование, ДополнительныеПараметры = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.СкопироватьФайл.';uk='Не задано значення параметра ВладелецФайла в РаботаСФайламиКлиент.СкопироватьФайл.'");
	КонецЕсли;
...
#Если МобильныйКлиент Тогда
	ПоказатьПредупреждение(, НСтр("ru='Печать файлов данного типа не поддерживается в мобильном клиенте.';uk='Друк файлів даного типу не підтримується в мобільному клієнті.'"));
	Возврат;
#Иначе
	...
	Если СтрНайти(РасширенияИсключения, " " + Расширение + ",") > 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Печать файлов данного типа не поддерживается.';uk='Друк файлів даного типу не підтримується.'"));
		Возврат;
...
Процедура ЗадатьВопросОЗаписиВладельца(ПараметрыОбработчикаЗавершения)
	...
	ТекстВопроса = НСтр("ru='Данные еще не записаны.
        ...
        |;uk='Дані ще не записані.
        |Перехід до приєднаних файлів можливий тільки після запису даних.
...
Процедура ОбновитьОбластьПредпросмотра(Форма, НомерЭлемента, Файл)
	...
	Если ЭлементКартинки <> Неопределено Тогда
		...
		ИначеЕсли ДанныеОбновления.ФайлПоврежден Тогда
			Форма[ИмяРеквизита] = Неопределено;
			ЭлементКартинки.ТекстНевыбраннойКартинки = НСтр("ru='Изображение отсутствует';uk='Зображення відсутнє'");
		Иначе
	...
	Если ЭлементЗаголовка <> Неопределено Тогда
		...
		Если ДанныеФайла = Неопределено Тогда
			ЭлементЗаголовка.Заголовок = НСтр("ru='загрузить';uk='завантажити'");
