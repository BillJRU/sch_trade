		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
                        ...
                        |;uk='Помилка з''єднання з комп''ютера %1 до кластеру серверів через:
                        |%2
                        |
                        |Якщо на комп''ютері %1 не запущений сервер адміністрування (ras), слід його запустити.
                        |Наприклад:
                        |""%3"" cluster --port=%4 %5:%6
                        |
                        |Також рекомендується перевірити параметри підключення і мережевого екрану.'"),
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Значение.Добавить("1CV8");               // идентификатор приложения BAF в режиме запуска "Толстый клиент".
	Значение.Добавить("1CV8C");              // идентификатор приложения BAF в режиме запуска "Тонкий клиент".
	Значение.Добавить("WebClient");          // идентификатор приложения BAF в режиме запуска "Веб-клиент".
	Значение.Добавить("Designer");           // идентификатор приложения Конфигуратор.
	Значение.Добавить("COMConnection");      // идентификатор сессии внешнего соединения платформы BAF через COM.
...
Функция ИдентификаторКластера(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не обнаружен кластер серверов с портом %1';uk='Не виявлено кластер серверів з портом %1'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
Функция ПривестиЭлементВывода(ЭлементВывода, ТипЭлемента)
	...
	ИначеЕсли ТипЭлемента = Тип("Число") Тогда
		...
		Исключение
			ВызватьИсключение НСтр("ru='Неправильный формат.';uk='Неправильний формат.'");
		КонецПопытки;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Неверный тип элемента.';uk='Невірний тип елемента.'");
...
Функция ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, Словарь = Неопределено, Фильтр = Неопределено, ТипыСвойств = Неопределено)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru='В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера';uk='У моделі сервісу неприпустимо виконання прикладною інформаційною базою функцій адміністрування кластера'");
	КонецЕсли;
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден
                       ...
                       |Для его установки:
                       |- Для компьютеров с ОС Windows требуется переустановить платформу, установив компонент ""Сервер BAF"";
                       |- Для компьютеров с ОС Linux требуется установить пакет BAF-server*.'
                       |;uk='Неможливо виконати операцію адміністрування кластера серверів через: файл %1 не знайдено
                       |
                       ...
                       |Для його установки:
                       |- Для комп''ютерів з ОС Windows потрібно перевстановити платформу, встановивши компонент ""Сервер BAF"";
                       |- Для комп''ютерів з ОС Linux потрібно встановити пакет BAF-server*.'"),
...
Функция ПоддерживаемыеСвойстваПрофиля(ПараметрыАдминистрированияКластера)
	...
	ОписаниеПрофиля = НСтр("ru='Служебный профиль для проверки поддерживаемых свойств.';uk='Службовий профіль для перевірки підтримуваних властивостей.'");
