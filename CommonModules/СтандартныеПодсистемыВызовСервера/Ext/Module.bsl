...
Функция ЗаписатьОшибкуВЖурналРегистрацииПриЗапускеИлиЗавершении(ПрекратитьРаботу, Знач Событие, Знач ТекстОшибки) Экспорт
	...
	Если Событие = "Запуск" Тогда
		ИмяСобытия = НСтр("ru='Запуск программы';uk='Запуск програми'",ОбщегоНазначения.КодОсновногоЯзыка());
		...
		КонецЕсли;
	Иначе
		ИмяСобытия = НСтр("ru='Завершение программы';uk='Завершення програми'",ОбщегоНазначения.КодОсновногоЯзыка());
