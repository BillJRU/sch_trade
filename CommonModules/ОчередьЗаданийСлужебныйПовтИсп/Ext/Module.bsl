...
//   * Ключ - Строка псевдоним метода
//   * Значение - Строка - имя метода для вызова.
//
Функция СоответствиеИменМетодовПсевдонимам() Экспорт
	...
	ИнтеграцияПодсистемБТС.ПриОпределенииПсевдонимовОбработчиков(МетодыКонфигурации);
...
//  ФиксированноеСоответствие: 
//   * Ключ - Строка - псевдоним метода
//   * Значение - Строка - полное имя метода обработчика.
//
Функция СоответствиеОбработчиковОшибокПсевдонимам() Экспорт
	...
	ИнтеграцияПодсистемБТС.ПриОпределенииОбработчиковОшибок(ОбработчикиОшибок);
...
// Возвращает описание реквизитов (параметров) справочника заданий очереди.
//
// Возвращаемое значение:
//  ТаблицаЗначений - описание реквизитов (параметров):
...
//  * Добавление - Булево - параметр допустимо указывать при добавлении задания в очередь.
//  * Изменение - Булево - параметр допустимо изменять.
//  * Шаблон - Булево - параметр допустимо изменять для заданий созданных по шаблону.
//  * РазделениеДанных - Булево - параметр используется только при работе разделенными заданиями.
//  * ЗначениеДляНеразделенныхЗаданий - Строка - значение, которое необходимо  возвращать из API для разделенных
//     параметров неразделенных заданий (в виде строки, пригодной для подстановки в тексты запросов).
//
Функция ПараметрыЗаданийОчереди() Экспорт
	
	ТипНеограниченнаяСтрока = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(0, ДопустимаяДлина.Переменная));
	 
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Имя", ТипНеограниченнаяСтрока);
	Результат.Колонки.Добавить("ИмяВРег", ТипНеограниченнаяСтрока);
	Результат.Колонки.Добавить("Поле", ТипНеограниченнаяСтрока);
	...
	ОписаниеПараметра = Результат.Добавить();
	ОписаниеПараметра.Имя = ОчередьЗаданий.ПолеИдентификаторЗадания();
	...
	ОписаниеПараметра = Результат.Добавить();
	ОписаниеПараметра.Имя = ОчередьЗаданий.ПолеПараметрыМетода();
	ОписаниеПараметра.Поле = ОписаниеПараметра.Имя;
	...
	ОписаниеПараметра = Результат.Добавить();
	ОписаниеПараметра.Имя = "ВыполнятьЧерезРегламентноеЗадание";
	ОписаниеПараметра.Поле = ОписаниеПараметра.Имя;
	ОписаниеПараметра.Тип = Новый ОписаниеТипов("Булево");
	ОписаниеПараметра.Добавление = Истина;
	ОписаниеПараметра.Изменение = Истина;
	...
	ОписаниеПараметра = Результат.Добавить();
	ОписаниеПараметра.Имя = "ОжидатьЗавершенияВыполненияРегламентногоЗадания";
	ОписаниеПараметра.Поле = ОписаниеПараметра.Имя;
	ОписаниеПараметра.Тип = Новый ОписаниеТипов("Булево");
	ОписаниеПараметра.Добавление = Истина;
	ОписаниеПараметра.Изменение = Истина;
...
// Возвращает часть текста запроса получения заданий для возврата через программный интерфейс.
//
// Параметры:
//  СправочникЗаданий - СправочникМенеджер - менеджер справочника, для которого получаются задания очереди. 
//   Используется для фильтрации полей выборки, которые применимы не для всех справочников заданий.
//
// Возвращаемое значение:
// 	Строка - поля выборки справочника очереди заданий.
...
//
// Возвращаемое значение:
// 	ФиксированныйМассив Из СправочникМенеджер - справочники заданий.
