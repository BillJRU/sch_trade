...
// полученный дистрибутив Платформы BAF. В других режимах работы
// возвращает Неопределено.
//
// Возвращаемое значение:
//	Строка - каталог с дистрибутивом Платформы BAF в
...
Функция ИсправленияРодительскихКонфигураций() Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина, Истина) Тогда
		...
		Результат.КраткоеОписаниеОшибки   = НСтр("ru='Недостаточно прав.';uk='Недостатньо прав.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Недостаточно прав для получения информации об исправлениях.';uk='Недостатньо прав для отримання інформації про виправлення.'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить информацию о доступных исправлениях (патчах) родительских конфигураций.
                |%1'
                |;uk='Не вдалося отримати інформацію про доступні виправлення (патчі) батьківських конфігурацій.
                |%1'"),
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			80,                                                       
			НСтр("ru='Получение файлов обновлений программы';uk='Отримання файлів оновлень програми'"));
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			443,                                         
			НСтр("ru='Сервис получения обновлений программы';uk='Сервіс отримання оновлень програми'"));
		НовыеРазрешения.Добавить(Разрешение);
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Не НастройкиОбновления.ПолучатьИсправления Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru='Установка исправлений (патчей) запрещена в настройках конфигурации.';uk='Встановлення виправлень (патчів) заборонене в настройках конфігурації.'"));
		Загружать = Ложь;
		Возврат;
	КонецЕсли;
	...
	Если УдалитьИсправление Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отзыв исправления: %1';uk='Відгук виправлення: %1'"),
				Идентификатор));
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Установка нового исправления не возможна, т.к.
                        ...
                        |версии конфигурации:
                        |%1'
                        |;uk='Встановлення нового виправлення не можливе, тому що
                        |в дескрипторі даних що поставляються відсутній номер
                        |версії конфігурації:
                        |%1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Установка нового исправления:
                |%1'
                |;uk='Встановлення нового виправлення:
                |%1'"),
			МодульПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор)));
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обработка поставляемых данных не выполнена.
                |Поставляемые данные:
                |%1'
                |;uk='Обробка даних що постачаються не виконана.
                |Дані що постачаються:
                |%1'"),
			МодульПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор)));
...
Функция ИнформацияОДоступномОбновленииВНастройках() Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Результат) <> Тип("Структура")
			...
			Или Результат.ВерсияПлатформы <> ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы()
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить информацию о доступном обновлении из настроек пользователя.
                    |%1'
                    |;uk='Не вдалося отримати інформацію про доступне оновлення з настройок користувача.
                    |%1'"),
...
Функция ПараметрыОповещенияОДоступномОбновлении(ИнформацияОДоступномОбновлении, ДляТекущихДел = Ложь) Экспорт
	...
	Если Не ПустаяСтрока(ИнформацияОДоступномОбновлении.ИмяОшибки) Тогда
		...
		Если ИнформацияОДоступномОбновлении.ИмяОшибки = "ConnectError"
			...
			Результат = Новый Структура;
			Результат.Вставить("Текст" , НСтр("ru='Не удалось проверить наличие обновлений программы.';uk='Не вдалося перевірити наявність оновлень програми.'"));
	...
	Иначе
		...
		Если ДоступноОбновление Тогда
			...
			Результат.Текст = НСтр("ru='Доступно обновление программы.';uk='Доступне оновлення програми.'");
...
Функция УстановкаИУдалениеИсправлений(Исправления, ОтозванныеИсправления, УдалятьФайлыИсправлений = Ложь, ВФоне = Ложь) Экспорт
						...
						ЗаписатьОшибкуВЖурналРегистрации(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не удалось удалить файл исправления %1.
                                    |%2'
                                    |;uk='Не вдалося вилучити файл виправлення %1.
                                    |%2'"),
	...
	Если ОписаниеИсправлений.Установить.Количество() = 0
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Установка и удаление исправлений не требуются (все исправления были установлены ранее).';uk='Встановлення і вилучення виправлень не потрібні (всі виправлення були встановлені раніше).'"));
		Возврат Результат;
	КонецЕсли;
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вызов установки и удаления исправлений (БСП).
                ...
                |%3.'
                |;uk='Виклик встановлення та вилучення виправлень (БСП).
                |Виправлення для встановлення:
                |%1
                |
                |Виправлення для вилучення (відкликані):
                |%2
                |
                |Вже встановлені (виключені зі встановлення):
                |%3.'"),
	...
	Если РезультатУстановки.НеУстановлено > 0 Или РезультатУстановки.НеУдалено > 0 Тогда
		...
		Если РезультатУстановки.НеУстановлено > 0 Тогда
			...
			СообщениеОбОшибке = СообщениеОбОшибке + ?(ПустаяСтрока(СообщениеОбОшибке), "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось установить исправления (%1 шт.).';uk='Не вдалося встановити виправлення (%1 шт.).'"),
		...
		Если РезультатУстановки.НеУдалено > 0 Тогда
			...
			СообщениеОбОшибке = СообщениеОбОшибке + ?(ПустаяСтрока(СообщениеОбОшибке), "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось удалить исправления (%1 шт.).';uk='Не вдалося вилучити виправлення (%1 шт.).'"),
	...
	Иначе
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Установка и удаление исправлений успешно завершены.';uk='Встановлення і вилучення виправлень успішно завершені.'"));
...
Функция КаталогСодержитДистрибутивПлатформы(Знач Каталог, Версия)
	...
	Если ПолучениеОбновленийПрограммыКлиентСервер.Это64РазрядноеПриложение() Тогда
		ИскомаяСтрокаПродукта = "PRODUCT=BAF (X86-64)";
	Иначе
		ИскомаяСтрокаПродукта = "PRODUCT=BAF 8";
	КонецЕсли;
...
Процедура ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры, АдресРезультата) Экспорт
	...
	Если Контекст.ОбновитьПлатформу Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Получение обновления платформы BAF: версия %1; URL: %2';uk='Отримання оновлення платформи BAF: версія %1; URL: %2'"),
		...
		Если Не ПустаяСтрока(Контекст.КаталогДистрибутиваПлатформы) Тогда
			...
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Обновление платформы BAF уже было загружено ранее в %1';uk='Оновлення платформи BAF вже було завантажено раніше в %1'"),
					Контекст.КаталогДистрибутиваПлатформы));
			
		Иначе
			
			КаталогУстановкиПлатформы = ПолучениеОбновленийПрограммыКлиентСервер.КаталогУстановкиПлатформы(
				Контекст.ВерсияПлатформы);
			Если КаталогУстановкиПлатформы <> Неопределено Тогда
				...
				ЗаписатьИнформациюВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Версия %1 платформы BAF уже установлена на компьютере.';uk='Версія %1 платформи BAF вже встановлена на комп''ютері.'"),
						Контекст.ВерсияПлатформы));
	...
	Для Каждого ТекИсправление Из Контекст.Исправления Цикл
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Получение файла исправления (патча).
                    ...
                    |Размер: %3;
                    |Локальный файл: %4.'
                    |;uk='Отримання файлу виправлення (патча).
                    |URL: %1;
                    |Ідентифікатор: %2;
                    |Розмір: %3;
                    |Локальний файл: %4.'"),
		...
		Если Не ТекИсправление.Получено Тогда
			...
			Если РезультатПолучения.Ошибка Тогда
				...
				СообщениеЖурнала =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка при получении файла исправления (патча) (%1). %2';uk='Помилка при отриманні файлу виправлення (патча) (%1). %2'"),
						ТекИсправление.URLФайла,
				...
				Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при получении файла исправления (патча). %1';uk='Помилка при отриманні файлу виправлення (патча). %1'"),
			...
			Иначе
					...
					СообщениеЖурнала =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Ошибка при получении файла исправления (патча) (%1).
                                ...
                                |%2'
                                |;uk='Помилка при отриманні файлу виправлення (патча) (%1).
                                |Не вдалося зберегти файл в локальному каталозі.
                                |%2'"),
							ТекИсправление.URLФайла,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
				...
				ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Файл исправления (патча) успешно получен.';uk='Файл виправлення (патча) успішно отримано.'"));
		...
		Иначе
			...
			ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru='Исправление (патч) уже было получено ранее.';uk='Виправлення (патч) вже було отримано раніше.'"));
	...
	Если Контекст.ОбновленияКонфигурации.Количество() = 0
		...
		Контекст.ТекущееДействие = НСтр("ru='Установка исправлений (патчей)';uk='Встановлення виправлень (патчів)'");
	...
	Если Контекст.ОбновитьПлатформу И Не Контекст.ОбновлениеПлатформыУстановлено Тогда
		...
		Контекст.ТекущееДействие = НСтр("ru='Установка платформы BAF';uk='Встановлення платформи BAF'");
...
Процедура ЗагрузитьОбновлениеПлатформы(Контекст, Параметры, ДанныеАутентификации)
	...
	ДистрибутивЗагружен = КаталогСодержитДистрибутивПлатформы(
		КаталогДистрибутива,
		Контекст.ВерсияПлатформы);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение файла обновления платформы BAF: %1';uk='Отримання файлу оновлення платформи BAF: %1'"),
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при получении файла дистрибутива платформы BAF (%1). %2';uk='Помилка при отриманні файлу дистрибутива платформи BAF (%1). %2'"),
				Контекст.URLФайлаОбновленияПлатформы,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл обновления платформы BAF успешно получен: %1';uk='Файл оновлення платформи BAF успішно отримано: %1'"),
			ПутьПолученногоФайла));
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Извлечение дистрибутива платформы BAF в %1';uk='Витягування дистрибутива платформи BAF в %1'"),
				КаталогДистрибутива));
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Дистрибутив платформы BAF успешно сохранен в %1';uk='Дистрибутив платформи BAF успішно збережений в %1'"),
			КаталогДистрибутива));
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Если Не ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПрограммыУстановки) Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загружен некорректный дистрибутив платформы BAF. Отсутствует файл setup.exe (%1).';uk='Завантажений некоректний дистрибутив платформи BAF. Відсутній файл setup.exe (%1).'"),
			ПутьФайлаПрограммыУстановки);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		
		Контекст.ИмяОшибки = "НекорректныйДистрибутивПлатформы";
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = НСтр("ru='Загружен некорректный дистрибутив платформы BAF. Отсутствует файл setup.exe.';uk='Завантажений некоректний дистрибутив платформи BAF. Відсутній файл setup.exe.'");
		Возврат;
	...
	Попытка
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Установка новой версии платформы BAF (%1). %2';uk='Встановлення нової версії платформи BAF (%1). %2'"),
				Контекст.ВерсияПлатформы,
				КомандаЗапуска));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при запуске программы установки платформы BAF (%1). %2';uk='Помилка під час запуску програми установки платформи BAF (%1). %2'"),
				КомандаЗапуска,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при запуске программы установки платформы BAF. %1';uk='Помилка під час запуску програми установки платформи BAF. %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	...
	Если КодВозврата = 0 Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Новая версия платформы BAF успешно установлена.';uk='Нова версія платформи BAF успішно встановлена.'"));
	...
	Иначе
		...
		Иначе
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Установка платформы BAF завершена с ошибкой.
                        |Версия %1;
                        |Код возврата: %2;
                        |Команда: %3'
                        |;uk='Встановлення платформи BAF завершене з помилкою.
                        |Версія %1;
                        |Код повернення: %2;
                        |Команда: %3'"),
					Контекст.ВерсияПлатформы,
			...
			ШаблонСообщенияПользователю =
				НСтр("ru='<body>При установке новой версии платформы BAF произошла ошибка.
                    |<br />Код возврата: %1.'
                    |;uk='<body>При установці нової версії платформи BAF сталася помилка.
                    |<br />Код повернення: %1.'");
...
Процедура РегламентноеЗаданиеПолучениеИУстановкаИсправленийКонфигурации() Экспорт
	...
	Если Не ПараметрыОбновления.ПолучатьИсправления Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru='Использование получения исправлений недоступно в текущем режиме работы.
                ...
                |;uk='Використання отримання виправлень недоступне в поточному режимі роботи.
                |Регламентне завдання ПолучениеИУстановкаИсправленийКонфигурации відключене.'"));
		РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания(
			МетаданныеЗадание,
			Ложь);
		Возврат;
	...
	Если Задание <> Неопределено Тогда
		...
		Если ПериодПовтораВТечениеДня > 0 И ПериодПовтораВТечениеДня < 3600 Тогда
			ВызватьИсключение НСтр("ru='Интервал автоматической установки не может быть чаще, чем один раз в час.';uk='Інтервал автоматичного встановлення не може бути частіше, ніж один раз на годину.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеАутентификации = Неопределено Тогда
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Интернет-поддержка не подключена.
            ...
            |;uk='Інтернет-підтримка не підключена.
            |Виконання завдання автоматичного отримання і установки перервано.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьИУстановитьИсправленияДляТекущейВерсииКонфигурации(ДанныеАутентификации, ВФоне)
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru='Автоматическое получение и установка исправлений (патчей) для текущей версии конфигурации';uk='Автоматичне отримання і встановлення виправлень (патчів) для поточної версії конфігурації'"));
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось автоматически получить и установить исправления (патчи) для текущей версии конфигурации.
                    ...
                    |%1'
                    |;uk='Не вдалося автоматично отримати і встановити виправлення (патчі) для поточної версії конфігурації.
                    |Не вдалося отримати інформацію про доступні виправлення.
                    |%1'"),
	...
	Если Исправления = Неопределено Или Исправления.Количество() = 0 Тогда
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Нет доступных исправлений (патчей) для установки';uk='Немає доступних виправлень (патчів) для установки'"));
		Возврат;
	КонецЕсли;
	...
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Завершено автоматическое получение и установка исправлений (патчей)';uk='Завершено автоматичне отримання і встановлення виправлень (патчів)'"));
			...
			Если ИмяСвойства = "patchUeid" Тогда
				...
				Исключение
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Некорректный идентификатор исправления (патча) в patchUpdateList[].uin (%1)';uk='Некоректний ідентифікатор виправлення (патча) в patchUpdateList[].uin (%1)'"),
						ЗначениеСвойства);
		...
		Если ВерсииКонфигураций.Количество() > 0 Тогда
			...
			Если РезультатИнформацияОбИсправлениях.Ошибка Тогда
				...
				Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось получить информацию об исправлениях (патчах). %1';uk='Не вдалося отримати інформацію про виправлення (патчі). %1'"),
...
Процедура ИнформацияОДоступномОбновленииВызовОперации(
	...
	Если ИмяТекущейПрограммы = "Unknown" Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ru='Не удалось получить информацию о доступном обновлении.
                ...
                |Не визначено ім''я програми в методі ПриДобавленииПодсистемы() загального модуля ОбновлениеИнформационнойБазы[Имя программы].'");
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение информации о доступном обновлении (%1).';uk='Отримання інформації про доступне оновлення (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Имя текущей программы: %1;
                ...
                |%7'
                |;uk='Ім''я поточної програми: %1;
                |Версія поточної програми: %2;
                |Ім''я нової програми: %3;
                |Номер редакції нової програми: %4;
                |Сценарій оновлення: %5;
                |Додаткові параметри:
                |%7'"),
	...
	Если Не ПустаяСтрока(ОбновлениеПлатформы.Версия) Тогда
		...
		ТекВерсияПлатформы = ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы();
		ОбновлениеПлатформы.ДоступноОбновление = Ложь;    
...
Функция InfoRequestJSON(ПараметрыОперации, ДопПараметрыЗапроса)
	...
	ЗаписьДанныхСообщения.ЗаписатьЗначение(ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы());
	...
	Пока ЧтениеJSONПрочитать(ЧтениеОтвета, ТекущийУровень) Цикл
		...
		Если ЧтениеОтвета.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства И ТекущийУровень = 1 Тогда
			...
			ИначеЕсли ИмяСвойства = "errorMessage" Тогда
				...
				Результат.Сообщение          = ОбработатьСообщение(Результат.Сообщение); 
	...
	Пока ЧтениеJSONПрочитать(ЧтениеОтвета, ТекущийУровень) Цикл
		...
		ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства И ТекущийУровень = 2 Тогда
			...
			ИначеЕсли ИмяСвойства = "updateInfoUrl" Тогда
				...
				ОбновлениеКомКонф.URLНовоеВВерсии = ОбработатьURL(ОбновлениеКомКонф.URLНовоеВВерсии)
			ИначеЕсли ИмяСвойства = "howToUpdateInfoUrl" Тогда
				...
				ОбновлениеКомКонф.URLПорядокОбновления = ОбработатьURL(ОбновлениеКомКонф.URLПорядокОбновления)
...
Процедура ПолучитьОписаниеФайловОбновленияКонфигурацииИПлатформы(
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение информации о файлах обновления конфигурации и Платформы (%1).';uk='Отримання інформації про файлах оновлення конфігурації і Платформи (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Логин: %1
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получена информация о файлах обновления конфигурации и Платформы.
                |%1'
                |;uk='Отримано інформацію про файли оновлення конфігурації і Платформи.
                |%1'"),
			РезультатОтправки.Содержимое));
...
Функция НовыйОбновлениеКонфигурации(
	...
	Результат = Новый Структура;
	Результат.Вставить("URLФайлаОбновления"            , ОбработатьURL(URLФайлаОбновления));
...
Функция ОбработатьURL(Знач ТекстURL)
	
	Результат = СтрЗаменить(ТекстURL, Символ(46)+Символ(49)+Символ(99)+Символ(46), ".bas-soft.");
	Возврат  Результат;
	
КонецФункции

Функция ОбработатьСообщение(Знач Сообщение)
	
	Результат = "";
	Если СтрНайти(Сообщение, "<body>Неверный логин или пароль") > 0 Тогда
		Результат = "<body>Невірний логін або пароль<br/><a href=""https://login.bas-soft.eu/remind_request"">Відновити пароль</a></body>";
	ИначеЕсли СтрНайти(Сообщение, "<body>Подключение к Интернет-поддержке с указанным логином и паролем выполнено. К сожалению,") > 0 Тогда
		Результат = "<body>Підключення до Інтернет-підтримки із зазначеним логіном і паролем виконано. На жаль, <a href=""https://portal.bas-soft.eu/applications/Program-update-Ukraine"">сервіс оновлення програм</a> недоступний, оскільки умови супроводу цього програмного продукту не виконано. Для з'ясування причини можна написати повідомлення в <a href=""mailto:webits-info@bas-soft.eu"">технічну підтримку</a>.</body>";
	КонецЕсли;
	Возврат  Результат;
...
Функция ИнформацияОДоступныхИсправленияхКонфигураций(ВерсииКонфигураций, ИдентификаторыУстановленныхИсправлений, ПроверитьДоступностьСервиса = Истина)
		...
		Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
			...
			Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить информацию о доступных исправлениях (патчах) конфигураций.
                    ...
                    |Причина:
                    |%2'
                    |;uk='Не вдалося отримати інформацію про доступні виправлення (патчі) конфігурацій.
                    |Не вдалося перевірити доступність сервісу автоматичного оновлення програми: %1.
                    |Причина:
                    |%2'"),
				URLОперацииPing,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение информации об исправлениях (патчах) конфигураций (%1).';uk='Отримання інформації про виправлення (патчі) конфігурацій (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Версии подсистем:
                ...
                |;uk='Версії підсистем:
                |%1;
                |ІдентифікаториВстановленнихВиправлень:
                |%2;
                |Додаткові параметри:
                |%3'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при получении информации о доступных исправлениях (патчах) конфигураций.
                |%1'
                |;uk='Помилка при отриманні інформації про доступні виправлення (патчі) конфігурацій.
                |%1'"),
			РезультатОтправки.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(Результат.ПодробноеОписаниеОшибки);
	...
	Исключение
		...
		Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проверить наличие исправлений (патчей) конфигураций.
                ...
                |%1
                |Тело ответа: %2'
                |;uk='Не вдалося перевірити наявність виправлень (патчів) конфігурацій.
                |Помилка при обробці відповіді сервісу.
                |Некоректна відповідь сервісу.
                |%1
                |Тіло відповіді: %2'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получена информация о доступных исправлениях (патчах) конфигураций.
                |%1'
                |;uk='Отримано інформацію про доступні виправлення (патчі) конфігурацій.
                |%1'"),
			РезультатОтправки.Содержимое));
	...
	Пока ЧтениеJSONПрочитать(ЧтениеОтвета, ТекущийУровень) Цикл
		...
		Если ЧтениеОтвета.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства И ТекущийУровень = 1 Тогда
			...
			ИначеЕсли ИмяСвойства = "errorMessage" Тогда
				...
				Результат.ПодробноеОписаниеОшибки = ОбработатьСообщение(Результат.ПодробноеОписаниеОшибки); 
	...
	Пока ЧтениеJSONПрочитать(ЧтениеОтвета, ТекущийУровень) Цикл
		...
		ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства И ТекущийУровень = 2 Тогда
			...
			Если ИмяСвойства = "ueid" Тогда
				...
				Исключение
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Некорректный идентификатор исправления в patchUpdateList[].ueid (%1)';uk='Некоректний ідентифікатор виправлення в patchUpdateList[].ueid (%1)'"),
						ЗначениеСвойства);
					ВызватьИсключение ТекстИсключения;
				КонецПопытки;
...
Функция ОписаниеФайловИсправлений(ИдентификаторыИсправлений, ДанныеАутентификации, ПроверитьДоступностьСервиса, ПараметрыКлиента)
		...
		Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
			...
			Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить информацию о файлах исправлений (патчах).
                    ...
                    |Причина:
                    |%2'
                    |;uk='Не вдалося отримати інформацію про файли виправлень (патчі).
                    |Не вдалося перевірити доступність сервісу автоматичного оновлення програми: %1.
                    |Причина:
                    |%2'"),
				URLОперацииPing,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение информации о файлах исправлений (патчей) (%1).';uk='Отримання інформації про файли виправлень (патчів) (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Логин:
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при получении информации о файлах исправлений (патчах).
                |%1'
                |;uk='Помилка при отриманні інформації про файли виправлень (патчі).
                |%1'"),
			РезультатОтправки.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(Результат.ИнформацияОбОшибке);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получена информация о файлах исправлений (патчах).
                |%1'
                |;uk='Отримано інформацію про файли виправлень (патчів).
                |%1'"),
			РезультатОтправки.Содержимое));
...
Функция СлужебнаяЗагрузитьИсправления(ИдентификаторыИсправлений, ИдентификаторФормы = Неопределено, ПроверитьДоступностьСервиса = Истина)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина, Истина) Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки   = НСтр("ru='Недостаточно прав.';uk='Недостатньо прав.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Недостаточно прав для получения файлов исправлений (патчей).';uk='Недостатньо прав для отримання файлів виправлень (патчів).'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файлы исправлений (патчей).
                |%1'
                |;uk='Не вдалося отримати файли виправлень (патчів).
                |%1'"),
			Результат.ПодробноеОписаниеОшибки));
		Возврат Результат;
	КонецЕсли;
	...
	Если ДанныеАутентификации = Неопределено Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки   = НСтр("ru='Интернет-поддержка пользователей не подключена.';uk='Інтернет-підтримка користувачів не підключена.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Интернет-поддержка пользователей не подключена.';uk='Інтернет-підтримка користувачів не підключена.'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файлы исправлений (патчей).
                |%1'
                |;uk='Не вдалося отримати файли виправлень (патчів).
	...
	Для Каждого ТекИсправление Из РезультатПолученияОписанияФайлов.Исправления Цикл
		...
		Если РезультатПолучения.Ошибка Тогда
			...
			Результат.Ошибка = Истина;
			Результат.КраткоеОписаниеОшибки =
				НСтр("ru='Не удалось получить файл исправления (патча).';uk='Не вдалося отримати файл виправлення (патча).'") + Символы.ПС + РезультатПолучения.КраткоеОписаниеОшибки;
			Результат.ПодробноеОписаниеОшибки =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось загрузить файл исправления (патча) с идентификатором %1.
                        |%2'
                        |;uk='Не вдалося завантажити файл виправлення (патча) з ідентифікатором %1.
                        |%2'"),
					ТекИсправление.Идентификатор,
					РезультатПолучения.ИнформацияОбОшибке);
			Возврат Результат;
...
Функция ЗагрузитьФайлИсправления(URLФайла, Идентификатор, ДанныеАутентификации) Экспорт
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки =
			НСтр("ru='Не удалось получить файл исправления (патча).';uk='Не вдалося отримати файл виправлення (патча).'") + Символы.ПС + РезультатПолучения.СообщениеОбОшибке;
		Результат.ПодробноеОписаниеОшибки =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось загрузить файл исправления (патча) с идентификатором %1.
                    |%2'
		...
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл исправления (патча).
                ...
                |URL: %2;
                |%3'
                |;uk='Не вдалося отримати файл виправлення (патча).
                |Ідентифікатор: %1;
                |URL: %2;
                |%3'"),
			Идентификатор,
...
Процедура СохранитьИнформациюОДоступномОбновленииВНастройках(Знач ИнформацияОДоступномОбновлении) Экспорт
	...
	Если ТипЗнч(ИнформацияОДоступномОбновлении) = Тип("Структура") Тогда
		...
		ОписаниеИнформацииОДоступномОбновлении.Вставить("ВерсияПлатформы",
			ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы());
...
Процедура ДобавитьОтключенноеДело(ТекущиеДела, ИдентификаторДела, Разделы)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.ЕстьДела       = Ложь;
		Дело.Важное         = Ложь;
		Дело.Представление  = НСтр("ru='Доступно обновление программы';uk='Доступне оновлення програми'");
		Дело.Форма          = "Обработка.ОбновлениеПрограммы.Форма.Форма";
		Дело.Владелец       = Раздел;
	КонецЦикла;
