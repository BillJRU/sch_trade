...
Функция ОтработатьПолучениеФайла(Сообщение) Экспорт
	...
	Событие = Событие + НСтр("ru='Запрос на передачу файла';uk='Запит на передачу файлу'",КодЯзыка); 
    ...
    |%5
    |
    |Відповідь прикладного обробника: %6'",КодЯзыка); 
...
Функция ОтработатьКвитанциюПередачиФайла(Сообщение) Экспорт
	...
	Если ТелоСообщения.Error Тогда
		...
		Ответ = РаспределенноеВыполнениеКомандПереопределяемый.ОбработатьОшибкуПередачиФайла(ИдентификаторВызова, 
 		...
 		Событие = Событие + НСтр("ru='Ошибка передачи файла';uk='Помилка передачі файлу'",КодЯзыка); 
	...
	Иначе
		...
		Событие = Событие + НСтр("ru='Квитанция о передаче файла';uk='Квитанція про передачу файлу'",КодЯзыка); 
        ...
        |CallCode: %4
        |
        |Відповідь прикладного обробника: %5'",КодЯзыка); 
...
Функция ОтработатьПрямойВызов(Сообщение) Экспорт
	...
	Событие = РаспределенноеВыполнениеКоманд.ИмяСобытияЖурналаРегистрации() + ".";
	Событие = Событие + НСтр("ru='Задание поставлено в очередь';uk='Завдання поставлено в чергу'",КодЯзыка); 
