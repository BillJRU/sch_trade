...
Процедура ВыгрузитьКлиентовДляОбмена(ЗаписьXML, УзелОбмена, ДопСвойства, ВыгружатьПолноеДосье = Ложь) Экспорт
	...
	ЗаголовокЗаписи = НСтр("ru='Обмен партнерами.Начало пакета выгрузки партнеров';uk='Обмін партнерами. Початок пакета вивантаження партнерів'",КодЯзыка);
	ТекстСообщения = СтрШаблон(НСтр("ru='Количество строк клиентов на начало: %1';uk='Кількість рядків клієнтів на початок: %1'",КодЯзыка), Формат(ТаблицаИзмененных.Количество(), "ЧГ="));
	...
	ЗаголовокЗаписи = НСтр("ru='Обмен партнерами.Конец пакета выгрузки партнеров';uk='Обмін партнерами. Кінець пакета вивантаження партнерів'",КодЯзыка);
	ТекстСообщения = СтрШаблон(НСтр("ru='Количество строк клиентов на конец: %1';uk='Кількість рядків клієнтів на кінець: %1'",КодЯзыка), Формат(ТаблицаИзмененных.Количество(), "ЧГ="));
	ЗаписьЖурналаРегистрации(ЗаголовокЗаписи, УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
КонецПроцедуры
...
Процедура ВыгрузитьПрайсЛистДляОбмена(ЗаписьXML, СообщениеОбмена, УзелОбмена, НомерСообщения, ВсеЦены, ДопСвойства) Экспорт
	...
	Если ТаблицаПрайсЛиста.Количество() = 0 Тогда
		...
		СообщениеОбмена = НСтр("ru='Нет доступных цен.';uk='Немає доступних цін.'",ОбщегоНазначения.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
	ВремяНачалаОбмена = ТекущаяДатаСеанса();
	ТребуетсяПродолжение = Ложь;
...
Процедура СоздатьОбновитьДоговорСоглашение(МобильныйДоговор, Сообщение) Экспорт
	...
	Иначе
		...
		Если ВСоглашенииИспользуютсяДоговоры Тогда
			...
			ДоговорОбъект.ОплатаВВалюте = Соглашение.ОплатаВВалюте;
...
Процедура СоздатьОбновитьЗаявкуНаВозвратТоваровОтКлиента(МобильныеДанные, Сообщение) Экспорт
	...
	Если СозданНаОсновании Тогда
		ТекстКомментария = СтрШаблон(НСтр("ru='<Создан по документу: %1/>';uk='<Створений за документом: %1/>'",ОбщегоНазначения.КодОсновногоЯзыка()), СокрЛП(ДокументОснование));
...
//
// Возвращаемое значение:
//  ПланОбменаСсылка - Узел обмена "Мобильное приложение "Заказы клиентов"".
//
...
|;uk='Синхронізація із пристроєм не налаштована. 
|Не вдалося знайти вузол обміну для користувача: %Пользователь%'",ОбщегоНазначения.КодОсновногоЯзыка());
...
|зверніться до адміністратора вашої ІБ для відкріплення настройки  іншого користувача від цього пристрою.'",ОбщегоНазначения.КодОсновногоЯзыка());
			СтруктураОтвета.ResultMessage = ТекстОтвета;
			СтруктураОтвета.Success = Ложь;
...
//
// Параметры:
//  Ссылка - ПланОбменаСсылка - Узел обмена "Мобильное приложение "Заказы"";
...
Процедура ЗаписатьКонтактнуюИнформациюСправочника(ИнформацияПоОбъекту, Знач ТаблицаКИ, Знач ИмяСправочника)
	...
	ИмяСправочника = СтрЗаменить(ИмяСправочника, НСтр("ru='Партнер';uk='Партнер'"), НСтр("ru='Клиент';uk='Клієнт'"));
	...
	Для Каждого СтрокаКИ Из ТаблицаКИ Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКИ.Тип) Тогда
			ТипКИ = Перечисления.ТипыКонтактнойИнформации.Индекс(СтрокаКИ.Тип);
			ОбъектКИ = Метаданные.Перечисления.ТипыКонтактнойИнформации.ЗначенияПеречисления.Получить(ТипКИ);//ОбъектМетаданныхПеречисление - 
			ТипКИСтрокой = ОбъектКИ.Имя;
		КонецЕсли;
...
Процедура ЗаписатьСправочник(ЗаписьXML, Знач СтруктураСправочника, Знач ИмяСправочника)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обмен мобильное приложение Заказы.Запись справочника';uk='Обмін мобільна прикладна програма Замовлення. Запис довідника'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаписатьВидКИСтроки(ЗаписьXML, Знач СтрокаДанных, ВыгруженныеОбъекты)
	...
	СтруктураВидаКИ.ТолькоНациональныйАдрес = СтрокаДанных.ВидКИТолькоНациональныйАдрес;
	...
	Если ЗначениеЗаполнено(СтрокаДанных.ВидКИТип) Тогда
		ТипКИ = Перечисления.ТипыКонтактнойИнформации.Индекс(СтрокаДанных.ВидКИТип);
		ОбъектКИ = Метаданные.Перечисления.ТипыКонтактнойИнформации.ЗначенияПеречисления.Получить(ТипКИ);//ОбъектМетаданныхПеречисление -
		ТипКИСтрокой = ОбъектКИ.Имя;
	КонецЕсли;
...
Процедура ЗаписатьВидЦеныСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	ИнформацияНДС = СтрНайти(СтрокаДанных.ВидЦеныНаименование, НСтр("ru='НДС';uk='ПДВ'",КодОсновногоЯзыка));
	Наименование = СтрокаДанных.ВидЦеныНаименование;
	
	Если ИнформацияНДС = 0 Тогда
		Если СтруктураВидаЦены.ЦенаВключаетНДС Тогда
			ТекстПроНДС = НСтр("ru='(с НДС)';uk='(з ПДВ)'",КодОсновногоЯзыка);
		Иначе
			ТекстПроНДС = НСтр("ru='(без НДС)';uk='(без ПДВ)'",КодОсновногоЯзыка);
		КонецЕсли;
...
Процедура ЗаписатьОрганизациюСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	Если ЗначениеЗаполнено(СтрокаДанных.ОрганизацияСистемаНалогообложенияСсылка) Тогда
		СистемаНалогообложенияСсылка =
			Перечисления.СистемыНалогообложения.Индекс(СтрокаДанных.ОрганизацияСистемаНалогообложенияСсылка);
		СистемаНалогообложенияОбъект = 
			Метаданные.Перечисления.СистемыНалогообложения.ЗначенияПеречисления.Получить(СистемаНалогообложенияСсылка);//ОбъектМетаданныхПеречисление - 
		СистемаНалогообложенияСтрокой = СистемаНалогообложенияОбъект.Имя;
	КонецЕсли;
	СтруктураОрганизации.СистемаНалогообложения = СистемаНалогообложенияСтрокой;
	
	СтруктураОрганизации.КодПоЕДРПОУ = СтрокаДанных.ОрганизацияКодПоЕДРПОУ;
	...
	Если ЗначениеЗаполнено(СтрокаДанных.ОрганизацияЮрФизЛицо) Тогда
		ЮрФизЛицоСсылка = Перечисления.ЮрФизЛицо.Индекс(СтрокаДанных.ОрганизацияЮрФизЛицо);
		ЮрФизЛицоОбъект = Метаданные.Перечисления.ЮрФизЛицо.ЗначенияПеречисления.Получить(ЮрФизЛицоСсылка);//ОбъектМетаданныхПеречисление - 
		ЮрФизЛицоСтрокой = ЮрФизЛицоОбъект.Имя;
	КонецЕсли;
...
Процедура ЗаписатьСостоянияЗаявкиНаВозврат(ЗаписьXML, ИдентификаторМобильногоУстройства)
	...
	Если ВыгружатьСостояниеЗаявок Тогда
		...
		Для Каждого Значение Из Перечисления.СостоянияЗаявокНаВозвратТоваровОтКлиентов Цикл
			...
			СтруктураСостояния.Вставить("ERPКод", СостояниеЗаявкиОбъект.Имя);
...
Функция КомпоновщикНастроекВыгрузкиКлиентов(Узел)
	...
	ПараметрСКД.Значение = НСтр("ru='Клиент входит в сегмент партнеров, которым отгрузка запрещена.';uk='Клієнт входить до сегменту партнерів, яким відвантаження заборонено.'",КодОсновногоЯзыка);
	ПараметрСКД.Использование = Истина;
	
	ПараметрСКД = КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ТекстПартнерПометкаУдаления");
	ПараметрСКД.Значение = НСтр("ru='Клиент помечен на удаление.';uk='Клієнт позначений для вилучення.'",КодОсновногоЯзыка);
	ПараметрСКД.Использование = Истина;
	...
	ПараметрСКД.Значение = НСтр("ru='Типовой договор';uk='Типовий договір'",КодОсновногоЯзыка);
	ПараметрСКД.Использование = Истина;
...
Процедура ВыгрузитьКонтактныхЛицКлиента(ЗаписьXML, Клиент, УдалениеДанных)
	...
	Для каждого СтрокаДанных Из Результат Цикл
		...
		ЗаписатьКонтактнуюИнформацию(СтруктураКонтактногоЛица, СтрокаДанных.КонтактноеЛицо, "КонтактныеЛицаПартнеров");
...
Процедура ЗаписатьЗаданиеВПакетОбмена(ЗаписьXML, Данные, ИспользоватьСоглашенияИлиДоговоры)
	...
	Если Данные.Задачи.Количество() > 0 Тогда
		Для Каждого ЭлементСписка Из Данные.Задачи Цикл
			...
			ТабличнаяЧастьОбъекта = МобильноеЗадание.МобильноеЗадание;//СписокXDTO - 
...
Процедура ЗаполнитьИнформациюПоДоставке(ДокументОбъект, МобильныеДанные, ТипДокумента)
	...
	Иначе
		ДокументОбъект.АдресДоставки = НСтр("ru='<адрес не указан/>';uk='<адреса не вказана/>'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ЗаполнитьСправочникСоглашенияПриИзменении(СоглашениеОбъект)
	...
	СоглашениеОбъект.ГруппаФинансовогоУчета = 
		Справочники.ГруппыФинансовогоУчетаРасчетов.ПолучитьГруппуФинансовогоУчетаПоУмолчанию(СоглашениеОбъект.ХозяйственнаяОперация);
...
Функция ЗаменитьПартнераНаКлиента(Знач Строка)
	...
	КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	СтрокаЗамены = Строка;
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru='Партнер';uk='Партнер'",КодОсновногоЯзыка),
		НСтр("ru='Клиент';uk='Клієнт'",КодОсновногоЯзыка));
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru='партнер';uk='партнер'",КодОсновногоЯзыка),
		НСтр("ru='клиент';uk='клієнт'",КодОсновногоЯзыка));
...
Функция ИнформацияОКонтрагенте(Знач СтрокаТаблицы)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.КонтрагентЮрФизЛицо) Тогда
		ЮрФизЛицоСсылка = Перечисления.ЮрФизЛицо.Индекс(СтрокаТаблицы.КонтрагентЮрФизЛицо);
		ЮрФизЛицоОбъект = Метаданные.Перечисления.ЮрФизЛицо.ЗначенияПеречисления.Получить(ЮрФизЛицоСсылка);//ОбъектМетаданныхПеречисление - 
		ЮрФизЛицо = ЮрФизЛицоОбъект.Имя;
	КонецЕсли;
...
Функция ИнформацияОПартнере(Знач СтрокаТаблицы, ВыгружатьПолноеДосье)
	...
	КонецЕсли;
	СтруктураПартнера.ФормаОплаты = ФормаОплатыСтрокой;
...
Функция ИнформацияПоСтатусамЗаказовКлиентов(ЗаказыКлиентов)
	...
	Пока ВыборкаДокументов.Следующий()Цикл
		...
		Если ЗначениеЗаполнено(ЗаказКлиентаИБ.Статус) Тогда
			СтатусСсылка = Перечисления.СтатусыЗаказовКлиентов.Индекс(ЗаказКлиентаИБ.Статус);
			СтатусОбъект = Метаданные.Перечисления.СтатусыЗаказовКлиентов.ЗначенияПеречисления.Получить(СтатусСсылка);//ОбъектМетаданныхПеречисление - 
			СтатусСтрокой = СтатусОбъект.Имя;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СостояниеСсылка) Тогда
			СостояниеИндекс = Перечисления.СостоянияЗаказовКлиентов.Индекс(СостояниеСсылка);
			СостояниеОбъект =
				Метаданные.Перечисления.СостоянияЗаказовКлиентов.ЗначенияПеречисления.Получить(СостояниеИндекс);//ОбъектМетаданныхПеречисление -
			СостояниеСтрокой = СостояниеОбъект.Имя;
		КонецЕсли;
...
Функция ИнформацияПоСтатусамЗаявокНаВозвратТоваров(ЗаявкиНаВозврат)
	...
	Пока ВыборкаДокументов.Следующий()Цикл
		...
		Если ЗначениеЗаполнено(ЗаявкаНаВозвратИБ.Статус) Тогда
			...
			СтатусОбъект =
				Метаданные.Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.ЗначенияПеречисления.Получить(СтатусИндекс);//ОбъектМетаданныхПеречисление - 
			СтатусСтрокой = СтатусОбъект.Имя;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СостояниеЗаявки.Состояние) Тогда
			...
			СостояниеОбъект =
				Метаданные.Перечисления.СостоянияЗаявокНаВозвратТоваровОтКлиентов.ЗначенияПеречисления.Получить(СостояниеИндекс);//ОбъектМетаданныхПеречисление - 
			СостояниеСтрокой = СостояниеОбъект.Имя;
		КонецЕсли;
...
Функция СоглашениеСКлиентомПоОрганизацииИВидуЦен(СтрокаТаблицы)
	...
	СтрокаСоглашения.Description = СтрШаблон(НСтр("ru='%1 (%2)';uk='%1 (%2)'"),
	...
	СтрокаСоглашения.Пояснение = СтрШаблон(НСтр("ru='%1 (%2, %3, %4)';uk='%1 (%2, %3, %4)'"),
		ПредставлениеДоговора, СокрЛП(СтрокаТаблицы.КонтрагентСсылка),
	...
	ФормаОплатыСтрокой = "";
	Если ЗначениеЗаполнено(СтрокаТаблицы.ПартнерФормаОплатыСсылка) Тогда
		ФормаОплатыСсылка = Перечисления.ФормыОплаты.Индекс(СтрокаТаблицы.ПартнерФормаОплатыСсылка);
		ФормаОплатыОбъект = Метаданные.Перечисления.ФормыОплаты.ЗначенияПеречисления.Получить(ФормаОплатыСсылка);//ОбъектМетаданныхПеречисление - 
		ФормаОплатыСтрокой = ФормаОплатыОбъект.Имя;
	КонецЕсли;
	СтрокаСоглашения.ФормаОплаты = ФормаОплатыСтрокой;
...
Процедура ЗаписатьДополнительныеРеквизитыОбъекта(ИнформацияПоОбъекту, Знач ДопРеквизиты, Знач ЛокальноеИмяОбъекта)
	...
	ЛокальноеИмяОбъекта = СтрЗаменить(ЛокальноеИмяОбъекта, НСтр("ru='Партнер';uk='Партнер'"), НСтр("ru='Клиент';uk='Клієнт'"));
