...
Функция ПараметрыПодбораПользователейВариантаОтчета(Форма, ПодборГруппВнешнихПользователей = Ложь)
	...
	ПараметрыПодбора.Вставить("ЗаголовокФормыПодбора", НСтр("ru='Подбор пользователей варианта отчета';uk='Підбір користувачів варіанту звіту'"));
...
Процедура ОбновитьВариантОтчетаИзФайла(ОписаниеФайла, СвойстваВариантаОтчетаОснования) Экспорт 
	...
	Если СвойстваВариантаОтчетаОснования.ИмяОтчета = СвойстваВариантОтчета.ИмяОтчета Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Вариант отчета обновлен из файла';uk='Варіант звіту оновлений з файлу'"));
	...
	ИначеЕсли СвойстваВариантаОтчетаОснования.ПредставлениеВарианта = СвойстваВариантОтчета.ПредставлениеВарианта Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Вариант отчета найден и обновлен из файла';uk='Варіант звіту знайдений і оновлений з файлу'"));
	...
	Иначе
		...
		ШаблонТекстаВопроса = НСтр("ru='Выбраны настройки варианта отчета ""%1"",
            ...
            |;uk='Обрані настройки варіанту звіту ""%1"",
            |які не збігаються з ""%2"".
            |Заміна настройок для вибраного варіанту звіту неможлива.
            |
            |Створити новий варіант звіту (або оновити існуючий при наявності)?'");
...
Процедура ПоделитьсяПользовательскимиНастройками(ОписаниеНастроек) Экспорт 
	...
	Если ОписаниеНастроек.Настройки.Элементы.Количество() = 0 Тогда 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Настройки (пользовательские) не установлены.';uk='Настройки (користувацькі) не встановлені.'"));
		Возврат;
	...
	ПараметрыПодбора.Вставить("ЗаголовокФормыПодбора", НСтр("ru='Поделиться настройками отчета с пользователями';uk='Поділитися настройками звіту з користувачами'"));
	ПараметрыПодбора.Вставить("ЗаголовокКнопкиЗавершенияПодбора", НСтр("ru='Поделиться';uk='Поділитися'"));
...
Процедура ПоделитьсяПользовательскимиНастройкамиПослеВыбораПользователей(ВыбранныеПользователи, ОписаниеНастроек) Экспорт 
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Настройки переданы';uk='Настройки передані'"),, Пояснение);
