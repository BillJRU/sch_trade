...
Функция БезопаснаяСтрокаКоманды(КомандаЗапуска) Экспорт
		...
		Если СодержитНебезопасныеДействия(КомандаЗапуска) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось запустить программу
                           ...
                           |з причини:
                           |Неприпустимий рядок команди
                           |%1
                           |з причини:
                           |Рядок команди не повинен містити символи: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(КомандаЗапуска) = Тип("Массив") Тогда
		...
		Если КомандаЗапуска.Количество() > 0 Тогда 
			...
			Если СодержитНебезопасныеДействия(КомандаЗапуска[0]) Тогда
				...
				НСтр("ru='Не удалось запустить программу
                           ...
                           |;uk='Не вдалося запустити програму
                           |через:
                           |Неприпустима команда або шлях до виконуваного файлу
                           |%1
                           |через:
                           |Команда не повинна містити символи: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска[0]);
			КонецЕсли;
		...
		Иначе
			ВызватьИсключение
				НСтр("ru='Ожидалось, что первый элемент массива КомандаЗапуска будет командой или путем к исполняемому файлу.';uk='Очікувалося, що перший елемент масиву КомандаЗапуска буде командою або шляхом до виконуваного файлу.'");
		КонецЕсли;
		
	Иначе 
		ВызватьИсключение 
			НСтр("ru='Ожидалось, что значение КомандаЗапуска будет <Строка> или <Массив>';uk='Очікувалося, що значення КомандаЗапуска буде <Рядок> або <Масив>'");
	КонецЕсли;
...
#Если МобильныйКлиент Тогда
	...
	СтрокаСообщенияОбОшибке = НСтр("ru='Подключение к другой программе не доступно в мобильном клиенте.';uk='Підключення до іншої програми не доступно в мобільному клієнті.'");
...
#Иначе
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не удалось подключится к другой программе: %1';uk='Не вдалося підключитися до іншої програми: %1'");
