...
Процедура ЗаполнитьПроизводственныйКалендарьВФорме(Форма, ПутьРеквизита) Экспорт
	...
	Иначе
		...
		Если ИспользуемыеКалендари.Количество() > 0 Тогда
			Календарь =  ИспользуемыеКалендари[0];
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеКлассификатора() Экспорт
	...
	Если ДанныеФайлов = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Не удалось получить данные календаря.
                               ...
                               |Класифікатори не підтримуються або підсистема «Робота з класифікаторами» відсутня.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ДанныеФайлов.КодОшибки) Тогда
		ИмяСобытия = НСтр("ru='Календарные графики.Получение файла классификатора';uk='Календарні графіки.Отримання файлу класифікатора'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьНачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("ru='Календарные графики.Обработка файла классификатора';uk='Календарні графіки.Обробка файлу класифікатора'",ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
