		...
		Если СертификатИздателя = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В сертификатах подписи не удалось найти сертификат издателя
                           |%1
                           |сертификата %2'
                           |;uk='У сертифікатах підпису не вдалося знайти сертифікат видавця
                           |%1
                           |сертифіката %2'"),
...
Процедура ЗарегистрироватьПодписаниеДанныхВЖурнале(ЭлементДанных, ОписаниеОшибки = "") Экспорт
	...
	Если ЭтоОшибкаПодписания Тогда
		ИмяСобытия = НСтр("ru='Электронная подпись.Ошибка подписания данных';uk='Електронний підпис.Помилка підписання даних'",ОбщегоНазначения.КодОсновногоЯзыка());
		
		СообщениеЖурналаРегистрации = СообщениеЖурналаРегистрации + "
		|
		|" + ОписаниеОшибки;
	Иначе
		ИмяСобытия = НСтр("ru='Электронная подпись.Подписание данных';uk='Електронний підпис.Підписання даних'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ЗаписатьСертификатПослеПроверки(Контекст) Экспорт
	...
	Для каждого ОписаниеПрограммы Из ОписанияПрограмм Цикл
		...
		Исключение
			...
			Контекст.ЗаголовокФормы = НСтр("ru='Ошибка добавления сертификата';uk='Помилка додавання сертифіката'");
			Контекст.ЗаголовокОшибки = НСтр("ru='Не удалось добавить сертификат по причине:';uk='Не вдалося додати сертифікат через:'");
...
Функция СертификатИзДвоичныхДанных(ДанныеСертификата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись.Удаление временного файла';uk='Електронний підпис.Вилучення тимчасового файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура НастроитьПояснениеВводаПароля(Форма, ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча = "", ИмяЭлементаПояснениеУсиленногоПароля = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча) Тогда
		...
		Элемент.Заголовок = НСтр("ru='Вводить пароль в программе электронной подписи';uk='Вводити пароль в програмі електронного підпису'");
		Элемент.Подсказка =
			НСтр("ru='- Включается интерактивный режим работы программы электронной подписи,
                       ...
                       |- Отключается запрос пароля в форме платформы BAF.
                       |
                       ...
                       |- Відключається запит пароля в формі платформи BAF.
                       |
                       |Обов''язково для закритих ключів сертифікатів, для яких в ОС включений посилений захист.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяЭлементаПояснениеУсиленногоПароля) Тогда
		...
		Элемент.Подсказка =
			НСтр("ru='Пароль запрашивает программа электронной подписи, а не программа BAS,
                       |так как для выбранного сертификата включен режим ""Вводить пароль в программе электронной подписи"".'
                       |;uk='Пароль запитує програма електронного підпису, а не програма BAS,
                       |так як для вибраного сертифіката включений режим ""Вводити пароль в програмі електронного підпису"".'");
	КонецЕсли;
...
Функция OIDАлгоритмаПодписи(ОбъектКомпоненты, СертификатКриптографииBase64)
	...
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';uk='Помилка виклику методу GetSignOIDFromCert компоненти XMLDSig.'")
...
Функция РезультатHash(ОбъектКомпоненты, КаноникализированныйТекстXML, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		АтрибутDigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Hash компоненты XMLDSig.';uk='Помилка виклику методу Hash компоненти XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция РезультатSign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo,
	...
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Sign компоненты XMLDSig.';uk='Помилка виклику методу Sign компоненти XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция РезультатVerifySign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo,
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';uk='Помилка виклику методу VerifySign компоненти XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция РезультатCMSSign(ОбъектКомпоненты, ДанныеДляПодписания, ПараметрыCMS, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода CMSSign компоненты XMLDSig.';uk='Помилка виклику методу CMSSign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если Не ЗначениеЗаполнено(АтрибутSignatureValue) Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода CMSSign компоненты XMLDSig.';uk='Помилка виклику методу CMSSign компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция РезультатCMSVerifySign(ОбъектКомпоненты, Подпись, Данные, ПараметрыCMS, ТипКриптопровайдера)
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.CMSVerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода CMSVerifySign компоненты XMLDSig.';uk='Помилка виклику методу CMSVerifySign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если ПодписьВерна = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода CMSVerifySign компоненты XMLDSig.';uk='Помилка виклику методу CMSVerifySign компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция ПроверитьШифрованиеИРасшифровку(МенеджерКриптографии, ДвоичныеДанныеСертификата,
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		...
		ТекстТекущейОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%2'
            |;uk='%1:
            |Не вдалося пройти перевірку шифрування через:
            |%2'"),
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		...
		ТекстТекущейОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%2'
            |;uk='%1:
            |Не вдалося пройти перевірку розшифровки через: 
            |%2'"),
			ОписаниеПрограммы.Наименование,
			ПредставлениеОшибки);
...
Функция ПроверитьПодписание(МенеджерКриптографии, ДвоичныеДанныеСертификата,
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		...
		ТекстТекущейОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%2'
            |;uk='%1:
            |Не вдалося пройти перевірку підписання через:
            |%2'"),
			ОписаниеПрограммы.Наименование,
			ПредставлениеОшибки);
...
Функция АдресКлассификатораОшибок()
	...
	АдресКлассификатора.Вставить("АдресСервера", "downloads.bas-soft.eu");
...
Процедура ОбновитьКлассификатор()
	...
	Если ДанныеКлассификатора = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись.Обновление классификатора ошибок';uk='Електронний підпис.Оновлення класифікатора помилок'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Предупреждение,,,
			НСтр("ru='Не удалось обновить классификатор ошибок криптографии.';uk='Не вдалося оновити класифікатор помилок криптографії.'"));
...
Процедура ДополнитьТехническойИнформациейОСервере(ТехническаяИнформация) Экспорт
	...
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая()
		...
		Если Не ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере()
			...
			ТехническаяИнформация = ТехническаяИнформация + Символы.ПС
				+ НСтр("ru='Электронная подпись на сервере не используется.';uk='Електронний підпис на сервері не використовується.'") + Символы.ПС;
		Иначе
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		РежимРаботы = ?(ОбщегоНазначения.КлиентПодключенЧерезВебСервер(),
			НСтр("ru='Файловый через веб';uk='Файловий через веб'"), НСтр("ru='Файловый';uk='Файловий'"));
	Иначе
		РежимРаботы = НСтр("ru='Клиент-серверный';uk='Клієнт-Серверний'");
	КонецЕсли;
	
	ТехническаяИнформация = ТехническаяИнформация
		+ Символы.ПС + НСтр("ru='Режим работы информационной базы';uk='Режим роботи інформаційної бази'")+ " - " + РежимРаботы
		+ Символы.ПС + Символы.ПС + ТехническаяИнформацияОВерсияхПодсистем() + Символы.ПС
		+ НСтр("ru='Расширения';uk='Розширення'") + ":" + Символы.ПС + ТехническаяИнформацияОРасширениях();
	
КонецПроцедуры

Функция ТехническаяИнформацияОКомпьютере()
	
	Возврат НСтр("ru='Сервер:';uk='Сервер:'") + Символы.ПС
...
Функция ТехническаяИнформацияОПрограммах()
	...
	ДиагностическаяИнформация = Символы.ПС + НСтр("ru='Программы на сервере';uk='Програми на сервері'") + ":" + Символы.ПС;
	Для Каждого Программа Из ИспользуемыеПрограммы Цикл
		...
		ДиагностическаяИнформация = ДиагностическаяИнформация
			...
			"OK", НСтр("ru='Ошибка';uk='Помилка'") + " : " + ОписаниеОшибки) + Символы.ПС;
...
Функция ТехническаяИнформацияОВерсияхПодсистем()
	...
	ДиагностическаяИнформация = НСтр("ru='Версии подсистем';uk='Версії підсистем'") + ":" + Символы.ПС;
...
Функция ТехническаяИнформацияОРасширениях()
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		ПредставлениеРасширений = ПредставлениеРасширений
			...
			+ Формат(Расширение.Активно, НСтр("ru='БЛ=Отключено; БИ=Включено';uk='БЛ=Відключено; БИ=Включено'")) + Символы.ПС;
