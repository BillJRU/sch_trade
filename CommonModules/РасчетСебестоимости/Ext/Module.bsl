...
Процедура РассчитатьВсе(ПараметрыЗапуска, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	Если РасчетСебестоимостиЛокализация.РассчитатьВсе(ПараметрыЗапуска.Дата, ПараметрыЗапуска.МассивОрганизаций, ВыполняетсяЗакрытиеМесяца) Тогда
		Возврат;
	КонецЕсли;
	...
	Если СхемаРасчета.Количество() > 0 И НачалоМесяца(СхемаРасчета[0].Дата) < ДатаПереходаНаПартионныйУчетВерсии22 Тогда
		...
		РасчетСебестоимостиЛокализация.РассчитатьВсе(СтрокаСхемыРасчета.Дата, СтрокаСхемыРасчета.Организации, ВыполняетсяЗакрытиеМесяца);
	...
	Если СхемаРасчета.Количество() = 0 Тогда
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='За период ""%1"" по ""%2"" расчет партий версии 2.5 не требуется.';uk='За період ""%1"" по ""%2"" розрахунок партій версії 2.5 не потрібен.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ЗаписьЖурналаРегистрации(
			РасчетСебестоимостиПрикладныеАлгоритмы.ИмяСобытияЖурналаРегистрации(
				...
				НСтр("ru='Расчет не требуется';uk='Розрахунок не потрібний'",ОбщегоНазначения.КодОсновногоЯзыка())),
	...
	ИначеЕсли НачалоМесяца(СхемаРасчета[0].Дата) < ДатаПереходаНаПартионныйУчетВерсии22 Тогда
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='За период ""%1"" по ""%2"" расчет партий версии 2.5 невозможен - расчет партий версии 2.1 за предыдущие периоды выполнен с ошибками.';uk='За період ""%1"" по ""%2"" розрахунок партій версії 2.5 неможливий - розрахунок партій версії 2.1 за попередні періоди виконаний з помилками.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПараметрыЗапуска.Дата),
			РасчетСебестоимостиПрикладныеАлгоритмы.ПредставлениеОрганизаций(ПараметрыЗапуска.МассивОрганизаций, ", ", Истина));
		ЗаписьЖурналаРегистрации(
			РасчетСебестоимостиПрикладныеАлгоритмы.ИмяСобытияЖурналаРегистрации(
				Новый Структура("ЗапущенРасчетПартий", Истина),
				НСтр("ru='Расчет невозможен';uk='Розрахунок неможливий'",ОбщегоНазначения.КодОсновногоЯзыка())),
	...
	Для Каждого СтрокаСхемыРасчета Из СхемаРасчета Цикл
		...
		ПараметрыЗапускаРасчетаПериода.Вставить("РежимОтладки", РежимОтладки);
...
Функция РассчитатьФоновымЗаданием(Период, Организация = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Расчет партий и себестоимости';uk='Розрахунок партій і собівартості'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОписаниеПриемника.ЭтоОписаниеРегистра И НЕ ЗначениеЗаполнено(Запись.Регистратор) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДанныеДвижения.Регистратор) Тогда
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не заполнено свойство Регистратор в данных для формирования движений по регистру ""%1""';uk='Не заповнена властивість Реєстратор в даних для формування рухів за регістром ""%1""'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ТаблицаПриемник);
		Иначе
...
Функция ТекущаяВерсияМеханизма() Экспорт
    Возврат НСтр("ru='Партионный учет, версия 2.5';uk='Партіонний облік, версія 2.5'",ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Возврат; // нечего регистрировать - обновляется расписание регламентных заданий
	
КонецПроцедуры

// Обработчик обновления - заполняет расписание регламентного задания РасчетСебестоимости.
//
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	// Подготовим вспомогательные данные.
	РасписаниеПоУмолчанию = Новый РасписаниеРегламентногоЗадания; // пустое расписание
	НовоеРасписание 	  = Новый РасписаниеРегламентногоЗадания; // каждый день; с 1:00:00 по 7:00:00 один раз в день
	...
	Для ПорядковыйНомер = 1 По 12 Цикл
		...
		Месяцы.Добавить(ПорядковыйНомер);
		
		Если ПорядковыйНомер <= 7 Тогда
			ДниНедели.Добавить(ПорядковыйНомер);
		КонецЕсли;
		
	КонецЦикла;
	
	НовоеРасписание.Месяцы    		  = Месяцы;
	НовоеРасписание.ДниНедели 		  = ДниНедели;
	НовоеРасписание.ПериодПовтораДней = 1;
	НовоеРасписание.ВремяНачала 	  = Дата(1,1,1,1,0,0);
	НовоеРасписание.ВремяКонца 		  = Дата(1,1,1,7,0,0);
	
	ДанныеДляИзмененияЗадания = Новый Структура;
	ДанныеДляИзмененияЗадания.Вставить("Расписание", НовоеРасписание);
	
	// Получим задания.
	ОтборЗаданий = Новый Структура;
	ОтборЗаданий.Вставить("Метаданные", Метаданные.РегламентныеЗадания.РасчетСебестоимости);
	
	Задания = РегламентныеЗаданияСервер.НайтиЗадания(ОтборЗаданий);
	
	Для Каждого Задание Из Задания Цикл
		
		// Проверим необходимость изменения задания.
		Если Задание.Использование Тогда
			// По умолчанию задание в метаданных отключено.
			// Если пользователь включил задание, то изменять расписание не нужно.
			Параметры.ОбработкаЗавершена = Истина;
			Возврат;
		ИначеЕсли ТипЗнч(Задания) = Тип("Массив") И НЕ Задание.Предопределенное Тогда
			// В локальном режиме изменяем только предопределенное регламентное задание.
			Параметры.ОбработкаЗавершена = Истина;
			Возврат;
		ИначеЕсли СокрЛП(Задание.Расписание) <> СокрЛП(РасписаниеПоУмолчанию) Тогда
			// Расписание изменено пользователем.
			Параметры.ОбработкаЗавершена = Истина;
			Возврат;
		КонецЕсли;
