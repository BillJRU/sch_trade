...
Функция ОписаниеЦепочекПартийТоваров(ПараметрыРасчета) Экспорт
	...
	// Распределение <- (Перемещение, ПеремещениеОбособленно, Остаток, Партия, ПеремещениеОбособленно, ВозвратПрошлыхПериодов, Распределение)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
	...
	// СторноСписанияНаВыпуск <- (СписанияНаВыпуск)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
		Перечисления.ТипыЗаписейПартий.СторноСписанияНаВыпуск, "АналитикаУчетаНоменклатуры, Организация, ЗаказНаПроизводство");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.СторноСписанияНаВыпуск,
		Перечисления.ТипыЗаписейПартий.СписанияНаВыпуск, "АналитикаУчетаНоменклатуры, Организация, ЗаказНаПроизводство");
	...
	// ПотреблениеПроизводство <- (Партия, Перемещение, ПеремещениеОбособленно, Остаток, СторноСписанияНаВыпуск)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ПотреблениеПроизводство,
		Перечисления.ТипыЗаписейПартий.ПеремещениеОбособленно, "АналитикаУчетаНоменклатуры, Организация");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ПотреблениеПроизводство,
		Перечисления.ТипыЗаписейПартий.СторноСписанияНаВыпуск, "АналитикаУчетаНоменклатуры, Организация");
...
Функция ОписаниеДвиженийПартийТоваров(ПараметрыРасчета)
	...
	ОписаниеДвижений.Вставить("Показатели", "Знаменатель, Количество, Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС,
        |СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница,
    ...
    ОписаниеДвижений.Вставить("ПоляСортировки", "Партия, НалоговоеНазначение, АналитикаФинансовогоУчета");
...
Функция ТекстОписаниеДанныхДляПартийТоваров() Экспорт
	Возврат 
        ...
        |	Т.АналитикаУчетаПартий.НалоговоеНазначение 		КАК НалоговоеНазначениеПартии,
		|	Т.Период										КАК Период,
        ...
        |	Т.ПостатейныеПеременныеУпрБезНДС,
		|	Т.ДопРасходы,
		|	Т.ДопРасходыБезНДС,
		|	Т.Трудозатраты,
        ...
        |	Т.ПостатейныеПеременныеРеглБезНДС,
		|
        ...
        |	Т.КорНалоговоеНазначение КАК НалоговоеНазначениеДокумента,
		|	Т.НДСУпр,
        |	Т.НДСРеглРеквизит,
		|	Т.СтатьяКалькуляции,
		|	Т.ДокументИсточник,
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ЕстьДекларации, ВозвратныеОтходы
	Возврат 
		...
		|	(ВЫБОР
		//++ Локализация
        |		КОГДА НЕ Т.СлужебноеВидДвиженияПриход И Т.Регистратор ССЫЛКА Документ.КорректировкаНалоговогоНазначенияЗапасов ТОГДА 15
		//-- Локализация
		...
		|		КОГДА Т.ТипЗаписи В (&НепересчитываемыеТипыЗаписей)
		|		 ИЛИ Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
        |			ТОГДА Т.НалоговоеНазначение 
		|		КОГДА НЕ Т.СлужебноеВидДвиженияПриход
        |		 И ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.Назначение.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
        ...
        |			ТОГДА Т.КорАналитикаУчетаНоменклатуры.Назначение.НалоговоеНазначение 
		//++ Локализация
        |		КОГДА НЕ КорректировкаНДС.НалоговоеНазначение ЕСТЬ NULL И Т.СлужебноеВидДвиженияПриход
        |			ТОГДА КорректировкаНДС.НовоеНалоговоеНазначение
        |		КОГДА НЕ КорректировкаНДС.НалоговоеНазначение ЕСТЬ NULL И НЕ Т.СлужебноеВидДвиженияПриход
        |			ТОГДА КорректировкаНДС.НалоговоеНазначение
		//-- Локализация
        |		КОГДА НЕ ВнутреннееПотребление.НалоговоеНазначение ЕСТЬ NULL И НЕ Т.СлужебноеВидДвиженияПриход
        |			ТОГДА ВЫБОР
        |				КОГДА ВнутреннееПотребление.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |					ТОГДА ВнутреннееПотребление.НалоговоеНазначение
        |					ИНАЧЕ УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |				КОНЕЦ
        |		КОГДА НЕ Пересортица.НалоговоеНазначение ЕСТЬ NULL
        |			ТОГДА
        |				ВЫБОР КОГДА Пересортица.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        
        |					ТОГДА Пересортица.НалоговоеНазначение
        |					ИНАЧЕ УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |				КОНЕЦ
        |		КОГДА НЕ Порча.НалоговоеНазначение ЕСТЬ NULL
        |			ТОГДА
        |				ВЫБОР КОГДА Порча.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |					ТОГДА Порча.НалоговоеНазначение
        |					ИНАЧЕ УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |				КОНЕЦ
        ...
        |		КОГДА Т.КорНалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |		 ИЛИ Т.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВозвратПрошлыхПериодов)
        |			ТОГДА УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |		ИНАЧЕ Т.КорНалоговоеНазначение
        ...
        |			ТОГДА Т.КорАналитикаУчетаНоменклатуры.Назначение.НалоговоеНазначение
        |		КОГДА НЕ ВнутреннееПотребление.НалоговоеНазначение ЕСТЬ NULL И НЕ Т.СлужебноеВидДвиженияПриход
        |		 И ВнутреннееПотребление.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА ВнутреннееПотребление.НалоговоеНазначение
        ...
        |			ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |		ИНАЧЕ Т.КорНалоговоеНазначение
        |	КОНЕЦ КАК НалоговоеНазначениеДокумента,
		|	Т.НДСУпр,
        |	Т.НДСРеглРеквизит,
		|	Т.СтатьяКалькуляции,
		|	ВЫБОР
		...
		//++ Локализация
        |	ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаНалоговогоНазначенияЗапасов КАК КорректировкаНДС
...
Функция ТекстТекущиеЗакупкиПоИмпорту()
	Возврат 
        ...
        |	ДД.КорНалоговоеНазначение КАК НалоговоеНазначениеДокумента,
		|	ДД.НДСУпр,
        |	ДД.НДСРеглРеквизит,
...
Функция ТекстТекущиеДопРасходыБезПартииКРаспределению() // Использует Декларации
	Возврат 
        ...
        |	СУММА(ДД.ПостатейныеПостоянныеУпрБезНДС) КАК ПостатейныеПостоянныеУпрБезНДС,
        |	СУММА(ДД.ПостатейныеПеременныеУпрБезНДС) КАК ПостатейныеПеременныеУпрБезНДС,
        ...
        |	СУММА(ДД.ПостатейныеПостоянныеРеглБезНДС) КАК ПостатейныеПостоянныеРеглБезНДС,
        |	СУММА(ДД.ПостатейныеПеременныеРеглБезНДС) КАК ПостатейныеПеременныеРеглБезНДС,
		|
		|	ДД.ХозяйственнаяОперация,
		|	ДД.КорАналитикаУчетаНоменклатуры,
		|	ДД.КорРазделУчета,
...
Процедура ЗаполнитьРасчетнуюПартиюСебестоимостиТоваров(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
    ...
    ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "Партия, АналитикаУчетаПартий, НалоговоеНазначениеПартии, АналитикаФинансовогоУчета, ПериодПоступления");
	...
	Если РасчетнаяПартия.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.Перемещение
        ...
        Если РасчетнаяПартия.ТипЗаписи <> Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов
         ИЛИ Не ЗначениеЗаполнено(РасчетнаяПартия.НалоговоеНазначение) Тогда
	...
	Иначе
        ...
        Если ЗначениеЗаполнено(Приход.КорНалоговоеНазначение) Тогда
        	РасчетнаяПартия.НалоговоеНазначение = Приход.КорНалоговоеНазначение;
        Иначе
        	РасчетнаяПартия.НалоговоеНазначение = Приход.НалоговоеНазначение;
        КонецЕсли;
		Если Приход.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально 
			И РасчетнаяПартия.НалоговоеНазначение <> Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность
			Тогда
    ...
    Если РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Расход
    	И Не ЗначениеЗаполнено(РасчетнаяПартия.НалоговоеНазначениеДокумента) Тогда
    	РасчетнаяПартия.КорНалоговоеНазначение = Приход.НалоговоеНазначение;
    КонецЕсли;
	Если РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Расход 
		И Приход.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально 
		И РасчетнаяПартия.КорНалоговоеНазначение <> Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность
		Тогда
    ...
    Если (РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ДопРасходыБезПартии
     ...
     И РасчетнаяПартия.НалоговоеНазначение <> РасчетнаяПартия.КорНалоговоеНазначение Тогда
        ...
        Если РасчетСебестоимостиПовтИсп.УсловнаяПродажа(РасчетнаяПартия.НалоговоеНазначение, РасчетнаяПартия.КорНалоговоеНазначение) Тогда
            ВключениеИсключениеНДС.ДопРасходыРегл = РасчетнаяПартия.НДСРеглРеквизит;
            ВключениеИсключениеНДС.ДопРасходыУпр = ?(ПараметрыРасчета.УправленческийУчетОрганизаций, РасчетнаяПартия.НДСУпр, 0);
            ВключениеИсключениеНДС.ДопРасходыРеглБезНДС = РасчетнаяПартия.НДСРеглРеквизит;
            ВключениеИсключениеНДС.ДопРасходыУпрБезНДС = ?(ПараметрыРасчета.УправленческийУчетОрганизаций, РасчетнаяПартия.НДСУпр, 0);
        КонецЕсли;
        
        Если РасчетСебестоимостиПовтИсп.ВосстановлениеНДС(РасчетнаяПартия.НалоговоеНазначение, РасчетнаяПартия.КорНалоговоеНазначение) Тогда
            ВключениеИсключениеНДС.ДопРасходыРегл = -РасчетнаяПартия.НДСРеглРеквизит;
            ВключениеИсключениеНДС.ДопРасходыУпр = ?(ПараметрыРасчета.УправленческийУчетОрганизаций, -РасчетнаяПартия.НДСУпр, 0);
            ВключениеИсключениеНДС.ДопРасходыРеглБезНДС = -РасчетнаяПартия.НДСРеглРеквизит;
            ВключениеИсключениеНДС.ДопРасходыУпрБезНДС = ?(ПараметрыРасчета.УправленческийУчетОрганизаций, -РасчетнаяПартия.НДСУпр, 0);
    ...
    Если Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Тогда
		...
		Если ЗначениеЗаполнено(Приход.КорРазделУчета) Тогда
			...
			ЗаполнитьЗначенияСвойств(ОтклоненияНаСклад, Отклонения,
                "Период, Регистратор, ВидЗапасов, Организация, АналитикаФинансовогоУчета, НалоговоеНазначение, ХозяйственнаяОперация, ДокументИсточник,
                |НДСРеглРеквизит, НДСУпр, РасчетЗавершен");
            ...
            ОтклоненияНаСклад.ДопРасходыУпрБезНДС 	     = Отклонения.СтоимостьУпрБезНДС;
            ОтклоненияНаСклад.ДопРасходыРеглБезНДС 		 = Отклонения.СтоимостьРеглБезНДС;
            ОтклоненияНаСклад.НДСРегл 		             = Отклонения.НДСРегл;
...
Функция ТекстИнициализацияПартийВыручки()
	Возврат "
        ...
        |	СУММА(Т.ПостатейныеПостоянныеРеглБезНДС) КАК ПостатейныеПостоянныеРеглБезНДС,
        |	СУММА(Т.ПостатейныеПеременныеРеглБезНДС) КАК ПостатейныеПеременныеРеглБезНДС,
        ...
        |	СУММА(Т.ПостатейныеПостоянныеУпрБезНДС) КАК ПостатейныеПостоянныеУпрБезНДС,
        |	СУММА(Т.ПостатейныеПеременныеУпрБезНДС) КАК ПостатейныеПеременныеУпрБезНДС,
		|
        ...
        |	МИНИМУМ(Т.СтоимостьРеглБезНДС) КАК СтоимостьРеглБезНДС,
        |	МИНИМУМ(Т.НДСРегл) КАК НДСРегл,
        ...
        |	МИНИМУМ(Т.ДопРасходыРеглБезНДС) КАК ДопРасходыРеглБезНДС,
        |	МИНИМУМ(Т.ПостатейныеПостоянныеРеглБезНДС) КАК ПостатейныеПостоянныеРеглБезНДС,
        |	МИНИМУМ(Т.ПостатейныеПеременныеРеглБезНДС) КАК ПостатейныеПеременныеРеглБезНДС,
        ...
        |	МИНИМУМ(Т.СтоимостьУпрБезНДС) КАК СтоимостьУпрБезНДС,
        |	МИНИМУМ(Т.ДопРасходыУпрБезНДС) КАК ДопРасходыУпрБезНДС,
        |	МИНИМУМ(Т.ПостатейныеПостоянныеУпрБезНДС) КАК ПостатейныеПостоянныеУпрБезНДС,
        |	МИНИМУМ(Т.ПостатейныеПеременныеУпрБезНДС) КАК ПостатейныеПеременныеУпрБезНДС,
        |	Т.НалоговоеНазначениеСписания КАК НалоговоеНазначениеСписания,
...
Процедура ЗаполнитьРасчетнуюПартиюВыручкиИСебестоимостьПродаж(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	//	Заполняем партионную идентификацию в расчетной партии
    ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "Партия, АналитикаУчетаПартий, НалоговоеНазначение, АналитикаФинансовогоУчета");
