...
Функция СоздатьПисьмо(Сообщение, УчетнаяЗапись) Экспорт
	...
	Исключение
		...
		ШаблонТекстаСообщения = НСтр("ru='Не удалось сформировать письмо по причине:
            |%1'
            |;uk='Не вдалося сформувати лист через:
            |%1'");
	...
	Исключение
		...
		ШаблонТекстаСообщения = НСтр("ru='Не удалось отправить письмо по причине:
                |%1'
                |;uk='Не вдалося відправити лист через:
                |%1'");
	...
	Если НЕ Письмо.УдалятьПослеОтправки Тогда
		...
		Исключение
			...
			ШаблонТекстаСообщения = НСтр("ru='Не удалось сохранить письмо в программе после успешной отправки по причине:
                |%1'
                |;uk='Неможливо зберегти лист в програмі після успішного відправлення через: 
                |%1'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru='Технологический перенос данных идентификаторов сообщения IMAP (реквизит ИдентификаторСообщенияОтправкаIMAP) для предотвращения потери данных
                   ...
                   |;uk='Технологічне перенесення даних ідентифікаторів повідомлення IMAP (реквізит 
                   |ИдентификаторСообщенияОтправкаIMAP) для запобігання втрати даних 
                   |при переході на наступні релізи програми. Виконує дозаповнення реквізиту ВзаимодействиеОснование для вихідних листів, у яких лист підстава в ІБ є, але не вказано'");
...
Функция СписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоEmail", НСтр("ru='По email';uk='По email'"), Адрес);
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоДомену", НСтр("ru='По доменному имени';uk='За доменним ім''ям'"), ДоменныйАдрес);
	
	Если Не ДляАдреснойКниги И (Параметры.Свойство("ТолькоEmail") И Не Параметры.ТолькоEmail) Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоТелефону", НСтр("ru='По телефону';uk='По телефону'"), Адрес);
	КонецЕсли;
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "НачинаетсяС", НСтр("ru='Начинается с';uk='Починається з'"), Представление);
...
Функция ПолнотекстовыйПоискКонтактовПоСтроке(Знач СтрокаПоиска, НайденныеКонтакты, Знач ДляАдреснойКниги = Ложь) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Запретить
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПолнотекстовыйПоиск") Тогда
		Возврат НСтр("ru='Использование полнотекстового поиска данных запрещено.';uk='Використання повнотекстового пошуку даних заборонено.'");
	КонецЕсли;
	...
	Исключение
		Возврат НСтр("ru='Не удалось выполнить поиск, попробуйте изменить выражение поиска.';uk='Не вдалося виконати пошук, спробуйте змінити вираз пошуку.'");
	КонецПопытки;
...
Процедура УстановитьЗаголовокФормыЭлектронногоПисьма(Форма) Экспорт
	...
	Если НЕ ОбъектПисьмо.Ссылка.Пустая() Тогда
		...
		ЗаголовокФормы = ЗаголовокФормы                                                                  
		         + " " + ?(ТипЗнч(ОбъектПисьмо.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее"), НСтр("ru='(Входящее)';uk='(Вхідне)'"), НСтр("ru='(Исходящее)';uk='(Вихідне)'"));
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ЕстьКолонкиАдрес[ОписаниеКонтакта.Имя] Тогда
			...
			КолонкаАдрес.Заголовок = НСтр("ru='Адрес';uk='Адреса'");
		КонецЕсли;
...
Функция СформироватьЭлементДанныхШапкиПисьма(ЭлементРодитель, ШапкаПисьма, ТолькоПоПредставлениюОтправителя = Ложь)
	...
	Тема = ?(ПустаяСтрока(ШапкаПисьма.Тема), НСтр("ru='<Без Темы>';uk='<Без Теми>'"), ШапкаПисьма.Тема);
...
Процедура ДополнитьТелоПисьмаПодваломВложения(ДокументHTML, Вложения) Экспорт
	...
	СтрокаКоличествоВложений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вложений: %1.';uk='Вкладень: %1.'"), Вложения.Количество());
	ДобавитьТекстовыйУзел(ЭлементШрифт, СтрокаКоличествоВложений, Истина, Истина);
	
	Для Каждого Вложение Из Вложения Цикл 
		
		ПредставлениеВложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2).';uk='%1 (%2).'"), Вложение.ИмяФайла, Вложение.РазмерПредставление);
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані настройки відправки SMS.'"),,"Объект");
...
Процедура ОтправкаSMS() Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		...
		НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані настройки відправки SMS.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru='Не удалось отправить : %Ссылка% по причине: %Причина%';uk='Не вдалося відправити : %Ссылка% з причини: %Причина%'");
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru='Не удалось обновить статусы доставки : %Ссылка% по причине: %Причина%';uk='Не вдалося оновити статуси доставки: %Ссылка% з причини: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", ВыборкаДокументы.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
...
Процедура ВыполнитьУдалениеПапкиЭлектронныхПисем(Папка, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%1'
            |;uk='Вилучення папки не виконане через:
...
Процедура ЗаполнитьСписокВыбораВремени(ПолеВводаФормы, Интервал = 3600) Экспорт
	...
	Пока НачалоЧаса(ВремяСписка) <= НачалоЧаса(ОкончаниеРабочегоДня) Цикл
		...
		Иначе
			ПредставлениеВремени = Формат(ВремяСписка, НСтр("ru='ДФ=ЧЧ:мм';uk='ДФ=ЧЧ:мм'"));
		КонецЕсли;
...
Процедура ОбработатьПодменюОтборПоТипуВзаимодействий(Форма) Экспорт
	...
	ШаблонЗаголовка = НСтр("ru='Показывать: %1';uk='Показувати: %1'");
...
Функция КомментарийПоНаименованиюШаблона(НаименованиеШаблона)
	...
	Возврат НСтр("ru='Создано и отправлено по шаблону';uk='Створено і відправлено за шаблоном'") + " - " + НаименованиеШаблона;
...
Функция ПредставлениеПисьма(ТемаПисьма, ДатаПисьма) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2';uk=' %1 від %2'"), 
