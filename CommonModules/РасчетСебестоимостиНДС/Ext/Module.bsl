...
Процедура РаспределитьУсловныеПродажиПоНомерамГТД(ПараметрыРасчета)
	...
	|	ЕСТЬNULL(АналитикаНоменклатуры.Серия, НЕОПРЕДЕЛЕНО) КАК Серия,
	...
	|	ЕСТЬNULL(АналитикаНоменклатуры.Серия, НЕОПРЕДЕЛЕНО),
	...
	ТаблицаВидовЗапасов.Индексы.Добавить("Регистратор, Номенклатура, Характеристика, Серия");
	...
	|	ЕСТЬNULL(Данные.АналитикаУчетаНоменклатуры.Серия, НЕОПРЕДЕЛЕНО) КАК Серия,
	...
	|	ЕСТЬNULL(Данные.АналитикаУчетаНоменклатуры.Серия, НЕОПРЕДЕЛЕНО),
	|	Данные.СтавкаНДС,
	...
	ПараметрыОтбораВидовЗапасов = Новый Структура("Регистратор, Номенклатура, Характеристика, Серия");
	...
	Для Каждого СтрокаДанные Из ТаблицаДанные Цикл
		...
		ПараметрыОтбораВидовЗапасов.Серия 			= СтрокаДанные.Серия;
	...
	|	ТаблицаНовыеДанные.Серия КАК Серия,
...
|;uk='Виявлено некоректні рухи по регістру ""%1"" за період %2.
