...
Процедура ОтправитьЗагрузитьПочтуПользователяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Заголовок = НСтр("ru='Отправка и получение почты';uk='Відправлення та отримання пошти'");
		...
		Если РезультатВыполнения.ЕстьОшибки Тогда
			ПоказатьОповещениеПользователя(Заголовок, ДополнительныеПараметры.НавигационнаяСсылка, 
				НСтр("ru='Не удалось выполнить все действия. Технические подробности для администратора в журнале регистрации.';uk='Не вдалося виконати всі дії. Технічні подробиці для адміністратора в журналі реєстрації.'"), 
...
Функция РезультатОтправкиПолученияПисем(РезультатВыполнения)
	...
	Если РезультатВыполнения.ПолученоПисем > 0 И РезультатВыполнения.ОтправленоПисем > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получено: %1, отправлено: %2';uk='Отримано: %1, відправлено: %2'"), 
			РезультатВыполнения.ПолученоПисем, РезультатВыполнения.ОтправленоПисем);
	ИначеЕсли РезультатВыполнения.ПолученоПисем > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получено: %1';uk='Отримано: %1'"), 
			РезультатВыполнения.ПолученоПисем);
	ИначеЕсли РезультатВыполнения.ПолученоПисем > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отправлено: %1';uk='Відправлено: %1'"), 
			РезультатВыполнения.ОтправленоПисем);
	Иначе
		ТекстСообщения = НСтр("ru='Нет новых писем';uk='Немає нових повідомлень'");
	КонецЕсли;	
