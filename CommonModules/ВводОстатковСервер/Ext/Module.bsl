...
//
// Параметры:
// Документ                            - ДокументСсылка.ВводОстатков - Документ ввода остатков
// НомерДокумента             - Строка - Номер документа ВводОстатков
// ДатаДокумента                  - Дата - Дата документа ВводОстатков
// ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операции документа ввода остатков.
// Возвращаемое значение:
//  Строка - строковое представление  заголовка документа.
//
Функция ЗаголовокДокументаВводОстатковПоХозяйственнойОперации(Документ, НомерДокумента, ДатаДокумента, ХозяйственнаяОперация) Экспорт
	...
	ДанныеПоХозяйственнойОперации = ОписаниеРазделовВводаОстатков().Найти(ХозяйственнаяОперация, "ХозяйственнаяОперация");
	
	Если ДанныеПоХозяйственнойОперации = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	ШаблонЗаголовкаДокумента = ДанныеПоХозяйственнойОперации.ШаблонЗаголовкаДокумента;
	ОкончаниеЗаголовка       = "";
	
	Если ЗначениеЗаполнено(Документ.Ссылка) Тогда
		ОкончаниеЗаголовка = НСтр("ru='%1 от %2';uk=' %1 від %2'");
		ОкончаниеЗаголовка = СтрШаблон(ОкончаниеЗаголовка, НомерДокумента, ДатаДокумента);
	Иначе
		ОкончаниеЗаголовка = НСтр("ru='(создание)';uk='(створення)'");
	КонецЕсли;
	
	Возврат СтрШаблон(ШаблонЗаголовкаДокумента, ОкончаниеЗаголовка);
...
// 
// Возвращаемое значение:
// 	Структура - Описание: Структура содержащая описание раздела учета ввода остатков
...
// 
// Возвращаемое значение:
// 	Структура - Описание: Структура содержащая имена реквизитов документа ввода остатков и значения их заполнения 
...
Функция ИнициализироватьОписаниеХозяйственнойОперацииВводаОстатков() Экспорт
	...
	ОписаниеХозяйственнойОперацииВводаОстатков.Колонки.Добавить("ХозяйственнаяОперация",               Новый ОписаниеТипов("ПеречислениеСсылка.ХозяйственныеОперации"), НСтр("ru='Хозяйственная операция';uk='Господарська операція'"));
	ОписаниеХозяйственнойОперацииВводаОстатков.Колонки.Добавить("ШаблонЗаголовкаДокумента",            ОбщегоНазначения.ОписаниеТипаСтрока(250), НСтр("ru='Шаблон заголовка документа';uk='Шаблон заголовка документа'"));
	...
	Для Каждого ХозяйственнаяОперация Из ДоступныеХозяйственныеОперации Цикл
		...
		Для Каждого СтрокаТаблицы Из ОстаткиПоХозяйственнойОперации Цикл
			...
			Если КлючевыеПоляОтличаются(ДокументВводаНачальныхОстатков, СтрокаТаблицы, ПоляСравнения) Тогда
				...
				ДанныеЗаполнения.Комментарий    = НСтр("ru='#Ввод остатков по данным оперативного учета.';uk='# Введення залишків за даними оперативного обліку.'");
...
Функция ИнициализироватьОписаниеРазделовВводаОстатков()
	...
	ОписаниеРазделовВводаОстатков.Колонки.Добавить("РазделУчета",               ОбщегоНазначения.ОписаниеТипаСтрока(30), НСтр("ru='Раздел учета';uk='Розділ обліку'"));
	ОписаниеРазделовВводаОстатков.Колонки.Добавить("ХозяйственнаяОперация",     Новый ОписаниеТипов("ПеречислениеСсылка.ХозяйственныеОперации"), НСтр("ru='Хозяйственная операция';uk='Господарська операція'"));
	ОписаниеРазделовВводаОстатков.Колонки.Добавить("ПояснениеРазделаУчета",     ОбщегоНазначения.ОписаниеТипаСтрока(250), НСтр("ru='Пояснение раздела учета';uk='Пояснення розділу обліку'"));
	ОписаниеРазделовВводаОстатков.Колонки.Добавить("ПорядокВДереве",            ОбщегоНазначения.ОписаниеТипаЧисло(1,0));
	...
	ОписаниеРазделовВводаОстатков.Колонки.Добавить("ШаблонЗаголовкаДокумента",  ОбщегоНазначения.ОписаниеТипаСтрока(250), НСтр("ru='Шаблон заголовка документа';uk='Шаблон заголовка документа'"));
...
Функция ОписаниеРазделаВводаОстатковДенежныхСредств()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Денежные средства';uk='Грошові кошти'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки наличных и безналичных денежных средств организаций';uk='Вводяться залишки готівкових і безготівкових коштів організацій'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковДенежныхСредств()
	...
	Если ИспользуетсяНесколькоСчетов Тогда
		ШаблонЗаголовка = НСтр("ru='Ввод начальных остатков на банковских счетах %1';uk='Введення початкових залишків на банківських рахунках %1'");
	Иначе
		ШаблонЗаголовка = НСтр("ru='Ввод начальных остатков на банковском счете %1';uk='Введення початкових залишків на банківському рахунку %1'");
	КонецЕсли;
	...
	Если ИспользуетсяНесколькоКасс Тогда
		ШаблонЗаголовка = НСтр("ru='Ввод начальных остатков в кассах %1';uk='Введення початкових залишків в касах %1'");
	Иначе
		ШаблонЗаголовка = НСтр("ru='Ввод начальных остатков в кассе %1';uk='Введення початкових залишків в касі %1'");
	КонецЕсли;
...
Функция ОписаниеРазделаВводаОстатковКредитовИДепозитов()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Кредиты и депозиты';uk='Кредити та депозити'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки действующих кредитов и депозитов организаций';uk='Вводяться залишки діючих кредитів і депозитів організацій'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковКредитовИДепозитов()
	...
	ШаблонЗаголовка = НСтр("ru='Ввод начальных остатков расчетов по кредитам, депозитам и выданным займам %1';uk='Введення початкових залишків розрахунків по кредитах, депозитах та виданих позиках %1'");
...
Функция ОписаниеРазделаВводаОстатковПоЭквайрингу()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Эквайринг';uk='Еквайринг'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки по операциям эквайринга организаций';uk='Вводяться залишки за операціями еквайрингу організацій'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковПоЭквайрингу()
	...
	ШаблонЗаголовка = НСтр("ru='Ввод остатков денежных средств к поступлению от эквайера %1';uk='Введення залишків грошових коштів до надходження від еквайра %1'");
...
Функция ОписаниеРазделаВводаОстатковРасчетовСПодотчетниками()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Подотчетные лица';uk='Підзвітні особи'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки по взаиморасчетов с подотчетными лицами';uk='Вводяться залишки взаєморозрахунків з підзвітними особами'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковРасчетовСПодотчетниками()
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков задолженности подотчетных лиц %1';uk='Введення початкових залишків заборгованості підзвітних осіб %1'");
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков перерасходов подотчетных средств %1';uk='Введення початкових залишків перевитрат підзвітних коштів %1'");
...
Функция ОписаниеРазделаВводаОстатковОстаткиВзаиморасчетовСПартнерами()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Расчеты с партнерами';uk='Розрахунки з партнерами'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки взаиморасчетов между организациями и партнерами компании (клиентами и поставщиками)';uk='Вводяться залишки взаєморозрахунків між організаціями та партнерами компанії (клієнтами і постачальниками)'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковВзаиморасчетовСПартнерами()
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков задолженности клиентов %1';uk='Введення початкових залишків заборгованості клієнтів %1'");
	
	ОписаниеХозяйственнойОперации = ОписаниеХозяйственныхОперацийВводаОстатковВзаиморасчетовСПартнерами.Добавить();
	ОписаниеХозяйственнойОперации.ХозяйственнаяОперация               = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов;
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков авансов, полученных от клиентов %1';uk='Введення початкових залишків авансів, отриманих від клієнтів %1'");
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков задолженности перед поставщиками %1';uk='Введення початкових залишків заборгованості перед постачальниками %1'");
	
	ОписаниеХозяйственнойОперации = ОписаниеХозяйственныхОперацийВводаОстатковВзаиморасчетовСПартнерами.Добавить();
	ОписаниеХозяйственнойОперации.ХозяйственнаяОперация               = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам;
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков авансов, выданных поставщикам %1';uk='Введення початкових залишків авансів, виданих постачальникам %1'");
...
Функция ОписаниеРазделаВводаОстатковТоваров()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Товары';uk='Товари'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки товаров организаций';uk='Вводяться залишки товарів організацій'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковТоваров()
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков собственных товаров %1';uk='Введення початкових залишків власних товарів %1'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКомиссиюПриЗакупках") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков полученных на комиссию товаров %1';uk='Введення початкових залишків отриманих на комісію товарів %1'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКомиссиюПриПродажах") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков переданных на комиссию товаров %1';uk='Введення початкових залишків переданих на комісію товарів %1'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМногооборотнуюТару") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков переданной клиентам возвратной тары %1';uk='Введення початкових залишків переданої клієнтам зворотної тари %1'");
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков принятой от поставщиков возвратной тары %1';uk='Введення початкових залишків прийнятої від постачальників зворотної тари %1'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАдресноеХранение") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Переход на использование адресного хранения остатков %1';uk='Перехід на використання адресного зберігання залишків %1'");
	КонецЕсли;
	...
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьОрдернуюСхемуПриОтгрузке")
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Переход на использование складских помещений %1';uk='Перехід на використання складських приміщень %1'");
		
	КонецЕсли;
	
	ФункциональнаяОпцияИспользоватьПроизводствоНаСтороне = Ложь;
	...
	Если Не ПолучитьФункциональнуюОпцию("УправлениеТорговлей")
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков переданных переработчику материалов %1';uk='Введення початкових залишків переданих переробнику матеріалів %1'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство")
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков переданных в производство материалов %1';uk='Введення початкових залишків переданих у виробництво матеріалів %1'");
	КонецЕсли;
...
Функция ОписаниеРазделаВводаОстатковПрочиеРасходы()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Прочее';uk='Інше'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся остатки по прочим операциям';uk='Вводяться залишки за іншими операціями'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковПрочихРасходов()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихАктивовПассивов") Тогда
		...
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков прочих активов и пассивов %1';uk='Введення початкових залишків інших активів і пасивів %1'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихДоходовРасходов") Тогда
		...
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков прочих расходов %1';uk='Введення початкових залишків інших витрат %1'");
	КонецЕсли;
...
Функция ОписаниеРазделаВводаОстатковРезультатовЗаПрошлыйПериод()
	...
	ОписаниеРаздела.РазделУчета           = НСтр("ru='Результаты прошлых периодов';uk='Результати минулих періодів'");
	ОписаниеРаздела.ПояснениеРазделаУчета = НСтр("ru='Вводятся результаты (обороты) прошлых периодов';uk='Вводяться результати (обороти) минулих періодів'");
...
Функция ОписаниеХозяйственныхОперацийВводаОстатковРезультатовЗаПрошлыйПериод()
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Оптовые продажи за прошлые периоды %1';uk='Гуртові продажі за минулі періоди %1'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРозничныеПродажи") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Розничные продажи за прошлые периоды %1';uk='Роздрібні продажі за минулі періоди %1'");
	КонецЕсли;
	...
	ОписаниеХозяйственнойОперации.ДокументВводаОстатков     = ИмяДокументаВводаНачальныхОстатков;
	ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента  = НСтр("ru='Финансовый результат за прошлые периоды %1';uk='Фінансовий результат за минулі періоди %1'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодарочныеСертификаты") Тогда
		...
		ОписаниеХозяйственнойОперации.ДокументВводаОстатков               = ИмяДокументаВводаНачальныхОстатков;
		ОписаниеХозяйственнойОперации.ШаблонЗаголовкаДокумента            = НСтр("ru='Ввод начальных остатков подарочных сертификатов %1';uk='Введення початкових залишків подарункових сертифікатів %1'");
	КонецЕсли;
