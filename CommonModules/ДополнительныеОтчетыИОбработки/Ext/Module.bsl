...
#КонецЕсли
	...
	ЗаписатьПримечание(Ссылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Подключение, БезопасныйРежим = ""%1"".';uk='Підключення, БезопасныйРежим = ""%1"".'"), БезопасныйРежим));
...
//                                     Запрещены внешние по отношению к платформе «платформы BAF» действия:
//                                      COM;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ТипыСозданияСвязанныхОбъектов.Количество() > 0 Тогда
		...
		Иначе
			...
			Команда.Представление      = НСтр("ru='Создание связанных объектов...';uk='Створення пов''язаних об''єктів...'");
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках программы.';uk='Елемент ""%1"" не підключений, тому що ""Додаткові звіти і обробки"" відключені в настройках програми.'"),
			"'" + Строка(Ссылка) + "'");
		Возврат;
	КонецЕсли;
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение НСтр("ru='""Дополнительные отчеты и обработки"" отключена в настройках программы.';uk='""Додаткові звіти і обробки"" відключена в настройках програми.'");
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';uk='Звіт ""%1"" недоступний, тому що додаткові звіти і обробки відключені в настройках програми.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';uk='Звіт ""%1"" недоступний, тому що відсутнє право читання додаткових звітів і обробок.'"),
					СтрокаТаблицы.Представление);
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Команда %1: Запуск.';uk='Команда %1: Запуск.'"), ИдентификаторКоманды));
	...
	Исключение
		ЗаписатьОшибку(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Команда %1: Ошибка выполнения:
                |%2'
                |;uk='Команда %1: Помилка виконання:
                |%2'"),
	...
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Команда %1: Завершение.';uk='Команда %1: Завершення.'"), ИдентификаторКоманды));
...
Функция ПредставлениеОбъектаМетаданных(Объект) Экспорт
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		...
		Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
			Возврат НСтр("ru='<Не существует>';uk='<Не існує>'");
		КонецЕсли;
...
Функция НаименованиеПодсистемы()
	Возврат НСтр("ru='Дополнительные отчеты и обработки';uk='Додаткові звіти й обробки'",ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Процедура ВыгрузитьОтчетыИОбработкиВФайлы(ОтчетыИОбработки)
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		Иначе
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Неподдерживаемый вид объектов метаданных: ""1""';uk='Непідтримуваний вид об''єктів метаданих: ""1""'"), СтрокаТаблицы.Вид));
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
		...
		Если ТекстСхемы = Неопределено Тогда
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не обнаружен файл ""%1"".';uk='Не знайдений файл ""%1"".'"), 
		...
		Если Не СозданиеОбработки.Успех Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось создать ""%1"" из внешнего файла ""%2"":
                    |%3
                    |%4'
                    |;uk='Не вдалося створити ""%1"" з зовнішнього файлу ""%2"":
                    |%3
                    |%4'"),
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если Не РегистрационныеДанные.БезопасныйРежим И Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Результат.ТекстОшибки = НСтр("ru='Для подключения обработки, запускаемой в небезопасном режиме, требуется роль ""Администратор системы"".';uk='Для підключення обробки, яка запускається в небезпечному режимі, потрібна роль ""Адміністратор системи"".'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для команды ""%1"" не определен способ запуска.';uk='Для команди ""%1"" не визначено спосіб запуску.'"), КомандаОписание.Представление));
		КонецЕсли;
