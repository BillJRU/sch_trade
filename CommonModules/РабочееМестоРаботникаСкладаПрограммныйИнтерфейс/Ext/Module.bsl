...
//
// Возвращаемое значение:
//	Структура - параметры задания, см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыЗадания.
//
...
//	Пользователь - СправочникСсылка.Пользователи - исполнитель складского задания,
//	ПараметрыЗадания - Структура - параметры задания, см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыЗадания.
//
// Возвращаемое значение:
//	Структура - параметры задания, см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыЗадания.
//
...
//
// Возвращаемое значение:
//	Структура - параметры товара, см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыТовара.
//
Функция НайтиТоварИлиУпаковочныйЛист(Штрихкод, ИскатьУпаковочныеЛисты) Экспорт
	...
	Если ТоварНеНайден(Штрихкод, КешированныеЗначения.Штрихкоды) Тогда
		...
		ПараметрыТовара.ТекстСообщения = НСтр("ru='Неизвестный штрихкод.';uk='Невідомий штрихкод.'");
	...
	Если Не ЗначениеЗаполнено(ДанныеПоШтрихкодуТовара.Упаковка) Тогда
		...
		Если ДанныеПоШтрихкодуТовара.НесколькоЕдиничныхУпаковок Тогда
			...
			ПараметрыТовара.ТекстСообщения = НСтр("ru='У товара не указана упаковка.';uk='У товару не вказана упаковка.'");
...
//
// Возвращаемое значение:
//	Структура - служебная информация о товаре, содержит следующие поля:
...
//
// Возвращаемое значение:
//	Структура - параметры серии, см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыСерии.
//
...
//
// Возвращаемое значение:
//	Структура - см. РабочееМестоРаботникаСкладаПрограммныйИнтерфейс.ПараметрыЗадания
//
...
|виконати розміщення товарів. Зверніться до адміністратора.'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при создании нового задания на размещение.';uk='Помилка при створенні нового завдання на розміщення.'",ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ЗавершитьВыполнениеСкладскогоЗадания(Задание, ИсходныйИсполнитель, ТЗОтсканированныеТовары, ТоварыДляСканирования,
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при записи и проведении складского задания из рабочего места работника склада';uk='Помилка при запису та проведенні складського завдання з робочого місця працівника складу'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
			КомментарийЖурнала,
...
|Зверніться до адміністратора.'");
		...
		ИмяСобытия = НСтр("ru='Ошибка при записи и проведении складского ордера из рабочего места работника склада';uk='Помилка при запису та проведенні складського ордера з робочого місця працівника складу'",ОбщегоНазначения.КодОсновногоЯзыка());
		
		Для Каждого Сообщение Из ПолучитьСообщенияПользователю(Истина) Цикл
			
			КомментарийЖурнала = НСтр("ru='При записи и проведении складского задания произошла ошибка. Описание ошибки: ""%1"".';uk='При записі і проведенні складського завдання сталася помилка. Опис помилки: ""%1"".'");
...
|Сталася помилка при отриманні товарів завдання. Не заповнена таблична частина складського завдання.'");
		...
		Если Не КоличествоТарыЗаданияСоответствуетКоличествуМногооборотнойТарыТоваров(Задание) Тогда
			...
			ПараметрыЗадания.КодВозврата = 0;
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. Задание на проверку отбора составлено некорректно. Обратитесь к администратору.';uk='Не вдалося взяти складське завдання в роботу. Завдання на перевірку відбору складено некоректно. Зверніться до адміністратора.'");
			
			РазблокироватьДанныеДляРедактирования(Задание);
			
			ИмяСобытия = НСтр("ru='Принятие складского задания ""Проверка отбора"" в работу.';uk='Прийняття складського завдання ""Перевірка відбору"" в роботу.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
|;uk='Не вдалося взяти складське завдання в роботу.
|Не підтримується сценарій роботи завданнь ""Перевірка відбору""
|в списку товарів, яких вказані товари, що відвантажуються з многооборотной тарою і 
|тарою, що відвантажується окремо.'");
...
//	
// Возвращаемое значение:
// 	Структура - результат алгоритма отбора товаров из ячеек, см. СкладыСервер.СтруктураТаблицАлгоритмаОтборТоваров.//!! дополнить
//
