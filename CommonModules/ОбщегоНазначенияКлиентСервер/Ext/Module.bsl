...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(
		...
		НСтр("ru='Недопустимое значение параметра ИмяПроцедурыИлиФункции';uk='Неприпустиме значення параметра ИмяПроцедурыИлиФункции'"), 
		Контекст);
	
	Проверить(
		ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ru='Недопустимое значение параметра ИмяПараметра';uk='Неприпустиме значення параметра ИмяПараметра'"), 
		Контекст);
	...
	Проверить(
		...
		НСтр("ru='Недопустимое значение параметра ОжидаемыеТипы';uk='Неприпустиме значення параметра ОжидаемыеТипы'"), 
		Контекст);
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru='Запуск программ недоступен в веб-клиенте.';uk='Запуск програм недоступний у вебклієнті.'");
#Иначе
...
#Если ВнешнееСоединение Тогда
		ВызватьИсключение 
			НСтр("ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       ...
                       |Підвищення привілеїв системи не доступні з зовнішнього з''єднання.'");
#КонецЕсли
...
#Если Сервер Тогда
		ВызватьИсключение 
			НСтр("ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       ...
                       |;uk='Неприпустиме значення параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       |Підвищення привілеїв системи не доступні на сервері.'");
#КонецЕсли
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("ru='Выполнение диагностики соединения недоступно в веб-клиенте.';uk='Виконання діагностики з''єднання недоступне у вебклієнті.'");
#Иначе
		...
		Для Каждого Элемент Из ОжидаемыеТипы Цикл
			...
			Если Индекс > 10 Тогда
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1,... (всего %2 типов)';uk='%1,... (всього %2 типів)'"), 
					Результат, 
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат НСтр("ru='Подключение проводится на сервере BAF в интернете.';uk='Підключення проводиться на сервері BAF в інтернеті.'");
	Иначе 
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			Если КлиентПодключенЧерезВебСервер() Тогда 
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Подключение проводится из файловой базы на веб-сервере <%1>.';uk='Підключення проводиться з файлової бази на вебсервері <%1>.'"), ИмяКомпьютера());
			Иначе 
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
