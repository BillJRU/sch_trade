...
Функция ТекстНДСПоСтавке(СтавкаНДС, ЦенаВключаетНДС, КодЯзыка="ru") Экспорт
	...
	ТекстНДСПоСтавке = СтрЗаменить(ТекстНДСПоСтавке, "%СтавкаНДС%", МультиязычностьУкр.ПолучитьЛокализованноеПредставление(СтавкаНДС, КодЯзыка));
...
Функция СоответствиеСтавокИСуммНДС() Экспорт
	...
	Пока СтавкиНДС.Следующий() Цикл
        Если (НЕ СтавкиНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.БезНДС")) ИЛИ (НЕ СтавкиНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.НеНДС")) Тогда
	...
	Если СоответствиеСтавокНДС.Количество() > 0 Тогда
		...
		Для Каждого ТекСтавкаНДС Из СоответствиеСтавокНДС Цикл
			...
			Если ТекСтавкаНДС.Значение <> 0 Тогда
				...
				ТекстНДС = СтрЗаменить(ТекстНДС, "%СтавкаНДС%", МультиязычностьУкр.ПолучитьЛокализованноеПредставление(ТекСтавкаНДС.Ключ, КодЯзыка));
...
Функция СформироватьТекстНДС(НалогообложениеНДС, ЦенаВключаетНДС, КодЯзыка = "ru") Экспорт 
	...
	Возврат МультиязычностьУкр.ПолучитьЛокализованноеПредставление(НалогообложениеНДС, КодЯзыка) + 
			?(ЦенаВключаетНДС,
	...
	Для Счетчик = 1 по ЧислоПараметров Цикл
		...
		Иначе
			...
			ИначеЕсли МетаданныеТип <> Неопределено И Метаданные.Документы.Содержит(МетаданныеТип) Тогда
				ЗначениеПараметра = МультиязычностьУкр.ПолучитьЛокализованноеПредставление(ЗначениеПараметра, КодЯзыка);
...
Процедура СформироватьПараметрыОтправки(СоответствиеТиповДанныхОбъектовПечати, ПараметрыОтправки, МассивДокументовРодителей, ТребуетсяТелефоны = Ложь) Экспорт
	...
	Для каждого ЭлементСоответствия Из СоответствиеТиповДанныхОбъектовПечати Цикл
		...
		Для каждого ИмяРеквизитаПолучателя Из МассивРеквизитовПолучателей Цикл
			...
			//++ НЕ БЗК	
				ИначеЕсли ОписаниеТиповРеквизита.СодержитТип(Тип("СправочникСсылка.Партнеры")) Тогда
