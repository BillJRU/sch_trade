...
#КонецЕсли
	...
	Если ВыполнитьБезФоновогоЗадания Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка выполнения';uk='Помилка виконання'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Длительные операции.Отмена выполнения фонового задания';uk='Тривалі операції.Скасування виконання фонового завдання'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания) Экспорт
	...
	ТекстОшибки = НСтр("ru='Не удалось выполнить данную операцию. 
        ...
        |Технічні подробиці див. в журналі реєстрації.'");
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Длительные операции.Фоновое задание не найдено';uk='Тривалі операції.Фонове завдання не знайдене'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Если ОшибкаЗадания <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |
                |Технічні подробиці:
                |Помилка виконання фонового завдання %2 з ідентифікатором %3 по причині
                |%4
                |
                |Див. Також журнал реєстрації.'"),
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Длительные операции.Фоновое задание отменено администратором';uk='Тривалі операції.Фонове завдання скасоване адміністратором'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , НСтр("ru='Задание завершилось с неизвестной ошибкой.';uk='Завдання завершилося з невідомою помилкою.'"));
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Если Задание = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Длительные операции';uk='Тривалі операції'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВызватьФункцию(ИмяФункции, ПараметрыПроцедуры, АдресРезультата)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Неверный формат параметра ИмяФункции (переданное значение: %1)';uk='Невірний формат параметра ИмяФункции (передане значення: %1)'"), ИмяФункции);
