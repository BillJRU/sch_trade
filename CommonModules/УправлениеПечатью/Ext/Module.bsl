...
Процедура ЗаполнитьПрофильРедактированиеПечатныхФорм(ОписанияПрофилей) Экспорт
	...
	ОписаниеПрофиля.Наименование = НСтр("ru='Редактирование, отправка по почте, сохранение в файл печатных форм (дополнительно)';uk='Редагування, відправка поштою, збереження у файл друкованих форм (додатково)'",ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Описание = НСтр("ru='Дополнительно назначается пользователям, которым должна быть доступна возможность редактирования,
        ...
        |перед друком, відправка поштою і збереження у файл сформованих друкованих форм.'");
...
Функция МакетПечатнойФормы(ПутьКМакету, Знач КодЯзыка = "") Экспорт
	...
	Макет = НайтиМакет(ПутьКМакету, КодЯзыка);
	Если ТипЗнч(Макет) = Тип("ТабличныйДокумент") Тогда
		Если СтрНайти(ПутьКМакету, "ПФ_MXL") И ЗначениеЗаполнено(КодЯзыка) Тогда
...
Функция ИнициализироватьПечатнуюФорму(Знач УдалитьТипДокумента, Знач УдалитьНастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Необходимо указать значение параметра ""Макет""';uk='Необхідно вказати значення параметра ""Макет""'");
	КонецЕсли;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Формирование QR-кода';uk='Формування QR-коду'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИменаМакета(Знач ИмяМакета, КодЯзыка = "")
...
Функция ПодготовитьКоллекциюПечатныхФорм(Знач Идентификаторы) Экспорт
	...
	КонецЦикла;
	Результат.Колонки.Добавить("ЭтоМногоязычнаяПечатнаяФорма", Новый ОписаниеТипов("Булево"));
...
Функция ПодготовитьСтруктуруПараметровВывода(ПараметрыВыводаОснование = Неопределено) Экспорт
	...
	Иначе	
		...
		КодЯзыка = МодульМультиязычностьУкр.ПолучитьЯзыкФормированияПечатныхФорм();
	КонецЕсли;
...
Функция НайтиМакет(ПутьКМакету, КодЯзыка)
...
Функция КомпонентаФормированияQRКода()
	...
	ТекстОшибки = НСтр("ru='Не удалось подключить внешнюю компоненту для генерации QR-кода. Подробности в журнале регистрации.';uk='Не вдалося підключити зовнішню компоненту для генерації QR-коду. Подробиці в журналі реєстрації.'");
