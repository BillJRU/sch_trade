...
|и пароль в настройках авторизации в BAS Документообороте.'
|;uk='Невірне ім''я користувача або пароль. Перевірте ім''я 
|та пароль в настройках авторизації в BAS документообігу.'");
...
// Устанавливает параметры сеанса работы с подсистемой "Интеграция с BAS Документооборотом".
//
		...
		Иначе
			...
			Иначе // получим версию сервиса асинхронно, в фоновом задании
				ЗаписьЖурналаРегистрации(
					...
					НСтр("ru='Начато получение версии сервиса';uk='Розпочато отримання версії сервісу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПолучитьВерсиюВызовомСервиса(ИмяПользователя, Пароль, ИспользуетсяАутентификацияОС)
	...
	Если Прокси = Неопределено Тогда
		ЗаписьЖурналаРегистрации(
			ИмяСобытияЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru='Сервис недоступен';uk='Сервіс недоступний'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат "0.0.0.0";
	Иначе
		...
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Подключен сервис версии %1';uk='Підключений сервіс версії %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				Ответ.versionNumber);
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Информация,,,
...
Процедура УстановитьВерсиюСервиса(АдресВХранилище, ИмяПользователя, Пароль, ИспользуетсяАутентификацияОС) Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(
			ИмяСобытияЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru='Завершено получение версии сервиса';uk='Завершено отримання версії сервісу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
...
Функция НовоеИсходящееПисьмо(Прокси, Предмет = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Эта версия BAS Документооборота не поддерживает создание исходящих писем';uk='Ця версія BAS Документообігу не підтримує створення вихідних листів'");
	КонецЕсли;
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, ИмяПользователя = Неопределено, Пароль = Неопределено,
	...
	Если Определения = Неопределено Тогда
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("ru='Ошибка подключения к BAS Документообороту.
|
|Возможно, не прошла авторизация, указан неверный адрес веб-сервиса 
|или база BAS Документооборота не опубликована на веб-сервере.
|
|Подробности в журнале регистрации. Обратитесь к администратору системы.'
|;uk='Помилка підключення до BAS Документообігу. 
|
|Можливо, не пройшла авторизація, вказана невірна адреса вебсервісу 
|або база BAS Документообігу не опублiкована на вебсервері. 
|
...
Функция СоздатьОбъект(Прокси, ТипОбъекта) Экспорт
	...
	Если Прокси = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Интеграция с BAS Документооборотом не настроена, или сервис
|BAS Документооборота недоступен. Обратитесь к администратору.'
|;uk='Інтеграція з BAS Документообігом не налаштована, або сервіс 
|BAS Документообігу недоступний. Зверніться до адміністратора.'");
	КонецЕсли;
...
//      * id - идентификатор объекта по данным BAS Документооборота;
//      * type - тип объекта по данным BAS Документооборота.
//
// Возвращаемое значение:
...
|;uk='Недостатньо прав для доступу до цього об''єкта (%1).
...
|Обратитесь к администратору.'
|;uk='Недостатньо прав для створення нових елементів (%1).
|Зверніться до адміністратора.'"),
...
//
// Параметры:
//   Прокси - WSПрокси - прокси веб-сервиса BAS Документооборота.
...
// Получает текущего пользователя BAS Документооборота.
//
// Возвращаемое значение:
...
// Определяет возможность использования аутентификации ОС платформой платформы BAF.
...
//
// Возвращаемое значение:
//   Строка - Интеграция с BAS Документооборотом
//        или Интеграция с BAS Документооборотом.<ИмяСобытияВторогоУровня>
//
Функция ИмяСобытияЖурналаРегистрации(СобытиеВторогоУровня = "") Экспорт
	
	Если СобытиеВторогоУровня = "" Тогда
		Возврат НСтр("ru='Интеграция с BAS Документооборотом';uk='Інтеграція з BAS Документообігом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Возврат СтрШаблон("%1.%2",
			НСтр("ru='Интеграция с BAS Документооборотом';uk='Інтеграція з BAS Документообігом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПолучитьФайлыСвязанныхДокументов(СвязанныеДокументы) Экспорт
	...
	ФоновыеЗадания.Выполнить(
		...
		МассивПараметров, КлючЗадания, НСтр("ru='Получение файлов связанных документов';uk='Отримання файлів пов''язаних документів'"));
		...
		Для Каждого ПодходящийОбъект Из ПодходящиеОбъекты Цикл
			...
			Если ДанныеОбъектаДО = Неопределено Тогда // создадим документ в ДО
				...
				Иначе // сообщение об ошибке
					ИмяСобытия = НСтр("ru='Автоматическое создание связанного объекта';uk='Автоматичне створення пов''язаного об''єкта'");
					ЗаписьЖурналаРегистрации(
...
|%2'
|;uk='Не вдалося створити пов''язаний документ для ""%1"":
|%2'"),
						ПодходящийОбъект,
						НовыйОбъектДО);
...
// Возвращает количество прикрепленных файлов к связанному объекту BAS Документооборот.
//
// Параметры:
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Исключение
			ПредставлениеОшибки = СтрШаблон(
				НСтр("ru='Не удалось получить параметры хронометража (%1)';uk='Не вдалося отримати параметри хронометражу (%1)'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Ошибка,,,
