...
Функция КомандаИсторияИзменений(Форма) Экспорт
	...
	Команда.Заголовок = НСтр("ru='История изменений объекта';uk='Історія змін об''єкта'");
	Команда.Подсказка = НСтр("ru='История изменений объекта';uk='Історія змін об''єкта'");
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Попытка
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.Комментарий = НСтр("ru='Отклоненная версия (автоматическое разрешение конфликта).';uk='Відхилена версія (автоматичне вирішення конфлікту).'");
			НаборЗаписей.Записать();
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Версионирование';uk='Версіонування'",ОбщегоНазначения.КодОсновногоЯзыка()),
