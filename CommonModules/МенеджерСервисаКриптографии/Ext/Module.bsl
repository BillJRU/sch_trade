...
// 	 * НомерКода - 
// 	 * ВремяДействияКода -
// 	 * ЗадержкаПередПовторнойОтправкой - 
...
// Функция - Создает контейнер закрытого ключа и запроса на сертификат
//
// Параметры:
//  ПараметрыЗаявления - Структура - Содержит поля для формирования заявления на сертификат
//     Структура - ключевые параметры, используемые для работы:
...
//  АдресРезультат - Строка - адрес временного хранилища куда выкладывается результат функции Структура.
// 
...
// Функция - Привязывает сертификат к закрытому ключу в облачном хранилище
//
// Параметры:
//  ПараметрыЗаявления - Структура - Содержит поля для формирования заявления на сертификат:
//   * ИдентификаторЗаявления   - Строка - идентификатор поиска запроса на сертификат
...
Процедура УстановитьСертификатВКонтейнерИХранилище(ПараметрыЗаявления, АдресРезультата) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись в модели сервиса.Менеджер сервиса криптографии';uk='Електронний підпис моделі сервісу.Менеджер сервісу криптографії'",ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПоискСертификатаПоОтпечаткуИлиСерийномуНомеру(
	...
	Если Не ЗначениеЗаполнено(Отпечаток) И Не ЗначениеЗаполнено(СерийныйНомер) Тогда 
		Возврат Новый Структура("Выполнено, КодОшибки, ОписаниеОшибки", 
						...
						НСтр("ru='Необходимо заполнить thumbprint или serialnumber.';uk='Необхідно заповнити thumbprint або serialnumber.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
