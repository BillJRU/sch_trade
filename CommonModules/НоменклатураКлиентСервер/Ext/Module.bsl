...
//	ЗначенияРеквизитов	 - Структура, ФормаКлиентскогоПриложения, СправочникОбъект.СерииНоменклатуры,
//							ДанныеФормыЭлементКоллекции	 - коллекция структуры данных, хранящая значения свойств, используемых в качестве 
...
Функция ДополнительныеПараметрыПредставлениеНоменклатурыДляПечати() Экспорт
    ...
    ДополнительныеПараметры.Вставить("КодЯзыка", "ru");
...
Функция ПредставлениеНоменклатурыДляПечати(НаименованиеНоменклатурыДляПечати,
	...
	Если ТипЗнч(НаименованиеНоменклатурыДляПечати) <> Тип("Строка") Тогда
		ТекстИсключения = НСтр("ru='В функцию НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати не передано наименование номенклатуры для печати.';uk='У функцію НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати не передано найменування номенклатури для друку.'");	
	...
	Если ЗначениеЗаполнено(НаименованиеХарактеристикиДляПечати) 
		...
		ТекстИсключения = НСтр("ru='В функцию НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати не передано наименование характеристики для печати.';uk='У функцію НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати не передано найменування характеристики для друку.'");	
		ВызватьИсключение ТекстИсключения;	
	КонецЕсли;
	...
	Иначе
		...
		Если ДополнительныеПараметры.ВозвратнаяТара Тогда
            ТекстВСкобках.Добавить(НСтр("ru='возвратная тара';uk= 'зворотна тара'", ДополнительныеПараметры.КодЯзыка));
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстВСкобкахСтрока) Тогда
			ПредставлениеНоменклатуры = НСтр("ru='%НаименованиеНоменклатурыДляПечати% (%ТекстВСкобках%)';uk='%НаименованиеНоменклатурыДляПечати% (%ТекстВСкобках%)'");
			ПредставлениеНоменклатуры = СтрЗаменить(ПредставлениеНоменклатуры, "%ТекстВСкобках%", СокрЛП(ТекстВСкобкахСтрока));
		Иначе
			ПредставлениеНоменклатуры = НСтр("ru='%НаименованиеНоменклатурыДляПечати%';uk='%НаименованиеНоменклатурыДляПечати%'");
		КонецЕсли;
