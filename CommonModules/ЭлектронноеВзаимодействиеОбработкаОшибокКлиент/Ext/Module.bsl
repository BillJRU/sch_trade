...
Функция ПараметрыВидаОшибки(ВидОшибки) Экспорт
	...
	Если ПараметрыВидаОшибки.ЗаголовокПроблемы = "" Тогда
		...
		ПараметрыВидаОшибки.ЗаголовокПроблемы = НСтр("ru='Неизвестная ошибка';uk='Невідома помилка'");
		ОписаниеРешения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru='<a href = ""Обратитесь"">Обратитесь</a> в тех. поддержку';uk='<a href = ""Обратитесь"">Зверніться</a> в тех. підтримку'"));
...
Процедура НачатьПолучениеИнформацииДляТехПоддержкиЗавершение(ТехническаяИнформация, Контекст) Экспорт
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru='Выполняется формирование файла с технической информацией';uk='Виконується формування файлу з технічною інформацією'");
...
Процедура ПослеПолученияФайлаДляТехПоддержки(Результат, Контекст) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Ошибка при получении файла для тех.поддержки:';uk='Помилка при отриманні файлу для техпідтримки:'") + Результат.КраткоеПредставлениеОшибки);
	Иначе 
