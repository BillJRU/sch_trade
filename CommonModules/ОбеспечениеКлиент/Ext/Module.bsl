...
Процедура СообщитьОбОтсутствииТовараКОбособленномуОбеспечению() Экспорт
	...
	ТекстОшибки = НСтр("ru='Заказ не является объектом обособления. Выполнение команды не предусмотрено.';uk='Замовлення не є об''єктом відокремлення. Виконання команди не передбачено.'");
...
|;uk='Резервування для зазначеного в рядках призначення доступно з відповідного замовлення, 
|або за допомогою документа ""Коригування призначення товарів"".'");
		
	Иначе
		
		ТекстОшибки = НСтр("ru='Снятие резерва для указанного в строках назначения доступно из соответствующего заказа,
|или с помощью документа ""Корректировка назначения товаров"".'
|;uk='Зняття резерву для зазначеного в рядках призначення доступно з відповідного замовлення, 
|або за допомогою документа ""Коригування призначення товарів"".'");
...
Процедура СообщитьОНеобходимомМинимальномСтатусеДокумента(ЭтоРезервирование, МинимальныйСтатус) Экспорт
	...
	ТекстОшибки = ?(ЭтоРезервирование,
		НСтр("ru='Минимальный статус документа для резервирования под назначение - ""%1"".';uk='Мінімальний статус документа для резервування під призначення - ""%1"".'"),
		НСтр("ru='Минимальный статус документа для снятия резерва под назначение - ""%1"".';uk='Мінімальний статус документа для зняття резерву під призначення - ""%1"".'"));
