...
Функция ОстаткиИДвиженияБонусныхБаллов(БонуснаяПрограммаЛояльности, Партнер) Экспорт
	...
	// Текущий остаток
	РезультатТекущийОстаток = МассивРезультатов[1]; // РезультатЗапроса
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СокрЛП(НоваяСтрока.Период) = "Сейчас" Тогда
			 НоваяСтрока.Период = НСтр("ru='Сейчас';uk='Зараз'");
		КонецЕсли;
	...
	// Списания баллов
	РезультатСписанияБаллов = МассивРезультатов[2]; // РезультатЗапроса
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Изменение > 0 Тогда
			НоваяСтрока.Период = НСтр("ru='Начисление через';uk='Нарахування через'")
			                   + " " + СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(Цел((Выборка.Период - ТекущаяДатаСеанса())/(24*60*60)), НСтр("ru='день, дня, дней';uk='день, дні, днів'"))
