...
Процедура ОповеститьОбИзмененииСпискаВыбораЕдиницИзмерения(КоличествоНовыхЭлементов, МассивЭлементов) Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Добавление элементов';uk='Додавання елементів'");
	...
	Если КоличествоНовыхЭлементов = 1 Тогда
		...
		ТекстСообщения = НСтр("ru='В список добавлена новая единица измерения: %ЕдиницаИзмерения%.';uk='У список додана нова одиниця вимірювання: %ЕдиницаИзмерения%.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЕдиницаИзмерения%", Строка(ДобавляемыйЭлемент));
	Иначе
		СсылкаЕдиницыИзмерения = "";
		
		СклоняемоеВыражение = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(НСтр("ru='новая единица измерения';uk='нова одиниця вимірювання'"),
																					НСтр("ru='новые единицы измерения';uk='нові одиниці вимірювання'"),
																					НСтр("ru='новых единиц измерений';uk='нових одиниць вимірювань'"),
																					КоличествоНовыхЭлементов);
		
		ТекстСообщения = НСтр("ru='В список добавлено %КоличествоНовыхЭлементов% %СклоняемоеВыражение%.';uk='У список додано %КоличествоНовыхЭлементов% %СклоняемоеВыражение%.'");
	...
	Иначе
		...
		Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(ТекущиеДанные.СтатусУказанияСерий, ПараметрыУказанияСерий)
			...
			ИначеЕсли ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПоступлениеТоваровОтХранителя"
				...
				Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.СписаниеТоваровУХранителя" Тогда
...
Процедура ОповеститьОПерезаполненииСерий(ЕстьИзменения, КоличествоИзначальноВерное) Экспорт
	...
	Если ЕстьИзменения Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Серии перезаполнены';uk='Серії перезаповнені'"),
				,
				НСтр("ru='В строках перезаполнены данные по сериям.';uk='У рядках перезаповнені дані по серіях.'"),
				БиблиотекаКартинок.Информация32);
	...
	ИначеЕсли КоличествоИзначальноВерное Тогда
			...
			ПоказатьОповещениеПользователя(
				...
				,
				НСтр("ru='Перезаполнение не требуется, т.к. во всех строках количество серий указано верно, либо поступление серий еще не выполнено.';uk='Перезаповнення не потрібно, тому що у всіх рядках кількість серій зазначено вірно, або надходження серій ще не виконано.'"),
				БиблиотекаКартинок.Информация32);
	...
	Иначе
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru='Перезаполнение не требуется';uk='Перезаповнення не потрібно'"),
				,
				НСтр("ru='Перезаполнение не требуется, т.к. поступление серий еще не выполнено.';uk='Перезаповнення не потрібно, тому що надходження серій ще не виконано.'"),
				БиблиотекаКартинок.Информация32);
...
// Функция-конструктор результата обработки оповещений 
// Возвращаемое значение:
// 	Структура - дополнительные параметры:
// 	* Действие - Строка - 
// 	* Ссылка - СправочникСсылка.Номенклатура, СправочникСсылка.ХарактеристикиНоменклатуры, Неопределено - 
