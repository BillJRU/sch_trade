...
Функция ПредставлениеТипаОбъектаXDTO(ТипXDTO) Экспорт
	...
	// Справочники.
	ПредставленияТипов.Вставить("DMProject", НСтр("ru='Проекты';uk='Проєкти'"));
	ПредставленияТипов.Вставить("DMProjectTask", НСтр("ru='Проектные задачи';uk='Проєктні задачі'"));
	...
	ПредставленияТипов.Вставить("DMCorrespondent", НСтр("ru='Контрагенты';uk='Контрагенти'"));
	...
	ПредставленияТипов.Вставить("DMCashFlowItem", НСтр("ru='Статьи движения денежных средств';uk='Статті руху грошових коштів'"));
	ПредставленияТипов.Вставить("DMMeasurementUnit", НСтр("ru='Единицы измерения';uk='Одиниці вимірювання'"));
	ПредставленияТипов.Вставить("DMProduct", НСтр("ru='Товары и услуги';uk='Товари і послуги'"));
	...
	ПредставленияТипов.Вставить("DMVATRate", НСтр("ru='Ставки НДС';uk='Ставки ПДВ'"));
...
Функция ПредставлениеСрокаДействияДокумента(ДатаНачала, Бессрочный, ДатаОкончания, 
	...
	Если Бессрочный = Истина Тогда
		Результат = СтрШаблон(НСтр("ru='%1, бессрочный';uk='%1, безстроковий'"),
			Результат);
	ИначеЕсли ЗначениеЗаполнено(ДатаОкончания) Тогда
		Результат = СтрШаблон(НСтр("ru='%1 по %2';uk=' %1 по %2'"),
			Результат,
...
Функция ПолучитьОбзорДокумента(Форма) Экспорт
	...
	Если Форма.Тип = "DMIncomingDocument" Тогда
		...
		Если ЗначениеЗаполнено(Форма.ДатаРегистрации) И ЗначениеЗаполнено(Форма.РегистрационныйНомер) Тогда
			...
			ДобавитьРеквизитКHTML(HTMLТекст, "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Рег. № %1 от %2';uk='Реєстр. № %1 від %2'"),
		...
		Если Форма.Сумма <> 0 Тогда
			...
			Если Форма.ИмяФормы = "Обработка.ИнтеграцияС1СДокументооборот.Форма.ВнутреннийДокумент"
				...
				ДобавитьРеквизитКHTML(HTMLТекст, НСтр("ru='Сумма НДС:';uk='Сума ПДВ:'"),
			...
			Если Форма.ИмяФормы = "Обработка.ИнтеграцияС1СДокументооборот.Форма.ВнутреннийДокумент"
				...
				Если КоличествоСтатей = 1 Тогда
					ДобавитьРеквизитКHTML(HTMLТекст, НСтр("ru='Статья ДДС:';uk='Стаття РГК:'"),
						СтатьиДДС[0].СтатьяДДС);
				ИначеЕсли КоличествоСтатей > 1 Тогда
					ДобавитьПодписьКHTML(HTMLТекст, НСтр("ru='Статьи ДДС:';uk='Статті РГК:'"));
					Разделитель = "";
	...
	ИначеЕсли Форма.Тип = "DMInternalDocument" Тогда
		...
		Если ЗначениеЗаполнено(Форма.Контрагент) Тогда
			...
			Иначе
				ДобавитьРеквизитКHTML(HTMLТекст, НСтр("ru='Контрагент:';uk='Контрагент:'"), "");
			КонецЕсли;
			HTMLТекст = HTMLТекст + "<p>";
			ДобавитьРеквизитКHTML(HTMLТекст, "",
				Форма.Контрагент, 
	...
	Если ЗначениеЗаполнено(Форма.Проект) Тогда
		ДобавитьРеквизитКHTML(HTMLТекст, НСтр("ru='Проект:';uk='Проєкт:'"), Форма.Проект);
	КонецЕсли;
	
	ДобавитьПредставлениеСостояний(HTMLТекст, Форма);
	
	Если ЗначениеЗаполнено(Форма.Комментарий) Тогда
		ДобавитьРеквизитКHTML(HTMLТекст, НСтр("ru='Комментарий:';uk='Коментар:'"), Форма.Комментарий);
	КонецЕсли;
...
Функция РазделитьЧисло(Число, Генератор = Неопределено, Частей = 2, Обязательных = 2)
	...
	ВызватьИсключение НСтр("ru='Функция не поддерживается в веб-клиенте.';uk='Функція не підтримується в вебклієнті.'")
