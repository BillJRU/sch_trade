...
Процедура ВыгрузитьОсновныеСправочники(Фабрика, ЗаписьXML) Экспорт
	...
	|	СправочникиДляВыгрузки.ВидКИСсылка.ВидРедактирования = ""Диалог"" КАК ВидКИРедактированиеТолькоВДиалоге,
	|	СправочникиДляВыгрузки.ВидКИСсылка.МожноИзменятьСпособРедактирования КАК ВидКИМожноИзменятьСпособРедактирования,
	|	СправочникиДляВыгрузки.ВидКИСсылка.ТолькоНациональныйАдрес КАК ВидКИТолькоНациональныйАдрес,
...
//  Данные - СправочникОбъект, ДокументОбъект, УдалениеОбъекта - данные, подлежащие записи в формат XML;
	...
	Если ОписаниеОбъекта.ИмяКласса = "Справочники" Тогда
		...
		Если СокрЛП(ОписаниеОбъекта.ИмяОбъекта) = "ВариантыОтчетов" Тогда
			...
			Если Не НастройкиПользователя = Неопределено Тогда
				...
				ОбъектОбменаСтрокаТаблицы.ПредставлениеНастройки =
					НСтр("ru='Текущие пользовательские настройки';uk='Поточні користувацькі настройки'",ОбщегоНазначения.КодОсновногоЯзыка());
				ОбъектОбменаСтрокаТаблицы.Результат = РезультатОтчета(Данные.Ссылка, Данные.Настройки,,
					"ТекущиеПользовательскиеНастройки");
				ТабличнаяЧастьОбъекта =	ОбъектОбмена.ПользовательскиеНастройкиВариантовОтчетов;//СписокXDTO - 
				ТабличнаяЧастьОбъекта.Добавить(ОбъектОбменаСтрокаТаблицы);
			КонецЕсли;
			...
			Если Данные.Размещение.Количество()=0 Тогда
				Для Каждого СтрокаОтчетов Из Данные.ПредопределенныйВариант.Размещение Цикл
					...
					ОбъектОбменаСтрокаТаблицы.БыстрыйДоступ = БыстрыйДоступ(Данные.Ссылка, СтрокаОтчетов.Подсистема,
						ТекущийПользователь);
					ТабличнаяЧастьОбъекта =	ОбъектОбмена.Размещение;//СписокXDTO -
					ТабличнаяЧастьОбъекта.Добавить(ОбъектОбменаСтрокаТаблицы);
				КонецЦикла;
			Иначе
		...
		Если СокрЛП(ОписаниеОбъекта.ИмяОбъекта) = "ВариантыАнализаЦелевыхПоказателей" Тогда
			...
			Для Каждого СтрокаОтчетов Из Данные.ОтчетыДляРасшифровки Цикл
				...
				ТабличнаяЧастьОбъекта =	ОбъектОбмена.ОтчетыДляРасшифровки;//СписокXDTO -
				ТабличнаяЧастьОбъекта.Добавить(ОбъектОбменаСтрокаТаблицы);
			КонецЦикла;
		КонецЕсли;
...
//
// Параметры:
//  Данные - СправочникОбъект - Объект, набор записей, который нужно проверить.
//
...
//  Подсистема - СправочникСсылка.ИдентификаторыОбъектовРасширений, СправочникСсылка.ИдентификаторыОбъектовМетаданных - раздел или группа варианта отчета;
//  Пользователь - СправочникСсылка.Пользователи - пользователь для которого производится проверка.
//
// Возвращаемое значение:
...
// Параметры:
//  ЗаписьXML - ЗаписьXML - сообщение обмена;
//  Клиент - СправочникСсылка.Партнеры - клиент, чьих контактных лиц необходимо выгрузить в обмен;
...
Процедура ВыгрузитьКонтактныхЛицКлиента(Фабрика, ЗаписьXML, Клиент, УдалениеДанных)
	...
	Для каждого СтрокаДанных Из Результат Цикл
		...
		ОбъектОбмена.ref = СокрЛП(СтрокаДанных.КонтактноеЛицо.УникальныйИдентификатор());
		ОбъектОбмена.ERPКод = СокрЛП(СтрокаДанных.КонтактноеЛицо.УникальныйИдентификатор());
		ЗаписатьКонтактнуюИнформацию(Фабрика, ОбъектОбмена, СтрокаДанных.КонтактноеЛицо, "КонтактныеЛицаПартнеров");
		Фабрика.ЗаписатьXML(ЗаписьXML, ОбъектОбмена);
	КонецЦикла;
КонецПроцедуры
...
Функция ДосьеПартнера(Партнер, ВыгружатьКонтактныеТелефоны = Истина)
	...
	ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Монитор ERP. Синхронизация.';uk='Монітор ERP. Синхронізація.'",КодОсновногоЯзыка));
	...
	Если Не ВыгружатьКонтактныеТелефоны Тогда
		...
		КомментарийСПараметрами = НСтр("ru='Досье клиента сформировано успешно.';uk='Досьє клієнта сформовано успішно.'",КодОсновногоЯзыка);
	...
	Запрос.УстановитьПараметр("Ответственный", НСтр("ru='Ответственный менеджер';uk='Відповідальний менеджер'",КодОсновногоЯзыка));
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	ЗаголовокТелефон = НСтр("ru='тел.:';uk='тел.:'",КодОсновногоЯзыка)+" ";
	...
	Пока Выборка.Следующий() Цикл
		...
		ПредставлениеКонтакта = СтрЗаменить(ПредставлениеКонтакта,
			НСтр("ru='Телефон';uk='Телефон'",КодОсновногоЯзыка), ЗаголовокТелефон);
		ПредставлениеКонтакта = СтрЗаменить(ПредставлениеКонтакта,
			НСтр("ru='телефон';uk='телефон'",КодОсновногоЯзыка), ЗаголовокТелефон);
...
Процедура ЗаписатьКонтактнуюИнформациюСправочника(Фабрика, ИнформацияПоОбъекту, Знач ТаблицаКИ,  Знач ИмяСправочника)
	...
	Для Каждого СтрокаКИ Из ТаблицаКИ Цикл
		...
		ТабличнаяЧастьОбъекта = ИнформацияПоОбъекту.КонтактнаяИнформация;//СписокXDTO - 
		ТабличнаяЧастьОбъекта.Добавить(ОбъектОбмена);
	КонецЦикла;
КонецПроцедуры
...
Функция РезультатВарианта(ВариантАнализа)
	...
	// Параметры записи в журнал регистрации.
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Монитор ERP. Синхронизация.';uk='Монітор ERP. Синхронізація.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если Не ИсточникДанных = Неопределено Тогда
		...
		Попытка
			...
			КомментарийСПараметрами = НСтр("ru='Показатель сформирован успешно.';uk='Показник сформований успішно.'",ОбщегоНазначения.КодОсновногоЯзыка());
		Исключение
		...
		КонецЕсли;
	Иначе
		ИнформацияОбОшибке =  НСтр("ru='Не определен источник данных для показателя.';uk='Не визначено джерело даних для показника.'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция РезультатОтчета(ВариантОтчета, НастройкиОтчетаХранилище, ПериодОтчета = Неопределено,
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		Если Не ПериодОтчета = Неопределено И НЕ НастройкиОтчета = Неопределено Тогда
			НастройкиОтчетаТекущего = НастройкиОтчета;//НастройкиКомпоновкиДанных - 
			ПараметрыДанныхСКД = НастройкиОтчетаТекущего.ПараметрыДанных;//ПараметрКомпоновкиДанных - 
			ПараметрыДанныхОтчета = ПараметрыДанныхСКД.Элементы.Найти("Период");//КоллекцияЗначенийПараметровКомпоновкиДанных -  
			ПараметрПериод = ПараметрыДанныхОтчета.Найти("Период");
	...
	Попытка
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		КомментарийСПараметрами = НСтр("ru='Вариант отчета сформирован успешно.';uk='Варіант звіту сформований успішно.'",ОбщегоНазначения.КодОсновногоЯзыка());
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	КонецПопытки;
