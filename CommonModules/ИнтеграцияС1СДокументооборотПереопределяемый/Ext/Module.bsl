...
Процедура ОписаниеОбработчиковОбновленияПравилИнтеграции(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru='Установка режима изменения «Разрешено с перепроведением» для реквизита «Статус» заказов клиента.';uk='Встановлення режиму зміни «Дозволено з перепроведенням» для реквізиту «Статус» замовлень клієнта.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Не удалось обработать объект: %1 по причине: %2';uk='Не вдалося обробити об''єкт: %1 по причині: %2'"),
				Выборка.Ссылка,
...
//Вызывается при создании форм бизнес-процессов BAS Документооборота. Позволяет выполнить дополнительные действия.
//
//Параметры:
...
Процедура ЗаполнитьРеквизитыИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель) Экспорт
	...
	Если ИнтеграцияС1СДокументооборот.ПроверитьТип(Прокси, ОбъектXDTO, "DMCorrespondent")
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаПотребитель, "ИННПлательщикаНДС, ЮрФизЛицо");
		ОбъектXDTO.inn = Реквизиты.ИННПлательщикаНДС;
...
//			Свойства[ИмяСвойства] = НСтр("ru='Согласовано';uk='Погоджено'");
...
Процедура ПриЗаполненииСвойстваОбъектаXDTOПоЗначению(Значение, Свойства, ТипОбъекта, ИмяСвойства, ТипСвойства,
	...
	Если ТипСвойства = "DMDocumentStatus" И ИмяСвойства = "СостояниеСогласование" Тогда
		...
		ИначеЕсли Значение = Перечисления.СтатусыКоммерческихПредложенийКлиентам.НеСогласовано
			...
			Свойства[ИмяСвойства] = НСтр("ru='Не согласовано';uk='Не погоджено'");
		...
		ИначеЕсли Значение = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.НеСогласована
			...
			Свойства[ИмяСвойства] = НСтр("ru='На согласовании';uk='На погодженні'");
...
Процедура ЗаполнитьПечатныеФормы(ВнешнийОбъект, ИмяКоманды, КоллекцияПечатныхФорм) Экспорт
	...
	Если ОписаниеКоманды.ДополнительныеПараметры.Свойство("Ссылка")
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Не удалось сформировать печатную форму по причине: %2';uk='Не вдалося сформувати друковану форму по причині: %2'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Вызывается при записи констант подсистемы Интеграция с BAS Документооборотом.
//
...
Функция ЗависимостиКонстант() Экспорт
	...
	// Интеграция с BAS Документооборот
	...
	//++ НЕ БЗК	
	// Согласование через BAS Документооборот
	ДобавитьСтрокуТаблицыЗависимостиКонстант(
		Результат,
		"ИспользоватьИнтеграциюС1СДокументооборот",
		Ложь,
