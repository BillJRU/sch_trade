...
Функция СохранитьЭталонныеДанные(КонтрольныеРегистры, ПараметрыЗапроса = Неопределено, МассивРегистраторов = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Тестирование.Начато сохранение эталонных данных';uk='Тестування.Розпочато збереження еталонних даних'",ОбщегоНазначения.КодОсновногоЯзыка()));
	ЗапросДанных.ВыполнитьПакет();
	ЗаписьЖурналаРегистрации(НСтр("ru='Тестирование.Завершено сохранение эталонных данных';uk='Тестування.Завершено збереження еталонних даних'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СравнитьСЭталоннымиДанными(ВременныеТаблицы, КонтрольныеРегистры, ПараметрыЗапроса = Неопределено, МассивРегистраторов = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Тестирование.Начато сравнение c эталонными данными';uk='Тестування.Розпочато порівняння c еталонними даними'",ОбщегоНазначения.КодОсновногоЯзыка()));
	
	Для каждого КонтрольныйРегистр Из КонтрольныеРегистры Цикл
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Выгрузили';uk='Вивантажили'",ОбщегоНазначения.КодОсновногоЯзыка()) + " " + КонтрольныйРегистр);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Тестирование.Завершено сравнение c эталонными данными';uk='Тестування.Завершено порівняння c еталонними даними'",ОбщегоНазначения.КодОсновногоЯзыка()));
