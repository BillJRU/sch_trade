...
Процедура ПостроитьДеревоЦен(Форма, ИмяТаблицы = "ДеревоЦен") Экспорт
	...
	ПодчиненныеЭлементыТаблицы = Форма.Элементы[ИмяТаблицы].ПодчиненныеЭлементы; // ЭлементыФормы
	...
	Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
		...
		Если ВидЦены.Выбрана ИЛИ ?(Форма.ОтображатьВлияющиеЦены, ВидЦены.Влияет, Ложь) Тогда
			...
			Если ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛиста(Форма) Или
				...
				КонецЕсли;
				ЗаголовокКолонкиСтараяЦена = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Цена на %1';uk='Ціна на %1'"), Формат(ДатаДействующихЦен, "ДЛФ=D"));
				
			Иначе
				
				ЗаголовокКолонкиСтараяЦена = ?(ВидЦены.Выбрана, НСтр("ru='Действующая цена';uk='Діюча ціна'"), НСтр("ru='Цена';uk='Ціна'"));
			...
			Если ВидЦены.Выбрана Тогда
				...
				СтруктураПараметров = Новый Структура();
				СтруктураПараметров.Вставить("Заголовок",              НСтр("ru='Изменение';uk='Зміна'"));
				...
				НовоеПоле.Подсказка = НСтр("ru='Изменение цены';uk='Зміна ціни'");
				...
				НовоеПоле.ЦветТекста = WebЦвета.Серый;
				НовоеПоле.ВыделятьОтрицательные = Истина;
				НовоеПоле.Подсказка = НСтр("ru='Процент изменения цены';uk='Відсоток зміни ціни'");
				
				НоваяГруппаЦена = ДобавитьГруппуФормы(Форма, "ГруппаЦена" + ИмяКолонки, НСтр("ru='Цена';uk='Ціна'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
				...
				СтруктураПараметров = Новый Структура();
				СтруктураПараметров.Вставить("Заголовок",              НСтр("ru='Цена';uk='Ціна'"));
				СтруктураПараметров.Вставить("ОбработчикПриИзменении", "Подключаемый_ДеревоЦенЦенаПриИзменении");
				СтруктураПараметров.Вставить("ШиринаПоля",             12);
				СтруктураПараметров.Вставить("ЦветФона",               ЦветФона);
				СтруктураПараметров.Вставить("ЦветФонаЗаголовка",      ЦветЗаголовка);
			...
			Если Форма.ИспользоватьУпаковкиНоменклатуры Тогда
				НоваяГруппаУпаковка = ДобавитьГруппуФормы(Форма, "ГруппаУпаковки" + ИмяКолонки, НСтр("ru='Ед. изм.';uk='Од. вим.'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
				...
				СтруктураПараметров = Новый Структура();
				СтруктураПараметров.Вставить("Заголовок",              НСтр("ru='Упаковка';uk='Упаковка'"));
				...
				СтруктураПараметров = Новый Структура();
				СтруктураПараметров.Вставить("Заголовок",              НСтр("ru='Ед. изм.';uk='Од. вим.'"));
	...
	ДеревоЦенЗначение = Форма.РеквизитФормыВЗначение(ИмяТаблицы); // ДеревоЗначений
...
//
// Возвращаемое значение:
//  см. ПодготовитьДанныеДляРасчетаВычисляемыхЦен
//
Функция ДобавитьТоварыПоОтбору(Форма,
	...
	Если ЗначениеЗаполнено(ЗаполняемыйВидЦен) Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаТовары Цикл
			...
			Если Форма.ИспользоватьУпаковкиНоменклатуры Тогда
				...
				СтрокаДереваЦен["Упаковка" + ИмяКолонкиВидаЦены] = СтрокаТаблицы.Упаковка;
	...
	Если НЕ ЭтоРегистрацияЦенПоставщика
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			ДанныеДляРасчетаВычисляемыхЦенНаКлиенте = ПодготовитьДанныеДляРасчетаВычисляемыхЦен(Форма, ТаблицаНоменклатуры, КэшДанных, МассивСтрокВидовЦен);
		Иначе
...
//
// Возвращаемое значение:
//  Структура - Данные для расчета, вычисляемых цен на клиенте:
//  * СтрокиТаблицыЦен - Массив из Структура - Структура состоит из:
//    ** Номенклатура - СправочникСсылка.Номенклатура - Номенклатура
//    ** Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - Характеристика номенклатуры
//  * ВидыЦен - Массив из СправочникСсылка.ВидыЦен - Отфильтрованный массив выидов цен
//            - Массив из СправочникСсылка.ВидыЦенПоставщиков -  
//  * СоответствиеКоэффициентовУпаковокНоменклатуры - см. КоэффициентыУпаковокНоменклатурыДереваТоваров
//  * КурсыВалют - см. КурсыВалют
//  * ТолькоНезаполненные - Булево - Указывает на необходимость расчета ранее заполненных цен
//
...
//   * НоменклатураПоставщика - СправочникСсылка.НоменклатураПоставщиков
...
Функция ДобавитьПолеФормы(Форма, Имя, ДополнительныеПараметры)
	...
	НовоеПоле                     = Форма.Элементы.Добавить("ДеревоЦен" + Имя, Тип("ПолеФормы"), ?(СтруктураПараметров.Родитель = Неопределено, Форма.Элементы.ДеревоЦен, СтруктураПараметров.Родитель)); // ПолеФормы
