...
Процедура ПриФормированииМанифестаКонфигурации(РасширенныеСведения) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ru='Операция не может быть выполнена при активной внешней транзакции';uk='Операція не може бути виконана за активної зовнішньої транзакції'");
	КонецЕсли;
...
Процедура ПодготовитьОбластьИзВыгрузки(Знач КодОбластиДанных, Знач ИдентификаторВыгрузки, Знач СопоставлениеПользователей, Знач ОбщиеДанные = Ложь) Экспорт
	...
	Попытка
		...
		Если Не МодульВыгрузкаЗагрузкаОбластейДанных.ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(ИмяФайлаВыгрузки) Тогда
			...
			ОтправитьСообщениеОСостоянииОбластиДанных(ТипСообщения, КодОбластиДанных, СообщениеОбОшибке, ДополнительныеСвойства);
			РазблокироватьДанныеДляРедактирования(КлючОбласти);
			Если ОбщиеДанные Тогда
...
Процедура ПриОшибкеПодготовкиОбластиИзВыгрузки(ПараметрыЗадания, ИнформацияОбОшибке) Экспорт
	...
	ИмяСобытия = НСтр("ru='Область данных.Ошибка при загрузке из файла';uk='Область даних.Помилка при завантаженні файлу'",ОбщегоНазначения.КодОсновногоЯзыка());
