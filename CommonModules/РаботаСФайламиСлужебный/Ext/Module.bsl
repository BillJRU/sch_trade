...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Файлы.Извлечение текста';uk='Файли.Витягування тексту'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Для Каждого ФайлБезТекста Из ФайлыДляИзвлеченияТекста Цикл
		...
		Если ЗначениеЗаполнено(ИмяФайлаСДвоичнымиДанными) Тогда
			...
			Если Файл.Существует() Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Извлечение текста';uk='Файли.Витягування тексту'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьСписокТипамиФайлов(Список) Экспорт
	...
	Список.Добавить("Изображения", НСтр("ru='Изображения (JPG, JPEG, PNG ...)';uk='Зображення (JPG, JPEG, PNG...)'"));
	Список.Добавить("ОфисныеДокументы", НСтр("ru='Офисные документы (DOC, DOCX, XLS ...)';uk='Офісні документи (DOC, DOCX, XLS ...)'"));
			...
			Если ЕстьФайлыВТомах И Не ПустаяСтрока(ПутьКАрхивуСФайламиТомов) И (Лев(ПутьКАрхивуСФайламиТомов, 2) <> "\\"
				...
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru='Путь к архиву с файлами томов должен быть задан
                               ...
                               |;uk='Шлях до архіву з файлами томів повинен бути заданий
			...
			Если Не ПустаяСтрока(ПолноеИмяФайловойБазы) И (Лев(ПолноеИмяФайловойБазы, 2) <> "\\" ИЛИ СтрНайти(ПолноеИмяФайловойБазы, ":") <> 0) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru='Путь к файловой информационной базе должен быть задан
                               |в формате UNC (\\servername\resource)'
                               |;uk='Шлях до файлової інформаційної бази повинен бути заданий
                               |у форматі UNC (\\servername\resource)'"),
					,
...
Процедура ВыполнитьОчисткуДанныхОВерсии(ФайлСсылка)
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Очистка ненужных файлов';uk='Очищення непотрібних файлів'",ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Если ДвоичныеДанные = Неопределено Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Не удалось добавить файл ""%1"" ни в один из томов, т.к. он отсутствует.
                                ...
                                |Обратитесь к администратору.'
                                |;uk='Не вдалося додати файл ""%1"" до жодного з томів, оскільки він відсутній.
                                |Можливо, файл вилучений антивірусною програмою.
                                |Зверніться до адміністратора.'"),
								ЭлементДанных.Наименование + "." + ЭлементДанных.Расширение);
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Добавление файла в том';uk='Файли.Додавання файлу в тому'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат НСтр("ru='Файлы.Не удалось отправить файл при обмене данными';uk='Файли.Не вдалося відправити файл при обміні даними'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбновитьДвоичныеДанныеФайлаВИнформационнойБазе(ФайлОбъект, ФайлСсылка, ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
		НСтр("ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';uk='Файли.Оновлення даних приєднаного файлу в сховище файлів'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СобытиеЖурналаРегистрацииСинхронизация()
	...
	Возврат НСтр("ru='Синхронизация файлов с облачным сервисом';uk='Синхронізація файлів з хмарним сервісом'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИзменитьФайлВОблачномСервисе(Знач ИзмененоСодержимоеНаСервере, ОбновитьСтатусСинхронизацииФайла, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	ТекстСобытия = НСтр("ru='Файл изменен: %1';uk='Файл змінено: %1'");
...
Функция УдалитьФайлВОблачномСервисе(Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСобытия = НСтр("ru='Не удален файл: %1';uk='Не видалений файл: %1'");
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), 
			ПараметрыСинхронизации.УчетнаяЗапись);
		Возврат Ложь;
	...
	ТекстСобытия = НСтр("ru='Удален файл: %1';uk='Вилучений файл: %1'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), 
		ПараметрыСинхронизации.УчетнаяЗапись);
