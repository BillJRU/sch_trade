	...
	Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда // прочтем настройки из пользовательского хранилища.
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ru='Для пользователя регламентного задания обмена не указано имя пользователя BAS Документооборота';uk='Для користувача регламентного завдання обміну не вказано ім''я користувача BAS Документообігу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		...
		Если ПарольСохранен <> Истина Тогда
			...
			ЗаписьЖурналаРегистрации(
				ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Ошибка,,,
				НСтр("ru='Для пользователя регламентного задания обмена не сохранен пароль BAS Документооборота';uk='Для користувача регламентного завдання обміну не збережено пароль BAS Документообігу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
//Возвращает измененные объекты, которые интегрированы с BAS Документооборотом. 
// Параметры:
...
Процедура ПодготовитьДанныеДляОтправки()
	...
	Для Каждого ИнтегрированныйОбъект Из ИнтегрированныеОбъекты Цикл
		...
		Исключение
			Инфо = ОписаниеОшибки();
			ЗаписьЖурналаРегистрации(
				ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(НСтр("ru='Выгрузка XDTO в XML';uk='Вивантаження XDTO в XML'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
				УровеньЖурналаРегистрации.Ошибка,
				,
				ОбъектXDTO.Тип().Имя,
...
Процедура ОтправитьДанные()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Интеграция с BAS Документооборотом.Отправка данных';uk='Інтеграція з BAS Документообігом.Відправка даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура ПолучитьДанные()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
				НСтр("ru='Получение данных';uk='Одержання даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка,
			,
			Запрос.Тип().Имя,
			Запрос.Тип().Имя + Символы.ПС + Инфо);
