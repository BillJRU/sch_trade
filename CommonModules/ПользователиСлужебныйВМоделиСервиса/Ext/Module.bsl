		...
		Если НЕ МодульРаботаВМоделиСервиса.СеансЗапущенБезРазделителей() Тогда
			ВызватьИсключение
				НСтр("ru='Изменение сведений о пользователях через удаленное администрирование
                           ...
                           |;uk='Зміна відомостей про користувачів через віддалене адміністрування
                           |доступне тільки нерозділеним користувачам.'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтарыйПользователь.ИдентификаторПользователяСервиса) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |;uk='Помилка під час запису користувача ""%1"".
                           |Заборонено змінювати вже встановлений ідентифікатор користувача сервісу в елементі довідника.'"),
		...
		Если ПользователиСлужебный.ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |;uk='Помилка під час запису користувача ""%1"".
                           |Заборонено встановлювати ідентифікатор користувача сервісу ""%2""
                           |в цей елемент довідника, тому що він вже використовується в іншому елементі ""%3"".'"),
				ПользовательОбъект.Наименование,
				АвтоРеквизиты.ИдентификаторПользователяСервиса,
				НайденныйПользователь);
		КонецЕсли;
...
Функция ДействияССуществующимПользователемСервиса(Знач Пользователь)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		ВызватьИсключение НСтр("ru='Работа в модели сервиса не предусмотрена.';uk='Робота в моделі сервісу не передбачена.'");
	КонецЕсли;
	...
	Для каждого ПраваДоступаОбъектаXDTO Из ПраваДоступаОбъектовXDTO.Item Цикл
		...
		ИначеЕсли ПраваДоступаОбъектаXDTO.Object.Тип() = ТипКонтактнойИнформации Тогда
			...
			Если ВидКонтактнойИнформации = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Неизвестный вид контактной информации: %1';uk='Невідомий вид контактної інформації: %1'"), ПраваДоступаОбъектаXDTO.Object.ContactType);
			КонецЕсли;
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неизвестный тип объектов доступа: %1';uk='Невідомий тип об''єктів доступу: %1'"), ПредставлениеТипа);
		КонецЕсли;
...
Функция ЭтоНеразделенныйПользовательИБ()
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если Не ПользовательЗарегистрированКакНеразделенный(ИдентификаторПользователя) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Пользователь с идентификатором %1 не зарегистрирован в качестве неразделенного.';uk='Користувач з ідентифікатором %1 не зареєстрований в якості нерозділеного.'"),
...
Функция ТекстИсключенияЗаписьНеразделенныхПользователейЗапрещена()
	...
	Возврат НСтр("ru='Запись неразделенных пользователей при включенном использовании разделителей запрещена.';uk='Запис нерозділених користувачів при включеному використанні розділювачів заборонена.'");
