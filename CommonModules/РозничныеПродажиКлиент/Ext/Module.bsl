...
// 	Дисплеи - СписокЗначений - Список дисплеев
...
Процедура ВывестиТекстНаДисплеиПокупателя(Форма, Дисплеи, Текст) Экспорт
	...
	Для каждого ЭлементСписка Из Дисплеи Цикл
		...
		Дисплей = ЭлементСписка.Значение; // СправочникСсылка.ПодключаемоеОборудование
...
//  		** ЭквайринговыеТерминалы - ДанныеФормыКоллекция - содержит:
...
Процедура ДобавитьОплатуКартой(РезультатПроведения, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ДополнительныеПараметры.ЭквайринговыеТерминалы.Количество() > 1 Тогда
		...
		Для каждого Терминал Из ДополнительныеПараметры.ЭквайринговыеТерминалы Цикл
			ТерминалЗначение = Терминал.Значение; // СправочникСсылка.ПодключаемоеОборудование
	...
	Если ЗначениеЗаполнено(ИдентификаторУстройстваЭТ)
		...
		Если РезультатЭТ Тогда
			...
			Иначе
				...
				ТекстВопроса = ТекстВопроса + НСтр("ru='Сумма операции:';uk='Сума операції:'")  + " "+ ТекущиеДанные.Сумма               + Символы.ПС;
				ТекстВопроса = ТекстВопроса + НСтр("ru='Номер карты:';uk='Номер карти:'")     + " "+ ТекущиеДанные.НомерПлатежнойКарты + Символы.ПС;
				ТекстВопроса = ТекстВопроса + НСтр("ru='Ссылочный номер:';uk='Посилальний номер:'") + " "+ ТекущиеДанные.СсылочныйНомер      + Символы.ПС;
				ТекстВопроса = ТекстВопроса + НСтр("ru='Номер чека:';uk='Номер чека:'")      + " "+ ТекущиеДанные.НомерЧекаЭТ         + Символы.ПС;
...
Процедура ДобавитьОплатуКартойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекстВопроса = ТекстВопроса + НСтр("ru='Сумма операции:';uk='Сума операції:'") + " "+ Результат.Сумма + Символы.ПС;
		ТекстВопроса = ТекстВопроса + Символы.ПС;
...
|;uk='При виконанні операції виникла помилка:
|""%1"".
|Скасування оплати по карті не була проведена.'"),
			РезультатВыполнения.ОписаниеОшибки);
...
|%1'
|;uk='При відкритті зміни сталася помилка. 
|Зміна не відкрита на фіскальному пристрої. 
|Додатковий опис: 
|%1'"),
			РезультатВыполнения.ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
|
|Закрыть смену?'
|;uk='При знятті звіту на фіскальному пристрої сталася помилка.
|""%ОписаниеОшибки%"" 
|
|Якщо зміна на фіскальному пристрої закрита, то
...
|;uk='При знятті звіту на фіскальному пристрої сталася помилка.
|""%ОписаниеОшибки%"" 
|Звіт на фіскальному пристрої не сформований.'");
...
|Дополнительное описание:
|%1'
|;uk='При внесенні коштів у касу ККМ сталася помилка. 
|Додатковий опис: 
|%1'"),
				ОписаниеОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
