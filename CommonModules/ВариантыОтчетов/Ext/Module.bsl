...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Неприпустиме значення параметра Звіт в функції ВариантыОтчетов.ОписаниеОтчета.
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недопустимое значение параметра КлючВарианта в функции ВариантыОтчетов.ОписаниеВарианта:
                ...
                |;uk='Неприпустиме значення параметра КлючВарианта в функції ВариантыОтчетов.ОписаниеВарианта:
                |варіант ""%1"" відсутній в звіті ""%2"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав для просмотра.';uk='Недостатньо прав для перегляду.'");
		КонецЕсли;
...
Функция ИнформацияОбОтчете(Знач ОтчетПолноеИмя, Знач ВызыватьИсключение = Ложь) Экспорт
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный тип отчета ""%1"" (ожидался ""Отчет"" или ""ВнешнийОтчет"").';uk='Невідомий тип звіту ""%1"" (очікувалося ""Отчет"" або ""ВнешнийОтчет"").'"),
			ОтчетПолноеИмя);
...
Функция ХранилищеВариантовДополнительногоОтчетаКорректное(МетаданныеОтчета, ТекстПредупреждения = "") Экспорт 
	...
	ШаблонПредупреждения = НСтр("ru='Свойство отчета ""Хранилище вариантов"" %1.
        ...
        |;uk='Властивість звіту ""Сховище варіантів"" %1.
        |Збереження (вибір) варіантів звіту буде працювати в обмеженому режимі.
        |Зверніться до розробника додаткового (зовнішнього) звіту.'");
	
	ПараметрПредупреждения = НСтр("ru='заполнено не корректно';uk='заповнено некоректно'");
	Если МетаданныеОтчета.ХранилищеВариантов = Неопределено Тогда 
		ПараметрПредупреждения = НСтр("ru='не заполнено';uk='не заповнено'");
	КонецЕсли;
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось прочитать схему отчета:
                        |%1'
                        |;uk='Не вдалося розпізнати схему звіту:
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось прочитать список вариантов отчета:
                            |%1'
                            |;uk='Не вдалося розпізнати список варіантів звіту:
                            |%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось прочитать список вариантов отчета из модуля менеджера:
                            |%1'
                            |;uk='Не вдалося розпізнати список варіантів звіту з модуля менеджера:
                            |%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не удалось прочитать настройки варианта ""%1"":
                                    |%2'
                                    |;uk='Не вдалося розпізнати настройки варіанта ""%1"":
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Перенос всех настроек доступности вариантов отчетов в регистр сведений ""Настройки вариантов отчетов"".
        ...
        |;uk='Перенесення всіх налаштувань доступності варіантів звітів в регістр відомостей ""Налаштування варіантів звітів"".
        |До завершення обробки, настройка доступності варіантів звітів в розрізі користувачів (груп користувачів) буде працювати некоректно.'");
...
Функция НаименованиеПодсистемы(КодЯзыка)
	Возврат НСтр("ru='Варианты отчетов';uk='Варіанти звітів'",?(КодЯзыка=Неопределено,ОбщегоНазначения.КодОсновногоЯзыка(),КодЯзыка));
КонецФункции
...
Процедура ОперативноеОбновлениеСлужебногоПользователя() Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Варианты отчетов.Обновление служебного пользователя';uk='Варіанти отчетов.Обновление службового користувача'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОбновитьИндексПоиска(Режим, ИндексироватьСхему)
	...
	Уточнение = НРег(ПредставлениеРежима(Режим)) + ", " + ?(ИндексироватьСхему, НСтр("ru='полное';uk='повне'"), НСтр("ru='по изменениям';uk='по змінам'"));
	
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление индекса поиска (%1)';uk='Оновлення індексу пошуку (%1)'"), Уточнение);
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ОбщиеДанные Тогда
				...
				Если Найденные.Количество() = 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Вариант ""%1"" не найден для отчета ""%2""';uk='Варіант ""%1"" не знайдено для звіту ""%2""'"), 
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось перестроить индекс поиска для варианта ""%1"" отчета ""%2"". Возможно, отчет неисправен.';uk='Не вдалося перебудувати індекс пошуку для ""%1"" звіту ""%2"". Можливо, звіт несправний.'"), 
					ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обновление ключей вариантов отчетов (%1)';uk='Оновлення ключів варіантів звітів (%1)'"), 
		?(Режим = "ОбщиеДанныеКонфигурации", НСтр("ru='метаданные конфигурации';uk='метадані конфігурації'"), НСтр("ru='метаданные расширений';uk='метадані розширень'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьУдаленныеПредопределенные(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обновление настроек предопределенных (%1)';uk='Оновлення настройок напередвизначених (%1)'"), 
		?(Режим = "ОбщиеДанныеКонфигурации", НСтр("ru='метаданные конфигурации';uk='метадані конфігурації'"), НСтр("ru='метаданные расширений';uk='метадані розширень'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьВариантыОтчетовПоПредопределенным(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление вариантов отчетов (%1)';uk='Оновлення варіантів звітів (%1)'"), 
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удаление вариантов удаленных отчетов (%1)';uk='Вилучення варіантів віддалених звітів (%1)'"), 
		НРег(ПредставлениеРежима(Режим)));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПараметрыЗаполненияПредставленийВариантовОтчетов(Языки = Неопределено, ИндексТекущегоЯзыка = 0)
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Заполнение представлений предопределенных вариантов отчетов для языка %1';uk='Заповнення представлень напередвизначених варіантів звітів для мови %1'"), КодЯзыка);
...
Функция СлужебныйПользователь(Знач КодЯзыка)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Варианты отчетов.Создание служебного пользователя';uk='Варіанти отчетов.Сотворення службового користувача'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.Пользователи,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПредставлениеРежима(Режим)
	...
	Режимы.Вставить("ОбщиеДанныеКонфигурации", НСтр("ru='Общие данные конфигурации';uk='Спільні дані конфігурації'"));
	Режимы.Вставить("ОбщиеДанныеРасширений", НСтр("ru='Общие данные расширений';uk='Спільні дані розширень'"));
	Режимы.Вставить("РазделенныеДанныеКонфигурации", НСтр("ru='Разделенные данные конфигурации';uk='Розділені дані конфігурації'"));
	Режимы.Вставить("РазделенныеДанныеРасширений", НСтр("ru='Разделенные данные расширений';uk='Розділені дані розширень'"));
...
Процедура УстановитьУсловноеОформлениеСпискаПользователейВариантаОтчета(Форма) Экспорт 
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Все пользователи';uk='Всі користувачі'"));
...
Функция ПользовательскиеНастройкиВариантаОтчета(ВариантОтчета, Пользователь, КлючНастройки = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Чтение пользовательских настроек отчета';uk='Читання користувацьких настройок звіту'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,
...
Функция ЗаполнитьПоляДляПоиска(ВариантОбъект, СведенияОбОтчете = Неопределено) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Для варіанту ""%1"" звіту ""%2"" не заповнені настройки пошуку:
            |Найменування полів, параметрів і відборів.'"),
			ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Функция ОписаниеНастроекВариантаОтчета(ИмяКаталога)
	...
	ШаблонОписанияОшибки = НСтр("ru='Некорректный формат описания настроек: %1';uk='Некоректний формат опису настройок: %1'");
	...
	Исключение
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует файл SettingsDescription.xml';uk='відсутній файл SettingsDescription.xml'"));
	...
	Если ДокументDOM.ДочерниеУзлы.Количество() = 0 Тогда 
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует элемент SettingsDescription';uk='відсутній елемент SettingsDescription'"));
	...
	Если ИмяОтчета = Неопределено Тогда 
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут ReportName';uk='відсутній атрибут ReportName'"));
	...
	Если Состав.Количество() = 0 Тогда 
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует элемент Settings';uk='відсутній елемент Settings'"));
	...
	Если КлючВарианта = Неопределено Тогда 
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут Key элемента Settings';uk='відсутній атрибут Key елемента Settings'"));
	...
	Если ПредставлениеВарианта = Неопределено Тогда 
		ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут Presentation элемента Settings';uk='відсутній атрибут Presentation елемента Settings'"));
	...
	Для Индекс = 1 По ГраницаСостава Цикл 
		...
		Если КлючНастроек = Неопределено Тогда 
			ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут Key элемента UserSettings';uk='відсутній атрибут Key елемента UserSettings'"));
		...
		Если ПредставлениеНастроек = Неопределено Тогда 
			ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут Presentation элемента UserSettings';uk='відсутній атрибут Presentation елемента UserSettings'"));
		...
		Если ЭтоТекущиеПредставление = Неопределено Тогда 
			ОписаниеНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписанияОшибки, НСтр("ru='отсутствует атрибут isCurrent элемента UserSettings';uk='відсутній атрибут isCurrent елемента UserSettings'"));
...
Функция ДесериализованныеНастройки(ИмяФайла)
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректный формат настроек: отсутствует файл %1';uk='Некоректний формат настройок: відсутній файл %1'"),
...
Функция ШаблонНаименованияВариантаОтчета(НаименованияЗанятые, Знач Наименование, ШаблонСчетчикаКопий)
	...
	ПризнакКопии = " - " + НСтр("ru='копия';uk='копія'");
...
Процедура ПоделитьсяПользовательскимиНастройками(ВыбранныеПользователи, ШаблонОписанияНастроек) Экспорт 
	...
	Если СвойстваПользователей.Действительных.Количество() = 0 Тогда 
		...
		ШаблонОписанияНастроек.Вставить("Предупреждение", НСтр("ru='Выбранные пользователи недействительны';uk='Вибрані користувачі недійсні'"));
		Возврат;
	...
	Если СвойстваПользователей.Недействительных.Количество() > 0 Тогда 
		...
		ШаблонПояснения = НСтр("ru='Некоторые из выбранных пользователей недействительны: %1';uk='Деякі з обраних користувачів недійсні: %1'");
...
Процедура УстановитьПредставлениеПользовательскихНастроек(ШаблонОписанияНастроек, ТекущийПользователь)
	...
	ШаблонПредставления = НСтр("ru='Настройки %1';uk='Настройки %1'");
...
Процедура ОповеститьПользователейНастроекОтчета(СвойстваПользователей, ОписаниеНастроек)
	...
	ЗаголовокОбсуждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Доступ к настройкам отчета ""%1""';uk='Доступ до настройок звіту ""%1""'"), ПредставлениеВариантаОтчета);
	...
	Если ЗначениеЗаполнено(ОписаниеНастроек.Представление) Тогда 
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Предоставлены настройки отчета ""%1""';uk='Надані настройки звіту ""%1""'"), ОписаниеНастроек.Представление);
	Иначе
		Текст = НСтр("ru='Предоставлены настройки отчета';uk='Надані настройки звіту'");
	КонецЕсли;
	
	ИмяДействия = ВариантыОтчетовКлиентСервер.ИмяДействияПрименитьПереданныеНастройки();
	
	Сообщение.Текст = Текст;
	Сообщение.Действия.Добавить(ИмяДействия, НСтр("ru='Применить настройки?';uk='Застосувати настройки?'"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Варианты отчетов.Оповещение о предоставлении доступа к настройкам отчета';uk='Варіанти звітів.Сповіщення про надання доступу до настройок звіту'",КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,
