...
Процедура ЗаполнитьРеквизитРасчетПартийВДвижениях(ПараметрыРасчета) Экспорт
	...
	ОписаниеРегистра    = ПараметрыРасчета.Движения[Метаданные.РегистрыНакопления.КорректировкиНДСПартий.Имя];
	ТекстРасчетПартий   = "
	|   ВЫБОР КОГДА Т.РасчетПартий ТОГДА ИСТИНА
	|		КОГДА ( 
	|   		(Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПредполагаемаяУсловнаяПродажа)
	|   			ИЛИ Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПредполагаемоеВосстановлениеНДС) 
	|			)
	|			И НЕ Т.Регистратор ССЫЛКА Документ.ВыкупВозвратнойТарыУПоставщика
	|		)
	|		ТОГДА ИСТИНА 
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ
	|";
	ТекстСоединения     = "";
	ТекстУпорядочивание = "Т.Организация, Т.АналитикаУчетаНоменклатуры";
	ЗаполнитьРеквизитРасчетПартий(ПараметрыРасчета, ОписаниеРегистра, ТекстРасчетПартий, ТекстСоединения, ТекстУпорядочивание);
	
	// НДСУсловныеПродажи
	ОписаниеРегистра    = ПараметрыРасчета.Движения[Метаданные.РегистрыНакопления.НДСУсловныеПродажи.Имя];
	ТекстРасчетПартий   = "
	|   ВЫБОР КОГДА Т.РасчетПартий ТОГДА ИСТИНА
	|		КОГДА ( 
	|			(Т.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|   			ИЛИ Т.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|			)
	|			И НЕ Т.Регистратор ССЫЛКА Документ.ВыкупВозвратнойТарыУПоставщика
	|		)
	|		ТОГДА ИСТИНА 
	...
	|";
	ТекстСоединения     = "";
	ТекстУпорядочивание = "Т.Организация, Т.Номенклатура";
	ЗаполнитьРеквизитРасчетПартий(ПараметрыРасчета, ОписаниеРегистра, ТекстРасчетПартий, ТекстСоединения, ТекстУпорядочивание);
...
Функция ПроверитьНеобходимостьЗаполненияРеквизитаРасчетПартий(ПараметрыРасчета)
	...
	// Проверим наличие движений КорректировкиНДСПартий с незаполненным расчетом партий
	ТекстЗапроса = ТекстЗапроса
	+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + "
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	""%1"" КАК РегистрСоСтарымиДвижениями
	...
	|	И Т.Организация В(&МассивОрганизаций)
	|	И НЕ Т.РасчетПартий      
	|   И (Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПредполагаемаяУсловнаяПродажа)
	|   	ИЛИ Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПредполагаемоеВосстановлениеНДС)
	|	)
	|	И НЕ Т.Регистратор ССЫЛКА Документ.ВыкупВозвратнойТарыУПоставщика
	|";
	ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстЗапроса,
		Метаданные.РегистрыНакопления.КорректировкиНДСПартий.ПолноеИмя()
	); 
	
	// Проверим наличие движений НДСУсловныеПродажи с незаполненным расчетом партий
	ТекстЗапроса = ТекстЗапроса
	+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + "
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	""%1"" КАК РегистрСоСтарымиДвижениями
	...
	|	И Т.Организация В(&МассивОрганизаций)
	|	И НЕ Т.РасчетПартий      
	|   И (Т.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|   	ИЛИ Т.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|	)
	|	И НЕ Т.Регистратор ССЫЛКА Документ.ВыкупВозвратнойТарыУПоставщика
	|";
	ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстЗапроса,
		Метаданные.РегистрыНакопления.НДСУсловныеПродажи.ПолноеИмя()
	);
		...
		Если Выборка.Количество() > 0 Тогда
			...
			РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Очищено устаревших движений по регистру %1: %2';uk='Очищено застарілих рухів по регістру %1: %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
					ИмяРегистра,
...
Процедура ОчиститьОстаткиРегистраПУВерсии21(ПараметрыРасчета, Запрос, ИмяРегистра)
	...
	Если НЕ ДвиженияПоРегистру.Пустой() Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Сформировано движений сторнирования остатков по регистру %1: %2';uk='Сформовано рухів сторнування залишків по регістру %1: %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяРегистра,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.Количество())));
	...
	ИначеЕсли Не РегистраторыКОчистке.Пустой() Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Очищено движений сторнирования остатков по регистру %1: %2';uk='Очищено рухів сторнування залишків по регістру %1: %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяРегистра,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.Количество())));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КодПричины = 1 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
                |    - " + НСтр("ru='есть движения по себестоимости товаров, сформированные партионным учетом версии 2.5';uk='є рухи по собівартості товарів, сформовані партіонним обліком версії 2.5'",ОбщегоНазначения.КодОсновногоЯзыка());
		ИначеЕсли Выборка.КодПричины = 3 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru='в данной информационной базе партионный учет версии 2.1 не использовался';uk='в даній інформаційній базі облік по партіях версії 2.1 не використовувався'",ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			ВызватьИсключение НСтр("ru='Неизвестный код ошибки.';uk='Невідомий код помилки.'",ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
		ТекстПредупреждения = НСтр("ru='Невозможен возврат к партионному учету версии 2.1:';uk='Неможливо повернення до партіонного обліку версії 2.1:'",ОбщегоНазначения.КодОсновногоЯзыка())
			+ ТекстПредупреждения;;
	КонецЕсли;
...
Функция ТекстРеализацииПрошлыхМесяцев_ПУ21() // использует ПрошлыеРеализации 
	Возврат 
        ...
        |			ТОГДА Партии.АналитикаУчетаПартий.НалоговоеНазначение
        |		ИНАЧЕ Партии.НалоговоеНазначение КОНЕЦ) КАК НалоговоеНазначение,
		|
        ...
        |	ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)     КАК НалоговоеНазначениеДокумента,
		|	ВЫРАЗИТЬ(0 КАК ЧИСЛО(31,2)) 											КАК НДСУпр,
        |	ВЫРАЗИТЬ(0 КАК ЧИСЛО(31,2)) 											КАК НДСРеглРеквизит,
        ...
        |		КОГДА Партии.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА Партии.АналитикаУчетаПартий.НалоговоеНазначение
        |		ИНАЧЕ Партии.НалоговоеНазначение КОНЕЦ),
...
Функция ТекстНачальныеОстаткиДляКорректировкиПриобретения_ПУ21()
	Возврат 
        ...
        |	ВЫБОР
        |		КОГДА Партии.НалоговоеНазначение ЕСТЬ NULL
        |			ТОГДА ДД.НалоговоеНазначение
        |		КОГДА Партии.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА Партии.АналитикаУчетаПартий.НалоговоеНазначение 
        |		ИНАЧЕ Партии.НалоговоеНазначение
...
Процедура СкорректироватьСтоимостьСписанияЗапасовПоФИФОСкользящая(ПараметрыРасчета)
	...
	|	УчетСебестоимости.НалоговоеНазначение21      	КАК НалоговоеНазначение21,
    ...
    |	УчетСебестоимости.НалоговоеНазначение21,
	...
	|		КОГДА Партии.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца))
	|			ТОГДА 0
	|		ИНАЧЕ Партии.СтоимостьРеглБезНДС КОНЕЦ) КАК СтоимостьРеглБезНДС,
	|	СУММА(ВЫБОР
	|		КОГДА Партии.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца))
	|			ТОГДА 0
	|		ИНАЧЕ Партии.НДСРеглРесурс КОНЕЦ) КАК НДСРегл,
	|	СУММА(ВЫБОР
	|		КОГДА Партии.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца))
    ...
    |ДокументДвижения, ИдентификаторСтроки, КодСтроки, ИсточникГФУНоменклатуры, КорИсточникГФУНоменклатуры,
    |НалоговоеНазначение21, КорНалоговоеНазначение,
    |ПериодПродажи, ГруппаПродукции, НаправлениеДеятельности, КорНаправлениеДеятельности,
    ...
    |ДопРасходы, ДопРасходыБезНДС, СтоимостьРегл, СтоимостьЗабалансоваяРегл, НДСРеглРеквизит, НДСУпр
	|");
	
    СуммыРаспределения = Новый Структура("Количество, Стоимость, СтоимостьБезНДС, СтоимостьЗабалансовая, СтоимостьРегл, СтоимостьЗабалансоваяРегл, НДСРеглРеквизит,
	|СтоимостьРеглБезНДС, НДСРегл, 
    ...
    |Количество, Стоимость, СтоимостьБезНДС, СтоимостьЗабалансовая, СтоимостьРегл, СтоимостьЗабалансоваяРегл, НДСРеглРеквизит,
    |СтоимостьРеглБезНДС, НДСРегл,
	|ДопРасходы, ДопРасходыБезНДС, ПостояннаяРазница, ВременнаяРазница
	|");
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
			...
			Если (Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваров
				 ...
				 И Выборка.РазделУчета <> Перечисления.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку Тогда
					...
					Иначе
                        ...
                        СуммовыеПоказатели.Вставить("СуммаРегл", ЗаписьРаспределения.СтоимостьРегл + ЗаписьРаспределения.НДСРеглРеквизит
                        	+ ?(ЭтоВыпускПродукции, Выборка.ИсходнаяСтоимостьРегл, 0));
                        СуммовыеПоказатели.Вставить("СуммаРеглБезНДС", ЗаписьРаспределения.СтоимостьРеглБезНДС 
                        	+ ?(ЭтоВыпускПродукции, Выборка.ИсходнаяСтоимостьРеглБезНДС, 0));
                        СуммовыеПоказатели.Вставить("НДСРегл", ЗаписьРаспределения.НДСРегл 
                        	+ ?(ЭтоВыпускПродукции, Выборка.ИсходнаяНДСРегл, 0));
			 ...
			 СуммаПереоценки = ЗаписьРаспределения.Стоимость + ЗаписьРаспределения.ДопРасходы - Выборка.КорСтоимость;
				...
				ИначеЕсли СуммаПереоценки < 0 Тогда
					...
					РасчетСебестоимостиКорректировкаСтоимости.СформироватьДвиженияПрочиеДоходы(
						...
						ЗаписьРаспределения,
						- СуммаПереоценки,
                        - (ЗаписьРаспределения.СтоимостьРегл + ЗаписьРаспределения.НДСРеглРеквизит),
						0,
            ...
            Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровПоставщику 
                И ЗаписьРаспределения.КорСтоимость <> 0
    			И ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов 
                И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.НалоговоеНазначение21, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая 
                ...
                Тогда
                    ...
                    СуммовыеПоказатели.Вставить("СуммаРегл", Выборка.ИсходнаяНДСРегл);
                    СуммовыеПоказатели.Вставить("СуммаРеглБезНДС", Выборка.ИсходнаяНДСРегл);
...
Процедура СкорректироватьСтоимостьПродажПоФИФОСкользящая(ПараметрыРасчета)
	...
	|	0 КАК СтоимостьЗабалансоваяРегл,
    |	Партии.НДСРегл КАК НДСРеглРеквизит,
    ...
    |СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, СтоимостьЗабалансоваяРегл, НДСРеглРеквизит, ПостояннаяРазница, ВременнаяРазница
	|");
	
	СтрокаОстатка = Новый Структура("
	|Регистратор, Организация, АналитикаУчетаНоменклатуры, ВидЗапасов, ТипЗапасов,
    |Количество, Стоимость, СтоимостьБезНДС, СтоимостьЗабалансовая, СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, СтоимостьЗабалансоваяРегл, НДСРеглРеквизит,
	|ДопРасходы, ДопРасходыБезНДС, ПостояннаяРазница, ВременнаяРазница
	|");
...
Процедура СформироватьКорДвиженияСебестоимостьТоваров(ПараметрыРасчета, ДанныеДвижения, ПромежуточнаяСебестоимость = Ложь)
	...
	Запись.НалоговоеНазначение21      = ДанныеДвижения.КорНалоговоеНазначение;
	Запись.КорНалоговоеНазначение     = ДанныеДвижения.НалоговоеНазначение21;
	Запись.Количество 				  = 0;
    ...
    ИначеЕсли ДанныеДвижения.НДСРеглРеквизит <> 0 ИЛИ ДанныеДвижения.НДСУпр <> 0 Тогда // это не передача
        ...
        КопируемыеПоля = " Период, ХозяйственнаяОперация, ДокументДвижения, ИдентификаторСтроки, 
		|ГруппаПродукции,
        ...
        Если ПараметрыРасчета.ПартионныйУчетВерсии22 Тогда
            ...
            КонецЕсли;
        ИначеЕсли ПараметрыРасчета.ПартионныйУчетВерсии21 Тогда
    		Запись.НалоговоеНазначение21        = ДанныеДвижения.КорНалоговоеНазначение;
    		Запись.КорНалоговоеНазначение       = ДанныеДвижения.НалоговоеНазначение21;
		КонецЕсли;
...
Процедура ПолучитьСебестоимостьПартийТоваров(ПараметрыРасчета, ДляСтоимостиПродаж = Ложь)
	...
	Запрос.УстановитьПараметр("ВключатьНДСВСебестоимостьПриУсловнойПродаже", НДСОбщегоНазначенияПовтИсп.ВключатьНДСВСебестоимостьПриУсловнойПродаже());
	Запрос.УстановитьПараметр("ИсключатьНДСИзСебестоимостиПриВосстановленииНДС", НДСОбщегоНазначенияПовтИсп.ИсключатьНДСИзСебестоимостиПриВосстановленииНДС());
    ...
    |	ВЫБОР КОГДА (НЕ Возврат.Ссылка ЕСТЬ NULL ИЛИ НЕ Корректировка.Ссылка ЕСТЬ NULL ИЛИ НЕ ВозвратРозница.Ссылка ЕСТЬ NULL)
    ...
    |			ИЛИ НАЧАЛОПЕРИОДА(ВозвратРозница.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Корректировка.Дата, МЕСЯЦ)
	...
	|              И Партии.НалоговоеНазначениеПартии.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая)
	|		) 
	|		ТОГДА
	|			ВЫБОР КОГДА Партии.Знаменатель < 0 ТОГДА
	...
	|		И Партии.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
    |	И НЕ (Партии.Регистратор ССЫЛКА Документ.КорректировкаНалоговогоНазначенияЗапасов И Партии.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
	|
	|СГРУППИРОВАТЬ ПО
	|	Партии.Организация,
	|	(ВЫБОР
	...
	|		0,
	|		ВЫБОР
	|		    КОГДА ПартииНаКомиссии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.НеОблагаемая) 
	|				ТОГДА
	...
	|		ВЫБОР КОГДА 
	|			(
	|				(ПартииНаКомиссии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|	   				И ПартииНаКомиссии.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
	|				ИЛИ (ПартииНаКомиссии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
	|	   				И ПартииНаКомиссии.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
	|           )
	|		    И &ВключатьНДСВСебестоимостьПриУсловнойПродаже
	|		ТОГДА
	|			ВЫБОР КОГДА ПартииНаКомиссии.Количество < 0 ТОГДА
	...
	|
	|		КОГДА ПартииНаКомиссии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
	|		    И ПартииНаКомиссии.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|		    И &ИсключатьНДСИзСебестоимостиПриВосстановленииНДС
	|		ТОГДА
	|			ВЫБОР КОГДА ПартииНаКомиссии.Количество < 0 ТОГДА
	|				ПартииНаКомиссии.НДСРегл
    ...
    |		ВЫБОР
    |			КОГДА ПартииНаКомиссии.Количество < 0 
    |			ТОГДА 0 - ПартииНаКомиссии.СтоимостьРегл
    |			ИНАЧЕ ПартииНаКомиссии.СтоимостьРегл
    |		КОНЕЦ КАК СтоимостьРеглБезНДС,
    |		ВЫБОР
    |			КОГДА ПартииНаКомиссии.Количество < 0 
    |			ТОГДА 0 - ПартииНаКомиссии.НДСРегл
    |			ИНАЧЕ ПартииНаКомиссии.НДСРегл
...
Процедура ЗарегистрироватьСтоимостьФИФО(ПараметрыРасчета)
	...
	|	ВЫРАЗИТЬ((Себестоимость.СтоимостьРеглБезНДС / Себестоимость.Количество) КАК Число(31,10)) 		КАК СтоимостьРеглБезНДС,
	|	ВЫРАЗИТЬ((Себестоимость.НДСРегл / Себестоимость.Количество) КАК Число(31,10)) 				    КАК НДСРегл,
...
Процедура ВедомостьПоПартиямТоваров_ДополнитьТекстЗапроса(ТекстЗапроса) Экспорт
	ТекстЗапросаПУ21 = "
		...
		|	ДопРасходы.АналитикаУчетаПартий.НалоговоеНазначение КАК НалоговоеНазначение,
        ...
        |		ВЫБОР КОГДА ДопРасходы.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 	
		|		ТОГДА
		...
		|	Затраты.АналитикаУчетаПартий.НалоговоеНазначение КАК НалоговоеНазначение,
