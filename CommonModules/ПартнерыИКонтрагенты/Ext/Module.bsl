...
//      ВидКонтактнойИнформации - СправочникСсылка.ВидыКонтактнойИнформации, ПеречислениеСсылка.ТипыКонтактнойИнформации,
...
Процедура УстановитьОтборыДинамическийСписокПартнеры(Форма) Экспорт
	...
	Если Форма.СписокОтборПоТипуПартнера.Количество() > 0 Тогда
		...
		Для каждого ЭлементСписка Из Форма.СписокОтборПоТипуПартнера Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Разделитель = " "  + ?(УстанавливатьОтборПоТипуПартнераКакИЛИ, НСтр("ru='или';uk='або'"), НСтр("ru='и';uk='і'")) + " ";
		УсловияОтбора = СтрСоединить(МассивУсловийОтбора, Разделитель);
		
		Форма.АвтоЗаголовок = Ложь;
		ШаблонЗаголовка = ?(Форма.ИспользоватьПартнеровКакКонтрагентов, НСтр("ru='Контрагенты (%1)';uk='Контрагенти (%1)'"), НСтр("ru='Партнеры (%1)';uk='Партнери (%1)'"));
...
Функция ПредставлениеОтбораПоТипуПартнера(ЗначениеОтбора)
	...
	Если ЗначениеОтбора = "Клиент" Тогда
		Возврат НСтр("ru='Клиенты';uk='Клієнти'");
	ИначеЕсли ЗначениеОтбора = "Поставщик" Тогда
		Возврат НСтр("ru='Поставщики';uk='Постачальники'");
	ИначеЕсли ЗначениеОтбора = "Конкурент" Тогда
		Возврат НСтр("ru='Конкуренты';uk='Конкуренти'");
	ИначеЕсли ЗначениеОтбора = "ПрочиеОтношения" Тогда
		Возврат НСтр("ru='Прочие отношения';uk='Інші відносини'");
	ИначеЕсли ЗначениеОтбора = "ОбслуживаетсяТорговымиПредставителями" Тогда
		Возврат НСтр("ru='Обслуживается торговыми представителями';uk='Обслуговується торговими представниками'");
	ИначеЕсли ЗначениеОтбора = "Перевозчик" Тогда
		Возврат НСтр("ru='Перевозчики';uk='Перевізники'");
	Иначе
		Возврат "";
	КонецЕсли;
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер, СИсторическимиДанными = Истина) Экспорт
	...
	КонецЕсли;
	ПартнерыИКонтрагентыЛокализация.ДополнитьДанныеКонтрагентаПартнера(Приемник, Партнер, СИсторическимиДанными = Истина);
КонецПроцедуры

// Заполняет контактную информацию контрагента по контактной информации партнера.
// 
// Параметры:
// 	КонтрагентОбъект - СправочникСсылка.Контрагенты - 
// 	Форма     - ФормаКлиентскогоПриложения - 
// 	КонтактнаяИнформацияПартнера - ТаблицаЗначений -
// 	ПереноситьЮридическийАдрес - Булево - 
...
Процедура ПроверитьКорректностьЗаполненияКонтрагента(Объект, Контрагент, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.РегистрационныйНомер) И Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
		...
		Если ВыборкаКонтрагент <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='Данные регистрационного номера в этой же стране регистрации уже указаны для контрагента с кодом %1, ответственный - %2.';uk='Дані реєстраційного номера в цій же країні реєстрації вже вказані для контрагента з кодом %1, відповідальний - %2.'");
...
Процедура ЗаполнитьСписокВыбораТипФильтраСписокПартнеров(Форма, СписокВыбора)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГруппыДоступаПартнеров")
		...
		СписокВыбора.Добавить("ГруппыДоступа", НСтр("ru='группе доступа';uk='групі доступу'"));
	КонецЕсли;
