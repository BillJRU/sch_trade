...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ТоварыКОформлениюТаможенныхДеклараций);
		...
		Если Документ.ДополнительныеСвойства.Свойство("ЗаписыватьРезервыТоваровОрганизацийВместеСоВсеми")
			...
			ПроведениеДокументов.ДобавитьТаблицуДанныхДокумента(Документ, "ТаблицаРезервыТоваровОрганизаций",
				Документ.ДополнительныеСвойства.ТаблицаРезервыТоваровОрганизаций);
		Иначе
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Свойства.ЭтоНовый
			...
			Или Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
				И ТипДокумента = Тип("ДокументОбъект.ТаможеннаяДекларацияИмпорт") Тогда
			Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ТоварыКОформлениюТаможенныхДеклараций);
		КонецЕсли;
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения , "ТоварыКОформлениюТаможенныхДеклараций");
...
//  		** ДвиженияТоварыКОформлениюТаможенныхДекларацийИзменение - Произвольный
//
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	#Область ТоварыКОформлениюТаможенныхДеклараций
	
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТоварыКОформлениюТаможенныхДекларацийИзменение") Тогда
		...
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ОшибкиТоварыКОформлениюТаможенныхДеклараций");
	...
	Если Не Документ.ДополнительныеСвойства.Свойство("ОтказПриЗаполненииВидовЗапасов")
		...
		Пока Индекс < КоличествоСтрок Цикл
			...
			Если КонтролироватьОтрицательныеОстатки
				И СтрокаОшибки.КоличествоОстаток + СтрокаОшибки.РезервОстаток < 0 Тогда
				
				ТекстСообщения = НСтр("ru='По организации %Организация% на %Месяц% обнаружен отрицательный остаток %Количество% %ЕдиницаИзмерения% товара %Товар% в месте хранения %МестоХранения%';uk='По організації %Организация% на %Месяц% виявлений від''ємний залишок%Количество% %ЕдиницаИзмерения% товару %Товар% в місці зберігання %МестоХранения%'");
				
				Если СтрокаОшибки.Период = ДатаАктуальныхОстатков() Тогда
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Месяц%", НСтр("ru='дату актуальности остатков';uk='дату актуальності залишків'"));
				КонецЕсли;
			...
			Если КонтролироватьОтрицательныеОстатки 
				...
				ТекстСообщения = НСтр("ru='По организации %Организация% на %Месяц% обнаружен отрицательный остаток к оформлению списания %Количество% %ЕдиницаИзмерения% товара %Товар% в месте хранения %МестоХранения%';uk='За організаці ї%Организация% на%Месяц% виявлений від''ємний залишок до оформлення списання %Количество% %ЕдиницаИзмерения% товару %Товар% в місці зберігання %МестоХранения%'");
...
|;uk='Під час запису %Регистратор%  в регістри ТовариОрганізацій і РезервиТоваровОрганізацій
|виявлено зайвий резерв за вимірами: %Организация% - %АналитикаНоменклатуры% - %ВидЗапасов% - %НомерГТД%, зайвий резерв %Дельта%.'");
...
|;uk='Номенклатура %1 
|Перевищено залишок товару до оформлення на складі %2 на %3 %4'");
		ШаблонСообщенияСПоступлением = ШаблонСообщенияБезПоступления + " " + НСтр("ru='по документу %5';uk='за документом %5'");
		
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиТоварыКОформлениюТаможенныхДеклараций Цикл
...
Функция ПроверитьИзменениеТоваровПоКоличеству(МенеджерВременныхТаблиц) Экспорт
	...
	ДобавитьНалоговоеНазначениеВТекстЗапроса(Запрос);
...
Процедура ДобавитьНалоговоеНазначениеВТекстЗапроса(Запрос)
	
	НаличиеНалоговогоНазначения = Новый Структура("ТаблицаТоваров, ТаблицаВидыЗапасов", Ложь, Ложь);
	ВременныеТаблицы = Запрос.МенеджерВременныхТаблиц.Таблицы;
	Для каждого ТребуемаяТаблица Из НаличиеНалоговогоНазначения Цикл
		Для каждого ВременнаяТаблица Из ВременныеТаблицы Цикл
			Если ТребуемаяТаблица.Ключ = ВременнаяТаблица.ПолноеИмя Тогда
				Если ВременнаяТаблица.Колонки.Найти("ЦелевоеНалоговоеНазначение") <> Неопределено Тогда
					НаличиеНалоговогоНазначения.Вставить(ТребуемаяТаблица.Ключ, Истина);
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	ДобавлятьНалоговоеНазначение = Истина;
	Для каждого ТребуемаяТаблица Из НаличиеНалоговогоНазначения Цикл
		ДобавлятьНалоговоеНазначение = ДобавлятьНалоговоеНазначение И ТребуемаяТаблица.Значение;
		Если Не ДобавлятьНалоговоеНазначение Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;	
	...
	Для каждого Пакет Из СхемаЗапроса.ПакетЗапросов Цикл
		Для каждого Оператор Из Пакет.Операторы Цикл
			Для каждого Источник Из Оператор.Источники Цикл
				Для каждого ОператорВложенногоЗапроса ИЗ Источник.Источник.Запрос.Операторы Цикл
					ОператорВложенногоЗапроса.Источники[0].Источник.ДоступныеПоля.Добавить("ЦелевоеНалоговоеНазначение");
					ОператорВложенногоЗапроса.ВыбираемыеПоля.Добавить("ЦелевоеНалоговоеНазначение");
				КонецЦикла;
			КонецЦикла;
			ПолеНалоговогоНазначения = Оператор.ВыбираемыеПоля.Добавить("ЦелевоеНалоговоеНазначение");
			Оператор.Группировка.Добавить(ПолеНалоговогоНазначения);
		КонецЦикла;
	КонецЦикла;
...
// Функция проверяет необходимость принудительного перезаполнения видов запасов и устанавливает соответствующий параметр
// запроса.
//
...
Функция ПараметрыЗаполненияВидовЗапасов() Экспорт
    ...
    Результат.Вставить("НалоговоеНазначение", Неопределено);
...
// 			(НомерСтроки, Номенклатура, Организация, ХозяйственнаяОперация, ТипЗапасов, Поставщик, Соглашение, Валюта, НалоговоеНазначение)
...
Процедура ЗаполнитьВидыЗапасовПоУмолчанию(
    ...
    ПоляКлюча = "ГруппаФинансовогоУчета, ГруппаПродукции, ТипЗапасов, НалоговоеНазначение, ЭтоВозвратнаяТара";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ГруппаФинансовогоУчета <> КлючСтроки.ГруппаФинансовогоУчета
			...
			Или Выборка.НалоговоеНазначение <> КлючСтроки.НалоговоеНазначение 
...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект,
	...
	Если Не ПараметрыЗаполненияВидовЗапасов.ТаблицаОстатковУжеСформирована
		...
		ТекстИсключения = НСтр("ru='Процедуру ЗаполнитьВидыЗапасовПоТоварамОрганизаций можно вызывать только с таблицей остатков ТоварыОрганизаций или ПереданныеМеждуОрганизациямиТовары';uk='Процедуру ЗаполнитьВидыЗапасовПоТоварамОрганизаций можна викликати тільки з таблицею залишків ТоварыОрганизаций або ПереданныеМеждуОрганизациямиТовары'");
	...
	СформироватьВтОтборыОстатковПоТаблицеТоваров(МенеджерВременныхТаблиц, Истина, ПараметрыЗаполненияВидовЗапасов.ПодбиратьЗапасыРазныхПериодов);
...
Процедура ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если ПараметрыЗаполненияВидовЗапасов.ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию = Неопределено Тогда
		ТекстИсключения = НСтр("ru='В вызывающем коде не задано значение параметра ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию.';uk='У коді, що викликає, не задано значення параметра ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков = "ТаблицаВидыЗапасов"
		...
		ТекстИсключения = НСтр("ru='ТаблицаВидыЗапасов нужна для заполнения видов запасов по умолчанию, поэтому ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию должен быть выстален в ИСТИНА.';uk='ТаблицаВидыЗапасов потрібна для заповнення видів запасів по умовчанню, тому ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию повинен бути виставлений в ІСТИНА.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
...
//	ПараметрыЗаполненияВидовЗапасов - Неопределено, Структура - См. ЗапасыСервер.ПараметрыЗаполненияВидовЗапасов().
//
Процедура СформироватьРезервыПоТоварамОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если Не ПараметрыЗаполненияВидовЗапасов.ДокументДелаетИПриходИРасход
		...
		ПроведениеДокументов.УстановитьДопСвойстваРегистра(НаборРезервыТоваровОрганизаций, ДокументОбъект,
...
//	ПараметрыЗаполненияВидовЗапасов - Неопределено, Структура - См. ЗапасыСервер.ПараметрыЗаполненияВидовЗапасов().
//
Процедура ПодготовитьЗаписьТоваровОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, РежимЗаписи, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Запрос.УстановитьПараметр("ТоварыОрганизаций", ДокументОбъект.Движения.ТоварыОрганизаций);
	Иначе
	...
	УстановитьДопСвойстваКонтролироватьОстаткиИФормироватьСторно(ДокументОбъект, ПараметрыЗаполнения, МенеджерВременныхТаблиц);
...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ИмяВТИсточникаТекущихВидовЗапасов) Экспорт
    ...
    |	ВЫБОР
    |		КОГДА Настройка.СистемаНалогообложения В (ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалог), ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрибыль), ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Неплательщик))
	...
	|ИЗ
    |	РегистрСведений.НастройкиСистемыНалогообложения.СрезПоследних(&Дата, НЕ &ПоВсемНалоговымНазначениям) КАК Настройка
    ...
    Если КонтролироватьНалоговоеНазначение(ПараметрыЗаполненияВидовЗапасов) Тогда
    	Запрос.УстановитьПараметр("НалоговоеНазначение", ОтборыВидовЗапасов.НалоговоеНазначение);
    	Запрос.УстановитьПараметр("ПоВсемНалоговымНазначениям", Ложь);
    Иначе
    	Запрос.УстановитьПараметр("НалоговоеНазначение", Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка());
    	Запрос.УстановитьПараметр("ПоВсемНалоговымНазначениям", Истина);
    КонецЕсли;
...
// * ДокументМодифицирован - Булево
// * ЦенаВключаетНДС - Булево
// * Организация - СправочникСсылка.Организации
// * Склад - СправочникСсылка.Склады
// * ОтображатьДокументыРеализации - Булево
// * РедактироватьВидыЗапасов - Булево
// * ИмяКолонкиХарактеристика - Строка
// * ИмяКолонкиНоменклатура - Строка
// * ТоварВШапке - Булево
// * АдресВидовЗапасовВХранилище - Строка
// * АдресТоваровВХранилище - Строка
// * ИмяТЧВидыЗапасов - Строка
// * ИмяТЧТовары - Строка
// 
...
Процедура ПроверитьЗаполнениеПодразделенияВТабличнойЧасти(ДокументОбъект, Товары, ТипыНоменклатуры, ТекстОшибки, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаТовары",     Товары.Выгрузить(, "Номенклатура, Подразделение, НомерСтроки, Назначение"));
...
Процедура ПроверитьЗаполнениеНомеровГТД(ДокументОбъект, Отказ, ИмяТЧ = "Товары") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            НСтр("ru='Не заполнена колонка ""Код для НН и номер ГТД"" в строке %1 списка ""%2""';uk='Не заповнена колонка ""Код для ПН і номер ВМД"" в рядку %1 списку ""%2""'"),
			Выборка.НомерСтроки,
			ПредставлениеТЧ);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
...
Процедура ПроверитьНаличиеИзделийВТЧСУчетомПоГТД(ДокументОбъект, Отказ, ИмяТЧ = "ВыходныеИзделия") Экспорт
	...
	ШаблонСообщения = НСтр("ru='В строке %1 списка ""%2"" выбрана номенклатура, для которой ведется учет по кодам для НН и номерам ГТД.
|При включенной опции ""Запретить оформление товародвижения без кодов для НН"" выбор такой номенклатуры запрещен.'
|;uk='У рядку %1 списку ""%2"" обрана номенклатура, для якої ведеться облік за кодами для ПН і номерами ВМД. 
|При включеній опції ""Заборонити оформлення товару без кодів для ПН"" вибір такої номенклатури заборонений.'");
...
Процедура ПроверитьНаличиеИзделийВШапкеСУчетомПоГТД(ДокументОбъект, Отказ) Экспорт
	...
	Если РеквизитыИзделия.ВестиУчетПоГТД = Истина
		...
		ШаблонСообщения = НСтр("ru='В документе выбрана номенклатура, для которой ведется учет по кодам для НН и номерам ГТД.
|При включенной опции ""Запретить оформление товародвижения без кодов для НН"" выбор такой номенклатуры запрещен.'
|;uk='У документі обрана номенклатура, для якої ведеться облік за кодами для ПН і номерами ВМД.
|При включеній опції ""Заборонити оформлення товару без кодів для ПН"" вибір такої номенклатури заборонений.'");
...
Функция ИсправитьОстаткиПоТоварамКПередачеВФоновомЗадании(ПараметрыЗадания) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Исправление отрицательных остатков по товарам к передаче';uk='Виправлення від''ємних залишків по товарах до передачі'");
...
|;uk='Помилка в алгоритмі виправлення залишків по товарах до передачі. 
|Зверніться до розробників програми.'");
...
|;uk='Помилка виправлення залишків по товарах до передачі: 
|є надлишкові витратні рухи в коригування регістрів за вимірюваннями 
|%Организация% - %АналитикаУчетаНоменклатуры% - %ВидЗапасов% - %НомерГТД%,
|зайва витрата %КоличествоОтрицательное%. Необхідно скоригувати рухи документів і провести вручну.'");
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='ИсправлениеОтрицательныхОстатков.Ошибка исправления остатков';uk='Виправлення від''ємних залишків. Помилка виправлення залишків'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Для i = 0 По МаксимальныйИндекс Цикл
		...
		Попытка
			...
			РезультатПроведения = НСтр("ru='перепроведен успешно.';uk='перепроведено успішно.'");
		Исключение
			ИнформацияОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			РезультатПроведения = НСтр("ru='Ошибка при проведении. Необходимо отменить проведение и провести вручную.';uk='Помилка при проведенні. Необхідно скасувати проведення і провести вручну.'");
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru='ИсправлениеОтрицательныхОстатков.Ошибка перепроведения документа';uk='Виправлення від''ємних залишків. Помилка перепроведення документа'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
...
Функция ИмяСобытияВыключенКонтрольОстатков() Экспорт
	...
	Возврат НСтр("ru='Контроль остатков товаров организаций';uk='Контроль залишків товарів організацій'",КодОсновногоЯзыка);
КонецФункции
...
Процедура ЗаполнитьОбщиеПараметрыТекстеСообщенияОбОшибкахПроведенияПоТоварамОрганизацийИРезервам(ТекстСообщения, Регистратор, Выборка) Экспорт
	...
	Иначе
        ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерГТД%", НСтр("ru='<без кода для НН и номера ГТД>';uk='<без коду для ПН і номера ВМД>'"));
	КонецЕсли;
...
Процедура ЗаполнитьНоменклатуруГТДОприходованиеВидовЗапасов(Ссылка, ВидыЗапасов, Отказ, ИгнорироватьПризнакВестиУчетПоГТД = Истина, ИсточникНовойНоменклатуры = "НоменклатураОприходование", ПутьКНовойНоменклатуре = "НоменклатураОприходование") Экспорт	
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(НомерГТД КАК Справочник.НоменклатураГТД) КАК ИсходнаяНоменклатураГТД,
	|	ИмяИсточникаНовойНоменклатуры КАК ИмяИсточникаНовойНоменклатуры
	|ПОМЕСТИТЬ втВидыЗапасов
	|ИЗ
	|	&ВидыЗапасов КАК ВидыЗапасов
	|ГДЕ
	|	ВЫРАЗИТЬ(НомерГТД КАК Справочник.НоменклатураГТД) <> ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)
	//|	И НЕ ПутьКНовойНоменклатуре IS NULL И ПутьКНовойНоменклатуре <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|;
	|
	|ВЫБРАТЬ
	|	ИсходнаяНоменклатураГТД КАК ИсходнаяНоменклатураГТД,
	|	ИмяИсточникаНовойНоменклатуры КАК ИсточникНовойНоменклатуры,
	|	ПутьКНовойНоменклатуре КАК Номенклатура
	|ПОМЕСТИТЬ ВидыЗапасов
	|ИЗ
	|	втВидыЗапасов КАК ВидыЗапасов
	|ГДЕ
	|	НЕ ПутьКНовойНоменклатуре IS NULL И ПутьКНовойНоменклатуре <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|;
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВидыЗапасов.ИсточникНовойНоменклатуры КАК ИсточникНовойНоменклатуры,
	...
	|		ПО ВидыЗапасов.Номенклатура = СтараяНоменклатураГТД.Номенклатура
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяИсточникаНовойНоменклатуры", ИсточникНовойНоменклатуры);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПутьКНовойНоменклатуре", ПутьКНовойНоменклатуре);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ВидыЗапасов", ВидыЗапасов.Выгрузить(,"НомерГТД, " + ИсточникНовойНоменклатуры));	
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		?(Ссылка.Пустая(), 
            МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(Ссылка, КодЯзыка), 
            МультиязычностьУкр.ПолучитьЛокализованноеПредставление(Ссылка, КодЯзыка)
        )
    );
	...
	Отбор = Новый Структура("ИсходнаяНоменклатураГТД, ИсточникНовойНоменклатуры");
	Для каждого СтрокаВидыЗапасов Из ВидыЗапасов Цикл		
		...
		Отбор.ИсточникНовойНоменклатуры   = СтрокаВидыЗапасов[ИсточникНовойНоменклатуры];
...
// Устанавливает использование регламентного задания свертки резервов товаров организаций.
//
// Параметры:
//  Параметры	 - Структура - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке.
//
Процедура УстановитьИспользованиеРегламентногоЗаданияСверткиРезервов(Параметры) Экспорт
	
	НачатьТранзакцию();
	Попытка
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Константа.ИспользоватьОтветственноеХранениеВПроцессеЗакупки");
		ЭлементБлокировки = Блокировка.Добавить("Константа.ИспользоватьПередачиТоваровМеждуОрганизациями");
		ЭлементБлокировки = Блокировка.Добавить("Константа.ИспользоватьУправлениеПроизводством2_2");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		
		Задания = РегламентныеЗаданияСервер.НайтиЗадания(
			Новый Структура("Метаданные", Метаданные.РегламентныеЗадания.СверткаРезервовТоваровОрганизаций));
		// Гарантированно есть элемент, потому что регламентное задание предопределенное.
		Задание = Задания[0];
		
		Использование = Константы.ИспользоватьОтветственноеХранениеВПроцессеЗакупки.Получить()
			Или Константы.ИспользоватьПередачиТоваровМеждуОрганизациями.Получить()
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru='Не удалось установить использование регламентного задания свертки резервов товаров организаций по причине: %Причина%';uk='Не вдалося встановити використання регламентного завдання згортки резервів товарів організацій по причині: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.РегламентныеЗадания.СверткаРезервовТоваровОрганизаций,
			,
			ТекстСообщения);
	КонецПопытки;
...
Процедура ДобавитьЭтап_ИсправлениеДублейВидовЗапасов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ТекстВыполнить     = НСтр("ru='Исправить';uk='Виправити'");
...
Процедура Использование_ИсправлениеДублейВидовЗапасов(ПараметрыОбработчика) Экспорт
	...
	Если Результат.Пустой() Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru='Дублей видов запасов не найдено.';uk='Дублів видів запасів не знайдено.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		
	Иначе
		
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
			НСтр("ru='Обнаружены дубли видов запасов. Требуется выполнить замену дублей и удалить помеченные виды запасов.';uk='Виявлені дублі видів запасів. Потрібно виконати заміну дублів і вилучити помічені види запасів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			,
			,
...
Процедура ПроверкаКорректностьРеализацииУказаннойВВозврате(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Возврат",						НСтр("ru='Документ возврата';uk='Документ повернення'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Реализация",					НСтр("ru='Документ реализации';uk='Документ реалізації'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены возвращенные от клиента товары, отсутствующие в документе реализации организации ""%1""';uk='Виявлено повернуті від клієнта товари, відсутні в документі реалізації організації ""%1""'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Возврат");
...
Процедура ПроверкаКорректностиЗаполненияСуммВОтчетахПоКомиссииМеждуОрганизациями(ПараметрыПроверки) Экспорт
	...
	ШаблонТекстаОшибки  = НСтр("ru='Обнаружены отчеты по комиссии между организациями с некорректными суммовыми показателями по организации ""%1"" за период %2';uk='Виявлено звіти по комісії між організаціями з некоректними підсумковими показниками по організації ""%1"" за період %2'",ОбщегоНазначения.КодОсновногоЯзыка());
	
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация",  НСтр("ru='Организация';uk='Організація'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить(ИмяПоляОбъекта, НСтр("ru='Отчет по комиссии между организациями';uk='Звіт з комісії між організаціями'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура Оформление_ИсправлениеОтрицательныхОстатковТоваровОрганизаций(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru='Исправление отрицательных остатков товаров организации';uk='Виправлення від''ємних залишків товарів організації'");
	КонецЕсли;
	
	ПараметрыОбработчика.ДанныеЭтапа.ТекстВыполнить = НСтр("ru='Исправить';uk='Виправити'");
...
Процедура ОписаниеПроверок_ИсправлениеОтрицательныхОстатковТоваровОрганизаций(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Отрицательные остатки в регистре ""Товары организаций""';uk='Від''ємні залишки в регістрі ""Товари організацій""'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Отрицательных остатков быть не должно.';uk='Від''ємних залишків бути не повинно.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ЗапросОтрицательныеОстаткиТоваровОрганизаций()
	...
	|	И СУММА(ОстаткиТекущийПериод.КоличествоОстаток - ЕСТЬNULL(ОстаткиПрошлыйПериод.КоличествоОстаток, 0)) < 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 10
	...
	|	И СУММА(ОстаткиТекущийПериод.КОформлениюСписанияОстаток - ЕСТЬNULL(ОстаткиПрошлыйПериод.КОформлениюСписанияОстаток, 0)) < 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПроверкаОтрицательныхОстатковТоваровОрганизаций(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru='Аналитика номенклатуры';uk='Аналітика номенклатури'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru='Вид запасов';uk='Вид запасів'",ОбщегоНазначения.КодОсновногоЯзыка()));
    СписокПолей.Добавить("НомерГТД", 				   НСтр("ru='Код для НН и номер ГТД';uk='Код для ПН та номер ВМД'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Количество",		 		   НСтр("ru='Количество';uk='Кількість'",ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ОтрицательныеОстатки",
		НСтр("ru='Обнаружены отрицательные остатки товаров организации ""%1"" на конец периода %2';uk='Виявлено від''ємні залишки товарів організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеПередачТоваров(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст = ТекстЗапросаОформленияПоРезервамТоваровОрганизаций(Запрос, "ВТПоНоменклатуреВТКОформлениюПередач");
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлениюПередач", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет остатков в регистре ""Резервы товаров организаций""';uk='Немає залишків в регістрі ""Резерви товарів організацій""'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеПередачТоваров(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные передачи товаров между организациями';uk='Не оформлені передачі товарів між організаціями'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должны быть оформлены документы ""Передача товаров между организациями"".';uk='Повинні бути оформлені документи ""Передача товарів між організаціями"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиОформленияПередачТоваров(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Требуется оформить передачу товаров в организацию ""%1"" на конец периода %2';uk='Потрібно оформити передачу товарів в організацію ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеОстатковТоваровКПередаче(ПараметрыОбработчика) Экспорт
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ОстаткиКПередаче", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет остатков в регистре ""Товары организаций к передаче""';uk='Немає залишків в регістрі ""Товари організацій до передачі""'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеОстатковТоваровКПередаче(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные остатки передач товаров между организациями';uk='Не оформлені залишки передач товарів між організаціями'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должны быть оформлены документы ""Передача товаров между организациями"".';uk='Повинні бути оформлені документи ""Передача товарів між організаціями"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковПередачТоваров(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ОстаткиКПередаче",
		НСтр("ru='Обнаружены товары, требующие оформления передачи/возврата от организации ""%1"" на конец периода %2';uk='Виявлено товари, що вимагають оформлення передачі / повернення від організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеВыкупаТоваров(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст = ТекстЗапросаОформленияПоРезервамТоваровОрганизаций(Запрос, "ВТПоНоменклатуреВТКОформлениюВыкупов");
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлениюВыкупов", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет остатков товаров к выкупу в регистре ""Резервы товаров организаций""';uk='Немає залишків товарів до викупу в регістрі ""Резерви товарів організацій""'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеВыкупаТоваров(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные выкупы товаров, принятых на ответственное хранение';uk='Не оформлені викупи товарів, прийнятих на відповідальне зберігання'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должны быть оформлены документы ""Выкуп товаров с хранения"".';uk='Повинні бути оформлені документи ""Викуп товарів зі зберігання"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковТоваровКВыкупу(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены товары, требующие оформления выкупа по организации ""%1"" на конец периода %2';uk='Виявлено товари, що вимагають оформлення викупу по організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеОтчетовПоКомиссииМеждуОрганизациями(ПараметрыОбработчика) Экспорт
	...
	Если РазмерыВременныхТаблиц.ВТОстаткиКомитента = 0 И РазмерыВременныхТаблиц.ВТОстаткиКомиссионера = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет остатков в регистре ""Товары к оформлению отчетов комитенту"" по схеме Интеркампани';uk='Немає залишків в регістрі ""Товари до оформлення звітів комітенту"" за схемою Інтеркампані'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеОтчетовПоКомиссииМеждуОрганизациями(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные отчеты по комиссии между организациями';uk='Не оформлені звіти по комісії між організаціями'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должны быть оформлены документы ""Отчет по комиссии между организациями"" и ""Отчет по комиссии между организациями о списании"".';uk='Повинні бути оформлені документи ""Звіт з комісії між організаціями"" і ""Звіт з комісії між організаціями про списання"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуМеждуОрганизациями(ПараметрыПроверки) Экспорт
	...
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация", 			   НСтр("ru='Организация';uk='Організація'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Комиссионер", 			   НСтр("ru='Организация-комиссионер';uk='Організація-комісіонер'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru='Аналитика номенклатуры';uk='Аналітика номенклатури'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru='Вид запасов';uk='Вид запасів'",ОбщегоНазначения.КодОсновногоЯзыка()));
    СписокПолей.Добавить("НомерГТД", 				   НСтр("ru='Код для НН и номер ГТД';uk='Код для ПН та номер ВМД'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Количество",		 		   НСтр("ru='Количество';uk='Кількість'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТОстаткиКомитента",
		НСтр("ru='Обнаружены товары, требующие оформления отчетов комитенту ""%1"" на конец периода %2';uk='Виявлено товари, що вимагають оформлення звітів комітенту ""%1"" на кінець періоду %2'"),
		СписокПолей);
		
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация", 			   НСтр("ru='Организация';uk='Організація'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Комитент", 				   НСтр("ru='Организация-комитент';uk='Організація-комітент'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru='Аналитика номенклатуры';uk='Аналітика номенклатури'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru='Вид запасов';uk='Вид запасів'",ОбщегоНазначения.КодОсновногоЯзыка()));
    СписокПолей.Добавить("НомерГТД", 				   НСтр("ru='Код для НН и номер ГТД';uk='Код для ПН та номер ВМД'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Количество",		 		   НСтр("ru='Количество';uk='Кількість'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru='Обнаружены товары, требующие оформления отчетов комитенту от организации ""%1"" на конец периода %2';uk='Виявлено товари, що вимагають оформлення звітів комітенту від організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеОтчетовКомитентамОСписании(ПараметрыОбработчика) Экспорт
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлению", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет остатков в регистре ""Товары к оформлению отчетов комитенту"" по сторонним комитентам';uk='Немає залишків в регістрі ""Товари до оформлення звітів комітенту"" по сторонніх комітентах'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеОтчетовКомитентамОСписании(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные отчеты комитентам о списании';uk='Не оформлені звіти комітентам про списання'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должны быть оформлены документы ""Отчет комитенту о списании"".';uk='Повинні бути оформлені документи ""Звіт комітенту про списання"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуОСписании(ПараметрыПроверки) Экспорт
	...
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация", 			   НСтр("ru='Организация';uk='Організація'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Комитент", 				   НСтр("ru='Комитент';uk='Комітент'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru='Аналитика номенклатуры';uk='Аналітика номенклатури'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru='Вид запасов';uk='Вид запасів'",ОбщегоНазначения.КодОсновногоЯзыка()));
    СписокПолей.Добавить("НомерГТД", 				   НСтр("ru='Код для НН и номер ГТД';uk='Код для ПН та номер ВМД'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Количество",		 		   НСтр("ru='Количество';uk='Кількість'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТКОформлению",
		НСтр("ru='Обнаружены товары, требующие оформления отчетов комитенту о списании от организации ""%1"" на конец периода %2';uk='Виявлено товари, що вимагають оформлення звітів комітенту про списання від організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Функция ПустаяТаблицаРеквизитовВидовЗапасовПоУмолчанию()
	...
	Результат.Колонки.Добавить("НомерГТД",
                        Новый ОписаниеТипов("СправочникСсылка.НоменклатураГТД"));
...
Функция ОрганизацияДокумента(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов)
	...
	Иначе
		ТекстИсключения = НСтр("ru='Ошибка в программе: невозможно определить организацию документа.';uk='Помилка в програмі: неможливо визначити організацію документа.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура СформироватьВТТоварыОрганизацийИзменения(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	ГДЕ
	|		ВЫРАЗИТЬ(ТоварыОрганизаций.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Номенклатура.ТипНоменклатуры В (
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), 
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		)
	|	
...
Функция ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение(ПараметрыЗаполненияВидовЗапасов, ВыборкаТоваров)
	...
	ИначеЕсли ВРег(ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоХозяйственнойОперации") Тогда
		...
		Если ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) <> ВРег("Всегда")
			...
			ТекстИсключения = НСтр("ru='В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение для хозяйственной операции %ХозяйственнаяОперация%.';uk='В параметрах заповнення видів запасів не визначено підставляти чи ні в табличну частину товари, прийняті на відповідальне зберігання для господарської операції %ХозяйственнаяОперация%'");
	...
	Иначе
		ТекстИсключения = НСтр("ru='В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение.';uk='В параметрах заповнення видів запасів не визначено підставляти чи ні в табличну частину товари, прийняті на відповідальне зберігання.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента
		...
		ТекстИсключения = НСтр("ru='Функция ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы не предназначена для вызова с хозяйственной операцией %ХозяйственнаяОперация%';uk='Функція ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы не призначена для виклику з господарською операцією %ХозяйственнаяОперация%'");	
	...
	ВидыЗапасовПоУмолчанию.Колонки.Добавить("НалоговоеНазначение",
											Новый ОписаниеТипов("СправочникСсылка.НалоговыеНазначенияАктивовИЗатрат"));
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ВидЗапасов) Тогда
                ...
                //		ПараметрыУчетаПоОрганизации = УчетНДСУП.ПараметрыУчетаПоОрганизации(Выборка.НовыйВидЗапасовВладелецТовара, ДокументОбъект.Дата);
			...
			ИначеЕсли (КоличествоТовара > 0 И КоличествоОстаток > 0)
			 Или (КоличествоТовара < 0 И КоличествоОстаток < 0) Тогда
				...
				Иначе
					...
					ТекстИсключения = НСтр("ru='Ненулевое количество при пустом виде запасов.';uk='Ненульова кількість при порожньому виді запасів.'");
					ВызватьИсключение ТекстИсключения;
...
|;uk='Встановлено прапор отримання власника товарів з таблиці 
|реквізитів видів запасів по умовчанню, але таблиці не передана.'");
					ВызватьИсключение ТекстИсключения;
				КонецЕсли;
	...
	Если ТаблицаОшибок.Количество() > 0 И ПроводитьБезКонтроляОстатковТоваровОрганизаций Тогда
		ТекстСообщения = НСтр("ru='Документ %Ссылка% проведен с выключенным контролем остатков товаров организаций.';uk='Документ %Ссылка% проведено з вимкненим контролем залишків товарів організацій.'",Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура РаспределитьНовыеРезервыПоТаблицеОстатков(
	...
	Для Каждого СтрокаРаспределения Из ТаблицаРаспределения Цикл
		...
		Для Каждого СтрокаОстатка Из СтрокиОстатка Цикл
			...
			РегистрыНакопления.РезервыТоваровОрганизаций.ДобавитьВНаборРезерваКорЗапись(ТаблицаНовыеРезервы, РезервПродавца);
...
Функция КонтролироватьНалоговоеНазначение(ПараметрыЗаполненияВидовЗапасов)
	НалоговоеНазначение = ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.НалоговоеНазначение;
	
	Если Не ЗначениеЗаполнено(НалоговоеНазначение) Или Не ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС") Тогда
		КонтролироватьНалоговоеНазначение = Ложь;
	Иначе
		КонтролироватьНалоговоеНазначение = Истина;
	КонецЕсли;
	
	Возврат КонтролироватьНалоговоеНазначение;
КонецФункции
...
Процедура СформироватьВТСторноРезервов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
    ...
    |	И (&ПоВсемНалоговымНазначениям
    |			ИЛИ ВидыЗапасов.НалоговоеНазначение В (&НалоговоеНазначение))
	|	И (&ПоВсемВладельцамТоваров
	|			ИЛИ ВидыЗапасов.ВладелецТовара В (&ВладелецТовара))
	|	И (&ПоВсемСоглашениям
	|			ИЛИ ВидыЗапасов.Соглашение В (&Соглашение))
    ...
    Запрос.УстановитьПараметр("НалоговоеНазначение", ОтборыВидовЗапасов.НалоговоеНазначение);
    Запрос.УстановитьПараметр("ПоВсемНалоговымНазначениям", ОтборыВидовЗапасов.НалоговоеНазначение = Неопределено);
	...
	Результаты = Запрос.ВыполнитьПакет();
	РезервыДляСторно = Результаты[Результаты.ВГраница() - 1].Выгрузить(); // ТаблицаЗначений
	...
	Если ТаблицаСторноДвижений.Количество() > 0 Тогда
		ГлубинаРекурсии = 0;
		ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ТаблицаСторноДвижений, ТаблицаСторноДвижений, ГлубинаРекурсии);
	КонецЕсли;
...
Процедура ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ИсходнаяТаблицаСторноДвижений, РезультирующаяТаблицаСторноДвижений, ГлубинаРекурсии)
	...
	Если ГлубинаРекурсии > 100 Тогда
		...
		ТекстИсключения = НСтр("ru='Превышена максимальная глубина рекурсии при формировании сторно резервов по расходу. Сохраните текущее состояние информационной базы и передайте ее разработчикам программы.';uk='Перевищена максимальна глибина рекурсії при формуванні сторно резервів за витратою. Збережіть поточний стан інформаційної бази і передайте її розробникам програми.'");
		ВызватьИсключение ТекстИсключения;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если РаспределенноеВПериодеСторно = Неопределено Тогда
			РаспределенноеСторно.Вставить(Выборка.ПериодСторноРезерва, 0);
			РаспределенноеВПериодеСторно = 0;
		КонецЕсли;
		
		КлючПоискаЗачтенногоРезерва = КлючПоискаЗачтенногоРезерва(Выборка.Период, Выборка.КорОрганизация, Выборка.КорВидЗапасов);
		...
		Если ЗачтенныйВПериодеРезерв = Неопределено Тогда
			ЗачтенныеРезервы.Вставить(КлючПоискаЗачтенногоРезерва, 0);
			ЗачтенныйВПериодеРезерв = 0;
		КонецЕсли;
		
		КоличествоКСторнированию = Мин(СторноРасходаРезерваДляРаспределения - РаспределенноеВПериодеСторно, Выборка.Количество - ЗачтенныйВПериодеРезерв);
		...
		РаспределенноеСторно[Выборка.ПериодСторноРезерва] = РаспределенноеСторно[Выборка.ПериодСторноРезерва] + КоличествоКСторнированию;
		ЗачтенныеРезервы[КлючПоискаЗачтенногоРезерва]     = ЗачтенныеРезервы[КлючПоискаЗачтенногоРезерва] + КоличествоКСторнированию;
	...
	Если ТаблицаСторноДвижений.Количество() > 0 Тогда
		...
		ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ТаблицаСторноДвижений, РезультирующаяТаблицаСторноДвижений, ГлубинаРекурсии);
...
Процедура СформироватьВтОтборыОстатковПоТаблицеТоваров(МенеджерВременныхТаблиц, ДобавлятьАналитикиИзДвиженийПередЗаписью, ПодбиратьЗапасыРазныхПериодов = Ложь)
	...
	|			ПО ИСТИНА
	|	ГДЕ
	|		Товары.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры В (
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), 
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		)
	|
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения 
		...
		Для Каждого СтрокаТаблицы Из СтрокиСПустымВидомЗапасовКомитента Цикл
			...
			Если СтрокаТаблицы.Количество <> 0 Тогда
				ТекстСообщения = НСтр("ru='По комитенту %Комитент% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, реализованного комиссионером %Комиссионер%';uk='По комітенту %Комитент% не вистачає %Количество% %ЕдиницаИзмерения% товару %ПредставлениеНоменклатуры%, реалізованого комісіонером %Комиссионер%'"); 
			...
			Если ЭтоОтчетОПродажах И СтрокаТаблицы.СуммаСНДС <> 0 Тогда
				ТекстСообщения = НСтр("ru='Товар: %ПредставлениеНоменклатуры% Сумма продажи превышает сумму реализованного товара комиссионером на %Сумма% %Валюта%';uk='Товар: %ПредставлениеНоменклатуры% Сума продажу перевищує суму реалізованого товару комісіонером на %Сумма% %Валюта%'"); 
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеНоменклатуры%",
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(СтрокаТаблицы.АналитикаУчетаНоменклатуры.Номенклатура, СтрокаТаблицы.АналитикаУчетаНоменклатуры.Характеристика));
...
Процедура УстановитьБлокировкуРегистров(ДокументОбъект, МенеджерВременныхТаблиц)
	...
	Если ЕстьДвижениеВКоллекции(ДокументОбъект,"ТоварыОрганизаций") Тогда
		...
		Если ЕстьДвижениеВКоллекции(ДокументОбъект,"РезервыТоваровОрганизаций") = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Ошибка в описании движений документа: документ является регистратором ТоварыОрганизаций и не является регистратором РезервыТоваровОрганизаций.';uk='Помилка в описі рухів документа: документ є реєстратором ТоварыОрганизаций і не є реєстратором РезервыТоваровОрганизаций.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	...
	Если ЕстьДвижениеВКоллекции(ДокументОбъект,"ТоварыКОформлениюОтчетовКомитенту") Тогда
	...
	Если ЕстьДвижениеВКоллекции(ДокументОбъект,"ТоварыПереданныеНаКомиссию") Тогда
...
Процедура ЗаполнитьВидыЗапасовРазныхПериодов(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	Если Не ПараметрыЗаполненияВидовЗапасов.ТаблицаОстатковУжеСформирована Тогда
		...
		ТаблицаОстатковТоваровОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, Истина, МинимальнаяДатаОстатков, ДатыОстатков);
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатков(ДокументОбъект,
	...
	РезультатЗапроса = МассивРезультатов[МассивРезультатов.ВГраница()]; // РезультатЗапроса
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		СуммаВознагражденияВсего	= ВыборкаОстаткиПоСтрокам.СуммаВознаграждения;
		СуммаНДСВознагражденияВсего	= ВыборкаОстаткиПоСтрокам.СуммаНДСВознаграждения;
        СуммаНДСПропорциональноВсего= ВыборкаОстаткиПоСтрокам.СуммаНДСПропорционально;
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецЕсли;
 		   СтруктураВидаЗапасовПоУмолчанию.НалоговоеНазначение = Выборка.ЦелевоеНалоговоеНазначение;
				...
				Иначе
					...
					Если ВидЗапасовОтгрузки = Неопределено Тогда
						ВидЗапасовОтгрузки = Справочники.ВидыЗапасов.ВидЗапасовДляПередачиМеждуОрганизациями(
							...
							Организация,
							ВыборкаДанныеДокумента.ЦелевоеНалоговоеНазначение);
			...
			ИначеЕсли (КоличествоТовара > 0 И КоличествоОстаток > 0)
			 ИЛИ (КоличествоТовара < 0 И КоличествоОстаток < 0) Тогда
				...
				СтруктураДанныхСтроки.СуммаВознаграждения		= ?(КоличествоТовара <> 0, Окр(Количество * СуммаВознагражденияВсего / КоличествоТовара, 2, 1), 0);
				СтруктураДанныхСтроки.СуммаНДСВознаграждения	= ?(КоличествоТовара <> 0, Окр(Количество * СуммаНДСВознагражденияВсего / КоличествоТовара, 2, 1), 0);
                СтруктураДанныхСтроки.СуммаНДСПропорционально	= ?(КоличествоТовара <> 0, Окр(Количество * СуммаНДСПропорциональноВсего / КоличествоТовара, 2, 1), 0);
				...
				СуммаВознагражденияВсего	= СуммаВознагражденияВсего		- СтруктураДанныхСтроки.СуммаВознаграждения;
				СуммаНДСВознагражденияВсего	= СуммаНДСВознагражденияВсего	- СтруктураДанныхСтроки.СуммаНДСВознаграждения;
                СуммаНДСПропорциональноВсего= СуммаНДСПропорциональноВсего	- СтруктураДанныхСтроки.СуммаНДСПропорционально;
				СуммаОстаток				= СуммаОстаток					- СтруктураДанныхСтроки.СуммаСНДС;
...
|реквізитів видів запасів по умовчанню, але таблиці не передана.'");
				...
				Если ВыборкаТаблицаТоваров.НайтиСледующий(Новый Структура("НомерСтроки", ВыборкаОстаткиПоСтрокам.НомерСтроки)) Тогда
					...
					СтруктураВидаЗапасовПоУмолчанию.НалоговоеНазначение = ВыборкаТаблицаТоваров.ЦелевоеНалоговоеНазначение;
					...
					Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков = "ТоварыОрганизацийКПередаче" Тогда
						НоваяСтрока.ВидЗапасовПолучателя = Справочники.ВидыЗапасов.ВидЗапасовДляПередачиМеждуОрганизациями(
							...
							СтруктураВидаЗапасовПоУмолчанию.ЦелевоеНалоговоеНазначение,
							ДокументОбъект.Дата);
					КонецЕсли;
					...
					СтруктураДанныхСтроки.СуммаВознаграждения		= 0;
					СтруктураДанныхСтроки.СуммаНДСВознаграждения	= 0;
                    СтруктураДанныхСтроки.СуммаНДСПропорционально	= 0;
	...
	СообщитьОбОшибкахЗаполненияВидовЗапасовПоТаблицеКОформлению(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов, ХозяйственнаяОперация);
...
Функция ЗапросРаспределенияБезУчетаПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства, ПараметрыЗаполнения)
    ...
    |	&ТекстПоляТаблицаТоваровСуммаНДСПропорционально_ КАК СуммаНДСПропорционально,
	...
	|		ВидыЗапасов.Знак * Остатки.КоличествоОстаток
	|			- ВидыЗапасов.Знак * ВидыЗапасов.Количество < 0
	|	КОНЕЦ) КАК ВтОстаткиБезГруппировки
	|СГРУППИРОВАТЬ ПО
	...
	|			ИЛИ ТаблицаДокумента.НомерГТД = ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)
	|			ИЛИ Остатки.НомерГТД = ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)
	|			ИЛИ &БезОтбораПоНомерамГТД)
	...
	|	МАКСИМУМ(СтавкаНДС),
	|	МАКСИМУМ(СуммаНДС),
    |	МАКСИМУМ(СуммаНДСПропорционально),
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства)
    ...
    |	ТаблицаТоваров.ЦелевоеНалоговоеНазначение КАК ЦелевоеНалоговоеНазначение,
    |	ТаблицаТоваров.ЦелевоеНалоговоеНазначение.ВидДеятельностиНДС КАК ЦелевойВидДеятельностиНДС,
    ...
    |	ТаблицаТоваров.ЦелевоеНалоговоеНазначение,
    |	ТаблицаТоваров.ЦелевоеНалоговоеНазначение.ВидДеятельностиНДС,
    ...
    |	ТаблицаДанныхДокумента.НалоговоеНазначениеОрганизации,
    ...
    |	ВидыЗапасов.НалоговоеНазначение КАК НалоговоеНазначениеОстатков,
    |	ВидыЗапасов.НалоговоеНазначение.ВидДеятельностиНДС КАК ВидДеятельностиНДСОстатков,
	...
	|	ВЫБОР 
	|		КОГДА Не ТаблицаДанныхДокумента.ИспользоватьРаздельныйУчетТоваровПоНалогообложениюНДС 
	|			ТОГДА 0
	|		КОГДА ТаблицаДокумента.ЦелевоеНалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка) 
	|             И ТаблицаДокумента.ЦелевоеНалоговоеНазначение = ЕСТЬNULL(Остатки.НалоговоеНазначениеОстатков, Неопределено)  
	|			ТОГДА 1                     
	|		КОГДА ТаблицаДокумента.ЦелевоеНалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка) 
	|             И ТаблицаДокумента.ЦелевойВидДеятельностиНДС = ЕСТЬNULL(Остатки.ВидДеятельностиНДСОстатков, Неопределено) 
	|			ТОГДА 2                     
	|		КОГДА ТаблицаДокумента.ЦелевоеНалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка) 
	|             И ЕСТЬNULL(Остатки.ВидДеятельностиНДСОстатков, Неопределено) = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
	|			ТОГДА 3                     
	|		КОГДА ТаблицаДокумента.ЦелевоеНалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка) 
	|               И ТаблицаДанныхДокумента.НалоговоеНазначениеОрганизации = ЕСТЬNULL(Остатки.НалоговоеНазначениеОстатков, Неопределено) 
	|			ТОГДА 4 
	|		ИНАЧЕ
	|			5
	...
	|
	|	ЕСТЬNULL(Остатки.ВидЗапасов, Неопределено) КАК ВидЗапасов,
    |	ЕСТЬNULL(Остатки.НалоговоеНазначениеОстатков, Неопределено) КАК НалоговоеНазначениеВидаЗапаса,
    ...
    |	ЕСТЬNULL(Остатки.НалоговоеНазначениеОстатков,
    |		Неопределено)                               КАК НовыйВидЗапасовНалоговоеНазначениеКомитента,
	...
	//
	//|		И НЕ (       
	//|			ТаблицаДокумента.ЦелевоеНалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)	
	//|         	И ТаблицаДокумента.ЦелевойВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая)
	//|			И Остатки.НалоговоеНазначениеОстатков = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность)
	//|         )
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступления
...
Процедура ЗаполнитьСтруктуруПоСтрокеТоваров(Структура, СтрокаТоваров, ДанныеШапки)
    ...
    КонецЕсли;
    Структура.НалоговоеНазначение = СтрокаТоваров.ЦелевоеНалоговоеНазначение;
	...
	Для Каждого Организация Из МассивОрганизаций Цикл
		...
		Если ТаблицаОшибок.Количество() <> 0 Тогда
			...
			Если ТипЗнч(ДокументОбъект) <> Тип("Структура") Тогда
				СообщитьОбОшибкахКонтроляТоваровОрганизаций(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов, Организация);
			КонецЕсли;
...
Функция РаспределитьРезервыТоваровОрганизацийПоПериодам(КоличествоПериодовПриФормированииРезервовОрганизаций, ТаблицаНовыеРезервыИсходная, МенеджерВременныхТаблиц) Экспорт
	...
	ОбъединенныйТекстЗапросаПоМесяцам = СтрСоединить(МассивЗапросовПоМесяцам, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ОбъединенныйТекстЗапросаПоМесяцам", "(" + ОбъединенныйТекстЗапросаПоМесяцам + ")");
	...
	Пока ВыборкаОстатков.Следующий() Цикл
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(СтруктураПоиска, ВыборкаОстатков) Тогда
			...
			Если НовыйРезервВсегоНеРаспределено <> 0 Тогда
				ТекстИсключения = НСтр("ru='Ошибка распределения резервов по месяцам';uk='Помилка розподілу резервів по місяцях'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
			
			НовыйРезервВсегоНеРаспределено = ВыборкаОстатков.НовыйРезервВсего;
			РезервыПоАналитике             = ОбщегоНазначенияУТ.НайтиСтрокиССохранениемПорядка(ТаблицаНовыеРезервы, СтруктураПоиска);
		...
		Пока Индекс >=0 Цикл
			...
			РезервВЭтотМесяцВЭтуЗапись = Мин(НераспределенныйРезерв.КоличествоНеРаспределенноеПоПериодам, РезервВЭтотМесяцВсегоНераспределено);				
			...
			Иначе
				НераспределенныйРезерв.КоличествоНеРаспределенноеПоПериодам = НераспределенныйРезерв.КоличествоНеРаспределенноеПоПериодам - РезервВЭтотМесяцВЭтуЗапись;
	...
	Если ЕстьИзмененияВТаблице Тогда
		...
		Для Индекс = 0 По КоличествоСтрок - 1 Цикл
			...
			РегистрыНакопления.РезервыТоваровОрганизаций.ДобавитьВНаборРезерваКорЗапись(ТаблицаНовыеРезервы, СтрокаПрихода);
		КонецЦикла;
...
Процедура СообщитьОбОшибкахКонтроляТоваровОрганизаций(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов, Организация = Неопределено)
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		ОрганизацияСообщенияОбОшибке = ?(ЗначениеЗаполнено(Организация), Организация, ОрганизацияДокумента(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов)); 
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
            ...
            ТекстСообщения = НСтр("ru='По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД% %НалоговоеНазначение%';uk='По організації %Организация% не вистачає %Количество% %ЕдиницаИзмерения% товару %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД% %НалоговоеНазначение%'");
			...
			Если СтрокаТаблицы.НеУказанНомерГТД Тогда
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КонтрольГТД%", НСтр("ru='с указанными кодами для НН и номерами ГТД';uk='із зазначеними кодами для ПН і номерами ВМД'"));
			Иначе
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КонтрольГТД%", "");
			КонецЕсли;
			
            Если КонтролироватьНалоговоеНазначение(ПараметрыЗаполненияВидовЗапасов) Тогда
            	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НалоговоеНазначение%", НСтр("ru='по налоговому назначению';uk='за податковим призначенням'") + " " + ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.НалоговоеНазначение);
            Иначе
            	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НалоговоеНазначение%", "");
            КонецЕсли;
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Склад)
				...
				ТекстСообщения = НСтр("ru='По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%';uk='По організації %Организация% не вистачає %ТипДвижения% %Количество% %ЕдиницаИзмерения% на суму %Сумма% %Валюта% товару: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%'");
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Склад) Тогда
				ТекстСообщения = НСтр("ru='По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% товара: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%';uk='По організації %Организация% не вистачає %ТипДвижения% %Количество% %ЕдиницаИзмерения% товару: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%'");
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Сумма)
				И ЗначениеЗаполнено(СтрокаТаблицы.Валюта) Тогда
				ТекстСообщения = НСтр("ru='По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры% %КонтрольГТД%';uk='По організації %Организация% не вистачає %ТипДвижения% %Количество% %ЕдиницаИзмерения% на суму %Сумма% %Валюта% товару: %ПредставлениеНоменклатуры% %КонтрольГТД%'");
			Иначе
				ТекстСообщения = НСтр("ru='По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% товара: %ПредставлениеНоменклатуры% %КонтрольГТД%';uk='По організаці ї%Организация% не вистачає %ТипДвижения% %Количество% %ЕдиницаИзмерения% товару: %ПредставлениеНоменклатуры% % КонтрольГТД%'");
			КонецЕсли;	
			...
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика Тогда
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТипДвижения%", НСтр("ru='к возврату';uk='до повернення'"));
			Иначе
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТипДвижения%", НСтр("ru='к оформлению';uk='до оформлення'"));
			КонецЕсли;
...
Процедура СообщитьОКонтролеНалогообложенияПриПодбореВидовЗапасов(Знач ДокументОбъект, Знач ПараметрыЗаполненияВидовЗапасов)
    ...
    //		ТекстСообщения = НСтр("ru='Обратите внимание, что тип налогообложения ""%НалогообложениеНДС%"" не поддерживается учетной политикой организации. В этом случае могут быть списаны только комиссионные товары.'");
...
Функция НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям(СтрокаТаблицы, ХозяйственнаяОперация = Неопределено)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика Тогда
		МестоХраненияНаименование = НСтр("ru='от переработчика';uk='від переробника'");
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Склад) = Тип("СправочникСсылка.Партнеры") Тогда
		МестоХраненияНаименование = НСтр("ru='у партнера';uk='у партнера'");
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Склад) = Тип("СправочникСсылка.Организации") Тогда
		МестоХраненияНаименование = НСтр("ru='у организации';uk='у організації'");
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Склад) = Тип("СправочникСсылка.СтруктураПредприятия") Тогда
		МестоХраненияНаименование = НСтр("ru='в подразделении';uk='в підрозділі'");
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Склад) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		МестоХраненияНаименование = НСтр("ru='по договору';uk='за договором'");
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Склад) = Тип("СправочникСсылка.Склады") Тогда
		МестоХраненияНаименование = НСтр("ru='на складе';uk='на складі'");
	Иначе
		ТекстИсключения = НСтр("ru='В процедуре НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям не учтен тип места хранения ""%1""';uk='У процедурі НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям не врахована тип місця зберігання ""%1""'");
...
Процедура СформироватьВТТаблицаОстатков(ИмяТаблицыОстатков,
	...
	Если ИмяТаблицыОстатков = "ТоварыОрганизаций" Тогда
		Если ВычитатьДвиженияПоТоварамОрганизаций = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Ошибка формирования таблицы остатков товаров организаций: не передан параметр ВычитатьДвиженияПоТоварамОрганизаций.';uk='Помилка формування таблиці залишків товарів організацій: не переданий параметр ВычитатьДвиженияПоТоварамОрганизаций.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	...
	ИначеЕсли ИмяТаблицыОстатков = "ПродукцияКОформлениюВыпуска" Тогда
		ТаблицаПродукцияКОформлениюВыпуска(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков);
	...
	Иначе
		ТекстИсключения = НСтр("ru='Неизвестное имя таблицы остатков.';uk='Невідоме ім''я таблиці залишків.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
КонецПроцедуры

Процедура ТаблицаОстатковТоваровОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ВычитатьДвиженияПоТоварамОрганизаций, МинимальнаяДатаОстатков, ДатыОстатков = Неопределено)
	...
	ВсегоПериодов = ДополнитьЗапросКонтролемОстатковНаКаждыйМесяц(Запрос, ТекстЗапроса, ПараметрыДополнения, МинимальнаяДатаОстатков, ДатыОстатков);
...
Процедура ТаблицаОстатковРеализованныхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	ТЧВидыЗапасов.АналитикаУчетаНоменклатурыОтгрузки,
	|	ТЧВидыЗапасов.ДокументРеализации,
	|	ТЧВидыЗапасов.ВидЗапасовОтгрузки,
	|	ТЧВидыЗапасов.НомерГТД,
	|	-ТЧВидыЗапасов.Количество
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.ВидыЗапасовПоВозвратамЗакрытойСмены КАК ТЧВидыЗапасов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ТаблицаДанныхДокумента
	|		ПО ТЧВидыЗапасов.Ссылка.Организация = ТаблицаДанныхДокумента.Организация
	|ГДЕ
	|	ТЧВидыЗапасов.Ссылка <> &Ссылка
...
Функция ДополнитьЗапросКонтролемОстатковНаКаждыйМесяц(Запрос, ТекстЗапроса, ПараметрыДополнения, МинимальнаяДатаОстатков, ДатыОстатков = Неопределено) Экспорт
	...
	Иначе
		...
		ТекстВТОстаткиПоМесяцамШаблон = СтрЗаменить(ТекстВТОстаткиПоМесяцамШаблон, "//%ТекстВыборкаНомерГТД%", "ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка),");
	...
	Для Каждого МесяцКонтроля из МесяцыКонтроля Цикл
		...
		Если ПараметрыДополнения.ЕстьТаблицаПриходуемыхВидовЗапасов Тогда
			...
			ТекстЗапросаВТОстаткиНаТекущийМесяц = СтрЗаменить(ТекстЗапросаВТОстаткиНаТекущийМесяц, "//%ТекстПриходуемыеВидыЗапасов%",
		...
		Если ПараметрыДополнения.ПодбиратьЗапасыРазныхПериодов Тогда
			...
			Для Каждого СтрМас из ДатыОстатков Цикл
				...
				Если ТекущийМесяц >= КонецМесяцаПолучаемыхОстатков
					...
					ТекстЗапросаНаТекущийМесяц = СтрЗаменить(ТекстЗапросаНаТекущийМесяц, "&ТекстВыборкаПериода", "&Конец" + СтрокаДатыОстатков);
					...
					ТекстЗапросаНаТекущийМесяц = СтрЗаменить(ТекстЗапросаНаТекущийМесяц, "&ТекстНомерПериода", Формат(НомерПериода, "ЧН=0; ЧГ="));
	...
	ТекстОстаткиПоМесяцам = СтрСоединить(МассивТекстовЗапросовПоМесяцам, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
		
	ТекстВТОстаткиПоМесяцам = СтрСоединить(МассивВТОстаткиПоМесяцам, ОбщегоНазначенияУТ.РазделительЗапросовВПакете()) + 
...
Процедура ТаблицаОстатковПереданныхМеждуОрганизациямиТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	СформироватьВТТаблицаОстатков("ТоварыОрганизаций", ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, Истина);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаОстатков.Организация КАК Организация,
	|	ТаблицаОстатков.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	...
	ТаблицаТоваров = ДокументОбъект.Товары.Выгрузить(, "АналитикаУчетаНоменклатуры, ДокументПередачи, НомерГТД, Количество");
	ТаблицаТоваров.Свернуть("АналитикаУчетаНоменклатуры, ДокументПередачи, НомерГТД", "Количество");
...
Процедура ВычестьИзОстатковПереданныеТовары(ТаблицаОстатковТоваровОрганизаций, ТаблицаОстатковПереданныхТоваров, ТаблицаТоваров)
	...
	Для Каждого СтрПереданных Из ТаблицаОстатковПереданныхТоваров Цикл
		...
		Для Каждого СтрОстатков Из СтрокиОстатковТоваровОрганизаций Цикл
			Если КоличествоКВычету = 0 Или СтрОстатков.КоличествоОстаток = 0 Тогда
				Прервать;
			КонецЕсли;
...
Функция ДатаПоследнегоДвижения() Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Период > ДобавитьМесяц(ТекущаяДатаСеанса(), 12) Тогда
			...
			ТекстИсключения = НСтр("ru='Документ %Регистратор% проведен по регистру ТоварыОрганизаций с датой, которая больше текущей на 12 месяцев. Отмените проведение этого документа и исправьте дату.';uk='Документ %Регистратор% проведено за регістром ТовариОрганізацій з датою, яка більше поточної на 12 місяців. Скасуйте проведення цього документа і виправте дату.'");
...
Процедура ПередУдалениемРегистратораПроверитьНаличиеЗаписейВРезервахТоваровОрганизаций(Источник, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ТекстИсключения = НСтр("ru='По документу %Документ% есть движения в регистре РезервыТоваровОрганизаций. Удаление документа невозможно.';uk='За документом %Документ% є руху в регістрі РезервыТоваровОрганизаций. Видалення документа неможливо.'");
...
//
Функция ТекстЗапросаОформленияПоРезервамТоваровОрганизаций(Запрос, ТипРезультата = "ТаблицаЗначений", Знач ИмяВТОтборыНоменклатуры = "") Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//%ТекстЗапросаОбъединенияСВТВстречныеРезервы%",
	...
	Запрос.УстановитьПараметр("ВидЦеныПоУмолчанию", Справочники.ВидыЦен.ВидЦеныПоУмолчанию(Справочники.ВидыЦен.ПустаяСсылка(),
	...
	ИначеЕсли ВРег(ТипРезультата) = ВРег("ВыборкаНаличияЗаписей") Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//%ТекстПараметрПроверкиНаличияЗаписей%", ТекстПараметрПроверкиНаличияЗаписей);	
	...
	ИначеЕсли ВРег(ТипРезультата) = ВРег("ВыборкаЗаполненияДокумента") Тогда		
		...
		|	РезервыТоваровОрганизаций.ВидЗапасов КАК ВидЗапасов,
		|	РезервыТоваровОрганизаций.ВидЗапасов.НалоговоеНазначение КАК НалоговоеНазначение,
	...
	Иначе
		ТекстИсключения = НСтр("ru='Неизвестный тип результата.';uk='Невідомий тип результату.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;		
