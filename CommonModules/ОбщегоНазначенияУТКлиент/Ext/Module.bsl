...
Процедура УстановитьПометкуУдаленияДокументаВопрос(Форма)
	...
	ЗаголовокВопроса = НСтр("ru='Изменение пометки удаления';uk='Зміна позначки вилучення'");
	Если НоваяПометкаУдаления Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Пометить ""%1"" на удаление?';uk='Відмітити ""%1"" для вилучення?'"),Форма.Объект.Ссылка);
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Снять с ""%1"" пометку на удаление?';uk='Зняти з ""%1"" позначку на вилучення?'"),Форма.Объект.Ссылка);
	КонецЕсли; 
	...
	Иначе
		...
		Иначе
			Оповещение = Новый ОписаниеОповещения("ВвестиДатуСКонтролемПустогоЗначенияПослеПредупреждения", ЭтотОбъект,
				ДополнительныеПараметры);
			ПоказатьПредупреждение(Оповещение, НСтр("ru='Поле ""Дата"" не заполнено';uk='Поле ""Дата"" не заповнене'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьФормуПослеПроверкиПроведенностиДокумента(ИмяФормы, ПараметрыФормы, ФормаВладелец, РежимОткрытияФормы = "Независимый", ИмяПараметраСсылка = "") Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru='Неизвестное значение параметра РежимОткрытияФормы';uk='Невідоме значення параметра РежимОткрытияФормы'");
...
Процедура ВыполнитьОбработкуОповещенияПослеПроверкиПроведенностиДокумента(Форма, Оповещение) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.Объект.Ссылка)
		...
		ТекстВопроса = НСтр("ru='Для продолжения необходимо провести документ. Провести?';uk='Для продовження необхідно провести документ. Провести?'");
...
Процедура ПриОпределенииВозможностиОбновления() Экспорт
    ...
    МинимальноДопустимаяВерсия = "3.2.16"; // для BAS УТ3
...
|;uk='Не вибрано розпорядження для оформлення документа.
|Для створення документа без розпорядження необхідно перейти в журнал ""%1"".'"),
		Параметры.СинонимЖурнала);
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Перейти в журнал';uk='Перейти в журнал'"));
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Не выбрано распоряжение';uk='Не вибрано розпорядження'"));
