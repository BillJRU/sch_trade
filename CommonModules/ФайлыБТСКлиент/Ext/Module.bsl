...
//    * БлокируемаяФорма - УправляемаяФорма, Неопределено - Владелец формы получения файла для блокировки.
...
//          ** Результат - Структура, Неопределено - структура со свойствами, либо Неопределено при отказе от 
//            помещения файла
//            *** ИмяФайлаИлиАдрес - Строка - Адрес во временном хранилище, либо имя файла на сервере (без пути)
...
//            ОписаниеОповещения
//    * БлокируемаяФорма - УправляемаяФорма, Неопределено - Владелец формы загрузки файла для блокировки.
//        Если указано Неопределено, на время загрузки будет заблокирован весь интерфейс
...
Процедура ПолучитьФайлИнтерактивно(ПараметрыПолученияФайла) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыПолученияФайла.ИмяФайлаИлиАдрес) Тогда
		ТекстОшибки = НСтр("ru='Не укано исходное хранение файла на сервере';uk='Не вказано вихідне зберігання файлу на сервері'");
...
Процедура ПоместитьФайлИнтерактивно(ПараметрыПомещенияФайла) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыПомещенияФайла.ИмяФайлаИлиАдрес) Тогда
		ТекстОшибки = НСтр("ru='Не указано целевое хранение файла на сервере';uk='Не вказано цільове зберігання файлу на сервері'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	ТекстПредложения = СтрШаблон(НСтр("ru='Для загрузки файлов, размером более %1, требуется установить расширение для работы с BAFм.
    |С этим расширением работа в веб-клиенте станет удобней не только при работе с большими файлами.'
    |;uk='Для завантаження файлів, розміром понад %1, потрібно встановити розширення для роботи з BAF. 
    |З цим розширенням робота в вебклієнті стане зручніше не тільки при роботі з великими файлами.'"), ФайлыБТСКлиентСервер.ПредставлениеРазмераФайла(
...
Процедура ПроверкаФайлаПередПомещением(СсылкаНаФайл, Отказ, ПараметрыПроверки)
	...
	Если СсылкаНаФайл = Неопределено Тогда
		ОбработатьОшибкуПроверки(Отказ, НСтр("ru='Не указана ссылка на файл';uk='Не вказано посилання на файл'"));
		Возврат;
	КонецЕсли;
	...
	Если СвойстваФильтра <> Неопределено 
        ...
        |%1'
        |;uk='Обрано невірний тип файлу. Виберіть:
	...
	Если НЕ ПараметрыПроверки.РасширениеПодключено Тогда
		...
		Если СсылкаНаФайл.Размер > МаксРазмерБезРасширения Тогда
            ...
            |Для работы в веб клиенте с такими файлами требуется установка расширения платформы BAF.
            |Воспользуйтесь тонким клиентом, либо установите расширение'
            |;uk='Обрано файл %1 більше %2 
            |Для роботи в вебклієнта з такими файлами потрібне встановлення розширення платформи BAF. 
            |Скористайтеся тонким клієнтом, або встановіть розширення'"),
	...
	Если СсылкаНаФайл.Размер > МаксимальныйРазмер Тогда
		Если МаксимальныйРазмер <= 0 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru='Размер файла должен быть равен %1';uk='Розмір файлу повинен бути рівний %1'"), ФайлыБТСКлиентСервер.ПредставлениеРазмераФайла(0));
		Иначе
			ТекстОшибки = СтрШаблон(НСтр("ru='Файл %1 слишком большой. Выберите файл менее %2';uk='Файл %1 занадто великий. Виберіть файл менше %2'"),
