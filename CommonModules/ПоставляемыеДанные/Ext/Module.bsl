...
Процедура СохранитьПоставляемыеДанныеВКэш(Знач Дескриптор, Знач ПутьКФайлу) Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			Если Данные = Неопределено Тогда
				...
				Если Данные.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
					ПутьКСтаромуФайлу = РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(
						Новый Структура("Том, ПутьКФайлу", Данные.Том, Данные.ПутьКФайлу));
				КонецЕсли;
			Иначе
		...
		Если Данные = Неопределено Тогда
			...
			СсылкаНаДанные = Справочники.ПоставляемыеДанные.ПолучитьСсылку();
			
			Данные = Справочники.ПоставляемыеДанные.СоздатьЭлемент();
			Данные.УстановитьСсылкуНового(СсылкаНаДанные);
			
		Иначе
			СсылкаНаДанные = Данные.Ссылка;
		КонецЕсли;
		...
		Иначе
			...
			ПараметрыДобавления = РаботаСФайламиВТомахСлужебный.ПараметрыДобавленияФайла();
			ПараметрыДобавления.Ссылка = СсылкаНаДанные;
			ПараметрыДобавления.Наименование = Строка(Данные.ИдентификаторФайла);
			ПараметрыДобавления.ДатаМодификацииУниверсальная = Данные.ДатаДобавления;
			РаботаСФайламиВТомахСлужебный.ДобавитьФайл(ПараметрыДобавления, ПутьКФайлу);
			
			Данные.ХранимыйФайл = ПараметрыДобавления.ХранимыйФайл;
			Данные.Том = ПараметрыДобавления.Том;
			Данные.ПутьКФайлу = ПараметрыДобавления.ПутьКФайлу;
			
		КонецЕсли;
		
		Данные.Записать();
		Если ПутьКСтаромуФайлу <> Неопределено Тогда
			РаботаСФайламиВТомахСлужебный.УдалитьФайл(ПутьКСтаромуФайлу);
		КонецЕсли;
...
Процедура УдалитьПоставляемыеДанныеИзКэша(Знач СсылкаИлиИдентификатор) Экспорт
	...
	Если Данные.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		ПолныйПуть = РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(
						Новый Структура("Том, ПутьКФайлу", Данные.Том, Данные.ПутьКФайлу));
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые данные.Получение файла из тома';uk='Дані, які поставляються.Отримання файлу з тому'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПолучитьОписаниеДанных(Знач Дескриптор, Знач ДескрипторJSON = Ложь) Экспорт
	...
	Если ТипЗнч(Дескриптор) = Тип("ОбъектXDTO") ИЛИ ДескрипторJSON Тогда
...
//
// Параметры:
//  ВидДанных - Строка - имя вида данных
//  Характеристики - Массив из Структура - структуры:
//  * Код - Строка
//  * Значение - Строка
//
// Возвращаемое значение:
...
//
// Параметры:
//  Выборка - ВыборкаИзРезультатаЗапроса - выборка запроса, содержащего информацию об обновлении данных:
//   * ХарактеристикиДанных - ВыборкаИзРезультатаЗапроса - выборка характеристик.
//
Функция ПолучитьXDTOДескриптор(Выборка)
	...
	Дескриптор.Properties = СоздатьОбъект(ФабрикаXDTO.Тип("http://www.1c.ru/SaaS/SuppliedData", "ArrayOfProperty"));
	ВыборкаХарактеристик = Выборка.ХарактеристикиДанных.Выбрать();
	ХарактеристикиXDTO = Дескриптор.Properties.Property; // СписокXDTO
	Пока ВыборкаХарактеристик.Следующий() Цикл
		...
		ХарактеристикиXDTO.Добавить(Характеристика);
	КонецЦикла; 
	Возврат Дескриптор;
...
Функция ПолучитьДескриптор(Знач Выборка)
	...
	ХарактеристикиДескриптора = Новый Массив;
	...
	Пока ВыборкаХарактеристик.Следующий() Цикл
		...
		ХарактеристикиДескриптора.Добавить(Характеристика);
	КонецЦикла; 
	...
	Дескриптор.Характеристики = ХарактеристикиДескриптора;
