	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Приложение было перемещено.
                       ...
                       |Регламентне завдання ""%1"", яке працює з зовнішніми ресурсами, відключено.'"), 
	...
	Иначе 
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменилась строка соединения информационной базы.
                       ...
                       |Регламентне завдання ""%1"" відключено.'"), 
			РегламентноеЗадание.Синоним);
	КонецЕсли;
...
Процедура ЗаписатьИдентификаторФайловойБазыВФайлПроверки(ИдентификаторБазы)
	...
	СодержимоеФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1
                   ...
                   |
                   |При копіюванні файлів інформаційної бази, в тому числі при створенні резервної копії, не слід копіювати цей файл.
                   |Одночасне використання двох копій інформаційної бази з однаковим ідентифікатором може привести до конфліктів
                   |при синхронізації даних, відправленні пошти та іншій роботі з зовнішніми ресурсами.
                   |
                   |Якщо файл відсутній в каталозі з інформаційною базою, то програма запросить адміністратора, чи дозволено їй працювати із зовнішніми ресурсами.'"), 
		ИдентификаторБазы, 
		Метаданные.Синоним);
...
Функция УстановитьБлокировкуРаботыСВнешнимиРесурсами()
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если Не ФайлПроверкиИдентификатораФайловойБазыСуществует() Тогда
			ТекстСообщения = НСтр("ru='В каталоге информационной базы отсутствует файл проверки DoNotCopy.txt.';uk='В каталозі інформаційної бази відсутній файл перевірки DoNotCopy.txt.'");
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИзменилсяИдентификаторИнформационнойБазы Тогда
			ТекстСообщения = 
				НСтр("ru='Идентификатор информационной базы в файле проверки DoNotCopy.txt не соответствует идентификатору в текущей базе.';uk='Ідентифікатор інформаційної бази в файлі перевірки DoNotCopy.txt не відповідає ідентифікатору в поточній базі.'");
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИзменилосьИмяБазы Или ИзменилосьИмяКомпьютера Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Изменились параметры контроля уникальности клиент-серверной базы.
                           ...
                           |;uk='Змінилися параметри контролю унікальності клієнт-серверної бази.
                           |
                           |Було:
                           |Рядок з''єднання: <%1>
                           |Ім''я комп''ютера: <%2>
...
Функция ПредставлениеПричиныБлокировки(ПараметрыБлокировки)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Блокировка выполнена на сервере <b>%1</b> в <b>%2</b> %3.
                   ...
                   |на 
                   |<b>%5</b>'
                   |;uk='Блокування виконано на сервері <b>%1</b> в <b>%2</b> %3.
                   |
                   |Розміщення інформаційної бази змінилося з
                   |<b>%4</b> 
                   |на
                   |<b>%5</b>'"),
		ИмяКомпьютера(),
...
Функция ПредставлениеТекущейОперации()
	...
	Если ЭтоСеансРегламентногоЗадания Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='при попытке выполнения регламентного задания <b>%1</b>';uk='при спробі виконання регламентного завдання <b>%1</b>'"),
			ФоновоеЗадание.РегламентноеЗадание.Наименование);
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='при входе пользователя <b>%1</b>';uk='при вході користувача <b>%1</b>'"),
		ИмяПользователя());
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт 
	...
	Возврат НСтр("ru='Работа с внешними ресурсами заблокирована';uk='Робота з зовнішніми ресурсами заблокована'",ОбщегоНазначения.КодОсновногоЯзыка());
