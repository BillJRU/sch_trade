...
// возвращая идентичный результат вне зависимости от того, подключено или нет расширение для работы с BAF.
// Ограничения:
...
// Ограничения:
//   Не используется для выбора каталогов - эта опция не поддерживается веб-клиентом.
//   Не поддерживается множественный выбор в веб-клиенте, если не установлено расширение для работы с BAF.
//
// Параметры:
//   ОбработчикЗавершения - ОписаниеОповещения - содержит описание процедуры, которая будет вызвана после
...
Процедура ЗагрузитьФайлы(
	...
	Если Не ПараметрыЗагрузки.Интерактивно
		...
		ВызватьИсключение НСтр("ru='Не указаны файлы для загрузки в неинтерактивном режиме.';uk='Не вказаі файли для завантаження в неінтерактивному режимі.'");
...
//                                          удалось подключить расширение для работы с BAF.
...
Процедура ОткрытьФайл(
	...
	Если ЭтоАдресВременногоХранилища(РасположениеФайла) Тогда
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ВызватьИсключение НСтр("ru='Не указано имя файла.';uk='Не вказано ім''я файлу.'");
		КонецЕсли;
...
//  ФайловаяСистемаКлиент.ОткрытьПроводник("C:\Program Files\BAF\common\1cestart.exe");
//  // Для Linux
//  ФайловаяСистемаКлиент.ОткрытьПроводник("/home/");
//  ФайловаяСистемаКлиент.ОткрытьПроводник("/opt/BAF/v8.3/x86_64/");
//
Процедура ОткрытьПроводник(ПутьККаталогуИлиФайлу) Экспорт
	...
	ТекстПредложения = НСтр("ru='Для открытия папки необходимо установить расширение для работы с BAF.';uk='Для відкриття папки необхідно встановити розширення для роботи з BAF.'");
...
//  ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://www.bas-soft.eu");
//  ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("mailto:help@bas-soft.eu");
...
#КонецЕсли
	...
	Иначе 
		...
		ТекстПредложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для открытия ссылки ""%1"" необходимо установить расширение для работы с BAF.';uk='Для відкриття посилання ""%1"" необхідно встановити розширення для роботи з BAF.'"),
			НавигационнаяСсылка);
		ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
...
//   Например, в пакетном режиме платформы BAF предусмотрен ключ "/L en";
...
#КонецЕсли
	...
	ТекстПредложения = 
		НСтр("ru='Для создания временного каталога необходимо установить расширение для работы с BAF.';uk='Для створення тимчасового каталогу необхідно встановити розширення для роботи з BAF.'");
	ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
...
//      - ПутьККаталогу - Строка - полный путь к каталогу. Если не установлено расширение для работы с BAF
...
Процедура СоздатьВременныйКаталог(Знач Оповещение, Расширение = "") Экспорт 
	...
	ТекстПредложения = НСтр("ru='Для создания временного каталога необходимо установить расширение для работы с BAF.';uk='Для створення тимчасового каталогу необхідно встановити розширення для роботи з BAF.'");
	ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
	
КонецПроцедуры

// Предлагает пользователю установить расширение для работы с BAF в веб-клиенте.
...
//  ТекстСообщения = НСтр("ru='Для печати документа необходимо установить расширение для работы с BAF.'");
...
Процедура СократитьПолноеИмяФайлаДоДопустимойДлиныNTFS(ПолноеИмяФайла)
	...
	Если ДлинаПути > ДопустимаяДлинаNTFS - ДлинаРасширения - 1 Тогда
        ...
        |%1'
        |;uk='Шлях до файлу занадто довгий:
        |%1'"), ПолноеИмяФайла);
	КонецЕсли;
