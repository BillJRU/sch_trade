...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("ru='Ошибка настройки полнотекстового поиска';uk='Помилка налаштування повнотекстового пошуку'");
	...
	Дело.Подсказка = 
		НСтр("ru='Рассинхронизация значения константы ИспользоватьПолнотекстовыйПоиск
                   ...
                   |;uk='Розсинхронізація значення константи ИспользоватьПолнотекстовыйПоиск
                   |і встановленого режиму повнотекстового пошуку в інформаційній базі.
                   |Спробуйте вимкнути повнотекстовий пошук і знову включити.'");
	...
	Если Состояние = "ТребуетсяОбновлениеИндекса" Тогда 
		...
		Если ДатаАктуальностиИндекса > ТекущаяДата Тогда
			Интервал = НСтр("ru='менее одного дня назад';uk='менше одного дня назад'");
		Иначе
			Интервал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 назад';uk='%1 назад'"),
	...
	Иначе 
		Интервал = НСтр("ru='Никогда';uk='Ніколи'");
		ЕстьДела = Ложь;
	КонецЕсли;
	...
	Дело.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Последнее обновление %1';uk='Останнє оновлення %1'"),
		Интервал);
	Дело.Владелец = Раздел;
...
Функция ИндексПолнотекстовогоПоискаАктуален()
	...
	Исключение
		ЗаписьЖурнала(
			...
			НСтр("ru='Не удалось проверить состояние индекса полнотекстового поиска';uk='Не вдалося перевірити стан індексу повнотекстового пошуку'"),
			ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(
		Неопределено, 
		НСтр("ru='Запуск процедуры ""%1"".';uk='Запуск процедури ""%1"".'"),,
	...
	Попытка
		...
		ЗаписьЖурнала(
			Неопределено, 
			НСтр("ru='Успешное завершение процедуры ""%1"".';uk='Успішне завершення процедури ""%1"".'"),, 
			ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(
			УровеньЖурналаРегистрации.Предупреждение, 
			НСтр("ru='Не удалось выполнить процедуру ""%1"":';uk='Не вдалося виконати процедуру ""%1"":'"),
			ИнформацияОбОшибке(), 
			ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Полнотекстовое индексирование';uk='Повнотекстове індексування'",ОбщегоНазначения.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
