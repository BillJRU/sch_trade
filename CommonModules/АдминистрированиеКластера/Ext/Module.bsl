...
//     * Передано - Число - объем данных, переданных меду сервером "платформы BAF" и клиентским приложением
//          данного сеанса с момента начала сеанса (в байтах),
//     * ПереданоЗа5Минут - Число - объем данных, переданных между сервером "платформы BAF" и клиентским
//          приложением данного сеанса за последние 5 минут (в байтах),
//     * СерверныхВызовов - Число - количество вызовов сервера "платформы BAF" от имена данного сеанса с
//          момента начала сеанса,
//     * СерверныхВызововЗа5Минут - Число - количество вызовов сервера "платформы BAF" от имени данного сеанса
//          за последние 5 минут,
//     * ДлительностьСерверныхВызовов - Число - время исполнения вызовов сервера "платформы BAF" от имени
...
//          обращения, в случае, если сеанс выполняется вызов сервера "платформы BAF" (иначе - значение равно 0),
//     * ДлительностьСерверныхВызововЗа5Минут - Число - время исполнения вызовов сервера платформы BAF от имени
...
//     * ВыданаСервером - Булево - Истина - лицензия получена сервером "платформы BAF" и выдана клиентскому приложению.
...
// Возвращаемое значение: 
//   Структура - структура с полями:
//     * ИмяПриложения - Строка - содержит имя приложения, установившего соединение с фермой серверов "платформы BAF".
...
//     * ИмяПользователя - Строка - имя пользователя платформы BAF, подсоединенного к информационной базе.
...
//     * ПереданоСУБД - Число - объем данных, переданных между сервером платформы BAF и сервером баз данных,
//                  с момента установки данного соединения,
//     * ПереданоСУБДЗа5Минут - Число - объем данных, переданных между сервером платформы BAF и сервером баз
...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Неверный тип параметра Фильтр, ожидалось <Структура> или <Массив>';uk='Невірний тип параметра Фильтр, очікувалося <Структура> або <Массив>'");
...
Функция ПроверитьУсловияФильтра(Знач ПроверяемыйОбъект, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru='Неверный тип значения параметра Фильтр, ожидалось <Структура> или <КлючИЗначение>';uk='Невірний тип значення параметра Фильтр, очікувалося <Структура> або <КлючИЗначение>'");
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный тип параметра ПараметрыАдминистрирования (%1), ожидалось ""COM"" или ""RAS""';uk='Невідомий тип параметра ПараметрыАдминистрирования (%1), очікувалося ""COM"" або ""RAS""'"), 
