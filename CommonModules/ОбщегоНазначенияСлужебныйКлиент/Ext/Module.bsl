		...
		Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось подключить внешнюю компоненту на клиенте
                           ...
                           |%1
                           |через:
                           |Не припустимо одночасно не вказувати Идентификатор і ИдентификаторыСозданияОбъектов''ектов'"), 
	...
	Иначе 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при регистрации компоненты comcntr.
                       ...
                       |;uk='Помилка при реєстрації компоненти comcntr.
                       |Код помилки regsvr32: %1'"),
			КодВозврата);
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru='Регистрация компоненты comcntr';uk='Реєстрація компонента comcntr'",ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка",
			ТекстСообщения,,
			Истина);
...
Процедура ПроверитьРасширениеРаботыСФайламиПодключеноЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		ТекстСообщения = НСтр("ru='Действие недоступно, так как не установлено расширение для работы с BAF.';uk='Дія недоступна, так як не встановлено розширення для роботи з BAF.'")
	КонецЕсли;
