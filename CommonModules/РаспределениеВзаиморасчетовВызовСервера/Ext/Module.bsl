	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Не удалось заблокировать регистр заданий';uk='Не вдалося заблокувати регістр завдань'",КодЯзыка), УровеньЖурналаРегистрации.Ошибка);
				ВызватьИсключение;
			КонецПопытки;
		Иначе
			ДанныеКРасчетуСКлиентамиЗаМесяц = ДанныеКРасчетуСКлиентамиЗаМесяц(НачалоПериода, ОкончаниеПериода, АналитикиРасчета, ИспользуемыеМетаданные, НомерЗаданияДоРасчета);
		...
		НДСОбщегоНазначенияСервер.ДополнитьТаблицуНачальныхГраниц(НачальныеГраницыНДС, НачалоПериода, ДанныеКРасчетуСКлиентамиЗаМесяц.АналитикиУчетаПоПартнерам);
		...
		Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Не удалось установить границы расчетов';uk='Не вдалося встановити межі розрахунків'",КодЯзыка), УровеньЖурналаРегистрации.Ошибка);
				ВызватьИсключение;
			КонецПопытки
		Иначе
			УстановитьГраницыРасчетов(НачалоПериода, ОкончаниеПериода, ИспользуемыеМетаданные.ИмяРегистраЗаданий, АналитикиРасчета, ДанныеКРасчетуСКлиентамиЗаМесяц.ВременныеТаблицы, НомерЗаданияДоРасчета, КоличествоПовторяющихсяДанных);
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		НДСОбщегоНазначенияСервер.ДополнитьТаблицуНачальныхГраниц(НачальныеГраницыНДС, НачалоПериода, ДанныеКРасчетуСПоставщикамиЗаМесяц.АналитикиУчетаПоПартнерам);
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Взаиморасчеты.Распределены расчеты с поставщиками за период';uk='Взаєморозрахунки. Розподілені розрахунки з постачальниками за період'",КодЯзыка), УровеньИнформация, , Формат(НачалоПериода, "ДФ='yyyy.MM'"));
...
Процедура РассчитатьРасчетыСКлиентами(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	Если РегистраторыКОтражению.Количество() > 0 Тогда
		РегистрыСведений.СуммыДокументовВВалютахУчета.РассчитатьПоДаннымВзаиморасчетов(Контекст, РегистраторыКОтражению);
	КонецЕсли;
...
Функция ТекстДвиженияРасчетовСКлиентами()
	Возврат "
		...
		|	ДД.Регистратор КАК Регистратор,
		|	ДД.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		|	ВЫБОР КОГДА ДД.ОбъектРасчетов.Объект = НЕОПРЕДЕЛЕНО
		|		ТОГДА NULL
		|		ИНАЧЕ ДД.ОбъектРасчетов
		|	КОНЕЦ КАК ЗаказКлиента,
...
Функция ТекстДвиженияРасчетовСПоставщиками()
	Возврат "
		...
		|		ИНАЧЕ 9
		|	КОНЕЦ КАК Приоритет,
		|	ВЫБОР КОГДА ДД.ОбъектРасчетов.Объект ССЫЛКА Документ.СписаниеБезналичныхДенежныхСредств
		|		ИЛИ ДД.ОбъектРасчетов.Объект ССЫЛКА Документ.РасходныйКассовыйОрдер
		|		ИЛИ ДД.ОбъектРасчетов.Объект ССЫЛКА Документ.ПервичныйДокумент
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК ПриоритетОбъектаРасчетовПлатежки,
...
Процедура ЗаполнитьРасчетыСПоставщиками(РасчетнаяПартия, Расход, Приход, Валюты, РасчетныеПартии) // РасчетыСПоставщикамиПоДокументам
	...
	Иначе
		...
		Если Приход.Регистратор = Неопределено
			...
			ИЛИ Приход.ТипЗаписи = "ОстатокПоступлений" И (Приход.РасчетныйДокумент = Приход.ЗаказПоставщику.Объект) Тогда
			РасчетнаяПартия.РасчетныйДокумент = Приход.РасчетныйДокумент;
		Иначе
...
Процедура ПроверитьДатуЗапрета(Период)
	...
	Если ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ШаблонЗапретаДанных) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось запустить распределение взаиморасчетов за %1 из-за установленной даты запрета.';uk='Не вдалося запустити розподіл взаєморозрахунків за %1 через встановлену дату заборони.'"),
				Формат(Период,"ДФ='MMMM yyyy'"));
				
		ЗаписьЖурналаРегистрации(НСтр("ru='Взаиморасчеты';uk='Взаєморозрахунки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
