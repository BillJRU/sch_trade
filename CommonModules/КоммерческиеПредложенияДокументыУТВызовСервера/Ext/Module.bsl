...
Функция УсловияОплатыТекстом(ИсточникУсловийОплаты) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеУсловий.ФормаОплаты) Тогда
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru='Форма оплаты: %1.';uk='Форма оплати: %1.'"), Строка(ДанныеУсловий.ФормаОплаты)));
		МассивСтрок.Добавить(Символы.ПС);
	КонецЕсли;
	
	Если ДанныеУсловий.ЭтапыОплаты.Количество() > 0 Тогда
		МассивСтрок.Добавить(НСтр("ru='Этапы оплаты:';uk='Етапи оплати:'"));
	КонецЕсли;
	
	Для Каждого ЭтапОплаты Из ДанныеУсловий.ЭтапыОплаты Цикл
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru='%1, отсрочка (дн.) - %2, процент платежа - %3 %%';uk='%1, відстрочка (дн.) - %2, відсоток платежу - %3 %%'"),
