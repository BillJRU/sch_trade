...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Выгрузка данных для перехода в сервис';uk='Файли.Вивантаження даних для переходу в сервіс'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОбработатьОчередьИзвлеченияТекста() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ru='Транзакция активна. Выполнение запроса в транзакции невозможно.';uk='Транзакція активна. Виконання запиту в транзакції неможливе.'"));
	КонецЕсли;
