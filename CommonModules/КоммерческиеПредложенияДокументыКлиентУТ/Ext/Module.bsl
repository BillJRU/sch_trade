...
Процедура ПриВыполнениеКомандыЗаполнитьУсловияОплаты(Форма) Экспорт
	...
	Если Не ПустаяСтрока(Объект.УсловияОплатыТекст) Тогда
		...
		ТекстВопроса = НСтр("ru='Условия оплаты будут перезаполнены. Продолжить?';uk='Умови оплати будуть перезаповнені. Продовжити?'");
...
Процедура ПриВыполненииКомандыЗаполнитьЦеныПоУсловиямПродаж(Форма, ОповещениеОВыборе) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.Валюта) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указана валюта. Цены не могут быть заполнены.';uk='Не вказана валюта. Ціни не можуть бути заповнені.'");
	...
	Если Объект.Товары.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Табличная часть ""Товары"" пустая. Цены не могут быть заполнены.';uk='Таблична частина ""Товари"" порожня. Ціни не можуть бути заповнені.'");
	...
	Если Не ЗначениеЗаполнено(Объект.Клиент) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указан клиент. Цены не могут быть заполнены.';uk='Не вказаний клієнт. Ціни не можуть бути заповнені.'");
	...
	Если Не ЗначениеЗаполнено(Объект.ХозяйственнаяОперация) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указана операция. Цены не могут быть заполнены.';uk='Не вказана операція. Ціни не можуть бути заповнені.'");
...
Процедура ПриВыполненииКомандыНазначитьРучнуюСкидку(Форма, ВыделенныеСтроки, СтандартнаяОбработка, ТребуетсяСерверныйВызов) Экспорт
	...
	Если СкидкиНаценкиКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "Товары", НСтр("ru='Товары';uk='Товари'")) Тогда
		ТребуетсяСерверныйВызов = Истина;
	КонецЕсли;
...
Процедура ПриВыполненииКомандыОтменитьРучныеСкидки(Форма, ВыделенныеСтроки, ЦенаВключаетНДС, Налогообложение, ТребуетсяСерверныйВызов) Экспорт
	...
	Если СкидкиНаценкиКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "Товары", НСтр("ru='Товары';uk='Товари'")) Тогда
		ТребуетсяСерверныйВызов = Истина;
	КонецЕсли;
...
Процедура КоммерческоеПредложениеКлиентуОткрытьФормуПодбораНоменклатуры(Форма, ОповещениеПриЗавершении)
	...
	Если Не ЗначениеЗаполнено(Объект.Валюта) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Поле ""Валюта"" не заполнено';uk='Поле ""Валюта"" не заповнено'"), Объект.Ссылка, "Объект.Валюта");
		Возврат;
	КонецЕсли;
	...
	Если Форма.НастройкиУчета.ИспользуютсяВидыЦен Тогда
		...
		ПараметрыОткрытияФормы.Вставить("Заголовок", НСтр("ru='Вид цены для отображения в подборе';uk='Вид ціни для відображення в підборі'"));
