...
Процедура РаспределитьРасходыБудущихПериодов(ПараметрыРасчета) Экспорт
    ...
    |	(ВЫБОР
    |		КОГДА Распределение.РаспределитьРегл ТОГДА Расходы.СуммаРеглБезНДС
    |		ИНАЧЕ 0 КОНЕЦ) КАК СуммаРеглБезНДС,
    |	(ВЫБОР
    |		КОГДА Распределение.РаспределитьРегл ТОГДА Расходы.НДСРегл
    |		ИНАЧЕ 0 КОНЕЦ) КАК НДСРегл,
    |	(ВЫБОР
    |		КОГДА Распределение.РаспределитьУпр ТОГДА Расходы.СуммаУпрБезНДС
    ...
    |		И Распределение.НалоговоеНазначение = Расходы.НалоговоеНазначение
		...
		ИначеЕсли СтрокаОстатка.Организация = Выборка.Организация
		 ...
		 И СтрокаОстатка.НаправлениеДеятельности = Выборка.НаправлениеДеятельности Тогда
		 	...
		 	Если СтрокаОстатка.ДоляСтоимостиУпр <= Выборка.ДоляСтоимостиУпр Тогда
                ЗаполнитьЗначенияСвойств(СуммовыеРесурсы, СтрокаОстатка, "Сумма, СуммаУпр, СуммаУпрБезНДС");
			...
			Иначе
				Если СтрокаОстатка.ДоляСтоимостиУпр <> 0 Тогда
                    ...
                    СуммовыеРесурсы.СуммаУпрБезНДС = 
                    	Окр(Выборка.ДоляСтоимостиУпр * СтрокаОстатка.СуммаУпрБезНДС / СтрокаОстатка.ДоляСтоимостиУпр, 2, 1);
				КонецЕсли;
			...
			Если СтрокаОстатка.ДоляСтоимостиРегл <= Выборка.ДоляСтоимостиРегл Тогда
                ЗаполнитьЗначенияСвойств(СуммовыеРесурсы, СтрокаОстатка, "СуммаРегл, СуммаРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница");
			...
			Иначе	
				Если СтрокаОстатка.ДоляСтоимостиРегл <> 0 Тогда
                    ...
                    СуммовыеРесурсы.СуммаРеглБезНДС = 
                    	Окр(Выборка.ДоляСтоимостиРегл * СтрокаОстатка.СуммаРеглБезНДС / СтрокаОстатка.ДоляСтоимостиРегл, 2, 1);
                    СуммовыеРесурсы.НДСРегл = 
                    	Окр(Выборка.ДоляСтоимостиРегл * СтрокаОстатка.НДСРегл / СтрокаОстатка.ДоляСтоимостиРегл, 2, 1);
            ...
            СтрокаОстатка.СуммаУпрБезНДС 	= СтрокаОстатка.СуммаУпрБезНДС - СуммовыеРесурсы.СуммаУпрБезНДС;
			СтрокаОстатка.СуммаРегл 		= СтрокаОстатка.СуммаРегл - СуммовыеРесурсы.СуммаРегл;
            СтрокаОстатка.СуммаРеглБезНДС 	= СтрокаОстатка.СуммаРеглБезНДС - СуммовыеРесурсы.СуммаРеглБезНДС;
            СтрокаОстатка.НДСРегл 			= СтрокаОстатка.НДСРегл - СуммовыеРесурсы.НДСРегл;
...
Процедура СформироватьДвиженияРаспределениеРБП(ПараметрыРасчета, ДанныеДвижения, Суммы)
    ...
    КопируемыеПоля = "Период, Организация, НалоговоеНазначение, Подразделение, СтатьяРасходов, АналитикаРасходов, НаправлениеДеятельности";
    ...
    ЗаполнитьЗначенияСвойств(ЗаписьРасход, Суммы, "Сумма, СуммаУпр, СуммаУпрБезНДС, СуммаРегл, СуммаРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница");
	...
	ЗаполнитьЗначенияСвойств(ЗаписьПриход, Суммы, "Сумма, СуммаУпр, СуммаУпрБезНДС, СуммаРегл, СуммаРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница");
...
Функция ПолучитьДанныеДляДопРасходов(ПараметрыРасчета) Экспорт
	...
	Если ПараметрыРасчета.РежимОтладки Тогда	
		 ВТКэшРасчетныеОборотыСебестоимостьТоваров  = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "ВТКэшРасчетныеОборотыСебестоимостьТоваров");
		 ВТСтоимостьПартийТоваров 				    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "ВТСтоимостьПартийТоваров"); 
		 АналитикиРасходовВсе 	                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "АналитикиРасходовВсе");
	 КонецЕсли;
...
Функция СформироватьПартииПрочихРасходов(ПараметрыРасчета, ДанныеДляРасчета) Экспорт
    ...
    УзлыДвижений = Новый Соответствие; // [Организация, [АналитикаРасходов, [ИндексСтроки, [НалоговоеНазначениеСписания, Базис]]]]
    Ключи = Новый Массив(4); // {Организация, АналитикаРасходов, ИндексСтроки, НалоговоеНазначениеСписания}
	...
	Для Каждого Движение Из ДанныеДляРасчета Цикл
        ...
        // Накапливаем приход партий прочих расходов для распределения по налоговому назначению.
        ...
        // Непосредственно расчет базисов по налоговому назначению списания.
		...
		Для Каждого Значение Из АналитикиОбсчетаПартийПрочих(Движение, ПустыеСсылки) Цикл
			...
			Для Каждого ИндексСтроки Из ИндексыСтрок Цикл
                ...
                // Увеличим базу по налоговому назначению списания (Ключи[3]).
    ...
    // Шаг 3: Создаем расчетные партии расходов по каждому накопленному налоговому назначению списания.
...
Функция ПолучитьДанныеПриходовПартийРасходов(ПараметрыРасчета, ДанныеДляРасчета)
    ...
    ДанныеДляРасчета.Сортировать("Организация, НалоговоеНазначениеСписания, Приоритет, Период, Регистратор, АналитикаУчетаНоменклатуры", Новый СравнениеЗначений);
	УзлыДвижений = Новый Соответствие;
    Ключи = Новый Массив(7); // {Организация, НалоговоеНазначениеСписания, ПравилоРаспределения, АналитикаРасходов, ДокументПоступленияРасходов, НомерСтроки}
	...
	Для Каждого Движение Из ДанныеДляРасчета Цикл
		...
		Иначе
            Ключи[1] = Движение.НалоговоеНазначениеСписания;
		КонецЕсли;
		...
		Для Каждого Правило Из ПравилаРаспределения Цикл
			...
			Для Каждого Значение Из АналитикиОбсчетаПартийПрочих(Движение, ПустыеСсылки) Цикл
				...
				Если Не ЗначениеЗаполнено(Значение) И Движение.Период < ПараметрыРасчета.РасчетныйПериод.НачалоПериода Тогда
					Продолжить;
				КонецЕсли;
				ПоступленияРасходов = Аналитики[Значение]; // Соответствие
						...
						Для Каждого НаправлениеДеятельности Из НомерСтроки.Значение Цикл
							Если ЗначениеЗаполнено(НаправлениеДеятельности.Ключ)
							 ...
							 ИЛИ НЕ ЗначениеЗаполнено(НаправлениеДеятельности.Ключ) Тогда
							 	МассивНаправлений = НаправлениеДеятельности.Значение; // Массив
								МассивНаправлений.Добавить(Движение);
							КонецЕсли;
						КонецЦикла;
					КонецЦикла
				КонецЦикла;
...
Процедура СформироватьДопРасходыВСебестоимостьТоваров(ПараметрыРасчета, РасчетныеПриходыПартийРасходов)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
        ...
        РасчетнаяПартия.НалоговоеНазначение  		= Движение.НалоговоеНазначениеСписания;
        ...
        Если ЗначениеЗаполнено(РасчетнаяПартия.НалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РасчетнаяПартия.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая Тогда 
        ...
        РасчетнаяПартия.ДопРасходыРеглБезНДС        = Движение.СтоимостьРегл;
        РасчетнаяПартия.ДопРасходыУпрБезНДС         = ?(ПараметрыРасчета.УправленческийУчетОрганизаций, Движение.СтоимостьБезНДС, 0);
        
		РасчетнаяПартия.НДСРегл 					= Движение.НДСРегл;
		РасчетнаяПартия.НДСРеглРеквизит             = Движение.НДСРегл;
...
Процедура СформироватьДопРасходыВПрочиеРасходы(ПараметрыРасчета, РасчетныеПриходыПартийРасходов)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
		...
		РасчетнаяПартия.ХозяйственнаяОперация 	= Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика;
        РасчетнаяПартия.НалоговоеНазначение     = Движение.КорНалоговоеНазначение;
        
		РасчетнаяПартия.ДокументДвижения 		= Движение.Регистратор;
        
        Если ЗначениеЗаполнено(Движение.КорНалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Движение.КорНалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая Тогда 
...
Функция ТекстПрошлыеВыбытияТоваров() // вт ПрошлыеВыбытияТоваров
	...
	ТекстЗапроса = "
        ...
        |	ВЫРАЗИТЬ(СУММА(ДД.НДСРегл
        |		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК ЧИСЛО(31,2)) КАК НДСРегл,
        |	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьУпрБезНДС
        |		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК ЧИСЛО(31,2)) КАК СтоимостьУпрБезНДС,
        |	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьРеглБезНДС
        |		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК ЧИСЛО(31,2)) КАК СтоимостьРеглБезНДС,
        ...
        |	Остатки.СтоимостьУпрБезНДСОстаток КАК СтоимостьУпрБезНДС,
        |	Остатки.СтоимостьРеглБезНДСОстаток КАК СтоимостьРеглБезНДС,
        ...
        |	СУММА(ДД.СтоимостьУпрБезНДС) КАК СтоимостьУпрБезНДС,
...
Функция ОстаткиПрошлыхПоступленийСводно() // вт ОстаткиПрошлыхПоступленийСводно
	Возврат "
        ...
        |	МАКСИМУМ(Остатки.НалоговоеНазначение) КАК НалоговоеНазначение
...
Функция ТекстПрошлыеПоступления() // вт ПрошлыеПоступления
	Возврат 
        ...
        |		ИНАЧЕ ОстаткиСводно.НалоговоеНазначение КОНЕЦ) КАК НалоговоеНазначение,
		|	(ВЫБОР
		|		КОГДА ДД.Период >= &ДатаПереходаНаПартионныйУчетВерсии22
		...
		|		КОГДА ДД.Период >= &ДатаПереходаНаПартионныйУчетВерсии22
        |			ТОГДА ДД.НалоговоеНазначение
        |		ИНАЧЕ ОстаткиСводно.НалоговоеНазначение КОНЕЦ),
		|	(ВЫБОР
		|		КОГДА ДД.Период >= &ДатаПереходаНаПартионныйУчетВерсии22
		|			ТОГДА ДД.АналитикаФинансовогоУчета
...
Функция ТекстПрошлыеПеремещенияПоступленияСборка() // вт ПрошлыеПеремещенияПоступленияСборка
	Возврат 
		...
		|	(ВЫБОР
        |		КОГДА ДД.Период >= &ДатаПереходаНаПартионныйУчетВерсии22
        |			ТОГДА ДД.КорНалоговоеНазначение
        |		ИНАЧЕ ОстаткиСводно.НалоговоеНазначение КОНЕЦ) КАК НалоговоеНазначение,
		|	(ВЫБОР
		|		КОГДА ДД.Период >= &ДатаПереходаНаПартионныйУчетВерсии22
...
Функция ТекстОтборОстатковПрошлыхПоступлений() // ВТ ОтборОстатковПрошлыхПоступлений
	Возврат
        ...
        |		И ОстаткиСВидамиЗапасов.НалоговоеНазначение = Партии.НалоговоеНазначение
		|";
КонецФункции
...
Функция ТекстБазисПередачиТоваровСебестоимость()
	...
	ТекстЗапроса = 
		...
		|	0 КАК ВременнаяРазница,
        |	ДД.КорНалоговоеНазначение КАК НалоговоеНазначение,
        |	ДД.КорНалоговоеНазначение КАК НалоговоеНазначениеСписания,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеРасходов,
		|	НЕОПРЕДЕЛЕНО КАК ДокументРеализации,
		|	НЕОПРЕДЕЛЕНО КАК ДокументИсточник,
...
Функция ТекстБазисПеремещенияПоступленияСборкаСебестоимость()
	...
	ТекстЗапроса = 
		...
		|			КлючиРасходов.АналитикаРасходов = ДД.Регистратор
		|			ИЛИ
		|			ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.СборкаТоваров)
		|			И КлючиРасходов.АналитикаРасходов В (
		|				АналитикиТоваров.Номенклатура,
		|				ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка),
		|				АналитикиТоваров.МестоХранения,
...
Функция ТекстБазисПрошлыеПриходыТоваровСебестоимость()
	...
	ТекстЗапроса = 
        ...
        |		И ПрошлыеВыбытияТоваров.НалоговоеНазначение = ДД.НалоговоеНазначение
...
Функция ТекстНоменклатураДоходыРасходыИзСебестоимости()
	Возврат
        ...
        |	СУММА(ДД.ДопРасходыРеглБезНДС) 		 КАК ДопРасходыРеглБезНДС,
        |	СУММА(ДД.ДопРасходыУпрБезНДС) 		 КАК ДопРасходыУпрБезНДС,
		|
		...
		|ГДЕ
		|	ДД.Количество = 0
        |	И (ДД.ДопРасходы <> 0 ИЛИ ДД.ДопРасходыБезНДС <> 0 ИЛИ ДД.ДопРасходыРегл <> 0 ИЛИ ДД.ДопРасходыУпр <> 0 ИЛИ ДД.ДопРасходыРеглБезНДС <> 0 ИЛИ ДД.ДопРасходыУпрБезНДС <> 0)
...
Процедура ЗаполнитьРасчетнуюПартиюПриходПартииРасходов(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
    ...
    РасчетнаяПартия.КорНалоговоеНазначение   = Приход.НалоговоеНазначениеСписания;
...
Функция ТекстРаспределениеРасходовНаВыбытиеТоваровПрошлыхПериодов() // ВТ РаспределениеРасходов - общая для регистров себестоимости, выручки, прочих расходов
    ...
    |	СУММА(ДД.ДопРасходыРеглБезНДС) КАК ДопРасходыРеглБезНДС,
    |	СУММА(ДД.ДопРасходыУпрБезНДС) КАК ДопРасходыУпрБезНДС,
    |	СУММА(ДД.НДСРеглРеквизит) КАК НДСРегл,
	|	СУММА(ДД.НДСУпр) КАК НДСУпр,
	|	СУММА(ДД.ПостояннаяРазница) КАК ПостояннаяРазница,
	|	СУММА(ДД.ВременнаяРазница) КАК ВременнаяРазница
	|
    ...
    |	ДД.ДопРасходыРегл,
    |	ДД.ДопРасходыУпр,
    |	ДД.ДопРасходыРеглБезНДС,
    |	ДД.ДопРасходыУпрБезНДС,
    |	ДД.НДСРегл,
	|	ДД.НДСУпр,
	|	ДД.ПостояннаяРазница,
	|	ДД.ВременнаяРазница,
    ...
    |	ВЫБОР КОГДА ПрошлыеВыбытия.ЭтоОстатки
    |		ТОГДА ПрошлыеВыбытия.НалоговоеНазначение
	...
	|
	|	ВЫРАЗИТЬ(ДД.ДопРасходы * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходы,
	|	ВЫРАЗИТЬ(ДД.ДопРасходыБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходыБезНДС,
    |	ВЫРАЗИТЬ(ДД.ДопРасходыРегл * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходыРегл,
    |	ВЫРАЗИТЬ(ДД.ДопРасходыУпр * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходыУпр,
    |	ВЫРАЗИТЬ(ДД.ДопРасходыРеглБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходыРеглБезНДС,
    |	ВЫРАЗИТЬ(ДД.ДопРасходыУпрБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ДопРасходыУпрБезНДС,
    |	ВЫРАЗИТЬ(ДД.НДСРегл * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК НДСРегл,
	|	ВЫРАЗИТЬ(ДД.НДСУпр * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК НДСУпр,
	|	ВЫРАЗИТЬ(ДД.ПостояннаяРазница * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ПостояннаяРазница,
	|	ВЫРАЗИТЬ(ДД.ВременнаяРазница * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) КАК ВременнаяРазница
	|
	...
	|				И ПрошлыеВыбытия.ЭтоОстатки)
	|ГДЕ
	|	(ВЫРАЗИТЬ(ДД.ДопРасходы * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
	|		ИЛИ ВЫРАЗИТЬ(ДД.ДопРасходыБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
    |		ИЛИ ВЫРАЗИТЬ(ДД.ДопРасходыРегл * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
    |		ИЛИ ВЫРАЗИТЬ(ДД.ДопРасходыУпр * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
    |		ИЛИ ВЫРАЗИТЬ(ДД.ДопРасходыРеглБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
    |		ИЛИ ВЫРАЗИТЬ(ДД.ДопРасходыУпрБезНДС * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
    |		ИЛИ ВЫРАЗИТЬ(ДД.НДСРегл * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
	|		ИЛИ ВЫРАЗИТЬ(ДД.НДСУпр * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
	|		ИЛИ ВЫРАЗИТЬ(ДД.ПостояннаяРазница * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0
	|		ИЛИ ВЫРАЗИТЬ(ДД.ВременнаяРазница * ПрошлыеВыбытия.Количество / ДД.Количество КАК ЧИСЛО(31,2)) <> 0)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
    ...
    |	База.ДопРасходыРеглБезНДС - СУММА(ДД.ДопРасходыРеглБезНДС) КАК ДопРасходыРеглБезНДС,
    |	База.ДопРасходыУпрБезНДС - СУММА(ДД.ДопРасходыУпрБезНДС) КАК ДопРасходыУпрБезНДС,
    ...
    |		И ДД.НалоговоеНазначение = База.НалоговоеНазначение
    ...
    |	База.ДопРасходыРегл,
    |	База.ДопРасходыУпр,
    |	База.ДопРасходыРеглБезНДС,
    |	База.ДопРасходыУпрБезНДС,
    |	База.НДСРегл,
	|	База.НДСУпр,
    ...
    |	ИЛИ СУММА(ДД.ДопРасходыРеглБезНДС) <> 0
    |	ИЛИ СУММА(ДД.ДопРасходыУпрБезНДС) <> 0
    ...
    |		И НаправленияВыбытия.НалоговоеНазначение = ДД.НалоговоеНазначение
    ...
    |	НаправленияВыбытия.КорНалоговоеНазначение,
...
Функция ТекстСписанияРасходовНаВыбытиеТоваровРегистрПрочиеРасходы()
	...
	Возврат "
        ...
        |				И НЕ &ИспользуетсяУправлениеВНА_2_4
        |			ТОГДА 0
        |		ИНАЧЕ ДД.ДопРасходыРеглБезНДС КОНЕЦ) КАК СуммаРеглБезНДС,
        |	СУММА(ВЫБОР
        |		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
        |		 ИЛИ НЕ &УправленческийУчетОрганизаций
        |			ТОГДА 0
        |		ИНАЧЕ ДД.ДопРасходыУпрБезНДС КОНЕЦ) КАК СуммаУпрБезНДС,
		|	СУММА(ВЫБОР
		|		КОГДА НЕ &ИспользоватьУчетПрочихДоходовРасходовРегл ТОГДА 0
		|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
		|			ТОГДА 0
