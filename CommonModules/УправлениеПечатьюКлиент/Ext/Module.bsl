...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';uk='Немає прав для виведення друкованої форми на принтер, зверніться до адміністратора.'"));
		Возврат;
	КонецЕсли;
...
// Перед выполнением функции в веб-клиенте необходимо подключить расширение для работы с BAF.
//
// Параметры:
