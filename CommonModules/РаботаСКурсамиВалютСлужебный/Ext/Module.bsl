	...
	Для каждого ОбластьДанных Из ОбластиДляОбновления Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось установить разделение сеанса %1 по причине:
                |%2'
                |;uk='Не вдалося встановити поділ сеансу %1 з причини:
                |%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(ОбластьДанных, "ЧН=0; ЧГ=0"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые данные.Распространение курсов валют по областям данных';uk='Дані,  що поставляються. Поширення курсів валют по областях даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось обновить курсы валют в области %1 по причине:
                |%2'
                |;uk='Не вдалося оновити курси валют в області %1 з причини:
                |%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(ОбластьДанных, "ЧН=0; ЧГ=0"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые данные.Распространение курсов валют по областям данных';uk='Дані,  що поставляються. Поширення курсів валют по областях даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Пока ВыборкаВалюты.Следующий() Цикл
		...
		Пока ВыборкаОбщий.Следующий() Цикл
			...
			Если Записывать Тогда
				НаборЗаписей.Записать();
			Иначе
				Комментарий = НСтр("ru='Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.';uk='Завантаження курсу валюти %1 на дату %2 скасовано через порушення дати заборони змін.'"); 
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Комментарий, ВыборкаВалюты.Код, ВыборкаОбщий.Дата);
				ИмяСобытия = НСтр("ru='Поставляемые данные.Отмена загрузки курсов валюты';uk='Дані, що постачаються.Скасування завантаження курсів валют'",ОбщегоНазначения.КодОсновногоЯзыка());
