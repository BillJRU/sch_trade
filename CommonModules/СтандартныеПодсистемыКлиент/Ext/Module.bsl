...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("ru='Название программы: [НазваниеПрограммы] 
                       ...
                       |Версия Платформы BAF: [ВерсияПлатформы] [РазрядностьПлатформы]; 
                       ...
                       |Версія Платформи BAF: [ВерсияПлатформы] [РазрядностьПлатформы];
	...
	ТекстНедоступно = НСтр("ru='недоступно';uk='недоступно'");
...
Процедура ИнтерактивнаяОбработкаПроверкиВерсииПлатформыПриЗапуске(Параметры, Контекст) Экспорт
	...
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения =
				НСтр("ru='Рекомендуется завершить работу программы и обновить версию платформы платформы BAF.
                         ...
                         |;uk='Радимо закрити програми та оновити версію платформи BAF.
                         |Нова версія платформи містить виправлення помилок, які дозволять програмі працювати більш стабільно.
                         |Ви також можете відмовитися від оновлення і продовжити роботу на поточній версії.
                         |Мінімально необхідна версія платформи %1.'");
		Иначе
			ТекстСообщения = 
				НСтр("ru='Рекомендуется завершить работу программы и обратиться к администратору для обновления версии платформы платформы BAF.
                         |Новая версия платформы содержит исправления ошибок, которые позволят программе работать более стабильно.
                         |Вы также можете отказаться от обновления и продолжить работу на текущей версии.
                         |Минимально необходимая версия платформы %1.'
                         |;uk='Радимо закрити програму та звернутися до адміністратора для оновлення версії платформи платформи BAF.
                         |Нова версія платформи містить виправлення помилок, які дозволять програмі працювати більш стабільно.
                         |Ви також можете відмовитися від оновлення і продовжити роботу на поточній версії.
                         |Мінімально необхідна версія платформи %1.'");
		КонецЕсли;
	...
	Если ПараметрыКлиента.ВыбратьНачальныеРегиональныеНастройкиИБ = Ложь Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru='Вход в программу невозможен до установки начальных региональных настроек программы.
                       |Обратитесь к администратору за подробностями.'
                       |;uk='Вхід в програму неможливий до установки початкових регіональних настройок програми.
...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Требуется перезапуск';uk='Потрібен перезапуск'"),,
		ТекстВопроса, БиблиотекаКартинок.Предупреждение32, СтатусОповещенияПользователя.Важное);
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Перезапустить", НСтр("ru='Перезапустить';uk='Запустити знову'"));
	КнопкиВопроса.Добавить("НапомнитьПозже", НСтр("ru='Напомнить позже';uk='Нагадати пізніше'"));
	КнопкиВопроса.Добавить("НеСегодня", НСтр("ru='Не напоминать сегодня';uk='Не нагадувати сьогодні'"));
...
Функция ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры)
	...
	Иначе
		...
		Если Параметры.ОповещениеЗавершения = Неопределено Тогда
			...
			// как обработчика события, до открытия главного окна платформы BAF 8.
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Рекомендуется увеличить объем памяти до %1 Гб.';uk='Рекомендується збільшити обсяг пам''яті до %1 Гб.'"), РекомендуемыйОбъем);
