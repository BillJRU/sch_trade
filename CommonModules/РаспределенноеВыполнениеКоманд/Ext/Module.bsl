...
Процедура ВыполнитьКомандуДополнительнойОбработки(ИдентификаторОбработки, ИдентификаторКоманды, ИдентификаторОперации, СообщитьМенеджеру = Ложь) Экспорт
	...
	Событие = Событие + НСтр("ru='Начало выполнения команды';uk='Початок виконання команди'",ОбщегоНазначения.КодОсновногоЯзыка()); 
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			Событие = ИмяСобытияЖурналаРегистрации() + ".";
			Событие = Событие + НСтр("ru='Завершение выполнения команды';uk='Завершення виконання команди'",ОбщегоНазначения.КодОсновногоЯзыка()); 
		...
		Исключение
			...
			Событие = НСтр("ru='Ошибка выполнения команды';uk='Помилка виконання команди'",ОбщегоНазначения.КодОсновногоЯзыка()); 
	...
	Иначе
		ОшибкаВыполнения = Истина;
		Событие = НСтр("ru='Обработка не найдена';uk='Обробку не знайдено'",ОбщегоНазначения.КодОсновногоЯзыка()); 
...
Функция ВыполнитьПередачуФайлаПриложению(ИмяФайла, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ Указатель.Существует() Тогда
		Комментарий = НСтр("ru='Файл с указанным именем не найден на файловой системе.';uk='Файл із зазначеним ім''ям не знайдений на файловій системі.'",КодЯзыка); 
	...
	Если НЕ ЗначениеЗаполнено(КодПолучателя) И НЕ ТипЗнч(КодПолучателя) = Тип("Число") Тогда
		Комментарий = НСтр("ru='Параметр КодПолучателя не содержит валидный код области данных.';uk='Параметр КодПолучателя не містить валідний код області даних.'",КодЯзыка); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Процедура ВыслатьКвитанциюПередачиФайла(ИдентификаторВызова, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторВызова) ИЛИ НЕ ТипЗнч(ИдентификаторВызова) = Тип("УникальныйИдентификатор") Тогда
		Комментарий = НСтр("ru='Параметр ИдентификаторВызова не содержит валидный идентификатор.';uk='Параметр ИдентификаторВызова не містить валідний ідентифікатор.'",КодЯзыка); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Распределенное выполнение команд';uk='Розподілене виконання команд'",ОбщегоНазначения.КодОсновногоЯзыка()); 
