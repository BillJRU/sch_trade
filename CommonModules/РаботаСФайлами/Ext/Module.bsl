	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Двоичные данные файла отсутствуют в регистре ДвоичныеДанныеФайлов.
                       |Файл: ""%1"".'
                       |;uk='Двійкові дані файлу відсутні в регістрі ДвоичныеДанныеФайлов.
                       |Файл: ""%1"".'"),
			ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл));
		ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Открытие файла';uk='Файли.Відкриття файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ДобавитьФайлСДиска(ВладелецФайлов, ПутьКФайлуНаДиске) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.ДобавитьФайлСДиска.';uk='Не задано значення параметра ВладелецФайлов в РаботаСФайлами.ДобавитьФайлСДиска.'");
	КонецЕсли;
...
Функция ДобавитьФайл(ПараметрыФайла,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ПараметрыФайла.ВладелецФайлов в РаботаСФайлами.ДобавитьФайл.';uk='Не задано значення параметра ПараметрыФайла.ВладелецФайлов в РаботаСФайлами.ДобавитьФайл.'");
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Добавление присоединенного файла';uk='Файли.Додавання приєднаного файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Функция СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.СконвертироватьФайлыВПрисоединенные.';uk='Не задано значення параметра ВладелецФайлов в РаботаСФайлами.СконвертироватьФайлыВПрисоединенные.'");
	КонецЕсли;
	
	Результат = Новый Соответствие;
	ЗаголовокОшибки = НСтр("ru='Ошибка при конвертации присоединенных файлов.';uk='Помилка при конвертації приєднаних файлів.'");
	ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(
		ВладелецФайлов, ИмяСправочника, ЗаголовокОшибки);
...
Процедура ИзменитьСправочникХраненияФайлов(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.ИзменитьСправочникХраненияФайлов.';uk='Не задано значення параметра ВладелецФайлов в РаботаСФайлами.ИзменитьСправочникХраненияФайлов.'");
	КонецЕсли;
...
Функция ГиперссылкаФайлов() Экспорт
	...
	ПараметрыГиперссылки.Вставить("Заголовок",                 НСтр("ru='Файлы';uk='Файли'"));
...
Функция ПолеФайла() Экспорт
	...
	ПараметрыПоля.Вставить("ТекстНевыбраннойКартинки",  НСтр("ru='Добавить изображение';uk='Додати зображення'"));
	...
	ПараметрыПоля.Вставить("ФильтрДиалогаВыбора",       НСтр("ru='Все файлы (*.*)|*.*';uk='Всі файли (*.*)|*.*'"));
...
Процедура ВыполнитьДействияПередЗаписьюПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(Источник.ВладелецФайла) Тогда
		...
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Ошибка записи файла при обновлении ИБ';uk='Файли.Помилка запису файлу при оновленні ІБ'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				Источник.Ссылка,
				ОписаниеОшибки);
...
Процедура СоздатьГиперссылкуФайлов(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыГиперссылки)
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы
		...
		ПодменюДобавления.Заголовок   = НСтр("ru='Присоединить файлы';uk='Приєднати файли'");
		ПодменюДобавления.Подсказка   = НСтр("ru='Присоединить файлы';uk='Приєднати файли'");
		...
		ЗагрузитьФайл.Подсказка = НСтр("ru='Загрузить файл с диска';uk='Завантажити файл з диска'");
		ЗагрузитьФайл.Заголовок = НСтр("ru='Загрузить...';uk='Завантажити...'");
		...
		СоздатьПоШаблону.Заголовок = НСтр("ru='Создать по шаблону...';uk='Створити з шаблону...'");
		...
		Сканировать.Заголовок = НСтр("ru='Сканировать...';uk='Сканувати...'");
...
Процедура СоздатьПолеФайла(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыПоляФайла)
	...
	СвойстваКнопкиЗагрузка.Вставить("Заголовок",            НСтр("ru='Загрузить...';uk='Завантажити...'"));
	...
	СвойстваКнопкиВыбор.Вставить("Заголовок",            НСтр("ru='Выбрать из присоединенных...';uk='Вибрати з приєднаних ...'"));
		...
		Если ЗначениеЗаполнено(РеквизитРазмещения)
			...
			Если ДанныеФайла <> Неопределено Тогда
				...
				Если ЗначениеДвоичныхДанных = Неопределено Тогда
					...
					ЭлементПредпросмотра.ТекстНевыбраннойКартинки = НСтр("ru='Изображение отсутствует';uk='Зображення відсутнє'");
				КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ВыводитьЗаголовокФайла Тогда
		...
		Если Не ЗначениеЗаполнено(РеквизитРазмещения) Тогда
			...
			ГиперссылкаЗаголовка.Заголовок = НСтр("ru='загрузить';uk='завантажити'");
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(РеквизитРазмещения)) Тогда
			ЗаголовокФайла.Подсказка       = НСтр("ru='Открыть файл';uk='Відкрити файл'");
	...
	Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
		...
		ПодменюДобавления.Заголовок   = НСтр("ru='Заменить';uk='Замінити'");
		ПодменюДобавления.Отображение = ОтображениеКнопки.Картинка;
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы Тогда
		...
		Если Не ДобавляемыйЭлемент.ТолькоОдинФайл Тогда
			...
			Сканировать = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиСканировать_" + НомерЭлемента);
			Сканировать.Заголовок = ?(ОбщегоНазначения.ЭтоМобильныйКлиент(), НСтр("ru='Сфотографировать';uk='Сфотографувати'"), НСтр("ru='Сканировать';uk='Сканувати'")) + "...";
			ЗаполнитьЗначенияСвойств(Сканировать, СвойстваКомандыФормы);
	...
	Если ДобавляемыйЭлемент.ВыбиратьФайл
		...
		ВыбратьФайл.Подсказка = НСтр("ru='Выбрать файл из присоединенных';uk='Вибрати файл з приєднаних'");
	...
	Если ДобавляемыйЭлемент.ПросматриватьФайл Тогда
		...
		ПросмотретьФайл.Заголовок = НСтр("ru='Просмотреть';uk='Переглянути'");
	...
	Если ДобавляемыйЭлемент.ОчищатьФайл
		...
		ОчиститьФайл.Заголовок = НСтр("ru='Очистить';uk='Очистити'");
	...
	Если ДобавляемыйЭлемент.РедактироватьФайл = "ВФорме" Тогда
		...
		РедактироватьФайл.Заголовок = НСтр("ru='Открыть карточку';uk='Відкрити картку'");
		РедактироватьФайл.Подсказка = НСтр("ru='Открыть карточку присоединенного файла';uk='Відкрити картку приєднаного файлу'");
	...
	ИначеЕсли ДобавляемыйЭлемент.РедактироватьФайл = "Непосредственно"
		...
		РедактироватьФайл.Заголовок = НСтр("ru='Редактировать';uk='Редагувати'");
		РедактироватьФайл.Подсказка = НСтр("ru='Открыть файл для редактирования';uk='Відкрити файл для редагування'");
		ЗаполнитьЗначенияСвойств(РедактироватьФайл, СвойстваКомандыФормыКартинка);
		...
		ПоместитьФайл.Заголовок = НСтр("ru='Закончить редактирование';uk='Закінчити редагування'");
		ПоместитьФайл.Подсказка = НСтр("ru='Сохранить и освободить файл в информационной базе';uk='Зберегти й звільнити файл в інформаційній базі'");
		...
		ОтменитьРедактирование.Заголовок = НСтр("ru='Отменить редактирование';uk='Скасувати редагування'");
		ОтменитьРедактирование.Подсказка = НСтр("ru='Освободить занятый файл';uk='Звільнити зайнятий файл'");
