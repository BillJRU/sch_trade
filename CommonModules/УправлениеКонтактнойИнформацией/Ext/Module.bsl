...
//                    XDTO пакетов КонтактнаяИнформация или Адрес.
...
Функция ДоменАдресаКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	ВызватьИсключение НСтр("ru='Невозможно определить домен, ожидается электронная почта или веб-ссылка.';uk='Неможливо визначити домен, очікується електронна пошта або вебпосилання.'");
КонецФункции
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Найти("КонтактнаяИнформация") = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не содержит контактную информацию.';uk='%1 не містить контактну інформацію.'"), МетаданныеОбъекта.Имя);
		КонецЕсли;
...
Функция ДанныеСтраныМира(Знач КодСтраны = Неопределено, Знач Наименование = Неопределено) Экспорт
	...
	|	МеждународноеНаименование, КодАльфа2, КодАльфа3
	|ИЗ
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			ГруппаПодменю.Заголовок = НСтр("ru='Контактная информация';uk='Контактна інформація'");
...
Функция ПараметрыИзВидаКонтактнойИнформации(Знач ВидКонтактнойИнформации)
	...
	Если РезультатЗапроса.Количество() = 0 Тогда
		ШаблонТекстаОшибки = НСтр("ru='При получении свойств контактной информации был передан неверный вид контактной информации. %1';uk='При отриманні властивостей контактної інформації був переданий невірний вид контактної інформації.  %1'");
