...
Функция ТекстОшибкиСервисаСертификатНедействителен() Экспорт
	...
	Возврат НСтр("ru='Облачный сервис сообщил, что сертификат недействителен.';uk='Хмарний сервіс повідомив, що сертифікат недійсний.'");
...
Функция ТекстОшибкиСервисаПодписьНедействительна() Экспорт
	...
	Возврат НСтр("ru='Облачный сервис сообщил, что подпись недействительна.';uk='Хмарний сервіс повідомив, що підпис недійсний.'");
...
Функция ПредставлениеСертификата(Сертификат, ДобавкаВремени, МодульЭлектроннаяПодписьЛокализацияКлиентСервер) Экспорт
	...
	Если ПустаяСтрока(Представление) Тогда
		ДатыСертификата = ДатыСертификата(Сертификат, ДобавкаВремени);
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, до %2';uk='%1, до %2'"),
...
Функция ПараметрыКомпонентыCMSSign(ПараметрыCMS, ОписаниеДанных) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный параметр ТипПодписи метода CMSSign компоненты XMLDSig.';uk='Некоректний параметр ТипПодписи методу CMSSign компоненти XMLDSig.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный параметр ТипВходныхДанных метода CMSSign компоненты XMLDSig.';uk='Некоректний параметр ТипВходныхДанных методу CMSSign компоненти XMLDSig.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный параметр Открепленная метода CMSSign компоненты XMLDSig.';uk='Некоректний параметр Открепленная методу CMSSign компоненти XMLDSig.'");
	КонецЕсли;
...
Функция ДиагностическаяИнформацияОКомпьютере() Экспорт
	...
	Возврат НСтр("ru='Операционная система:';uk='Операційна система:'") + СисИнфо.ВерсияОС
		+ Символы.ПС + НСтр("ru='Версия приложения:';uk='Версія прикладної програми:'") + СисИнфо.ВерсияПриложения
		+ Символы.ПС + НСтр("ru='Тип платформы:';uk='Тип платформи:'") + СисИнфо.ТипПлатформы;
