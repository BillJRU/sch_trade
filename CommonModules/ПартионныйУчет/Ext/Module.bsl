...
//
Процедура РассчитатьВсе(Знач ОкончаниеПериодаРасчета, Организация = Неопределено, ВыполняетсяЗакрытиеМесяца = Ложь) Экспорт
	...
	Если СхемаРасчета.Количество() = 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Партионный учет.Расчет не требуется';uk='Партіонний облік.Розрахунок не вимагається'",КодЯзыка),
			УровеньИнформация,
			,
			ОкончаниеПериодаРасчета,
			Комментарий);
	...
	Если НеобходимРасчет Тогда
		...
		Комментарий = НСтр("ru='Выполняется расчет партий версии 2.1 по %ОкончаниеПериода%';uk='Виконується розрахунок партій версії 2.1 по %ОкончаниеПериода%'",КодЯзыка);
		Комментарий = СтрЗаменить(Комментарий, "%ОкончаниеПериода%", Формат(ОкончаниеПериодаРасчета, ФорматПериода));
		ЗаписьЖурналаРегистрации(
...
Процедура РассчитатьПартииТоваров(НачалоПериода, ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)
	...
	Комментарий = НСтр("ru='Рассчитано %КолвоСтрок% стр. за %Замер% сек.';uk='Розраховано %КолвоСтрок% ряд. за %Замер% сек.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет.ПартииТоваровОрганизаций';uk='Партіонний облік.ПартииТоваровОрганизаций'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура РассчитатьПартииПроизводства(НачалоПериода, ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)
	...
	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет.ПартииПроизводственныхЗатрат';uk='Партіонний облік.ПартииПроизводственныхЗатрат'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,, Комментарий);
КонецПроцедуры
...
Процедура РассчитатьПартииЗатрат(НачалоПериода, ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		Выпуски 						= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ДанныеДляПартийЗатрат, "Выпуски");
		ВыпускиПродукции 				= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ДанныеДляПартийЗатрат, "ВыпускиПродукции"); 	        
		МатериалыИРаботыВПроизводстве	= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ДанныеДляПартийЗатрат, "МатериалыИРаботыВПроизводстве");
		Данные 							= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ДанныеДляПартийЗатрат, "Данные"); 	                    
	    ДолиСтоимости                   = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ДанныеДляПартийЗатрат, "ДолиСтоимости"); 	            
	КонецЕсли;
	...
	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет.ПартииЗатратНаВыпуск';uk='Партіонний облік.ПартииЗатратНаВыпуск'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,, Комментарий);
КонецПроцедуры
...
// Область РасчетПриходыРасходовСебестоимость
	...
	РасчетныеПриходыПрочихРасходов = РасчетныеПриходыПрочихРасходов(НачалоПериода, РасчетныеПриходыПартийРасходов, Неопределено);
	...
	ЗаписатьРасчетныеПартии(РегистрыНакопления.ПрочиеРасходы, РасчетныеПриходыПрочихРасходов, Неопределено);
...
Процедура РассчитатьПартииРасходов(НачалоПериода, ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)
	...
	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет.ПартииРасходовНаСебестоимостьТоваров';uk='Партіонний облік.ПартииРасходовНаСебестоимостьТоваров'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,, Комментарий);
КонецПроцедуры
...
// Расчет записей регистров при корректировках налогового назначения НДС
Процедура РассчитатьЗаписиКорректировокНДС(НачалоПериода, ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)		
	
	НачалоЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	// Выборка данных по восстановлению НДС и условным продажам
	ДанныеДляКорректировокНДС = ДанныеДляКорректировокНДС(НачалоПериода, ОкончаниеПериода, МассивОрганизаций);
    
    // Выборка устаревших регистраторов по регистру КорректировкиНДСПартий
	...
	// Дополнение ДанныеДляКорректировокНДС движениями организаций, по которым расчет сейчас не выполняется
	...
	// Фильтрация данных по восстановлению НДС
	РасчетныеНДСВосстановлениеНДС = РасчетныеНДСВосстановлениеНДС(ДанныеДляКорректировокНДС);
	// Фильтрация данных по условным продажам
	РасчетныеНДСУсловнаяПродажа = РасчетныеНДСУсловнаяПродажа(ДанныеДляКорректировокНДС);
	// Расчет данных по корректировкам НДС
	РасчетныеКорректировкиНДСПартий = РасчетныеКорректировкиНДСПартий(ДанныеДляКорректировокНДС);
	// Запись данных по корректировкам НДС в регистр КорректировкиНДСПартий
	ЗаписатьРасчетныеПартии(РегистрыНакопления.КорректировкиНДСПартий, РасчетныеКорректировкиНДСПартий, Неопределено);
    
	// Очистка движений устаревших регистраторов по регистру КорректировкиНДСПартий
	...
	// Дополнение РасчетныеНДСУсловныеПродажи движениями организаций, по которым расчет сейчас не выполняется
    ...
    РасчетныеНДСУсловныеПродажи = РаспределитьУсловныеПродажиПоНомерамГТД(РасчетныеНДСУсловныеПродажи);
    // Запись данных по регистру НДСУсловныеПродажи 
	ЗаписатьРасчетныеПартии(РегистрыНакопления.НДСУсловныеПродажи, РасчетныеНДСУсловныеПродажи, Неопределено);
    
	// Очистка движений устаревших регистраторов по регистру НДСУсловныеПродажи
	...
	КоличествоЗаписей = ДанныеДляКорректировокНДС.Количество();	
	
	ДанныеДляКорректировокНДС     = Неопределено;	
	...
	Замер = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЗамера;
	Комментарий = НСтр("ru='Рассчитано %КолвоСтрок% стр. за %Замер% сек.';uk='Розраховано %КолвоСтрок% ряд. за %Замер% сек.'",ОбщегоНазначения.КодОсновногоЯзыка());
	Комментарий = СтрЗаменить(Комментарий, "%КолвоСтрок%", КоличествоЗаписей);
	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет.РассчитатьЗаписиКорректировокНДС';uk='Партійний облік.РассчитатьЗаписиКорректировокНДС'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура СформироватьДвиженияПоНДС(ОкончаниеПериода, МассивОрганизаций, КоличествоЗаписей = 0)
	...
	// Корректировки использования НДС
	РассчитатьЗаписиКорректировокНДС(НачалоМесяца(ОкончаниеПериода), КонецМесяца(ОкончаниеПериода), МассивОрганизаций, КоличествоЗаписей);
...
Функция ДанныеДляПартийТоваров(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		ИсходныеДанные = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(МВТ, "ИсходныеДанные");
	КонецЕсли;
...
Функция РасчетныеПартииПрочих(ДанныеДляРасчета, Регистраторы, УзлыДвижений = Неопределено, Тест = Ложь, КоличествоЗаписей = 0) // ДанныеДляРасчета будут изменены
	...
	// Шаг 1: Рассчитываем базу распределения для каждого прихода прочих расходов с учетом налогового назначения партии распределения.
	УзлыДвижений = Новый Соответствие; // [Организация, [АналитикаРасходов, [ИндексСтроки, [НалоговоеНазначение, Базис]]]]
	...
	Для Каждого Движение Из ДанныеДляРасчета Цикл
		...
		Ключи[3] = Движение.НалоговоеНазначениеСписания;
	...
	Для Индекс = МинимальныйИндекс По 0 Цикл
		...
		КонецЕсли;
	КонецЦикла;
	// Шаг 5: Копируем рассчитанные партии с указанием налогового назначения в ДанныеДляРасчета
...
Функция ТекстОстаткиПартийСборок() // вт ОстаткиЗатратПоСборкам, ОстаткиПартийСборок
	...
	Возврат "
		...
		|	ДД.ДокументПоступления,
		|	ДД.ВидЗапасов,
        |	СпрАналитикиУчетаПартий.НалоговоеНазначение КАК НалоговоеНазначение,
		|	ДД.КоличествоОстаток КАК Количество
		|ПОМЕСТИТЬ
		|	ОстаткиПартийСборок
		|ИЗ
...
Функция ТекстПриходыПартийСборок() // вт ПриходыПартийСборок
	...
	Возврат "
		...
		|	ДД.АналитикаУчетаНоменклатуры,
		|	ДД.ДокументПоступления,
        |	СпрАналитикиУчетаПартий.НалоговоеНазначение,
		|	ДД.ВидЗапасов
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	АналитикаУчетаНоменклатуры,
...
Функция ТекстБазисОстатки() // использует АналитикиРасходовВсе, ОстаткиСебестоимости
	Возврат "
		...
		|	ДД.ВременнаяРазницаОстаток КАК ВременнаяРазница,
        |	ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка) КАК НалоговоеНазначениеПартии,
        |	ЕСТЬNULL(АналитикиПартий.НалоговоеНазначение,
        |		ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) КАК НалоговоеНазначение,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеРасходов,
		|	НЕОПРЕДЕЛЕНО КАК ДокументРеализации,
...
Функция РасчетныеПриходыПартийРасходов(ДанныеДляРасчета, Регистраторы, УзлыДвижений = Неопределено, Тест = Ложь) // ДанныеДляРасчета будут изменены
    ...
    ДанныеДляРасчета.Колонки.НалоговоеНазначениеСписания.Имя = "НалоговоеНазначение"; 
...
Функция РасчетныеПриходыПрочихРасходов(НачалоПериода, РасчетныеПриходыПартийРасходов, ИндексРегистраторов, Тест = Ложь)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Движение.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая Тогда
...
Функция ТекстИсходныеДвиженияСебестоимость() // использует &Регистраторы
	Возврат "
		...
		|	ДД.АналитикаАктивовПассивов,
		|	ДД.ДокументДвижения,
		|	ДД.НалоговоеНазначение21 КАК НалоговоеНазначение,
...
Функция ДанныеДляКорректировокНДС(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)			
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		ТекстПартииДляРасчетаКорректировокНДС  = ТекстПартииДляРасчетаКорректировокНДС() + " ; " + "ВЫБРАТЬ * ИЗ Партии";
		ЗапросПартииДляРасчетаКорректировокНДС = Новый Запрос(ТекстПартииДляРасчетаКорректировокНДС);
		ЗапросПартииДляРасчетаКорректировокНДС.УстановитьПараметр("НачалоПериода", НачалоПериода);
		ЗапросПартииДляРасчетаКорректировокНДС.УстановитьПараметр("ОкончаниеПериода", ОкончаниеПериода);
		ЗапросПартииДляРасчетаКорректировокНДС.УстановитьПараметр("МассивОрганизаций", МассивОрганизаций);	
		ЗапросПартииДляРасчетаКорректировокНДС.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		ДанныеДляРасчетаКорректировокНДС = ЗапросПартииДляРасчетаКорректировокНДС.Выполнить().Выгрузить();
	КонецЕсли;
...
Функция РасчетныеКорректировкиНДСПартий(ДанныеДляРасчета, ВнутренниеДанные = Неопределено, Тест = Ложь)
	РасчетныеПартии = ДанныеДляРасчета.Скопировать();
	РасчетныеПартии.Сортировать("Регистратор", Новый СравнениеЗначений);
	Возврат РасчетныеПартии;
КонецФункции
...
Функция ТекстПартииДляРасчетаКорректировокНДС()	
	...
	Возврат "
		...
		|			(ЕСТЬNULL(АналитикаПродукции.Номенклатура.СтавкаНДС.Ставка, 0) / 100)
		...
		|				  	ИЛИ Партии.ДокументПоступленияРасходов ССЫЛКА Документ.ТаможеннаяДекларацияИмпорт
		|				  ) 
		|				ТОГДА
		|					ВЫБОР 
		|						КОГДА ЕСТЬNULL(АналитикаПартий.СтавкаНДС.Ставка, 0) = 0 
		|							ТОГДА 0 
		|						ИНАЧЕ Партии.НДСРегл * 100 / ЕСТЬNULL(АналитикаПартий.СтавкаНДС.Ставка, 0)
		|                   КОНЕЦ
		...
		|		Партии.СтатьяРасходов 										КАК СтатьяРасходовСписания,
		|		Партии.АналитикаРасходов 									КАК АналитикаРасходов,
		...
		|		Партии.НалоговоеНазначениеСписания 							КАК НалоговоеНазначениеПоФакту
		|
		|	ИЗ
		...
		|		И Партии.НалоговоеНазначениеСписания <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
		|		И Партии.НалоговоеНазначениеСписания <> Партии.АналитикаУчетаПартий.НалоговоеНазначение
		|
		|	) КАК Партии
		|
		|СГРУППИРОВАТЬ ПО
...
Функция ТекстКорректировкиНДСУсловнаяПродажа()	
	...
	Возврат "
		...
		|   Партии.НДСРегл <> 0.
		|	И (
		|		(Партии.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
		|			И Партии.НалоговоеНазначениеПоФакту.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
		|		)
		|		ИЛИ (Партии.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
		|			И Партии.НалоговоеНазначениеПоФакту = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность)
		|		)
		|   )
		|";
...
Функция ТаблицаНДСУсловныеПродажи(ТаблицаНДСУсловнаяПродажа, ТаблицаНДСВосстановлениеНДС)
    ...
    ТаблицаНДСУсловныеПродажи.Колонки.Добавить("АналитикаУчетаНоменклатуры", Новый ОписаниеТипов("СправочникСсылка.КлючиАналитикиУчетаНоменклатуры"));
    ...
    Для Каждого СтрокаУсловнаяПродажа Из ТаблицаНДСУсловнаяПродажа Цикл
		...
		Если ТипЗнч(СтрокаУсловнаяПродажа.АналитикаУчетаНоменклатуры) = Тип("СправочникСсылка.КлючиАналитикиУчетаНоменклатуры") Тогда
            ...
            НоваяСтрока.АналитикаУчетаНоменклатуры 	= СтрокаУсловнаяПродажа.АналитикаУчетаНоменклатуры;
    ...
    Для Каждого СтрокаВосстановлениеНДС Из ТаблицаНДСВосстановлениеНДС Цикл
		...
		Если ТипЗнч(СтрокаВосстановлениеНДС.АналитикаУчетаНоменклатуры) = Тип("СправочникСсылка.КлючиАналитикиУчетаНоменклатуры") Тогда
            ...
            НоваяСтрока.АналитикаУчетаНоменклатуры 	= СтрокаВосстановлениеНДС.АналитикаУчетаНоменклатуры;
...
Процедура ПроверитьВозможностьФормированияОнлайнДвиженийКорректировокНДСПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	Если НЕ НаборЗаписей.ДополнительныеСвойства.Свойство(ИмяДополнительногоСвойстваЗаписываемогоНабора()) 
		И НЕ (НаборЗаписей.ДополнительныеСвойства.Свойство("РасчетПартийИСебестоимости") 
			И НаборЗаписей.ДополнительныеСвойства.РасчетПартийИСебестоимости
		)
		Тогда
...
Функция РаспределитьУсловныеПродажиПоНомерамГТД(РасчетныеНДСУсловныеПродажи) Экспорт
	...
	ЗапросВидыЗапасов.УстановитьПараметр("Данные", РасчетныеНДСУсловныеПродажи);
	
	ТаблицаВидовЗапасов = ЗапросВидыЗапасов.Выполнить().Выгрузить();
	ТаблицаВидовЗапасов.Индексы.Добавить("Регистратор, АналитикаУчетаНоменклатуры");
	...
	|	Данные.УдалитьСтавкаНДС КАК УдалитьСтавкаНДС,
	|   Данные.РасчетПартий КАК РасчетПартий,
	|   Данные.РасчетСебестоимости КАК РасчетСебестоимости,
	|   Данные.РасчетНеЗавершен КАК РасчетНеЗавершен,
    |
	|	Данные.Количество КАК Количество,
	|	Данные.СтоимостьРегл КАК СтоимостьРегл,
	|	Данные.НДСРегл КАК НДСРегл,
	|	Данные.НДСРеглРучныеКорректировки КАК НДСРеглРучныеКорректировки,
	|	Данные.НДСУпр КАК НДСУпр,
    |	Данные.НДСУпрРучныеКорректировки КАК НДСУпрРучныеКорректировки
	|
	|ПОМЕСТИТЬ втДанные
	|ИЗ 
	|	&Данные КАК Данные 
	...
	|	Данные.УдалитьСтавкаНДС,
	|   Данные.РасчетПартий,
	|   Данные.РасчетСебестоимости,
	|   Данные.РасчетНеЗавершен,
	|
	...
	ЗапросТаблицаДанные.УстановитьПараметр("Данные", РасчетныеНДСУсловныеПродажи);
	
	ТаблицаДанные = ЗапросТаблицаДанные.Выполнить().Выгрузить();
	ТаблицаНовыеДанные = ТаблицаДанные.СкопироватьКолонки(); 

	ПараметрыОтбораВидовЗапасов = Новый Структура("Регистратор, АналитикаУчетаНоменклатуры"); 
	
	Для Каждого СтрокаДанные Из ТаблицаДанные Цикл
		
		ПараметрыОтбораВидовЗапасов.Регистратор                = СтрокаДанные.Регистратор;  
		ПараметрыОтбораВидовЗапасов.АналитикаУчетаНоменклатуры = СтрокаДанные.АналитикаУчетаНоменклатуры;
...
Процедура ЗавершитьКорректировкуОстатковРегистра(ПараметрыРасчета, ПараметрыКорректировки) Экспорт
	...
	Исключение
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ЗарегистрироватьПроблемуВыполненияРасчета(
			ПараметрыРасчета,
			,
			НСтр("ru='При записи движений диагностированы ошибки';uk='При запису рухів діагностовано помилки'",ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола);
...
Функция ТекстЗапросаДвиженияПоСебестоимостиТоваровСформированныеВВерсииПУ22() Экспорт
	Возврат
		...
		|		ИЛИ ДД.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
		|";
КонецФункции
...
Функция ТекстЗапросаВключениеУчетаСебестоимостиПоВидамЗапасовПУ21()
	Возврат "
		...
		// Выбираем движения с ненулевым количеством, что бы исключить оффлайные движения
		...
		// Выбираем движения документов "Корректировка приобретения"
		...
		Иначе
			Если РасчетнаяПартия.ТипЗаписи = "Перемещение"
			 И ТипЗнч(РасчетнаяПартия.Регистратор) = Тип("ДокументСсылка.ПересортицаТоваров") Тогда
			  	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Приход.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая Тогда
...
Процедура ЗаполнитьПартиюПрочих(РасчетнаяПартия, Расход, Приход) // ПартииПрочихРасходов
	...
	// Заполняем партионную идентификацию в расчетной партии
	Если ТипЗнч(Приход.Ключ) = Тип("СправочникСсылка.НалоговыеНазначенияАктивовИЗатрат") Тогда
		РасчетнаяПартия.НалоговоеНазначениеСписания = Приход.Ключ;
	Иначе
...
Процедура ЗаписатьРасчетныеПартии(МенеджерРегистра, РасчетныеПартии, Регистраторы)
	...
	КолонкаНалоговоеНазначениеПереименована = Ложь;
	Если МетаданныеРегистра.Имя = "СебестоимостьТоваров" Тогда
		Если РасчетныеПартии.Колонки.Найти("НалоговоеНазначение") <> Неопределено 
			И РасчетныеПартии.Колонки.Найти("НалоговоеНазначение21") = Неопределено Тогда
		
			РасчетныеПартии.Колонки.НалоговоеНазначение.Имя = "НалоговоеНазначение21"; 
		    КолонкаНалоговоеНазначениеПереименована = Истина;
	...
	Если КолонкаНалоговоеНазначениеПереименована Тогда
		РасчетныеПартии.Колонки.НалоговоеНазначение21.Имя = "НалоговоеНазначение"; 
	КонецЕсли;
...
Функция ЕстьФИФОСкользящая(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)
	...
	НастройкиНалоговУчетныхПолитик.ДополнитьМенеджерВременныхТаблицДействующимиПараметрамиНалоговУчетныхПолитик("УчетнаяПолитикаФинансовогоУчета",
...
Процедура ОптимизироватьНумерациюВЦепочках(МенеджерВременныхТаблиц)
	...
	ЕстьПриоритетВДанных         = (КолонкиТаблицыДанных.Найти("Приоритет") <> Неопределено);
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Запрос.Текст,
		?(ЕстьПриоритетВДанных, "ЕСТЬNULL(Т.Приоритет, 0)", "0"),
	...
	ПараметрыНумерации = РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьПараметрыНумерацииСтрокВременнойТаблицы(
		...
		"", // ресурсы
		"Приоритет, ПериодПоступления, Период, Регистратор", // порядок
		"Порядок", // номер
		"Приоритет, ПериодПоступления, Период, Регистратор", // индекс
		"", // накопление
	...
	|		ПО Т.Приоритет = ПоляУпорядочивания.Приоритет 
	|		 И Т.ПериодПоступления = ПоляУпорядочивания.ПериодПоступления
...
Процедура РассчитатьПартииПоЦепочкам(ОписаниеДвижений, ДанныеДляРасчета, РасчетныеПартии, Регистраторы, Тест)
	...
	Пока ИндексСтроки <= КоличествоЗаписей Цикл
		...
		Если ИндексСтроки > 0 И ИндексСтроки % 10000 = 0 Тогда
			...
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Рассчитано: %1 стр., %2 связей';uk='Розраховано: %1 стр., %2 зв''язків'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				ИндексСтроки,
				КоличествоДуг);
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Партионный учет.';uk='Партіонний облік.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + ОписаниеДвижений.Контекст,
				УровеньЖурналаРегистрации.Информация,,, Комментарий);
		КонецЕсли;
	КонецЦикла;
...
Функция АналитикаУчетаПартий(СтараяАналитика, НалоговоеНазначениеПартии, НалоговоеНазначение)
	
	Если ЗначениеЗаполнено(СтараяАналитика) И ТипЗнч(СтараяАналитика) = Тип("СправочникСсылка.КлючиАналитикиУчетаПартий")
	 И ЗначениеЗаполнено(НалоговоеНазначениеПартии) И ЗначениеЗаполнено(НалоговоеНазначение)
	 И НалоговоеНазначениеПартии <> НалоговоеНазначение Тогда
		...
		РеквизитыНовойАналитики.Вставить("НалоговоеНазначение", НалоговоеНазначение); // новое налоговое назаначение
...
Функция АналитикаУчетаПартийПоНалоговомуНазначению(Знач НалоговоеНазначениеПартии)
	...
	Если ЗначениеЗаполнено(НалоговоеНазначениеПартии) Тогда
		...
		СтруктураАналитикУчетаПартий.Вставить("Дата", Дата(1, 1, 1)); // для того, чтобы режим ПУ точно определился как "не равен ПУ 2.2"
	 
		АналитикаУчетаПартий = Справочники.КлючиАналитикиУчетаПартий.ПолучитьКлючАналитики(СтруктураАналитикУчетаПартий);
