...
//   * СписокОшибок - Структура - см. ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю
//
...
Процедура ЗаполнитьСостоянияОрдера(СписокВыбора, ТолькоАктивные = Ложь) Экспорт
	...
	Иначе
		...
		СписокВыбора.Добавить("1", НСтр("ru='Не оформлены';uk='Не оформлені'"),               Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("3", НСтр("ru='Не соответствуют накладным';uk='Не відповідають накладним'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
		СписокВыбора.Добавить("4", НСтр("ru='Не используются';uk='Не використовуються'"),            Ложь, БиблиотекаКартинок.ПустаяКартинка);
	КонецЕсли;
	...
	Для каждого Группировка Из ГруппировкиРаспоряжений Цикл
		...
		Для каждого Распоряжения Из Группировка.МассивЗаказов Цикл
			...
			Если Не Ошибка Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(
						НСтр("ru='Ошибка при проведении документа';uk='Помилка при проведенні документа'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			...
			Если Ошибка Тогда
				...
				ТекстОшибки = НСтр("ru='Не удалось провести документ - %1';uk='Не вдалося провести документ - %1'");
...
Функция ТекстЗапросаСостояний(ИмяТаблицыСостояний, ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению = Ложь) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка формирования текста запроса состояний: некорректное имя таблицы состояний.';uk='Помилка формування тексту запиту станів: некоректне ім''я таблиці станів.'");;
	КонецЕсли;
...
Функция ПроверитьВозможностьВводаНаОсновании(Документ, Статус, ЕстьОшибкиПроведен, ЕстьОшибкиСтатус, МассивДопустимыхСтатусов) Экспорт
	...
	ИначеЕсли ЕстьОшибкиСтатус Тогда
		...
		Если МассивДопустимыхСтатусов.Количество() = 1 Тогда
			...
			Иначе
				ТекстОшибки = НСтр("ru='Ввод на основании разрешен только в статусе %СтрокаДопустимыхСтатусов%.';uk='Введення на підставі дозволений тільки в статусі %СтрокаДопустимыхСтатусов%.'");
			КонецЕсли;
		Иначе
			Если Статус <> Неопределено Тогда
			...
			Иначе
				ТекстОшибки = НСтр("ru='Ввод на основании разрешен только в статусах %СтрокаДопустимыхСтатусов%.';uk='Введення на підставі дозволений тільки в статусах %СтрокаДопустимыхСтатусов%.'");
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаГруппировки(
			КлючевыеПоля, ИмяИсточника, ИмяВременнойТаблицы, ИмяПараметра, КлючевыеПоляАдаптированные = Неопределено)
	...
	Возврат СтрШаблон(
				...
				ИмяПараметра, 
				КлючевыеПоля, 
				?(ЗначениеЗаполнено(КлючевыеПоляАдаптированные), КлючевыеПоляАдаптированные, КлючевыеПоля));
