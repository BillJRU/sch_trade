...
//                             завершить работу платформы BAF.
//
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("ru='Запуск от имени пользователя с правом Администрирование невозможен,
                       ...
                       |режим конфигурирования платформы BAF для этого использовать не следует.'
                       ...
                       |режим конфігурування платформи BAF для цього використовувати не слід.'"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Пользователи.Администратор зарегистрирован в справочнике Пользователи';uk='Користувачі.Адміністратор зареєстрований у довіднику Користувачі'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Ошибка смены пароля';uk='Користувачі.Помилка зміни паролю'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	...
	Если Не ЭтоПолноправныйПользователь Тогда
		Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
            ...
            |;uk='Вимоги до довжини і складності пароля задаються окремо.
            |За подробицями зверніться до адміністратора.'");
	...
	Если Не ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода Тогда
		Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
            ...
            |;uk='Вимоги до довжини і складності пароля задаються окремо.
            |Див. Параметри інформаційної бази в конфігураторі в меню Адміністрування.'");
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	...
	Если ДляВнешнихПользователей Тогда
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
                ...
                |;uk='Вимоги до довжини і складності пароля задаються окремо.
                |Див. <a href = ""НастройкиВходаПользователей"">Настройки входу зовнішніх користувачів</a> в розділі Адміністрування, 
                |пункт Настройки користувачів і прав.'");
		Иначе
			Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
                |См. <a href = ""НастройкиВходаПользователей"">Настройки входа внешних пользователей</a>.'
                |;uk='Вимоги до довжини і складності пароля задаються окремо.
                |Див. <a href = ""НастройкиВходаПользователей"">Настройки входу зовнішніх користувачів</a> .'");
		КонецЕсли;
	Иначе
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
                ...
                |пункт Настройки пользователей и прав.'
                |;uk='Вимоги до довжини і складності пароля задаються окремо.
                |Див. <a href = ""НастройкиВходаПользователей"">Настройки входу користувачів</a> в розділі Адміністрування,
                |пункт Настройки користувачів і прав.'");
		Иначе
			Подсказка = НСтр("ru='Требования к длине и сложности пароля задаются отдельно.
                |См. <a href = ""НастройкиВходаПользователей"">Настройки входа пользователей</a>.'
                |;uk='Вимоги до довжини і складності пароля задаються окремо.
                |Див. <a href = ""НастройкиВходаПользователей"">Настройка входу користувачів</a> .'");
		КонецЕсли;
	КонецЕсли;
	 ...
	 Или Не ЗначениеЗаполнено(ОписаниеПользователяИБ.Пароль) Тогда
		...
		Если ЗначениеЗаполнено(НастройкиВхода.МинимальнаяДлинаПароля) Тогда
			...
			Если ПарольЗадан Или ПроверитьПустойПароль Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru='Требуется установить пароль.';uk='Потрібно встановити пароль.'"),, "СменитьПароль",, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьРолиВнешнихПользователей(Знач МассивВнешнихПользователей = Неопределено) Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Роль не найдена в метаданных';uk='Користувачі.Роль не знайдена в метаданних'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения,
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СообщениеПользователю = НСтр("ru='""%1"" исключен из группы ""%2""';uk='""%1"" виключений з групи ""%2""'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СообщениеПользователю = НСтр("ru='""%1"" перемещен в группу ""%2""';uk='""%1"" переміщений в групу ""%2""'");
		Иначе
			СообщениеПользователю = НСтр("ru='""%1"" включен в группу ""%2""';uk='""%1"" включений в групу ""%2""'");
		КонецЕсли;
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"", НСтр("ru='пользователь,пользователя,пользователей,,,,,,0';uk='користувач,користувачі,користувачів,,,,,,0'"));
		
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			СообщениеПользователю = НСтр("ru='%1 исключены из группы ""%2""';uk='%1 виключені з групи ""%2""'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СообщениеПользователю = НСтр("ru='%1 перемещены в группу ""%2""';uk='%1 переміщені в групу ""%2""'");
		Иначе
			СообщениеПользователю = НСтр("ru='%1 включены в группу ""%2""';uk='%1 включені в групу ""%2""'");
		КонецЕсли;
...
Процедура ЗаполнитьНаименованиеПредопределенныхГруппПользователей() Экспорт
	...
	ГруппаПользователейОбъект.Наименование = НСтр("ru='Все пользователи';uk='Всі користувачі'");
	...
	ГруппыВнешнихПользователейОбъект.Наименование = НСтр("ru='Все внешние пользователи';uk='Всі зовнішні користувачі'");
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка входа в программу';uk='Пользователи.Ошибка входу в програму'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	ИмяСобытия = НСтр("ru='Пользователи.Ошибка при установке ролей пользователю ИБ';uk='Користувачі.Помилка при установці ролей користувачу ІБ'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступную роль:
                       |%2.'
                       |;uk='Спроба призначити користувачу %1 недоступну роль:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступные роли:
                       |%2.'
                       |;uk='Спроба призначити користувачу %1 недоступні ролі:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь, РегистрироватьВЖурнале = Истина)
	...
	Исключение
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Пользователи.Ошибка обновления даты последней активности';uk='Пользователи.Ошибка оновлення дати останньої активності'",ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаОшибки,
...
// Проверяет пароль на требования сложности, как реализованные в BAF.
//
// Параметры:
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если РегистрироватьВЖурнале Тогда
				...
				КонецЕсли;
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Пользователи.Ошибка автоматического запрещения входа в программу';uk='Пользователи.Ошибка автоматичної заборони входу в програму'",ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
					Пользователь,
