...
Процедура ПодключитьИнтернетПоддержкуПользователей(
	...
	Если Не ДоступноПодключениеИнтернетПоддержки() Тогда
		...
		ПоказатьПредупреждение(
			ОповещениеАвторизацияНедоступна,
			НСтр("ru='Недостаточно прав для подключения Интернет-поддержки пользователей. Обратитесь к администратору.';uk='Недостатньо прав для підключення Інтернет-підтримки користувачів. Зверніться до адміністратора.'"));
		Возврат;
...
//		- "webIts" - соответствует адресу "webits-info@bas-soft.eu"
...
//			устанавливается на сервере BAF:Предприятие;
...
// страница сайта может быть открыта с учетными данными пользователя Портала ИТС,
...
Функция ВидПриложения()
	...
	#Если ВебКлиент Тогда
	Возврат НСтр("ru='Веб-клиент';uk='Вебклієнт'");
