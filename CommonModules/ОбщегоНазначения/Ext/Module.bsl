...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда 
		...
		Исключение
			...
			НСтр("ru='Неверный первый параметр Ссылка в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта:
                       |%1'
                       |;uk='Невірний перший параметр Посилання в функції ОбщегоНазначения.ЗначенияРеквизитовОбъекта:
		...
		Исключение
			ВызватьИсключение 
				НСтр("ru='Неверный первый параметр Ссылка в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: 
                           ...
                           |;uk='Невірний перший параметр Ссылка в функції ОбщегоНазначения.ЗначенияРеквизитовОбъекта:
                           |- Значення має бути посиланням або ім''ям напередвизначеного елемента'");
		КонецПопытки;
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Исключение 
				...
				Если Результат.Ошибка Тогда 
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';uk='Невірний другий параметр Реквізити в функції ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1'"),
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный тип второго параметра Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';uk='Невірний тип другого параметра Реквізити в функції ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1'"), 
	...
	Если ПустаяСтрока(ИмяРеквизита) Тогда 
		ВызватьИсключение 
			НСтр("ru='Неверный второй параметр ИмяРеквизита в функции ОбщегоНазначения.ЗначениеРеквизитаОбъекта: 
                       ...
                       |;uk='Невірний другий параметр ИмяРеквизита в функції ОбщегоНазначения.ЗначениеРеквизитаОбъекта:
                       |- Ім''я реквізиту повинно бути заповнене'");
	КонецЕсли;
...
Функция ЗначенияРеквизитовОбъектов(Ссылки, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(Реквизиты) Тогда 
		ВызватьИсключение 
			НСтр("ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: 
                       ...
                       |;uk='Невірний другий параметр Реквизиты в функції ОбщегоНазначения.ЗначенияРеквизитовОбъектов:
                       |- Поле об''єкту має бути зазначено'");
	КонецЕсли;
	...
	Если СтрНайти(Реквизиты, ".") <> 0 Тогда 
		ВызватьИсключение 
			НСтр("ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: 
                       ...
                       |;uk='Невірний другий параметр Реквизиты в функції ОбщегоНазначения.ЗначенияРеквизитовОбъектов:
                       |- Звернення через крапку не підтримується'");
	КонецЕсли;
		...
		Исключение
			ВызватьИсключение 
				НСтр("ru='Неверный первый параметр Ссылки в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: 
                           ...
                           |;uk='Невірний перший параметр Ссылки в функції ОбщегоНазначения.ЗначенияРеквизитовОбъектов:
	...
	Исключение
		...
		Если ЗначениеЗаполнено(СписокОшибок) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1';uk='Невірний другий параметр Реквізити в функції ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1'"), 
...
Функция ЗначениеРеквизитаОбъектов(МассивСсылок, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИмяРеквизита) Тогда 
		ВызватьИсключение НСтр("ru='Неверный второй параметр ИмяРеквизита в функции ОбщегоНазначения.ЗначениеРеквизитаОбъектов: 
                                     |- Имя реквизита должно быть заполнено'
                                     |;uk='Невірний другий параметр ИмяРеквизита в функції ОбщегоНазначения.ЗначениеРеквизитаОбъектов:
                                     |- Ім''я реквізиту повинно бути заповнене'");
	КонецЕсли;
...
// Возвращает Истина, если эта информационная база подключена к hmarka.BAS.
//
// Возвращаемое значение:
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	Если Не ПустаяСтрока(Минимальная)
		...
		ТекстСообщения = НСтр("ru='Минимальная версия платформы BAF указана выше рекомендуемой.
            |Минимальная версия - ""%1"", рекомендуемая версия - ""%2"".'
            |;uk='Мінімальна версія платформи BAF вказана вище рекомендованої.
...
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("МультиязычностьУкр") = Неопределено Тогда
		КодЯзыка = "ru";	
	Иначе	
	    МодульМультиязычностьУкр = ОбщегоНазначения.ОбщийМодуль("МультиязычностьУкр");
		КодЯзыка = МодульМультиязычностьУкр.КодЯзыкаИнформационнойБазы();
	КонецЕсли; 
...
// Возвращает CLSID COM-класса для работы с BAF через COM-соединение.
//
// Параметры:
//  ИмяCOMСоединителя - Строка - имя COM-класса для работы с BAF через COM-соединение.
//
// Возвращаемое значение:
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если Менеджер = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Объект метаданных ""%1"" не имеет менеджера объекта.';uk='Об''єкт метаданих ""%1"" не має менеджера об''єкта.'"), ПолноеИмя);
	КонецЕсли;
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='WSПрокси';uk='WSПроксі'",КодОсновногоЯзыка()),
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение версий интерфейса';uk='Одержання версій інтерфейсу'",КодОсновногоЯзыка()),
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Ключ) = Тип("Строка"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недопустимое значение параметра %1 в %2.
                       ...
                       |;uk='Неприпустиме значення параметра %1 в %2.
                       |параметр повинен містити рядок; передано значення: %3 (тип %4).'"),
...
//
// Параметры:
//  Объект    - Произвольный - объект встроенного языка платформы BAF, содержащий методы (например, ОбработкаОбъект).
...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если Тест = Неопределено Тогда 
			ВызватьИсключение НСтр("ru='Синтетический тест';uk='Синтетичний тест'");
		КонецЕсли;
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
// Выполняет произвольный алгоритм на встроенном языке платформы BAF, предварительно устанавливая
...
//
// Параметры:
//  Алгоритм  - Строка - алгоритм на встроенном языке "платформы BAF".
...
//
// Параметры:
//  Выражение - Строка - выражение на встроенном языке платформы BAF.
...
Функция ПодключитьКомпонентуПоИдентификатору(Знач Идентификатор, Знач Местоположение) Экспорт
	...
	Если Не МестоположениеКомпонентыКорректно(Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Указан несуществующий макет ""%1"" при подключении внешней компоненты.';uk='Вказано неіснуючий макет ""%1"" при підключенні зовнішньої компоненти.'"),
			Местоположение);
	КонецЕсли;
	...
	Если Не ПодключитьВнешнююКомпоненту(Местоположение, Идентификатор + "SymbolicName") Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на сервере по причине:
                       ...
                       |;uk='Не вдалося підключити зовнішню компоненту ""%1"" на сервері через:
                       |Метод ПодключитьВнешнююКомпоненту повернув Хибність.'"),
			Идентификатор);
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Подключение внешней компоненты на сервере';uk='Підключення зовнішньої компоненти на сервері'",КодОсновногоЯзыка()),
	...
	Если ПодключаемыйМодуль = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось создать объект внешней компоненты ""%1"", подключенной на сервере, по причине:
                       |%2'
                       |;uk='Не вдалося створити об''єкт зовнішньої компоненти ""%1"", підключеної на сервері, через:
                       |%2'"),
...
Функция УдалитьОбъектНемонопольно(Результат, Знач УдаляемаяСсылка, Знач ВсеМестаИспользования, Знач ПараметрыВыполнения, 
	...
	Исключение
		Если УдалятьНепосредственно Тогда
			ТекстОшибки = НСтр("ru='Элемент не был удален по причине:';uk='Елемент не був вилучений через:'");
		Иначе	
			ТекстОшибки = НСтр("ru='Элемент не был помечен на удаление по причине:';uk='Елемент не був позначений на вилучення через:'");
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	...
	Возврат НСтр("ru='Поиск и удаление ссылок';uk='Пошук і вилучення посилань'",КодОсновногоЯзыка());
...
Функция ОписаниеИзмеренийНабора(Знач МетаданныеРегистра, КэшИзмеренийРегистров)
	...
	Если Метаданные.РегистрыСведений.Содержит(МетаданныеРегистра) Тогда
		...
		ИначеЕсли МетаПериод = Периодичность.Год Тогда
			...
			ДанныеИзмерения.Формат        = НСтр("ru='ДФ=''yyyy ""г.""''; ДП=''Дата не задана''';uk='ДФ=''yyyy ""г.""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.День Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("ru='Период';uk='Період'");
			ДанныеИзмерения.Формат        = НСтр("ru='ДЛФ=D; ДП=''Дата не задана''';uk='ДЛФ=D; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Квартал Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("ru='Период';uk='Період'");
			ДанныеИзмерения.Формат        =  НСтр("ru='ДФ=''к """"квартал """"yyyy """"г.""""''; ДП=''Дата не задана''';uk='ДФ=''к """"квартал """"yyyy """"р.""""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Месяц Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("ru='Период';uk='Період'");
			ДанныеИзмерения.Формат        = НСтр("ru='ДФ=''ММММ yyyy """"г.""""''; ДП=''Дата не задана''';uk='Л=uk; ДФ=''ММММ yyyy """"г.""""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Секунда Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("ru='Период';uk='Період'");
			ДанныеИзмерения.Формат        = НСтр("ru='ДЛФ=DT; ДП=''Дата не задана''';uk='ДЛФ=DT; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
...
Функция СравниваемыеКолонки(Знач КоллекцияСтрок, Знач ИменаКолонок, Знач ИсключаяКолонки)
	...
	Если ПустаяСтрока(ИменаКолонок) Тогда
		...
		Иначе	
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение в ОбщегоНазначения.КоллекцииИдентичны';uk='Для колекції типу %1 необхідно вказати імена полів, за якими проводиться порівняння в ОбщегоНазначения.КоллекцииИдентичны'"),
				ТипКоллекции);
		КонецЕсли;
	Иначе
...
// на встроенном языке платформы BAF перед использованием его в операторах Выполнить и Вычислить
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Безопасное выполнение метода';uk='Безпечне виконання методу'",КодОсновногоЯзыка()),
