...
Функция ВыполнитьПроверкуСостоянияСистемы(Проверка, ЗначенияДопПараметров = Неопределено) Экспорт
	...
	РеквизитыПроверки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Проверка,
		"КонтекстВыполнения, Обработчик, Важность, ДетализацияДоОрганизации, ДетализацияДоПериода"); // СправочникСсылка.ПроверкиСостоянияСистемы
	...
	Для Каждого ТекущийОтбор Из ПараметрыПроверки.Отборы Цикл
		...
		ПредставлениеПроверки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка ""%1"" по организации ""%2"" за период %3';uk='Перевірка ""%1"" по організації ""%2"" за період %3'"),
			СокрЛП(Проверка),
			ТекущийОтбор.Организация,
			Формат(ТекущийОтбор.ПроверяемыйПериод, ""));
			
		Если ПараметрыПроверки.ПроверкаНеВыполнена Тогда
			КомментарийСобытия = НСтр("ru='%1 не выполнена по причине: %2.';uk='%1 не виконано через: %2.'");
		ИначеЕсли КоличествоОбъектов > 0 Тогда
			КомментарийСобытия = НСтр("ru='%1 завершена, выявлено проблем: %3.';uk='%1 завершена, виявлено проблем: %3.'");
		Иначе
			КомментарийСобытия = НСтр("ru='%1 завершена успешно, проблем не выявлено.';uk='%1 завершена успішно, проблем не виявлено.'");
		КонецЕсли;
...
// 	Важность - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы -
...
//
// Параметры:
// 	Проверка - СправочникСсылка.ПроверкиСостоянияСистемы - 
...
// * Наименование - Строка -
// * КонтекстВыполнения - ОпределяемыйТип.КонтекстПроверкиСостоянияСистемы -
			...
			Если ЗначениеЗаполнено(ГруппаПроверок) Тогда
				...
				Если ГруппаОбъект.КонтекстВыполнения <> Описание.КонтекстВыполнения
				 ИЛИ ГруппаОбъект.Идентификатор <> Описание.Идентификатор Тогда
					ВызватьИсключение НСтр("ru='Ошибка поиска группы проверок по ключевым полям';uk='Помилка пошуку групи перевірок по ключових полях'");
			 	КонецЕсли;
...
|%2'
|;uk='Не вдалося створити групу перевірок ""%1"" через:
|%2'"),
			...
			Если ЗначениеЗаполнено(Проверка) Тогда
				...
				Если ПроверкаОбъект.КонтекстВыполнения <> Описание.КонтекстВыполнения
				 ИЛИ ПроверкаОбъект.Идентификатор <> Описание.Идентификатор Тогда
					ВызватьИсключение НСтр("ru='Ошибка поиска проверки по ключевым полям';uk='Помилка пошуку перевірки по ключових полях'");
			 	КонецЕсли;
...
|%2'
|;uk='Не вдалося створити перевірку ""%1"" через:
|%2'"),
			Описание.Наименование,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Возвращает таблицу дополнительных параметров проверки.
// 
// Возвращаемое значение:
// 	ТаблицаЗначений - Описание:
// * ИмяПараметра - Строка -
// * ТипПараметра - Строка -
// * Заголовок - Строка -
// * ИспользуетсяДляАвтопроверки - Булево -
// * Обязательный - Булево -
// * ЗначениеПоУмолчанию - Произвольный -
// 
Функция ИнициализироватьТаблицуДополнительныхПараметровПроверки() Экспорт
	...
	ТаблицаПараметров.Колонки.Добавить("Заголовок", Новый ОписаниеТипов("Строка"));
	ТаблицаПараметров.Колонки.Добавить("ИспользуетсяДляАвтопроверки", Новый ОписаниеТипов("Булево"));
	ТаблицаПараметров.Колонки.Добавить("Обязательный", Новый ОписаниеТипов("Булево"));
...
// Проверяет наличие изменений в описании проверки.
//
// Параметры:
// 	Проверка - СправочникСсылка.ПроверкиСостоянияСистемы - 
// 	Описание - СтрокаТаблицыЗначений - Описание:
// * Наименование - Строка -
// * Комментарий - Строка -
// * УточнениеКонтекстаВыполнения - ОпределяемыйТип.УточнениеКонтекстаПроверкиСостоянияСистемы -
// * ВыполняетсяТолькоВКонтексте - Булево -
// * СпособВыполнения - ПеречислениеСсылка.СпособыВыполненияПроверокСостоянияСистемы -
// * Обработчик - Строка -
...
// * РасписаниеВыполнения - РасписаниеРегламентногоЗадания -
// * ОписаниеДополнительныхПараметров - Произвольный -
// 	СуществующиеПроверки - ТаблицаЗначений - Описание:
// * Наименование - Строка -
// * Комментарий - Строка -
// * УточнениеКонтекстаВыполнения - ОпределяемыйТип.УточнениеКонтекстаПроверкиСостоянияСистемы -
// * ВыполняетсяТолькоВКонтексте - Булево -
...
|%2'
|;uk='Не вдалося записати перевірку ""%1"" через:
|%2'"),
			СокрЛП(Проверка),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция РезультатПроверкиВДлительнойОперации(Форма, МассивПроверок, ЗначенияДопПараметров = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Проверка состояния системы';uk='Перевірка стану системи'");
...
// Параметры:
// 	Проверка - СправочникСсылка.ПроверкиСостоянияСистемы - 
// 	КонтекстВыполнения - ОпределяемыйТип.КонтекстПроверкиСостоянияСистемы, Null -
