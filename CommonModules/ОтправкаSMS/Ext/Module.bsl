...
//
// Параметры:
//  НомераПолучателей  - Массив - массив строк номеров получателей в формате +38ХХХХХХХХХ;
...
Функция ОтправитьSMS(НомераПолучателей, Знач Текст, ИмяОтправителя = Неопределено, ПеревестиВТранслит = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтрСоединить(НомераПолучателей, "")) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не указан номер получателя SMS.';uk='Не вказаний номер одержувача SMS.'");
		Возврат Результат;
	КонецЕсли;
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(Результат.ОписаниеОшибки) И Не ЗначениеЗаполнено(Результат.ОтправленныеСообщения) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при выходе из процедуры ОтправкаSMSПереопределяемый.ОтправитьSMS:
                    ...
                    |;uk='Помилка при виході з процедури ОтправкаЅМЅПереопределяемый.ОтправитьЅМЅ:
                    |Не заповнені вихідні параметри ОписаниеОшибки і ОтправленныеСообщения (провайдер: %1).
                    |Очікується заповнення щонайменше одного з цих параметрів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Неверный тип значения в коллекции Результат.ОтправленныеСообщения:
                        ...
                        |;uk='Невірний тип значення в колекції Результат.ОтправленныеСообщения:
                        |очікується тип ""Структура"", переданий тип ""%1""'"),
...
Функция СпособыАвторизацииПровайдераПоУмолчанию()
	...
	ПоляАвторизации.Добавить("Логин", НСтр("ru='Логин';uk='Логін'"));
	ПоляАвторизации.Добавить("Пароль", НСтр("ru='Пароль';uk='Пароль'"), Истина);
