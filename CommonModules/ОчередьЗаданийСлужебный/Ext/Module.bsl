...
Процедура СоздатьНовоеРегламетноеЗаданиеОбработки(Знач Задание) Экспорт 
	
	РазделениеУстановил = УстановитьРазделениеДанныхДляРегламентнойОбработки(Задание);
	
	Если РегламентноеЗаданиеСоздано(Строка(Задание.Ссылка.УникальныйИдентификатор()), Задание) Тогда
		Возврат;
	КонецЕсли;
	                                
	НовоеРегЗадание = РегламентныеЗадания.СоздатьРегламентноеЗадание(МетаданныеРегламентногоЗадания());
	НовоеРегЗадание.КоличествоПовторовПриАварийномЗавершении = 0;
	НовоеРегЗадание.Параметры.Добавить(Задание.Ссылка);
	НовоеРегЗадание.Ключ = Строка(Задание.Ссылка.УникальныйИдентификатор());
	НовоеРегЗадание.Использование = Истина;
	НовоеРегЗадание.Наименование = НовоеРегЗадание.Наименование + СтрШаблон(" (%1)", Задание.Ссылка.УникальныйИдентификатор());
	
	Расписание = Новый РасписаниеРегламентногоЗадания;
	Расписание.ДатаНачала = Задание.ЗапланированныйМоментЗапуска;
	НовоеРегЗадание.Расписание = Расписание;
	                     
	НовоеРегЗадание.Записать();	
	
	ОбработатьРучнуюУстановкуРазделителя(РазделениеУстановил);
	
КонецПроцедуры

Процедура ОжидатьЗавершенияЗадания(Знач Задание) Экспорт 
	
	КоличествоПопыток = 0;
	
	Пока ЗаданиеВыполняется(Задание) Цикл
		
		ОбщегоНазначенияБТС.Пауза(3);
		
		КоличествоПопыток = КоличествоПопыток + 1;
		Если КоличествоПопыток = 10 Тогда
			ЗаданиеФактическиВыполняется = ПроверитьНазначениеИсполняющегоФоновогоЗадания(Задание);
			Если ЗаданиеФактическиВыполняется Тогда
				КоличествоПопыток = 0;
			Иначе
				СнятьНазначениеРегламентногоЗадания(Задание);
				Прервать;
			КонецЕсли;
		КонецЕсли;
...
Процедура НазначитьРегламентноеЗадание(ЗаданиеОчереди) Экспорт
	
	ОбластьДанных = ОпределитьОбластьДанныхЗадания(ЗаданиеОчереди);
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить(ЗаданиеОчереди.Метаданные().ПолноеИмя());
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ЗаданиеОчереди);
		Блокировка.Заблокировать();
		
		Задание = ЗаданиеОчереди.ПолучитьОбъект();
		Задание.Ключ = ЗаданиеОчереди.УникальныйИдентификатор();
...
Функция ЗаданиюНазначеноРегламентноеЗадание(ЗаданиеОчереди) Экспорт 
	
	КлючЗадания = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеОчереди, "Ключ");
	Возврат Строка(ЗаданиеОчереди.УникальныйИдентификатор()) = КлючЗадания; 
	
КонецФункции

Процедура УдалитьРегламентныеЗаданияОбработки(Знач Ключ, Знач Задание) Экспорт 
	
	РазделениеУстановил = УстановитьРазделениеДанныхДляРегламентнойОбработки(Задание);
	
	Отбор = Новый Структура("Ключ, Метаданные", Строка(Ключ), МетаданныеРегламентногоЗадания());
	СуществующиеЗадания = РегламентныеЗадания.ПолучитьРегламентныеЗадания(Отбор);
	
	Для Каждого РегЗадание Из СуществующиеЗадания Цикл
		РегЗадание.Удалить();	
	КонецЦикла;
...
Процедура ПланированиеОбработкиЗаданий() Экспорт
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		ИмяМетаданных = СправочникЗаданий.ПустаяСсылка().Метаданные().ПолноеИмя();
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() 
		   И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяМетаданных, РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			|ГДЕ
			|	Очередь.СостояниеЗадания В (
			|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.Выполняется), 
			|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.Завершено), 
			|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.НеЗапланировано), 
			|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.ОшибкаВыполнения))", ИмяМетаданных);
...
Процедура ОбработатьОчередьЗаданий(КлючФоновогоЗадания, ЗаданиеОчереди = Неопределено, ИсполняющееФоновоеЗадание = Неопределено) Экспорт
	...
	Если ЗаданиеОчереди = Неопределено Тогда
		ЗаданиеОчереди = Справочники.ОчередьЗаданий.ПустаяСсылка();	
	КонецЕсли;
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(СправочникЗаданий.СоздатьЭлемент().Метаданные().ПолноеИмя(), РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			, "&Таблица", СправочникЗаданий.ПустаяСсылка().Метаданные().ПолноеИмя());
	...
	ТекстЗапроса = ТекстЗапросаБлокировки + СтрЗаменить(
		"ВЫБРАТЬ ПЕРВЫЕ 111
		...
		|	Идентификатор", "&ВложенныйЗапрос", СтрШаблон("(%1)", ТекстЗапроса)); 
	...
	Если ЗначениеЗаполнено(ЗаданиеОчереди) Тогда
		Запрос.УстановитьПараметр("Задание", ЗаданиеОчереди);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Задание", "ВложенныйЗапрос.Идентификатор = &Задание"); 
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Задание", "НЕ ВложенныйЗапрос.Идентификатор В (&Задание)");
	КонецЕсли;	
	
	Пока МожноВыполнять Цикл 
		
		Если НЕ ЗначениеЗаполнено(ЗаданиеОчереди) Тогда
			Запрос.УстановитьПараметр("Задание", МассивЗаданийИсключений);
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл 
			...
			Попытка
				...
				Если Выборка.ВыполнятьЧерезРегламентноеЗадание И ЗаданиюНазначеноРегламентноеЗадание(Выборка.Идентификатор) Тогда
					Продолжить;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон)
					...
					ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Выполнение';uk='Черга завдань.Виконання'",ОбщегоНазначения.КодОсновногоЯзыка()), 
		...
		Иначе
			...
			Если Выборка.ВыполнятьЧерезРегламентноеЗадание И НЕ ЗначениеЗаполнено(ЗаданиеОчереди) Тогда		
				
				УдалитьРегламентныеЗаданияОбработки(Выборка.Идентификатор.УникальныйИдентификатор(), Выборка.Идентификатор);
				НазначитьРегламентноеЗадание(Выборка.Идентификатор);
				СоздатьНовоеРегламетноеЗаданиеОбработки(Выборка.Идентификатор);
				РазблокироватьДанныеДляРедактирования(Выборка.Идентификатор);
				                   
				МассивЗаданийИсключений.Добавить(Выборка.Идентификатор);
				
				Если Выборка.ОжидатьЗавершенияВыполненияРегламентногоЗадания Тогда
					ОжидатьЗавершенияЗадания(Выборка.Идентификатор);
				КонецЕсли;
...
Процедура РегламентнаяОбработкаОчередиЗаданий(Задание) Экспорт
	...
	Если НЕ ЗаданиеЗаблокировано Тогда
		...
		ТекстОшибки = НСтр("ru='Исполняющему фоновому задание не удалось заблокировать задание очереди';uk='Виконуючому фонового завдання не вдалось заблокувати завдання черги'") + Символы.ПС + ТекстОшибки;
		
		ВызватьИсключение ТекстОшибки;	
	КонецЕсли;
		
	// Назначение рег. задания снимается для того, что бы при аварийном завершении 
	// при следующем проходе, планировщик перепланировал задание снова
	СнятьНазначениеРегламентногоЗадания(Задание);
	ОбработатьОчередьЗаданий(ФоновоеЗадание.Ключ, Задание, ФоновоеЗадание);
	...
	Если ПараметрыОбработчикаОшибок.ОбработчикСуществует Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Очередь регламентных заданий.Ошибка обработчика ошибок';uk='Черга регламентних завдань.Помилка обробника помилок'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстКомментария);
...
Процедура СнятьЗаданияОбработкиОшибок(Знач ПараметрыВызова, Знач ИнформацияОбОшибке = Неопределено, Знач СчетчикРекурсии = 1) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Очередь регламентных заданий.Снятие заданий обработки ошибок';uk='Черга регламентних завдань.Зняття завдань обробки помилок'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Очередь регламентных заданий.Выполнение';uk='Черга регламентних завдань.Виконання'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Функция ИспользованиеРегламентныхЗаданий()
	...
	ИнтеграцияПодсистемБТС.ПриОпределенииИспользованияРегламентныхЗаданий(ТаблицаИспользования);
...
Процедура ЗапланироватьЗадание(Знач Выборка)
	...
	ТребуетсяЗапускРегЗадания = Ложь;
	ТребуетсяУдалениеРегЗадания = Ложь;
	...
	Если Выборка.ОбластьДанных <> -1 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
			    НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",ОбщегоНазначения.КодОсновногоЯзыка()), 
			    УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
	...
	Попытка
		...
		ИдентификаторЗадания = Выборка.Идентификатор.УникальныйИдентификатор();
		Задание = Выборка.Идентификатор.ПолучитьОбъект();
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",ОбщегоНазначения.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					ТекстСообщения);
		...
		Если Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.ОшибкаВыполнения
			...
			ТребуетсяЗапускРегЗадания = Задание.ВыполнятьЧерезРегламентноеЗадание;
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначения.КодОсновногоЯзыка()), Выборка.Идентификатор, 
		...
		Иначе
			...
			Если Расписание = Неопределено Или (Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Завершено
				...
				// Разовое задание
				ТребуетсяУдалениеРегЗадания = Задание.ВыполнятьЧерезРегламентноеЗадание;
		...
		Если ТребуетсяЗапускРегЗадания Тогда
			УдалитьРегламентныеЗаданияОбработки(ИдентификаторЗадания, Задание);
			СоздатьНовоеРегламетноеЗаданиеОбработки(Задание);
		ИначеЕсли ТребуетсяУдалениеРегЗадания Тогда
			УдалитьРегламентныеЗаданияОбработки(ИдентификаторЗадания, Задание);
		КонецЕсли;
...
Процедура ВыполнитьЗаданиеОчереди(Знач Ссылка, Знач ИсполняющееФоновоеЗадание, 
	...
	ОбластьДанных = ОпределитьОбластьДанныхЗадания(Ссылка);	
	...
	Попытка
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Старт';uk='Черга регламентних завдань.Старт'",ОбщегоНазначения.КодОсновногоЯзыка()), Ссылка);
		...
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено успешно';uk='Черга регламентних завдань.Завершено успішно'",ОбщегоНазначения.КодОсновногоЯзыка()), Ссылка);
	...
	Исключение
		...
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначения.КодОсновногоЯзыка()), Ссылка,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибкеВыполненияЗадания));
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
...
Процедура ЗапланироватьНеАктивныеЗадания() Экспорт
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(СправочникЗаданий.СоздатьЭлемент().Метаданные().ПолноеИмя(), РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			|	И Очередь.Использование", "&Таблица", СправочникЗаданий.ПустаяСсылка().Метаданные().ПолноеИмя());
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Обновление очереди заданий';uk='Оновлення черги завдань'",ОбщегоНазначения.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
...
Функция ЗаданиеВыполняется(Знач Задание)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ОчередьЗаданий.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ОчередьЗаданий КАК ОчередьЗаданий
	|ГДЕ
	|	ОчередьЗаданий.СостояниеЗадания В (ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.Запланировано), ЗНАЧЕНИЕ(Перечисление.СостоянияЗаданий.Выполняется))
	|	И ОчередьЗаданий.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", Задание);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.ОчередьЗаданий", Задание.Метаданные().ПолноеИмя());
...
Функция ОпределитьОбластьДанныхЗадания(Задание)
	...
	Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() Тогда
		...
		ПереопределеннаяОбластьДанных = МодульОчередьЗаданийСлужебныйРазделениеДанных.ОпределитьОбластьДанныхДляЗадания(Задание);
...
Функция РегламентноеЗаданиеСоздано(Знач Ключ, Знач Задание)
	
	РазделениеУстановил = УстановитьРазделениеДанныхДляРегламентнойОбработки(Задание);
	
	Отбор = Новый Структура("Ключ, Метаданные", Ключ, МетаданныеРегламентногоЗадания());
	СуществующиеЗадания = РегламентныеЗадания.ПолучитьРегламентныеЗадания(Отбор);
...
Функция УстановитьРазделениеДанныхДляРегламентнойОбработки(Знач Задание)
	
	РазделениеУстановлено = РаботаВМоделиСервиса.ИспользованиеРазделителяСеанса();
	РазделениеУстановил = Ложь;
		
	Если (ТипЗнч(Задание) <> Тип("СправочникСсылка.ОчередьЗаданий") И ТипЗнч(Задание) <> Тип("СправочникОбъект.ОчередьЗаданий")) 
		И НЕ РазделениеУстановлено Тогда
			
		РаботаВМоделиСервиса.УстановитьРазделениеСеанса(Истина, Задание.ОбластьДанныхВспомогательныеДанные);
		РазделениеУстановил = Истина;
		
	КонецЕсли;
	
	Возврат РазделениеУстановил;
	
КонецФункции

Процедура ОбработатьРучнуюУстановкуРазделителя(РазделениеУстановил)
	
	Если РазделениеУстановил Тогда
		РаботаВМоделиСервиса.УстановитьРазделениеСеанса(Ложь);		
	КонецЕсли;
...
Функция МетаданныеРегламентногоЗадания()
	...
	Если РаботаВМоделиСервиса.ИспользованиеРазделителяСеанса() Тогда
		Возврат Метаданные.РегламентныеЗадания.РегламентнаяОбработкаОчередиЗадачОбластейДанных;
	Иначе
		Возврат Метаданные.РегламентныеЗадания.РегламентнаяОбработкаОчередиЗадач;	
	КонецЕсли;
	
КонецФункции

Процедура СнятьНазначениеРегламентногоЗадания(ЗаданиеОчереди)
...
Функция ПроверитьНазначениеИсполняющегоФоновогоЗадания(Задание)
	
	ИдентификаторФЗ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задание, "ИсполняющееФоновоеЗадание");
	Если ЗначениеЗаполнено(ИдентификаторФЗ) 
		И ИдентификаторФЗ <> ОбщегоНазначенияБТС.ПустойУникальныйИдентификатор() Тогда
		Возврат Истина;	
	КонецЕсли;
