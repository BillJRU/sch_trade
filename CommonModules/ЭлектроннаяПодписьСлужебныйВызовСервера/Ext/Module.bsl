...
Функция ВыполнитьНаСторонеСервера(Знач Параметры, АдресРезультата, ОперацияНачалась, ОшибкаНаСервере) Экспорт
	...
	ИначеЕсли ЭтоCMS Тогда
		...
		Если Параметры.Операция <> "Подписание" Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки",
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Параметры.Операция)
				+ Символы.ПС + НСтр("ru='Внешняя компонента XMLDSig (включающая CMS) может использоваться только для подписания.';uk='Зовнішня компонента XMLDSig (що включає CMS) може використовуватися тільки для підписання.'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если ТипЗнч(ИнформацияОФайле.Данные) = Тип("Строка") Тогда
		...
		Иначе
			ТекстСостояния.ДобавитьСтроку(
				...
				НСтр("ru='Не удалось добавить %1. Данные файла не являются двоичными данными или адресом во временном хранилище.';uk='Не вдалося додати %1. Дані файлу не є двійковими даними або адресою у тимчасовому сховищі.'"),
				ИнформацияОФайле.Имя));
		КонецЕсли;
