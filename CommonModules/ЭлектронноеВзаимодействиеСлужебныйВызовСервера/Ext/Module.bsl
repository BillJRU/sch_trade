...
|;uk='Функціональність в мобільному клієнті поки недоступна.
|Скористайтеся вебклієнтом або тонким клієнтом'");
	...
	Если ЗначениеЗаполнено(Ошибки) Тогда
		...
		Иначе
			...
			Если ЭтоПолноправныйПользователь И ЗначениеЗаполнено(Ошибки) И ЗначениеЗаполнено(ПодробныйТекстОшибки)
				...
				Ошибки = Ошибки + Символы.ПС + НСтр("ru='Подробности см. в журнале регистрации.';uk='Подробиці див. у журналі реєстрації.'");
			КонецЕсли;
...
// Проверяет будет ли пользователю показываться оповещение о наличии новых эд в сервисе Сервис-ЭДО
// Возвращаемое значение:
