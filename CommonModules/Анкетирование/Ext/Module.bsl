...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение значения нового реквизита ""Режим анкетирования"" у документов ""Анкета"" прошлых периодов.
        ...
        |;uk='Заповнення значення нового реквізиту ""Режим анкетування"" в документах ""Анкета"" минулих періодів.
        |До завершення обробки ""Режим анкетування"" даних документів буде відображатися некоректно.'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение значения нового реквизита ""Способ отображения подсказки"" в справочнике ""Вопросы шаблона анкеты"".
        ...
        |;uk='Заповнення значення нового реквізиту ""Режим перегляду підказки"" в довіднику ""Питання шаблону анкети"".
        |До завершення обробки ""Режим перегляду підказки"" в даному довіднику буде відображатися некоректно.'");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение значений нового реквизита ""Вид"" в вопросах для анкетирования.
        ...
        |;uk='Заповнення значень нового реквізиту ""Вид"" в питаннях для анкетування.
        |До завершення обробки вид питань для анкетування буде відображатися некоректно.'");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ОчередьОтложеннойОбработки = 1;
...
Процедура УстановитьКорневойЭлементДереваАнкеты(ДеревоАнкеты) Экспорт
	...
	НоваяСтрока.Формулировка = НСтр("ru='Анкета';uk='Анкета'");
...
Процедура ПостроениеФормыЗаполненияПоРазделу(Форма,ТекущиеДанныеДеревоРазделов) Экспорт
	...
	Если ТекущиеДанныеДеревоРазделов.ТипСтроки = "Раздел" Тогда
		...
		ЭлементНадписьВступление.Заголовок  = НСтр("ru='Раздел';uk='Розділ'") + " " + ТекущиеДанныеДеревоРазделов.Формулировка;
		...
		ИначеЕсли РеквизитыЭлементарногоВопроса.ТипОтвета = Перечисления.ТипыОтветовНаВопрос.Число Тогда
			...
			Если РеквизитыЭлементарногоВопроса.МинимальноеЗначение <> 0 ИЛИ РеквизитыЭлементарногоВопроса.МаксимальноеЗначение <> 0 Тогда
				...
				ИначеЕсли РеквизитыЭлементарногоВопроса.МинимальноеЗначение = 0 И РеквизитыЭлементарногоВопроса.МаксимальноеЗначение <> 0 Тогда
					ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Допустим ввод значения до %1';uk='Припустимо введення значення до %1'"), РеквизитыЭлементарногоВопроса.МаксимальноеЗначение);
				Иначе
					ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Допустим ввод значения от %1';uk='Припустиме введення значення від %1'"), РеквизитыЭлементарногоВопроса.МинимальноеЗначение);
				КонецЕсли;
