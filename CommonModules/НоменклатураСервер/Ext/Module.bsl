...
|В інформаційній базі зберігається кілька записів додаткових реквізитів і відомостей з найменуванням ""%ИмяРеквизита%"", що входять до складу набору додаткових реквізитів і відомостей, що записується елемента довідника ""%ИмяОбъекта%"".'");
...
//          * СписокСтрок - Массив, Неопределено - значение по умолчанию Неопределенно
...
//								  - ПеречислениеСсылка.АмортизационныеГруппы - реквизиты, которые не нужно проверять платформенной проверкой;
...
Процедура ПроверитьЗаполнениеСерий(ДокументОбъект,ПараметрыУказанияСерий,Отказ,МассивНепроверяемыхРеквизитов = Неопределено, ПараметрыПроверки = Неопределено) Экспорт
	...
	Если ПараметрыУказанияСерий.ТоварВШапке Тогда
		...
		Если НоменклатураКлиентСервер.СтатусыСерийСерияНеУказана().Найти(ДокументОбъект.СтатусУказанияСерий) <> Неопределено
			...
			ТекстСообщения = НСтр("ru='Количество товара ""%Товар%"" не соответствует указанному для него количеству серий. Исправьте серии.';uk='Кількість товару ""%Товар%"" не відповідає зазначеному для нього кількістю серій. Виправте серії.'");
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ОшибкаКоличества Тогда
					Если Выборка.ЭтоТоварноеМесто Тогда
						ТекстСообщения = НСтр("ru='Политика учета серий товара ""%НоменклатураПредставление%"" предусматривает, что количество товарных мест будет соответствовать количеству товара равному 1, т.е. %КоличествоТоварныхМест%';uk='Політика обліку серій товару ""%НоменклатураПредставление%"" передбачає, що кількість товарних місць буде відповідати кількості товару рівному 1, тобто %КоличествоТоварныхМест%'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоТоварныхМест%", Выборка.КоличествоТоварныхМест);
					Иначе
						ТекстСообщения = НСтр("ru='Указанное количество товара ""%НоменклатураПредставление%"" с серией ""%Серия%"" равно %КоличествоСерий%. Политика учета серий данного товара предусматривает, что количество по любой серии этого товара всегда будет равно 1.';uk='Зазначена кількість товару ""%НоменклатураПредставление%"" з серією ""%Серия%"" одно%КоличествоСерий%. Політика обліку серій даного товару передбачає, що кількість з будь-якої серії цього товару завжди дорівнюватиме 1.'");
				...
				Если Выборка.ОшибкаУпаковки Тогда
					ТекстСообщения = НСтр("ru='Политика учета серий товара ""%НоменклатураПредставление%"" предусматривает, что серии указываются для каждого экземпляра. Укажите единичную упаковку в строке %НомерСтроки% списка ""%ПредставлениеТЧ%"".';uk='Політика обліку серій товару ""%НоменклатураПредставление%"" передбачає, що серії вказуються для кожного екземпляра. Вкажіть одиничну упаковку в рядку %НомерСтроки% списку ""%ПредставлениеТЧ%"".'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НоменклатураПредставление%",	Выборка.НоменклатураПредставление);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%",				Строка(Выборка.НомерСтроки));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеТЧ%",			ПредставлениеТЧ);
...
//
// Параметры:
//  ЗначенияПолейСвязи	 - Структура, ФиксированнаяСтруктур 
// 
// Возвращаемое значение:
//   Строка 
//
...
Процедура УстановитьУсловноеОформлениеСодержания(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<для работ, оформляемых актом>';uk='<для робіт, що оформляються актом>'"));
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура УстановитьУсловноеОформлениеНомераГТД(Форма,
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
    ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<Коды для НН и номера ГТД не используются>';uk='<Коди для ПН і номери ВМД не використовуються>'"));
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура ПродолжитьОбходПоСериям(Выборка, ТоварныеМестаПоНоменклатуре, ЕстьОшибки)
	...
	Пока ВыборкаПоСерии.Следующий() Цикл 
		...
		Если ОписанияИзлишков.Количество() <> 0 Или ОписанияНедостач.Количество() <> 0 Тогда
			ЕстьОшибки = Истина;
			СообщениеПользователю = НСтр("ru='Из следующих товарных мест нельзя собрать целое количество %ЕдИзм% товара ""%Товар%"": %ОписаниеИзлишков%. Необходимо или удалить эти товарные места, или дополнить товарными местами: %ОписаниеНедостач%.';uk='З наступних товарних місць не можна зібрати цілу кількість %ЕдИзм% товару ""%Товар%"": %ОписаниеИзлишков%. Необхідно або видалити ці товарні місця, або доповнити товарними місцями: %ОписаниеНедостач%.'");
...
Процедура ЗаполнитьДействиеПоКомплектностиТоварныхМест(Объект) Экспорт 
	...
	Для Каждого Строка Из ТаблицаТоваров Цикл
		...
		НайденныеСтроки = ТаблицаКоличествоУпаковок.НайтиСтроки(СтруктураПоиска); // Массив из ДокументТабличнаяЧастьСтрока.АктОРасхожденияхПослеПриемки.Товары - указано для типизации
...
#Область ПроцедурыРаботыСНоменклатуройГТД

// Параметры заполнения номенклатуры ГТД в табличной части объекта.
//
// Возвращаемое значение:
//	Структура - содержит следующие свойства:
// ОБЯЗАТЕЛЬНЫЕ.
//	* ПараметрыЗапроса	- Структура	- параметры запроса остатков номенклатуры ГТД:
//		** Ссылка		- ДокументСсылка						- ссылка на документ, в котором заполняются номенклатура ГТД (обязательно к заполнению
//																	при варианте получения данных 'ПоРаспоряжениям').
//		** Период		- Дата, Граница							- период, за который получаются сведения об остатках номенклатуры ГТД (обязательно
...
//																	номенклатуры ГТД (обязательно к заполнению при варианте получения данных 'ПоОстаткамУХранителя').
//		** Партнер		- СправочникСсылка.Партнеры				- партнер, для которого получаются сведения об остатках номенклатуры ГТД
//																	(обязательно к заполнению при варианте получения данных 'ПоОстаткамПереданныхНаКомиссию').
//		** Договор		- СправочникСсылка.ДоговорыКонтрагентов	- договор, по которому получаются сведения об остатках номенклатуры ГТД
//																	(обязательно к заполнению при варианте получения данных 'ПоОстаткамУХранителя').
// НЕОБЯЗАТЕЛЬНЫЕ.
//	* ИмяПоляРаспоряжение	- Строка	- имя поля распоряжения, по которому получаются сведения о номенклатуре ГТД.
//	* СтруктураДействий		- Структура	- действия по обработке строк табличной части.
//
Функция ПараметрыЗаполненияНоменклатурыГТД() Экспорт    
...
// Заполняет номенклатуру ГТД в табличной части объекта.
//
// Параметры:
//	ТаблицаТоваров			- ДанныеФормыКоллекция, ТабличнаяЧасть	 - таблица в которой заполняются номенклатура ГТД. Должна содержать
//																		следующие колонки:
...
//		* НоменклатураГТД	    - СправочникСсылка.НоменклатураГТД				- номенклатура ГТД.
//		* Количество			- Число											- количество товара.
//	ВариантПолученияДанных	- Строка								 - вариант получения данных о номенклатуре ГТД.
//	ДополнительныеПараметры	- Структура								 - параметры заполнения номенклатуры ГТД 
//																		(см. НоменклатураСервер.ПараметрыЗаполненияНоменклатурыГТД()).
//
Процедура ЗаполнитьНоменклатуруГТД(ТаблицаТоваров, ВариантПолученияДанных, ДополнительныеПараметры) Экспорт
    ...
    ТекстЗапроса = ТекстЗапросаФормированияТаблицыНоменклатурыГТД(ВариантПолученияДанных);
	...
	ТоварыКЗаполнению.ЗаполнитьЗначения(НоменклатураГТДПоУмолчанию, "НомерГТД");
	...
	РезультатЗапроса = Запрос.Выполнить();
    НоменклатураГТДКЗаполнению = РезультатЗапроса.Выгрузить();
    ...
    Для Каждого СтрНоменклатураГТДКЗаполнению Из НоменклатураГТДКЗаполнению Цикл
		
        КоличествоРаспределить = СтрНоменклатураГТДКЗаполнению.Количество;
		
        ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрНоменклатураГТДКЗаполнению);
		...
		Для Каждого СтрТовары Из СтрокиТоваров Цикл
            ...
            СтрТовары.НомерГТД = СтрНоменклатураГТДКЗаполнению.НомерГТД;
...
Функция ОтборПолитикУчетаСерийПоПараметрам(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры)
		...
		Отбор.Вставить("УчетТоваровВПутиОтПоставщикаПоСериям",
					РеквизитыТекущейПолитикиУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям);
		
		Отбор.Вставить("УчетСерийВНеотфактурованныхПоставкахТоваров",
					РеквизитыТекущейПолитикиУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям);
...
Функция ОтборВладельцаПодчиненныхСправочников(ВидНоменклатуры, ЗначащаяЧастьИмениНадписи) Экспорт
...
Процедура ЗаполнитьСерииПоFEFOВТЧТовары(Объект, ПараметрыУказанияСерий)
	...
	Пока ВыборкаНезаполненные.Следующий() Цикл
		...
		Если ВыборкаНезаполненные.ЭтоТоварноеМесто Тогда
			...
			ТекстСообщения = НСтр("ru='В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. указана упаковка - товарное место. Заполнение серий FEFO по товарным местам не поддерживается.';uk='У рядку %НомерСтроки% для товару ""%ПредставлениеТовара%"" серії незаповнені, тому що вказана упаковка - товарне місце. Заповнення серій FEFO по товарним місцях не підтримується.'");
			
		ИначеЕсли ВыборкаНезаполненные.Количество = 0 Тогда
			
			ТекстСообщения = НСтр("ru='В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. не указано количество товаров.';uk='У рядку %НомерСтроки% для товару ""%ПредставлениеТовара%"" серії незаповнені, тому що не вказано кількість товарів.'");
			
		Иначе
			
			ТекстСообщения = НСтр("ru='В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены. На складе ""%ПредставлениеСклада%"" не хватает ""%Количество%"" ""%ЕдиницаИзмерения%""';uk='У рядку %НомерСтроки% для товару ""%ПредставлениеТовара%"" серії незаповнені. На складі ""%ПредставлениеСклада%"" не вистачає ""%Количество%"" ""% ЕдиницаИзмерения%""'");
...
Функция ТекстЗапросаФормированияТаблицыНоменклатурыГТД(ВариантПолученияДанных)
	...
	ИначеЕсли ВариантПолученияДанных = "ПоОстаткамПереданныхНаКомиссию" Тогда
		...
		|	) КАК НаборДанных
		|ГДЕ
		|	НаборДанных.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	НаборДанных.ДокументРеализации,
		|	НаборДанных.Номенклатура,
	...
	Иначе
		ВызватьИсключение НСтр("ru='Неподдерживаемый вариант получения данных по номерам ГТД.';uk='Варіант, що не підтримується отримання даних за номерами ВМД.'");
	КонецЕсли;
