...
Процедура УстановитьУсловноеОформлениеПоляДата(ЭтотОбъект, 
	...
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru='ДФ=ЧЧ:мм';uk='ДФ=ЧЧ:мм'"));
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("ru='Для разработчика: возможно требуется обновить вспомогательные данные,
                    ...
                    |- либо запустить программу с параметром командной строки платформы BAF 8
                    ...
                    |- або запустити програму з параметром командного рядка платформи BAF 8
                    |  ""/С ЗапуститьОбновлениеИнформационнойбазы"",
...
Функция ПредставлениеНачальнойСтраницы() Экспорт 
	...
	Возврат НСтр("ru='Главное';uk='Головне'");
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Обмен данными.Отключение связи с главным узлом';uk='Обмін даними.Відключення зв''язку з головним вузлом'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПередЗапускомПрограммы()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для запуска необходима версия платформы BAF %1 или выше.';uk='Для запуску необхідна версія платформи BAF %1 або вище.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НеПоддерживаемаяВерсияПлатформы) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Режим совместимости конфигурации с платформой версии %1 не поддерживается.
                       ...
                       |Для запуску встановіть в конфігурації режим сумісності ""Не використовувати"" при розробці на версії %2
                       |(або ""Версія %2"" при розробці на більш старших версіях).'"),
	...
	Если Не ТекущийПользовательИБ.АутентификацияСтандартная
	   ...
	   И Не ТекущийПользовательИБ.АутентификацияOpenID Тогда
		ВызватьИсключение НСтр("ru='Вход в программу запрещен. Обратитесь к администратору.';uk='Вхід в програму заборонений. Зверніться до адміністратора.'");
	КонецЕсли;
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("ru='В конфигурации в свойстве ОсновныеРоли не указаны
                       ...
                       |;uk='У конфігурації властивості ОсновныеРоли не зазначені
                       |стандартні ролі АдминистраторСистемы і ПолныеПрава.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПрофильИнформационнойБазы) Тогда
		...
		Если БезопасныйРежим() <> ПрофильИнформационнойБазы Тогда
			...
			Если Не ДоступенПривилегированныйРежим Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Невозможно установить параметры сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов платформы BAF, или для него запрещено использование в качестве профиля безопасности безопасного режима.
                        |
                        ...
                        |;uk='Неможливо встановити параметри сеансу через: профіль безпеки %1 відсутній в кластері серверів платформи BAF, або для нього заборонено використання в якості профілю безпеки безпечного режиму.
                        |
