...
Процедура ПриСозданииНаСервере(Форма, Представление, ИмяОсновногоРеквизитаФормы = "Объект") Экспорт
	...
	ЗаголовокРеквизита = НСтр("ru='Изменено представление';uk='Змінено представлення'");
...
Процедура ПроверитьВерсиюПлатформы()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.14.1970") < 0
		...
		ВызватьИсключение
		НСтр("ru='Для корректной работы склонения представлений рекомендуется обновить технологическую платформу ""BAF"" до версии 8.3.14.1970 или более новой (либо до версии 8.3.15.1747 или более новой).';uk='Для коректної роботи відмінювання представленm рекомендується оновити технологічну платформу ""BAF"" до версії 8.3.14.1970 чи новішої (або до версії 8.3.15.1747 чи новішої).'");
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеСтрокиПоПараметрамСклонения(ОписаниеСтроки, ПараметрыСклонения, ИмяПроцедурыИлиФункции)
	...
	Если ПараметрыСклонения.Пол = 1 Тогда
		ОписаниеСтроки = "ПЛ=Мужской;Л=ru_RU";
	КонецЕсли;
	
	Если ПараметрыСклонения.Пол = 2 Тогда
		ОписаниеСтроки = "ПЛ=Женский;Л=ru_RU";
	КонецЕсли;
...
|параметр повинен числом 1 (чоловічий) або 2 (жіночий); передано значення: %3 (тип %4).'"),
...
|;uk='Неприпустиме значення параметра Відмінок в СклонениеПредставлениеОбъектов.Просклонять.
|Параметр повинен бути числом, що позначає порядковий номер відмінка, від 1 до 6.
|Передано значення %1 (тип %2).'"),
		Падеж,
		ТипЗнч(Падеж));
