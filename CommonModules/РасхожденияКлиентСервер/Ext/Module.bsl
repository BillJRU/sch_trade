...
Процедура УправлениеДоступностью(Форма) Экспорт
	...
	Иначе
		...
		ПодборДокументовОснованийДоступен = ЗначениеЗаполнено(Форма.Объект.Партнер)
		                                    ...
		                                    И ЗначениеЗаполнено(Форма.Объект.Организация)
		                                    И ЗначениеЗаполнено(Форма.Объект.Валюта)
 		                                    И ЗначениеЗаполнено(Форма.НалогообложениеНДСПоУмолчанию)
		                                    И ЗначениеЗаполнено(Форма.Объект.ХозяйственнаяОперация);
	...
	Если Не ТипОснованияПеремещениеТоваров(Форма.Объект.Ссылка) Тогда
		...
		Форма.Элементы.ЦенаВключаетНДС.ТолькоПросмотр       = НЕ ВлияющиеРеквизитыДоступны;
 		Форма.Элементы.НалогообложениеНДСПоУмолчанию.ТолькоПросмотр    = НЕ ВлияющиеРеквизитыДоступны;
	...
	Иначе
		...
		Если ТипОснованияПеремещениеТоваров(Форма.Объект.Ссылка)
			...
			Форма.Элементы.ОформитьДокументы.Заголовок = НСтр("ru='Оформление документов доступно при указанном способе отражения и наличии расхождений в статусах ""Отрабатывается"" и ""Отработано""';uk='Оформлення документів доступно при зазначеному способі відображення і наявності розбіжностей в статусах ""Відпрацьовується"" і ""Відпрацьовано""'");
		Иначе
...
|;uk='Електронний документ з придбання підтверджений. Можливе тільки коригування придбання, 
|але їх використання відключено.'");
			                             
		Иначе
			
			ТекстПояснения = НСтр("ru='Электронный документ по приобретению подтвержден. Возможна только корректировка приобретения.';uk='Електронний документ з придбання підтверджений. Можливе тільки коригування придбання.'");
	...
	Если Форма.ДоступныеСпособыОтработкиАктаПоСостояниюЭДО.НайтиПоЗначению(
			...
			ТекстПояснения = НСтр("ru='Электронный документ по приобретению был отклонен. Возможно только изменение поступления.';uk='Електронний документ з придбання був відхилений. Можлива тільки зміна надходження.'");
	
	КонецЕсли;
	
	Если ТребуетсяСообщениеОНесоответствииСпособа Тогда
		
		СообщениеОНесоответствииСпособа = НСтр("ru='Текущий способ отражения некорректен. Изменить его можно в статусе ""Не согласовано"" или ""Обеспечивается""';uk='Поточний спосіб відображення некоректний. Змінити його можна в статусі ""Не погоджено"" або ""Забезпечується""'");
...
|;uk='Документ-підстава бере участь в ЕДО, але покупець поки ні відхилив, ні підтвердив документ. 
|Відпрацювання розбіжностей недоступне.'");
...
|но их использование отключено.'
|;uk='Електронний документ з реалізації підтверджений покупцем. Можливе тільки коригування реалізації, 
|але їх використання відключено.'");
			                             
		Иначе
			
			ТекстПояснения = НСтр("ru='Электронный документ по реализации подтвержден покупателем. Возможна только корректировка реализации.';uk='Електронний документ з реалізації підтверджений покупцем. Можливе тільки коригування реалізації.'");
	...
	ИначеЕсли Форма.ДоступныеСпособыОтработкиАктаПоСостояниюЭДО.НайтиПоЗначению(
			...
			ТекстПояснения = НСтр("ru='Электронный документ по реализации отклонен покупателем. Возможно только изменение реализации.';uk='Електронний документ з реалізації відхилений покупцем. Можлива тільки зміна реалізації.'");
