...
Функция ДанныеДляОформленияПоЗаказам(
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		ИначеЕсли Строка.СостояниеНакладной = 4 Тогда
			ТекстОшибки = НСтр("ru='Распоряжение %1 является накладной. Невозможно оформить накладную по накладной.';uk='Розпорядження %1 є накладною. Неможливо оформити накладну за накладною.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Строка.Ссылка);
		КонецЕсли;
	...
	Если ГруппировкиРаспоряжений.Количество() = 0 И ИдентификаторыСтрок.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru='Не выбрано ни одного документа, который можно было бы оформить по заказам.';uk='Не вибрано жодного документа, який можна було б оформити за замовленнями.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, ПолеОшибки, ТекстОшибки, Неопределено);
...
Функция ДанныеДляОформленияПоОрдерам(
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если Строка.НакладнаяНаОтгрузку И Строка.НакладнаяНаПриемку Тогда
			ПоОрдерам = НСтр("ru='по отгрузке (приемке)';uk='по відвантаженню (прийманню)'");
		ИначеЕсли Строка.НакладнаяНаОтгрузку Тогда
			ПоОрдерам = НСтр("ru='по отгрузке';uk='за відвантаженням'");
		ИначеЕсли Строка.НакладнаяНаПриемку Тогда
			ПоОрдерам = НСтр("ru='по приемке';uk='за прийманням'");
		КонецЕсли;
		...
		Если (Строка.СостояниеОрдераНаОтгрузку = 0 Или Не Строка.НакладнаяНаОтгрузку)
			...
			ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить накладную %2, т.к. ордера соответствуют накладным.';uk='Для розпорядження %1 неможливо оформити накладну %2, тому що ордери відповідають накладними.'");
		...
		ИначеЕсли (Строка.СостояниеОрдераНаОтгрузку = 1 Или Не Строка.НакладнаяНаОтгрузку)
			...
			ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить накладную %2, т.к. нет ни одного ордера.';uk='Для розпорядження %1 неможливо оформити накладну %2, тому що немає жодного ордера.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Строка.Ссылка, ПоОрдерам);
		ИначеЕсли (Строка.СостояниеОрдераНаОтгрузку = 4 Или Не Строка.НакладнаяНаОтгрузку)
			...
			ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить накладную %2, т.к. ордера не используются.';uk='Для розпорядження %1 неможливо оформити накладну %2, тому що ордери не використовуються.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Строка.Ссылка, ПоОрдерам);
		КонецЕсли;
	...
	Если ОдновременноОформляемыеНакладные.Количество() > 1 Или ЗначениеЗаполнено(ГруппировкиРаспоряжений) Тогда
		...
		Для каждого ДанныеОформляемыхНакладных Из ОдновременноОформляемыеНакладные Цикл
			...
			ТекстОшибки = НСтр("ru='Одновременное оформление %1 накладной %2 вместе с другими распоряжениями невозможно. Выберите накладную отдельно.';uk='Одночасне оформлення %1 накладної %2 разом з іншими розпорядженнями неможливо. Виберіть накладну окремо.'");
	...
	Если ГруппировкиРаспоряжений.Количество() = 0 И ИдентификаторыСтрок.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru='Не выбрано ни одного документа, который можно было бы оформить по ордерам.';uk='Не вибрано жодного документа, який можна було б оформити за ордерами.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, ПолеОшибки, ТекстОшибки, Неопределено);
