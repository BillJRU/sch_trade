...
Процедура ИнициализироватьВедениеПротоколаОбмена(КомпонентыОбмена, ИмяФайлаПротоколаОбмена) Экспорт
	...
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';uk='Помилка при спробі запису у файл протоколу даних: %1. Опис помилки: %2'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Направление: %1.
            ...
            |%2.'
            |;uk='Напрямок: %1. Обробник: ПередКонвертацией.
            |
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ru='При формировании сообщения обмена данными произошли ошибки. Подробнее см. в журнале регистрации.';uk='При формуванні повідомлення обміну даними відбулися помилки. Детальніше див. в журналі реєстрації.'");
	КонецЕсли;
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неизвестный тип свойства <%1>. Тип объекта: %2';uk='Невідомий тип властивості <%1>. Тип об''єкта: %2'"),
				Свойство.Имя,
				Строка(ТипXDTO));
		КонецЕсли;
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |
                |%3'
                |;uk='Помилка формування об''єкта XDTO: Тип властивості <%1>. Ім''я властивості: <%2>.
                |
                |%3'"),
				ТипСвойства,
				Свойство.Имя,
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.';uk='Спроба зміни неподілених даних (%1: %2) в розділеному режимі.'"),
			...
			Попытка
				...
				Если ЗначениеЗаполнено(КомпонентыОбмена.ИдентификаторКорреспондента)
					...
					ИнформационноеСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Изменен идентификатор узла корреспондента (%1 -> %2).';uk='Змінено ідентифікатор вузла кореспондента (%1 -> %2).'"),
...
Процедура ПроверитьОбъектXDTOПоСхеме(ОбъектXDTO, ТипXDTO, Контекст, Отказ, ОписаниеОшибки)
    ...
    |;uk='Не вдалося виконати конвертацію в об''єкт формату ""%1"":
    |%2 
    |
		...
		Если Не ОбъектXDTO.Установлено(Свойство) Тогда
			Если Свойство.НижняяГраница = 1
				...
				СообщениеОбОшибке = Отступ + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1: не заполнено обязательное поле.';uk='%1: не заповнено обов''язкове поле.'"),
					Свойство.Имя);
		...
		Иначе
			...
			Если ТипЗнч(ЗначениеСвойстваXDTO) = Тип("ЗначениеXDTO") Тогда
				...
				Исключение
					Отступ = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
					СообщениеОбОшибке = Отступ + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='%1: значение поля не соответствует заданным ограничениям.';uk='%1: значення поля не відповідає заданим обмеженням.'"),
						Свойство.Имя);
					СтекОшибок.Добавить(СообщениеОбОшибке);
				КонецПопытки;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.';uk='Помилка конвертації властивості об''єкта XDTO, ім''я властивості: <%1>.'"), ПКС.СвойствоКонфигурации)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(УдалитьНепосредственно,
				НСтр("ru='Попытка удаления неразделенных данных (%1: %2) в разделенном режиме.';uk='Спроба видалення нерозділених даних (%1: %2) в розділеному режимі.'"),
				НСтр("ru='Попытка пометки на удаление неразделенных данных (%1: %2) в разделенном режиме.';uk='Спроба позначки на видалення нерозділені даних (%1: %2) в розділеному режимі.'")),
...
Функция ПредставлениеОбъектаДляПротокола(Объект, МетаданныеДляПредставленияСтруктуры = Неопределено)
	...
	ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, %2 (%3)';uk='%1, %2 (%3)'"),
...
Функция ПредставлениеКоллекцииСвойствДляПротокола(КоллекцияСвойств)
	...
	Если КоллекцияСвойств.Свойство("Номер", ЗначениеСвойства)
		...
		Если КоллекцияСвойств.Свойство("Дата", ЗначениеСвойстваДата)
			...
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='№%1 от %2';uk='№%1 від %2'"), ЗначениеСвойства, Формат(ЗначениеСвойстваДата, "ДЛФ=D")));
		Иначе
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='№%1';uk='№%1'"), ЗначениеСвойства));
		КонецЕсли;
	...
	ИначеЕсли КоллекцияСвойств.Свойство("Дата", ЗначениеСвойства)
		...
		ЭлементыПредставления.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='от %1';uk='від %1'"), ЗначениеСвойства));
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неканоническое представление версии формата обмена: <%1>.';uk='Неканонічне представлення версії формату обміну: <%1>.'"), Версия);
	КонецЕсли;
