...
|в зону приемки.'
|;uk='Відібрану кількість товарів неможливо розмістити у відповідності до складського завдання. 
...
|""%1"".'"),
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='При подключении оборудования произошла ошибка.';uk='При підключенні обладнання сталася помилка.'",ОбщегоНазначения.КодОсновногоЯзыка()), 
...
// см. Обработка.РабочееМестоРаботникаСклада.Формы.ФормаРабочегоМеста
// 
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма рабочего места работника склада, которая содержит:
// 		* Элементы - ВсеЭлементыФормы - элементы формы рабочего места работника склада, которые содержат:
...
Процедура ПерейтиНаСтраницу(Форма, ИмяСтраницы) Экспорт
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВводКоличества Тогда
		...
		Если Не ЭтоТара(ТекущаяСтрокаСканирования)
			...
			Элементы.НадписьЕдиницаИзмеренияВес.Заголовок = НСтр("ru='Вес';uk='Вага'") + ", " + ?(РежимВебСервиса, 
		...
		Иначе
			...
			Элементы.НадписьЕдиницаИзмеренияВес.Заголовок = НСтр("ru='Вес';uk='Вага'");
		КонецЕсли;
		...
		Если Не ЭтоТара(ТекущаяСтрокаСканирования)
			...
			Элементы.НадписьЕдиницаИзмеренияОбъем.Заголовок = НСтр("ru='Объем';uk='Об''єм'") + ", " + ?(РежимВебСервиса, 
		...
		Иначе
			...
			Элементы.НадписьЕдиницаИзмеренияОбъем.Заголовок = НСтр("ru='Объем';uk='Об''єм'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыборЗначения Тогда
		...
		ТекстЗаголовка = НСтр("ru='Ввод штрихкода';uk='Введення штрихкоду'");
		УстановитьЗаголовокФормы(Форма, ТекстЗаголовка);
...
|відбір товарів в інших комірках'"));
		...
		Если Не (КоличествоЯчеек = НомерЯчейки) Тогда
			...
			ТекстВопроса = НСтр("ru='Задание по отбору не закончено.';uk='Завдання з відбору не закінчене.'");
...
|не закончено.'
|;uk='Завдання з розміщення товарів 
|не закінчено.'");
			...
			ИначеЕсли Не ЭтоПоследнийТоварВЯчейке Тогда
				...
				ТекстВопроса = НСтр("ru='Задание по размещению не закончено.';uk='Завдання з розміщення не закінчене.'");
...
|не закінчено.'");
		...
		Иначе
			...
			Если Не ЭтоПоследняяЯчейка Тогда
				...
				ТекстВопроса = НСтр("ru='Задание по отбору товаров не закончено.';uk='Завдання з відбору товарів не закінчено.'");
			...
			ИначеЕсли Не ЭтоПоследнийТоварВЯчейке Тогда
				...
				ТекстВопроса = НСтр("ru='Задания по отбору не закончено.';uk='Завдання з відбору не закінчене.'");
...
|к размещению товаров?'
|;uk='Завершити відбір і перейти 
|до розміщення товарів?'"),
							НСтр("ru='Завершить отбор?';uk='Завершити відбір?'")));
...
|;uk='Товар ""[Товар]""  
|в комірці ""[Ячейка]"" 
|останній в завданні на [ТипЗадания]. 
|
|[ЗавершитьОтбор]'"), 
						ПараметрыЗамены);
...
Процедура ДобавитьТекстНадписиЭлемента(Форма, ИдНадписи)
	...
	ИначеЕсли ИдНадписи = "ОшибкаСерияНеУникальна" Тогда
		Если НЕ (РазрешениеЭкрана = Разрешение240х320) Тогда
			ТекстНадписи = НСтр("ru='Ошибка: серия товара не уникальна.';uk='Помилка: серія товару не унікальна.'");
		Иначе
			ТекстНадписи = НСтр("ru='Ошибка: серия не уникальна.';uk='Помилка: серія не унікальна.'");
		КонецЕсли;
	...
	ИначеЕсли ИдНадписи = "ПредупреждениеТоварОтсутствуетВЗадании" Тогда
		...
		Иначе
			ТекстНадписи = НСтр("ru='Товара нет в зад., отл. товар.';uk='Товару немає в завд., відкл. товар.'");
		КонецЕсли;
	...
	ИначеЕсли ИдНадписи = "ПодсказкаОтскШКЯчейкиКПересчету" Тогда
		Если НЕ (РазрешениеЭкрана = Разрешение240х320) Тогда
			ТекстНадписи = НСтр("ru='Отсканируйте штрихкод или введите адрес ячейки.';uk='Сканувати штрихкод, або введіть адресу комірки.'");
		Иначе
...
// Параметры:
//	СтрокаСканирования - Структура, СтрокаТаблицыЗначений - строка сканирования.
//	Товар - Структура - сведения о товаре, см. РабочееМестоРаботникаСкладаСервер.Товар.
//
...
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма рабочего места работника склада.
//	Серия - Структура - сведения о серии номенклатуры, см. РабочееМестоРаботникаСкладаСервер.СерияНоменклатуры.
...
//
// Параметры:
//	ПараметрыТовара - Структура - сведения о товаре, см. РабочееМестоРаботникаСкладаСервер.Товар.
//
// Возвращаемое значение:
//	Структура - дополненные сведения о товаре, см. РабочееМестоРаботникаСкладаСервер.Товар.
//
...
Процедура ЗавершитьВыполнениеЗаданияПриемка(Форма)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при завершении задания на приемку.';uk='Помилка при завершенні завдання на приймання.'",ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, 
			Задание.Метаданные(), 
			Задание, 
			КомментарийЖурнала, 
...
Процедура ВывестиИнформациюОТоваре(Форма, ПараметрыТовара)
	...
	Если Не ЗначениеЗаполнено(ПараметрыТовара.Описание) Тогда
		ОбластьОписание.Параметры.Заполнить(Новый Структура("Описание", НСтр("ru='<нет>';uk='<ні>'")));
	КонецЕсли;
...
Процедура УстановитьЗаголовокКоличества(Форма)
	...
	Если ЭтоНаборТоваров Тогда
		...
		Иначе
			Элементы.НадписьВзяли.Заголовок = НСтр("ru='Добавлено:';uk='Додано:'");
		КонецЕсли;
	...
	ИначеЕсли ТипЗадания = "Пересчет" Тогда
		...
		Иначе
			Элементы.НадписьВзяли.Заголовок = НСтр("ru='Отсканировано:';uk='Відскановано:'");
		КонецЕсли;
		Элементы.СканированиеВвестиКоличество.Заголовок = ТекущаяСтрокаСканирования.КоличествоУпаковокОтсканировано;
	...
	ИначеЕсли ТипЗадания = "ПроверкаОтбора" Тогда
		...
		Иначе
			Элементы.НадписьВзяли.Заголовок = НСтр("ru='В наличии:';uk='В наявності:'");
		КонецЕсли;
		Элементы.СканированиеВвестиКоличество.Заголовок = ТекущаяСтрокаСканирования.КоличествоУпаковокОтсканировано;
...
//
// Параметры:
//	СтрокаСканирования - Структура - описание сканируемой строки складского задания,
//											см. РабочееМестоРаботникаСкладаСервер.СтрокаСканирования.
//
// Возвращаемое значение:
...
//	ТекущаяСтрокаСканирования - Структура - описание сканируемой строки складского задания,
//											см. РабочееМестоРаботникаСкладаСервер.СтрокаСканирования.
//
...
Процедура ДополнитьНеОтобраннымиТоварамиТаблицуДляСканированияТоваров(Форма, ЭтоПереходКСледующейЯчейке)
	...
	Если СтруктураРезультата = Неопределено Тогда
		...
		ИмяСобытия = НСтр("ru='Ошибка при подборе ячеек для выполнения складской операции отбора (размещения) товаров';uk='Помилка при підборі комірок для виконання складської операції відбору (розміщення) товарів'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция УстановитьПредставлениеСерии(СтрокаСканирования)
	...
	Представление = НСтр("ru='%НомерСерии%';uk='%НомерСерии%'");
	...
	Если СтрокаСканирования.ИспользоватьСрокГодностиСерии Тогда
		...
		Представление = НСтр("ru='%НомерСерии% до %ГоденДо%';uk='%НомерСерии% до %ГоденДо%'");
...
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма рабочего места работника склада.
//	Серия - Структура - описание серии товара, см. РабочееМестоРаботникаСкладаСервер.СерияНоменклатуры.
// 
// Возвращаемое значение:
...
//
// Параметры:
//	Товар - Структура - описание сканируемого товара складского задания, см. РабочееМестоРаботникаСкладаСервер.Товар.
//
...
// Параметры:
//	Штрихкод - Строка - отсканированный штрихкод.
//	Товар - Структура, Неопределено - описание сканируемого товара складского задания,
//										см. РабочееМестоРаботникаСкладаСервер.Товар.
//
// Возвращаемое значение:
