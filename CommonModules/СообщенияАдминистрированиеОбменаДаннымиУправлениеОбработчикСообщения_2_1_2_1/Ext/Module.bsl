...
Процедура ПодключитьКорреспондента(Сообщение, Отправитель)
	...
	Если ЭтаКонечнаяТочка.Пустая()
		...
		ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.';uk='Кінцева точка не відповідає очікуваній. Код очікуваної кінцевої точки %1. Код поточної кінцевої точки %2.'"),
			Тело.SenderId,
	...
	Если Корреспондент.Пустая() Тогда // Подключаем конечную точку корреспондента.
		...
		Если Отказ Тогда // Отправляем сообщение в менеджер сервиса об ошибке.
			...
			ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.';uk='Помилка підключення кінцевої точки кореспондента обміну. Код кінцевої точки кореспондента обміну %1.'"),
				Тело.RecipientId);
		...
		Если ПодключенныйКорреспондентКод <> Тело.RecipientId Тогда
			...
			ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |Настройки підключення вебсервісу не відповідають очікуваним.
...
Функция СобытиеЖурналаРегистрацииПодключениеКорреспондента()
	...
	Возврат НСтр("ru='Обмен данными.Подключение корреспондента обмена';uk='Обмін даними.Підключення кореспондента обміну'",ОбщегоНазначения.КодОсновногоЯзыка());
