...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	СтрокаСообщения = НСтр("ru='Начало процесса обмена данными для узла %1';uk='Початок процесу обміну даними для вузла %1'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		...
		Результат = НСтр("ru='30 минут назад';uk='30 хвилин тому'");
...
//	  * ИмяСервера1СПредприятия                     - Строка - Имя сервераплатформы BAF;
//	  * ИмяИнформационнойБазыНаСервере1СПредприятия - Строка - Имя информационной базы на сервереплатформы BAF;
	...
	Попытка
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не подходящий режим вызова функции: %1';uk='Не відповідний режим виклику функції: %1'"), Контекст.Режим);
		КонецЕсли;
			...
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОчередьЗаданий") Тогда
					...
					КлючЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Обмен данными с внешней системой (%1)';uk='Обмін даними із зовнішньою системою (%1)'"),
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Попытка
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
			...
			Если УзелИнформационнойБазы <> Неопределено Тогда
				...
				Если ПравилаРегистрацииОбновлены = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не выполнено обновление кэша правил регистрации данных для плана обмена ""%1""';uk='Не виконано оновлення кешу правил реєстрації даних для плану обміну ""%1""'"),
						ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелИнформационнойБазы));
				КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обмен данными.Загрузка приоритетных данных';uk='Обмін даними.Завантаження пріоритетних даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ,
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Транспорт сообщений обмена';uk='Транспорт повідомлень обміну'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОписаниеВариантаНастройкиОбменаПоУмолчанию(ИмяПланаОбмена) Экспорт
	...
	ЗаголовокФормыПомощника = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Синхронизация данных с %1 (настройка)';uk='Синхронізація даних з %1 (настройка)'"),
		СинонимПланаОбмена);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "3.0.1.91";
	Обработчик.Комментарий =
		НСтр("ru='Первоначальное заполнение настроек обмена данными XDTO.';uk='Початкове заповнення настройок обміну даними XDTO.'");
	...
	Обработчик.Комментарий =
		НСтр("ru='Перенос результатов выполнения обменов данными в новый регистр.';uk='Перенесення результатів виконання обмінів даними в новий регістр.'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ru='Синхронизация данных с другими программами';uk='Синхронізація даних з іншими програмами'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Установка подключения к web-сервису';uk='Обмін даними. Встановлення підключення до web-сервісу'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Загрузка правил';uk='Обмін даними. Завантаження правил'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Создание обмена данными';uk='Обмін даними.Створення обміну даними'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеОбменаДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Удаление обмена данными';uk='Обмін даними.Видалення обміну даними'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииРегистрацияДанныхДляНачальнойВыгрузки() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Регистрация данных для начальной выгрузки';uk='Обмін даними.Реєстрація даних для початкової вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхДляСопоставления() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Выгрузка данных для сопоставления';uk='Обмін даними.Вивантаження даних для співставлення'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Удаление временного файла';uk='Обмін даними.Вилучення тимчасового файлу'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными';uk='Обмін даними'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхВСервисПередачиФайлов() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Сервис передачи файлов.Выгрузка данных';uk='Обмін даними.Сервіс передачі файлів.Вивантаження даних'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзСервисаПередачиФайлов() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Сервис передачи файлов.Загрузка данных';uk='Обмін даними.Сервіс передачі файлів.Завантаження даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("ru='Начало процесса обмена данными по настройке %1';uk='Початок процесу обміну даними по настройці %1'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ru='При выгрузке данных возникли ошибки во второй информационной базе: %1';uk='При вивантаженні даних виникли помилки в другій інформаційній базі: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ДлительнаяОперация Тогда
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание получения данных от базы-корреспондента...';uk='Очікування одержання даних від бази-кореспондента...'",ОбщегоНазначения.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
		Возврат Результат;
	КонецЕсли;
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1';uk='Виникли помилки при отриманні повідомлення обміну із сервісу передачі файлів: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		Отказ = Истина;
		Сообщение = НСтр("ru='При получении сообщения обмена возникли ошибки: %1';uk='При одержанні повідомлення обміну виникли помилки: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И Не Отказ Тогда
				...
				Попытка
					...
					Иначе
						...
						Если ПараметрыОбмена.ДлительнаяОперация Тогда
							ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание загрузки данных в базе-корреспонденте...';uk='Очікування завантаження даних у базі-кореспонденті...'",ОбщегоНазначения.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
							Возврат;
						КонецЕсли;
					КонецЕсли;
		...
		Если ПроверятьНаличиеУзлаВКорреспонденте Тогда
			...
			Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
				...
				Если СтруктураПараметров.Ошибка Тогда
					ЕстьОшибки = Истина;
					НСтрока = НСтр("ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';uk='Помилка при переході на синхронізацію даних через універсальний формат: %1. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';uk='Не задані правила конвертації в другій інформаційній базі для плану обміну %1. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ИнициализироватьЧтениеСообщенияДляСтандартнойЗагрузки(ПараметрыЗагрузки, РезультатВыполненияОбмена, СообщениеОбОшибке)
	...
	Если ЧтениеСообщения.Отправитель <> ПараметрыЗагрузки.УзелИнформационнойБазы Тогда
		...
		СообщениеОбОшибке = НСтр("ru='Сообщение обмена содержит данные для другого узла информационной базы.';uk='Повідомлення обміну містить дані для іншого вузла інформаційної бази.'",ОбщегоНазначения.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
...
Функция ЭтоОшибкаНомерСообщенияМеньшеИлиРавенНомеруРанееПринятогоСообщения(ОписаниеОшибки)
	...
	Возврат СтрНайти(НРег(ОписаниеОшибки), НРег(НСтр("ru='Номер сообщения меньше или равен';uk='Номер повідомлення менше або дорівнює'"))) > 0;
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстИсключения = НСтр("ru='Недостаточно прав для выполнения действия.';uk='Недостатньо прав для виконання дії.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан узел информационной базы, с которой нужно производить обмен информацией. Обмен отменен.';uk='Не заданий вузол інформаційної бази, з якої потрібно проводити обмін інформацією. Обмін скасований.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
		
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан вид транспорта обмена. Обмен отменен.';uk='Не заданий вид транспорту обміну. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ДействиеПриОбмене) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';uk='Не зазначена виконувана дія (вивантаження / завантаження). Обмін відмінний.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Узел информационной базы помечен на удаление. Обмен отменен.';uk='Вузол інформаційної бази позначений на вилучення. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';uk='Не можна організувати обмін даними з поточним вузлом інформаційної бази. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Один из узлов обмена имеет пустой код. Обмен отменен.';uk='Один з вузлів обміну має порожній код. Обмін скасовано.'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження вивантаження не існує. Обмін відмінний.'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження завантаження не існує. Обмін відмінний.'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';uk='Не задані правила конвертації для плану обміну %1. Вивантаження даних скасоване.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';uk='Не задано правила конвертації для плану обміну %1. Завантаження даних скасоване.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(
	...
	Если Не ПустаяСтрока(ТекстСообщения) Тогда
		СтрокаСообщения = НСтр("ru='Не удалось провести документ %1, полученный из другой информационной базы.
            ...
            |;uk='Не вдалося провести документ %1, отриманий з іншої інформаційної бази.
            |Через: %2.
            |Можливо, не заповнені всі реквізити, обов''язкові до заповнення.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда
		СтрокаСообщения = НСтр("ru='%1, %2';uk='%1, %2'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		СтрокаСообщения = НСтр("ru='%1, %2; Объектов обработано: %3';uk='%1, %2; Об''єктів оброблено: %3'",ОбщегоНазначения.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%3'
            |;uk='Помилка при розпакуванні файлів архіву ""%1"" в каталог ""%2"" через:
            |%3'"),
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%3'
            |;uk='Помилка при запаковуванні файлів ""%1"" в архів ""%2"" через:
            |%3'"),
			МаскаУпаковкиФайлов,
			ПолноеИмяФайлаАрхива,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция КлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';uk='Обмін даними.[ИмяПланаОбмена].[ДействиеПриОбмене]'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаСообщения = НСтр("ru='Для планов обмена %1 используются правила обмена, загруженные из файла.
                ...
                |Для попередження можливого виникнення помилок при роботі з програмою рекомендується актуалізувати правила обміну з файлу.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';uk='Користувачу, який вказаний для підключення до іншї програми, повинні бути призначені ролі ""Адміністратор системи"" і ""Повні права""'");
...
Процедура ИнициализироватьWSПроксиДляУправленияОбменомДанными(Прокси,
	...
	Если Не СостояниеНастройки.НастройкаСуществует Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена настройка синхронизации данных ""%1"" с идентификатором ""%2"".';uk='Не знайдена настройка синхронізації даних ""%1"" з ідентифікатором ""%2"".'"),
			СтруктураНастроек.ИмяПланаОбмена,
			СтруктураНастроек.ТекущийУзелПланаОбменаКод);
		Отказ = Истина;
	КонецЕсли;
		...
		Если СостояниеНастройки.НастройкаСуществует Тогда
			...
			Иначе
				...
				СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='У програмі ""%1"" виконується оновлення настройок синхронізації.
                    |Завантаження даних скасоване. Необхідно запустити синхронізацію даних повторно.'"),
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент,
	...
	СтруктураНастроекОбмена.Вставить("КлючСообщенияЖурналаРегистрации",
		НСтр("ru='Обмен данными.Проверка подключения';uk='Обмін даними.Перевірка підключення'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СтруктураНастроекОбмена.Вставить("ТекущийУзелПланаОбмена",
		ОбменДаннымиПовтИсп.ПолучитьЭтотУзелПланаОбмена(СтруктураНастроекОбмена.ИмяПланаОбмена));
	СтруктураНастроекОбмена.Вставить("ТекущийУзелПланаОбменаКод",
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ru='Настройка синхронизации данных';uk='Настройка синхронізації даних'"));
...
Функция ОписаниеСтандартногоВариантаБезДополнения()
	...
	Результат.Вставить("Пояснение",     НСтр("ru='Будут отправлены только данные согласно общим настройкам.';uk='Будуть відправлені тільки дані відповідно до загальних настройок'"));
...
Функция ОписаниеСтандартногоВариантаВсеДокументы()
	...
	Результат.Вставить("Заголовок",     "");
	Результат.Вставить("Пояснение",     НСтр("ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';uk='Додатково будуть відправлені всі документи за період, що задовольняють умовам відбору.'"));
...
Функция ОписаниеСтандартногоВариантаПроизвольныйОтбор()
	...
	Результат.Вставить("Заголовок",     "");
	Результат.Вставить("Пояснение",     НСтр("ru='Дополнительно будут отправлены данные согласно отбору.';uk='Додатково будуть відправлені дані відповідно до відбору.'"));
