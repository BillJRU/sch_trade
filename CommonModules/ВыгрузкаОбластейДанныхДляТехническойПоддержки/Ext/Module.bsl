...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Если РеквизитыОбъектаИмеющиеСсылкиНаИсключаемыеИзВыгрузки = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаОбластейДанныхДляТехподдержки.ПередВыгрузкойОбъекта()!';uk='Об''єкт метаданих %1 не може бути оброблений обробником ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта()'"),
