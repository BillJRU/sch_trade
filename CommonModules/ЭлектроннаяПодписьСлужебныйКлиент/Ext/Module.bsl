...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения сертификата';uk='Виберіть файл для збереження сертифіката'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru='Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*';uk='Файли сертифікатів (*.cer)|*.cer|Все файли (*.*)|*.*'");
...
Процедура СохранитьПодпись(АдресПодписи) Экспорт
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения подписи';uk='Виберіть файл для збереження підпису'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл сертификата (только для шифрования)';uk='Оберіть файл сертифіката (тільки для шифрування)'");
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы (*.*)|*.*';uk='Сертифікат X.509 (*.cer;*.Crt)|*.cer;*.Crt|Всі файли (*.*)|*.*'");
	...
	Если ТекущиеСвойства.ВыполнятьВМоделиСервиса
		...
		Элементы.ПояснениеУсиленногоПароля.Подсказка =
			НСтр("ru='Облачный сервис вышлет одноразовый пароль по SMS или E-Mail,
                       ...
                       |;uk='Хмарний сервіс вишле одноразовий пароль по SMS або E-Mail,
                       |який потрібно буде ввести після отримання.'");
	...
	ИначеЕсли ДополнительныеПараметры.УсиленнаяЗащитаЗакрытогоКлюча Тогда
		...
		Элементы.ПояснениеУсиленногоПароля.Подсказка =
			НСтр("ru='Пароль запрашивает программа электронной подписи, а не программа BAS,
                       ...
                       |так як для вибраного сертифіката вказаний посилений захист закритого ключа.'");
...
Процедура НачатьИнициализациюСертификатаКриптографииДляПроверкиПодписиCMS(ОповещенияПриЗавершении, КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии)
	...
	ВызватьИсключение НСтр("ru='Проверка подписи CMS не поддерживается в компоненте XMLDSig.';uk='Перевірка підпису CMS не підтримуються в компоненті XMLDSig.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Операция невозможна. Требуется установка компоненты XMLDSig.';uk='Операція неможлива. Потрібне встановлення компоненти XMLDSig.'"));
...
Процедура ПослеУстановкиСвойстваЗапретитьПользовательскийИнтерфейс(Контекст) Экспорт
	...
	ИначеЕсли Контекст.ТипПодписи = "CMS" Тогда
		...
		Если Контекст.Режим = "РежимПроверка" Тогда
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Проверка подписи CMS не поддерживается в компоненте XMLDSig.';uk='Перевірка підпису CMS не підтримуються в компоненті XMLDSig.'"));
		ИначеЕсли Контекст.Режим = "РежимПодписание" Тогда
	...
	Иначе
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Не установлен тип подписи компоненты XMLDSig.';uk='Не встановлено тип підпису компоненти XMLDSig.'"));
	КонецЕсли;
...
Процедура НачатьПодписаниеCMS_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПараметрыПодключения = ОбщегоНазначенияКлиент.ПараметрыПодключенияКомпоненты();
	ПараметрыПодключения.ТекстПояснения = НСтр("ru='Для подписания CMS необходима установка компоненты XMLDSig';uk='Для підписання CMS необхідне встановлення компоненти XMLDSig'");
...
Процедура НачатьПодписаниеCMS(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода CMSSign компоненты XMLDSig.';uk='Помилка виклику методу CMSSign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеВыполненияCMSSign(АтрибутSignatureValue, Параметры, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(АтрибутSignatureValue) Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода CMSSign произошла ошибка:';uk='При виконанні методу CMSSign сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеПослеВыгрузкиСертификатаВМоделиСервиса(РезультатПоиска, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(РезультатПоиска.Сертификат) Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция) + Символы.ПС
				+ НСтр("ru='Сертификат отсутствует в облачном сервисе (возможно удален).';uk='Сертифікат відсутній в хмарному сервісі (можливо вилучений).'"));
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьТехническуюИнформацию(Причина,
	...
	СопроводительныйТекст = Причина + Символы.ПС + НСтр("ru='Информация о компьютере:';uk='Інформація про комп''ютер:'")
...
Функция ДиагностическаяИнформацияОКомпьютере()
	...
	Возврат НСтр("ru='Клиент:';uk='Клієнт:'") + Символы.ПС
...
Функция ДиагностическаяИнформацияОПрограммах(Оповещение)
	...
	Заголовок = Символы.ПС + НСтр("ru='Программы на клиенте:';uk='Програми на клієнті:'") + Символы.ПС;
	...
	Если ИспользуемыеПрограммы.Количество() = 0 Тогда
		ВыполнитьОбработкуОповещения(Оповещение, Заголовок + НСтр("ru='Не используется';uk='Не використовується'"));
	КонецЕсли;
