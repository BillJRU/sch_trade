////////////////////////////////////////////////////////////////////////////////
// Подсистема "Интеграция с BAS Документооборотом"
...
Процедура ОткрытьОбъект(ТипОбъекта, ID, ВладелецФормы = Неопределено, 
	...
	Иначе
		Если ЗначениеЗаполнено(ТипОбъекта) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Просмотр объектов такого типа не поддерживается в этой версии BAS Документооборота.';uk='Перегляд об''єктів такого типу не підтримується в цій версії BAS Документообігу.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыбратьЗначениеПоТипуОбъекта(ТипОбъектаВыбора, Отбор = Неопределено, ВыбранноеЗначение = "", 
	...
	СоответствиеЗаголовков.Вставить("DMProject", 		НСтр("ru='Выбор проекта';uk='Вибір проєкту'"));
	СоответствиеЗаголовков.Вставить("DMWorkType", 		НСтр("ru='Выбор вида работ';uk='Вибір виду робіт'")); 
	СоответствиеЗаголовков.Вставить("DMProjectTask", 	НСтр("ru='Выбор проектной задачи';uk='Вибір проєктної задачі'"));
	...
	Иначе
		СоответствиеЗаголовков.Вставить("DMCorrespondent", 	НСтр("ru='Выбор контрагента';uk='Вибір контрагента'"));
	КонецЕсли;
	
	СоответствиеЗаголовков.Вставить("DMContactPerson", 	НСтр("ru='Выбор контактного лица';uk='Вибір контактної особи'"));
	СоответствиеЗаголовков.Вставить("DMSubdivision", 	НСтр("ru='Выбор подразделения';uk='Підрозділ відбір'"));
...
// Выполняет подключаемую команду интеграции с BAS Документооборотом из формы объекта или списка.
//
// Параметры:
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	ИначеЕсли Правила.Количество() = 0 Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Создать в BAS Документообороте';uk='Створити в BAS Документообігу'"));
	...
	Иначе // без открытия формы
		...
		Если ТипЗнч(ОбъектДОИлиСообщение) = Тип("Строка") Тогда // сообщение об ошибке
			...
			ПоказатьПредупреждение(ОписаниеОповещения, ОбъектДОИлиСообщение,,
				НСтр("ru='Не удалось записать объект BAS Документооборота';uk='Не вдалося записати об''єкт BAS Документообігу'"));
...
// Сохраняет имя пользователя и пароль для доступа к BAS Документообороту.
...
// Получает имя пользователя и пароль для доступа к BAS Документообороту.
	...
	Если ПарольСохранен Тогда
		...
		Исключение // файл удален или поврежден
			...
			ИнтеграцияС1СДокументооборотВызовСервера.ЗаписатьПредупреждение(
				НСтр("ru='Не найдены сохраненные учетные данные пользователя BAS Документооборота';uk='Не знайдені збережені облікові дані користувача BAS Документообігу'"));
		КонецПопытки;
		Если ПарольСохранен
...
// Проверяет подключение к BAS Документообороту, при необходимости запрашивая имя пользователя и пароль.
//
// Параметры:
//
...
Процедура ПроверитьПодключение(ОписаниеОповещения, ФормаВладелец = Неопределено,
	...
	ИначеЕсли ПовторноПоказыватьОкноАвторизации // проверим подключение с аутентификацией платформы BAF
...
Процедура ПоказатьВопросДаНет(ОписаниеОповещенияОЗавершении, ТекстВопроса,
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недопустимое значение кнопки по умолчанию: %1';uk='Неприпустиме значення кнопки по умовчанню: %1'"),
			КнопкаПоУмолчанию);
...
Процедура НачатьСогласование(Параметры) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотВызовСервера.ПользователюРазрешенЗапускСогласования(
		...
		Если ТекстПредупреждения = "" Тогда // текст по умолчанию
			ТекстПредупреждения = НСтр("ru='Недостаточно прав для запуска согласования в BAS Документообороте.';uk='Недостатньо прав для запуску погодження в BAS документообігу.'");
		КонецЕсли;
...
Процедура ПрерватьСогласование(Параметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru='В BAS Документообороте не запущено согласование.';uk='У BAS Документообігу не запущене погодження.'");
		ПоказатьПредупреждение(ОписаниеОповещенияПослеПредупреждения, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	Параметры.Вставить("Предмет", Результат);
	...
	Если Не ИнтеграцияС1СДокументооборотВызовСервера.ПользователюРазрешеноПрерываниеСогласования(
		...
		Если ТекстПредупреждения = "" Тогда // предупреждение по умолчанию
			ТекстПредупреждения = НСтр("ru='Недостаточно прав для прерывания согласования в BAS Документообороте.';uk='Недостатньо прав для переривання погодження в BAS документообігу.'");
		КонецЕсли;
		ПоказатьПредупреждение(ОписаниеОповещенияПослеПредупреждения, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСогласование(Параметры) Экспорт
	...
	Если ЗадачиСогласования.Количество() = 0 Тогда
		ТекстВопроса = НСтр("ru='В BAS Документообороте нет адресованных вам задач согласования.';uk='У BAS документообігу немає адресованих вам задач погодження.'");
		Кнопки = Новый СписокЗначений;
...
// Продолжает процесс добавления предмета после выбора объекта BAS Документооборота.
//
// Параметры:
...
Процедура ОстановитьПроцесс(Форма) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Процесс остановлен';uk='Процес зупинений'"),,
		Форма.Заголовок,
...
Процедура ПродолжитьПроцесс(Форма) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Процесс продолжен';uk='Процес продовжений'"),,
		Форма.Заголовок,
		БиблиотекаКартинок.Информация32);
				...
				Иначе // уверенности нет, требуется вмешательство пользователя
					...
					Если РазницаДат > 0 Тогда // файл на диске свежее
						...
						ПараметрыФормы.Вставить("Сообщение",
							НСтр("ru='Файл на диске новее, чем файл в BAS Документообороте.
|Возможно, он был изменен.
|
|Использовать существующий файл или заменить его на файл из BAS Документооборота,
|возможно, с потерей изменений?'
|;uk='Файл на диску новіше, ніж файл в BAS документообігу. 
|Можливо, він був змінений. 
|
|Використовувати існуючий файл або замінити його на файл з BAS Документообігу, 
|можливо, з втратою змін?'"));
					
					Иначе // файл на диске старше
						ПараметрыФормы.Вставить("Заголовок", НСтр("ru='В BAS Документообороте новый файл';uk='У BAS Документообігу новий файл'"));
						ПараметрыФормы.Вставить("Сообщение",
							НСтр("ru='Файл в BAS Документообороте новее, чем файл на диске.
|Возможно, он был изменен.
|
|Взять файл из BAS Документооборота, заменив файл на диске с потерей
|изменений, или использовать существующий файл?'
|;uk='Файл в BAS документообігу новіше, ніж файл на диску. 
|Можливо, він був змінений. 
|
|Взяти файл з BAS Документообігу, замінивши файл на диску з втратою 
|змін, або використовувати існуючий файл?'"));
		...
		Если ЗначениеЗаполнено(СведенияОРедактировании.РекомендуемаяПапка) Тогда
			...
			Иначе // попытаемся создать эту папку
				...
				Исключение // нет доступа или иные проблемы.
					...
					ИнтеграцияС1СДокументооборотВызовСервера.ЗаписатьПредупреждение(
						СтрШаблон(
							НСтр("ru='Нет доступа к рабочей папке: %1';uk='Немає доступу до робочої папки: %1'"),
...
|""%2"".'"),
	...
	КонецЦикла;
	Если Не Успех Тогда
		ВызватьИсключение НСтр("ru='Не удается создать файл в каталоге временных файлов.';uk='Не вдається створити файл у каталозі тимчасових файлів.'");
	КонецЕсли;
	...
	Если Результат = "ВзятьИзДокументооборота" Тогда
		...
		Исключение
			ПоказатьПредупреждение(, 
				НСтр("ru='Не удалось заменить файл на диске файлом из BAS Документооборота.
|Возможно, он заблокирован другим приложением.'
|;uk='Не вдалося замінити файл на диску файлом з BAS Документообігу. 
|Можливо, він заблокований іншим додатком.'"));
			Возврат;
		КонецПопытки;
...
|Возможно, он был изменен.
|
|Использовать существующий файл или взять файл из BAS Документооборота?'
|;uk='Файл на диску новіше, ніж файл в BAS документообігу. 
|Можливо, він був змінений. 
|
|Використовувати існуючий файл або взяти файл з BAS Документообігу?'"));
					ПараметрыФормы.Вставить("ПолныйПуть", СведенияОРедактировании.ПолныйПуть);
					ПараметрыФормы.Вставить("ДатаВремяНаДиске", ДатаВремяНаДиске);
					ПараметрыФормы.Вставить("ДатаВремяВДокументообороте", ДатаВремяВДокументообороте);
					ПараметрыФормы.Вставить("РазмерНаДиске", РазмерНаДиске);
...
//   ID - Строка - идентификатор документа BAS Документооборота
//   Тип - Строка - тип документа BAS Документооборота
//   Представление - Строка - представление документа BAS Документооборота
...
Процедура НачатьФормированиеПодписиОбъекта(Форма, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	Для каждого ОбъектДляПодписи Из МассивОбъектовДляПодписи Цикл
		...
		ИначеЕсли ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			...
			Если Строки.Количество() = 1 Тогда
				ПредставлениеФайла = НСтр("ru='Файл: ';uk='Файл: '") + Строки[0].Наименование;
				ДанныеДляПодписи.Вставить("Представление",
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru='Объекты (%1)';uk='Об''єкти (%1)'"));
...
Процедура ПрерватьПроцессПослеВопроса(Результат, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Процесс прерван';uk='Процес перерваний'"),,
