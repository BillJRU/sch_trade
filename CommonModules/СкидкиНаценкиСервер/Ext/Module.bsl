...
Процедура НастроитьКомандуПоказатьСообщения(Объект, ЭлементФормы, Видимость = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		ЭлементФормы.Заголовок = СтрШаблон(НСтр("ru='Показать сообщения (%1)';uk='Показати повідомлення (%1)'"), СтруктураСообщений.Сообщения.Количество());
	КонецЕсли;
...
Процедура НазначитьРучнуюСкидку(Объект, ИмяТЧ, СуммаСкидкиКРаспределению, ВходныеПараметры) Экспорт
	...
	ИначеЕсли ВходныеПараметры.Свойство("ИмяКолонкиКоличество") Тогда
	...
	// Schueco_SCT: режим расчета базы распределения ручной скидки.
	// 0 = от суммы без скидок (стандарт), 1 = от суммы с учетом автоматической скидки (опция диалога).
	БазаРасчетаРаспределения = 0;
	Если ТипЗнч(Данные) = Тип("Структура") И Данные.Свойство("БазаРасчетаРаспределения_SCT") Тогда
		БазаРасчетаРаспределения = Данные.БазаРасчетаРаспределения_SCT;
	КонецЕсли;

	// В Mode 1 пересобираем Ограничение.СуммаБезСкидки как сумму чистых баз (Гросс − Авто, не ниже 0)
	// чтобы инвариант "ОстатокСкидки/ОстатокГросса = const" выполнялся при распределении.
	Если БазаРасчетаРаспределения = 1 Тогда
		Для Каждого Ограничение Из Данные.Ограничения Цикл
			НеттоОграничения = 0;
			Для Каждого СтрокаТЧ Из СтрокиТабличнойЧасти Цикл
				ДанныеНомОграничения = Данные.Товары.Найти(СтрокаТЧ.Номенклатура, "Номенклатура");
				Если ДанныеНомОграничения = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если НЕ (ДанныеНомОграничения.ЦеноваяГруппа = Ограничение.ЦеноваяГруппа
						ИЛИ (Данные.Ограничения.Найти(ДанныеНомОграничения.ЦеноваяГруппа, "ЦеноваяГруппа") = Неопределено
							И Ограничение.ЦеноваяГруппа = Справочники.ЦеновыеГруппы.ПустаяСсылка())) Тогда
					Продолжить;
				КонецЕсли;
				ГроссСтроки = СтрокаТЧ[ИмяКоличества] * СтрокаТЧ.Цена;
				АвтоСтроки  = СтрокаТЧ.СуммаАвтоматическойСкидки;
				НеттоСтроки = ГроссСтроки - АвтоСтроки;
				Если НеттоСтроки < 0 Тогда
					НеттоСтроки = 0;
				КонецЕсли;
				НеттоОграничения = НеттоОграничения + НеттоСтроки;
			КонецЦикла;
			Ограничение.СуммаБезСкидки = НеттоОграничения;
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого СтрокаТЧ Из СтрокиТабличнойЧасти Цикл
		...
		Для Каждого Ограничение Из Данные.Ограничения Цикл
			...
			// Schueco_SCT: база распределения зависит от выбранного в диалоге режима.
			// Mode 0 (стандарт) — гросс. Mode 1 — гросс − авто (но не ниже 0, чтобы экстремальная авто-скидка не флипнула знак).
			Если БазаРасчетаРаспределения = 1 Тогда
				БазаРаспределения = СуммаБезСкидки - СтрокаТЧ.СуммаАвтоматическойСкидки;
				Если БазаРаспределения < 0 Тогда
					БазаРаспределения = 0;
				КонецЕсли;
			Иначе
				БазаРаспределения = СуммаБезСкидки;
			КонецЕсли;

			Если Ограничение.СуммаБезСкидки <> 0 Тогда
				СтрокаТЧ.СуммаРучнойСкидки = БазаРаспределения * (Ограничение.СуммаСкидкиКРаспределению / Ограничение.СуммаБезСкидки);
			Иначе
			...
			Ограничение.СуммаБезСкидки           = Ограничение.СуммаБезСкидки - БазаРаспределения;
...
//  ДополнительныеПараметры - Структура - Дополнительные параметры очистки скидок.
//
Процедура ОчиститьСкидкиВТЧ(Объект, ИмяТЧ, РассчитыватьСуммуСНДС, РассчитыватьСуммуВзаиморасчетов=Ложь, ОтменитьТолькоАвтоСкидки = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
...
//  РассчитыватьСуммуВзаиморасчетов - Булево - Рассчитывать сумму взаиморасчетов.
//  ОтменитьТолькоАвтоСкидки - Булево - Отменить только автоматические скидки (наценки).
//  ДополнительныеПараметры - Структура - Дополнительные параметры отмены скидок.
//
Процедура ОтменитьСкидки(Объект, ИмяТЧ, РассчитыватьСуммуСНДС, РассчитыватьСуммуВзаиморасчетов=Ложь, ОтменитьТолькоАвтоСкидки = Ложь, ДополнительныеПараметры  = Неопределено) Экспорт
	
	Объект.СкидкиНаценки.Очистить();
	ОчиститьСкидкиВТЧ(Объект, ИмяТЧ, РассчитыватьСуммуСНДС, РассчитыватьСуммуВзаиморасчетов, ОтменитьТолькоАвтоСкидки, ДополнительныеПараметры);
...
Процедура ОтменитьРучныеСкидки(Объект, ИмяТЧ, ИспользуютсяАвтоматическиеСкидки,
	РассчитыватьСуммуСНДС, РассчитыватьСуммуВзаиморасчетов=Ложь, РеализацияСверхЗаказа = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
...
Функция Рассчитать(Объект, ВходныеПараметры) Экспорт
	...
	Иначе
		...
		СкидкиНаценкиЛокализация.ПриРасчетеСкидокНаценок(Объект, ВходныеПараметры, РезультатРасчета);
...
Процедура ПрименитьРезультатРасчета(Объект, ПримененныеСкидки, РеализацияСверхЗаказа = Ложь) Экспорт
	...
	Иначе
		...
		СкидкиНаценкиЛокализация.ПриПримененииРезультатовРасчета(Объект, ПримененныеСкидки, РеализацияСверхЗаказа = Ложь);
...
Процедура ПрименитьРезультатРасчетаКЧекуККМ(Объект, ПримененныеСкидкиНаценки) Экспорт
	...
	ПрименитьРезультатРасчетаКОбъекту(Объект, "Товары", ПримененныеСкидкиНаценки, Ложь);
...
Функция АдресДанныхДляРасчетаРучныхСкидокВоВременномХранилище(Форма, ИмяТаблицы, УникальныйИдентификатор, ТолькоДляВыделенныхСтрок,
	...
	СкидкиНаценкиЛокализация.ПриПодготовкеПомещенияРезультатаРасчетаРучныхСкидокВоВременноеХранилище(Форма, ИмяКоличества);
...
Процедура УстановитьУсловноеОформлениеФормыИсточникаДействияСкидок(УсловноеОформление, Элементы, ИмяТаблицы = "СкидкиНаценки") Экспорт
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ЭлементТаблица = Элементы[ИмяТаблицы + "ДатаОкончанияДействия"]; // ТаблицаФормы
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементТаблица.Имя);
...
// Параметры:
// 	СтрокаДерева - СтрокаДереваЗначений - см. СформироватьДеревоСкидок
// 
...
Функция ЗапросУсловияЗаДеньРождения(УсловиеПредоставления, ПараметрыРасчета)
	...
	ДатаОкончания = КонецДня(ДатаРожденияТекущегоГода) + УсловиеПредоставления.КоличествоДнейПослеДняРождения * 24*60*60;
	ДатаНачала = НачалоДня(ДатаРожденияТекущегоГода) - УсловиеПредоставления.КоличествоДнейДоДняРождения * 24*60*60;
...
Процедура ПередЗаписью(ТекущийОбъект, Отказ) Экспорт
	...
	Если ТипЗнч(НастройкиКомпоновкиДанных) = Тип("Структура") Тогда
		...
		Если НастройкиКомпоновкиДанных.Свойство("Запрос", ЗапросНастройкиКомпоновкиДанных) Тогда
				...
				Если ЗапросНастройкиКомпоновкиДанных.Свойство("ТекстЗапроса", ТекстЗапросаНастройкиКомпоновкиДанных) И 
					 ...
					 Для каждого ПараметрЗапроса из ЗапросДляПроверкиПараметров.Параметры Цикл
						 Если Не ПараметрыЗапросаНастройкиКомпоновкиДанных.Свойство(ПараметрЗапроса.Ключ) Тогда
							 Отказ = Истина;
							 ТекстИсключения = НСтр("ru='В реквизите ХранилищеНастроекКомпоновкиДанных коллекция параметров не соответствует параметрам текста запроса';uk='У реквізиті ХранилищеНастроекКомпоновкиДанных колекція параметрів не відповідає параметрам тексту запиту'");
							 Прервать;
						 КонецЕсли;	 
					 КонецЦикла;	 
			...
			Иначе	
				Отказ = Истина;	
				ТекстИсключения = НСтр("ru='В реквизите ХранилищеНастроекКомпоновкиДанных нет данных о запросе';uk='У реквізиті ХранилищеНастроекКомпоновкиДанных немає даних про запит'");
			КонецЕсли;
		КонецЕсли;
...
Процедура УдалитьВременныеТаблицы(Запрос)
	...
	Для Каждого ИмяТаблицы Из ТаблицыДляУничтожения Цикл
		Запрос.Текст = Запрос.Текст + СтрЗаменить(
			ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + "УНИЧТОЖИТЬ ИмяТаблицы", "ИмяТаблицы", ИмяТаблицы) + Символы.ПС;
	КонецЦикла;
...
Процедура ПакетЗапросовВставитьЗапросВПакет(Запрос,
	...
	Иначе
		ПакетЗапросов.Запрос.Текст = ПакетЗапросов.Запрос.Текст + ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + Запрос.Текст;
	КонецЕсли;
...
// * Товары - Массив из СправочникСсылка.Номенклатура
