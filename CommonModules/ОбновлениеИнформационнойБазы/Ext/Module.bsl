...
Процедура ПроверитьПараметрыВыборки(Параметры)
	...
	Если Не Параметры.ВыбиратьПорциями И ЭтоПостраничнаяВыборка(Параметры) Тогда
		ВызватьИсключение НСтр("ru='Многопоточный обработчик обновления обязан выбирать данные порциями.';uk='Багатопотоковий обробник оновлення зобов''язаний вибирати дані порціями.'");
	КонецЕсли;
...
Функция ЭтоПростойИсточникДанных(ДополнительныеИсточникиДанных)
	...
	Если ПростойИсточник И СложныйИсточник Тогда
		Ошибка = НСтр("ru='Источник данных задан неверно (см. ДополнительныеПараметрыВыборкиДанныхДляОбработки()).';uk='Джерело даних задано невірно (див. ДополнительныеПараметрыВыборкиДанныхДляОбработки()).'");
