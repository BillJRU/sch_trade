...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Если ТипОборудованияИспользуемый = "ККТ" И РевизияИнтерфейса < 2002 Тогда  
		СообщениеПользователю = НСтр("ru='Используется стандарт подключения драйверов, поддерживающий формат фискальных документов версии 1.0.';uk='Використовується стандарт підключення драйверів, що підтримує формат фіскальних документів версії 1.0.'") +
			Символы.ПС + НСтр("ru='Возможно некорректное формирование чеков. Необходимо обновить драйвер ККТ.';uk='Можливо некоректне формування чеків. Необхідно оновити драйвер ККТ.'");
...
Функция ОткрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если НЕ ПустаяСтрока(ВходныеПараметры.КассирИНН) Тогда
		...
		Если НЕ МенеджерОборудованияКлиентСервер.ИННСоответствуетТребованиям(ВходныеПараметры.КассирИНН, Ложь, ОписаниеОшибки) Тогда
			Сообщение = НСтр("ru='ИНН кассира некорректен (%Ошибка%)';uk='ІПН касира некоректний (%Ошибка%)'");
...
Функция ВыгрузитьТаблицу(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаВыгрузки, ПолнаяВыгрузка, ВыходныеПараметры)
	...
	ТекущийПроцент = 0;
	МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Инициализация выгрузки...';uk='Ініціалізація вивантаження...'"), Окр(ТекущийПроцент));
	...
	Для Каждого Позиция Из ТаблицаВыгрузки  Цикл
		...
		Если (ЗаписьВПакете = РазмерПакета) ИЛИ (ЗаписейВыгружено = ЗаписейВсего) Тогда  
			...
			МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Выгрузка данных...';uk='Вивантаження даних...'"), Окр(ТекущийПроцент));
...
Функция ВыгрузитьТаблицуПакетно(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаВыгрузки, ПолнаяВыгрузка, ВыходныеПараметры)
	...
	МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Подготовка данных...';uk='Підготовка даних...'"));
...
Функция ЗагрузитьТаблицу(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	ДанныеЗагрузки = "";
	МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Загрузка данных...';uk='Завантаження даних...'"));
...
Функция ОчиститьТаблицу(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Выполнение операции...';uk='Виконання операції...'"));	
...
Функция ПечатьЭтикеток(ОбъектДрайвера, Параметры, ПараметрыПодключения, ШаблонЭтикетки, МассивЭтикеток, ВыходныеПараметры)
	...
	ТекущийПроцент = 0;
	МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Инициализация печати этикеток...';uk='Ініціалізація друку етикеток...'"), Окр(ТекущийПроцент));
	ПроцентИнкремент = 100 / (ЗаписейВсего / РазмерПакета);
	...
	Для Каждого Позиция Из МассивЭтикеток  Цикл
		...
		Если (ЗаписьВПакете = РазмерПакета) ИЛИ (ЗаписейВыгружено = ЗаписейВсего) Тогда  
			...
			ТекущийПроцент = ТекущийПроцент + ПроцентИнкремент;
			МенеджерОборудованияКлиент.СостояниеПроцесса(НСтр("ru='Печать этикеток...';uk='Друк етикеток...'"), Окр(ТекущийПроцент));
...
Функция ПолучитьДанныеМетокRFID(ОбъектДрайвера, ПараметрыПодключения, ИДПакета, ТаблицаМеток, ВыходныеПараметры) 
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Ошибка вызова метода <ОбъектДрайвера.ПолучитьДанныеМетокRFID>.';uk='Помилка виклику методу <ОбъектДрайвера.ПолучитьДанныеМетокRFID>.'") +  ОписаниеОшибки());
	КонецПопытки;
...
Функция ОткрытьСессиюRFID(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Ошибка вызова метода <ОбъектДрайвера.ОткрытьСессиюRFID>.';uk='Помилка виклику методу <ОбъектДрайвера.ОткрытьСессиюRFID>.'") +  ОписаниеОшибки());
	КонецПопытки;
...
Функция ЗакрытьСессиюRFID(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Ошибка вызова метода <ОбъектДрайвера.ЗакрытьСессиюRFID>.';uk='Помилка виклику методу <ОбъектДрайвера.ЗакрытьСессиюRFID>.'") +  ОписаниеОшибки());
	КонецПопытки;
...
Функция ЗаписатьДанныеВМеткуRFID(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры, ПараметрыЗаписи, Таймаут)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Ошибка вызова метода <ОбъектДрайвера.ЗаписатьДанныеВМеткуRFID>.';uk='Помилка виклику методу <ОбъектДрайвера.ЗаписатьДанныеВМеткуRFID>.'") +  ОписаниеОшибки());
	КонецПопытки;
