...
Функция ПрочитатьСообщениеИзНетипизированногоТела(Знач НетипизированноеТело) Экспорт
	...
	ПервыйСимволТела = Лев(СокрЛП(НетипизированноеТело), 1); 
	
	Если ПервыйСимволТела = "<" Тогда
		ЭтоJSON = Ложь;
	ИначеЕсли ПервыйСимволТела = "{" ИЛИ ПервыйСимволТела = "[" Тогда
		ЭтоJSON = Истина;
	Иначе
		ВызватьИсключение НСтр("ru='Не удалось определить формат нетипизированного тела';uk='Неможливо визначити формат нетипізованого тіла'");
	КонецЕсли;
	...
	Если ЭтоJSON Тогда
		...
		Сообщение = ПрочитатьJSON(Чтение);
		Чтение.Закрыть();
...
Процедура ЗаписатьСобытиеНачалоОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Начало обработки';uk='Повідомлення в моделі сервісу.Початок обробки'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаписатьСобытиеОкончаниеОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Окончание обработки';uk='Повідомлення в моделі сервісу.Закінчення обробки'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Процедура ДоставитьБыстрыеСообщения() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Доставка быстрых сообщений';uk='Доставка швидких повідомлень'",ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , ,
...
//
// Возвращаемое значение:
//  ФиксированныйМассив Из ТипОбъектаXDTO - типы сообщений найденные в пакете.
//
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
			Если РаботаВМоделиСервиса.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка сообщения';uk='Повідомлення в моделі сервісу.Відправлення повідомлення'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка';uk='Повідомлення в моделі сервісу.Відправка'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Процедура ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Получение';uk='Повідомлення в моделі сервісу.Отримання'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Функция ПредставлениеСообщенияДляЖурнала(Знач Сообщение)
	...
	Шаблон = НСтр("ru='Канал: %1';uk='Канал: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
		Шаблон = НСтр("ru='Область данных: %1';uk='Область даних: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
