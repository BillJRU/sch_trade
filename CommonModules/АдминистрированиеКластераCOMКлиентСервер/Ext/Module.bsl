...
#Иначе
			ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В случае ошибки о несоответствии версии компоненты ""comcntr"" следует зарегистрировать ее на компьютере %1
                    |для учетной записи ОС Windows, от имени которой выполняется BAF. Например:
                    |regsvr32.exe ""%2\comcntr.dll""'
                    |;uk='У разі помилки про невідповідність версії компоненти ""comcntr"" слід зареєструвати її на комп''ютері %1
                    |для облікового запису ОС Windows, від імені якої виконується BAF. Наприклад:
...
#ИначеЕсли МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru='Внимание! Администрирование кластера невозможно в мобильном клиенте';uk='Увага! Адміністрування кластера неможливо в мобільному клієнті'");
#Иначе
