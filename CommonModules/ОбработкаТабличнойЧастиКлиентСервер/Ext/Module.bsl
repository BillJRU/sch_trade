...
// Параметры:
//	Объект - ДанныеФормыСтруктура - данные документа.
//	ЦенаВключаетНДС - Неопределено или Булево - если не Неопределено, ЦенаВключаетНДС в возвращаемой структуре будет установлена этим параметром
//
// Возвращаемое значение:
...
//
Функция ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект, ЦенаВключаетНДС = Неопределено) Экспорт
	
	СтруктураЗаполненияЦены = Новый Структура;
    СтруктураЗаполненияЦены.Вставить("ЦенаВключаетНДС", ?(ЦенаВключаетНДС <> Неопределено, ЦенаВключаетНДС, Объект.ЦенаВключаетНДС));
    СтруктураЗаполненияЦены.Вставить("Дата", ?(НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Дата"), '00010101', Объект.Дата));
...
//		* НалогообложениеНДС - ПеречислениеСсылка.ТипыНалогообложенияНДС - налогообложение документа.
//
Функция ПараметрыПересчетаСуммыНДСВТЧ(Объект, ЦенаВключаетНДС = Неопределено) Экспорт
...
Процедура ПересчитатьКоличествоУпаковокВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьКоличествоУпаковок") Тогда
		...
		Иначе
			...
			Иначе
				ТекстИсключения = НСтр("ru='При попытке пересчета количества в %ЕдИзмерения% превышена допустимая разрядность.';uk='При спробі перерахунку кількості в %ЕдИзмерения% перевищена допустима розрядність.'");
...
Процедура ЗаполнитьСпособОпределенияСебестоимостиСтрокой(ТекущаяСтрока, СтруктураДействий) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСпособОпределенияСебестоимостиСтрокой", СтруктураПараметров)
		...
		Если ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента") Тогда
			Представление = НСтр("ru='Равна сумме в данной строке';uk='Дорівнює сумі в цьому рядку'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПродажи") Тогда
			Представление = НСтр("ru='Определяется из документа продажи';uk='Визначається з документа продажу'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПередачи") Тогда
			Представление = НСтр("ru='Определяется из документа передачи';uk='Визначається з документа передачі'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.Вручную") Тогда
			Если СтруктураПараметров.ИспользоватьРеглУчет Тогда
				Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
					Представление = НСтр("ru='Упр. с НДС: %1% (%2%), Регл. без НДС: %3% (%4%)';uk='Упр. з ПДВ: %1% (%2%), Регл. без ПДВ: %3% (%4%)'");
				Иначе
					Представление = НСтр("ru='Упр.: %1% (%2%), Регл.: %3% (%4%)';uk='Упр.: %1% (%2%), Регл.: %3% (%4%)'");
				КонецЕсли;
			Иначе
				Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
					Представление = НСтр("ru='Упр. с НДС: %1% (%2%)';uk='Упр. з ПДВ: %1% (%2%)'");
				Иначе
					Представление = НСтр("ru='Упр.: %1% (%2%)';uk='Упр .: %1% (%2%)'");
				КонецЕсли;
			КонецЕсли; 
