...
Процедура ОчиститьЗначениеВСтрокеДерева(Дерево, Реквизит, ПараметрыОбработкиОшибок = Неопределено) Экспорт
	...
	Если СтрокаДерева = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru='Ошибка очистки реквизита дерева данных. Не найден реквизит ""%1""';uk='Помилка очищення реквізиту дерева даних. Не вдалося знайти реквізит ""%1""'"), Реквизит);
...
Процедура ЗаполнитьЗначениеРеквизитаВДереве(Дерево, Реквизит, Значение, ПараметрыОбработкиОшибок = Неопределено) Экспорт
	...
	СтрокаДерева = Дерево.Строки.Найти(Реквизит, "ПолныйПуть", Истина);
	Если СтрокаДерева = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru='Ошибка заполнения дерева данных. Не найден реквизит ""%1""';uk='Помилка заповнення дерева даних. Не вдалося знайти реквізит ""%1""'"), Реквизит);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
