...
Процедура ЗаполнитьЭтапыОплатыДокументаПоЗаказам(Параметры, ВызовИзОбработчикаОбновления = Ложь) Экспорт
	...
	Если ВызовИзОбработчикаОбновления Тогда
		СформироватьВТСуммЭтаповПоЗаказам24(Запрос, Параметры);
		ТекстВтЗаказы = "";
	Иначе 
		ТекстВтЗаказы = "
		|ВЫБРАТЬ
	...
	Для Каждого СтрокаЗаказа Из Заказы Цикл
		...
		Если СтрокаЗаказа.ДатаПлатежаСдвиг > СтрокаЗаказа.ДатаПлатежа Тогда
			Если ТипЗнч(НовСтр.ВариантОплаты) = Тип("ПеречислениеСсылка.ВариантыОплатыПоставщику") Тогда
...
Процедура СформироватьВТСуммЭтаповПоЗаказам24(Запрос, Параметры)
