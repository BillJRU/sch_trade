...
Процедура НастроитьОтбор(Форма, Имя, Адреса) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Редактирование отбора';uk='Редагування відбору'"));
...
Процедура ОткрытьФормуУстановкиСтатусаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Источник)Тогда
		ОткрытьФормуУстановкиСтатусаФрагмент(Источник, Статус, Форма);
	Иначе
		ТекстОшибки = НСтр("ru='Не удалось выполнить изменение статусов скидок (наценок).';uk='Не вдалося виконати зміну статусів знижок (націнок).'");
