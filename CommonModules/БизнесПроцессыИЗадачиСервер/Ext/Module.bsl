...
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект, 
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		Если Элемент = Неопределено Тогда
			...
			Элемент.Картинка = БиблиотекаКартинок.ИнформацияБСП;
		КонецЕсли;
	...
	Если БизнесПроцессыИЗадачиВызовСервера.ЭтоВедущаяЗадача(ЗадачаОбъект.Ссылка) Тогда
		...
		Если Элемент = Неопределено Тогда
			...
			Элемент.Вид = ВидДекорацииФормы.Надпись;
			Элемент.Заголовок = НСтр("ru='Это ведущая задача для вложенных бизнес-процессов. Она будет выполнена автоматически при их завершении.';uk='Це ведуча задача для вкладених бізнес-процесів. Вона буде виконана автоматично при їх завершенні.'");
			Элемент.Высота = 0; // автовысота
			Элемент.АвтоМаксимальнаяШирина = Ложь;
		КонецЕсли;
	КонецЕсли;
...
//           "Главный бухгалтер (ТОВ Добро)" - если роль указана вместе с основным объектом адресации;
//           "Главный бухгалтер (ТОВ Добро, Филиал в г.Киев)" - если роль указана вместе с обоими объектами
//                                                                   адресации.
//
...
Процедура СтартоватьОтложенныйПроцесс(БизнесПроцесс) Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Во время отложенного старта этого процесса произошла ошибка:
            ...
            |;uk='Під час відкладеного старту цього процесу сталася помилка:
            |%1
            |Спробуйте запустити процес вручну, а не * ліниво*.'");
...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	ОписаниеОшибки = "";
	ВидСообщения = НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроконтролироватьЗадачи() Экспорт
	...
	Если НЕ СистемнаяУчетнаяЗаписьНастроена(ОписаниеОшибки) Тогда
		ВидСообщения = НСтр("ru='Бизнес-процессы и задачи.Мониторинг задач';uk='Бізнес-процеси та задачі.Моніторінг задач'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ВидСообщения, УровеньЖурналаРегистрации.Ошибка,
...
Процедура ОтправитьУведомлениеОПросроченныхЗадачах(ПисьмоИзНабора)
	...
	Если ПустаяСтрока(ПисьмоИзНабора.ПочтовыйАдрес) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомление не было отправлено, так как у пользователя %1 не задан адрес электронной почты.';uk='Повідомлення не було відправлено, тому що в користувача %1 не задано адресу електронної пошти.'"), 
			ПисьмоИзНабора.Получатель);
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о просроченных задачах';uk='Бізнес-процеси та задачі.Повідомлення про прострочені задачі'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю)
	...
	Если ПустаяСтрока(ПочтовыйАдресПолучателя) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Информация,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомление не отправлено, так как не указан почтовый адрес у пользователя %1.';uk='Повідомлення не відправлене, тому що не зазначено поштову адресу в користувача %1.'"), Строка(Исполнитель)));
		Возврат Ложь;
	КонецЕсли;
	...
	КонецЦикла;
	ТемаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Направлены задачи - %1';uk='Спрямовано задачі - %1'"), Метаданные.КраткаяИнформация);
...
Функция СформироватьПредставлениеЗадачи(ЗадачаСтруктура)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1
		...
		Формат(ЗадачаСтруктура.СрокИсполнения, НСтр("ru='ДЛФ=DD; ДП=''не указан''';uk='ДЛФ=DD; ДП=''не вказано'''")));
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Исполнитель) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Исполнитель: %1';uk='Виконавець: %1'"), ЗадачаСтруктура.Исполнитель) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.РольИсполнителя) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль: %1';uk='Роль: %1'"), ЗадачаСтруктура.РольИсполнителя) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ОсновнойОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Основной объект адресации: %1';uk='Основний об''єкт адресації: %1'"), ЗадачаСтруктура.ОсновнойОбъектАдресации) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ДополнительныйОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Доп. объект адресации: %1';uk='Дод. об''єкт адресації: %1'"), ЗадачаСтруктура.ДополнительныйОбъектАдресации) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Автор) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Автор: %1';uk='Автор: %1'"), ЗадачаСтруктура.Автор) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Описание) Тогда
		Результат = Результат + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ЗадачаСтруктура.Описание) + Символы.ПС;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru='Бизнес-процессы и задачи';uk='Бізнес-процеси та задачі'",ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗаполнитьОтветственногоЗаКонтрольИсполнения() Экспорт
	...
	РольОбъект.Наименование = НСтр("ru='Координатор выполнения задач';uk='Координатор виконання задач'");
...
Функция СистемнаяУчетнаяЗаписьНастроена(ОписаниеОшибки)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		ОписаниеОшибки = НСтр("ru='Отправка почты не предусмотрена в программе.';uk='Відправлення пошти не передбачене в програмі.'");
	Иначе
		МодульРаботаСПочтовымиСообщениями = ОбщегоНазначения.ОбщийМодуль("РаботаСПочтовымиСообщениями");
		Если МодульРаботаСПочтовымиСообщениями.УчетнаяЗаписьНастроена(МодульРаботаСПочтовымиСообщениями.СистемнаяУчетнаяЗапись(), Истина, Ложь) Тогда
			Возврат Истина;
		КонецЕсли;
		ОписаниеОшибки = НСтр("ru='Системная учетная запись электронной почты не настроена для отправки.';uk='Системний обліковий запис електронної пошти не налаштований для відправки.'");
	КонецЕсли;
...
Процедура ЗаполнитьНаименованиеПредопределенногоЭлементаВсеОбъектыАдресации() Экспорт
	...
	ВсеОбъектыАдресации.Наименование = НСтр("ru='Все объекты адресации';uk='Всі об''єкти адресації'");
