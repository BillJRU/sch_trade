			...
			Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда 
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
                        ...
                        |%2
                        |
                        |Якщо в тексті помилки зазначено, що ""клас не зареєстрований"" або не збігається версія компоненти ""comcntr"" з необхідною,
                        |то слід зареєструвати її на комп''ютері %1, для цього зверніться до адміністратора.'"),
...
Функция ПолучитьИБ(СоединенияСРабочимиПроцессами, Кластер, Знач ИмяВКластере, Знач ИмяАдминистратораИБ, Знач ПарольАдминистратораИБ)
	...
	Иначе		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В кластере серверов %1:%2 не найдена информационная база ""%3""';uk='У кластері серверів %1:%2 не знайдена інформаційна база ""%3""'"),
