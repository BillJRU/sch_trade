...
// Параметры:
//  Выборка              - ВыборкаИзРезультатаЗапроса - 
//  ДокументУстановкиЦен - ДокументОбъект - Документ, для которого необходимо вывести сообщения об ошибках
//  Отказ                - Булево - Флаг отказа от проведения документа.
//
...
Функция ЦеныНоменклатурыПоставщика(Таблица, ПараметрыПолученияЦен) Экспорт
	...
	Запрос.Текст = Запрос.Текст +
	    ...
	    |		КОГДА
		|			&НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОсвобожденаОтНДС)
	    |		ТОГДА
