...
Функция ПечатьТТН(ОписаниеКоманды) Экспорт
	...
	Если ОписаниеКоманды.ОбъектыПечати.Количество() = 0
		...
		Если ЭтоПечатьОдногоДокумента Тогда
			ТекстСообщения = НСтр("ru='Нельзя напечатать транспортные накладные и ТТН по данному документу.';uk='Неможна надрукувати транспортні накладні та ТТН за цим документом.'");
		Иначе
			ТекстСообщения = НСтр("ru='Нет документов, по которым можно напечатать транспортные накладные или ТТН.';uk='Немає документів, за якими можна надрукувати транспортні накладні або ТТН.'");
		КонецЕсли;
	...
	Если РезультатыПроверки.ОбъектыПоКоторымМожноИНужноСоздатьТранспортныеНакладные.Количество() > 0 Тогда
		...
		Если ЭтоПечатьОдногоДокумента Тогда
			ЗаголовокВопроса = НСтр("ru='По документу не создана транспортная накладная.';uk='За документом не створена транспортна накладна.'");
		Иначе
			ЗаголовокВопроса = НСтр("ru='Для некоторых распоряжений не созданы транспортные накладные.';uk='Для деяких розпоряджень не створені транспортні накладні.'");
		КонецЕсли;
		
		Если ОписаниеКоманды.ДополнительныеПараметры.ИнтерактивноеДобавление Тогда
			СписокЗначенийКнопок.Добавить("СоздатьИНапечатать", НСтр("ru='Создать и вывести на печать';uk='Створити і вивести на друк'"));
		КонецЕсли;
				
		Если ОписаниеКоманды.ОбъектыПечати.Количество() > 0 Тогда
			СписокЗначенийКнопок.Добавить("НапечататьСуществующие", НСтр("ru='Напечатать уже созданные';uk='Надрукувати вже створені'"));
		КонецЕсли;
		
		СписокЗначенийКнопок.Добавить("Отмена", НСтр("ru='Отмена';uk='Відмінити'"));
...
Процедура НеСозданыТранспортныеНакладныеЗавершение(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если КодВозврата = "СоздатьИНапечатать" Тогда 
		...
		Если РезультатыПроверки.ЗаданияНаПеревозкуДляСозданияТранспортныхНакладных.Количество() > 0 Тогда
			СозданныеТранспортныеНакладные = УправлениеПечатьюУТВызовСервераЛокализация.СоздатьТранспортныеНакладные(РезультатыПроверки.ЗаданияНаПеревозкуДляСозданияТранспортныхНакладных);
		Иначе
			СозданныеТранспортныеНакладные = УправлениеПечатьюУТВызовСервераЛокализация.СоздатьТранспортныеНакладные(РезультатыПроверки.ОбъектыПоКоторымМожноИНужноСоздатьТранспортныеНакладные);
		КонецЕсли;
