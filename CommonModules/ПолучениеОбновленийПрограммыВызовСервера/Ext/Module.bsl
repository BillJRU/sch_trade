...
Процедура ВключитьОтключитьАвтоматическуюУстановкуИсправлений(Знач ЗначениеНастройки) Экспорт
	...
	Если Не ПараметрыОбновления.ПолучатьИсправления Тогда
		ВызватьИсключение НСтр("ru='Получение исправлений недоступно в текущем режиме работы.';uk='Отримання виправлень недоступно в поточному режимі роботи.'");
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// Проверка версии платформы BAF при начале работы программы.
...
Функция ПараметрыПроверкиВерсииПлатформыПриЗапуске() Экспорт
	...
	ТекущаяВерсияПлатформы       = ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы();
	...
	ТекстСообщения = "<body>" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Для работы с программой %1 использовать версию платформы &quot;BAF&quot; не ниже: <b>%2</b>
            |<br />Используемая сейчас версия: %3'
            |;uk='Для роботи з програмою %1 використовувати версію платформи &quot;BAF&quot"" не нижче:  <b>%2<br />
	...
	Если Не Результат.ЭтоАдминистраторСистемы Тогда
		...
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("ru='Вход в программу невозможен.<br />
                |Необходимо обратиться к администратору для обновления версии платформы BAF.'
                |;uk='Вхід в програму неможливий.<br />
                |Необхідно звернутися до адміністратора для оновлення версії платформи BAF.'");
		
	ИначеЕсли ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена Тогда
		
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("ru='Вход в программу невозможен.<br />
                |Необходимо предварительно обновить версию платформы BAF.'
                |;uk='Вхід в програму неможливий.<br />
                |Необхідно попередньо оновити версію платформи BAF.'");
		
	Иначе
		
		ТекстСообщения = ТекстСообщения + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"<br /><br />"
				+ НСтр("ru='Рекомендуется обновить версию платформы платформы BAF. Новая версия платформы содержит исправления
                    |ошибок, которые позволят программе работать более стабильно.
                    |<br />
                    |<br />
                    |Вы так же можете отказаться от обновления и
                    |продолжить работу на текущей версии. Минимально необходимая версия платформы %1'
                    |;uk='Рекомендується оновити версію платформи платформи BAF. Нова версія платформи містить виправлення
                    |помилок, які дозволять програмі працювати більш стабільно.
                    |<br />
                    |<br />
                    |Ви так само можете відмовитися від оновлення і
                    |продовжити роботу на поточній версії. Мінімально необхідна версія платформи %1'"),
	...
	Попытка
		...
		Если Результат.Статус = "Отменено" Тогда
			...
			ПолучениеОбновленийПрограммы.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru='Не удалось выполнить фоновую проверку наличия обновлений. Задание отменено администратором.';uk='Не вдалося виконати фонову перевірку наявності оновлень. Завдання скасовано адміністратором.'"));
			Возврат Неопределено;
...
Функция РезультатЗаданияПроверкиНаличияОбновлений(Знач ОписаниеЗадания) Экспорт
	...
	Исключение
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось определить результат фоновой проверки наличия обновлений. %1';uk='Не вдалося визначити результат фонової перевірки наявності оновлення. %1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат -1;
	КонецПопытки;
