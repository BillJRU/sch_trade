...
// Подключает на сервере BAF внешнюю компоненту из хранилища внешних компонент,
...
Функция АвтоматическиОбновляемыеКомпоненты() Экспорт
	...
	Иначе
		ВызватьИсключение 
			НСтр("ru='Ожидается существование подсистемы ""ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент""';uk='Очікується існування підсистеми ""ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент""'");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент") Тогда
		...
		Для каждого СтрокаРезультата Из ДанныеВнешнихКомпонент Цикл
			...
			Если ЗначениеЗаполнено(КодОшибки) Тогда
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Обновление внешних компонент';uk='Оновлення зовнішніх компонент'",ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Если Выборка.НайтиСледующий(Отбор) Тогда 
				...
				Попытка
					...
					Результат = Результат + ПредставлениеКомпоненты
						+ " - " + НСтр("ru='Успешно обновлена.';uk='Успішно оновлено.'") + Символы.ПС;
