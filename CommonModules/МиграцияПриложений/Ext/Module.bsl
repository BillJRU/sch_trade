...
Процедура НачатьВыгрузку(АдресПриложения, Логин, Пароль, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически, ДополнительныеСвойства) Экспорт
	...
	Попытка
		...
		ИнтеграцияПодсистемБТС.ЗаписатьДанныеВБезопасноеХранилище(НовыйУзел.Ссылка, ПараметрыДоступа, "ПараметрыДоступа");
...
Процедура ОтменитьВыгрузку() Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Запись.УзелОбмена) Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
			ПараметрыДоступа = ИнтеграцияПодсистемБТС.ПрочитатьДанныеИзБезопасногоХранилища(Запись.УзелОбмена, "ПараметрыДоступа");
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если Не СостояниеВыгрузки.Выбран() Тогда
		...
		РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийВыгрузка);
		Возврат;
	КонецЕсли;
	...
	УстановитьПривилегированныйРежим(Истина);
	ПараметрыДоступа = ИнтеграцияПодсистемБТС.ПрочитатьДанныеИзБезопасногоХранилища(УзелОбмена, "ПараметрыДоступа");
	УстановитьПривилегированныйРежим(Ложь);
	...
	Исключение
		...
		Шаблон = НСтр("ru='Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %2';uk='Не вдалося отримати стан завантаження, можливо сервіс тимчасово недоступний: %1 %2'",ОбщегоНазначения.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(Шаблон, Символы.ПС, ПредставлениеОшибки);
		РегистрацияПримечания(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
	...
	Исключение
		...
		Иначе
			...
			РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не СостояниеЗагрузки.Выбран() Тогда
		Комментарий = НСтр("ru='Состояние загрузки не найдено. Загрузка прервана';uk='Стан завантаження не знайдено. Завантаження перервано'",ОбщегоНазначения.КодОсновногоЯзыка());
		РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
		УдалитьОчередь();
	...
	Исключение
		...
		Иначе 
			РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Сервис не поддерживает миграцию';uk='Сервіс не підтримує міграцію'"));
		Возврат Ложь;
	КонецЕсли;
...
//
// Параметры:
//   ПараметрыДоступа - см. ВызватьМетодПрограммногоИнтерфейса.ПараметрыДоступа - параметры доступа. 
//
// Возвращаемое значение:
...
Функция Зарегистрироваться(ИмяСервера, Наименование, Логин, ЭлектроннаяПочта, Пароль, Телефон) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru='Не удалось выполнить запрос, код ответа: %1';uk='Не вдалося виконати запит, код відповіді: %1'"), Ответ.КодСостояния));
		Возврат Ложь;
	КонецЕсли;
...
Функция Загрузить(ОписаниеДанных) Экспорт
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			РегистрацияОшибки(НСтр("ru='Загрузка сообщения';uk='Завантаження повідомлення'",ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки)
	...
	Если Манифест.Свойство("ОбъектыВыгруженныеПолностью", ОбъектыВыгруженныеПолностью) Тогда
		Шаблон = НСтр("ru='Не удалось выгрузить порциями объекты: %1.';uk='Не вдалося вивантажити порціями об''єкти: %1.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически)
	...
	Если ВыгруженоОбъектов < ПараметрыВыгрузки.РазмерПорцииДанных Тогда
		Если ЗавершитьМиграциюАвтоматически Тогда
			...
			Исключение
				Комментарий = НСтр("ru='Не удалось установить монопольный режим.';uk='Не вдалося встановити монопольний режим.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
//     * СсылкиОбщихДанных - Соответствие - коллекция всех общих ссылок, которые будут обнаружены, Ключ -
//                                          СправочникСсылка, Значение - Истина.
...
//     * СправочникиФайлов - Структура - описание см. РаботаСФайламиСлужебныйВМоделиСервисаПовтИсп.СправочникиФайловИОбъектыХранения().
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	ШаблонНачалоВыгрузки = НСтр("ru='Выгрузка объекта: %1';uk='Вивантаження об''єкту: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
	ШаблонОкончаниеВыгрузки = НСтр("ru='Выгружен объект: %1, Количество: %2, Длительность (сек): %3';uk='Вивантажений об''єкт: %1, Кількість: %2, Тривалість (сек): %3'",ОбщегоНазначения.КодОсновногоЯзыка());
	ВсегоОбъектов = 0;
...
Функция ВыгрузитьСсылочныйТип(ОбъектМетаданных, Узел, НомерСообщения, ОсталосьВыгрузить, ПараметрыВыгрузки)
	...
	Если ЭтоСправочникФайлов Тогда
		ОбработчикФайлов = ИнтеграцияПодсистемБТС.ОбщийМодуль(ОбработчикФайлов);
		КоличествоФайлов = 0;
	КонецЕсли;
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
				Продолжить;
			КонецПопытки;
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	ШаблонНачалоЗагрузки = НСтр("ru='Загружается объект: %1';uk='Завантажується об''єкт: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
	ШаблонОкончаниеЗагрузки = НСтр("ru='Загружен объект: %1, Количество: %2, Длительность (сек): %3';uk='Завантажений об''єкт: %1, Кількість: %2, Тривалість (сек): %3'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЗагрузитьСсылочныйТип(ОбъектМетаданных, Идентификатор, ПараметрыЗагрузки)
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		// Временный обход ошибки платформы.
	...
	Если Метаданные.Последовательности.Содержит(ОбъектМетаданных) И ОбъектМетаданных.Измерения.Количество() Тогда
		...
		Если Выборка.Количество() Тогда
			...
			Для Каждого МетаданныеДокумента Из ОбъектМетаданных.Документы Цикл
				Прервать;
			КонецЦикла;
			МенеджерДокумента = ИнтеграцияПодсистемБТС.МенеджерОбъектаПоПолномуИмени(МетаданныеДокумента.ПолноеИмя());
...
Функция ТипПоляПоиска(ОбъектМетаданных, ИмяПоля)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
        ...
        |;uk='Поле %1 не може використовуватися в якості поля природного ключа об''єкта %2: 
        |поле об''єкта не виявлено'",ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяПоля,
		ОбъектМетаданных.ПолноеИмя());
...
// Заменяет ссылки в объекте. Следующие ссылки требуют замены: Ссылки планов обмена, общих данных и ссылки разделенных
	...
	ИначеЕсли ПараметрыЗагрузки.ТипыПересоздаваемыхСсылок[ТипЗначения] <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Значение) Тогда
			...
			Если НоваяСсылка = Неопределено Тогда
				МенеджерОбъекта = ИнтеграцияПодсистемБТС.МенеджерОбъектаПоПолномуИмени(Значение.Метаданные().ПолноеИмя());
...
Процедура ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура РегистрацияОшибки(Событие, Комментарий)
	...
	ИмяСобытия = СтрШаблон(НСтр("ru='Миграция приложений.%1';uk='Міграція прикладних програм.%1'",ОбщегоНазначения.КодОсновногоЯзыка()), Событие);
