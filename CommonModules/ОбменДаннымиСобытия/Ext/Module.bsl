...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Правила регистрации объектов';uk='Обмін даними. Правила реєстрації об''єктів'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					Комментарий = НСтр("ru='Предыдущая версия (автоматическое разрешение конфликта).';uk='Попередня версія (автоматичне вирішення конфлікту).'");
				Иначе
					Комментарий = НСтр("ru='Текущая версия (автоматическое разрешение конфликта).';uk='Поточна версія (автоматичне вирішення конфлікту).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru='Текущая версия (автоматическое разрешение конфликта).';uk='Поточна версія (автоматичне вирішення конфлікту).'"));
			Иначе
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
				СведенияОВерсииОбъекта.Вставить("ТипВерсииОбъекта", "НепринятыеДанныеПоКоллизии");
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru='Отклоненная версия (автоматическое разрешение конфликта).';uk='Відхилена версія (автоматичне вирішення конфлікту).'"));
			КонецЕсли;
