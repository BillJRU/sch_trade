		...
		Если ЗначениеЗаполнено(ТекущийРежим.Сообщение) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Администратором приложения установлена блокировка работы пользователей на период с %1 по %2 по причине:
                    |%3.'
                    |;uk='Адміністратором додатка встановлено блокування роботи користувачів на період з %1 по %2 по причині:
                    |%3.'"), ТекущийРежим.Начало, ТекущийРежим.Конец, ТекущийРежим.Сообщение);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Администратором приложения установлена блокировка работы пользователей на период с %1 по %2 для проведения регламентных работ.';uk='Адміністратором додатка встановлено блокування роботи користувачів на період з %1 по %2 для проведення регламентних робіт.'"), 
		...
		Если ЗначениеЗаполнено(ТекущийРежим.Сообщение) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Администратором приложения установлена блокировка работы пользователей на период с %1 по причине:
                    |%2.'
                    |;uk='Адміністратором додатка встановлено блокування роботи користувачів на період з %1 з причини:
                    |%2.'"), ТекущийРежим.Начало, ТекущийРежим.Сообщение);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Администратором приложения установлена блокировка работы пользователей на период с %1 для проведения регламентных работ.';uk='Адміністратором додатка встановлено блокування роботи користувачів на період з %1 для проведення регламентних робіт.'"), 
				ТекущийРежим.Начало);
		КонецЕсли;		
	КонецЕсли;
	Параметры.Вставить("СеансыОбластиДанныхЗаблокированы", ТекстСообщения + Символы.ПС + Символы.ПС + НСтр("ru='Приложение временно недоступно.';uk='Прикладна програма тимчасово недоступно.'"));
	ТекстСообщенияДляВхода = "";
	Если Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстСообщенияДляВхода = ТекстСообщения + Символы.ПС + Символы.ПС + НСтр("ru='Войти в заблокированное приложение?';uk='Увійти в заблоковану прикладну програму?'");
	КонецЕсли;
...
Функция СформироватьСообщениеБлокировки(Знач Сообщение, Знач КодРазрешения) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='%1
            ...
            |http://<вебадреса сервера>/?C= РазрешитьРаботуПользователей'");
	...
	Иначе
		ТекстСообщения = ТекстСообщения + НСтр("ru='%1
            |Для того чтобы разрешить работу пользователей, воспользуйтесь консолью кластера серверов или запустите BAF с параметрами:
            |ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3'
            |;uk='%1
            |Для того, щоб дозволити роботу користувачів, скористайтеся консоллю кластера серверів або запустіть BAF з параметрами:
            |ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СоединенияИБКлиентСервер.ТекстДляАдминистратора(),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'",ОбщегоНазначения.КодОсновногоЯзыка());
