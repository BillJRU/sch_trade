			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, 
                        ...
                        |;uk='Неприпустима спроба зміни значення реквізиту %1 для додаткової обробки %2,
                        |отриманої з каталогу додаткових обробок менеджера сервісу.'"), 
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Или Ссылка = Справочники.ДополнительныеОтчетыИОбработки.ПустаяСсылка() Тогда
		ВызватьИсключение НСтр("ru='Запрошено подключение несуществующей дополнительной обработки.';uk='Запрошено підключення неіснуючої додаткової обробки.'");
	КонецЕсли;
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ИмяОбработки = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Запрошено создание объекта несуществующей дополнительной обработки.';uk='Запрошено створення об''єкта неіснуючої додаткової обробки.'");
	КонецЕсли;
...
Процедура ПередОбновлениемЗадания(Объект, Команда, Задание, Изменения) Экспорт
	...
	Если Не Константы.РазрешитьВыполнениеДООРегламентнымиЗаданиямиВМоделиСервиса.Получить() Тогда
		ВызватьИсключение НСтр("ru='Периодическое выполнение команд дополнительных обработок в качестве заданий запрещено администратором сервиса.';uk='Періодичне виконання команд додаткових обробок в якості завдань заборонено адміністратором сервісу.'");
	КонецЕсли;
	...
	Если Задание.Расписание.ТребуетсяВыполнение(ПроверяемаяДата, ИсходнаяДата) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд.';uk='Розклад, що задається для виконання команд додаткового звіту або обробки в якості завдань, повинен бути не частіше, ніж 1 раз в %1 секунд.'"), 
			МинимальныйИнтервал);
	КонецЕсли;
...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, 
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, що поставляються.Ініційовано встановлення обробки, яка поставляється, в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Если ПоставляемаяОбработкаСуществует Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';uk='Додаткові звіти та обробки, що поставляються.Встановлення в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';uk='Додаткові звіти та обробки, що поставляються.Встановлення в область даних відкладено'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
				Строка(ОписаниеИнсталляции.Инсталляция));
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';uk='Додаткові звіти та обробки, які поставляються.Вилучення з області даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка,
	...
	Если Не ПустаяСтрока(ТекстИсключения) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка вилучення з області даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';uk='Додаткові звіти та обробки, що поставляються.Помилка встановлення в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		ПоставляемаяОбработка,
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Дополнительная обработка с идентификатором %1 не является поставляемой.';uk='Додаткова обробка з ідентифікатором %1 не є поставляємою.'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';uk='Додаткові звіти та обробки, що поставляються.Завантаження обробки, яка поставляється, скасовано'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Запрос обновлений дополнительных отчетов и обработок';uk='Запит оновлень додаткових звітів і обробок'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		,
...
Функция ПоставляемаяОбработка(ИспользуемаяОбработка)
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Использование функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ПоставляемаяОбработка
                                ...
                                |доступно тільки з сеансів з встановленим розподілом даних.'");
	КонецЕсли;
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Завантаження обробки, що поставляється'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';uk='Додаткові звіти та обробки, що поставляються.Завантаження обробки, яка поставляється, завершено'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
	...
	Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Запланована актуалізація настройок обробки, яка поставляється'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка.Ссылка,
	...
	Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, що поставляються.Заплановано відкладене встановлення обробки, яка поставляється, в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка.Ссылка,
...
Процедура ПроверкаВозможностиВыполненияПоставляемойОбработки(Знач ИспользуемаяОбработка)
	...
	Если ПараметрыПубликацииИспользуемойОбработки.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена Тогда
		ВызватьИсключение НСтр("ru='Использование дополнительной обработки запрещено. Обратитесь за помощью к пользователю, обладающего правами администратора в приложении.';uk='Використання додаткової обробки заборонено. Зверніться по допомогу до користувача, що володіє правами адміністратора в прикладній програмі.'");
	КонецЕсли;
	...
	Если ПараметрыПубликацииИспользуемойОбработки.Версия <> ПараметрыПубликацииПоставляемойОбработки.Версия Тогда
		ВызватьИсключение НСтр("ru='Использование дополнительной обработки временно недоступно. Повторите попытку через несколько минут. Приносим извинения на доставленные неудобства.';uk='Використання додаткової обробки тимчасово недоступно. Повторіть спробу через декілька хвилин. Приносимо вибачення на незручності.'");
	КонецЕсли;
