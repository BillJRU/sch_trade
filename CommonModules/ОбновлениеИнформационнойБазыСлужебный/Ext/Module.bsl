...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Обновление информационной базы';uk='Оновлення інформаційної бази'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + "." + НСтр("ru='Протокол выполнения';uk='Протокол виконання'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ДобавитьПотокПоискаДанныхДляОбновления(Поток, Обработчик, КонтекстОбработчика, СведенияОбОбновлении)
	...
	Если Не Описание.ВыполняетсяПоискПорции Тогда
		...
		ШаблонНаименования = НСтр("ru='Поиск данных для обработчика обновления ""%1""';uk='Пошук даних для обробника оновлення ""%1""'");
...
Процедура ПроверитьПараметрыВыборки(ПараметрыВыборки)
	...
	Если Не ИзвестныйСпособ Тогда
		ШаблонСообщения = НСтр("ru='Укажите способ выборки в процедуре регистрации данных к обновлению.
            |Указывается в ""Параметры.ПараметрыВыборки.СпособВыборки"".
            |Сейчас указан неизвестный способ выборки ""%1"".'
            |;uk='Вкажіть спосіб вибірки в процедурі реєстрації даних до оновлення.
            |Вказується в ""Параметры.ПараметрыВыборки.СпособВыборки"".
            |Зараз зазначений невідомий спосіб вибірки ""%1"".'");
	...
	Если Не ТаблицыУказаны Тогда
		ВызватьИсключение НСтр("ru='Укажите обрабатываемые таблицы в процедуре регистрации данных к обновлению.
            |Указывается в ""Параметры.ПараметрыВыборки.ПолныеИменаОбъектов"" и/или
            |""Параметры.ПараметрыВыборки.ПолныеИменаРегистров"".'
            |;uk='Вкажіть оброблювані таблиці в процедурі реєстрації даних до оновлення.
            |Вказується в ""Параметры.ПараметрыВыборки.ПолныеИменаОбъектов"" і/або
            |""Параметры.ПараметрыВыборки.ПолныеИменаРегистров"".'");
	КонецЕсли;
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + "." + НСтр("ru='Выполнение обработчиков';uk='Виконання обробників'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, СведенияОбОбновлении)
	...
	Попытка
		...
		Если Параметры.ОбработкаЗавершена = Неопределено Тогда
            ...
            |Виконання перерване через явну помилку в коді обробника.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
