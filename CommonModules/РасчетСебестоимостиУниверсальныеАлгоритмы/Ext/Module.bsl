...
Функция ИнициализироватьПараметрыРаспределенияМетодомУменьшаемогоОстатка(
	...
	Шаблоны.Вставить("РазделительСтрок", "," + Символы.ПС + Символы.Таб);
	...
	Для Каждого ИмяПоля Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПоляСвязи, ",", Истина, Истина) Цикл
		ДополнитьСтроку(ПоляСвязиИменованные,   "%1." + ИмяПоля + " КАК " + ИмяПоля,,,, Шаблоны.РазделительСтрок);
		ДополнитьСтроку(ПоляСвязиНеИменованные, "%1." + ИмяПоля ,,,, Шаблоны.РазделительСтрок);
...
Процедура РаспределитьМетодомУменьшаемогоОстатка(МенеджерВременныхТаблиц, ПараметрыРаспределения) Экспорт
	...
	Для Каждого ПравилоРаспределения Из ПараметрыРаспределения.ПравилаРаспределения Цикл // по базисам распределения
		...
		Если НЕ РасчетЗавершен Тогда
			ВызватьИсключение НСтр("ru='Ошибка алгоритма распределения: превышено максимальное расчетное количество итераций';uk='Помилка алгоритму розподілу: перевищено максимальну розрахункову кількість ітерацій'",ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
