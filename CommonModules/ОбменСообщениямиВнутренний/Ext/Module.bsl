...
Процедура ПриЗагрузкеДанных(СтандартнаяОбработка,
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Данные = ПрочитатьXML(ЧтениеXML); // СправочникОбъект.СообщенияСистемы, СправочникОбъект.СообщенияОбластейДанных
...
Процедура ОбработатьОчередьСообщенийСистемы(Отбор = Неопределено) Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            ...
            НачатьТранзакцию();
            Попытка
                ОбъектСообщения = Выборка.Ссылка.ПолучитьОбъект(); // СправочникОбъект.СообщенияСистемы, СправочникОбъект.СообщенияОбластейДанных
...
Процедура УстановитьВедущуюКонечнуюТочкуНаСторонеПолучателя(КодЭтойКонечнойТочки, КодВедущейКонечнойТочки) Экспорт
	...
	Если ПланыОбмена.ОбменСообщениями.НайтиПоКоду(КодЭтойКонечнойТочки) <> ЭтотУзел() Тогда
		...
		СтрокаСообщенияОбОшибкеДляЖурналаРегистрации = НСтр("ru='Заданы неверные параметры подключения к конечной точке.
            ...
            |Параметри підключення вказують на іншу кінцеву точку.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
//   Булево - признак поддержки частичного получения сообщений.
//
...
//
// Возвращаемое значение:
//   Массив - номера версий, поддерживаемых интерфейсом корреспондента.
//
...
Функция СобытиеЖурналаРегистрацииПодключениеКонечнойТочки() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями.Подключение конечной точки';uk='Обмін повідомленнями.Підключення кінцевої точки'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУстановкаВедущейКонечнойТочки() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями.Установка ведущей конечной точки';uk='Обмін повідомленнями.Встановлення ведучою кінцевої точки'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииЭтойПодсистемы() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями';uk='Обмін повідомленнями'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьПодключениеКонечнойТочкиНаСторонеОтправителя(Отказ,
	...
	Если Не УзелКонечнойТочки.Пустая() Тогда
		Отказ = Истина;
		СтрокаСообщенияОбОшибке = НСтр("ru='Конечная точка уже подключена к информационной базе; наименование точки: %1';uk='Кінцева точка вже підключена до інформаційної бази; найменування точки: %1'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОбработчикиСообщений()
	...
	ИнтеграцияПодсистемБТС.ПриОпределенииОбработчиковКаналовСообщений(Результат);
...
Процедура ПриОтправкеСообщения(Знач КаналСообщений, ТелоСообщения, ОбъектСообщения)
	...
	СообщенияВМоделиСервиса.ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения);
...
Процедура ПриПолученииСообщения(Знач КаналСообщений, ТелоСообщения, ОбъектСообщения)
	...
	СообщенияВМоделиСервиса.ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения);
