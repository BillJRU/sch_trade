...
Процедура ЗарегистрироватьТарифицируемыеУслуги_ОбработчикОбновления() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Регистрация тарифицируемых услуг.Обработчик обновления';uk='Реєстрація послуг, що тарифікуються. Обробниік оновлення'",КодЯзыка());
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Регистрация тарифицируемых услуг.Регламентное задание';uk='Реєстрація послуг, що тарифікуються. Регламентне завдання'",КодЯзыка());
...
Процедура ЗапроситьЛицензииУникальныхУслуг_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Запрос лицензий уникальных услуг.Регламентное задание';uk='Запит ліцензій унікальних услуг. Регламентне завдання'",КодЯзыка());
...
Функция СформироватьСообщениеЗарегистрироватьТарифицируемыеУслуги(Знач ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		...
		Для Каждого Услуга Из Поставщик.Услуги Цикл
			...
			УслугиСписокXDTO = ПоставщикXDTO.Services; // СписокXDTO
			УслугиСписокXDTO.Добавить(УслугаXDTO);
			
		КонецЦикла;
		
		ПровайдерыXDTO = Сообщение.Body.ServiceProviders; // СписокXDTO
		ПровайдерыXDTO.Добавить(ПоставщикXDTO);
...
//  - Нет одинаковых идентификаторов у поставщиков услуг.
//
// Параметры:
//  ПоставщикиУслуг - см. ТарификацияПереопределяемый.ПриФормированииСпискаУслуг.ПоставщикиУслуг
//
Процедура ПроверитьСтруктуруПоставщиковУслуг(ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		Идентификатор = Поставщик.Идентификатор;
		Если МассивПроверенных.Найти(Идентификатор) = Неопределено Тогда 
			МассивПроверенных.Добавить(Идентификатор);
		Иначе
			ВызватьИсключение НСтр("ru='Имеется несколько поставщиков услуг с идентификатором:';uk='Є кілька постачальників послуг з ідентифікатором:'") + " " + Идентификатор;
		КонецЕсли;
	КонецЦикла;
