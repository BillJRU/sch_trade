...
//
// Параметры:
//  Партнер  - СправочникСсылка.КонтактныеЛицаПартнеров - контактное лицо партнера, для которого получаем значение основного менеджера.
//
// Возвращаемое значение:
...
//
// Параметры:
//  Партнер  - СправочникСсылка.ВнешниеПользователи - внешний пользователь, для которого получаем значение основного менеджера.
//
// Возвращаемое значение:
...
Функция МенеджерДокументаПоУмолчанию() Экспорт
	...
	Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		ОсновнойМенеджерВнешнегоПользователя(Пользователи.АвторизованныйПользователь());
	Иначе
...
Процедура ОформитьЗаказ(СтруктураЗаказа, Проводить = Истина) Экспорт
	...
	Иначе
		...
		ДокументЗаказ.ОплатаВВалюте = ВзаиморасчетыСервер.ПолучитьОплатуВВалютеПоУмолчанию(ВалютаОплаты);
	
	КонецЕсли;
	
	ПараметрыВзаиморасчеты = Документы.ЗаказКлиента.ПараметрыВзаиморасчеты(ДокументЗаказ.ХозяйственнаяОперация);
	ВзаиморасчетыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ДокументЗаказ, ПараметрыВзаиморасчеты);
