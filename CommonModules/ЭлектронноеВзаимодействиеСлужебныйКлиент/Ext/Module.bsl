...
Процедура ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, КолСформированных, КолУтвержденных, КолПодписанных, КолПодготовленных, КолОтправленных = 0) Экспорт
	...
	Если КолПодготовленных + КолОтправленных > 0 Тогда
		ДопТекст = ?(КолОтправленных > 0, НСтр("ru='отправлено';uk='відправлено'"), НСтр("ru='подготовлено к отправке';uk='підготовлено до відправлення'"));
...
Функция ПредставлениеОшибкиПодписания(КлючОшибки) Экспорт
	...
	Если КлючОшибки = "ОшибкиВМаршруте" Тогда
		Результат = НСтр("ru='Для подписания документа есть доступные сертификаты, но подписи по ним уже установлены. Проверьте настройки маршрута подписания.';uk='Для підписання документа є доступні сертифікати, але підписи з них вже встановлені. Перевірте параметри маршруту підписання.'");
	КонецЕсли;
...
|;uk='Виникла помилка при копіюванні в буфер обміну.
|Скористайтеся комбінацією клавіш Ctrl+C.'"));
	КонецПопытки;
...
Процедура ЗаблокироватьОткрытиеФормыНаМобильномКлиенте(Отказ) Экспорт
	...
	#Если МобильныйКлиент Тогда
		ТекстСообщения = НСтр("ru='Функциональность в мобильном клиенте пока недоступна. Воспользуйтесь веб-клиентом или тонким клиентом';uk='Функціональність в мобільному клієнті поки недоступна. Скористайтеся вебклієнтом або тонким клієнтом'");
...
Функция ВыполнитьОпределениеПрограммыСертификатаКриптографии_ВводПароля(ПроцессВыполнения)
	...
	Если ПроцессВыполнения.Пароль = Неопределено Тогда
		...
		ПараметрыВвода.Вставить("Заголовок", НСтр("ru='Проверка сертификата криптографии';uk='Перевірка сертифіката криптографії'"));
		ПараметрыВвода.Вставить("Подсказка", НСтр("ru='Введите пароль закрытого ключа сертификата:';uk='Введіть пароль закритого ключа сертифіката:'"));
