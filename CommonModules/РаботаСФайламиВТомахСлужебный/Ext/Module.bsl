...
Функция ДанныеФайла(ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Получение файла из тома';uk='Файли.Отримання файла з тома'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ДобавитьФайл(ПрисоединенныйФайл,
	...
	Если ТипЗнч(ПрисоединенныйФайл) = Тип("Структура")
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не заполнено значение свойства %1 в параметре %2 (Структура) в процедуре %3.';uk='Не заповнене значення властивості %1 в параметрі %2 (Структура) в процедурі %3.'"),
			"Ссылка",
			"ПрисоединенныйФайл",
	...
	Если ТомХранения = Неопределено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось добавить файл ""%1"" ни в один из томов, т.к. в томах недостаточно места.';uk='Не вдалося додати файл ""%1"" до жодного з томів, тому що в томах недостатньо місця.'"),
			ПрисоединенныйФайл.Наименование + "." + ПрисоединенныйФайл.Расширение);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Добавление файла';uk='Файли.Додавання файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура ОбновитьФайл(ПрисоединенныйФайлОбъект, ДвоичныеДанные) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Запись файла на диск';uk='Файли.Запис файлу на диск'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура УдалитьФайл(ПутьКФайлу) Экспорт
	...
	Если ФайлНаДиске.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Удаление файлов в томе';uk='Файли.Вилучення файлів в томі'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПоместитьФайлВРеквизитСправочника(ЭлементДанных) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Не удалось отправить файл при обмене данными';uk='Файли.Не вдалося відправити файл при обміні даними'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если ЗначениеЗаполнено(Выборка.ПутьКФайлу)
			...
			Если СуществующийФайл.Количество() = 0 Тогда
				...
				НесуществующийФайл.СтатусПроверки = ?(Выборка.ПометкаУдаления,
					НСтр("ru='Целостные данные';uk='Цілісні дані'"),
					НСтр("ru='Отсутствуют данные в томе на диске';uk='Відсутні дані в томі на диску'"));
