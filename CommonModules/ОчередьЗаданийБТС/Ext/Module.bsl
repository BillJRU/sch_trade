...
//
// Параметры:
//   ПараметрыЗадания - СправочникСсылка.ДополнительныеОбработчикиОчередиЗаданий - параметры задания.
	...
	Пока БлокировкаУстановлена = Неопределено Или ТекущаяУниверсальнаяДата() < ДатаЗавершения Цикл
		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если Выборка.ВыполнятьЧерезРегламентноеЗадание И ОчередьЗаданийСлужебный.ЗаданиюНазначеноРегламентноеЗадание(Выборка.Ссылка) Тогда
					Продолжить;
				КонецЕсли;
		...
		Иначе
			...
			Если Выборка.ВыполнятьЧерезРегламентноеЗадание Тогда
				...
				ОчередьЗаданийСлужебный.УдалитьРегламентныеЗаданияОбработки(Выборка.Ссылка.УникальныйИдентификатор(), Выборка.Ссылка);
				ОчередьЗаданийСлужебный.НазначитьРегламентноеЗадание(Выборка.Ссылка);
				...
				Если Выборка.ОжидатьЗавершенияВыполненияРегламентногоЗадания Тогда
					ОчередьЗаданийСлужебный.ОжидатьЗавершенияЗадания(Выборка.Ссылка);
				КонецЕсли;
...
//
// Параметры:
//   Задание - СправочникОбъект.ОчередьЗаданий, СправочникОбъект.ОчередьЗаданийОбластейДанных - задание.
//   Расписание - РасписаниеРегламентногоЗадания - расписание задания.
//   КоличествоПовторовПриАварийномЗавершении - Число - колисчество повторов.
//   ИнтервалПовтораПриАварийномЗавершении - Число - интервал повтора.
//   ЧасовойПояс - Строка - часовой пояс исполнения.
Процедура ЗапланироватьЗадание(Задание, Расписание, КоличествоПовторовПриАварийномЗавершении, ИнтервалПовтораПриАварийномЗавершении, ЧасовойПояс)
	...
	Попытка
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначения.КодОсновногоЯзыка()), Задание.Ссылка, 
			...
			Если ПараметрыОбработчика.ОбработчикСуществует Тогда
				...
				СправочникМенеджер = Справочники[Задание.Метаданные().Имя]; // СправочникМенеджер.ОчередьЗаданий, СправочникОбъект.ОчередьЗаданийОбластейДанных
				НовоеЗадание = СправочникМенеджер.СоздатьЭлемент();
...
// Параметры:
// 	Ссылка - СправочникСсылка.ОчередьЗаданий, СправочникСсылка.ОчередьЗаданийОбластейДанных - задание очереди.
// 	ИсполняющееФоновоеЗадание - ФоновоеЗадание - исполняющее фоновое задание.
...
Функция ТекстЗапросаЗаданияВОчереди()
	...
	Для Каждого СправочникЗаданий Из ОчередьЗаданийСлужебныйПовтИсп.ПолучитьСправочникиЗаданий() Цикл
		...
		МетаданныеОбъекта = СправочникЗаданий.ПустаяСсылка().Метаданные(); // ОбъектМетаданныхСправочник
...
// Обновляет настройки дополнительных обрбаботчиков по полученному сообщению.
// 
// Параметры:
// 	Настройки - Структура - настройки обработки:
//	* Действия - Массив Из Структура - описание настройки:
//		** Действие - Строка - имя действия, принимает 3 значения:
//			"УдалитьВсеОбработчики" - дополнительные Настройки не передаются
//			"УдалитьОбработчик" - передается только Идентификатор удаляемого обработчика
//			"ОбновитьОбработчик" - передаются Настройки дополнительного обработчика.
//		** Идентификатор - УникальныйИдентификатор - идентификатор обработчика.
//		** Наименование - Строка - наименование обработчика.
//		** КоличествоЗаданий - Число - количество заданий. 
//		** ДлительностьЗадания - Число - длительность задания.
//		** Методы - Массив из Строка - имена методов очереди заданий.
//		** Использование - Булево - признак использования
//		** Расписание - РасписаниеРегламентногоЗадания - расписание обработчика.
Процедура ОбновлениеНастроек(Настройки)
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		Блокировка.Добавить("Справочник.ДополнительныеОбработчикиОчередиЗаданий");
		Блокировка.Заблокировать();
		
		Для Каждого Настройка Из Настройки.Действия Цикл
			
			Действие = Настройка.Действие;
			
			Если Действие = "УдалитьВсеОбработчики" Тогда
			...
			ИначеЕсли Действие = "ОбновитьОбработчик" Тогда
			...
			ИначеЕсли Действие = "УдалитьОбработчик" Тогда
