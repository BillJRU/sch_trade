...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Запуск по требованию';uk='Розсилка звітів. Запуск на вимогу'",ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение, НСтр("ru='Не выбран способ доставки.';uk='Не обраний спосіб доставки.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(Отчеты, ПараметрыДоставки, НаименованиеРассылки = "", ПараметрыЖурнала = Неопределено) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Инициализирована рассылка ''%1'', автор: ''%2''';uk='Ініціалізована розсилка ''%1'', автор: ''%2'''"),
	...
	Для Каждого СтрокаОтчет Из Отчеты Цикл
		...
		Если ПараметрыДоставки.Персонализирована И НЕ ПараметрыОтчета.Персонализирован Тогда
			ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчет ''%1'' не может сформирован, так как в его настройках не указан отбор по получателю рассылки.';uk='Звіт ''%1'' не може бути сформований, так як в його налаштуваннях не вказано відбір по одержувачу розсилки.'"),
				ПредставлениеОтчета);
		...
		Попытка
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчет ''%1'' успешно сформирован';uk='Звіт ''%1'' успішно сформований'"), ПредставлениеОтчета);
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчет ''%1'' не сформирован:';uk='Звіт ''%1'' не сформований:'"), ПредставлениеОтчета);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось отправить отчеты получателю ''%1'':';uk='Не вдалося відправити звіти одержувачу ''%1'':'"), ПредставлениеПолучателя);
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ru='Рассылка отчетов. Запуск по расписанию';uk='Розсилка звітів. Запуск за розкладом'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ТекстСообщенияОбОшибкеДляЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось сформировать список получателей ""%1"" по причине:';uk='Не вдалося сформувати список одержувачів ""%1"" через:'"), Строка(ТипПолучателей));
	...
	Если СписокПолучателей.Количество() = 0 Тогда
        ...
        |;uk='Не вдалося сформувати список одержувачів ""%1"" по одній з можливих причин:
        |- У одержувачів не заповнена адреса електронної пошти ""%2"";
        | - Не заповнений список одержувачів або одержувачі відмічені для вилучення;
        | - Обрано порожні групи одержувачів;
        | - Виключені всі одержувачі (виняток має найвищий пріоритет; учасники виключених груп також виключаються зі списку);
        | - Недостатньо прав доступу до довідника ""%1"".'");
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ''%1'' не инициализирован';uk='Звіт ''%1'' не ініціалізований'"), Строка(ПараметрыОтчета.Отчет)));
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"":';uk='Звіт ""%1"":'"),
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = НСтр("ru='Отчеты не доставлены';uk='Звіти не доставлені'");
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка записи отчета ''%1'' в формат ''%2'':';uk='Не вдалося записати звіт ''%1'' в формат ''%2 '':'");
			...
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					...
					НСтр("ru='Формат не поддерживается';uk='Формат не підтримується'"));
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОшибки + Символы.ПС + НСтр("ru='Файл ''%3'' не существует.';uk='Файл ''%3'' не існує.'"),
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, НаименованиеРассылки, ПараметрыЖурнала)
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В рассылке есть недоступные отчеты (%1):%2';uk='В розсилці є недоступні звіти (%1):%2'"),
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недостаточно прав для создания файлов в папке ""%1"".';uk='Недостатньо прав для створення файлів у папці ""%1"".'"),
					ПредставлениеПапки));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';uk='Мережевий каталог %1 не обраний, доставка в мережевий каталог відключена'"),
				ЗначениеПодстановки));
	...
	Если ПараметрыДоставки.Архивировать Тогда
		Если Не ПараметрыДоставки.Свойство("ИмяАрхива") Или Не ЗначениеЗаполнено(ПараметрыДоставки.ИмяАрхива) Тогда
			ПараметрыДоставки.Вставить("ИмяАрхива", ШаблонИмениАрхива(ЯзыкШаблона));
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У пользователя ''%1'' недостаточно прав на вывод информации';uk='У користувача ''%1'' недостатньо прав на виведення інформації'"),
