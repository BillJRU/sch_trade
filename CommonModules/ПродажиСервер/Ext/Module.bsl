...
// Выводит сообщения о том, что скидки есть корректировки на основании документа
//
// Параметры:
...
Процедура СообщитьОбОшибкахОтменаПроведенияЕстьКорректировки(Знач ДокументПродажи, Отказ) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='На основании документа %1 введена корректировка реализации. Отмена проведения невозможна.';uk='На підставі документа %1 введено коригування реалізації. Скасування проведення неможливо.'"),
		ДокументПродажи);
...
//
Функция ПерезаполнитьНакладнуюПоОрдерам(Объект,НалогообложениеНДС) Экспорт
 
	
	ЭтоРеализация = Не (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПередачаТоваровХранителю"));
...
//
// Параметры:
// ДокументПродажи  - ДокументОбъект,
// 					- ОбработкаОбъектИмяОбработки.ПомощникПродаж - Объект, для которого необходимо осуществить проверки
// Отказ            - Булево - Флаг отказа от проведения документа.
//
Процедура ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ) Экспорт
	...
	// Проверим возможность и состояние согласования через BAS Документооборот
	...
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.ВыкупТоваровХранителем") Тогда
		...
		НужноСогласоватьРучныеСкидкиПоПользователям = Ложь;
		ИспользоватьСкидки                          = Ложь;
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.СписаниеТоваровУХранителя") Тогда
	...
	ИначеЕсли ИмяТаблицы = "Документ.СписаниеТоваровУХранителя" Тогда
    ...
    ПараметрыЗапроса.Вставить("ОрганизацияПлательщикНДС", НДСОбщегоНазначенияПовтИсп.ОрганизацияПлательщикНДС(РеквизитыДокумента.Организация, РеквизитыДокумента.Дата));
...
|;uk='Сума платежів по всіх етапах ""%СуммаПоЭтапам% %Валюта%"" не повинна відрізнятися від суми замовлених рядків ""%СуммаВсего% %Валюта%""'");
...
|;uk='Сума застави за тару по всіх етапах ""%СуммаПоЭтапам% %Валюта%"" не повинна 
|відрізнятися від суми застави по документу ""%СуммаВсего% %Валюта%""'");
...
Процедура ПроверитьНеобходимостьСогласованияУсловийПродажи(Знач ДокументПродажи,
	...
	ПараметрыЗапроса.Вставить("НеПроверятьКодСтроки", НеПроверятьКодСтроки);
	ПараметрыЗапроса.Вставить("ИспользоватьСоглашенияСКлиентами", ИспользоватьСоглашенияСКлиентами);
	ПараметрыЗапроса.Вставить("ОрганизацияПлательщикНДС", НДСОбщегоНазначенияПовтИсп.ОрганизацияПлательщикНДС(ВыборкаПоШапке.Организация, ВыборкаПоШапке.Дата));
...
Процедура ПроверитьВозможностьВводаНаОснованииСоглашения(ИмяДокумента, ОперацияСоглашения) Экспорт
	...
	Если ОперацииДокумента.Найти(ОперацияСоглашения) = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Нельзя выполнить ввод документа на основании соглашения с операцией ""%Операция%"".';uk='Не можна виконати введення документа на підставі угоди з операцією ""%Операция%"".'");
...
//
// Параметры:
// ЭтапыГрафикаОплаты               - ДокументТабличнаяЧасть.АктВыполненныхРабот.ЭтапыГрафикаОплаты - 
//                                    Таблица значений, для которой необходимо осуществить проверки,
//                                    документ АктВыполненныхРабот указан для типизации и примера.
// СуммаОплатыПоДокументу           - Число - Сумма платежей по документу, в котором необходимо осуществить проверку
// СуммаЗалогаПоДокументу           - Число - Сумма залога за тару по документу, в котором необходимо осуществить проверку
// УчитыватьВариантОплаты           - Булево - признак необходимости проверки корректности вариантов оплаты
// Отказ                            - Булево - Флаг отказа от записи документа
...
Процедура ВыполнитьКонтрольЗаказаПослеПроведения(КонтролируемыйЗаказ, Отказ) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|			ПО ОбъектыРасчетов.Объект = &Ссылка
		|ГДЕ
		|	&КонтролироватьРасчеты
		|	И РасчетыСКлиентамиОстатки.КОплатеОстаток > 0
		|	И (ОбъектыРасчетов.Объект.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Закрыт)
		|				ИЛИ ОбъектыРасчетов.Объект.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена))
...
Процедура СформироватьЗапросШапкаДокументаПродажи(ТекстЗапроса, ИмяТаблицы)
	...
	+ ?(ИмяТаблицы = "Справочник.СоглашенияСКлиентами","СоглашениеСКлиентом.ОплатаВВалюте","
	|	ВЫБОР
	|		КОГДА ДокументПродажи.Договор ЕСТЬ НЕ NULL И СоглашениеСКлиентом.ИспользуютсяДоговорыКонтрагентов 
	|			ТОГДА ВЫРАЗИТЬ(ДокументПродажи.Договор КАК Справочник.ДоговорыКонтрагентов).ОплатаВВалюте
	|		ИНАЧЕ СоглашениеСКлиентом.ОплатаВВалюте
...
Процедура СформироватьЗапросКорректностьУказанияСтавкиНДСПродажи(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса
		...
		|			И &ВозвратМногооборотнойТары
        |			И ВременнаяТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
        |			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
...
Процедура СообщитьОбОшибкахКорректностьСуммыНДС(Знач Выборка, ДокументПродажи, ИмяТЧ, ПредставлениеТЧ, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПроверятьСуммуНДС Тогда
			...
			ТекстОшибки = НСтр("ru='Сумма НДС %СуммаНДС% %Валюта% при ставке ""%СтавкаНДС%"" отличается от расчетной %СуммаНДСРасчетная% %Валюта%';uk='Сума ПДВ %СуммаНДС% %Валюта% при ставці ""%СтавкаНДС%"" відрізняється від розрахункової %СуммаНДСРасчетная% %Валюта%'");
			...
			АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""%ПредставлениеТЧ%""';uk='в рядку%НомерСтроки% списку ""%ПредставлениеТЧ%""'");
...
Процедура СообщитьОбОшибкахКорректностьУказанияСтавкиНДСПродажи(Знач Выборка, ДокументПродажи, ИмяТЧ, ПредставлениеТЧ, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КорректнаяСтавкаНДС) Тогда
			ТекстОшибки = НСтр("ru='В строке %НомерСтроки% списка ""%ПредставлениеТЧ%"" необходимо выбрать ставку НДС ""%СтавкаНДС%"".';uk='У рядку %НомерСтроки% списку ""%ПредставлениеТЧ%"" необхідно вибрати ставку ПДВ ""%СтавкаНДС%"".'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",        Выборка.НомерСтроки);
...
Процедура СообщитьОбОшибкахКорректностьСуммыИСуммыНДС(Знач Выборка, ДокументПродажи, ИмяТЧ, ПредставлениеТЧ, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПроверятьСуммуНДС Тогда
			...
			ТекстОшибки = НСтр("ru='Сумма НДС %СуммаНДС% %Валюта% при ставке ""%СтавкаНДС%"" больше общей суммы по строке %Сумма% %Валюта%';uk='Сума ПДВ %СуммаНДС% % Валюта% при ставці ""%СтавкаНДС%"" більше загальної суми по рядку %Сумма% % Валюта%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СуммаНДС%",          Выборка.СуммаНДС);
...
Процедура СообщитьОбОшибкеВыбораНаправленияДеятельности(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстОшибки = НСтр("ru='Для поставки под принципала направление деятельности должно учитывать и доходы и затраты.';uk='Для поставки під принципала напрям діяльності має враховувати доходи та витрати.'");
...
Процедура СообщитьОбОшибкеВозможностиИспользованияЗаказаКлиента(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстОшибки = НСтр("ru='Для поставки под принципала при обособлении по договору с клиентом, документ ""Заказ клиента"" не используется.';uk='Для поставки під принципала при відокремленні за договором з клієнтом, документ ""Замовлення клієнта"" не використовується.'");
...
Процедура СообщитьОбОшибкахШапкаДокументаПродажи(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиСтатусСоглашения Тогда
			Если НЕ ВыводитьВОтчет Тогда
				ТекстОшибки = НСтр("ru='Соглашение находится в статус ""Не согласовано"". Проведение документа запрещено.';uk='Оферта перебуває в статус ""Не погоджено"". Проведення документа заборонено.'");
				...
				СообщитьПользователюОбОшибке(
					...
					Отказ);
			Иначе
				ТекстОшибки = НСтр("ru='Соглашение находится в статус ""Не согласовано"".';uk='Оферта перебуває в статус ""Не погоджено"".'");
				
				Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Соглашение';uk='Оферта'"), ТекстОшибки, МассивОтклонений);
		...
		Если Выборка.ЕстьОшибкиЗаполненияОплатаВВалюте Тогда
			...
			Если НЕ ВыводитьВОтчет Тогда
				Если Выборка.ЭтоСоглашение Тогда
					ТекстОшибки = НСтр("ru='Признак оплаты в иностранной валюте не должен отличаться от значения в типовом соглашении';uk='Ознака оплати в іноземній валюті не повинна відрізнятися від значення в типовій оферті'");
				Иначе
					ТекстОшибки = НСтр("ru='Признак оплаты в иностранной валюте не должен отличаться от значения в соглашении';uk='Ознака оплати в іноземній валюті не повинан відрізнятися від значення в оферті'");
				КонецЕсли;
			...
			Иначе
				Если Выборка.ЭтоСоглашение Тогда
					ТекстОшибки = НСтр("ru='Признак оплаты в иностранной валюте отличается от оплаты в типовом соглашении';uk='Ознака оплати в іноземній валюті відрізняється від оплати в типовій оферті'");
				Иначе
					ТекстОшибки = НСтр("ru='Признак оплаты в иностранной валюте отличается от оплаты в соглашении';uk='Ознака оплати в іноземній валюті відрізняється від оплати в оферті'");
				КонецЕсли;
				Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Оплата';uk='Оплата'"), ТекстОшибки, МассивОтклонений);
			КонецЕсли;
		КонецЕсли;
		...
		Если Выборка.ЕстьОшибкиЗаполненияВозвращатьМногооборотнуюТару Тогда
			...
			Если Выборка.ЭтоСоглашение Тогда
				ТекстОшибки = НСтр("ru='Признак возврата многооборотной тары не должен отличаться от значения в типовом соглашении';uk='Ознака повернення багатооборотної тари не повинна відрізнятися від значення в типовій оферті'");
			Иначе
				ТекстОшибки = НСтр("ru='Признак возврата многооборотной тары не должен отличаться от значения в соглашении';uk='Ознака повернення багатооборотної тари не повинна відрізнятися від значення в оферті'");
			КонецЕсли;
		...
		Если Выборка.ЕстьОшибкиЗаполненияРассчитыватьДатуВозвратаТарыПоКалендарю Тогда
			...
			ТекстОшибки = НСтр("ru='Способ расчета даты возврата многооборотной тары не должен отличаться от значения в типовом соглашении ""%РассчитыватьДатуВозвратаТарыПоКалендарю%""';uk='Спосіб розрахунку дати повернення багатооборотної тари не повинен відрізнятися від значення в типовій угоді ""%РассчитыватьДатуВозвратаТарыПоКалендарю%""'");
		...
		Если Выборка.ЕстьОшибкиЗаполненияСрокВозвратаМногооборотнойТары И Выборка.ВозвратМногооборотнойТары Тогда
			...
			Если НЕ ВыводитьВОтчет Тогда 
				...
				ТекстОшибки = НСтр("ru='Срок возврата многооборотной тары не должен отличаться от значения в соглашении (%СрокВозврата%)';uk='Термін повернення багатооборотної тари не повинен відрізнятися від значення в угоді (%СрокВозврата%)'");
		...
		Если Выборка.ЕстьОшибкиДатаВозвратаМногооборотнойТары И Выборка.ВозвратМногооборотнойТары Тогда
			...
			Если ДатаВозвратаМногооборотнойТары <> ДокументПродажи.ДатаВозвратаМногооборотнойТары Тогда
				ТекстОшибки = НСтр("ru='Дата возврата многооборотной тары не должна отличаться от указанной в соглашении ""%ДатаВозвратаМногооборотнойТары%""';uk='Дата повернення багатооборотної тари не повинна відрізнятися від зазначеної в угоді ""%ДатаВозвратаМногооборотнойТары%""'");
		...
		Если Выборка.ЕстьОшибкиМинимальнаяСуммаЗаказа Тогда
			...
			Если Выборка.ЭтоСоглашение Тогда
				ТекстОшибки = НСтр("ru='Минимальная сумма заказа не должна отличаться от значения в типовом соглашении ""%МинимальнаяСуммаЗаказа% %Валюта%""';uk='Мінімальна сума замовлення не повинна відрізнятися від значення в типовій угоді ""%МинимальнаяСуммаЗаказа% %Валюта%""'");
			Иначе
				ТекстОшибки = НСтр("ru='Сумма заказа меньше, чем минимально допустимая сумма заказа в соглашении ""%МинимальнаяСуммаЗаказа% %Валюта%""';uk='Сума замовлення менше, ніж мінімально допустима сума замовлення в угоді ""%МинимальнаяСуммаЗаказа% %Валюта%""'");
			КонецЕсли;
			...
			Если ЕстьОшибкиМинимальнаяСуммаЗаказа Тогда
				Если НЕ ВыводитьВОтчет Тогда
					...
					СообщитьПользователюОбОшибке(
						...
						Отказ);
				Иначе
					Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Мин. сумма заказа';uk='Мін. сума замовлення'"), ТекстОшибки, МассивОтклонений);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СообщитьОбОшибкахСоответствиеГруппЦенНоменклатурыУсловиямПродаж(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		НомерСтроки = Выборка.НомерСтроки;
		АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""Группы цен номенклатуры""';uk='в рядку %НомерСтроки% списку ""Групи цін номенклатури""'");
		АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", НомерСтроки);
...
Процедура СообщитьОбОшибкахКорректностьЭтаповГрафикаОплаты(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		НомерСтроки = Выборка.НомерСтроки;
		АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""Этапы оплаты""';uk='в рядку %НомерСтроки% списку ""Етапи оплати""'");
		АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", НомерСтроки);
...
Процедура СообщитьОбОшибкахНаличиеУслугТарыВДокументеПродажи(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='Нельзя указывать номенклатуру с типом ""Услуга"" или ""Тара"" (строка %НомерСтроки% списка ""%ПредставлениеТЧ%"")';uk='Не можна вказувати номенклатуру з типом ""Послуга"" або ""Тара"" (рядок %НомерСтроки% списку ""%ПредставлениеТЧ%"")'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",    Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",     Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПредставлениеТЧ%", ПредставлениеТЧ);
...
Процедура СообщитьОбОшибкахКорректностьЭтаповОплатыСоглашения(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СдвигШаблона < Выборка.Сдвиг Тогда
			...
			ТекстОшибки = НСтр("ru='Срок должен быть не больше, чем в типовом соглашении ""%СдвигШаблона%""';uk='Термін повинен бути не більше, ніж в типовій угоді ""%СдвигШаблона%""'");
...
Функция ПолучитьТекстЗапросаВременнойТаблицыЦенПоСоглашению(ИмяТаблицы, ИмяВременнойТаблицыТоваров)
	...
	|			И ВременнаяТаблицаТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ТОГДА
    |			ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
	...
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОсвобожденаОтНДС)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
	|	КОНЕЦ КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СоглашениеТовары.Цена, 0) > 0
