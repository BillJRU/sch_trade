...
//                             * АутентификацияСтандартная - Булево - аутентификация платформы BAF.
		...
		Иначе
			...
			ТекстОшибки =
				НСтр("ru='Список пользователей информационной базы не пустой, однако не удалось
                           ...
                           |знайти жодного користувача з ролями Повні права і Адміністратор системи.
                           |
                           |Ймовірно, користувачі створювалися в конфігураторі.
                           |Потрібно призначити ролі Повні права і Адміністратор системи хоча б одному користувачу.'");
		...
		Если ОбновляемыеСвойства.Свойство("СтарыйПароль")
		   И ТипЗнч(ОбновляемыеСвойства.СтарыйПароль) = Тип("Строка") Тогда
			...
			Если Не СтарыйПарольСовпадает Тогда
				ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("ru='При записи пользователя информационной базы %1 старый пароль указан не верно.';uk='Під час запису користувача інформаційної бази %1 старий пароль вказано не вірно.'"),
					СтарыеСвойства.Имя,
					СтарыеСвойства.УникальныйИдентификатор);
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибкиПароля) Тогда
		ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru='Не удалось записать свойства пользователя информационной базы %1 по причине:
                       |%2.'
                       |;uk='Не вдалося записати властивості користувача інформаційної бази %1 через:
                       |%2.'"),
			ПользовательИБ.Имя,
	...
	Исключение
		ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru='Не удалось удалить пользователя информационной базы %1 по причине:
                       |%2.'
                       |;uk='Не вдалося вилучити користувача інформаційної бази %1 через:
                       |%2.'"),
			ПользовательИБ.Имя,
...
Процедура УстановитьПравоОткрытияВнешнихОтчетовИОбработок(ОткрытиеРазрешено) Экспорт
	...
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		...
		ОписаниеНастроек.Представление = НСтр("ru='Предупреждение безопасности';uk='Попередження безпеки'");
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка записи пользователя ИБ';uk='Користувачі.Помилка запису користувача ІБ'",ОбщегоНазначения.КодОсновногоЯзыка()),
