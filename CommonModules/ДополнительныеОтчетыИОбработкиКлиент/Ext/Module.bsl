...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	...
	ПараметрыСохранения.ТекстПредложения = НСтр("ru='Для сохранения внешней обработки (отчета) в файл рекомендуется установить расширение для работы с BAF.';uk='Для збереження зовнішньої обробки (звіту) в файл рекомендується встановити розширення для роботи з BAF.'");
	ПараметрыСохранения.Диалог.Фильтр = ДополнительныеОтчетыИОбработкиКлиентСервер.ФильтрДиалоговВыбораИСохранения();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Укажите файл';uk='Укажіть файл'");
