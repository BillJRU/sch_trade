...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	Если ТаблицыДляДвижений.Свойство("ТаблицаПланыСборкиРазборкиДляРазузлования")
		И ЗначениеЗаполнено(ТаблицыДляДвижений.ТаблицаПланыСборкиРазборкиДляРазузлования) Тогда
		
		ТаблицаКомплектующих = ТаблицыДляДвижений.ТаблицаПланыСборкиРазборкиДляРазузлования.СкопироватьКолонки(); // ТаблицаЗначений -
		
		Для каждого ЭлементКомплект Из ТаблицыДляДвижений.ТаблицаПланыСборкиРазборкиДляРазузлования Цикл
...
Процедура ПодготовитьТаблицуПериодовПередЗаполнением(Параметры) Экспорт 
	...
	Если ИспользоватьВПланированииОстатков И Параметры.СмещениеПериода = 0 Тогда
...
//
// Параметры:
//  ТаблицаПериоды см. ТаблицаПериоды - Таблица, которую заполняем отрезками заданной периодичности
...
Процедура ОбработкаПроверкиЗаполненияВФорме(Таблица, СтруктураДействий, Отказ, ПроверяемыеРеквизиты, Знач КроссТаблица = Истина) Экспорт 
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Не заполнен список ""%ПредставлениеТаблицы%""';uk='Не заповнений список ""%ПредставлениеТаблицы%""'");
	...
	ТекстОшибкиНоменклатура 	= НСтр("ru='Не заполнена колонка ""Номенклатура""';uk='Не заповнена колонка ""Номенклатура""'");
	ТекстОшибкиТоварнаяКатегория= НСтр("ru='Не заполнена колонка ""Товарная категория""';uk='Не заповнена колонка ""Товарна категорія""'");
	...
	ШаблонАдресаОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""%Товары%""';uk='в рядку%НомерСтроки% списку ""%Товары%""'");
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		Если СтруктураДействий.Свойство("ПроверкаКоличестваПоПериодам") Тогда 
			...
			Если НЕ ЕстьЗаполненныеСтроки Тогда
				...
				ТекстОшибкиКоличества = НСтр("ru='Не заполнено количество';uk='Не заповнена кількість'");
...
Процедура ОбработкаПроверкиДублированияСтрок(Таблица, ПутьКТаблице, ПоляГруппировки, Отказ) Экспорт
	...
	Для Каждого СтрокаДублей Из ТаблицаПроверкиДублирующихСтрок Цикл
		Если СтрокаДублей.КоличествоДублирующихСтрок > 1 Тогда
			ТекстОшибкиКоличества = НСтр("ru='Найдены дублирующиеся по ключевым полям строки:';uk='Знайдено дублюючі за ключовими полями рядки:'");
			...
			Для Каждого Строка Из Таблица.НайтиСтроки(Отбор) Цикл
				ТекстОшибкиКоличества = ТекстОшибкиКоличества + " " + НСтр("ru='строка';uk='рядок'") + " " + Строка.НомерСтроки + ";";
			КонецЦикла;
	...
	Если ЗначениеЗаполнено(Объект.ВидПлана) Тогда
		...
		Если НастройкиЗаполнения.ЗаполнятьПоДефициту Тогда
			...
			Если Запрос.Выполнить().Выгрузить().Количество()>0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В рамках вида плана, заполняемого по дефицитам, может быть создан только один план в периоде.';uk='В рамках виду плану, що заповнюється за дефіцитами, може бути створений тільки один план в періоді.'"),
				Объект,
				,
				, 
				Отказ);
...
Процедура ДобавитьПолеКроссТаблицыЗамещен(ПараметрыВывода, СоздаватьЭлемент = Истина, СтруктураДействий = Неопределено) Экспорт 
	...
	СтруктураПоля.Вставить("ТипЭлемента", "Булево");
	СтруктураПоля.Вставить("ВидПоля", ВидПоляФормы.ПолеФлажка);
	СтруктураПоля.Вставить("Заголовок", НСтр("ru='Замещен';uk='Заміщено'"));
	СтруктураПоля.Вставить("СоздаватьИтоговыеРеквизиты", Ложь);
	СтруктураПоля.Вставить("СвойстваЭлемента", Новый Структура());
...
Процедура ДобавитьПолеКроссТаблицыЗамещенКЗаказу(ПараметрыВывода, СоздаватьЭлемент = Истина, СтруктураДействий = Неопределено) Экспорт 
	...
	СтруктураПоля.Вставить("ТипЭлемента", "Булево");
	СтруктураПоля.Вставить("ВидПоля", ВидПоляФормы.ПолеФлажка);
	СтруктураПоля.Вставить("Заголовок", НСтр("ru='Замещен к заказу';uk='Заміщено до замовлення'"));
	СтруктураПоля.Вставить("СоздаватьИтоговыеРеквизиты", Ложь);
	СтруктураПоля.Вставить("СвойстваЭлемента", Новый Структура());
	СтруктураПоля.СвойстваЭлемента.Вставить("ТолькоПросмотр", Истина);
...
//
// Параметры:
//  Таблица              - ТабличнаяЧасть - Табличная часть объекта
...
#Область ПланыРасхода
	...
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru='Весь планируемый расход';uk='Весь заплановані видаток'"));
	...
	ДопПараметр.Вставить("ЗаголовокОперанда", 	НСтр("ru='Весь планируемый расход';uk='Весь заплановані видаток'"));
...
#Область ПланыПрихода
	...
	ДопПараметр.Вставить("Тип", 				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ДопПараметр.Вставить("Путь", 				ПутьКРеквизиту);
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru='Весь планируемый приход';uk='Весь запланований прибуток'"));
	...
	ДопПараметр.Вставить("ЗаголовокОперанда", 	НСтр("ru='Весь планируемый приход';uk='Весь запланований прибуток'"));
	ДопПараметр.Вставить("ВходитВОперанд",		"ОсновныеОперанды");
...
#Область КорректировкиПлановыхПотребностей
	...
	ДопПараметр.Вставить("Тип", 				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ДопПараметр.Вставить("Путь", 				ПутьКРеквизиту);
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru='Корректировки плановых потребностей';uk='Коригування планових потреб'"));
	ДопПараметр.Вставить("СоздаватьРеквизит", 	ИспользоватьПланыРасхода);
	ДопПараметр.Вставить("УдалятьРеквизитыТаблицы", Истина);
	
	// Свойства операнда
	ДопПараметр.Вставить("СоздаватьОперанд", 	ИспользоватьПланыРасхода);
	ДопПараметр.Вставить("ЗаголовокОперанда", 	НСтр("ru='Корректировки плановых потребностей';uk='Коригування планових потреб'"));
...
Процедура ИнициализироватьДереваОператоров(Дерево) Экспорт
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Значение колонки';uk='Значення колонки'"),      "ЗначениеКолонки()",1);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Номер текущей колонки';uk='Номер поточної колонки'"), "НомерТекущейКолонки()",0);
...
Функция ОсновнойОперандПлана(Форма) Экспорт
	...
	ИначеЕсли ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланВнутреннихПотреблений") Тогда
		Возврат "ПланыВнутреннихПотреблений";
	КонецЕсли;
...
Функция ПоместитьДополнительныеПоляВХранилище(Форма, Смещение) Экспорт
	...
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ОсновныеОперанды", НСтр("ru='Основные операнды';uk='Основні операнди'")));
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ОсновныеОперандыСОтбором",  НСтр("ru='Основные операнды*';uk='Основні операнди*'")));
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ДополнительныеОперандыПланы",  НСтр("ru='Дополнительные операнды (планы)';uk='Додаткові операнди (плани)'")));
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ДополнительныеОперандыПланыСОтбором",  НСтр("ru='Дополнительные операнды (планы)*';uk='Додаткові операнди (плани) *'")));
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ДополнительныеОперандыФакты",  НСтр("ru='Дополнительные операнды (факты)';uk='Додаткові операнди (факти)'")));
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ВспомогательныеОперанды",  НСтр("ru='Вспомогательные операнды';uk='Допоміжні операнди'")));
...
Процедура ЗапускВыполненияФоновогоПроведенияПлана() Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Замещение/отмена замещения планов';uk='Заміщення/скасування заміщення планів'");
...
Процедура ВыполнитьФоновоеПроведенияПлана(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru='Фоновое замещение/отмена замещения планов остановлена. Количество неудачных попыток: %КоличествоОшибок%';uk='Фонове заміщення/скасування заміщення планів зупинено. Кількість невдалих спроб: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru='Фоновое замещение/отмена замещения плана';uk='Фонове заміщення/скасування заміщення плану'",КодОсновногоЯзыка),
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось выполнить блокировку регистра ""Замещение планов"" фоновым заданием. Количество попыток: %КоличествоОшибок%';uk='Не вдалося виконати блокування регістра ""Заміщення планів"" фоновим завданням. Кількість спроб: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
		
			ЗаписьЖурналаРегистрации(НСтр("ru='Фоновое замещение плана';uk='Фонове заміщення плану'",КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения);
...
Функция ВыполнитьПроведенияПлана(ПараметрыЗадания)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось выполнить замещение/отмену замещения плана %ЗамещенныйПлан% фоновым заданием по причине: %Причина%. Количество попыток: %КоличествоОшибок%';uk='Не вдалося виконати заміщення / скасування заміщення плану %ЗамещенныйПлан% фоновим завданням через: %Причина%. Кількість спроб: %КоличествоОшибок%'");
...
Процедура ОбновитьГиперссылкуЗамещение(Форма) Экспорт
	...
	Если ТЗЗамещенныеПланы.Количество() > 0 Тогда
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Замещенные (';uk='Заміщені ('") + ТЗЗамещенныеПланы.Количество() + ")",,ЦветаСтиля.ЦветТекстаУспех,,"Замещенные"));
	КонецЕсли;
	Если ТЗЗамещающиеПланы.Количество() > 0 Тогда
		Если МассивСтроки.Количество()>0 Тогда
			МассивСтроки.Добавить(", ");
		КонецЕсли;
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Замещающие (';uk='Заміщуючі ('") + ТЗЗамещающиеПланы.Количество() + ")",,ЦветаСтиля.ЦветТекстаПроблема,,"Замещающие"));
	КонецЕсли;
	Если МассивСтроки.Количество()= 0 Тогда
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Без замещения';uk='Без заміщення'"),,ЦветаСтиля.ЦветГиперссылки));
	КонецЕсли;
...
Процедура ОтменитьПроведениеПланов(МассивСсылок, ИмяДокумента) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ШаблонОшибки = НСтр("ru='Не удалось выполнить отмену проведения документа %Документ%. %ОписаниеОшибки%';uk='Не вдалося виконати скасування проведення документа %Документ%. %ОписаниеОшибки%'");
...
Функция ПроверитьПравилоЗаполнения(Знач ПравилоЗаполнения, Знач АдресСхемыПреобразованияДанных, Знач ВидПлана = Неопределено, Знач ИмяРеквизитаПравила = "", ОтборПоРесурсам) Экспорт 
	...
	Для каждого ЭлементПравила Из НайденныеСтроки Цикл
		...
		Если Реквизиты.СхемаКомпоновкиДанных = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru='Источник заполнения не заполнен.';uk='Джерело заповнення не заповнене.'");
	...
	Если Не ЕстьПравилаЗаполнения Тогда
		ПравилоКорректно = Ложь;
		ТекстОшибки = НСтр("ru='Не заполнены источники заполнения количества.';uk='Не заповнені джерела заповнення кількості.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,ИмяРеквизитаПравила);
	КонецЕсли;
	
	Если НЕ СоответствиеПолямДокумента ИЛИ НЕ СоответствиеПолямИсточников И НЕ НаличиеОбязательныхПолей Тогда
		ПравилоКорректно = Ложь;
	КонецЕсли;
	...
	ТекстОшибки = НСтр("ru='В списке полей источника отсутствует обязательное поле ""%ОбязательноеПоле%"". Отредактируйте группировки и выбранные поля.';uk='У списку полів джерела відсутня обов''язкове поле ""%ОбязательноеПоле%"". Відредагуйте групування і вибрані поля.'");
	ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ОбязательноеПоле%",ОбязательныеПоля[0]);
	...
	Если КоличествоИсточниковЗаполненияКоличестваПервогоУровня > 1 
		...
		ПравилоКорректно = Ложь;
		ТекстОшибки = НСтр("ru='Установлен отбор на поле ресурса ""Количество"", для корректного применения отбора источники должны быть сгруппированы на первом уровне.';uk='Встановлено відбір на поле ресурсу ""Кількість"", для коректного застосування відбору джерела повинні бути згруповані на першому рівні.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,ИмяРеквизитаПравила);
	КонецЕсли;
	...
	Если КоличествоИсточниковЗаполненияЦеныПервогоУровня > 1 
		...
		ПравилоКорректно = Ложь;
		ТекстОшибки = НСтр("ru='Установлен отбор на поле ресурса ""Цена"", для корректного применения отбора источники должны быть сгруппированы на первом уровне.';uk='Встановлено відбір на поле ресурсу ""Ціна"", для коректного застосування відбору джерела повинні бути згруповані на першому рівні.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,ИмяРеквизитаПравила);
	КонецЕсли;
	...
	Если (КоличествоИсточниковЗаполненияЦеныПервогоУровня > 1
		...
		ПравилоКорректно = Ложь;
		ТекстОшибки = НСтр("ru='Установлен отбор на поле ресурса ""Сумма"", для корректного применения отбора источники должны быть сгруппированы на первом уровне.';uk='Встановлено відбір на поле ресурсу ""Сума"", для коректного застосування відбору джерела повинні бути згруповані на першому рівні.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,ИмяРеквизитаПравила);
	КонецЕсли;
...
Процедура СбалансированностьПлановОбработкаПроверкиЗаполнения(КомпоновщикНастроек, Отказ) Экспорт 
	...
	Если Сценарий <> Неопределено И ЗначениеЗаполнено(Сценарий.Значение) Тогда
		...
		ИспользоватьРасчетПлановОстатков = ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеОстатков")
								...
								И КомпоновщикНастроек.Настройки.ДополнительныеСвойства.ВариантНаименование = НСтр("ru='Сбалансированность планов за период';uk='Збалансованість планів за період'");
		...
		Если ИспользоватьРасчетПлановОстатков Тогда
			...
			Если ПериодыПлановОстаткаЗаПериодБольшеНачалоПериода.Количество()>0 Тогда
				...
				ТекстСообщения = НСтр("ru='В пределах периода отчета найдены планы остатков в следующих периодах: ""%СтрокаПериодыПланаОстатков%"". Сбалансированность можно отобразить, если планы остатков не разрывают период отчета.';uk='У межах періоду звіту знайдені плани залишків в наступних періодах: ""%СтрокаПериодыПланаОстатков%"". Збалансованість можна відобразити, якщо плани залишків не розривають період звіту.'");
	...
	Если НЕ ПривилегированныйРежим() 
		...
		Если Объект.Статус = Перечисления.СтатусыПланов.Утвержден 
			...
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ТекстСообщения = НСтр("ru='Недостаточно прав для проведения документа в статусе ""Утвержден"".';uk='Недостатньо прав для проведення документа в статусі ""Затверджено"".'")
			Иначе
				ТекстСообщения = НСтр("ru='Недостаточно прав для отмены проведения документа в статусе ""Утвержден"".';uk='Недостатньо прав для скасування проведення документа в статусі ""Затверджено"".'")
			КонецЕсли;
...
Функция ПроверитьСуществованиеУжеСозданныхПланов(ДанныеФормы, ИмяТипаДокумента) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В базе уже создан документ за текущий период: %1 в статусе %2, который необходимо утвердить.';uk='У базі вже створено документ за поточний період: %1 в статусі %2, який необхідно затвердити.'"), 
...
// Параметры:
//  Сценарий  - Справочники.СценарииТоварногоПланирования - сценарий для которого производится расчет.
//  ВидПлана  - Справочники.ВидыПланов - вид плана для которого производится расчет.
//
Процедура ЗапускВыполненияФоновойПроверкиРасчетаДефицитаПоЭтапам(Сценарий, ВидПлана) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Проверка расчета дефицитов планов по этапам для вида плана';uk='Перевірка розрахунку дефіцитів планів по етапах для виду плану'") + Строка(ВидПлана);
...
Процедура ВыполнитьФоновуюПроверкуРасчетаДефицитовПоЭтапам(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru='Фоновая проверка расчета дефицитов планов по этапам остановлена. Количество неудачных попыток: %КоличествоОшибок%';uk='Фонова перевірка розрахунку дефіцитів планів по етапах зупинена. Кількість невдалих спроб:%КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru='Фоновая проверка расчета дефицитов по этапам';uk='Фонова перевірка розрахунку дефіцитів по етапах'",КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось выполнить блокировку регистра ""Очередь расчета дефицитов по этапам"" фоновым заданием. Количество попыток: %КоличествоОшибок%';uk='Не вдалося виконати блокування регістра ""Черга розрахунку дефіцитів по етапах"" фоновим завданням. Кількість спроб: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
...
// Функция проверяет расхождения в движениях планов и регистре "РасчетДефицитовПоЭтапам", результат записывается в регистр "ОчередьРасчетаДефицитовПоЭтапам"
// Параметры:
//  Сценарий  - Справочники.СценарииТоварногоПланирования - сценарий для которого производится расчет.
//  ВидПлана  - Справочники.ВидыПланов - вид плана для которого производится расчет.
//
Функция ТребуетсяПересчитатьДефициты(Сценарий, ВидПлана) Экспорт
	...
	Запрос.УстановитьПараметр("ТипПлана", ТипПлана);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "МЕСЯЦ", ОбщегоНазначения.ИмяЗначенияПеречисления(Периодичность));
...
Процедура ЗапускВыполненияФоновогоРасчетаДефицитаПоЭтапам(Сценарий) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Расчет дефицитов планов по этапам';uk='Розрахунок дефіцитів планів по етапах'");
...
Процедура ВыполнитьФоновыйРасчетДефицитовПоЭтапам(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru='Фоновый расчет дефицитов планов по этапам остановлен. Количество неудачных попыток: %КоличествоОшибок%';uk='Фоновий розрахунок дефіцитів планів по етапах зупинений. Кількість невдалих спроб: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru='Фоновый расчет дефицитов планов по этапам';uk='Фоновий розрахунок дефіцитів планів по етапах'",КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось выполнить блокировку регистра ""Очередь расчета дефицитов по планам"" фоновым заданием. Количество попыток: %КоличествоОшибок%';uk='Не вдалося виконати блокування регістра ""Черга розрахунку дефіцитів за планами"" фоновим завданням. Кількість спроб: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
	...
	Если ЗначениеЗаполнено(РезультатыРасчета)
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Не удалось выполнить формирование планов по дефициту по причине %Причина%';uk='Не вдалося виконати формування планів щодо дефіциту через %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Формирование планов по дефициту';uk='Формування планів з дефіциту'",КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения);
...
Функция ВыполнитьРасчетДефицитовПоЭтапам(Сценарий, Период) Экспорт
	...
	ЗапросПакет = Запрос.ВыполнитьПакет();
	ТаблицаНоменклатурыПоВидамПланов = ЗапросПакет[2].Выгрузить();
	НаборыДанныхОбъекты = Новый Структура;
	...
	ВыборкаВидыПлановСОтбором = ЗапросПакет[3].Выбрать();
	Счетчик = 0;
	Пока ВыборкаВидыПлановСОтбором.Следующий() Цикл
		
		Счетчик = Счетчик + 1;
		
		СтруктураНастроек = ВыборкаВидыПлановСОтбором.СтруктураНастроек.Получить();
	...
	|							И (ОборотыПоВидамПланов.ИнтервалПотребностиОт <= -РасчетПоЭтапамИВидамПланам.НачальныйОстаток + ЕСТЬNULL(ОборотыПоВидамПланов.КорректировкаРасхода, 0)
	|								ИЛИ ОборотыПоВидамПланов.ИнтервалПотребностиОт = 0)
	|							И (ОборотыПоВидамПланов.ИнтервалПотребностиДо >= -РасчетПоЭтапамИВидамПланам.НачальныйОстаток + ЕСТЬNULL(ОборотыПоВидамПланов.КорректировкаРасхода, 0)
	...
	|			И РасчетПоЭтапамИВидамПланам.Номенклатура = ОтборНоменклатурыПоВидамПланов.Номенклатура
	|			И РасчетПоЭтапамИВидамПланам.Характеристика = ОтборНоменклатурыПоВидамПланов.Характеристика
	|ГДЕ
...
Процедура ЗаполнитьПланыПоДефициту(Сценарий, ВидПлана, НачалоПериода, ОкончаниеПериода, ПланОбъект = Неопределено) Экспорт
	...
	Попытка
		...
		Если ПланОбъект = Неопределено Тогда
			...
			Если ВидПлана.ТипПлана = Перечисления.ТипыПланов.ПланЗакупок Тогда
				ПланОбъект = Документы.ПланЗакупок.СоздатьДокумент();
			ИначеЕсли ВидПлана.ТипПлана = Перечисления.ТипыПланов.ПланСборкиРазборки Тогда
		...
		Если ПараметрыВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланЗакупок
			...
			Планирование.ЗаполнитьТаблицуПериодов(ТаблицаПериоды, Сценарий.Периодичность, НачалоПериода, ОкончаниеПериода);
...
Функция СформироватьТекстЗапросаЗаполненияПоДефициту(ПараметрыВидаПлана, ПланОбъект, ЗаполнитьПоДефицитамИзПлана)
	...
	|ИЗ
	|	Документ."+ПланОбъект.Метаданные().Имя+"." + ИмяТабличнойЧасти + " КАК ПланТаблица
	|ГДЕ
	...
	|ЕСТЬNULL(ПланТаблица."+ИмяКолонкиПериода+",ТаблицаПересчитать."+ИмяКолонкиПериода+") КАК ПериодПланирования,
...
//
// Параметры:
//  Сценарий  - Справочники.СценарииТоварногоПланирования - сценарий для которого рассчитывается необходимость заполнения плана
//
...
//
// Параметры:
//  Сценарий  - Справочники.СценарииТоварногоПланирования - сценарий для которого выполняется полный пересчет
//
Процедура ЗапускВыполненияФоновогоПересчетаДефицитаПоЭтапам(Сценарий) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось выполнить блокировку регистров ""Очередь расчета дефицитов по планам"" и ""Расчет дефицитов по этапам""';uk='Не вдалося виконати блокування регістрів ""Черга розрахунку дефіцитів за планами"" і ""Розрахунок дефіцитів по етапах""'");
		ЗаписьЖурналаРегистрации(НСтр("ru='Запуск полного пересчета по дефицитам';uk='Запуск повного перерахунку по дефіцитах'",КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ТекстСообщения);
