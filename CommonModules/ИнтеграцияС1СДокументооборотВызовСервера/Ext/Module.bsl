...
//Возвращает:
//	Строку представления типа объекта BAS Документооборота
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
				НСтр("ru='Не удалось добавить связь';uk='Не вдалося додати зв''язок'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
Процедура УдалитьСвязь(ID, Тип, ИнтегрированныйОбъект) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
				НСтр("ru='Не удалось удалить связь';uk='Не вдалося вилучити зв''язок'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверитьПодключение(Знач АдресСервиса = Неопределено, Знач ИмяПользователя, Знач Пароль,
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ТекстСообщенияОбОшибке = НСтр("ru='Не указан адрес веб-сервиса BAS Документооборота.';uk='Не вказано адресу вебсервісу BAS Документообігу.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ИмяПользователя)
		...
		ТекстСообщенияОбОшибке = НСтр("ru='Не указано имя пользователя BAS Документооборота.';uk='Не вказано ім''я користувача BAS Документообігу.'");
		Возврат Ложь;
	КонецЕсли;
...
// Переключает хронометраж и возвращает параметры хронометража по объектам BAS Документооборота.
//
// Параметры:
		...
		Если Результат.documents.Количество() > 0 Тогда
			...
			Если ДанныеВозврата.type = "DMInternalDocument"
				...
				Если Объект.documentType = Неопределено Тогда
					ВызватьИсключение СтрШаблон(НСтр("ru='Невозможно получить данные BAS Документооборота: %1.
|Обратитесь к администратору.'
|;uk='Неможливо отримати дані BAS Документообігу: %1. 
...
//
// Параметры:
//   Форма - форма бизнес-процесса BAS Документооборота.
//   ДанныеОШаблоне - структура шаблона, содержащая поля:
//      - РеквизитID - идентификатор шаблона BAS Документооборота.
...
|з зауваженнями'"));
		...
		ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" Тогда
			...
			Если РеквизитыЗадачи.РезультатID = "НеСогласовано" Тогда
				Результат.ВыполнитьЗадачуПервая = НСтр("ru='Завершить согласование';uk='Завершити погодження'");
	...
	ИначеЕсли РеквизитыЗадачи.ПроцессТип = "DMBusinessProcessConfirmation" Тогда
		Если РеквизитыЗадачи.ТочкаМаршрутаКратко = "Утвердить" Тогда
			Если РеквизитыЗадачи.ЭтоПодписание Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Подписать';uk='Підписати'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Отклонить';uk='Відхилити'"));
			Иначе
		...
		ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" Тогда
			...
			Если РеквизитыЗадачи.РезультатID = "НеУтверждено" Тогда
				Если РеквизитыЗадачи.ЭтоПодписание Тогда
					Результат.ВыполнитьЗадачуПервая = НСтр("ru='Завершить подписание';uk='Завершити підписання'");
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить подписание...';uk='Повторити підписання...'"));
				Иначе
					Результат.ВыполнитьЗадачуПервая = НСтр("ru='Завершить утверждение';uk='Завершити затвердження'");
	...
	ИначеЕсли РеквизитыЗадачи.ПроцессТип = "DMBusinessProcessPerformance" Тогда
		...
		ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Снять с контроля';uk='Зняти з контролю'"));
		ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Проверить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Завершить';uk='Завершити'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Вернуть...';uk='Повернути...'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		КонецЕсли;
	...
	ИначеЕсли РеквизитыЗадачи.ПроцессТип = "DMBusinessProcessRegistration" Тогда
		Если РеквизитыЗадачи.ТочкаМаршрутаКратко = "Зарегистрировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Зарегистрировать';uk='Зареєструвати'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Не регистрировать';uk='Не реєструвати'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" Тогда
			Если РеквизитыЗадачи.РезультатID = "НеЗарегистрировано" Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Завершить регистрацию';uk='Завершити реєстрацію'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить регистрацию';uk='Повторити реєстрацію'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			Иначе
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Ознакомился';uk='Ознайомився'"));
			КонецЕсли;
	...
	ИначеЕсли РеквизитыЗадачи.ПроцессТип = "DMBusinessProcessIssuesSolution" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			...
			ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "РассмотрениеИнициатором"
				...
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Перенести срок';uk='Перенести строк'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Отказать в переносе';uk='Відмовити в перенесенні'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		...
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			...
			ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" Тогда
				...
				ИначеЕсли РеквизитыЗадачи.РезультатID = "НеПринятоОбязательнымиУчастниками"
					...
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Повторить приглашение...';uk='Повторити запрошення...'"));
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Отменить приглашение';uk='Скасувати запрошення'"));
					Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
				КонецЕсли;
...
// Создает файлы печатных форм объектов по имени команды формы документа BAS Документооборот и присоединяет их к документам.
//
// Параметры:
//   ВнешнийОбъект - ЛюбаяСсылка - ссылка на объект текущей базы данных.
...
Функция ПолучитьФайлИПоместитьВХранилище(ID, УникальныйИдентификатор, ДатаЗаема = Неопределено,
	...
	Иначе // получение с захватом
		Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
			ВызватьИсключение НСтр("ru='Заем файлов не поддерживается используемой версией BAS Документооборота';uk='Позика файлів не підтримується використовуваної версією BAS Документообігу'");
		КонецЕсли;
...
Функция СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, ТекстСообщения = "") Экспорт
	...
	Если Не ДоступенФункционалВерсииСервиса("1.4.6.1") Тогда
		ТекстСообщения = 
			НСтр("ru='Для работы с версиями нужен BAS Документооборот версии не ниже 1.4.6.1';uk='Для роботи з версіями потрібен BAS Документообіг версії не нижче 1.4.6.1'");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьПоляФайлаДаннымиВладельца(ID) Экспорт
	...
	Если Не ДоступенФункционалВерсииСервиса("2.0.15.1") Тогда
		ТекстСообщения = 
			НСтр("ru='Для заполнения файлов нужен BAS Документооборот версии не ниже 2.0.15.1';uk='Для заповнення файлів потрібен BAS Документообіг версії не нижче 2.0.15.1'");
...
Функция СтруктураРеквизитовЗаполняемогоВидаДокументовДО(ТипОбъектаДО) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Тип ""%1"" не относится к видам документов ДО';uk='Тип "" %1"" не відноситься до видів документів ДО'"),
			ТипОбъектаДО);
	КонецЕсли;
