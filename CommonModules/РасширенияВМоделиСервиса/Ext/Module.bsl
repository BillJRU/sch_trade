...
Функция ПоставляемоеРасширение(ИдентификаторИспользуемогоРасширения) Экспорт
	...
	Если НЕ ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ru='Использование функции РасширенияВМоделиСервиса.ПоставляемоеРасширение()
                                ...
                                |є тільки для сеансів з встановленим розподілом даних'");
	...
	Попытка
		...
		Попытка
			...
			ИначеЕсли ИзменяетСтруктуруДанных Тогда
				ВызватьИсключение НСтр("ru='Не возможно удалить расширение изменяющее структуру данных без монопольного режима.';uk='Неможливо вилучити розширення що змінює структуру даних без монопольного режиму.'");
			КонецЕсли;
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Удаление из области данных';uk='Розширення, що поставляються. Видалення з області даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка удаления из области данных';uk='Розширення, що поставляються. Помилка видалення з області даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиРасширенияВОбластьДанных(Знач ПоставляемоеРасширение, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка установки в область данных';uk='Розширення, що поставляються. Помилка встановлення в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,
		ПоставляемоеРасширение,
...
Процедура НачатьУстановкуПоставляемогоРасширенияВОбластьДанных(Знач ОписаниеИнсталляции, Знач Ответственный, Знач ПоставляемоеРасширение)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые расширения.Инициирована установка поставляемого расширения в область данных';uk='Розширення, що поставляються. Ініційован встановлення розширення, що поставляється, в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
	...
	Если НЕ ОбщегоНазначения.СсылкаСуществует(ПоставляемоеРасширение) Тогда	
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных отложена';uk='Розширення, що поставляються. Встановлення в область даних відкладено'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			Строка(ОписаниеИнсталляции.Идентификатор),
			Строка(ОписаниеИнсталляции.Инсталляция));
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных';uk='Розширення, що поставляються. Встановлення в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		ПоставляемоеРасширение,
		Строка(ОписаниеИнсталляции.Инсталляция));
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать, Знач ДескрипторJSON = Ложь) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Если НЕ Загружать Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения отменена';uk='Розширення, що поставляються. Завантаження розширення скасовано'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,,
...
//	* ИдентификаторВерсии - УникальныйИдентификатор - см. Справочник.ПоставляемыеРасширения.
Функция РасширенияТекущийОбластиДанных() Экспорт 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИспользованиеРасширений.ПоставляемоеРасширение КАК ПоставляемоеРасширение,
		|	ИспользованиеРасширений.ИспользуемоеРасширение КАК ИспользуемоеРасширение,
		|	ИспользованиеРасширений.Инсталляция КАК Инсталляция,
		|	ИспользованиеРасширений.ПоставляемоеРасширение.Отключено КАК Отключено,
		|	ИспользованиеРасширений.ПоставляемоеРасширение.ПричинаОтключения КАК ПричинаОтключения,
		|	ИспользованиеРасширений.ПоставляемоеРасширение.Наименование КАК Наименование,
		|	ИспользованиеРасширений.ПоставляемоеРасширение.ИдентификаторВерсии КАК ИдентификаторВерсии
		|ИЗ
		|	РегистрСведений.ИспользованиеПоставляемыхРасширенийВОбластяхДанных КАК ИспользованиеРасширений";
...
Функция АктивироватьОтключенныеРасширенияВОбласти(Знач НомерОбласти, ЕстьРасширенияИзменяющиеСтруктуру) Экспорт 
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОбластьДанных", НомерОбласти);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	1
	|ИЗ
	...
	|ГДЕ
	|	ИспользованиеПоставляемыхРасширенийВОбластяхДанных.ПоставляемоеРасширение.ИзменяетСтруктуруДанных
	|	И ИспользованиеПоставляемыхРасширенийВОбластяхДанных.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных";
...
Процедура ОбработатьПоставляемыеРасширения(Дескриптор, ПутьКФайлу, ДескрипторJSON)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения';uk='Розширення, що поставляються. Завантаження розширення'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
	...
	Если ПоставляемоеРасширение.ИдентификаторВерсии <> ОписаниеПоставляемогоРасширения.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения завершена';uk='Розширення, що поставляються. Завантаження розширення завершено'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,,
		...
		Для Каждого ИнсталляцияРасширения Из ИспользуемыеРасширения Цикл
			...
			ПараметрыЗадания.Вставить("ОжидатьЗавершенияВыполненияРегламентногоЗадания", ПараметрыЗадания.ВыполнятьЧерезРегламентноеЗадание);
			
			ОчередьЗаданий.ДобавитьЗадание(ПараметрыЗадания);
			
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Запланирована актуализация используемого расширения';uk='Розширення, що поставляються. Запланована актуалізація використовуваного розширення'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Запланирована отложенная установка поставляемого расширения в область данных';uk='Розширення, що поставляються. Заплановано відкладене встановлення розширення в область даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
...
Процедура ВыполнитьУстановкуМонопольногоРежима(Знач Ответственный)
	...
	ВызватьИсключение НСтр("ru='Не удалось установить монопольный режим для установки расширения.';uk='Не вдалося встановити монопольний режим для установки розширення.'");
...
Процедура ЗапланироватьУдалениеРасширенияИзОбласти(Знач ПоставляемоеРасширение, Знач Ответственный, Знач Область = Неопределено)
	...
	ПараметрыЗадания.Вставить("ОжидатьЗавершенияВыполненияРегламентногоЗадания", Истина);
