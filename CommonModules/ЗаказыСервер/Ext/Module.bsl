...
|;uk='Дублюється значення реквізиту ""КодСтроки"" рядка %1 і рядка %2 замовлення.
|Документ створений некоректно.'");
...
Функция СкорректироватьСтрокиЗаказа(ЗаказОбъект, СтруктураДополнительныхПараметров) Экспорт
	...
	Если ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
		...
		Если СтруктураРезультата.КоличествоСтрок <> 0 ИЛИ СтруктураРезультата.СуммаКорректировки > 0 Тогда
			...
			ПараметрыВзаиморасчеты = Документы.ЗаказКлиента.ПараметрыВзаиморасчеты(ЗаказОбъект.ХозяйственнаяОперация);
			ВзаиморасчетыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ЗаказОбъект, ПараметрыВзаиморасчеты);
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказПоставщику") Тогда
		...
		Если СтруктураРезультата.КоличествоСтрок <> 0 Тогда
			...
			ПараметрыВзаиморасчеты = Документы.ЗаказПоставщику.ПараметрыВзаиморасчеты(ЗаказОбъект.ХозяйственнаяОперация);
			ВзаиморасчетыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ЗаказОбъект, ПараметрыВзаиморасчеты);
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		Если СтруктураРезультатаЗТ.КоличествоСтрок > 0 ИЛИ СтруктураРезультатаЗТ.СуммаКорректировки > 0 Тогда
			...
			ПараметрыВзаиморасчеты = Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПараметрыВзаиморасчеты(ЗаказОбъект.ХозяйственнаяОперация);
			ВзаиморасчетыСервер.ПроверитьЗаполнитьЭтапыГрафикаОплаты(ЗаказОбъект, ПараметрыВзаиморасчеты);
		КонецЕсли;
...
Функция РезультатОтменыНеотработанныхСтрок(ОтмененоСтрок, Модифицированность = Ложь) Экспорт
	...
	Если ОтмененоСтрок = Неопределено Тогда
		РезультатОтмены.Оповещение = НСтр("ru='Отмена неотработанных строк не выполнена';uk='Скасування невідпрацьованих рядків не виконано'");
		РезультатОтмены.КоличествоСтрок = 0;
	ИначеЕсли ОтмененоСтрок = 0 Тогда
		РезультатОтмены.Оповещение = НСтр("ru='Строки не отменены, поскольку полностью отработаны';uk='Рядки не скасовані, оскільки повністю відпрацьовані'");
		РезультатОтмены.КоличествоСтрок = 0;
	Иначе
		РезультатОтмены.Оповещение = СтрШаблон(НСтр("ru='Отменено неотработанных строк: %1';uk='Скасовано невідпрацьованих рядків: %1'"), ОтмененоСтрок);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Не удалось заблокировать объект при автоматической корректировке строк мерных товаров.';uk='Не вдалося заблокувати об''єкт при автоматичному коригуванні рядків мірних товарів.'",КодОсновногоЯзыка),
		...
		Если ЗаказИзменен Тогда
			Попытка
				...
				ТекстСообщения = НСтр("ru='Выполнена корректировка строк мерных товаров по заказу %Заказ%';uk='Виконана коригування рядків мірних товарів на замовлення %Заказ%'",КодОсновногоЯзыка);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Окончание корректировки строк мерных товаров.';uk='Закінчення коригування рядків мірних товарів.'",КодОсновногоЯзыка),
					УровеньИнформация,
					,
					Выборка.Заказ,
					ТекстСообщения);
...
|;uk='Не вдалося виконати коригування рядків
| мірних товарів на замовлення %Заказ% через: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Фоновая корректировка строк заказов мерных товаров';uk='Фонове коригування рядків замовлень мірних товарів'",КодОсновногоЯзыка),
...
|;uk='Не вдалося видалити %Заказ% з черги на коригування рядків 
| мірних товарів через: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
