...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон.ПредназначенДляSMS Тогда
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ru='Для отправки сообщения необходимо ввести номер телефонов получателей.';uk='Для відправки повідомлення необхідно ввести номер телефонів одержувачів.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
			...
			Иначе
				...
				Результат.ОписаниеОшибки = НСтр("ru='Сообщение SMS не может быть отправлено сразу.';uk='SMS повідомлення не може бути відправлено відразу.'");
	...
	Иначе
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ru='Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.';uk='Повідомлення не може бути відправлено відразу, оскільки необхідно ввести адресу електронної пошти.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			Иначе
				...
				Результат.ОписаниеОшибки  = НСтр("ru='Сообщение не может быть отправлено сразу.';uk='Повідомлення не може бути відправлено відразу.'");
				Возврат Результат;
...
Процедура ОпределитьСписокРеквизитовИВложений(СведенияОШаблоне, ПараметрыШаблона)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		Если МетаданныеОбъект <> Неопределено Тогда
			...
			СсылкаНаОбъект.Представление = НСтр("ru='Ссылка на';uk='Посилання на'") + " """ + Представление + """";
	...
	Для каждого ПроизвольныйПараметр Из ПараметрыШаблона.Параметры Цикл
		...
		Если ПроизвольныйПараметр.Значение.ОписаниеТипа.Типы().Количество() > 0 Тогда
			...
			Иначе
				ПроизвольныеПредставление = НСтр("ru='Произвольные';uk='Довільні'");
...
Функция ВидыШаблонов() Экспорт
	...
	ТипыШаблонов.Добавить("Email", НСтр("ru='Электронного письма';uk='Електронного листа'"));
	ТипыШаблонов.Добавить("SMS", НСтр("ru='Сообщения SMS';uk='Повідомлення SMS'"));
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Владелец, Сведения, ИмяФайла, Размер, КоличествоПустыхИменВоВложениях = 0) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Если ПустаяСтрока(ИмяБезРасширения) Тогда
			...
			ИмяБезРасширения = НСтр("ru='Вложение без имени';uk='Вкладення без імені'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						...
						УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Ошибка при создании внешней печатной формы. По причине:';uk='Помилка при створенні зовнішньої друкованої форми. По причині:'") + Символы.ПС
...
Процедура ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ПараметрыШаблона) Экспорт
	...
	Если ЕстьНезаполненныйПараметр Тогда
		ВызватьИсключение НСтр("ru='Не удалось сформировать сообщение, т.к. отсутствуют необходимые параметры.';uk='Не вдалося сформувати повідомлення, оскільки відсутні необхідні параметри.'");
	КонецЕсли;
...
Функция ЗаголовокОбщиеРеквизиты() Экспорт
	Возврат НСтр("ru='Общие реквизиты';uk='Загальні реквізити'");
КонецФункции
...
Функция ОпределитьОбщиеРеквизиты() Экспорт
	...
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущаяДата", НСтр("ru='Текущая дата';uk='Поточна дата'"), Новый ОписаниеТипов("Дата"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ЗаголовокСистемы", НСтр("ru='Заголовок системы';uk='Заголовок системи'"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "АдресБазыВИнтернете", НСтр("ru='Адрес базы в Интернете';uk='Адреса бази в Інтернеті'"), Новый ОписаниеТипов("Строка"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "АдресБазыВЛокальнойСети", НСтр("ru='Адрес базы в локальной сети';uk='Адреса бази даних у локальній мережі'"), Новый ОписаниеТипов("Строка"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущийПользователь", НСтр("ru='Текущий пользователь';uk='Поточний користувач'"), Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru='Формирование шаблона сообщений';uk='Формування шаблону повідомлень'",ОбщегоНазначения.КодОсновногоЯзыка());
