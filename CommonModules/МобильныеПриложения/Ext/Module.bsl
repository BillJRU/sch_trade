...
Процедура ОбработатьЗаданиеТорговомуПредставителюПередЗаписьюВПакет(Объект, СтруктураПараметровОбменаДанными)
	...
	Если ТребуетсяЗаполнениеЗаданийПоДаннымИсторииПродаж ИЛИ ТребуетсяЗаполнениеДанныхОРасчетахСКлиентами Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при записи задания торговому представителю';uk='Помилка при запису завдання торговому представнику'",КодОсновногоЯзыка),
...
Процедура СоздатьЗаказКлиентаПоЗаданию(Задание)
	...
	Исключение
		...
		ТекстСообщения = ИнформацияОбОшибке().Описание;
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при записи заказа клиента';uk='Помилка при запису замовлення клієнта'",КодОсновногоЯзыка),
								УровеньЖурналаРегистрации.Ошибка,
								,
								,
								ТекстСообщения);
	...
	Для Каждого СтрокаТаблицыЗаписей Из МассивСтрокТаблицыЗаписей Цикл
		...
		Для Каждого Поле Из МассивПолей Цикл
			...
			Исключение
				...
				ТекстСообщения = ИнформацияОбОшибке().Описание;
				ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при формировании пакета обмена';uk='Помилка при формуванні пакета обміну'",КодОсновногоЯзыка),
								УровеньЖурналаРегистрации.Ошибка,
								,
								,
								ТекстСообщения);
...
//
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - объект метаданных, которому принадлежит реквизит.
//  ИмяКласса - имя класса, к которому принадлежит объект (Справочники, Документы).
...
//
// Возвращаемое значение:
//  Массив из Строка - массив, содержащий имена полей регистра.
//
