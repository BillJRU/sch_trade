	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если Получатели.Количество() > 0
			...
			Иначе
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными';uk='Обмін даними'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СформироватьНаименованиеПредопределенногоУзла() Экспорт
	...
	ИмяПриложенияПоУмолчанию = НСтр("ru='Приложение в Интернете';uk='Прикладна програма в Інтернеті'");
...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	ОписаниеВыполняемогоДействияЖР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполнение шага сценария синхронизации (%1/%2).';uk='Виконання кроку сценарію синхронізації (%1/%2).'"), ИндексСтрокиСценария + 1, СценарийОбменаДанными.Количество())
		...
		Попытка
			...
			Если Не ОбменДаннымиСервер.НастройкаСинхронизацииЗавершена(УзелИнформационнойБазы) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Настройка синхронизации данных в приложении ""%1"" не завершена.';uk='Настройка синхронізації даних в додатку ""%1"" не завершена.'"),
					УзелИнформационнойБазы);
			ИначеЕсли ОбменДаннымиСервер.ПолученоСообщениеСДаннымиДляСопоставления(УзелИнформационнойБазы) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В приложение ""%1"" загружено сообщение для ручного сопоставления.';uk='В прикладну програму ""%1"" завантажено повідомлення для ручного співставлення.'"),
					УзелИнформационнойБазы);
	...
	ИначеЕсли СтрокаСценария.НомерИнформационнойБазы = 2 Тогда
		...
		Попытка
			...
			Если ОписаниеWSПрокси.WSПрокси = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось получить описание WS-прокси для приложения ""%1"".';uk='Не вдалося отримати опис WS-проксі для програми ""%1"".'"),
					УзелИнформационнойБазы);
			Иначе
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые правила обмена данными.Загрузка поставляемых правил отменена';uk='Поставляються правила обміну даними.Завантаження поставляються правил скасовано'",ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
...
Функция МенеджерПланаОбменаКонечныхТочек() Экспорт
	...
	Если МетаданныеОбменСообщениями = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не определен план обмена конечных точек.';uk='Не визначено план обміну кінцевих точок.'");
	КонецЕсли;
...
Процедура ЗавершитьВыполнениеСценарияОбменаДанными(ИндексСтрокиСценария, СценарийОбменаДанными, СтруктураРезультат = Неопределено)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииСинхронизацииДанных(),
		УровеньЖурналаРегистрации.Информация, , , НСтр("ru='Завершение выполнения сценария синхронизации.';uk='Завершення виконання сценарію синхронізації.'"));
...
Функция ОписаниеСтрокиСценарияОбменаДанными(ИндексСтрокиСценария, СценарийОбменаДанными)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='ПорядковыйНомер: %1, Приложение: %2, Действие: %3(%4), Режим: %5.';uk='ПорядковийНомер: %1, Прикладна програма: %2, Дія: %3(%4), Режим: %5.'"),
...
Функция ОтправитьСообщение(Знач Сообщение) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса") Тогда
		ВызватьИсключение НСтр("ru='Отсутствует менеджер сервиса.';uk='Відсутній менеджер сервісу.'");
	КонецЕсли;
...
Процедура СохранитьДанныеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	СтрокаСообщения = НСтр("ru='Сессия обмена сообщениями системы ""%1"" успешно завершена.%2';uk='Сесія обміну повідомленнями системи ""%1"" успішно завершена.%2'",ОбщегоНазначения.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
...
Процедура ЗафиксироватьНеуспешноеВыполнениеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	СтрокаСообщения = НСтр("ru='Ошибка выполнения сессии обмена сообщениями системы ""%1"".%2
        ...
        |Опис помилки з кореспондента: %3'",ОбщегоНазначения.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
...
Функция СобытиеЖурналаРегистрацииНастройкаСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Настройка синхронизации данных';uk='Обмін даними в моделі сервісу.Настройка синхронізації даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииМониторСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Монитор синхронизации данных';uk='Обмін даними в моделі сервісу.Монітор синхронізації даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Синхронизация данных';uk='Обмін даними в моделі сервісу.Синхронізація даних'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСессииОбменаСообщениямиСистемы()
	
	Возврат НСтр("ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы';uk='Обмін даними в моделі сервісу.Сесії обміну повідомленнями системи '",ОбщегоНазначения.КодОсновногоЯзыка());
