...
Процедура ОбновитьТекстГиперссылкиОграничениеЗадолженности(Форма) Экспорт
	...
	Для Каждого СтруктураПараметров Из ДополненныеПараметрыМеханизма.МассивПараметров Цикл
		...
		ЕстьРасчетыПоДоговорам = СтруктураПараметров.ИзменяетРасчеты 
									И (СтруктураПараметров.ДокументРасчетовСКлиентами И СистемныеНастройки.ИспользоватьДоговорыСКлиентами
		...
		Если ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов)
			...
			Если ПревышенаСумма ИЛИ ПревышенСрок Тогда
				ЭлементТекст.Заголовок = НСтр("ru='Отгрузка запрещена';uk='Відвантаження заборонене'");
			...
			ИначеЕсли ДанныеДоговора.ОграничиватьСуммуЗадолженности Тогда
				ЭлементТекст.Заголовок = НСтр("ru='Остаток допустимого кредита:';uk='Залишок припустимого кредиту:'") + " " + Формат(ОстатокДопустимогоКредита, "ЧДЦ=2; ЧН=0,00") + " " + Строка(ДанныеДоговора.ВалютаВзаиморасчетов);
				ЭлементТекст.ЦветТекста = ЦветаСтиля.ЦветГиперссылки;
				ЭлементТекст.Видимость = Истина;
...
// Возвращает признак того, что разрешено использовать разные валюты документа и взаиморасчетов
//
// Параметры:
//	Организация - СправочникСсылка.Организации - Ссылка на элемент справочника Организация.
//	Контрагент  - СправочникСсылка.Контрагенты - Ссылка на элемент справочника Контрагенты.
//
// Возвращаемое значение:
//	Булево - признак.
//
Функция РазрешеныРазличныеВалюты(Знач Организация, Знач Контрагент) Экспорт
    
    РазрешеныРазличныеВалюты = Ложь;
    Если Организация = Справочники.Организации.УправленческаяОрганизация Тогда
        РазрешеныРазличныеВалюты = Истина;    
    ИначеЕсли Контрагент = Неопределено Тогда
        РазрешеныРазличныеВалюты = Ложь;    
    ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ЮрФизЛицо") = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
        РазрешеныРазличныеВалюты = Истина;    
    Иначе
        РазрешеныРазличныеВалюты = Ложь;    
    КонецЕсли;     
    
    Возврат РазрешеныРазличныеВалюты;
