...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru='Начато регламентное получение и отправка электронной почты';uk='Розпочато регламентне отримання та відправлення електронної пошти'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончено регламентное получение и отправка электронной почты';uk='Закінчено регламентне отримання та відправлення електронної пошти'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ОбработатьОшибкуОтправкиПисьма(Знач ПисьмоОбъект, Знач ВыборкаПисьма, Знач ПараметрыПисьма, Знач Интерактивно)
	...
	Если ЭтоПроблемаОтвергнутыхСерверомПочтовыхАдресов И Не ВсеАдресатыПисьмаОтвергнутыСервером Тогда
		ШаблонСообщенияОбОшибке = НСтр("ru='Следующие адресаты письма %1 не приняты почтовым сервером:
            ...
            |;uk='Наступні адресати листа %1 не прийняті поштовим сервером: 
            |%2. Лист відправлено решті адресатів.'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		ШаблонСообщенияОбОшибке = НСтр("ru='Не удалось отправить письмо %1 по причине:
            |%2'
            |;uk='Не вдалося відправити лист %1 з причини:
...
Процедура ЗагрузитьПочтуПользователя(Результат)
	...
	ДлительныеОперации.СообщитьПрогресс(, НСтр("ru='Получение почты';uk='Отримання пошти'"));
	...
	Если Результат.ДоступноУчетныхЗаписей = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Нет доступных учетных записей для получения почты.';uk='Немає доступних облікових записів для отримання пошти.'"));
...
Процедура ОтправитьПочтуПользователя(Результат)
	...
	ДлительныеОперации.СообщитьПрогресс(, НСтр("ru='Отправка почты';uk='Відправлення пошти'"));
...
Процедура СинхронизироватьПризнакРассмотреноССервером(Почта, ДанныеУчетнойЗаписи, МассивЗагруженныхПисем)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru='Не удалось изменить информацию о признаке рассмотрено для писем по причине : %Причина%.';uk='Не вдалося змінити інформацію про ознаку розглянуто для листів через : %Причина%.'");
...
Процедура УдалитьИдентификаторыРанееПолученныхПисем(УчетнаяЗапись, ИдентификаторыНаСервере, ИдентификаторыУдалять)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru='Не удалось очистить информацию о ранее полученных идентификаторах по причине: %Причина%.';uk='Не вдалося очистити інформацію про раніше отримані ідентифікатори через: %Причина%.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не установить письмо основание у %1 по причине %2';uk='Не встановити лист підставу у %1 з причини %2'"), 
...
Процедура ЗаписатьВложениеЭлектронногоПисьма(Объект, Вложение,МассивПодписей,КоличествоПустыхИменВоВложениях)
	...
	Если ЕстьПодписи Тогда
		...
		Для Каждого ПодписьВложения Из МассивПодписей Цикл
			...
			ПараметрыПодписи.Вставить("Комментарий", НСтр("ru='Вложение электронного письма';uk='Вкладення електронної пошти'"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Взаимодействия';uk='Взаємодії'",ОбщегоНазначения.КодОсновногоЯзыка());
