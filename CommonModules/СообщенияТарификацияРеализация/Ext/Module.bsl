	...
	Попытка
		...
		Для Каждого УслугаXDTO Из ОбщиеДанныеПоТарификации.Services Цикл 
			...
			Если ПоставщикУслуги.Пустая() Тогда 
				ВызватьИсключение НСтр("ru='Не найден поставщик услуги с кодом:';uk='Не знайдений постачальник послуги з кодом:'") + " " + УслугаXDTO.ServiceProviderID;
			КонецЕсли;
...
Процедура ОбновитьДанныеАбонентаПоТарификацииВПриложении(Знач ДанныеАбонентаПоТарификации) Экспорт
	...
	ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации()
		+ НСтр("ru='Обновление данных по тарификации в области данных';uk='Оновлення даних по тарифікації в області даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УдалитьДанныеПоТарификацииВПриложениеАбонента(Знач УдаляемыеПодписки) Экспорт
	...
	Для Каждого Подписка Из УдаляемыеПодписки.SubscriptionsID Цикл
		...
		Исключение
			...
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации()
				+ НСтр("ru='Удаление данных по подписке';uk='Вилучення даних за підпискою'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура ОбновитьДанныеПоЛицензиям(ОбновляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации()
				+ НСтр("ru='Обновление данных по лицензиям';uk='Оновлення даних по ліцензіях'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура УдалитьДанныеПоЛицензиям(УдаляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации()
				+ НСтр("ru='Удаление данных по лицензиям';uk='Вилучення даних за ліцензіями'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
