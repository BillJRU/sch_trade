...
//
Функция ЗаголовкиПолейЕдиницИзмерений(КомпоновщикНастроек) Экспорт
...
// Параметры:
//		КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - настройки отчета.
//		Скобки - Булево - Флаг необходимости добавления скобок.
// Возвращаемое значение:
//		Структура - в ключе задается шаблон замены заголовка, в значении - значение замены.
//		 *Ключ - Строка - шаблон замены заголовка.
//		 *Значение - Строка - значение замены.
...
Функция СтруктураЗаголовковВалютСквознаяСебестоимость(КомпоновщикНастроек, Скобки = Истина) Экспорт
	...
	Если ИспользоватьНесколькоВалют Тогда
		...
		Если ДанныеОтчета = 1 
			Или ДанныеОтчета = 2
            Или ДанныеОтчета = 3 Или ДанныеОтчета = 5 Тогда
			СтруктураЗаголовковПолей.Вставить("ВалютаОтчета", ?(Скобки, "(", "") + ВалютаУправленческогоУчета + ?(Скобки, ")", ""));
        ИначеЕсли ДанныеОтчета = 4 Или ДанныеОтчета = 6 Тогда
			СтруктураЗаголовковПолей.Вставить("ВалютаОтчета", ?(Скобки, "(", "") + ВалютаРегламентированногоУчета + ?(Скобки, ")", ""));
		КонецЕсли;
			...
			Если ДополнятьПериод Тогда
				...
				Если ПараметрПериод <> Неопределено Тогда
					...
					Для Каждого Группировка Из Группировки Цикл
						...
						Если ЭлементГруппировки.ПоляГруппировки.Элементы.Количество() = 1
							И ЭлементГруппировки.ПоляГруппировки.Элементы[0].Поле = ПолеДинамическийПериод Тогда
...
//
Функция УстановитьПараметрСКД(СхемаКомпоновкиДанных, Параметр, Значение) Экспорт
...
Функция НайтиПолеСКДПоПолномуИмени(Элементы, ПолноеИмя) Экспорт
	...
	Поле = НайтиПолеСКДПоИмени(Элементы, текИмя); // ДоступноеПолеКомпоновкиДанных -
	...
	Для Сч = 2 По колЧастей Цикл
		...
		Поле = НайтиПолеСКДПоИмени(Поле.Элементы, текИмя); // ДоступноеПолеКомпоновкиДанных -
		Если Поле = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
...
//
// Параметры
//  НастройкиКомпоновкиДанных - КомпоновщикНастроек, НастройкиКомпоновкиДанных - Настройки.
...
//	Параметры - Структура - параметры замены текста заголовка поля компоновки данных.
...
Функция СкопироватьНаборыДанных(ПриемникНаборовДанных, ИсточникНаборовДанных, ОсновноеИмяНабораДанных = "") Экспорт
	...
	Иначе 
		ВызватьИсключение НСтр("ru='Некорректный тип параметра 2';uk='Некоректний тип параметра 2'") ;
	КонецЕсли;
...
Функция ДобавитьСвязьНаборовДанных(СхемаКомпоновкиДанных, НаборДанныхИсточник, НаборДанныхПриемник, ВыражениеИсточник, ВыражениеПриемник, Параметр = Неопределено, РазрешенСписокПараметров = Ложь) Экспорт
	...
	Иначе 
		ВызватьИсключение НСтр("ru='Некорректный тип параметра 3';uk='Некоректний тип параметра 3'") ;
	КонецЕсли;
...
Функция ВыгрузитьРезультатСКД(Параметры, АдресВременногоХранилища = "") Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru='Неизвестный тип результата %ТипРезультата%';uk='Невідомий тип результату %ТипРезультата%'");
...
Функция ФорматнаяСтрокаПериодовКомпоновкиДанных(Периодичность)
	...
	Если Периодичность = Перечисления.Периодичность.День Тогда
		ФорматнаяСтрока = НСтр("ru='ДЛФ=D';uk='ДЛФ=D'");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда
		ФорматнаяСтрока = НСтр("ru='ДЛФ=D';uk='ДЛФ=D'");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
		ФорматнаяСтрока = НСтр("ru='ДЛФ=D';uk='ДЛФ=D'");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Месяц Тогда
		ФорматнаяСтрока = НСтр("ru='Л=ru; ДФ=''ММММ гггг''';uk='Л=uk; ДФ=''MMMM yyyy'''");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Квартал Тогда
		ФорматнаяСтрока = НСтр("ru='ДФ=''%1 ""кв."" гггг''';uk='ДФ=''%1 ""кв."" гггг'''");
	...
	Если ТипЗнч(НастройкиКомпоновки) = Тип("Строка") Тогда
		Если ЭтоАдресВременногоХранилища(НастройкиКомпоновки) Тогда
			...
			Если ТипЗнч(НастройкиКомпоновки) <> Тип("НастройкиКомпоновкиДанных") Тогда
				ТекстИсключения = НСтр("ru='По переданному адресу временного хранилища не удалось получить настройки компоновки данных.';uk='За переданому адресою тимчасового сховища не вдалося отримати настройки компонування даних.'");
...
Функция СхемаКомпоновкиПоПараметру(Знач СхемаКомпоновки) Экспорт
	...
	Если ТипЗнч(СхемаКомпоновки) = Тип("Строка") Тогда
		Если ЭтоАдресВременногоХранилища(СхемаКомпоновки) Тогда
			...
			Если ТипЗнч(СхемаКомпоновки) <> Тип("СхемаКомпоновкиДанных") Тогда
				ТекстИсключения = НСтр("ru='По переданному адресу временного хранилища не удалось получить схему компоновки данных.';uk='За переданому адресою тимчасового сховища не вдалося отримати схему компанування даних.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СхемаКомпоновки) = Тип("СхемаКомпоновкиДанных") Тогда
		Возврат СхемаКомпоновки;
	Иначе
		ТекстИсключения = НСтр("ru='Неподдерживаемый тип параметра ""СхемаКомпоновки""';uk='Непідтримуваний тип параметра ""СхемаКомпоновки""'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
