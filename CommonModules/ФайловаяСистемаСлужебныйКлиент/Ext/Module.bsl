...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено
		...
		ВызватьИсключение НСтр("ru='Невозможно загрузить файл без установленного расширения работы с файлами.';uk='Неможливо завантажити файл без встановленого розширення роботи з файлами.'");
	КонецЕсли;
...
Процедура ОткрытьФайлВПрограммеПросмотра(ПутьКФайлу, Знач Оповещение = Неопределено,
	...
	ТекстПредложения = НСтр("ru='Для открытия файла необходимо установить расширение для работы с BAF.';uk='Для відкриття файлу необхідно встановити розширення для роботи з BAF.'");
...
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = НСтр("ru='Не установлено расширение для работы с BAF, открытие файла недоступно.';uk='Не встановлено розширення для роботи з BAF, відкриття файлу недоступно.'");
...
Процедура ОткрытьПроводникПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		ФайлИнфо.НачатьПроверкуСуществования(Оповещение);
	Иначе
		ОписаниеОшибки = НСтр("ru='Не установлено расширение для работы с BAF, открытие папки недоступно.';uk='Не встановлено розширення для роботи з BAF, відкриття папки недоступно.'");
	...
	Иначе 
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена папка, которую требуется открыть в проводнике:
                       |""%1""'
                       |;uk='Не знайдена папка, яку потрібно відкрити в провіднику:
                       |""%1""'"),
			ФайлИнфо.ПолноеИмя);
		ОткрытьПроводникОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ЗапуститьПрограммуПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Не установлено расширение для работы с BAF, запуск программы невозможен.';uk='Не встановлено розширення для роботи з BAF, запуск програми неможливий.'");
	...
	Иначе 
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось запустить программу
                       ...
                       |%2'
                       |;uk='Не вдалося запустити програму
                       |%1
                       |з причини:
                       |Не існує каталог, що вказаний як ТекущийКаталог
                       |%2'"),
			СтрокаКоманды,
			ТекущийКаталог);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	...
	Иначе
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось запустить программу
                       ...
                       |%1
                       |з причини:
                       |ТекущийКаталог не є каталогом %2'"),
			СтрокаКоманды,
			ТекущийКаталог);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ЗапуститьПрограммуПослеЗапускаПриложения(КодВозврата, Контекст) Экспорт 
	...
	Если Контекст.ДождатьсяЗавершения И КодВозврата = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Произошла неизвестная ошибка при запуске программы.';uk='Сталася невідома помилка при запуску програми.'");
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
		Возврат;
	КонецЕсли;
...
#Если ВебКлиент Тогда
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось запустить программу
                   ...
                   |%1
                   |з причини:
                   |Запуск програм з підвищенням привілеїв недоступний у вебклієнті.'"),
...
#ИначеЕсли МобильныйКлиент Тогда
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось запустить программу
                   ...
                   |%1
                   |з причини:
                   |Запуск програм з підвищенням привілеїв недоступний в мобільному клієнті.'"),
		Контекст.СтрокаКоманды);
	ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
#Иначе
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось запустить программу
                       ...
                       |%1
                       |з причини:
                       |Запуск програм з підвищенням привілеїв доступний лише для Windows і Linux.'"),
			Контекст.СтрокаКоманды);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура СоздатьВременныйКаталогПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = 
			НСтр("ru='Не установлено расширение для работы с BAF, создание временного каталога невозможно.';uk='Не встановлено розширення для роботи з BAF, створення тимчасового каталогу неможливо.'");
