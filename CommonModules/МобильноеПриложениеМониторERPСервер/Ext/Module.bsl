...
//
// Параметры:
//  Данные - СправочникОбъект, ДокументОбъект, УдалениеОбъекта, Неопределено - объект метаданных, набор записей, который нужно преобразовать.
//
...
Функция ЗаполняемыеДанныеНаСервере(ПараметрыОбменаСМонитором)
	...
	НаименованиеЗадания = НСтр("ru='Подготовка данных для обмена с ""Монитор ERP""';uk='Підготовка даних для обміну з ""Монітор ERP""'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции';uk='Опрацювання операції'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Иначе
			...
			Если ТипЗнч(Значение.Row) = Тип("СписокXDTO") Тогда
				Для Каждого ЭлементСписка Из Значение.Row Цикл
					...
					ТабличнаяЧастьОбъекта = МобильныйОбъект[СокрЛП(Свойство)]; //СписокXDTO - 
					ТабличнаяЧастьОбъекта.Добавить(МобильныеДанные);
				КонецЦикла;
			Иначе
				МобильныеДанные = ФабрикаXDTO.Создать(ТипОбъектаКИ);
