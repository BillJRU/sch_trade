	...
	Пока ИндексВидаДоступа > 0 Цикл
		...
		Если Параметры.Назначение <> Неопределено
		  И Не ВидДоступаСоответствуетНазначениюПрофиля(СтрокаВидаДоступа.ВидДоступа, НазначениеПрофиля) Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" в строке %2 не соответствует назначению профиля.';uk='Вид доступу ""%1"" в рядку %2 не відповідає призначенню профілю.'"),
		...
		Если НайденныеВидыДоступа.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" в строке %2 повторяется.';uk='Вид доступу ""%1"" в рядку %2 повторюється.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление, "%1"),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Продолжить;
