...
Функция ОстаткиНоменклатуры(Номенклатура, Характеристика, Склады = Неопределено) Экспорт
	...
	// Текущие остатки.
	Результат = МассивРезультатов[СоставРазделовЗапроса.Найти("ДоступныеТовары")]; // РезультатЗапроса
	Выборка = Результат.Выбрать();
...
//			** СписокРасширенныйПоискНоменклатураПоставщика - ПолеФормы
//			** СписокСтандартныйПоискНоменклатураПоставщика - ПолеФормы
//			** СписокРасширенныйПоискНоменклатураПоставщикаНоменклатура - ПолеФормы
//			** СписокСтандартныйПоискНоменклатураПоставщикаНоменклатура - ПолеФормы
//			** ИерархияНоменклатуры - ТаблицаФормы
//			** ИерархияНоменклатурыПоставщика - ТаблицаФормы
...
//			** Параметры - ЗначенияПараметровДанныхКомпоновкиДанных
//		* СписокНоменклатураПоставщика - ДинамическийСписок - где:
//			** Параметры - ЗначенияПараметровДанныхКомпоновкиДанных
...
Функция ВыполнитьПоискНоменклатурыПоставщика(Форма) Экспорт
	...
	ПоискВыполнен = ВыполнитьПоиск(Форма, "НоменклатураПоставщиков");
	Возврат ПоискВыполнен;
...
Функция ВыполнитьПоиск(Форма, ИмяСпискаПоиска)
	...
	Если ИмяСпискаПоиска = "Номенклатура" Тогда 
		...
		ПараметрыПоиска.СтрокаПоиска = ФорматироватьСтрокуПоиска(Форма.СтрокаПоискаНоменклатура, ПараметрыПоиска.ТочноеСоотвествие);
	...
	ИначеЕсли ИмяСпискаПоиска = "ХарактеристикиНоменклатуры" Тогда
		...
		ПараметрыПоиска.СтрокаПоиска = ФорматироватьСтрокуПоиска(Форма.СтрокаПоискаХарактеристики, ПараметрыПоиска.ТочноеСоотвествие);
	...
	ИначеЕсли ИмяСпискаПоиска = "НоменклатураПоставщиков" Тогда	
		ПараметрыПоиска.ТочноеСоотвествие = Форма.НайтиНоменклатуруПоставщикаПоТочномуСоответствию;
		ПараметрыПоиска.СтрокаПоиска = ФорматироватьСтрокуПоиска(Форма.СтрокаПоискаНоменклатураПоставщика, ПараметрыПоиска.ТочноеСоотвествие);
		ПараметрыПоиска.ИмяРеквизитаПоискНеудачный = "ПоискНоменклатурыПоставщикаНеУдачный";
		ПараметрыПоиска.ИмяРеквизитаСписок = "СписокНоменклатураПоставщика";
		
	Иначе
		ТекстИсключения = НСтр("ru='Неизвестное значение параметра ИмяСпискаПоиска';uk='Невідоме значення параметра ИмяСпискаПоиска'");
...
Процедура ЗаполнитьДеревоЗначенийОтборов(Форма, ДеревоОтборов, ПоХарактеристикам)
	...
	Для Каждого Реквизит Из РеквизитыБыстрогоОтбора Цикл
		...
		Если ЗначениеЗаполнено(Реквизит.ЗаголовокРеквизита) Тогда
			СтрокаРеквизита.Представление = Реквизит.ЗаголовокРеквизита;
		КонецЕсли;
...
Функция РезультатНеполнотекстовогоПоиска(Форма, ПараметрыПоиска)
	...
	ТекстЗапроса = СтрСоединить(ТекстыЗапросаПоискаПоОбласти, ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%МаксКоличество%", Формат(МаксимальноеКоличествоЭлементовПоиска() + 1, "ЧЦ=10; ЧГ=0"));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ВидСравнения%", ?(ПараметрыПоиска.ТочноеСоотвествие, "=", "ПОДОБНО"));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%СПЕЦСИМВОЛ%", ?(ПараметрыПоиска.ТочноеСоотвествие, "", "СПЕЦСИМВОЛ ""~"""));
...
Функция ОписанияОбластейПоиска(Форма, ПараметрыПоиска)
	...
	Если (ПараметрыПоиска.ИмяСпискаПоиска = "Номенклатура"
		Или ПараметрыПоиска.ИмяСпискаПоиска = "НоменклатураПоставщиков")
		И ПодборТоваровКлиентСервер.ЭтоФормаПодбораВДокументыЗакупки(Форма) Тогда
		
		ОписаниеОбластиПоиска                             = ОписаниеОбластиПоиска();
		ОписаниеОбластиПоиска.ОбъектМетаданных            = Метаданные.Справочники.НоменклатураПоставщиков;
		ОписаниеОбластиПоиска.ТекстЗапросаПоискаПоОбласти =
...
Процедура ДополнитьРезультатыПоискаНоменклатуры(РезультатПоиска)
	...
	Для Каждого СтрМас из РезультатПоиска.ОбластиПоиска Цикл
		Если Не ПустаяСтрока(СтрМас.ТекстЗапросаПоискаНоменклатуры)
			...
			Запрос.УстановитьПараметр(СтрМас.ОбъектМетаданных.Имя, РезультатПоиска[СтрМас.ОбъектМетаданных.Имя]); 
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьУсловноеОформлениеДинамическихСписков(Форма, ЭтоФормаПодбораВДокументыЗакупки = Ложь) Экспорт
	...
	Если Не ЭтоПартнер 
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не выбран склад>';uk='<не вибраний склад>'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
