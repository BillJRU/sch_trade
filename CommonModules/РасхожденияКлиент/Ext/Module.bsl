...
Процедура ДокументыОснованиеПредставлениеНажатие(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ДокументыОснования.Количество() > 1 Тогда
		...
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ПриобретениеТоваровУслуг") Тогда
			ЗаголовокФормыПросмотра = НСтр("ru='Приобретения товаров и услуг (%КоличествоДокументов%)';uk='Придбання товарів і послуг (%КоличествоДокументов%)'");
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			ЗаголовокФормыПросмотра = НСтр("ru='Передачи товаров хранителю (%КоличествоДокументов%)';uk='Передачі товарів зберігачу (%КоличествоДокументов%)'");
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			ЗаголовокФормыПросмотра = НСтр("ru='Поступления товаров от хранителя (%КоличествоДокументов%)';uk='Надходження товарів від зберігача (%КоличествоДокументов%)'");
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение") Тогда
			ЗаголовокФормыПросмотра = НСтр("ru='Приемки товаров на хранение (%КоличествоДокументов%)';uk='Приймання товарів на зберігання (%КоличествоДокументов%)'");
		...
		Иначе
			ЗаголовокФормыПросмотра = НСтр("ru='Отгрузки товаров с хранения (%КоличествоДокументов%)';uk='Відвантаження товарів зі зберігання (%КоличествоДокументов%)'");
		КонецЕсли;
...
Процедура ИзменитьДействиеВыделенныхСтрок(Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь) Экспорт
	...
	Если МассивДействийПоИзлишкам.Количество() = 0
		...
		ТекстСообщения = НСтр("ru='В выделенных строках нет расхождений. Указание способа отработки расхождений не требуется.';uk='У виділених рядках немає розбіжностей. Зазначення способу відпрацювання розбіжностей не потрібно.'");
...
Процедура ОткрытьПодбор(Форма, ЭтоАктОРасхожденияхПослеОтгрузки) Экспорт
	...
	ПараметрыФормы.Вставить("Организация",             Форма.Объект.Организация);
 	ПараметрыФормы.Вставить("НалогообложениеНДС",      Форма.НалогообложениеНДСПоУмолчанию);
...
Процедура ОформитьДокументыНажатие(Форма) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Если Форма.Объект.Проведен Тогда
			СтрокаДействие = НСтр("ru='перепроведение';uk='перепроведення'");
		Иначе
			СтрокаДействие = НСтр("ru='запись';uk='запис'");
		КонецЕсли;
		...
		ТекстВопроса = СтрШаблон(НСтр("ru='Акт о расхождениях был изменен. Выполнить %1 документа?';uk='Акт про розбіжності був змінений. Виконати %1 документа?'"), СтрокаДействие);
...
Процедура ПоказатьПредупреждениеДляЗаполненныхНаОснованииСтрок() Экспорт
 	...
 	ТекстСообщения = НСтр("ru='Для строк, заполненных по основанию, возможно редактирование колонки ""Фактическое количество"", 
|а при наличии расхождений колонки ""Сумма (факт)"", ""НДС (факт)"" и ""Как отработать расхождения"".'
|;uk='Для рядків, заповнених на підставі, можливо редагування колонки ""Фактична кількість"",
|а при наявності розбіжностей колонки ""Сума (факт)"", ""ПДВ (факт)"" і ""Як відпрацювати розбіжності"".'");
...
Функция ТекстЗаголовкаЗагрузкиИзВнешнихФайлов() Экспорт
	...
	Возврат НСтр("ru='Загрузка фактического количества товаров из внешнего файла';uk='Завантаження фактичної кількості товарів із зовнішнього файлу'");
	
КонецФункции

Функция ТекстДопПоясненияПриЗагрузкеИзВнешнихФайлов() Экспорт
	
	Возврат НСтр("ru='Для сопоставления с данными документа будут использованы колонки ""Номенклатура"", ""Характеристика"", ""Упаковка"" и ""Цена"".';uk='Для співставлення з даними документа будуть використані колонки ""Номенклатура"", ""Характеристика"", ""Упаковка"" і ""Ціна"".'");
