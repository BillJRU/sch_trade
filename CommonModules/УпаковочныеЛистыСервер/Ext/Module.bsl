...
Процедура ЗаполнитьСлужебныеРеквизиты(Форма,
	...
	ШаблонКоличестваПозиций	= НСтр("ru=';%1 позиция; ;%1 позиции;%1 позиций;';uk=';%1 позиція; ; %1 позиції; %1 позицій;'");
	ФорматнаяСтрокаШаблона	= НСтр("ru='Л = ru_RU';uk='Л = uk_UA'");
...
Процедура ПриПереходеНаДругойУровень(Форма,
	...
	Если Форма.ИмяФормы <> Метаданные.Обработки.ПроверкаКоличестваТоваровВПриходномОрдере.ОсновнаяФорма.ПолноеИмя() Тогда
		КнопкаНаУровеньВверх = Форма.Элементы[ИмяТЧТовары + "НаУровеньВверх"]; // КнопкаФормы
