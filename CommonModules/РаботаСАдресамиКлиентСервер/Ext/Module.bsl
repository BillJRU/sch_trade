...
//     * AddressType - Строка - установленный пользователем тип адреса(только для адресов Украины).
...
Функция ПредставленияОбъектовАдресации(Язык = Неопределено) Экспорт
	...
	Если Язык = Неопределено Тогда
		Язык = МультиязычностьУкр.КодЯзыкаИнтерфейса();
	КонецЕсли;
...
Функция ПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление, ТипАдреса = Неопределено) Экспорт
	...
	ПредставленияДопЭлементов = ПредставленияОбъектовАдресации(МультиязычностьУкр.КодЯзыкаИнформационнойБазы());
	...
	ИначеЕсли Адрес.Свойство("stead") И Не ПустаяСтрока(Адрес.stead) Тогда
		СписокЗаполненныхУровней.Добавить(НСтр("ru='участок';uk='ділянка'",МультиязычностьУкр.КодЯзыкаИнформационнойБазы()) + " " + Адрес.stead);
	КонецЕсли;
