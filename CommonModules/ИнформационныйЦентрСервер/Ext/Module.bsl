...
Функция ПолучитьИмяСобытияДляЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Информационный центр';uk='Інформаційний центр'",ИнтеграцияПодсистемБТС.КодОсновногоЯзыка());
...
Процедура ЧтениеНовостейСлужбыПоддержки() Экспорт
	...
	Для каждого НовостьXDTO Из Ответ.news Цикл
		...
		ТекстHTMLXDTO = НовостьXDTO.textHTML; // ОбъектXDTO
		...
		Новость.ТекстHTML = ТекстHTMLXDTO.htmlText;

		Если ТекстHTMLXDTO.Свойства().Получить("images") <> Неопределено Тогда
			Картинки = Новый Структура;
			Если ТипЗнч(ТекстHTMLXDTO.images) = Тип("ОбъектXDTO") Тогда
				Картинки.Вставить(ТекстHTMLXDTO.images.name, Новый Картинка(XMLЗначение(Тип("ДвоичныеДанные"), ТекстHTMLXDTO.images.data), Истина));
			Иначе
				Для каждого КартинкаXDTO Из ТекстHTMLXDTO.images Цикл
