...
//	ВариантАнализа - СправочникСсылка.ВариантыАнализаЦелевыхПоказателей
//	ВариантОтображения - ПеречислениеСсылка.ВариантыОтображенияВариантовАнализа - кратко, диаграмма, таблица
//	ДинамическиеПараметры - См. ПолучитьИсточникДанныхВариантаАнализа.ДинамическиеПараметры
//	ТаблицаДанных - Неопределено - 
//					- ТаблицаЗначений - таблица с возможными колонками (см. подготовку таблицы в Справочник.ВариантыАнализаЦелевыхПоказателей.Форма.НастройкаДемоДанных.ИсточникДемоДанных):
//						*Период - Дата - 
...
Функция ПредставлениеПериодаВариантаАнализа(Знач ИсточникДанных) Экспорт
	...
	Если ИсточникДанных.ВариантАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.ПокомпонентноеСравнение
		...
		СтандартныйПериодАнализа = ИсточникДанных.ВариантАнализа.ПериодАнализа.Получить(); // СтандартныйПериод -
	...
	ИначеЕсли ИсточникДанных.ВариантАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.ДинамикаИзменения
		...
		ПериодАнализа = ИсточникДанных.ВариантАнализа.ПериодАнализа.Получить();
	...
	ИначеЕсли ИсточникДанных.ВариантАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.СравнениеСПрошлымПериодом Тогда
		СтандартныйПериодАнализа = ИсточникДанных.ВариантАнализа.ПериодАнализа.Получить(); // СтандартныйПериод - 
		СтандартныйПериодСравнения = ИсточникДанных.ВариантАнализа.ПериодСравнения.Получить(); // СтандартныйПериод - 
...
Процедура ЗаполнитьСтруктуруЦелейИВариантыАнализа() Экспорт
	...
	Исключение
		СобытиеЖР = НСтр("ru='Загрузка демонстрационной модели целевых показателей';uk='Завантаження демонстраційної моделі цільових показників'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьИндексКартинкиСостоянияДляПоказателя(ВариантАнализа, ИсточникДанных) Экспорт
	...
	ЗонаАнализа = ЗонаАнализаЗначения(ИсточникДанных,
			ВариантАнализа.Владелец.ЦелевойТренд,
	...
	Если Не ИсточникДанных.СвойстваДанныхПоПериодам.ВидОтклоненияОтЦелевогоЗначения = Неопределено И Не ИсточникДанных.СвойстваДанныхПоПериодам.ВидОтклоненияОтЦелевогоЗначения.Пустая() Тогда
		...
		Если ИсточникДанных.СвойстваДанныхПоПериодам.ВидЦелевогоЗначения = ВидыЦелевыхЗначенийВариантовАнализа.Значение Тогда
			Если ИсточникДанных.СвойстваДанныхПоПериодам.ВидОтклоненияОтЦелевогоЗначения = ВидыОтклоненийОтЦелевыхЗначенийПоказателей.Абсолютное Тогда
				...
				ШаблонТекстаЦелевогоЗначения = НСтр("ru='%1%2';uk='%1%2'");
				ШаблонТекстаЦелевогоЗначения = ШаблонТекстаЦелевогоЗначения + Символы.ПС
					+ НСтр("ru='с позитивным отклонением %3%4 и негативным отклонением %5%6';uk='з додатним відхиленням %3 %4 і від''ємним відхиленням %5 %6'");
		...
		Иначе 
			Если ИсточникДанных.СвойстваДанныхПоПериодам.ВидОтклоненияОтЦелевогоЗначения = ВидыОтклоненийОтЦелевыхЗначенийПоказателей.Абсолютное Тогда
				ШаблонТекстаЦелевогоЗначения = НСтр("ru='%1 - %2%3';uk='%1 - %2%3'");
				ШаблонТекстаЦелевогоЗначения = ШаблонТекстаЦелевогоЗначения + Символы.ПС
					+ НСтр("ru='с позитивным отклонением %4%5 и негативным отклонением %6%7';uk='з додатним відхиленням %4 %5 і від''ємним відхиленням %6 %7'");
...
Функция ПолучитьСокращенноеПредставлениеЧисла(ИсходноеЧисло, Разрядность, ВидПредставленияЧисла) Экспорт 
	...
	Если ВидПредставленияЧисла = ВидыПредставленияЧисел.ВТысячах Тогда
		СокращенноеПредставлениеПорядкаЧисел = НСтр("ru='%1 тыс.';uk=' %1 тис.'");
	ИначеЕсли ВидПредставленияЧисла = ВидыПредставленияЧисел.ВМиллионах Тогда
		СокращенноеПредставлениеПорядкаЧисел = НСтр("ru='%1 млн.';uk='%1 млн.'");
	Иначе
		СокращенноеПредставлениеПорядкаЧисел = НСтр("ru='%1';uk='%1'");
	КонецЕсли;
	
	ФорматнаяСтрокаЧисла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='ЧДЦ=%1;ЧН=';uk='ЧДЦ=%1;ЧН='"),
...
Функция ПроверитьСхемуКомпоновкиДанных(МакетСКДПоказателя) Экспорт
	...
	Если МакетСКДПоказателя = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru='Шаблон расчета целевого показателя не заполнен.';uk='Шаблон розрахунку цільового показника не заповнений.'"));
...
Функция ПрименитьФорматДаты(Дата, Периодичность)
	...
	Если Периодичность = Перечисления.Периодичность.День Тогда
		ФорматнаяСтрока = НСтр("ru='ДФ=dd.MM.yy';uk='ДФ=dd.MM.yy'");
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
		...
		ФорматированнаяДата = Формат(Дата(Дата), ФорматнаяСтрока);
	ИначеЕсли Периодичность = Перечисления.Периодичность.Месяц Тогда
		ФорматнаяСтрока = НСтр("ru='ДФ=''MMM yyyy''';uk='ДФ=''MMM yyyy'''");
		ФорматированнаяДата = Формат(Дата(Дата), ФорматнаяСтрока);
	ИначеЕсли Периодичность = Перечисления.Периодичность.Квартал Тогда
		ФорматнаяСтрока = НСтр("ru='ДФ=''%1""""КВ""""''';uk='ДФ=''%1""""КВ""""'''");
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Год Тогда
		ФорматнаяСтрока = НСтр("ru='ДФ=yyyy';uk='ДФ=yyyy'");
		ФорматированнаяДата = Формат(Дата(Дата), ФорматнаяСтрока);
	КонецЕсли;
		...
		Для Каждого СтрокаИсточникаДанных Из ДанныеИсточникаДанных Цикл
			...
			Иначе
				...
				Если ТипАнализа = ТипыАнализа.ПокомпонентноеСравнение И ПоВесуСерии Тогда
					Если ЕстьГрадиент Тогда
						ШаблонТекста = НСтр("ru='%1 = %2#%3#';uk=' %1 = %2 # %3 #'");
					Иначе
						ШаблонТекста = НСтр("ru='%1 = %2';uk='%1 = %2'");
					КонецЕсли;
				...
				ИначеЕсли ТипАнализа = ТипыАнализа.ИзмерениеТекущегоСостояния Тогда
					ПредставлениеДаты = Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения,
						НСтр("ru='ДФ=dd.MM.yy; ДЛФ=D';uk='ДФ=dd.MM.yy; ДЛФ=D'"));
					
					ШаблонТекста = НСтр("ru='%1 на %2 = %3';uk='%1 на %2 = %3'");
					ТекстСерии = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонТекста,
...
Процедура УстановитьНастройкиПоУмолчаниюОбластиПостроенияДиаграммы(Диаграмма, ВариантАнализа)
	...
	Если ВариантАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.ДинамикаИзменения
		...
		ОбластьПостроения.ОриентацияМеток = ОриентацияПодписейДиаграммы.Горизонтально;
	...
	ИначеЕсли ВариантАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.ПокомпонентноеСравнение
		...
		ОбластьПостроения.ОриентацияМеток = ОриентацияПодписейДиаграммы.Авто;
...
//
// Параметры:
//	ТабличныйДокументАнализаПоказателя - ТабличныйДокумент - Табличный документ для заполнения.
//	ВариантАнализа - СправочникСсылка.ВариантыАнализаЦелевыхПоказателей - Ссылка на вариант анализа.
//	ИсточникДанных - См. ИнициализироватьИсточникДанных
//
...
Процедура ПодготовитьИсточник(ИсточникДанных, ВариантАнализа)
	...
	ИначеЕсли ТипАнализа = ТипыАнализа.СравнениеСПрошлымПериодом Тогда 
		...
		ПредставлениеКолонки = НСтр("ru='Период сравнения';uk='Період порівняння'");
		...
		ПредставлениеКолонки = НСтр("ru='Целевое значение';uk='Цільове значення'");
		КолонкиДанных.Добавить("ЦелевоеЗначение",
			ОбщегоНазначения.ОписаниеТипаЧисло(31,3),
			ПредставлениеКолонки);
		
		ПредставлениеКолонки = НСтр("ru='Приведенное граничное позитивное отклонение';uk='Приведене граничне додатне відхилення'");
		КолонкиДанных.Добавить("ПриведенноеГраничноеПозитивноеОтклонение",
			ОбщегоНазначения.ОписаниеТипаЧисло(31,3),
			ПредставлениеКолонки);
		
		ПредставлениеКолонки = НСтр("ru='Приведенное граничное негативное отклонение';uk='Приведене граничне від''ємне відхилення'");
		...
		СтрокаПрошлогоПериода.ПериодСравнения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Прошлый %1';uk='Минулий %1'"),
		...
		Если ВариантАнализа.РассчитыватьПрогноз
			...
			ДатаПрогноза = НачалоДня(ДатаСоСмещением(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения,
					ВариантАнализа.ПериодичностьРасчетаПоказателя,
					1));
			...
			СтрокаПрогнозногоПериода.ПериодСравнения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Прогноз на %1';uk='Прогноз на %1'"),
...
Функция КонтрольКорректностиНастроекИДанных(ТипПроверяемогоОбъекта, ПроверяемыйОбъект, ИсточникДанных, ПараметрыРасчета)
	...
	Если ТипПроверяемогоОбъекта = "РезультатПолученияДанных"
		...
		ИсточникДанных.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный шаблон расчета: %1';uk='Невірний шаблон розрахунку: %1'"),
		...
		Если СхемаКомпоновкиДанных.Параметры.Найти("НеИзменятьСвязьПоДатеРасчета") = Неопределено
			...
			Для Каждого СвязьНаборовДанных Из СвязиНаборовДанных Цикл 
				...
				ИначеЕсли СвязьНаборовДанных.НаборДанныхПриемник = "ЦелевыеЗначения" Тогда 
					СвязьНаборовДанных.ВыражениеИсточник = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Для Каждого СвязьНаборовДанных Из СвязиНаборовДанных Цикл 
				Если СтрНайти(СвязьНаборовДанных.ВыражениеИсточник, "Период") Тогда
					СвязьНаборовДанных.ВыражениеИсточник = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"КОНЕЦПЕРИОДА(Период, ""%1"")",
						ВРег(ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя)));
					СвязьНаборовДанных.ВыражениеПриемник = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"КОНЕЦПЕРИОДА(ДатаРасчета, ""%1"")",
						ВРег(ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя)));
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ПолучитьИсточникДанныхВариантаАнализаИзКэша(ВариантАнализа, ВариантОтображения)
	...
	ПериодВМинутах = ПериодАктуальностиРасчетаВМинутах(ВариантАнализа.ПериодАктуальностиРасчета,
	                                                   ВариантАнализа.ВидПериодаАктуальностиРасчета);
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВариантАнализа", ВариантАнализа);
...
Процедура ПривестиВыраженияПолейИтоговКУказаннойТочности(СхемаКомпоновки, ВидПредставленияЧисла, Разрядность)
	...
	Если ЗнаменательТребуется Тогда
		ШаблонСУчетомЗнаменателя = "ВЫРАЗИТЬ((%1) / %3 ""Число(31, %2)"")";
	Иначе
		ШаблонСУчетомЗнаменателя = "ВЫРАЗИТЬ((%1) ""Число(31, %2)"")";
	КонецЕсли;
	Для Каждого ПолеИтога Из ПоляИтогов Цикл 
		Если ПолеИтога.ПутьКДанным = "ГраничноеПозитивноеОтклонение"
			ИЛИ ПолеИтога.ПутьКДанным = "ГраничноеНегативноеОтклонение" Тогда
				
			ЧастьВыражения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонСУчетомЗнаменателя,
				ПолеИтога.Выражение,
				Разрядность,
				Знаменатель);
			ПолеИтога.Выражение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ВЫБОР
...
Функция СвойстваЗоныАнализа(ВариантАнализа, ИсточникДанных) Экспорт
	...
	ВидЦелевогоЗначения = ПолучитьВидЦелевогоЗначения(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения,
		ВариантАнализа);
	ЗонаАнализа = ЗонаАнализаЗначения(ИсточникДанных,
		ЦелевойПоказатель.ЦелевойТренд,
		ВидЦелевогоЗначения,
		ВариантАнализа.ТочностьРасчетаДробнойЧасти);
	ИмяОбластиМатрицыСостояний = "Нестабильный" + "_" + ЗонаАнализа.ФактическийТренд + "_" + "Зона_"
		+ ЗонаАнализа.НомерЗоны + "_" + ПолучитьИмяЗначенияПеречисления(ЦелевойПоказатель.ЦелевойТренд);
	
	ПояснениеКСтатусу = МатрицаСостояний.ПолучитьОбласть(ИмяОбластиМатрицыСостояний + "|" + "ПояснениеКСтатусу").ТекущаяОбласть.Текст;
	ПояснениеКТренду  = МатрицаСостояний.ПолучитьОбласть(ИмяОбластиМатрицыСостояний + "|" + "ПояснениеКТренду").ТекущаяОбласть.Текст;
...
Функция ДанныеОбновленыНаНовуюВерсиюПрограммы(МетаданныеИОтбор) Экспорт
	...
	Если МетаданныеИОтбор.Метаданные = Метаданные.Справочники.ВариантыАнализаЦелевыхПоказателей Тогда
		...
		СвязанныеОбъекты.Добавить(Метаданные.Справочники.СтруктураЦелей);
		
	ИначеЕсли МетаданныеИОтбор.Метаданные = Метаданные.Отчеты.МониторЦелевыхПоказателей Тогда
		
		СвязанныеОбъекты.Добавить(Метаданные.Справочники.СтруктураЦелей);
		СвязанныеОбъекты.Добавить(Метаданные.Справочники.ВариантыАнализаЦелевыхПоказателей);
