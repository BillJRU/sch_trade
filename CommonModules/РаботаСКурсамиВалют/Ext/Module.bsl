...
//
Функция СформироватьСуммуПрописью(СуммаЧислом, Валюта, БезДробнойЧасти = Ложь, КодЯзыка = "ru") Экспорт
	...
	Иначе	
		...
		ПараметрыПрописи = МодульМультиязычностьУкр.ПараметрыПрописи(Валюта, КодЯзыка);
		Результат = ЧислоПрописью(Сумма, "Л=" + МодульМультиязычностьУкр.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДП=Ложь", ПараметрыПрописи);
	КонецЕсли;
