...
Процедура ЗаполнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока.Наименование = НСтр("ru='Данные операции требуется выполнить вручную';uk='Дані операції потрібно виконати вручну'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыВзаиморасчеты();
	НоваяСтрока.Наименование = НСтр("ru='Взаиморасчеты и переоценка валютных средств';uk='Взаєморозрахунки і переоцінка валютних коштів'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыФормированиеЗатрат();
	НоваяСтрока.Наименование = НСтр("ru='Формирование затрат';uk='Формування витрат'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыПроизводственныеЗатраты();
	НоваяСтрока.Наименование = НСтр("ru='Распределение производственных затрат';uk='Розподіл виробничих витрат'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыСебестоимость();
	НоваяСтрока.Наименование = НСтр("ru='Расчет себестоимости';uk='Розрахунок собівартості'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыФормированиеФинансовогоРезультата();
	НоваяСтрока.Наименование = НСтр("ru='Формирование финансового результата';uk='Формування фінансового результату'");
	
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыРегламентированныйУчет();
	НоваяСтрока.Наименование = НСтр("ru='Регламентированный учет';uk='Регламентований облік'");
...
Процедура Использование_ПереходНаНовыеРежимыРаботы(ПараметрыОбработчика) Экспорт
	...
	ТекстГиперссылкиПоУмолчанию = НСтр("ru='Перейти';uk='Перейти'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если НЕ ПолучитьФункциональнуюОпцию("БазоваяВерсия") И РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии21() Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			...
			НСтр("ru='Партионный учет (версия 2.5)';uk='Партіонний облік (версія 2.5)'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22() Тогда
		ШаблонТекста = НСтр("ru='Для организации ""%1"" использовать метод оценки стоимости ""ФИФО (скользящая оценка)""';uk='Для організації ""%1"" використовувати метод оцінки вартості ""ФІФО (змінна оцінка)""'",ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ШаблонТекста = НСтр("ru='Для организации ""%1"" использовать метод оценки стоимости ""Средняя за месяц""';uk='Для організації ""%1"" використовувати метод оцінки вартості ""Середня за місяць""'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Если НЕ СостояниеЭтапаОпределено(ПараметрыОбработчика.ДанныеЭтапа) Тогда
		...
		УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Устаревшие режимы работы не используются.';uk='Застарілі режими роботи не використовуються.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ИсправлениеОшибокВИсходныхДанных(ТаблицаПроверок)
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность реализации, указанной в возврате.';uk='Коректність реалізації, зазначеної в поверненні.'"),
...
|;uk='При зміні заднім числом документа реалізації товарний склад документа повернення від клієнта може розсінхронізуватися з реалізацією. 
|В цьому випадку в документі повернення необхідно або вказати коректний документ реалізації або взагалі не вказувати його.'"));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность реализации, указанной в возврате.';uk='Коректність реалізації, зазначеної в поверненні.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='При изменении задним числом документа реализации товарный состав документа возврата от клиента может рассинхронизироваться с реализацией.
|В этом случае в документе возврата необходимо или указать корректный документ реализации или вообще не указывать его.'
|;uk='При зміні заднім числом документа реалізації товарний склад документа повернення від клієнта може розсінхронізуватися з реалізацією. 
|В цьому випадку в документі повернення необхідно або вказати коректний документ реалізації або взагалі не вказувати його.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
        НСтр("ru='Корректность указания кодов для НН и номеров ГТД, указанных в возврате.';uk='Коректність зазначення кодів для ПН та номерів ВМД, зазначених у поверненні.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Были оформлены возвраты товаров, для которых включен учет по кодам для НН и номерам ГТД, но коды для НН и номера ГТД не были указаны.
|В этом случае в документе возврата необходимо указать коды для НН и номера ГТД.'
|;uk='Було оформлено повернення товарів, для яких включено облік за кодами для ПН та номерами ВМД, але коди для ПН та номери ВМД не були вказані.
|У цьому випадку у документі повернення необхідно вказати коди для ПН та номери ВМД.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Актуализация регистра сведений ""Устаревшие виды запасов с остатками""';uk='Актуалізація регістра відомостей ""Застарілі види запасів із залишками""'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|;uk='Регістр відомостей містить список застарілих видів запасів, 
|по яких ще є залишки в регістрі ""Товари організацій"" і які повинні бути списані при витрачанні.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность заполнения суммовых показателей в документе ""Отчет по комиссии между организациями"".';uk='Коректність заповнення сумових показників в документі ""Звіт з комісії між організаціями"".'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|;uk='Оформлено звіти по комісії між організаціями, у яких сумові показники табличній частині ""Товари"" не відповідають даним табличній частині ""Види запасів"". 
|Це прояв раніше виправленої помилки № 00-00190359. 
|Проблемні документи необхідно перепровести, після цього, можливо, буде потрібно повторно виконати закриття періоду.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаКорректностьПередачиУказаннойВВозврате(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Передача",					НСтр("ru='Документ передачи продукции';uk='Документ передачі продукції'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены возвращенные от организации ""%1"" товары, отсутствующие в документе передачи товаров';uk='Виявлено повернуті від організації ""%1"" товари, відсутні в документі передачі товарів'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Возврат");
...
Процедура ПроверкаКорректностьЗаполненияНомеровГТДВВозвратах(ПараметрыПроверки) Экспорт
    ...
    |	И Товары.НомерГТД = ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)
	...
	ПараметрыРегистрации = ИнициализироватьПараметрыРегистрацииПроблемПроверки(
        ...
        НСтр("ru='Обнаружены возвращенные товары без кодов для НН и номеров ГТД в организацию 
|""%1""'
|;uk='Виявлено повернуті товари без кодів для ПН та номерів ВМД в організацію
...
//	УточнениеКонтекстаВыполнения - ПеречислениеСсылка.МоментЗапускаПроверкиОперацииЗакрытияМесяца - когда надо выполнять проверку
//	Обработчик 					 - Строка - полное имя экспортной процедуры-обработчика, вызываемого для выполнения проверки
//	Важность 					 - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы - статус по умолчанию для результатов выполнения проверки.
// 
...
//	ПредшествующийЭтапУпорядочивание - ПеречислениеСсылка.ОперацииЗакрытияМесяца - предыдущий этап закрытия месяца для целей упорядочивания.
//
// Возвращаемое значение:
...
Функция ДобавитьЭтапВТаблицу(ТаблицаЭтапов, Родитель, Код, ОперативныйУчет = Истина,
	...
	Индекс = ИндексДобавляемогоЭтапа(ТаблицаЭтапов, ПредшествующийЭтапУпорядочивание);
	НоваяСтрока = ТаблицаЭтапов.Вставить(Индекс);
...
Функция ИндексДобавляемогоЭтапа(ТаблицаЭтапов, ПредшествующийЭтапУпорядочивание) Экспорт
	ИндексНовойСтроки = ТаблицаЭтапов.Количество();
	Если ЗначениеЗаполнено(ПредшествующийЭтапУпорядочивание) Тогда
		НайденнаяСтрока = ТаблицаЭтапов.Найти(ПредшествующийЭтапУпорядочивание, "Код");
		Если НайденнаяСтрока <> Неопределено Тогда
			ИндексНовойСтроки = ТаблицаЭтапов.Индекс(НайденнаяСтрока) + 1;
		КонецЕсли;
	КонецЕсли;
	Возврат ИндексНовойСтроки
КонецФункции
	...
	Если ЗначениеЗаполнено(Выборка.Месяц) Тогда
		...
		УстановитьСостояниеНеВыполнен(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Требуется пересчет операции %1 %2';uk='Потрібний перерахунок операції %1 %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				?(ВыполнятьССамогоРаннего, НСтр("ru='начиная с периода';uk='починаючи з періоду'",ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru='за период';uk='за період'",ОбщегоНазначения.КодОсновногоЯзыка())),
...
Функция ПроверитьНаличиеРегламентногоДокументаЭтапаЗакрытияМесяца(ПараметрыОбработчика, ИмяДокумента, ИмяТаблицыОтбораОрганизаций = "") Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='За период %1 нет заданий для расчета данной операции и отсутствуют документы ""%2"".';uk='За період %1 немає завдань для розрахунку даної операції і відсутні документи ""%2"".'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
//	ДействиеПодробнее 	 - Строка - см. функции вида "ОписаниеДействия_..."
//	Состояние 			 - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы - статус пояснения, по умолчанию "Информация".
//
...
//	ТекстПодробнее 		 - Строка - текст гиперссылки "Подробнее" вместо текста по умолчанию
//	ДействиеПодробнее 	 - Строка - см. функции вида "ОписаниеДействия_..."
//	Состояние 			 - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы - статус пояснения, по умолчанию "Ошибка".
//
...
//	ТекстПодробнее 		 - Строка - текст гиперссылки "Подробнее" вместо текста по умолчанию
//	ДействиеПодробнее 	 - Строка - см. функции вида "ОписаниеДействия_..."
//	Состояние 			 - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы - статус пояснения, по умолчанию "Предупреждение".
//
...
//	Важность 			 	  	   - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы - статус проблемы; по умолчанию "Ошибка"
//
...
Функция УвеличитьНомерЗадания(Знач ИмяКонстанты = "") Экспорт
	...
	Попытка
		...
		МенеджерКонстанты.Установить(НомерДоРасчета + 1);
	...
	Исключение
		...
		ИнформацияОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			+ " " + НСтр("ru='Значение константы';uk='Значення константи'") + " " + ИмяКонстанты + " = " + НомерДоРасчета;
		
		ЗаписьЖурналаРегистрации(
		    ИмяСобытияЖурналаРегистрации(НСтр("ru='Увеличение номера задания для расчета';uk='Збільшення номера завдання для розрахунку'",ОбщегоНазначения.КодОсновногоЯзыка())),
...
//
Процедура ОтразитьЗаданияКЗакрытиюМесяца(Документ, МенеджерВременныхТаблиц) Экспорт
	
    Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда // задания устанавливаются только в главном узле.
		Возврат;
	КонецЕсли;
	
	ОтменаПроведения = ПроведениеДокументов.СвойстваДокумента(Документ).РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения;
...
Функция ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, Знач СледующийМесяц) Экспорт
	...
	СледующийМесяц = ЗакрытиеМесяцаЛокализация.ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, СледующийМесяц);
...
Функция ИмяСобытияЖурналаРегистрации(ДополнениеИмени) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Закрытие месяца.%1';uk='Закриття місяця.%1'",ОбщегоНазначения.КодОсновногоЯзыка()),
		ДополнениеИмени);
...
Процедура ВыполнитьЗакрытиеМесяца(Организация = Неопределено) Экспорт
	...
	Если НЕ ОписаниеЗадания.НастройкаСуществует Тогда
		ВызватьИсключение НСтр("ru='Не задана настройка регламентного задания закрытия месяца.';uk='Не задана настройка регламентного завдання закриття місяця.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректная настройка регламентного задания закрытия месяца.';uk='Некоректна настройка регламентного завдання закриття місяця.'");
	КонецЕсли;
	...
	АктивныеРасчеты = РегистрыСведений.ВыполнениеОперацийЗакрытияМесяца.УстановитьПризнакЗапускаРасчета(
		...
		НСтр("ru='регламентное задание ""Закрытие месяца""';uk='регламентне завдання ""Закриття місяця""'"),
		ПервыйПериод);
	...
	ФоновыеЗадания.Выполнить(
		...
		НСтр("ru='Регламентное задание закрытия месяца';uk='Регламентне завдання закриття місяця'"));
...
Процедура ВыполнитьЗакрытиеМесяцаВФоновомЗадании(ОписаниеЗадания, ИдентификаторРасчета) Экспорт
	...
	Пока ТекущийПериод <= ОписаниеЗадания.ПоследнийПериод Цикл
		...
		Если СтруктураДанных.ИзмененияЗапрещены Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='ЗакрытиеМесяца';uk='ЗакрытиеМесяца'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
...
Процедура ОчиститьЗаданияПередУдалениемДокумента(Источник, Отказ) Экспорт
	...
	Пока ВыборкаЗаданий.Следующий() Цикл
		...
		Исключение
			Отказ = Истина;
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Удаление помеченных объектов';uk='Вилучення відмічених об''єктів'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.ОбщиеМодули.ЗакрытиеМесяцаСервер,
				,
...
Процедура ЗаполнитьГруппыПроверокДляРегистрации(ТаблицаГруппПроверок) Экспорт
	...
	ОписаниеГруппыПроверок.Наименование = НСтр("ru='Закрытие месяца';uk='Закриття місяця'");
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	Для Каждого МетаЗначение Из Метаданные.Перечисления.ОперацииЗакрытияМесяца.ЗначенияПеречисления Цикл
		...
		ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Проблемы, диагностированные при выполнении операции ""%1""';uk='Проблеми, діагностовані при виконанні операції ""%1""'",ОбщегоНазначения.КодОсновногоЯзыка()),
				МетаЗначение.Синоним),
			НСтр("ru='Служебная контекстная проверка для регистрации проблем, диагностированных в ходе расчета.';uk='Службова контекстна перевірка для реєстрації проблем, діагностованих в ході розрахунку.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СлужебнаяПроверкаЭтапа(ЭтапРасчета, ВыдаватьОшибкуПриОтсутствии = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Проверка) И ВыдаватьОшибкуПриОтсутствии Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена служебная контекстная проверка для операции ""%1""';uk='Не знайдена службова контекстна перевірка для операції ""%1""'"),
			СокрЛП(ЭтапРасчета));
...
Функция ОписаниеГруппПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеГруппы.Наименование = НСтр("ru='Регистрация ошибок';uk='Реєстрація помилок'");
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, влияющие на регистрацию ошибок при расчете этапов закрытия месяца.';uk='Параметри, що впливають на реєстрацію помилок при розрахунку етапів закриття місяця.'");
	
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "Отладка";
	ОписаниеГруппы.Наименование = НСтр("ru='Отладка расчета';uk='Налагодження розрахунку'");
...
|;uk='Параметри, які використовуються для налагодження етапів закриття місяця. 
|Змінювати ці параметри рекомендується тільки в тестових інформаційних базах!'");
...
Функция ОписаниеТехнологическихПараметровЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru='Количество регистрируемых ошибок одного вида';uk='Кількість помилок одного виду, що реєструються'");
...
|;uk='Призначений для прискорення оновлення стану і виконання розрахунку операцій. 
|Якщо в результаті виконання будь-якої перевірки виявлено велику кількість однотипних помилок, 
|то як правило немає сенсу реєструвати їх всі - потрібно розбиратися з причинами виникнення цих помилок. 
|Якщо вказано значення 0, то будуть зареєстровані всі знайдені помилки.'");
	
	ОписаниеПараметра = ОписаниеПараметров.Добавить();
	ОписаниеПараметра.Родитель 			  = Родитель;
	ОписаниеПараметра.Имя 				  = "ДублироватьНайденныеПроблемыВЖурналеРегистрации";
	ОписаниеПараметра.Наименование 		  = НСтр("ru='Дублировать найденные проблемы в журнале регистрации';uk='Дублювати знайдені проблеми в журналі реєстрації'");
...
|;uk='Визначає поведінку механізму закриття місяця в частині роботи з журналом реєстрації. 
|Якщо встановлено значення Істина, то інформація про всі знайдені проблеми, що реєструються в механізмі ""Аудит стану системи"", 
|буде дублюватися в журналі реєстрації.'");
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru='Не проверять окончание обновления информационной базы';uk='Не перевіряти закінчення оновлення інформаційної бази'");
	ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
...
|;uk='Дозволяє пропустити перевірку закінчення оновлення інформаційної бази.
|Якщо цю властивість не встановлено і оновлення вхідних даних механізму розрахунку ще не завершено, 
|то розрахунок виконуватися не буде, тому що його вхідні дані можуть бути некоректними.'");
...
Функция СформироватьОписаниеДанныхЭтапа(Код) Экспорт
	...
	Если СтрокаЭтапа = Неопределено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдено описание операции закрытия месяца ""%1""';uk='Не знайдено опис операції закриття місяця ""%1""'"),
			СокрЛП(Код));
...
Процедура ДобавитьИнформациюОНезарегистрированныхОбъектахКЗафиксированнойПроблеме(ПараметрыПроверки, ОписаниеПроблемы, КоличествоОбъектов) Экспорт
	...
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Еще найдено аналогичных проблем: %1';uk='Ще знайдено аналогічних проблем: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если Задание = Неопределено Тогда
		...
		Если ТипЗнч(КлючИлиИдентификаторЗадания) = Тип("Строка") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Фоновое задание с ключом ""%1"" не найдено.';uk='Фонове завдання з ключем ""%1"" не знайдено.'"),
				СокрЛП(КлючИлиИдентификаторЗадания));
		Иначе
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Фоновое задание с идентификатором ""%1"" не найдено.';uk='Фонове завдання з ідентифікатором ""%1"" не знайдено.'"),
				СокрЛП(КлючИлиИдентификаторЗадания));
		КонецЕсли;
...
Функция ОтменитьВыполнениеФоновогоЗадания(ИдентификаторЗадания) Экспорт 
	...
	Если СостояниеЗадания.Активно Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ru='Отмена выполнения фонового задания';uk='Скасування виконання фонового завдання'",ОбщегоНазначения.КодОсновногоЯзыка())),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстОшибки);
...
Процедура ТестированиеЗакрытияМесяца(Период) Экспорт
	...
	АктивныеРасчеты = РегистрыСведений.ВыполнениеОперацийЗакрытияМесяца.УстановитьПризнакЗапускаРасчета(
		,
		ИдентификаторРасчета,
		НСтр("ru='Тестирование операций закрытия месяца';uk='Тестування операцій закриття місяця'"),
		Период);
...
Функция СохранитьСостояниеЭтаповЗакрытияМесяцаДляТестирования(КаталогДляВыгрузки, Период, МассивОрганизаций = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Период) ИЛИ НЕ ЗначениеЗаполнено(КаталогДляВыгрузки) Тогда
		Возврат НСтр("ru='Не указаны обязательные параметры';uk='Не зазначені обов''язкові параметри'");
	КонецЕсли;
...
Функция СравнитьСостояниеЭтаповЗакрытияМесяцаДляТестирования(Период,
	...
	Если НРег(Каталог1) = НРег(Каталог2) Тогда
		ТекстыСообщений.Добавить(НСтр("ru='Имена каталогов выгрузки №1 И №2 не должны совпадать';uk='Імена каталогів вивантаження №1 І №2 не повинні збігатися'"));
	...
	ПроверитьНаличиеФайлаВыгрузкиСостояниеЭтаповДляТестирования(
		...
		НСтр("ru='операций';uk='операцій'"),
		ТекстыСообщений);
	
	ПроверитьНаличиеФайлаВыгрузкиСостояниеЭтаповДляТестирования(
		Каталог1,
		ИменаФайлов.ТаблицаСгруппированныхПояснений,
		НСтр("ru='пояснений';uk='пояснень'"),
		ТекстыСообщений);
...
|%1'
|;uk='Неможливо порівняти таблиці стану етапів: 
|%1'"),
				ТаблицаОтличийЭтапы));
	КонецЕсли;
...
|%1'
|;uk='Неможливо порівняти таблиці пояснень до етапів: 
|%1'"),
	...
	Для Каждого ТекущийЭтап Из МассивЭтапов Цикл
		...
		Если НЕ ЗначениеЗаполнено(Состояние1) И НЕ ЗначениеЗаполнено(Состояние2) Тогда
			...
			Если СтрокаИсходнойТаблицы = Неопределено Тогда
				...
				ТекстыСообщений.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка: операция ""%1"" отсутствует в таблице выгрузки операций';uk='Помилка: операція ""%1"" відсутня в таблиці вивантаження операцій'"),
						ТекущийЭтап));
		...
		Если НЕ ЗначениеЗаполнено(Состояние1) И ЗначениеЗаполнено(Состояние2) Тогда
			...
			ТекстОтличия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция присутствует только в выгрузке №2: ""%1""';uk='Операція присутня тільки у вивантаженні №2: ""%1""'"),
				Состояние2);
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Состояние2) И ЗначениеЗаполнено(Состояние1) Тогда
			...
			ТекстОтличия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция присутствует только в выгрузке №1: ""%1""';uk='Операція присутній тільки у вивантаженні №1: ""%1""'"),
				Состояние1);
				
		ИначеЕсли Состояние1 <> Состояние2 Тогда
			ТекстОтличия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Изменилось состояние операции: ""%1"" - ""%2""';uk='Змінився стан операції: ""%1"" - ""%2""'"),
				Состояние1,
				Состояние2);
		Иначе
			...
			ТекстОтличия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Состояние операции не изменилось: ""%1""';uk='Стан операції не змінилося: ""%1""'"),
				Состояние1);
		...
		ТекстыСообщений.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция ""%1""';uk='Операція ""%1""'"),
				ТекущийЭтап));
		...
		Если СтрокиЭтапа.Количество() = 0 Тогда 
			ТекстыСообщений.Добавить(НСтр("ru='Отличий в пояснениях к этапу нет';uk='Відмінностей в поясненнях до етапу немає'"));
			Продолжить;
		КонецЕсли;
		
		ТекстыСообщений.Добавить(НСтр("ru='Изменились пояснения к этапу';uk='Змінилися пояснення до етапу'") + ":");
		
		ШаблонТекстОтличия = "	" + НСтр("ru='Есть только в выгрузке №%1: ""%2"" - ""%3""';uk='Є тільки в вивантаженні № %1: ""%2"" - ""%3""'");
...
|Выгрузка №1: ""%2""
|Выгрузка №2: ""%3""'
|;uk='Звіт про порівняння станів операцій закриття місяця. 
|У звіт включені тільки операції, що мають відмінності в стані і / або в описі проблем (поясненнях). 
|
|Період, що перевірявся: %1 
|Вивантаження №1: ""%2"" 
|Вивантаження №2: ""%3""'"),
...
Процедура ПроверитьНаличиеФайлаВыгрузкиСостояниеЭтаповДляТестирования(Каталог, ИмяФайла, НазначениеФайла, ТекстыСообщений)
	...
	Если Файлы.Количество() <> 1 Тогда
		...
		ТекстыСообщений.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найден файл выгрузки таблицы %1 ""%2""';uk='Не знайдено файл вивантаження таблиці %1 ""%2""'"),
				НазначениеФайла,
				Каталог + ИмяФайла));
