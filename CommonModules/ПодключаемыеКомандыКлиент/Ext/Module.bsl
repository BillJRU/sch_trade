...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("ru='Для продолжения необходимо установить расширение для работы с BAF.';uk='Для продовження необхідно встановити розширення для роботи з BAF.'");
		...
		Если Источник.Ссылка.Пустая()
			...
			Если Контекст.ТребуетсяПроведение Тогда
				ШаблонВопроса = НСтр("ru='Для выполнения команды ""%1""
                    ...
                    |;uk='Для виконання команди ""%1""
                    |документ буде проведений. Продовжити?'");
				Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Провести и продолжить';uk='Провести і продовжити'"));
			Иначе
				ШаблонВопроса = НСтр("ru='Для выполнения команды ""%1""
                    |данные будут записаны. Продолжить?'
                    |;uk='Для виконання команди ""%1""
                    |дані будуть записані. Продовжити?'");
				Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Записать и продолжить';uk='Записати й продовжити'"));
			КонецЕсли;
