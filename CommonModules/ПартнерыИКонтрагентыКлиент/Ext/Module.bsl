...
Процедура ПартнерыФормаСпискаВыбораОснованиеВыбораНажатие(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если Основание.Количество() > 0 Тогда
		...
		ПоказатьЗначение(Неопределено, Партнер.Ссылка);
	КонецЕсли;
	...
	Если ИсторияНаименований.Количество() > 1 Тогда
		Для Каждого СтрокаНаименование Из ИсторияНаименований Цикл
			...
			Иначе
				...
				НаименованиеСокращенное = СтрШаблон(НСтр("ru='ИП %1';uk='ІП %1'"), СтрокаНаименование.Фамилия);
				
				ПартнерыИКонтрагентыЛокализацияКлиент.ДополнитьДанныеИсторияНаименованияИП(СтрокаНаименование,
					ДанныеЗаполнения,
