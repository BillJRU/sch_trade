...
Функция ИсправленияИзменены(ТолькоПроверка = Ложь) Экспорт
	...
	Если Исправления.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='В составе конфигурации обнаружены установленные исправления (%1).
            ...
            |;uk='У складі конфігурації виявлені встановлені виправлення (%1).
            |Початок перевірки необхідності видалення застарілих патчів.'");
		...
		Для Каждого Исправление Из Исправления Цикл
			...
			Иначе
				ТекстСообщения = НСтр("ru='Проверка патча ""%1"".';uk='Перевірка патча ""%1"".'");
			...
			Если УдалитьИсправление Тогда
				ТекстСообщения = НСтр("ru='Патч ""%1"" устарел и будет удален.';uk='Патч ""%1"" застарів і буде видалений.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Исправление.Имя);
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Удаление';uk='Виправлення.Видалення'",ОбщегоНазначения.КодОсновногоЯзыка()),
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При записи исправления ""%1"" произошла ошибка:
                                       ...
                                       |
                                       |%2'"), Исправление.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Изменение';uk='Виправлення.Зміна'",ОбщегоНазначения.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
						ВызватьИсключение ТекстОшибки;
					КонецПопытки;
				КонецЕсли;
	...
	Если Изменения.УдаленныеИсправления.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Удаление устаревших патчей завершено.
            ...
            |;uk='Вилучення застарілих патчів завершено.
            |Вилучено: %1
            |Залишилося діючих патчів: %2'");
...
Функция УстановкаИУдалениеИсправлений(Исправления, ВФоне = Ложь) Экспорт
		...
		Для Каждого Патч Из Устанавливаемые Цикл
			Попытка
				...
				Если Не ИсправлениеНайдено Тогда
					ВызватьИсключение НСтр("ru='Исправление не найдено.';uk='Виправлення не знайдено.'");
				КонецЕсли;
				...
				Если Не ИсправлениеПрименимо Тогда
					ВызватьИсключение НСтр("ru='Исправление не применимо для данной версии конфигурации.';uk='Виправлення не застосовується для даної версії конфігурації.'");
				КонецЕсли;
			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При установке исправления ""%1"" произошла ошибка:
                               ...
                               |
                               |%2'"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Установка';uk='Виправлення.Встановлення'",ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И ВФоне Тогда
		ТекстДляАсинхронногоВызова = НСтр("ru='Обновление параметров работы расширений после установки или
            ...
            |;uk='Оновлення параметрів роботи розширень після установки або
            |видалення патчів завершилося з помилкою.'")
	КонецЕсли;
...
Процедура ПроверитьНаличиеУстаревшихИсправлений() Экспорт
	...
	Если Результат.ЕстьИзменения Тогда
		ТекстСообщения = НСтр("ru='Некорректный вызов функции ""ОбновлениеИнформационнойБазы.ВыполнитьОбновлениеИнформационнойБазы"".
            ...
            |;uk='Некоректний виклик функції ""ОбновлениеИнформационнойБазы.ВыполнитьОбновлениеИнформационнойБазы"".
            |Перед викликом не були видалені застарілі патчі, можливо, оновлення виконується позаштатними засобами.'");
...
Процедура ОбновитьИсправленияИзСкрипта(НовыеИсправления, УдаляемыеИсправления) Экспорт
	...
	ТекстСообщения = НСтр("ru='Обновление исправлений конфигурации из внешнего скрипта.';uk='Оновлення виправлень конфігурації з зовнішнього скрипта.'");
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("ru='Недостаточно прав для обновления исправлений конфигурации.';uk='Недостатньо прав для оновлення виправлень конфігурації.'");
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
// Получить глобальные настройки обновления на сеанс платформы BAF.
//
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru='Обновление конфигурации';uk='Оновлення конфігурації'",ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Функция ИзвлечьИсправлениеИзАрхива(Знач ПомещенныйФайл)
	...
	Попытка
		...
		Если Не ИсправлениеНайдено Тогда
			ВызватьИсключение НСтр("ru='Файл не является исправлением.';uk='Файл не є виправленням.'");
		КонецЕсли;
...
Функция ИсправлениеПрименимо(ЧтениеZip, ВременныйКаталог)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка при чтении файла манифеста исправления';uk='Помилка при читанні файлу маніфесту виправлення'") + ":" + Символы.ПС + ТекстОшибки;
	КонецПопытки;
...
Функция СведенияОбОбновлении(Знач ИмяФайлаПоставкиОбновления) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Результат.ТекстОшибки = НСтр("ru='Сведения не доступны';uk='Відомості не доступні'");
