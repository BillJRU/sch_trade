...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Обновление видов контактной информации.
        ...
        |;uk='Оновлення видів контактної інформації.
        |До завершення обробки види контактної інформації в документах в ряді випадків будуть відображатися некоректно.'");
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Наименование                 = НСтр("ru='Выявление некорректных настроек видов контактной информации';uk='Виявлення некоректних настройок видів контактної інформації'");
	Проверка.Причины                      = НСтр("ru='В карточке или документе отсутствуют поля контактной информации, или возникает ситуация, блокирующая работу с ними.';uk='У картці або документі відсутні поля контактної інформації, або виникає ситуація, що блокує роботу з ними.'");
    ...
    |;uk='Виконати часткове автоматичне відновлення видів контактної інформації (для цього натиснути посилання нижче).
    |
    |Якщо робота ведеться в розподіленій інформаційній базі (РІБ), то виправлення слід запускати тільки в головному вузлі.
    |Потім виконати синхронізацію з підпорядкованими вузлами.'");
...
Процедура ПроверитьВидыКонтактнойИнформации(Проверка, ПараметрыПроверки) Экспорт
	...
	ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки("КонтактнаяИнформация", НСтр("ru='Виды контактной информации';uk='Види контактної інформації'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока СтрокаИзЗапроса.Следующий() Цикл
		...
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не заполнена группа определяющая владельца контактной информации';uk='Не заповнена група яка визначає власника контактної інформації'"), СтрокаИзЗапроса.Наименование);
...
Процедура ЗаполнитьНаименованияВидовКонтактнойИнформацииПользователя() Экспорт
	...
	ПараметрыГруппы.Наименование = НСтр("ru='Контактная информация справочника ""Пользователи""';uk='Контактна інформація довідника ""Користувачі""'");
	...
	ПараметрыВида.Наименование = НСтр("ru='Электронная почта';uk='Електронна пошта'");
	...
	ПараметрыВида.Наименование = НСтр("ru='Телефон';uk='Телефон'");
	ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ПараметрыВида);
...
Процедура КонтактнаяИнформацияОбработкаПроверкиЗаполнения(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Для каждого ВидКонтактнойИнформации Из ВидыКонтактнойИнформации Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( НСтр("ru='Поле ""%1"" не заполнено.';uk='Поле ""%1"" не заповнене.'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Контактная информация';uk='Контактна інформація'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполнено обязательное поле Тип у вида контактной информации ""%1"".';uk='Не заповнене обов''язкове поле Тип у виду контактної інформації ""%1"".'"),
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Наименование) Тогда
		Результат.ЕстьОшибки = Истина;
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполнено обязательное поле Наименование у вида контактной информации ""%1"".';uk='Не заповнене обов''язкове поле Найменування у виду контактної інформації ""%1"".'"),
		...
		Если ТекстОшибки = Неопределено И ОжидаемыйТип <> Неопределено Тогда
			...
			Иначе
				...
				ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, НСтр("ru='веб-страницы';uk='вебсторінки'"));
