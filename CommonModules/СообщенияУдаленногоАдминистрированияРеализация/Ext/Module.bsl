...
Процедура УстановитьПолныеПраваОбластиДанных(Знач ИдентификаторПользователяСервиса, Знач ДоступРазрешен) Экспорт
	...
	Попытка
		...
		Если ПользователиСлужебный.ЗапретРедактированияРолей()
			...
			ПользовательСлужебный = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПользовательОбластиДанных, "Служебный");
			Если ПользовательСлужебный И ДоступРазрешен Тогда
				ПользовательОбъект = ПользовательОбластиДанных.ПолучитьОбъект(); // СправочникОбъект.Пользователи
				ПользовательОбъект.Служебный = Не ДоступРазрешен;
				ПользовательОбъект.Записать();
			КонецЕсли;
...
Процедура ПодготовитьОбластьДанныхДляМиграции(КодОбластиДанных, Параметры) Экспорт
	...
	Если Не ИнтеграцияПодсистемБТС.ПодсистемаСуществует("ТехнологияСервиса.МиграцияПриложений") Тогда
		ТекстСообщения = НСтр("ru='Миграция приложений не поддерживается';uk='Міграція програм не підтримується'",ОбщегоНазначения.КодОсновногоЯзыка());
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрированиеУстановитьПараметры() Экспорт
	...
	Возврат НСтр("ru='Удаленное администрирование в модели сервиса.Установить параметры';uk='Віддалене адміністрування в моделі сервісу.Встановити параметри'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрированиеУдалитьОбласть() Экспорт
	
	Возврат НСтр("ru='Удаленное администрирование в модели сервиса.Удалить область';uk='Віддалене адміністрування в моделі сервісу.Видалити область'",ОбщегоНазначения.КодОсновногоЯзыка());
