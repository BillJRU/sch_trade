...
Функция НаименованиеФоновогоРезервногоКопирования() Экспорт
	...
	Возврат НСтр("ru='Резервное копирование области данных';uk='Резервне копіювання області даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Если НЕ ТребуетсяСоздание Тогда
		ЗаписьЖурналаРегистрации(
			СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ru='Пропуск создания';uk='Пропуск створення'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Создание резервной копии области данных.Не удалось удалить временный файл';uk='Створення резервної копії області даних.Не вдалося видалити тимчасовий файл'",ОбщегоНазначения.КодОсновногоЯзыка()), 
		...
		ЗаписатьСобытиеВЖурнал(
			НСтр("ru='Создание';uk='Створення'",ОбщегоНазначения.КодОсновногоЯзыка()),
			ПараметрыСобытия);
			
	Исключение
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Создание резервной копии области данных';uk='Створення резервної копії області даних'",ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Попытка
...
Процедура ОшибкаСозданияКопии(Знач ПараметрыЗадания, Знач ИнформацияОбОшибке) Экспорт
	...
	Если ПараметрыЗадания.НомерПопытки < ПараметрыЗадания.КоличествоПовторовПриАварийномЗавершении Тогда
		...
		Событие = НСтр("ru='Ошибка итерации создания';uk='Помилка ітерації створення'",ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		...
		Событие = НСтр("ru='Ошибка создания';uk='Помилка створення'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ВыгрузитьОбластьВХранилищеМС(Знач Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЕстьИзмененияВСтруктуреДанных Тогда
			...
			НаименованиеЗадания = НСтр("ru='Выгрузка области с расширениями';uk='Вивантаження області з розширеннями'");
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Резервное копирование приложений';uk='Резервне копіювання додатків'",ОбщегоНазначения.КодОсновногоЯзыка());
