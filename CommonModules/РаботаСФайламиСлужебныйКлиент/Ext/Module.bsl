...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	ФильтрДиалога = ?(ПараметрыВыполнения.Свойство("ФильтрДиалогаВыбора"),
		ПараметрыВыполнения.ФильтрДиалогаВыбора, НСтр("ru='Все файлы (*.*)|*.*';uk='Всі файли (*.*)|*.*'"));
	...
	Если ПараметрыВыполнения.Свойство("МаксимальныйРазмер")
		...
		ТекстОшибки = НСтр("ru='Размер файла превышает %1 Мб.';uk='Розмір файлу перевищує %1 Мб.'");
...
Процедура ПоказатьПредупреждениеОНеобходимостиРасширенияРаботыСФайлами(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	Если Не КлиентПоддерживаетСинхронныеВызовы() Тогда
		ТекстПредупреждения = НСтр("ru='В веб-браузерах Google Chrome и Mozilla Firefox команда ""%1"" недоступна.';uk='У веббраузерах Google Chrome і Mozilla Firefox команда ""%1"" недоступна.'");
	Иначе
		ТекстПредупреждения = НСтр("ru='Для выполнения команды ""%1"" необходимо
            |установить расширение для работы с BAF.'
            |;uk='Для виконання команди ""%1"" необхідно
            |встановити розширення для роботи з BAF.'");
	КонецЕсли;
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru='Файл в программе, отмеченный как занятый для редактирования,
                           ...
                           |;uk='Файл у програмі, відмічений як зайнятий для редагування,
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Файлы';uk='Файли'",ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
			...
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось поместить файл
                               ...
                               |""%1"" (%2)
                               |в програму, тому що він не існує в робочому каталозі на комп''ютері.
                               |
                               |Звільнити файл?'"),
					Строка(ДанныеФайла.Ссылка),
...
Процедура ОбновитьИзФайлаНаДиске(
	...
	Если ПараметрыДобавленияФайла <> Неопределено
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Размер файла превышает %1 Мб.';uk='Розмір файлу перевищує %1 Мб.'"), ПараметрыДобавленияФайла.МаксимальныйРазмер);
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1""
                       ...
                       |;uk='Файл ""%1""
                       |зайнятий для редагування.'"), Строка(ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.Информация32);
	...
	Если ПараметрыВыполнения.НеЗадаватьВопрос = Ложь Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отмена редактирования файла
                       ...
                       |""%1""
                       |може призвести до втрати внесених змін.
                       |
                       |Продовжити?'"),
			Строка(ПараметрыВыполнения.ОбъектСсылка));
...
Процедура ПолучитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='Не установлено расширение для работы с BAF.';uk='Не встановлено розширення для роботи з BAF.'"));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ru='Установка свойств файла не выполнена по причине:';uk='Встановлення властивостей файлу не виконане через:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	#Если ВебКлиент Тогда
		...
		ВернутьРезультатПослеПоказаПредупреждения(
			ОбработчикРезультата,
			НСтр("ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';uk='Очищення робочого каталогу не підтримується у вебклиєнті.'"),
			Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, ОчищатьВсе) Экспорт
	...
	#Если ВебКлиент Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';uk='Очищення робочого каталогу не підтримується у вебклиєнті.'"), Неопределено);
		Возврат;
	#КонецЕсли
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге на компьютере уже есть файл
                           ...
                           |;uk='У робочому каталозі на комп''ютері вже є файл 
                           |""%1"",
                           |відповідний до іншого файлу в програмі.
                           |
                           |Рекомендується перейменувати один з файлів в програмі.'"),
				ПараметрыВыполнения.ПолноеИмяФайла);
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге на компьютере уже есть файл
                           ...
                           |відповідний до іншого файлу в програмі.
                           |
                           |Рекомендується вказати інший робочий каталог для однієї з папок в програмі.
...
Процедура Расшифровать(ОбработчикРезультата, ФайлСсылка, УникальныйИдентификатор, ДанныеФайла) Экспорт
	...
	Если ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Версий: %1';uk='Версій: %1'"), ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
	СписокПредставлений = Новый СписокЗначений;
...
Процедура ДобавитьФайл(
	...
	Если ПараметрыДобавления = Неопределено
		...
		ПараметрыВыполнения.Вставить("ФильтрДиалогаВыбора",  НСтр("ru='Все файлы (*.*)|*.*';uk='Всі файли (*.*)|*.*'"));
...
Процедура СравнитьФайлы(ИдентификаторФормы, ПервыйФайл, ВторойФайл, Расширение, ВладелецВерсии = Неопределено) Экспорт
	...
	Если Не РасширениеПоддерживается Тогда
		...
		НСтр("ru='Сравнение файлов поддерживается только для файлов следующих типов:
            ...
            |;uk='Порівняння файлів підтримується тільки для файлів наступних типів:
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Исключение
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сравнить файлы по причине: %1';uk='Не вдалося порівняти файли через: %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru='Файлы. Сравнение файлов';uk='Файли. Порівняння файлів'",ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
// Для работы метода на веб-клиенте необходимо предварительно подключить расширение для работы с BAF.
//
...
Процедура УдалитьДанные(ОбработчикЗавершения, ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	ТекстВопроса = ?(ЭтоВерсияФайла,
		НСтр("ru='Удалить версию файла без возможности восстановления?';uk='Видалити версію файлу без можливості відновлення?'"),
		НСтр("ru='Удалить файл без возможности восстановления?';uk='Видалити файл без можливості відновлення?'"));
	
	ТекстЗаголовка = ?(ЭтоВерсияФайла, НСтр("ru='Удаление версии файла';uk='Вилучення версії файлу'"), НСтр("ru='Удаление файла';uk='Вилучення файлу'"));
