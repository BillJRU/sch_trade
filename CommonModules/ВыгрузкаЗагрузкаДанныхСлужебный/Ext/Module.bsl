...
Процедура УдалитьВременныйФайл(Знач Путь) Экспорт
	...
	Попытка
		УдалитьФайлы(Путь);
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Удаление файла';uk='Вилучення файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьДанныеИзКаталога(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.2.19.0") < 0
		...
		ВызватьИсключение
			НСтр("ru='Для выполнения загрузки данных требуется обновить технологическую платформу ""BAF"".
                  ...
                  |;uk='Для виконання завантаження даних потрібно оновити технологічну платформу ""BAF"". 
	...
	Если ЕстьУстановленныеРасширения Тогда
		...
		ФоновоеЗадание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(ИмяМетода, МассивПараметров, Новый УникальныйИдентификатор, НСтр("ru='Восстановление области данных из архива';uk='Відновлення області даних з архіву'"));
...
Функция XMLТипСсылки(Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным';uk='Помилка при визначенні XMLТипа посилання для об''єкта %1: об''єкт не є посилальним'"),
