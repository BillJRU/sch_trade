...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Результат = Результат
		+ Символы.ПС
		+ НСтр("ru='Установленные расширения:';uk='Встановлені розширення:'")
		+ Символы.ПС
	...
	Результат = Результат
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Идентификатор ИБ: %1';uk='Ідентифікатор IБ: %1'"),
			?(ЗначениеЗаполнено(ИдентификаторЦентрМониторинга),
				ИдентификаторЦентрМониторинга,
				НСтр("ru='<Нет>';uk='<Немає>'")));
...
Функция ТекстОписаниеУстановленныхРасширений()
	...
	Для Каждого Расширение Из УстановленныеРасширения Цикл
		ОписаниеРасширенийМассивСтр.Добавить(
			Символы.Таб
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (%2); Акт.: %3; Распр. ИБ: %4; БР: %5';uk='%1 (%2); Акт.: %3; Розд. IБ: %4; БР: %5'"),
				Расширение.Имя,
	...
	Если ОписаниеРасширенийМассивСтр.Количество() = 0 Тогда
		ОписаниеРасширенийМассивСтр.Добавить(НСтр("ru='<Нет>';uk='<Немає>'"));
	КонецЕсли;
