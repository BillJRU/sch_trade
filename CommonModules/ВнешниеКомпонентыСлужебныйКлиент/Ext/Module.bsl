...
Процедура СохранитьКомпонентуВФайл(Ссылка) Экспорт 
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения внешней компоненты';uk='Виберіть файл для збереження зовнішньої компоненти'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru='Файлы внешних компонент (*.zip)|*.zip|Все файлы (*.*)|*.*';uk='Файли зовнішніх компонент (*.zip)|*.zip|Всі файли (*.*)|*.*'");
