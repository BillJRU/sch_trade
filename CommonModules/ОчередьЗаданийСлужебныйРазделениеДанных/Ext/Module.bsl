...
Процедура ОбновитьЗаданияОчередиПоШаблонам(Параметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если ИзмененияШаблонов.Удаленные.Количество() > 0
			...
			|	Очередь.Ссылка КАК ИдентификаторЗадания,
			...
			Пока Выборка.Следующий() Цикл
				...
				Если СтрокаШаблона = Неопределено Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.ИдентификаторЗадания);
					ВызватьИсключение(ТекстСообщения);
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Выборка.ИдентификаторЗадания) Тогда
					Задание = Выборка.ИдентификаторЗадания.ПолучитьОбъект();
				Иначе
...
Функция ОбновитьШаблоныЗаданийОчереди(Параметры)
	...
	Попытка
		...
		ИсходнаяТаблицаШаблонов = ШаблоныЗаданийОчереди(); 
		
		// Обработка добавленных / измененных шаблонов.
		Для каждого СтрокаТаблицы Из ТаблицаШаблонов Цикл
			
			ШаблонИзменен = Ложь;
			ШаблонСсылка = СтрокаТаблицы.Ссылка;
			ИмяШаблона = СтрокаТаблицы.Имя;			
			ИсходнаяСтрокаШаблона = ИсходнаяТаблицаШаблонов.Найти(ШаблонСсылка, "Ссылка");
			Если ИсходнаяСтрокаШаблона = Неопределено
				...
				СтрокаИзменения.Ссылка = ШаблонСсылка;
			...
			Если ШаблонИзменен
				...
				ИЛИ Шаблон.Имя <> ИмяШаблона Тогда
		...
		Для Каждого ИсходнаяСтрокаШаблона Из ИсходнаяТаблицаШаблонов Цикл
			...
			ИсходныйШаблон = ИсходнаяСтрокаШаблона.Ссылка;
			Шаблон = ИсходныйШаблон.ПолучитьОбъект();
			Шаблон.ОбменДанными.Загрузка = Истина;
			Шаблон.Удалить();
			
			УдаленныеШаблоны.Добавить(ИсходныйШаблон);
		КонецЦикла;
...
// Возвращает таблицу значений с перечнем всех шаблонов заданий очереди.
// 
// Возвращаемое значение:
// 	ТаблицаЗначений - описание:
// 	 * Ссылка - СправочникСсылка.ШаблоныЗаданийОчереди - 
// 	 * Использование - Булево - признак использования.
// 	 * Ключ - Строка - ключ задания.
// 	 * Расписание - ХранилищеЗначения - хранит расписание регламентного задания.
Функция ШаблоныЗаданийОчереди()
	...
	ШаблоныЗаданийОчереди = Запрос.Выполнить().Выгрузить();
	
	Возврат ШаблоныЗаданийОчереди;
