...
// - возможна, если установлен конфигуратор (полный дистрибутив технологической платформы BAF для Windows);
...
Функция ПоддерживаетсяУстановкаОбновлений() Экспорт
	...
	Если ПараметрыРаботыКлиента.РазделениеВключено Тогда 
		Результат.ОписаниеОшибки = 
			НСтр("ru='Установка обновлений для приложения в Интернете выполняется централизованно через Менеджер сервиса.';uk='Встановлення оновлень для прикладної програми в Інтернеті виконується централізовано через Менеджер сервісу.'");
		Возврат Результат;
	КонецЕсли;
	
	Если Не ПараметрыРаботыКлиента.ЭтоАдминистраторСистемы Тогда 
		Результат.ОписаниеОшибки = НСтр("ru='Для установки обновления требуются права администрирования.';uk='Для установки оновлення необхідні права адміністрування.'");
		Возврат Результат;
	КонецЕсли;
...
#Если ВебКлиент Тогда
	Результат.ОписаниеОшибки =
		НСтр("ru='Установка обновления не доступна в веб-клиенте.
                   |Необходимо установить полный дистрибутив технологической платформы BAF для Windows.'
                   |;uk='Встановлення оновлення не доступне в вебклієнті.
                   |Необхідно встановити повний дистрибутив технологічної платформи BAF для Windows.'");
	
#ИначеЕсли МобильныйКлиент Тогда
	Результат.ОписаниеОшибки = НСтр("ru='Установка обновления доступна только в ОС Windows.';uk='Встановлення оновлення доступне тільки в ОС Windows.'");
#Иначе
	...
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда 
		...
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("ru='Установка обновления не доступна при подключении через веб-сервер.
                         ...
                         |;uk='Встановлення оновлення не доступне при підключенні через вебсервер.
                         |Необхідно налаштувати пряме підключення до інформаційної бази.'");
	КонецЕсли;
	...
	Если Не ПоддерживаетсяПакетныйРежимКонфигуратора() Тогда 
		...
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("ru='Для установки обновления требуется конфигуратор.
                         |Необходимо установить полный дистрибутив технологической платформы BAF для Windows.'
                         |;uk='Для установки оновлення потрібен конфигуратор.
                         |Необхідно встановити повний дистрибутив технологічної платформи BAF для Windows.'");
	КонецЕсли;
...
Функция ЗаголовокСозданияРезервнойКопии(Параметры) Экспорт
	...
	Если Параметры.СоздаватьРезервнуюКопию = 0 Тогда
		Возврат НСтр("ru='Не создавать резервную копию ИБ';uk='Не створювати резервну копію ІБ'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 1 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("ru='Создавать временную резервную копию ИБ и выполнять откат при нештатной ситуации';uk='Створювати тимчасову резервну копію ІБ і виконувати відкат при позаштатній ситуації'");
		Иначе 
			Возврат НСтр("ru='Создавать временную резервную копию ИБ и не выполнять откат при нештатной ситуации';uk='Створювати тимчасову резервну копію ІБ і не виконувати відкат при нештатній ситуації'");
		КонецЕсли;
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("ru='Создавать резервную копию ИБ и выполнять откат при нештатной ситуации';uk='Створювати резервну копію ІБ і виконувати відкат при позаштатній ситуації'");
		Иначе 
			Возврат НСтр("ru='Создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';uk='Створювати резервну копію ІБ і не виконувати відкат при нештатній ситуації'");
		КонецЕсли;
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда // АПК:566 синхронные вызовы вне веб-клиента разрешены;
			ПоказатьПредупреждение(,
				НСтр("ru='Укажите существующий каталог для сохранения резервной копии ИБ.';uk='Укажіть існуючий каталог для збереження резервної копії ІБ.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
		...
		Если ЭтоФайловаяБаза
			...
			ПоказатьПредупреждение(,
				НСтр("ru='Невозможно продолжить обновление конфигурации,
                           ...
                           |;uk='Неможливо продовжити оновлення конфігурації,
                           |так як не завершені всі з''єднання з інформаційною базою.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если УказанНеДопустимыйАдресЭлектроннойПочты Тогда
			ПоказатьПредупреждение(,
				НСтр("ru='Укажите допустимый адрес электронной почты.';uk='Укажіть припустиму адресу електронної пошти.'"));
			Возврат Ложь;
		КонецЕсли;
		
		Если Не ПоддерживаетсяПланировщикЗаданий() Тогда
			ПоказатьПредупреждение(,
				НСтр("ru='Планировщик заданий поддерживается только начиная с операционной системы версии 6.0 Vista.';uk='Планувальник завдань підтримується тільки починаючи з операційної системи версії 6.0 Vista.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru='Обновление конфигурации';uk='Оновлення конфігурації'",ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
КонецФункции
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоОтложенноеОбновление Тогда 
		...
		ОписаниеЗадачи = НСтр("ru='Обновление конфигурации BAS';uk='Оновлення конфігурації BAS'");
