...
//			устанавливается на сервере BAF;
...
// которого интегрирована с Порталом ИТС.
...
// страницы происходит с учетными данными пользователя Портала ИТС.
...
// Возвращает номер версии платформы платформы BAF.
//
...
Процедура ПроверитьURL(URL) Экспорт
	...
	Если Прав(НРег(СокрЛП(СтруктураURI.Хост)), СтрДлина(".bas-soft.eu")) <> ".bas-soft.eu" 
		И Прав(НРег(СокрЛП(СтруктураURI.Хост)), 6) <> Символ(46)+Символ(49)+Символ(99)+".eu" Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ТипЗнч(ОбъектХДТО) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			...
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:БазовыйФункционал.Отладка';uk='БІП:БазовыйФункционал.Налагодження'",ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
				НСтр("ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';uk='БазовыйФункционал. Налагодження. ПредставлениеОбъектаХДТО'",ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Функция ВыгрузитьВсеСобытияЖурналаРегистрации(ПараметрыОтбора, ПараметрыФайлаВыгрузки = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:БазовыйФункционал.Отладка';uk='БІП:БазовыйФункционал.Налагодження'",ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия
			НСтр("ru='БазовыйФункционал. Отладка. Экспорт журнала регистрации';uk='БазовыйФункционал. Налагодження. Експорт журналу реєстрації'",ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка()
	...
	Возврат НСтр("ru='Интернет-поддержка пользователей.Ошибка';uk='Інтернет-підтримка користувачів.Помилка'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация()
	...
	Возврат НСтр("ru='Интернет-поддержка пользователей.Информация';uk='Інтернет-підтримка користувачів.Інформація'",ОбщегоНазначения.КодОсновногоЯзыка());
...
// в БИП.
//
// Версия платформы BAF 8.3.7 или выше и не работа в модели сервиса.
//
