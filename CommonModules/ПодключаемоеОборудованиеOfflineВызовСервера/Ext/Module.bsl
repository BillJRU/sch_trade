...
Процедура РегламентноеЗаданиеОбновитьКодыТоваров() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Обновление кодов товаров подключаемого оборудования Offline';uk='Оновлення кодів товарів обладнання для підключення Offline'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ДанныеТоваровПоПравилуОбмена(ПравилоОбмена, ВидЦены) Экспорт
	...
	ТаблицаТоваров = Новый ТаблицаЗначений;
	ПодключаемоеОборудованиеOfflineВызовСервераЛокализация.ДанныеТоваровПоПравилуОбмена(ТаблицаТоваров, ПравилоОбмена, ВидЦены);
