...
#Область ПланыОбмена_ОбменССайтом_ЗаполнитьРеквизитыНастройкиОбмена
	...
	Обработчик.ЧитаемыеОбъекты = "";
	Обработчик.ИзменяемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru='Обмен с сайтом: Заполнение настроек обмена с сайтом.';uk='Обмін з сайтом: Заповнення настройок обміну з сайтом.'");
...
#Область ПланыОбмена_ОбменССайтом_ПеренестиПарольВБезопасноеХранилище
	...
	Обработчик.ЧитаемыеОбъекты = "";
	Обработчик.ИзменяемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru='Обмен с сайтом: Перенос пароля в безопасное хранилище.';uk='Обмін з сайтом: Перенесення пароля в безпечне сховище.'");
...
Функция ВыгрузитьДанныеНаСайт(Параметры, СтрокаТаблицыИнформации, ТипСоединения = Неопределено)
	...
	Иначе
		...
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ ТекущаяДатаСеанса()+ " " + НСтр("ru='Выгрузка на сайт завершилась с ошибками.';uk='Вивантаження на сайт завершилося з помилками.'")
			+ Символы.ПС + ОписаниеОшибки;
...
Процедура ЗаписатьДанныеВФайл(Параметры, КоммерческаяИнформацияXDTO, СтрокаТаблицыИнформации, ФайлЗаписан)
	...
	Исключение
		ОписаниеИсключительнойОшибки = ОписаниеИсключительнойОшибки(
										НСтр("ru='Не удалось записать XML-файл изменений предложений на диск:';uk='Не вдалося записати XML-файл змін пропозицій на диск:'")
...
Процедура ЗаполнитьДанныеВладельцаКаталога(Параметры, ДанныеОрганизацииВладельцаКаталога)
	...
	ИмяРеквизитаЕДРПОУ = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЕДРПОУОрганизации");
	...
	РеквизитыОрганизации.Добавить(ИмяРеквизитаЕДРПОУ);
	...
	ДанныеОрганизацииВладельцаКаталога.Вставить("КодПоЕДРПОУ", ПараметрыОрганизации[ИмяРеквизитаЕДРПОУ]);
...
Функция ВыгрузитьКаталог(Параметры, СтрокаТаблицыКаталога, ТаблицаИнформации, КаталогИПакетПредложений = Ложь)
	...
	СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДатаСеанса()) + " " + НСтр("ru='Запуск выгрузки товаров';uk='Запуск вивантаження товарів'");
...
Функция ВыгрузитьОстаткиИЦены(Параметры, СтрокаТаблицыКаталога, ТаблицаИнформации)
	...
	СтрокаТаблицыИнформации = ТаблицаИнформации.Добавить();
	СтрокаТаблицыИнформации.ДействиеПриОбмене = Перечисления.ДействияПриОбменеССайтом.ВыгрузкаДанных;
	СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДатаСеанса()) + " " + НСтр("ru='Запуск выгрузки обновление пакета';uk='Запуск вивантаження оновлення пакета'");
...
Процедура ПолучитьДанныеСкладыПакетаПредложений(ДанныеПакетаПредложений, Параметры, СкладыПакетаПредложений)
 	...
 	Если Параметры.ПрикладныеПараметры.Свойство("СписокТочекСамовывоза") Тогда
	 	СписокТочекСамовывоза = Параметры.ПрикладныеПараметры.СписокТочекСамовывоза;
	Иначе
	 	СписокТочекСамовывоза = Новый СписокЗначений;
	КонецЕсли; 
...
Функция ВыгрузитьПакетПредложений(Параметры, СтрокаТаблицыКаталога, ТаблицаИнформации, КаталогИПакетПредложений = Ложь)
	...
	СтрокаТаблицыИнформации = ТаблицаИнформации.Добавить();
	СтрокаТаблицыИнформации.ДействиеПриОбмене = Перечисления.ДействияПриОбменеССайтом.ВыгрузкаДанных;
	СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДатаСеанса()) + " " + НСтр("ru='Запуск выгрузки пакета предложений';uk='Запуск вивантаження пакету пропозицій'");
	...
	Пока ВыполнятьОбмен Цикл
		...
		Исключение
			...
			ТекстСообщенияОбОшибке = НСтр("ru='Ошибка записи пакета предложений на диск.';uk='Помилка запису пакета пропозицій на диск.'") + Символы.ПС 
...
Процедура ЗаполнитьЦеныXDTO(ЦеныXDTO, ТаблицаЦен)
	...
	Для Каждого ТекСтрока Из ТаблицаЦен Цикл
		...
		ПредставлениеЦены = " " + СокрЛП(ТекСтрока.ЦенаЗаЕдиницу) + " " + СокрЛП(ТекСтрока.ТипЦеныВалютаКод) + " "+"за"+" " + СокрЛП(Единица);
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, Параметры, ОписаниеОшибки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка документов с сайта';uk='Завантаження документів з сайту'",ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Для Каждого СсылкаНаДокумент Из СтруктураСтатистики.Загружено Цикл  
		...
		Если Не СсылкаНаДокумент.Проведен Тогда
			...
			исключение 
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось провести заказ №';uk='Не вдалося провести замовлення №'") + ДокументОбъект.Номер));
			КонецПопытки;
	...
	Для Каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если ЗаказЗакрытОтменен Тогда
				...
				ТекстОшибки = Строка(НайденнаяСтрока.ЗаказПокупателя) + " " + НСтр("ru='пропущен по причине:';uk='пропущений через:'") + Символы.ПС 
...
Процедура ИдентифицироватьСклад(ДанныеСклада, Склад, ОписаниеОшибки)
	...
	Если Не ЗначениеЗаполнено(ДанныеСклада.Ид) 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru='Не заполнены поля поиска точки самовывоза: ""Ид"" и ""Наименование""';uk='Не заповнені поля пошуку точки самовивозу: ""Ід"" і ""Найменування""'"));
...
Функция УточненноеПредставлениеАдреса(АдресXDTO)
	...
	Если ОбъектXDTOСодержитСвойство(АдресXDTO, "АдресноеПоле")
		...
		ПредставлениеАдреса = ?(ЗначениеЗаполнено(ПочтовыйИндекс), ПочтовыйИндекс+","+ " ","")
			...
			+ ?(ЗначениеЗаполнено(Город), Город + " " + НСтр("ru='г';uk='г'") + ","+ " ","")
			+ ?(ЗначениеЗаполнено(Улица), Улица + " " + НСтр("ru='ул';uk='вул'") + "," + " ","")
			+ ?(ЗначениеЗаполнено(Дом),НСтр("ru='дом №';uk='будинок №'") + " " + Дом + "," + " ","")
			+ ?(ЗначениеЗаполнено(Корпус), Корпус + "," + " ","")
			+ ?(ЗначениеЗаполнено(Квартира),НСтр("ru='квартира';uk='квартира'") + " " + Квартира + " ","");
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки Цикл
		...
		Если ZIPФайлыРазрешены Тогда
			...
			Исключение
				...
				ШаблонСообщения = НСтр("ru='Ошибка удаления файла %1';uk='Помилка видалення файлу %1'");
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ОписаниеОшибки, ПараметрыЗапроса="", Заголовки = Неопределено)
	...
	ИначеЕсли HTTPОтветСервера.КодСостояния <> 200 Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru='Отправка файла на сервер: ответ сервера = %1';uk='Відправлення файлу на сервер: відповідь сервера = %1'"), HTTPОтветСервера.КодСостояния);
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
	КонецЕсли;
...
|Перевірте правильність адреси сервера, порт, ім''я користувача та пароль,'")
	...
	Если HTTPОтветСервера = Неопределено Тогда
		...
		Возврат Неопределено;
	ИначеЕсли HTTPОтветСервера.КодСостояния <> 200 Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru='Получение данных с сервера: ответ сервера = %1';uk='Отримання даних з сервера: відповідь сервера = %1'"), HTTPОтветСервера.КодСостояния);
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
	КонецЕсли;
	
	ТекстОтвета = HTTPОтветСервера.ПолучитьТелоКакСтроку();
	Если Не ЗначениеЗаполнено(ТекстОтвета) Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Получение данных с сервера: Получен пустой ответ сервера.';uk='Отримання даних з сервера: Отримано порожній відповідь сервера.'"));
	Иначе
		ОтветСервера = ТекстОтвета;
	КонецЕсли;
...
Функция ДатаВремяИзСтроки(ДатаСтрока, ВремяСтрока = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка преобразования даты';uk='Помилка перетворення дати'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
