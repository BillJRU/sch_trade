...
Процедура ОчиститьДанныеОбласти(Знач УдалитьПользователей) Экспорт
	...
	ИсключенияОчистки.Добавить(Метаданные.Справочники.ОчередьЗаданийОбластейДанных.ПолноеИмя());
...
Процедура ОбслуживаниеОбластейДанных() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Менеджер.Повтор < МаксимальноеКоличествоПовторов Тогда
			...
			ПараметрыЗадания.Вставить("ВыполнятьЧерезРегламентноеЗадание", Менеджер.Статус = Перечисления.СтатусыОбластейДанных.КУдалению);
...
Процедура ОбработатьИнформациюОбОшибкеWebСервиса(Знач ИнформацияОбОшибке, Знач ИмяПодсистемы = "", Знач ИмяВебСервиса = "", Знач ИмяОперации = "") Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1.Ошибка вызова операции web-сервиса';uk='%1.Помилка виклику операції web-сервісу'",ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяПодсистемы);
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при вызове операции %1 веб-сервиса %2: %3';uk='Помилка при виклику операції %1 вебсервісу %2: %3'",ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяОперации,
		ИмяВебСервиса,
...
Функция СобытиеЖурналаРегистрацииКопированиеОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Копирование области данных';uk='Робота в моделі сервісу.Копіювання області даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииБлокировкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Блокировка области данных';uk='Робота в моделі сервісу.Блокування області даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПодготовкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Подготовка области данных';uk='Робота в моделі сервісу.Підготовка області даних'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Получение файла из хранилища';uk='Робота в моделі сервісу.Отримання файлу з сховища'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменЧерезФС() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Добавление файла.Обмен через ФС';uk='Робота в моделі сервісу.Додавання файлу.Обмін через ФС'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменНеЧерезФС() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Добавление файла.Обмен не через ФС';uk='Робота в моделі сервісу.Додавання файлу.Обмін не через ФС'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Удаление временного файла';uk='Робота в моделі сервісу.Видалення тимчасового файлу'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru='Извлечение';uk='Витягування'",ОбщегоНазначения.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Исключение
		...
		ФайлОбъединен = Ложь;
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции объединения файлов';uk='Виконання операції об''єднання файлів'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлОбъединен Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции чтения zip файла';uk='Виконання операції читання zip файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции удаления временного файла';uk='Виконання операції вилучення тимчасового файлу'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// 
// Параметры:
//   Дескриптор - ОбъектXDTO, Струкура - Дескриптор.
...
// 
// Параметры:
//   Дескриптор - Струкура - Дескриптор.
//   Загружать - Булево - Истина, если загружать, Ложь - иначе.
//
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			Для Каждого КонфликтующийСеанс Из КонфликтующиеСеансы Цикл
				...
				ТекстСеансы = ТекстСеансы + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 (сеанс - %2)';uk='%1 (сеанс - %2)'",ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';uk='Операція не може бути виконана, т. я. в додатку працюють інші користувачі: %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ТекстСеансы);
...
Процедура УстановитьИспользованиеПредопределенногоРегламентногоЗадания(ЗаданиеМетаданные, Использование) Экспорт
	...
	КонецЕсли;
	Если Задания.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru='Не найдено задание в очереди для предопределенного задания с именем %1';uk='Не знайдено завдання у черзі для напередвизначеного завдання з ім''ям %1'");
...
//    или доп. обработка для которой необходимо установить режим работы.
//   РежимРаботы - ОпределяемыйТип.БезопасныйРежим - Режим работы доп. отчета или обработки.
//
Процедура УстановитьРежимПодключенияДополнительногоОтчетаИлиОбработкиВОбластиДанных(ПоставляемаяОбработка, РежимПодключения) Экспорт
...
//
// Параметры:
//  МенеджерЗаписи - РегистрыСведенийМенеджерЗаписи.ОбластиДанных - менеджер записи
	...
	Если ОбменЧерезФС Тогда
		...
		Если ЦелевойФайл.Существует() Тогда
			...
			Если СвойстваФайла.ПолноеИмя = ЦелевойФайл.ПолноеИмя Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru='Помещение';uk='Приміщення'",ОбщегоНазначения.КодОсновногоЯзыка()),
					Результат,
...
Процедура ЗаписатьВЖурналСобытиеФайловогоХранилища(Знач Событие,
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Хранилище файлов';uk='Сховище файлів'",ОбщегоНазначения.КодОсновногоЯзыка()) + "." + Событие,
...
//
// Параметры:
//   ЗначениеСтруктурногоТипа - Массив Из Структура, Массив Из Соответствие - или их фиксированные аналоги.
//
// Возвращаемое значение:
...
Процедура КонтрольНеразделенныхДанныхПриЗаписи(Знач Источник)
	...
	Если РазделениеВключено() И ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ПредставлениеИсключения = НСтр("ru='Нарушение прав доступа.';uk='Порушення прав доступу.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УдалитьДанныеОчередиЗаданий()
