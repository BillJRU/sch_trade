	...
	Если Не ПустаяСтрока(КодЭтогоУзла) И КодЭтогоУзла <> Тело.Code Тогда
		...
		Если ПсевдонимЭтогоУзла <> Тело.Code Тогда
			СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Очікуваний код напередвизначеного вузла в цьому додатку ""%1""
                |не відповідає фактичному ""%2"" або псевдоніму ""%3"". План обміну: %4.'"),
...
Процедура ЗагрузитьСообщениеОбмена(Сообщение, Отправитель) Экспорт
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В процессе загрузки данных из программы  ""%1"" возникли ошибки.';uk='У процесі завантаження даних з програми ""%1"" виникли помилки.'"),
...
Функция КорреспондентОбмена(Знач ИмяПланаОбмена, Знач Код)
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2.';uk='Не вдалося знайти вузол плану обміну; ім''я плану обміну %1; код вузла %2.'"),
			ИмяПланаОбмена, Код);
		ВызватьИсключение СтрокаСообщения;
	КонецЕсли;
