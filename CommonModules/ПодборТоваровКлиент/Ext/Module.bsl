...
Процедура ПолучитьИнформациюОТовареПриПродаже(Форма) Экспорт
	...
	Для Каждого СтрокаТбл Из ИнформацияОТоваре.ТекущиеОстатки Цикл
		...
		ПланируемыеОстаткиПоДатам = СтрокаОстаткиПоСкладам.ПолучитьЭлементы(); // ДанныеФормыКоллекцияЭлементовДерева
...
Функция ПредставлениеИнтервалаОтбора(ИнтервалОт, ИнтервалДо)
	...
	Если Не ЗначениеЗаполнено(ИнтервалОт)
		...
		ПредставлениеОтбора =  НСтр("ru='<не задано>';uk='<не зазначено>'");
		Возврат ПредставлениеОтбора;
	ИначеЕсли ЗначениеЗаполнено(ИнтервалОт)
		И ЗначениеЗаполнено(ИнтервалДо) Тогда
		ПредставлениеОтбора =  НСтр("ru='от %ИнтервалОт% до %ИнтервалДо%';uk='від %ИнтервалОт% до %ИнтервалДо%'");
	ИначеЕсли ЗначениеЗаполнено(ИнтервалОт) Тогда 
		ПредставлениеОтбора =  НСтр("ru='от %ИнтервалОт%';uk='від %ИнтервалОт%'");
	Иначе
		ПредставлениеОтбора =  НСтр("ru='до %ИнтервалДо%';uk='до %ИнтервалДо%'");
	КонецЕсли;
...
Процедура УстановитьОтборПоРодителюТекущейСтрокиНоменклатурыПоставщика(Форма)
	...
	ТаблицаФормыПоиска = Форма.Элементы[ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоставщикаПоВариантуПоиска(Форма)]; // ТаблицаФормы
