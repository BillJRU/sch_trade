...
Процедура ОтослатьСтатистику(Знач ИдентификаторСчетчика, Знач ЗначениеСчетчика, Знач Данные = Неопределено, Знач ДатаСчетчика = неопределено) Экспорт
	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='ИнтеграцияЦКК';uk='ИнтеграцияЦКК'",ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ru='Ошибка при передаче счетчика в ЦКК';uk='Помилка при передачі лічильника до ЦКЯ'",ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(КомандаЗапроса, Данные)
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтрШаблон(
			НСтр("ru='Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2';uk='Помилка (код %3) команди %1 сервісу інцидентів ЦКЯ: %2'",ОбщегоНазначения.КодОсновногоЯзыка()), 
