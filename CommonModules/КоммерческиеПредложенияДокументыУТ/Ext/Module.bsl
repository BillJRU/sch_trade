...
Процедура ПриЗаполненииСпискаТекущихДелКоммерческихПредложений(ТекущиеДела, ИмяДокумента) Экспорт
	...
	Если ИмяДокумента = "КоммерческоеПредложениеКлиенту"
		...
		Если Доступность Тогда
			...
			Дело.Представление  = НСтр("ru='На согласовании';uk='На погодженні'");
			...
			Дело.Представление  = НСтр("ru='Подготавливается';uk='Підготовлюється'");
			...
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("СостояниеОтбор",   СписокСостояний);
			ПараметрыОтбора.Вставить("Актуальность",     НСтр("ru='Не истек';uk='Не минув'"));
			ПараметрыОтбора.Вставить("ДатаАктуальности", ОбщиеПараметрыЗапросов.ПустаяДата);
			ПараметрыОтбора.Вставить("Менеджер",         ОбщиеПараметрыЗапросов.Пользователь);
			...
			Дело.Представление  = НСтр("ru='Действует';uk='Діє'");
...
Процедура АнализируемыеИсточникиПредложений(Источники) Экспорт
	...
	НоваяСтрока.КраткоеПредставление   =  НСтр("ru='Условия закупок';uk='Умови купівель'");
	...
	НоваяСтрока.КраткоеПредставление   = НСтр("ru='Цены поставщиков';uk='Ціни постачальників'");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИсточники Цикл
		...
		Если Метаданные.Документы.Содержит(Источник.Метаданные()) Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Коммерческие предложения.Сохранения настроек';uk='Комерційні пропозиції. Збереження настройок'",ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
					, Источник, НСтр("ru='Сохранение настроек запроса коммерческих предложений';uk='Збереження настройок запиту комерційних пропозицій'"));
...
Процедура КоммерческоеПредложениеКлиентуПриФормированииСтруктурыДанныхПечати(МассивОбъектов, СтруктураДанныхДляПечати, ПараметрыПечати, СтандартнаяОбработка) Экспорт
	...
	|	ТаблицаТовары.Активность													  КАК Активность
	|
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	СтруктураДанныхДляПечати.Вставить("РезультатПоТоварам",      ПакетРезультатовЗапроса[9]);
	...
	ИначеЕсли НаборыСервер.ИспользоватьОбластьКомплектующие(СтрокаТовары, Истина) Тогда
		...
		Если СтрокаТовары.ВариантПредставленияНабораВПечатныхФормах = Перечисления.ВариантыПредставленияНаборовВПечатныхФормах.НаборИКомплектующие Тогда
			...
			Иначе
				...
				ОбластьСтрокаТаблицыТовары.Параметры.Заполнить(ДанныеСтроки);
				СтрокаТовары.Номенклатура = СтрШаблон(НСтр("ru='%1, %2 %3';uk='%1, %2 %3'"), СтрокаТовары.Номенклатура, СтрокаТовары.Количество, СтрокаТовары.ЕдиницаИзмерения);
...
Процедура ПриВставкеСтрокИзБуфераОбмена(Форма, КоличествоСтрок, СтандартнаяОбработка) Экспорт
	...
	Для каждого СтрокаТовара Из ДанныеБуфера Цикл
		...
		КонецЕсли;
		НалогообложениеНДС = КоммерческиеПредложенияДокументыКлиентСерверУТ.НалогообложениеНДСПоНалогообложениюКоммерческихПредложений(Форма.НалогообложениеНДСПоУмолчанию);
...
#Область ПараметрыВыбора
	...
	// Контрагент
	НовыеПараметры = Новый Массив;
	НовыеПараметры.Добавить(Новый ПараметрВыбора("ЗаголовокПоПартнеру", НСтр("ru='По клиенту';uk='По клієнту'")));
...
#Область ДобавлениеЭлементовФормы
	...
	КомандаФормы.Заголовок   = НСтр("ru='Есть сообщения';uk='Є повідомлення'");
...
#Область Наборы
	...
	Если ИспользоватьНаборы
		...
		Элемент.Заголовок        = НСтр("ru='Наборы';uk='Набори'");
		...
		КомандаФормы.Заголовок                 = НСтр("ru='Состав набора';uk='Склад набору'");
		КомандаФормы.Действие                  = "Подключаемый_ПереопределяемаяКоманда";
		...
		КомандаФормы.Заголовок   = НСтр("ru='Показать наборы';uk='Показати набори'");
		КомандаФормы.Действие    = "Подключаемый_ПереопределяемаяКоманда";
...
Процедура КоммерческоеПредложениеКлиентуЗаполнитьНалогообложениеНДСПродажи(Форма)
	...
	Если Налогообложение <> Форма.НалогообложениеНДСПоУмолчанию Тогда
		Форма.НалогообложениеНДСПоУмолчанию = Налогообложение;		
		КоммерческоеПредложениеКлиентуПриИзмененииНалогообложенияНДС(Форма);
	КонецЕсли;
...
Процедура КоммерческоеПредложениеКлиентуЗаполнитьЦеныПоУсловиюПродаж(Форма, УсловиеПродаж, ДополнительныеПараметры)
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомРучнойСкидки", Новый Структура("Очищать, ИмяКоличества", Ложь, "Количество"));
	СтруктураДействий.Вставить("ОчиститьАвтоматическуюСкидку");
	НалогообложениеНДС = КоммерческиеПредложенияДокументыКлиентСерверУТ.НалогообложениеКоммерческихПредложенийПоНалогообложениюНДС(Форма.НалогообложениеНДСПоУмолчанию);
	ПараметрыЗаполнения = Новый Структура();
	ПараметрыЗаполнения.Вставить("Дата",               Объект.Дата);
	ПараметрыЗаполнения.Вставить("Валюта",             Объект.Валюта);
...
Функция ПараметрыОткрытияСоставаНабора(Форма)
	...
	ПараметрыОткрытия.Вставить("НалогообложениеНДС", КоммерческиеПредложенияДокументыКлиентСерверУТ.НалогообложениеНДСПоНалогообложениюКоммерческихПредложений(Форма.НалогообложениеНДСПоУмолчанию));
...
Процедура КоммерческоеПредложениеКлиентуПриОтменеРучныхСкидокНаСервере(Форма, РезультатОбработки)
	...
	СкидкиНаценкиСервер.ОтменитьРучныеСкидки(Объект, "Товары", Истина, Истина, Ложь, Ложь, ДополнительныеПараметры);
...
Процедура ЗаполнитьКоммерческоеПредложениеКлиентуНаОснованииСделкиСКлиентом(Объект, ДанныеЗаполнения)
	...
	Объект.Товары.Загрузить(Выборка.ПервичныйСпрос.Выгрузить());
...
Процедура ЗаполнитьКоммерческоеПредложениеКлиентуНаОснованииКоммерческогоПредложенияКлиенту(Объект, ДанныеЗаполнения)
	...
	|	Документ.КоммерческоеПредложениеКлиенту.Товары КАК Товары
	|ГДЕ
	|	Товары.Ссылка В(&ДокументОснование) И Товары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
	|СГРУППИРОВАТЬ ПО Товары.Ссылка 
	|;
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КоммерческоеПредложениеКлиенту.Организация                         КАК Организация,
	...
	|	КоммерческоеПредложениеКлиентуТовары.Активность								   КАК Активность,
	|	КоммерческоеПредложениеКлиентуТовары.ТекстовоеОписание						   КАК ТекстовоеОписание,
	...
	КонецЦикла;
	ВыборкаСкидки = РезультатЗапроса[3].Выбрать();
...
#Область КоммерческиеПредложенияДокументыУТ_КоммерческиеПредложенияКлиентуОбработатьДанныеДляПереходаНаНовуюВерсию

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("064c93d7-6ed8-411b-a8b4-0a442a25d0ea");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru='Заполнение реквизита СтавкаНДС с типом СправочникСсылка.СтавкиНДС';uk='Заповнення реквізиту СтавкаНДС з типом СправочникСсылка.СтавкиНДС'");
	...
	НоваяСтрока.Процедура = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД";
	НоваяСтрока.Порядок = "Любой";

#КонецОбласти

#Область КоммерческиеПредложенияДокументыУТ_КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД";
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("0da389b1-4567-47dc-a99d-c3ba801480a2");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуЗарегистрироватьДокументыДляПереводаНаВерсиюБЭД";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЗапускатьТолькоВГлавномУзле = Истина;
	Обработчик.Комментарий = НСтр("ru='Обновление документа ""Коммерческое предложение клиенту""';uk='Оновлення документа ""Комерційна пропозиція клієнту""'");
	...
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.БизнесПроцессы.СогласованиеПродажи.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Задачи.ЗадачаИсполнителя.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.РегистрыСведений.СостоянияКоммерческихПредложенийКлиентам.ПолноеИмя());
	Обработчик.БлокируемыеОбъекты = СтрСоединить(Блокируемые, ",");
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "КоммерческиеПредложенияДокументыУТ.КоммерческиеПредложенияКлиентуОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "Любой";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ИспользоватьУчетНДС И ДокументОбъект <> Неопределено Тогда
				...
				ЗаполнитьКолонкуТЧСтавкаНДС(ДокументОбъект, МассивТЧ, "СтавкаНДС", ОбъектИзменен);
...
Процедура КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД(Параметры) Экспорт
	...
	Для Каждого ДокументСсылка Из КоммерческиеПредложенияСсылки Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru='Не удалось записать документ %1 по причине:';uk='Не вдалося записати документ %1 з причини:'");
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru='Не удалось обработать некоторые коммерческие предложения контрагенту (пропущены): %1';uk='Не вдалося обробити деякі комерційні пропозиції контрагенту (пропущені): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонСообщения = НСтр("ru='Обработана очередная порция коммерческих предложений клиенту: %1';uk='Опрацьована чергова порція комерційних пропозицій клієнту: %1'");
...
Процедура ЗаполнитьДокументКоммерческоеПредложениеКлиенту(ДокументОбъект, Контрагенты)
	...
	Если ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		УсловияДоставкиТекст = НСтр("ru='Самовывоз (с нашего склада)';uk='Самовивіз (з нашого складу)'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.ДоКлиента Тогда
		УсловияДоставкиТекст = НСтр("ru='Наша транспортная служба до клиента';uk='Наша транспортна служба до клієнта'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаПоАдресу Тогда
		УсловияДоставкиТекст = НСтр("ru='Наша транспортная служба до перевозчика';uk='Наша транспортна служба до перевізника'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчика Тогда
		УсловияДоставкиТекст = НСтр("ru='Перевозчик (с нашего склада)';uk='Перевізник (з нашого складу)'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.КПолучателюОпределяетСлужбаДоставки Тогда
		УсловияДоставкиТекст = НСтр("ru='На усмотрение транспортной службы';uk='На розсуд транспортної служби'");
	Иначе
	...
	Для Каждого ТекущийТовар Из ДокументОбъект.Товары Цикл
		...
		Если Не ДокументОбъект.Проведен И Не ЗначениеЗаполнено(ТекущийТовар.Номенклатура) Тогда
			ТекущийТовар.КомментарийДляПокупателя = ТекущийТовар.ТекстовоеОписание;
		КонецЕсли;

		Если ИспользоватьУчетНДС И ТипЗнч(ТекущийТовар.СтавкаНДС) <> Тип("СправочникСсылка.СтавкиНДС") Тогда
			ТекущийТовар.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(ТекущийТовар.СтавкаНДС);
		КонецЕсли;
...
Процедура ЗаполнитьКолонкуТЧСтавкаНДС(ДокументОбъект, МассивТЧ, ИмяКолонки, ОбъектИзменен) Экспорт
	
	Для Каждого ТабличнаяЧасть Из МассивТЧ Цикл
		Для Каждого СтрокаТЧ Из ДокументОбъект[ТабличнаяЧасть] Цикл
			Если ТипЗнч(СтрокаТЧ[ИмяКолонки]) <> Тип("СправочникСсылка.СтавкиНДС") Тогда
				СтрокаТЧ[ИмяКолонки] = УчетНДСЛокализация.СтавкаНДСПоПеречислению(СтрокаТЧ[ИмяКолонки]);
