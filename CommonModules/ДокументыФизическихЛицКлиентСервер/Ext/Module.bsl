...
Функция НомерДокументаУказанПравильно(ВидДокумента, Знач Номер, ТекстОшибки) Экспорт
	...
	ИначеЕсли ТипДокумента = 5 Тогда
		Если НЕ ((ДлинаНомера  = 6 ) Или ( ДлинаНомера = 7 ) Или ( ДлинаНомера = 21 )) Тогда
			ТекстОшибки = НСтр("ru='Номер документа должен состоять из 6, 7 или 21 символа.';uk='Номер документа має складатися з 6, 7 або 21 символу.'");
			Возврат Ложь;
		КонецЕсли;
