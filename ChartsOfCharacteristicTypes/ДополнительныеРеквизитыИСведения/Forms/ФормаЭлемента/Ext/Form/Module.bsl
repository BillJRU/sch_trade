	...
	Если НЕ ПараметрыЗаписи.Свойство("ПодтвержденоИзменениеНаименования") Тогда
		Если ЗначениеЗаполнено(ТекущийЗаголовок) И ТекущийЗаголовок <> Объект.Заголовок Тогда
			ТекстВопроса = НСтр("ru='Наименование было изменено. Потребуется заново настроить отображение
                                      ...
                                      |в отборах.'
                                      |;uk='Найменування було змінено. Буде потрібно заново настроїти відображення
                                      |додаткового реквізиту в списках і звітах, а також використання
                                      |в відборах.'");
			ТекстВопроса = СтрЗаменить(ТекстВопроса, Символы.ПС, " ");
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru='Переименовать';uk='Перейменувати'"));
			Кнопки.Добавить("ВернутьНаименование", НСтр("ru='Отмена';uk='Відмінити'"));
...
Процедура ДополнительныеЗначенияСВесомПриИзменении(Элемент)
	...
	Иначе
		ТекстВопроса = НСтр("ru='Данные будут записаны.';uk='Дані будуть записані.'");
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ЭтоДополнительноеСведение = 1 Тогда
		Элементы.НеиспользуемыеРеквизиты.Заголовок = НСтр("ru='Неиспользуемые дополнительные сведения';uk='Не використовувані додаткові відомості'");
	Иначе
		Элементы.НеиспользуемыеРеквизиты.Заголовок = НСтр("ru='Неиспользуемые дополнительные реквизиты';uk='Не використовувані додаткові реквізити'");
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеСведения", НСтр("ru='Неиспользуемые дополнительные сведения';uk='Не використовувані додаткові відомості'"), Истина);
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеРеквизиты", НСтр("ru='Неиспользуемые дополнительные реквизиты';uk='Не використовувані додаткові реквізити'"), Истина);
...
Процедура ЗаписатьОбъект(ВариантТекстаВопроса, ОбработчикПродолжения, ДополнительныеПараметры = Неопределено, ПараметрыЗаписи = Неопределено)
	...
	Если ВариантТекстаВопроса = "ИзменениеПометкиУдаления" Тогда
		Если Модифицированность Тогда
			Если Объект.ПометкаУдаления Тогда
				ТекстВопроса = НСтр("ru='Для снятия пометки удаления необходимо записать внесенные изменения. Записать данные?';uk='Для зняття позначки вилучення необхідно записати внесені зміни. Записати дані?'");
			Иначе
				ТекстВопроса = НСтр("ru='Для установки пометки удаления необходимо записать внесенные изменения. Записать данные?';uk='Для установки позначки вилучення необхідно записати внесені зміни. Записати дані?'");
			КонецЕсли;
		Иначе
	...
	Если ВариантТекстаВопроса = "ПереходКСпискуЗначений" Тогда
		ТекстВопроса = НСтр("ru='Перед переходом к списку значений данные будут записаны.';uk='Перед переходом до списку значень дані будуть записані.'");
	Иначе
		ТекстВопроса = НСтр("ru='Данные будут записаны.';uk='Дані будуть записані.'")
	КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		...
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		    ...
		    ШаблонЗаголовкаСписка = НСтр("ru='Выберите дополнительное сведение для включения в набор ""%1""';uk='Виберіть додаткову відомість для включення до набору ""%1""'");
		Иначе
			Заголовок = НСтр("ru='Добавление дополнительного реквизита';uk='Додавання додаткового реквізиту'");
		    ШаблонЗаголовкаСписка = НСтр("ru='Выберите дополнительный реквизит для включения в набор ""%1""';uk='Виберіть додатковий реквізит для включення до набору ""%1""'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru='Добавление дополнительного сведения';uk='Додавання додаткової відомості'");
			ШаблонЗаголовкаПереключателя = НСтр("ru='Выберите вариант добавления дополнительного сведения ""%1"" в набор ""%2""';uk='Виберіть варіант додавання додаткової відомості ""%1"" до набору ""%2""'");
		Иначе
			Заголовок = НСтр("ru='Добавление дополнительного реквизита';uk='Додавання додаткового реквізиту'");
			ШаблонЗаголовкаПереключателя = НСтр("ru='Выберите вариант добавления дополнительного реквизита ""%1"" в набор ""%2""';uk='Виберіть варіант додавання додаткового реквізиту ""%1"" до набору ""%2""'");
		КонецЕсли;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
				   ...
				   И НЕ Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Недопустимо удалять тип ""%1"",
                                   ...
                                   |
                                   |Удаление отменено.'
                                   |;uk='Неприпустимо вилучати тип ""%1"",
                                   |так як додаткові значення вже введені.
                                   |Спочатку потрібно вилучити введені додаткові значення.
                                   |
                                   |Вилучення скасовано.'"),
	...
	Иначе
		...
		Если СписокНаборов.Количество() = 0 Тогда
			...
			Если Объект.ЭтоДополнительноеСведение Тогда
				ТекстКомментария = НСтр("ru='Сведение не входит в наборы';uk='Відомість не входить в набори'");
			Иначе
				ТекстКомментария = НСтр("ru='Реквизит не входит в наборы';uk='Реквізит не входить в набори'");
			КонецЕсли;
		...
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				ШаблонУточнения = НСтр("ru='Сведение входит в %1 %2';uk='Відомість входить в %1 %2'");
			Иначе
				ШаблонУточнения = НСтр("ru='Реквизит входит в %1 %2';uk='Реквізит входить в %1 %2'");
			КонецЕсли;
			
			СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
				"", НСтр("ru='набор,набора,наборов,,,,,,0';uk='набір,набори,наборів,,,,,,0'"));
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ru='Добавить сведение как есть (рекомендуется)
            ...
            |;uk='Додати відомості як є (рекомендується)
            |
            |У цьому випадку буде можливо відбирати по ньому дані різних типів в списках і звітах.'");
		СделатьПоОбразцу = НСтр("ru='Сделать копию сведения по образцу (с общим списком значений)
            ...
            |
            |Список значень цієї відомості буде спільним з вихідною відомістю.
            |За допомогою цього варіанта зручно виконувати централізовану настройку списку значень одразу для декількох однотипних відомостей.
		...
		Если РеквизитСоСпискомДополнительныхЗначений Тогда
			СоздатьКопированием = НСтр("ru='Сделать копию сведения
                ...
                |
                |Буде створено копію відомості та всіх її значень.'");
		Иначе
			СоздатьКопированием = НСтр("ru='Сделать копию сведения
                |
                ...
                |;uk='Зробити копію відомості
                |
                |Буде створено копію відомості.'");
		КонецЕсли;
	Иначе
		ДобавитьОбщий = НСтр("ru='Добавить реквизит как есть (рекомендуется)
            |
            |В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'
            |;uk='Додати реквізит як є (рекомендується)
            |
            |У цьому випадку буде можливо відбирати по ньому дані різних типів в списках і звітах.'");
		СделатьПоОбразцу = НСтр("ru='Сделать копию реквизита по образцу (с общим списком значений)
            ...
            |
            |Список значень цього реквізиту буде спільним з вихідним реквізитом.
            |За допомогою цього варіанта зручно виконувати централізовану настройку списку значень відразу для декількох однотипних реквізитів.
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ru='с условием';uk='з умовою'");
