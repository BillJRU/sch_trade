...
Процедура Сохранить(Команда)
	...
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru='Архив (*.zip)|*.zip';uk='Архів (*.zip)|*.zip'");
...
Процедура УпаковатьНастройкиВариантаОтчетаЗавершение(Файлы, ДополнительныеПараметры) Экспорт 
	...
	Если ТипЗнч(Файлы) = Тип("Массив")
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Вариант отчета сохранен в файл';uk='Варіант звіту збережений в файл'"),, ИмяФайла);
	КонецЕсли;
...
Функция ТекстПредложения()
	...
	Возврат НСтр("ru='Для сохранения варианта отчета в файл рекомендуется
        ...
        |;uk='Для збереження варіанту звіту в файл рекомендується
