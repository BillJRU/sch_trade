...
Процедура ЗачестьВстречныеРезервы()
	...
	Попытка
		...
		Для Каждого СтрТабл из ТаблицаВстречныхРезервов Цикл
			...
			Если СтрТабл.Организация <> ТекущаяОрганизация Тогда
				...
				КорректировкаРегистров.Комментарий = НСтр("ru='Создан автоматически при зачете встречных резервов товаров организаций.';uk='Створено автоматично при заліку зустрічних резервів товарів організацій.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Возникла ошибка зачете встречных резервов: %ПричинаОшибки%';uk='Виникла помилка заліку зустрічних резервів: %ПричинаОшибки%'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПричинаОшибки%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(НСтр("ru='Зачет встречных резервов товаров организаций';uk='Залік зустрічних резервів товарів організацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Пока ЕстьРегистраторыСторно Цикл
		...
		Если Не ВыборкаРегистратора.Следующий() Тогда
			...
			Если ЕстьЗаписиСторно() Тогда
				ТекстИсключения = НСтр("ru='Не удалось зачесть все сторно записи резервов товаров организаций. Подробнее - см. Журанал регистрации';uk='Не вдалося зарахувати всі сторно записи резервів товарів організацій. Детальніше - див. Журанал реєстрації'");
...
Функция СвернутьДвиженияПоРезервамТоваровОрганизацийПоТекущемуРегистраторуСторно(ТекущийРегистраторСторно)
	...
	Попытка
		...
		Для Каждого СтрокаСторно из ДвиженияРегистратораСторно Цикл
			...
			Если СтрокиРезервов.Количество() = 0 Тогда
				ТекстИсключения = НСтр("ru='Лишнее сторно';uk='Зайве сторно'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось свернуть резервы товаров организаций по регистратору сторно %ТекущийРегистраторСторно% по причине ""%ПричинаОшибки%"".';uk='Не вдалося згорнути резерви товарів організацій по реєстратору сторно %ТекущийРегистраторСторно% через ""%ПричинаОшибки%"".'");
...
Процедура ЗаписатьОшибкуСверткиВЖР(ТекстСообщения)
	...
	ИмяСобытия = НСтр("ru='Свертка движений по резервам товаров организаций';uk='Згортка рухів по резервах товарів організацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция РаспределитьРезервыВПериоде(ТекущийМесяц)
	...
	Попытка
		...
		Если ТаблицаНовыеРезервы.Количество() > 0 Тогда
			...
			КорректировкаРегистров.Комментарий = НСтр("ru='Создан автоматически при распределении резервов товаров организаций по периодам.';uk='Створено автоматично при розподілу резервів товарів організацій по періодах.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Если ЛишниеРезервы.Количество() > 0 Тогда
			...
			КорректировкаРегистров = Документы.КорректировкаРегистров.СоздатьДокумент();
			КорректировкаРегистров.Дата = ТекущийМесяц;
			КорректировкаРегистров.Комментарий = НСтр("ru='Создан автоматически при сторнированнии лишнего резерва.';uk='Створено автоматично при сторнуванню зайвого резерву.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Для Каждого СтрТабл Из ЛишниеРезервы Цикл
				...
				Если НераспределенныйЛишнийРезерв <> 0 Тогда
					ТекстИсключения = НСтр("ru='Ошибка при распределении лишних резервов. Обратитесь к разработчикам.';uk='Помилка при розподілі надлишкових резервів. Зверніться до розробників.'");
					ВызватьИсключение ТекстИсключения;
				КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Возникла ошибка при переносе резервов товаров организаций за период %ТекущийМесяц%: %ПричинаОшибки%';uk='Виникла помилка при перенесенні резервів товарів організацій за період %ТекущийМесяц%: %ПричинаОшибки%'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПричинаОшибки%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ИмяСобытияПереносаРезервов() Экспорт
	Возврат НСтр("ru='Перенос резервов товаров организаций';uk='Перенесення резервів товарів організацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ДоформироватьРезервыДляПокрытияЛишнихСторно()
	...
	Попытка
		...
		КорректировкаРегистров = Документы.КорректировкаРегистров.СоздатьДокумент();
		КорректировкаРегистров.Дата = ТекущаяДатаСеанса();
		КорректировкаРегистров.Комментарий = НСтр("ru='Создан автоматически при удалении лишних сторно-резервов.';uk='Створено автоматично при вилученні зайвих сторно-резервів.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
