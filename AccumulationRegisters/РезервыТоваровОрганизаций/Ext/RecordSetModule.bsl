...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Пока ВыборкаОшибок.Следующий() Цикл
		...
		Если ВыборкаОшибок.ТипОшибки = "ОшибкаБаланса" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: нарушен баланс по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі %Регистратор% в регістр РезервиТоваровОрганізацій: порушений баланс за вимірюваннями%АналитикаНоменклатуры%-%Организация%-%ВидЗапасов% -%НомерГТД%. Зверніться до розробників програми.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаНулевоеКоличество" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: в месяце %Месяц% записывается нулевое количество по измерениям: %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі%Регистратор% в регістр РезервиТоваровОрганізацій: в місяці %Месяц% записується нульове кількість за вимірюваннями:%АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Зверніться до розробників програми.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПринадлежностиВидовЗапасов" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: по измерениям записывается вид запасов, не принадлежащий организации %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі %Регистратор% в регістр РезервиТоваровОрганізацій: за вимірюваннями записується вид запасів, що не належить організації %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Зверніться до розробників програми.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПодобранногоВидаЗапасов" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: организация и кор. организация совпадают, но при этом тип запасов вида запасов не ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі %Регистратор% в регістр РезервиТоваровОрганізацій: організація і кор. організація збігаються, але при цьому тип запасів виду запасів не ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Зверніться до розробників програми.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПриходРезерваПоТоварНаХраненииСПравомПродажи" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: приход резерва по типу запасов ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі %Регистратор% в регістр РезервыТоваровОрганизаций: прихід резерву за типом запасів ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Зверніться до розробників програми.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаТипаНоменклатуры" Тогда
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: пишется резерв не по товару или многооборотной таре по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі% Реєстратор% в регістр РезервиТоваровОрганізацій: пишеться резерв не по товару або багатооборотна тарі за вимірюваннями %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Зверніться до розробників програми.'");
		Иначе
			ТекстИсключения = НСтр("ru='Ошибка в алгоритмах записи %Регистратор% в регистр РезервыТоваровОрганизаций: не классифицированная ошибка по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%';uk='Помилка в алгоритмах запису %Регистратор% в регістр РезервиТоваровОрганізацій: не класифікована помилка за вимірюваннями %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%'");
		КонецЕсли;
	...
	Пока ВыборкаПоРезервам.Следующий() Цикл
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(
			...
			Если НераспределенноеСторно <> 0 Тогда
				ТекстИсключения = НСтр("ru='Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: образуется сторно, которое не нужно для покрытия резервов по изменениям: %Организация% - %АналитикаНоменклатуры% - %ВидЗапасов% - %НомерГТД%, лишнее сторно %НераспределенноеСторно%. Обратитесь к разработчикам программы.';uk='Помилка в алгоритмах при записі %Регистратор% в регістр РезервиТоваровОрганізацій: утворюється сторно, яке не потрібно для покриття резервів щодо змін: %Организация%-%АналитикаНоменклатуры%-%ВидЗапасов% -%НомерГТД%, зайве сторно %НераспределенноеСторно%. Зверніться до розробників програми.'");
