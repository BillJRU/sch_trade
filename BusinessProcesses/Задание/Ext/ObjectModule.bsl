...
Функция РезультатВыполненияТочкиВыполнить(Знач ЗадачаСсылка)
	...
	СтрокаФормат = ?(Выполнено,
		"%1, %2 " + НСтр("ru='выполнил(а) задачу';uk='виконав(ла) задачу'") + ":
		           |%3
		           |",
		"%1, %2 " + НСтр("ru='отклонил(а) задачу';uk='відхилив(ла) задачу'") + ":
		           |%3
		           |");
...
Функция РезультатВыполненияТочкиПроверить(Знач ЗадачаСсылка)
	...
	Если НЕ Подтверждено Тогда
		СтрокаФормат = "%1, %2 " + НСтр("ru='вернул(а) задачу на доработку';uk='повернув(ла) задачу на доопрацювання'") + ":
			|%3
			|";
	Иначе
		СтрокаФормат = ?(Выполнено,
			"%1, %2 " + НСтр("ru='подтвердил(а) выполнение задачи';uk='підтвердив(ла) виконання задачі'") + ":
			           |%3
			           |",
			"%1, %2 " + НСтр("ru='подтвердил(а) отмену задачи';uk='підтвердив(ла) скасування задачі'") + ":
			           |%3
			           |");
	КонецЕсли;
