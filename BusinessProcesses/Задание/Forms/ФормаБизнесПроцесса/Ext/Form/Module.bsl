...
Процедура УстановитьСвойстваГруппеСостояний(Форма)
	...
	ИначеЕсли Форма.Объект.Стартован Тогда
		ТекстСостояния = ?(Форма.ИзменятьЗаданияЗаднимЧислом, 
			НСтр("ru='Изменения формулировки, важности, автора, а также перенос сроков исполнения и проверки задания вступят в силу немедленно для ранее выданной задачи.';uk='Зміни формулювання, важливості, автора, а також перенесення термінів виконання і перевірки завдання набудуть чинності негайно для раніше виданого завдання.'"), 
			НСтр("ru='Изменения формулировки, важности, автора, а также перенос сроков исполнения и проверки задания не будут отражены в ранее выданной задаче.';uk='Зміни формулювання, важливості, автора, а також перенесення термінів виконання і перевірки завдання не будуть відображені в раніше виданій задачі.'"));
		МассивСтрок.Добавить(ТекстСостояния);
		Высота = 2;
	...
	ИначеЕсли Форма.Отложен Тогда
		...
		ТекстСостояния = НСтр("ru='Задание будет запущено';uk='Завдання буде запущено'") + " ";
		МассивСтрок.Добавить(ТекстСостояния);
...
Процедура ПроверитьДатуЗавершенияОтложенногоПроцесса(ПроверяемыйОбъект, Отказ)
	...
	Если ПроверяемыйОбъект.СрокИсполнения < ДатаОтложенногоСтарта Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru='Срок исполнения задания не может быть меньше даты отложенного старта.';uk='Термін виконання завдання не може бути меншим дати відкладеного старту.'"),,
...
Функция КлючевыеРеквизитыФормыЗаполнены()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Исполнитель) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Поле ""Исполнитель"" не заполнено.';uk='Поле ""Виконавець"" не заповнено.'"),,
			"Исполнитель", "Объект.Исполнитель");
		РеквизитыФормыЗаполнены = Ложь;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Поле ""Задание"" не заполнено.';uk='Поле ""Завдання"" не заповнено.'"),,
			"Исполнитель", "Объект.Наименование");
		РеквизитыФормыЗаполнены = Ложь;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.СрокИсполнения) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Поле ""Срок"" исполнения не заполнено.';uk='Поле ""Тремін"" виконання не заповнено.'"),,
			"СрокИсполнения", "Объект.СрокИсполнения");
		РеквизитыФормыЗаполнены = Ложь;
	КонецЕсли;
