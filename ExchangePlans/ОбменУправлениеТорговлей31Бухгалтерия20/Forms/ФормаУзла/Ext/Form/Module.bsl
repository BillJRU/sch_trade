...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			ТекстВопроса = НСтр("ru='Изменены настройки узла ""%УзелИнформационнойБазы%""';uk='Змінені настройки вузла ""%УзелИнформационнойБазы%""'");
			ТекстВопроса = СтрЗаменить(ТекстВопроса, "%УзелИнформационнойБазы%", Строка(Объект.Наименование));
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Закрыть без сохранения изменений?';uk='Закрити без збереження змін?'");
