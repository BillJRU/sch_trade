...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ФормироватьДоговора() И Не ЗначениеЗаполнено(Объект.ПравилаСозданияДоговоровКонтрагентов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо заполнить правила формирования договоров!';uk='Необхідно заповнити правила формування договорів!'"), ,
	...
	Если Объект.ВариантНастройки = "ОбменКасса" И ЭтотОбъект.ИспользоватьНесколькоВидовЦен
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо заполнить вид цен!';uk='Необхідно заповнити вид цін!'"), ,
			"ВидЦен");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОбновитьНаименованиеКомандФормы()
	...
	НовыйЗаголовокРазделыУчета = НСтр("ru='Выбрать разделы учета';uk='Вибрати розділи обліку'");
