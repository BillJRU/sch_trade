...
Процедура ПереходАдресСайтаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Модифицированность Или Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru='Перед настройкой перехода на сайт существующую настройку необходимо записать, продолжить?';uk='Перед настройкою переходу на сайт існуючу настройку необхідно записати, продовжити?'");
...
Процедура СформироватьЗаголовокФормы()
	...
	Если ПереключательНазначениеОбмена = 1 Тогда
		...
		ТекстЗаголовка = НСтр("ru='Обмен через %1';uk='Обмін через %1'");
		Каталог = ОпределитьКаталогВыгрузки();
		Если ЗначениеЗаполнено(Каталог) Тогда
			ШаблонЗаголовка = НСтр("ru='Обмен через %1';uk='Обмін через %1'");
			ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, Каталог);
		Иначе
			
			ТекстЗаголовка = НСтр("ru='Обмен с сайтом';uk='Обмін з сайтом'");
		КонецЕсли;
		
	Иначе
		
		ШаблонЗаголовка = НСтр("ru='Обмен с %1';uk='Обмін з %1'");
		ИмяСайта = ИмяСайта();
...
Функция ПереходНаСайтЗаполнен(АдресПереходаНаСайт)
	...
	Если АдресПереходаНаСайт = НСтр("ru='Настроить переход на сайт';uk='Настроїти перехід на сайт'")
		Или АдресПереходаНаСайт = НСтр("ru='Настроить переход в админзону';uk='Настроїти перехід в адмінзону'") Тогда
...
Процедура ОбновитьПереходыНаСайт()
	...
	Если ЗначениеЗаполнено(Объект.ДанныеСайта) Тогда
		...
		Иначе
			ПереходАдресСайта = НСтр("ru='Настроить переход на сайт';uk='Настроїти перехід на сайт'");
		...
		Иначе
			ПереходURLАдминЗоны = НСтр("ru='Настроить переход в админзону';uk='Настроїти перехід в адмінзону'");
			ВидимостьДекорацияНастройка = Ложь;
		КонецЕсли;
		...
		Если ТипЗнч(ПараметрыПрикладногоРешения) = Тип("Структура") Тогда
			Для Каждого КлючЗначение Из ПараметрыПрикладногоРешения Цикл
				Попытка
					...
					КонецЕсли;
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка параметров обмена с сайтами';uk='Помилка параметрів обміну з сайтами'",ОбщегоНазначения.КодОсновногоЯзыка()),
