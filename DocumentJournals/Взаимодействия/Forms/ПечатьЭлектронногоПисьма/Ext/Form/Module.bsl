...
Процедура ЗаполнитьВложенияИСФормироватьТекстHTMLНаОснованииПисьмаВложения(ПисьмоВложение)
	...
	Если ПочтовоеСообщение.СтатусРазбора = СтатусРазбораИнтернетПочтовогоСообщения.ОбнаруженыОшибки Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось разобрать почтовое сообщение.';uk='Не вдалося розібрати поштове повідомлення.'"));
		Возврат;
	КонецЕсли;
...
Процедура СформироватьСтрокуПисьмаОснования();
	...
	ЗаголовокСтроки = Новый ФорматированнаяСтрока(НСтр("ru='Вложение к письму:';uk='Вкладення до листа:'"));
