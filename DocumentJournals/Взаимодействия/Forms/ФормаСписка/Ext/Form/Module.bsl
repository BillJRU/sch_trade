...
Процедура ПереслатьКакВложение(Команда)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Пересылать как вложения можно только отправленные и полученные письма.';uk='Пересилати як вкладення можна тільки відправлені і отримані листи.'");
	...
	Для Каждого Строка Из РодительскаяСтрока.Строки Цикл
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Строка.Значение) Тогда
			...
			НоваяСтрока.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 от %2 %3';uk='%1 від %2 %3'"),
				?(ПустаяСтрока(СтрокаДетали.Тема),НСтр("ru='Тема не указана';uk='Тема не вказана'"), СтрокаДетали.Тема),
