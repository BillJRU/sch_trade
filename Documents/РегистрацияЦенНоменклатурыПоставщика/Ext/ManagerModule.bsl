...
Процедура ЗаполнитьРегистрациюЦенПоДокументуЗакупки(Основание, ДокументОбъект) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Товары.ВидЦеныПоставщика.ЦенаВключаетНДС",	"&ЦенаВидаЦенВключаетНДС");
		...
		Иначе
			РеквизитыВидаЦеныПоставщика = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидЦеныПоставщика,
				"Валюта, ЦенаВключаетНДС");
			
			Валюта 			= РеквизитыВидаЦеныПоставщика.Валюта;
			ЦенаВключаетНДС = РеквизитыВидаЦеныПоставщика.ЦенаВключаетНДС;
		КонецЕсли;
		
		Запрос.УстановитьПараметр("ВалютаВидаЦен", 			Валюта);
		Запрос.УстановитьПараметр("ВидЦеныПоставщика", 		ВидЦеныПоставщика);
		Запрос.УстановитьПараметр("ЦенаВидаЦенВключаетНДС", ЦенаВключаетНДС);
...
Процедура ЗаполнитьПоПоступлениюТоваров(Основание, ДокументОбъект) Экспорт
	...
	|				ТОГДА ВЫБОР КОГДА ТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|								ТОГДА 1
