...
&НаКлиенте
Процедура РасходыНалоговоеНазначениеБезРазбиенияПриИзменении(Элемент)
	ИмяТЧ = "ПрочиеРасходы";
	ИмяРеквизита = "НалоговоеНазначение";
	ЗаполнитьРеквизитВТабличнойЧасти(ИмяТЧ, ИмяРеквизита, Элементы[ИмяТЧ].ТекущиеДанные[ИмяРеквизита]);
КонецПроцедуры
...
Процедура ПрочиеДоходыСуммаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сумма) Тогда
		...
		ТекущиеДанные.СуммаРегл = ТекущиеДанные.Сумма * КоэффициентПересчетаВВалютуРегл;
		ТекущиеДанные.СуммаУпрБезНДС = ТекущиеДанные.СуммаУпр;
		ТекущиеДанные.СуммаРеглБезНДС = ТекущиеДанные.СуммаРегл;
...
Процедура ЗаполнитьСтатьюАктивовПассивов(Команда)
	...
	ДоходыИРасходыКлиент.ЗаполнитьСтатьюИАналитикуВВыделенныхСтроках(
		ЭтотОбъект, Элементы[ИмяТекущейТаблицы + "СтатьяАктивовПассивов"], Элементы[ИмяТекущейТаблицы].ВыделенныеСтроки);
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		Если ИмяТЧ = "ПрочиеРасходы" Тогда
			Если ПереключательРасшифровкиРасходы = 0 Тогда
				...
				Если ИсходнаяСтрока <> Неопределено Тогда
					ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ИсходнаяСтрока,
						"Подразделение, НаправлениеДеятельности, НалоговоеНазначение, СтатьяРасходов, АналитикаРасходов,
...
Процедура ЗаполнитьРеквизитыШапки()
	...
	Если Не ЭтоРасходы Тогда
		...
		|	СУММА(Доходы.СуммаРеглБезНДС) КАК СуммаРеглБезНДС,
		|	СУММА(Доходы.СуммаУпр) КАК СуммаУпр,
		|	СУММА(Доходы.СуммаУпрБезНДС) КАК СуммаУпрБезНДС
		|
		|ИЗ
...
Процедура УправлениеФормойСервер()
	...
	РасходыСуммаЗаголовокПолный = СтрШаблон(НСтр("ru='Суммы в упр. учете (%1):';uk='Суми в упр. обліку (%1):'"), ВалютаУправленческогоУчета);
	Элементы.РасходыСумма.Заголовок = ?(ИспользоватьУчетНДС, НСтр("ru='С НДС';uk='З ПДВ'"), РасходыСуммаЗаголовокПолный);
	
	Элементы.ДекорацияРасходыСуммыУпр.Заголовок =
		СтрШаблон(НСтр("ru='Суммы расходов в упр. учете и упр. учете организаций (%1):';uk='Суми витрат у упр. обліку та упр. обліку організацій (%1):'"), ВалютаУпр);
	Элементы.ДекорацияРасходыСуммыУпр.Видимость = ИспользоватьУчетНДС;
	
	Элементы.ДекорацияРасходыСуммыРегл.Заголовок =
		СтрШаблон(НСтр("ru='Суммы расходов в регламентированном учете (%1):';uk='Суми витрат в регламентованому обліку (%1):'"), ВалютаРегламентированногоУчета);
		
	ПрочиеРасходыСуммаШаблон = ?(ИспользоватьУчетНДС, НСтр("ru='Упр. учет с НДС (%1)';uk='Упр. облік з ПДВ (%1)'"), НСтр("ru='Упр. учет (%1)';uk='Упр. облік (%1)'"));
	...
	Элементы.ПрочиеРасходыСуммаБезНДС.Заголовок =
		СтрШаблон(НСтр("ru='Упр. учет без НДС (%1)';uk='Упр. облік без ПДВ (%1)'"), ВалютаУпр);
	Элементы.ПрочиеРасходыСуммаРегл.Заголовок =
		СтрШаблон(НСтр("ru='Регл. учет без НДС (%1):';uk='Регл. облік без ПДВ (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ПрочиеРасходыНДСРегл.Заголовок =
		СтрШаблон(НСтр("ru='НДС регл. (%1):';uk='ПДВ регл. (%1):'"), ВалютаРегламентированногоУчета);
		
	// Закладка "Доходы".	
	Элементы.ДекорацияДоходыСуммыУпр.Заголовок =
		СтрШаблон(НСтр("ru='Суммы доходов в управленческим учете (%1):';uk='Суми доходів в управлінським обліку (%1):'"), ВалютаУпр);
	Элементы.ДекорацияДоходыСуммыРегл.Заголовок =
		СтрШаблон(НСтр("ru='Суммы доходов в регламентированном учете (%1):';uk='Суми доходів в регламентованому обліку (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ДекорацияДоходыСуммыУпрМУ.Заголовок =
		СтрШаблон(НСтр("ru='Суммы доходов в упр. учете организаций (%1):';uk='Суми доходів в упр. обліку організацій (%1):'"), ВалютаУпр);
		
	Элементы.ПрочиеДоходыСумма.Заголовок =
		СтрШаблон(НСтр("ru='Упр. учет с НДС (%1)';uk='Упр. облік з ПДВ (%1)'"), ВалютаУпр);
	Элементы.ПрочиеДоходыСуммаУпр.Заголовок =
		СтрШаблон(НСтр("ru='Упр. учет организаций (%1)';uk='Упр. облік організацій (%1)'"), ВалютаУпр);
	Элементы.ПрочиеДоходыСуммаУпрБезНДС.Заголовок =
		СтрШаблон(НСтр("ru='Упр. учет организаций без НДС (%1)';uk='Упр. облік організацій без ПДВ (%1)'"), ВалютаУпр);
	Элементы.ПрочиеДоходыСуммаРегл.Заголовок =
		СтрШаблон(НСтр("ru='Бух. учет (%1):';uk='Бух. облік (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ПрочиеДоходыСуммаРеглБезНДС.Заголовок =
		СтрШаблон(НСтр("ru='Бух. учет без НДС (%1):';uk='Бух. облік без ПДВ (%1):'"), ВалютаРегламентированногоУчета);
...
Процедура УправлениеЭлементамиФормыНаКлиенте()
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаПрочихРасходовМеждуФилиалами") Тогда
		Элементы.Организация.Заголовок = НСтр("ru='Организация-отправитель';uk='Організація-відправник'");
	Иначе
		Элементы.Организация.Заголовок = НСтр("ru='Организация';uk='Організація'");
	КонецЕсли;
	...
	Если ЭтоРасходы Тогда
		...
		Элементы.ПереключательРасшифровкиРасходыКорЧасть.Заголовок =
			?(РеклассификацияРасходов, НСтр("ru='Кор. расходы';uk='Кор. витрати'"), НСтр("ru='Активы/пассивы';uk='Активи/пасиви'"));
		...
		Элементы.РасходыНалоговоеНазначениеБезРазбиения.Видимость = РасходыБезРазбиения;
		...
		Элементы.ПрочиеРасходыНалоговоеНазначение.Видимость = НЕ РасходыБезРазбиения;
	...
	Иначе
		...
		Элементы.ПереключательРасшифровкиДоходыКорЧасть.Заголовок =
			?(РеклассификацияДоходов, НСтр("ru='Кор. доходы';uk='Кор. доходи'"), НСтр("ru='Активы/пассивы';uk='Активи/пасиви'"));
...
Процедура УстановитьПодсказкиВводаРасходов()
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаПрочихРасходовМеждуФилиалами") Тогда
		Элементы.КорСтатьяРасходов.ПодсказкаВвода					= НСтр("ru='<Совпадает со статьей расходов>';uk='<Збігається зі статтею витрат>'");
		Элементы.КорСтатьяРасходовБезРазбиения.ПодсказкаВвода		= НСтр("ru='<Совпадает со статьей расходов>';uk='<Збігається зі статтею витрат>'");
	Иначе
...
Процедура ПриИзмененииПереключателяРасшифровкиРасходы(ЗначениеПереключателя)
	Если ПереключательРасшифровкиРасходы = 0
	 ИЛИ ПереключательРасшифровкиРасходыКорЧасть = 0 Тогда
		...
		Если ПустаяСтрока(ПредставлениеОтраженияВРеглУчете) Тогда
			ПредставлениеОтраженияВРеглУчете = НСтр("ru='Настроить';uk='Настроїти'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиРасходы = 0
	 ...
	 И Объект.ПрочиеРасходы.Количество() > 1 Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в табличной части введено более одной строки!';uk='Перемикання в режим без розбиття неможливо, якщо в табличній частині введено більше одного рядка!'");
	...
	Если ПереключательРасшифровкиРасходы = 0
	 ...
	 И НЕ РасходыВСтрокахОдинаковые()Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные расходы!';uk='Перемикання в режим без розбиття неможливо, якщо в рядках табличної частини вказані різні витрати!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиРасходы = 1
	 ...
	 И НЕ КорЧастьВСтрокахОдинаковая("ПрочиеРасходы") Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные активы/пассивы!';uk='Перемикання в режим без розбиття неможливо, якщо в рядках табличної частини вказані різні активи/пасиви!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиРасходы = 1
	 ...
	 И НЕ КорРасходыВСтрокахОдинаковые() Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные кор. расходы!';uk='Перемикання в режим без розбиття неможливо, якщо в рядках табличної частини вказані різні кор. витрати!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
...
Функция РасходыВСтрокахОдинаковые()
	...
	Таблица.Свернуть("Подразделение, НаправлениеДеятельности, НалоговоеНазначение, СтатьяРасходов, АналитикаРасходов, ДатаОтражения");
...
Процедура ПриИзмененииПереключателяРасшифровкиДоходы(ЗначениеПереключателя)
	...
	Если ПереключательРасшифровкиДоходы = 0
	 ...
	 И НЕ ДоходыВСтрокахОдинаковые()Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные доходы!';uk='Перемикання в режим без розбиття неможливо, якщо в рядках табличної частини вказані різні доходи!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиДоходы = 1
	 ...
	 И НЕ КорДоходыВСтрокахОдинаковые() Тогда
		ТекстСообщения = НСтр("ru='Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные кор. доходы!';uk='Перемикання в режим без розбиття неможливо, якщо в рядках табличної частини вказані різні кор. доходи!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
