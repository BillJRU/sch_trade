...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Заказ на перемещение';uk='Замовлення на переміщення'",КодОсновногоЯзыка);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаказНаПеремещение") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Заказ на перемещение';uk='Замовлення на переміщення'"),
 			СформироватьПечатнуюФормуЗаказНаПеремещение(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина);
...
Функция СформироватьПечатнуюФормуЗаказНаПеремещение(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	ШаблонОшибкиТовары = НСтр("ru='В документе %1 отсутствуют товары. Печать заказа на перемещение не требуется';uk='У документі %1 відсутні товари. Друк замовлення на переміщення не потрібний'");
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Заказы на перемещение';uk='Замовлення на переміщення'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.БлокируемыеОбъекты = "Документ.ЗаказНаПеремещение,"
		+ "Справочник.НаправленияДеятельности";
	Обработчик.Комментарий = НСтр("ru='Заполняет реквизиты ""Использовать длительность"", ""Назначение"" и ""Вариант приемки"".
                                   ...
                                   |;uk='Заповнює реквізити ""Використовувати тривалість"", ""Призначення"" та ""Варіант приймання"".
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ЗаказНаПеремещение";
	НоваяСтрока.Порядок    = "После";
	НоваяСтрока = Исключения.Добавить();
...
#Область ОбработатьДанныеДляГенерацииНазначений
	...
	Обработчик.Комментарий = НСтр("ru='Создает новые назначения связанные с документом ""Заказ на перемещение""';uk='Створює нові призначення, пов''язані з документом ""Замовлення на переміщення""'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Операция.Ссылка
	|ИЗ
	|	Документ.ЗаказНаПеремещение КАК Операция
	|ГДЕ
	...
	|ИЗ
	|	Документ.ЗаказНаПеремещение КАК Операция
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПеремещение.Товары КАК Строки
	|		ПО Операция.Ссылка = Строки.Ссылка
	|ГДЕ
	|	НЕ Операция.ПометкаУдаления
	|	И Операция.Проведен
	|	И НЕ Операция.ИспользоватьДлительностьПеремещения
	|	И Строки.НачалоОтгрузки <> Строки.ОкончаниеПоступления
	|	И Операция.ДлительностьПеремещения = 0";
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				Если НЕ Объект.ИспользоватьДлительностьПеремещения Тогда
					Если Объект.ДлительностьПеремещения <> 0 Тогда
						Объект.ИспользоватьДлительностьПеремещения = Истина;				
						ОбъектИзменен = Истина;
					КонецЕсли;	
					Для Каждого СтрокаТовары Из Объект.Товары Цикл
						Если СтрокаТовары.НачалоОтгрузки <> СтрокаТовары.ОкончаниеПоступления И Объект.ДлительностьПеремещения = 0 Тогда
							Объект.ИспользоватьДлительностьПеремещения = Истина;				
							ОбъектИзменен = Истина;
							Прервать;
						КонецЕсли;	
