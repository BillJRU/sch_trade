...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	Реквизиты = Новый Структура("ХозяйственнаяОперация, Организация, ОрганизацияПолучатель, 
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.СкладОтправитель) Тогда
		ШаблонСтроки = НСтр("ru='Перемещение с ""%1""';uk='Переміщення з ""%1""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если ЗначениеЗаполнено(Реквизиты.СкладПолучатель) Тогда
		ШаблонСтроки = НСтр("ru='Перемещение на ""%1""';uk='Переміщення на ""%1""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяНаПеремещение") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Накладная на перемещение';uk='Накладна на переміщення'"),
		  	СформироватьПечатнуюФормуНакладнойНаПеремещениеТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			,
			,
			Истина
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СертификатыНоменклатурыРеестр") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Справочники.СертификатыНоменклатуры.СформироватьПечатнуюФормуРеестрСертификатовНоменклатуры(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			СформироватьПечатнуюФормуНакладнойНаПеремещениеТоваров(КомплектПечати.Объекты, ОбъектыПечати, ПараметрыВывода),
			,
			,
			,
			Истина
...
//
Функция СформироватьПечатнуюФормуНакладнойНаПеремещениеТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Для Каждого ИспользованиеСерий Из ДеревоОбъектов.Строки Цикл
		...
		Для Каждого Шапка Из ИспользованиеСерий.Строки Цикл
			...
			Если Шапка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами Тогда
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru='Перемещения товаров к оформлению';uk='Переміщення товарів до оформлення'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет назначение в аналитике учета номенклатуры. 
                                   |Перезаполняет реквизит ""СтатусУказанияСерийОтправитель"".
                                   ...
                                   |Перезаповнює реквізит ""СтатусУказанияСерийОтправитель"".
...
#Область СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "Документы.ПеремещениеТоваров.СгенерироватьКлючиАналитикиНоменклатуры";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ПеремещениеТоваров.ЗарегистрироватьДанныеКОбновлениюКлючейАналитики";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("13eec474-fce7-414a-9ab5-e16654f9023d");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ПеремещениеТоваров";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПеремещениеТоваровВидыЗапасов.Ссылка
	|ИЗ
	|	Документ.ПеремещениеТоваров.ВидыЗапасов КАК ПеремещениеТоваровВидыЗапасов
	|ГДЕ
	|	(ПеремещениеТоваровВидыЗапасов.ВидЗапасовПолучателя.РеализацияЗапасовДругойОрганизации
	|			ИЛИ ПеремещениеТоваровВидыЗапасов.ВидЗапасовПолучателя = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметрыВыборки = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДополнительныеПараметрыВыборки.ДополнительныеИсточникиДанных.Вставить("ВидыЗапасов.ВидЗапасовПолучателя");	
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТДокументыДляОбработки.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТДокументыДляПерезаполненияВидаЗапасовПолучателя
	|ИЗ
	|	ВТДокументыДляОбработки КАК ВТДокументыДляОбработки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.ВидыЗапасов КАК ПеремещениеТоваровВидыЗапасов
	|		ПО ВТДокументыДляОбработки.Ссылка = ПеремещениеТоваровВидыЗапасов.Ссылка
	|ГДЕ
	|	(ПеремещениеТоваровВидыЗапасов.ВидЗапасовПолучателя.РеализацияЗапасовДругойОрганизации
	|			ИЛИ ПеремещениеТоваровВидыЗапасов.ВидЗапасовПолучателя = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
	|;
	...
	|	МАКСИМУМ(ОбъектыДляОбработки.Проведен) КАК Проведен,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЕСТЬNULL(Товары.АналитикаУчетаНоменклатуры.Назначение, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) <> ЕСТЬNULL(Товары.НазначениеОтправителя, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) 
	|					И ЕСТЬNULL(Товары.НазначениеОтправителя, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК НужноПеренестиНазначение,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЕСТЬNULL(Товары.СтатусУказанияСерий, 0) = 6
	|					И ЕСТЬNULL(Товары.СтатусУказанияСерийОтправитель, 0) = 5
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК НужноПересчитатьСтатусУказанияСерий,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА ОбъектыДляОбработки.ВариантПриемкиТоваров <> ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ) КАК ТребуетсяУказаниеВариантаПриемки,
	|	МАКСИМУМ(ОбъектыДляОбработки.АдресДоставки <> """"
	...
	|			И НЕ ОбъектыДляОбработки.АдресДоставкиПеревозчикаЗначенияПолей ПОДОБНО ""%</КонтактнаяИнформация>"") КАК ТребуетсяПересчетЗначенийПолейАдреса,
	|	МАКСИМУМ(НЕ ВТДокументыДляПерезаполненияВидаЗапасовПолучателя.Ссылка ЕСТЬ NULL) КАК ТребуетсяПерезаполнениеВидаЗапасовПолучателя
	|ИЗ
	|	ТаблицаСсылок КАК ОбъектыДляОбработки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.Товары КАК Товары
	|		ПО Товары.Ссылка = ОбъектыДляОбработки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДокументыДляПерезаполненияВидаЗапасовПолучателя КАК ВТДокументыДляПерезаполненияВидаЗапасовПолучателя
	|		ПО ОбъектыДляОбработки.Ссылка = ВТДокументыДляПерезаполненияВидаЗапасовПолучателя.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Выборка.ТребуетсяПерезаполнениеВидаЗапасовПолучателя Тогда
				ДокументОбъект.ЗаполнитьВидЗапасовПолучателя();
				ОбъектИзменен = Истина;
			КонецЕсли;
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоРегистратору", " ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.ПеремещениеТоваров)");
...
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПеремещениеТоваров";
	МетаданныеДокумента = Метаданные.Документы.ПеремещениеТоваров;
