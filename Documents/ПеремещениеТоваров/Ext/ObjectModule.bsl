...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДополнительныеСвойства.Вставить("ПараметрыЗаполненияВидовЗапасов", ПараметрыЗаполненияВидовЗапасов("ОрганизацияПолучатель"));
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(ПараметрыЗаполненения1, ПараметрыЗаполненения2) Тогда 
		...
		ДополнительныеСвойства.Вставить("ПараметрыЗаполненияВидовЗапасов", ПараметрыЗаполненения1);
	Иначе
		ДополнительныеСвойства.Вставить("ПараметрыЗаполненияВидовЗапасов", Новый Массив(2));
		ДополнительныеСвойства.ПараметрыЗаполненияВидовЗапасов[0] = ПараметрыЗаполненения1;
		ДополнительныеСвойства.ПараметрыЗаполненияВидовЗапасов[1] = ПараметрыЗаполненения2;
	КонецЕсли;
...
Процедура ЗаполнитьПоПриобретениюТоваров(ДанныеЗаполнения)
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОснованиеПеремещения", "Документ.ПриемкаТоваровНаХранение");
	КонецЕсли;
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	ИменаРеквизитов = "Организация, ОрганизацияПолучатель, Дата, СкладПолучатель, СкладОтправитель, ХозяйственнаяОперация, Статус";
