...
Процедура ЗаполнитьСписокЯчеекПоОтбору(Команда)
	Если Ячейки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСписокЯчеекПоОтборуЗавершение", ЭтотОбъект), НСтр("ru='Перед заполнением список ячеек будет очищен. Продолжить?';uk='Перед заповненням список комірок буде очищений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
