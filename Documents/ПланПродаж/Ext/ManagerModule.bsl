...
#Область ФактыПродаж
	...
	ДопПараметр.Вставить("Тип", 				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ДопПараметр.Вставить("Путь", 				ПутьКРеквизиту);
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором, НСтр("ru='Факт продаж*';uk='Факт продажів*'"), НСтр("ru='Факт продаж';uk='Факт продажів'")));
	ДопПараметр.Вставить("СоздаватьРеквизит", 	ЧтениеФактаПродаж);
	ДопПараметр.Вставить("УдалятьРеквизитыТаблицы", Истина);
	
	// Общие свойства
	ДопПараметр.Вставить("СоздаватьОперанд",	ЧтениеФактаПродаж);
	ДопПараметр.Вставить("ЗаголовокОперанда",	?(СоздаватьРеквизитСОтбором, НСтр("ru='Факт продаж*';uk='Факт продажів*'"), НСтр("ru='Факт продаж';uk='Факт продажів'")));
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f0b673b9-54ba-4ca1-920f-8147da7f1485");
...
|;uk='Виконує заміщення планів продажів: 
...
|- в планах у статусі ""Скасовано"" знімається прапор ""Заміщено""'");
	...
	Изменяемые.Добавить(Метаданные.Документы.ПланПродаж.ПолноеИмя());
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РегистрыНакопления.ПланыОплатКлиентов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";
...
Функция ТекстЗапросаДанныеКОбработке(ВыбратьДляРегистрацииДвиженийОборотыДвижений = Ложь) Экспорт
	...
	|	ПланыПродаж.Статус КАК СтатусЗамещения
	...
	|			И (ПланыПродаж.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.НаУтверждении)
	|					И ПланыПродаж1.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден)
	|				ИЛИ ПланыПродаж.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден)
	|					И ПланыПродаж1.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден))
	...
	|ПОМЕСТИТЬ ПланыПродажКОбработке
	|ИЗ
	...
	|			И ЗамещающиеИЗамещенныеПланыПродаж.СтатусЗамещения = ЗамещениеПланов.СтатусЗамещения
	|ГДЕ
	...
	|		ПО (ЗамещениеПланов.ЗамещенныйПлан = ПланыПродаж.Ссылка)
	|ГДЕ
	|	ПланыПродаж.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Отменен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПланыПродажКОбработке.Ссылка КАК Ссылка
	|ИЗ
	|	ПланыПродажКОбработке КАК ПланыПродажКОбработке
	|ГДЕ
	|	&УсловиеОтбораДляРегистрацииДвиженийОборотыБюджетов";
	
	Если ВыбратьДляРегистрацииДвиженийОборотыДвижений Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораДляРегистрацииДвиженийОборотыБюджетов", "ПланыПродажКОбработке.Ссылка.ОтражаетсяВБюджетировании");
	Иначе
