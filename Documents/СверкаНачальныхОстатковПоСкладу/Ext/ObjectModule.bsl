...
Процедура КонтрольКорректностиРеквизитовДокумента(Отказ)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьУчетСебестоимости") Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru='Документ можно вводить только при включенной настройке ""Использовать учет себестоимости""';uk='Документ можна вводити тільки при включеній настройці ""Використовувати облік собівартості""'"),
			Ссылка,
			"Дата",
			,
			Отказ);
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ДатаУчета) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru='Документ можно вводить только при установленной константе ""Дата начала учета себестоимости""';uk='Документ можна вводити тільки при встановленій константі ""Дата початку обліку собівартості""'"),
				Ссылка,
				"Дата",
				,
				Отказ);
		...
		ИначеЕсли КонецМесяца(Дата) + 1 <> ДатаУчета Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru='Документ можно вводить только в месяце, предшествующем месяцу константы ""Дата начала учета себестоимости""';uk='Документ можна вводити тільки в місяці, що передує місяцю константи ""Дата початку обліку собівартості""'"),
				Ссылка,
				"Дата",
				,
				Отказ);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru='По указанным организации и складу существуют другие документы сверки начальных остатков по складу';uk='По вказаних організації та складу існують інші документи звірки початкових залишків по складу'"),
			Ссылка,
			"Склад",
			,
			Отказ);
