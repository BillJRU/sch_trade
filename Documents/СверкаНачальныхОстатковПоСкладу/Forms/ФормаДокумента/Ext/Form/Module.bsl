...
Процедура ТекстДокументыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Модифицированность Тогда
		...
		ПоказатьВопрос(
			Оповещение,
			НСтр("ru='Необходимо сохранить документ для выполнения операции. Сохранить документ?';uk='Необхідно зберегти документ для виконання операції. Зберегти документ?'"),
...
Процедура ОбновитьЭлементыФормы()
	...
	Если Объект.КоличествоИзОУ Тогда
		Элементы.РезультатыСверкиКоличествоУпаковок.Заголовок = НСтр("ru='Начальное количество';uk='Початкова кількість'");
	...
	Иначе
		Элементы.РезультатыСверкиВнешнееКоличество.Заголовок  = НСтр("ru='Начальное количество';uk='Початкова кількість'");
...
Процедура ОбновитьТекстСформированныеДокументы()
	...
	Если НЕ Объект.Проведен Тогда
		ТекстДокументы = Новый ФорматированнаяСтрока(НСтр("ru='Для отражения результатов сверки в учете документ сверки должен быть проведен.';uk='Для відображення результатів звірки в обліку документ звірки повинен бути проведений.'"));
		Возврат;
	КонецЕсли;
	...
	Если Объект.ТребуетсяПерезаполнитьРезультаты Тогда
		...
		ДобавитьГиперссылкуДокументаТекстаСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Необходимо обновить результаты сверки и сформировать (обновить) документы корректировки остатков';uk='Необхідно оновити результати звірки і сформувати (оновити) документи коригування залишків'"),
	...
	ИначеЕсли НЕ СтруктураДокументы.ВводОстатковСебестоимости.ТребуетсяДокумент
	 ...
	 И НЕ СтруктураДокументы.СписаниеПосле.ТребуетсяДокумент Тогда
		...
		ДобавитьТекстИнформацияСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Нет данных для формирования документов';uk='Немає даних для формування документів'"));
	...
	ИначеЕсли (СтруктураДокументы.ВводОстатковСебестоимости.ТребуетсяДокумент И СтруктураДокументы.ВводОстатковСебестоимости.КоличествоДокументов = 0)
	 ...
	 ИЛИ (НЕ СтруктураДокументы.СписаниеПосле.ТребуетсяДокумент И СтруктураДокументы.СписаниеПосле.КоличествоДокументов <> 0) Тогда
		...
		ДобавитьГиперссылкуДокументаТекстаСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Необходимо сформировать (обновить) документы корректировки остатков';uk='Необхідно сформувати (оновити) документи коригування залишків'"),
			"ПакетноеФормированиеДокументов");
		
	Иначе
		
		ДобавитьЗаголовокТекстаСформированныеДокументы(МассивСтрок, НСтр("ru='Документ ввода остатков';uk='Документ введення залишків'"));
		...
		Если НЕ СтруктураДокументы.ВводОстатковСебестоимости.ТребуетсяДокумент
		 И НЕ СтруктураДокументы.ВводОстатковКоличества.ТребуетсяДокумент Тогда
			...
			ДобавитьТекстИнформацияСформированныеДокументы(
				МассивСтрок,
				НСтр("ru='Не требуются';uk='Не потрібні'"));
		...
		Иначе
			...
			ДобавитьРазделТекстаСформированныеДокументы(
				...
				СтруктураДокументы,
				"ВводОстатковСебестоимости",
				НСтр("ru='документ ввода остатков по себестоимости';uk='документ введення залишків по собівартості'"),
				Истина);
			...
			ДобавитьРазделТекстаСформированныеДокументы(
				...
				СтруктураДокументы,
				"ВводОстатковКоличества",
				НСтр("ru='документ корректировки остатков по количеству';uk='документ коригування залишків по кількості'"));
			
		КонецЕсли;
		
		ДобавитьЗаголовокТекстаСформированныеДокументы(МассивСтрок, НСтр("ru='Документы оприходования излишков товаров';uk='Документи оприбуткування надлишків товарів'"));
	 	...
	 	Иначе
			...
			ДобавитьРазделТекстаСформированныеДокументы(
				...
				СтруктураДокументы,
				"ОприходованиеПосле",
				НСтр("ru='документ оприходования';uk='документ оприбуткування'"));
			
	 	КонецЕсли;
		
		ДобавитьЗаголовокТекстаСформированныеДокументы(МассивСтрок, НСтр("ru='Документы списания недостач товаров';uk='Документи списання нестач товарів'"));
	 	...
	 	Иначе
			...
			ДобавитьРазделТекстаСформированныеДокументы(
				...
				СтруктураДокументы,
				"СписаниеПосле",
				НСтр("ru='документ списания';uk='документ списання'"));
		...
		ДобавитьГиперссылкуДокументаТекстаСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Обновить список сформированных документов';uk='Оновити список сформованих документів'"),
			"ОбновитьИнформацию");
		
		ДобавитьГиперссылкуДокументаТекстаСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Перезаполнить все сформированные документы';uk='Перезаповнити всі сформовані документи'"),
			"ПакетноеФормированиеДокументов");
...
Процедура ДобавитьРазделТекстаСформированныеДокументы(МассивСтрок, СтруктураДокументы, ТипДокумента, ЗаголовокДокумента, ДобавлятьТекстНеТребуется = Ложь)
	...
	ИначеЕсли СтруктураДокументы[ТипДокумента].КоличествоДокументов = 0 Тогда
		...
		ДобавитьГиперссылкуДокументаТекстаСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Создать';uk='Створити'") + " " + ЗаголовокДокумента,
	...
	Иначе
		...
		ДобавитьТекстОшибкиСформированныеДокументы(
			МассивСтрок,
			НСтр("ru='Ошибка: найдено несколько подчиненных документов';uk='Помилка: знайдено кілька підлеглих документів'")
...
Процедура ДобавитьТекстНеТребуетсяСформированныеДокументы(МассивСтрок, ЗаголовокДокумента)
	...
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Формировать';uk='Формувати'") + " " + ЗаголовокДокумента + " " + НСтр("ru='не требуется';uk='не потрібно'")));
	МассивСтрок.Добавить(Символы.ПС);
...
Процедура ПоказатьФормуЗагрузкиИзФайла()
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка начальных остатков по складу из файла';uk='Завантаження початкових залишків по складу з файлу'");
