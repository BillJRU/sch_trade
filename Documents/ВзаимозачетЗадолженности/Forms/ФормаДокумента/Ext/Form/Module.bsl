...
Процедура ОбъектРасчетовДебиторКредиторНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ТипДебитора = ПредопределенноеЗначение("Перечисление.ТипыУчастниковВзаимозачета.ОрганизацияКлиент") Тогда
...
Процедура РассчитатьВзаимозачетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЕстьСтрокиДтЗадолженности = НЕ Объект.ДебиторскаяЗадолженность.Количество() = 0
		И НЕ (Объект.ДебиторскаяЗадолженность.Количество() = 1 И Не ЗначениеЗаполнено(Объект.ДебиторскаяЗадолженность[0].ОбъектРасчетов));
		
	ЕстьСтрокиКтЗадолженности = НЕ Объект.КредиторскаяЗадолженность.Количество() = 0
		И НЕ (Объект.КредиторскаяЗадолженность.Количество() = 1 И Не ЗначениеЗаполнено(Объект.КредиторскаяЗадолженность[0].ОбъектРасчетов));
	...
	Если ЕстьСтрокиДтЗадолженности ИЛИ ЕстьСтрокиКтЗадолженности Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("РассчитатьВзаимозачет", ЭтотОбъект), 
			НСтр("ru='При выполнении операции табличные части дебиторской и кредиторской задолженности будут очищены. Продолжить?';uk='При виконанні операції табличні частини дебіторської та кредиторської заборгованості будуть очищені. Продовжити?'"),
...
Процедура ЗаполнитьПоДебиторскойЗадолженностиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СуммаРеглДт = 0 Тогда
		ТекстСообщения = НСтр("ru='Дебиторская задолженность не заполнена!';uk='Дебіторська заборгованість не заповнена!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоКредиторскойЗадолженностиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СуммаРеглКт = 0 Тогда
		ТекстСообщения = НСтр("ru='Кредиторская задолженность не заполнена!';uk='Кредиторська заборгованість не заповнена!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементовДебиторскаяЗадолженностьБезРазбиения()
	...
	Если Объект.ДебиторскаяЗадолженность.Количество() Тогда
		...
		ТолькоПросмотрПартнераВалюты = ЗначениеЗаполнено(ТекСтрока.ОбъектРасчетов);
...
Процедура РассчитатьСуммыВзаимозачета(Знач СуммаДокументаРегл = 0)
	...
	Если СуммаВзаимозачета < СуммаДокументаРегл Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Взаимозачет возможен на сумму %1 %2, скорректировать сумму?';uk='Взаємозалік можливий на суму %1 %2, скорегувати суму?'"),
			СуммаВзаимозачета,
			ВалютаРегламентированногоУчета);
...
Процедура ПересчитатьСуммыРеглУпр(ОписаниеОповещения)
	...
	ТекстВопроса = НСтр("ru='Суммы регламентированного и управленческого учёта по строкам в табличной части не равны суммам документам, пересчитать суммы документа?';uk='Суми регламентованого та управлінського обліку по рядках в табличній частині не рівні сумам документам, перерахувати суми документа?'");
	...
	Если Состояние = Неопределено Тогда
		...
		Если НЕ РасчетыРаспределены Тогда
			...
			Если ФормаДлительнойОперации = Неопределено Тогда
				...
				Элемент.Заголовок = НСтр("ru='Пожалуйста, подождите...';uk='Будь ласка, зачекайте...'")+Символы.ПС+
					НСтр("ru='Выполняется распределение расчетов по документам.';uk='Виконується розподіл розрахунків за документами.'");
			Иначе
...
Процедура УстановитьВидимостьБазоваяВерсия()
	...
	Если НЕ ЭтоНеБазовая Тогда
		Элементы.ДебиторскаяЗадолженностьБезРазбиенияСуммаРегл.Заголовок = НСтр("ru='Сумма';uk='Сума'");
		Элементы.КредиторскаяЗадолженностьБезРазбиенияСуммаРегл.Заголовок = НСтр("ru='Сумма';uk='Сума'");
		Элементы.ДекорацияСуммаРеглПодвал.Заголовок = НСтр("ru='Сумма';uk='Сума'");
		Элементы.КредиторскаяЗадолженностьСуммаРегл.Заголовок = НСтр("ru='Сумма';uk='Сума'");
		Элементы.ДебиторскаяЗадолженностьСуммаРегл.Заголовок = НСтр("ru='Сумма';uk='Сума'");
	КонецЕсли;
