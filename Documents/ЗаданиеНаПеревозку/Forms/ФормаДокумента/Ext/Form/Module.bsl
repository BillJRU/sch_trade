...
Процедура ВодительФИООбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ВыбранноеЗначение.Свойство("ДолжностьВодителя",		Объект.ДолжностьВодителя); 
КонецПроцедуры
...
Процедура АвтомобильГосударственныйНомерОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ВыбранноеЗначение.Свойство("АвтомобильМасса",							Объект.АвтомобильМасса);
	ВыбранноеЗначение.Свойство("АвтомобильДлина",			Объект.АвтомобильДлина);
	ВыбранноеЗначение.Свойство("АвтомобильШирина",			Объект.АвтомобильШирина);
	ВыбранноеЗначение.Свойство("АвтомобильВысота",			Объект.АвтомобильВысота);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Объект.Операция = Перечисления.ВидыДоставки.НаСклад Тогда
		Элементы.МаршрутПолучателиОтправители.Заголовок      = НСтр("ru='Отправители';uk='Відправники'");
	...
	Иначе
		Элементы.МаршрутПолучателиОтправители.Заголовок      = НСтр("ru='Получатели';uk='Отримувачі'");
		Элементы.Статус.СписокВыбора.Очистить();
		Элементы.Статус.СписокВыбора.Добавить(Перечисления.СтатусыЗаданийНаПеревозку.Формируется);
...
Процедура ТранспортноеСредствоПриИзмененииНаСервере()
	...
	ПараметрыТТН.Вставить("АвтомобильМасса");
	ПараметрыТТН.Вставить("АвтомобильДлина");
	ПараметрыТТН.Вставить("АвтомобильШирина");
	ПараметрыТТН.Вставить("АвтомобильВысота");
	...
	Если ЗначениеЗаполнено(Объект.ТранспортноеСредство) Тогда
		...
		|	ТранспортныеСредства.Масса КАК АвтомобильМасса,
		|	ТранспортныеСредства.Длина КАК АвтомобильДлина,
		|	ТранспортныеСредства.Ширина КАК АвтомобильШирина,
		|	ТранспортныеСредства.Высота КАК АвтомобильВысота,
...
Процедура ЗаполнитьСлужебныеРеквизитыМаршрута(ОбновляемыеСтроки = Неопределено)
	...
	ШаблонСПеревозчиком = НСтр("ru='%Перевозчик% (для %ПолучателиОтправители%)';uk='%Перевозчик% (для %ПолучателиОтправители%)'");
	Пока Результат.СледующийПоЗначениюПоля("НомерСтрокиМаршрута") Цикл
		...
		СтрокаМаршрута.ПолучателиОтправители = ПолучателиОтправителиПоПункту
			+ СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоРаспоряжений, НСтр("ru='распоряжение, распоряжения, распоряжений';uk='розпорядження, розпорядження, розпоряджень'"))
			+ НСтр("ru='...';uk='...'");
...
Процедура НастроитьПоТипуИсполнителя()
	...
	Элементы.ГруппаПеревозчик.Видимость       = ВыполняетПеревозчик;
...
Процедура ВодительПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ТранспортноеСредство) Тогда
		...
		|	ФизическиеЛица.КодПоДРФО КАК ВодительКод, 
		...
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ПараметрыТТН, Выборка);
			ПараметрыТТН.ВодительФИО = СокрЛП(Выборка.ВодительФИО) + ?(ПустаяСтрока(Выборка.ВодительКод), "", " "+ СокрЛП(Выборка.ВодительКод));
		КонецЕсли;			
	КонецЕсли;
...
Процедура МаршрутДополнительнаяИнформацияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормыРедактирования = НСтр("ru='Дополнительная информация';uk='Додаткова інформація'");
...
Процедура ЗаполнитьСпискиВыбораВодителейИТС()
	...
	|	ЗаданиеНаПеревозку.ДолжностьВодителя КАК ДолжностьВодителя, 
	...
	|	ЗаданиеНаПеревозку.ДолжностьВодителя, 
	...
	Пока РезультатЗапроса.Следующий() Цикл
		ИнформацияВодителя = Новый Структура("Водитель, ДолжностьВодителя, УдостоверениеСерия, УдостоверениеНомер");
	...
	|	ЗаданиеНаПеревозку.АвтомобильМасса КАК АвтомобильМасса,
	|	ЗаданиеНаПеревозку.АвтомобильВместимостьВКубическихМетрах КАК АвтомобильВместимостьВКубическихМетрах,   
	|	ЗаданиеНаПеревозку.АвтомобильДлина КАК АвтомобильДлина,
	|	ЗаданиеНаПеревозку.АвтомобильШирина КАК АвтомобильШирина,
	|	ЗаданиеНаПеревозку.АвтомобильВысота КАК АвтомобильВысота,   
	...
	|	ЗаданиеНаПеревозку.АвтомобильМасса,
	|	ЗаданиеНаПеревозку.АвтомобильВместимостьВКубическихМетрах,
	|	ЗаданиеНаПеревозку.АвтомобильДлина,
	|	ЗаданиеНаПеревозку.АвтомобильШирина,
	|	ЗаданиеНаПеревозку.АвтомобильВысота,   
	...
	Пока РезультатЗапроса.Следующий() Цикл       
		ИнформацияТранспортногоСредства = Новый Структура(
...
Функция СоздатьТранспортныеНакладныеНаСервере(ВыделенныеСтрокиИдентификаторы)
	...
	Если РезультатПроверки.ЗаданияНаПеревозкуПоКоторымНакладныеУжеСозданы.Количество() <> 0 Тогда
		ТекстСообщения = НСтр("ru='По выделенным адресам не требуется оформлять транспортные накладные, т.к. они уже оформлены.';uk='За виділеними адресами не потрібно оформляти транспортні накладні, тому що вони вже оформлені.'");
