	...
	Если ЭтотОбъект.Операция = Перечисления.ВидыДоставки.СоСклада
		...
		Если ДоставкаТоваров.ПерезаполнитьЗаданиеНаПеревозкуПоРасходнымОрдерам(ЭтотОбъект) Тогда // есть изменения после перезаполнения
			...
			Если Распоряжения.Количество() = 0 Тогда
				Текст = НСтр("ru='Статус документа %Задание% не установлен, т.к. после перезаполнения по расходным ордерам в нем не остается распоряжений.';uk='Статус документа %Задание% не встановлений, тому що після перезаповнення за видатковими ордерами в ньому не залишається розпоряджень.'");
