...
Функция СформироватьПечатнуюФормуМаршрутныйЛист(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока РезультатШапкаДокументов.Следующий() Цикл
		...
		НазваниеДокумента = НСтр("ru='Маршрутный лист';uk='Маршрутний лист'", КодЯзыкаПечать);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ЗаданиеНаПеревозку.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЗаданиеНаПеревозку.Маршрут КАК ЗаданиеНаПеревозку
	|ГДЕ
	|	(ЗаданиеНаПеревозку.Адрес <> """"
	|		И (
	|			(ВЫРАЗИТЬ(ЗаданиеНаПеревозку.АдресЗначенияПолей КАК СТРОКА(21))) = ""<КонтактнаяИнформация""
	|				И НЕ ЗаданиеНаПеревозку.АдресЗначенияПолей ПОДОБНО ""%</КонтактнаяИнформация>""
	|			ИЛИ (ВЫРАЗИТЬ(ЗаданиеНаПеревозку.АдресЗначенияПолей КАК СТРОКА(21))) <> ""<КонтактнаяИнформация""
	|				И ЗаданиеНаПеревозку.АдресЗначенияПолей ПОДОБНО &ДлинаНевалидногоJSON
	|			)
	|	)";
...
Функция ЗаполнитьВремяРейсаПланФакт(Объект) Экспорт
	...
	Если Объект.Статус <> Перечисления.СтатусыЗаданийНаПеревозку.Формируется
		...
		ТекстыСообщений.Добавить(НСтр("ru='план. время начала рейса';uk='план. час початку рейсу'"));
	...
	Если Объект.Статус <> Перечисления.СтатусыЗаданийНаПеревозку.Формируется
		...
		ТекстыСообщений.Добавить(НСтр("ru='план. время окончания рейса';uk='план. час закінчення рейсу'"));
	...
	Если (Объект.Статус = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено
		...
		ТекстыСообщений.Добавить(НСтр("ru='факт. время начала рейса';uk='факт. час початку рейсу'"));
	...
	Если Объект.Статус = Перечисления.СтатусыЗаданийНаПеревозку.Закрыто
		...
		ТекстыСообщений.Добавить(НСтр("ru='факт. время окончания рейса';uk='факт. час закінчення рейсу'"));
