...
Процедура ЗаполнитьПоДаннымДругогоФормата(Команда)
	Если Объект.ТоварныеКатегории.Количество()>0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоДаннымДругогоФорматаЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть содержит строки и будет перезаполнена';uk='Таблична частина містить рядки і буде перезаповнена'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
