...
Процедура ОформитьПриобретениеТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ИСТИНА                                      КАК СверхЗаказа, // Отказ от движений по КОформлениюНакладныхПоРаспоряжению
	|	ТаблицаТовары.Серия                         КАК Серия,
...
Процедура ОформитьПоступлениеТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.СтатусУказанияСерийПолучатель КАК СтатусУказанияСерийТоварыУПартнеров,
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ИСТИНА                                                                      КАК СверхЗаказа, // Для отказа от движений по КОформлениюНакладныхПоРаспоряжению
	|	ТаблицаТовары.Серия                                                         КАК Серия,
	|	ТаблицаТовары.Количество                                                    КАК Количество,
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет реквизит ""Вариант приемки товаров"" документа ""Акт о расхождениях после перемещения"". Пока обработчик не выполнен, возможны ошибки при изменении имеющихся документов.';uk='Заповнює реквізит ""Варіант приймання товарів"" документа ""Акт про розбіжності після переміщення"". Поки обробник не виконано, можливі помилки при зміні наявних документів.'");
