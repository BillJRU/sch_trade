	...
	Если Объект.ПоступлениеПоЗаказам Тогда
		...
		Иначе
			...
			ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
				"Документ.ПриемкаТоваровНаХранение.Форма.ФормаДокумента.ПоступлениеПоЗаказамПриИзменении");
...
Функция ПривязатьСтрокиПоступленияКЗаказамПоставщикам(ОтобратьПоЗаказу)
	...
	Иначе
		ТекстЗапросаОстатки =
			...
			|		И ЗаказыПоставщику.ЦенаВключаетНДС = &ЦенаВключаетНДС
			|
			|	ОБЪЕДИНИТЬ ВСЕ
			|
			|	ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("СкладГруппой",              Объект.Склад.ЭтоГруппа);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
    ...
    СтруктураДействийДобавленныеСтроки.Вставить("ЗаполнитьНоменклатуруГТД");
...
Процедура ЗаполнитьСписокВыбораПоляСдал()
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	МАКСИМУМ(ПриемкаТоваровНаХранение.Дата) УБЫВ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Дата,
	|	Сдал
