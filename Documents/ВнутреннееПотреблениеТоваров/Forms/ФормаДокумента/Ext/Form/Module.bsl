...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ВнутреннееПотреблениеТоваров.ПараметрыВыбораСтатейИАналитик(Объект.ХозяйственнаяОперация);
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаВнутреннееПотребление) Тогда
		...
		Текст =
			...
			|	&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_СОЕДИНЕНИЯ
			|ИЗ
	...
	Текст =
		...
		|	&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_СОЕДИНЕНИЯ_ОБЪЕДИНЕНИЕ
	...
	Подстановки = Новый Структура("ПоляВыборкиИзСоединения,ПоляВыборкиИзСоединенияЗаказНаРемонт,ПоляВыборкиИзСоединенияОбъединение,
		|ПоляВыборки,ПоляГруппировки,ПоляВыборкиИзТаблицыЗаказы,СоединениеПоПолям");
	
	Подстановки.ПоляВыборкиИзСоединения              = "ИСТИНА КАК ФиктивноеПоле";
	Подстановки.ПоляВыборкиИзСоединенияЗаказНаРемонт = "ИСТИНА КАК ФиктивноеПоле";
	Подстановки.ПоляВыборкиИзСоединенияОбъединение   = "ИСТИНА КАК ФиктивноеПоле";
	Подстановки.ПоляВыборки                          = "ИСТИНА КАК ФиктивноеПоле";
	Подстановки.ПоляГруппировки                      = "ИСТИНА";
	Подстановки.ПоляВыборкиИзТаблицыЗаказы           = "ИСТИНА КАК ФиктивноеПоле";
	Подстановки.СоединениеПоПолям                    = "ИСТИНА";
	
	ВнутреннееПотреблениеТоваровЛокализация.ПодстановкиВТекстЗапросаДляПроцедурыПривязатьСтрокиКЗаказамНаВнутреннееПотребление(Подстановки);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_СОЕДИНЕНИЯ_ЗАКАЗНАРЕМОНТ", Подстановки.ПоляВыборкиИзСоединенияЗаказНаРемонт);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_СОЕДИНЕНИЯ_ОБЪЕДИНЕНИЕ", Подстановки.ПоляВыборкиИзСоединенияОбъединение);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_СОЕДИНЕНИЯ", Подстановки.ПоляВыборкиИзСоединения);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ_ИЗ_ТАБЛИЦЫ_ЗАКАЗЫ", Подстановки.ПоляВыборкиИзТаблицыЗаказы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ВЫБОРКИ",                   Подстановки.ПоляВыборки);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_ПОЛЯ_ГРУППИРОВКИ",               Подстановки.ПоляГруппировки);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЛОКАЛИЗАЦИЯ_СОЕДИНЕНИЕ_ПО_ПОЛЯМ",            Подстановки.СоединениеПоПолям);
	...
	ВнутреннееПотреблениеТоваровЛокализация.ДобавитьКолонкиВТаблицуДляПроцедурыПривязатьСтрокиКЗаказамНаВнутреннееПотребление(
		ТаблицаНераспределенныхТоваров.Колонки);
	...
	Пока Выборка.Следующий() Цикл
		...
		ВнутреннееПотреблениеТоваровЛокализация.ВставитьПоляВСтруктуруДляПроцедурыПривязатьСтрокиКЗаказамНаВнутреннееПотребление(
			СтруктураПоиска);
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,КатегорияЭксплуатации,ФизическоеЛицо,
		|СтатьяРасходов,АналитикаРасходов,АналитикаАктивовПассивов";
