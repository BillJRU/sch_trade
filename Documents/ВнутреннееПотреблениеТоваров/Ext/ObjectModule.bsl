...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ВнутреннееПотреблениеТоваров.ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПотреблениеПоЗаказам И Не ПраваПользователяПовтИсп.ВнутреннееПотреблениеСверхЗаказа() Тогда
		...
		ШаблонОшибки = НСтр("ru='Нет доступа для оформления внутреннего потребления сверхзаказа (строка %1 списка ""Товары"")';uk='Немає доступу для оформлення внутрішнього споживання понад замовлення (рядок %1 списку ""Товари"")'");
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	...
	|ГДЕ
	|	(ВидыЗапасов.НалоговоеНазначение <> &НалоговоеНазначение
...
Процедура ПроверитьДублированиеАналитикиНоменклатуры(Отказ)
	...
	|       )
	|		И НЕ (ТоварыПроверка.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|		   И ТоварыДубль.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|       )
	|ГДЕ 
