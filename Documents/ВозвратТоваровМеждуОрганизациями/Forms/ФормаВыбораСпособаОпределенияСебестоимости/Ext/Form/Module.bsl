...
Процедура ПеренестиДанныеВДокумент()
	...
	Модифицированность = Ложь;
		...
		Если СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.Вручную") Тогда
			...
			ПараметрыЗакрытия.Вставить("СебестоимостьНДСРегл", СебестоимостьНДСРегл);
			ПараметрыЗакрытия.Вставить("СебестоимостьНДСУпр", СебестоимостьНДСУпр);
		Иначе
			...
			ПараметрыЗакрытия.Вставить("СебестоимостьНДСРегл", 0);
			ПараметрыЗакрытия.Вставить("СебестоимостьНДСУпр", 0);
		КонецЕсли;
