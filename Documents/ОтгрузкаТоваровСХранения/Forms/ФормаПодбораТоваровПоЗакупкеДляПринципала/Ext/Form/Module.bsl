...
Процедура ЗаполнитьТаблицуТовары()
	...
	Для каждого СтрокаОтгрузки Из ДанныеПоОтгрузке Цикл
		...
		Если Не ПересчитыватьСумму Тогда
			...
			СтрокаТовара.СуммаНДС = СтрокаТовара.СуммаДокументаНДС;
			СтрокаТовара.СуммаСНДС = СтрокаТовара.СуммаДокументаСНДС;
		КонецЕсли;
...
&НаКлиенте
Процедура ПодобранныеТоварыСтавкаНДСПриИзменении(Элемент)
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПодобранныеТоварыСуммаСНДС.Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПодобранныеТоварыСтавкаНДС.Имя);
...
Процедура ЗаполнитьСписокВыбораРежимОстатков()
	...
	Если ПоЗаказу Тогда
		РежимыОстатковДоступныеЗначения.Добавить("ВсеЗаказанные", НСтр("ru='Все заказанные';uk='Всі замовлені'"));
		РежимыОстатковДоступныеЗначения.Добавить("ПоОтчету", НСтр("ru='По отчету';uk='За звітом'"));
		Если ОрдернаяСхемаПриОтгрузке Тогда
			РежимыОстатковДоступныеЗначения.Добавить("ПоОрдеру", НСтр("ru='По ордеру';uk='По ордеру'"));
		КонецЕсли;
	Иначе
		РежимыОстатковДоступныеЗначения.Добавить("ПоОстатку", НСтр("ru='Все доступные';uk='Всі доступні'"));
		РежимыОстатковДоступныеЗначения.Добавить("ПоОтчету", НСтр("ru='По отчету';uk='За звітом'"));
		Если ОрдернаяСхемаПриОтгрузке Тогда
			РежимыОстатковДоступныеЗначения.Добавить("ПоОрдеру", НСтр("ru='По ордеру';uk='По ордеру'"));
		КонецЕсли;
...
Функция РазрешитьПереносРезультатовВДокумент()
	...
	Для Каждого СтрокаТоваров Из ПомеченныеТовары Цикл
		...
		Если СтрокаТоваров.КоличествоУпаковок = 0 Тогда
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Количество в отгрузке после"" в строке номер %НомерСтроки%';uk='Не заповнена колонка ""Кількість в відвантаженні після"" в рядку номер %НомерСтроки% '");
	...
	Если ПоЗаказу
		...
		ТекстСообщения =  НСтр("ru='Запрещена отгрузка по нескольким заказам. Необходимо выбрать товары или работы по одному заказу.';uk='Заборонено відвантаження по декількох замовленнях. Необхідно вибрати товари або роботи за одним замовленням.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, Поле, "", Отказ);
	КонецЕсли;
	Возврат Не Отказ;
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	Если Не ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		ИнформационнаяНадписьОтборы = НСтр("ru='Отбор по: %Организация%, %Партнер%, %Договор%, %Склад%';uk='Відбір по: %Организация%, %Партнер%, %Договор%, %Склад%'");
	Иначе
		ИнформационнаяНадписьОтборы = НСтр("ru='Отбор по: %Договор%, %Склад%';uk='Відбір по: %Договор%, %Склад%'");
	КонецЕсли;
	
	Если Склады.Количество() > 1 Тогда
		НадписьСклад = НСтр("ru='группе складов: ""%Склад%""';uk='групі складів: ""%Склад%""'");
...
Процедура РассчитатьСуммуСтроки(ТекущаяСтрока)
	...
	Если Не ПересчитыватьСумму Тогда
		...
		ТекущаяСтрока.СуммаНДС = ТекущаяСтрока.СуммаДокументаНДС;
		ТекущаяСтрока.СуммаСНДС = ТекущаяСтрока.СуммаДокументаСНДС;
	КонецЕсли;
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий, ПересчитыватьСумму);
	КэшированныеЗначения = Неопределено;
