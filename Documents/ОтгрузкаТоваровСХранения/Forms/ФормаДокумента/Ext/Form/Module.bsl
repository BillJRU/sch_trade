...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Заказ не выбран';uk='Замовлення не вибрано'"));
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОказаниеАгентскихУслугПриЗакупке") Тогда
		ЭлементСписка = Элементы.ХозяйственнаяОперация.СписокВыбора.НайтиПоЗначению(Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала);
...
Процедура ПодоборТоваровПринятыхНаОтветственноеХранение()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Склад) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Склад"" не заполнено';uk='Поле ""Склад"" не заповнено'"), Объект.Ссылка,
			"Объект.Склад", "", Отказ);
	КонецЕсли;
...
Процедура ОповеститьОбОкончанииЗаполненияЦен(ЦеныРассчитаны = Истина)
	...
	Если ЦеныРассчитаны Тогда
		ПоказатьОповещениеПользователя(
			...
			,
			НСтр("ru='Проверьте правильность цен в отчете принципалу';uk='Перевірте правильність цін в звіті принципалу'"),
			БиблиотекаКартинок.Информация32);
	Иначе
		ПоказатьОповещениеПользователя(
			НСтр("ru='Цены не заполнены';uk='Ціни не заповнені'"),
			,
			НСтр("ru='Необходимо вручную заполнить цены';uk='Необхідно вручну заповнити ціни'"),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура НадписьЗаголовокЗаказыКлиентовНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			СписокОснований,
			НСтр("ru='Заказы принципала (%КоличествоДокументов%)';uk='Замовлення принципала (%КоличествоДокументов%)'")),
...
Процедура ИспользоватьОснованиеПриИзменении(Элемент)
	...
	Если Не (Объект.ПоЗаказу ИЛИ ИспользоватьРеализациюПоНесколькимЗаказам) Тогда
		Если ЗначениеЗаполнено(Объект.Основание) Тогда
			ТекстСообщенияПользователю = НСтр("ru='Строки таблицы ""Товары"" больше не связаны с документом %ОбъектОснование%.';uk='Рядки таблиці ""Товари"" більше не пов''язані з документом %ОбъектОснование%.'");
...
Процедура ХозяйственнаяОперацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоставкаПодПринципала")
		...
		И Объект.Товары.Количество() > 0 Тогда 
				...
				СписокКнопок.Добавить("ОчиститьВведенныеДанные", НСтр("ru='Очистить введенные данные';uk='Очистити введені дані'"));
...
|;uk='Для відвантаження принципалу на замовлення необхідно робити підбір товарів по замовленнях. 
|Очисти підібрані товари?'"),
				СписокКнопок,
				,
				"ОтменитьИзменения");
	КонецЕсли;	
