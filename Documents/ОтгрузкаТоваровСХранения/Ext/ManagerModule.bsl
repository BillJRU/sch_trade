...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеОтгрузкиПоОтгрузкамТоваровСХранения") Тогда
		...
		Если КомандаСоздания <> Неопределено Тогда
			КомандаСоздания.Представление = НСтр("ru='Акт о расхождениях после отгрузки с хранения';uk='Акт про розбіжності після відвантаження зі зберігання'");
		КонецЕсли;
	КонецЕсли;
...
|Заполнение документа не выполнено.'
|;uk='Для складів документа ""%Распоряжение%"" оформлення видаткових ордерів не потрібно.
|Заповнення документа не виконано.'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Распоряжение%", Строка(Распоряжение));
...
Функция ТекстЗапросаВтТаблицаСерииТоваров(Запрос, ТекстыЗапроса)
	...
	|	И ТаблицаТовары.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление                  = НСтр("ru='Отгрузка товаров с хранения';uk='Відвантаження товарів зі зберігання'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ИмяМакета) Тогда
		СинонимМакета = НСтр("ru='Отгрузка товаров с хранения';uk='Відвантаження товарів зі зберігання'");
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, ИмяМакета, СинонимМакета,
 			СформироватьПечатнуюФормуОтгрузкаТоваровСХранения(МассивОбъектов, ОбъектыПечати,ПараметрыВывода),,,,Истина);
	КонецЕсли;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Представление,
 			СформироватьПечатнуюФормуОтгрузкаТоваровСХранения(КомплектПечати.Объекты, ОбъектыПечати, ПараметрыВывода),,,,Истина);
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Обработки.ПечатьЗаданияНаОтборРазмещениеТоваров.СформироватьЗаданиеНаОтборРазмещениеТовара(СтруктураТипов,
																										ОбъектыПечати,
 																										ПараметрыПечати, ПараметрыВывода),,,,Истина);
	КонецЦикла;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм,
																			"ОтгрузкаТоваровСХранения",
																			НСтр("ru='Отгрузка товаров с хранения';uk='Відвантаження товарів зі зберігання'"),
																			1);
...
//
Функция СформироватьПечатнуюФормуОтгрузкаТоваровСХранения(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		// Макет необходимо получать для каждого документа, т.к. размеры колонок изменяются динамически.
 		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ОтгрузкаТоваровСХранения.ПФ_MXL_ОтгрузкаТоваровСХранения",КодЯзыкаПечать);
		...
		СтруктураДанныхШапки.Вставить("ТекстЗаголовка",
										ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
											ДанныеПечати,
											НСтр("ru='Отгрузка товаров с хранения';uk='Відвантаження товарів зі зберігання'",КодЯзыкаПечать),КодЯзыкаПечать));
		...
		СтруктураДанныхИтоговаяСтрока.Вставить("СуммаПрописью",
 												РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаКПрописи, ДанныеПечати.Валюта,,КодЯзыкаПечать));
...
Функция СформироватьТаблицуОтгрузки()
	...
	ТоварыДляОтгрузки.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"));
	ТоварыДляОтгрузки.Колонки.Добавить("Сумма", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("СуммаНДС", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("СуммаСНДС", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	...
	ТоварыДляОтгрузки.Колонки.Добавить("СуммаДокументаСНДС", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
	ТоварыДляОтгрузки.Колонки.Добавить("СуммаДокументаНДС", ОбщегоНазначенияУТ.ОписаниеТипаДенежногоПоля());
...
//   * ЦенаВключаетНДС - Булево - включает цена НДС.
...
Функция ПолучитьТаблицуОстатковПоОтгрузке(ПараметрыДанных) Экспорт
	...
	Запрос.УстановитьПараметр("ЦенаВключаетНДС", ПараметрыДанных.ЦенаВключаетНДС);
	...
	Для каждого СтрокаЦена Из ДанныеПоЦенам Цикл
		...
		Для каждого СтрокаОстаток Из СтрокиПоОстаткам Цикл
			Если СтрокаОстаток.КоличествоОстаток > СтрокаЦена.КоличествоОстаток 
				...
				СтрокаОстаток.Цена = ?(ПараметрыДанных.ЦенаВключаетНДС,
				СтрокаЦена.Цена, СтрокаЦена.Цена - ?(СтрокаЦена.СуммаНДСОстаток = 0, 0, СтрокаЦена.СуммаНДСОстаток/СтрокаЦена.КоличествоОстаток));
				СтрокаОстаток.СуммаСНДС = СтрокаЦена.СуммаСНДСОстаток;
				СтрокаОстаток.СуммаНДС = СтрокаЦена.СуммаНДСОстаток;
	...
	Для каждого СтрокаПоЗаказуИОтгрузки Из ДанныеПоЗаказуИОтгрузке Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрокаОтгрузки, СтрокаПоЗаказуИОтгрузки, "Номенклатура, Характеристика, Серия, Назначение, ЗаказКлиента, Цена, СтавкаНДС, Упаковка, Коэффициент, КоличествоВНакладной, Склад, ОрдернаяСхемаПриОтгрузке");
		...
		Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
			Если СтрокаОстатка.КоличествоОстаток = 0 Тогда
				Продолжить;
			КонецЕсли;
			КоличествоОстаток = Мин(Цел(СтрокаОстатка.КоличествоОстаток/СтрокаПоЗаказуИОтгрузки.Коэффициент)*СтрокаПоЗаказуИОтгрузки.Коэффициент, СтрокаПоЗаказуИОтгрузки.КоличествоВЗаказе);
		...
		Для каждого СтрокаОстаткаПоОрганизации Из СтрокиСОстаткамиПоОрганизации Цикл
			Если СтрокаОстаткаПоОрганизации.КоличествоОстаток = 0 Тогда
				Продолжить;
			КонецЕсли;
			КоличествоОстаток = Мин(Цел(СтрокаОстаткаПоОрганизации.КоличествоОстаток/СтрокаПоЗаказуИОтгрузки.Коэффициент)*СтрокаПоЗаказуИОтгрузки.Коэффициент, НоваяСтрокаОтгрузки.КоличествоВЗаказе);
			НоваяСтрокаОтгрузки.КоличествоОстаток = КоличествоОстаток;
			НоваяСтрокаОтгрузки.Количество = КоличествоОстаток;
			НоваяСтрокаОтгрузки.Цена = СтрокаОстаткаПоОрганизации.Цена;
...
Запрос.Текст = "ВЫБРАТЬ
   ...
   |	ОтгрузкаТоваровСХраненияТовары.СтавкаНДС КАК СтавкаНДС,
   ...
   |	ВтТоварыРеализации.СтавкаНДС КАК СтавкаНДС,
   ...
   |					И ЗаказТовары.Ссылка.ЦенаВключаетНДС = &ЦенаВключаетНДС
   |					И ВЫБОР
   ...
   |					КОНЕЦ
   |					И &ИспользоватьРасширенныеВозможностиЗаказаКлиента) КАК ЗаказыОстатки
   |	
   |	ОБЪЕДИНИТЬ ВСЕ
   |	
   |	ВЫБРАТЬ
   ...
   |		ТоварыЗаказаУпаковка.СтавкаНДС,
   ...
   |	ЦеныПоЗакупки.СуммаСНДСОстаток КАК СуммаСНДСОстаток,
   |	ЦеныПоЗакупки.СуммаНДСОстаток КАК СуммаНДСОстаток,
   ...
   |		СУММА(ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.СуммаСНДСПриход) КАК СуммаСНДСОстаток,
   |		СУММА(ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.СуммаНДСПриход) КАК СуммаНДСОстаток,
   ...
   |		ВЫРАЗИТЬ(ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.СуммаНДСПриход / ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.КоличествоПриход КАК ЧИСЛО(31, 2)) КАК НДСПозиции
   |	ИЗ
   ...
   |		ВЫРАЗИТЬ(ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.СуммаНДСПриход / ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОбороты.КоличествоПриход КАК ЧИСЛО(31, 2))
   |	) КАК ЦеныПоЗакупки
   |;
   |
   |////////////////////////////////////////////////////////////////////////////////
   ...
   |		ЗаказыОстатки.ЗаказКлиента.Назначение
   |	ИЗ
...
Функция СоздатьОтгрузкуТоваровСХраненияНаОснованииЗаказаКлиентаПроверкаХозОперации(ОбъектОснование) Экспорт
	...
	Если ТипЗнч(ОбъектОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		ХозяйственнаяОперация = ОбъектОснование.ХозяйственнаяОперация;
		...
		Если Не ЗначениеЗаполнено(ХозяйственнаяОперация)
			...
			РезультатПроверки.ТекстОшибки = НСтр("ru='Команда не может быть выполнена для данной хозяйственной операции';uk='Команда не може бути виконана для даної господарської операції'");
