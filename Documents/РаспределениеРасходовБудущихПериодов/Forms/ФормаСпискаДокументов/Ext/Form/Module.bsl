...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РасходыКРаспределениюГруппаКРаспределениюУпр.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='К распределения в упр. учете (%1)';uk='До розподілу в упр. обліку (%1)'"),
	...
	Элементы.РасходыКРаспределениюГруппаКРаспределениюРегл.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='К распределения в регл. учете (%1)';uk='До розподілу в регл. обліку (%1)'"),
...
Процедура СоздатьРаспределениеРасходов(Команда)
	...
	Если Строка <> Неопределено Тогда
		...
		СтруктураОснование.Вставить("НалоговоеНазначение", Строка.НалоговоеНазначение);
		СтруктураОснование.Вставить("СуммаДокументаРеглБезНДС", Строка.СуммаРеглБезНДС);
		СтруктураОснование.Вставить("СуммаДокументаНДСРегл", Строка.НДСРегл);
		...
		Если ДокументРаспределения <> Неопределено Тогда
			...
			ТекстВопроса = НСтр("ru='Для данной статьи расходов существует документ распределения в периоде с %1 по %2.';uk='Для даної статті витрат існує документ розподілу в періоді з %1 по %2.'");;
