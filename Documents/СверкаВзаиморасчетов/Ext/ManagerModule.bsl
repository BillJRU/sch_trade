...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиВзаимныхРасчетов") Тогда
		...
		Если ДокументыНаПечать.Новые.Количество() > 0 Тогда
			МакетНаПечать = СформироватьПечатнуюФормуАктСверкиВзаиморасчетов(ДокументыНаПечать.Новые, ОбъектыПечати, ПараметрыВывода);
			МассивДокументов = ДокументыНаПечать.Новые;
			
		КонецЕсли;
		//++ Локализация
		Если ДокументыНаПечать.Старые.Количество() > 0 Тогда
			МакетНаПечать = СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовПоСтарому(ДокументыНаПечать.Старые, ОбъектыПечати, МакетНаПечать, ПараметрыВывода);
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			НСтр("ru='Акт сверки взаимных расчетов';uk='Акт звірки взаємних розрахунків'"),
			МакетНаПечать,,,, Истина);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиВзаимныхРасчетовСДаннымиКонтрагента") Тогда
		...
		//++ Локализация
		Если ДокументыНаПечать.Старые.Количество() > 0 Тогда
			МакетНаПечать = СформироватьПечатнуюФормуАктСДаннымиКонтрагента(ДокументыНаПечать.Старые, ОбъектыПечати, МакетНаПечать, ПараметрыВывода);
			МассивДокументов = ДокументыНаПечать.Старые;
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			НСтр("ru='Акт сверки взаимных расчетов (с задолженностью по данными контрагента)';uk='Акт звірки взаємних розрахунків (з заборгованістю по даними контрагента)'"),
			МакетНаПечать,,,, Истина);
...
Функция СформироватьПечатнуюФормуАктСверкиВзаиморасчетов(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Для Каждого ДанныеДокумента Из ДанныеСверки.Строки Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СверкаВзаиморасчетов.ПФ_MXL_АктСверкиВзаимныхРасчетов", КодЯзыкаПечать); // ТабличныйДокумент - 
		...
		Если ДанныеДокумента.НачалоПериода = '00010101' Тогда
			...
			ПредставлениеПериода = СтрШаблон(НСтр("ru='по состоянию на %1';uk='станом на %1'",КодЯзыкаПечать), Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать),);
			
			ПредставлениеПериодаРасчетов = СтрШаблон(НСтр("ru='По состоянию на %1';uk='Станом на %1'",КодЯзыкаПечать), Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать),);
		...
		Иначе
			...
			Если  КодСимвола(ПервыйСимволПредставлениеПериода) >= 48
				...
				ПредставлениеПериода = СтрШаблон(НСтр("ru='за период: %1';uk='за період: %1'",КодЯзыкаПечать), ПредставлениеПериода);
				
			Иначе
				ПредставлениеПериода = СтрШаблон(НСтр("ru='за %1';uk='за %1'",КодЯзыкаПечать), ПредставлениеПериода);
			КонецЕсли;
			
			ШаблонПериодаРасчетов = НСтр("ru='В период с %1 по %2 были осуществлены следующие расчеты';uk='У період з %1 по %2 були здійснені наступні розрахунки'",КодЯзыкаПечать)+":";
			Если НЕ ЕстьОстаткиОборотыЗаПериод Тогда
				ШаблонПериодаРасчетов = НСтр("ru='В период с %1 по %2 операции по расчетам не производились.';uk='У період з %1 по %2 операції по розрахунках не відбувались.'",КодЯзыкаПечать);
			КонецЕсли;
			
			ПредставлениеПериодаРасчетов = СтрШаблон(
				ШаблонПериодаРасчетов,
				Формат(ДанныеДокумента.НачалоПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать),
				Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать),);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеДокумента.Договор) Тогда
			ОбластьМакета.Параметры.ПредставлениеДоговора = НСтр("ru='по договору';uk='за договором'",КодЯзыкаПечать) + ": " + ДанныеДокумента.ПредставлениеДоговора;
		КонецЕсли;
			...
			Для Каждого ДанныеПоВалюте Из ДанныеПоТипуРасчетов.Строки Цикл
				...
				Для Каждого ДанныеПоОбъектуГруппировки Из ДанныеПоВалюте.Строки Цикл
					...
					Если ЕстьДанныеГруппировки Тогда
						ТабличныйДокумент.Вывести(ВывестиДетальныеЗаписи(ДанныеПоОбъектуГруппировки, ДанныеДокумента, Макет, ИтогоЗадолженность, НастройкиПечати, КодЯзыкаПечать));
					КонецЕсли;
		...
		Если ИтогоЗадолженность.Количество() > 0 Тогда
			...
			ОбластьМакетаЗаголовок.Параметры.КонецПериодаСверки = Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать);
			...
			Для Каждого ДолгВВалюте Из ЗадолженностьПоВалютам Цикл
				...
				КонецЕсли;
				ОбластьДолг.Параметры.СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаДолга,ДолгВВалюте.Валюта,, КодЯзыкаПечать);
				...
				Если ДолгВВалюте.Валюта <> ВалютаРегл Тогда
					...
					ОбластьДолгРегл.Параметры.СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаДолгаРегл, ВалютаРегл,, КодЯзыкаПечать);
		...
		Иначе
			...
			ОбластьМакета.Параметры.КонецПериодаСверки = Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л="+КодЯзыкаПечать);
			ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета.Параметры.Заполнить(ДанныеДокумента);
		ОбластьМакета.Параметры.ФИООтветственногоЛица 	   = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(ДанныеДокумента.ФИООтветственногоЛица);
		...
		ОбластьМакета.Параметры.ПредставлениеОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, 
															  "ПолноеНаименование,КодПоЕДРПОУ,ЮридическийАдрес",,КодЯзыкаПечать);
...
Функция ВывестиДетальныеЗаписи(
	...
	ИначеЕсли НастройкиПечати.ВыводитьАвансРегл Тогда
		...
		ОбластьМакетаОбороты 		= Макет.ПолучитьОбласть("ОборотыЗаПериодАвансРегл");
		ОбластьМакетаКонОстаток  = Макет.ПолучитьОбласть("СтрокаТаблицыКонОстатокАвансРегл");
		Если НастройкиПечати.НеЗаполнятьДанныеКонтрагента Тогда
			ОбластьМакетаНачОстаток.Область("СуммаКонтрагентНачОстатокАвансРегл").Формат = "ЧДЦ=2";
			ОбластьМакетаСтрока.Область("СуммаКонтрагентАвансРегл").Формат = "ЧДЦ=2";
			ОбластьМакетаОбороты.Область("СуммаКонтрагентОборотыАвансРегл").Формат = "ЧДЦ=2";
			ОбластьМакетаКонОстаток.Область("СуммаКонтрагентКонОстатокАвансРегл").Формат = "ЧДЦ=2";
		КонецЕсли;
	...
	Валюта = СтрШаблон(НСтр("ru='в валюте';uk='у валюті'",КодЯзыкаПечать) + " %1", ДанныеПоОбъектуГруппировки.Валюта);
	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов = "ОбъектРасчетов"
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru='по объекту расчетов';uk='по об''єкту розрахунків'",КодЯзыкаПечать) + " %1 ";
	ИначеЕсли НастройкиПечати.ДетализацияВзаиморасчетов = "Договор" И НЕ ЭтоФинансовыеИнструменты
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru='по договору';uk='за договором'",КодЯзыкаПечать) + " %1 ";
	КонецЕсли;
	
	ТекстОбъектаГруппировки = "<" + НСтр("ru='не указан';uk='не зазначений'",КодЯзыкаПечать) + ">";
	...
	ОбластьМакетаШапка.Параметры.Дебет = НСтр("ru='Долг';uk='Борг'",КодЯзыкаПечать);
	ОбластьМакетаШапка.Параметры.Кредит = НСтр("ru='Аванс';uk='Аванс'",КодЯзыкаПечать);
	ОбластьМакетаНачОстаток.Параметры.ИмяОстатка = НСтр("ru='Начальный остаток';uk='Початковий залишок'",КодЯзыкаПечать);
	ОбластьМакетаКонОстаток.Параметры.ИмяОстатка = НСтр("ru='Конечный остаток';uk='Кінцевий залишок'",КодЯзыкаПечать);
	Если НастройкиПечати.ДолгКакДебет ИЛИ ЭтоФинансовыеИнструменты Тогда
		ОбластьМакетаШапка.Параметры.Дебет = НСтр("ru='Дебет';uk='Дебет'",КодЯзыкаПечать);
		ОбластьМакетаШапка.Параметры.Кредит = НСтр("ru='Кредит';uk='Кредит'",КодЯзыкаПечать);
		ОбластьМакетаНачОстаток.Параметры.ИмяОстатка = НСтр("ru='Сальдо начальное';uk='Сальдо початкове'",КодЯзыкаПечать);
		ОбластьМакетаКонОстаток.Параметры.ИмяОстатка = НСтр("ru='Сальдо конечное';uk='Сальдо кінцеве'",КодЯзыкаПечать);
	КонецЕсли;
	...
	Если Задолженность.СуммаДт <> 0 Тогда
		Задолженность.СуммаРеглДт = ДанныеПоОбъектуГруппировки.КонечноеСальдоРеглДт;
	КонецЕсли;
	Задолженность.СуммаКт = ДанныеПоОбъектуГруппировки.КонечноеСальдоКт;
	Если Задолженность.СуммаКт <> 0 Тогда
		Задолженность.СуммаРеглКт = ДанныеПоОбъектуГруппировки.КонечноеСальдоРеглКт;
	КонецЕсли;
...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВложенныЗапрос.Ссылка КАК Ссылка,
	|	ВложенныЗапрос.Валюта КАК Валюта,
	|	МАКСИМУМ(ВложенныЗапрос.Период) КАК Период
	|ПОМЕСТИТЬ ДатыКурсовВалют
	|ИЗ 
	|(
	|	ВЫБРАТЬ
	|		ВалютыДокументов.Ссылка КАК Ссылка,
	|		ВалютыДокументов.Валюта КАК Валюта,
	|		МАКСИМУМ(КурсыВалют.Период) КАК Период
	|	ИЗ
	|		Документ.СверкаВзаиморасчетов.ГруппировкиРасчеты КАК ВалютыДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО ВалютыДокументов.Валюта = КурсыВалют.Валюта
	|			И ВалютыДокументов.Ссылка.КонецПериода >= КурсыВалют.Период
	|	ГДЕ
	|		ВалютыДокументов.Ссылка В(&МассивОбъектов)
	|		И ВалютыДокументов.Валюта <> &ВалютаРегл
	|
	|	СГРУППИРОВАТЬ ПО
	|		ВалютыДокументов.Ссылка,
	|		ВалютыДокументов.Валюта
	|		
	|	ОБЪЕДИНИТЬ
	|
	|	ВЫБРАТЬ
	...
	|		МАКСИМУМ(КурсыВалют.Период) КАК Период
	|	ИЗ
	|		Документ.СверкаВзаиморасчетов.ГруппировкиФинансовыеИнструменты КАК ВалютыДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО ВалютыДокументов.Валюта = КурсыВалют.Валюта
	|			И ВалютыДокументов.Ссылка.КонецПериода >= КурсыВалют.Период
	|	ГДЕ
	...
	|	ИЗ
	|		Документ.СверкаВзаиморасчетов КАК ВалютыДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО КурсыВалют.Валюта = &ВалютаРегл
	|	ГДЕ
	|		ВалютыДокументов.Ссылка В(&МассивОбъектов)
	|)КАК ВложенныЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныЗапрос.Ссылка,
	|	ВложенныЗапрос.Валюта
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	Валюта,
	|	Период
	...
	|	ДатыКурсовВалют.Период КАК ДатаКурса,
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО ТЧГруппировкиРасчетов.Валюта = КурсыВалют.Валюта
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыКурсовВалют КАК ДатыКурсовВалют
	|		ПО КурсыВалют.Валюта = ДатыКурсовВалют.Валюта
	|		И КурсыВалют.Период = ДатыКурсовВалют.Период
	|ГДЕ
	...
	|	КурсыВалют.Курс КАК Курс,
	|	КурсыВалют.Кратность КАК Кратность,
	|	СУММА(ТЧГруппировкиРасчетов.НачальноеСальдо) КАК НачальноеСальдо,
	|	СУММА(ТЧГруппировкиРасчетов.НачальноеСальдоРегл) КАК НачальноеСальдоРегл,
	|	СУММА(ТЧГруппировкиРасчетов.ОборотПриход) КАК ОборотПриход,
	|	СУММА(ТЧГруппировкиРасчетов.ОборотРасход) КАК ОборотРасход,
	|	СУММА(ТЧГруппировкиРасчетов.КонечноеСальдо) КАК КонечноеСальдо,
	|	СУММА(ВЫБОР КОГДА ТЧГруппировкиРасчетов.КонечноеСальдо > 0
	|			ТОГДА ВЫРАЗИТь(ТЧГруппировкиРасчетов.КонечноеСальдо * КурсыВалют.Курс / КурсыВалют.Кратность КАК ЧИСЛО(31,2))
	|			ИНАЧЕ ТЧГруппировкиРасчетов.КонечноеСальдоРегл
	|		КОНЕЦ) КАК КонечноеСальдоРегл
	|ПОМЕСТИТЬ ТЧГруппировкиРасчетов
	|ИЗ
	|	Документ.СверкаВзаиморасчетов.ГруппировкиРасчеты КАК ТЧГруппировкиРасчетов
	|	ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалют КАК КурсыВалют
	|	ПО ТЧГруппировкиРасчетов.Ссылка = КурсыВалют.Ссылка
	|		И ТЧГруппировкиРасчетов.Валюта = КурсыВалют.Валюта
	|ГДЕ
	|	ТЧГруппировкиРасчетов.Ссылка В(&МассивОбъектов)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СУММА(ВЫБОР КОГДА ТЧГруппировкиРасчетов.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|		ТОГДА
	|			-ТЧГруппировкиРасчетов.НачальноеСальдоРегл
	|		ИНАЧЕ
	|			ТЧГруппировкиРасчетов.НачальноеСальдоРегл
	|	КОНЕЦ) КАК НачальноеСальдоРегл,
	|	СУММА(ТЧГруппировкиРасчетов.ОборотПриход) КАК ОборотПриход,
	|	СУММА(ТЧГруппировкиРасчетов.ОборотРасход) КАК ОборотРасход,
	|	СУММА(ВЫБОР КОГДА ТЧГруппировкиРасчетов.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|		ТОГДА
	...
	|	СУММА(ВЫБОР КОГДА ТЧГруппировкиРасчетов.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|		ТОГДА
	|			-ТЧГруппировкиРасчетов.КонечноеСальдоРегл
	|		ИНАЧЕ
	|			ТЧГруппировкиРасчетов.КонечноеСальдоРегл
	|	КОНЕЦ) КАК КонечноеСальдоРегл
	...
	|	СУММА(ГруппировкаРасчетов.НачальноеСальдоРегл) КАК НачальноеСальдоРегл,
	...
	|	СУММА(ГруппировкаРасчетов.КонечноеСальдоРегл) КАК КонечноеСальдоРегл
	...
	|	Группировки.НачальноеСальдоРегл КАК НачальноеСальдоРегл,
	|	ВЫБОР
	...
	|		КОГДА (Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|					ИЛИ Группировки.ТипРасчетов = НЕОПРЕДЕЛЕНО)
	|				И Группировки.НачальноеСальдоРегл > 0
	|			ТОГДА 
	|				Группировки.НачальноеСальдоРегл
	|			
	|		КОГДА Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|				И Группировки.НачальноеСальдоРегл < 0
	|			ТОГДА 
	...
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК НачальноеСальдоРеглДт,
	|	ВЫБОР
	|		КОГДА (Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|					ИЛИ Группировки.ТипРасчетов = НЕОПРЕДЕЛЕНО)
	|				И Группировки.НачальноеСальдо < 0
	...
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК НачальноеСальдоРеглКт,
	...
	|	Группировки.КонечноеСальдоРегл КАК КонечноеСальдоРегл,
	|	ВЫБОР
	|		КОГДА (Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|					ИЛИ Группировки.ТипРасчетов = НЕОПРЕДЕЛЕНО)
	...
	|		КОГДА (Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|					ИЛИ Группировки.ТипРасчетов = НЕОПРЕДЕЛЕНО)
	|				И Группировки.КонечноеСальдоРегл > 0
	|			ТОГДА 
	|				Группировки.КонечноеСальдоРегл
	|			
	|		КОГДА Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|				И Группировки.КонечноеСальдоРегл < 0
	|			ТОГДА 
	...
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК КонечноеСальдоРеглДт,
	|	ВЫБОР
	|		КОГДА (Группировки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|					ИЛИ Группировки.ТипРасчетов = НЕОПРЕДЕЛЕНО)
	|				И Группировки.КонечноеСальдо < 0
	...
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ КАК КонечноеСальдоРеглКт,
	|	
	...
	|	ДеталиРасчетов.НачальноеСальдоРегл КАК НачальноеСальдоРегл,
	|	ДеталиРасчетов.НачальноеСальдоРеглДт КАК НачальноеСальдоРеглДт,
	|	ДеталиРасчетов.НачальноеСальдоРеглКт КАК НачальноеСальдоРеглКт,
	...
	|	ДеталиРасчетов.КонечноеСальдоРегл КАК КонечноеСальдоРегл,
	|	ДеталиРасчетов.КонечноеСальдоРеглДт КАК КонечноеСальдоРеглДт,
	|	ДеталиРасчетов.КонечноеСальдоРеглКт КАК КонечноеСальдоРеглКт,
	|	
	...
	|	СУММА(ДеталиРасчетов.СуммаАвансРегл) КАК СуммаАвансРегл,
	...
	|	ДеталиРасчетов.НачальноеСальдоРегл,
	|	ДеталиРасчетов.НачальноеСальдоРеглДт,
	|	ДеталиРасчетов.НачальноеСальдоРеглКт,
	...
	|	ДеталиРасчетов.КонечноеСальдоРегл,
	|	ДеталиРасчетов.КонечноеСальдоРеглДт,
	|	ДеталиРасчетов.КонечноеСальдоРеглКт,
	|	
	...
	|	0 КАК НачальноеСальдоРеглДт,
	|	0 КАК НачальноеСальдоРеглКт,
	...
	|	0 КАК КонечноеСальдоРеглДт,
	|	0 КАК КонечноеСальдоРеглКт,
	|	
	...
	|		ТОГДА ТаблицаОтветственныеЛица.ГлавныйБухгалтерПолноеФИО
	|		ИНАЧЕ ТаблицаОтветственныеЛица.РуководительПолноеФИО
	...
	|	ЕСТЬNULL(ДеталиДокумента.НачальноеСальдоРегл, 0) КАК НачальноеСальдоРегл,
	|	ЕСТЬNULL(ДеталиДокумента.НачальноеСальдоРеглДт, 0) КАК НачальноеСальдоРеглДт,
	|	ЕСТЬNULL(ДеталиДокумента.НачальноеСальдоРеглКт, 0) КАК НачальноеСальдоРеглКт,
	...
	|	ЕСТЬNULL(ДеталиДокумента.КонечноеСальдоРегл, 0) КАК КонечноеСальдоРегл,
	|	ЕСТЬNULL(ДеталиДокумента.КонечноеСальдоРеглДт, 0) КАК КонечноеСальдоРеглДт,
	|	ЕСТЬNULL(ДеталиДокумента.КонечноеСальдоРеглКт, 0) КАК КонечноеСальдоРеглКт,
	|	
	...
	|	МАКСИМУМ(НачальноеСальдоРегл),
	|	МАКСИМУМ(НачальноеСальдоРеглДт),
	|	МАКСИМУМ(НачальноеСальдоРеглКт),
	|	МАКСИМУМ(ОборотПриход),
	|	МАКСИМУМ(ОборотРасход),
	|	МАКСИМУМ(КонечноеСальдо),
	|	МАКСИМУМ(КонечноеСальдоДт),
	|	МАКСИМУМ(КонечноеСальдоКт),
	|	МАКСИМУМ(КонечноеСальдоРегл),
	|	МАКСИМУМ(КонечноеСальдоРеглДт),
	|	МАКСИМУМ(КонечноеСальдоРеглКт),
	|	
...
// 
Функция ОписаниеРасчетногоДокумента(ДокументСсылка, Номер, Дата, КодЯзыкаПечать) Экспорт
	ПредставленияДокументов = ПредставленияДокументов(КодЯзыкаПечать);
	ВходящиеДокументы = ВходящиеДокументы();
	ПредставлениеДокумента = "";
	ОписаниеДокумента = "№ __________ " + НСтр("ru='от';uk='від'",КодЯзыкаПечать) + " __________";
...
Функция ПредставленияДокументов(КодЯзыкаПечать)
	...
	ВидыДокументовКонтрагента.Вставить("ВводОстатковВзаиморасчетов",			НСтр("ru='Корректировка задолженности';uk='Коригування заборгованості'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ВводОстатковПоФинансовымИнструментам",	НСтр("ru='Корректировка задолженности';uk='Коригування заборгованості'",КодЯзыкаПечать));
	...
	ВидыДокументовКонтрагента.Вставить("ВозвратТоваровМеждуОрганизациями",		НСтр("ru='Возврат товаров';uk='Повернення товарів'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ВозвратТоваровОтКлиента",				НСтр("ru='Возврат товаров';uk='Повернення товарів'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ВозвратТоваровПоставщику",				НСтр("ru='Возврат товаров';uk='Повернення товарів'",КодЯзыкаПечать));
	...
	ВидыДокументовКонтрагента.Вставить("КорректировкаПриобретения",				НСтр("ru='Корректировка задолженности';uk='Коригування заборгованості'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("НачисленияКредитовИДепозитов", 			НСтр("ru='Начисление';uk='Нарахування'",КодЯзыкаПечать));
	...
	ВидыДокументовКонтрагента.Вставить("ОтчетПоКомиссииМеждуОрганизациямиОСписании",НСтр("ru='Отчет комитенту';uk='Звіт комітенту'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПередачаТоваровМеждуОрганизациямиЗакупка",  НСтр("ru='Закупка';uk='Купівля'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПередачаТоваровМеждуОрганизациямиПродажа",НСтр("ru='Продажа';uk='Продаж'",КодЯзыкаПечать));
	...
	ВидыДокументовКонтрагента.Вставить("ПриобретениеТоваровУслуг",				НСтр("ru='Закупка';uk='Купівля'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПрочихАктивов",		НСтр("ru='Акт';uk='Акт'",КодЯзыкаПечать));
	...
	ВидыДокументовКонтрагента.Вставить("РасчетКурсовыхРазниц",					НСтр("ru='Курсовые разницы';uk='Курсові різниці'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("РеализацияТоваровУслуг", 				НСтр("ru='Продажа';uk='Продаж'",КодЯзыкаПечать));
...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом
			...
			ГруппировкаДетальныхЗаписей = "ТипРасчетов, ОбъектРасчетов, РасчетныйДокумент, ВалютаДокумента";
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			НоваяСтрока.ОписаниеДокумента = ОписаниеРасчетногоДокумента(СтрокаРасчетов.РасчетныйДокумент, СтрокаРасчетов.НомерДокумента, СтрокаРасчетов.ДатаДокумента, МультиязычностьУкр.ПолучитьЯзыкФормированияПечатныхФорм());
			//++ Локализация
			//-- Локализация
	...
	ГрупповыеЗаписи.Свернуть(Группировки + "ОбъектРасчетов, Валюта", "НачальноеСальдо, НачальноеСальдоРегл, ОборотПриход, ОборотРасход, КонечноеСальдо, КонечноеСальдоРегл");
	ДанныеДокумента.ГруппировкиРасчеты.Очистить();
	
	ДетальныеЗаписи = ДанныеДокумента.ДетальныеЗаписиРасчеты.Выгрузить();
	ДетальныеЗаписи.Свернуть("ТипРасчетов, ОбъектРасчетов, РасчетныйДокумент, ОписаниеДокумента, ВалютаДокумента", "СуммаДокумента, СуммаДолг, СуммаАванс, СуммаАвансРегл");
	
	КэшКурсов = ПолучитьКэшКурсовВалют(ГрупповыеЗаписи, ДанныеДокумента.КонецПериода);
	Для Каждого Группировка Из ГрупповыеЗаписи Цикл
		...
		ВзаиморасчетыКлиентСервер.РассчитатьКонечноеСальдоРегл(Группировка, ДетальныеЗаписи, КэшКурсов);
...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикОтбор, УникальныйИдентификаторФормы = Неопределено, НастройкиОтбора = Неопределено) Экспорт
	...
	ПартнерыИКонтрагенты.ЗаголовокПоляСКДВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
		СхемаКомпоновкиДанных.НаборыДанных.ОстаткиИОбороты, "Партнер", НСтр("ru='Контрагент';uk='Контрагент'"));
...
Функция СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовПоСтарому(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент = Неопределено, ПараметрыВывода)
	УстановитьПривилегированныйРежим(Истина);
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Если ДетальныеРасчеты = Неопределено Тогда 
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По документу %1 нет расчетов с партнерами. Печать акта сверки взаимных расчетов не требуется.';uk='За документом %1 немає розрахунків з партнерами. Друк акту звірки взаємних розрахунків не потрібний.'",КодЯзыкаПечать),
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СверкаВзаиморасчетов.ПФ_MXL_УстаревшийАктСверкиВзаиморасчетов", КодЯзыкаПечать);
		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
		...
		Если ДанныеДокумента.НачалоПериода = '00010101' Тогда
			...
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='по состоянию на %1';uk='станом на %1'",КодЯзыкаПечать),
				Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л=" + КодЯзыкаПечать),);
		...
		Иначе
			...
			ПредставлениеПериодаРасчетов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В период с %1 по %2';uk='У період з %1 по %2'",КодЯзыкаПечать),
				Формат(ДанныеДокумента.НачалоПериода, "ДЛФ=ДД; Л=" + КодЯзыкаПечать),
				Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД; Л=" + КодЯзыкаПечать),);
		...
		Пока ДетальныеРасчеты.Следующий() Цикл
			...
			Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
				...
				Пока ВыборкаПоДетальнымРасчетам.Следующий() Цикл
					ТабличныйДокумент.Вывести(ВывестиДетальныеЗаписиПоСтарому(ВыборкаПоДетальнымРасчетам, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности, ДетальныеРасчеты.Партнер, ,КодЯзыкаПечать));
				КонецЦикла; 
			ИначеЕсли ДанныеДокумента.РасшифровкаПоДоговорам Тогда
				ВыборкаПоДетальнымРасчетам = ДетальныеРасчеты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаПоДетальнымРасчетам.Следующий() Цикл
					ТабличныйДокумент.Вывести(ВывестиДетальныеЗаписиПоСтарому(ВыборкаПоДетальнымРасчетам, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности, , ДетальныеРасчеты.Договор, КодЯзыкаПечать));
				КонецЦикла; 
			Иначе
				ТабличныйДокумент.Вывести(ВывестиДетальныеЗаписиПоСтарому(ДетальныеРасчеты, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности, , , КодЯзыкаПечать));
			КонецЕсли;
		...
		Если ДанныеДокумента.РасшифровкаПоЗаказам Тогда
			...
			Если ТаблицаОстатков <> Неопределено Тогда
				...
				ТекстЗаголовкаПриложения = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеДокумента,
					НСтр("ru='к акту сверки взаимных расчетов';uk='до акта звірки взаємних розрахунків'",КодЯзыкаПечать));
				...
				Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
					Для Каждого Партнер Из МассивПартнеров Цикл
						ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, Партнер, , , КодЯзыкаПечать));
					КонецЦикла; 
				Иначе
					ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, , , , КодЯзыкаПечать));
				КонецЕсли;	
...
Функция СформироватьПечатнуюФормуАктСДаннымиКонтрагента(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент = Неопределено, ПараметрыВывода)
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СверкаВзаиморасчетов.ПФ_MXL_УстаревшийАктСДаннымиКонтрагента", КодЯзыкаПечать);
		...
		Если ТаблицаЗадолженностиН <> Неопределено Тогда
			...
			Если ДанныеДокумента.РасшифровкаПоЗаказам Тогда
				Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
					Для Каждого Партнер Из МассивПартнеров Цикл
						ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, Партнер, ТаблицаРасхождений, , КодЯзыкаПечать));
					КонецЦикла; 
				Иначе
					ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, , ТаблицаРасхождений, , КодЯзыкаПечать));
				КонецЕсли;
			Иначе
		...
		Если ТаблицаЗадолженностиК <> Неопределено Тогда
			...
			ОбластьМакета.Параметры.Заполнить(СтруктураПредставленийУчастников);
			ОбластьМакета.Параметры.КонецПериодаСверки = Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДДД; Л=" + КодЯзыкаПечать);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			...
			Если ДанныеДокумента.РасшифровкаПоЗаказам Тогда
				Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
					Для Каждого Партнер Из МассивПартнеров Цикл
						ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, Партнер, ТаблицаРасхождений, Истина, КодЯзыкаПечать));
					КонецЦикла; 
				Иначе
					ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(ПараметрыТаблицыЗадолженностей, , ТаблицаРасхождений, Истина, КодЯзыкаПечать));
				КонецЕсли;
			Иначе
				ОбластьМакета = Макет.ПолучитьОбласть("СтрокаЗадолженность");
...
Функция ВывестиДетальныеЗаписиПоСтарому(
	...
	Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1, в валюте: %2';uk='%1, у валюті: %2'",КодЯзыкаПечать), СтрокаЗадолженности.Партнер, ВыборкаДетальныхЗаписей.Валюта);
	ИначеЕсли ДанныеДокумента.РасшифровкаПоДоговорам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1, в валюте: %2';uk='%1, у валюті: %2'",КодЯзыкаПечать), СтрокаЗадолженности.Договор, ВыборкаДетальныхЗаписей.Валюта);
	Иначе
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='в валюте: %1';uk='у валюті: %1'",КодЯзыкаПечать), ВыборкаДетальныхЗаписей.Валюта);
	КонецЕсли;
	...
	Пока ДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			ОписаниеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				?(ПредставлениеДокумента <> Неопределено, ПредставлениеДокумента, ""),
				НомерНаПечать,
				?(ЗначениеЗаполнено(ДетальныеЗаписи.ДатаВходящегоДокумента), Формат(ДетальныеЗаписи.ДатаВходящегоДокумента, "ДЛФ=D; Л=" + КодЯзыкаПечать) + " " + НСтр("ru='г.';uk='р.'",КодЯзыкаПечать), НСтр("ru='  .  .    ';uk='  .  .    '",КодЯзыкаПечать)));
			...
			ОбластьМакетаСтрока.Параметры.Дата = Формат(ДетальныеЗаписи.Дата, "ДЛФ=D; Л=" + КодЯзыкаПечать);
...
Функция ВывестиТаблицуЗадолженности(
	...
	Для Каждого ВалютаВзаиморасчетов Из МассивВалют Цикл
		...
		Если МассивСтрокЗадолженности.Количество() > 0 Тогда
			Если Партнер <> Неопределено Тогда
				ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1, в валюте: %2';uk='%1, у валюті: %2'",КодЯзыкаПечать), ПредставлениеПартнера, ВалютаВзаиморасчетов);
			Иначе
				ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='в валюте: %1';uk='у валюті: %1'",КодЯзыкаПечать), ВалютаВзаиморасчетов);
			КонецЕсли;
			ТабличныйДокумент.Вывести(ОбластьМакетаГруппировка);
			ОбластьМакетаШапка.Параметры.Заполнить(СтруктураПредставленийУчастников);
			ТабличныйДокумент.Вывести(ОбластьМакетаШапка);
...
Функция УстаревшееОписаниеРасчетногоДокумента(
	...
	КодЯзыкаПечать) Экспорт
	ПредставленияДокументов     	  = УстаревшиеПредставленияДокументов(КодЯзыкаПечать);
	ПредставленияВладельцевДокументов = ПредставленияВладельцевДокументов(КодЯзыкаПечать);
	...
	Иначе
		...
		ОписаниеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Предоплата от %1';uk='Передоплата від %1'",КодЯзыкаПечать), ПредставлениеДебитора);
	КонецЕсли;
...
Функция УстаревшиеПредставленияДокументов(КодЯзыкаПечать)	
	ВидыДокументовКонтрагента = Новый Структура;
	...
	ВидыДокументовКонтрагента.Вставить("ПоступлениеБезналичныхДенежныхСредств", НСтр("ru='Платежное поручение';uk='Платіжне доручення'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПриобретениеТоваровУслуг",				НСтр("ru='Накладная';uk='Накладна'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПрочихАктивов",			НСтр("ru='Акт';uk='Акт'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПриходныйКассовыйОрдер",				НСтр("ru='Приходный кассовый ордер';uk='Прибутковий касовий ордер'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("РасходныйКассовыйОрдер",					НСтр("ru='Расходный кассовый ордер';uk='Видатковий касовий ордер'",КодЯзыкаПечать));
...
Функция ПредставленияВладельцевДокументов(КодЯзыкаПечать)	
	...
	ВидыДокументовКонтрагента.Вставить("ПриобретениеТоваровУслуг",				НСтр("ru='от';uk='від'",КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПрочихАктивов", 		НСтр("ru='от';uk='від'",КодЯзыкаПечать));
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1ddfd7e1-e9f8-462f-b683-f61fb0a44dd1");
	...
	Читаемые.Добавить(Метаданные.Документы.СверкаВзаиморасчетов.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ГДЕ
	|				ТЧГруппировкиРасчеты.Ссылка = СверкаВзаиморасчетов.Ссылка
	|				И ТЧГруппировкиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)))
	|	ИЛИ (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.СверкаВзаиморасчетов.ДетальныеЗаписиРасчеты КАК ТЧДетальныеЗаписиРасчеты
	|			ГДЕ
	|				ТЧДетальныеЗаписиРасчеты.Ссылка = СверкаВзаиморасчетов.Ссылка
	|				И ТЧДетальныеЗаписиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ТЧГруппировкиРасчеты.НомерСтроки  КАК НомерСтроки,
	|	ОбъектыРасчетов.Ссылка            КАК ОбъектРасчетов
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов.ГруппировкиРасчеты КАК ТЧГруппировкиРасчеты
	|			ПО ОбновляемыеДанные.Ссылка = ТЧГруппировкиРасчеты.Ссылка
	|				И ТЧГруппировкиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов КАК СверкаВзаиморасчетов
	|			ПО ОбновляемыеДанные.Ссылка = СверкаВзаиморасчетов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|			ПО (ТЧГруппировкиРасчеты.УдалитьОбъектРасчетов = ОбъектыРасчетов.Объект
	|				ИЛИ ТЧГруппировкиРасчеты.УдалитьОбъектРасчетов В (&ПустыеЗначенияОбъектаРасчетов)
	|				И ОбъектыРасчетов.Объект = НЕОПРЕДЕЛЕНО
	|				И ТЧГруппировкиРасчеты.Договор = ОбъектыРасчетов.Договор)
	|				И ТЧГруппировкиРасчеты.ТипРасчетов = ОбъектыРасчетов.ТипРасчетов
	|				И СверкаВзаиморасчетов.Организация = ОбъектыРасчетов.Организация
	|				И ТЧГруппировкиРасчеты.Партнер = ОбъектыРасчетов.Партнер
	|				И СверкаВзаиморасчетов.Контрагент = ОбъектыРасчетов.Контрагент
	|				И ТЧГруппировкиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|ИТОГИ
	|ПО
	|	Ссылка,
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		ВыборкаВерсияДанных = ВыборкаДокумент.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Ссылка = ВыборкаДокумент.Ссылка;
		
			Пока ВыборкаВерсияДанных.Следующий() Цикл
			...
			Попытка
				...
				ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
					Ссылка,
					ВыборкаВерсияДанных.ВерсияДанных,
					Параметры.Очередь);
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Если Не НомерСтроки = Null Тогда
						
						СтрокаТЧ = ДокументОбъект.ГруппировкиРасчеты[НомерСтроки-1];
						Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ОбъектРасчетов) Тогда
							СтрокаТЧ.ОбъектРасчетов = ВыборкаДетальныеЗаписи.ОбъектРасчетов;
						ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.Партнер) 
							И ЗначениеЗаполнено(ДокументОбъект.Контрагент) 
							И ЗначениеЗаполнено(СтрокаТЧ.УдалитьОбъектРасчетов)  Тогда
								ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Не удалось заполнить объект расчетов в документе: %1';uk='Не вдалося заповнити об''єкт розрахунків в документі: %1'"),
									Ссылка));
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
				
				СвернутаяТЧГруппировкиРасчеты = ДокументОбъект.ГруппировкиРасчеты.Выгрузить();
				СвернутаяТЧГруппировкиРасчеты.Свернуть("ТипРасчетов, Партнер, УдалитьОбъектРасчетов, Валюта, ОбъектРасчетов");
				СвернутаяТЧГруппировкиРасчеты.Индексы.Добавить("ТипРасчетов, Партнер, УдалитьОбъектРасчетов, Валюта");
				
				Для Каждого ДетальнаяЗаписьРасчеты Из ДокументОбъект.ДетальныеЗаписиРасчеты Цикл
					Если Не ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ОбъектРасчетов) Тогда
						
						СтруктураПоиска = Новый Структура();
						СтруктураПоиска.Вставить("ТипРасчетов", ДетальнаяЗаписьРасчеты.ТипРасчетов);
						СтруктураПоиска.Вставить("Партнер", ДетальнаяЗаписьРасчеты.УдалитьПартнер);
						СтруктураПоиска.Вставить("УдалитьОбъектРасчетов", ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов);
						СтруктураПоиска.Вставить("Валюта", ДетальнаяЗаписьРасчеты.ВалютаДокумента);
						
						СоответствиеВТЧГруппировкиРасчеты = СвернутаяТЧГруппировкиРасчеты.НайтиСтроки(СтруктураПоиска);
						
						Если СоответствиеВТЧГруппировкиРасчеты.Количество() = 1 Тогда
							ДетальнаяЗаписьРасчеты.ОбъектРасчетов = СоответствиеВТЧГруппировкиРасчеты[0].ОбъектРасчетов;
						Иначе
							
							СтруктураДопПоиска = Новый Структура();
							СтруктураДопПоиска.Вставить("УдалитьОбъектРасчетов", ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов);
							СтруктураДопПоиска.Вставить("Валюта", ДетальнаяЗаписьРасчеты.ВалютаДокумента);
							ПоискДополнительный = СвернутаяТЧГруппировкиРасчеты.НайтиСтроки(СтруктураДопПоиска);
							
							Если ПоискДополнительный.Количество() = 1 Тогда
								ДетальнаяЗаписьРасчеты.ОбъектРасчетов = ПоискДополнительный[0].ОбъектРасчетов;
							Иначе
								СтруктураДопПоискаБезВалюты = Новый Структура();
								СтруктураДопПоискаБезВалюты.Вставить("УдалитьОбъектРасчетов", ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов);
								ПоискДополнительныйБезВалюты = СвернутаяТЧГруппировкиРасчеты.НайтиСтроки(СтруктураДопПоискаБезВалюты);
								
								Если ПоискДополнительныйБезВалюты.Количество() = 1 Тогда
									ДетальнаяЗаписьРасчеты.ОбъектРасчетов = ПоискДополнительныйБезВалюты[0].ОбъектРасчетов;
								Иначе
									Если ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов) Тогда
										ДопКритерииПоиска = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
										ДопКритерииПоиска.Контрагент = ДокументОбъект.Контрагент;
										ДопКритерииПоиска.Партнер = ДетальнаяЗаписьРасчеты.УдалитьПартнер;
										ДопКритерииПоиска.ОбновлениеИБ = Истина;
										
										НайденныйОР = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов,
											ДокументОбъект.Организация,
											?(ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ТипРасчетов), ДетальнаяЗаписьРасчеты.ТипРасчетов, Неопределено),
											ДопКритерииПоиска);
											
										Если ЗначениеЗаполнено(НайденныйОР) Тогда
											ДетальнаяЗаписьРасчеты.ОбъектРасчетов = НайденныйОР;
										Иначе				
											ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='Не удалось заполнить объект расчетов в документе: %2';uk='Не вдалося заповнити об''єкт розрахунків в документі: %2'"),
											Ссылка));
										КонецЕсли;
									КонецЕсли;
								КонецЕсли
...
Функция ТекстЗапросаРегистрацияДанныхДляГенерацииПустогоОбъектаРасчетов() Экспорт
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.СверкаВзаиморасчетов.ГруппировкиРасчеты КАК ГруппировкиРасчеты
	|		ГДЕ
	|			СверкаВзаиморасчетов.Ссылка = ГруппировкиРасчеты.Ссылка
	|			И ГруппировкиРасчеты.УдалитьОбъектРасчетов В (&ПустыеЗначенияОбъектРасчетов)
	|			И ГруппировкиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка))";
...
Функция ТекстЗапросаПолучениеДанныхДляГенерацииПустогоОбъектаРасчетов() Экспорт
	...
	|	ТЧГруппировкиРасчеты.Партнер,
	|	ИсточникДанных.Контрагент,
	|	ТЧГруппировкиРасчеты.Договор,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка),
	|	ТЧГруппировкиРасчеты.Валюта
	|ИЗ
	|	Документ.СверкаВзаиморасчетов.ГруппировкиРасчеты КАК ТЧГруппировкиРасчеты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов КАК ИсточникДанных
	|		ПО ТЧГруппировкиРасчеты.Ссылка = ИсточникДанных.Ссылка
	|ГДЕ
	|	ТЧГруппировкиРасчеты.УдалитьОбъектРасчетов В (&ПустыеЗначенияОбъектРасчетов)
	...
	|			ГДЕ
	|				ИсточникДанных.Организация = ОбъектыРасчетов.Организация
	|				И ТЧГруппировкиРасчеты.Партнер = ОбъектыРасчетов.Партнер
	|				И ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) = ОбъектыРасчетов.НаправлениеДеятельности
	|				И ИсточникДанных.Контрагент = ОбъектыРасчетов.Контрагент
	|				И ТЧГруппировкиРасчеты.Договор = ОбъектыРасчетов.Договор
	|				И ТЧГруппировкиРасчеты.ТипРасчетов = ОбъектыРасчетов.ТипРасчетов
	|				И ОбъектыРасчетов.Объект = НЕОПРЕДЕЛЕНО)";
