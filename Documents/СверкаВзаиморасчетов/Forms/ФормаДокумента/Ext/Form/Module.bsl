...
Процедура ПоказатьРасчетыПриИзменении(Элемент)
	Если Объект.ГруппировкиРасчеты.Количество() > 0
		...
		ВзаиморасчетыКлиентСервер.РассчитатьКонечноеСальдоРегл(
			Элементы.ГруппировкиРасчеты.ТекущиеДанные,
			Объект.ДетальныеЗаписиРасчеты,
			КэшКурсов);
	...
	Иначе
		...
		Если Элементы.ГруппировкиРасчеты.ВыделенныеСтроки.Количество() 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru='Не заполнен объект расчетов';uk='Не заповнений об''єкт розрахунків'"),
				,
...
Процедура РаспределениеПоДокументамЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ДанныеКонтрагента) Тогда
		...
		ПараметрыПроверки.ЗаголовокТабличнойЧасти = НСтр("ru='Задолженность по данным контрагента';uk='Заборгованість за даними контрагента'");
	ИначеЕсли Не ЗначениеЗаполнено(Объект.ГруппировкиРасчеты) И ЗначениеЗаполнено(Объект.ГруппировкиФинансовыеИнструменты) Тогда
		...
		ПараметрыПроверки.ЗаголовокТабличнойЧасти = НСтр("ru='Расчеты по финансовым инструментам';uk='Розрахунки за фінансовими інструментами'");
	Иначе
		ПараметрыПроверки.ТабличнаяЧасть          = Объект.ГруппировкиРасчеты;
		ПараметрыПроверки.ЗаголовокТабличнойЧасти = НСтр("ru='Взаиморасчеты';uk='Взаєморозрахунки'");
	КонецЕсли;
	...
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("КонецПериода", НСтр("ru='Конец периода';uk='Кінець періоду'"));
...
Функция РасчетныйДокументОбработкаВыбораНаСеврере(ВыбранноеЗначение)
	...
	ОписаниеДокумента = Документы.СверкаВзаиморасчетов.ОписаниеРасчетногоДокумента(ДанныеДокумента.Ссылка, ДанныеДокумента.Номер, ДанныеДокумента.Дата, КодЯзыкаПечать);
...
Процедура УстановитьОформлениеДтКт(Элемент, ВидСчета)
	...
	ИначеЕсли ВидСчета = "ДтКт" Тогда
		ТекстПоля = НСтр("ru='Дт/Кт';uk='Дт/Кт'");
...
Процедура ЗаполнитьСтрокуПоРасчетномуДокументуСервер(ДокументСсылка, ОписаниеДокумента, ВалютаВзаиморасчетов)
	...
	Если ЗначениеЗаполнено(ЗначенияРеквизитов.Номер) Тогда
		...
		ЗначенияРеквизитов.Номер, ЗначенияРеквизитов.Дата, КодЯзыкаПечать);
...
Функция ПараметрыОтбораДетализации(ВидРасчетов)
	...
	Иначе
		...
		ТекстНеВыбранОбъектРасчетов = НСтр("ru='Не выбран объект расчетов на закладке ""Объекты расчетов""';uk='Не вибрано об''єкт розрахунків на закладці ""Об''єкти розрахунків""'");
		ТекстНеВыбранДоговор = НСтр("ru='Не выбран договор на закладке ""Договоры""';uk='Не вибрано договір на закладці ""Договори""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПереключитьНаДетализацию(ПараметрыОтбора)
	...
	Если ПараметрыОтбора.ТаблицаГруппировки = "ГруппировкиРасчеты" Тогда
		ОбновитьКэшКурсовВалют();
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьКэшКурсовВалют()
	
	ТаблицаРасчетов = Объект.ГруппировкиРасчеты.Выгрузить();
	КурсыВалют = Документы.СверкаВзаиморасчетов.ПолучитьКэшКурсовВалют(ТаблицаРасчетов, Объект.КонецПериода);
	КэшКурсов = Новый ФиксированноеСоответствие(КурсыВалют);
...
Процедура ВключитьОтборДетальныхЗаписей(ВидРасчетов, ПолеОтбора, ЗначениеОтбора)
	...
	ПараметрыОтбора.Отбор.Вставить("ТипРасчетов", ПолучитьТипРасчетов(ЗначениеОтбора));
...
&НаСервере
Функция ПолучитьТипРасчетов(Ссылка)
		
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ТипРасчетов");
...
Функция ПараметрыФормыРасширенногоОтбора()
	...
	Если ОтборДоговорыБезОборотов = Неопределено Тогда
		...
		ОтборДоговорыБезОборотов = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(КомпоновщикОтбор.Настройки.Отбор, "ДоговорыБезОборотов");
		ОтборДоговорыБезОборотов.Представление = НСтр("ru='Включать договора без оборотов в периоде';uk='Включати договори без оборотів в періоді'");
	КонецЕсли;
...
Функция ЕстьЗаданияКРаспределению()
	...
	Если ЕстьЗадания Тогда
		...
		НаименованиеЗадания = НСтр("ru='Выполняется распределение расчетов по документам.';uk='Виконується розподіл розрахунків за документами.'");
...
&НаСервере
Процедура УстановитьВидимостьУстаревшей()
...
|;uk='Згорнути рядки табличній частини відповідно""
|""до настройок деталізації взаєморозрахунків?'");
