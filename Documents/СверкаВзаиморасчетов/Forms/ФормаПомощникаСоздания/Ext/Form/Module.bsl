...
Процедура СоздатьВФоновомЗаданииНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Создание документов ""Сверка взаиморасчетов""';uk='Створення документів ""Звірка взаєморозрахунків""'");
...
Процедура ОбработатьСтрокуПротокола(СтрокаПротокола, КоличествоСозданных)
	...
	Если СтрокаПротокола.Действие = "Создание" Тогда 
		...
		ОповеститьОДействии(СтрокаПротокола.ДокументСверки, НСтр("ru='Создание:';uk='Створення:'"));
		КоличествоСозданных = КоличествоСозданных + 1;
		
	ИначеЕсли СтрокаПротокола.Действие = "ПометкаНаУдаление" Тогда 
		
		ОповеститьОДействии(СтрокаПротокола.ДокументСверки, НСтр("ru='Пометка удаления установлена:';uk='Позначка вилучення встановлено:'"));
		
	ИначеЕсли СтрокаПротокола.Действие = "Перезаполнение" Тогда 
		
		ОповеститьОДействии(СтрокаПротокола.ДокументСверки, НСтр("ru='Перезаполнение:';uk='Перезаповнення:'"));
...
Процедура УстановитьПометкуНаУдаление(Команда)
	...
	ТекстВопроса = НСтр("ru='Изменить пометку удаления для выделенных элементов?';uk='Змінити позначку вилучення для виділених елементів?'");
...
Процедура УстановитьПометкуНаУдалениеНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Изменение пометки на удаление документов ""Сверка взаиморасчетов""';uk='Зміна позначки на вилучення документів ""Звірка взаєморозрахунків""'");
...
Процедура ВыполнитьПереходПоСтраницам(Команда)
	...
	Если Команда.Имя = "Далее"
		...
		Если НЕ (РасчетыСКлиентами ИЛИ РасчетыСПоставщиками ИЛИ РасчетыСКредитором ИЛИ РасчетыСДебитором ИЛИ РасчетыСЛизингодателем) Тогда
			ТекстСообщения = НСтр("ru='Не выбран тип расчетов для сверки.';uk='Не встановлено тип розрахунків для звірки.'");
...
Функция ПараметрыЗаполнены()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаТаблицаРасчетов Тогда
		...
		Если БудетСоздано = 0 И БудетПерезаполнено = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не отмечена ни одна строка для создания или перезаполнения';uk='Не відмічений жоден з рядоків для створення або перезаповнення'"),,"ТаблицаРасчетов");
			Отказ = Истина;
		КонецЕсли;
...
Процедура УправлениеКнопкамиПодвал()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаТаблицаРасчетов Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ru='Выполнить';uk='Виконати'");
		Элементы.Назад.Заголовок = "<< " + НСтр("ru='Назад';uk='Назад'");

	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаРезультат Тогда
		
		Элементы.Далее.Заголовок = НСтр("ru='Завершить';uk='Завершити'");
		Элементы.Назад.Заголовок = "<< " + НСтр("ru='Вернуться в начало';uk='Повернутися в початок'");
...
Процедура ОбновитьГиперссылкуОтбора(КнопкаРасширенногоОтбора, Отбор)
	...
	КнопкаРасширенногоОтбора.Заголовок = НСтр("ru='Расширенный отбор';uk='Розширений відбір'");
	...
	Если УстановленРасширенныйОтбор Тогда
		КнопкаРасширенногоОтбора.Заголовок = НСтр("ru='Установлен расширенный отбор';uk='Встановлено розширений відбір'");
...
Процедура УстановитьОтборНаФорме(ИмяОтбора)
	...
	Элемент.ПодсказкаВвода = НСтр("ru='Укажите значение отбора';uk='Вкажіть значення відбору'");
	...
	Если ЭлементОтбора <> Неопределено Тогда
		...
		Иначе
			...
			Если ЭлементОтбора.Использование ИЛИ ЭлементВИспользуемойГруппеОтбора Тогда
				...
				Элемент.ПодсказкаВвода = НСтр("ru='Установлен расширенный отбор';uk='Встановлено розширений відбір'");
			КонецЕсли;
