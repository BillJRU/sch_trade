...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Заказ на сборку (разборку)';uk='Замовлення на збирання (розбирання)'",КодОсновногоЯзыка);
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ОтменаНеотработанныхСтрок") Тогда
		...
		Если ОформленоКомплектов = 0 Тогда
			...
			ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к.';uk='У документа %Документ% статус ""%Статус%"" не встановлений, тому що'") + " ";
...
Функция ЗаполнитьВариантОбеспечения(Объект, Форма, Операция, ДанныеЗаполнения, ПараметрыУказанияСерий = Неопределено) Экспорт
	...
	Если Операция = "СтрокаТовары" Или Операция = "СтрокиТовары" Или Операция = "Заказ" Или Операция = "Комплект" Тогда
...
Процедура ОтразитьРезерв(Запрос, ТекстыЗапроса, Регистры)
	...
	|													ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.СоСклада))
	|		ИЛИ ДанныеШапки.Статус В(ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КВыполнению),
	|									ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.Закрыт))
	...
	|			И ТоварыДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КОбеспечению))
	|	И ТоварыДокумента.Количество <> 0
	...
	|		ИЛИ ДанныеШапки.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить)
	|			И ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КОбеспечению))
	|	И ТоварыДокумента.Количество <> 0
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТоварыДокумента.Характеристика		КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ТоварыДокумента.ВариантОбеспечения В(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно),
	|													ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|			ТОГДА ВЫБОР
	...
	|	ТоварыДокумента.Характеристика		КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ДанныеШапки.ВариантОбеспечения В(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно),
	|												ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	...
	|	И ДанныеШапки.Статус В(ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КВыполнению),
	|								ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.Закрыт))
	|	И ДанныеШапки.ВариантОбеспечения В(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
	|											ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|	И ТоварыДокумента.Количество <> 0
	|	И ДанныеШапки.СтатусУказанияСерий = 10";
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.БлокируемыеОбъекты = "Документ.ЗаказНаСборку,"
		+ "Справочник.НаправленияДеятельности";
	Обработчик.Комментарий = НСтр("ru='Заполняет реквизиты ""Хозяйственная операция"", ""Назначение"" и ""Вариант приемки товаров"".
                                   |Пока обработчик не выполнен, возможны ошибки при изменении имеющихся документов.'
                                   |;uk='Заповнює реквізити ""Господарська операція"", ""Призначення"" та ""Варіант приймання товарів"".
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ЗаказНаСборку";
	НоваяСтрока.Порядок    = "После";
	НоваяСтрока = Исключения.Добавить();
...
#Область ОбработатьДанныеДляГенерацииНазначений
	...
	Обработчик.Комментарий = НСтр("ru='Создает новые назначения связанные с документом ""Заказ на сборку""';uk='Створює нові призначення, пов''язані з документом ""Замовлення на складання""'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ЗаказНаСборку КАК ЗаказНаСборку
	|ГДЕ
	|	ЗаказНаСборку.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
	|	И ЗаказНаСборку.УдалитьТипОперации <> ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийЗаказаНаСборку.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	КОбработке.Ссылка.УдалитьТипОперации КАК УдалитьТипОперации,
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				УдалитьТипОперации = Выборка.УдалитьТипОперации;
				Если ЗначениеЗаполнено(УдалитьТипОперации) Тогда
					Если УдалитьТипОперации = Перечисления.УдалитьТипыОперацийЗаказаНаСборку.СборкаИзКомплектующих Тогда
...
Процедура ЗарегистрироватьДанныеКОбработкеДляГенерацииНазначений(Параметры) Экспорт
	...
	|	(ЗаказНаСборку.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|				И (ЗаказНаСборку.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)
	|                  ИЛИ ЗаказНаСборку.УдалитьТипОперации = ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийЗаказаНаСборку.РазборкаНаКомплектующие)
	|               )
	|			ИЛИ ЗаказНаСборку.НазначениеТовары = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|				И (ЗаказНаСборку.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров)
	|				   ИЛИ ЗаказНаСборку.УдалитьТипОперации = ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийЗаказаНаСборку.СборкаИзКомплектующих)
	|               )
	|	)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(Объект.Назначение) 
						И (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров
							ИЛИ Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийЗаказаНаСборку.РазборкаНаКомплектующие
					)
					Или Не ЗначениеЗаполнено(Объект.НазначениеТовары) 
						И (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров
							ИЛИ Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийЗаказаНаСборку.СборкаИзКомплектующих
						) 
					Тогда
...
Функция ВременнаяТаблицаТоварыРаспоряженияПоступлениеЗаказДляОтчетаСостояниеВыполнения()
	...
	ТекстЗапроса = "
		...
		|			ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КВыполнению),
		|			ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.Закрыт))
		|		И ДокументТовары.Ссылка.ВариантОбеспечения В (
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
		|
		|
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Заказы на сборку (разборку)';uk='Замовлення на збирання (розбирання)'");
