...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если НЕ ПроверитьВозможностьСозданияСчетовНаОплату(ПараметрКоманды) Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ввод счета на оплату на основании договора %1 возможен только при детализации расчетов ""По договорам"".';uk='Введення рахунку на оплату на підставі договору %1 можливе тільки при деталізації розрахунків ""За договорами"".'"),
			ПараметрКоманды);
