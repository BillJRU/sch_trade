...
|;uk='У рядку %НомерСтроки% списку ""Товари"" не збігаються типи номенклатури. 
|У номенклатури що списується тип %НоменклатураТипНоменклатуры%, у номенклатури що оприбутковується - %НоменклатураОприходованиеТипНоменклатуры%.'");
...
Процедура СверитьКоличествоВБазовыхЕдиницах(Отказ)
	...
	ШаблонТекстаСообщения = НСтр("ru='Списываемое количество номенклатуры ""%НоменклатураСписания%"" в строке ""%НомерСтроки%"" списка ""Товары"" отличается от количества приходуемой номенклатуры ""%НоменклатураОприходования%"" на ""%Количество%"" единиц хранения';uk='Кількість що списується номенклатури ""%НоменклатураСписания%"" в рядку ""%НомерСтроки%"" списку ""Товари"" відрізняється від кількості яка оприбутковується номенклатури ""%НоменклатураОприходования%"" на ""%Количество%"" одиниць зберігання'");
...
Процедура ПроверитьОрдерныйСклад(Отказ)
	...
	Если Не СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Склад, Дата) Тогда
		...
		ТекстСообщения = НСтр("ru='На складе ""%Склад%"" на %Дата% не используется ордерная схема при отражении излишков, недостач, пересортицы и порчи.';uk='На складі ""%Склад%"" на %Дата% не використовується ордерна схема при відображенні надлишків, нестач, пересортиці і псування.'");
