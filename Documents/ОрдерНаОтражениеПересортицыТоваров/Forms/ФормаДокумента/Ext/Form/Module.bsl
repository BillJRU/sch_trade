...
Процедура ЗаполнитьОстаткамиТоваровВЯчейках(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Табличная часть будет очищена и заполнена товарами, по которым нужно оформить пересортицу. Продолжить?';uk='Таблична частина буде очищена і заповнена товарами, за якими потрібно оформити пересортицю. Продовжити?'");
...
Процедура ЗаполнитьОстаткамиТоваровВЯчейкахЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если Объект.Товары.Количество() > 0 Тогда
			ТекстСообщения = НСтр("ru='Табличная часть заполнена';uk='Таблична частина заповнена'");
			ТекстПояснения = НСтр("ru='Табличная часть заполнена остатками товаров в ячейках';uk='Таблична частина заповнена залишками товарів в комірках'");
			
			ПоказатьОповещениеПользователя(ТекстСообщения, , ТекстПояснения, БиблиотекаКартинок.Информация32);
		Иначе
			ТекстСообщения = НСтр("ru='Табличная часть не заполнена';uk='Таблична частина не заповнена'");
			ТекстПояснения = НСтр("ru='Нет остатков товаров в ячейках';uk='Немає залишків товарів в комірках'");
...
Процедура ЗаполнитьПриходуемыйТоварПоСписываемому(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Приходуемый товар заполнен';uk='Оприбутковуваний товар заповнений'");
		ТекстПояснения = НСтр("ru='Приходуемый товар заполнен по списываемому';uk='Оприбутковуваний товар заповнений по списуваному'");
		
		ПоказатьОповещениеПользователя(ТекстСообщения, , ТекстПояснения, БиблиотекаКартинок.Информация32);
	Иначе
		ТекстСообщения = НСтр("ru='Приходуемый товар не заполнен';uk='Оприбутковуваний товар не заповнений'");
		ТекстПояснения = НСтр("ru='Приходуемый товар не заполнен по списываемому';uk='Оприбутковуваний товар не заповнений по списуваному'");
