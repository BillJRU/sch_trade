...
Функция ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация) Экспорт
	...
	ПараметрыВыбора.ДоступностьПоОперации = ХозяйственнаяОперация = Перечисления.ВидыОперацийНалоговыхДокументов.УсловнаяПродажа;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ТипПричиныНевыдачиПокупателю = 22 И НЕ ТЧ.ЕстьПричина501 ЕСТЬ NULL
	|			ТОГДА ВЫБОР
   ...
   |			И Приложение2КНалоговойНакладнойТовары.КодПричины = 501
   |		
   |		ОБЪЕДИНИТЬ
   |		
   |		ВЫБРАТЬ ПЕРВЫЕ 1
   ...
   |		ГДЕ
   |			Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Ссылка = &Ссылка
   |			И Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.КодПричины = 501) КАК ТЧ
   |		ПО (ИСТИНА)
	|ГДЕ
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	ЗначенияПараметровПроведения.Вставить("ИдентификаторМетаданных",	ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.Приложение2КНалоговойНакладной"));
...
Функция ТекстЗапросаТаблицаНДСУсловныеПродажи(Запрос, ТекстыЗапроса, Регистры)
	...
	|	СУММА(-ПоставкаДокумента.Количество) КАК Количество,
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.Приложение2КНалоговойНакладной) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.Приложение2КНалоговойНакладной.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.Приложение2КНалоговойНакладной);
...
Функция СформироватьПечатнуюФормуПриложение2КНалоговойНакладной(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь) Экспорт	
	...
	|	Приложение2КНалоговойНакладной.Договор.Дата КАК ДатаДоговора,
	|	Приложение2КНалоговойНакладной.Договор.Номер КАК НомерДоговора,
	|	ISNULL(Приложение2КНалоговойНакладной.Договор.ФорвардныйКонтракт, ЛОЖЬ) КАК ФорвардныйКонтракт,
	|	Приложение2КНалоговойНакладной.НомерГТД КАК НомерГТД,
	|	Приложение2КНалоговойНакладной.ДатаГТД КАК ДатаГТД,
	...
	|	ВЫБОР КОГДА ISNULL(ВложенныйЗапрос.КодУКТВЭД.РежимЭкспортногоОбеспечения, Ложь) И СУММА(ВложенныйЗапрос.Вес) <> 0 И СУММА(ВложенныйЗапрос.КоличествоУпаковок) <> 0
	...
	|	ВЫБОР КОГДА ISNULL(ВложенныйЗапрос.КодУКТВЭД.РежимЭкспортногоОбеспечения, Ложь) И СУММА(ВложенныйЗапрос.Вес) <> 0 И СУММА(ВложенныйЗапрос.ИзменениеКоличества) <> 0 
	|		ТОГДА СУММА(ВложенныйЗапрос.Вес) ИНАЧЕ СУММА(ВложенныйЗапрос.ИзменениеКоличества) КОНЕЦ КАК ИзменениеКоличества,
	|	ВЫБОР КОГДА ISNULL(ВложенныйЗапрос.КодУКТВЭД.РежимЭкспортногоОбеспечения, Ложь) И СУММА(ВложенныйЗапрос.Вес) <> 0 И СУММА(ВложенныйЗапрос.ИзменениеЦены) <> 0
	|		ТОГДА ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаБезНДС) / СУММА(ВложенныйЗапрос.Вес) КАК Число(15,2)) ИНАЧЕ СУММА(ВложенныйЗапрос.ИзменениеЦены) КОНЕЦ КАК ИзменениеЦеныБезНДС,
	...
	|		КОНЕЦ КАК КоличествоУпаковок,
	|		Приложение2КНалоговойНакладнойТовары.Вес КАК Вес,
	//|		ВЫБОР
	//|			КОГДА Приложение2КНалоговойНакладнойТовары.ЭтоКорректировкаКоличества
	...
	|		ВЫБОР КОГДА Приложение2КНалоговойНакладнойТовары.Ссылка.Дата < &ГраницаИспользованияНомеровГТД
	|			ТОГДА Приложение2КНалоговойНакладнойТовары.НомерГТД.НомерГТД.Код
	|			ИНАЧЕ Неопределено КОНЕЦ КАК НомерГТД,
	|		ВЫБОР КОГДА Приложение2КНалоговойНакладнойТовары.Ссылка.Дата < &ГраницаИспользованияНомеровГТД
	|			ТОГДА Приложение2КНалоговойНакладнойТовары.НомерГТД.НомерГТД.Дата
	...
	|		КОНЕЦ,
	|		Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Вес КАК Вес,
	|		ВЫБОР
	|			КОГДА Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.ЭтоКорректировкаКоличества
	...
	|		ВЫБОР КОГДА Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Ссылка.Дата < &ГраницаИспользованияНомеровГТД
	|			ТОГДА Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.НомерГТД.НомерГТД.Код
	|			ИНАЧЕ Неопределено КОНЕЦ КАК НомерГТД,
	|		ВЫБОР КОГДА Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Ссылка.Дата < &ГраницаИспользованияНомеровГТД
	|			ТОГДА Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.НомерГТД.НомерГТД.Дата
	|			ИНАЧЕ Неопределено КОНЕЦ  КАК ДатаГТД,
	...
	Пока ДанныеПечати.Следующий() цикл
		...
		Если НДСОбщегоНазначенияСервер.ПечатнаяФорма2024(ДанныеПечати.Дата, ДанныеПечати.Организация) Тогда
			СуффиксПечатнойФормыП2    = "2024";
			НомерПоХронологииФормыП2  = 10;
		...
		ИначеЕсли НДСОбщегоНазначенияСервер.ПечатнаяФорма2022(ДанныеПечати.Дата, ДанныеПечати.Организация) Тогда
			...
			НомерПоХронологииФормыП2  = 7; //это не ошибка, процедура печати не изменилась
		...
		Если НомерПоХронологииФормыП2 >= 4 Тогда
			Если НомерПоХронологииФормыП2 = 10 Тогда
            	СтруктураПоказателей = СформироватьПечатнуюФорму2024(ДанныеПечати, СтрокиПоТоварам, Макет, ТабличныйДокумент, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы);
			ИначеЕсли НомерПоХронологииФормыП2 = 9 Тогда
		...
		Для Страница = 1 По 2 Цикл
			...
			// Номер налоговой
			ТекНомерНалоговой = Прав("       " + СокрЛП(ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.НомерНалоговойНакладной, Истина, Истина)), 7);
			Для Инд = 1 По 7 Цикл                                                                 
			...
			// Номер
			ТекНомер = Прав("        " + СокрЛП(ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Истина)), 8);
			Для Инд = 1 По 8 Цикл                                                                 
			...
			Иначе
				...
				Если НомерПоХронологииФормыП2 >= 2 Тогда
					...
					ИначеЕсли (НЕ (ДанныеПечати.ПоставкаДипПредставительству = Истина И ДанныеПечати.ТипПричиныНевыдачиПокупателю <=2))
					   И (НЕ НДСОбщегоНазначенияСервер.ОрганизацияКонтрагентПлательщикНДС(ДанныеПечати.Контрагент, ДанныеПечати.Дата)) Тогда
...
Функция СформироватьПечатнуюФорму2018(ДанныеПечати, СтрокиПоТоварам, Макет, ТабличныйДокумент, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы)
	...
	Для Каждого СтрокаТовары Из СтрокиПоТоварам Цикл	
		...
		ОбластьМакета.Параметры.ИзменениеСуммыБезНДС = НДСОбщегоНазначенияСервер.ФорматСумм(СтрокаТовары.ИзменениеСуммыБезНДС, ,"0,00");
		РасчИзменениеСуммыНДС6зн = Окр(ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СтрокаТовары.ИзменениеСуммыБезНДС, СтрокаТовары.СтавкаНДС, Ложь), 6 );
...
Функция СформироватьПечатнуюФорму2024(ДанныеПечати, СтрокиПоТоварам, Макет, ТабличныйДокумент, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы)
	...
	Если ДанныеПечати.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.РаботыОтНерезидента Тогда
		...
		Если НЕ ЗначениеЗаполнено(Страна) Тогда
			ПредупрежденияПечати.Вставить("СтранаКонтрагент","" + ДанныеПечати.Контрагент + ": " + НСтр("ru='не указана страна регистрации контрагента-нерезидента (в контактной информации) - страна регистрации в налоговой накладной заполнена не будет';uk='не вказано країну реєстрації контрагента-нерезидента (у контактній інформації) - країна реєстрації в податковій накладній заповнена не буде'"));
		Иначе
			НазваниеОрганизации = НазваниеОрганизации + ", " + Страна;
		КонецЕсли;
	...
	// филиал/спецрежим налоговой
	ТекНомерФилиалНалоговой  = КодСпецРежима + ФилиалПродавца;
	Если НЕ ЗначениеЗаполнено(ТекНомерФилиалНалоговой) Тогда
	...
	// филиал/спецрежим налоговой
	Для Инд = 1 По 5 Цикл                                                                 
		ОбластьМакета.Параметры["НомерФилиалНалоговой_"+Инд]    = СокрЛП(Сред(ТекНомерФилиалНалоговой, Инд, 1)); 
	КонецЦикла;		
	...
	Если ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Истина Тогда
			...
			СтруктураПоказателей.НалоговыйДокумент.Вставить("HPODFILL",	ДанныеПечати.ДатаНалоговойНакладной);
			СтруктураПоказателей.НалоговыйДокумент.Вставить("HPODNUM",	Прав(ТекНомерНалоговой,7));
			СтруктураПоказателей.НалоговыйДокумент.Вставить("HPODNUM1",	СокрЛП(Лев(ТекНомерФилиалНалоговой, 1))); 
			СтруктураПоказателей.НалоговыйДокумент.Вставить("HPODNUM2",	СокрЛП(Сред(ТекНомерФилиалНалоговой, 2))); 
	...
	Если ДанныеПечати.ТипПричиныНевыдачиПокупателю = 22 Тогда
		...
		Если СтрокиПоТоварам.Количество() > 2 Тогда
			ПредупрежденияПечати.Вставить("КоличествоСтрок", НСтр("ru='в разделе Б должна быть не больше двух строк';uk='у розділі Б має бути не більше двох рядків'"));
		КонецЕсли; 
	...
	Для Каждого СтрокаТовары Из СтрокиПоТоварам Цикл	
		...
		ИначеЕсли СтрокаТовары.КодВид = Перечисления.ВидыКодовДляНалоговойНакладной.КодТовараИмпортированного Тогда
			ОбластьМакета.Параметры.Импортированный = "X";
		ИначеЕсли СтрокаТовары.КодВид = Перечисления.ВидыКодовДляНалоговойНакладной.КодСХПродукции Тогда
		...
		Если ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Истина Тогда
			...
			СтрокаСтруктуры.G5  = ОбластьМакета.Параметры.ИзменениеКоличества;
			СтрокаСтруктуры.G6  = ОбластьМакета.Параметры.ЦенаБезНДС;
			Если Не ЗначениеЗаполнено(СтрокаСтруктуры.G5) Тогда
				СтрокаСтруктуры.G6  = "";	
				СтрокаСтруктуры.G7  = ОбластьМакета.Параметры.ИзменениеЦеныБезНДС;
			Иначе
				СтрокаСтруктуры.G7  = "";
			КонецЕсли;

			СтрокаСтруктуры.G8  = ОбластьМакета.Параметры.Количество;
			Если Не ЗначениеЗаполнено(СтрокаСтруктуры.G7) Тогда
				СтрокаСтруктуры.G8  = "";	
			КонецЕсли;                 
...
Функция ОпределитьКолонкуТаблицы(СтавкаНДС=Неопределено, Дата='00010101')
	...
	Если СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(Перечисления.СтавкиНДС.НДС0) Тогда
		Возврат "0";
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ВидОперации, СтатьяДекларацииНДСНалоговыеОбязательства, ТипПричиныНевыдачиПокупателю = Неопределено, КодПричины = Неопределено) Экспорт
	...
	ИначеЕсли ТипПричиныНевыдачиПокупателю = 22 Тогда
		
		Если КодПричины = 501 Тогда
			СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОЭкспортОтдВидовСХПродукции;
		ИначеЕсли КодПричины = 502 Тогда
			СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОКорректировка_ЭкспОтдВидовСХПрод;
		ИначеЕсли КодПричины = 503 Тогда
			СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОКорректировка_ЭксОтдВидСХПрИзмНДС;
		КонецЕсли;
...
Процедура ЗаполнитьСуммыРеглВТабличныхЧастях(Объект, ИмяТЧ = Неопределено) Экспорт
	...
	Для каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
		...
		Для каждого СтрокаТаблицы ИЗ Объект[ТабличнаяЧасть] Цикл
			...
			Если СтрокаТаблицы[ИмяСуммыБезНДС] < 0 И СтрокаТаблицы.НомерСтрокиНН <> 0 И СтрокаТаблицы.СуммаБезНДСРегл < 0 Тогда
				Продолжить;
			КонецЕсли;
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.Приложение2КНалоговойНакладной.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3178f584-5a91-496a-9e2c-9fa198d9847f");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.Приложение2КНалоговойНакладной.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	...
	Изменяемые.Добавить(Метаданные.Документы.Приложение2КНалоговойНакладной.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.Приложение2КНалоговойНакладной.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.Приложение2КНалоговойНакладной";
	...
	|				Документ.Приложение2КНалоговойНакладной.Товары КАК Приложение2КНалоговойНакладнойТовары
	|			ГДЕ
	|				Приложение2КНалоговойНакладной.Ссылка = Приложение2КНалоговойНакладнойТовары.Ссылка
	|				И Приложение2КНалоговойНакладнойТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И Приложение2КНалоговойНакладнойТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|				И Приложение2КНалоговойНакладной.Ссылка.Проведен
	|			)
	|	ИЛИ 
	|	ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|			ГДЕ
	|				Приложение2КНалоговойНакладной.Ссылка = Приложение2КНалоговойНакладнойТовары.Ссылка
	|				И (Приложение2КНалоговойНакладнойТовары.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|	    			И Приложение2КНалоговойНакладнойТовары.УдалитьОбъектРасчетов НЕ В (&ПустыеЗначенияОбъектРасчетов)
	|				)
	|				И Приложение2КНалоговойНакладной.Ссылка.Проведен
	|			)    
	|	ИЛИ 
	...
	|				Документ.Приложение2КНалоговойНакладной.ТоварыПоДаннымПользователя КАК Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя
	|			ГДЕ
	|				Приложение2КНалоговойНакладной.Ссылка = Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Ссылка
	|				И Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|				И Приложение2КНалоговойНакладной.Ссылка.Проведен
	|			)
	|	ИЛИ 
	|	ИСТИНА В
	...
	|			ГДЕ
	|				Приложение2КНалоговойНакладной.Ссылка = Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Ссылка
	|				И (Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|	    			И Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.УдалитьОбъектРасчетов НЕ В (&ПустыеЗначенияОбъектРасчетов)
	|				)
	|				И Приложение2КНалоговойНакладной.Ссылка.Проведен
	|			)
	|";
