...
Процедура ПроверитьЗаполнениеТелефона(Адресат, Отказ)
	...
	Если СтрРазделить(Адресат.КакСвязаться, ";", Ложь).Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 указан более чем один номер телефона.';uk='У рядку %1 вказано більше ніж один номер телефону.'"), Адресат.НомерСтроки),
			,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Адресаты", Адресат.НомерСтроки, "КакСвязаться"),
			,
			Отказ);
	...
	Если Не Взаимодействия.КорректноВведенНомерТелефона(Адресат.КакСвязаться) Тогда
		ОбщегоНазначения.СообщитьПользователю(
            ...
            |Например, ""+38 (044) 456-78-90"".'
            |;uk='Введіть номер телефону в міжнародному форматі.
            |Допускається використовувати в номері прогалини, дужки і дефіси.
            |Наприклад, ""+38 (044) 456-78-90"".'"),
			,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Адресаты", Адресат.НомерСтроки, "КакСвязаться"),
			,
			Отказ);
