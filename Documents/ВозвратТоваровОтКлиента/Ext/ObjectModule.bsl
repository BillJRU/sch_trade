...
Функция ВременныеТаблицыДанныхДокумента(ОтборПоСпособуОпределенияСебестоимости = "Все") Экспорт
	...
	|	ТаблицаТоваров.СебестоимостьНДСРегл КАК СебестоимостьНДСРегл,
	|	ТаблицаТоваров.СебестоимостьНДСУпр КАК СебестоимостьНДСУпр,
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Параметры_Взаиморасчеты = Документы.ВозвратТоваровОтКлиента.ПараметрыВзаиморасчеты(ХозяйственнаяОперация, СпособКомпенсации);
	...
	Для Каждого СтрТабл Из Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрТабл.ДокументРеализации)
			И (СтрТабл.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПередачи
				...
				ТекстОшибки = НСтр("ru='Не заполнена колонка ""Документ продажи""';uk='Не заповнена колонка ""Документ продажу""'");
...
Процедура ЗаполнитьДокументНаОснованииРеализацииТоваровИУслуг(Знач ДокументОснование)
	...
	ТаблицаТоваров 				= ПакетЗапросов[4].Выгрузить();
	ТаблицаПредыдущихВозвратов 	= ПакетЗапросов[5].Выгрузить();
	...
	Для Каждого СтрокаВозвратов Из ТаблицаПредыдущихВозвратов Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаВозвратов);
		СтрокиПродаж = ТаблицаТоваров.НайтиСтроки(СтруктураПоиска);
		
		Для Каждого СтрокаПродажи Из СтрокиПродаж Цикл
			
			Если СтрокаВозвратов.Количество <= 0 Тогда
				Прервать;
			КонецЕсли;
			
			Если СтрокаВозвратов.Количество > СтрокаПродажи.Количество Тогда
			...
			Иначе
				КоличествоНаВычет 	= СтрокаВозвратов.Количество;
				СуммаНаВычет 		= СтрокаВозвратов.Сумма;
				СуммаНДСНаВычет 	= СтрокаВозвратов.СуммаНДС;
				СуммаСНДСНаВычет 	= СтрокаВозвратов.СуммаСНДС;
			КонецЕсли;
			
			СтрокаПродажи.Количество 	= СтрокаПродажи.Количество - КоличествоНаВычет;
			СтрокаВозвратов.Количество 	= СтрокаВозвратов.Количество - КоличествоНаВычет;
			
			СтрокаПродажи.Сумма 		= СтрокаПродажи.Сумма - СуммаНаВычет;
			СтрокаВозвратов.Сумма 		= СтрокаВозвратов.Сумма - СуммаНаВычет;
			
			СтрокаПродажи.СуммаНДС 		= СтрокаПродажи.СуммаНДС - СуммаНДСНаВычет;
			СтрокаВозвратов.СуммаНДС 	= СтрокаВозвратов.СуммаНДС - СуммаНДСНаВычет;
			
			СтрокаПродажи.СуммаСНДС 	= СтрокаПродажи.СуммаСНДС - СуммаСНДСНаВычет;
			СтрокаВозвратов.СуммаСНДС 	= СтрокаВозвратов.СуммаСНДС - СуммаСНДСНаВычет;
...
Процедура ЗаполнитьДокументНаОснованииПереданнойТары(Знач РеквизитыЗаполнения)
	...
	Если ЭтоАдресВременногоХранилища(РеквизитыЗаполнения.АдресТарыВоВременномХранилище) Тогда
		...
		Для каждого ТекущаяСтрока Из Товары Цикл
			...
			ТекущаяСтрока.СтавкаНДС = Справочники.СтавкиНДС.НеНДС;
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	Для каждого СтрокаТовара из ТаблицаНакладная Цикл
		
		СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтотОбъект);
		
		Если СтрокаТовара.КоличествоДоИзменения <> СтрокаТовара.Количество Тогда
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ОплатаВВалюте        = ВзаиморасчетыСервер.ПолучитьОплатуВВалютеПоУмолчанию(Валюта);
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Иначе
		...
		ПоДокументамПродажи = Товары.НайтиСтроки(Новый Структура("СпособОпределенияСебестоимости", Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПродажи));
		...
		Если ЕстьПоДокументамПродажи Тогда
			...
			Если Не Проведен
				...
				Если ЕстьБезДокументовПродажи Тогда
					ВидыЗапасовБезДокументовПродажи = ВидыЗапасов.Выгрузить(Новый Структура("СпособОпределенияСебестоимости",
																				Перечисления.СпособыОпределенияСебестоимости.Вручную));
					ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ВидыЗапасовБезДокументовПродажи, ВидыЗапасов.Выгрузить(Новый Структура("СпособОпределенияСебестоимости",
																				Перечисления.СпособыОпределенияСебестоимости.ИзТекущегоДокумента)));
					ВидыЗапасов.Очистить();																
				КонецЕсли;
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	ИменаРеквизитов = "Организация, ДокументРеализации, Дата";
...
Функция ПроверитьИзменениеТоваров(МенеджерВременныхТаблиц)
	...
	|	ИЛИ СУММА(ТаблицаТоваров.СебестоимостьНДСРегл) <> 0
	|	ИЛИ СУММА(ТаблицаТоваров.СебестоимостьНДСУпр) <> 0
