...
Процедура РассчитатьСостояние(Знач ЗаявкаНаВозвратТоваровОтКлиента,
	...
	Если ЗначениеЗаполнено(ЗаявкаНаВозвратТоваровОтКлиента) И ПолучитьФункциональнуюОпцию("ИспользоватьРасширенныеВозможностиЗаказаКлиента")
		...
		Запрос = Новый Запрос("
			...
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрНакопления.РасчетыСКлиентами.Остатки(,ОбъектРасчетов.Объект = &РасчетныйДокумент) КАК РасчетыСКлиентамиОстатки
			|ПО
			|	ИСТИНА
			|
			|ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|ПО
		|	ДокументЗаявка.Ссылка = РасчетыСКлиентамиОстатки.ОбъектРасчетов.Объект
		|	
		|ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ПараметрыВзаиморасчеты(ХозяйственнаяОперация = Неопределено, СпособКомпенсации = Неопределено) Экспорт
	...
	|	И ИсточникДанных.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя)
	...
	СтруктураПараметров.ИзменяетПланОтгрузкиПоставки     = ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера
															И ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратОтХранителя
															И СпособКомпенсации = Перечисления.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары;
...
Функция ТекстЗапросаВтКонтрольОплаты(Запрос, ТекстыЗапроса)
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	МИНИМУМ(ВЫБОР
		|		КОГДА (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению)
		|					ИЛИ ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке))
...
Функция ПравилаПечатиЗаданияНаОтборРазмещение() Экспорт
	...
	ПравилаПечатиЗадания.ИмяТЧТовары = "ЗаменяющиеТовары";
...
Функция СформироватьПечатнуюФормуЗаявкиНаВозврат(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода)
	...
	|ГДЕ
	|	ВозвращаемыеТовары.Ссылка В(&МассивОбъектов) И ВозвращаемыеТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
 	...
 	ЗаполнитьТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент, Запрос, ОбъектыПечати, ПараметрыВывода);
...
Процедура ЗаполнитьТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент, Запрос, ОбъектыПечати, ПараметрыВыводаПФ)
	...
	ПакетРезультатовЗапроса = Запрос.ВыполнитьПакет();
 	РезультатЗапроса = ПакетРезультатовЗапроса[1]; // РезультатЗапроса
	ДанныеПечати = РезультатЗапроса.Выбрать();
	РезультатПоТабличнойЧастиВозвращаемыеТовары = ПакетРезультатовЗапроса[9];
...
Процедура ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент,
	Макет, Таблица, ИмяКолонкиКодов, ИмяКолонкиТоваров,
 	ДанныеПечати, ПараметрыВывода, ПараметрыВыводаПФ)
	...
	Для Каждого Строка Из ТЧ Цикл
		...
		Если  ИмяКолонкиТоваров = НСтр("ru='Заменяющий товар';uk= 'Товар, яким заміняється'", КодЯзыкаПечать) Тогда
			...
			ПараметрыПредставлениеНоменклатурыДляПечати.КодЯзыка = КодЯзыкаПечать;
		...
		Если НЕ ПараметрыВывода.ТолькоКоличество Тогда
			Если ПараметрыВывода.УчитыватьНДС И ПоказыватьНДС Тогда
				СтруктураДанныхСтроки.Вставить("СтавкаНДС",МультиязычностьУкр.ПолучитьЛокализованноеПредставление(Строка.СтавкаНДС, КодЯзыкаПечать));
			КонецЕсли;
		КонецЕсли;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВыбиратьВариантВыводаСкидокПриПечатиДокументовПродажи") Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ЗаявкаНаВозврат", НСтр("ru='Заявка на возврат (выводить скидки)';uk='Заявка на повернення (виводити знижки)'"), 1);
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ЗаявкаНаВозвратБезСкидок", НСтр("ru='Заявка на возврат (не выводить скидки)';uk='Заявка на повернення (не виводити знижки)'"), 0);
	...
	Если ПолучитьФункциональнуюОпцию("НеВыбиратьВариантВыводаСкидокПриПечатиДокументовПродажи") Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ЗаявкаНаВозврат", НСтр("ru='Заявка на возврат';uk='Заявка на повернення'"), 1);
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм,
		...
		НСтр("ru='Заявление на возврат товаров';uk='Заява на повернення товарів'"),0);
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Представление,
 			СформироватьПечатнуюФормуЗаявкиНаВозврат(КомплектПечати.Объекты, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода),,,,Истина);
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
 			Обработки.ПечатьЗаявленияНаВозвратТоваровОтКлиента.СформироватьПечатнуюФормуЗаявленияНаВозвратТоваровОтКлиента(СтруктураТипов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода),,,,Истина);
	КонецЦикла;
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ad390e99-4380-4cd5-92a9-8ae0986a6b40");
	...
	Обработчик.Комментарий = НСтр("ru='Исправляет некорректно заполненный реквизит ""Способ доставки"".
                                   |Заполняет реквизиты ""Назначение"" и ""Вариант приемки товаров"".
                                   ...
                                   |;uk='Виправляє некоректно заповнений реквізит ""Спосіб доставки"".
                                   |Заповнює реквізити ""Призначення"" та ""Варіант приймання товарів"".
	...
	Изменяемые.Добавить(Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПолноеИмя());
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ЗаявкаНаВозвратТоваровОтКлиента";
	НоваяСтрока.Порядок    = "После";
	НоваяСтрока = Исключения.Добавить();
...
#Область ОбработатьДанныеДляГенерацииНазначений
	...
	Обработчик.Комментарий = НСтр("ru='Создает новые назначения связанные с документом ""Заявка на возврат товаров от клиента""';uk='Створює нові призначення, пов''язані з документом ""Заявка на повернення товарів від клієнта""'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ЗаявкаНаВозвратТоваровОтКлиента.СгенерироватьОбъектыРасчетов";
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("b5acb3c8-723f-4d4f-b0f4-b63952b36f5a");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаявкаНаВозвратТоваровОтКлиента.ЗарегистрироватьДанныеКГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваровОтКлиента
	|ГДЕ ЗаявкаНаВозвратТоваровОтКлиента.УдалитьПорядокОплаты = Значение(Перечисление.УдалитьПорядокОплатыПоСоглашениям.ПустаяСсылка)
	|	ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ОтОтправителяОпределяетСлужбаДоставки)
	|");
	...
	|	ИЗ
	|		(ВЫБРАТЬ
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.Ссылка КАК Ссылка,
	|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.НомерСтроки) КАК Строк,
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ВариантОплаты КАК ВариантОплаты,
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ДатаПлатежа КАК ДатаПлатежа
	|		ИЗ
	|			Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЭтапыГрафикаОплаты КАК ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты
	|		ГДЕ
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.Ссылка.Проведен
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.Ссылка,
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ВариантОплаты,
	|			ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ДатаПлатежа
	|		
	|		ИМЕЮЩИЕ
	|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.НомерСтроки) > 1) КАК ВложенныйЗапрос) КАК ЗаявкаНаВозвратТоваровОтКлиента";
	...
	|		ГДЕ
	|			ЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка
	|			И ((ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.ИдентификаторСтроки = """"))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары
	|		ГДЕ
	|			ЗаявкаНаВозвратТоваровОтКлиента.Ссылка = ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.Ссылка
	|			И ((ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.ИдентификаторСтроки = """")))
	|	ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.ДатаПлатежа
	|	ИМЕЮЩИЕ
	|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаявкаНаВозвратТоваровОтКлиентаЭтапыГрафикаОплаты.НомерСтроки) > 1) КАК ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Ссылка
	|;
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		ПО ДанныеДляОбработки.НаправлениеДеятельности = НаправленияДеятельности.Ссылка
	|			И (НаправленияДеятельности.УчетЗатрат)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаказы КАК Заказы
	|			ПО Заказы.Заказ = ДанныеДляОбработки.Ссылка
	|
	|";
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда 
				...
				МассивТЧ.Добавить("ВозвращаемыеТовары");
				МассивТЧ.Добавить("ЗаменяющиеТовары");
				...
				ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ДокументОбъект.ВозвращаемыеТовары, ОбъектИзменен);
				ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ДокументОбъект.ЗаменяющиеТовары, ОбъектИзменен);
				#КонецОбласти
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Заявки на возврат';uk='Заявки на повернення'");
