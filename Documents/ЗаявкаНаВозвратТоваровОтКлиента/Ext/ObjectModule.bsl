...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Параметры_Взаиморасчеты = Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПараметрыВзаиморасчеты(ХозяйственнаяОперация, СпособКомпенсации);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И АвторасчетНДС И НДСИсходящийСервер.НуженАвторасчетНДС(ВозвращаемыеТовары) Тогда
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И АвторасчетНДС И НДСИсходящийСервер.НуженАвторасчетНДС(ЗаменяющиеТовары) Тогда
		// соответствие для хранения погрешностей округлений
		ПогрешностиОкругления = Новый Соответствие();
		// пересчет сумм НДС с учетом ошибок округления
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ВозвращаемыеТовары);
		ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ЗаменяющиеТовары);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для ТекИндекс = 0 По ВозвращаемыеТовары.Количество()-1 Цикл
		...
		АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""Возвращаемые товары""';uk='в рядку%НомерСтроки% списку ""Повернуті товари""'");
		...
		Если ЗначениеЗаполнено(ТекущаяСтрокаВозвращаемыеТовары.ДатаПоступления) И ТекущаяСтрокаВозвращаемыеТовары.ДатаПоступления < НачалоДня(Дата) Тогда
	...
	Если СпособКомпенсации = Перечисления.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары Тогда
		...
		Для ТекИндекс = 0 По ЗаменяющиеТовары.Количество()-1 Цикл
			...
			АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""Заменяющие товары""';uk='в рядку%НомерСтроки% списку ""Замінюючі товари""'");
			АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", ТекущаяСтрокаЗаменяющиеТовары.НомерСтроки);

			Если Не ТекущаяСтрокаЗаменяющиеТовары.Отменено 
				И ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения <> НеТребуется
				И ((Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована
							Или Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату)
						И (ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = СоСклада
							Или ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = Отгрузить
							Или ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = ОтгрузитьОбособленно
							Или ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = ИзЗаказов)
					Или Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению
					Или Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОтгрузке
					Или Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена) Тогда

				Если Не ЗначениеЗаполнено(ТекущаяСтрокаЗаменяющиеТовары.ДатаОтгрузки) И Не НеОтгружатьЧастями Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						...
						ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ЗаменяющиеТовары",
							ТекущаяСтрокаЗаменяющиеТовары.НомерСтроки, "ДатаОтгрузки"),
						,
						Отказ);
			...
			Если Не ТекущаяСтрокаЗаменяющиеТовары.Отменено И (
					(Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована
							Или Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату)
						И (ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = СоСклада
							ИЛИ ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = Отгрузить
							ИЛИ ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = ОтгрузитьОбособленно
							Или ТекущаяСтрокаЗаменяющиеТовары.ВариантОбеспечения = ИзЗаказов)) Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовКлиентов")
				И ТекущаяСтрокаЗаменяющиеТовары.Отменено И
				Не ЗначениеЗаполнено(ТекущаяСтрокаЗаменяющиеТовары.ПричинаОтмены) Тогда
...
Функция ПоместитьСуммыПоЗаказамВоВременноеХранилище() Экспорт
	...
	Запрос.УстановитьПараметр("ВернутьМногооборотнуюТару", ВернутьМногооборотнуюТару);
	Запрос.УстановитьПараметр("ТребуетсяЗалогЗаТару", ТребуетсяЗалогЗаТару);
	Запрос.УстановитьПараметр("Таблица", ЗаменяющиеТовары);
...
Процедура КонтрольЗаполненияЦенВозвращаемыхТоваров(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ЕстьОшибкиЗаполненияЦен Тогда
			...
			ТекстОшибки = НСтр("ru='Не заполнена колонка ""Цена"" в строке %НомерСтроки% списка ""Возвращаемые товары""';uk='Не заповнена колонка ""Ціна"" в рядку %НомерСтроки% списку ""Товари що повертаються ""'");
