...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ЗаполнятьПоОрдеру
		...
		Если ЭтаФорма.ТолькоПросмотр Тогда    	
			ТекстСообщения = НСтр("ru='Нельзя перезаполнить документ ""%Документ%"", т.к. установлена дата запрета изменений!';uk='Неможна перенаповнений документ ""%Документ%"", тому що встановлена дата заборони змін!'");
	...
	Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
		Элементы.ТоварыПодобратьТоварыПоЗаказамОрдерам.Заголовок = НСтр("ru='Подобрать товары из заказов';uk='Підібрати товари з замовлень'");
	КонецЕсли;
	...
	Параметры_Взаиморасчеты = Документы.ПриобретениеТоваровУслуг.ПараметрыВзаиморасчеты(Объект.ХозяйственнаяОперация, 
...
|;uk='Обрано оферту з таким варіантом приймання, при якому придбання буде проводитися в рамках оферти в цілому.
|Можливість створення документів з такими офертами залишена для того, що б закрити вже розпочаті руху за офертами.
|Створювати нові документи з такими офертами не рекомендується.'");
...
Процедура ТекстАвансовыйОтчетОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОчиститьАвансовыйОтчет" Тогда
		...
		Объект.АвансовыйОтчет = Неопределено;
		Модифицированность = Истина;
		ТекстАвансовыйОтчет = Новый ФорматированнаяСтрока(НСтр("ru='Включить в авансовый отчет';uk='Включити до авансового звіту'"),,,, "ВключитьВАвансовыйОтчет");
...
Процедура ПодобратьТовары(Команда)
    ...
    ХозяйственныеОперацииКомиссии = ХозяйственныеОперацииКомиссии();
    ...
    Если ХозяйственныеОперацииКомиссии.Найти(Объект.ХозяйственнаяОперация) <> Неопределено Тогда
...
Процедура ИзменитьКачествоНоменклатуры(Команда)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.КодСтроки) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Для строки по заказу запрещено изменять качество. Воспользуйтесь командой ""Отвязать строки от заказа"".';uk='Для рядка за замовленням заборонено змінювати якість. Скористайтеся командою ""Відв''язати рядки від замовлення"".'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьИзВнешнегоФайла(Команда)
    ...
    ХозяйственнаяОперацияПриемНаКомиссию = (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаКомиссию") ИЛИ Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаКомиссиюИмпорт"));
...
Процедура УстановитьУсловноеОформление()
	...
	// Снятие отметки незаполненного подразделения, если Товар или Работа по агентской закупке
    ...
    УчетНДСУП.УстановитьУсловноеОформлениеСуммНДСПоСтавкеНДСЗакупки(ЭтаФорма);
...
Процедура СоглашениеПриИзмененииСервер(ПересчитыватьЦены=Истина)
	...
	Если ЗначениеЗаполнено(Объект.Соглашение) Тогда
		...
		Если ПересчитыватьЦены Тогда
            ...
            ПараметрыЗаполненияЦены.Вставить("НалогообложениеНДС", НалогообложениеНДСПоУмолчанию);
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер(ВызовПоКоманде = Истина)
	...
	ПараметрыВзаиморасчеты = Документы.ПриобретениеТоваровУслуг.ПараметрыВзаиморасчеты(Объект.ХозяйственнаяОперация, 
		Объект.ПоступлениеПоЗаказам, 
		ЕстьКорректировки);
...
Процедура ПоступлениеПоНесколькимЗаказамПриИзменении()
	...
	ИзмененныеРеквизиты = ВзаиморасчетыСервер.ФормаПриИзмененииРеквизитов(ЭтаФорма, "ПоступлениеПоЗаказам");
	Если ИзмененныеРеквизиты.Свойство("ПорядокРасчетов") Тогда
		УстановитьСвойстваЭлементовПоПорядкуРасчетов();
	КонецЕсли;
...
|;uk='На підставі документа введено коригування придбання. 
|Недостатньо прав для створення коригування надходження.'");
		КонецЕсли;
	...
	ИначеЕсли ЕстьКорректировки И ИспользоватьКорректировкиПриобретений Тогда
		...
		СписокКнопок.Добавить("ВвестиКорректировку", НСтр("ru='Ввести корректировку';uk='Ввести коригування'"));
		СписокКнопок.Добавить("ОтменитьИзменения", НСтр("ru='Отмена';uk='Відмінити'"));
		
		ПоказатьВопрос(
			Оповещение,
			НСтр("ru='На основании документа введена корректировка приобретения.
|Изменение исходного документа запрещено.'
|;uk='На підставі документа введена коригування придбання. 
|Зміну вихідного документа заборонено.'"),
			СписокКнопок,
			,
			"ВвестиКорректировку");
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
    ...
    ХозяйственныеОперацииКомиссии = ХозяйственныеОперацииКомиссии();
	ПоступлениеПоКомиссии = 
		ХозяйственныеОперацииКомиссии.Найти(Объект.ХозяйственнаяОперация) <> Неопределено;
	
    Если ПоступлениеПоРаздельнойЗакупке ИЛИ ПоступлениеПоКомиссии Тогда
...
Процедура РассчитатьИтоговыеПоказателиПоступления(Форма)
    ...
    Если ХозяйственныеОперацииИмпорта.Найти(Форма.Объект.ХозяйственнаяОперация) <> Неопределено Тогда
	...
	Иначе
        ...
        ЗакупкаОблагаетсяНДС = НЕ (Форма.НалогообложениеНДСПоУмолчанию  = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС") ИЛИ Форма.НалогообложениеНДСПоУмолчанию  = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОсвобожденаОтНДС"));
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	МассивЭлементов.Добавить("ПоступлениеПоЗаказу");
	МассивЭлементов.Добавить("ТоварыЗаполнитьНоменклатуруПоНоменклатуреПоставщика");
	МассивЭлементов.Добавить("ФормаНеВыполнятьПроверкуСопоставленнойНоменклатурыПоставщика");
	МассивЭлементов.Добавить("ТоварыПодобратьТоварыПоЗаказамОрдерам");
	МассивЭлементов.Добавить("ТоварыНоменклатураПоставщикаКод");
	МассивЭлементов.Добавить("ТоварыНоменклатураПоставщикаАртикул");
	МассивЭлементов.Добавить("ТоварыЗаполнитьЦеныПоСоглашению");
	МассивЭлементов.Добавить("ТоварыЗаполнитьЦеныВыделенныхСтрокПоВидуЦеныПоставщика");
	МассивЭлементов.Добавить("ТоварыЗаполнитьЦеныПоПоступлениям");
...
Процедура УстановитьОбязательностьПоставщика()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо
        И НалогообложениеНДСПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
    ...
    СтруктураДействийИзмененныеСтроки.Вставить("ЗаполнитьСтавкуНДС", Новый Структура("НалогообложениеНДС, Дата", НалогообложениеНДСПоУмолчанию, Объект.Дата));
...
&НаСервереБезКонтекста
Функция ХозяйственныеОперацииКомиссииСервер()
	
	Возврат Документы.ПриобретениеТоваровУслуг.ХозяйственныеОперацииКомиссии();
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ХозяйственныеОперацииКомиссии()
	
	Возврат ХозяйственныеОперацииКомиссииСервер();
