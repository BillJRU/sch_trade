...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И МассивСтрок.Количество() > 0 Тогда
		...
		Если ЗавершениеРаботы Тогда
			...
			ТекстПредупреждения = НСтр("ru='Есть выбранные товары, не перенесенные в документ. Продолжить закрытие?';uk='Є вибрані товари, що не перенесені в документ. Продовжити закриття?'");
		КонецЕсли;
...
Процедура ПеренестиВДокумент()
	...
	СтруктураЗначенияВыбора = Новый Структура("АдресПлатежейВХранилище, НалоговоеНазначение",
...
Процедура ЗаполнитьТаблицуТоваров()
	...
	|				И АналитикаУчетаНоменклатуры.МестоХранения = &Склад) КАК ТоварыОрганизаций
	|ГДЕ
	...
	|				И НалоговоеНазначение = &НалоговоеНазначение
	|				И АналитикаУчетаНоменклатуры.МестоХранения = &Склад) КАК Себестоимость
	|ГДЕ
