...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обработать анкету: %1 по причине:
                    |%2'
                    |;uk='Не вдалося обробити анкету: %1 з причини:
                    |%2'"), 
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ЗаполнитьРеквизитРежимАнкетирования не удалось обработать некоторые анкеты (пропущены): %1';uk='Процедурі ЗаполнитьРеквизитРежимАнкетирования не вдалося обробити деякі анкети (пропущені): %1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.Анкета,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Процедура ЗаполнитьРеквизитРежимАнкетирования обработала очередную порцию анкет: %1';uk='Процедура ЗаполнитьРеквизитРежимАнкетирования опрацювала чергову порцію анкет: %1'"),
					ОбъектовОбработано));
	КонецЕсли;
