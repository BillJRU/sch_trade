	...
	Если Объект.КроссТаблица Тогда
		...
		Для каждого Период Из ТаблицаПериоды Цикл
			...
			Если Объект.Периодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год") Тогда
				КолонкиПоиска.Добавить(СокрЛП(СтрЗаменить(Период.Заголовок, НСтр("ru='г.';uk='р.'"), "")),
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	ПараметрыОтбора.Вставить(
		"Номенклатура.ТипНоменклатуры", НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Ложь));
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План внутренних потреблений +15%';uk='План внутрішніх споживань + 15%'"),
		"[ПланыВнутреннихПотреблений] * 1.15");

	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров,
		НСтр("ru='План внутренних потреблений с учетом корректировки по факту внутренних потреблений';uk='План внутрішніх споживань з урахуванням коригування за фактом внутрішніх споживань'"),
		"Макс([ФактыВнутреннихПотреблений], [ПланыВнутреннихПотреблений])"
		+ ?(ИспользоватьПланыОстатков," - [ПланыОстатков]",
		?(ПланированиеПоНазначениям," - [ФактическиеНачальныеОстатки]"," - [СвободныеОстатки]")));

	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров,
		НСтр("ru='Факт внутренних потреблений с учетом наличия товаров на остатках';uk='Факт внутрішніх споживань з урахуванням наявності товарів на залишках'"),
		"[ФактыВнутреннихПотреблений] / [ДоляДнейНаличияТовараНаОстатках]");
		...
		Если ДополнительныеПоля <> Неопределено Тогда
			...
			#Если Клиент Тогда
				РаботаСФормуламиПроверитьФормулу = РаботаСФормуламиКлиент.ПроверитьФормулу(СтруктураНастроек.Формула,
					МассивОперандов,,,,,ДополнительныеПараметры);
			#Иначе
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьСклад,
		|ПланированиеПоНазначениям, ЗаполнятьНазначениеВТЧ");
	...
	Для каждого Период Из ТаблицаПериоды Цикл
		Для Каждого СтрокаТоваров Из Форма.ТоварыПоПериодам Цикл
			...
			Если Форма.ИспользуютсяПланируемыеОстатки Тогда
				...
				Планирование.ВычислитьРассчитываемыеОперанды(СтрокаТоваров, Период, ПредыдущийПериод,
					НовыйПланОстатковСОтбором, "СОтбором_", ОсновнойОперандПлана, ЗнакКорректировки);
			КонецЕсли;
...
Процедура ТоварыПоПериодамПриАктивизацииЯчейкиНаКлиенте()
	...
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		Для каждого Период Из АктивныеПериоды Цикл
			Если Элемент.ТекущийЭлемент.Имя = "ТоварыПоПериодамКоличество_"+Период.ИмяКолонки Тогда
				ЯчейкаОтменена = ТекущиеДанные["Отменено_"+ Период.ИмяКолонки];
				КомментарийКЯчейке = ТекущиеДанные["Комментарий_"+ Период.ИмяКолонки];
				ЯчейкаНайдена = Истина;
				Прервать;
