...
Функция ПроверитьЗаполнениеПолейОтбора()
	...
	Если Не ЗначениеЗаполнено(Объект.ХозяйственнаяОперация) Тогда
		Текст = НСтр("ru='Для заполнения начислений требуется указание хозяйственной операции.';uk='Для заповнення нарахувань потрібне зазначення господарської операції.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст,, "ХозяйственнаяОперация", "Объект");
		Результат = Ложь;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		Текст = НСтр("ru='Для заполнения начислений требуется указание организации.';uk='Для заповнення нарахувань потрібне зазначення організації.'");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихАктивовПассивов") Тогда
		Элементы.СтатьяДоходовРасходов.Заголовок = НСтр("ru='Статья доходов / расходов';uk='Стаття доходів / витрат'");
	КонецЕсли;
...
Процедура ПолучитьНачисленияИзХранилища(АдресНачисленийВХранилище)
	...
	Если Объект.Начисления.Количество() = 0 Тогда
		...
		ОрганизацияЗаПериод = НСтр("ru='Для организации ""%1"" за период с %2 по %3';uk='Для організації ""%1"" за період з %2 по %3'");
