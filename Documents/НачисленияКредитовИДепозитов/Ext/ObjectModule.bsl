...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ДатаНеВПериоде Тогда
			Текст = СтрШаблон(НСтр("ru='Дата начисления %1 в строке %2 не входит в заданный период начисления.';uk='Дата нарахування %1 в рядку %2 не входить в заданий період нарахування.'"),
				Формат(Выборка.ДатаНачисления, "ДЛФ=D"), Выборка.НомерСтроки);
			ПолеТабЧасти = "Дата";
		ИначеЕсли Выборка.НеСовпадаютОрганизации Тогда
			Текст = СтрШаблон(НСтр("ru='Организация договора %1 в строке %2 не совпадает с организацией %3, выбранной в документе.';uk='Організація договору %1 в рядку %2 не збігається з організацією %3, яку обрано у документі.'"),
