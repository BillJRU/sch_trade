...
Процедура ЗаполнитьПоОтобраннымТоварам(Команда)
	...
	Если (Модифицированность Или Не Объект.Проведен) И Объект.ОтгружаемыеТовары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Перед заполнением документ будет проведен, а табличная часть - очищена. Продолжить?';uk='Перед заповненням документ буде проведений, а таблична частина - очищена. Продовжити?'");
