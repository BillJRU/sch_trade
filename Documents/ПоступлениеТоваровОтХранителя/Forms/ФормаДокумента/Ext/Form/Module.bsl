...
Процедура ОткрытьПодбор(Команда)
	...
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru='поступление товаров от хранителя';uk='надходження товарів від зберігача'"));
	КонецЕсли;
...
Процедура ПодобратьПереданныеТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Партнер) Тогда
		ТекстОшибки = НСтр("ru='Поле ""Партнер"" не заполнено';uk='Поле ""Партнер"" не заповнено'");
...
Процедура ПерезаполнитьПоПриемке(Команда)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Количество перезаполнено';uk='Кількість перезаповнено'");
		ТекстПояснения = НСтр("ru='В строках перезаполнено количество.';uk='У рядках перезаповнено кількість.'");
...
Процедура ЗаполнитьЦеныПоВидуЦены()
	...
	Если ЗаполнитьЦены Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВидЦены) Тогда
			ТекстПредупреждения = НСтр("ru='В документе не указан вид цены. Цены не могут быть заполнены';uk='У документі не зазначено вид ціни. Ціни не можуть бути заповнені'");
...
Процедура УстановитьСвойстваЭлементов()
	...
	МассивЭлементовНДС = Новый Массив;
	МассивЭлементовНДС.Добавить("ТоварыСтавкаНДС");
	МассивЭлементовНДС.Добавить("ТоварыСуммаНДС");
	МассивЭлементовНДС.Добавить("ТоварыСуммаСНДС");
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, МассивЭлементовНДС, "Видимость", Ложь);
...
Процедура ЗаполнитьНомераГТДПоПереданнымТоварамНаСервере()
	...
	НоменклатураСервер.ЗаполнитьНоменклатуруГТД(Объект.Товары, "ПоОстаткамУХранителя", ПараметрыЗаполнения);
