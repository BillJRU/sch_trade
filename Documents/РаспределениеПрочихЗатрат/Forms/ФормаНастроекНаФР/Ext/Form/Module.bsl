...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПоказательРаспределения.СписокВыбора.Добавить("ВыручкаОтПродаж", НСтр("ru='Выручка от продаж';uk='Виторг від продажів'"));
	Элементы.ПоказательРаспределения.СписокВыбора.Добавить("СебестоимостьПродаж", НСтр("ru='Себестоимость продаж';uk='Собівартість продажів'"));
	Элементы.ПоказательРаспределения.СписокВыбора.Добавить("ВаловаяПрибыль", НСтр("ru='Валовая прибыль';uk='Валовий прибуток'"));
...
Процедура ОбработатьИзмененияОтбораПоНаправлениямДеятельности(Форма)
	...
	Элементы.ПредставлениеУказанныхНД.Заголовок = 
		СформироватьПредставлениеОтбора("НаправленияДеятельности", Форма.ОтборПоНаправлениямДеятельности.ВыгрузитьЗначения(), 
			НСтр("ru='направление деятельности, направления деятельности, направлений деятельности';uk='напрям діяльності, напрямки діяльності, напрямків діяльності'"));
...
Процедура ВариантОтраженияВУчетеПриИзмененииНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(ВариантОтраженияВУчете) Тогда
		ВариантОтраженияВУчете = "ВУправленческомУчете";
	КонецЕсли; 
	
	Объект.УправленческийУчет = (ВариантОтраженияВУчете = "ВУправленческомУчете") ИЛИ (ВариантОтраженияВУчете = "ВУправленческомИРегламентированномУчетах");
			
	Объект.РегламентированныйУчет = (ВариантОтраженияВУчете = "ВРегламентированномУчете") ИЛИ (ВариантОтраженияВУчете = "ВУправленческомИРегламентированномУчетах");
...
Функция ПредставлениеОтбора(СписокОтбора, ПредметИсчисления, Гиперссылка)
	...
	ПодстрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru='(Изменить)';uk='(Змінити)'"),,,, Гиперссылка);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Объект.УправленческийУчет И Объект.РегламентированныйУчет Тогда
		ВариантОтраженияВУчете = "ВУправленческомИРегламентированномУчетах";
	ИначеЕсли Объект.УправленческийУчет Тогда
		ВариантОтраженияВУчете = "ВУправленческомУчете";
	ИначеЕсли Объект.РегламентированныйУчет Тогда
		ВариантОтраженияВУчете = "ВРегламентированномУчете";
	КонецЕсли; 
...
Процедура УстановитьЗаголовокФормы()
	...
	УточнениеПредставленияОбъекта = НСтр("ru='между направлениями деятельности';uk='між напрямами діяльності'");
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		ШаблонЗаголовка = НСтр("ru='%1 %2 (создание)';uk=' %1 %2 (створення)'");
		Заголовок = СтрШаблон(ШаблонЗаголовка, ПредставлениеОбъекта, УточнениеПредставленияОбъекта);
		
	Иначе
		
		ШаблонЗаголовка = НСтр("ru='%1 %2 №%3 от %4';uk=' %1 %2 № %3 з %4'");
