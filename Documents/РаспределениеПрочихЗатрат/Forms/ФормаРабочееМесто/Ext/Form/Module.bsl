...
Процедура СтатьиРасходовСформироватьДокументы(Команда)
	...
	Если Элементы.СтатьиРасходов.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выделено ни одной строки.';uk='Не виділено жодного рядка.'"));
		Возврат;
...
Функция ОписанияФоновыхЗаданий()
	...
	ОписаниеЗаданий.Вставить("СформироватьДокументы",
		ХарактеристикиФоновогоЗадания(НСтр("ru='Формирование документов распределения расходов.';uk='Формування документів розподілу витрат.'"),
			"Документы.РаспределениеПрочихЗатрат.СформироватьДокументы",
			,
			Истина));
		
	ОписаниеЗаданий.Вставить("ЗаполнитьСтатьиРасходов",
		ХарактеристикиФоновогоЗадания(НСтр("ru='Заполнение статей расходов.';uk='Заповнення статей витрат.'"),
			"Документы.РаспределениеПрочихЗатрат.СтатьиКРаспределениюВФоне"));
			
	ОписаниеЗаданий.Вставить("ОбновитьГиперссылкуКОформлению",
		ХарактеристикиФоновогоЗадания(НСтр("ru='Получение данных к оформлению.';uk='Отримання даних до оформлення.'"),
...
Процедура ОбработатьРезультатФормированияДокументов(АдресХранилища)
	...
	Для Каждого РезультатПоОрганизации Из Результат Цикл
		...
		ШаблонСформированоДокументов = НСтр("ru='По организации %1 сформировано %2 документов распределения.';uk='По організації %1 сформовано %2 документів розподілу.'");
...
Функция ОписаниеОперацийИТиповДокументов()
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru='Распределение расходов будущих периодов';uk='Розподіл витрат майбутніх періодів'");
...
Функция ОпределитьПараметрыОткрытияФормыДокумента(ТекущиеДанные, ПараметрыОткрытия)
	...
	ВариантыОтвета.Добавить(ПредопределенноеЗначение("Перечисление.РазрезыУчета.Регламентированный"), 
		НСтр("ru='Для регламентированного учета';uk='Для регламентованого обліку'"));
	ВариантыОтвета.Добавить(ПредопределенноеЗначение("Перечисление.РазрезыУчета.Управленческий"), 
		НСтр("ru='Для управленческого учета';uk='Для управлінського обліку'"));
	ВариантыОтвета.Добавить(Неопределено, НСтр("ru='Отмена';uk='Відмінити'"));
...
|;uk='Правила розподілу в упр. і регл. обліку розрізняються. 
|Для якого виду обліку відкрити настройку правила розподілу?'"), 
		ВариантыОтвета);
