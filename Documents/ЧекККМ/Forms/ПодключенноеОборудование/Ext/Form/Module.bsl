...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		...
		ПодключаемоеОборудованиеПараметры = глПодключаемоеОборудование.ПараметрыПодключенияПО; // Массив из см. Справочники.ПодключаемоеОборудование.ПолучитьДанныеУстройства
...
Процедура Подключить(Команда)
	...
	Если ТекущиеДанные.Подключено = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Оборудование уже подключено.';uk='Обладнання вже підключено.'"));
		Возврат;
	КонецЕсли;
...
Процедура Отключить(Команда)
	...
	Если ТекущиеДанные.Подключено = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Оборудование не подключено.';uk='Обладнання не підключено.'"));
		Возврат;
	КонецЕсли;
...
|""%ОписаниеОшибки%"".'
|;uk='При підключенні пристрою %Устройство% сталася помилка: 
|""%ОписаниеОшибки%"".'");
...
|""%ОписаниеОшибки%"".'
|;uk='При відключенні пристрою %Устройство% сталася помилка: 
|""%ОписаниеОшибки%"".'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", Данные.ОписаниеОшибки);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Устройство%",     СтрокаТЧ.Ссылка);
