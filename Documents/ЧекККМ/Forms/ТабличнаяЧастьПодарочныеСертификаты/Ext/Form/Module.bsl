...
Процедура ПодарочныеСертификатыПриИзменении(Элемент)
	...
	СуммаОплатыПослеИзменения = 0;
	Для Каждого СтрокаТЧ Из ПодарочныеСертификаты Цикл
		СуммаОплатыПослеИзменения = СуммаОплатыПослеИзменения + СтрокаТЧ.Сумма;
	КонецЦикла;
	
	ДельтаИзменения = СуммаОплаты - СуммаОплатыПослеИзменения;
	СуммаОплаты = СуммаОплаты - ДельтаИзменения;
	
	ОсталосьДоплатить = Доплатить + ДельтаИзменения;
