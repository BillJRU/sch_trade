...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КнопокСоздания = 1 Тогда
		...
		ЭлементСоздания.Заголовок = НСтр("ru='Создать';uk='Створити'");
	...
	Параметры_Взаиморасчеты = Документы.АвансовыйОтчет.ПараметрыВзаиморасчеты(Объект.Статус);
	...
	ВидимостьСозданияПоступленияДД = Ложь;
...
|;uk='Сума витрачених коштів дорівнює 0. 
|Необхідно заповнити дані на закладці ""Купівлі"", ""Витрати"", ""Оплата постачальникам"" або ""Конвертація валюти""'");
		Иначе
			Текст = НСтр("ru='Сумма израсходованных денежных средств равна 0.
|Необходимо заполнить данные на закладке ""Закупки"", ""Расходы "" или ""Оплата поставщикам""'
|;uk='Сума витрачених коштів дорівнює 0. 
|Необхідно заповнити дані на закладці ""Купівлі"", ""Витрати"" або ""Оплата постачальникам""'");
		КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если ЗадаватьВопрос Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Очистить суммы взаиморасчетов?';uk='Очистити суми взаєморозрахунків?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
...
Процедура ВалютаПриИзмененииКлиент()
	...
	ОчитститьТЧОплатаПоставщикам = Ложь;
	...
	Если ТекущаяВалюта <> Объект.Валюта
		...
		Для Каждого СтрокаТаблицы Из Объект.ОплатаПоставщикам Цикл
			Если СтрокаТаблицы.ВалютаВзаиморасчетов = Объект.Валюта Тогда
				Продолжить;
			Иначе 
				ОчитститьТЧОплатаПоставщикам = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	ОчитститьТЧЗакупки = Ложь;
	Если ТекущаяВалюта <> Объект.Валюта
		И ЗначениеЗаполнено(Объект.Валюта)		
		И ЗначениеЗаполнено(СписокДокументовЗакупки) Тогда
			ОчитститьТЧЗакупки = ПолучитьЗначениеДокументыТЧЗакупки();
	КонецЕсли;

	Если ОчитститьТЧЗакупки И ОчитститьТЧОплатаПоставщикам Тогда 
		ПредставлениеТЧ = НСтр("ru='Оплата поставщикам';uk='Оплата постачальникам'");
		ПредставлениеТЧЗакупки = НСтр("ru='Закупки';uk='Купівлі'");
		
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Табличные части ""%1"" и ""%2"" будут очищены, продолжить?';uk='Табличні частини ""%1"" та ""%2"" будуть очищені, продовжити?'"),
			ПредставлениеТЧ, ПредставлениеТЧЗакупки);
	...
	ИначеЕсли ОчитститьТЧОплатаПоставщикам Тогда 	
		ПредставлениеТЧ = НСтр("ru='Оплата поставщикам';uk='Оплата постачальникам'");
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВопросВалютаНеСовпадаетЗавершение", ЭтотОбъект, ), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	ИначеЕсли ОчитститьТЧЗакупки Тогда
		ПредставлениеТЧ = НСтр("ru='Закупки';uk='Купівлі'");
...
&НаСервере
Функция ПолучитьЗначениеДокументыТЧЗакупки()
	
	ВыбранныеДокументы = ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(СписокДокументовЗакупки.ВыгрузитьЗначения());
	ОчитститьТЧЗакупки = Ложь;
	Для каждого Элемент Из ВыбранныеДокументы Цикл
		Если Элемент.Валюта = Объект.Валюта Тогда
			Продолжить;
		Иначе 
			ОчитститьТЧЗакупки = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;	
	Возврат ОчитститьТЧЗакупки; 
...
Процедура ВопросВалютаНеСовпадаетЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если КодОтвета = КодВозвратаДиалога.Да Тогда
		...
		Если ЗначениеЗаполнено(СписокДокументовЗакупки) Тогда
	        ДокументыЗакупкиОчистить = Новый Массив;
			Для каждого Элемент Из СписокДокументовЗакупки Цикл
				ДокументыЗакупкиОчистить.Добавить(Элемент.Значение);
			КонецЦикла;	
			УдалитьДокументыЗакупкиИзСписка(ДокументыЗакупкиОчистить);
		КонецЕсли;
...
Процедура ВалютаПриИзмененииКлиентЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Текст = СтрШаблон(НСтр("ru='Суммы в документе пересчитаны в валюту %1';uk='Суми у документі перераховані у валюту %1'"), Объект.Валюта);
...
Процедура СтатусПриИзмененииНаСервере()
	...
	ПараметрыВзаиморасчеты = Документы.АвансовыйОтчет.ПараметрыВзаиморасчеты(Объект.Статус);
	...
	УстановитьВидимостьПрочиеРасходыГруппаЗаполнитьВСтроках();
...
Процедура УстановитьВидимостьНалоговыхНазначений()
	...
	НДСОбщегоНазначенияСервер.УстановитьВидимостьСуммаНДСПропорционально(ЭтаФорма, Ложь, (НЕ Объект.Мультивалютный) И (НЕ Объект.Валюта = ВалютаРеглУчета));
...
&НаСервере
Процедура УстановитьВидимостьПрочиеРасходыГруппаЗаполнитьВСтроках()

	Элементы.ПрочиеРасходыГруппаЗаполнитьВСтроках.Видимость = (Объект.Статус = Перечисления.СтатусыАвансовогоОтчета.Утвержден);
...
Процедура ПрочиеРасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		ТекущиеДанные.СтавкаНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.НеНДС");
...
Процедура ПрочиеРасходыЭтоСуточныеПриИзменении(Элемент)	
	...
	Если ТекущаяСтрока.ЭтоСуточные = Истина Тогда
		...
		ТекущаяСтрока.СтавкаНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.НеНДС");
...
//<< a.daliuk 23.08.2020
//>> a.daliuk - конец изменений 23.08.2020
...
Процедура ЗаполнитьПодразделение(Команда)
	...
	ИначеЕсли ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение( , НСтр("ru='В списке ""Расходы"" не выделено ни одной строки.';uk='У списку ""Витрати» не виділено жодної рядки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗакупкиСоздатьПТиУ(Команда)
	...
	ЗначенияЗаполнения.Вставить("Валюта", Объект.Валюта);
	ЗначенияЗаполнения.Вставить("ВалютаВзаиморасчетов", Объект.Валюта);
...
|;uk='Авансовий звіт ще не збережено.
|Створення документа купівлі можливо тільки після запису авансового звіту.
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = Справочники.СтавкиНДС.НеНДС;
	...
	//6ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	//>> a.daliuk - конец изменений 23.08.2020
...
Процедура РассчитатьСуммуДокумента(Форма)
	...
	Если Не Объект.Мультивалютный Тогда
		...
		Если Форма.СуммаПолученоИтог < Форма.СуммаИзрасходовано Тогда
			...
			Форма.Элементы.ОстатокПерерасход.Заголовок = НСтр("ru='Перерасход';uk='Перевитрата'");
		Иначе
			Форма.ОстатокПерерасход = Форма.СуммаПолученоИтог - Форма.СуммаИзрасходовано;
			Форма.Элементы.ОстатокПерерасход.Заголовок = НСтр("ru='Остаток';uk='Залишок'");
		КонецЕсли;
	...
	Иначе
		...
		Для каждого КлючИЗначение Из ИтогиПоВалютам Цикл
			...
			Если СтрокаИтогов.ПолученоИтог + СтрокаИтогов.Конвертировано <= СтрокаИтогов.Израсходовано Тогда
				СтрокаИтогов.Перерасход = СтрокаИтогов.Израсходовано - СтрокаИтогов.ПолученоИтог - СтрокаИтогов.Конвертировано;
				СтрокаИтогов.Остаток = 0;
			Иначе
				СтрокаИтогов.Перерасход = 0;
				СтрокаИтогов.Остаток = СтрокаИтогов.ПолученоИтог + СтрокаИтогов.Конвертировано - СтрокаИтогов.Израсходовано;
			КонецЕсли;
...
Процедура ЗаполнитьСуммуПолучено()
	...
	Иначе
		...
		КонецЦикла;
		Таблица.Свернуть("Валюта, Курс, Растяжка", "Получено, ПолученоИтог, Израсходовано, Перерасход, Отклонено, Остаток, Конвертировано, СуммаЗакупок");
...
Процедура АвтоТест_ЗаполнитьПолученныеАвансыЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Объект.ПрочиеРасходы.Количество() = 0 Тогда
		Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
			Текст = СтрШаблон(
				НСтр("ru='У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в подразделении %2 в валюте %3';uk='У підзвітної особи відсутні видані аванси, за які він не відзвітував перед організацією %1 в підрозділі %2 у валюті %3'"),
				Объект.Организация, Объект.Подразделение, Объект.Валюта);
		Иначе
			Текст = СтрШаблон(
				НСтр("ru='У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в валюте %2 без указания подразделения';uk='У підзвітної особи відсутні видані аванси, за які він не відзвітував перед організацією %1 у валюті %2 без зазначення підрозділу'"),
...
Процедура ЗаполнитьПолученныеАвансы()
	...
	Объект.ПрочиеРасходы.Загрузить(Запрос.Выполнить().Выгрузить());
	
	Для каждого Строка Из Объект.ПрочиеРасходы Цикл
		ДоходыИРасходыКлиентСервер.ПриДобавленииСтрокиВТаблицу(ЭтотОбъект, Строка, "Объект.ПрочиеРасходы");
	КонецЦикла;
...
Процедура ЗаполнитьКонвертациюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.КонвертацияВалюты.Количество() = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У подотчетного лица отсутствуют денежные средства от организации %1';uk='У підзвітної особи відсутні кошти від організації %1'"),
			Объект.Организация);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
...
Функция СтрокаМультивалютныеСуммы(ИтогиПоВалютам)
	...
	Если Не СтрокаОстаток.Количество() И Не СтрокаПерерасход.Количество() Тогда
		ИтоговаяСтрока = Новый ФорматированнаяСтрока(НСтр("ru='Остаток -';uk='Залишок -'"));
	Иначе
		ИтоговаяСтрокаМассив = Новый Массив;
		
		Если СтрокаОстаток.Количество() Тогда
			ИтоговаяСтрокаМассив.Добавить(НСтр("ru='Остаток';uk='Залишок'") + " ");
			ИтоговаяСтрокаМассив.Добавить(СтрокаОстаток);
		КонецЕсли;
		
		Если СтрокаПерерасход.Количество() Тогда
			ИтоговаяСтрокаМассив.Добавить(НСтр("ru='Перерасход';uk='Перевитрата'") + " ");
