...
Процедура ВывестиДополнительнуюИнформациюПоНоменклатуре(Номенклатура = Неопределено , Характеристика = Неопределено, Упаковка = Неопределено)
	...
	Если ЗначениеЗаполнено(РеквизитыНоменклатуры.ФайлКартинки) Тогда
		...
		Исключение
			АдресКартинки = "";
			Элементы.АдресКартинки.ТекстНевыбраннойКартинки = НСтр("ru='Ошибка при чтении файла изображения';uk='Помилка при читанні файлу зображення'");
			Элементы.АдресКартинки.ЦветТекста 			   = ЦветаСтиля.ПоясняющийОшибкуТекст;
		КонецПопытки;
	Иначе
		АдресКартинки = "";
		Элементы.АдресКартинки.ТекстНевыбраннойКартинки = НСтр("ru='Изображение отсутствует';uk='Зображення відсутнє'");
...
Функция ПараметрыДляПечати(ВыделенныеСтроки, ЕстьОшибки = Ложь)
	...
	Для Каждого Идентификатор Из ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТоваров.Номенклатура) Тогда
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Номенклатура"" в строке %НомерСтроки% списка ""Товары"".';uk='Не заповнена колонка ""Номенклатура"" в рядку %НомерСтроки% списку ""Товари"".'");
...
Процедура Автотест_ЗаполнитьПоРаспоряжению(ВидЗаполнения) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.Распоряжение)Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано распоряжение.';uk='Не зазначене розпорядження.'"));
		Возврат;
	КонецЕсли;
	...
	Если ВидЗаполнения = "Количество" Тогда
		...
		Если Объект.Товары.Итог("КоличествоУпаковок") <> 0 Тогда
			...
			Если ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры Тогда 
				ТекстСообщения = НСтр("ru='В некоторых строках количество уже заполнено и будет перезаполнено остатками по распоряжению, заполненные серии будут очищены. Продолжить?';uk='У деяких рядках кількість вже заповнено і буде перенаповнено залишками за розпорядженням, заповнені серії будуть очищені. Продовжити?'");
			Иначе
				ТекстСообщения = НСтр("ru='В некоторых строках количество уже заполнено и будет перезаполнено остатками по распоряжению. Продолжить?';uk='У деяких рядках кількість вже заповнена і буде перезаповнена залишками за розпорядженням. Продовжити?'");
			КонецЕсли;
	...
	Иначе
		...
		Если Объект.Товары.Количество() > 0 Тогда
			...
			Если ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры Тогда 
				ТекстСообщения = НСтр("ru='Перед заполнением табличная часть будет очищена, указанные серии будут очищены. Продолжить?';uk='Перед заповненням таблична частина буде очищена, зазначені серії будуть очищені. Продовжити?'");
			Иначе
				ТекстСообщения = НСтр("ru='Перед заполнением табличная часть будет очищена. Продолжить?';uk='Перед заповненням таблична частина буде очищена. Продовжити?'");
			КонецЕсли;			
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		Если ЗначениеЗаполнено(СтрокаТоваров.УпаковочныйЛист)
			...
			ТекстСообщения = НСтр("ru='%УпаковочныйЛист% уже был добавлен в табличную часть.';uk='%УпаковочныйЛист% вже був доданий до табличної частини.'");
...
Функция ПроверитьТоварыИПоместитьВоВременноеХранилище(АдресХранилища)
	...
	ШаблонСообщения = НСтр("ru='Для открытия обработки проверки количества товаров в ордере необходимо заполнить номенклатуру в строке %НомерСтроки%.';uk='Для відкриття обробки перевірки кількості товарів в ордері необхідно заповнити номенклатуру в рядку %НомерСтроки%.'");
