...
Процедура ЗаполнитьДоходыПоОстаткам() Экспорт
	...
	|	ПрочиеДоходы.СуммаУпрБезНДСОстаток КАК СуммаУпрБезНДС,
	|	ПрочиеДоходы.СуммаРеглБезНДСОстаток КАК СуммаРеглБезНДС,
	|
	...
	|	ИЛИ ПрочиеДоходы.СуммаУпрБезНДСОстаток <> 0
	|	ИЛИ ПрочиеДоходы.СуммаРеглБезНДСОстаток <> 0
	|");
...
Процедура ЗаполнитьРасходыПоОстаткам() Экспорт
    ...
    |			И Расходы.НалоговоеНазначение = ПрочиеРасходы.НалоговоеНазначение
	|ГДЕ
    ...
    |	СУММА(Остатки.СуммаРеглБезНДСОстаток) КАК СуммаРеглБезНДСОстаток,
    |	СУММА(Остатки.НДСРеглОстаток) КАК НДСРеглОстаток,
    |	СУММА(Остатки.СуммаУпрБезНДСОстаток) КАК СуммаУпрБезНДСОстаток,
    ...
    |		Остатки.СуммаРеглБезНДСОстаток КАК СуммаРеглБезНДСОстаток,
    |		Остатки.НДСРеглОстаток КАК НДСРеглОстаток,
    |		Остатки.СуммаУпрБезНДСОстаток КАК СуммаУпрБезНДСОстаток,
    ...
    |		КОГДА Статья.ВариантРаспределенияРасходовУпр
    |				= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности)
    |			ТОГДА Остатки.СуммаУпрБезНДСОстаток
    ...
    |		КОГДА Статья.ВариантРаспределенияРасходовРегл
    |				= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности)
    |			ТОГДА Остатки.СуммаРеглБезНДСОстаток
    |		ИНАЧЕ 0 КОНЕЦ) КАК СуммаРеглБезНДС,
    |	(ВЫБОР
    |		КОГДА Статья.ВариантРаспределенияРасходовРегл
    |				= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности)
    |			ТОГДА Остатки.НДСРеглОстаток
    ...
    |		И Остатки.НалоговоеНазначение = Аналитика.НалоговоеНазначение
	|ГДЕ
	|	(ВЫБОР
	|		КОГДА Статья.ВариантРаспределенияРасходовУпр
	|				= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности)
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		Расходы 		= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "Расходы");
		Аналитика 		= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "Аналитика");
		ОстаткиРасходов	= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(Запрос, "ОстаткиРасходов");
	КонецЕсли;
