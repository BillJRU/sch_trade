...
|%2'
|;uk='При проведенні документа ""%1"" були виявлені помилки:
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
					СокрЛП(ДокументРаспределения),
					РасчетСебестоимостиПротоколРасчета.ПредставлениеМногострочногоТекста(ТекстОшибки, " - "));
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемуВыполненияРасчета(
				...
				НСтр("ru='При распределение доходов и расходов по организации ""%1"" за период %2 были диагностированы ошибки';uk='При розподілі доходів і витрат по організації ""%1"" за період %2 були діагностовані помилки'",ОбщегоНазначения.КодОсновногоЯзыка()),
				,
				ТекстОшибки,
...
Процедура Использование_РаспределениеДоходовПоНаправлениямДеятельности(ПараметрыОбработчика) Экспорт
	...
	|	ИЛИ Доходы.СуммаУпрБезНДСПриход <> 0
	|	ИЛИ Доходы.СуммаРеглБезНДСПриход <> 0
	|	ИЛИ Доходы.СуммаУпрБезНДСРасход <> 0
	|	ИЛИ Доходы.СуммаРеглБезНДСРасход <> 0
	|
	|";
	...
	Если РезультатЗапроса.Пустой() И Не ЕстьЗаданиеКРасчету Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Нет оборотов по регистрам прочих доходов за период %1';uk='Немає оборотів по регістрах інших доходів за період %1'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|%2'
|;uk='Розподіл доходів по напрямах діяльності за період %1 завершилося з помилкою: 
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОписаниеПроверок_РаспределениеДоходовПоНаправлениямДеятельности(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не распределены прочие доходы на финансовый результат';uk='Не розподілені інші доходи на фінансовий результат'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Все прочие доходы должны быть распределены на финансовый результат.';uk='Всі інші доходи повинні бути розподілені на фінансовий результат.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковПрочихДоходов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СтатьяДоходов", 			НСтр("ru='Статья доходов';uk='Стаття доходів'",ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаДоходов", 		НСтр("ru='Аналитика доходов';uk='Аналітика доходів'",ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТОстаткиПрочихДоходов",
		НСтр("ru='По организации ""%1"" на конец периода %2 есть остатки по регистру прочих доходов.';uk='По організації ""%1"" на кінець періоду %2 є залишки по регістру інших доходів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ШаблонПредставления = НСтр("ru='%1 %2 от %3';uk='%1 %2 від %3'");
...
Функция ПредставлениеРаннихДокументов() Экспорт
	...
	Возврат НСтр("ru='Распределение доходов и расходов';uk='Розподіл доходів і витрат'");
...
Функция ТекстЗапросаТаблицаВтПрочиеДоходы(Запрос, ТекстыЗапроса = Неопределено) Экспорт
	...
	|	,ПрочиеДоходы.СуммаУпрБезНДС КАК СуммаУпрБезНДС,
	|	ПрочиеДоходы.СуммаРеглБезНДС КАК СуммаРеглБезНДС
	|
...
Функция ТекстЗапросаТаблицаПрочиеДоходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	,СУММА(ТаблицаДоходы.ДоходыУпрБезНДС)    КАК СуммаУпрБезНДС,
	|	СУММА(ТаблицаДоходы.ДоходыРеглБезНДС)    КАК СуммаРеглБезНДС
	|ИЗ
	...
	|	ИЛИ ТаблицаДоходы.ДоходыУпрБезНДС <> 0
	|	ИЛИ ТаблицаДоходы.ДоходыРеглБезНДС <> 0
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТаблицаВтПрочиеРасходы(Запрос, ТекстыЗапроса = Неопределено) Экспорт
    ...
    |	ПрочиеРасходы.СуммаУпрБезНДС КАК СуммаУпрБезНДС,
...
Функция ТекстЗапросаТаблицаПрочиеРасходы(Запрос, ТекстыЗапроса, Регистры)
    ...
    |	СУММА(ТаблицаРасходы.РасходыУпрБезНДС)    КАК СуммаУпрБезНДС,
    |	СУММА(ТаблицаРасходы.РасходыРегл)         КАК СуммаРегл,
    |	СУММА(ТаблицаРасходы.РасходыРеглБезНДС)   КАК СуммаРеглБезНДС,
    |	СУММА(ТаблицаРасходы.РасходыНДСРегл)      КАК НДСРегл,
    ...
    |	ИЛИ ТаблицаРасходы.РасходыУпрБезНДС <> 0
    |	ИЛИ ТаблицаРасходы.РасходыРегл <> 0
    |	ИЛИ ТаблицаРасходы.РасходыРеглБезНДС <> 0
    |	ИЛИ ТаблицаРасходы.РасходыНДСРегл <> 0
...
Функция ТекстЗапросаТаблицаФинансовыеРезультаты(Запрос, ТекстыЗапроса, Регистры)
	...
	|	СУММА(Таблица.ДоходыРеглБезНДС) КАК ДоходыРеглБезНДС,
	|	СУММА(Таблица.РасходыРеглБезНДС) КАК РасходыРеглБезНДС,
	|	СУММА(Таблица.ДоходыУпрБезНДС)  КАК ДоходыУпрБезНДС,
	|	СУММА(Таблица.РасходыУпрБезНДС) КАК РасходыУпрБезНДС,
	|
	...
	|	ИЛИ СУММА(Таблица.ДоходыРеглБезНДС) <> 0
	|	ИЛИ СУММА(Таблица.РасходыРеглБезНДС) <> 0
	|	ИЛИ СУММА(Таблица.ДоходыУпрБезНДС) <> 0
	|	ИЛИ СУММА(Таблица.РасходыУпрБезНДС) <> 0
	|
	|";
...
Функция ТекстЗапросаТаблицаДоходыРасходыКРаспределению()
	...
	|	СУММА(ТаблицаДоходы.СуммаУпрБезНДС)  КАК ДоходыУпрБезНДС,
	|	СУММА(ТаблицаДоходы.СуммаРеглБезНДС) КАК ДоходыРеглБезНДС,
	|	0                                   КАК Расходы,
    |	0                                   КАК РасходыУпр,
    |	0                                   КАК РасходыУпрБезНДС,
    |	0                                   КАК РасходыРегл,
    |	0                                   КАК РасходыРеглБезНДС,
    |	0                                   КАК РасходыНДСРегл,
	...
	|	0                                       КАК ДоходыУпр,
	|	0                                       КАК ДоходыРегл,
	|	0                                       КАК ДоходыУпрБезНДС,
	|	0                                       КАК ДоходыРеглБезНДС,
	|	СУММА(ТаблицаРасходы.Сумма)             КАК Расходы,
    |	СУММА(ТаблицаРасходы.СуммаУпр)          КАК РасходыУпр,
    |	СУММА(ТаблицаРасходы.СуммаУпрБезНДС)    КАК РасходыУпрБезНДС,
    |	СУММА(ТаблицаРасходы.СуммаРегл)         КАК РасходыРегл,
    |	СУММА(ТаблицаРасходы.СуммаРеглБезНДС)   КАК РасходыРеглБезНДС,
    |	СУММА(ТаблицаРасходы.НДСРегл)           КАК РасходыНДСРегл,
	|	СУММА(ТаблицаРасходы.ПостояннаяРазница) КАК ПостояннаяРазница,
	|	СУММА(ТаблицаРасходы.ВременнаяРазница)  КАК ВременнаяРазница
	|ИЗ
...
Функция ТекстЗапросаТаблицаОборотыДоходовРасходов()
	...
	|	,СУММА(ФинансовыеРезультаты.ДоходыРеглБезНДС) КАК ДоходыРеглБезНДСОборот,
	|	СУММА(ФинансовыеРезультаты.РасходыРеглБезНДС) КАК РасходыРеглБезНДСОборот,
	|	СУММА(ФинансовыеРезультаты.ДоходыУпрБезНДС) КАК ДоходыУпрБезНДСОборот,
	|	СУММА(ФинансовыеРезультаты.РасходыУпрБезНДС) КАК РасходыУпрБезНДСОборот
	|
...
Процедура СформироватьВтФинансовыеРезультаты(Запрос, ВыводитьСообщенияВЖурналРегистрации = Ложь, Отказ = Ложь) Экспорт
	...
	ЗапросРаспределения.Текст = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		ВтПрочиеРасходы 			 = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ЗапросРаспределения.МенеджерВременныхТаблиц, "ВтПрочиеРасходы");
		ВтПрочиеДоходы 				 = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ЗапросРаспределения.МенеджерВременныхТаблиц, "ВтПрочиеДоходы");
		ТаблицаСпособовРаспределения = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ЗапросРаспределения.МенеджерВременныхТаблиц, "ТаблицаСпособовРаспределения");
		ОборотыДоходовРасходов		 = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ЗапросРаспределения.МенеджерВременныхТаблиц, "ОборотыДоходовРасходов");
	КонецЕсли;
	...
	|	Таблица.ДоходыУпрБезНДС         КАК ДоходыУпрБезНДС,
	|	Таблица.ДоходыРеглБезНДС        КАК ДоходыРеглБезНДС,
	|
	|	Таблица.Расходы                 КАК Расходы,
    |	Таблица.РасходыУпр              КАК РасходыУпр,
    |	Таблица.РасходыУпрБезНДС        КАК РасходыУпрБезНДС,
    |	Таблица.РасходыРегл             КАК РасходыРегл,
    |	Таблица.РасходыРеглБезНДС       КАК РасходыРеглБезНДС,
    |	Таблица.РасходыНДСРегл          КАК РасходыНДСРегл,
...
Функция РаспределитьДоходыИРасходыПоБазе(ТаблицаФинансовыеРезультаты, РезультатЗапросаПоБазе, СсылкаНаДокумент, ВыводитьСообщенияВЖурналРегистрации, Отказ)
	...
	НаборЗаписей.Колонки.Добавить("НалоговоеНазначение", Метаданные.РегистрыНакопления.ПрочиеРасходы.Измерения.НалоговоеНазначение.Тип);
    ...
    НаборЗаписей.Колонки.Добавить("РасходыНДСРегл", Метаданные.РегистрыНакопления.ДвиженияДоходыРасходыПрочиеАктивыПассивы.Ресурсы.СуммаУпр.Тип);
	...
	СуммовыеРесурсы = Новый Структура("Доходы, ДоходыУпр, ДоходыРегл, ДоходыУпрБезНДС, ДоходыРеглБезНДС, Расходы, РасходыУпр, РасходыУпрБезНДС, РасходыРегл, РасходыРеглБезНДС, РасходыНДСРегл, ПостояннаяРазница, ВременнаяРазница");
			...
			Если ВыборкаПоСпособуРаспределения.База = 0
			 ...
			 И ФормироватьФинансовыйРезультат Тогда
				...
				Если МассивСтрокТаблицы[0].Доходы <> 0
				 ...
				 ИЛИ МассивСтрокТаблицы[0].ДоходыУпр <> 0 Тогда
					...
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Отсутствует база для распределения доходов по способу распределения ""%1"". Укажите другой способ распределения или удалите строку доходов из документа.';uk='Відсутня база для розподілу доходів за способом розподілу ""%1"". Вкажіть інший спосіб розподілу або вилучіть рядок доходів з документа.'",КодЯзыка),
						ВыборкаПоСпособуРаспределения.СпособРаспределения);
					
					Если ВыводитьСообщенияВЖурналРегистрации Тогда
						ЗаписьЖурналаРегистрации(
							НСтр("ru='Ошибка при проведении документа';uk='Помилка при проведенні документа'",КодЯзыка), 
				...
				Если МассивСтрокТаблицы[0].Расходы <> 0
                 ...
                 ИЛИ МассивСтрокТаблицы[0].РасходыРеглБезНДС <> 0
                 ИЛИ МассивСтрокТаблицы[0].РасходыНДСРегл <> 0
                 ИЛИ МассивСтрокТаблицы[0].РасходыУпр <> 0
                 ИЛИ МассивСтрокТаблицы[0].РасходыУпрБезНДС <> 0
				 ИЛИ МассивСтрокТаблицы[0].ПостояннаяРазница <> 0
				 ИЛИ МассивСтрокТаблицы[0].ВременнаяРазница <> 0 Тогда
					
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Отсутствует база для распределения расходов по способу распределения ""%1"". Укажите другой способ распределения или удалите строку расходов из документа.';uk='Відсутня база для розподілу витрат за способом розподілу ""%1"". Вкажіть інший спосіб розподілу або вилучіть рядок витрат з документа.'",КодЯзыка),
						ВыборкаПоСпособуРаспределения.СпособРаспределения);
