...
Процедура Заполнить(Команда)
	...
	ИначеЕсли (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.СнятьРезерв")
		...
		ТекстПредупреждения = НСтр("ru='По данному назначению нет потребностей. Заполнение возможно только через операцию ""Произвольная корректировка""';uk='За даним призначенням немає потреб. Заповнення можливо тільки через операцію ""Довільне коригування""'");
...
Процедура ОбновитьИнформациюПоЗаказам()
	...
	ШаблонВсегоНазначений = ?(МогутБытьНазначенияБезЗаказа, НСтр("ru='Всего назначений: %1';uk='Всього призначень: %1'"), НСтр("ru='Всего заказов: %1';uk='Всього замовлень: %1'"));
