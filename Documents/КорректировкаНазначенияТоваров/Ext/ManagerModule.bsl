...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
    ...
    |			ТОГДА ЕСТЬNULL(ТаблицаВидыЗапасов.НовоеНазначение.НалоговоеНазначение,ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
    |		ИНАЧЕ
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru='В табличных частях документа в аналитике учета номенклатуры заполняется новый реквизит ""Назначение"".
                                   |Заполняет реквизит ""Вид запасов оприходование"" в табличной части ""Виды запасов"", а также реквизит ""Аналитика учета номенклатуры оприходование"" в табличных частях ""Виды запасов"" и ""Товары"" документов ""Корректировка назначения товаров"".'
                                   |;uk='У табличних частинах документа в аналітиці обліку номенклатури заповнюється новий реквізит ""Призначення"".
                                   |Заповнює реквізит ""Вид запасів оприбуткування"" у табличній частині ""Види запасів"", а також реквізит ""Аналітика обліку номенклатури оприбуткування"" у табличних частинах ""Види запасів"" та ""Товари"" документів ""Коригування призначення товарів"".'");
...
#Область СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Версия = "3.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "Документы.КорректировкаНазначенияТоваров.СгенерироватьКлючиАналитикиНоменклатуры";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.КорректировкаНазначенияТоваров.ЗарегистрироватьДанныеКОбновлениюКлючейАналитики";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("0ecb4570-a959-4adb-bd69-b3a4f9d5bbfe");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	Обработчик.БлокируемыеОбъекты = "Справочник.КлючиАналитикиУчетаНоменклатуры,"
		+ "РегистрСведений.АналитикаУчетаНоменклатуры";
	Обработчик.Комментарий = НСтр("ru='Генерирует ключи аналитики учета номенклатуры по данным документа ""Корректировка назначения товаров"". До завершения обработчика работа с документами не возможна.';uk='Генерує ключі аналітики обліку номенклатури за даними документа ""Коригування призначення товарів"". До завершення обробника робота з документами неможлива.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	НоваяСтрока.ИмяОбъекта = "Документы.КорректировкаНазначенияТоваров";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ИЛИ ВидыЗапасов.ВидЗапасовОприходование.РеализацияЗапасовДругойОрганизации)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	МАКСИМУМ(Товары.АналитикаУчетаНоменклатуры.Назначение <> Товары.ИсходноеНазначение И Товары.ИсходноеНазначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК ОбрабатыватьНазначение,
	...
	|	МАКСИМУМ(ВидыЗапасов.ВидЗапасовОприходование = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|			ИЛИ ВидыЗапасов.ВидЗапасовОприходование.РеализацияЗапасовДругойОрганизации) КАК ОбработатьВидЗапасов
	|ИЗ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
	|		ПО АналитикаУчетаНоменклатурыВидЗапасов.ВидЗапасов = ВидыЗапасов.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Попытка
			...
			ДокОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(ВыборкаПоДокументам.Ссылка,
			...
			Если ВыборкаПоДокументам.ОбработатьВидЗапасов Тогда
				...
				ТребуетсяЗаполнитьВидыЗапасовОприходования = Ложь;
				
				Пока ВыборкаПоАналитикам.Следующий() Цикл
					Если ВыборкаПоАналитикам.КоличествоВидовЗапасов <> 1
						Или ВыборкаПоАналитикам.РеализацияЗапасовДругойОрганизации Тогда
						ТребуетсяЗаполнитьВидыЗапасовОприходования = Истина;
					КонецЕсли;
					
					НайденныеСтрокиВидовЗапасов = ДокОбъект.ВидыЗапасов.НайтиСтроки(Новый Структура("АналитикаУчетаНоменклатурыОприходование",
						ВыборкаПоАналитикам.АналитикаУчетаНоменклатуры));
					Для Каждого СтрокаВидовЗапасов Из НайденныеСтрокиВидовЗапасов Цикл
						Если ТребуетсяЗаполнитьВидыЗапасовОприходования Тогда
							Если СтрокаВидовЗапасов.ВидЗапасов.РеализацияЗапасовДругойОрганизации Тогда
								СтрокаВидовЗапасов.ВидЗапасовОприходование = 
								Справочники.ВидыЗапасов.ВидЗапасовДокумента(
								СтрокаВидовЗапасов.ВидЗапасов.Организация,, 
								СтрокаВидовЗапасов.ВидЗапасов);
							Иначе
...
Функция СоздатьКорректировкуНазначенияТоваровНаОснованииЗаказаКлиентаПроверкаТипаНазначения(ОбъектОснование) Экспорт
	...
	Если ТипЗнч(ОбъектОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если ЗначениеЗаполнено(Назначение)
			...
			РезультатПроверки.ТекстОшибки = НСтр("ru='Для данного типа назначения корректировка недоступна. Необходимо переделать исходные документы.';uk='Для цього типу призначення коригування недоступне. Необхідно переробити вихідні документи.'");
