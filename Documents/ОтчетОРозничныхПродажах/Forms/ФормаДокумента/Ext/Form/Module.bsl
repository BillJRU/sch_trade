...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуНДСиАкцизногоНалога", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Объект));
...
Процедура ОткрытьВидыЗапасовВозвратыЗакрытойСмены(Команда)
	...
	ПараметрыРедактированияВидовЗапасов = ПоместитьТоварыИВидыЗапасовВХранилище(Истина);
	ФинансыКлиент.ОткрытьВидыЗапасов(ЭтотОбъект, ПараметрыРедактированияВидовЗапасов);
...
&НаСервере
Функция ПоместитьТоварыИВидыЗапасовВХранилище(ПоВозврату = Ложь)

	ПараметрыРедактированияВидовЗапасов = ЗапасыСервер.ПараметрыРедактированияВидовЗапасов();
	ПараметрыРедактированияВидовЗапасов.ИмяТЧВидыЗапасов = ?(ПоВозврату, "ВидыЗапасовПоВозвратамЗакрытойСмены", "ВидыЗапасов");
	...
	Для каждого СтрокаТоваров Из ТаблицаТоваров Цикл
		...
		Если СтрокаТоваров.КоличествоУпаковок = 0 ИЛИ ПоВозврату = (СтрокаТоваров.КоличествоУпаковок > 0) Тогда
			
			МассивСтрокКУдалению.Добавить(СтрокаТоваров);
			
		ИначеЕсли ПоВозврату Тогда
			...
			СтрокаТоваров.СуммаРучнойСкидки = - СтрокаТоваров.СуммаРучнойСкидки;
	...
	Для каждого СтрокаТоваров Из МассивСтрокКУдалению Цикл
		
		ТаблицаТоваров.Удалить(СтрокаТоваров);
	...
	ТаблицаВидовЗапасов = Объект[ПараметрыРедактированияВидовЗапасов.ИмяТЧВидыЗапасов].Выгрузить();
	
	ПараметрыРедактированияВидовЗапасов.АдресВидовЗапасовВХранилище = ПоместитьВоВременноеХранилище(
		ТаблицаВидовЗапасов,
		УникальныйИдентификатор);
		
	ЗаполнитьЗначенияСвойств(ПараметрыРедактированияВидовЗапасов, Объект, "Организация, ВидыЗапасовУказаныВручную, ЦенаВключаетНДС, Склад");

	ПараметрыРедактированияВидовЗапасов.ДокументМодифицирован = Модифицированность;
	
	ПараметрыРедактированияВидовЗапасов.РедактироватьВидыЗапасов =
		ПараметрыРедактированияВидовЗапасов.РедактироватьВидыЗапасов
		И Не ЭтаФорма.ТолькоПросмотр
		И ЭтаФорма.Доступность
...
Процедура ЗаполнитьВыручкуНаСервере(ДатаНач, ДатаКон)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСРасчетНалоговыхОбязательств.СтавкаНДС КАК СтавкаНДС,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСРасчетНалоговыхОбязательств.СтавкаНДС,
