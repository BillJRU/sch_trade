...
Процедура УстановитьЗаголовок()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Заголовок = СтрШаблон(НСтр("ru='%1 %2 от %3';uk='%1 %2 від %3'"), Объект.ТипПервичногоДокумента, СокрЛП(Объект.Номер), Объект.Дата);
	Иначе
		Заголовок = СтрШаблон(НСтр("ru='%1 (создание)';uk='%1 (створення)'"), Объект.ТипПервичногоДокумента);
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораПорядкаРасчетов(ПроверитьЗаполнение = Истина)
	...
	СписокВыбора.Добавить(Перечисления.ПорядокРасчетов.ПоНакладным, НСтр("ru='По накладным';uk='За накладними'"));
	
	Если Объект.Партнер = Справочники.Партнеры.НашеПредприятие Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыМеждуОрганизациями") Тогда
			СписокВыбора.Добавить(Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов, НСтр("ru='По договорам';uk='По договорах'"));
		КонецЕсли;
	Иначе
		Если Объект.ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ОплатаОтКлиента
				ИЛИ Объект.ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.РеализацияКлиенту Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов") Тогда
				СписокВыбора.Добавить(Перечисления.ПорядокРасчетов.ПоЗаказамНакладным, НСтр("ru='По заказам';uk='По замовленнях'"));
			КонецЕсли;
		Иначе
