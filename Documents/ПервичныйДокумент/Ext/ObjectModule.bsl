...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Документы.ПервичныйДокумент.АктуализироватьОбъектРасчетов(ЭтотОбъект, Отказ);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(Организация) И Организация = Контрагент Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Организация и контрагент не должны совпадать';uk='Організація і контрагент не повинні збігатися'"),
			ЭтотОбъект,
			"Контрагент",
			,
			Отказ);
