...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если АвторизованВнешнийПользователь Тогда
		...
		Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
			...
			Если КоличествоРеализацийОснований > 1 И НЕ ЗначениеЗаполнено(ТекущаяСтрока.Реализация) Тогда
				...
				ТекстОшибки = НСтр("ru='Не заполнено поле ""%Реализация%"" в строке %НомерСтроки% списка ""Товары""';uk='Не заповнене поле ""%Реализация%"" в рядку %НомерСтроки% списку ""Товари""'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекущаяСтрока.НомерСтроки);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Реализация%", ?(Не АктПоПередаче, НСтр("ru='Реализация';uk='Реалізація'"), НСтр("ru='Передача';uk='Передача'")));
	...
	Иначе
		...
		Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
			...
			Если Не ТекущаяСтрока.ЗаполненоПоРеализации
				И Не ЗначениеЗаполнено(ТекущаяСтрока.Сумма) Тогда
			...
			Если Не ЗначениеЗаполнено(ТекущаяСтрока.Реализация)
				...
				ИмяКолонки = ?(ТипОснованияРеализация, НСтр("ru='Реализация';uk='Реалізація'"), НСтр("ru='Возврат поставщику';uk='Повернення постачальнику'"));
				ТекстОшибки = СтрШаблон(НСтр("ru='Не заполнено поле ""%1"" в строке ""%2"" списка ""Товары""';uk='Не заповнене поле ""%1"" в рядку ""%2"" списку ""Товари""'"), ИмяКолонки, ТекущаяСтрока.НомерСтроки);
		...
		Если Не ЗначениеЗаполнено(Партнер) Тогда
			...
			Если ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратПоставщику Тогда
				ТекстОшибки = НСтр("ru='Поле ""Поставщик"" не заполнено';uk='Поле ""Постачальник"" не заповнено'");
			Иначе
				ТекстОшибки = НСтр("ru='Поле ""Клиент"" не заполнено';uk='Поле ""Клієнт"" не заповнено'");
			КонецЕсли;
	...
	Если ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.РеализацияТоваровУслуг
		ИЛИ РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(ТипОснованияАктаОРасхождении) Тогда
...
Процедура ЗаполнитьНаОсновании(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПередачаТоваровХранителю") Тогда
		...
		ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаТоваровХранителю;
