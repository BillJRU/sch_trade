...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru='УИ_Назначение';uk='UI_Призначення'"),"ИдентификаторНазначение");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Если Сценарий.ИспользоватьРасчетПоСкоростиПродаж Тогда
		...
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж +15%';uk='План продажів +15%'"),
			"[ПланыПродажПоКатегориямПоСкоростиПродаж] * 1.15");
