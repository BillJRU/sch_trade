...
Процедура ЗакрытьСмену(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Перед закрытием смены документ будет записан. Продолжить?';uk='Перед закриттям зміни документ буде записаний. Продовжити?'");
...
Процедура ЗакрытьСменуВыполнить()
	...
	ПараметрыОперации.Кассир = ?(Не СтандартнаяОбработка, Кассир, НСтр("ru='Администратор';uk='Адміністратор'")); 
...
Процедура ЗакрытьСменуПослеЗакрытияСмены(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.Результат Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Кассовая смена закрыта';uk='Касова зміна закрита'")
			,
			,
			,
	...
	Иначе
			...
			ПоказатьОповещениеПользователя(
			НСтр("ru='Ошибка закрытия кассовой смены';uk='Помилка закриття касової зміни'")
			,
			,
