...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("КассоваяСмена") Тогда
		...
		Если XMLСтрока = Неопределено Или Не ТипЗнч(XMLСтрока) = Тип("Строка") Или XMLСтрока = "" Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("ru='Нет фискальных данных';uk='Немає фіскальних даних'");
		Иначе
			ЗаполнитьДеревоЗначенийПоXMLСтроке(ДеревоВсехДанных, XMLСтрока);
		КонецЕсли;
		
	Иначе
		ТекстПредупреждения = НСтр("ru='Форма фискальных данных предназначена для открытия из элемента Кассовой смены';uk='Форма фіскальних даних призначена для відкриття з елемента Касової зміни'");
