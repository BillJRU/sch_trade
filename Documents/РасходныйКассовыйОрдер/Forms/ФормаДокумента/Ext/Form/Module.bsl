...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры_Взаиморасчеты = Документы.РасходныйКассовыйОрдер.ПараметрыВзаиморасчеты(Объект.ХозяйственнаяОперация);
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаздатчиком") Тогда
			Если ЗначениеЗаполнено(Объект.Ведомость)Тогда
				Объект.Основание = ОбщегоНазначенияУТ.ПолучитьПредставлениеДокумента(Объект.Ведомость,
						ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Объект.Ведомость.Номер),
						Объект.Ведомость.Дата);
...
Процедура ХозяйственнаяОперацияПриИзмененииКлиент(Оповещение)
	...
	Если Объект.ВыплатаЗаработнойПлаты.Количество() = 0 
		И ЗначениеЗаполнено(Объект.Ведомость) Тогда
	...
	Если ЗначениеЗаполнено(Объект.ПодотчетноеЛицо) Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументФизическогоЛица) Тогда
			ДокументФизическогоЛица = ФизическиеЛицаУТ.ПолучитьДокументФизическогоЛицаПоУмолчанию(
				Объект.ПодотчетноеЛицо, Справочники.ВидыДокументовФизическихЛиц.Паспорт).Представление;
		КонецЕсли;
...
Процедура ПодборПоОстаткам(Команда)
	...
	Если ЭтоВыдачаВДругуюОрганизацию Тогда
		ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("КассаПолучатель", НСтр("ru='Получатель';uk='Одержувач'"));
	Иначе
		ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Контрагент", НСтр("ru='Получатель';uk='Одержувач'"));
	КонецЕсли;
...
Процедура ОбновитьОтображениеСтрокаФизическиеЛица()
	...
	Элементы.СтрокаФизическиеЛица.ЦветТекста =
		?(Объект.ВыплатаЗаработнойПлаты.Количество() = 0, ЦветаСтиля.ЦветОсобогоТекста, ЦветаСтиля.ЦветГиперссылки);
