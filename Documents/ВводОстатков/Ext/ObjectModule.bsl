...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Документы.ВводОстатков.ЗаполнитьОбъектыРасчетов(ЭтотОбъект);
...
|тому не можна оформити документ по складу (приміщення) з адресним зберіганням решток. Зверніться до адміністратора'");
		...
		Для Каждого Строка Из ПрочиеРасходы Цикл
			Если Строка.Сумма = 0 И Строка.СуммаБезНДС = 0 И Строка.СуммаРегл = 0 
			 И Строка.НДСРегл = 0 И Строка.СуммаВР = 0 И Строка.СуммаПР = 0 Тогда
				ТекстОшибки = НСтр("ru='Не указаны суммы в строке %НомерСтроки% табличной части';uk='Не вказані суми в рядку %НомерСтроки% табличної частині'");
	...
	ВводОстатковЛокализация.ВводОстатковОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты);
...
Процедура ЗаполнитьВидыЗапасов()
	...
	Иначе
		...
		ТаблицаТоваров.Колонки.Добавить("НалоговоеНазначение", Новый ОписаниеТипов("СправочникСсылка.НалоговыеНазначенияАктивовИЗатрат"));
		ТаблицаТоваров.ЗаполнитьЗначения(НалоговоеНазначение,"НалоговоеНазначение");
	КонецЕсли;
		...
		Если (Не ЗначениеЗаполнено(СтрокаТаблицы.ОбъектРасчетов) И СтрокаТаблицы.ОбъектРасчетов <> Неопределено)
			...
			ИначеЕсли ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")
				...
				Если НовыйОбъект.ТипДоговора = Перечисления.ТипыДоговоровМеждуОрганизациями.КупляПродажа Тогда
					ТипДоговораСтрокой = НСтр("ru='купли-продажи';uk='купівлі-продажу'");
				ИначеЕсли НовыйОбъект.ТипДоговора = Перечисления.ТипыДоговоровМеждуОрганизациями.Комиссионный Тогда
					ТипДоговораСтрокой = НСтр("ru='комиссии';uk='комісії'");
				Иначе
					ТипДоговораСтрокой = "";
				КонецЕсли;
