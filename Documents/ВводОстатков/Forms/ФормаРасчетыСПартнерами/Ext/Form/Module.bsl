...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НДСОбщегоНазначенияСервер.ОрганизацияКонтрагентПлательщикНДС(Объект.Организация, Объект.Дата)
		И Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковАвансовКлиентов")
...
&НаКлиенте
Процедура ЗадолженностьПоставщикамСуммаРеглПриИзменении(Элемент)
	РассчитатьПредположительнуюСуммуНалоговогоКредита(Элементы.ЗадолженностьПоставщикам.ТекущаяСтрока);
КонецПроцедуры
...
&НаКлиенте
Процедура АвансыПоставщикамСуммаРеглПриИзменении(Элемент)
	РассчитатьПредположительнуюСуммуНалоговогоКредита(Элементы.АвансыПоставщикам.ТекущаяСтрока);
КонецПроцедуры
...
Процедура ОбъектРасчетовПриИзменении(ТекущаяСтрока) 
	...
	ТекущиеДанные.СуммаНДС = ПредположительнаяСуммаНалоговогоКредита(ТекущиеДанные.СуммаРегл, Объект.ХозяйственнаяОперация, 
		ТекущиеДанные.НетНалоговойНакладной, ТекущиеДанные.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета);
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "Объект.РасчетыСПартнерами.ТипРасчетов", НСтр("ru='Договор с контрагентом';uk='Договір з контрагентом'"));
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.АвансыПоставщикамНетНалоговойНакладной.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.АвансыПоставщикамДатаВходящегоДокумента.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.АвансыПоставщикамСуммаНДС.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.ЗадолженностьПоставщикамНетНалоговойНакладной.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.ЗадолженностьПоставщикамДатаВходящегоДокумента.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.ЗадолженностьПоставщикамВзаиморасчетыЗакрыты.Имя);
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.ЗадолженностьПоставщикамСуммаНДС.Имя);
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор, "ОрганизацияПлательщикНДС", Ложь);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор, "Объект.РасчетыСПартнерами.НетНалоговойНакладной", Ложь);
...
Процедура ЗаполнитьСпискиВыбораТипаРасчетов()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов 
		...
		Если Объект.ВводОстатков22 Тогда
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСКлиентами") Тогда
				...
				Если ПолучитьФункциональнуюОпцию("ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи") Тогда
					СписокВыбора.Добавить(Перечисления.ТипыДоговоров.СХранителем, НСтр("ru='Договор с хранителем';uk='Договір зі зберігачем'"));
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСКлиентами") Тогда
				СписокВыбора.Добавить(Справочники.ДоговорыКонтрагентов.ПустаяСсылка(), НСтр("ru='Договор с контрагентом';uk='Договір з контрагентом'"));
			КонецЕсли;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыМеждуОрганизациями") Тогда
				СписокВыбора.Добавить(Справочники.ДоговорыМеждуОрганизациями.ПустаяСсылка(), НСтр("ru='Договоры между организациями';uk='Договори між організаціями'"));
			КонецЕсли;
		КонецЕсли;
		...
		Если Объект.ВводОстатков22 Тогда
			...
			КонецЕсли;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьПередачуНаОтветственноеХранениеСПравомПродажи") Тогда
				СписокВыбора.Добавить(Документы.ВыкупТоваровХранителем.ПустаяСсылка(), НСтр("ru='Выкуп товаров хранителем';uk='Викуп товарів зберігачем'"));
			КонецЕсли;
		Иначе
			Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов Тогда
				СписокВыбора.Добавить(Перечисления.ТипыПервичныхДокументов.ОплатаОтКлиента, НСтр("ru='Оплата от клиента';uk='Оплата від клієнта'"));
			Иначе
				СписокВыбора.Добавить(Перечисления.ТипыПервичныхДокументов.РеализацияКлиенту, НСтр("ru='Реализация клиенту';uk='Реалізація клієнту'"));
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам 
		...
		Если Объект.ВводОстатков22 Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
				СписокВыбора.Добавить(Перечисления.ТипыДоговоров.СПоклажедателем, НСтр("ru='Договор с поклажедателем';uk='Договор с поклажодавцем'"));
			КонецЕсли
		Иначе
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСПоставщиками") Тогда
				СписокВыбора.Добавить(Справочники.ДоговорыКонтрагентов.ПустаяСсылка(), НСтр("ru='Договор с контрагентом';uk='Договір з контрагентом'"));
		...
		// Накладные
		Если Объект.ВводОстатков22 Тогда
			СписокВыбора.Добавить(Документы.ПриобретениеТоваровУслуг.ПустаяСсылка(), НСтр("ru='Приобретение товаров и услуг';uk='Придбання товарів і послуг'"));
			СписокВыбора.Добавить(Документы.ПриобретениеУслугПрочихАктивов.ПустаяСсылка(), НСтр("ru='Приобретение услуг и прочих активов';uk='Придбання послуг та інших активів'"));
			Если ПолучитьФункциональнуюОпцию("ИспользоватьКомиссиюПриПродажах") Тогда
				СписокВыбора.Добавить(Документы.ОтчетКомиссионера.ПустаяСсылка(), НСтр("ru='Отчет комиссионера о продажах';uk='Звіт комісіонера про продажі'"));
			КонецЕсли;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьКомиссиюПриЗакупках") Тогда
			...
			КонецЕсли;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
				СписокВыбора.Добавить(Документы.ВыкупПринятыхНаХранениеТоваров.ПустаяСсылка(), НСтр("ru='Выкуп товаров с хранения';uk='Викуп товарів зі зберігання'"));
			КонецЕсли;
		Иначе
			Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам Тогда
				СписокВыбора.Добавить(Перечисления.ТипыПервичныхДокументов.ОплатаПоставщику, НСтр("ru='Оплата поставщику';uk='Оплата постачальнику'"));
			Иначе
				СписокВыбора.Добавить(Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика, НСтр("ru='Приобретение у поставщика';uk='Придбання у постачальника'"));
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Объект.РасчетыСПартнерами Цикл
		...
		Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
			...
			ИначеЕсли ТипДоговора = Перечисления.ТипыДоговоров.СПоклажедателем Тогда
				СтрокаТаблицы.ТипРасчетов = НСтр("ru='Договор с поклажедателем';uk='Договор с поклажодавцем'");
			ИначеЕсли ТипДоговора = Перечисления.ТипыДоговоров.СХранителем Тогда
				СтрокаТаблицы.ТипРасчетов = НСтр("ru='Договор с хранителем';uk='Договір зі зберігачем'");
			Иначе
...
Процедура ПроверитьОбъектРасчетовСервер(ТекущаяСтрокаИдентификатор)
	...
	Если ЗначениеЗаполнено(ЗначенияРеквизитов.Организация)
		...
		ТекстСообщения = НСтр("ru='В документе ""%ОбъектРасчетов%"" организация отличается от указанной во вводе остатков';uk='У документі ""%ОбъектРасчетов%"" організація відрізняється від зазначеної у введенні залишків'");
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Партнер)
		...
		ТекстСообщения = НСтр("ru='В %Объект% ""%ОбъектРасчетов%"" партнер отличается от указанного во вводе остатков';uk='У %Объект% ""%ОбъектРасчетов%"" партнер відрізняється від зазначеного у введенні залишків'");
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Контрагент)
		...
		ТекстСообщения = НСтр("ru='В %Объект% ""%ОбъектРасчетов%"" контрагент отличается от указанного во вводе остатков';uk='У %Объект% ""%ОбъектРасчетов%"" контрагент відрізняється від зазначеного у введенні залишків'");
		Если ТипЗнч(ТекущиеДанные.ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			ИЛИ ТипЗнч(ТекущиеДанные.ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями") Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Объект%", НСтр("ru='договоре';uk='договорі'"));
		Иначе
...
&НаКлиенте
Процедура РассчитатьПредположительнуюСуммуНалоговогоКредита(ДанныеСтроки)
	
	ДанныеСтроки.СуммаНДС = ПредположительнаяСуммаНалоговогоКредита(ДанныеСтроки.СуммаРегл, Объект.ХозяйственнаяОперация, 
		ДанныеСтроки.НетНалоговойНакладной, ДанныеСтроки.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ПредположительнаяСуммаНалоговогоКредита(СуммаРегл, ХозяйственнаяОперация, НетНН, ЭтоВалютаРегл)
	
	СуммаНДС = 0;
	
	Если ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковЗадолженностиПоставщикам")
			И НетНН И ЭтоВалютаРегл 
		ИЛИ 
			ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам")
				И НЕ НетНН И ЭтоВалютаРегл Тогда
		
		СуммаНДС = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СуммаРегл, НДСОбщегоНазначенияПовтИсп.СтавкаНДСПоУмолчанию(), Истина);
