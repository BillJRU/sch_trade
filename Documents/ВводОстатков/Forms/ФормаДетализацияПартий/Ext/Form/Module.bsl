...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы И Модифицированность Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru='Данные не были перенесены в ввод остатков. После закрытия, сделанные Вами изменения сохранены не будут.';uk='Дані не були перенесені в введення залишків. Після закриття, зроблені Вами зміни збережені не будуть.'");
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Данные не перенесены в ввод остатков. Перенести?';uk='Дані не перенесені в введення залишків. Перенести?'");
...
Процедура ПроверитьЗаполнениеТабличнойЧасти(Отказ)
	...
	Для ТекИндекс = 0 По ДетализацияПартий.Количество()-1 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].Контрагент) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен партнер в строке %НомерСтроки%';uk='Не заповнений партнер в рядку %НомерСтроки%'");
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].Контрагент) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен контрагент в строке %НомерСтроки%';uk='Не заповнений контрагент в рядку %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].ДокументПоступления) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен документ поступления в строке %НомерСтроки%';uk='Не заповнений документ надходження в рядку %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].ВидЦенности) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен Вид ценности в строке %НомерСтроки%';uk='Не заповнений Вид цінності в рядку %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].СтавкаНДС) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен Ставка НДС в строке %НомерСтроки%';uk='Не заповнена Ставка ПДВ в рядку %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если ДетализацияПартий[ТекИндекс].СуммаРегл = 0
			...
			ТекстОшибки = НСтр("ru='Не заполнены суммовые показатели в строке %НомерСтроки%';uk='Не заповнені сумові показники в рядку %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
	...
	Если ИтогСуммаРегл > СуммаРегл Тогда
		ТекстОшибки = НСтр("ru='Сумма поступлений не может превышать стоимость товара';uk='Сума надходжень не може перевищувати вартість товару'");
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			,
			"ДетализацияПартий",
	...
	Если ИтогНДСРегл > НДСРегл Тогда
		ТекстОшибки = НСтр("ru='Сумма НДС поступлений не может превышать итоговую суммы НДС по товару';uk='Сума ПДВ надходжень не може перевищувати підсумкову суми ПДВ по товару'");
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			,
			"ДетализацияПартий",
...
Процедура УстановитьТекстДанныеПартии()
	...
	Если НДСРегл <> 0 Тогда
		Шаблон = НСтр("ru='По партии ввода остатков будет отражена сумма (регл.) %1 НДС (регл.) %2';uk='По партії введення залишків буде відображена сума (регл.) %1 ПДВ (регл.) %2'");
	Иначе
		Шаблон = НСтр("ru='По партии ввода остатков будет отражена сумма (регл.) %1';uk='По партії введення залишків буде відображена сума (регл.) %1'");
	КонецЕсли;
