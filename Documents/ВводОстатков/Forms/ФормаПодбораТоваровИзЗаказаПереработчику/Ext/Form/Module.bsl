...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтрШаблон(НСтр("ru='Подбор материалов из заказа: %1';uk='Підбір матеріалів з замовлення: %1'"), Параметры.ЗаказПереработчику);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ЗавершениеРаботы И Модифицированность Тогда
		...
		ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, ЗавершениеРаботы, НСтр("ru='Данные были изменены. Перенести изменения в документ?';uk='Дані були змінені. Перенести зміни в документ?'"));
	КонецЕсли;
		...
		Если ЗаказНайден Тогда
			...
			Если КурсВалюты.Количество() = 1 Тогда
				...
				СтрокаТаб.Цена = РаботаСКурсамиВалютКлиентСервер. ПересчитатьПоКурсу(СтрокаТаб.Цена, 
						Новый Структура("Валюта, Курс, Кратность", РеквизитыЗаказов.Валюта, КурсВалюты[0].Курс, КурсВалюты[0].Кратность),
						Новый Структура("Валюта, Курс, Кратность", ДанныеОтбора.Валюта, СтруктураКурсовНовойВалюты.Курс, СтруктураКурсовНовойВалюты.Кратность));
