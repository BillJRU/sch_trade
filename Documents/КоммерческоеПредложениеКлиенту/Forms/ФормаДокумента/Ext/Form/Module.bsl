...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	// Schueco_SCT: перед открытием формы ручной скидки гарантируем актуальность авто-скидок.
	// Иначе режим «Від суми з автоматичною знижкою» оперирует устаревшими данными.
	// Условие зеркалирует ПередЗаписью (КоммерческиеПредложенияДокументыКлиентУТ:874-877).
	Если НастройкиУчета.ИспользуютсяАвтоматическиеСкидкиВПродажах
		И Не Объект.СкидкиРассчитаны Тогда
		Оповещение = Новый ОписаниеОповещения(
			"ПослеВопросаОРасчетеСкидокПередРучнойСкидкой_SCT",
			ЭтотОбъект);
		СкидкиНаценкиКлиент.ПредложитьПользователюРассчитатьСкидки(Оповещение);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеВопросаОРасчетеСкидокПередРучнойСкидкой_SCT(Результат, ДополнительныеПараметры) Экспорт

	// Schueco_SCT: callback после диалога «Автоматичні знижки не розраховані. Розрахувати?».
	// На ОК запускаем стандартный пересчёт скидок (Объект.СкидкиРассчитаны станет Истина),
	// затем повторно вызываем команду — на этот раз условие не сработает и откроется форма ручной скидки.
	// На Отмена ничего не делаем — пользователь отказался открывать форму.
	Если Результат <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	РассчитатьСкидки(Неопределено);
	НазначитьРучнуюСкидку(Неопределено);
