...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы
		...
		Если ЕстьРедактируемыеФайлы Тогда
			...
			ТекстВопроса = НСтр("ru='Данные были изменены, Записать?';uk='Дані були змінені, Записати?'");
			ДополнительныеПараметры = Новый Структура;
...
Процедура СписокПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		ШаблонТекстаОшибки = НСтр("ru='Адрес %1 уже есть в списке.';uk='Адреса %1 вже є у списку.'");
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Если СписокПолучателей.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru='Необходимо указать хотя бы одного получателя письма.';uk='Необхідно вказати хоча б одного одержувача листа.'"),, "СписокПолучателей");
		Возврат;
		
	ИначеЕсли (СписокПолучателей.Количество() = 1 И ПустаяСтрока(СписокПолучателей[0].Адрес)) Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru='Необходимо указать хотя бы одного получателя письма.';uk='Необхідно вказати хоча б одного одержувача листа.'"),, "СписокПолучателей[0].Представление");
		Возврат;
...
Процедура ПрикрепитьВходящееПисьмоОснованиеКакВложениеЕслиНеобходимо(ТекущийОбъект)
	...
	Если ТекущийОбъект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее 
		...
		ПараметрыФайла.ИмяБезРасширения = НСтр("ru='Пересылаемое сообщение';uk='Повідомлення, що пересилається'");
...
Функция СформироватьИсходящееОбычныйТекст(ВыборкаДанныеВходящегоПисьма, ТекущийОбъект)
	...
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Отправлено';uk='Відправлено'") + ": " 
...
Процедура РедактироватьСписокПолучателей(ДляПодбора, ГруппаПодбора = "")
	...
	Для каждого Получатель Из СписокПолучателей Цикл
		Если Получатель.ВариантОтправки = НСтр("ru='Обратный адрес:';uk='Зворотна адреса:'") Тогда
		...
		ИначеЕсли Получатель.ВариантОтправки = НСтр("ru='Скрытая копия:';uk='Прихована копія:'") Тогда
...
Процедура СформироватьСпискиПолучателейПисьма()
	...
	ДобавитьАдресВСписокПолучателей(СписокПолучателей, Объект.ПолучателиПисьма, НСтр("ru='Кому:';uk='Кому:'"));
	ДобавитьАдресВСписокПолучателей(СписокПолучателей, Объект.ПолучателиКопий, НСтр("ru='Копия:';uk='Копія:'"));
	ДобавитьАдресВСписокПолучателей(СписокПолучателей, Объект.ПолучателиСкрытыхКопий, НСтр("ru='Скрытая копия:';uk='Прихована копія:'"));
	ДобавитьАдресВСписокПолучателей(СписокПолучателей, Объект.ПолучателиОтвета, НСтр("ru='Обратный адрес:';uk='Зворотна адреса:'"));
	...
	Исключение
		...
		Если ЭтоПроблемаОтвергнутыхСерверомПочтовыхАдресов И Не ВсеАдресатыПисьмаОтвергнутыСервером Тогда
			Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Наступні адресати електронного листа не прийняті поштовим сервером:
                |%1. Лист відправлено решті адресатів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
