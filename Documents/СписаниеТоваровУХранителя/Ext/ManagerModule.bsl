...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Акт о списании товаров у хранителя';uk='Акт про списання товарів у зберігача'");
...
Функция СформироватьПечатнуюФормуАктСписанияТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	ПервыйДокумент   = Истина;
 	СинонимДокумента = НСтр("ru='Акт о списании товаров у хранителя';uk='Акт про списання товарів у зберігача'",КодЯзыкаПечать);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		// Макет необходимо получать для каждого документа, т.к. размеры колонок изменяются динамически.
 		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СписаниеТоваровУХранителя.ПФ_MXL_АктОСписанииТоваров_ru",КодЯзыкаПечать);
		...
		СтруктураДанныхШапки.Вставить("ТекстЗаголовка",
									ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати,
 																								СинонимДокумента,КодЯзыкаПечать));
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Если ВыводитьКоды Тогда
				СтруктураДанныхКоды = Новый Структура("ДопКолонка", ВыборкаПоТоварам[КолонкаКодов]);
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='В списаниях товаров у хранителя в табличной части Товары заполняет номер ГТД. В случае если одной строке из табличной части Товары соответствует несколько номеров ГТД из табличной части ВидыЗапасов, то разбивает строки в табличной части Товары. До окончания выполнения обработчика вы не сможете редактировать списания товаров у хранителя.';uk='У списаних товарів у зберігача в табличній частині Товари заповнює номер ВМД. У разі якщо одному рядку з табличній частині Товари відповідає кілька номерів ВМД з табличній частині Види запасів, то розбиває рядки в табличній частині Товари. До закінчення виконання обробника ви не зможете редагувати списання товарів у зберігача.'");
