...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ТоварыКОформлениюИзлишковНедостач)
	 И ПравоДоступа("Изменение", Метаданные.Документы.ПересчетТоваров) Тогда
		...
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Обработчик = "УправлениеПечатьюУТКлиентЛокализация	.ПечатьМ21";
...
Функция ПолучитьДанныеДляПечатнойФормыОтборРазмещениеТоваров(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&ТекстУсловияТоваровЗадания", ТекстУсловияТоваровЗадания);
...
Функция СоздатьИЗаписатьЗаданиеНаПересчет(Склад, Помещение, МассивЯчеек)
	...
	Для Каждого Ячейка Из ЯчейкиПересчетов Цикл
		ТекстСообщения = НСтр("ru='Есть невыполненные задания на пересчет ячейки %Ячейка%';uk='Є невиконані завдання на перерахунок комірки %Ячейка%'");
