...
|Перезаполнить этапы в соответствии с графиком?'
|;uk='Сума етапів плану оплати не збігається з сумою запланованих рядків за періоди: %ТекстПериод%. 
|Перезаповнити етапи відповідно до графіка?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_КорректировкаПлановыхПотребностей" Тогда
		...
		Если Параметр.Свойство("План")
			...
			ТекстВопроса = НСтр("ru='Потребность в товарах изменилась, перезаполнить табличную часть?';uk='Потреба в товарах змінилася, перезаповнити табличну частину?'");
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗаполнятьПоДефициту Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещение, 
			НСтр("ru='Количество к обеспечению будет скорректировано. Продолжить?';uk='Кількість до забезпечення буде скориговано. Продовжити?'"), 
...
Процедура ЗаполнитьЭтапыГрафикаОплаты(Команда)
	...
	Если Объект.СуммаДокумента = СуммаОплатыПоЭтапам Тогда
		...
		ТекстВопроса = НСтр("ru='Сумма запланированных строк совпадает с суммой этапов оплаты';uk='Сума запланованих рядків збігається із сумою етапів оплати'") + Символы.ПС +
...
Процедура ВставитьСтроки(Команда)
	...
	Если Объект.КроссТаблица Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещение, 
			НСтр("ru='Копирование и вставка строк возможны в режиме ""Периоды в строках"". Переключить режим?';uk='Копіювання і вставка рядків можливі в режимі ""Періоди в рядках"". Переключити режим?'"), 
			РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьВидимость()
	...
	Элементы.ТоварыЗагрузитьИзВнешнегоФайлаПоФормуле.Видимость = Объект.ЗаполнятьПоФормуле И Не Объект.ЗаполнятьПоДефициту;
...
Процедура ОбновитьОтображенияКорректировок()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.КоличествоКПроверке = 0 Тогда
			Элементы.НадписьКорректировкиПлановыхПотребностей.Заголовок = НСтр("ru='Имеются корректировки потребностей';uk='Є коригування потреб'");
		...
		Иначе
			Элементы.НадписьКорректировкиПлановыхПотребностей.Заголовок = НСтр("ru='Имеются отмененные корректировки потребностей к проверке';uk='Є скасовані коригування потреб до перевірки'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru='Уникальный идентификатор (Назначение)';uk='Унікальний ідентифікатор (Призначення)'"),"ИдентификаторНазначение");
	...
	Если ЗаполнятьНазначениеВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru='Назначение';uk='Призначення'"),"ЗагружаемоеНазначение");
	КонецЕсли;
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План закупок +15%';uk='План купівель +15%'"),
		"[ПланыЗакупок] * 1.15");
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров,
		НСтр("ru='Закупки прошедшего периода с учетом остатка';uk='Купівлі минулого періоду з урахуванням залишку'"),
		"Макс([ФактыЗакупок], [ПланыЗакупок])"
		+ ?(ИспользоватьПланыОстатков," - [ПланыОстатков]",
		?(ПланированиеПоНазначениям," - [ФактическиеНачальныеОстатки]"," - [СвободныеОстатки]")));

	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Пополнение остатка';uk='Поповнення залишку'"),
		?(ИспользоватьПланыОстатков,"?([ПланыОстатков] = 0 И ",
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Продажи за вычетом остатка +15%';uk='Продажі за вирахуванням залишку +15%'"), "("
		+ ?(ИспользоватьПланыПродаж,"[ПланыПродаж]","[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]")
		+ ?(ИспользоватьПланыОстатков," - [ПланыОстатков]) * 1.15",
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Значение предыдущей колонки';uk='Значення попередньої колонки'"),
