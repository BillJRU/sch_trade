...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ПередачаПоЗаказам
		...
		Если ИспользоватьПередачуПоНесколькимЗаказам Тогда
			ТекстСообщения = НСтр("ru='Уточните склад передачи и заполните товары с помощью команды ""Заполнить по заказам""';uk='Уточніть склад передачі і заповніть товари за допомогою команди ""Заповнити за замовленнями""'");
		Иначе
			ТекстСообщения = НСтр("ru='Уточните склад передачи и заполните товары с помощью команды ""Заполнить по заказу""';uk='Уточніть склад передачі і заповніть товари за допомогою команди ""Заповнити за замовленнями""'");
		КонецЕсли;
...
Процедура ПередачаПоЗаказамПриИзменении(Элемент)
	...
	Если НачатаОтгрузка
		...
		ТекстПредупреждения = НСтр("ru='По данному документу уже начата отгрузка. Менять признак ""Передача по заказу"" после начала отгрузки не допускается.';uk='За даним документом вже розпочато відвантаження. Змінювати ознаку ""Передача за замовленням"" після початку відвантаження не допускається.'");
...
Процедура НадписьЗаголовокЗаказыКлиентовНажатие(Элемент, СтандартнаяОбработка)
	...
	ТекстЗаголовка    = НСтр("ru='Заказы клиентов (%КоличествоДокументов%)';uk='Замовлення клієнтів (%КоличествоДокументов%)'");
	...
	ИначеЕсли Поле = Элементы.ТоварыЦена Тогда
		...
		Если ПравоНаЧтениеВидаЦены Тогда
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.ВидЦены) Тогда
				...
				ТекстСообщения = НСтр("ru='Для редактирования цены выберите вид цены ""<произвольная>""';uk='Для редагування ціни виберіть вид ціни ""<довільна>""'");
...
Процедура ЗакрытьЗаказ(Команда)
	...
	Если Модифицированность
		...
		ТекстВопроса       = НСтр("ru='Необходимо провести документ для выполнения операции. Провести документ?';uk='Необхідно провести документ для виконання операції. Провести документ?'");
...
Процедура СоставНабора(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущаяСтрока.НоменклатураНабора) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Номенклатура выбранной строки не входит в состав набора.';uk='Номенклатура вибраного рядка не входить до складу набору.'"));		
		Возврат;
	КонецЕсли;
...
Процедура ОповеститьОбОкончанииОтвязкиСтрок(КоличествоОтработанныхСтрок, СтрокиОтвязаны = Истина)
	...
	Если СтрокиОтвязаны Тогда
		ТекстСообщения = НСтр("ru='Строки отвязаны';uk='Рядки відв''язані'");
		ТекстПояснения = НСтр("ru='В документе от заказов отвязано строк (%Количество%).';uk='У документі від замовлень відв''язано рядків (%Количество%).'");
		ТекстПояснения = СтрЗаменить(ТекстПояснения, "%Количество%", КоличествоОтработанныхСтрок);
	Иначе
		ТекстСообщения = НСтр("ru='Строки не отвязаны';uk='Рядки не відв''язані'");
		ТекстПояснения = НСтр("ru='Ни одна строка не была отвязана.';uk='Жоден рядок не був відв''язаний.'");
	КонецЕсли;
...
Процедура ОткрытьПодбор(Команда)
	...
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru='передачу товаров хранителю';uk='передачу товарів зберігачу'"));
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НалогообложениеНДСПоУмолчанию", "Доступность", Ложь);	
...
Процедура ОбновитьЗависимыеРеквизитыФормыПоЗаказу(ЭлементыФормы, Товары, НадписьРасхождениеЗаказ)
	...
	Если КоличествоРасхождений > 0 Тогда
		НадписьРасхождениеЗаказ = НСтр("ru='Строк сверх заказа: %КоличествоРасхождений%';uk='Рядків понад замовлення: %КоличествоРасхождений%'");
...
Процедура ПередачаПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.Товары.Количество() = 0 Тогда
		ТекстВопроса = НСтр("ru='Список Товары будет заполнен остатками неоформленных товаров по документу %ЗаказКлиента%. Продолжить?';uk='Список Товари буде заповнений залишками неоформлених товарів за документом %ЗаказКлиента%. Продовжити?'");
		ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ЗаказКлиента%", ЗаказКлиента);
	Иначе
		ТекстВопроса = НСтр("ru='Строки в списке Товары будут привязаны к строкам документа %ЗаказКлиента%. Строки, которые невозможно привязать к строкам заказа, будут удалены. Продолжить?';uk='Рядки в списку Товари будуть прив''язані до рядків документа %ЗаказКлиента%. Рядки, які неможливо прив''язати до рядків замовлення, будуть вилучені. Продовжити?'");
		ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ЗаказКлиента%", ЗаказКлиента);
	КонецЕсли;
...
Процедура УстановитьНалогообложениеНДСПоУмолчанию(ПриОткрытииФормы = Ложь)
	...
	ПараметрыЗаполнения = Документы.ПередачаТоваровХранителю.ПараметрыЗаполненияНалогообложенияНДСПродажи(Объект);
