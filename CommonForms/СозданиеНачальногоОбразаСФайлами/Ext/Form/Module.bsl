	...
	Если Не ПустаяСтрока(СтруктураПути.Путь) Тогда
		...
		Если НайтиФайлы(ПутьКФайлу).Количество() = 0 Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Каталог ""%1"" не існує або недоступний.
                |Перевірте правильність вказаного шляху.'"),
				ПутьКФайлу);
			Возврат Ложь;
		КонецЕсли;
	Иначе
		СообщениеОбОшибке = НСтр("ru='Укажите полный путь к файлу информационной базы.';uk='Вкажіть повний шлях до файлу інформаційної бази.'");
		Возврат Ложь;
	КонецЕсли;
