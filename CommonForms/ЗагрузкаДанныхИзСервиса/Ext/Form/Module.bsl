...
Процедура Загрузить(Команда)
	...
	Иначе
		...
		ПараметрыПередачи.ЗаголовокДиалогаВыбора = НСтр("ru='Загрузка файла';uk='Завантаження файлу'");
		ПараметрыПередачи.ФильтрДиалогаВыбора = СтрШаблон(НСтр("ru='Архивы %1';uk='Архіви %1'"), "(*.zip)|*.zip");
...
Процедура ПродолжитьЗагрузкуДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВыгрузкаДляТехническойПоддержки(СвойстваВременногоФайла) Тогда
        ...
        |;uk='Файл даних створено в режимі вивантаження для технічної підтримки.
        |      Програма отримане з такого вивантаження призначена тільки для цілей тестування і розбору проблем. Продовжити завантаження?'"),
...
Функция ФайлДанныхСодержитРасширения(СвойстваВременногоФайла)
	...
	СтрокаСообщения = НСтр("ru='Выбранный файл содержит данные добавленные расширениями.';uk='Вибраний файл містить дані додані розширеннями.'") 
		+ Символы.ПС + НСтр("ru='Для восстановления из данного архива установите следующие расширения:';uk='Для відновлення з даного архіву встановіть наступні розширення:'");
	...
	ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Если все необходимые расширения уже установлены, тогда нажмите ''Продолжить''';uk='Якщо всі необхідні розширення вже встановлені, тоді натисніть ''Продовжити'''"));
...
Процедура ВыполнитьЗагрузку(Знач СвойстваВременногоФайла, Знач ЗагружатьИнформацияОПользователях)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Загрузка данных';uk='Завантаження даних'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ШаблонИсключения = НСтр("ru='При загрузке данных произошла ошибка: %1.
                                 ...
                                 |
                                 |Розширена інформація для служби підтримки записана в журнал реєстрації. Якщо причина помилки невідома - рекомендується звернутися в службу технічної підтримки, надавши для розслідування вивантаження журналу реєстрації і файл, з якого робилася спроба завантажити дані.'");
...
Процедура УдалитьФайлыВПопытке(ИмяПапкиИлиФайла)
	...
	ИмяСобытияЖР = НСтр("ru='Удаление файла.Загрузка файла выгрузки';uk='Вилучення файла. Загрузка файла вивантаження'",ОбщегоНазначения.КодОсновногоЯзыка());
