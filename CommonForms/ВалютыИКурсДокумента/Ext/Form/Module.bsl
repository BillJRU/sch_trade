...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазрешеныРазличныеВалюты           = Параметры.Свойство("РазрешеныРазличныеВалюты")
											И Параметры.РазрешеныРазличныеВалюты;
...
Процедура КурсВзаиморасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = 0 Тогда
		...
		ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаДокумента, НСтр("ru='Укажите дату курса валюты';uk='Вкажіть дату курсу валюти'"), ЧастиДаты.Дата);
	КонецЕсли;
...
Процедура ВалютаВзаиморасчетовПриИзменении(Элемент)
    ...
    Если НЕ РазрешеныРазличныеВалюты Тогда
...
Процедура ЗаполнитьСписокКурсовВалют(ИзмененаВалюта=Ложь)
	...
	Если ВалютаДокумента = ВалютаРегУчета 
		...
		Если НЕ ВалютаДокумента = ВалютаРегУчета Тогда
			...
			СписокКурсовВалют.Добавить(КурсНаДату.Курс, Формат(КурсНаДату.Курс, "ЧДЦ=4") + " "
				+ СтрШаблон(НСтр("ru='(на %1)';uk='(на %1)'"), Формат(ДатаДокумента, "ДЛФ = D")));
			...
			Для ДеньМинус = 1 По 5 Цикл
				...
				СписокКурсовВалют.Добавить(КурсНаДату.Курс, Формат(КурсНаДату.Курс, "ЧДЦ=4") + " "
					+ СтрШаблон(НСтр("ru='(на %1)';uk='(на %1)'"), Формат(ДатаКурса, "ДЛФ = D")));
					
			КонецЦикла;
			СписокКурсовВалют.Добавить(0, НСтр("ru='<Выбрать другую дату>';uk='<Вибрати іншу дату>'"));
	...
	ИначеЕсли ЗначениеЗаполнено(ВалютаДокумента) И ЗначениеЗаполнено(ВалютаВзаиморасчетов) Тогда
		...
		КонецЕсли;
		СписокКурсовВалют.Добавить(Окр(КурсПересчета.Курс,4), Формат(КурсПересчета.Курс, "ЧДЦ=4") + " " + СтрШаблон(НСтр("ru='(на %1)';uk='(на %1)'"), Формат(ДатаДокумента, "ДЛФ = D")));
		Для ДеньМинус = 1 По 5 Цикл
			ДатаКурса = ДатаДокумента - (ДеньМинус * 86400);
			...
			КурсПересчета = РаботаСКурсамиВалютУТ.ПолучитьКроссКурсВалют(КурсВалютыДокументаНаДату,КурсВалютыВзаиморасчетовНаДату);
			СписокКурсовВалют.Добавить(Окр(КурсПересчета.Курс,4), Формат(КурсПересчета.Курс, "ЧДЦ=4") + " " + СтрШаблон(НСтр("ru='(на %1)';uk='(на %1)'"), Формат(ДатаКурса, "ДЛФ = D")));
		КонецЦикла;
		СписокКурсовВалют.Добавить(0, НСтр("ru='<Выбрать другую дату>';uk='<Вибрати іншу дату>'"));
...
Процедура КурсВзаиморасчетовОбработкаВыбораНаСервере(ДатаКурса)
	...
	Если НЕ ДатаКурса > ТекущаяДатаСеанса() Тогда
		ТекстНаДату = " " + СтрШаблон(НСтр("ru='(на %1)';uk='(на %1)'"), Формат(ДатаКурса, "ДЛФ = D"));
	Иначе
		ТекстНаДату = "";
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
    ...
    Если НЕ РазрешеныРазличныеВалюты И НЕ НеПоказыватьРасчеты Тогда
        Элементы.ВалютаДокумента.ТолькоПросмотр     = Истина;
        Элементы.ВалютаДокумента.Доступность        = Ложь;
        Элементы.КурсВалютыДокумента.ТолькоПросмотр = Истина;
    КонецЕсли;    
