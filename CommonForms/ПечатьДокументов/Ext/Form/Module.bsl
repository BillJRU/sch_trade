...
Функция СформироватьПечатныеФормы(ИменаМакетов, Отказ)
	...
	Если ПустаяСтрока(КодЯзыкаДляМногоязычныхПечатныхФорм) Тогда
		...
		Иначе	
			...
			КодЯзыкаДляМногоязычныхПечатныхФорм = МодульМультиязычностьУкр.ПолучитьЯзыкФормированияПечатныхФорм();
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	Если Не ЗначениеЗаполнено(ЗаголовокФормы) Тогда
		Если ЭтоПечатьКомплекта() Тогда
			ЗаголовокФормы = НСтр("ru='Печать комплекта';uk='Друк комплекту'");
		ИначеЕсли ОбъектыПечати.Количество() > 1 Тогда
			ЗаголовокФормы = НСтр("ru='Печать документов';uk='Друк документів'");
		...
		Иначе
			ЗаголовокФормы = НСтр("ru='Печать документа';uk='Друк документу'");
		КонецЕсли;
	КонецЕсли;
