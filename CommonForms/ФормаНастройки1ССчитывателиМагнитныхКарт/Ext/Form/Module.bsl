...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПолучитьВерсиюДрайвера",
		...
		Если ВерсияИзБПОСтр > ВерсияСтр Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Установленная на компьютере версия драйвера устарела. Необходимо обновление до версии:';uk='Встановлена на комп''ютері версія драйвера застаріла. Необхідно оновлення до версії:'") + Символы.НПП + ВерсияИзБПО);
		КонецЕсли;
	Иначе
