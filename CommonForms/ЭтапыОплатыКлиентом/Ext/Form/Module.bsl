...
Процедура УстановитьУсловноеОформление()
	...
	Если Параметры.СпециальныеЗаголовкиЭтаповДляВозврата Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Кредит или возврат (плановый)';uk='Кредит або повернення (плановий)'"));
...
Процедура ПриИзмененииФормыОплатыСервер()
	...
	ОплатаВВалютеРеглУчета = НЕ ОплатаВВалюте;
	...
	Если ОплатаВВалютеРеглУчета Тогда
...
Процедура ЗаполнитьЭтапыГрафикаОплаты()
	...
	Если Не ПараметрыЗаполнения.ЗаполнитьДату Тогда
		...
		Если СуммаОплатыПоДокументу = СуммаОплатыПоЭтапам  И СуммаЗалогаПоДокументу = СуммаЗалогаПоЭтапам Тогда
			...
			ТекстВопроса = НСтр("ru='Сумма заказанных строк совпадает с суммой этапов оплаты';uk='Сума замовлених рядків збігається із сумою етапів оплати'") + Символы.ПС +
...
Процедура УстановитьСпециальныеЗаголовкиЭтаповДляВозврата()
	...
	Если Параметры.СпециальныеЗаголовкиЭтаповДляВозврата Тогда
		...
		Элементы.ЭтапыОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыКлиентом.АвансДоОбеспечения, НСтр("ru='Аванс или возврат (до обеспечения)';uk='Аванс або повернення (до забезпечення)'"));
		Элементы.ЭтапыОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыКлиентом.ПредоплатаДоОтгрузки, НСтр("ru='Предоплата или возврат (до отгрузки)';uk='Передоплата або повернення (до відвантаження)'"));
		Элементы.ЭтапыОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки, НСтр("ru='Кредит или возврат (плановый)';uk='Кредит або повернення (плановий)'"));
		Элементы.ЭтапыОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыКлиентом.КредитСдвиг, НСтр("ru='Кредит или возврат (после отгрузки)';uk='Кредит або повернення (після відвантаження)'"));
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.СуммаОтклоненияМерныхТоваров.Видимость = ЭтоЗаказ;
	Элементы.СуммаОтклоненийПоДокументу.Видимость = ЭтоЗаказ;
	...
	Если ИспользоватьОтрицательныеСуммыПлатежа Тогда
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтапыГрафикаОплатыДатаСдвиг", "Заголовок",
			НСтр("ru='Дата платежа';uk='Дата платежу'"));
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыПриИзмененииПорядкаРасчетов()
	...
	Если УпрощенныйРежим Тогда
		...
		МассивЭлементов.Добавить("СтраницаРасширеннаяНастройка");
	ИначеЕсли НЕ ЭтоЗаказ И НЕ РасширенныйРежим Тогда
	...
	Иначе 
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПредоплата", "Заголовок",
			НСтр("ru='Предоплата (до отгрузки)';uk='Передоплата (до відвантаження)'"));
...
Процедура УстановитьТекстКнопкиЗаполнения(Форма)
	...
	Если Форма.НакладнаяПоЗаказам Тогда
		ТекстКнопкиЗаполнения = НСтр("ru='Заполнить по заказам';uk='Заповнити по замовленнях'");
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ГрафикИсполненияДоговора) Тогда
		ТекстКнопкиЗаполнения = НСтр("ru='Заполнить по графику договора';uk='Заповнити за графіком договору'");
	КонецЕсли;
...
Процедура ЗаполнитьЭтапыПоЗаказамСервер()
	...
	ПараметрыЗаполнения.ЕстьТара                  = ТребуетсяЗалогЗаТару;
	ПараметрыЗаполнения.ИмяПоляЗаказ              = "Заказ";
