	...
	Иначе
		...
		Если ОбщегоНазначения.ЭтоMacOSКлиент() Тогда
			...
			Элементы.ПрограммыLinuxПутьКПрограмме.СписокВыбора.Добавить(
				"ПутьMac", НСтр("ru='Путь для Mac OS';uk='Шлях для Mac OS'") + ": " + ПутьПоУмолчанию);
		...
		Если ОбщегоНазначения.ЭтоLinuxКлиент()
			...
			Элементы.ПрограммыLinuxПутьКПрограмме.СписокВыбора.Добавить(
				"ПутьLinux", НСтр("ru='Путь для Linux';uk='Шлях для Linux'") + ": " + ПутьПоУмолчанию);
...
Процедура ПрограммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПрограммыПодробнее
		...
		ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", НСтр("ru='Результат проверки программы';uk='Результат перевірки програми'"));
	...
	Если Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		...
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу на компьютер пользователя
                       ...
                       |
                       |Важно: на каждый компьютер, где работает сервер BAF
                       ...
                       |для перевірки електронних підписів і сертифікатів.
                       |
                       |Важливо: на кожен комп''ютер, де працює сервер BAF
                       |або вебсервер, підключений до файлової інформаційної бази,
	...
	Если Не Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		...
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу и сертификат
                       ...
                       |Важливо: на кожен комп''ютер, де працює сервер BAF
                       |або вебсервер, підключений до файлової інформаційної бази,
                       |повинна бути встановлена програма і сертифікат з закритим ключем.'");
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		...
		ОбновитьЗначение(ОписаниеПрограммы.Подробнее, НСтр("ru='Подробнее';uk='Докладніше'") + "...");
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
