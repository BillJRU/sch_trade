...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Patch SCT: вибір активної сторінки Pages-групи. Якщо trace-дерево заповнене (адреса
	// тимчасового сховища була доступна) — показуємо стандартний механізм типової з повними
	// умовами розрахунку. Інакше (перевідкритий документ — адреса вже втрачена з RAM) —
	// показуємо плоский snapshot з табчастини документа. Так користувач завжди бачить рівно
	// одну табличку, без дублювання UI.
	Если ИнформацияОРасчетеСкидокПоСтроке.ПолучитьЭлементы().Количество() > 0 Тогда
		Элементы.ГруппаСтраницыСкидок.ТекущаяСтраница = Элементы.СтраницаTrace;
	Иначе
		Элементы.ГруппаСтраницыСкидок.ТекущаяСтраница = Элементы.СтраницаСнапшот;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Скидка предоставляется на %1';uk='Знижка надається на %1'"),
