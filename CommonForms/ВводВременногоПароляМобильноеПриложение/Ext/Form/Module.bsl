...
Процедура Подключаемый_ОбработчикОбратногоОтсчета()
	...
	Если Таймер >= 0 Тогда
		Если Отсрочка = 0 Тогда
			ОшибкаСостояние = СтрШаблон(НСтр("ru='Повторить через %1 сек.';uk='Повторити через %1 сек.'"), Таймер);
		КонецЕсли;	
		ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОбратногоОтсчета", 1, Истина);		
	Иначе
		ОшибкаСостояние = "";
		...
		Элементы.ФормаНастройки.Заголовок = НСтр("ru='Изменить адрес';uk='Змінити адресу'");
...
Процедура ПроверитьВведеныйКод(ТекущийКод)
	...
	ИначеЕсли ЗначениеЗаполнено(ТекущийКод) И СтрДлина(ТекущийКод) <> 6 Тогда
		ОшибкаСостояние = НСтр("ru='Пароль из 6 цифр';uk='Пароль з 6 цифр'");
		Отсрочка		= 10;
	Иначе
		ОшибкаСостояние = НСтр("ru='Пароль не указан';uk='Пароль не вказано'");
		Отсрочка		= 10;
	КонецЕсли;
...
Процедура ПодтвердитьПарольПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Если СтрНайти(ТекстИсключения, "УказанНеверныйПароль") Тогда
			ОшибкаСостояние = НСтр("ru='Указан неверный пароль';uk='Вказаний невірний пароль'");
			Отсрочка		= 10;
			ТекущийЭлемент = Элементы.Цифры;
			#Если МобильноеПриложениеКлиент Тогда
				НачатьРедактированиеЭлемента();
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.СпособДоставкиПаролей = "phone" Тогда
		Если Форма.ПарольОтправляется Тогда
			ТемаОтправки = НСтр("ru='Выполняется отправка пароля в SMS-сообщении на номер';uk='Надсилання пароля у SMS-повідомленні на номер'");
		Иначе
			ТемаОтправки = НСтр("ru='Пароль отправлен в SMS-сообщении на номер';uk='Пароль надіслано у SMS-повідомленні на номер'");
		КонецЕсли;
		Элементы.ФормаНастройки.Заголовок = НСтр("ru='Изменить номер';uk='Змінити номер'");
	...
	ИначеЕсли Форма.СпособДоставкиПаролей = "email" Тогда
		Если Форма.ПарольОтправляется Тогда
			ТемаОтправки = НСтр("ru='Выполняется отправка пароля в письме на адрес';uk='Надсилання пароля в листі на адресу'");
		Иначе
			ТемаОтправки = НСтр("ru='Пароль отправлен в письме на адрес';uk='Пароль надіслано у листі на адресу'");
		КонецЕсли;	
		Элементы.ФормаНастройки.Заголовок = НСтр("ru='Изменить адрес';uk='Змінити адресу'");
