...
Процедура СохранитьКак(Команда)
	...
	Иначе
		Заголовок = НСтр("ru='Выберите каталог для сохранения расширений конфигурации';uk='Виберіть каталог для збереження розширень конфігурації'");
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения расширения конфигурации';uk='Оберіть файл для збереження розширення конфігурації'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru='Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*';uk='Файли розширень конфігурації (*.cfe)|*.cfe|Всі файли (*.*)|*.*'");
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		Если ПодключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			...
			ЭлементРасширения.СостояниеПодключения = НСтр("ru='Подключено';uk='Підключено'");
		ИначеЕсли ОтключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			ЭлементРасширения.Подключено = 2;
			ЭлементРасширения.СостояниеПодключения = НСтр("ru='Отключено';uk='Відключено'");
		Иначе
			ЭлементРасширения.Подключено = 1;
			ЭлементРасширения.СостояниеПодключения = НСтр("ru='Требуется перезапуск';uk='Потрібен перезапуск'");
		КонецЕсли;	
...
Процедура ЗагрузитьРасширениеПослеПодтверждения(Ответ, Контекст) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Расширение конфигурации';uk='Розширення конфігурації'")+ " (*.cfe)|*.cfe";
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru='Выберите файл расширения конфигурации';uk='Оберіть файл розширення конфігурації'");
...
Процедура ЗагрузитьРасширениеЗавершение()
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда    
			ТекстОповещения = НСтр("ru='Расширения конфигурации добавлены';uk='Розширення конфігурації додано'");
		Иначе
			ТекстОповещения = НСтр("ru='Расширение конфигурации добавлено';uk='Розширення конфігурації додано'");
		КонецЕсли;
	Иначе                                            
		ТекстОповещения = НСтр("ru='Расширение конфигурации обновлено';uk='Розширення конфігурації оновлено'");
	КонецЕсли;
				...
				Если Расширения.Количество() > 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Нельзя добавить расширение ""%1""
                                   ...
                                   |;uk='Не можна додати розширення ""%1""
                                   |з файлу ""%2"",
                                   |так як розширення з таким ім''ям вже існує.'"),
						ОписаниеРасширения.Имя,
						ПомещенныйФайл.Имя);
					Прервать;
				КонецЕсли;
		...
		Если ИдентификаторыСтрок <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обновить расширение по причине:
                           |%1'
                           |;uk='Не вдалося оновити розширення через:
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяФайлаДобавляемогоРасширения) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось добавить расширение ""%1""
                           ...
                           |%3'
                           |;uk='Не вдалося додати розширення ""%1""
                           |з файлу ""%2"" через:
                           |%3'"),
				ОписаниеРасширения.Имя,
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось добавить по причине:
                           |%1'
                           |;uk='Не вдалося додати по причині:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецЕсли;
	КонецПопытки;
		...
		Если Расширение.БезопасныйРежим Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После включения безопасного режима, при подготовке расширений к работе, произошла ошибка:
                           |%1'
                           |;uk='Після включення безпечного режиму, при підготовці розширень до роботи, сталася помилка:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После отключения безопасного режима, при подготовке расширений к работе, произошла ошибка:
                           |%1'
                           |;uk='Після відключення безпечного режиму, при підготовці розширень до роботи, сталася помилка:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецЕсли;
	КонецПопытки;
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При попытке отменить изменение флажка безопасного режима расширения произошла еще одна ошибка:
                               |%1'
                               |;uk='При спробі скасувати зміну прапорця безпечного режиму розширення сталася ще одна помилка:
                               |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
		КонецПопытки;
		Если ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			СтрокаСписка.БезопасныйРежимФлаг = Расширение.БезопасныйРежим;
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ НСтр("ru='Отменено изменение флажка безопасного режима расширения.';uk='Скасована зміна прапорця безпечного режиму розширення.'");
		КонецЕсли;
	КонецЕсли;
		...
		Если Расширение.Активно Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После включения расширения, при подготовке расширений к работе, произошла ошибка:
                           |%1'
                           |;uk='Після включення розширення, при підготовці розширень до роботи, сталася помилка:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После отключения расширения, при подготовке расширений к работе, произошла ошибка:
                           |%1'
                           |;uk='Після відключення розширення, при підготовці розширень до роботи, сталася помилка:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецЕсли;
	КонецПопытки;
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При попытке отменить изменение флажка подключения расширения произошла еще одна ошибка:
                               |%1'
                               |;uk='При спробі скасувати зміну прапорця підключення розширення сталася ще одна помилка:
                               |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
		КонецПопытки;
		Если ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			СтрокаСписка.Подключать = Расширение.Активно;
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ НСтр("ru='Отменено изменение флажка подключения расширения.';uk='Скасована зміна прапорця підключення розширення.'");
		КонецЕсли;
	КонецЕсли;
