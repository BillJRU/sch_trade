...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Источник = "ПанельАдминистрированияБСП" Тогда
		Элементы.ОК.Заголовок = НСтр("ru='Изменить';uk='Змінити'")
	КонецЕсли;
	...
	Элементы.ГруппаОсновнойЯзык.Доступность = Ложь;
	Элементы.ГруппаДополнительныеЯзыки.Видимость = Ложь;
...
Процедура УстановитьОсновнойЯзык()
	...
	Если ПустаяСтрока(ОсновнойЯзык) ИЛИ ОсновнойЯзык <> МультиязычностьУкр.КодЯзыкаИнформационнойБазы() Тогда
		ОсновнойЯзык = МультиязычностьУкр.КодЯзыкаИнформационнойБазы();
	КонецЕсли;
...
Процедура ОК(Команда)
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Указаны некорректные значения региональных настроек';uk='Вказані некоректні значення регіональних настройок'"));
	КонецЕсли;
...
Функция ЗапуститьФоновоеПерезаполнениеНаСервере(Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Перезаполнение строк предопределенных элементов и классификаторов.';uk='Перезаповнення рядків напередвизначених елементів і класифікаторів.'");
...
Процедура ПослеФоновогоПерезаполнения(Задание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = НСтр("ru='Не удается перезаполнить строки предопределенных элементов и классификаторов.';uk='Не вдається перезаповнити рядки визначених елементів і класифікаторів.'");
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + Задание.ПодробноеПредставлениеОшибки;
