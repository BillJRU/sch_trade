...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Schueco_SCT: кешируем адрес ВПХ — Параметры.* живёт только в ПриСозданииНаСервере, а нам нужно
	// записать туда выбор режима в ОКФрагмент уже на клиенте.
	АдресВПХ_SCT = Параметры.АдресВоВременномХранилище;

	// Schueco_SCT: переключатель базы распределения ручной скидки по строкам.
	// Виден только когда вообще есть смысл — есть автоматические скидки и это не закупки.
	БазаРасчетаРаспределения = 0;
	Элементы.БазаРасчетаРаспределения.СписокВыбора.Добавить(0, НСтр("ru='От суммы без скидок';uk='Від суми без знижок'"));
	Элементы.БазаРасчетаРаспределения.СписокВыбора.Добавить(1, НСтр("ru='От суммы с автоматической скидкой';uk='Від суми з автоматичною знижкою'"));
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"БазаРасчетаРаспределения",
		"Видимость",
		ИспользоватьАвтоматическиеСкидкиВПродажах И Не ЭтоЗакупки,
		Истина);
...
Процедура ОКФрагмент()
    ...
    // Schueco_SCT: пишем выбранный режим базы распределения в ВПХ — НазначитьРучнуюСкидку прочитает оттуда.
    // Адрес ВПХ закеширован в ПриСозданииНаСервере (Параметры.* недоступен после создания формы).
    ЗаписатьБазуРасчетаВПХ(АдресВПХ_SCT, БазаРасчетаРаспределения);
...
Процедура ЗаписатьБазуРасчетаВПХ(АдресВПХ, БазаРасчета)

    // Schueco_SCT: канал для передачи режима из диалога в СкидкиНаценкиСервер.НазначитьРучнуюСкидку.
    Если НЕ ЗначениеЗаполнено(АдресВПХ) Тогда
        Возврат;
    КонецЕсли;
    Данные = ПолучитьИзВременногоХранилища(АдресВПХ);
    Если ТипЗнч(Данные) <> Тип("Структура") Тогда
        Возврат;
    КонецЕсли;
    Данные.Вставить("БазаРасчетаРаспределения_SCT", БазаРасчета);
    ПоместитьВоВременноеХранилище(Данные, АдресВПХ);
