...
Процедура НайтиКонтактыВыполнить()
	...
	Если ПустаяСтрока(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не задано, что нужно найти.';uk='Не задано, що потрібно знайти.'"),, "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УстановитьАдресКонтактаВыполнить()
	...
	ТекущиеДанные = Элементы.ПолучателиПисьма.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Необходимо выбрать адрес получателя в списке справа.';uk='Необхідно вибрати адресу одержувача в списку праворуч.'"));
		Возврат;
	КонецЕсли;
	...
	Если Результат.Количество() = 0 Тогда
		ПоказатьПредупреждение(, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У контакта ""%1"" нет адресов электронной почты.';uk='У контакту ""%1"" немає адреси електронної пошти.'"),
