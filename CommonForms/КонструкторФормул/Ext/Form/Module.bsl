...
Процедура УстановитьПояснениеКОперандам(Форма, Отбор)
	...
	Если ЕстьПодразделениеПродажи Тогда
		ТекстПояснения = Новый ФорматированнаяСтрока(ТекстПояснения,
														...
														Новый ФорматированнаяСтрока(НСтр("ru='подразделению продажи';uk='підрозділу продажу'") + " ",
	...
	Если ЕстьПодразделениеЗакупки Тогда
		ТекстПояснения = Новый ФорматированнаяСтрока(ТекстПояснения,
														?(ЕстьСклад ИЛИ ЕстьФорматМагазина,", ",""),
														Новый ФорматированнаяСтрока(НСтр("ru='подразделению закупки';uk='підрозділу купівлі'") + " ",
	...
	Если ЕстьПодразделениеДиспетчер Тогда
		ТекстПояснения = Новый ФорматированнаяСтрока(ТекстПояснения,
														?(ЕстьСклад ИЛИ ЕстьФорматМагазина,", ",""),
														Новый ФорматированнаяСтрока(НСтр("ru='подразделению диспетчеру';uk='підрозділу диспетчеру'") + " ",
	...
	Если ЕстьПодразделениеВнутреннихПотреблений Тогда
		ТекстПояснения = Новый ФорматированнаяСтрока(ТекстПояснения,
			?(ЕстьСклад ИЛИ ЕстьФорматМагазина,", ",""),
			Новый ФорматированнаяСтрока(НСтр("ru='подразделению внутренних потреблений';uk='підрозділу внутрішніх споживань'") + " ",
	...
	Если ЕстьНазначение Тогда
		ТекстПояснения = Новый ФорматированнаяСтрока(ТекстПояснения,
														...
														Новый ФорматированнаяСтрока(НСтр("ru='назначению';uk='призначенням'") + " ",
														"""", Новый ФорматированнаяСтрока(?(ЗначениеЗаполнено(Отбор.ОтборНазначение), Строка(Отбор.ОтборНазначение),НСтр("ru='Без назначения';uk='Без призначення'")), Шрифт),""""));
	КонецЕсли;
...
Процедура ЗагрузитьДеревоОперандовИзХранилища(АдресХранилища)
	...
	Дерево = ПолучитьИзВременногоХранилища(АдресХранилища); // см. ОбщиеФормы.КонструкторФормул.Форма.ДеревоОперандов
