...
Процедура ПриОткрытии(Отказ)
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		ПоказатьСообщение("Получение настроек", Истина);
...
Процедура ПовторитьКодНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьСообщение(НСтр("ru='Выполняется отправка пароля в SMS-сообщении на номер';uk='Надсилання пароля у SMS-повідомленні на номер'"), Истина);
...
Процедура Подтвердить(Команда)
	...
	ИначеЕсли НЕ ПарольПроверен Тогда
		ПоказатьПредупреждение(, 
            ...
            |;uk='З метою безпеки, для відключення підтвердження операцій з ключем, 
            |слід правильно вказати пароль користувача 1С.'"), 30, 
			НСтр("ru='Процедура не закончена.';uk='Процедура не закінчена.'"));
	...
	Иначе
		ПоказатьПредупреждение(, 
            ...
            |;uk='З метою безпеки, для відключення підтвердження операцій з ключем, 
            |слід повністю пройти процедуру ідентифікації за SMS.'"), 30, 
			НСтр("ru='Процедура не закончена.';uk='Процедура не закінчена.'"));
...
Процедура ОбратныйОтсчетКодТелефона()
	...
	ИначеЕсли ОтключитьОбратныйОтсчет <> 99 Тогда
		Если Таймер > 0 Тогда
			ПовторитьКод = СтрШаблон(НСтр("ru='Запросить пароль повторно можно будет через %1 сек.';uk='Запитати пароль повторно можна буде через %1 сек.'"), Таймер);
		ИначеЕсли Элементы.ПовторитьКод.Гиперссылка = Ложь Тогда
			ПовторитьКод = НСтр("ru='Отправить пароль повторно';uk='Відправити пароль повторно'");
		...
		ИначеЕсли ТаймерОбший > 0 Тогда
			СтатусКода = СтрШаблон(НСтр("ru='Истекает через %1:%2';uk='Завершується через %1: %2'"), Цел(ТаймерОбший / 60), Формат(ТаймерОбший % 60, "ЧЦ=2; ЧВН="));
		Иначе
			СтатусКода = НСтр("ru='Истек срок действия пароля';uk='Закінчився термін дії пароля'");
...
Процедура ПолучитьВременныйПарольПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ПоказатьСообщение(НСтр("ru='Пароль отправлен на номер: ';uk='Пароль відправлений на номер:'") + ПредставлениеТелефона(Получатель));

		ЗапуститьОбратныйОтсчетКодТелефона(Таймер, 600);
		УправлениеФормой(ЭтотОбъект);
		
	Иначе
		ПоказатьСообщение(НСтр("ru='Сервис отправки SMS-сообщений временно недоступен.';uk='Сервіс відправки SMS-повідомлень тимчасово недоступний.'"), , Истина);
...
Процедура ПроверитьКодТелефона(ТекущийКод)
	...
	ИначеЕсли ВыполняетсяПроверка Тогда
		СтатусКода 	= НСтр("ru='Ожидается ответ';uk='Очікується відповідь'");
		ОтключитьОбратныйОтсчет = 10;
		
	ИначеЕсли ОтключитьОбратныйОтсчет = 99 Тогда
		СтатусКода 	= НСтр("ru='Необходимо повторить процедуру';uk='Необхідно повторити процедуру'");
		
	ИначеЕсли ЗначениеЗаполнено(ТекущийКод) И СтрДлина(ТекущийКод) = 6 Тогда
		ВыполняетсяПроверка = Истина;
		ПоказатьСообщение(НСтр("ru='Выполняется проверка пароля';uk='Виконується перевірка пароля'"), Истина);
	...
	ИначеЕсли ЗначениеЗаполнено(ТекущийКод) И СтрДлина(ТекущийКод) <> 6 Тогда
		СтатусКода 	= НСтр("ru='Пароль должен быть из 6 цифр';uk='Пароль повинен бути з 6 цифр'");
		ОтключитьОбратныйОтсчет = 10;
		
	Иначе
		СтатусКода 	= НСтр("ru='Пароль не указан';uk='Пароль не вказано'");
		ОтключитьОбратныйОтсчет = 10;
...
Процедура ПолучитьНастройкиПолученияВременныхПаролейПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе	
		ПоказатьСообщение(НСтр("ru='Сервис отправки SMS-сообщений временно недоступен. Повторите попытку позже.';uk='Сервіс відправки SMS-повідомлень тимчасово недоступний. Повторіть спробу пізніше.'"), , Истина);
...
Функция ПодготовитьМассивРесурсов()
	...
	Если Метаданные.ОбщиеКартинки.Найти("ВидимостьЗакрыто") <> Неопределено Тогда
		Результат.Добавить(БиблиотекаКартинок.ВидимостьЗакрыто);
	Иначе
		Результат.Добавить(Новый Картинка);
	КонецЕсли;	
	Если Метаданные.ОбщиеКартинки.Найти("ВидимостьОткрыто") <> Неопределено Тогда
		Результат.Добавить(БиблиотекаКартинок.ВидимостьОткрыто);
	Иначе
		Результат.Добавить(Новый Картинка);
	КонецЕсли;	
...
Процедура ПодтвердитьПарольПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		СтатусКода		= НСтр("ru='Указан верный пароль';uk='Вказано вірний пароль'");
		ПроверитьСледующийШаг();
		УправлениеФормой(ЭтотОбъект);
	...
	Иначе
		...
		Если СтрНайти(ТекстИсключения, "УказанНеверныйПароль") Тогда
			СтатусКода = НСтр("ru='Указан неверный пароль';uk='Вказаний невірний пароль'");
			ОтключитьОбратныйОтсчет = 10;
		ИначеЕсли СтрНайти(ТекстИсключения, "ПревышенЛимитПопытокВводаПароля") Тогда
			СтатусКода = НСтр("ru='Превышен лимит попыток';uk='Перевищено ліміт спроб'"); 
			ИдентификаторСессии = "";
			ОтключитьОбратныйОтсчет = 99;
		ИначеЕсли СтрНайти(ТекстИсключения, "СрокДействияПароляИстек") Тогда
			СтатусКода = НСтр("ru='Срок действия пароля истек';uk='Термін дії пароля закінчився '");
			ИдентификаторСессии = "";
			ОтключитьОбратныйОтсчет = 99;
		Иначе 
			СтатусКода = НСтр("ru='Выполнение операции временно невозможно';uk='Виконання операції тимчасово неможливе'");
			ОтключитьОбратныйОтсчет = 99;
		КонецЕсли;		
	КонецЕсли;
