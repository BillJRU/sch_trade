...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобразитьСостояниеОтчета(НСтр("ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';uk='Звіт не сформований. Натисніть ""Сформувати"" для одержання звіту.'"));
...
Процедура ЗагрузитьСхему(Команда)
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Файлы XML (*.xml)|*.xml';uk='Файли XML (*.xml)|*.xml'");
...
Процедура ОбновитьВариантОтчетаИзФайла(Команда)
	...
	ПараметрыЗагрузки = ФайловаяСистемаКлиент.ПараметрыЗагрузкиФайла();
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Архив ZIP (*.zip)|*.zip';uk='Архів ZIP (*.zip)|*.zip'");
	ПараметрыЗагрузки.ИдентификаторФормы = УникальныйИдентификатор;
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	Если Результат = Неопределено Тогда 
		ПоказатьОшибкиФормирования(НСтр("ru='Формирование отчета прервано администратором';uk='Формування звіту перервано адміністратором'"));
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Если Результат.Статус <> "Выполняется" Тогда
		ЗагрузитьРезультатФормированияОтчета();
	Иначе	
		ОтобразитьСостояниеОтчета(НСтр("ru='Отчет формируется...';uk='Звіт формується...'"), БиблиотекаКартинок.ДлительнаяОперация48);
	КонецЕсли;
	Возврат Результат;
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	Если Не ЭтоАдресВременногоХранилища(ФоновоеЗаданиеАдресХранилища) Тогда 
		ПоказатьОшибкиФормирования(НСтр("ru='Не удалось сформировать отчет';uk='Не вдалося сформувати звіт'"));
		Возврат;
	КонецЕсли;
	...
	Если Результат = Неопределено Тогда 
		ПоказатьОшибкиФормирования(НСтр("ru='Не удалось сформировать отчет (пустой результат)';uk='Не вдалося сформувати звіт (порожній результат)'"));
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьПереданныеНастройки(ОписаниеНастроек)
	Если ВариантМодифицирован Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Вариант отчета был изменен.
            ...
            |;uk='Варіант звіту був змінений.
            |Збережіть зміни перед застосуванням настройок.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	УстановитьТекущийВариант(КлючВарианта);
	ОтобразитьСостояниеОтчета(НСтр("ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';uk='Обраний інший варіант звіту. Натисніть кнопку ""Сформувати"" для одержання звіту.'"),
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.КомандыИПоказатели.Заголовок = НСтр("ru='показатели';uk='показники'");
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	...
	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыОбновления.ВариантМодифицирован
			...
			ОтобразитьСостояниеОтчета(НСтр("ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';uk='Змінилися настройки. Натисніть кнопку ""Сформувати"" для одержання звіту.'"));
		КонецЕсли;
	КонецЕсли;
