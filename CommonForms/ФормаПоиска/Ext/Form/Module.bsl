...
Процедура ОбновитьФорму(РезультатыПоиска)
	...
	ИначеЕсли СостояниеПоиска = "ВыполняетсяОбновлениеИндекса"
		...
		СостояниеПоискаПредставление = НСтр("ru='Результаты поиска могут быть неточными, повторите поиск позднее.';uk='Результати пошуку можуть бути неточними, повторіть пошук пізніше.'");
	ИначеЕсли СостояниеПоиска = "ОшибкаНастройкиПоиска" Тогда 
		
		// Для не администратора
		СостояниеПоискаПредставление = НСтр("ru='Полнотекстовый поиск не настроен, обратитесь к администратору.';uk='Повнотекстовий пошук не налаштований, зверніться до адміністратора.'");
		
	ИначеЕсли СостояниеПоиска = "ПоискЗапрещен" Тогда 
		СостояниеПоискаПредставление = НСтр("ru='Полнотекстовый поиск отключен.';uk='Повнотекстовий пошук відключений.'");
	КонецЕсли;
...
Процедура ОбновитьПредставлениеОбластиПоиска()
	...
	Если Не ИскатьВРазделах Или Не УказаныОбластиПоиска Тогда
		ОбластиПоискаПредставление = НСтр("ru='Везде';uk='Скрізь'");
		Возврат;
	КонецЕсли;
	...
	Иначе	
		ОбластиПоискаПредставление = НСтр("ru='В выбранных разделах';uk='У вибраних розділах'");
	КонецЕсли;
