...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		Если Не Параметры.Свойство("ТипЭлементаСтруктурыНастроек", ТипЭлементаСтруктурыНастроек) Тогда
			ВызватьИсключение НСтр("ru='Не передан служебный параметр ""ТипЭлементаСтруктурыНастроек"".';uk='Не переданий службовий параметр ""Тип елемента структури настройок"".'");
		КонецЕсли;
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройки отчета ""%1""';uk='Настройки звіту ""%1""'"), ВариантНаименование);
	КонецЕсли;
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ИмяФайла = НСтр("ru='Настройки.xml';uk='Настройки.xml'");
	
	ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения настроек отчета';uk='Виберіть файл для збереження настройок звіту'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru='Настройки отчета (*.xml)|*.xml';uk='Настройки звіту (*.xml)|*.xml'");
...
Процедура ОбновитьПоляГруппировки()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоПолеГруппировкиКомпоновкиДанных") Тогда 
			Строка.Заголовок  = НСтр("ru='Авто (по всем полям)';uk='Авто (по всіх полях)'");
...
Процедура ОбновитьПараметрыДанных()
	...
	Раздел.Заголовок = НСтр("ru='Параметры';uk='Параметри'");
...
Процедура ОбновитьОтборы(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда 
		Раздел = Отборы.ПолучитьЭлементы().Добавить();
		Раздел.ЭтоРаздел = Истина;
		Раздел.Заголовок = НСтр("ru='Отборы';uk='Відбори'");
...
Функция ПараметрыГруппировкиОтборов()
	...
	Если Строки.Количество() = 0 Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Выберите элементы.';uk='Оберіть елементи.'"));
		Возврат Неопределено;
	КонецЕсли;
	
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути згруповані, оскільки вони належать різним батькам.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыСдвигаОтборов()
	...
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути переміщені, оскільки вони належать різним батькам.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда 
		...
		Раздел.ЭтоРаздел = Истина;
		Раздел.Заголовок = НСтр("ru='Поля';uk='Поля'");
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("ru='Авто (поля родителя)';uk='Авто (поля батька)'");
			Строка.Картинка = 6;
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьСортировку()
	...
	Раздел.ЭтоРаздел = Истина;
	Раздел.Заголовок = НСтр("ru='Сортировки';uk='Сортування'");
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоЭлементПорядкаКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("ru='Авто (сортировки родителя)';uk='Авто (сортування батька)'");
...
Процедура ОбновитьОформление()
	...
	Иначе
		...
		Раздел.Заголовок = НСтр("ru='Условное оформление';uk='Умовне оформлення'");
		Раздел.Представление = НСтр("ru='Условное оформление';uk='Умовне оформлення'");
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Заголовок: %1';uk='Заголовок: %1'"),
		?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение, "<Отсутствует>"));
	Строка.Представление = НСтр("ru='Заголовок';uk='Заголовок'");
	...
	Строка.Заголовок = НСтр("ru='Выводить параметры и отборы';uk='Виводити параметри і відбори'");
	Строка.Представление = НСтр("ru='Выводить параметры и отборы';uk='Виводити параметри і відбори'");
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Элемент условного оформления отчета ""%1""';uk='Елемент умовного оформлення звіту ""%1""'"), ВариантНаименование));
...
Процедура ОбновитьСтруктуру()
	...
	Раздел.Представление = НСтр("ru='Отчет';uk='Звіт'");
	Раздел.ЭтоРаздел = Истина;
	Раздел.Картинка = -1;
...
Процедура ОбновитьКоллекциюСтруктуры(Знач Узел, Знач ИмяКоллекции, Строки)
	...
	ИначеЕсли ИменаКоллекций.Найти(ИмяКоллекции) <> Неопределено
		...
		ИначеЕсли ИмяКоллекции = "Строки" Тогда 
			Строка.Представление = НСтр("ru='Строки';uk='Рядки'");
		ИначеЕсли ИмяКоллекции = "Колонки" Тогда 
			Строка.Представление = НСтр("ru='Колонки';uk='Колонки'");
		ИначеЕсли ИмяКоллекции = "Точки" Тогда 
			Строка.Представление = НСтр("ru='Точки';uk='Точки'");
		ИначеЕсли ИмяКоллекции = "Серии" Тогда 
			Строка.Представление = НСтр("ru='Серии';uk='Серії'");
		КонецЕсли;
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	ИначеЕсли ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда 
		...
		Если Не ЗначениеЗаполнено(СвойстваЭлемента.Представление) Тогда
			СвойстваЭлемента.Представление = НСтр("ru='Вложенная группировка';uk='Вкладене групування'");
		КонецЕсли;
		
	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда 
		
		СвойстваЭлемента.Представление = НСтр("ru='Таблица';uk='Таблиця'");
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда 
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
	...
	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Тогда 
		...
		СвойстваЭлемента.Представление = НСтр("ru='Диаграмма';uk='Діаграма'");
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда 
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
...
Функция ПредставлениеПолейГруппировки(ЭлементНастройки, ПометкаУдаления)
	...
	Если Поля.Элементы.Количество() = 0 Тогда 
		Возврат НСтр("ru='<Детальные записи>';uk='<Детальні записи>'");
	КонецЕсли;
...
Процедура ДобавитьЭлементСтруктурыНастроек(ТипЭлемента)
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		...
		Строка.Представление = НСтр("ru='Диаграмма';uk='Діаграма'");
		...
		ПодчиненнаяСтрока.Представление = НСтр("ru='Точки';uk='Точки'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru='Серии';uk='Серії'");
	ИначеЕсли Строка.Тип = "ТаблицаКомпоновкиДанных" Тогда
		Строка.Представление = НСтр("ru='Таблица';uk='Таблиця'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru='Строки';uk='Рядки'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru='Колонки';uk='Колонки'");
	КонецЕсли;
...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		ПредставлениеЭлемента = НСтр("ru='диаграммы';uk='діаграми'");
	Иначе
		ПредставлениеЭлемента = НСтр("ru='группировки';uk='групування'");
	КонецЕсли;
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	Если ЭтоРасширенныйРежим Тогда
		Элементы.СтраницаОтборы.Заголовок = НСтр("ru='Отборы';uk='Відбори'");
	Иначе
		Элементы.СтраницаОтборы.Заголовок = НСтр("ru='Основное';uk='Основне'");
	КонецЕсли;
	...
	Иначе
		Элементы.ЗакрытьИСформировать.Заголовок = НСтр("ru='Закрыть и сформировать';uk='Закрити і сформувати'");
