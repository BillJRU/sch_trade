...
Процедура ПочтовыйАдресаПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(Адрес, Истина) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Необходимо ввести корректный адрес электронной почты.';uk='Необхідно ввести коректну адресу електронної пошти.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьПисьмо()
	...
	Если ПоляЗаполненыКорректно() И ОтправитьПочтовоеСообщение(ЕстьОшибочныеПолучатели) Тогда
		...
		Если ЕстьОшибочныеПолучатели Тогда
			ПоказатьПредупреждение(, НСтр("ru='Письмо отправлено не всем получателям.';uk='Лист відправлено не всім одержувачам.'"));
		Иначе
			Закрыть();
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение табличного документа';uk='Одержання табличного документа'",ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , 
