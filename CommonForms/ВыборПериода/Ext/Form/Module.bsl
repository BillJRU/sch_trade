...
Процедура ОбновитьОтображение()
	...
	Если РежимВыбораПериода = "МЕСЯЦ" Тогда
		...
		КонецЦикла;
		ПериодСтрокой = МультиязычностьУкр.ФорматДляИнтерфейса(ВыбираемыйПериод, "ДФ='ММММ гггг'")
	...
	ИначеЕсли РежимВыбораПериода = "КВАРТАЛ" Тогда
		...
		ПериодСтрокой = Формат(КварталМесяца, "ЧЦ=1") + НСтр("ru=' квартал ';uk=' квартал '") + Формат(ВыбираемыйПериод,"ДФ=гггг");
