...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеБезналичныхДенежныхСредств) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru='Сформировать списания ДС...';uk='Сформувати списання ГК ...'") ;
...
Процедура ПеренестиЗаявки(Параметры, АдресРезультата) Экспорт
		...
		Для каждого ОбъектОплаты Из ДанныеПереноса Цикл
			...
			Исключение
				ТекстСообщения = СтрШаблон(
					НСтр("ru='%1 находится в процессе редактирования пользователем или системой и не может быть изменена.';uk='%1 знаходиться в процесі редагування користувачем або системою і не може бути змінена.'"),
					ЗаявкаОбъект);
			...
			Если ЗаявкаОбъект.ПроверитьЗаполнение() Тогда
				...
				НаборЗаписей.Загрузить(Таблицы.ТаблицаДенежныеСредстваКВыплате);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось перенести заявки по причине: %1';uk='Не вдалося перенести заявки через: %1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		
		ТекстОшибки = НСтр("ru='Не удалось перенести заявки.';uk='Не вдалося перенести заявки.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Для каждого Заявка Из СписокЗаявок Цикл
		...
		Если СуммаРаспределения Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Не удалось распределить оплату документа %1';uk='Не вдалося розподілити оплату документа %1'"), Заявка),
				Заявка);
		КонецЕсли;
	КонецЦикла;
