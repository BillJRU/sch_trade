...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РежимЗаявкиКалендарь.Заголовок = НСтр("ru='Заявки';uk='Заявки'") + " " + Символ(8594) + " " + НСтр("ru='Календарь';uk='Календар'");
	Элементы.РежимКалендарьПлатежи.Заголовок = НСтр("ru='Календарь';uk='Календар'") + " " + Символ(8594) + " " + НСтр("ru='Платежи';uk='Платежі'");
	Элементы.РежимСписокЗаявок.Заголовок = НСтр("ru='Список заявок';uk='Список заявок'");
...
Функция СформироватьСписанияДСНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В списке нет заявок к оплате для формирования документов списания.';uk='У списку немає заявок до оплати для формування документів списання.'"));
...
Процедура ОтборОрганизацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление", НСтр("ru='Отбор организаций';uk='Відбір організацій'"));
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='не распределено';uk='не розподілено'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Остаток';uk='Залишок'"));
...
Процедура УстановитьОтборВКалендареНаСервере()
	...
	Если ТипЗнч(ТекущаяГруппировкаДерева) = Тип("Строка") Тогда
		Если Найти(ТекущаяГруппировкаДерева, НСтр("ru='не распределено';uk='не розподілено'")) Тогда
			ВидСравненияСчетаКассы = ВидСравненияКомпоновкиДанных.НеЗаполнено;
			ИспользоватьОтборПоСчетуКассе = Истина;
		ИначеЕсли Найти(ТекущаяГруппировкаДерева, НСтр("ru='распределено';uk='розподілено'")) Тогда
...
Процедура ОткрытьФормуНастройкиБанковскихСчетов()
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОграничиватьВыборУказаннымиЗначениями", Истина);
	ПараметрыФормы.Вставить("Представление", НСтр("ru='Отбор банковских счетов / касс';uk='Відбір банківських рахунків / кас'"));
...
Процедура ОтметитьФильтрСчетов()
	...
	Если Объект.ФильтрСчетовКалендаря = 0 Тогда
		ЗаголовокФильтра = НСтр("ru='Счета / Кассы: Все';uk='Рахунки / Каси: Всі'");
	ИначеЕсли Объект.ФильтрСчетовКалендаря = 1 Тогда
		ЗаголовокФильтра = НСтр("ru='Счета / Кассы: Выбранные';uk='Рахунки / Каси: Вибрані'");
	ИначеЕсли Объект.ФильтрСчетовКалендаря = 2 Тогда
		ЗаголовокФильтра = НСтр("ru='Счета / Кассы: Подходящие';uk='Рахунки / Каси: Відповідні'");
	КонецЕсли;
...
Процедура ДополнитьКолонкиДней()
	...
	Если ЭлементДень0 <> Неопределено Тогда
		ЭлементДень0.Заголовок = НСтр("ru='Просрочено';uk='Прострочено'");
...
Процедура СдвинутьЗаявки(ИмяКоманды)
	...
	ИначеЕсли ИмяКоманды = "НаДату" Тогда
		...
		ТекстПодсказки = НСтр("ru='Выберите дату для переноса оплаты заявок.';uk='Виберіть дату для перенесення оплати заявок.'");
	...
	ИначеЕсли ИмяКоманды = "НаСчет" Тогда
		...
		ТекстПодсказки = НСтр("ru='Выберите банковский счет для переноса оплаты заявок.';uk='Виберіть банківський рахунок для перенесення оплати заявок.'");
		...
		Иначе
			ТекстСообщения = НСтр("ru='Нет доступных счетов для распределения платежа.';uk='Немає доступних рахунків для розподілу платежу.'");
	...
	ИначеЕсли ИмяКоманды = "Разбить" Тогда
		...
		Если ЗаявкиКПереносу.Количество() > 1 Тогда
			...
			ТекстСообщения = НСтр("ru='Разбиение оплаты доступно при условии выделения одной заявки.';uk='Розбиття оплати доступно за умови виділення однієї заявки.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПеренестиЗаявкиСервер(ЗаявкиКПереносу, ДатаПлатежа, БанковскийСчетКасса)
	...
	Если Не ДанныеПереноса.Количество() Тогда
		...
		Результат.Вставить("КраткоеПредставлениеОшибки",
			НСтр("ru='Не удалось обработать заявки.';uk='Не вдалося обробити заявки.'"));
		Результат.Вставить("ПодробноеПредставлениеОшибки", "");
		
		Возврат Результат;
	КонецЕсли;
	
	НаименованиеЗадания = НСтр("ru='Перенос заявок на расходование денежных средств';uk='Перенесення заявок на витрачання грошових коштів'");
...
Функция РаспределитьКОплатеНаСервере(ЗаявкиКРаспределению)
	...
	НаименованиеЗадания = НСтр("ru='Распределение заявок на расходование денежных средств';uk='Розподіл заявок на витрачання грошових коштів'");
	ВыполняемыйМетод = "Обработки.ПлатежныйКалендарь.ПеренестиЗаявки";
...
Процедура РассчитатьПереносВСтрокахДерева(СтрокаДерева, СтруктураПереноса, НайденаСтрокаОборот = Истина)
	...
	Если СтрокаДерева.ВидСтроки = 1
		...
		Если ТипЗнч(СтрокаДерева.Группировка) = Тип("Строка")
			И Найти(СтрокаДерева.Группировка, НСтр("ru='не распределено';uk='не розподілено'")) Тогда
...
Процедура ЗаполнитьСписокСчетовКассНаСервере()
	...
	Запрос.УстановитьПараметр("СтрокаНераспределенныеПлатежи", НСтр("ru='<Нераспределенные платежи>';uk='<Нерозподілені платежі>'"));
...
Функция ПроверитьСтрокиПлатежей(СтрокиПлатежей)
	...
	ТекстСообщения = НСтр("ru='Изменять в платежном календаре можно только заявки на расходование денежных средств.';uk='Змінювати в платіжному календарі можна тільки заявки на витрачання грошових коштів.'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Возврат Ложь;
