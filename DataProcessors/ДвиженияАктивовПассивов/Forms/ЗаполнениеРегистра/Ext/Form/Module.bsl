...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НеЗаписыватьДвижения") Тогда
		...
		Элементы.НадписьВыполняетсяЗаполнение.Заголовок = НСтр("ru='Выполняется тестирование';uk='Виконується тестування'") + "...";
	КонецЕсли;
...
|;uk='Виконується формування рухів управлінського балансу. 
|Формування запустив %1 %2.'");
...
Функция ПолучитьДанныеНаСервере(Параметры)
	...
	НаименованиеЗадания = НСтр("ru='Заполнение регистра активов и пассивов';uk='Заповнення реєстру активів і пасивів'");
...
Процедура ОбновитьПрогресс(ДанныеЗадания)
	...
	Сведения = Константы.СведенияОбОбновленииИБ.Получить().Получить();
	Если ТипЗнч(Сведения) = Тип("Структура") И Сведения.Свойство("ЗаполнениеАктивовПассивов") Тогда
		Процесс = Сведения.ЗаполнениеАктивовПассивов; // см. Обработка.ДвиженияАктивовПассивов.ИнициализироватьСведенияОЗаполнении
