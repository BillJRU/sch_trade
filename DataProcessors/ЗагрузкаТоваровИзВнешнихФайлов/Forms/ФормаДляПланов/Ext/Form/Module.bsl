	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТаблицыТоваров) Тогда
		...
		Для каждого СтруктураТовар Из МассивСтруктурТоваров Цикл
			...
			Если СтруктураТовар.Свойство("ИдентификаторУпаковка") 
				...
				СтруктураТовар.Вставить("Упаковка",       ?(Найти(Строка(Упаковка), НСтр("ru='<Объект не найден>';uk='<Об`єкт не знайдено>'"))> 0, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Упаковка));
			КонецЕсли; 
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	МакетЗаполненияПоВнешнемуИсточнику = Обработки.ЗагрузкаТоваровИзВнешнихФайлов.ПолучитьМакет("МакетЗагрузкиДанныхДляПланов" + СуфиксМакета);
