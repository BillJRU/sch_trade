...
Процедура УправлениеЭлементами()
	...
	Если НЕ ИспользоватьСделки Тогда
		...
		Элементы.ГруппаНастроекСделки.Заголовок = НСтр("ru='Проекты';uk='Проєкти'");
	...
	Если ИспользоватьПатнеровКакКонтрагентов Тогда
		...
		Если КомандаРолиПартнеровВСделкахИПроектах <> Неопределено Тогда
			КомандаРолиПартнеровВСделкахИПроектах.Заголовок = НСтр("ru='Роли контрагентов в сделках и проектах';uk='Ролі контрагентів в угодах та проєктах'");
			КомандаРолиПартнеровВСделкахИПроектах.Подсказка = 
			       НСтр("ru='Классификатор, используемый для понимания, какую роль выполняют косвенно или непосредственно участвующие в сделке или проекте контрагенты.';uk='Класифікатор, що використовується для розуміння, яку роль виконують контрагенти, які побічно або безпосередньо беруть участь в угоді або проєкті.'");
		КонецЕсли;
	КонецЕсли;
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	           ...
	           ИспользоватьПартнеровКакКонтрагентов,
	           НСтр("ru='Классификатор деления контрагентов на группы, по которым можно ограничивать доступ на просмотр, добавление (изменение) партнеров и доступ к документам по партнерам.';uk='Класифікатор розподілу контрагентів на групи, по яких можна обмежувати доступ на перегляд, додавання (зміну) партнерів і доступ до документів по партнерах.'"));
