...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеФоновогоЗаданияПерехода  = НСтр("ru='Заполнение регистров расчетов в новой архитектуре';uk='Заповнення регістрів розрахунків в новій архітектурі'");
...
Процедура НоваяАрхитектураРасчетовПриИзменении(Элемент)
	...
	Если РежимРаспределенияВзаиморасчетов И НЕ НаборКонстант.НоваяАрхитектураВзаиморасчетов Тогда
		Ответ = Неопределено;
		ТекстВопроса = НСтр("ru='Включение новой архитектуры может занять продолжительное время. Продолжить?';uk='Включення нової архітектури може зайняти багато часу. Продовжити?'");
...
|;uk='Після відключення нової архітектури буде необхідно виконати розподіл взаєморозрахунків по документах, 
|і в разі потреби виконати закриття місяця (з %1 р.). Продовжити?'"),
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.НоваяАрхитектураВзаиморасчетов"
		...
		Элементы.НоваяАрхитектураВзаиморасчетов.Доступность = ДоступноВключениеНовойАрхитектуры;
		
		ТекстКомментария = НСтр("ru='Не закончено отложенное обновление регистров взаиморасчетов.';uk='Не закінчено відкладене оновлення регістрів взаєморозрахунків.'");
		Если ЗаданияПереносаРасчетов.Количество() > 0 Тогда
			ТекстКомментария = НСтр("ru='Выполняется перенос данных взаиморасчетов на новую архитектуру.';uk='Виконується перенесення даних взаєморозрахунків на нову архітектуру.'");
		КонецЕсли;
...
Функция ЗапуститьРаспределениеРасчетовФоновымЗаданием ()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =НСтр("ru='Выполняется распределение офлайн расчетов';uk='Виконується розподіл офлайн розрахунків'");
...
Функция ОчиститьРегистрыВзаиморасчетов()
	...
	НаименованиеЗадания = НСтр("ru='Очистка регистров при отключении новой архитектуры расчетов';uk='Очищення регістрів при відключенні нової архітектури розрахунків'");
...
Процедура УстановитьСписокВыбораВариантаИспользованияЛимитов()
	...
	Если СписокВыбора.НайтиПоЗначению(ЗначениеВыбора) = Неопределено Тогда
		ПредставлениеЗначения = НСтр("ru='Не используются';uk='Не використовуються'");
		СписокВыбора.Добавить(ЗначениеВыбора, ПредставлениеЗначения);
	КонецЕсли;
	
	ЗначениеВыбора = "ДокументЛимитов";
	Если СписокВыбора.НайтиПоЗначению(ЗначениеВыбора) = Неопределено Тогда
		ПредставлениеЗначения = НСтр("ru='По документам лимитов расхода ДС';uk='За документами лімітів витрачання ГК'");
		СписокВыбора.Добавить(ЗначениеВыбора, ПредставлениеЗначения);
	КонецЕсли;
...
Процедура ДополнитьТекстРасширеннойПодсказкиВариантаИспользованияЛимитов()
	...
	Если ВариантИспользованияЛимитов = "" Тогда
		ТекстЗаголовка = НСтр("ru='Лимиты расхода денежных средств не используются.';uk='Ліміти витрачання грошових коштів не використовуються.'");
	ИначеЕсли ВариантИспользованияЛимитов = "ДокументЛимитов" Тогда
		ТекстЗаголовка = НСтр("ru='Лимиты задаются документом ""Лимиты расхода денежных средств"".';uk='Ліміти задаються документом ""Ліміти витрачання грошових коштів"".'");
	КонецЕсли;
