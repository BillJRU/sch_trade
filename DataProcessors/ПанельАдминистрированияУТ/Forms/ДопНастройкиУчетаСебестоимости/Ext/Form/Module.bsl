...
Процедура ДатаПереходаНаПартионныйУчетВерсии22ПриИзменении(Элемент)
		...
		Если ЗначениеЗаполнено(ПериодДвиженийСебестоимости)
		 И НаборКонстант.ДатаПереходаНаПартионныйУчетВерсии22 < ПериодДвиженийСебестоимости Тогда
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Указанный период не может быть раньше даты первых движений по регистрам: %1';uk='Зазначений період не може бути раніше дати перших рухів по регістрах: %1'"),
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru='Указанный период не может быть позже даты первых движений партионного учета версии 2.5: %1';uk='Зазначений період не може бути пізніше дати перших рухів партіонного обліку версії 2.5: %1'"),
		...
		Если РеквизитПутьКДанным = "НаборКонстант.ДатаНачалаВеденияУправленческогоУчетаОрганизаций" Тогда
			...
			Если НачалоВеденияУправленческогоУчетаОрганизаций = 0
			 И НаборКонстант.ДатаНачалаВеденияУправленческогоУчетаОрганизаций < НачалоМесяца(НаборКонстант.ДатаПереходаНаПартионныйУчетВерсии22) Тогда
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Указанный период должен быть позже даты начала использования партионного учета: %1';uk='Зазначений період повинен бути пізніше дати початку використання партіонного обліку: %1'"),
			...
			Если (РежимПартионногоУчета = "Версия22") <> ПолучитьФункциональнуюОпцию("ПартионныйУчетВерсии22") Тогда
				...
				Если РежимПартионногоУчета = "Версия22" И НЕ ЗначениеЗаполнено(НаборКонстант.ДатаПереходаНаПартионныйУчетВерсии22) Тогда
					...
					Если ЗначениеЗаполнено(ПериодДвиженийСебестоимости) Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							НСтр("ru='Партионный учет версии 2.5. включен с текущего периода.
|Установите требуемый период перехода на партионный учет версии 2.5.'
|;uk='Партіонний облік версії 2.5. включений з поточного періоду. 
|Встановіть необхідний період переходу на партіонний облік версії 2.5.'"),
							,
							"НаборКонстант.ДатаПереходаНаПартионныйУчетВерсии22");
...
Процедура НастройкиПартионногоУчета()
	...
	Если РежимПартионногоУчета = "НеИспользуется" ИЛИ НЕ ЗначениеЗаполнено(ПричиныЗапретаВыключения) Тогда
		ДоступныеРежимы.Добавить("НеИспользуется", НСтр("ru='Не используется';uk='Не використовується'"));
	КонецЕсли;
	...
	Если (РежимПартионногоУчета = "Версия22" И НЕ ЗначениеЗаполнено(ПричиныЗапретаПонижения))
	 ...
	 Если ПолучитьФункциональнуюОпцию("УправлениеПредприятием") Тогда
			ИмяКонфигурации = НСтр("ru='BAS ERP 2.1';uk='BAS ERP 2.1'");
		ИначеЕсли ПолучитьФункциональнуюОпцию("КомплекснаяАвтоматизация") Тогда
			ИмяКонфигурации = НСтр("ru='BAS КУП 2.1';uk='BAS КУП 2.1'");
		Иначе
			ИмяКонфигурации = НСтр("ru='BAS УТ 3.2';uk='BAS УТ 3.2'");
		КонецЕсли;
		
		ДоступныеРежимы.Добавить("Версия21",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Версия 2.1 (режим совместимости с %1)';uk='Версія 2.1 (режим сумісності з %1)'"),
				ИмяКонфигурации));
		
	КонецЕсли;
	ДоступныеРежимы.Добавить("Версия22", НСтр("ru='Версия 2.5';uk='Версія 2.5'"));
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КодПричины = 1 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru='есть документы ""Расчет себестоимости товаров"" с методом оценки стоимости товаров ""ФИФО (скользящая оценка)""';uk='є документи ""Розрахунок собівартості товарів"" з методом оцінки вартості товарів ""ФІФО (змінна оцінка)""'");
		ИначеЕсли Выборка.КодПричины = 2 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru='есть организации с методом оценки стоимости товаров ""ФИФО (скользящая оценка)""';uk='є організації з методом оцінки вартості товарів ""ФІФО (змінна оцінка)""'");
		Иначе
			ВызватьИсключение НСтр("ru='Неизвестный код причины.';uk='Невідомий код причини.'");
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
		ТекстПредупреждения = НСтр("ru='Невозможно отключение партионного учета:';uk='Неможливо відключення партіонного обліку:'")
			+ ТекстПредупреждения;;
	КонецЕсли;
	...
	Иначе
		Если РежимПартионногоУчета <> "Версия22" Тогда
			КомментарийОпции = КомментарийОпции + Символы.ПС
				+ НСтр("ru='- установить опции ""Партионный учет"" значение ""Версия 2.5""';uk='- встановити опції ""Партіонний облік"" значення ""Версія 2.5""'");
		КонецЕсли; 
		Если ЗначениеЗаполнено(КомментарийОпции) Тогда
			КомментарийОпции = НСтр("ru='Для включения возможности использования управленческого учета по правилам МФУ необходимо:';uk='Для включення можливості використання управлінського обліку за правилами МФО необхідно:'") + КомментарийОпции;
		КонецЕсли; 
	КонецЕсли;
...
Процедура ЗапланироватьПересчетСебестоимостиПриОтключенииПУВерсии22()
	...
	|		ИЛИ СебестоимостьТоваров.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	|	СебестоимостьТоваров.Организация
	|
