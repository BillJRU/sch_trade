...
Процедура ОбновитьВсеРасширения(Команда)
	...
	Если Список.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет расширений для обновлений';uk='Немає розширень для оновлень'"));	
		Возврат;
	КонецЕсли;
	
	УстановитьДоступностьЭлементовОбновления(Ложь);
	
	ТекстВопроса = НСтр("ru='Выполнить обновление всех расширений?';uk='Виконати оновлення всіх розширень?'")
		+ Символы.ПС + НСтр("ru='Работа всех пользователей будет завершена';uk='Робота всіх користувачів буде завершена'");
	Описание = Новый ОписаниеОповещения("ОбработатьОбновлениеВсехРасширений", ЭтотОбъект);
	
	ПоказатьВопрос(Описание, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ru='Обновление расширений';uk='Оновлення розширень'"));
...
Процедура ОбновитьВыбранноеРасширение(Команда)
	...
	ТекстВопроса = СтрШаблон(НСтр("ru='Выполнить обновление расширения: %1 ?';uk='Виконати оновлення розширення: %1?'")
		+ Символы.ПС + НСтр("ru='Работа всех пользователей будет завершена';uk='Робота всіх користувачів буде завершена'"), СтрокаДанных.Наименование);
...
Процедура ОбработатьОбновлениеВсехРасширений(Ответ, ДопПараметры) Экспорт 
	...
	Текст = НСтр("ru='Обновление расширений';uk='Оновлення розширень'");
...
Процедура ОбработатьОбновлениеТекущегоРасширения(Ответ, СтрокаДанныхРасширения) Экспорт 
	...
	НачатьОбновлениеРасширения(СтрокаДанныхРасширения, 0, 1, НСтр("ru='Обновление расширений';uk='Оновлення розширень'"));
	ОбновитьСписокОбновляемыхРасширений();
...
Процедура НачатьОбновлениеРасширения(СтрокаДанных, ТекущийПрогресс, ВсегоРасширений, Текст)
	...
	Пояснение = СтрШаблон(НСтр("ru='Обновление: %1';uk='Оновлення: %1'"), СтрокаДанных.Наименование);
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Для вступления изменений в силу требуется перезапустить приложение';uk='Для вступу змін в силу потрібно перезапустити програму'"));
