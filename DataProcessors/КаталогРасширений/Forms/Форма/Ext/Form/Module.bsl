...
Процедура ТолькоУстановленныеПриИзменении(Элемент)
	...
	ПоказатьОповещениеОЗагрузкеДанных(НСтр("ru='Обработка списка расширений';uk='Обробка списку розширень'"));
...
Процедура НачатьПодключениеКМенеджеруСервиса()
	...
	ПоказатьОповещениеОЗагрузкеДанных(НСтр("ru='Подключение к Менеджеру сервиса...';uk='Підключення до Менеджеру сервісу ...'"));
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru='Не удалось подключиться к Менеджеру сервиса.
                            ...
                            |;uk='Не вдалось підключитися до Менеджеру сервісу. 
                            |Будь ласка, спробуйте пізніше або зверніться до адміністратора сервісу.'");
...
Процедура ПоказатьОповещениеОЗагрузкеДанных(Пояснение)
	...
	Текст = НСтр("ru='Загрузка данных ...';uk='Завантаження даних ...'");
	
	ПоказатьОповещениеПользователя(Текст,, Пояснение, БиблиотекаКартинок.ИнформацияБСП,, "ОбновлениеДанных");
...
Процедура ОбработатьРезультатВыполненияЗапросаКМС()
	...
	Если НЕ ДанныеХранилища.success Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось выполнить запрос';uk='Не вдалося виконати запит'") + Символы.ПС 
			+ НСтр("ru='Код ошибки: %1';uk='Код помилки: %1'") + Символы.ПС 
			+ НСтр("ru='Описание: %2';uk='Опис: %2'"), ДанныеХранилища.error.code, ДанныеХранилища.error.msg);	
	...
	Если ДанныеХранилища.ИмяМетода = "ПолучитьСписокДоступныхРасширений" 
		...
		Если НЕ ЕстьЕщеРасширения Тогда
			ПоказатьОповещениеОЗагрузкеДанных(НСтр("ru='Это все доступные расширения.';uk='Це все доступні розширення.'"));
		КонецЕсли;
...
Функция ЗагрузитьДанныеРасширений(Знач МассивРасширений)
	...
	ТекстЕще = НСтр("ru='Показать еще';uk='Показати ще'");
	...
	Для Каждого ДанныеРасширения Из МассивРасширений Цикл
		...
		НоваяСтрока.НаименованиеВерсии = СтрШаблон(НСтр("ru='Версия: %1';uk='Версія: %1'"), 
			НоваяСтрока.НаименованиеВерсии);
			
		НоваяСтрока.Разработчик = СтрШаблон(НСтр("ru='Разработчик: %1';uk='Розробник: %1'"), 
...
Функция СостояниеРасширения(Знач Состояние)
	Возврат СтрШаблон(НСтр("ru='Состояние: %1';uk='Стан: %1'"), Состояние);
КонецФункции
