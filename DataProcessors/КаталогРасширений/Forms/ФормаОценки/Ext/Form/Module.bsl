...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения, 
		"Отзыв, Оценка, ДатаРедактирования, Версия, Автор, Наименование" );
	
	Элементы.Версия.Видимость = ЗначениеЗаполнено(Версия);	
	Заголовок = СтрШаблон(НСтр("ru='Автор: %1 от %2';uk='Автор: %1 з %2'"), Автор, Формат(ДатаРедактирования, "ДЛФ=DD"));
...
Процедура ОтобразитьИнформациюОбОценке()
	...
	Если Оценка = 1 Тогда
		ОписаниеПоказателя = НСтр("ru='Ужасно';uk='Жахливо'");
		Цвет = Новый Цвет(191, 55, 1);
	ИначеЕсли Оценка = 2 Тогда
		ОписаниеПоказателя = НСтр("ru='Плохо';uk='Погано'");
		Цвет = Новый Цвет(255, 99, 72);
	ИначеЕсли Оценка = 3 Тогда
		ОписаниеПоказателя = НСтр("ru='Нормально';uk='Нормально'");
		Цвет = Новый Цвет(255, 165, 0);
	ИначеЕсли Оценка = 4 Тогда
		ОписаниеПоказателя = НСтр("ru='Хорошо';uk='Добре'");
		Цвет = Новый Цвет(28, 116, 28);
	ИначеЕсли Оценка = 5 Тогда
		ОписаниеПоказателя = НСтр("ru='Отлично';uk='Відмінно'");
