...
Функция ДанныеДляПечатиЭтикетокДоставкиАдаптироватьКРаспоряжению(МетаданныеРаспоряжения, ТекстЗапросаПоРаспоряжению)
	...
	Иначе
		...
		Если ИмяТЧТовары = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Для типа %ТипРаспоряжения% распоряжения на доставку не определен запрос выборки данных доставки.';uk='Для типу %ТипРаспоряжения% розпорядження на доставку не визначений запрос вибірки даних доставки.'");
