...
Процедура ВыбратьВариантЗаполненияУпаковок(Команда)
	...
	Если Элементы.Товары.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Не выделено ни одной строки. Заполнение упаковок невозможно.';uk='Не виділено жодного рядка. Заповнення упаковок неможливе.'");
		...
		Если Строка <> Неопределено Тогда
			Если Строка.Выбран Тогда
				Если Строка.КоличествоЭтикеток = 0 Тогда
					ПроверитьОчиститьСообщения(ОчищатьСообщения);
					ТекстСообщения = НСтр("ru='В строке %Номер% не указано количество этикеток. Упаковки в ней не будут заполнены.';uk='У рядку %Номер% не вказано кількість етикеток. Упаковки в ній не будуть заповнені.'");
			...
			Иначе
				ПроверитьОчиститьСообщения(ОчищатьСообщения);
				ТекстСообщения = НСтр("ru='Строка %Номер% не выбрана. Упаковки в ней не будут заполнены.';uk='Рядок %Номер% не вибраний. Упаковки у ньому не будуть заповнені.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", Строка.НомерСтроки);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
...
Процедура РазбитьСтроку(Команда)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Команда разбиения строк не поддерживается в режиме печати ценников и этикеток';uk='Команда розбиття рядків не підтримується в режимі друку цінників і етикеток'");
	КонецЕсли;
