...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Для Каждого Организация Из Организации Цикл
		...
		Для i = 0 По МаксимальныйИндексТаблицы Цикл
			Стр = ТаблицаТоварыОрганизаций[МаксимальныйИндексТаблицы - i];
		...
		Попытка
			...
			Отказ = Ложь;
			МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
			ЗапасыСервер.ПодготовитьЗаписьТоваровОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц,
				РежимЗаписиДокумента.Проведение, ПараметрыЗаполненияВидовЗапасов);
			ЗапасыСервер.СформироватьРезервыПоТоварамОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц,
				Отказ, ПараметрыЗаполненияВидовЗапасов);
			...
			Если ДокументОбъект.ДополнительныеСвойства.Свойство("ТаблицаРезервыТоваровОрганизаций")
				...
				ШаблонКомментария = НСтр("ru='Документ сформирован автоматически из помощника исправления остатков товаров организаций %ТекущаяДатаСеанса%';uk='Документ сформований автоматично з помічника виправлення залишків товарів організацій %ТекущаяДатаСеанса%'");
	...
	Если БылиОшибки Тогда
		ТекстСообщения = НСтр("ru='При переформировании резервов товаров организации возникли ошибки. Подбробности в журнале регистрации.';uk='При переформування резервів товарів організації виникли помилки. Подробиці в журналі реєстрації.'");
...
Процедура СформироватьИсправленияРазвернутогоСальдо() Экспорт
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		ЗапросИсточникиПокрытияРазвернутогоСальдо.УстановитьПараметр("Организация", ВыборкаОрганизацииСОтрицательнымОстатком.Организация);
		ЗапросИсточникиПокрытияРазвернутогоСальдо.УстановитьПараметр("ТипЗапасов", ВыборкаОрганизацииСОтрицательнымОстатком.ТипЗапасов);
		...
		Попытка
			...
			Для Каждого ТипЗачета Из ТипыЗачетаОстатков Цикл
				ТаблицаОстатковКРаспределению = 
					РезультатЗапроса[ТипЗачета.НомерПакетаОстаткиКРаспределению].Выгрузить(); // ТаблицаЗначений
				ТаблицаОстатковКРаспределению.Индексы.Добавить(ТипЗачета.СтруктураПоиска);
				ВыборкаОтрицательныхОстатков = 
					РезультатЗапроса[ТипЗачета.НомерПакетаОтрицательныеОстатки].Выбрать();
				...
				Пока ВыборкаОтрицательныхОстатков.Следующий() Цикл
					...
					Для Каждого СтрокаОстатка Из СтрокиОстатковКРаспределению Цикл
						...
						Если ДокументОбъект = Неопределено Тогда
							...
							ДокументОбъект.ДополнительныеСвойства.Вставить("ОкончаниеПериодаПомощникаИсправленияОстатков", КонецМесяца(МесяцИсправления));
							ШаблонКомментария = НСтр("ru='Документ сформирован автоматически из помощника исправления остатков товаров организаций %ТекущаяДатаСеанса%';uk='Документ сформований автоматично з помічника виправлення залишків товарів організацій %ТекущаяДатаСеанса%'");
							ДокументОбъект.Комментарий = СтрЗаменить(ШаблонКомментария, "%ТекущаяДатаСеанса%", Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT"));
						КонецЕсли;
						
						СтруктураПоиска = Новый Структура(
							"АналитикаУчетаНоменклатуры, ВидЗапасовОприходования, ВидЗапасовСписания, НомерГТДОприходования, НомерГТДСписания");
						...
						КонецЕсли;
						СтрокаИсправления[ТипЗачета.ИмяРеквизитаКоличествоВТЧ] = СтрокаИсправления[ТипЗачета.ИмяРеквизитаКоличествоВТЧ] + Количество;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				,
				ПодробноеПредставлениеОшибки);
			ТекстСообщения = НСтр("ru='Не удалось сформировать исправления развернутого сальдо. Подробности в журнале регистрации.';uk='Не вдалося сформувати виправлення розгорнутого сальдо. Подробиці в журналі реєстрації.'");
			ВызватьИсключение ТекстСообщения;
...
Процедура ПерезаполнитьВидыЗапасов() Экспорт
	...
	Пока Не Завершение Цикл
 		Если Итерация > 50 Тогда
			ТекстИсключения = НСтр("ru='Превышен лимит итераций перезаполнения видов запасов';uk='Перевищено ліміт ітерацій перезаповненяя видів запасів'",Метаданные.ОсновнойЯзык.КодЯзыка);
			ЗаписьЖурналаРегистрации(
				ИмяСобытия(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Обработки.ПомощникИсправленияОстатковТоваровОрганизаций,
...
Процедура ПерезаполнитьВидыЗапасовИтерация()
	...
	Для Каждого Стр Из ДокументыКПерезаполнению Цикл 
		...
		Исключение
			ОтменитьТранзакцию();
			ТекстОшибки = НСтр("ru='Возникла ошибка при перезаполнении видов запасов за период %ТекущийМесяц%: %ПричинаОшибки%';uk='Виникла помилка при перезаповненні видів запасів за період %ТекущийМесяц%: %ПричинаОшибки%'");
...
Процедура СообщитьПрогрессПерезаполненияВидовЗапасов()
	...
	ТекстСообщения = НСтр("ru='Выполняется перезаполнение видов запасов';uk='Виконується перезаповнення видів запасів'");
...
Процедура ЗаполнитьСписокДокументовКПерезаполнению()
	...
	Если РезультатЗапроса[1].Пустой() <> РезультатЗапроса[2].Пустой() Тогда
		ТекстИсключения = 
			НСтр("ru='По аналитике учета номенклатуры %АналитикаУчетаНоменклатуры% не найдены документы для перезаполнения видов запасов';uk='По аналітиці обліку номенклатури %АналитикаУчетаНоменклатуры% не знайдені документи для перезаповнення видів запасів'",Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция ИмяСобытия()
	Возврат НСтр("ru='Ошибка при работе помощника исправления остатков товаров организаций';uk='Помилка при роботі помічника виправлення залишків товарів організацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ДобавитьОтборыКонтроляОформленияДокументов(КомпоновщикНастроек)
	...
	ГруппаИли = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
		КомпоновщикНастроек.Настройки.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли); // ГруппаЭлементовОтбораКомпоновкиДанных
