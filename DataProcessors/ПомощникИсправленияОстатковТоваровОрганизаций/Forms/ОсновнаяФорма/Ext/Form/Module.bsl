...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.НавигацияПерейтиВНачало.Заголовок = ?(ЕстьПроблема, НСтр("ru='Перейти в начало';uk='Перейти на початок'"), НСтр("ru='Следующий месяц';uk='Наступний місяць'"));
	...
	Если НомерСтраницы = 0 Тогда
		...
		Элементы.НавигацияПерейтиВНачало.Заголовок = НСтр("ru='Начать';uk='Почати'");
	...
	Если НомерСтраницы = 3 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных корректировок';uk='Відкрити список сформованих коригувань'"), 
	...
	Если НомерСтраницы = 4 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список измененных документов';uk='Відкрити список змінених документів'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных исправлений';uk='Відкрити список сформованих виправлень'"), 
	...
	Если НомерСтраницы = 5 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных корректировок по новым настройкам';uk='Відкрити список сформованих коригувань за новими настройками'"), 
	...
	Если НомерСтраницы = 7 Тогда
		ЗаголовокОтчета = НСтр("ru='Отчет о проделанной работе за %МесяцИсправления%';uk='Звіт про виконану роботу за %МесяцИсправления%'");
		ЗаголовокОтчета = 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных корректировок резервов';uk='Відкрити список сформованих коригувань резервів'"), 
			Объект.КорректировкиРезервов.Количество(),
			Элементы.ОткрытьСписокСформированныхКорректировокШаг7);
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных исправлений развернутого сальдо товаров организаций';uk='Відкрити список сформованих виправлень розгорнутого сальдо товарів організацій'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru='Открыть список сформированных корректировок резервов по новым настройкам';uk='Відкрити список сформованих коригувань резервів за новими настройками'"), 
...
Функция СформироватьЗаголовокШаг3Пояснение2()
	...
	ТекстЗаголовка = НСтр("ru='При запуске переформирования резервов будут очищены движения регистра ""Резервы товаров организаций"" по проблемным позициям, начиная с %МесяцРодительныйПадеж% %Год% года, и сформированы новые резервы по отрицательным остаткам. Новые резервы будут записаны в документы корректировок регистров. Для каждой организации будет создан отдельный документ.';uk='При запуску переформування резервів будуть очищені рухи регістра ""Резерви товарів організацій"" з проблемних позиціях, починаючи з %МесяцРодительныйПадеж% %Год% року, і сформовані нові резерви по від''ємних залишках. Нові резерви будуть записані в документи коригувань регістрів. Для кожної організації буде створено окремий документ.'");
...
Процедура ПроверитьРольНастройкаНСИДоступна(СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны(Метаданные.Роли.НастройкаНСИ.Имя) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстСообщения = НСтр("ru='Для настройки передачи требуется роль ""%НастройкаНСИ%"". Обратитесь к администратору.';uk='Для настройки передачі потрібна роль ""%НастройкаНСИ%"". Зверніться до адміністратора.'");
...
Процедура ПроверитьРазрешениеДоступа(Отказ)
	...
	Если Константы.КонтролироватьОстаткиТоваровОрганизаций.Получить()
		...
		Отказ = Истина;
		ТекстСообщения = 
		НСтр("ru='Для работы с помощником требуется роль ""%РазрешитьОтключениеКонтроляТоваровОрганизацийНаВремяСеанса%"" или отключение функциональной опции ""%КонтролироватьОстаткиТоваровОрганизаций%"". Обратитесь к администратору.';uk='Для роботи з помічником потрібна роль ""%РазрешитьОтключениеКонтроляТоваровОрганизацийНаВремяСеанса%"" або відключення функціональної опції ""%КонтролироватьОстаткиТоваровОрганизаций%"". Зверніться до адміністратора.'");
	...
	Исключение
		Отказ = Истина;
		ТекстСообщения = 
		НСтр("ru='Для работы с помощником требуется доступ на чтение всех организаций в информационной базе. Обратитесь к администратору.';uk='Для роботи з помічником потрібен доступ на читання всіх організацій в інформаційній базі. Зверніться до адміністратора.'");	
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
