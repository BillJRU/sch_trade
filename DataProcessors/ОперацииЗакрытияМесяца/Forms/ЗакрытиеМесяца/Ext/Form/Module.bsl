...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗакрытиеМесяцаЛокализация.ПриСозданииНаСервереФормаЗакрытиеМесяца(Отказ, СтандартнаяОбработка, ЭтаФорма);
	...
	Если НЕ РежимРаботы.ДоступностьВидовУчета.ОперативныйУчет
	 ...
	 И НЕ РежимРаботы.ДоступностьВидовУчета.МеждународныйУчет Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Недостаточно прав на выполнение операций закрытия месяца.';uk='Недостатньо прав на виконання операцій закриття місяця.'"));
...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		...
		ИнформацияОЗапускеРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователь ""%1"" через веб-клиент';uk='Користувач ""%1"" через вебклієнт'",КодОсновногоЯзыка),
			ИмяПользователя());
	#Иначе
		ШиринаКолонок = 4;
		ИнформацияОЗапускеРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователь ""%1"" с компьютера ""%2""';uk='Користувач ""%1"" з комп''ютера ""%2""'",КодОсновногоЯзыка),
			ИмяПользователя(),
			ИмяКомпьютера());
	#КонецЕсли
...
Процедура Подключаемый_ВыполнитьКомандуЛокализации(Команда)
	...
	ЗакрытиеМесяцаКлиентЛокализация.ВыполнитьКомандуЛокализации(Команда, ЭтаФорма);
...
Процедура ОткрытьЗаданияКЗакрытиюМесяца(Команда)
	...
	ОткрытьФорму(
		...
		ЗакрытиеМесяцаКлиентЛокализация.ПараметрыФормыРегистраЗаданий(ЭтаФорма));
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не требуется';uk='Не потрібно'"));
...
Процедура НастроитьЭлементыПриИнициализацииФормы(ВыполняетсяЗагрузкаНастроек = Ложь)
	...
	НастроитьОтметкуКнопокНастроек(Объект, Элементы);
	ЗакрытиеМесяцаЛокализация.НастроитьЭлементыПриИнициализацииФормы(ЭтаФорма, ВыполняетсяЗагрузкаНастроек);
КонецПроцедуры
...
Процедура НастроитьЭлементыВЗависимостиОтНаличияФоновыхЗаданий()
	...
	ИначеЕсли ЗначениеЗаполнено(ОбщаяДатаНачалаРасчета) И ОбщаяДатаНачалаРасчета < Объект.ПериодРегистрации Тогда
		...
		Элементы.ДекорацияПериода.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Требуется пересчет операций начиная с периода %1';uk='Потрібний перерахунок операцій починаючи з періоду %1'"),
...
Функция ЗапуститьОбновлениеДанныхФормыВФоне(ФормироватьОписаниеОрганизаций = Истина)
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Формирование списка операций закрытия месяца %1';uk='Формування списку операцій закриття місяця %1'"),
...
|%1'
|;uk='Список організацій, обраних для розрахунку, доповнений пов''язаними організаціями:
|%1'"), 
...
|%1.'
|;uk='При формуванні списку операцій закриття місяця виникла помилка:
|%1.'"),
...
|фонове завдання %1.'"),
				?(СостояниеЗадания.Отменено,
					НСтр("ru='отменено администратором';uk='скасовано адміністратором'"),
					?(СостояниеЗадания.ЗавершеноАварийно,
						НСтр("ru='завершено аварийно';uk='завершено аварійно'"),
						НСтр("ru='не найдено в списке заданий';uk='не знайдено в списку завдань'"))));
...
Процедура ПроверитьАктивностьФоновогоЗаданияРасчета()
	...
	ИначеЕсли СостояниеЗадания.Отменено ИЛИ СостояниеЗадания.ЗавершеноАварийно ИЛИ СостояниеЗадания.НеНайдено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Фоновое задание расчета %1.';uk='Фонове завдання розрахунку %1.'",КодОсновногоЯзыка),
			?(СостояниеЗадания.Отменено,
				НСтр("ru='отменено администратором';uk='скасовано адміністратором'",КодОсновногоЯзыка),
				?(СостояниеЗадания.ЗавершеноАварийно,
					НСтр("ru='завершено аварийно';uk='завершено аварійно'",КодОсновногоЯзыка),
					НСтр("ru='не найдено в списке заданий';uk='не знайдено в списку завдань'",КодОсновногоЯзыка))));
	...
	Если СостояниеЗадания.Активно // еще выполняется расчет этапа
	 ИЛИ ЗапуститьРасчетСледующегоЭтапаВФоне() Тогда // запущен расчет следующего этапа
		...
		ПредставлениеРассчитываемогоЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполняется операция ""%1""...';uk='Виконується операція ""%1"" ...'"),
...
Процедура ЗаполнитьСостояниеЗакрытияМесяца()
	...
	Если МесяцЗакрытУспешно Тогда
		...
		Если ЭтаповРассчитано = 0 Тогда
			ПредставлениеСтатусаРасчета = НСтр("ru='Закрытие месяца не требуется';uk='Закриття місяця не потрібно'");
		Иначе
			ПредставлениеСтатусаРасчета = НСтр("ru='Все операции выполнены успешно';uk='Всі операції виконані успішно'");
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ПредставлениеСтатусаРасчета = НСтр("ru='Существуют невыполненные операции';uk='Існують невиконані операції'");
	...
	ИначеЕсли НЕ МесяцЗакрытУспешно Тогда
		Элементы.ДекорацияСтатусаРасчета.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Операций к пересчету за период %1: %2 шт.';uk='Операцій до перерахунку за період %1: %2 шт.'"),
			РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(Объект.ПериодРегистрации),
	...
	ИначеЕсли ЭтаповРассчитано > 0 Тогда
		Элементы.ДекорацияСтатусаРасчета.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Рассчитано операций за период %1: %2 шт.';uk='Розраховано операцій за період %1: %2 шт.'"),
			РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(Объект.ПериодРегистрации),
...
Функция ЗапуститьРасчетСледующегоЭтапаВФоне()
	...
	Если Объект.ОстанавливатьсяПоОшибке И СостояниеРасчета.НомерПредыдущегоЭтапа >= 0 Тогда
		...
		Если ПредыдущаяСтрока.Состояние = Перечисления.СостоянияОперацийЗакрытияМесяца.НеВыполнено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция ""%1"" рассчитана с ошибками. Выполнение операций закрытия месяца остановлено.';uk='Операція ""%1"" розрахована з помилками. Виконання операцій закриття місяця зупинено.'"),
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция ""%1"" рассчитана с ошибками. Выполнение операций закрытия месяца остановлено.';uk='Операція ""%1"" розрахована з помилками. Виконання операцій закриття місяця зупинено.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				ПредыдущаяСтрока.Наименование);
			
			ЗаписьЖурналаРегистрации(
				ЗакрытиеМесяцаСервер.ИмяСобытияЖурналаРегистрации(
					НСтр("ru='Остановка закрытия месяца из-за ошибок расчета';uk='Зупинка закриття місяця через помилки розрахунку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
	...
	Для НомерЭтапа = СостояниеРасчета.НомерТекущегоЭтапа + 1 По СостояниеРасчета.КоличествоЭтапов - 1 Цикл
		...
		НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполнение операций закрытия месяца %1';uk='Виконання операцій закриття місяця %1'"),
...
|%2'
|;uk='При виконанні операції закриття місяця ""%1"" сталася помилка:
|%2'",КодОсновногоЯзыка),
		СокрЛП(ТекущийЭтап),
		ТекстОшибки);
...
|;uk='Не вдалося скасувати виконання фонового завдання формування списку операцій.
	 ...
	 ИЛИ (ЗначениеЗаполнено(ОписаниеОрганизаций)
	   И ОбщегоНазначенияУТКлиентСервер.МассивыРавны(МассивОрганизаций, ОписаниеОрганизаций.ДоступныеОрганизации, Ложь)) Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеОрганизаций) И ОписаниеОрганизаций.ЕстьОграниченияДоступа Тогда
			ПредставлениеОрганизаций = НСтр("ru='По всем доступным организациям';uk='За всіма доступними організаціям'");
		Иначе
			ПредставлениеОрганизаций = НСтр("ru='По всем организациям';uk='По всіх організаціях'");
		КонецЕсли;
