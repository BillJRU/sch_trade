...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ПериодРегистрации) ИЛИ НЕ ЗначениеЗаполнено(ОписаниеОрганизаций) Тогда
		ВызватьИсключение НСтр("ru='Некорректный контекст открытия формы';uk='Некоректний контекст відкриття форми'");
	КонецЕсли;
...
Процедура ВыбратьОрганизации(Команда)
	...
	Если МассивОрганизаций.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо выбрать организации для закрытия месяца.';uk='Необхідно вибрати організації для закриття місяця.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоОрганизаций()
	...
	Для Каждого ТекущаяГруппа Из ОписаниеОрганизаций.ГруппыОрганизаций Цикл
		...
		НаименованиеГруппы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Группа организаций №%1: %2%3%4%5';uk='Група організацій №%1: %2%3%4%5'",ОбщегоНазначения.КодОсновногоЯзыка()),
			СокрЛП(КоличествоГрупп),
			?(ЕстьИнтеркампани, """" + Обработки.ОперацииЗакрытияМесяца.НазваниеГруппыОрганизацийИнтеркампани() + """", ""),
			?(ЕстьИнтеркампани И ЕстьОСиНМА, " " + НСтр("ru='и';uk='і'",ОбщегоНазначения.КодОсновногоЯзыка()) + " ", ""),
			?(ЕстьОСиНМА, """" + Обработки.ОперацииЗакрытияМесяца.НазваниеГруппыОрганизацийОСиНМА() + """", ""),
			?(ЕстьНедоступные, " (" + НСтр("ru='нет прав на все организации группы';uk='Немає прав на всі організації групи'",ОбщегоНазначения.КодОсновногоЯзыка()) + ")", ""));
