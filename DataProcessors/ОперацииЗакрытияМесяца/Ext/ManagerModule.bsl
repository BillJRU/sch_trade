...
// * ТипыРегламентныхОпераций - Массив Из ПеречислениеСсылка.ТипыРегламентныхОпераций -
//                            - Массив Из ПеречислениеСсылка.ТипыРегламентныхОперацийМеждународныйУчет -
...
// * Код - Строка -
// * Наименование - Строка -
// * Состояние - ПеречислениеСсылка.ВариантыВажностиПроблемыСостоянияСистемы -
...
Функция ИнициализироватьПараметрыЗапускаДляЗаполненияСостоянияЭтапов(СтруктураДанных, АдресХранилища = "",
	...
	Если ЗначениеЗаполнено(ДопСвойства) И ДопСвойства.Свойство("РежимОтладки") Тогда
		ПараметрыЗапуска.Вставить("РежимОтладки", ДопСвойства.РежимОтладки);
	Иначе
		ПараметрыЗапуска.Вставить("РежимОтладки", ОбщегоНазначения.РежимОтладки());
	КонецЕсли;
...
|Период: %2
|Организации: %3'
|;uk='Час формування списку: %1
|Період: %2
|Організації: %3'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ЗаписьЖурналаРегистрации(
			ЗакрытиеМесяцаСервер.ИмяСобытияЖурналаРегистрации(НСтр("ru='Формировании списка операций';uk='Формуванні списку операцій'",ОбщегоНазначения.КодОсновногоЯзыка())),
...
Функция ИнициализироватьПараметрыОбработчикаЭтапа(СтруктураДанных, СтрокаТаблицыЭтапов,
	...
	Если ЗначениеЗаполнено(ДопСвойства) И ДопСвойства.Свойство("РежимОтладки") Тогда
		ПараметрыОбработчика.Вставить("РежимОтладки", ДопСвойства.РежимОтладки);
	Иначе
		ПараметрыОбработчика.Вставить("РежимОтладки", ОбщегоНазначения.РежимОтладки());
	КонецЕсли;
	 ...
	 И ПараметрыОбработчика.Свойство("МеждународныйУчет") Тогда
		...
		Если НЕ ЭтапДоступен Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
				ПараметрыОбработчика,
				НСтр("ru='Данный вид учета недоступен.';uk='Даний вид обліку недоступний.'",ОбщегоНазначения.КодОсновногоЯзыка()));
			Возврат;
...
|%2'
|;uk='При визначенні стану операції %1 сталася помилка:
...
Процедура ПроверитьПредшествующиеЭтапы(СтруктураДанных, ТекущаяСтрока, ПараметрыОбработчика)
	...
	Для Каждого ПредшествующийЭтап Из ТекущаяСтрока.ПредшествующиеЭтапы Цикл
		...
		Если СтруктураДанных.ТаблицаЭтапы.Индекс(СтрокаПредшествующегоЭтапа) >= ИндексТекущейСтроки Тогда
			...
			ТекстОшибки = НСтр("ru='Порядок выполнения операций ""%1"" и ""%2"" не совпадает с настройками взаимозависимости операций.';uk='Порядок виконання операцій ""%1"" і ""%2"" не збігається з настройками взаємозалежності операцій.'",ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки,
				ТекущаяСтрока.Наименование,
		 ...
		 ИЛИ СтрокаПредшествующегоЭтапа.Состояние = Перечисления.СостоянияОперацийЗакрытияМесяца.Неопределено Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не выполнена предшествующая операция ""%1""';uk='Не виконана попередня операція ""%1""'",ОбщегоНазначения.КодОсновногоЯзыка()),
					СтрокаПредшествующегоЭтапа.Наименование),
					,
					,
...
Процедура ВыполнитьПроверкиЭтапа(СтруктураДанных, ТекущаяСтрока, ПараметрыОбработчика, ВыполнятьПроверкиДоЭтапа, ВыполнятьПроверкиПослеЭтапа)
	...
	ДополнительныеПараметрыПроверок.Вставить("РежимОтладки", 			   ПараметрыОбработчика.РежимОтладки);
...
|%2'
|;uk='При виконанні перевірки ""%1"" сталася помилка:
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
// 
// Параметры:
//  СтруктураДанных - см. Обработка.ОперацииЗакрытияМесяца.ИнициализироватьСтруктуруДанныхДляЗаполненияСостоянияЭтапов
//
	...
	Если ТекущаяСтрока.Состояние <> Перечисления.СостоянияОперацийЗакрытияМесяца.НеТребуется Тогда
		...
		Для Каждого СтрокаПояснения Из ТаблицаПроблем Цикл
			...
			НовоеПояснение.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (%2 шт.)';uk='%1 (%2 шт.)'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|%2'
|;uk='Під час настройки відображення операції %1 сталася помилка:
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ПараметрыОбработчика.ДанныеЭтапа.Наименование,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверитьДатыЗапретаИзменения(СтруктураДанных)
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаФинансовогоУчета КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО Организации.ГоловнаяОрганизация = Т.Организация
	|ГДЕ
	...
	Если НЕ ЗначениеЗаполнено(ПериодПолитики) Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = НСтр("ru='Отсутствуют учетные политики';uk='Відсутні облікові політики'");
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаЗапрета) ИЛИ ПериодПолитики > ПериодПроверки Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Расчет возможен с периода %1';uk='Розрахунок можливий з періоду %1'"),
...
|%2'
|;uk='При виконанні операції ""%1"" сталася помилка:
|%2'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ШаблонТекстаПроблемыОшибкаВыполненияКода() Экспорт
	...
	Возврат НСтр("ru='При выполнении расчета периода %2 по организации ""%1"" произошла исключительная ситуация';uk='При виконанні розрахунку періоду %2 по організації ""%1"" сталася виняткова ситуація'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция НазваниеГруппыОрганизацийИнтеркампани() Экспорт
	Возврат НСтр("ru='Интеркампани';uk='Інтеркампані'");
КонецФункции

Функция НазваниеГруппыОрганизацийОСиНМА() Экспорт
	Возврат НСтр("ru='Правила учета ОС и НМА';uk='Правила обліку ОЗ і НМА'");
КонецФункции
...
Процедура ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ() Экспорт
	...
	ЗакрытиеМесяцаСервер.ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(
		ВходящиеДанные,
		НСтр("ru='Закрытие месяца';uk='Закриття місяця'",ОбщегоНазначения.КодОсновногоЯзыка()));
