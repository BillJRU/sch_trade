...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если Не ВидПлана.ЭтоГруппа Тогда
		Элементы.ПланыВидПлана.Видимость = Ложь;
	КонецЕсли;
	
	ТекстСообщения = НСтр("ru='%ОбъектПланирования%%ВидПлана% %Товар%за период с %НачалоПериода% по %ОкончаниеПериода%';uk='%ОбъектПланирования%%ВидПлана% %Товар%за період з %НачалоПериода% по %ОкончаниеПериода%'");
	
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		Если ВидПлана.ЭтоГруппа Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбъектПланирования%", НСтр("ru='Планы по этапу планирования';uk='Плани по етапу планування'") + " ");
		Иначе
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбъектПланирования%", НСтр("ru='Планы по виду плана';uk='Плани за видом плану'") + " ");
		КонецЕсли;
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВидПлана%", ВидПлана);
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбъектПланирования%", НСтр("ru='Все планы';uk='Всі плани'"));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВидПлана%", ВидПлана);
	КонецЕсли;
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Товар%", НСтр("ru='по %Номенклатура%%Характеристика%%Назначение%';uk='по %Номенклатура% %Характеристика% %Назначение%'") + " ");
...
Процедура УстановитьСтатусНаУтверждении(Команда)
	...
	УстановитьСтатус("НаУтверждении", НСтр("ru='В работе';uk='В роботі'"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Создать';uk='Створити'"));
