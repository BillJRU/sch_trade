...
Процедура ДекорацияПоказатьСкрытьДетализациюНажатие(Элемент)
	...
	Элементы.ДекорацияПоказатьСкрытьДетализацию.Заголовок = ?(Элементы.ДетализацияТоваров.Видимость,
		НСтр("ru='Скрыть детализацию';uk='Приховати деталізацію'"),
		НСтр("ru='Показать детализацию';uk='Показати деталізацію'"));
...
Процедура ЭтапыПланированияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтапыПланированияТекущиеДанные.ПравоДоступаИзменение 
		...
		ЗапуститьФоновоеЗадание(
			...
			НСтр("ru='Заполнить планы по дефицитам';uk='Заповнити плани за дефіцитами'"),
...
Процедура ДетализацияТоваровВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Характеристика = ДетализацияТоваровТекущиеДанные.Характеристика;
...
Процедура ПересчитатьДоКолонки(Команда)
	...
	ЗапуститьФоновоеЗадание(
		"ЗаполнитьПланыПоДефицитамДоКолонки",
		НСтр("ru='Заполнить планы по дефицитам до колонки';uk='Заповнити плани за дефіцитами до колонки'"),
		"Обработки.УправлениеПроцессомПланирования.ЗаполнитьПланыПоДефицитуПоПериодам",
		ПараметрыПроцедуры);
...
Процедура ПересчитатьВсеПериоды(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Заполнить планы по дефицитам во всех периодах';uk='Заповнити плани за дефіцитами у всіх періодах'"),
		"Обработки.УправлениеПроцессомПланирования.ЗаполнитьПланыПоДефицитуПоПериодам",
		ПараметрыПроцедуры);
КонецПроцедуры
...
Процедура ПересчитатьДефициты(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Запустить пересчет дефицитов по этапам по всем периодам';uk='Запустити перерахунок дефіцитів за етапами по всіх періодах'"),
...
Процедура ОбновитьСтруктуруЭтаповПланированияПоПериодам(ТаблицаПериодов)
	...
	СтруктураПоля.Вставить("ПрефиксРеквизитаКолонки", "Статус_");
	...
	СтруктураПоля = Новый Структура;
	СтруктураПоля.Вставить("ПрефиксРеквизитаКолонки", "Состояние_");
	СтруктураПоля.Вставить("ТипЭлемента", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 2));
...
Функция ОбновитьДеревоЭтаповНаСервере()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ЕСТЬNULL(ЭтапыПланированияНовые.ВидПлана,
	...
	|	МАКСИМУМ(ЕСТЬNULL(ЭтапыПланированияНовые.Состояние, 0) <> ЕСТЬNULL(ЭтапыПланированияСтарые.Состояние, 0)
	...
	|	МАКСИМУМ(ЕСТЬNULL(ЭтапыПланированияНовые.ОтборПоОтветственному,
	...
	СтатусПересчета = ЗапросПакет[7].Выбрать();
	...
	ТаблицаЭтапыПланирования = ЗапросПакет[8].Выгрузить();
			...
			Если ПорядокПланирования <> Строка.ПорядокПланирования Тогда
				Если СтатусПересчета.ИзмениласьСтруктура Тогда
					...
					Если Не ЗначениеЗаполнено(Строка.ВидПлана) Тогда
						ЭлементЭтапПланирования.Представление = НСтр("ru='Баланс планов';uk='Баланс планів'");
					КонецЕсли;
				Иначе
...
Процедура ОтобразитьДетализацию(ВидПлана, ПериодПланирования, УчитыватьНачальныйОстаток, ЭтоКорректировка)
	...
	Элементы.ДетализацияТоваровНачальныйОстаток.Видимость = ВидПлана.ПланированиеОстатков
												ИЛИ	ВидПлана.ТипПлана = Перечисления.ТипыПланов.ПланОстатков
												ИЛИ ВидПлана = Справочники.ВидыПланов.ПустаяСсылка();
											
	Элементы.ДетализацияТоваровПриход.Видимость = ВидПлана = Справочники.ВидыПланов.ПустаяСсылка()
													ИЛИ Не ВидПлана.ПланированиеПотребностей
													И Не ВидПлана.Родитель.ПланированиеПотребностей
													И Не ЭтоКорректировка
													И ВидПлана.ТипПлана <> Перечисления.ТипыПланов.ПланОстатков
													И Не ВидПлана.ПланированиеОстатков;
													
	Элементы.ДетализацияТоваровРасход.Видимость = ВидПлана = Справочники.ВидыПланов.ПустаяСсылка()
													ИЛИ ВидПлана.ПланированиеПотребностей
													ИЛИ ВидПлана.Родитель.ПланированиеПотребностей;
													
	Элементы.ДетализацияТоваровКорректировкаРасхода.Видимость = ВидПлана <> Справочники.ВидыПланов.ПустаяСсылка()
														И Не ВидПлана.Родитель.ПланированиеПотребностей
														И Не ВидПлана.ПланированиеПотребностей
														И ВидПлана.ТипПлана <> Перечисления.ТипыПланов.ПланОстатков
														И Не ВидПлана.ПланированиеОстатков;
												
	Элементы.ДетализацияТоваровПотребность.Видимость = (ВидПлана.ЗаполнятьПоДефициту <> null И ВидПлана.ЗаполнятьПоДефициту) 
													ИЛИ (ВидПлана.ЭтоГруппа И НЕ ВидПлана.ПланированиеПотребностей И НЕ ВидПлана.ПланированиеОстатков);
	
	Элементы.ДетализацияТоваровКИзменению.Видимость = (ВидПлана.ЗаполнятьПоДефициту <> null И ВидПлана.ЗаполнятьПоДефициту) 
													ИЛИ (ВидПлана.ЭтоГруппа И НЕ ВидПлана.ПланированиеПотребностей И НЕ ВидПлана.ПланированиеОстатков);
...
Процедура ЗапускФоновогоЗаданияПомощникПланирования()
	...
	Если ВключитьЗаполнениеПоДефицитамПомощникПланирования И НовоеФоновоеЗадание Тогда 
		...
		НСтр("ru='Глобальный перерасчет помощник планирования';uk='Глобальний перерахунок помічник планування'"),
...
Процедура СоздатьДанныеКорриктировок()
	...
	СоздатьЗаголовок(КорректировкаГруппа,Ложь,НСтр("ru='Проверить корректировки';uk='Перевірити коригування'"),"Корректировка");
	...
	Для Каждого План Из ВыборкаДетальныеЗаписи Цикл  
		...
		ЗаголовокЭлемента=НСтр("ru='Скорректировать';uk='Скоригувати'");
...
Процедура СоздатьГрафическийИнтерфейс()
	...
	НСтр("ru='Актуализировать исходные данные';uk='Актуалізувати вихідні дані'"));
	...
	НСтр("ru='Перерасчитать по дефицитам';uk='Перерахувати за дефіцитами'"),
	"Дефицит");
	...
	Для Каждого Элемент Из ТаблицаЭлементовПомощникаПланированияТекущийПериод Цикл
		Если Не Элемент.ЗаполнениеПоДефецитам Тогда
			...
			Если Элемент.СтатусПорядок = 9 Тогда 
				ЗаголовокЭлемента=НСтр("ru='Создать';uk='Створити'");
			КонецЕсли;
				...
				Иначе
					Если  Элемент.ТипПланов = "Остатки" Тогда
						Если Не ПодсказкаДобавленаОстатки Тогда
							...
							НовыйЭлемент.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Для измения планов с текущего момента заведите планы остатков:';uk='Для измения планів з поточного моменту заведіть плани залишків:'"),,ЦветаСтиля.ПоясняющийТекст);
					...
					Если  Элемент.ТипПланов = "Исходные" Тогда
						Если  Не ПодсказкаДобавленаИсходныеПланы Тогда
							...
							НовыйЭлемент.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Для планирования на следующие периоды заполните планы:';uk='Для планування на наступні періоди заповніть плани:'"),,ЦветаСтиля.ПоясняющийТекст);
		...
		Иначе
			Если  Элемент.Состояние = 1 Тогда
				Если Не ПодсказкаДобавленаДефициты Тогда
					...
					КнопкаПересчета.Заголовок = НСтр("ru='Перерасчитать по дефицитам все планы';uk='Перерахувати за дефіцитами всі плани'");
				...
				КонецЕсли;
				ЗаголовокЭлемента=НСтр("ru='Перерасчитать';uk='Перерахувати'");
				ПараметрыПроцедуры = Новый Структура;
				ПараметрыПроцедуры.Вставить("ТипДействия",ЗаголовокЭлемента);
				ПараметрыПроцедуры.Вставить("ТипДокумента", Элемент.ВидПланаНаименование);
				ПараметрыПроцедуры.Вставить("ВидПлана", Элемент.ЭтапПланированияНаименование);
...
Процедура СоздатьЭлементыПомощника(ПараметрыПроцедуры,Группа,ЭлементТаблицы ,Корректировка = Ложь )//Тип Действия - Создать,Перерасчитать
	...
	ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(ТипДокумента + ?(ВидПлана = "",""," " + НСтр("ru='по';uk='по'"))+" ",,,));
	ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(ВидПлана + " " + НСтр("ru='на';uk='на'") + " ",,,,));
	Если Объект.Сценарий.ОтображатьНомерПериода Тогда
		ТаблицаПериоды = ЭтаФорма.Периоды; // см. Планирование.ТаблицаПериоды
	...
	Если Не Корректировка  Тогда
		...
		Элемент.Подсказка = НСтр("ru='Показать в списке';uk='Показати у списку'");
...
Процедура СоздатьЗаголовок(Группа, РазделСвернут,НазваниеЗаголовка,ТипЗаголовка="Актуализировать")
	...
	ИначеЕсли ТипЗаголовка = "Корректировка" Тогда 
		Элемент.Картинка = БиблиотекаКартинок.ВидимостьОткрыто;
	КонецЕсли;
...
Процедура СкрытьПомощникПланированияНажатиеНаСервере()
	...
	Элементы.СкрытьПомощникПланирования.Заголовок = ?(Элементы.ПомощникПланирования.Видимость,НСтр("ru='Скрыть Помощник Планирования';uk='Приховати Помічник Планування'"),НСтр("ru='Показать Помощник Планирования';uk='Показати Помічник Планування'"));		
...
Процедура ЗаполнитьПоДефицитамНаСервере()
	...
	НСтр("ru='Перерасчитать по дефицитам все планы';uk='Перерахувати за дефіцитами всі плани'"),
	НСтр("ru='Остановить перерасчет планов';uk='Зупинити перерахунок планів'"));
