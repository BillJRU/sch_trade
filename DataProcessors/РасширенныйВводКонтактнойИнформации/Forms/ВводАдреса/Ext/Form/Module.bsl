...
Процедура НаселенныйПунктОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЕстьПравоЗагружатьКлассификатор И ВыбранноеЗначение.ПредлагатьЗагрузкуКлассификатора = Истина Тогда
		...
		ПредложениеЗагрузкиКлассификатора(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Данные для ""%1"" не загружены.';uk='Дані для ""%1"" не завантажені.'"), 
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОбработкаИзмененияАдреса();
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о улицах для введенного адреса.';uk='Вибір зі списку недоступний, т.я.  в адресному класифікаторі відсутня інформація про вулицях для введеного адреси.'"));
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	ИначеЕсли ЭтоНациональныйАдрес Тогда
		ТекстВопроса = НСтр("ru='Ввести адрес в свободной форме?';uk='Ввести адресу у вільній формі?'");
...
Процедура Подключаемый_ДобавитьСтроениеИлиПомещение(Команда)
	...
	Если НайденныеСтроки[0].Тип = "Помещение" Тогда
		Значение = ?(НайденныеСтроки[0].Значение <> НСтр("ru='Другое';uk='Інше'"), НайденныеСтроки[0].Значение, "");
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Если АвтоподборДоступен Тогда
			Элементы.НаселенныйПункт.ПодсказкаВвода      = НСтр("ru='Введите первые буквы названия населенного пункта';uk='Введіть перші літери назви населеного пункту'");
			Элементы.Улица.ПодсказкаВвода                = НСтр("ru='Введите первые буквы названия улицы';uk='Введіть перші літери назви вулиці'");
		Иначе 
...
// Проверять, вводить в свободной форме и искать по индексу можем только национальные адреса.
	...
	Элементы.Улица.КнопкаВыбора                                = ЭтоНациональныйАдрес И ЕстьЗагруженныйКлассификатор;
	Элементы.Дом.КнопкаВыбора                                  = ЭтоНациональныйАдрес И ЕстьЗагруженныйКлассификатор;
...
&НаСервере
Процедура СкопироватьСписокВыбора(НовыйСписокВыбора, ИсходныйСписокВыбора)

	НовыйСписокВыбора.Очистить();
	Для каждого ЭлементСписка Из ИсходныйСписокВыбора Цикл
		НовыйСписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	КонецЦикла;
...
Функция ДобавитьПолеСтроенияИлиПомещения(Идентификатор, Тип, Значение)
	...
	Если Тип = "Помещение" Тогда
		...
		СкопироватьСписокВыбора(НовыйТип.СписокВыбора, Элементы.ТипПомещения.СписокВыбора);
		...
		СкопироватьСписокВыбора(НовыйЭлемент.СписокВыбора, Элементы.Помещение.СписокВыбора);
	...
	Иначе
		...
		СкопироватьСписокВыбора(НовыйТип.СписокВыбора, Элементы.ТипСтроения.СписокВыбора);
		...
		СкопироватьСписокВыбора(НовыйЭлемент.СписокВыбора, Элементы.Строение.СписокВыбора);
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		// Нет ошибок   
		ПоказатьПредупреждение(, НСтр("ru='Адрес введен корректно';uk='Адресу введено коректно'"));
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].АдресПроверен = Ложь И НЕ ВебСервисИспользуется Тогда
			ТекстВопроса = НСтр("ru='Адрес не может быть проверен.';uk='Адреса не може бути перевірена.'") + Символы.ПС 
				+ СписокОшибок[0].ТекстОшибки + Символы.ПС + НСтр("ru='Загрузить классификатор для проверки адреса?';uk='Завантажити класифікатор для перевірки адреси?'");
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса(Параметры)
	...
	ПредставленияДопЭлементов = РаботаСАдресамиКлиентСервер.ПредставленияОбъектовАдресации();
	
	Для каждого ВариантДанных Из ВариантыДанныхСтроение.ВариантыТипа Цикл
		ДобавитьОписаниеСтроенияИлиПомещения(ВариантДанных, "Строение", ПредставленияДопЭлементов.Получить(ВариантДанных));
	КонецЦикла;
	...
	Для каждого ВариантДанных Из ВариантыДанныхПомещение.ВариантыТипа Цикл
		...
		Представление = ?(ВариантДанных = НаименованиеПриДобавлениеПроизвольногоПомещения(), ВариантДанных, ПредставленияДопЭлементов.Получить(ВариантДанных));
		ДобавитьОписаниеСтроенияИлиПомещения(ВариантДанных, "Помещение", Представление, СтрЗаменить(ВариантДанных, "/", ""));
...
Процедура СформироватьМенюДобавленияСтроенийИПомещений()
	...
	Для каждого СтроениеИлиПомещение Из ДополнительныеСтроенияИПомещения Цикл
		...
		Если ЗначениеЗаполнено(СтроениеИлиПомещение.Ключ) Тогда
			...
			Команда.Подсказка                 = НСтр("ru='Добавить';uk='Додати'") + " " + НРег(СтроениеИлиПомещение.Представление);
			...
			Кнопка.Заголовок  = СтроениеИлиПомещение.Представление;
		КонецЕсли;
...
&НаСервере            
Процедура ДобавитьОписаниеСтроенияИлиПомещения(ИмяПомещенияИлиСтроения, Тип, Представление, Ключ = "")
	...
	ОписаниеПомещения.Представление = Представление;
...
Процедура ПереключитьНаСтруктурированныйВводЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ ?(НЕ ПустаяСтрока(ПредставлениеАдреса), НСтр("ru='Исходный адрес:';uk='Вихідна адреса:'") + " " + ПредставлениеАдреса, "");
	НаселенныйПунктДетально.comment = Комментарий;
