...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	// Национальный                                                                             
	НациональныйАдрес = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(РаботаСАдресами.ПространствоИмен(), "Адрес"+РаботаСАдресами.СуффиксНациональный()));
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда  
			НациональныйАдрес["Субъект"+РаботаСАдресами.СуффиксНациональный()] = РегионКода(ЭлементСписка.Значение);
			
		ИначеЕсли ИмяПоля = "РЕГИОН" Тогда
			НациональныйАдрес["Субъект"+РаботаСАдресами.СуффиксНациональный()] = ЭлементСписка.Значение;
...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	АдресНациональный = НациональныйАдрес(XDTOАдрес);
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение              
		Результат = АдресНациональный.Получить(РаботаСАдресами.XPathПочтовогоИндекса() );
	...
	ЗаписьИндекса = АдресНациональный.Получить(РаботаСАдресами.XPathПочтовогоИндекса());
	Если ЗаписьИндекса = Неопределено Тогда
		ЗаписьИндекса = АдресНациональный.ДопАдрЭл.Добавить(ФабрикаXDTO.Создать(XDTOАдрес.ДопАдрЭл.ВладеющееСвойство.Тип));
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(РаботаСАдресами.ПространствоИмен(), "Адрес"+РаботаСАдресами.СуффиксНациональный()) Тогда
		...
		Если ТипЗнч(АдресНациональный) = Тип("ОбъектXDTO") Тогда
			Возврат ПолучитьXDTOРеквизитОбъекта(АдресНациональный, РаботаСАдресами.XPathРайона());
		КонецЕсли;
...
Функция ЗначениеДополнительныхЭлементов(Знач XDTOАдрес) Экспорт
	...
	ДополнительныйЭлементАдреса = НайтиДополнительныйЭлементАдреса(АдресНациональный).Значение;

	Результат.ДополнительныйЭлемент = ДополнительныйЭлементАдреса;
	Результат.ПодчиненныйЭлемент = ДополнительныйЭлементАдреса(АдресНациональный, РаботаСАдресами.XPathДополнительногоОбъектаАдресации(91));
...
Функция ДополнительныйЭлементАдреса(XDTOАдрес, XPathЭлемента)
	...
	Результат = АдресНациональный.Получить(XPathЭлемента);
	Если Результат <> Неопределено Тогда
...
//                             
Функция НайтиДополнительныйЭлементАдреса(АдресНациональный)
	...
	Для каждого ВариантДополнительныйЭлементАдреса Из ВариантыДополнительногоЭлементаАдреса Цикл
		...
		ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный, XPath);
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	// Чтение                           
	Для Каждого ДопЭлемент Из АдресНациональный.ДопАдрЭл Цикл
...
//
//  Параметры:
//      ДанныеАдреса  - Строка, СписокЗначений - XML, XDTO с данными адреса.
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверУКР.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.DistrictType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для района ""%1"" не указан тип (район, поселение и пр.)';uk='Для району ""%1"" не вказано тип (район, поселення та ін.)'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверУКР.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.CityType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для города ""%1"" не указан тип (город, поселок, сельское поселение и пр.)';uk='Для міста ""%1"" не вказано тип (місто, селище, сільське поселення та ін.)'"), Город), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверУКР.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.LocalityType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для населенного пункта ""%1"" не указан тип (город, деревня, село и пр.)';uk='Для населеного пункту ""%1"" не вказано тип (місто, селище, село та ін.)'"), НаселенныйПункт),
					НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверУКР.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.StreetType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для улицы ""%1"" не указан тип (улица, переулок, шоссе и пр.)';uk='Для вулиці ""%1"" не вказано тип (вулиця, провулок, шосе та ін.)'"), Улица), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверУКР.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
	...
	Если ПустаяСтрока(ДанныеАдреса.houseNumber) И ДанныеАдреса.buildings.Количество() = 0 И ПустаяСтрока(ДанныеАдреса.stead) Тогда
		Поле = "Дом";
		ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
			НСтр("ru='Не указан номер дома';uk='Не вказаний номер будинку'"),
...
Процедура ЗаполнитьОшибкиАдресаПоКлассификатору(АдресНациональный, ФлагиПроверки, Результат)
...
//        ** Регион           - Строка - текстовое представление области (только для адресов Украины).
...
//        ** Район            - Строка - текстовое представление района (только для адресов Украины).
//        ** РайонСокращение  - Строка - сокращение района (если СтарыйСоставПолей = Ложь).
//        ** Город            - Строка - текстовое представление города (только для адресов Украины).
//        ** ГородСокращение  - Строка - сокращение города (только для адресов Украины).
//        ** НаселенныйПункт  - Строка - текстовое представление населенного пункта (только для адресов Украины).
//        ** НаселенныйПунктСокращение - сокращение населенного пункта (если СтарыйСоставПолей = Ложь).
//        ** ТипДома          - Строка - см. РаботаСАдресами.ТипыОбъектовАдресацииАдреса().
//        ** Дом              - Строка - текстовое представление дома (только для адресов Украины).
//        ** ТипКорпуса       - Строка - см. РаботаСАдресами.ТипыОбъектовАдресацииАдреса().
//        ** Корпус           - Строка - текстовое представление корпуса (только для адресов Украины).  
//        ** ТипКвартиры      - Строка - см. РаботаСАдресами.ТипыОбъектовАдресацииАдреса().
//        ** Квартира         - Строка - текстовое представление квартиры (только для адресов Украины).
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	ИначеЕсли XDTOТип = ФабрикаXDTO.Тип(РаботаСАдресами.ПространствоИмен(), "Адрес"+РаботаСАдресами.СуффиксНациональный()) Тогда
		АдресНациональный = XDTOАдрес;
	...
	ДобавитьЗначение(Список, "Индекс", ПочтовыйИндексАдреса(АдресНациональный) );

	Регион = АдресНациональный["Субъект"+РаботаСАдресами.СуффиксНациональный()]; 
	ДобавитьЗначение(Список, "Регион", Регион);
	ДобавитьЗначение(Список, "КодРегиона", РаботаСАдресами.КодРегиона(Регион) );
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РегионСокращение", Сокращение(Регион));
	КонецЕсли;

	Район = РайонАдреса(АдресНациональный);
	ДобавитьЗначение(Список, "Район", Район);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РайонСокращение", Сокращение(Район));
	КонецЕсли;
	
	ДобавитьЗначение(Список, "Город", АдресНациональный.Город);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "ГородСокращение", Сокращение(АдресНациональный.Город));
	КонецЕсли;

	НаселенныйПункт                 = АдресНациональный.НаселПункт;
	Улица                           = АдресНациональный.Улица;
	ДополнительныйЭлементАдреса     = НайтиДополнительныйЭлементАдреса(АдресНациональный).Значение;
	ПодчиненныйЭлементЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный,
	...
	// Дом и корпус                             
	ЗданияИПомещения = ЗданияИПомещенияАдреса(АдресНациональный);
	...
	Иначе
		...
		ЗначениеКорпуса = "";
		Разделитель = "";
		Сокращения = РаботаСАдресами.СокращенияОбъектовАдресацииАдреса();
		Для Каждого СтрокаОбъекта Из ПараметрыОбъекта Цикл
