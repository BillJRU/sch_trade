	...
	Иначе
		...
		Иначе
			...
			Если ЕстьИзмененияКоличества Тогда
				ПоказатьОповещениеПользователя(НСтр("ru='Количество скорректировано';uk='Кількість скориговано'"),,
					НСтр("ru='Из указанного количества к снятию резерва исключены давальческие материалы';uk='Із вказаної кількості до зняття резерву виключені давальницькі матеріали'"),
...
Процедура НастроитьЭлементыФормыПриСоздании()
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезерв
		...
		Если МогутБытьНазначенияБезЗаказа Тогда
			Заголовок = СтрЗаменить(НСтр("ru='Снятие резерва (Назначение: %1)';uk='Зняття резерву (Призначення: %1)'"), "%1", Назначение);
		Иначе
			Заголовок = СтрЗаменить(НСтр("ru='Снятие резерва (%1)';uk='Зняття резерву (%1)'"), "%1", Заказ);
		КонецЕсли;
	...
	Иначе
		...
		Если МогутБытьНазначенияБезЗаказа Тогда
			Заголовок = СтрЗаменить(НСтр("ru='Резервирование (Назначение: %1)';uk='Резервування (Призначення: %1)'"), "%1", Назначение);
		Иначе
			Заголовок = СтрЗаменить(НСтр("ru='Резервирование (%1)';uk='Резервування (%1)'"), "%1", Заказ);
		КонецЕсли;
	...
	НадписьАналитика = НСтр("ru='Номенклатура, Характеристика, Заказ, Склад';uk='Номенклатура, Характеристика, Замовлення, Склад'");
	НадписьВетви = НСтр("ru='Серия, Помещение, Ячейка';uk='Серія, Приміщення, Комірка'");
