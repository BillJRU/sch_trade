...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаРеквизитыОтсутствауют.Видимость = РеквизитыОтсутсвуют;
	Попытка
		МетаданныеСтрокой = ОбщегоНазначения.ИмяТаблицыПоСсылке(ДокументОбъект.Ссылка);
		НаборДопРеквизитов = ПредопределенноеЗначение(
			"Справочник.НаборыДополнительныхРеквизитовИСведений."+СтрЗаменить(МетаданныеСтрокой,".","_"));
		
		Элементы.ГруппаЗаго.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1 ""%2""",
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения,  СтандартнаяОбработка)
	...
	Если Модифицированность И Не СохранитьПараметры И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Дополнительные реквизиты были изменены. Закрыть без сохранения реквизитов?';uk='Додаткові реквізити були змінені. Закрити без збереження реквізитів?'"),
			СписокКнопок);
...
Процедура ИзменитьСоставДополнительныхРеквизитов(Команда)
	...
	ПараметрыПерехода = Новый Структура("Набор,Свойство,ЭтоДополнительноеСведение");
	ПараметрыПерехода.Набор = НаборДопРеквизитов;
	ПараметрыПерехода.Свойство = Неопределено;
	ПараметрыПерехода.ЭтоДополнительноеСведение = Ложь;
