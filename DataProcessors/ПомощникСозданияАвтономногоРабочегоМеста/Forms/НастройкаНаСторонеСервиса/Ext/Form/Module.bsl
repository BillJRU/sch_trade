...
Процедура ЗагрузитьНачальныйОбразНаКомпьютерПользователя(Команда)
	...
	Если ПоддерживаетсяПередачаБольшихФайлов Тогда
		...
		ПараметрыПолученияФайла.ЗаголовокДиалогаСохранения = НСтр("ru='Сохранение пакета установки';uk='Збереження пакету установки'");
		ПараметрыПолученияФайла.ФильтрДиалогаСохранения    = НСтр("ru='Архивы ZIP (*.zip)|*.zip';uk='Архіви ZIP (*.zip)|*.zip'");
	...
	Иначе
		...
		ПараметрыДиалога.Вставить("Фильтр", НСтр("ru='Архивы ZIP (*.zip)|*.zip';uk='Архіви ZIP (*.zip)|*.zip'"));
...
Процедура КакУстановитьИлиОбновитьВерсиюПлатформы1СПредприятие(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как установить или обновить версию платформы BAF';uk='Як встановити або оновити версію платформи BAF'"));
...
Процедура СохранитьИнструкциюКак(Команда)
	...
	ПолучаемыйФайл = Новый Структура;
	ПолучаемыйФайл.Вставить("Имя",      НСтр("ru='Инструкция по настройке автономного рабочего места.html';uk='Інструкція по настройці автономного робочого місця.html'"));
	ПолучаемыйФайл.Вставить("Хранение", ПолучитьМакет());
	
	ПараметрыДиалога = Новый Структура;
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ru='Веб-страница, только HTML (*.html)|*.html';uk='Вебсторінка, тільки HTML (*.html)|*.html'"));
...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМестаНаСервере(Отказ)
	...
	Попытка
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Создание начального образа автономного рабочего места';uk='Створення початкового образу автономного робочого місця'");
...
Функция АдресПриложенияВИнтернете()
	...
	Если Не ПараметрыСоединения.Свойство("ws") Тогда
		ВызватьИсключение НСтр("ru='Создать автономное рабочее место можно только в режиме веб-клиента.';uk='Створити автономне робоче місце можна тільки в режимі вебклієнта.'");
	КонецЕсли;
...
Функция Подключаемый_НастройкаВыгрузки_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеАвтономногоРабочегоМеста) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не задано наименование автономного рабочего места.';uk='Не задане найменування автономного робочого місця.'"),
