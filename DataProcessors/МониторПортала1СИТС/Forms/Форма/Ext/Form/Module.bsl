...
Функция ЗаголовокКакОформитьДоговорыНаСервисы()
	Возврат НСтр("ru='Как подключить сервисы ИТС?';uk='Як підключити сервіси ІТС?'");
КонецФункции
...
Функция ТекстОбновлениеПлатформыНеТребуется()
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='<body>● Информация о платформе BAF<br />
                |&nbsp;&nbsp;&nbsp;Текущая версия платформы BAF <strong>%1</strong></body>'
                |;uk='<body>● Інформація про платформу BAF<br />
                |&nbsp;&nbsp;&nbsp;Поточна версія платформи BAF <strong>%1</strong></body>'"),
		...
		Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='<body>● Исправления (патчи)<br />
                    ...
                    |;uk='<body>● Виправлення (патчі)<br />
                    |&nbsp;&nbsp;&nbsp;Виправлення помилок для версії <strong>%1</strong> не потрібно</body>'"),
				?(ДляНовойВерсии,
					ОбновлениеКонфигурации.Версия,
					ИнтернетПоддержкаПользователей.ВерсияКонфигурации())));
	Иначе
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>● Исправления (патчи)<br />
                        ...
                        |;uk='<body>● Виправлення (патчі)<br />
                        |&nbsp;&nbsp;&nbsp;<strong>Для нової версії конфігурації %1</strong><br />
                        |&nbsp;&nbsp;&nbsp;Виправляються помилки: <strong>%2</strong></body>'"),
					ОбновлениеКонфигурации.Версия,
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>● Исправления (патчи)<br />
                        ...
                        |&nbsp;&nbsp;&nbsp;Исправляемые ошибки: <strong>%2</strong></body>'
                        |;uk='<body>● Виправлення (патчі)<br />
                        |&nbsp;&nbsp;&nbsp;<strong>Для поточної версії конфігурації %1</strong><br />
                        |&nbsp;&nbsp;&nbsp;Виправляються помилки: <strong>%2</strong></body>'"),
					ИнтернетПоддержкаПользователей.ВерсияКонфигурации(),
					КоличествоИсправлений));
		КонецЕсли;
		...
		Иначе
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				...
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='● Обновление платформы BAF<br />
                        |&nbsp;&nbsp;&nbsp;<strong>Версия %1</strong><br />
                        |&nbsp;&nbsp;&nbsp;Текущая версия платформы BAF <strong>%2</strong>'
                        |;uk='● Оновлення платформи BAF<br />
                        |&nbsp;&nbsp;&nbsp;<strong>Версія %1</strong><br /> 
                        |&nbsp;&nbsp;&nbsp;Поточна версія платформи BAF <strong>%2</strong>'"),
					ОбновлениеПлатформы.Версия,
					ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы())
				+ ?(ПустаяСтрока(ТекстПояснениеПлатформа), "", "<br />&nbsp;&nbsp;&nbsp;" + ТекстПояснениеПлатформа)
				+ "</body>");
...
Функция ЗаголовокДоступноОбновлениеПрограммы()
	Возврат НСтр("ru='Доступно обновление программы';uk='Доступне оновлення програми'");
КонецФункции
