			...
			Если СтруктураСвойств.ИмяТипа = "Перечисление"
				...
				Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СсылкаНаИсточник))
					...
					ЗП.Текст               = НСтр("ru='В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника.
                                                        ...
                                                        |Якщо відповідного значення приймача нема, то вказати порожнє значення.'");
					//
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("ru='%1, %2; Обработано %3 объектов';uk='%1, %2; Оброблено %3 об''єктів'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("ru='Выгрузка объекта: %1';uk='Вивантаження об''єкту: %1'");
...
Функция РеквизитыРегистрацииСвойств(ТаблицаПКС)
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Загрузка правил конвертации';uk='Обмін даними.Завантаження правил конвертації'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПредставлениеКонфигурацииИзПравилОбмена(ИмяОпределения)
	...
	Если ЗначениеЗаполнено(ТочнаяВерсия) Тогда
		...
		ИмяКонфигурации = ИмяКонфигурации + " " + НСтр("ru='версия';uk='версія'") + " " + ТочнаяВерсия;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ru='Не найден узел обмена для загрузки данных. План обмена: %1, код: %2.';uk='Не вдалося знайти вузол обміну для завантаження даних. План обміну: %1, код: %2.'");
