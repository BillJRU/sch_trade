...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		ВызватьИсключение НСтр("ru='Работа со всеми внешними ресурсами (синхронизация данных, отправка почты и т.п.) заблокирована.';uk='Робота з усіма зовнішніми ресурсами (синхронізація даних, відправка пошти тощо) заблокована.'");
	КонецЕсли;
	...
	Если Состояние <> Перечисления.СостоянияМиграцииПриложения.Выполняется Тогда
		...
		Наименование = СтрШаблон(НСтр("ru='%1 (Миграция приложения)';uk='%1 (Міграція прикладної програми)'"), Метаданные.Синоним);
...
Функция СтраницаАдресСервисаДалее()
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнен адрес сервиса';uk='Не заповнений адресу сервісу'"), , "АдресСервиса");
	...
	Если ЧастиАдреса.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Некорректный адрес сервиса';uk='Некоректну адресу сервісу'"), , "АдресСервиса");
		Возврат Неопределено;
	КонецЕсли;
...
Функция СтраницаИдентификационныеДанныеДалее()
	...
	Если Не ЗначениеЗаполнено(РегистрацияНаименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Полное имя (Ф.И.О.)""';uk='Не заповнено поле ""Повне ім''я (П. І. Б.)""'"), , "РегистрацияНаименование", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Логин) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Имя(логин)""';uk='Не заповнено поле ""Ім''я(логін)""'"), , "Логин", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РегистрацияЭлектроннаяПочта) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Электронная почта""';uk='Не заповнено поле ""Електронна пошта""'"), , "РегистрацияЭлектроннаяПочта", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Пароль) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Пароль""';uk='Не заповнено поле ""Пароль""'"), , "Пароль", , Отказ);
	ИначеЕсли Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Подтверждение пароля не совпадает с паролем';uk='Підтвердження паролю не збігається з паролем'"), , "ПодтверждениеПароля", , Отказ);
	КонецЕсли;
...
Функция СтраницаВыборАбонентаДалее()
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбран абонент';uk='Не вибраний абонент'"), , "КодАбонента");
	КонецЕсли;
...
Функция СтраницаСозданиеПриложениеДалее()
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Наименование""';uk='Не заповнене поле ""Найменування""'"), , "Наименование", , Отказ);
	КонецЕсли;
