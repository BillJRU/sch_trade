...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru='Обработка не предназначена для работы в режиме веб-клиента';uk='Обробка не призначена для роботи в режимі вебклієнта'"));
	КонецЕсли;
	
	Если НЕ ЭтоУправлениеТорговлей Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Помощник работает только в УТ';uk='Помічник працює тільки в УТ'"));
		Закрыть();
	КонецЕсли;
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Выберите путь к файлу выгрузки данных из УТ 10.3';uk='Виберіть шлях до файлу вивантаження даних з УТ 10.3'"));
	...
	ТекстПредупреждения = НСтр("ru='Для данной операции необходимо установить расширение для веб-клиента BAF.';uk='Для цієї операції необхідно встановити розширення для вебклієнта BAF.'");
...
Процедура Подключаемый_СтраницаПриветствие_ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(СокрЛП(Объект.ИмяФайлаОбмена)) Тогда
		СообщитьПользователюОбОшибке(НСтр("ru='Не указан путь к файлу с данными';uk='Не зазначений шлях до файлу з даними'"));
		Отказ = Истина;
	Иначе
...
Процедура ОбработчикПроверкиСуществованияФайлОбмена(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СообщитьПользователюОбОшибке(НСтр("ru='По указанному пути файл с данными не найден';uk='За вказаним шляхом файл з даними не знайдено'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура Подключаемый_СтраницаЗавершение_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Элементы.НадписьСтатусЗагрузки.Заголовок = ?(СтатусВыполненнойЗагрузки,
												НСтр("ru='Загрузка данных успешно завершена!';uk='Завантаження даних успішно завершено!'"),
												НСтр("ru='Загрузка данных выполненна с ошибками!';uk='Завантаження даних виконано з помилками!'"));
	Элементы.НадписьВариантовПродолжения.Заголовок = ?(
												...
												НСтр("ru='Для того чтобы попробовать загрузить еще раз, нажмите ""Назад""';uk='Для того, щоб спробувати завантажити ще раз, натисніть ""Назад""'") + ", "
												+ НСтр("ru='для выхода из помошника, нажимите ""Готово""';uk='для виходу з помічника, натисніть ""Готово""'"));
...
Процедура Подключаемый_СтраницаОжидания_ПриПереходеДалее(Отказ)
	...
	Состояние(НСтр("ru='Загрузка данных...';uk='Завантаження даних...'"),, НСтр("ru='Выполняется загрузка данных из конфигурации ""Управление торговлей"" ред. 10.3';uk='Виконується завантаження даних з конфігурації ""Управління торгівлею"" ред. 10.3'"));
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если СтатусВыполненнойЗагрузки Тогда
		...
		Если ЗагруженыНастройки Тогда
			...
			Запрос.УстановитьПараметр("ВариантИспользованияЗаказов1", НСтр("ru='Заказ со склада и под заказ';uk='Замовлення зі складу та під замовлення'"));
			Запрос.УстановитьПараметр("ВариантИспользованияЗаказов2", НСтр("ru='Заказ как счет';uk='Замовлення як рахунок'"));
		...
		КонецЕсли;
	Иначе
		ИтоговаяИнформация = ИтоговаяИнформация + Символы.ПС+ НСтр("ru='Протокол ошибок';uk='Протокол помилок'") + ": ";
...
Процедура СформироватьСписокЗарегистрированныхБаз(АдресФайлаСоСпискомИБ, ФайловыйРежим)
	...
	СписокИБДерево = Обработки.ПомощникПереходаСУправлениеТорговлей23.СформироватьСписокЗарегистрированныхБаз(АдресФайлаСоСпискомИБ, ФайловыйРежим);
...
Функция ПолучитьФайлыССервера(ДанныеПользователя)
	...
	Для каждого ЭлемСсылка Из СсылкаНаСкачивание.fileDtoList Цикл
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неудачная попытка соединения.';uk='Невдала спроба з''єднання.'"));
			Возврат Неопределено;
