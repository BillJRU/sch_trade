...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ФайлОбработкиИмя) Или Не ЗначениеЗаполнено(ФайлОбработкиАдрес) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Укажите файл внешнего отчета или обработки.';uk='Вкажіть файл зовнішнього звіту чи обробки.'"),, 
			"ФайлОбработкиИмя");
		Отказ = Истина;
	Иначе
		...
		Если НРег(СвойстваФайла.Расширение) <> НРег(".epf") И НРег(СвойстваФайла.Расширение) <> НРег(".erf") Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Выбранный файл не является внешним отчетом или обработкой.';uk='Обраний файл не є зовнішнім звітом або обробкою.'"),,
				"ФайлОбработкиИмя");
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(БезопасныйРежим) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Укажите безопасный режим для подключения внешнего модуля.';uk='Виберіть безпечний режим для підключення зовнішнього модуля.'"),, 
			"БезопасныйРежим");
		Отказ = Истина;
	КонецЕсли;
