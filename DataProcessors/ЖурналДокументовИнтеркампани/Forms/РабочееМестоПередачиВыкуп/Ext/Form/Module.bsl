...
Процедура МестоХраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если МестоХранения = Неопределено
		...
		СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"), НСтр("ru='Склад (складская территория)';uk='Склад (складська територія)'"));
		СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.СтруктураПредприятия.ПустаяСсылка"), НСтр("ru='Подразделение';uk='Підрозділ'"));
		СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка"), НСтр("ru='Договоры контрагентов';uk='Договори контрагентів'"));
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru='Выбор типа данных';uk='Вибір типу даних'"));
...
Функция ПеренестиРезервыСервер()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Перенос резервов товаров организаций';uk='Перенесення резервів товарів організацій'");
...
Процедура ПослеЗаполненияКОформлениюЗаПериод(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить выполнения задания по причине: %Причина%';uk='Не вдалося закінчити виконання завдання з причини: %Причина%'");
...
Процедура НастроитьФорму(Параметры)
	...
	Если ОформляемыеДокументы = "ТолькоПередачи" Тогда
		...
		Заголовок = НСтр("ru='Передачи товаров к оформлению';uk='Передачі товарів до оформлення'");
	...
	Иначе
		...
		Заголовок = НСтр("ru='Выкупы товаров к оформлению';uk='Викупи товарів до оформлення'");
		...
		ЗаголовокОтправитель = НСтр("ru='Поклажедатель';uk='Поклажодавець'");
	...
	СсылкаПерейти = ОбщегоНазначенияУТ.СформироватьГиперссылкуСмТакжеВРаботе(МассивМенеджеровРасчетаСмТакжеВРаботе, ПараметрыФормирования, НСтр("ru='Перейти:';uk='Перейти:'")); 
...
Функция ПараметрыЖурнала()
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала", НСтр("ru='Документы между организациями';uk='Документи між організаціями'"));
