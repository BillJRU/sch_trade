...
Процедура УстановитьЗаголовкиПодсказок(Форма)
	...
	Если Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.День") Тогда
		ТекущийПериод     = НСтр("ru='текущий день';uk='поточний день'");
		АналогичныйПериод = НСтр("ru='аналогичный текущему день прошлого года';uk='аналогічний поточному день минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Неделя") Тогда
		ТекущийПериод     = НСтр("ru='текущая неделя';uk='поточний тиждень'");
		АналогичныйПериод = НСтр("ru='аналогичная текущей неделя прошлого года';uk='аналогічний поточному тиждень минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Декада") Тогда
		ТекущийПериод     = НСтр("ru='текущая декада';uk='поточна декада'");
		АналогичныйПериод = НСтр("ru='аналогичная текущей декада прошлого года';uk='аналогічна поточній декада минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Месяц") Тогда
		ТекущийПериод     = НСтр("ru='текущий месяц';uk='поточний місяць'");
		АналогичныйПериод = НСтр("ru='аналогичный текущему месяц прошлого года';uk='аналогічний поточному місяць минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Квартал") Тогда
		ТекущийПериод     = НСтр("ru='текущий квартал';uk='поточний квартал'");
		АналогичныйПериод = НСтр("ru='аналогичный текущему квартал прошлого года';uk='аналогічний поточному квартал минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Полугодие") Тогда
		ТекущийПериод     = НСтр("ru='текущее полугодие';uk='поточне півріччя'");
		АналогичныйПериод = НСтр("ru='аналогичное текущему полугодие прошлого года';uk='аналогічне поточному півріччя минулого року'");
	ИначеЕсли Форма.ПериодРасчета = ПредопределенноеЗначение("Перечисление.Периодичность.Год") Тогда
		ТекущийПериод     = НСтр("ru='текущий год';uk='поточний рік'");
		АналогичныйПериод = НСтр("ru='прошлый год';uk='минулий рік'");
	Иначе
		ТекущийПериод     = НСтр("ru='<период не указан>';uk='<період не вказано>'");
		АналогичныйПериод = НСтр("ru='<период не указан>';uk='<період не вказано>'");
	КонецЕсли;
	
	ПредыдущийПериод = ОбеспечениеКлиентСервер.ОписаниеНастройки(Форма.ПериодРасчета, Форма.КоличествоПериодовРасчета);
	
	Форма.ПодсказкаПериодДействияРасчета = СтрШаблон(НСтр("ru='Период действия расчета: %1.';uk='Період дії розрахунку: %1.'"), ТекущийПериод);
	
	РасчетСреднедневного = НСтр("ru='Среднедневное потребление расчитывается по данным за период: %1.';uk='Середньоденне споживання розраховується за даними за період: %1.'");
	...
	Если Форма.ВариантУчетаСезонныхКолебаний = ПредопределенноеЗначение("Перечисление.УчетСезонныхКолебаний.ПоАналогичномуПериодуПрошлогоГода") Тогда
		Коррекция = НСтр("ru='Среднедневное потребление корректируется согласно динамике потребления соответствующей товарной категории. Динамика потребления по товарной категории формируется по данным за %1 и за аналогичный период относительно прошлого года.';uk='Середньоденне споживання коригується згідно з динамікою споживання відповідної товарної категорії. Динаміка споживання по товарній категорії формується за даними за %1 і за аналогічний період відносно минулого року.'");
	...
	ИначеЕсли Форма.ВариантУчетаСезонныхКолебаний = ПредопределенноеЗначение("Перечисление.УчетСезонныхКолебаний.ПоСезоннымКоэффициентам") Тогда
		...
		Коррекция = Новый ФорматированнаяСтрока(
			НСтр("ru='Среднедневное потребление корректируется согласно';uk='Середньоденне споживання коригується згідно з'"),
			" ",
			Новый ФорматированнаяСтрока(НСтр("ru='сезонным коэффициентам';uk='сезонними коефіцієнтами'"),,,, СсылкаНаФорму),
			" ",
			НСтр("ru='соответствующей сезонной группы.';uk='відповідної сезонної групи.'"));
