...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru='Изменения отбора товаров будут потеряны.';uk='Зміни відбору товарів будуть втрачені.'");
	Иначе
...
Процедура ДобавитьХарактеристики(Команда)
	...
	Если ДанныеСтроки = Неопределено ИЛИ НЕ ЗначениеЗаполнено(ДанныеСтроки.Номенклатура) Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Не выбрана номенклатура, для характеристик которой требуется установить индивидуальные параметры. Выполнение команды невозможно.';uk='Не обрана номенклатура, для характеристик якої потрібно встановити індивідуальні параметри. Виконання команди неможливе.'"));
	ИначеЕсли НЕ ДанныеСтроки.ХарактеристикиИспользуются Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Для выбранной номенклатуры характеристики не используются. Выполнение команды невозможно.';uk='Для обраної номенклатури характеристики не використовуються. Виконання команди неможливо.'"));
	Иначе
...
Процедура РассчитатьРекомендацииИСтатистикуПотребления(Команда)
	...
	СостояниеРекомендаций = НСтр("ru='Идет расчет рекомендаций';uk='Йде розрахунок рекомендацій'");
	...
	Если РезультатРасчета.ЗаданиеВыполнено Тогда
		СостояниеРекомендаций = НСтр("ru='Рекомендации рассчитаны';uk='Рекомендації розраховані'");
...
Процедура Сгруппировать(Команда)
	...
	Если ЕстьВыделенныеСтроки() Тогда
		...
		ПоказатьВводЗначения(ОписаниеОповещения, СтруктураРезультата.Наименование,
			НСтр("ru='Введите наименование группы';uk='Введіть найменування групи'"));
	КонецЕсли;
...
Процедура УдалитьХарактеристики(Команда)
	...
	Если ЕстьВыделенныеСтроки() Тогда
		...
		Если УдалитьДетальнуюНастройкуНаСервере(МассивЗаписей) Тогда
			Элементы.СписокПоддерживаемыхЗапасов.Обновить();
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Не выделено ни одной строки с индивидуальными параметрами для характеристик. Выполнение команды невозможно.';uk='Не виділено жодного рядка з індивідуальними параметрами для характеристик. Виконання команди неможливе.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",			НСтр("ru='<только для элементов>';uk='<тільки для елементів>'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Характеристика");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",			НСтр("ru='<только для характеристик>';uk='<тільки для характеристик>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста",	ЦветаСтиля.НезаполненноеПолеТаблицы);
	ОформляемыеПоля.Очистить();
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",			НСтр("ru='<в любой день>';uk='<у будь-який день>'"));
...
Процедура ОформитьИсточникОбеспечения(УсловноеОформление, ИмяПоляВводаИсточникаОбеспечения,
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<любые подразделения>';uk='<будь-які підрозділи>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<сборка товаров>';uk='<збірка товарів>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<любые переработчики>';uk='<будь-які переробники>'"));
...
Процедура ЗаполнитьРесурсПослеВводаЗначения(Число, ДополнительныеПараметры) Экспорт
	...
	Если ОбработаноСтрок > 0 Тогда
		ШаблонСтроки = НСтр("ru='Обработано позиций: %1.';uk='Опрацьовано позицій: %1.'");
		ТекстОбработаноСтрок = СтрШаблон(ШаблонСтроки, ОбработаноСтрок);
		ПоказатьОповещениеПользователя(НСтр("ru='Заполнение завершено';uk='Заповнення завершене'"), , ТекстОбработаноСтрок);
		Элементы.СписокПоддерживаемыхЗапасов.Обновить();
	Иначе
		ПоказатьОповещениеПользователя(НСтр("ru='Заполнение не произведено';uk='Заповнення не виконано'"), , ТекстСообщенияОбОшибке);
	КонецЕсли;
...
Функция РассчитатьРекомендацииПоддержанияЗапаса()
	...
	ИначеЕсли ИспользуетсяРегламентноеЗадание() Тогда
		...
		СостояниеРекомендаций = НСтр("ru='Рекомендации актуальны';uk='Рекомендації актуальні'");
		Элементы.РежимРекомендаций.Доступность = Истина;
...
Функция РасчетРекомендацийПоддержанияЗапасовСкладаВФоне(РучнойЗапуск = Ложь)
	...
	НаименованиеЗадания = НСтр("ru='Расчет рекомендаций поддержания запасов склада: %1';uk='Розрахунок рекомендацій підтримки запасів складу: %1'");
...
Процедура НастроитьФормуПоПараметрам()
	...
	Заголовок = НСтр("ru='Настройка поддержания запасов';uk='Настройка підтримання запасів'");
	...
	Если Источник = "ИзменениеАссортимента" Тогда
		...
		Шаблон = НСтр("ru='Настройка поддержания запасов (Документ: ""%1"")';uk='Настройка підтримки запасів (Документ: ""%1"")'");
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		...
		НСтр("ru='Поддерживать запас';uk='Підтримувати запас'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		СписокПоддерживаемыхЗапасов,
		"РекомендацияНеПоддерживать",
		НСтр("ru='Не поддерживать запас';uk='Не підтримувати запас'"));
...
Процедура УстановитьДоступностьКнопокДобавления(Форма, Доступно)
	...
	Если Не Доступно Тогда
		Форма.СостояниеРекомендаций = НСтр("ru='Рекомендации и добавление товаров доступны только при заданном отборе по складу';uk='Рекомендації і додавання товарів доступні тільки при заданому відборі за складом'");
	КонецЕсли;
...
Процедура ПереместитьВГруппуНаСервере(МассивЗаписей, Группировка, Склад)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Группировка настроек поддержания запаса';uk='Групування настройок підтримання запасу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьРесурс(Ресурс)
	...
	ЗаголовокОкнаВводаЗначения =
		...
		?(Ресурс = "НормаПотребления",             НСтр("ru='Введите норму потребления в день';uk='Введіть норму споживання в день'"),
			НСтр("ru='Выберите обеспечение заказов';uk='Виберіть забезпечення замовлень'")))));
	
	ТекстСообщенияОбОшибке =
		?(Ресурс = "МинимальноеКоличествоЗапаса", НСтр("ru='Заполнение минимального запаса возможно только для строк с поддержанием запаса ""Min - max"".';uk='Заповнення мінімального запасу можливо тільки для рядків з підтриманням запасу ""Min - max"".'"),
		?(Ресурс = "МаксимальноеКоличествоЗапаса", НСтр("ru='Заполнение максимального запаса возможно только для строк с поддержанием запаса ""Min - max"".';uk='Заповнення максимального запасу можливо тільки для рядків з підтриманням запасу ""Min - max"".'"),
		?(Ресурс = "СтраховоеКоличествоЗапаса", НСтр("ru='Заполнение страхового запаса возможно только для строк с поддержанием запаса ""Расчет по норме"", ""Расчет по статистике"".';uk='Заповнення страхового запасу можливо тільки для рядків з підтриманням запасу ""Розрахунок за нормою"", ""Розрахунок за статистикою"".'"),
		?(Ресурс = "НормаПотребления", НСтр("ru='Заполнение нормы потребления возможно только для строк с поддержанием запаса ""Расчет по норме"".';uk='Заповнення норми споживання можливо тільки для рядків з підтриманням запасу ""Розрахунок за нормою"".'"), 
		НСтр("ru='Заполнение обеспечения заказа возможно только для строк с поддержанием запаса.';uk='Заповнення забезпечення замовлення можливо тільки для рядків з підтриманням запасу.'")))));
	...
	ТекстРесурсЗаполнен =
		...
		?(Ресурс = "НормаПотребления",             НСтр("ru='Норма потребления заполнена.';uk='Норма споживання заповнена.'"),
			НСтр("ru='Обеспечение заказов заполнено.';uk='Забезпечення замовлень заповнене.'")))));
...
Процедура СохранитьНастройки()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.НастройкаПоддержанияЗапасов", "Основная", Настройки);
...
Процедура ВосстановитьНастройки()
	...
	ЗначениеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.НастройкаПоддержанияЗапасов", "Основная");
...
Функция ЕстьВыделенныеСтроки()
	...
	Если Элементы.СписокПоддерживаемыхЗапасов.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выделено ни одной строки. Выполнение команды невозможно.';uk='Не виділено жодного рядка. Виконання команди неможливе.'"));
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура УдалитьПоддержаниеЗапасаНаСервере(МассивЗаписей, Склад)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Удаление поддерживаемых запасов';uk='Вилучення підтримуваних запасів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
