...
Процедура ПодобратьЦелевоеВремя(Команда)                             
	...
	Если РезультатПроверки Тогда
		...
		ДиаграммаЗамеровВремени.ОбластьПостроения.ШкалаЗначений.ТекстЗаголовка = НСтр("ru='Количество замеров, шт';uk='Кількість замірів, шт'");
...
Функция ПроверкаЗаполнения()
	...
	Если КлючеваяОперация.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не указана ключевая операция.';uk='Не зазначена ключова операція.'");
	...
	Если Не ЗначениеЗаполнено(ТекущийApdex) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не указан текущий APDEX.';uk='Не вказаний поточний APDEX.'");
	...
	Если Не ЗначениеЗаполнено(Период.ДатаНачала) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не указана дата начала периода.';uk='Не вказана дата початку періоду.'");
	...
	Если Не ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не указана дата окончания периода.';uk='Не зазначена дата закінчення періоду.'");
		Сообщение.Поле = "Период";
		Сообщение.Сообщить();
		Успешно = Ложь;
	КонецЕсли;
...
Функция ПодборЦелевоеВремяНаСервере(ПараметрыПодбора)
	...
	Иначе
		РезультатПодбора.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось получить данные о замерах, попробуйте изменить настройки.';uk='Не вдалося отримати дані про виміри, спробуйте змінити настройки.'"));
	...
	Если РезультатПодбора.КоличествоЗамеров = 0 Тогда
		РезультатПодбора.Вставить("ОписаниеОшибки",
			ОценкаПроизводительностиКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Получено 0 замеров по ключевой операции %1. Измените период или выберите другую ключевую операцию.';uk='Отримано 0 вимірів по ключовій операції %1. Змініть тайм або виберіть іншу ключову операцію.'"),
