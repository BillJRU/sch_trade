...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КонтекстОперации.ЗаголовокОперации = "" Тогда
		Заголовок = НСтр("ru='При выполнении операции произошли ошибки:';uk='При виконанні операції відбулися помилки:'");
	Иначе 
		Заголовок = КонтекстОперации.ЗаголовокОперации + " " + НСтр("ru='произошли ошибки:';uk='відбулися помилки:'");
	КонецЕсли;
	...
	Если Не Параметры.Свойство("ПараметрыВидовОшибок", ПараметрыВидовОшибок) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не заполнен параметр ""ПараметрыВидовОшибок"", использование формы невозможно';uk='Не заповнений параметр ""ПараметрыВидовОшибок"", використання форми неможливо'"));
		Возврат; 
	КонецЕсли;
...
Процедура ВывестиОшибки(ПараметрыВидовОшибок)
	...
	Для каждого Ошибка Из КонтекстОперации.Диагностика.Ошибки Цикл
		...
		Если ПараметрыВидаОшибки.ВыводитьСсылкуНаСписокОшибок Тогда
			...
			ДекорацияСписокОшибок.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='См. <a href = ""список ошибок"">список ошибок</a>';uk='Див. <a href = ""список ошибок"">список помилок</a>'"));
		...
		ЭлементыКолонки.ДекорацияКартинка.Картинка = БиблиотекаКартинок.ИнформацияБСП;
		ЭлементыКолонки.ДекорацияОписание.Заголовок = НСтр("ru='Решение';uk='Рішення'");
...
Процедура ОбновитьЭлементыФормыРешения(ВидОшибки) 
	...
	Если КоличествоОшибок = 0 Тогда
		...
		Элементы[ИмяЭлементаПояснениеРешения(ВидОшибки)].Заголовок = НСтр("ru='Не требуется';uk='Не потрібно'");
		Элементы[ИмяЭлементаПояснениеОшибки(ВидОшибки)].Заголовок = НСтр("ru='Проблемы устранены';uk='Проблеми усунені'");
	КонецЕсли;
	ПараметрыВидаОшибки = ЭлектронноеВзаимодействиеОбработкаОшибокКлиент.ПараметрыВидаОшибки(ВидОшибки);
