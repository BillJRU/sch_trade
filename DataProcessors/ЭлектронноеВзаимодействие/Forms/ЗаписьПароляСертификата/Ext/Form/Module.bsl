...
|а программа платформы BAF должна передать пустой пароль, чтобы не было ошибки.
|
|Запоминание и запись пароля невозможны.'
|;uk='У сертифіката встановлена посилений захист закритого ключа.
|В такому випадку пароль запитує програма електронного підпису та шифрування,
|а програма платформи BAF повинна передати порожній пароль, щоб не було помилки. 
|
...
Процедура ЗаписатьПароль(Команда)
	...
	Если Не ЗначениеЗаполнено(Пароль) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Поле ""Пароль"" не заполнено';uk='Поле ""Пароль"" не заповнено'"));
		Возврат; 
	КонецЕсли;
...
Процедура ЗакончитьПроверкуПодписания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("ИнформацияОбОшибке") Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось пройти проверку подписания с помощью программы %1 по причине:';uk='Неможливо пройти перевірку підписання за допомогою програми %1 по причині:'"), Программа)
...
|
|Запис пароля можуть зробити користувачі:
|%1,
|%2
|або адміністратор.'");
...
Процедура УстановитьЗаголовокНадписиИПометкуКомандыДоступенВсем()
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru='Пароль доступен пользователю:';uk='Пароль доступний користувачу:'")
			+ " " + Пользователь;
	Иначе
