...
Процедура ДополнитьДвиженияДокументов(Параметры, АдресХранилища = "") Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Партионный учет сервер.Проведение документов по регистрам партий';uk='Партіонний облік сервер. Проведення документів по регістрах партій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОтменаПроведенияДокументовПоРегистрамПартий() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='ПартионныйУчет.Выключение партионного учета';uk='Партіонний облік. Виключення партіонного обліку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция ПропускаемыеИменаДокументов()
	Возврат Новый Структура("
		// Не надо допроводить, есть в BASУТ:
...
Процедура ОбработатьТоварныеДокументы(ОписаниеДокументов, НачалоПериода, КонецПериода);
	...
	Для Каждого Описание Из ОписаниеДокументов Цикл
		...
		Пока Выборка.Следующий() Цикл
			ЗаписьЖурналаРегистрации(НСтр("ru='Дополнение движений по партиям';uk='Додаток рухів по партіям'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), , , Выборка.Ссылка);
...
Процедура ОбработатьДокументыРасходов(ОписаниеДокументов, НачалоПериода, КонецПериода);
	...
	Для Каждого Описание Из ОписаниеДокументов Цикл
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ЗаписьЖурналаРегистрации(НСтр("ru='Дополнение движений по расходам';uk='Додаток рухів за видатками'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), , , Выборка.Ссылка);
			ДополнитьДвиженияДокумента(Выборка.Ссылка, РегистрыДополненияДвижений);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
