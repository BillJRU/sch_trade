...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если СвойстваВерсии.Версия < 5 Тогда
        ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Автоматическая оплата сервиса не поддерживается.';uk='Автоматична оплата сервісу не підтримується.'"),,,, Отказ);
        Возврат;
    КонецЕсли; 
    ...
    Если Абонент.РольПользователя <> Перечисления.РолиПользователейАбонентов.ВладелецАбонента Тогда
        ПоказатьОшибку(НСтр("ru='Оплата сервиса доступна только пользователю с ролью ''Владелец абонента''.';uk='Оплата сервісу доступна тільки користувачу з роллю ''Власник абонента''.'"));
    КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПродлитьТариф(Команда) Экспорт
...
&НаКлиенте
Процедура Подключаемый_ОткрытьОписание(Команда) Экспорт
...
// Параметры:
// 	Группа - ДанныеФормыЭлементКоллекции - информация по группе подписки, поля:
// 	* Наименование - Строка - наименование тарифа.
// 	* ДатаПодключения - Дата - дата подключения тарифа.
// 	* ДатаОтключения - Дата - дата отключения тарифа.
// 	* СоставГруппы - ДанныеФормыКоллекция - поля:
// 	  ** Наименование - Строка - расширение тарифа.
// 	  ** Количество - Число - количество подписок.
...
Процедура СоздатьЭлементыПодписки(Группа, СоздаватьВариантыПродления, ЦветФонаПодписки, РодительскаяГруппа)
    ...
    НаименованиеТарифа = Группа.Наименование;
    ДекорацияТариф.Заголовок = ?(Группа.Количество = 1, НаименованиеТарифа, 
        СтрШаблон("%1 
            |Х %2", НаименованиеТарифа, Группа.Количество));
    ...
    Расширения = РасширенияГруппы.НайтиСтроки(Новый Структура("Расширение", Истина)); // Массив из СтрокаТаблицыЗначений
    ...
    Для Каждого СтрокаРасширения Из Расширения Цикл
        ...
        НаименованиеТарифаРасширения = СтрокаРасширения.Наименование;
        ДекорацияРасширение.Заголовок = ?(СтрокаРасширения.Количество = 1, НаименованиеТарифаРасширения, 
            СтрШаблон("%1 
            |Х %2", НаименованиеТарифаРасширения, СтрокаРасширения.Количество));
	...
	Если Группа.ПериодыПродления.Количество() = 0 Тогда
        ТекстПродления = НСтр("ru='Эту подписку нельзя продлить.';uk='Цю підписку неможна продовжити.'"); 
    ...
    ТекстПродления = ?(Группа.ПериодыПродления.Количество() > 0, НСтр("ru='Продлить:';uk='Продовжити:'"), НСтр("ru='Эту подписку нельзя продлить.';uk='Цю підписку неможна продовжити.'")); 
    ДекорацияПродлитьНа = Элементы.Добавить(ИмяПродлитьНа, Тип("ДекорацияФормы"), ГруппаОсновная); 
    ДекорацияПродлитьНа.Вид = ВидДекорацииФормы.Надпись;
    ДекорацияПродлитьНа.Заголовок = ТекстПродления;
    ДекорацияПродлитьНа.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Центр;
...
Функция ВыставитьСчетПоВыбранномуТарифу(КодТарифа, КодПериода)
    ...
    Если ИспользоватьТарифыОО Тогда
        ...
        Если Не ЗначениеЗаполнено(ТарифОО) Тогда
            ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                НСтр("ru='Не найден тариф обслуживающей организации ''%1'' по коду тарифа ''%2''.';uk='Не знайдено тариф обслуговуючої організації '' %1'' за кодом тарифу '' %2''.'"), КодОО, КодТарифа);
        КонецЕсли;
    ...
    Иначе
        ...
        Если Не ЗначениеЗаполнено(Тариф) Тогда
            ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                НСтр("ru='Не найден тариф сервиса по коду тарифа ''%1''.';uk='Не знайдено тариф сервісу за кодом тарифу '' %1''.'"), КодТарифа);
        КонецЕсли; 
    КонецЕсли; 
...
Процедура ПриИзмененииОбслуживающейОрганизации()
    ...
    Если Не ТекущаяОО.РазрешеноАвтоматическоеВыставлениеСчетов Тогда
        ПоказатьОшибку(НСтр("ru='Для продления доступа к сервису обратитесь к обслуживающей организации.';uk='Для продовження доступу до сервісу зверніться до обслуговуючої організації.'"));
    КонецЕсли; 
