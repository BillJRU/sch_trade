...
Процедура ОбновитьКоличествоАктивныхСеансов(Форма)
	...
	Если Форма.КоличествоАктивныхСеансов > 0 Тогда
		ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Активные пользователи (%1)';uk='Активні користувачі (%1)'"), 
			Форма.КоличествоАктивныхСеансов);
	Иначе
		ТекстГиперссылки = НСтр("ru='Активные пользователи';uk='Активні користувачі'");
	КонецЕсли;	
	...
	Если Форма.КоличествоАктивныхСеансов = 0 И ПустаяСтрока(Форма.ОшибкаУстановкиМонопольногоРежима) Тогда
		Форма.Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru='Другие пользователи уже завершили свою работу:';uk='Інші користувачі вже завершили свою роботу:'");
		Форма.Элементы.ПояснениеПоОшибке.Заголовок = НСтр("ru='Для продолжения нажмите Повторить.';uk='Для продовження натисніть Повторити.'");
	Иначе	
		Если Форма.УдалениеПомеченныхОбъектов Тогда
			ТекстСообщенияОбОшибке = НСтр("ru='Невозможно выполнить удаление помеченных объектов, т.к. не удалось завершить работу пользователей:';uk='Неможливо виконати видалення позначених об''єктів, тому що не вдалося завершити роботу користувачів:'");
		Иначе	
			ТекстСообщенияОбОшибке = НСтр("ru='Невозможно выполнить обновление версии программы, т.к. не удалось завершить работу пользователей:';uk='Неможливо виконати оновлення версії програми, тому що не вдалося завершити роботу користувачів:'");
		КонецЕсли;
		Форма.Элементы.ТекстСообщенияОбОшибке.Заголовок = ТекстСообщенияОбОшибке;
		Форма.Элементы.ПояснениеПоОшибке.Заголовок = НСтр("ru='Для продолжения необходимо завершить их работу.';uk=' Для продовження необхідно завершити їхню роботу.'");
	КонецЕсли;	
...
Процедура ПроверитьМонопольныйРежимНаСервере()
	...
	Если КоличествоАктивныхСеансов = 0 Тогда
		...
		Исключение
			ОшибкаУстановкиМонопольногоРежима = НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + " " 
...
Процедура УстановитьБлокировкуФайловойБазы()
	...
	Иначе
		...
		Объект.СообщениеДляПользователей = НСтр("ru='Программа заблокирована для обновления на новую версию.';uk='Програма заблокована для оновлення на нову версію.'");
	КонецЕсли;
