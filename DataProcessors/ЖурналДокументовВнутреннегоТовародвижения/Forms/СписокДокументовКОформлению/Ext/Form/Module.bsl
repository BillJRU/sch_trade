...
Процедура ОформитьПоЗаказам(Команда)
	...
	Если ЗначениеЗаполнено(ГруппировкиРаспоряжений) Тогда
		...
		НаименованиеЗадания = НСтр("ru='Журнал внутренних документов: Оформление по заказам';uk='Журнал внутрішніх документів: Оформлення за замовленнями'");
...
Процедура ОформитьПоОтгрузкеПриемке(Команда)
	...
	Если ЗначениеЗаполнено(ГруппировкиРаспоряжений) Тогда
		...
		НаименованиеЗадания = НСтр("ru='Журнал внутренних документов: Оформление по отгрузке/приемке';uk='Журнал внутрішніх документів: Оформлення за відвантаженням / прийманням'");
...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда
			Если Строка.СостояниеНакладной = 1 Тогда
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить акт о расхождениях, т.к. нет ни одной накладной.';uk='Для розпорядження %1 неможливо оформити акт про розбіжності, тому що немає жодної накладної.'");
		...
		Иначе
			ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить акт о расхождениях, т.к. оно не является перемещением товаров.';uk='Для розпорядження %1 неможливо оформити акт про розбіжності, тому що воно не є переміщенням товарів.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Строка.Ссылка);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Накладные) Или ЗначениеЗаполнено(Заказы) Тогда
		...
		НаименованиеЗадания = НСтр("ru='Журнал внутренних документов: Оформление актов о расхождениях';uk='Журнал внутрішніх документів: Оформлення актів про розбіжності'");
	...
	ИначеЕсли Элементы.СписокРаспоряженияНаОформление.ВыделенныеСтроки.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru='Не выбрано ни одного документа, для которого можно было бы оформить акт о расхождениях.';uk='Не вибрано жодного документа, для якого можна було б оформити акт про розбіжності.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление", ТекстОшибки, Неопределено);
...
Процедура СостояниеВыполнения(Команда)
	...
	Иначе
		...
		Если СписокДокументов.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Для выбранных документов отчет не поддерживается. Выполнение команды невозможно.';uk='Для обраних документів звіт не підтримується. Виконання команди неможливо.'"));
			Возврат;
		КонецЕсли;
...
Процедура НастроитьФормуПоПравамИФункциональнымОпциям()
	...
	Иначе
		...
		Элементы.СписокРаспоряженияНаОформлениеОформитьПоЗаказам.Заголовок = НСтр("ru='Оформить';uk='Оформити'");
...
Функция ПараметрыЖурнала()
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала",				НСтр("ru='Внутренние документы';uk='Внутрішні документи'"));
...
Процедура НастроитьФормуПоВыбраннымОперациямИДокументам(ТЗХозОперацииИТипыДокументов)
	...
	ЗаголовокПоУмолчанию = НСтр("ru='Внутренние документы (к оформлению)';uk='Внутрішні документи (до оформлення)'");
	...
	Если Заголовок <> ЗаголовокПоУмолчанию Тогда
		Заголовок = Заголовок + " " + НСтр("ru='(к оформлению)';uk='(до оформлення)'");
	КонецЕсли;
...
Процедура СформироватьТекстыЗапросов()
	...
	Для каждого КЗ Из Состояния Цикл
		...
		Если КЗ.Ключ = "СостоянияРасходныхОрдеров" Тогда
			...
			|				ИЛИ ВЫБОР
			|					КОГДА СостоянияРасходныхОрдеров.Распоряжение ССЫЛКА Документ.ЗаказНаПеремещение
			|						ТОГДА ВЫРАЗИТЬ(СостоянияРасходныхОрдеров.Распоряжение КАК Документ.ЗаказНаПеремещение).ВариантПриемкиТоваров
			|					КОГДА СостоянияРасходныхОрдеров.Распоряжение ССЫЛКА Документ.ЗаказНаСборку
