...
|%2'");
	...
	Для каждого СтрокаДанныеЗаказа Из Объект.ФормированиеЗаказов Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Формирование заказов на сборку (разборку) по плану';uk='Формування замовлень на збирання (розбирання) за планом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ВводДоступен() Тогда
		Если ПроверитьЕстьДокументыДляОбновления() Тогда
			ПоказатьВопрос(
				...
				НСтр("ru='Заказы на сборку(разборку) были изменены. Хотите обновить данные? При обновлении строки будут очищены.';uk='Замовлення на збирання (розбирання) були змінені. Хочете оновити дані? При оновленні рядки будуть очищені.'"), 
				РежимДиалогаВопрос.ДаНет,
				,
				,
				НСтр("ru='Формирование заказов на сборку разборку по плану';uk='Формування замовлень на збирання розбирання за планом'"));
		КонецЕсли;
	Иначе
		ОбновитьДанныеКогдаФормаСтанетАктивной();
	КонецЕсли;
