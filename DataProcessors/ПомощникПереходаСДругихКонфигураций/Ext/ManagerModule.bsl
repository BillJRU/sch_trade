...
// Выполняет загрузку данных для перехода с других конфигураций "BAF".
// 
// Параметры:
...
Процедура ЗагрузитьДанные(ПараметрыЗагрузки, АдресХранилища, АдресДополнительногоРезультата) Экспорт
	...
	Для Каждого ЭтапЗагрузки Из ПараметрыЗагрузки.ЭтапыЗагрузки Цикл
		...
		Исключение
			РезультатВыполнения.ТекстСообщения = НСтр("ru='При извлечении данных из архива произошла ошибка';uk='При отриманні даних з архіву сталася помилка'") + ":" + Символы.ПС
				+ ОписаниеОшибки() + ".";
		...
		Если Не РезультатВыполнения.Ошибка Тогда
			...
			Исключение
				РезультатВыполнения.ТекстСообщения = НСтр("ru='При чтении данных произошла ошибка';uk='При читанні даних виникла помилка'") + ":" + Символы.ПС
					+ ОписаниеОшибки() + НСтр("ru='Ошибка в формате файла данных';uk='Помилка у форматі файлу даних'") + ".";
		...
		Если Не РезультатВыполнения.Ошибка Тогда
			...
			Если Не ОбработкаОбмена.мБылиПрочитаныПравилаОбменаПриЗагрузке Тогда
				...
				РезультатВыполнения.ТекстСообщения = НСтр("ru='При чтении данных произошла ошибка';uk='При читанні даних виникла помилка'") + ".";
				РезультатВыполнения.Ошибка         = Истина;
		...
		Если Не РезультатВыполнения.Ошибка Тогда
			...
			Исключение
				...
				РезультатВыполнения.ТекстСообщения = НСтр("ru='При загрузке данных произошла ошибка';uk='При завантаженні даних сталася помилка'") + ":" + Символы.ПС
					+ ОписаниеОшибки() + ".";
		...
		Если Не РезультатВыполнения.Ошибка Тогда
			...
			Если ОбработкаОбмена.ФлагОшибки Тогда
				...
				РезультатВыполнения.ТекстСообщения = НСтр("ru='При загрузке данных произошла ошибка';uk='При завантаженні даних сталася помилка'") + ".";
				РезультатВыполнения.Ошибка         = Истина;
				
			Иначе
				
				РезультатВыполнения.ТекстСообщения = НСтр("ru='Загрузка данных завершена';uk='Завантаження даних завершене'") + ".";
