...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если АвтоТестОбмена Тогда
		...
		Если ПустаяСтрока(Объект.ИмяФайлаДанных) Тогда
			ОписаниеОшибки = НСтр("ru='В переданных параметрах отсутствует путь к файлу с данными для загрузки.';uk='У переданих параметрах відсутній шлях до файлу з даними для завантаження.'");
			Рекомендации = НСтр("ru='В передаваемых параметрах должен быть элемент ПутьКФайлуЗагрузки.';uk='У переданих параметрах повинен бути елемент ПутьКФайлуЗагрузки.'");
		...
		Если ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
			Если Параметры.Свойство("КонфигурацияИсточник") И Не ПустаяСтрока(Параметры.КонфигурацияИсточник) Тогда
				ОписаниеОшибки = НСтр("ru='Загрузка данных из источника %1 не поддерживается.';uk='Завантаження даних з джерела %1 не підтримується.'");
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, Параметры.КонфигурацияИсточник);
				Рекомендации = НСтр("ru='Проверить правильность переданного параметра.';uk='Перевірити вірність переданого параметра.'");
			Иначе
				ОписаниеОшибки = НСтр("ru='В переданных параметрах отсутствует имя Конфигурации-источника.';uk='У переданих параметрах відсутнє ім''я Конфігурації-джерела.'");
				Рекомендации = НСтр("ru='В передаваемых параметрах должен быть элемент КонфигурацияИсточник.';uk='У переданих параметрах повинен бути елемент КонфигурацияИсточник.'");
			КонецЕсли;
			ЗафиксироватьОшибку(ЭтаФорма, ОписаниеОшибки, Рекомендации);
			Ошибка = Истина;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПустаяСтрока(Объект.КонфигурацияПриемник) Тогда
		...
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Данная обработка не предназначена для запуска в программе %1.';uk='Дана обробка не призначена для запуску в програмі %1.'"), МетаданныеСиноним));
...
Процедура ДанныеДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИспользуетсяКаталогПереноса Тогда
		...
		ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Выберите каталог с данными';uk='Виберіть каталог з даними'"));
	Иначе
		ПараметрыДиалога.Вставить("РежимДиалога", РежимДиалогаВыбораФайла.Открытие);
		ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Выберите путь к файлу с данными';uk='Виберіть шлях до файлу з даними'"));
		ПараметрыДиалога.Вставить("Фильтр",    НСтр("ru='ZIP-файл';uk='ZIP-файл'") + " (*.zip)|*.zip");
	КонецЕсли;
	...
	ТекстПредупреждения = НСтр("ru='Для данной операции необходимо установить расширение для веб-клиента BAF.';uk='Для цієї операції необхідно встановити розширення для веб-клієнта BAF.'");
...
Функция Подключаемый_СтраницаОжидание_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Элементы.ДекорацияНомерШагаСтраницаОжидание.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Шаг %1.';uk='Крок %1.'"), Строка(ПорядковыйНомерПерехода));
КонецФункции
...
Функция Подключаемый_СтраницаПошаговойЗагрузки_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Если ПорядковыйНомерПерехода >= 6 Тогда
		Элементы.ДекорацияЗаголовокПошаговаяЗагрузка.Заголовок = НСтр("ru='Шаг 6. Загрузка остатков.';uk='Крок 6. Завантаження залишків.'");
	ИначеЕсли ПорядковыйНомерПерехода >= 4 Тогда
		Элементы.ДекорацияЗаголовокПошаговаяЗагрузка.Заголовок = НСтр("ru='Шаг 4. Загрузка нормативно-справочной информации.';uk='Крок 4. Завантаження нормативно-довідкової інформації.'");
	Иначе
		Элементы.ДекорацияЗаголовокПошаговаяЗагрузка.Заголовок = НСтр("ru='Шаг 2. Загрузка настроек параметров учета.';uk='Крок 2. Завантаження настройок параметрів обліку.'");
	КонецЕсли;
...
Функция Подключаемый_СтраницаВыборДанных_ПриПереходеДалее(Отказ)
	...
	Если Не (ЗначениеЗаполнено(Объект.ИмяФайлаДанных) или ЗначениеЗаполнено(Объект.КаталогПереноса)) Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Необходимо указать данные для загрузки.';uk='Необхідно вказати дані для завантаження.'"), 30);	
	КонецЕсли;
...
Процедура ПеренумероватьЗаголовкиШагов()
	...
	Для Каждого СтрокаПереходов Из ТаблицаПереходов Цикл
		...
		Элементы[ИмяЭлементаДекорации].Заголовок = НСтр("ru='Шаг';uk='Крок'") + " " + Нумератор + ".";
	КонецЦикла;
...
Процедура ЗаписатьОшибкуРаботыПомощника(ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Помощник перехода с других конфигураций';uk='Помічник переходу з інших конфігурацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьОписаниеДоступныхКонфигурацийИсточника()
	...
	Если ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
		...
		Заголовок = НСтр("ru='Помощник переноса данных из других конфигураций ""BAF""';uk='Помічник перенесення даних з інших конфігурацій ""BAF""'");
		...
		Элементы.Декорация1_1.Заголовок = НСтр("ru='Помощник позволит вам загрузить данные из конфигураций';uk='Помічник дозволить вам завантажити дані з конфігурацій'");
	...
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Помощник переноса данных из ""%1""';uk='Помічник перенесення даних з ""%1""'"), СинонимИсточника);
			
		Элементы.Декорация1_1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Помощник позволит вам загрузить данные из конфигурации ""%1"".';uk='Помічник дозволить вам завантажити дані з конфігурації ""%1"".'"), СинонимИсточника);
...
Процедура ЗаполнитьДоступныеКонфигурацииИсточника()
	...
	Если Объект.КонфигурацияПриемник = "УП" Тогда
		ДоступныеКонфигурацииИсточника.Добавить("УПП", НСтр("ru='Управление производственным предприятием для Украины, редакция 1.3';uk='Управління виробничим підприємством для України, редакція 1.3'"));	
	КонецЕсли;
	Если Объект.КонфигурацияПриемник = "КА" Тогда
		ДоступныеКонфигурацииИсточника.Добавить("УТП",  НСтр("ru='Управление торговым предприятием для Украины, редакция 1.2';uk='Управління торговим підприємством для України, редакція 1.2'"));
	КонецЕсли;
	
	Если Объект.КонфигурацияПриемник = "УП"
		Или Объект.КонфигурацияПриемник = "КА"
		Или Объект.КонфигурацияПриемник = "УТ" Тогда
		//ДоступныеКонфигурацииИсточника.Добавить("УТ",  НСтр("ru='Управление торговлей для Украины, редакция 2.3';uk='Управління торгівлею для України, редакція 2.3'"));
	КонецЕсли;
...
Функция ИмяМакетаПравилПоКонфигурацииИсточника(КонфигурацияИсточника, ВидДанных)
	...
	Если ВидДанных = "ОД" Тогда
		Если КонфигурацияИсточника = "УПП" Тогда
			ИмяМакета = "ПравилаОбменаУПП";
		//ИначеЕсли КонфигурацияИсточника = "ЗУП21" Тогда
		//	ИмяМакета = "ПравилаОбменаЗУП";
		ИначеЕсли КонфигурацияИсточника = "УТП" Тогда
	...
	Для Каждого Этап Из СписокЭтапов Цикл
		...
		Если ИмяЭтапа = "Проверка" Тогда
			Если Не Объект.КонфигурацияИсточник = "УПП"
				И Не Объект.КонфигурацияИсточник = "УТП" Тогда
...
Процедура ОбработатьФайлПротокола(ФайлПротокола)
	...
	Иначе
		...
		СтрокиПоиска.Добавить(НСтр("ru='Начало выгрузки';uk='Початок вивантаження'"));
		СтрокиПоиска.Добавить(ВРег(НСтр("ru='Правило выгрузки данных';uk='Правило вивантаження даних'")));
		СтрокиПоиска.Добавить(ВРег(НСтр("ru='Выгрузка объекта';uk='Вивантаження об''єкта'")));
		СтрокиПоиска.Добавить(НСтр("ru='Конвертация объекта';uk='Конвертація об''єкта'"));
		СтрокиПоиска.Добавить(НСтр("ru='Ошибка';uk='Помилка'") + ".");
		СтрокиПоиска.Добавить(НСтр("ru='Окончание выгрузки';uk='Закінчення вивантаження'"));
		СтрокиПоиска.Добавить(НСтр("ru='Ошибка';uk='Помилка'") + "!!!");
		СтрокиПоиска.Добавить(НСтр("ru='ИмяПКО';uk='ИмяПКО'"));
		СтрокиПоиска.Добавить(НСтр("ru='ТипОбъекта';uk='ТипОбъекта'"));
		СтрокиПоиска.Добавить(НСтр("ru='КодСообщения';uk='КодСообщения'"));
		СтрокиПоиска.Добавить(НСтр("ru='Загрузка объекта';uk='Завантаження об''єкта'"));
...
Процедура ПомещениеФайлаВХранилищеНачать()
	...
	Исключение
		ТекстСообщения = НСтр("ru='При помещении файлов во временное хранилище произошла ошибка';uk='При приміщенні файлів у тимчасове сховище сталася помилка'")
...
Процедура ПомещениеФайлаВХранилищеЗавершить(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Не ПустаяСтрока(Адрес) Тогда
		...
		Иначе
			...
			ТекстПояснения = НСтр("ru='Проверьте корректность содержимого файла с данными';uk='Перевірте коректність вмісту файлу з даними'") + ".";
	...
	Иначе
		...
		ТекстПояснения = НСтр("ru='Укажите корректный путь к файлу данных';uk='Вкажіть правильний шлях до файлу даних'") + ".";
...
Процедура ЗагрузкаДанныхИзФайлаЗакончить(РезультатыВыполнения = Неопределено)
		...
		Иначе
			...
			Если ТекущийЭтап = "Загрузка"
				И (Объект.КонфигурацияИсточник = "УПП"   
					Или Объект.КонфигурацияИсточник = "УТП") Тогда
	...
	Если Не СостояниеТекущегоЭтапа() = "Ошибка" Тогда
		...
		Если ТаблицаОшибок.Количество() > 0 Тогда
			...
			Элементы.ГиперссылкаПротокол.Заголовок = НСтр("ru='Протокол загрузки данных';uk='Протокол завантаження даних'")
				+ " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='(сообщений: %1)';uk='(повідомлень: %1)'"), ТаблицаОшибок.Количество());
		Иначе
	...
	Иначе
		...
		Если ТаблицаОшибок.Количество() > 0 Тогда
			...
			Элементы.ГиперссылкаОшибки.Заголовок = НСтр("ru='Список комментариев и ошибок';uk='Список коментарів і помилок'")
			+ " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='(сообщений: %1)';uk='(повідомлень: %1)'"), ТаблицаОшибок.Количество());
		Иначе
...
Процедура ПроверитьАрхивСДанными(Отказ, ТекстСообщения)
	...
	Для Каждого ЭлементАрхива Из Архив.Элементы Цикл
			...
			Для Каждого КонфигурацияИсточника Из ДоступныеКонфигурацииИсточника Цикл
				...
				Если СтрНайти(ИмяФайла, ПодстрокаПоиска + "_ОД") = 1 
					...
					ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Загрузка основных данных из конфигурации %1';uk='Завантаження основних даних з конфігурації %1'"), КонфигурацияИсточника.Представление);
					Прервать;
				ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_ЗУП") = 1
					...
					ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Загрузка кадровых и зарплатных данных из конфигурации %1';uk='Завантаження кадрових і зарплатних даних з конфігурації %1'"), КонфигурацияИсточника.Представление);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
			...
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_Н") = 1
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Загрузка настроек учета из конфигурации %1';uk='Завантаження настройок обліку з конфігурації %1'"), КонфигурацияИсточника.Представление);
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_С") = 1
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Загрузка справочников из конфигурации %1';uk='Завантаження довідників з конфігурації %1'"), КонфигурацияИсточника.Представление);
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_О") = 1 
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Загрузка остатков из конфигурации %1';uk='Завантаження залишків з конфігурації %1'"), КонфигурацияИсточника.Представление);
			КонецЕсли;
		КонецЕсли;
		...
		Если Отказ Тогда
			ТекстСообщения = НСтр("ru='Ошибка формата файла. Выберите корректный файл с данными для загрузки.';uk='Помилка формату файлу. Виберіть коректний файл з даними для завантаження.'");
			Прервать;
		Иначе
...
Функция РезультатЗагрузкиДанныхВДлительнойОперации()
	...
	Если Не ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
		...
		НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Перенос данных из конфигурации ""%1""';uk='Перенесення даних з конфігурації ""%1""'"), КонфигурацияИсточника.Представление);
	Иначе
		НаименованиеЗадания = НСтр("ru='Перенос данных из другой конфигурации ""BAF""';uk='Перенесення даних з іншої конфігурації ""BAF""'");
	КонецЕсли;
...
Процедура ОбработатьПроверкуСуществованияФайлаТекущийЭтап(Существует, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ДополнительныеПараметры.ЭтоПереходДалее Тогда
		Если Элементы.ПанельОсновная.ТекущаяСтраница = Элементы.СтраницаВыборДанных Тогда
			ВыполнитьПереходПоТаблицеПереходов(Истина);
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Файл с данными не соответствует этапу переноса! Выгрузите данные из базы-источника.';uk='Файл з даними не відповідає етапу перенесення! Вивантажуйте дані з бази-джерела.'"), 30);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьПомещениеФайлаТекущийЭтап(Успешно, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Ошибка помещения файла текущего этапа';uk='Помилка приміщення файлу поточного етапу'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ОбновитьЗаголовокТипФайлаДанных(Шаг)
	...
	СтрокаЗаголовок = НСтр("ru='Файл данных не обнаружен в каталоге переноса.';uk='Файл даних не виявлено в каталозі перенесення.'");
	Если Шаг = 2 Тогда
		СтрокаЗаголовок = НСтр("ru='Файл данных с параметрами настроек учета.';uk='Файл даних з параметрами настройок обліку.'");	
	ИначеЕсли Шаг = 4 Тогда
		СтрокаЗаголовок = НСтр("ru='Файл данных с нормативно-справочной информацией.';uk='Файл даних з нормативно-довідковою інформацією.'");
	ИначеЕсли Шаг = 6 Тогда
		СтрокаЗаголовок = НСтр("ru='Файл данных с информацией об остатках.';uk='Файл даних з інформацією про залишки.'");
	КонецЕсли;
...
Процедура ОбработатьПроверкуСуществованияФайлаДанных(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Элементы.ДекорацияТипФайлаДанных.Заголовок     = НСтр("ru='Файл данных не обнаружен в каталоге переноса.';uk='Файл даних не виявлено в каталозі перенесення.'");
...
Процедура ОбработатьПолучениеРазмераФайла(Размер, ДополнительныеПараметры) Экспорт
	...
	Файл = ДополнительныеПараметры.Файл;
	Элементы.ДекорацияРазмерФайла.Заголовок = НСтр("ru='Размер файла';uk='Розмір файлу'") + ": " + Окр(Размер/1024,0) + " Кб.";
...
Процедура ОбработатьПолучениеВремениИзмененияФайла(ВремяИзменения, ДополнительныеПараметры) Экспорт
	...
	Элементы.ДекорацияДатаВыгрузкиДанных.Заголовок = НСтр("ru='Дата выгрузки';uk='Дата вивантаження'") + ": " + ВремяИзменения + ";";
...
Функция СоответствиеНаименованийКонфигураций()
  ...
  СоответствиеНаименованийКонфигураций.Вставить("УТП",       "UTP");
