...
Процедура ДействиеПриИзменении(Элемент)
	...
	Если Действие = "ПеренестиМеждуТомами"
		...
		ПоказатьПредупреждение(, НСтр("ru='Невозможно перенести файлы между томами: в настройках указан один том для размещения файлов.';uk='Неможливо перенести файли між томами: в настройках вказано один том для розміщення файлів.'"));
	КонецЕсли;
...
Процедура ВыполнитьПеренос(Команда)
	...
	Если Действие = "ПеренестиМеждуТомами"
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru='Не заполнен том, в который будут перенесены файлы.';uk='Не заповнений том, в який будуть перенесені файли.'"), , "ТомХраненияПриемник");
		Возврат;
	...
	Если ОшибкиПереноса.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Завершен перенос файлов. Перенесено файлов';uk='Завершено перенесення файлів. Перенесено файлів'")
...
Функция ВыполнитьПереносНаСервере()
	...
	Если Действие = "ПеренестиМеждуТомами" Тогда
		ДействиеДляЖурнала = НСтр("ru='между томами на диске.';uk='між томами на диску.'");
	ИначеЕсли Действие = "ПеренестиВТома" Тогда
		ДействиеДляЖурнала = НСтр("ru='в тома на диске.';uk='в тому на диску.'")
	Иначе
		ДействиеДляЖурнала = НСтр("ru='в информационную базу.';uk='в інформаційну базу.'")
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Начало переноса файлов.';uk='Файли.Початок перенесення файлів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru='Начат перенос файлов';uk='Розпочато перенесення файлів'") + " " + ДействиеДляЖурнала);
	...
	ТекстЗаписиОЗавершении = НСтр("ru='Завершен перенос файлов %1
        |Перенесено файлов: %2'
        |;uk='Завершено перенесення файлів %1
        |Перенесено файлів: %2'");
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Завершение переноса файлов.';uk='Файли.Завершення перенесення файлів.'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПеренестиФайл(Файл, СвойстваТома, ОшибкиПереноса, ПеренесеноФайлов)
	...
	Попытка
		...
		Если ПереноситьВТом
			...
			ВызватьИсключение НСтр("ru='""Превышен максимальный размер тома"".';uk='""Перевищено максимальний розмір тому"".'");
		КонецЕсли;
		...
		Если Действие = "ПеренестиМеждуТомами" Тогда
			...
			Если Не ФайлНаДиске.Существует() Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Файл ""%1"" не найден.';uk='Файл ""%1"" не знайдено.'"), ТекущийПутьКФайлу);
			КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Ошибка переноса файла.';uk='Файли.Помилка перенесення файлу.'",ОбщегоНазначения.КодОсновногоЯзыка()),
