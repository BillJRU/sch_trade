...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка данных из файла в табличную часть не поддерживается для объектов типа: %1';uk='Завантаження даних з файлу в табличну частина не підтримується для об''єктів типу: %1'"),
	...
	Для каждого СтрокаТаблицы Из СопоставленныеДанные Цикл
		...
		Исключение
			...
			НСтр("ru='Не удалось записать элемент справочника %1 по причине:
                |%2'
                |;uk='Не вдалося записати елемент довідника %1 з причини:
                |%2'"), 
...
Функция СобытиеЖурналаРегистрации() 
	...
	Возврат НСтр("ru='Загрузка данных из файла';uk='Завантаження даних з файлу'",ОбщегоНазначения.КодОсновногоЯзыка());
