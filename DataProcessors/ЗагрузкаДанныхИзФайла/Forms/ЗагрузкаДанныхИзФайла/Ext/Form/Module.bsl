...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = НСтр("ru='Введенные данные не будут записаны.';uk='Введені дані не будуть записані.'");
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru='Отменить сопоставление?';uk='Скасувати співставлення?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ШагНазад()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных
		...
		Элементы.Далее.Заголовок = ?(ТипЗагрузки = "ВставкаИзБуфераОбмена",
				НСтр("ru='Вставить в список';uk='Вставити в список'"), НСтр("ru='Далее >';uk='Далі >'"));
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ru='Строки, в которых не заполнены обязательные колонки, будут пропущены.';uk='Рядки, у яких не заповнені обов''язкові колонки, будуть пропущені.'")
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Выполнено сопоставление';uk='Виконано співставлення'"),, НСтр("ru='Сопоставлено элементов:';uk='Зіставлено елементів:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Элементы.ОбновлятьСуществующие.Заголовок       = НСтр("ru='Сопоставленные элементы (';uk='Зіставлені елементи ('") + Строка(Статистика.Сопоставленных) + ")";
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	Если ТипЗнч(ОписаниеСтрокиВыбора) = Тип("Структура") И ОписаниеСтрокиВыбора.Свойство("Представление") Тогда
		ЭтотОбъект.Заголовок = НСтр("ru='Загрузка данных в справочник';uk='Завантаження даних в довідник'") + " """ +ОписаниеСтрокиВыбора.Представление + """";
	Иначе
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			ПоказатьПредупреждение(, (НСтр("ru='Для вставки сопоставленных данных в список, необходимо заполнить текстовое поле.';uk='Для вставлення зіставлених даних до списку, необхідно заповнити текстове поле.'")));
			Возврат;
		КонецЕсли;
		
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ru='Для сопоставления и загрузки данных, необходимо заполнить таблицу.';uk='Для співставлення і завантаження даних, необхідно заповнити таблицю.'")));
		Иначе
            ...
            |;uk='Неможливо виконати співставлення даних, оскільки дані не були завантажені в таблицю. 
            |Можливо, імена колонок у файлі не відповідають колонкам в бланку.'")));
		КонецЕсли;
		ВыполнитьШагЗаполнениеТаблицыДаннымиНаКлиенте();
...
Процедура ВывестиСообщениеОбОшибке(ТекстОшибкиДляПользователя, ТехническаяИнформация)
	ТекстСообщенияОбОшибке = ТекстОшибкиДляПользователя + Символы.ПС
		+ НСтр("ru='Возможная причина: Загружаемые данные некорректные.
                    ...
                    |;uk='Можлива причина: Завантажуються некоректні дані.
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, НСтр("ru='Загрузить данные >';uk='Завантажити дані >'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';uk='Підсистема ЗагрузкаДанныхИзФайла: Виконання серверного методу завантаження даних з файлу'");
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СопоставитьЗагружаемыеДанные();
	Иначе
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести загрузку данных.';uk='Не вдалося провести завантаження даних.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Заполнение таблицы сопоставления загруженными данными из файла.';uk='Заповнення таблиці співставлення даними, що завантажені з файлу.'");
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести сопоставление данных.';uk='Не вдалося провести співставлення даних.'"),
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Запись загруженных данных из файла';uk='Запис завантажених даних з файлу'");
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести запись данных.';uk='Не вдалося виконати запис даних.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Создание отчета о загрузке данных из файла';uk='Створення звіту про завантаження даних з файлу'");
