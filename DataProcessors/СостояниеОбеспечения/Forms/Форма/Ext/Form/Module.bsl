...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗаказыНаТовар.Заголовок = НСтр("ru='Другие заказы выделенной позиции';uk='Інші замовлення виділеної позиції'");
...
Функция РекомендацияПоИндексуКартинки(Индекс, СтрокаТовары)
	...
	Если Индекс = 1 Тогда
		...
		Рекомендация = НСтр("ru='Резервировать позже';uk='Резервувати пізніше'");
		
	ИначеЕсли Индекс = 2 И СтрокаТовары.Заказано Тогда
		
		Рекомендация = НСтр("ru='Резервировать раньше';uk='Резервувати раніше'");
		
	ИначеЕсли Индекс = 2 И Не Заказано И ЕстьПолноеОбеспечениеПоВсемСкладам Тогда
		
		Рекомендация = НСтр("ru='Запланировать перемещение';uk='Запланувати переміщення'");
		
	ИначеЕсли Индекс = 2 И Не Заказано Тогда
		
		Рекомендация = НСтр("ru='Резервировать';uk='Резервувати'");
		
	ИначеЕсли Индекс = 3 Тогда
		
		Рекомендация = НСтр("ru='Исправить обеспечение';uk='Виправити забезпечення'");
...
Процедура ВыполнитьОбеспечениеЗаказа(Заказ, ИндексыСтрок, ПереченьВариантов, Форма, ОтгружатьЧастями)
	...
	Исключение
		...
		Сообщение.Сообщить();
		ИмяСобытияЖурнала = НСтр("ru='Состояние обеспечения заказов.Выполнение рекомендаций';uk='Стан забезпечення заказов. Виконання рекомендацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура НастроитьЭлементыФормыПоПараметрам()
	...
	Если ЭтоСостояниеОбеспеченияЗаказов Тогда
		...
		Элементы.Обновить.Заголовок             = НСтр("ru='Сформировать';uk='Сформувати'");
