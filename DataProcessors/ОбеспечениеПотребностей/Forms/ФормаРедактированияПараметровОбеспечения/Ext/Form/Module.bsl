...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимОткрытия = "Полный" ИЛИ Параметры.РежимОткрытия = "ТолькоСпособ" 
		...
		Если Параметры.КлючПотребности.Свойство("Группировка") Тогда
			Элементы.КлючПотребностиСтрокой.Заголовок = 
				СтрШаблон(НСтр("ru='Группировка: %1.';uk='Групування: %1.'"),Параметры.КлючПотребности.Группировка);
		Иначе
			ТекстХарактеристика = ?(ЗначениеЗаполнено(Параметры.КлючПотребности.Характеристика),
				СтрШаблон(НСтр("ru=', Характеристика: %1';uk=', Характеристика: %1'"), Параметры.КлючПотребности.Характеристика), "");
			ТекстСклад = ?(Параметры.КлючПотребности.ЭтоРабота, "",
				СтрШаблон(НСтр("ru=', Склад: %1';uk=', Склад: %1'"), Параметры.КлючПотребности.Склад));
			
			Элементы.КлючПотребностиСтрокой.Заголовок = СтрШаблон(НСтр("ru='Номенклатура: %1%2%3.';uk='Номенклатура: %1%2%3.'"),
		...
		Если Параметры.РежимОткрытия <> "ТолькоПараметры" Тогда
			Элементы.ПереключательСпособаУнаследованный.СписокВыбора[0].Представление =
				?(Не ЗначениеЗаполнено(СпособУнаследованный),
					...
					СтрШаблон(НСтр("ru='""%1"" (настроенный для схемы обеспечения номенклатуры)';uk='""%1"" (налаштований для схеми забезпечення номенклатури)'"), СпособУнаследованный));
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли Параметры.РежимОткрытия = "ТолькоПараметры" Тогда
		...
		Заголовок = НСтр("ru='Параметры поддержания запаса';uk='Параметри підтримання запасу'");
...
Процедура Ок(Команда)
	...
	Если НЕ ЗначениеЗаполнено(МетодОбеспечения) ИЛИ
		...
		ТекстСообщения = НСтр("ru='Не указан метод обеспечения потребностей';uk='Не вказаний метод забезпечення потреб'");
...
Функция ПолучитьСтатистикуПотребленияВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Расчет статистики потребления';uk='Розрахунок статистики споживання'");
...
Процедура РассчитатьЗапасы()
	...
	ИначеЕсли МетодОбеспечения = ПредопределенноеЗначение("Перечисление.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаНаСрокПоСтатистике") Тогда
		...
		СреднедневноеПотреблениеСтрокой =
			?(ЗначениеЗаполнено(ДлительнаяОперация), НСтр("ru='статистика рассчитывается...';uk='статистика розраховується ...'"),
