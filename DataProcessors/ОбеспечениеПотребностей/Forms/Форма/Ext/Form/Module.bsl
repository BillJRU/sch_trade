...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВесЗаказа.Заголовок = СтрШаблон(НСтр("ru='Вес (%1)';uk='Вага (%1)'"), ЕдиницаИзмеренияВеса);
	
	ВалютаУправленческогоУчета = Константы.ВалютаУправленческогоУчета.Получить();
	Элементы.СуммаЗаказа.Заголовок = СтрШаблон(НСтр("ru='Сумма (%1)';uk='Сума (%1)'"), ВалютаУправленческогоУчета);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Помощник.ТекущаяСтраница = Элементы.ШагОтборПотребностей И СохранитьНастройки Тогда
		Если ЗавершениеРаботы Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("ru='Отбор потребностей был изменен. Все изменения будут потеряны.';uk='Відбір потреб був змінений. Всі зміни будуть втрачені.'");
		Иначе
			СохранитьНастройки();
		КонецЕсли;
		Возврат;
	...
	Если ЗавершениеРаботы 
		...
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru='В списке потребностей имеются отмеченные позиции, заказы по которым не сформированы.';uk='У списку потреб є зазначені позиції, замовлення за якими не сформовані.'");
		Возврат;
...
|;uk='При продовженні закриття не буде виконано розміщення замовлень на відвантаження 
|в сформованих замовленнях на поставку.'");
		Возврат;
...
Процедура Назад(Команда)
	...
	Если Элементы.Помощник.ТекущаяСтраница = Элементы.ШагСформированныеЗаказы Тогда
		...
		Если ТаблицаДокументы.Количество() > 0 Тогда
			...
			Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Пометить на удаление сформированные заказы';uk='Помітити на вилучення сформовані замовлення'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена);
			
			ТекстВопроса = НСтр("ru='При переходе назад сформированные заказы будут помечены на удаление.';uk='При переході назад сформовані замовлення будуть позначені на вилучення.'");
...
Процедура Далее(Команда)
	...
	Если Не ИспользоватьЗаказыПоставщикам И ВКорзинеЗаказовЕстьТипОбеспеченияПокупка() Тогда
		СообщитьобОшибкеНезаполненногоСпособаОбеспечения();
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДатуПоставки(Команда)
	...
	ПоказатьВводДаты(ОписаниеОповещения, ДатаПоставки, НСтр("ru='Введите дату поставки';uk='Введіть дату поставки'"), ЧастиДаты.Дата);
...
Процедура ЗаказПоставщикуПодтвержден(Команда)
	...
	Если ЗначениеЗаполнено(МассивСсылок) Тогда
		...
		ОбновитьНадписьПредупреждениеОРазмещенииЗаказов();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно установить статус. Не выбраны заказы поставщику';uk='Неможливо встановити статус. Не вибрані замовлення постачальнику'"));
	КонецЕсли;
...
Функция ПолучениеТаблицыСпособовОбеспеченияВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование заказов: получение таблицы способов обеспечения';uk='Формування замовлень: отримання таблиці способів забезпечення'");
...
Функция ПолучениеТаблицыЗапасовВФоне()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование заказов: получение таблицы запасов';uk='Формування замовлень: отримання таблиці запасів'");
...
Функция ПолучениеТаблицыЗаказовКОбеспечениюВФоне()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование заказов: получение таблицы заказов к обеспечению';uk='Формування замовлень: отримання таблиці замовлень до забезпечення'");
...
Функция ФормированиеЗаказовВФоне()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование заказов: создание документов';uk='Формування замовлень: створення документів'");
...
Функция УдалениеЗаказовВФоне()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование заказов: удаление документов';uk='Формування замовлень: вилучення документів'");
...
Процедура ЗаполнитьСлужебныеРеквизитыТаблицыСпособов(СтрокаТаблицы)
	...
	ИначеЕсли СтрокаТаблицы.ДнейДоПлановогоЗаказа >= 0 Тогда
		...
		СтрокаТаблицы.ДнейДоЗаказаПоГрафикуПредставление =
			СтрШаблон(НСтр("ru='%1 дн';uk='%1 дн'"), СтрокаТаблицы.ДнейДоПлановогоЗаказа);
	Иначе
		
		СтрокаТаблицы.ДнейДоЗаказаПоГрафикуПредставление =
			СтрШаблон(НСтр("ru='просрочен на %1 дн';uk='прострочений на %1 дн'"), - СтрокаТаблицы.ДнейДоПлановогоЗаказа);
...
Процедура ЗапасыПриАктивизацииСтрокиОбработчикОжидания()
	...
	Иначе
		...
		ДоДатыЗаказаДней = СтрШаблон(НСтр("ru='До даты заказа %1 дн.';uk='До дати замовлення %1 дн.'"), Формат(КоличествоДней, "ЧДЦ="));
		...
		Если Строка.ФормироватьПлановыеЗаказы И КоличествоДней <= 0 Тогда
			...
			Иначе
				ТекстРекомендации = НСтр("ru='Заказать количество для пополнения до максимального запаса, пополнить страховой запас (при необходимости).';uk='Замовити кількість для поповнення до максимального запасу, поповнити страховий запас (при необхідності).'");
			КонецЕсли;
		...
		ИначеЕсли Строка.ФормироватьПлановыеЗаказы И Не МетодМинМакс Тогда
			...
			Если Строка.СтраховойЗапас = 0 Или Строка.ДоТочкиЗаказа > 0 Тогда
				ТекстРекомендации = НСтр("ru='Остаток товара на складе достиг минимального запаса. Необходимо пополнить запас до максимального.';uk='Залишок товару на складі досяг мінімального запасу. Необхідно поповнити запас до максимального.'");
			Иначе
				ТекстРекомендации = НСтр("ru='Остаток товара на складе достиг минимального запаса. Необходимо пополнить запас до максимального, пополнить страховой запас (при необходимости).';uk='Залишок товару на складі досяг мінімального запасу. Необхідно поповнити запас до максимального, поповнити страховий запас (при необхідності).'");
			КонецЕсли;
		...
		ИначеЕсли Строка.ЗаказыКПоступлению = 0 Тогда
			...
			ИначеЕсли Строка.СтраховойЗапас = 0 Тогда
				ТекстРекомендации = НСтр("ru='Заказать количество, равное максимальному запасу за вычетом страхового запаса.';uk='Замовити кількість, рівну максимальному запасу за вирахуванням страхового запасу.'");
			Иначе
				ТекстРекомендации = НСтр("ru='Заказать количество, равное максимальному запасу за вычетом страхового запаса, пополнить страховой запас (при необходимости).';uk='Замовити кількість, рівну максимальному запасу за вирахуванням страхового запасу, поповнити страховий запас (при необхідності).'");
			КонецЕсли;
		...
		ИначеЕсли Строка.ЗаказыКПоступлению < Строка.МаксимальныйЗапас Тогда
			...
			ИначеЕсли Строка.СтраховойЗапас = 0 Тогда
				ТекстРекомендации = НСтр("ru='Заказать количество, равное максимальному запасу за вычетом страхового запаса и за вычетом заказанного ранее.';uk='Замовити кількість, рівну максимальному запасу за вирахуванням страхового запасу і за вирахуванням замовленого раніше.'");
			Иначе
				ТекстРекомендации = НСтр("ru='Заказать количество, равное максимальному запасу, за вычетом страхового запаса и за вычетом заказанного ранее, пополнить страховой запас (при необходимости).';uk='Замовити кількість, рівну максимальному запасу, за вирахуванням страхового запасу і за вирахуванням замовленого раніше, поповнити страховий запас (при необхідності).'");
			КонецЕсли;
...
Функция ДалееКШагуСформированныеЗаказы(Результат)
	...
	Если ЗначениеЗаполнено(РезультатаФормированияЗаказов) Тогда // Задание прервалось до того, как было что-то записано
		...
		Если Результат = Неопределено Или Результат.Статус <> "Выполнено" Тогда
			...
			Сообщение.Текст = НСтр("ru='Заказы были сформированы не полностью';uk='Замовлення були сформовані в повному обсязі'");
			Сообщение.Сообщить();
		КонецЕсли;
...
|або розраховане за статистикою'"));
	...
	Подсказки.Вставить("Остаток", НСтр("ru='Текущий остаток товара на складе';uk='Поточний залишок товару на складі'"));
...
&НаКлиенте
Функция ВКорзинеЗаказовЕстьТипОбеспеченияПокупка()
	
	Строки = КорзинаЗаказов.ПолучитьЭлементы();
	Для Каждого Строка Из Строки Цикл
		
		Если Строка.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Покупка") Тогда
			 Возврат Истина;
		КонецЕсли;
...
&НаСервере
Процедура СообщитьобОшибкеНезаполненногоСпособаОбеспечения()
	...
	Если Элементы.СтраницыТаблицаПотребностей.ТекущаяСтраница = Элементы.СтраницаТаблицаЗапасы Тогда
		...
		ИмяТаблицы = "Запасы";
		ЭлементФормы = Элементы.СпособОбеспеченияЗапасы;
		ШаблонТекста = НСтр("ru='Не заполнена колонка ""Способ обеспечения"" в строке %НомерСтроки% списка ""Поддержание запасов""';uk='Не заповнена колонка ""Спосіб забезпечення"" в рядку %НомерСтроки% списку ""Підтримання запасів""'");
	...
	ИначеЕсли Элементы.СтраницыТаблицаПотребностей.ТекущаяСтраница = Элементы.СтраницаТаблицаЗаказы Тогда
		...
		ЭлементФормы = Элементы.СпособОбеспеченияЗапасы;
		ШаблонТекста = НСтр("ru='Не заполнена колонка ""Способ обеспечения"" в строке %НомерСтроки% списка ""Обеспечение заказов""';uk='Не заповнена колонка ""Спосіб забезпечення"" в рядку %НомерСтроки% списку ""Забезпечення замовлень""'");
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если Строка.Отметка И Не ЗначениеЗаполнено(Строка.СпособОбеспечения) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Неопределено, "Объект." + ПутьКТабЧасти);
...
Процедура ЗаполнитьИсточникОбеспечения(МассивТиповОбеспечения)
	...
	Если ПараметрыФормы.ТипОбеспечения <> Неопределено Тогда
		ОткрытьФорму("Обработка.ОбеспечениеПотребностей.Форма.ФормаВыбораИсточникаОбеспечения", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор);
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Не выделено ни одной строки с доступным для изменения источником обеспечения. Выполнение команды невозможно.';uk='Не виділено жодного рядка з доступним для зміни джерелом забезпечення. Виконання команди неможливе.'"));
	КонецЕсли;
