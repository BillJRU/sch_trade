...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипОбеспечения = Перечисления.ТипыОбеспечения.Покупка
		...
		Если Параметры.ГрупповоеЗаполнение Тогда
			...
			Заголовок = НСтр("ru='Выбор поставщика';uk='Вибір постачальника'");
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
				...
				ШаблонТекста = НСтр("ru='Выбор поставщика (Номенклатура: %1, Характеристика: %2, Склад: %3)';uk='Вибір постачальника (Номенклатура: %1, Характеристика: %2, Склад: %3)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Характеристика, Параметры.Склад);
				
			Иначе
				
				ШаблонТекста = НСтр("ru='Выбор поставщика (Номенклатура: %1, Склад: %2)';uk='Вибір постачальника (Номенклатура: %1, Склад: %2)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Склад);
				
			КонецЕсли;
			
			ВалютаУпрУчета = Константы.ВалютаУправленческогоУчета.Получить();
			Элементы.ПоставщикиЦенаУпрУчет.Заголовок = СтрШаблон(НСтр("ru='Цена (%1)';uk='Ціна (%1)'"), ВалютаУпрУчета);
	...
	ИначеЕсли Параметры.ТипОбеспечения = Перечисления.ТипыОбеспечения.Перемещение Тогда
		...
		Если Параметры.ГрупповоеЗаполнение Тогда
			...
			Заголовок = НСтр("ru='Выбор склада';uk='Вибір складу'");
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
				...
				ШаблонТекста = НСтр("ru='Выбор склада (Номенклатура: %1, Характеристика: %2)';uk='Вибір складу (Номенклатура: %1, Характеристика: %2)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Характеристика);
				
			Иначе
				
				Заголовок = СтрШаблон(НСтр("ru='Выбор склада (Номенклатура: %1)';uk='Вибір складу (Номенклатура: %1)'"), Параметры.Номенклатура);
				
			КонецЕсли;
			
			ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Номенклатура, "ЕдиницаИзмерения");
			Элементы.СкладыДоступно.Заголовок = СтрШаблон(НСтр("ru='Доступно (%1)';uk='Доступно (%1)'"), ЕдиницаИзмерения);
