	...
	Для Каждого Документ Из ДокументыОбъекты Цикл
		...
		Если Не Ошибка Тогда
			...
			Исключение
				...
				ИмяСобытияЖурнала = НСтр("ru='Формирование заказов: создание документов';uk='Формування замовлень: створення документів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УдалитьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ИмяСобытияЖурнала = НСтр("ru='Формирование заказов: установка пометки удаления документов';uk='Формування замовлень: встановлення позначки вилучення документів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВременнаяТаблицаСпособовОбеспечения(ТоварыПоддерживаемогоЗапаса)
	...
	ТекстЗапроса =
		...
		|	Т.Склад                                                КАК Склад,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК Подразделение,
		|	Т.ПотребностьОстаток                                   КАК Количество
		|
		...
		|ИЗ
		|	(
		|" + ?(Не ТоварыПоддерживаемогоЗапаса, "",
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|		Т.Номенклатура                   КАК Номенклатура,
		|		Т.Склад                          КАК Склад,
		|		СпрСпособ.Ссылка                 КАК СпособОбеспечения
...
Функция ВыполнитьОбеспечениеЗаказа(Заказ, ИндексыСтрок, ПереченьВариантов)
	...
	Исключение
		...
		Сообщение.Сообщить();
		ИмяСобытияЖурнала = НСтр("ru='Обеспечение потребностей.Размещение в заказах';uk='Забезпечення потреб. Розміщення в замовленнях'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЕстьОшибки = Истина;
		Возврат ЕстьОшибки;
	КонецПопытки;
