...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если ИспользоватьУправлениеДоставкой Тогда
		РеквизитыДоставки = НСтр("ru='Способ доставки';uk='Спосіб доставки'") + ": <EM>"
		...
		Если ЗначениеЗаполнено(ВремяДоставкиС)
			...
			РеквизитыДоставки = РеквизитыДоставки
				+ НСтр("ru='Желаемое время доставки';uk='Бажаний час доставки'") +": <EM>"
				+ ?(ЗначениеЗаполнено(ВремяДоставкиС), НСтр("ru='c';uk='c'") + " " + Формат(ВремяДоставкиС,"ДФ=HH:mm; ДЛФ=T") + " ","")
				+ ?(ЗначениеЗаполнено(ВремяДоставкиПо),НСтр("ru='до';uk='до'") + " " + Формат(ВремяДоставкиПо,"ДФ=HH:mm; ДЛФ=T"),"")
				+ "</EM>" + "<BR>";
		КонецЕсли;
	Иначе
		РеквизитыДоставки = "";
	...
	Если Элементы.СтраницыОсновные.ТекущаяСтраница = Элементы.СтраницаДополнительныеРеквизиты Тогда
		Если Команда.Имя = "Далее" Тогда
			...
			ИначеЕсли (СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.ДоКлиента")
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен адрес доставки.';uk='Не заповнена адреса доставки.'"),,"АдресДоставки");
				Отказ = Истина;
			ИначеЕсли СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчика") Тогда
				...
				Если Не ЗначениеЗаполнено(ПеревозчикПартнер) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран перевозчик.';uk='Не вибраний перевізник.'"),,"ПеревозчикПартнер");
					Отказ = Истина;
				КонецЕсли;
			КонецЕсли;
