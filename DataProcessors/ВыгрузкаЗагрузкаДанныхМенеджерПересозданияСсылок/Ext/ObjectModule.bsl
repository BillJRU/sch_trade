...
Процедура ВыполнитьПересозданиеСсылок() Экспорт
	...
	Для Каждого ФайлПересоздаваемыхСсылок Из ФайлыПересоздаваемыхСсылок Цикл
		...
		ИсходныеСсылки = ВыгрузкаЗагрузкаДанных.ПрочитатьОбъектИзФайла(ФайлПересоздаваемыхСсылок); // Массив Из ЛюбаяСсылка
		
		Для Каждого ИсходнаяСсылка Из ИсходныеСсылки Цикл
			
			ИмяТипаXML = ВыгрузкаЗагрузкаДанныхСлужебный.XMLТипСсылки(ИсходнаяСсылка);
			МетаданныеСсылки = ИсходнаяСсылка.Метаданные(); 
			НоваяСсылка = ИнтеграцияПодсистемБТС.МенеджерОбъектаПоПолномуИмени(МетаданныеСсылки.ПолноеИмя()).ПолучитьСсылку();
