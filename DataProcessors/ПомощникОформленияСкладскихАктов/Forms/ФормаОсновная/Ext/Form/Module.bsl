...
|;uk='Процес оформлення складських актів не завершений. 
|Робота помічника буде перервана.'");
	...
	Иначе
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru='Работа помощника оформления складских актов будет прервана.';uk='Робота помічника оформлення складських актів буде перервана.'");
...
Процедура ОтобратьПоТипуОперацииНачалоВыбораЗавершение(Знач ТипыОпераций, Знач ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекстСписание		= НСтр("ru='Списание';uk='Списання'");
		ТекстОприходование	= НСтр("ru='Оприходование';uk='Оприбуткування'");
		ТекстПорча			= НСтр("ru='Порча';uk='Псування'");
		ТекстПересортица	= НСтр("ru='Пересортица';uk='Пересортиця'");
...
Процедура ЗачестьПоПересортицеПоПорчеЗавершение(Знач ВведенноеЧисло, Знач Параметры) Экспорт
	...
	ИначеЕсли ВведенноеЧисло <= 0
		...
		Если ВведенноеЧисло > Параметры.МаксимальноеЗначение Тогда
			ТекстСообщения = НСтр("ru='Указываемое количество не может превышать максимальное, зачитываемое количество.';uk='Вказана кількість не може перевищувати максимальну, зачитувану кількість.'");
		ИначеЕсли ВведенноеЧисло = 0 Тогда
			ТекстСообщения = НСтр("ru='Указываемое количество не может быть равно нулю.';uk='Вказана кількість не може дорівнювати нулю.'");
		Иначе
			ТекстСообщения = НСтр("ru='Указываемое количество не может быть отрицательным.';uk='Вказана кількість не може бути від''ємною.'");
		КонецЕсли;
...
Процедура РаспределитьПоВладельцамЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Массив")
		...
		ТекстСообщения = НСтр("ru='Не выбрано организаций для распределения.';uk='Не вибрано організацій для розподілу.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
	...
	Если СозданоСтрок = 0 Тогда
		ТекстСообщения = НСтр("ru='Не удалось провести автоматическое распределение по организациям. Возможно у выбранных организаций нет остатков по выбранным товарам.';uk='Не вдалося провести автоматичний розподіл по організаціям. Можливо у вибраних організацій немає залишків за обраними товарами.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
	КонецЕсли;
...
Функция СоздатьСписаниеНедостачТоваровСервер(Знач Организация, Знач ДатаДокумента, Знач СтрокиРаспределения)
	...
	ДокументОбъект.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность;
...
Функция СоздатьПорчаТоваровСервер(Знач Организация, Знач ДатаДокумента, Знач СтрокиРаспределения)
	...
	ДанныеЗаполнения = СтруктураДанныхЗаполнения(Организация, ДатаДокумента);
	ДокументОбъект.Заполнить(ДанныеЗаполнения);
	ДокументОбъект.НалоговоеНазначение = Справочники.Организации.НалоговоеНазначениеНДС(Организация, ДатаДокумента);
...
Процедура ЗаписатьСкладскойАктСервер(СкладскойАкт)
	...
	Если СкладскойАкт.ПроверитьЗаполнение() Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось провести документ ""%Документ%"".';uk='Не вдалося провести документ ""%Документ%"".'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Оформление складских актов';uk='Оформлення складських актів'",КодОсновногоЯзыка),
