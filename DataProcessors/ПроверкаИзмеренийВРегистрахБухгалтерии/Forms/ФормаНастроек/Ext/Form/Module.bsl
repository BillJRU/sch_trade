...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЕстьВыбранные Тогда
		...
		Сообщение.Текст = НСтр("ru='""Не выбрано ни одного регистра""';uk='""Не вибрано жодного регістра""'");
	...
	Если Исправлять И ПустаяСтрока(ВременныеДанные) Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='""Для исправления требуется указать эталонные данные""';uk='""Для виправлення потрібно вказати еталонні дані""'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура УстановитьСостояние(ДанныеЗагружены)
	...
	Если ДанныеЗагружены Тогда
		Элементы.ДекорацияЗагрузить.Заголовок = НСтр("ru='Данные уже загружены, загрузить data_dump.zip';uk='Дані вже завантажені, завантажити data_dump.zip'");
	Иначе
		Элементы.ДекорацияЗагрузить.Заголовок = НСтр("ru='Загрузить data_dump.zip';uk='Завантажити data_dump.zip'");
	КонецЕсли;
...
Функция ПолучитьДанныеРегистровБухгалтерии(ИмяВременногоКаталога, ИмяФайлаАрхива, ИмяКонфигурации, ВерсияКонфигурации)
	...
	Если ЭлементZipФайла = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Неправильный формат файла, не найден файл DumpInfo.xml.';uk='Неправильний формат файлу, не знайдений файл DumpInfo.xml.'");		
	КонецЕсли;
	...
	Если ЭлементZipФайла = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Неправильный формат файла, не найден файл PackageContents.xml.';uk='Неправильний формат файлу, не знайдений файл PackageContents.xml.'");		
	КонецЕсли;
	ЧтениеZipФайла.Извлечь(ЭлементZipФайла , ИмяВременногоКаталога, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
	...
	Для Каждого Файл Из КонтентXDTO.File Цикл
		...
		Если Файл.Type = "InfobaseData" И СтрНачинаетсяС(Файл.DataType, "РегистрБухгалтерии.") Тогда
			...
			Если ЭлементZipФайла = Неопределено Тогда
				ВызватьИсключение НСтр("ru='Неправильный формат файла, не найден файл регистра бухгалтерии.';uk='Неправильний формат файлу, не знайдений файл регістра бухгалтерії.'");		
			КонецЕсли;
			ЧтениеZipФайла.Извлечь(ЭлементZipФайла , ИмяВременногоКаталога, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
...
Процедура ПолучитьДанныеРегистровБухгалтерииНаСервере(Адрес)
	...
	Если ДанныеРегистров.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Файлы регистров бухгалтерии не найдены.';uk='Файли регістрів бухгалтерії не знайдені.'");
		Сообщение.Сообщить();
	Иначе	
