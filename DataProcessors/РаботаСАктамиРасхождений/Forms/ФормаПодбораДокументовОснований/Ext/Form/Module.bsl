...
Процедура ЗаполнитьПериоды()
	...
	Если Не ДокументыОснования.Количество() = 0 Тогда
		...
		ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаТоваровХранителю Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru='Не известный тип документа-основания.""';uk='Не відомий тип документа-підстави. ""'");
...
Процедура ЗаполнитьТаблицуДокументовОснований()
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(ТипОснованияАктаОРасхождении) Тогда
		...
		|	И ПоступлениеОтХранителя.ЦенаВключаетНДС = &ЦенаВключаетНДС
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	Если НЕ ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
 		ИнформационнаяНадписьОтборы = НСтр("ru='Отбор по: %Организация%, %Партнер%,%Контрагент% %Валюта%, %Соглашение%, %Договор%%ХозяйственнаяОперация%, %ЦенаВключаетНДС%';uk='Відбір за: %Организация%, %Партнер%,%Контрагент% %Валюта%, %Соглашение%, %Договор%%ХозяйственнаяОперация%, %ЦенаВключаетНДС%'");
	Иначе
