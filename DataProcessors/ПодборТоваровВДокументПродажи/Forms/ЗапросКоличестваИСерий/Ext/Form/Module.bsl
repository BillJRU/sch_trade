...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначенияПолейСвязиСтруктура = Новый Структура;
	ЗначенияПолейСвязиСтруктура.Вставить("Номенклатура", ТекущаяНоменклатура);
	ЗначенияПолейСвязиСтруктура.Вставить("Характеристика", ТекущаяХарактеристика);
	ЗначенияПолейСвязиСтруктура.Вставить("Склад", ТекущийСклад);
	...
	НастройкиИспользованияСерий = Новый ФиксированнаяСтруктура(
		Справочники.ВидыНоменклатуры.НастройкиИспользованияСерий(ТекущийВидНоменклатуры, ПолитикаУчетаСерий, ЗначенияПолейСвязи));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Любое';uk= 'Будь-яка'"));
...
Процедура ДополнитьТаблицуДоступныхДействий()
	...
	Для Каждого Строка Из ТаблицаДоступныхДействий Цикл
		...
		Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
			...
			Строка.Действие = СтрШаблон(НСтр("ru='Резервировать к %1';uk='Резервувати до %1'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru='К обеспечению (срок %1)';uk='До забезпечення (строк %1)'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru='Обеспечивать обособленно (срок %1)';uk='Забезпечувати відокремлено (строк %1)'"), ДатаСтрокой);
...
Процедура КУказаниюСерийНаСервере()
	...
	Для Каждого Описание Из НастройкиИспользованияСерий.ОписанияИспользованияРеквизитовСерии Цикл
		Если Описание.ИмяНастройки <> "ИспользоватьRFIDМеткиСерии" Тогда
			...
			НастраиваемыйЭлемент = Элементы.Найти("ОстаткиСерийТекущие" + Описание.ИмяРеквизита);
			...
			Если Описание.ИмяРеквизита = "ГоденДо" Тогда
				Элементы.ОстаткиСерийЗаполнитьПоFEFO.Видимость = Описание.Использование;
			КонецЕсли;	
...
Функция РаспределитьОстатокНаСервере()
	...
	Если КоличествоКРаспределению <= 0 Тогда
		Возврат НСтр("ru='Все количество товара уже распределено по сериям.';uk='Всю кількість товару вже розподілено по серіях.'");
	КонецЕсли;
...
|;uk='Розподілено всю доступну кількість в виділених рядках, але її недостатньо для розподілу всієї кількості товару. 
|Спробуйте виділити інші рядки і повторити розподіл.'");
	КонецЕсли;
