...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗакрытьБезВопроса Тогда
		...
		Если ЕстьВыгруженныеФайлы Тогда
			...
			Кнопки.Добавить(1, НСтр("ru='Удалить файлы и закрыть';uk='Вилучити файли і закрити'"));
			Кнопки.Добавить(2, НСтр("ru='Закрыть без удаления';uk='Закрити без вилучення'"));
			Кнопки.Добавить(0, НСтр("ru='Отмена';uk='Відмінити'"));
...
|;uk='Вивантажені файли можуть бути несанкціоновано змінені.
| Рекомендується вилучити файли перед закриттям форми.'");
			ПоказатьВопрос(
...
Процедура ПериодПриИзменении(Элемент)
	...
	ИначеЕсли Период = "ЗаДату" Тогда
		...
		ПоказатьВводДаты(Оповещение, ЛокальнаяТекущаяДата, НСтр("ru='Дата отбора платежей';uk='Дата відбору платежів'"), ЧастиДаты.Дата);
		Возврат;
...
Процедура ВывестиОтчет(Команда)
	...
	Если ТаблицаОтчетОВыгрузке <> Неопределено Тогда
		...
		ТаблицаОтчетОВыгрузке.Показать(НСтр("ru='Отчет о выгрузке платежей';uk='Звіт про вивантаження платежів'"));
	КонецЕсли;
...
Процедура ОбработатьЗавершениеВыгрузкиНаСервере(ЕстьНеобходимостьОбновленияСпискаПлатежей, ВсегоФайлов)
	...
	Если ВсегоФайлов > 0 Тогда
		...
		СклонениеВыгружено = НСтр("ru='Выгружен, Выгружено, Выгружено';uk='Вивантажений, Вивантажено, Вивантажено'");
		...
		СклонениеФайлов = НСтр("ru='файл, файла, файлов';uk='файл, файлу, файлів'");
...
|;uk='З метою безпеки проводиться контроль відповідності файлів вивантаження і даних програми, поки ця форма відкрита. При закритті цієї форми файли будуть вилучені.
|Обов''язково перевірте правильність платіжних реквізитів в програмі Клієнт банку.'");
		Иначе
			Элементы.ДекорацияВыгружено.Заголовок = Выгружено + " " + Файлов + ". " +
...
|Обязательно проверьте правильность платежных реквизитов в программе Клиент банка.'
|;uk='Контроль відповідності файлів вивантаження і даних програми не проводиться. З метою безпеки потрібно встановити розширення для роботи з файлами. 
|Обов''язково перевірте правильність платіжних реквізитів в програмі Клієнт банку.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыборФайлаПриВыгрузке(Счет)
	...
	ДиалогВыбораФайла.Заголовок               =
		СтрШаблон(НСтр("ru='Укажите имя файла для выгрузки данных по счету %1';uk='Вкажіть ім''я файлу для вивантаження даних по рахунку %1'"), Строка(Счет.Ссылка));
...
Процедура ВыборФайлаОбменаСБанком(ТекущиеДанные)
	...
	ДиалогВыбораФайла.Фильтр                  = НСтр("ru='XML файл (*.xml)|*.xml';uk='XML файл (*.xml)|*.xml'");
	ДиалогВыбораФайла.Заголовок               = НСтр("ru='Укажите имя файла для выгрузки данных';uk='Вкажіть ім''я файлу для вивантаження даних'");
	ДиалогВыбораФайла.ПредварительныйПросмотр = Ложь;
	ДиалогВыбораФайла.Расширение              = "xml";
	ДиалогВыбораФайла.ИндексФильтра           = 0;
