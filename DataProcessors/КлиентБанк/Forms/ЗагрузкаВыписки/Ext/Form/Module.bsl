...
Процедура ВыбратьФайл(Команда)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для импорта выписки из банковского счета используется XML файл.
...
//					НСтр("ru='Для импорта выписки из банковского счета %1 используется XML файл. Импорт из XML не поддержуется для Web клиента. Выберите другой клиент.'"), Счет.Ссылка);
//				КонецЕсли; 
				#КонецЕсли
...
Процедура ВывестиОтчет(Команда)
	...
	Если ТаблицаОтчетОЗагрузке <> Неопределено Тогда
		...
		ТаблицаОтчетОЗагрузке.Показать(НСтр("ru='Отчет о загрузке платежей';uk='Звіт про завантаження платежів'"));
	КонецЕсли;
...
Процедура ПодключитьРасширениеРаботыСФайламиИЗагрузитьФайл(Счет)
	...
	ТекстСообщения = НСтр("ru='Для загрузки файлов выписки необходимо установить расширение работы с файлами.';uk='Для завантаження файлів виписки необхідно встановити розширення роботи з файлами.'");
