...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Заказы) Тогда
		...
		Если КоличествоНепроведенных = 1 Тогда
			ВызватьИсключение НСтр("ru='Для выполнения команды необходимо предварительно провести документ.';uk='Для виконання команди необхідно попередньо провести документ.'");
		ИначеЕсли КоличествоНепроведенных > 1 Тогда
			ВызватьИсключение НСтр("ru='Для выполнения команды необходимо предварительно провести документы.';uk='Для виконання команди необхідно попередньо провести документи.'");
		КонецЕсли;
	Иначе
		ВызватьИсключение НСтр("ru='Для выполнения команды необходимо выбрать документ.';uk='Для виконання команди необхідно вибрати документ.'");
	КонецЕсли;
...
Процедура СформироватьНадписьРасчеты()
	...
	Если ТипЗаказа = "ЗаказКлиента" ИЛИ ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		...
		Если Заказы.Количество() = 1 И ТипЗаказа = "ЗаказКлиента" Тогда
			...
			ТекстОплата = НСтр("ru='Оплачено по заказу: %СуммаОплаты%  %Валюта%  %ПроцентОплаты%%';uk='Оплачено за замовленням: %СуммаОплаты% %Валюта% %ПроцентОплати%%'");
			...
			ТекстОтгрузка = НСтр("ru='Отгружено: %СуммаОтгрузки% %Валюта%  %ПроцентОтгрузки%%';uk='Відвантажено: %СуммаОтгрузки% %Валюта% %ПроцентОтгрузки%%'");
				...
				Иначе
					...
					Если СтруктураРасчетов.НашДолг > СтруктураРасчетов.ДолгКлиента Тогда
						...
						Расчеты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Наш долг по заявке: %1 %2';uk='Наш борг за заявкою: %1 %2'"),
					...
					ИначеЕсли СтруктураРасчетов.ДолгКлиента > СтруктураРасчетов.НашДолг Тогда
						...
						Расчеты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Долг клиента по заявке: %1 %2';uk='Борг клієнта за заявкою: %1 %2'"),
	...
	ИначеЕсли ТипЗаказа = "ЗаказПоставщику" Тогда
		...
		Если Заказы.Количество() = 1 Тогда
			...
			ТекстОтгрузка = НСтр("ru='Поступило: %СуммаПоступления% %Валюта%  %ПроцентПоступления%%';uk='Надійшло: %СуммаПоступления% %Валюта% %ПроцентПоступления%%'");
...
Процедура ЗаполнитьПодсказкиИДекорации()
	...
	Если ТипЗаказа = "ЗаказКлиента" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена реализация и не осуществлялась отгрузка';uk='не оформлена реалізація і не здійснювалося відвантаження'");
	ИначеЕсли ТипЗаказа = "ЗаказПоставщику" Тогда
		ТекстНеОформленаПриемка = НСтр("ru='не оформлено поступление и не осуществлялась приемка';uk='не оформлене надходження і не здійснювалося приймання'");
	ИначеЕсли ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена реализация замены и не осуществлялась отгрузка';uk='не оформлена реалізація заміни і не здійснювалося відвантаження'");
		ТекстНеОформленаПриемка = НСтр("ru='не оформлен возврат';uk='не оформлено повернення'");
	ИначеЕсли ТипЗаказа = "ЗаказНаВнутреннееПотребление" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлено списание и не осуществлялась отгрузка';uk='не оформлене списання і не здійснювалося відвантаження'");
	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена сборка и не осуществлялась отгрузка';uk='не оформлено збирання і не здійснювалося відвантаження'");
		ТекстНеОформленаПриемка = НСтр("ru='не осуществлялась приемка';uk='не здійснювалось приймання'");
	ИначеЕсли ТипЗаказа = "ЗаказНаПеремещение" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлено перемещение и не осуществлялась отгрузка';uk='не оформлене переміщення і не здійснювалось відвантаження'");
		ТекстНеОформленаПриемка = НСтр("ru='не осуществлялась приемка';uk='не здійснювалось приймання'");
	КонецЕсли;
	
	Заголовок = СтрЗаменить(НСтр("ru='Помощник закрытия %РП%';uk='Помічник закриття %РП%'"),"%РП%",ТекстРП);
	
	ТекстИ             = ?(ЗначениеЗаполнено(ТекстНеОформленаОтгрузка) И ЗначениеЗаполнено(ТекстНеОформленаПриемка),
							НСтр("ru=', а также ';uk=', а також'"),
							"");
	ТекстПричинаОтмены = ?(ИспользоватьПричиныОтменыЗаказов, НСтр("ru='Необходимо указать причину отмены.';uk='Необхідно вказати причину скасування.'"), "");
	
	ТекстСтрокиОтгрузка = НСтр("ru='Строки';uk='Рядки'");
	ТекстСтрокиПриемка  = НСтр("ru='Строки';uk='Рядки'");
	
	Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		ТекстСтрокиОтгрузка = НСтр("ru='Строки заменяющих товаров';uk='Рядки заміняючих товарів'");
		ТекстСтрокиПриемка  = НСтр("ru='Строки возвращаемых товаров';uk='Рядки товарів, що повертаються'");
	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		ТекстСтрокиОтгрузка = НСтр("ru='Строки списываемых комплектующих / разбираемых комплектов';uk='Рядки списаних комплектуючих / розбираних комплектів'");
		ТекстСтрокиПриемка  = НСтр("ru='Строки приходуемых комплектующих / собираемых комплектов';uk='Рядки оприбутковуваних комплектуючих / збираних комплектів'");
	КонецЕсли;
	
	// Начальная страница
	
	Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерных товаров.';uk='Скорегувати рядки мірних товарів.'");
	Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных товаров.';uk='Скорегувати рядки мірних товарів.'");
	
	Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерных заменяющих товаров.';uk='Скоригувати рядки мірних заміняючих товарів.'");
		Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных возвращаемых товаров.';uk='Скоригувати рядки мірних товарів, що повертаються.'");
	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерных списываемых комплектующих / разбираемых комплектов.';uk='Скоригувати рядки мірних списаних комплектуючих/розбираних комплектів.'");
		Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных приходуемых комплектующих / собираемых комплектов.';uk='Скоригувати рядки мірних оприбутковуваних комплектуючих/збираних комплектів.'");
	КонецЕсли;
	
	Элементы.СкорректироватьМерныеТовары.РасширеннаяПодсказка.Заголовок =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В строках %1 с мерными товарами будут устранены расхождения с накладными в пределах допустимых отклонений.';uk='У рядках %1 з мірними товарами будуть усунені розбіжності з накладними в межах допустимих відхилень.'"),
			ТекстРП);
	...
	Элементы.ГруппаНачалоОтменитьНепоставленные.РасширеннаяПодсказка.Заголовок =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Строки %1, по которым %2%3%4, будут отменены. %5';uk='Рядки %1, за якими %2 %3 %4, будуть скасовані.  %5'"), 
			ТекстРП,
			ТекстНеОформленаОтгрузка,
			ТекстИ,
			ТекстНеОформленаПриемка,
	...
	Элементы.ЗакрыватьЗаказы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Устанавливать статус %1.';uk='Встановлювати статус %1.'"), 
	...
	Элементы.ЗакрыватьЗаказы.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='При выполнении необходимых условий %1 будет установлен статус %2.';uk='При виконанні необхідних умов %1 буде встановлено статус %2.'"), 
		ТекстДП,
		СтатусЗакрытПредставление);
		
	Если Заказы.Количество() = 1 Тогда
		Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
			ТекстЗаголовкаБезОплаты = СтрШаблон(НСтр("ru='Частично оплаченная %1 не будет закрыта.';uk='Частково оплачена %1 не буде закрита.'"),ТекстИП);
			ТекстЗаголовкаБезОтгрузки = СтрШаблон(НСтр("ru='Частично выполненная %1 не будет закрыта.';uk='Частково виконана %1 не буде закрита.'"),ТекстИП);
			ТекстЗаголовкаБезВсего = СтрШаблон(НСтр("ru='Частично выполненная или частично оплаченная %1 не будет закрыта.';uk='Частково виконана або частково оплачена %1 не буде закрита.'"),ТекстИП);
		Иначе
			ТекстЗаголовкаБезОплаты = СтрШаблон(НСтр("ru='Частично оплаченный %1 не будет закрыт.';uk='Частково оплачений %1 не буде закритий.'"),ТекстИП);
			ТекстЗаголовкаБезОтгрузки = СтрШаблон(НСтр("ru='Частично выполненный %1 не будет закрыт.';uk='Частково виконаний %1 не буде закритий.'"),ТекстИП);
			ТекстЗаголовкаБезВсего = СтрШаблон(НСтр("ru='Частично выполненный или частично оплаченный %1 не будет закрыт.';uk='Частково виконаний або частково оплачений %1 не буде закритий.'"),ТекстИП);
		КонецЕсли;
	Иначе
		ТекстЗаголовкаБезОплаты = СтрШаблон(НСтр("ru='Частично оплаченные %1 не будет закрыты.';uk='Частково оплачені %1 цієї статті не буде закрито.'"),ТекстИП);
		ТекстЗаголовкаБезОтгрузки = СтрШаблон(НСтр("ru='Частично выполненные %1 не будет закрыты.';uk='Частково виконані %1 не будуть закриті.'"),ТекстИП);
		ТекстЗаголовкаБезВсего = СтрШаблон(НСтр("ru='Частично выполненные или частично оплаченные %1 не будет закрыты.';uk='Частково виконані або частково оплачені %1 цієї статті не буде закрито.'"),ТекстИП);
	КонецЕсли;
	...
	Элементы.ДекорацияОтгрузкаБезОтгрузки.Заголовок = СтрШаблон(
		НСтр("ru='%1 Текущее состояние может измениться после корректировок строк.';uk='%1 Поточний стан може змінитися після коригувань рядків.'"), 
	...
	Элементы.МерныеТоварыККорректировкеОтгрузкаДекорация.Заголовок =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 %2 с мерными товарами, по которым имеются расхождения в пределах допустимых отклонений будут скорректированы.';uk='%1 %2 з мірними товарами, по яких є розбіжності в межах допустимих відхилень будуть скориговані.'"),
			ТекстСтрокиОтгрузка,
			ТекстРП);
	...
	Элементы.ТоварыКОтменеОтгрузкаДекорация.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 %2, по которым %3, будут отменены.';uk=' %1 %2, за якими %3, будуть скасовані.'"),
		ТекстСтрокиОтгрузка,
		ТекстРП,
		ТекстНеОформленаОтгрузка);
	...
	// Закрытие заказов
	Элементы.ЗакрытиеЗаказовДекорацияНеудача.Заголовок = СтрШаблон(НСтр("ru='Не удалось обработать %1:';uk='Не вдалося обробити %1:'"), ТекстИП);
	
	Если Заказы.Количество() = 1 Тогда
		Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
			ТекстЗаголовкаНеудача = СтрШаблон(НСтр("ru='Указанная %1 обработана.';uk='Зазначена %1 оброблена.'"), ТекстИП);
			ТекстЗаголовкаВсеЗакрыты = СтрШаблон(НСтр("ru='Указанная %1 уже закрыта или не требует закрытия.';uk='Зазначена %1 вже закрита або не потребує закриття.'"),ТекстИП);
		Иначе
			ТекстЗаголовкаНеудача = СтрШаблон(НСтр("ru='Указанный %1 обработан.';uk='Зазначений %1 оброблений.'"), ТекстИП);
			ТекстЗаголовкаВсеЗакрыты = СтрШаблон(НСтр("ru='Указанный %1 уже закрыт или не требует закрытия.';uk='Зазначене %1 вже закрите або не потребує закриття.'"),ТекстИП);
		КонецЕсли;
	Иначе
		ТекстЗаголовкаНеудача = СтрШаблон(НСтр("ru='Все %1 обработаны.';uk='Всі %1 оброблені.'"), ТекстИП);
		ТекстЗаголовкаВсеЗакрыты = СтрШаблон(НСтр("ru='Все %1 уже закрыты или не требуют закрытия.';uk='Всі %1 вже закриті або не потребують закриття.'"),ТекстИП);
	КонецЕсли;
		
	Элементы.ЗакрытиеЗаказовДекорацияНеудача.Заголовок =
		СтрШаблон(НСтр("ru='%1 Нажмите ""Закрыть"" для завершения.';uk='%1 Натисніть ""Закрити"" для завершення.'"), ТекстЗаголовкаНеудача);
	
	Элементы.ВсеЗаказыЗакрытыДекорация.Заголовок =
		СтрШаблон(НСтр("ru='%1 Нажмите ""Закрыть"" для завершения.';uk='%1 Натисніть ""Закрити"" для завершення.'"), ТекстЗаголовкаВсеЗакрыты);
...
Функция НастройкиПадежейПоТипуЗаказа(ТипЗаказа, ЗаказыКоличество)
	Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		Если ЗаказыКоличество = 1 Тогда
			ТекстРП = НСтр("ru='заявки';uk='заявки'");
			ТекстИП = НСтр("ru='заявка';uk='заявка'");
			ТекстДП = НСтр("ru='заявке';uk='заявці'");
		Иначе
			ТекстРП = НСтр("ru='заявок';uk='заявок'");
			ТекстИП = НСтр("ru='заявки';uk='заявки'");
			ТекстДП = НСтр("ru='заявкам';uk='заявками'");
		КонецЕсли;
		ТекстИПЕЧ = НСтр("ru='заявка';uk='заявка'");
		ТекстИПМЧ = НСтр("ru='заявки';uk='заявки'");
		СтатусЗакрытПредставление = НСтр("ru='""Выполнена""';uk='""Виконано""'");
	Иначе
		Если ЗаказыКоличество = 1 Тогда
			ТекстРП = НСтр("ru='заказа';uk='замовлення'");
			ТекстИП = НСтр("ru='заказ';uk='замовлення'");
			ТекстДП = НСтр("ru='заказу';uk='замовлення'");
		Иначе
			ТекстРП = НСтр("ru='заказов';uk='замовлень'");
			ТекстИП = НСтр("ru='заказы';uk='замовлення'");
			ТекстДП = НСтр("ru='заказам';uk='замовленнями'");
		КонецЕсли;
		ТекстИПЕЧ = НСтр("ru='заказ';uk='замовлення'");
		ТекстИПМЧ = НСтр("ru='заказы';uk='замовлення'");
		СтатусЗакрытПредставление = НСтр("ru='""Закрыт""';uk='""Закрито""'");
	КонецЕсли;
...
Функция СформироватьТекстОбработано()
	...
	Если Обработано = 0 Тогда
		Возврат СтрШаблон(НСтр("ru='Не удалось обработать %1:';uk='Не вдалося обробити %1:'"), ТекстИП);
	КонецЕсли;
	
	Если ТипЗаказа = "ЗаказКлиента" Тогда
		ТекстДля = НСтр("ru='Для заказов клиентов (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень клієнтів (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаказПоставщику" Тогда
		ТекстДля = НСтр("ru='Для заказов поставщикам (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень постачальникам (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		ТекстДля = НСтр("ru='Для заказов на сборку (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень на складання (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаказНаВнутреннееПотребеление" Тогда
		ТекстДля = НСтр("ru='Для заказов на внутреннее потребление (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень на внутрішнє споживання (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаказНаПеремещение" Тогда
		ТекстДля = НСтр("ru='Для заказов на перемещение (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень на переміщення (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		ТекстДля = НСтр("ru='Для заявок на возврат товаров от клиента (%КоличествоОбработанных%) %ТекстДействия%';uk='Для заявок на повернення товарів від клієнта (%КоличествоОбработанных%) %ТекстДействия%'");
	КонецЕсли;
	
	ТекстДействия = "";
	Если СкорректироватьМерныеТовары Тогда
		ТекстДействия = ТекстДействия + НСтр("ru='скорректированы строки мерных товаров';uk='скориговані рядки мірних товарів'");
	КонецЕсли;
	
	Если СкорректироватьЦены Тогда
		ТекстДействия = ТекстДействия + ?(ТекстДействия="","",", ") + НСтр("ru='скорректированы цены';uk='скориговані ціни'");
	КонецЕсли;
	
	Если ОтменитьНеотработанныеСтроки Тогда
		ТекстДействия = ТекстДействия + ?(ТекстДействия="","",", ") + НСтр("ru='отменены непоставленные строки';uk='скасовані непоставлені рядки'");
	КонецЕсли;
	
	Если ЗакрыватьЗаказы Тогда
		ТекстДействия = ТекстДействия + ?(ТекстДействия="","",", ") + НСтр("ru='установлен статус ""Закрыт""';uk='встановлено статус ""Закрито""'");
	КонецЕсли;
	
	ТекстДля = СтрЗаменить(ТекстДля, "%КоличествоОбработанных%", Заказы.Количество()-НеотработанныеЗаказы.Количество());
	
	Если НеотработанныеЗаказы.Количество() > 0 Тогда
		 ТекстДействия = ТекстДействия + Символы.ПС + СтрШаблон(НСтр("ru='Не удалось обработать %1:';uk='Не вдалося обробити %1:'"), ТекстИПМЧ);
	КонецЕсли;
...
Функция ТекстНеудачнойОбработки(Закрывать)
	...
	Если Закрывать Тогда 
		ШаблонНеУдалосьЗакрыть = НСтр("ru='Не удалось установить статус %1.';uk='Не вдалося встановити статус %1.'");
	...
	Иначе
		...
		Если СкорректироватьМерныеТовары
			...
			ТекстДействия = НСтр("ru='Не удалось скорректировать строки мерных товаров, скорректировать цены, отменить непоставленные строки.';uk='Не вдалося скорегувати строки мірних товарів, скорегувати ціни, скасувати непоставлені рядки.'");
		ИначеЕсли СкорректироватьМерныеТовары
			И СкорректироватьЦены
			И Не ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось скорректировать строки мерных товаров, скорректировать цены.';uk='Не вдалося скорегувати строки мірних товарів, скорегувати ціни.'");
		ИначеЕсли СкорректироватьМерныеТовары
			И Не СкорректироватьЦены
			И ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось скорректировать строки мерных товаров, отменить непоставленные строки.';uk='Не вдалося скорегувати строки мірних товарів, скасувати непоставлені рядки.'");
		ИначеЕсли СкорректироватьМерныеТовары
			И Не СкорректироватьЦены
			И Не ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось скорректировать строки мерных товаров.';uk='Не вдалося скорегувати строки мірних товарів.'");
		ИначеЕсли Не СкорректироватьМерныеТовары
			И СкорректироватьЦены
			И ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось скорректировать цены, отменить непоставленные строки.';uk='Не вдалося скорегувати ціни, скасувати непоставлені рядки.'");
		ИначеЕсли Не СкорректироватьМерныеТовары
			И СкорректироватьЦены
			И Не ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось скорректировать цены.';uk='Не вдалося скорегувати ціни.'");
		ИначеЕсли Не СкорректироватьМерныеТовары
			И Не СкорректироватьЦены
			И ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = НСтр("ru='Не удалось отменить непоставленные строки.';uk='Не вдалося скасувати непоставлені рядки.'");
		КонецЕсли;
...
Процедура ГотовоСервер(ЗакрыватьВТекущемВызове, ЗаказыКОбработке, СтруктураДополнительныхПараметров)
	...
	Для Каждого ТекСтрока Из ЗаказыКОбработке Цикл
		...
		Исключение
			...
			Если СтрокаНеотработанныхЗаказов.Неудача <> НСтр("ru='Не удалось заблокировать документ.';uk='Не вдалося заблокувати документ.'") Тогда
				СтрокаНеотработанныхЗаказов.Неудача = НСтр("ru='Не удалось заблокировать документ.';uk='Не вдалося заблокувати документ.'");
