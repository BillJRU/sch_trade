...
Процедура СформироватьДанные(ВводНовогоАдреса = Ложь)
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не допускается ввод адресов с одинаковыми датами.';uk='Не допускається введення адрес з однаковими датами.'"),, "История[" + Строка(Индекс) + "].ДействуетС");
			Возврат;
		КонецЕсли;
