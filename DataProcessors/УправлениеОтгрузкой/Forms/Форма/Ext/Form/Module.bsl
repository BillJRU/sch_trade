...
Процедура ПроверитьКоличество(Команда)
	Если Элементы.ОрдераВРаботе.ТекущиеДанные <> Неопределено Тогда
		...
		ПараметрЗаголовок = НСтр("ru='%ПроверкаУпаковка% товаров в документе %Документ%';uk='%ПроверкаУпаковка% товарів в документі %Документ%'");
...
Процедура СкладПомещениеПриИзмененииСервер()
	...
	Если ИспользоватьСтатусыРасходныхОрдеров Тогда
		Элементы.Состояние.СписокВыбора.Добавить("ВсеВРаботе", НСтр("ru='Все в работе';uk='Всі в роботі'"));		
		Элементы.Состояние.СписокВыбора.Добавить("ОжидаетсяОтбор", НСтр("ru='Ожидается отбор';uk='Очікується відбір'"));
		Элементы.Состояние.СписокВыбора.Добавить("ОжидаетсяОтгрузка", НСтр("ru='Ожидается отгрузка';uk='Очікується відвантаження'"));		
		Элементы.Состояние.СписокВыбора.Добавить("ОжидаетсяОформлениеНакладных", НСтр("ru='Ожидается оформление накладных';uk='Очікується оформлення накладних'"));
		Элементы.Состояние.СписокВыбора.Добавить("", НСтр("ru='Все';uk='Всі'"));
...
Процедура НазначитьЗаданиеНаПеревозкуСервер(ЗаданиеНаПеревозку, ДополнительныеПараметры)
	...
	Для Каждого Ордер Из ОбрабатываемыеОрдера Цикл
		...
		Если Ордер.Статус <> СтатусОтгружено
			...
			ТекстСообщения = НСтр("ru='Выбранное задание на перевозку находится в статусе ""%СтатусЗадания%"", поэтому его нельзя указать в ордере ""%Документ%"", который находится в статусе ""%СтатусОрдера%"". Измените статус ордера на ""%СтатусОтгружено%"".';uk='Обраний завдання на перевезення знаходиться в статусі ""%СтатусЗадания%"", тому його не можна вказати в ордері ""%Документ%"", який знаходиться в статусі ""%СтатусОрдера%"". Змініть статус ордера на ""%СтатусОтгружено%"".'");
...
Процедура ОповеститьОбОшибкахВыбораЗаданияНаПеревозку(НеобработанныеОрдера)
	...
	Для Каждого Ордер Из НеобработанныеОрдера Цикл
		...
		Если Не Ордер.ОтгрузкаПоЗаданиюНаПеревозку
			...
			ТекстСообщения = НСтр("ru='Для ордера ""%Документ%"" не требуется доставка.';uk='Для ордера ""%Документ%"" не потрібно доставка.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Документ%", Ордер.Ссылка);
...
Процедура СоздатьЗаданияНаКлиенте(Переупаковка = Ложь)
	...
	Если Не Отказ Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='Для создания заданий необходимо выделить распоряжения на отгрузку.';uk='Для створення завдань необхідно виділити розпорядження на відвантаження.'");
