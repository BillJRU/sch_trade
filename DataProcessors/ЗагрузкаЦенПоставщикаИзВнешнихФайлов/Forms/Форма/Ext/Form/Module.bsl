...
Процедура ДеревоЦенНоменклатураПоставщикаПриИзменении(Элемент)
	...
	Если ИспользоватьУпаковкиНоменклатуры Тогда		
		...
		ИзменитьУпаковкиПоВидамЦен(ТекущаяСтрока, ВидыЦен);
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьУпаковкиПоВидамЦен(ТекущаяСтрока, ВидыЦен)
	ИзменитьУпаковкиПоВидамЦенНаСервере(ТекущаяСтрока, ВидыЦен);
КонецПроцедуры

&НаСервере
Процедура ИзменитьУпаковкиПоВидамЦенНаСервере(ТекущаяСтрока, ВидыЦен)
	...
	Для Каждого ВидЦены Из ВидыЦен Цикл
		...
		Если ТекущаяСтрока[ИмяКолонкиУпаковка] <> ТекущаяСтрока.Упаковка Тогда
			...
			ДеревоЦенУпаковкаПриИзмененииНаСервере(ТекущаяСтрока.ПолучитьИдентификатор(), ИмяКолонкиУпаковка);
	...
	Пока ЗаполненаНоменклатураАртикул Цикл
		...
		Если МассивНоменклатурыПоставщика.Количество() = 1 Тогда
			...
			Если ИспользоватьНоменклатуруПоставщиков Тогда
				...
				НоваяСтрока.Упаковка = МассивНоменклатурыПоставщика[0].Упаковка;
				Если ИспользоватьУпаковкиНоменклатуры Тогда		
					ВидыЦен = УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтаФорма);
					ИзменитьУпаковкиПоВидамЦенНаСервере(НоваяСтрока, ВидыЦен);
				КонецЕсли;
			Иначе
			...
			Для Каждого ТекЭлемент Из МассивНоменклатурыПоставщика Цикл
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					Если ИспользоватьНоменклатуруПоставщиков Тогда
						...
						НоваяНоменклатураПоставщика.Упаковка = ТекЭлемент.Упаковка;
					КонецЕсли;
