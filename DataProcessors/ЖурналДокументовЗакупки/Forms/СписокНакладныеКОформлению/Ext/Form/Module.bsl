...
#КонецОбласти
	...
	Если ИспользоватьОтветственноеХранениеВПроцессеЗакупки Тогда
		Элементы.СтраницаРаспоряженияНаОформление.Заголовок = НСтр("ru='К оформлению приобретения (приемки)';uk='До оформлення придбання (приймання)'");
	КонецЕсли;
...
Процедура ОформитьПоПриемке(Команда)
	...
	ПараметрыСозданныхДокументов = ОформитьПриобритенияНаСервереПоОрдерам();
...
Процедура ОформитьПоступлениеТоваров(Команда)
	...
	// Выбор варианта действия и возврат резульата в виде имени формы которую необходимо открыть и ее входящих параметров.
	ПараметрыСозданныхДокументов = ОформитьПоступленияПоЗаказамНаСервере();
...
Функция НастройкиФормыПереоформленияНакладных(РаспоряжениеСсылка,
	...
	НастройкиФормы.Заголовок			= НСтр("ru='Переоформление документов закупки по выбранным распоряжениям';uk='Переоформлення документів купівлі за вибраними розпорядженням'");
...
Процедура ЗаполнитьСпискиВыбораПоСостояниям()
	...
	СписокВыбора.Добавить("1", НСтр("ru='Оформлены ордера, не оформлены накладные';uk='Оформлені ордери, не оформлені накладні'"), Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("5", НСтр("ru='Не оформлены ордера, оформлены накладные';uk='Не оформлено ордери, оформлені накладні'"), Ложь, БиблиотекаКартинок.НеОформленыОрдера);
	СписокВыбора.Добавить("3", НСтр("ru='Ордера не соответствуют накладным';uk='Ордери не відповідають накладним'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
	...
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("0", НСтр("ru='Поступления соответствуют распоряжениям';uk='Надходження відповідають розпорядженням'"), Ложь, БиблиотекаКартинок.ОформленаНакладная);
	СписокВыбора.Добавить("1", НСтр("ru='Создать поступление';uk='Створити надходження'"),Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("3", НСтр("ru='Поступления не соответствуют распоряжениям';uk='Надходження не відповідають розпорядженням'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
	
	СписокВыбора = Элементы.СостояниеПоступленияПриобретения.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("0", НСтр("ru='Приобретения соответствуют поступлениям';uk='Придбання відповідають надходженням'"), Ложь, БиблиотекаКартинок.ОформленаНакладная);
	СписокВыбора.Добавить("1", НСтр("ru='Не оформлены приобретения';uk='Не оформлені придбання'"), Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("3", НСтр("ru='Приобретения не соответствуют поступлениям';uk='Придбання не відповідають надходженням'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
...
Функция ПараметрыЖурнала(КлючНазначенияИспользования)
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала",НСтр("ru='Документы закупки';uk='Документи купівлі'"));
...
Функция ТекстЗапросаСпискаРаспоряженийНаПриемку()
	...
	|ГДЕ
	|	(Таблица.ДокументПоступления, Таблица.Склад, Таблица.ХозяйственнаяОперация) В
	|			(ВЫБРАТЬ
...
Функция ОформитьРядНакладныхПоТаблицам(ДокументыПоТипамНакладных, СписокОшибок)
	...
	Для Каждого Таблица Из ДокументыПоТипамНакладных Цикл
		...
		Если ПерваяСтрока.СостояниеПриобретения > 0 Тогда
			...
			Если ПерваяСтрока.Ссылка = Неопределено Тогда
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Иначе // оформление по заказам
			...
			Если Строка.СостояниеПриобретения = 0 Тогда
				...
				ТекстОшибки = НСтр("ru='Для распоряжения %1 уже создана накладная';uk='Для розпорядження %1 вже створена накладна'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ПрерватьОбработкуСтроки = Истина;
...
&НаСервере
Функция ОформитьПриобритенияНаСервереПоОрдерам()
...
Функция ТаблицыПриобретенийРазделенныхПоКлючевымПолямДляОрдеров(СписокРаспоряжений, СписокОшибок) 
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если Строка.СостояниеОрдера = 5 Тогда
			...
			ТекстОшибки = НСтр("ru='Перезаполнение по приемке %1 не требуется, так как нет ни одного ордера.';uk='Перезаповнення за прийманням %1 не потрібно, тому що немає жодного ордера.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
			ПрерватьОбработкуСтроки = Истина;
...
|;uk='У пакетному режимі можна або створювати нові документи або перезаповнювати.
|Документ %1 не буде оброблений, виберіть його окремо.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ПрерватьОбработкуСтроки = Истина;
			Иначе
...
|Документ %1 не буде оброблений, виберіть його окремо.'");
		...
		ИначеЕсли НЕ (Строка.СостояниеОрдера = 1
			...
			ТекстОшибки = НСтр("ru='По документу %1 не требуется оформление приемки или приобретения';uk='За документом %1 не потрібно оформлення приймання або придбання'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
			ПрерватьОбработкуСтроки = Истина;
	...
	Если МассивТаблиц.Количество() > 1 И ЕстьОформленныеПоступления Тогда
		МассивТаблиц = Новый Массив();
		ТекстОшибки = НСтр("ru='Переоформлять накладные можно только по заказам, имеющим одинаковые значимые реквизиты.';uk='Переоформляти накладні можна тільки по замовленнях, що мають однакові значущі реквізити.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаПриемку",ТекстОшибки, Неопределено);
	КонецЕсли;
...
|;uk='Для розпорядження %1 неможливо створити акт про розбіжності.
|Виберіть рядок із типом розпорядження ""Придбання товарів і послуг"" або ""Приймання товарів на зберігання"".'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ПрерватьОбработкуСтроки = Истина;
			Иначе
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно создать акт о расхождениях.
...
|;uk='Для розпорядження %1 неможливо створити акт про розбіжності.
|Виберіть рядок із типом розпорядження ""Придбання товарів і послуг"".'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ПрерватьОбработкуСтроки = Истина;
			КонецЕсли;
		...
		ИначеЕсли ХозОперацииПоступленияТоваров.Найти(Строка.ХозяйственнаяОперация) <> Неопределено Тогда
			НаименованиеАкта = ?(ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг"),
				НСтр("ru='поступления';uk='надходження'"),
				НСтр("ru='приемки';uk='приймання'"));
			
			ТекстОшибки = НСтр("ru='Невозможно оформить документ ""Акт о расхождениях после %1"" на основании распоряжения %2 с операцией ""%3"".';uk='Неможливо оформити документ ""Акт про розбіжності після %1"" на підставі розпорядження %2 з операцією ""%3"".'");
...
&НаСервере
Функция ОформитьПоступленияПоЗаказамНаСервере()
	
	Перем СписокОшибок;
	
	// Разбивка выделенных документов по группам
	ДокументыПоТипамНакладных = ТаблицыЗаказовРазделенныхПоКлючевымПолямДляПоступлений(СписокРаспоряжений, СписокОшибок);
	ВсегоТаблиц = ДокументыПоТипамНакладных.Количество();
	...
	Если ВсегоТаблиц > 0 Тогда
		...
		Если ДокументыПоТипамНакладных.Количество() > 1 Тогда
			// Пакетно
			ПараметрыФормы = ОформитьРядПоступленийПоТаблицам(ДокументыПоТипамНакладных, СписокОшибок);
		ИначеЕсли ДокументыПоТипамНакладных.Количество() = 1 Тогда
			// Индивидуально
			ПараметрыФормы = ОформитьПоступлениеПоТаблице(ДокументыПоТипамНакладных, СписокОшибок);
		КонецЕсли;
	ИначеЕсли СписокРаспоряжений.Количество() > 1 Тогда
...
&НаСервере
Функция ОформитьРядПоступленийПоТаблицам(ДокументыПоТипамНакладных, СписокОшибок)
	...
	Для Каждого Таблица Из ДокументыПоТипамНакладных Цикл
		...
		ДокументОбъект = Документы.ПоступлениеТоваровНаСклад.СоздатьДокумент();
		ДокументОбъект.Заполнить(ДанныеЗаполнения);
		...
		Иначе
			ТекстОшибки = НСтр("ru='Не удалось создать документ ""Поступления товаров"" на основании документа %1';uk='Не вдалося створити документ ""Надходження товарів"" на підставі документа %1'");
...
&НаСервере
Функция ОформитьПоступлениеПоТаблице(ДокументыПоТипамНакладных, СписокОшибок)
...
&НаСервере
Функция ТаблицыЗаказовРазделенныхПоКлючевымПолямДляПоступлений(СписокРаспоряжений, СписокОшибок) 
	КлючевыеПоля = "Ссылка";
	...
	ШаблонОшибки = НСтр("ru='По распоряжению %1 оформление документа ""Поступление товаров"" не требуется.';uk='За розпорядженням %1 оформлення документа ""Надходження товарів"" не потрібно.'");
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если Строка.СостояниеПоступленияТоваров = 0
...
Функция ОформитьРядПриобретенийПоТаблицам(ДокументыПоТипамНакладных, СписокОшибок)
	...
	Для Каждого Таблица Из ДокументыПоТипамНакладных Цикл
		...
		Иначе
			ТекстОшибки = НСтр("ru='Не удалось создать документ ""Приобретение товаров и услуг"" на основании документа %1';uk='Не вдалося створити документ ""Придбання товарів і послуг"" на підставі документа %1'");
...
Функция ТаблицыЗаказовРазделенныхПоКлючевымПолямДляПриобретений(СписокРаспоряжений, СписокОшибок) 
	...
	ШаблонОшибки = НСтр("ru='По распоряжению %1 оформление документа ""Приобретения товаров"" не требуется.';uk='За розпорядженням %1 оформлення документа ""Придбання товарів"" не потрібно.'");
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если ПакетныйРежим И ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
			...
			ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно перезаполнить приобретение вместе с другими распоряжениями. Выберите распоряжение отдельно.';uk='Для розпорядження %1 неможливо перезаповнита придбання разом з іншими розпорядженнями. Виберіть розпорядження окремо.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
