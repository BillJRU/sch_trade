...
Процедура СкладПриИзмененииСервер(ТекстОшибки = "")
	...
	Если СкладЗаполнен Тогда
		...
		Если ВыбранаГруппа
			...
			Склад = СкладБыл;
			ТекстОшибки = НСтр("ru='Выбранная группа не содержит складов.';uk='Вибрана група не містить складів.'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПеренестиСтроки(ИДСтрокиНазначения = Неопределено)
	...
	Иначе
		...
		Режим = Новый СписокЗначений;
		Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Создать';uk='Створити'"));
		Режим.Добавить(КодВозвратаДиалога.Отмена);
...
Функция ПроверитьСообщитьЗаданиеИзменено(СтрокаЗадание)
	...
	Если Не НовоеЗадание Тогда
		...
		Если ВерсияДанныхДокумента <> СтрокаЗадание.ВерсияДанных Тогда
			Текст = НСтр("ru='Не удалось внести изменения в %Задание%, т.к. версия документа отличается от ожидаемой. Рекомендуется обновить список.';uk='Не вдалося внести зміни в %Задание%, тому що версія документа відрізняється від очікуваної. Рекомендується оновити список.'");
...
Процедура ЗаполнитьСкладыПунктовИЗаданий(УчестьМодифицированность = Ложь);
	...
	Для Каждого СтрокаЗадание Из ЗаданияНаПеревозкуПланируемые.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаПункт Из СтрокаЗадание.ПолучитьЭлементы() Цикл
			...
			Иначе
				Текст = НСтр("ru='%КоличествоСкладов% погрузки';uk='%КоличествоСкладов% завантаження'");
...
Процедура ЗаданияНаПеревозкуПланируемыеПриАктивизацииСтрокиОбработчикОжидания()
	...
	Иначе
		...
		Текст = НСтр("ru='Распоряжения (%количество%)';uk='Розпорядження (%количество%)'");
...
Процедура НастроитьФормуПоВидуДоставки()
	...
	Если ВидДоставки = Перечисления.ВидыДоставки.СоСклада Тогда
		...
		Элементы.РаспоряженияНаДоставкуВремя.Заголовок                             = НСтр("ru='Время доставки (желаемое)';uk='Час доставки (бажаний)'");
	...
	Для каждого ИДСтроки Из МассивИДВыбранныхСтрок Цикл
		...
		Для Каждого СтрокаРаспоряжений Из КоллекцияПодчиненныхСтрок Цикл
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Не удалось исключить %Распоряжение% из доставки.';uk='Неможливо виключити %Распоряжение% з доставки.'");
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Не удалось исключить распоряжение из доставки.';uk='Неможливо виключити розпорядження з доставки.'",КодОсновногоЯзыка),
...
Процедура ПересчитатьИтогиПоРаспоряжениямНаДоставку()
	...
	Шаблон = НСтр("ru='%Количество% %ЕдиницаИзмерения%';uk='%Количество% %ЕдиницаИзмерения%'");
	...
	Если ТекущаяСтрока <> Неопределено
		...
		Если ТекущаяСтрока.Картинка = 4 Тогда
			...
			Текст = НСтр("ru='Особые условия перевозки.';uk='Особливі умови перевезення.'");
			
		ИначеЕсли ТекущаяСтрока.Картинка = 2 Тогда
			
			Картинка = БиблиотекаКартинок.Вопрос;
			Текст = НСтр("ru='Не указан способ доставки.';uk='Не вказаний спосіб доставки.'");
			
		ИначеЕсли ТекущаяСтрока.Картинка = 5 Тогда
			
			Картинка = БиблиотекаКартинок.ВопросСПредупреждением;
			Текст = НСтр("ru='Не указан способ доставки. Особые условия перевозки.';uk='Не вказаний спосіб доставки. Особливі умови перевезення.'");
			
		ИначеЕсли ТекущаяСтрока.Картинка = 0 Тогда
			
			Картинка = БиблиотекаКартинок.ЗаданиеВыполняетПеревозчик;
			Текст = НСтр("ru='Везет перевозчик.';uk='Везе перевізник.'");
			
		ИначеЕсли ТекущаяСтрока.Картинка = 3 Тогда
			
			Картинка = БиблиотекаКартинок.ЗаданиеВыполняетПеревозчикОсобыеУсловия;
			Текст = НСтр("ru='Везет перевозчик. Особые условия перевозки.';uk='Везе перевізник. Особливі умови перевезення.'");
