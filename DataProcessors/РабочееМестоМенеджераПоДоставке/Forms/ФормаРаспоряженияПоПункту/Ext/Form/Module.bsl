...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РейсПункт.Количество() > 0 Тогда
		РейсПункт.Вставить(0, Новый ФорматированнаяСтрока(НСтр("ru='Рейс:';uk='Рейс:'") + " ", Новый Шрифт(,,Истина)));
	КонецЕсли;
	...
	Если Параметры.Свойство("Адрес")
		...
		КонецЕсли;
		РейсПункт.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Пункт:';uk='Пункт:'") + " ", Новый Шрифт(,,Истина)));
		РейсПункт.Добавить(Параметры.Адрес);
		Если ЗначениеЗаполнено(Параметры.ВремяС) Тогда
			ШаблонВремени = НСтр("ru='с %ВремяС% по %ВремяПо%';uk='з %ВремяС% по %ВремяПо%'");
	...
	ЗаголовокВес = НСтр("ru='Вес,';uk='Вага,'") + " " + ЕдиницаИзмеренияВеса;
	ЗаголовокОбъем = НСтр("ru='Объем,';uk='Об''єм,'") + " " + ЕдиницаИзмеренияОбъема;
