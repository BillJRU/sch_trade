...
Процедура ОчиститьНесуществующиеСсылки()
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru='Выполняется очистка реестра документов от несуществующих ссылок.';uk='Виконується очистука рееєстру документів від неіснуючих посилань.'");
...
Функция ОчиститьНесуществующиеСсылкиНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Очистка несуществующих ссылок в реестре документов';uk='Очищення неіснуючих посилань в реєстрі документів'");
...
Процедура ОчисткаНесуществующихСсылокЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru='Завершена очистка несуществующих ссылок. Удалено записей: %1.';uk='Завершено очищення неіснуючих посилань. Вилучено записів: %1.'");
...
Процедура ПереотразитьДокументыВРеестре()
	...
	ПараметрыОжидания  = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ТекстСообщения = НСтр("ru='Выполняется отражение документов в реестре.';uk='Виконується відображення документів в реєстрі.'");
	ПараметрыОжидания.ВыводитьОкноОжидания = Истина;
...
Функция ПереотразитьДокументыВРеестреНаСервере()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Переотражение документов в реестре документов';uk='Перевідобаження документів в реєстрі документів'");
...
Процедура ПереотражениеДокументовВРеестреЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Отражение документов в реестре завершено.';uk='Відображення документів у реєстрі завершено.'");
		Сообщение.Сообщить();
	КонецЕсли;
