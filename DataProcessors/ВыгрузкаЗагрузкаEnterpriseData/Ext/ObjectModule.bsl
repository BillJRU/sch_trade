...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "") 
	...
	КонецЕсли;
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ru='Перенос данных через буфер обмена';uk='Перенесення даних через буфер обміну'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если МенеджерОбменаВнутренний Тогда
		...
		Если КомпонентыОбмена.МенеджерОбмена = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не поддерживается версия формата обмена: <%1>.';uk='Не підтримується версія формату обміну: <%1>.'"), ТекВерсияФормата);
		КонецЕсли;
	КонецЕсли;
...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если РезультатВыгрузки.ЕстьОшибки Тогда
		ТекстСообщения = НСтр("ru='В ходе выполнения операции возникли ошибки';uk='В ході виконання операції виникли помилки'") + ": "
			+ Символы.ПС + РезультатВыгрузки.ТекстОшибки
			+ Символы.ПС + НСтр("ru='Данные могут быть выгружены не полностью.';uk='Дані можуть бути вивантажені не повністю.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	ИначеЕсли Не РезультатВыгрузки.ЕстьВыгруженныеОбъекты Тогда
		ТекстСообщения = НСтр("ru='Не найдено ни одного объекта к выгрузке.';uk='Не знайдено жодного об''єкта для вивантаження.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция ЗагрузитьДанныеИзXML(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		РезультатЗагрузки.ТекстОшибки = НСтр("ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';uk='Помилка читання повідомлення завантаження. Невірний формат повідомлення.'");
...
Процедура ЗаполнениеПравилВыгрузки() Экспорт
	...
	УзелДереваСправочники.Наименование = НСтр("ru='Справочники';uk='Довідники'");
	
	УзелДереваДокументы = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваДокументы.ЭтоГруппа = Истина;
	УзелДереваДокументы.Наименование = НСтр("ru='Документы';uk='Документи'");

	УзелДереваПВХ = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваПВХ.ЭтоГруппа = Истина;
	УзелДереваПВХ.Наименование = НСтр("ru='Планы видов характеристик';uk='Плани видів характеристик'");
	...
	Для Каждого СтрокаПОД Из КомпонентыОбмена.ПравилаОбработкиДанных Цикл
		...
		Если Метаданные.Справочники.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ru='Справочник %1';uk='Довідник %1'");
		ИначеЕсли Метаданные.Документы.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ru='Документ %1';uk='Документ %1'");
		ИначеЕсли  Метаданные.ПланыВидовХарактеристик.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ru='План видов характеристик %1';uk='План видів характеристик %1'");
		Иначе
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректное имя метаданных ""%1""';uk='Некоректне ім''я метаданих ""%1""'"),
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неканоническое представление версии формата обмена: <%1>.';uk='Неканонічне представлення версії формату обміну: <%1>.'"), Результат.Версия);
	КонецЕсли;
