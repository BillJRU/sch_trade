...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='В информационной базе не поддерживается синхронизация данных через универсальный формат';uk='В інформаційній базі не підтримується синхронізація даних через універсальний формат'");
	КонецПопытки;
	...
	Если ВариантОткрытияФормы = "ТолькоЗагрузка" Тогда
		ЭтотОбъект.Заголовок = НСтр("ru='Загрузка данных EnterpriseData';uk='Завантаження даних EnterpriseData'");
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Истина;
	Иначе
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Ложь;
		ЭтотОбъект.Заголовок = НСтр("ru='Выгрузка и загрузка данных EnterpriseData';uk='Вивантаження і завантаження даних EnterpriseData'");
	КонецЕсли;
	...
	Если МассивДоступныхВерсий.Количество() = 0 Тогда
		СтрокаПоддерживаемыеВерсииФормата = НСтр("ru='Не обнаружены поддерживаемые версии формата';uk='Не виявлені підтримувані версії формату'");
	...
	Иначе
		...
		СтрокаПоддерживаемыеВерсииФормата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Поддерживаемые версии формата: %1';uk='Підтримувані версії формату: %1'"), ПредставлениеСпискаВерсий);
	...
	Если ВидОперации = "Загрузка" Тогда
		Если ИсточникЗагрузки = 1 Тогда // текстовое поле
			Если Не ЗначениеЗаполнено(ДанныеДляЗагрузкиXDTO) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Текстовое поле с данными для загрузки не заполнено.';uk='Текстове поле з даними для завантаження не заповнено.'"));
				Возврат;
			КонецЕсли;
		Иначе
...
Процедура ПрерватьВыгрузкуЗагрузку(Команда)
	...
	Сообщение.Текст = НСтр("ru='Выполнение операции прервано.';uk='Виконання операції перервано.'");
	Сообщение.Сообщить();
...
Процедура СохранитьНастройки(Команда)
	...
	ПолучитьФайл(АдресФайлаВХранилище, НСтр("ru='Файл настроек.xml';uk='Файл настройок.xml'"));
КонецПроцедуры
...
Процедура ЗагрузитьСообщение()
	...
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("ru='Выполняется загрузка данных...';uk='Виконується завантаження даних...'");
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Загрузка данных завершена неудачно.';uk='Завантаження даних завершена невдало.'");
...
Процедура ВыгрузитьДанные()
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("ru='Выполняется выгрузка данных...';uk='Виконується вивантаження даних...'");
	Элементы.ВыгрузкаЗагрузка.ТекущаяСтраница = Элементы.Ожидание;
	УстановитьВидимостьДоступностьКнопок(Ложь);
	МассивСообщений = Неопределено;
...
Процедура ПриЗавершенииОперацииВыгрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Выгрузка данных завершена неудачно. Отсутствует результат выгрузки.';uk='Вивантаження даних завершена невдало. Відсутній результат вивантаження.'");
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ВызватьИсключение Результат.КраткоеПредставлениеОшибки;
	КонецЕсли;
...
Процедура ПодключитьРасширениеРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru='Для продолжения необходимо установить расширение для работы с BAF.';uk='Для продовження необхідно встановити розширення для роботи з BAF.'"));
		Возврат;
	КонецЕсли;
	
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогОткрытияФайла.Фильтр = НСтр("ru='Данные загрузки';uk='Дані завантаження'")+ "(*.xml)|*.xml";
...
Процедура ВыбратьФайлДляВыгрузкиНаКлиенте(ВыгружатьПослеВыбора = Ложь, АдресХраненияРезультата = "")
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ru='Данные выгрузки';uk='Дані вивантаження'")+ "(*.xml)|*.xml";
...
Функция ВыгрузитьДанныеНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Выгрузка данных EnterpriseData';uk='Вивантаження даних EnterpriseData'");
...
Функция ЗагрузитьДанныеНаСервере()
	...
	МодульДлительныеОперации = ОбщегоНазначения.ОбщийМодуль("ДлительныеОперации");
	ПараметрыВыполнения = МодульДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка данных EnterpriseData';uk='Завантаження даних EnterpriseData'");
	РезультатФоновогоЗадания = МодульДлительныеОперации.ВыполнитьВФоне(ВыполняемыйМетод, ПараметрыЗадания, ПараметрыВыполнения);
	Возврат РезультатФоновогоЗадания;
	...
	Для Каждого СтрГруппыМД Из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрМД Из СтрГруппыМД.ПолучитьЭлементы() Цикл
			...
			ИначеЕсли СтрокиДополнения.Количество() = 0 Тогда
				...
				НовСтрока.ОтборСтрокой = НСтр("ru='Все объекты';uk='Всі об''єкти'");
...
Процедура НачалоВыбораМодуляМенеджера(РеквизитМодульМенеджера, СтандартнаяОбработка, ОбновлятьВыгрузку)
	...
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогОткрытияФайла.Фильтр = НСтр("ru='Менеджер обмена (*.epf)';uk='Менеджер обміну (*.epf)'") + "|*.epf" ;
	ОповещениеВыбора = Новый ОписаниеОповещения("ВыбранФайл", ЭтотОбъект, 
...
Процедура СообщитьОбОкончанииОперации(Загрузка = Ложь)
	...
	Сообщение = Новый СообщениеПользователю();
	Если Загрузка Тогда
		Сообщение.Текст = НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'");
	Иначе
		Сообщение.Текст = НСтр("ru='Выгрузка данных завершена.';uk='Вивантаження даних завершено.'");
	КонецЕсли;
	Сообщение.Сообщить();
КонецПроцедуры
