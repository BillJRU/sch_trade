...
Процедура ПриНачалеПолученияВариантовНастроекОбменаДанными(УникальныйИдентификатор, ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ОбменДаннымиСВнешнимиСистемами") Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Получение доступных вариантов настроек обмена данными с внешними системами.';uk='Отримання доступних варіантів настройок обміну даними з зовнішніми системами.'");
...
Функция КлючФоновогоЗадания(ИмяПланаОбмена, Действие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='План обмена: %1, действие: %2';uk='План обміну: %1, дія: %2'"), ИмяПланаОбмена, Действие);
	...
	Если НастройкиУдаления.УдалитьНастройкуВКорреспонденте Тогда
		...
		Если Не Результат.НастройкаУдаленаВКорреспонденте Тогда
			...
			Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='При вилученні настройки синхронізації у програмі ""%1"" виникла помилка: %2.
                |
                |Спробуйте виконати вилучення пізніше, або зніміть прапорець ""Вилучити настройку також в програмі..."".'"),
...
Функция НастройкиXDTOКорреспондентаИзXML(ИмяФайлаСтрокаXML, ЭтоФайл, УзелОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Идентификатор отправителя <%1> в файле с настройками EnterpriseData не соответствует ожидаемому формату GUID.';uk='Ідентифікатор відправника <%1> в файлі з настройками EnterpriseData не відповідає очікуваному формату GUID.'"),
			УИД);
	КонецПопытки;
