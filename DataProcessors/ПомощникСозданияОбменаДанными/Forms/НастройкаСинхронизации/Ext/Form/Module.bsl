...
Процедура ПолучитьПодтверждениеПодключения(Команда)
	...
	Если Не ОбменДаннымиСВнешнейСистемой
		...
		ПоказатьПредупреждение(, НСтр("ru='Подключение подтверждено.';uk='Підключення підтверджено.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьПравилаОтправкиИПолученияДанныхПродолжение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если НастройкаВыполнена Тогда
		ПоказатьПредупреждение(, НСтр("ru='Настройка правил отправки и получения данных выполнена.';uk='Настройка правил відправки та отримання даних виконана.'"));
...
Процедура УстановитьНачальноеОтображениеЭлементовФормы()
	...
	Если РезервноеКопирование Тогда
		Элементы.ДекорацияРезервноеКопированиеНадпись.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='Перед началом настройки новой синхронизации данных рекомендуется <a href=""%1"">создать резервную копию данных</a>.';uk='Перед початком настройки нової синхронізації даних рекомендується <a href=""%1"">створити резервну копію даних</a> .'"),
			НавигационнаяСсылкаОбработкиРезервногоКопирования);
	КонецЕсли;
