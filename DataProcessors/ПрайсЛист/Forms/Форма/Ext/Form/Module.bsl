...
Процедура ВыгрузитьВExcel(Команда)
	...
	Если ДеревоЦен.ПолучитьЭлементы().Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В прайс-листе отсутствуют позиции номенклатуры';uk='У прайс-листі відсутні позиції номенклатури'"),,"ДеревоЦен");
		Возврат;
	КонецЕсли;
