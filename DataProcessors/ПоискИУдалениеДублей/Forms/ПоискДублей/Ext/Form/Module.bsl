	...
	Если ТекущаяСтрока.ПолучитьРодителя() = Неопределено Тогда
		...
		Если ПомеченыНаУдалениеИНеИспользуются Тогда
			Описание = Описание + Символы.ПС
				+ НСтр("ru='Однако все они помечены на удаление и не используются, поэтому удаление дублей уже не требуется.';uk='Однак всі вони відмічені на вилучення і не використовуються, тому видалення дублів вже не потрібно.'");
		КонецЕсли;
				
		Элементы.ГруппаПодробно.Заголовок = НСтр("ru='Сведения:';uk='Відомості:'");
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = 
			НСтр("ru='Выбранный дубль не заменен из-за невозможности замены других дублей.';uk='Обраний дубль не замінено через неможливість заміни інших дублів.'");
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
...
Процедура Подробно(Команда)
	...
	Элементы.Подробнее.Заголовок = ?(Элементы.ГруппаПодробно.Видимость, НСтр("ru='<< Скрыть';uk='<< Приховати'"), НСтр("ru='Подробнее >>';uk='Детальніше >>'"));
КонецПроцедуры
...
Процедура ПриАктивацииШагаМастера()
	...
	Если ТекущаяСтраница = Элементы.ШагПоискНеВыполнялся Тогда
		...
		Для Каждого Правило Из ПравилаПоиска Цикл
			...
			ИначеЕсли Правило.Правило = "Подобно" Тогда
				Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru='совпадает по похожим словам';uk='збігається за схожими словами'");
			Иначе
				Продолжить;
			КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагДублейНеНайдено Тогда
		...
		Если ПустаяСтрока(ОписаниеОшибкиПоискаДублей) Тогда
			Сообщение = НСтр("ru='Не обнаружено дублей по указанным параметрам.';uk='Не виявлено дублів за вказаними параметрами.'");
		Иначе	
			Сообщение = ОписаниеОшибкиПоискаДублей;
		КонецЕсли;	
		Элементы.ДублейНеНайдено.ОтображениеСостояния.Текст = Сообщение + Символы.ПС 
			+ НСтр("ru='Измените условия и нажмите ""Найти дубли""';uk='Змініть умови і натисніть ""Знайти дублі""'");
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей()
	...
	Если ПустаяСтрока(ОписаниеОшибкиПоискаДублей) Тогда
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выбрано дублей: %1 из %2.';uk='Обрано дублів: %1 з %2.'"),
	...
	Если СпособУдаления = "Пометка" Тогда
		ТекстПодсказки = НСтр("ru='Выбранные элементы будут <a href = ""СпособУдаления"">помечены на удаление</a> и заменены на оригиналы (отмечены стрелкой).';uk='Вибрані елементи будуть <a href = ""СпособУдаления"">позначені на видалення</a> і замінені на оригінали (відзначені стрілкою).'");
	Иначе
		ТекстПодсказки = НСтр("ru='Выбранные элементы будут <a href = ""СпособУдаления"">безвозвратно удалены</a> и заменены на оригиналы (отмечены стрелкой).';uk='Вибрані елементи будуть <a href = ""СпособУдаления"">безповоротно видалені</a> і замінені на оригінали (відзначені стрілкою).'");
	КонецЕсли;
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ru='Совпадает по похожим словам';uk='Збігається за схожими словами'"));
КонецПроцедуры
...
Функция НайтиИУдалитьДубли()
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
		...
		Элементы.ВыполнениеПоиска.ОтображениеСостояния.Текст = НСтр("ru='Поиск дублей...';uk='Пошук дублів...'");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		Если ПарыЗаменДублей.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Необходимо выбрать хотя бы одну группу дублей.';uk='Необхідно вибрати хоча б одну групу дублів.'"),, "НайденныеДубли");
			Возврат Неопределено;
		КонецЕсли;
		
		Элементы.ВыполнениеУдаления.ОтображениеСостояния.Текст = НСтр("ru='Удаление дублей...';uk='Вилучення дублів...'");
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректная операция в НайтиИУдалитьДубли.';uk='Некоректна операція в НайтиИУдалитьДубли.'");
	КонецЕсли;
...
Процедура НайтиИУдалитьДублиПрогресс(Прогресс, ДополнительныеПараметры) Экспорт
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
		...
		Сообщение = НСтр("ru='Поиск дублей...';uk='Пошук дублів...'");
		Если Прогресс.Прогресс.Текст = "РассчитыватьМестаИспользования" Тогда 
			Сообщение = НСтр("ru='Выполняется расчет мест использования дублей...';uk='Виконується розрахунок місць використання дублів ...'");
		ИначеЕсли Прогресс.Прогресс.Процент > 0 Тогда
			Сообщение = Сообщение + " " 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='(найдено %1)';uk='(знайдено %1)'"), Прогресс.Прогресс.Процент);
		КонецЕсли;
		Элементы.ВыполнениеПоиска.ОтображениеСостояния.Текст = Сообщение;
		
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		
		Сообщение = НСтр("ru='Удаление дублей...';uk='Вилучення дублів...'");
		Если Прогресс.Прогресс.Процент > 0 Тогда
			Сообщение = Сообщение + " " 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='(удалено %1 из %2)';uk='(видалено %1 з %2)'"), Прогресс.Прогресс.Процент, ВсегоНайденоДублей);
		КонецЕсли;
...
Процедура НайтиИУдалитьДублиЗавершение(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус <> "Выполнено" Тогда
		...
		Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
			Кратко = НСтр("ru='Не удалость найти дубли по причине:';uk='Не вдалося знайти дублі через:'");
		ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
			Кратко = НСтр("ru='Удаление дублей не выполнено по причине:';uk='Вилучення дублів не виконано через:'");
		КонецЕсли;
