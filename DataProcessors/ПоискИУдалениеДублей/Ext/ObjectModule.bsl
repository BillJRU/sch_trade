...
Функция МенеджерОбластиПоискаДублей(Знач ИмяОбластиПоискаДанных) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Некорректный тип объекта метаданных ""%1""';uk='Некоректний тип об''єкта метаданих ""%1""'"), ИмяОбластиПоискаДанных);
КонецФункции
	...
	Для Каждого ИмяПоля Из ИменаПолейВВыборе Цикл
		...
		Если ДоступноеПолеКД = Неопределено Тогда
			ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле ""%1"" не существует.';uk='Поле ""%1"" не існує.'"), Строка(ПолеКД)));
			Продолжить;
		КонецЕсли;
	...
	Пока СледующийЭлементВыборки(ВыборкаЭталонныхОбъектов) Цикл
		...
		Если ПоляДляСравненияНаПодобие.Количество() > 0 Тогда
			...
			Если НечеткийПоиск = Неопределено Тогда
				Результат.ОписаниеОшибки = 
					НСтр("ru='Не удалось подключить компоненту нечеткого поиска. Подробнее см. в журнале регистрации.';uk='Не вдалося підключити компоненту нечіткого пошуку. Детальніше див. у журналі реєстрації.'");
				Возврат Результат;
			КонецЕсли;
			Для Каждого ПолеТаблицы Из ПоляДляСравненияНаПодобие Цикл
				ИмяПоля = ПолеТаблицы.Ключ;
		...
		Если РазмерВозвращаемойПорции > 0 И (ТаблицаДублей.Количество() > РазмерВозвращаемойПорции) Тогда
			...
			Если Найдено > 0 И ТаблицаДублей.Количество() = 0 Тогда
				Результат.ОписаниеОшибки = НСтр("ru='Найдено слишком много дублей одного элемента.';uk='Знайдено занадто багато дублів одного елемента.'");
			Иначе
				Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Найдено слишком много дублей. Показаны только первые %1.';uk='Знайдено занадто багато дублів. Показані тільки перші %1.'"), РазмерВозвращаемойПорции);
			КонецЕсли;
			Прервать;
		КонецЕсли;
	КонецЦикла;
