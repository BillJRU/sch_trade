...
Процедура СоздатьДокументыПередачи(Параметры, АдресВременногоХранилища) Экспорт
	...
	|	ТаблицаНоменклатурыСОтрицательнымиОстатками.НалоговоеНазначениеВидаЗапасов КАК НалоговоеНазначениеВидаЗапасов,
	...
	Если ДействиеСЦенамиПередачи = "ИсключитьТоварыБезЦен" Тогда
		...
		|	ВтОтрицательныеОстаткиДляЗапроса.НалоговоеНазначениеВидаЗапасов КАК НалоговоеНазначениеВидаЗапасов,
...
Функция СоздатьДокументыПоПериоду(ТекущийПериод, ТаблицаНоменклатурыСОтрицательнымиОстатками, МенеджерВременныхТаблиц, ПараметрыСоздания, СозданныеДокументы)
	...
	Пока ВыборкаПоОтрицательнымОстаткам.Следующий() Цикл
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(ПоляШапки, ВыборкаПоОтрицательнымОстаткам) Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				ИначеЕсли ХозяйственныеОперацииВыкупа().Найти(ВыборкаПоОтрицательнымОстаткам.ХозяйственнаяОперация) <> Неопределено Тогда
					...
					ЭтоПередача = Ложь;
				Иначе
					ТекстИсключения = НСтр("ru='По хозяйственной операции %ХозяйственнаяОперация% оформление документов не поддерживается.';uk='За господарською операцією %ХозяйственнаяОперация% оформлення документів не підтримується.'");
		...
		Если Не ЗначениеЗаполнено(ВыборкаПоОтрицательнымОстаткам.ВидЗапасовОтправителя) Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru='При определении источников передачи товаров не удалось определить вид запасов. Обратитесь к разработчикам.';uk='При визначенні джерел передачі товарів не вдалося визначити вид запасів. Зверніться до розробників.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		Иначе
...
Функция ПараметрыЗапросаОтрицательныхОстатков(Параметры)
	...
	ТаблицаКолонокОтключаемыхПоФО.Колонки.Добавить("НомерГТД", Новый ОписаниеТипов("СправочникСсылка.НоменклатураГТД"));
...
Процедура ПометитьНаУдалениеСозданныеДокументы(Параметры, АдресВременногоХранилища) Экспорт
	...
	Пока Индекс >0 Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации( НСтр("ru='Пакетное формирование документов';uk='Пакетне формування документів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ЗаголовокИФормируемыеДокументыПоКлючуНазначенияИспользования(КлючНазначенияИспользования) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки")
		...
		Заголовок = НСтр("ru='Помощник формирования передач и выкупов';uk='Помічник формування передач і викупів'");
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьПередачиТоваровМеждуОрганизациями")  Тогда
		...
		Если Не ЗначениеЗаполнено(КлючНазначенияИспользования)
			...
			ФормируемыеДокументы = "ТолькоПередачиМеждуОрганизациями";
			Заголовок = НСтр("ru='Помощник формирования передач';uk='Помічник формування передач'");
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
		...
		Если Не ЗначениеЗаполнено(КлючНазначенияИспользования)
			...
			ФормируемыеДокументы = "ТолькоВыкупыТоваровНаХранении";
			Заголовок = НСтр("ru='Помощник формирования выкупов';uk='Помічник формування викупів'");
