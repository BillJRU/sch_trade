...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ФормируемыеДокументы) Тогда
		ТекстИсключения = НСтр("ru='Использовать обработку с такими настройками функциональных опций не требуется.';uk='Використовувати обробку з такими настройками функціональних опцій не потрібно.'");
...
Процедура ПериодАнализаПриИзменении(Элемент)
	Если Не ЗначениеЗаполнено(ПериодАнализа.ДатаНачала)
		...
		ТекстСообщения = НСтр("ru='Даты начала и окончания периода анализа должны быть заполнены.';uk='Дати початку і закінчення періоду аналізу повинні бути заповнені.'");
...
Процедура ПерейтиКНастройкамФормирования(Команда)
	...
	Если ПроблемныеОрганизации.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Нет организаций, по которым требуется оформлять документы с указанными настройками анализа.';uk='Немає організацій, по яких потрібно оформляти документи зі вказаними настройками аналізу.'");
		ПоказатьПредупреждение(, ТекстСообщения);
	ИначеЕсли Элементы.ПроблемныеОрганизации.ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Выберите организацию, по которой нужно оформлять документы.';uk='Виберіть організацію, по якій потрібно оформляти документи.'");
		ПоказатьПредупреждение(, ТекстСообщения);
	Иначе
...
Процедура ПометитьНаУдалениеДокументы(Команда)
	ТекстВопроса = НСтр("ru='При возврате к настройкам созданные документы будут помечены на удаление. Продожить?';uk='При поверненні до настройок створені документи будуть позначені на вилучення. Продовжити?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Если Не ИспользоватьДоговорыМеждуОрганизациями Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<для выкупов>';uk='<для викупів>'"));
...
Функция СоздатьДокументыПередачиСервер(ДействиеСЦенамиПередачи, СохранятьОтправителей)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Формирование документов передачи товаров между организациями и выкупов';uk='Формування документів передачі товарів між організаціями і викупів'");
...
Процедура ПослеСозданияДокументовПередачи(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание <> Неопределено 
		...
		ИначеЕсли СозданныеДокументы.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru='Не удалось создать документы с указанными настройками формирования.';uk='Не вдалося створити документи із зазначеними настройками формування.'");
			ПоказатьПредупреждение(,ТекстСообщения);
	...
	Иначе
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить формирование документов по причине: %Причина%';uk='Не вдалося закінчити формування документів з причини: %Причина%'");
...
Функция ЗаполнитьПроблемныеОрганизацииСервер()
	...
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Заполнение списка проблемных организаций';uk='Заповнення списку проблемних організацій'");
...
Процедура ПослеЗаполненияПроблемныхОрганизации(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Элементы.ПроблемныеОрганизации.Доступность                    = Ложь;
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить заполнения списка организаций с отрицательными остатками по причине: %Причина%';uk='Не вдалося закінчити заповнення списку організацій з від''ємними залишками з причини: %Причина%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Причина%",  ФоновоеЗадание.ПодробноеПредставлениеОшибки);
			ПоказатьПредупреждение(, ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Функция ПометитьНаУдалениеСозданныеДокументыСервер()
	...
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Пометка на удаление созданных документов';uk='Позначка на вилучення створених документів'");
	ПараметрыВыполненияВФоне.ЗапуститьВФоне = Истина;
...
Процедура ПослеПометкиНаУдалениеСозданныхДокументов(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание <> Неопределено 
		...
		Если СозданныеДокументы.Количество() > 0 Тогда
			ОписаниеОшибки = НСтр("ru='Показанные в таблице документы не удалось пометить на удаление.';uk='Наведені в таблиці документи не вдалося помітити на вилучення.'");
			ПоказатьПредупреждение(, ОписаниеОшибки);
...
Функция ОбновитьСозданныеДокументыСервер()
	...
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Обновление списка созданных документов';uk='Оновлення списку створених документів'");
...
Процедура ПослеОбновленияСозданныхДокументов(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить обновление списка созданных документов по причине: %Причина%';uk='Не вдалося закінчити оновлення списку створених документів з причини: %Причина%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Причина%",  ФоновоеЗадание.ПодробноеПредставлениеОшибки);
			ПоказатьПредупреждение(, ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьКомитентовСервер()
	...
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Обновление списка организаций-коминетов';uk='Оновлення списку організацій-комінетів'");
...
Процедура ПослеЗаполниенияКомитентов(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить заполнения списка организаций-кометентов по причине: %Причина%';uk='Не вдалося закінчити заповнення списку організацій-кометентов з причини: %Причина%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Причина%",  ФоновоеЗадание.ПодробноеПредставлениеОшибки);
			ПоказатьПредупреждение(, ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьОтправителейСервер(ИмяТаблицы)
	...
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Обновление отправителей товаров';uk='Оновлення відправників товарів'");
...
Процедура ПослеЗаполненияОтправителей(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если ФоновоеЗадание <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось закончить заполнения списка отправителей по причине: %Причина%';uk='Не вдалося закінчити заповнення списку відправників з причини: %Причина%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Причина%",  ФоновоеЗадание.ПодробноеПредставлениеОшибки);
			ПоказатьПредупреждение(, ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьПредставлениеОтбораНоменклатуры()
	...
	МассивСтрок.Добавить(НСтр("ru='Отбор номенклатуры:';uk='Відбір номенклатури:'") + " ");
...
Процедура ЗагрузитьРезультатыВыполненияФоновыхЗаданий(АдресХранилища)
	...
	Для Каждого СтрТабл Из Результат Цикл
		...
		Если Не НайденРеквизитФормы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru='Неизвестное имя таблицы %ИмяТаблицы%';uk='Невідоме ім''я таблиці %ИмяТаблицы%'");
...
Функция НастроитьСтраницуНастройки()
	...
	Если ТипЗапасов = Перечисления.ТипыЗапасов.Товар Тогда
		...
		Если ВариантВыбораОтправителей = "ПоНастройкамПередачи" Тогда
			ТекстПояснения =
			НСтр("ru='По организации-получателю будут найдены отрицательные остатки на конец каждого %ПредставлениеПереодичностиФормирования% из указанного периода. Затем будут проанализированы остатки владельцев товаров и %ПредставлениеФормируемыхДокументов%. Сначала будут списываться товары, по которым сформированы резервы товаров организаций.';uk='По організації-одержувачу будуть знайдені від''ємні залишки на кінець кожного %ПредставлениеПереодичностиФормирования% із зазначеного періоду. Потім будуть проаналізовані залишки власників товарів і %ПредставлениеФормируемыхДокументов%. Спочатку будуть списуватися товари, по яких сформовані резерви товарів організацій.'");
		Иначе
			ТекстПояснения =
			НСтр("ru='По организации-владельцу будут найдены отрицательные остатки на конец каждого %ПредставлениеПереодичностиФормирования% из указанного периода. Затем будут проанализированы остатки владельцев товаров в порядке их приоритета и %ПредставлениеФормируемыхДокументов%.';uk='По організації-власнику будуть знайдені від''ємні залишки на кінець кожного %ПредставлениеПереодичностиФормирования% із зазначеного періоду. Потім будуть проаналізовані залишки власників товарів в порядку їх пріоритету та %ПредставлениеФормируемыхДокументов%.'");
		КонецЕсли;
		
		Если ПериодичностьФормированияДокументов = "ПоДням" Тогда
			ПредставлениеПереодичностиФормирования = НСтр("ru='дня';uk='дні'");
		Иначе
			ПредставлениеПереодичностиФормирования = НСтр("ru='месяца';uk='місяці'");
		КонецЕсли;
		...
		Если ОформлятьВыкупы(ФормируемыеДокументы) 
			...
			ТекстПроблемы = НСтр("ru='Недостаточно прав для формирования выкупов товаров на хранении.';uk='Недостатньо прав для формування викупів товарів на зберіганні.'");	
		...
		Если ОформлятьПередачи(ФормируемыеДокументы)
			...
			ТекстПроблемы = НСтр("ru='Недостаточно прав для формирования передач товаров между организациями.';uk='Недостатньо прав для формування передач товарів між організаціями.'");	
			ПроблемыСПравами.Добавить(ТекстПроблемы);
		КонецЕсли;
		...
		Если ОформлятьВыкупы(ФормируемыеДокументы)
			...
			ПредставлениеФормируемыхДокументов = НСтр("ru='сформированы передачи товаров между организациями и выкупы товаров на хранении';uk='сформовані передачі товарів між організаціями і викупи товарів на зберіганні'");
			ТекстКнопкиЗаполнения = НСтр("ru='Заполнить поклажедателями и организациями';uk='Заповнити поклажодавцями і організаціями'");
			ПодсказкаКнопкиЗаполнения = НСтр("ru='Заполнить поклажедателями, по которым есть остатки невыкупленных товаров, и организациями по настройкам передачи';uk='Заповнити поклажодавцями, по яких є залишки невикуплених товарів, і організаціями за настройками передачі'");
			
		ИначеЕсли ОформлятьПередачи(ФормируемыеДокументы) Тогда
			
			ПредставлениеФормируемыхДокументов = НСтр("ru='сформированы передачи товаров между организациями';uk='сформовані передачі товарів між організаціями'");
			ТекстКнопкиЗаполнения = НСтр("ru='Заполнить организациями';uk='Заповнити організаціями'");
			ПодсказкаКнопкиЗаполнения = НСтр("ru='Заполнить организациями по настройкам передачи';uk='Заповнити організаціями за настройкою передачі'");
			
		Иначе
			
			ПредставлениеФормируемыхДокументов = НСтр("ru='сформированы выкупы товаров на хранении';uk='сформовані викупи товарів на зберіганні'");
			ТекстКнопкиЗаполнения = НСтр("ru='Заполнить поклажедателями';uk='Заповнити поклажодавцями'");
			ПодсказкаКнопкиЗаполнения = НСтр("ru='Заполнить поклажедателями, по которым есть остатки невыкупленных товаров';uk='Заповнити поклажодавцями, по яких є залишки невикуплених товарів'");
			
		КонецЕсли;
		
		Если ОформлятьВыкупы(ФормируемыеДокументы) Тогда
			Элементы.ВыбранныеОтправителиДобавить.Заголовок = НСтр("ru='Добавить организацию';uk='Додати організацію'");
		Иначе
			Элементы.ВыбранныеОтправителиДобавить.Заголовок = НСтр("ru='Добавить';uk='Додати'");
		КонецЕсли;
		
		Если ОформлятьПередачи(ФормируемыеДокументы) Тогда
			Элементы.ВыбранныеОтправителиДобавитьПоклажедателя.Заголовок = НСтр("ru='Добавить поклажедателя';uk='Додати поклажодавця'");
		Иначе
			Элементы.ВыбранныеОтправителиДобавитьПоклажедателя.Заголовок = НСтр("ru='Добавить';uk='Додати'");
		КонецЕсли;
		...
		Если ЕстьПроблемаСПравами Тогда
			ТекстПроблемы = НСтр("ru='Обратитесь к администратору для назначения необходимых прав. Продолжить формирование документов с такой настройкой невозможно.';uk='Зверніться до адміністратора для призначення необхідних прав. Продовжити формування документів з такою настройкою неможливо.'");	
			ПроблемыСПравами.Добавить(ТекстПроблемы);
...
Функция ПроверитьОтправителей()
	...
	Если ТипЗапасов = Перечисления.ТипыЗапасов.Товар Тогда
		...
		Если ВариантВыбораОтправителей = "ПоУказаннымПриоритетам" Тогда
			Если Объект.ВыбранныеОтправители.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ru='Выберите отправителей.';uk='Виберіть відправників.'");
			...
			Для Каждого СтрТабл Из Объект.ВыбранныеОтправители Цикл
				Если Не ОформлятьВыкупы(ФормируемыеДокументы) 
					...
					Если Не ЗначениеЗаполнено(СтрТабл.Отправитель) Тогда
						ТекстСообщения = НСтр("ru='Укажите отправителя';uk='Вкажіть відправника'");
					...
					Если Не ЗначениеЗаполнено(СтрТабл.ВидЦен) Тогда
						ТекстСообщения = НСтр("ru='Укажите вид цен передачи';uk='Вкажіть вид цін передачі'");	
						Результат = Ложь;
					...
					Если ИспользоватьДоговорыМеждуОрганизациями
						...
						ТекстСообщения = НСтр("ru='Укажите договор';uk='Зазначте договір'");	
						Результат = Ложь;
					...
					Если Не ЗначениеЗаполнено(СтрТабл.ТипЗапасов) Тогда
						ТекстСообщения = НСтр("ru='Укажите передаваемый тип запасов';uk='Вкажіть тип запасів, що передається'");	
						Результат = Ложь;
		...
		ИначеЕсли ОформлятьПередачи(ФормируемыеДокументы) Тогда
			...
			Для Каждого СтрТабл из ТаблицаНастроекПередачи Цикл
				...
				Если Не ЗначениеЗаполнено(СтрТабл.ВидЦен) Тогда
					ТекстСообщения = НСтр("ru='Укажите вид цен в настройках передачи от организации %Отправитель%';uk='Вкажіть вид цін в налаштуваннях передачі від організації %Отправитель%'");	
				...
				Если ИспользоватьДоговорыМеждуОрганизациями
					И Не ЗначениеЗаполнено(СтрТабл.Договор) Тогда
					ТекстСообщения = НСтр("ru='Укажите договор в настройках передачи от организации %Отправитель%';uk='Вкажіть договір в налаштуваннях передачі від організації %Отправитель%'");	
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Отправитель%", СтрТабл.Отправитель);
					Результат = Ложь;
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
	...
	Иначе	
		...
		Для Каждого СтрТабл Из ОтправителиПоКомиссии Цикл
			...
			Если Не ЗначениеЗаполнено(СтрТабл.ВидЦен) Тогда
				ТекстСообщения = НСтр("ru='Укажите вид цен в настройках передачи';uk='Вкажіть вид цін в настройках передачі'");	
				Результат = Ложь;
			...
			Если ИспользоватьДоговорыМеждуОрганизациями
				И Не ЗначениеЗаполнено(СтрТабл.Договор) Тогда
				ТекстСообщения = НСтр("ru='Укажите договор в настройках передачи';uk='Вкажіть договір в настройках передачі'");	
				Результат = Ложь;
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОтправителиПоКомиссии",
																	ОтправителиПоКомиссии.Индекс(СтрТабл) + 1,
																	"Договор");
...
Процедура ИнициализироватьРежимИсправленияОстатков()
	...
	КонецЦикла;
	Элементы.НастройкиЗаголовок.Заголовок = НСтр("ru='Настройка пакетного формирования документов';uk='Настройка пакетного формування документів'");
	Элементы.РезультатыЗаголовок.Заголовок = НСтр("ru='Созданные документы';uk='Створені документи'");
	Элементы.ПроблемыИнформацияФормируется.Видимость = Ложь;
