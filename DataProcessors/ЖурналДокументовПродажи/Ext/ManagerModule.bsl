...
Процедура СоздатьДокументПродажи(ПоляДокумента, ПараметрыОснования, МассивДокументов, ПараметрыФормирования) Экспорт
	...
	ПечататьПередачуТоваровХранителю = ПараметрыФормирования.ПечататьПередачуТоваровХранителю;
	...
	Если НовыйДокумент.ПроверитьЗаполнение() Тогда
		...
		Если (ПечататьРеализациюТоваровУслуг
			...
			Или ПечататьПередачуТоваровХранителю
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	Если ПоказыватьНакладные
		...
		ТекстГиперссылки = НСтр("ru='Возвраты';uk='Повернення'");
...
Функция ТекстЗапросаЗаказыКОформлению(Организация = Неопределено, Склад = Неопределено)
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&УсловиеОтбораПоПередаче", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&УсловиеОтбораПоВозврату", "");
		
		УсловиеОтбора = "ИЛИ ДокументОтгрузки ССЫЛКА Документ.ПередачаТоваровХранителю";
		ТекстЗапроса  = СтрЗаменить(ТекстЗапроса, "//&УсловиеОтбораПоДокументуПередачи", УсловиеОтбора);
