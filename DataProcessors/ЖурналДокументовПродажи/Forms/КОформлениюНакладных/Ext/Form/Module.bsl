...
Процедура СохранитьНастройки(Настройки)
	...
	ПечататьПередачуТоваровХранителю    = Настройки.ПечататьПередачуТоваровХранителю;
...
Процедура ОткрытьФормуНастройкиПараметров(Оповещение)
	...
	ПараметрыФормы.Вставить("ПечататьПередачуТоваровХранителю",    ПечататьПередачуТоваровХранителю);
...
Функция ПараметрыЖурнала(КлючНазначенияИспользования)
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала",НСтр("ru='Документы продажи';uk='Документи продажу'"));
...
Функция ОформитьНакладнуюНаСервере()
			...
			ИначеЕсли ДокументыПоТипамНакладных.Количество() = 1 Тогда
				...
				Иначе
					...
					Если ЕстьНакладная И Не ЕстьОрдер И Не РежимБезВопросов Тогда
						ТекстОшибки = НСтр("ru='Недоступно оформление по отгрузке, так как не создано ни одного ордера.';uk='Недоступно оформлення за відвантаженням, тому що не створено жодного ордера.'");
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление",ТекстОшибки, Неопределено);
					Иначе
	...
	ИначеЕсли СписокРаспоряжений.Количество() > 1 
		...
		Если ПоОрдерам Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru='по отгрузке';uk='за відвантаженням'"));
		Иначе
...
Функция РазделитьЗаказыПоПолнымИменамНакладных(СписокОшибок)
	...
	Для каждого Строка Из Выгрузка Цикл
		...
		Если ПакетныйРежим
			...
			ТекстОшибки = НСтр("ru='Невозможно перезаполнить накладную %1 вместе с другими распоряжениями. Выберите распоряжение отдельно.';uk='Неможливо перезаповнити накладну %1 разом з іншими розпорядженнями. Виберіть розпорядження окремо.'");
		...
		ИначеЕсли ПоОрдерам Тогда
			...
			Если Строка.СостояниеОрдера = 0 Тогда
				...
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно перезаполнить накладную по отгрузке, т.к. ордера соответствуют накладным.';uk='Для розпорядження %1 неможливо перезаповнити накладну за відвантаженням, тому що ордера відповідають накладними.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
			...
			ИначеЕсли Строка.СостояниеОрдера = 4 Тогда
				...
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить накладную по отгрузке, т.к. склад не использует ордерную схему.';uk='Для розпорядження %1 неможливо оформити накладну за відвантаженням, тому що склад не використовує ордерну схему.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
		...
		Иначе // оформление по заказам
			...
			Если ТипЗнч(Строка.Ссылка)= Тип("ДокументСсылка.РеализацияТоваровУслуг")
				...
				ТекстОшибки = НСтр("ru='Распоряжение %1 явлется накладной. Для накладных оформление по заказам не доступно.';uk='Розпорядження %1 є накладною. Для накладних оформлення за замовленнями недоступно.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ПрерватьОбработкуСтроки = Истина;
...
Процедура ИсключитьЗаказыСНакладными(ДокументыПоТипамНакладных, СписокОшибок)
	...
	Для н = 1 По ИзначальноеКоличество Цикл
		...
		Если Исключить Тогда
			...
			Для Каждого Строка Из Таблица Цикл
				ТекстОшибки = НСтр("ru='Для распоряжения %1 действие не выполнено, так как по выбранным распоряжениям уже имеются накладные.';uk='Для розпорядження %1 дюя не виконано, так як за обраними розпорядженням вже є накладні.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление",ТекстОшибки, Неопределено);
			КонецЦикла;
...
Функция ОформитьРядНакладныхПоТаблицам(СтруктураПараметров)
	...
	НаименованиеЗадания = НСтр("ru='Оформление документов продажи';uk='Оформлення документів продажу'");
...
Функция ОформитьЧерезФормуПерезаполнения(ДокументыПоТипамНакладных, ПоОрдерам, СписокОшибок)
	...
	Иначе
		ОшибкаДоступа = НСтр("ru='Нет прав на добавление/изменение накладной';uk='Немає прав на додавання/зміну накладної'");
...
Функция ТекстЗапросаПоЗначащимРеквизитамРаспоряжений()
	...
	ТекстЗапроса = "
		...
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.ПередачаТоваровХранителю
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ПередачаТоваровХранителю).ЦенаВключаетНДС
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.ЗаказКлиента ТОГДА
...
Функция ПараметрыФормированияДокументов()
	...
	ПараметрыФормирования.Вставить("ПечататьПередачуТоваровХранителю", ПечататьПередачуТоваровХранителю);
