...
Процедура ОформитьНакладныеНаКлиенте(ПоОрдерам)
	...
	Если ПараметрыСозданныхДокументов.Свойство("ОформитьРядНакладных") И ПараметрыСозданныхДокументов.ОформитьРядНакладных Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОформитьНакладныеНаКлиентеЗавершение", ЭтотОбъект);
...
Функция НастройкиФормыПереоформленияНакладных(ИмяНакладной)
	...
	НастройкиФормы.Заголовок = НСтр("ru='Переоформление документов по выбранным распоряжениям';uk='Переоформлення документів за вибраним розпорядженням'");
...
Функция ОформитьНакладнуюНаСервере(ПоОрдерам)
			...
			ИначеЕсли ДокументыПоТипамНакладных.Количество() = 1 Тогда
				...
				Иначе
					...
					Если ЕстьНакладная И Не ЕстьОрдер И Не РежимБезВопросов Тогда
						ТекстОшибки = НСтр("ru='Недоступно оформление по приемке, так как не создано ни одного ордера.';uk='Недоступно оформлення за прийманням, тому що не створено жодного ордера.'");
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление",ТекстОшибки, Неопределено);
					Иначе
	...
	ИначеЕсли СписокРаспоряжений.Количество() > 1
		...
		Если ПоОрдерам Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru='по приемке';uk='за прийманням'"));
		Иначе
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если ПоОрдерам Тогда
			...
			Если Строка.СостояниеОрдера = 0 Тогда
				...
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно перезаполнить накладную по приемке, т.к. ордера соответствуют накладным.';uk='Для розпорядження %1 неможливо перезаповнити накладну за прийманням, тому що ордера відповідають накладними.'");
			...
			ИначеЕсли Строка.СостояниеОрдера = 4 Тогда
				...
				ТекстОшибки = НСтр("ru='Для распоряжения %1 невозможно оформить накладную по приемке, т.к. склад не использует ордерную схему.';uk='Для розпорядження %1 неможливо оформити накладну за прийманням, тому що склад не використовує ордерну схему.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
...
Функция ОформитьРядНакладныхПоТаблицам(СтруктураПараметров)
	...
	НаименованиеЗадания = НСтр("ru='Оформление документов возврата';uk='Оформлення документів повернення'");
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(ВыполняемыйМетод, СтруктураПараметров, ПараметрыВыполнения);
