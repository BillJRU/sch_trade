...
Процедура УстановитьПараметрыВыбораСервер(ПараметрыВыбора, ПараметрыВыбораМассив)
	...
	Для Индекс = 1 По СтрЧислоСтрок(ПараметрыВыбора) Цикл
		...
		Если ИмяТипа = Строка(Тип("Строка")) Тогда
			ИмяТипа = "Строка";
		КонецЕсли;		
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Иначе
		Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
			ШаблонПояснения = НСтр("ru='Изменить реквизит %1 в выбранных элементах';uk='Змінити реквізит %1 в обраних елементах'") // например, "Изменить реквизит ""Склад""..."
		ИначеЕсли ИзменяемыеРеквизиты.Количество() > 3 Тогда
			ШаблонПояснения = НСтр("ru='Изменить реквизиты (%1) в выбранных элементах';uk='Змінити реквізити (%1) в обраних елементах'"); // например, "Изменить реквизиты (5)..."
		ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
			ШаблонПояснения = НСтр("ru='Изменить реквизиты %1 в выбранных элементах';uk='Змінити реквізити %1 в обраних елементах'"); // например, "Изменить реквизиты ""Склад"", ""Офис""..."
		Иначе	
			ШаблонПояснения = "";
		КонецЕсли;
		...
		Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
			...
			Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru='Изменить реквизит %1 в табличной части ""%2""';uk='Змінити реквізит %1 в табличній частині ""%2""'"),
					НСтр("ru='реквизит %1 в табличной части ""%2""';uk='реквізит %1 в табличній частині ""%2""'")); 
			ИначеЕсли ИзменяемыеРеквизиты.Количество() > 3 Тогда
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru='Изменить реквизиты (%1) в табличной части ""%2""';uk='Змінити реквізити (%1) в табличній частині ""%2""'"),
					НСтр("ru='реквизиты (%1) в табличной части ""%2""';uk='реквізити (%1) в табличній частині ""%2""'")); 
			Иначе 
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru='Изменить реквизиты %1 в табличной части ""%2""';uk='Змінити реквізити %1 в табличній частині ""%2""'"),
					НСтр("ru='реквизиты %1 в табличной части ""%2""';uk='реквізити %1 в табличній частині ""%2""'")); 
		...
		Если Не ПустаяСтрока(Пояснение) Тогда
			...
			Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
				Если ЕстьОтборПоСтрокам Тогда 
					Пояснение = Пояснение + " " 
						+ НСтр("ru='Изменения будут выполнены в тех строках выбранных элементов, которые удовлетворяют <a href = ""ПерейтиКНастройкеОтбора"">условиям отбора</a>.';uk='Зміни будуть виконані в тих рядках обраних елементів, які задовольняють <a href = ""ПерейтиКНастройкеОтбора"">умовам відбору</a>.'")
				Иначе
					Пояснение = Пояснение + " " 
						+ НСтр("ru='Изменения будут выполнены <a href = ""ПерейтиКНастройкеОтбора"">во всех строках</a> выбранных элементов.';uk='Зміни будуть виконані <a href = ""ПерейтиКНастройкеОтбора"">у всіх рядках</a> обраних елементів.'")
				КонецЕсли;
			КонецЕсли;
		Иначе
	...
	Если ПустаяСтрока(КодАлгоритма) Тогда
        ...
        |;uk='// Доступні змінні:
        |// %1 - оброблюваний об''єкт'"), "Объект") + Символы.ПС;
	КонецЕсли;
...
Процедура ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru='Обработка предназначена для запуска на версии платформы
            |BAF с отключенным режимом совместимости или выше'
            |;uk='Обробка призначена для запуску на версії платформи
            |BAF з відключеним режимом сумісності або вище'");
