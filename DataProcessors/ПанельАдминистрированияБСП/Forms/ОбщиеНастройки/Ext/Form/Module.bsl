...
Процедура АдресПубликацииИнформационнойНачалоВыбораЗавершение(ИмяРеквизита)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Не удалось автоматически заполнить поле, т.к. клиентское приложение не подключено через веб-сервер.';uk='Не вдалося автоматично заповнити поле, оскільки клієнтська прикладна програма не підключена через вебсервер.'"));
	КонецЕсли;
