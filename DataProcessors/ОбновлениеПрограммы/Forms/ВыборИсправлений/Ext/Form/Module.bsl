...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ТекущееИсправление Из ОписаниеИсправлений Цикл
		...
		СтрокаИсправление.Наименование = ?(
			...
			ТекущееИсправление.Наименование + " " + НСтр("ru='(Исправление отозвано)';uk='(Виправлення відкликано)'"),
		...
		Если СтрокаИсправление.ВозможноРазвертывание Тогда
			...
			СтрокаИсправление.ЗаголовокСсылкиДействия = НСтр("ru='Подробнее';uk='Докладніше'");
		КонецЕсли;
...
Процедура ИсправленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ИсправленияЗаголовокСсылкиДействия" Тогда
		...
		Если ТекущиеДанные.Развернута Тогда
			...
			ТекущиеДанные.ЗаголовокСсылкиДействия = НСтр("ru='Подробнее';uk='Докладніше'");
		Иначе
			ТекущиеДанные.ОписаниеОтображаемое = ТекущиеДанные.Описание;
			ТекущиеДанные.Развернута = Истина;
			ТекущиеДанные.ЗаголовокСсылкиДействия = НСтр("ru='Свернуть';uk='Згорнути'");
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Иначе
		...
		Если Результат.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выберите исправления для установки.';uk='Виберіть виправлення для установки.'"));
			Возврат;
		КонецЕсли;
...
Процедура Подобрать(Команда)
	...
	ПоказатьВводСтроки(
		ОписаниеОповещения,
		"",
		НСтр("ru='Введите список наименований исправлений (патчей).';uk='Введіть список найменувань виправлень (патчів).'"),
		,
		Истина);
...
Процедура ОбновитьНадписьВсегоВыбраноИсправлений(Форма)
	...
	Форма.ТекстВыбраноИсправлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Всего выбрано исправлений %1 из %2';uk='Всього вибрано виправлень %1 з %2'"),
		ВыбраноКоличество,
