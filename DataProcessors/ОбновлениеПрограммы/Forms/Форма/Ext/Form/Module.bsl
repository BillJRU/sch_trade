...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоВебКлиент()
		...
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';uk='Оновлення програми неможливо. Оновлення програми недоступно при роботі в режимі вебклієнта.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />
                ...
                |Оновлення програми недоступно при роботі в режимі вебклієнта.<br />
	...
	ИначеЕсли ОбщегоНазначения.КлиентПодключенЧерезВебСервер()
		...
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме подключения к веб серверу.';uk='Оновлення програми неможливо. Оновлення програми недоступно при роботі в режимі підключення до вебсервера.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />
                ...
                |Откройте программу в тонком или толстом клиенте и повторите попытку.'
                |;uk='<b>Оновлення програми неможливо</b><br />
                |Оновлення програми недоступно при роботі в режимі підключення до вебсервера.<br />
                |Відкрийте програму в тонкому або товстому клієнті і спробуйте ще раз.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ru='Переход на новую версию платформы BAF недоступен в текущем режиме работы.';uk='Перехід на нову версію платформи BAF недоступний в поточному режимі роботи.'");
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ЭтотОбъект.Заголовок     = НСтр("ru='Переход на новую версию платформы BAF';uk='Перехід на нову версію платформи BAF'");
			...
			Иначе
				...
				Если ПараметрыПолученияОбновлений.ПолучатьОбновленияКонфигурации
					...
					Элементы.ДекорацияУстановитьОбновлениеИзФайла.Заголовок =
						НСтр("ru='Установить обновление конфигурации или исправления (патчи) из файла';uk='Встановити оновлення конфігурації або виправлення (патчі) з файлу'");
					Элементы.ДекорацияУстановитьОбновлениеИзФайлаПодсказка.Подсказка =
						НСтр("ru='Если у вас уже есть файл обновления конфигурации или файлы исправлений (патчи), перейдите';uk='Якщо у вас вже є файл оновлення конфігурації або файли виправлення (патчі), перейдіть'")
							+ " "
							+ НСтр("ru='по ссылке для установки обновления конфигурации из файла';uk='за посиланням для установки оновлення конфігурації з файлу'");
				ИначеЕсли ПараметрыПолученияОбновлений.ПолучатьОбновленияКонфигурации
					И Не ПараметрыПолученияОбновлений.ПолучатьИсправления Тогда
					Элементы.ДекорацияУстановитьОбновлениеИзФайла.Заголовок =
						НСтр("ru='Установить обновление конфигурации из файла';uk='Встановити оновлення конфігурації з файлу'");
					Элементы.ДекорацияУстановитьОбновлениеИзФайлаПодсказка.Подсказка =
						НСтр("ru='Если у вас уже есть файл обновления конфигурации, перейдите';uk='Якщо у вас вже є файл оновлення конфігурації, перейдіть'")
							+ " "
							+ НСтр("ru='по ссылке для установки обновления конфигурации из файла';uk='за посиланням для установки оновлення конфігурації з файлу'");
				Иначе
					Элементы.ДекорацияУстановитьОбновлениеИзФайла.Заголовок =
						НСтр("ru='Установить исправления (патчи) из файла';uk='Встановити виправлення (патчі) з файлу'");
					Элементы.ДекорацияУстановитьОбновлениеИзФайлаПодсказка.Подсказка =
						НСтр("ru='Если у вас уже есть файлы исправлений (патчи), перейдите';uk='Якщо у вас вже є файли виправлення (патчі), перейдіть'")
							+ " "
							+ НСтр("ru='по ссылке для установки обновления конфигурации из файла';uk='за посиланням для установки оновлення конфігурації з файлу'");
				КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПолучениеИУстановка
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru='Не завершена установка платформы BAF.';uk='Не завершено встановлення платформи BAF.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура КаталогСохраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбора.Заголовок = НСтр("ru='Каталог хранения дистрибутивов платформы BAF';uk='Каталог зберігання дистрибутивів платформи BAF'");
...
Процедура ЗагружатьИУстанавливатьИсправленияАвтоматическиПриИзменении(Элемент)
	...
	Если ЗагружатьИУстанавливатьИсправленияАвтоматически
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Подключить';uk='Підключити'"));
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Для автоматического получения и установки исправлений (патчей)
                ...
                |;uk='Для автоматичного отримання і установки виправлень (патчів)
                |необхідно підключити Інтернет-підтримку користувачів.'"),
			Кнопки);
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	ИначеЕсли Ссылка = "open:V8Update" Тогда
		...
		ОткрытьВебСтраницу(
			...
			НСтр("ru='Особенности перехода на новую версию платформы BAF';uk='Особливості переходу на нову версію платформи BAF'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(АдресПолучателя)
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		Тема = НСтр("ru='Интернет-поддержка. Переход на новую версию Платформы BAF';uk='Інтернет-підтримка. Перехід на нову версію Платформи BAF'");
	Иначе
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса Тогда
		...
		Иначе
				...
				Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КонтекстОбновления.ПутьФайлаПротокола) Тогда
					Вложения.Добавить(
						Новый Структура("Представление, ИмяФайла",
							НСтр("ru='Протокол установки платформы BAF_Предприятие.txt';uk='Протокол установки платформи BAF_Підприємство.txt'"),
			...
			Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
				...
				Сообщение = НСтр("ru='Ошибка при получении и установке платформы BAF:';uk='Помилка при одержанні й установці платформи BAF:'")
					+ Символы.ПС
...
Функция ПодготовитьФайлМакетаИнструкции(Знач ИмяМакета, Знач ИмяФайла)
	...
	Если Не ПолучениеОбновленийПрограммы.СлужебнаяДоступноИспользованиеПолученияОбновленийПлатформы() Тогда
		ВызватьИсключение НСтр("ru='Использование обновления платформы BAF недоступно в текущем режиме работы.';uk='Використання оновлення платформи BAF недоступно в поточному режимі роботи.'");
	КонецЕсли;
	...
	Если НЕ ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КаталогДляРаботыСОбновлениями) Тогда
		...
		Исключение
			ПолучениеОбновленийПрограммыВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при создании каталога для работы с обновлениями платформы BAF (%1).
                        |%2'
                        |;uk='Помилка при створенні каталогу для роботи з оновленнями платформи BAF (%1).
                        |%2'"),
					КаталогДляРаботыСОбновлениями,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
...
Процедура УстановитьОбновлениеКонфигурации()
	...
	Иначе
		Состояние(, , НСтр("ru='Пожалуйста, подождите...';uk='Будь ласка, зачекайте...'"));
...
Процедура НачатьУстановкуОбновления(ПомещенныеФайлыИсправлений = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Вызов интерфейса установки обновления (СтандартныеПодсистемы.ОбновлениеКонфигурации).
            ...
            |Платформа BAF: %4'
            |;uk='Виклик інтерфейсу установки оновлення (СтандартныеПодсистемы.ОбновлениеКонфигурации).
            |Файли оновлення конфігурації: %1;
            |Файли виправлення (патчів): %2;
            |Видалити виправлення (патчі): %3;
            |Платформа BAF: %4'"),
			СтрОписаниеФайловОбновлений,
...
Функция ПараметрыУстановкиОбновленияКонфигурации()
	...
	Если ОбновитьПлатформу Тогда
		Результат.Вставить("КаталогПлатформы",
			ПолучениеОбновленийПрограммыКлиентСервер.КаталогУстановкиПлатформы(
...
Процедура ОтобразитьИнформациюОДоступномОбновленииПлатформы(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(ОбновлениеПлатформы.Версия) Тогда
		...
		Элементы.ДекорацияНетВерсии.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='<body>Рекомендуемая версия платформы BAF: <b>%1</b>
                    |<br />Используемая сейчас версия: %2</body>'
                    |;uk='<body>Рекомендована версія платформи BAF: <b>%1</b>
                    |<br />Версія що зараз використовується: %2</body>'"),
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Иначе
		...
		Если ПрямоеОбновление Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ru='Доступно обновление программы.';uk='Доступне оновлення програми.'");
		Иначе
			ТекстСообщения = ТекстСообщения
...
Процедура ОтобразитьИсправления()
	...
	Если КоличествоИсправленийДляВерсии > 0 Тогда
		...
		Если ОбновитьКонфигурацию Тогда
			Элементы.ДекорацияПатчиВерсияКонфигурации.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><strong>Для новой версии конфигурации %1</strong></body>';uk='<body><strong>Для нової версії конфігурації %1</strong></body>'"),
						ИнформацияОДоступномОбновлении.Конфигурация.Версия));
		Иначе
			Элементы.ДекорацияПатчиВерсияКонфигурации.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><strong>Для текущей версии конфигурации %1</strong></body>';uk='<body><strong>Для поточної версії конфігурації %1</strong></body>'"),
						ИнтернетПоддержкаПользователей.ВерсияКонфигурации(),
						ИнтернетПоддержкаПользователей.СинонимКонфигурации()));
		КонецЕсли;
		...
		Если СписокВыбранныхИсправлений.Количество() = 0 Тогда
			...
			Элементы.ДекорацияПатчиИсправляемыеОшибки.Заголовок = НСтр("ru='Исправляемые ошибки (не выбраны)';uk='Виправляємі помилки (не вибрані)'");
		Иначе
			УстановитьИсправления = (ОбновитьКонфигурацию ИЛИ УстановитьИсправления);
			Элементы.ДекорацияПатчиИсправляемыеОшибки.Заголовок =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Исправляемые ошибки (%1 из %2)';uk='Помилки що виправляються (%1 з %2)'"),
					СписокВыбранныхИсправлений.Количество(),
					КоличествоИсправленийДляВерсии);
		КонецЕсли;
		
		Если РазмерВыбранныхИсправлений = 0 Тогда
			Элементы.ДекорацияЗаголовокПатчи.Заголовок = НСтр("ru='Исправления (патчи)';uk='Виправлення (патчі)'");
		Иначе
			Элементы.ДекорацияЗаголовокПатчи.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body>Исправления (патчи), <strong>%1</strong></body>';uk='<body>Виправлення (патчі), <strong>%1</strong></body>'"),
	...
	Иначе
		...
		Элементы.ДекорацияПатчиВерсияКонфигурации.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Исправление ошибок для версии <strong>%1</strong> не требуется.</body>';uk='<body>Виправлення помилок для версії <strong>%1</strong> не потрібно.</body>'"),
					?(ОбновитьКонфигурацию,
					ИнформацияОДоступномОбновлении.Конфигурация.Версия,
					ИнтернетПоддержкаПользователей.ВерсияКонфигурации())));
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	Если Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
			НСтр("ru='Обновление платформы BAF';uk='Оновлення платформи BAF'");
		Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Обновление не требуется. Текущая версия платформы BAF: <b>%1</b></body>';uk='<body>Оновлення не потрібне. Поточна версія платформи BAF: <b>%1</b></body>'"),
					ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы()));
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body>Обновление платформы BAF, <b>%1</b></body>';uk='<body>Оновлення платформи BAF, <b>%1</b></body>'"),
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
							ОбновлениеКомПл.РазмерОбновления)));
		Иначе
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
						НСтр("ru='<body>Обновление платформы BAF</body>';uk='<body>Оновлення платформи BAF</body>'"));
		КонецЕсли;
		...
		Иначе
			...
			Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b><br />
                            |Текущая версия платформы BAF: %2</body>'
                            |;uk='<body><b>Версія %1</b><br />
                            |Поточна версія платформи BAF: %2</body>'"),
						ОбновлениеКомПл.Версия,
						ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы()));
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если ОбновитьКонфигурацию Тогда
				...
				ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки = 1 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Новая версия %1 платформы BAF рекомендуется для работы с новой версией конфигурации %2.';uk='Нова версія %1 платформи BAF рекомендується для роботи з новою версією конфігурації %2.'"),
							ОбновлениеКомПл.Версия,
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				Иначе
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
			...
			ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки < 2 Тогда
				...
				Элементы.ДекорацияПояснениеПлатформа.Заголовок =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Рекомендуется установить новую версию платформы BAF.';uk='Рекомендується встановити нову версію платформи BAF.'"),
						ИнформацияОДоступномОбновлении.Конфигурация.Версия);
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='<body>Для работы новой версии конфигурации требуется платформа BAF не ниже версии <b>%1</b>.</body>';uk='<body>Для роботи нової версії конфігурації потрібна платформа BAF не нижче версії <b>%1</b>.</body>'"),
...
Процедура ПриИзмененииРасписанияУстановкиИсправлений(Расписание, Форма) Экспорт
	...
	Если ПериодПовтораВТечениеДня > 0
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Интервал автоматической установки не может быть чаще, чем один раз в час.';uk='Інтервал автоматичного встановлення не може бути частіше, ніж один раз на годину.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИтерацияПолученияФайловОбновления()
	...
	Иначе
		...
		КонтекстОбновления.ТекущееДействие = НСтр("ru='Установка исправлений';uk='Встановлення виправлень'");
...
Функция УстановитьУдалитьИсправленияНаСервере(Знач ИсправленияДляУстановки, Знач ОтозванныеИсправления)
	...
	Если Не ПолучениеОбновленийПрограммы.ЭтоАдминистраторСистемы() Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для установки исправлений (патчей).';uk='Недостатньо прав для установки виправлень (патчів).'");
	КонецЕсли;
...
Процедура ПриПолученииФайлаИсправления(ПолученныеФайлы, РезультатПолучения) Экспорт
	...
	Если ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru='Файл исправления (патча) успешно загружен.';uk='Файл виправлення (патча) успішно завантажений.'"));
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется получение файла дистрибутива платформы BAF (%1).';uk='Виконується отримання файлу дистрибутива платформи BAF (%1).'") + " "
...
Процедура НачатьПолучениеОбновленийВФоновомРежиме()
	...
	Попытка
		...
		Если РезультатНачалаЗадания.Статус = "Отменено" Тогда
			ВызватьИсключение НСтр("ru='Задание отменено администратором.';uk='Завдання скасовано адміністратором.'");
		ИначеЕсли РезультатНачалаЗадания.Статус = "Ошибка" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить фоновое задание. %1';uk='Не вдалося виконати фонове завдання. %1'"),
	...
	Исключение
		...
		ОтобразитьВнутреннююОшибку(НСтр("ru='Ошибка начала задания.';uk='Помилка початку завдання.'") + Символы.ПС
...
Процедура ОтобразитьСостояниеКонтекстаОбновления()
	...
	Если КонтекстОбновления.Свойство("ФайлыОбновленияПолучены") Тогда
		...
		ИначеЕсли КонтекстОбновления.ОбновитьПлатформу Тогда
			...
			// тогда отображается состояние установки платформы BAF.
...
Функция СостояниеЗадания(Знач ИДЗадания)
	...
	Исключение
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при проверке состояния задания. %1';uk='Помилка під час перевірки стану завдання. %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
...
Процедура ОтобразитьПолучениеОбновленийЗавершеноНаСервере(Знач Инициализировать)
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		КаталогУстановкиПлатформы =
			ПолучениеОбновленийПрограммыКлиентСервер.КаталогУстановкиПлатформы(УстановленнаяВерсияПлатформы);
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Иначе
			Строки.Добавить(НСтр("ru='информационной базы и запущен сеанс работы с программой на новой версии платформы.
                |<br />На компьютерах других пользователей программы необходимо обновить платформу BAF вручную.</body>'
                |;uk='інформаційної бази і запущений сеанс роботи з програмою на новій версії платформи.
                |<br />На комп''ютерах інших користувачів програми необхідно оновити платформу BAF вручну.</body>'"));
		КонецЕсли;
