	...
	Если ИсточникВыбора.ИмяФормы = "Справочник.СпособыОбеспеченияПотребностей.Форма.ФормаВыбора" Тогда
		...
		ИначеЕсли Контекст = "СпособыОбеспеченияНоменклатурой" Тогда
			Если НЕ ЗаполнитьСпособОбеспеченияНоменклатуройНаСервере(ВыбранноеЗначение) Тогда
				ПоказатьПредупреждение(, НСтр("ru='Не выделено ни одной строки со складом. Выполнение команды невозможно.';uk='Не виділено жодного рядка зі складом. Виконання команди неможливе.'"));
			КонецЕсли;
		КонецЕсли;
		Контекст = "";
...
Процедура ХарактеристикаПриИзменении(Элемент)
	...
	Если УстановкаСпособовДоступна Тогда
		СостояниеДоступностиИзмененияСпособа = ?(ЗначениеЗаполнено(ОтборХарактеристика), "",
			НСтр("ru='Установка способа обеспечения доступна только при заданном отборе по характеристике';uk='Встановлення способу забезпечення доступно тільки при заданому відборі за характеристикою'"));
...
Процедура ЗаполнитьУпаковкуЗаказаНаСкладе(Команда)
	...
	Если ЕстьВыделенныеСтрокиВТаблице(Элементы.СписокСпособовОбеспеченияСклада) Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Для выбранной номенклатуры упаковки не используются. Выполнение команды невозможно.';uk='Для обраної номенклатури упаковки не використовуються. Виконання команди неможливо.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьНаКлиенте()
	Если Модифицированность ИЛИ НЕ ЗначениеЗаполнено(СхемаОбеспечения.Ссылка) Тогда
		Если ЗаписьНаСервере() Тогда
			Шаблон = НСтр("ru='%1 (Схема обеспечения)';uk='%1 (Схема забезпечення)'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",			НСтр("ru='<не установлена>';uk='<не встановлена>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста",	ЦветаСтиля.НезаполненноеПолеТаблицы);
	#КонецОбласти
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",		НСтр("ru='Схема обеспечения';uk='Схема забезпечення'"));
	
	Элемент = ДобавитьЭлементОформленияСУсловием(УсловноеОформление,
		Элементы.ДеревоФорматовЕстьНастройкаДляТовара.Имя,
		"ДеревоФорматов.ЕстьНастройкаДляТовара", Истина);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",		НСтр("ru='Способ обеспечения';uk='Спосіб забезпечення'"));
...
Процедура НастроитьФормуПоПараметрам()
	...
	Если Источник = "Склад" Тогда
		...
		Шаблон = НСтр("ru='Настройка способов обеспечения потребностей (Склад: ""%1"")';uk='Настройка способів забезпечення потреб (Склад: ""%1"")'");
	...
	ИначеЕсли Источник = "Номенклатура" Тогда
		...
		Шаблон = НСтр("ru='Настройка способов обеспечения потребностей (Номенклатура: ""%1"")';uk='Настройка способів забезпечення потреб (Номенклатура: ""%1"")'");
	...
	ИначеЕсли Источник = "СхемаОбеспечения" Тогда
		...
		Если ЗначениеЗаполнено(ОтборСхемаОбеспечения) Тогда
			...
			Заголовок = СтрШаблон(Шаблон, СхемаОбеспечения.Наименование);
		Иначе
			Заголовок = НСтр("ru='Схема обеспечения (создание)';uk='Схема забезпечення (створення)'");
		КонецЕсли;
	...
	Если Элементы.ШапкаСхемыОбеспечения.Видимость Тогда
		...
		Если КоличествоИспользований > 0 Тогда
			...
			МассивСтрок.Добавить(НСтр("ru='Используется для товаров';uk='Використовується для товарів'"));
			МассивСтрок.Добавить(" (");
			ТекстВсего = СтрШаблон(НСтр("ru='всего: %1';uk='всього: %1'"),
		...
		Иначе
			Элементы.НадписьИспользованиеСхемы.Заголовок = НСтр("ru='Не назначена ни одному товару';uk='Не призначена жодному з товарів'");
		КонецЕсли;
	...
	Если Элементы.СтраницаСпособыОбеспеченияСклада.Видимость Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокСпособовОбеспеченияСклада,
			"ИсточникСхема", НСтр("ru='Схема обеспечения';uk='Схема забезпечення'"));
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокСпособовОбеспеченияСклада,
			"ИсточникСпособ", НСтр("ru='Способ обеспечения';uk='Спосіб забезпечення'"));
