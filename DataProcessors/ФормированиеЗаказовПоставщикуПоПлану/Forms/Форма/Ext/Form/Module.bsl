...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ИспользоватьЗаказыПоставщикам Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Для формирования заказов необходимо включить настройку ""Заказы поставщикам"" в ""Администрирование -> Закупки""';uk='Для формування замовлень необхідно включити настройку ""Замовлення постачальникам"" в ""Адміністрування -> Купівлі""'"));
		Отказ = Истина;
		Возврат;
...
Процедура ФормированиеЗаказовПодразделениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого Строка Из Строки Цикл
		Если ВыбранноеЗначение = Строка.Подразделение Тогда
			СтандартнаяОбработка = Ложь;
			ПоказатьПредупреждение(, НСтр("ru='По выбранному подразделению есть планы.';uk='За вбраним підрозділом є плани.'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если Строка <> Неопределено И (Поле.Имя = "КорзинаЗаказовПартнер"
		...
		ИначеЕсли Поле.Имя = "КорзинаЗаказовПодразделение" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Назначить для выделенного товара подразделение: %1?';uk='Призначити для виділеного товару підрозділ: %1?'"), СтрокаКорзины.Подразделение);
...
Процедура Заполнить(Команда)
	...
	Если Объект.ФормированиеЗаказов.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Оповещение,
			НСтр("ru='В таблице имеются строки. При обновлении данных из планов строки будут очищены. Продолжить?';uk='В таблиці є рядки. При відновленні даних з планів рядки будуть очищені. Продовжити?'"), 
			Кнопки, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПодразделение(Команда)
	...
	Если Элементы.ФормированиеЗаказов.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите строки в списке.';uk='Виділіть рядки в списку.'"));
		Возврат;
	КонецЕсли;
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите строки в которых разрешено редактирование подразделения.';uk='Виділіть рядки в яких дозволено редагування підрозділу.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСклад(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите строки в которых разрешено редактирование склада.';uk='Виділіть рядки в яких дозволено редагування складу.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоставщика(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите строки в которых разрешено редактирование поставщика.';uk='Виділіть рядки в яких дозволено редагування постачальника.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСоглашение(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите строки в которых разрешено редактирование соглашения.';uk='Виділіть рядки в яких дозволено редагування оферти.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДатуПоступления(Команда)
	...
	ПоказатьВводДаты(Оповещение, 
		...
		НСтр("ru='Введите дату поступления';uk='Введіть дату надходження'"),
		ЧастиДаты.Дата);
...
Процедура Далее(Команда)
	...
	Если Элементы.СтраницыОсновнаяПанель.ТекущаяСтраница = Элементы.СтраницаШаг1 Тогда
		...
		Если ЗначениеЗаполнено(Объект.ПериодОкончание)  
			...
			СообщениеПользователю.Текст = НСтр("ru='""Начало периода"" больше ""Окончания периода"". Отбор невозможен.';uk='""Початок періоду"" більше ""Закінчення періоду"". Відбір неможливий.'");
...
|;uk='Немає замовлень до оформлення. Позначте прапорцем кількість в колонці 
...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыОсновнаяПанель.ТекущаяСтраница = Элементы.СтраницаШаг3 Тогда
		...
		Если СформированныеЗаказы.Количество() > 0 Тогда
			...
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Пометить документы на удаление';uk='Помітити документи на вилучення'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена);
			
			ПоказатьВопрос
				(Новый ОписаниеОповещения("НазадЗавершение", ЭтотОбъект), 
				НСтр("ru='Список созданных заказов не пуст. Сформированные документы будут помечены на удаление. Пометить на удаление заказы и продолжить?';uk='Список створених замовлень не порожній. Сформовані документи будуть позначені на вилучення. Помітити на вилучення замовлення і продовжити?'"),
				Кнопки);
			Возврат;
		КонецЕсли;
...
Процедура ЗаполнитьНаКлиенте()
	...
	Если Объект.ФормированиеЗаказов.Количество() = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нет утвержденного плана для сценария %1, соответствующего установленным отборам. Необходимо изменить отборы.';uk='Немає затвердженого плану для сценарію %1, який відповідає встановленим відборам. Необхідно змінити відбори.'"),
			Объект.Сценарий);
		ПоказатьПредупреждение(, ТекстОшибки);
	КонецЕсли;
...
Процедура ЗаполнитьСоглашениеФрагмент(Знач ВыделенныеСтроки, Знач Партнер, Знач ПерваяСтрока)
	...
	Если НЕ ЗначениеЗаполнено(Партнер) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не выбран поставщик.';uk='Не обраний постачальник.'"),
			,
...
Функция СписокПлановПоСтроке(КлючСтроки)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СУММА(ПланыЗакупокОбороты.КЗаказуОборот) КАК Количество,
...
Процедура ЗаписатьЗаказНаСервере(ДокументОбъект, СкладРодитель, СписокОшибок)
	...
	Новый Структура("НалогообложениеНДС, Дата", 
			НДСОбщегоНазначенияСервер.ПолучитьНалогообложениеНДС(ДокументОбъект.Организация, 
...
|%2'");
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Формирование заказов поставщику по плану';uk='Формування замовлень постачальнику за планом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ВводДоступен() Тогда
		Если ПроверитьЕстьДокументыДляОбновления() Тогда
			ПоказатьВопрос(
				...
				НСтр("ru='Заказы поставщику были изменены. Хотите обновить данные? При обновлении строки будут очищены.';uk='Замовлення постачальнику були змінені. Хочете оновити дані? При оновленні рядки будуть очищені.'"), 
