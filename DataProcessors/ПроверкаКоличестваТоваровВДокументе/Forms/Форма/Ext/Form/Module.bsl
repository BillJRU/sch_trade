...
Процедура СчитатьРеквизитыПроверитьОрдер()
	...
	Если Объект.РежимИсправления Тогда
		...
		Если НЕ РезультатЗапроса[0].Пустой() Тогда
			ТекстИсключения = НСтр("ru='В ордере есть строки с действием ""%Отобрать%"". Исправление такого ордера не возможно. Его нужно проверять.';uk='В ордері є рядки з дією ""%Отобрать%"". Виправлення такого ордера неможливо. Його потрібно перевіряти.'");
	...
	Если Объект.РежимИсправления Тогда
		...
		Иначе
			Если СтатусОрдера <> Перечисления.СтатусыРасходныхОрдеров.Проверен
				...
				ТекстИсключения = НСтр("ru='Исправление количества в расходном ордере возможно, если он находится в статусе ""Проверен"", ""К отгрузке"" или ""Отгружен"".';uk='Виправлення кількості в видатковому ордері можливо, якщо він знаходиться в статусі ""Перевірено"", ""До відвантаження"" або ""Відвантажено"".'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		КонецЕсли
	Иначе
...
|;uk='Кількості товарних місць недостатньо для збирання цілої кількості товару.
|Кількість товару до відвантаження менше на %КоличествоВОрдере% %ЕдиницаИзмерения%, ніж потрібно за ордером.'");
