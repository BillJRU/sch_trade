...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Описание Из НастройкиИспользованияСерий.ОписанияИспользованияРеквизитовСерии Цикл
		Если Описание.ИмяНастройки = "ИспользоватьRFIDМеткиСерии" Тогда
			...
			Если Описание.ИмяРеквизита = "RFIDTID" Тогда
				Если Описание.Использование Тогда
					...
					Иначе
						ТекстСообщения = НСтр("ru='К рабочему месту подключено несколько считывателей RFID. Работа одновременно с несколькими считывателями не поддерживается. Оставьте только один считываетель.';uk='До робочого місця підключено кілька зчитувачів RFID. Робота одночасно з декількома зчитувачами не підтримується. Залиште тільки один зчитувач.'");
		...
		Иначе
			...
			ПолеФормыОстаткиСерий = Элементы["ОстаткиСерий" + Описание.ИмяРеквизита]; // ПолеФормы
			...
			Если Не ПустаяСтрока(Описание.ПредставлениеФорматнойСтрокиВЗаголовке) Тогда
				ПредставлениеЗаголовкаКолонки = НСтр("ru='%ПредставлениеРеквизита% (%ПредставлениеФормата%)';uk='%ПредставлениеРеквизита% (%ПредставлениеФормата%)'");
	...
	Если Не ПустаяСтрока(РежимПомощника) Тогда
		...
		Элементы.ОК.Заголовок = НСтр("ru='Готово';uk='Готово'");
...
Функция ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ДанныеШтрихкодов.Количество() <> 1 Тогда
		...
		ТекстСообщения = НСтр("ru='Не поддерживается одновременное распознование нескольких штрих-кодов серий. Сканируйте серии по одной.';uk='Не підтримується одночасне розпізнавання декількох штрихкодів серій. Скануйте серії по одній.'");
...
Процедура СгенерироватьНомерСервер()
	...
	НомерЧислом = Справочники.СерииНоменклатуры.ВычислитьМаксимальныйНомерСерии(РеквизитыНоменклатуры.ВидНоменклатуры) + 1;
...
Процедура ПодобратьСериюЗавершитьВвод()
	...
	Если Элементы.СтраницыСерий.ТекущаяСтраница = Элементы.СтраницаОстаткиСерий Тогда
		Если ТекущиеДанные = Неопределено Тогда
			ТекстПредупреждения = НСтр("ru='Выберите или введите серию.';uk='Виберіть або введіть серію.'");	
			ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
...
Функция ОбязательныеРеквизитыЗаполнены(ВыводитьСообщения = Истина, ЭтоСписываемаяМетка = Ложь)
	...
	Если Не ПустаяСтрока(РежимПомощника) 
		...
		ТекстСообщения = НСтр("ru='Параметры RFID-метки не соответствуют ожидаемым: либо считана неперсонифицированная метка, либо считана метка с другим GTIN, либо записанный в метку номер не соответствует TID. Считайте другую метку.';uk='Параметри RFID-мітки не відповідають очікуваним: або зчитана неперсонифікована мітка, або зчитана мітка з іншим GTIN, або записаний в мітку номер не відповідає TID. Зчитайте іншу мітку.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	...
	ИначеЕсли НастройкиИспользованияСерий.ИспользоватьRFIDМеткиСерии
		...
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru='Требуется записать RFID-метку.';uk='Потрібно записати RFID-позначку.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	...
	Для Каждого Описание Из НастройкиИспользованияСерий.ОписанияИспользованияРеквизитовСерии Цикл
		Если Описание.Использование
			...
			ТекстСообщения = НСтр("ru='Поле ""%ПредставлениеРеквизита%"" не заполнено';uk='Поле ""%ПредставлениеРеквизита%"" не заповнено'");
...
Процедура УстановитьЗаголовокДекорацииСерия()
	...
	Элементы.ДекорацияСерия.Заголовок = ?(ЗначениеЗаполнено(Серия), НСтр("ru='Зарегистрированная серия';uk='Зареєстрована серія'"),
		НСтр("ru='Новая серия';uk='Нова серія'"));
