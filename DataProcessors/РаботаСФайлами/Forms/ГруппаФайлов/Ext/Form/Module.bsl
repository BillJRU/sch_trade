...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОповещениеОбОтвете, НСтр("ru='Данные были изменены. Сохранить изменения?';uk='Дані були змінені. Зберегти зміни?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ТекущаяСсылкаНаФайл) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (Группа файлов)';uk='%1 (Група файлів)'"), Строка(ТекущаяСсылкаНаФайл));
	Иначе
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Ошибка записи группы присоединенных файлов';uk='Файли.Помилка запису групи приєднаних файлів'",ОбщегоНазначения.КодОсновногоЯзыка()),
