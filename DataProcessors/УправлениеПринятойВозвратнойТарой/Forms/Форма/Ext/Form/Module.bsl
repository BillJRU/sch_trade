...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты)
	...
	ТекстСообщения = НСтр("ru='У выделенных строк в документе приобретения отличается поле ""%ПредставлениеПоля%""';uk='У виділених рядків в документі придбання відрізняється поле ""%ПредставлениеПоля%""'");
...
Процедура УстановитьТекстЗапросаПринятаяТара()
	...
	|			,
	|			&ПриобретениеДоступно
	|				И ТИПЗНАЧЕНИЯ(ДокументПоступления) = ТИП(Документ.ПриобретениеТоваровУслуг)
