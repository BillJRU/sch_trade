	...
	Если Не ПустаяСтрока(КодировкаФайлаПротоколаОбмена) Тогда
		Если СтрНачинаетсяС(ПредставлениеКодировки, "КодировкаТекста.") Тогда
			...
			Исключение
                ...
                |Используется ANSI.'
                |;uk='Невідоме кодування файлу протоколу обміну: %1.
                |Використовується ANSI.'"), ПредставлениеКодировки);
				ЗаписьЖурналаРегистрации(НСтр("ru='Универсальный обмен данными в формате XML';uk='Універсальний обмін даними у форматі XML'",КодОсновногоЯзыка()),
...
Процедура ДобавитьЗначениеСвойстваВУзел(Значение, ТипЗначения, ТипПриемника, УзелСвойства, СвойствоУстановлено)
	...
	ИначеЕсли ТипЗначения = одТипВидДвиженияНакопления Тогда
		...
		одЗаписатьЭлемент(УзелСвойства, "Значение", XMLСтрока(Значение));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя сервера платформы BAF информационной базы-приемника';uk='Не вказано ім''я сервера платформи BAF інформаційної бази-приймача'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя информационной базы-приемника на сервере BAF';uk='Не вказано ім''я інформаційної бази-приймача на сервері BAF'");
