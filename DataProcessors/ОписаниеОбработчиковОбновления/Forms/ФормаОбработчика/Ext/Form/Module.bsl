...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьИдентфикаторыОбъектовМетаданных = Метаданные.Справочники.Найти("ИдентификаторыОбъектовМетаданных") <> Неопределено;
	
	МассивПорядка = Новый Массив;
	МассивПорядка.Добавить(НСтр("ru='Обработчик1';uk='Обробник1'"));
	МассивПорядка.Добавить(НСтр("ru='Обработчик2';uk='Обробник2'"));
	МассивПорядка.Добавить(НСтр("ru='Любой';uk='Будь-який'"));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, НСтр("ru='Данные были изменены. Сохранить изменения?';uk='Дані були змінені. Зберегти зміни?'"), РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
...
Процедура ОсновнойОбъектМетаданныхОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ПараметрыОбрабочикаВыбора = ПараметрыОбрабочикаВыбора(, ОсновнойОбъектМетаданных);
	ОбработкаВыбораОбъектаМетаданных(ВыбранноеЗначение, ПараметрыОбрабочикаВыбора);
...
Процедура ПроцедураПроверкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ЧитаемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0
		...
		ТекстСообщения = НСтр("ru='Процедура ""ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы"" не предназначена для проверки блокировки в обработчиках, которые контролируют блокировку читаемых данных или прочих объектов.';uk='Процедура ""ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы"" не призначена для перевірки блокування в обробниках, які контролюють блокування читаних даних або інших об''єктів.'");
...
Процедура КомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(
		Оповещение, Элементы.Комментарий.ТекстРедактирования, НСтр("ru='Служебный комментарий';uk='Службовий коментар'"));
...
Процедура ОбъектМетаданныхОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = ТипЗнч(ВыбранноеЗначение) = Тип("Строка");
	ИмяТаблицы = Элемент.Родитель.Родитель.Имя;
	ПараметрыОбрабочикаВыбора = ПараметрыОбрабочикаВыбора(ИмяТаблицы);
	ОбработкаВыбораОбъектаМетаданных(ВыбранноеЗначение, ПараметрыОбрабочикаВыбора);
...
Процедура ЗаписатьОбработчик(Команда)
	...
	Адрес = ПоместитьДанныеОбработчикаВХранилище(ВладелецФормы.УникальныйИдентификатор);
	Оповестить("ИзменилосьОписаниеОбработчиткаОбновления", Адрес);
	Элементы.Объявление.Видимость = Модифицированность;
...
&НаСервере
Функция ПоместитьДанныеОбработчикаВХранилище(УникальныйИдентификаторВладелцаФормы)
	...
	Возврат ПоместитьВоВременноеХранилище(Данные,  УникальныйИдентификаторВладелцаФормы);
...
Процедура ЗаполнитьСоответствияМодуля(МассивПорядка)
	...
	МножЧисло = Новый Соответствие;
	МножЧисло.Вставить("Константа", "Константы");
	МножЧисло.Вставить("Справочник", "Справочники");
	МножЧисло.Вставить("Документ", "Документы");
	МножЧисло.Вставить("ПланВидовХарактеристик", "ПланыВидовХарактеристик");
	МножЧисло.Вставить("ПланСчетов", "ПланыСчетов");
	МножЧисло.Вставить("ПланВидовРасчета", "ПланыВидовРасчета");
	МножЧисло.Вставить("РегистрСведений", "РегистрыСведений");
	МножЧисло.Вставить("РегистрНакопления", "РегистрыНакопления");
	МножЧисло.Вставить("РегистрБухгалтерии", "РегистрыБухгалтерии");
	МножЧисло.Вставить("РегистрРасчета", "РегистрыРасчета");
	МножЧисло.Вставить("БизнесПроцесс", "БизнесПроцессы");
	МножЧисло.Вставить("Задача", "Задачи");
	МножЧисло.Вставить("Обработка", "Обработки");
	
	ЕдЧисло = Новый Соответствие;
	Для Каждого Класс Из МножЧисло Цикл
		ЕдЧисло.Вставить(Класс.Значение, Класс.Ключ);
	КонецЦикла;
	
	МножественноеЧисло = Новый ФиксированноеСоответствие(МножЧисло);
...
Процедура ВыборОбъектаМетаданных(Элемент, ТекущееЗначение = "", ИмяТаблицы = Неопределено)
	...
	Если НЕ ЕстьИдентфикаторыОбъектовМетаданных Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОбрабочикаВыбора = ПараметрыОбрабочикаВыбора(ИмяТаблицы, ТекущееЗначение);
	ОбработчикВыбора = Новый ОписаниеОповещения("ОбработкаВыбораОбъектаМетаданных", ЭтотОбъект, ПараметрыОбрабочикаВыбора);
...
&НаКлиенте
Функция ПараметрыОбрабочикаВыбора(ИмяТаблицы = Неопределено, ТекущееЗначение = "")
	
	ПараметрыОбрабочикаВыбора = Новый Структура("ИмяРеквизита", "ОсновнойОбъектМетаданных");
	Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
		ПараметрыОбрабочикаВыбора = Новый Структура("ИмяТаблицы", ИмяТаблицы);
	КонецЕсли;
	ПараметрыОбрабочикаВыбора.Вставить("ПредыдущееЗначение", ТекущееЗначение);
	
	Возврат ПараметрыОбрабочикаВыбора;
...
&НаСервереБезКонтекста
Функция ДоступныеКлассыОбъектовМетаданных(ВключитьОбщиеМодули = Ложь) Экспорт
...
Процедура ОбновитьСведенияОбработчика2()
	...
	Для Каждого Конфликт Из Объект.КонфликтыОбработчиков Цикл
		Если Конфликт.ОбработчикПисатель = ТекущиеДанные.Ссылка
				И Конфликт.ОбработчикЧитательИлиПисатель2 = ТекущиеДанные.Обработчик2
			ИЛИ Конфликт.ОбработчикЧитательИлиПисатель2 = ТекущиеДанные.Ссылка
				И Конфликт.ОбработчикПисатель = ТекущиеДанные.Обработчик2 Тогда
...
Процедура ПодобратьОбъектыМетаданных(Текст, ДанныеВыбора, СтандартнаяОбработка, ВключатьОбщиеМодули = Ложь)
	...
	СтандартнаяОбработка = Ложь;
	ВключатьОбщиеМодули = Истина;
	ОбъектыМетаданых = НайтиОбъектыМетаданных(Текст, ВключатьОбщиеМодули);
	
	ДанныеВыбора = Новый СписокЗначений;
	ДанныеВыбора.ЗагрузитьЗначения(ОбъектыМетаданых);
...
Функция НайтиОбъектыМетаданных(ЧастьИмени, ВключатьОбщиеМодули)
	...
	ОбъектыМетаданых = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	Возврат ОбъектыМетаданых;
...
Процедура ДобавитьТипыРеквизита(ИмяТаблицы)
	...
	Если Элементы[ИмяТаблицы].ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не выбран объект метаднных';uk='Не вибрано об''єкт метаднних'"));
		Возврат;
	КонецЕсли;
	...
	Реквизиты.ПоказатьВыборЭлемента(ОбработчикВыбора, НСтр("ru='Выберите реквизит объекта';uk='Виберіть реквізит об''єкта'"));
...
Функция РеквизитыОбъектаМетаданных(ИмяОбъекта)
	...
	МетаданныеОбъекта = ОбщегоНазначенияУТ.МетаданныеПоИмени(ИмяОбъекта);
...
Процедура ДобавитьТипыРеквизитаВТаблицу(ИмяТаблицы, ПолноеИмяРеквизита)
	...
	Реквизит = ОбщегоНазначенияУТ.МетаданныеПоИмени(СтрСоединить(ИменаРеквизита, "."));
	Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
...
Функция ПорядокОбъектаПоУмолчанию(ИмяПроцедуры)
	...
	Типбъекта = Имена[0];
	Если Типбъекта = "РегистрыСведений" Тогда
		Имена.Удалить(Имена.ВГраница());
		ИмяОбъекта = СтрСоединить(Имена, ".");
		МетаданныеОбъекта = ОбщегоНазначенияУТ.МетаданныеПоИмени(ИмяОбъекта);
		Если Строка(МетаданныеОбъекта.РежимЗаписи) = "Независимый" Тогда
			Типбъекта = Типбъекта + "Независимые";
		КонецЕсли;
	КонецЕсли;
	
	Возврат ПриоритетыПоУмолчанию[Типбъекта];
