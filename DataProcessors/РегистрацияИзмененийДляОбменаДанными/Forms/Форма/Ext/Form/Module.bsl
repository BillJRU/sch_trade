...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ДлительнаяОперацияЗапущена Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Прервать выполнение регистрации данных?';uk='Перервати виконання реєстрації даних?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru='Обработка предназначена для запуска на версии платформы
            |платформа 8.3.5 с отключенным режимом совместимости или выше'
            |;uk='Обробка призначена для запуску на версії платформи
            |платформа 8.3.5 з відключеним режимом сумісності або вище'");
