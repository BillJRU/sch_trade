		...
		Если ТипЗнч(ТекущаяСтрокаДерева.ВидКатегорияМарка) = Тип("СправочникСсылка.ВидыНоменклатуры") Тогда
			Если ТекущаяСтрокаДерева.ЭтоГруппаВидов Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
					...
					Истина);
			Иначе
				НайденныеДополнительныеВиды = ДополнительныеВиды.Получить(ТекущаяСтрокаДерева.ВидКатегорияМарка); // Массив из СправочникСсылка.ВидыНоменклатуры
...
|Добавление его в документ не требуется.'
|;uk='Даний товар уже перебуває в стадії ""%1"".
|Додавання його в документ не потрібно.'");
				Иначе
...
|;uk='По товарній категорії ""%1"" і марці ""%2"" не встановлена квота.
|Присутність даного товару в асортименті
|суперечить асортиментній політиці.'"),
												ТоварнаяКатегория,
												Марка);
					Иначе
						ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
|противоречит ассортиментной политике.'
|;uk='По товарних категоріях вибраних товарів не встановлена квота.
|Присутність вибраних товарів в асортименті
|суперечить асортиментній політиці.'");
				КонецЕсли; 
			Иначе
				// Некоторые товары можно добавить без вопроса
...
Процедура ДобавитьТоварыВКорзинуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Не ПоказыватьПодобранныеТовары Тогда
		...
		Если СписокВыбранныхТоваров.Количество() = 1 Тогда
			...
			ТекстОповещения = НСтр("ru='Товар %Товар% добавлен в таблицу подбора';uk='Товар %Товар%  доданий в таблицю підбору'");
