...
Процедура ЗаписатьПараметрыПоддержанияЗапаса(Таблица) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Запись параметров поддержания запаса';uk='Запис параметрів підтримання запасу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
