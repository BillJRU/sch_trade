...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("ru='Задание отменено, но состояние отмены будет установлено сервером только через некоторое время,
                   ...
                   |;uk='Завдання скасовано, але стан відміни буде встановлено сервером тільки через деякий час,
                   |може виникнути потреба оновити дані вручну.'"));
...
Процедура СобытияЖурналаРегистрации(Команда)
	...
	Если Элементы.Задания.ТекущаяСтраница = Элементы.РегламентныеЗадания Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ДатаНачала) И Не ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончания) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Нет событий для просмотра, так как регламентное задание не выполнялось.';uk='Немає подій для перегляду, так як регламентне завдання не виконувалося.'"));
			Возврат;
		КонецЕсли;	
