...
//
Функция ДобавитьКомандуПечати(КомандыПечати, ТипЗадания, Порядок = Неопределено) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаданиеНаОтборРазмещениеТовара") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Задание на отбор (размещение) товара';uk='Завдання на відбір (розміщення) товару'"),
 			СформироватьЗаданиеНаОтборРазмещениеТовара(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода),,,,Истина);
	КонецЕсли;
...
//
Функция СформироватьЗаданиеНаОтборРазмещениеТовара(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ПервыйДокумент = Истина;
 		...
 		НазваниеФормы = НазваниеПечатнойФормы(ТипЗадания, КодЯзыкаПечать);
		...
		ВыборкаПоУпаковкам		= ?(ЗначениеЗаполнено(ДанныеПечати.РезультатПоУпаковкам),
									ДанныеПечати.РезультатПоУпаковкам.Выбрать(),
									Неопределено);
		...
		Если ЗначениеЗаполнено(ДанныеПоОшибкам)
			...
			Пока ВыборкаДокументовБезДанных.Следующий() Цикл
				СкладскяОперация = ?(ТипЗадания <> "ЗаданиеНаРазмещение", НСтр("ru='отгружаемая';uk='відвантажувана'"), НСтр("ru='размещаемая';uk='розміщувана'"));
...
|тому що в документі відсутня %СкладскяОперация% номенклатура.'");
		...
		Пока ВыборкаПоШапке.Следующий() Цикл
			...
			ИспользоватьУпаковки			= ВыводитьУпаковки
												Или ВыводитьПоУчету;
			СуффиксОбласти					= СуффиксОбласти(ИспользуетсяСправочноеХранение, ИспользоватьУпаковки, ВыводитьКоды);
			...
			Пока ВыборкаПоСкладам.НайтиСледующий(ОтборПоСкладу) Цикл
 				...
 				Макет = УправлениеПечатью.МакетПечатнойФормы(ПутьКМакету, КодЯзыкаПечать); // см. Обработки.ПечатьЗаданияНаОтборРазмещениеТоваров.Макеты.ПФ_MXL_ЗаданиеНаОтборРазмещениеТовара
				...
				ТабличныйДокумент.Вывести(ОбластьЗаголовок);
 				...
 				ПараметрыШапки = ПараметрыЗаполненияОбластиШапки(ВыборкаПоШапке,КодЯзыкаПечать);
				...
				Если ТипЗнч(ВыборкаПоШапке.Ссылка) = Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
 					...
 					ПараметрыОснования = ПараметрыЗаполненияОбластиОснования(ВыборкаПоШапке.Ссылка, ВыборкаПоРаспоряжениям, КодЯзыкаПечать);
				...
				ОбластьРабочийУчасток	= Макет.ПолучитьОбласть("РабочийУчасток"	+ СуффиксОбласти);
				...
				Пока ВыборкаРабочиеУчастки.Следующий() Цикл
					...
					Пока ВыборкаПоТовару.Следующий() Цикл
						...
						Если ВыводитьКоды Тогда
							ПараметрыОбластиСтрокиТЧ.Вставить("Артикул", ВыборкаПоТовару[КолонкаКодов]);
						КонецЕсли;
...
Функция ПредставлениеКомандыПечати(ТипЗадания)
	...
	Если ТипЗадания = "ЗаданиеНаРазмещение" Тогда
		ПредставлениеКоманды = НСтр("ru='Задание на размещение товаров';uk='Завдання на розміщення товарів'");
	ИначеЕсли ТипЗадания = "ЗаданиеНаОтбор" Тогда
		ПредставлениеКоманды = НСтр("ru='Задание на отбор товаров';uk='Завдання на відбір товарів'");;
	ИначеЕсли ТипЗадания = "ЗаданиеНаПересчет" Тогда
		ПредставлениеКоманды = НСтр("ru='Бланк задания на пересчет товаров (с ячейками справочно)';uk='Бланк завдання на перерахунок товарів (з комірками довідково)'");
	Иначе
		ПредставлениеКоманды = НСтр("ru='Задание на добор товаров';uk='Завдання на добір товарів'");
	КонецЕсли;
...
Функция НазваниеПечатнойФормы(ТипЗадания, КодЯзыкаПечать)
	
	Если ТипЗадания = "ЗаданиеНаРазмещение" Тогда
 		ТекстЗаголовка = НСтр("ru='Задание на размещение товаров';uk= 'Завдання на розміщення товарів'", КодЯзыкаПечать);
	ИначеЕсли ТипЗадания = "ЗаданиеНаОтбор" Тогда
 		ТекстЗаголовка = НСтр("ru='Задание на отбор товаров';uk= 'Завдання на відбір товарів'", КодЯзыкаПечать);
	ИначеЕсли ТипЗадания = "ЗаданиеНаДобор" Тогда
		ТекстЗаголовка = НСтр("ru='Задание на добор товаров';uk= 'Завдання на добір товарів'", КодЯзыкаПечать);
	Иначе
		ТекстЗаголовка = НСтр("ru='Задание на пересчет товаров';uk= 'Завдання на перерахунок товарів'", КодЯзыкаПечать);
	КонецЕсли;
...
Функция ПолучитьДанныеЗаданияНаОтборРазмещениеТоваров(ПравилаПечати, ИмяДокумента, ТипЗадания, МассивОбъектов)
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаВТТоварыЗадания());
...
Функция ШаблонТекстаЗапросаДанныхЗадания(ЭтоВременнаяТаблица)
	...
	|	ДанныеЗадания.СсылкаПредставление				КАК СсылкаПредставление,
	|	ДанныеЗадания.Номер								КАК Номер,
	|	ДанныеЗадания.Дата								КАК Дата,
	|	ДанныеЗадания.Склад								КАК Склад,
	|	ДанныеЗадания.СкладПредставление				КАК СкладПредставление,
	|	ДанныеЗадания.ПомещениеПредставление			КАК ПомещениеПредставление,
	...
	ТекстПоместить		= ?(ЭтоВременнаяТаблица, "ПОМЕСТИТЬ ДанныеЗадания", "");
	ШаблонТекстаЗапроса	= СтрЗаменить(ШаблонТекстаЗапроса, "//&ТекстПоместить", ТекстПоместить);
...
Функция ТекстЗапросаДанныхДокумента(ИмяДокумента, ПравилоПечати)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&ТекстУсловияДанныхДокумента",	ТекстУсловия);
...
Функция ТекстЗапросаВТТоварыЗадания()
...
|;uk='Неможливо надрукувати %НазваниеФормы% для ""%Документ%"", 
|тому що на складі ""%Склад%"" використовується ордерна схема, тому друк завдання повинен вестися з ордера.'");
...
|;uk='Неможливо надрукувати %НазваниеФормы% для ""%Документ%"",
|тому що для складу (приміщення) з осередками, використовуваними для зберігання залишків номенклатури, ця друкована форма не передбачена.
|Друк завдання на відбір (розміщення) можлива з документа ""Відбір (розміщення) товарів"".'");
...
Функция СуффиксОбласти(ИспользуетсяСправочноеХранение, ИспользоватьУпаковки, ВыводитьКоды)
	
	СуффиксОбласти = "";
	
	Если Не ИспользуетсяСправочноеХранение Тогда
		СуффиксОбласти = "БезЯчеек";
	КонецЕсли;
	
	Если Не ИспользоватьУпаковки Тогда
		СуффиксОбласти = СуффиксОбласти + "БезУпаковок";
	КонецЕсли;
	
	Если Не ВыводитьКоды Тогда
		СуффиксОбласти = СуффиксОбласти + "БезКодов";
	КонецЕсли;
	
	СуффиксОбласти = ?(ЗначениеЗаполнено(СуффиксОбласти), СуффиксОбласти, "Основа");
	
	Возврат СуффиксОбласти;
	
КонецФункции

Функция ПараметрыЗаполненияОбластиОснования(ДокументСсылка, ВыборкаПоРаспоряжениям, КодЯзыкаПечать)
	...
	Пока ВыборкаПоРаспоряжениям.НайтиСледующий(ОтборРаспоряжений) Цикл
		...
		РеквизитыДокумента.Представление	= ВыборкаПоРаспоряжениям.ОснованиеПредставление;
 		...
 		ТекстыПредставлений.Добавить(ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(РеквизитыДокумента,
									 МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ВыборкаПоРаспоряжениям.ОснованиеСсылка, КодЯзыкаПечать),КодЯзыкаПечать));
	КонецЦикла;
	
 	ЗаголовокОснования		= ?(ТекстыПредставлений.Количество() > 1, 
	                            НСтр("ru='Основания:';uk= 'Підстави:'", КодЯзыкаПечать), 
...
Функция ПараметрыЗаполненияОбластиШапки(ДанныеДокумента,КодЯзыкаПечать)
	...
	ПредставлениеРаспоряжения = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
        РеквизитыДокумента,
     	МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ДанныеДокумента.Ссылка, КодЯзыкаПечать),
        КодЯзыкаПечать,
    	Истина
    );
...
Функция ПараметрыЗаполненияОбластиШапкиТаблицы(ВыводитьКоды, КолонкаКодов, ИспользоватьУпаковки, ТипЗадания, КодЯзыкаПечать)
	...
	Если ИспользоватьУпаковки Тогда
		Если ТипЗадания <> "ЗаданиеНаПересчет" Тогда
 			ПараметрыЗаполнения.Вставить("НадписьКоличествоУпаковки", НСтр("ru='(упаковок)';uk= '(упаковок)'", КодЯзыкаПечать));
		Иначе
 			ПараметрыЗаполнения.Вставить("НадписьКоличество",			НСтр("ru='(Факт)';uk= '(Факт)'", КодЯзыкаПечать));
			ПараметрыЗаполнения.Вставить("НадписьКоличествоУпаковки",	НСтр("ru='(По учету)';uk= '(По обліку)'", КодЯзыкаПечать));
		КонецЕсли;
	КонецЕсли;
