...
Процедура ДобавитьКодЭлемента(Родитель, ТабличныеЧасти, Отступ = "	")
	...
	Для Каждого Элемент Из Родитель.Строки Цикл
		...
		Если Элемент.ЭтоГруппа Тогда
			...
			Код1С.ДобавитьСтроку(СтрШаблон("%1Элемент = Элементы.Добавить();  // см. СтрокаТЧПредопреденногоЭлемента", Отступ));
			Код1С.ДобавитьСтроку(СтрШаблон("%1Элемент.ИмяПредопределенныхДанных = ""%2"";", Отступ, Элемент.ID));
			Код1С.ДобавитьСтроку(СтрШаблон("%1МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, ""Наименование"", ""ru = '%2'"", КодыЯзыков); // @НСтр" + Символы.ПС, Отступ, Элемент.Наименование));
			ДобавитьКодЭлемента(Элемент, ТабличныеЧасти, Символы.Таб + Отступ);
			Код1С.ДобавитьСтроку(СтрШаблон("%1#КонецОбласти //%2" + Символы.ПС, Отступ, Элемент.ID));
			Продолжить;
		КонецЕсли;
		...
		Код1С.ДобавитьСтроку(СтрШаблон("%1МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, ""Наименование"", ""ru = '%2'"", КодыЯзыков); // @НСтр", Отступ, НаименованиеЭлемента));
		...
		ЕстьПоказатели = Булево(Элемент.ИспользоватьВМеждународномУчете) И НЕ ПустаяСтрока(Элемент.ИсточникДанных);
		Если ЕстьПоказатели Тогда
			Код1С.ДобавитьСтроку("");
			Код1С.ДобавитьСтроку(СтрШаблон("%1//++ НЕ УТ", Отступ));
