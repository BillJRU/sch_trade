...
//Параметры: 
//	Форма - УправляемаяФорма - форма документа BAS Документооборота
...
//
// Параметры:
//   Форма - УправляемаяФорма - управляемая форма документа BAS Документооборота
...
Функция ЗаполнитьДеревоСвязейИНачатьПолучениеФайлов(Форма, ОбъектXDTO, 
	...
	Если ОбъектXDTO.Свойства().Получить("relations") = Неопределено
		...
		Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
			ЭлементДляОбновленияЗаголовка.Заголовок = НСтр("ru='Связи';uk='Зв''язки'");
		КонецЕсли;
	...
	Для Каждого ПодходящийОбъект Из ПодходящиеОбъекты Цикл
		...
		Исключение
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Не удалось получить связанный объект Документооборота при автоматическом создании связей: %1';uk='Не вдалося отримати пов''язаний об''єкт Документообігу при автоматичному створенні зв''язків: %1'"),
		...
		Иначе // предопределенный
			ТипСвязи = Новый Структура("ID, Тип, Заголовок",
				"",
				"DMRelationType",
				НСтр("ru='Содержит ссылку на';uk='Містить посилання на'"));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидДокументаID) И ЗначениеЗаполнено(ВидДокументаТип) Тогда
		...
		Исключение
			ВызватьИсключение СтрШаблон(
				НСтр("ru='Вид документа ""%1"" не найден в BAS Документообороте';uk='Вид документа ""%1"" не знайдено в BAS Документообігу'"),
				Форма.ВидДокумента);
		КонецПопытки;
...
Функция ПроверитьВозможностьСозданияДокументаНеПоШаблону(Форма, documentType) Экспорт
	...
	Если documentType <> Неопределено
		...
		Сообщение.Текст = СтрШаблон(
			НСтр("ru='В настройках вида документа ""%1"" установлен запрет на создание документов не по шаблону.';uk='В настройках виду документа ""%1"" встановлено заборону на створення документів не за шаблоном.'"),
			Форма.ВидДокумента);
...
//Параметры:
//	Форма - УправляемаяФорма - форма объекта или списка BAS Документооборота
Процедура ОбработатьФормуПриНедоступностиФункционалаВерсииСервиса(Форма) Экспорт
	...
	Форма.Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
		НСтр("ru='Функционал не поддерживается в данной версии BAS Документооборота.';uk='Функціонал не підтримуються в цій версії BAS Документообігу.'");
...
Процедура ЗаполнитьСтандартнуюШапкуБизнесПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если Форма.Элементы.Найти("ГруппаСостояние") <> Неопределено Тогда
		Если Форма.СостояниеID = "Остановлен" Тогда
			...
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Остановлен';uk='Зупинений'");
			Форма.КартинкаСостояние = 1;
		ИначеЕсли Форма.СостояниеID = "Прерван" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Прерван';uk='Перерваний'");
			Форма.КартинкаСостояние = 2;
		ИначеЕсли ОбъектXDTO.completionMark = "ReadyToStart" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Старт отложен';uk='Старт відкладено'");
			Форма.КартинкаСостояние = 3;
		ИначеЕсли ОбъектXDTO.completionMark = "StartCanceled" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Старт отменен';uk='Старт відмінний'");
			Форма.КартинкаСостояние = 4;
		ИначеЕсли ОбъектXDTO.completionMark = "ReadyToExecute" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Процесс находится в очереди для запуска. Его старт произойдет автоматически в ближайшее время.';uk='Процес знаходиться у черзі для запуску. Його старт відбудеться автоматично найближчим часом.'");
			Форма.КартинкаСостояние = 0;
		ИначеЕсли ОбъектXDTO.completionMark = "ExecutionCanceled" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
			Форма.Элементы.ТекстСостояние.Заголовок = НСтр("ru='Ошибка при старте процесса';uk='Помилка при старті процесу'");
			Форма.КартинкаСостояние = 0;
		Иначе
	...
	Если Не ЗначениеЗаполнено(Форма.ID) Тогда
		Форма.Заголовок = СтрШаблон(НСтр("ru='%1 (Создание)';uk='%1 (Створення)'"), Форма.Заголовок);
...
//   Форма - УправляемаяФорма, Структура - управляемая форма или структура реквизитов документа BAS Документооборота.
//
...
Функция ПодготовитьЛистСогласования(ПредметыСогласования)
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.8.1") Тогда
		ВызватьИсключение НСтр("ru='Для вывода листа согласования нужен BAS Документооборот версии не ниже 1.4.8.1';uk='Для виведення листа погодження потрібен BAS Документообіг версії не нижче 1.4.8.1'");
	КонецЕсли;
