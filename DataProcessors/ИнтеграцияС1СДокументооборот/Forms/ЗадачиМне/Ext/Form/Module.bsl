...
|При підтвердженні вони будуть виключені зі списку учасників. Продовжити?'");
	...
	Если ПроцессТип = "DMBusinessProcessConsideration" И ТочкаМаршрута = "Рассмотреть" Тогда
		...
		ТекстСообщенияПользователю = НСтр("ru='Поле ""Резолюция"" не заполнено';uk='Поле ""Резолюція"" не заповнена'");
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	ИначеЕсли СтрокаЗадачи.ЗадачаТип = "DMBusinessProcessIssuesSolutionTaskAnswer" Тогда
		ТребоватьРезультатВыполнения = Истина;
		ТекстСообщенияПользователю = НСтр("ru='При отправке на уточнение требуется указать комментарий.';uk='При відправленні на уточнення потрібно вказати коментар.'");
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" И ТочкаМаршрута = "Утвердить" Тогда
		ТребоватьРезультатВыполнения = Истина;
		ТекстСообщенияПользователю = НСтр("ru='Укажите причину отклонения документа';uk='Вкажіть причину відхилення документа'");
	ИначеЕсли ПроцессТип = "DMBusinessProcessRegistration" И ТочкаМаршрута = "Зарегистрировать" Тогда
		ТребоватьРезультатВыполнения = Истина;
		ТекстСообщенияПользователю = НСтр("ru='Укажите причину отказа в регистрации';uk='Укажіть причину відмови в реєстрації'");
	КонецЕсли;
	...
	Иначе
		...
		Если ВерсияСервиса = "0.0.0.0" Тогда
			...
			Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
				НСтр("ru='Нет доступа к BAS Документообороту.';uk='Немає доступу до BAS Документообігу.'");
...
Процедура ЗаполнитьДанныеБПУтверждение(СтрокаЗадачи, ЗадачаXDTO)
	...
	Если СтрокаЗадачи.ЭтоПодписание Тогда
		Если СтрокаЗадачи.РезультатID = "Утверждено" Тогда
			СтрокаЗадачи.Результат = НСтр("ru='Подписано';uk='Підписано'");
		Иначе
			СтрокаЗадачи.Результат = НСтр("ru='Не подписано';uk='Не підписано'");
		КонецЕсли;
	КонецЕсли;
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
							НСтр("ru='Ошибка при получении представления связанного объекта';uk='Помилка при отриманні представлення зв''язаного об''єкта'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
Процедура ОбновитьПредставлениеЗадачиНаСервере(РеквизитыЗадачи, ДетальныеСведенияЗадачи)
	...
	Элементы.Комментарий.ПодсказкаВвода = НСтр("ru='Ваш комментарий';uk='Ваш коментар'");
...
Процедура ОбновитьПредставлениеЗадачиБПРассмотрение(РеквизитыЗадачи)
	...
	Если РеквизитыЗадачи.ТочкаМаршрутаКратко = "Рассмотреть" Тогда
		Элементы.Комментарий.ПодсказкаВвода = НСтр("ru='Ваша резолюция';uk='Ваша резолюція'");
	КонецЕсли;
