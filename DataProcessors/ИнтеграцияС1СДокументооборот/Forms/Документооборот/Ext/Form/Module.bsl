...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(ТипСоздаваемогоОбъекта) Тогда
		...
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMCorrespondent" Тогда 
			...
			Иначе
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Контрагент:';uk='Контрагент:'");
			КонецЕсли;
		КонецЕсли;
		Элементы.ГруппаИнфо.Видимость = Ложь;
	...
	Иначе // нет подходящих правил
		...
		Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Соответствие в BAS Документообороте';uk='Відповідність в BAS Документообігу'");
		Если ПравоДоступа("Добавление", Метаданные.Справочники.ПравилаИнтеграцииС1СДокументооборотом) Тогда
			Строки = Новый Массив;
			Строки.Добавить(СтрШаблон(
				НСтр("ru='Не настроено заполнение документа BAS Документооборота из ""%1"".';uk='Не налаштоване заповнення документа BAS Документообігу з ""%1"".'"), ИмяПотребителя));
			Строки.Добавить(" ");
			Строки.Добавить(Новый ФорматированнаяСтрока(
				НСтр("ru='Настроить.';uk='Настроїти.'"),,,, МетаданныеПотребителя.ПолноеИмя()));
...
Процедура ДекорацияОбъектВыбратьНажатие(Элемент)
	...
	Иначе // тип неизвестен, предложим выбор пользователю
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Выбрать в BAS Документообороте';uk='Вибрати в BAS Документообігу'"));
		...
		Иначе
			...
			Иначе // нет подходящих правил
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = 
					НСтр("ru='Соответствие в BAS Документообороте:';uk='Відповідність в BAS Документообігу:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура СоздатьФайлСДиска()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторКорневойПапки1СДокументооборот) Тогда
		ПоказатьПредупреждение(, 
			НСтр("ru='Не указана корневая папка для хранения файлов BAS Документооборота.';uk='Не вказана коренева папка для зберігання файлів BAS Документообігу.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДеревоСвязей(Знач ОбъектXDTO = Неопределено, ДобавленныйДокумент = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ДокументID) Тогда
		...
		Элементы.ГруппаСвязанныеДокументы.Заголовок = НСтр("ru='Связи';uk='Зв''язки'");
		Возврат;
	КонецЕсли;
