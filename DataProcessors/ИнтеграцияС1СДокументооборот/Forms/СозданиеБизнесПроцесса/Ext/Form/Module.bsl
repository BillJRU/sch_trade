...
Процедура УстановитьЗаголовокПредметаПоТипуОбъекта()
	...
	ИначеЕсли ПредметТип = "DMProject" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Проект';uk='Проєкт'");
		
	ИначеЕсли ПредметТип = "DMProjectTask" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Проектная задача';uk='Проєктна задача'");
...
Процедура ЗаполнитьДеревоШаблонов(ПолучатьВидДокумента = Ложь)
	...
	Если ИспользуетсяОтправкаПоПочте() Тогда
		...
		ОтправитьПоПочте.Наименование = НСтр("ru='По почте';uk='Поштою'");
		ОтправитьПоПочте.Подсказка = НСтр("ru='Создание почтового сообщения для отправки.';uk='Створення поштового повідомлення для відправки.'");
...
Процедура ОбработатьРезультат(Дерево, РезультатДеревоШаблонов)
	...
	Для Каждого Раздел из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если РазделДерева.Наименование = НСтр("ru='Другие шаблоны';uk='Інші шаблони'") Тогда
...
Процедура ОбработатьРезультат_2_1_16_2(Дерево, РезультатДеревоШаблонов)
	...
	Для Каждого Раздел из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если Раздел.name = НСтр("ru='Рекомендованные шаблоны';uk='Рекомендовані шаблони'") Тогда
		...
		Иначе
			...
			Если РазделДерева.Наименование = НСтр("ru='Другие варианты';uk='Інші варіанти'") Тогда
				РазделДерева.Развернуть = Ложь;
			Иначе
				РазделДерева.Развернуть = Истина;
			КонецЕсли;
