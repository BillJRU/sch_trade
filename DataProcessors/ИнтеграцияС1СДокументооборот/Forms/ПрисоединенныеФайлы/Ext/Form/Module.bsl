...
Процедура ФайлыПеретаскиваниеЗавершение(Результат, МассивФайлов) Экспорт
	...
	Если СостояниеРазрешаетДобавлениеСканКопии
		...
		ТекстВопроса = НСтр("ru='В текущем состоянии можно добавить только скан-копию оригинала документа. Продолжить?';uk='У поточному стані можна додати тільки скан-копію оригіналу документу. Продовжити?'");
		ЗаголовокВопроса = НСтр("ru='Добавление файла';uk='Додавання файлу'");
...
Процедура Печать(Команда)
	...
	#Если ВебКлиент Тогда 
		ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте печать файлов не поддерживается.';uk='У Вебклієнті друк файлів не підтримується.'"));
		Возврат;
	#КонецЕсли
...
Процедура ОбновитьПредпросмотр(ТекущаяСтрока)
	...
	Иначе
		АдресДанныхФайла = "";
		Элементы.КартинкаПредпросмотра.ТекстНевыбраннойКартинки = НСтр("ru='Предпросмотр файлов этого типа не поддерживается.';uk='Попередній перегляд файлів цього типу не підтримується.'");
	КонецЕсли;
...
Функция ПараметрыИсходящегоПисьма(Знач Владелец, Знач Вложения, Знач УникальныйИдентификатор)
	...
	Тема = СтрШаблон(НСтр("ru='%1, %2 к документу %3 %4 от %5';uk='%1, %2 до документу %3 %4 від %5'"),
		?(ОдноВложение, НСтр("ru='Файл';uk='Файл'"), НСтр("ru='Файлы';uk='Файли'")),
		?(ОдноВложение, НСтр("ru='присоединенный';uk='приєднаний'"), НСтр("ru='присоединенные';uk='приєднані'")),
		ТипЗнч(Владелец),
		НомерДокумента,
		ДатаДокумента);
	...
	// Текст.
	Текст = СтрШаблон(НСтр("ru='К письму %1 %2:';uk='До листа %1 %2:'"),
		?(ОдноВложение, НСтр("ru='присоединен';uk='приєднаний'"), НСтр("ru='присоединены';uk='приєднані'")),
		?(ОдноВложение, НСтр("ru='файл';uk='файл'"), НСтр("ru='файлы';uk='файли'")));
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		Текст = Текст + Символы.ПС + СтрШаблон(НСтр("ru='- %1';uk='- %1'"), Вложение.Представление);
	КонецЦикла;
