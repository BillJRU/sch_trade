...
Процедура ЗаполнитьСписокТиповИсточника(СписокТипов) 
	...
	ТипыИсточника.Вставить("DMProject", НСтр("ru='Проект';uk='Проєкт'"));
	ТипыИсточника.Вставить("DMProjectTask", НСтр("ru='Проектная задача';uk='Проєктна задача'"));
...
Процедура СформироватьЗаголовокФормы()
	...
	Если Не ЗначениеЗаполнено(ID) Тогда
		Заголовок = НСтр("ru='Ежедневный отчет (создание)';uk='Щоденний звіт (створення)'");
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ежедневный отчет за %1';uk='Щоденний звіт за %1'"), Формат(Дата, "ДЛФ=D"));
	КонецЕсли;
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Если ПустаяСтрока(ВерсияСервиса) Тогда // идет подключение
		...
		Заголовок = НСтр("ru='Ежедневный отчет (ожидается подключение...)';uk='Щоденний звіт (очікується підключення ...)'");
		
	Иначе
		
		Заголовок = НСтр("ru='Ежедневный отчет';uk='Щоденний звіт'");
