...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Если ТипЗнч(Параметры.ВнешнийОбъект) = Тип("Структура") Тогда
			...
			Если Найти(ВнешнийОбъектТип,"Document") <> 0 
			 ...
			 Или ВнешнийОбъектТип = "DMCorrespondent" Тогда
				Заголовок = НСтр("ru='Процессы и задачи';uk='Процеси й задачі'");
			ИначеЕсли Найти(ВнешнийОбъектТип,"BusinessProcess") <> 0 Тогда 
				Заголовок = НСтр("ru='Задачи процесса %Объект%';uk='Задачі процесу %Объект%'");
	...
	Если ЗначениеЗаполнено(СсылкаНаВнешнийОбъект) Тогда
		Заголовок = НСтр("ru='Задачи по ""%Объект%""';uk='Задачі по ""%Объект%""'");
...
Функция ЭтоСлужебнаяЗадача(Задача)
	...
	Если Задача.parentBusinessProcess.objectId.type = "DMComplexBusinessProcess" Тогда 
		Если Задача.businessProcessStep = "ВложенныйПроцесс"
			ИЛИ Задача.businessProcessStep = "Выполнить все действия процесса"
			ИЛИ Задача.businessProcessStep = "Виконати усі дії процеса" Тогда
			Служебная = Истина;
		КонецЕсли;
