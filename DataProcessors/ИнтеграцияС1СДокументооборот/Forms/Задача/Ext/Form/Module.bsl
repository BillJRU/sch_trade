...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РезультатВыполнения.ПодсказкаВвода = НСтр("ru='Ваш комментарий';uk='Ваш коментар'");
...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	Если ЭтоТочкаМаршрута("Зарегистрировать") ИЛИ ЭтоТочкаМаршрута("Зареєструвати") Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConsideration" и (ТочкаМаршрута = "Рассмотреть" ИЛИ ТочкаМаршрута = "Розглянути")
			И Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Резолюция"" не заполнено';uk='Поле ""Резолюція"" не заповнена'"),,
			"РезультатВыполнения");
		Возврат;
	КонецЕсли;
...
|;uk='Є учасники, що не прийняли запрошення. 
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	Если (ПроцессТип = "DMBusinessProcessApproval" И (ЭтоТочкаМаршрута("Согласовать") ИЛИ ЭтоТочкаМаршрута("Погодити")))
			Или (ПроцессТип = "DMBusinessProcessInvitation" И (ЭтоТочкаМаршрута("Пригласить") ИЛИ ЭтоТочкаМаршрута("Запросити"))) Тогда			
	...
	ИначеЕсли Тип = "DMBusinessProcessIssuesSolutionTaskAnswer" Тогда
		Если Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
			ОчиститьСообщения();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='При отправке на уточнение требуется указать комментарий.';uk='При відправленні на уточнення потрібно вказати коментар.'"),,
				"РезультатВыполнения");
			Возврат;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" И (ЭтоТочкаМаршрута("Утвердить") ИЛИ ЭтоТочкаМаршрута("Затвердити")) Тогда
		Если Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
			ОчиститьСообщения();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			...
			Возврат;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessRegistration" И (ЭтоТочкаМаршрута("Зарегистрировать") ИЛИ ЭтоТочкаМаршрута("Зареєструвати")) Тогда
		Если Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
			ОчиститьСообщения();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите причину отказа в регистрации';uk='Укажіть причину відмови в реєстрації'"),,
				"РезультатВыполнения");
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПроцессТип = "DMBusinessProcessApproval" И (ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися")) Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" И (ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися")) Тогда
		ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.БизнесПроцессУтверждение",
			ПараметрыФормы,,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	ИначеЕсли ПроцессТип = "DMBusinessProcessRegistration" И (ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися")) Тогда
		ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.БизнесПроцессРегистрация",
			ПараметрыФормы,,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	ИначеЕсли ПроцессТип = "DMBusinessProcessInvitation" И (ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися")) Тогда
...
Процедура ОтменитьПринятиеКИсполнению(Команда)
	...
	Если ОтменитьПринятиеКИсполнениюНаСервере() Тогда
		...
		Состояние(НСтр("ru='Задача помечена как НЕ принятая к исполнению!';uk='Задачі позначене як НЕ прийнята до виконання!'"));
...
Процедура ИзменитьДатуВыполнения(Команда)
	...
	ПоказатьВводДаты(
		...
		ДатаИсполнения,
		НСтр("ru='Укажите дату фактического выполнения задачи';uk='Вкажіть дату фактичного виконання задачі'"),
		ЧастиДаты.ДатаВремя);
...
Процедура ЗаполнитьФормуЗадачи(Параметры)
	...
	Если СостояниеID = "Остановлен" Тогда
		...
		Элементы.ТекстСостояние.Заголовок = НСтр("ru='Задача остановлена';uk='Задача зупинена'");
		КартинкаСостояние = 1;
	ИначеЕсли СостояниеID = "Прерван" Тогда
		Элементы.ГруппаСведенияОЗадаче.Видимость = Истина;
		Элементы.ТекстСостояние.Заголовок = НСтр("ru='Задача прервана';uk='Задача перерване'");
		КартинкаСостояние = 2;
	ИначеЕсли ОбъектXDTO.executionMark = "ReadyToExecute" Тогда
		Элементы.ГруппаСведенияОЗадаче.Видимость = Истина;
		Элементы.ТекстСостояние.Заголовок = НСтр("ru='Задача находится в очереди для выполнения. Выполнение задачи произойдет автоматически в ближайшее время.';uk='Задача знаходиться у черзі для виконання. Виконання задачі станеться автоматично найближчим часом.'");
		КартинкаСостояние = 9;
	ИначеЕсли ОбъектXDTO.executionMark = "ExecutionCanceled" Тогда
		Элементы.ГруппаСведенияОЗадаче.Видимость = Истина;
		Элементы.ТекстСостояние.Заголовок = НСтр("ru='Ошибка при выполнении задачи';uk='Помилка при виконанні задачі'");
	...
	Если Элементы.Найти("ПолноеОписаниеЗадачи") <> Неопределено Тогда
		...
		Если ПроцессОбъект.Свойства().Получить("state") <> Неопределено
			...
			Если ПроцессОбъект.state.name = "Остановлен" Тогда
				...
				КонецЕсли;
				ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + НСтр("ru='Задача остановлена';uk='Задача зупинена'") + Символы.ПС;
			ИначеЕсли ПроцессОбъект.state.name = "Прерван" Тогда
				Если Не ПустаяСтрока(ПолноеОписаниеЗадачи) Тогда
					ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + Символы.ПС;
				КонецЕсли;
				ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + НСтр("ru='Задача прервана';uk='Задача перерване'") + Символы.ПС;
			КонецЕсли;
		...
		ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
			+ СтрШаблон(НСтр("ru='Важность: %1';uk='Важливість: %1'"), ОбъектXDTO.importance.objectID.id)
			+ Символы.ПС;
		...
		Если ЗначениеЗаполнено(ОбъектXDTO.dueDate) Тогда
			...
			КонецЕсли;
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
				+ СтрШаблон(НСтр("ru='Срок: %1';uk='Строк: %1'"), Формат(ОбъектXDTO.dueDate, "dd.MM.yyyy HH:mm"));
		КонецЕсли;
		...
		Если ОбъектXDTO.Установлено("author") Тогда
			...
			КонецЕсли;
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
				+ СтрШаблон(НСтр("ru='Автор: %1';uk='Автор: %1'"), ОбъектXDTO.author.name);
		КонецЕсли;
...
Процедура УстановитьТочкуМаршрута(ИмяТочкиМаршрута)
	...
	Если ПроцессТип = "DMBusinessProcessApproval" Тогда
		Если ИмяТочкиМаршрута = "Ознакомиться с результатом согласования" ИЛИ ИмяТочкиМаршрута = "Ознайомитися з результатом погодження" Тогда	
			ТочкаМаршрута = "Ознакомиться";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessOrder" Тогда
		Если ИмяТочкиМаршрута = "Контролировать поручение" ИЛИ ИмяТочкиМаршрута = "Контролювати доручення" Тогда
			ТочкаМаршрута = "Контролировать";
		ИначеЕсли ИмяТочкиМаршрута = "Проверить поручение" ИЛИ ИмяТочкиМаршрута = "Перевірити доручення" Тогда
			ТочкаМаршрута = "Проверить";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessPerformance" Тогда
		Если ИмяТочкиМаршрута = "Ответственное исполнение" ИЛИ ИмяТочкиМаршрута = "Відповідальне виконання" Тогда
			ТочкаМаршрута = "ОтветственноеИсполнение";
		ИначеЕсли ИмяТочкиМаршрута = "Контролировать исполнение" ИЛИ ИмяТочкиМаршрута = "Контролювати виконання" Тогда
			ТочкаМаршрута = "Контролировать";
		ИначеЕсли ИмяТочкиМаршрута = "Проверить исполнение" ИЛИ ИмяТочкиМаршрута = "Перевірити виконання" Тогда
			ТочкаМаршрута = "Проверить";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessRegistration" Тогда
		Если ИмяТочкиМаршрута = "Ознакомиться с регистрацией" ИЛИ ИмяТочкиМаршрута = "Ознайомитися з реєстрацією" Тогда
			ТочкаМаршрута = "Ознакомиться";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" Тогда
		Если Найти(ИмяТочкиМаршрута, "Ознакомиться с результатом утверждения") <> 0 ИЛИ Найти(ИмяТочкиМаршрута, "Ознайомитися з результатом затвердження") <> 0 Тогда
			ТочкаМаршрута = "Ознакомиться";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessConsideration" Тогда
		Если ИмяТочкиМаршрута = "Обработать резолюцию" ИЛИ ИмяТочкиМаршрута = "Обробити резолюцію" Тогда
			ТочкаМаршрута = "Ознакомиться";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMComplexBusinessProcess" Тогда
		Если ИмяТочкиМаршрута = "Контролировать ход процесса" ИЛИ ИмяТочкиМаршрута = "Контролювати хід процесу" Тогда
			ТочкаМаршрута = "Контролировать";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessIncomingDocumentProcessing" Тогда
		Если ИмяТочкиМаршрута = "Исполнение \ ознакомление"  ИЛИ ИмяТочкиМаршрута = "Виконання \ ознайомлення" Тогда
			ТочкаМаршрута = "ИсполнениеОзнакомление";
		ИначеЕсли ИмяТочкиМаршрута = "В дело" ИЛИ ИмяТочкиМаршрута = "У справу" Тогда
			ТочкаМаршрута = "ВДело";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessInternalDocumentProcessing" Тогда
		Если ИмяТочкиМаршрута = "Рассмотрение" ИЛИ ИмяТочкиМаршрута = "Розгляд" Тогда
			ТочкаМаршрута = "ВложенныйПроцесс1";
		ИначеЕсли ИмяТочкиМаршрута = "Исполнение \ ознакомление" ИЛИ ИмяТочкиМаршрута = "Виконання \ ознайомлення" Тогда
		...
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" Тогда
		Если ИмяТочкиМаршрута = "Рассмотреть вопрос" ИЛИ ИмяТочкиМаршрута = "Розглянути питання" Тогда
			ТочкаМаршрута = "РассмотрениеИнициатором";
		ИначеЕсли ИмяТочкиМаршрута = "Ознакомиться с результатом рассмотрения" ИЛИ ИмяТочкиМаршрута = "Ознайомитися з результатом розгляду" Тогда
			ТочкаМаршрута = "ОзнакомлениеСРезультатомРассмотрения";
		Иначе
			ТочкаМаршрута = ИмяТочкиМаршрута;
		КонецЕсли;
	ИначеЕсли ПроцессТип = "DMBusinessProcessInvitation" Тогда
		Если ИмяТочкиМаршрута = "Пригласить" ИЛИ ИмяТочкиМаршрута = "Запросити" Тогда
			ТочкаМаршрута = "Пригласить";
		ИначеЕсли ИмяТочкиМаршрута = "Ознакомиться с результатом приглашения" ИЛИ ИмяТочкиМаршрута = "Ознайомитися з результатом запрошення" Тогда	
			ТочкаМаршрута = "Ознакомиться";
		ИначеЕсли ИмяТочкиМаршрута = "Оповестить о результатах приглашения" ИЛИ ИмяТочкиМаршрута = "Сповістити про результати запрошення" Тогда
...
Процедура ЗаполнитьДанныеБПСогласование(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Согласовать") ИЛИ ЭтоТочкаМаршрута("Погодити") Тогда
...
|з зауваженнями'");
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися") Тогда		

		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Ознакомился';uk='Ознайомився'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Повторить согласование...';uk='Повторити погодження...'");
		...
		Элементы.ИсполнителиИсполнитель.Заголовок = НСтр("ru='Согласующее лицо';uk='Особа, яка погоджує'");
		...
		Если РезультатID = "НеСогласовано" Тогда 
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Завершить согласование';uk='Завершити погодження'");
		Иначе	
...
Процедура ЗаполнитьДанныеБПУтверждение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Утвердить") ИЛИ ЭтоТочкаМаршрута("Затвердити") Тогда	

		Если ЭтоПодписание Тогда
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Подписать';uk='Підписати'");
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Отклонить';uk='Відхилити'");
		Иначе
		...
		Если Выполнена Тогда
			Если ЭтоПодписание Тогда
				Если РезультатID = "Утверждено" Тогда
					Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Подписано.';uk='Підписано.'");
				Иначе
					Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Не подписано.';uk='Не підписано.'");
				КонецЕсли;
			Иначе
				Элементы.ТекстРезультатаВыполнения.Заголовок = Строка(Результат) + ".";
			КонецЕсли;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Ознакомился';uk='Ознайомився'");
		Если ЭтоПодписание Тогда
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Повторить подписание...';uk='Повторити підписання...'");
			Элементы.РезультатУтверждения.Заголовок = НСтр("ru='Результат подписания';uk='Результат підписання'");
			Элементы.Утверждающий.Заголовок = "Подписант";
			Элементы.ДатаУтверждения.Заголовок = "Дата подписания";
		Иначе
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Повторить утверждение...';uk='Повторити затвердження...'");
		КонецЕсли;
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		...
		Если ЭтоПодписание Тогда
			Если РезультатID = "Утверждено" Тогда
				Результат = НСтр("ru='Подписано';uk='Підписано'");
			Иначе
				Результат = НСтр("ru='Не подписано';uk='Не підписано'");
			КонецЕсли;
		КонецЕсли;
		Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтаФорма,
		...
		Если РезультатID = "НеУтверждено" Тогда
			Если ЭтоПодписание Тогда
				Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Завершить подписание';uk='Завершити підписання'");
			Иначе
				Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Завершить утверждение';uk='Завершити затвердження'");
			КонецЕсли;
		Иначе
			Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		КонецЕсли;
		...
		Если ОтправленоНаПовторноеУтверждение <> Неопределено Тогда
			Если ОтправленоНаПовторноеУтверждение Тогда
				Если ЭтоПодписание Тогда
					Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Отправлено на повторную подпись.';uk='Відправлено на повторний підпис.'");
				Иначе
...
Процедура ЗаполнитьДанныеБППоручение(Прокси, ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Выполнить") ИЛИ ЭтоТочкаМаршрута("Виконати") Тогда
	...
	ИначеЕсли ЭтоТочкаМаршрута("Проверить") ИЛИ ЭтоТочкаМаршрута("Перевірити") Тогда
	...
	ИначеЕсли ЭтоТочкаМаршрута("Контролировать") ИЛИ ЭтоТочкаМаршрута("Контролювати") Тогда
...
Процедура ЗаполнитьДанныеБПИсполнение(Прокси, ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Исполнить") ИЛИ ЭтоТочкаМаршрута("Виконати") Тогда
	...
	ИначеЕсли ЭтоТочкаМаршрута("Проверить") ИЛИ ЭтоТочкаМаршрута("Перевірити") Тогда
		...
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Завершено.';uk='Завершене.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
...
Процедура ЗаполнитьДанныеБПОзнакомление()
	...
	Если ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися") Тогда
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Ознакомился.';uk='Ознайомився.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеБПРегистрация(ЗадачаОбъект)
	...
	Иначе
		...
		Если ЭтоТочкаМаршрута("Зарегистрировать") ИЛИ ЭтоТочкаМаршрута("Зареєструвати") Тогда
			...
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Зарегистрировать';uk='Зареєструвати'");
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Не регистрировать';uk='Не реєструвати'");
			Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
...
Процедура ЗаполнитьДанныеБПРассмотрение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Рассмотреть") ИЛИ ЭтоТочкаМаршрута("Розглянути") Тогда
		Элементы.РезультатВыполнения.ПодсказкаВвода = НСтр("ru='Ваша резолюция';uk='Ваша резолюція'");
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися") Тогда
		...
		Элементы.Резолюция.Заголовок = СтрШаблон(НСтр("ru='Резолюция (%1, %2)';uk='Резолюція (%1, %2)'"),
...
Процедура ЗаполнитьДанныеБПКомплексныйПроцесс(Прокси, ПроцессОбъект)
	...
	Если ЭтоТочкаМаршрута("Контролировать") ИЛИ ЭтоТочкаМаршрута("Контролювати") Тогда
...
Процедура ЗаполнитьДанныеБПРешениеВопросов(ЗадачаОбъект)
	...
	ИначеЕсли ЭтоТочкаМаршрута("РассмотрениеИнициатором") И ВидВопросаID = "ПереносСрока" Тогда
		...
		КонецЕсли;
		Элементы.ГруппаКомандыВыполнения.Видимость = ДоступноВыполнение;
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Перенести срок';uk='Перенести строк'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Отказать в переносе';uk='Відмовити в перенесенні'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Если ЗадачаОбъект.executionMark = "ExecutedPositive" Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Срок исполнения перенесен.';uk='Строк виконання перенесений.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
			ИначеЕсли ЗадачаОбъект.executionMark = "ExecutedNegative" Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Отказано в переносе срока.';uk='Відмовлено у перенесенні строку.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Рассмотрено.';uk='Розглянуто.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
	...
	ИначеЕсли ЭтоТочкаМаршрута("ОзнакомлениеСРезультатомРассмотрения") Тогда
		...
		Если ВидВопросаID = "ПереносСрока" Тогда
			...
			Если ЗадачаОбъект.postponingAmount = 0 Тогда
				РезультатРассмотрения = НСтр("ru='Перенос срока не согласован.';uk='Перенесення строку не погоджено.'");
				Элементы.РезультатРассмотрения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе
				РезультатРассмотрения = НСтр("ru='Автор задачи перенес срок.';uk='Автор задачі переніс строк.'");
			КонецЕсли;
		КонецЕсли;
		Если Выполнена Тогда
			Если ВозвращеноНаДоработку Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Отправлено на уточнение.';uk='Відправлено на уточнення.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаНейтральногоВыполненияЗадачи;
			Иначе
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Выполнено.';uk='Виконано.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
...
Процедура ЗаполнитьДанныеБППриглашение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Пригласить") ИЛИ ЭтоТочкаМаршрута("Запросити") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Принять приглашение';uk='Прийняти запрошення'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Отклонить приглашение';uk='Відхилити запрошення'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") ИЛИ ЭтоТочкаМаршрута("Ознайомитися") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Подтвердить приглашения';uk='Підтвердити запрошення'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("ru='Повторить приглашение...';uk='Повторити запрошення...'");
		Элементы.ВыполнитьЗадачуТретья.Заголовок = НСтр("ru='Отменить приглашение';uk='Скасувати запрошення'");
		Если Выполнена Тогда
			Если ЗадачаОбъект.invitationReturned Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Отправлено на повторное приглашение.';uk='Відправлено на повторне запрошення.'");
			Иначе	
	...
	ИначеЕсли ЭтоТочкаМаршрута("Оповестить") ИЛИ ЭтоТочкаМаршрута("Сповістити") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("ru='Ознакомился';uk='Ознайомився'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
...
Процедура ВывестиИнформациюОНедоступностиВыполненияЗадачи()
	...
	Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("ru='Выполнение задач такого типа не поддерживается в данной версии BAS Документооборота.';uk='Виконання задач такого типу не підтримується в цій версії BAS Документообігу.'");
	Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
...
Процедура ДополнитьИмяПредметаПоТипуОбъекта(ИмяПредмета, ТипОбъекта)
	...
	Если ТипОбъекта = "DMInternalDocument" Тогда
		ИмяТипаПредмета = НСтр("ru='Внутренний документ';uk='Внутрішній документ'");
	ИначеЕсли ТипОбъекта = "DMIncomingDocument" Тогда
		ИмяТипаПредмета = НСтр("ru='Входящий документ';uk='Вхідний документ'");
	ИначеЕсли ТипОбъекта = "DMOutgoingDocument" Тогда
		ИмяТипаПредмета = НСтр("ru='Исходящий документ';uk='Вихідний документ'");
	ИначеЕсли ТипОбъекта = "DMFile" Тогда
		ИмяТипаПредмета = НСтр("ru='Файл';uk='Файл'");
	ИначеЕсли ТипОбъекта = "DMActivity" Тогда
		ИмяТипаПредмета = НСтр("ru='Мероприятие';uk='Захід'");
	ИначеЕсли ТипОбъекта = "DMIncomingEMail" Тогда
		ИмяТипаПредмета = НСтр("ru='Входящее письмо';uk='Вхідний лист'");
	ИначеЕсли ТипОбъекта = "DMOutgoingEMail" Тогда
		ИмяТипаПредмета = НСтр("ru='Исходящее письмо';uk='Вихідний лист'");
	ИначеЕсли ТипОбъекта = "DMProject" Тогда
		ИмяТипаПредмета = НСтр("ru='Проект';uk='Проєкт'");
	ИначеЕсли ТипОбъекта = "DMProjectTask" Тогда
		ИмяТипаПредмета = НСтр("ru='Проектная задача';uk='Проєктна задача'");
	ИначеЕсли ТипОбъекта = "DMDiscussionMessage" Тогда
		ИмяТипаПредмета = НСтр("ru='Сообщение';uk='Повідомлення'");
