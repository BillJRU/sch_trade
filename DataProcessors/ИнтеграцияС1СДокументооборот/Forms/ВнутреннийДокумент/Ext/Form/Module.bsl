...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
		Элементы.ГруппаКонтрагент.Заголовок = НСтр("ru='Корреспондент';uk='Кореспондент'");
		Элементы.Контрагент.Заголовок = НСтр("ru='Корреспондент';uk='Кореспондент'");
	КонецЕсли;
...
Процедура ФайлыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если СостояниеРазрешаетДобавлениеСканКопии
		...
		ТекстВопроса = СтрШаблон(
			НСтр("ru='В текущем состоянии ""%1"" можно добавить только скан-копию оригинала документа. Продолжить?';uk='У поточному стані ""%1"" можна додати тільки скан-копію оригіналу документа. Продовжити?'"),
			Состояние);
...
|;uk='Перед додаванням друкованої форми потрібно зв''язати
|внутрішній документ з об''єктом %1'"), СокращенноеНаименованиеКонфигурации);
...
Процедура ЗаполнитьФайлДаннымиДокумента(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Текст = НСтр("ru='Поля в файле обновлены данными владельца.';uk='Поля у файлі оновлені даними власника.'");
...
Процедура ОбновитьДекорацииСвязи()
	...
	Если Не ДокументЗаполнен Тогда
		...
		Элементы.СтраницаСвязи.Заголовок = НСтр("ru='Связи';uk='Зв''язки'");
...
Процедура ПрочитатьОбъектВФорму(ОбъектXDTO)
	...
	Если ОбъектXDTO.Свойства().Получить("titleBlockedByTemplate") <> Неопределено
		...
		Элементы.Наименование.Подсказка = НСтр("ru='Наименование документа сформировано автоматически по шаблону, редактирование запрещено.';uk='Найменування документа сформовано автоматично за шаблоном, редагування заборонено.'");
...
Функция СтавкаНДСЧислом(СтавкаНДС)
	...
	Если СтавкаНДС = "НДС20" Тогда
		Возврат 0.20;
	ИначеЕсли СтавкаНДС = "НДС14" Тогда
		Возврат 0.14;
