...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.8.1.CORP") Тогда
		ВызватьИсключение(НСтр("ru='Функционал не поддерживается в данной версии BAS Документооборота.';uk='Функціонал не підтримуються в цій версії BAS Документообігу.'"));
	КонецЕсли;
...
Процедура ЗаполнитьФормуИзОбъектаXDTO(ПисьмоXDTO, НовоеПисьмо = Ложь)
	...
	Если ЗначениеЗаполнено(ПисьмоXDTO.subject) Тогда
		Заголовок = СтрШаблон(НСтр("ru='%1 (Исходящее письмо)';uk='%1 (Вихідний лист)'"), ПисьмоXDTO.subject);
	Иначе
