...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = СтрШаблон(НСтр("ru='Загрузка файла %1';uk='Завантаження файлу %1'"), Параметры.ПредставлениеФайла);
...
Процедура ОтправитьОчереднуюПорциюНаСервере(Знач ИмяФайлаИлиАдрес, Знач ПутьФайлаWindows, Знач ПутьФайлаLinux, Знач ДвоичныеДанные, Знач Начало)
	...
	Если РазмерЗаписанногоФрагмента <> Начало Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Размер файла %1 не соответствует ожидаемому %2';uk='Розмір файлу %1 не відповідає очікуваному %2'"), РазмерЗаписанногоФрагмента, Начало);
	КонецЕсли;
...
Процедура ЗавершитьЗагрузку()
	...
	Прогресс = 100;
	Состояние(НСтр("ru='Файл помещен';uk='Файл поміщений'"));
	ЗагрузкаЗавершена = Истина;
...
Процедура ОтменитьЗагрузку()
	...
	ИмяФайлаНаСервере = ФайлыБТС.ПолноеИмяФайлаВСеансе(ИмяФайлаИлиАдрес, ПутьФайлаWindows, ПутьФайлаLinux);
	ИмяСобытияЖР = НСтр("ru='Удаление файла.Отмена загрузки';uk='Вилучення файла. Відміна завантаження'",ОбщегоНазначения.КодОсновногоЯзыка());
