...
Функция ЗапуститьТестированиеВФоне(ПараметрыТестирования) Экспорт
	...
	Задание = ФоновыеЗадания.Выполнить(
		...
		НСтр("ru='Тестирование операций закрытие месяца';uk='Тестування операцій закриття місяця'"));
...
Функция ПараметрыЗапускаТестирования(ВходныеПараметры = Неопределено)
	...
	ПараметрыЗадания.Вставить("РежимОтладки", ОбщегоНазначения.РежимОтладки());
...
Функция РазделыТестирования() Экспорт
	...
	РазделыТестирования.Добавить("ВзаиморасчетыСПартнерами", " " + НСтр("ru='Взаиморасчеты с партнерами';uk='Взаєморозрахунки з партнерами'"), Истина);
	РазделыТестирования.Добавить("Себестоимость", " " + НСтр("ru='Себестоимость';uk='Собівартість'"), Истина);
	//++ Локализация
	РазделыТестирования.Добавить("НДС", " " + НСтр("ru='Корректировка налогового назначения капитальных инвестиций';uk='Коригування податкового призначення капітальних інвестицій'"), Истина);
	//-- Локализация
...
Функция ЭтапыТестирования(ТестироватьНовыеМеханизмы = Ложь) Экспорт
	...
	ЭтапыТестирования.Добавить("ПерезакрытьМесяц", " " + НСтр("ru='Перезакрытие месяца';uk='Перезакриття місяці'"), Истина);
	Если ТестироватьНовыеМеханизмы Тогда
		...
		ЭтапыТестирования.Добавить("ПереходНаНовыеВзаиморасчеты",
			" " + НСтр("ru='Переход на новые взаиморасчеты и перезакрытие месяца';uk='Перехід на нові взаєморозрахунки та перезакриття місяця'"),
		...
		ЭтапыТестирования.Добавить("ПерезаполнитьНовыеВзаиморасчеты",
			" " + НСтр("ru='Перезаполнение новых взаиморасчетов и перезакрытие месяца';uk='Перезаповнення нових взаєморозрахунків та перезакриття місяця'"),
...
Функция КонтрольныеРегистры(ВыбранныеРазделы = Неопределено) Экспорт
	...
	Если ВыбранныеРазделы.Найти("Себестоимость") <> Неопределено Тогда // Себестоимость
		...
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.ПрочаяВыручка.ПолноеИмя());
		...
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.ДетализацияПартийТоваровДляНДС.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.ДетализацияПартийТоваровДляНДС2_4.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.КорректировкиНДСПартий.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСУсловныеПродажи.ПолноеИмя());
		//-- Локализация
	...
	Если ВыбранныеРазделы.Найти("НДС") <> Неопределено Тогда //НДС
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСНоменклатурныйСоставДляНалоговыхНакладных.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСРасчетНалоговогоКредита.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСРасчетНалоговыхОбязательств.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСРеестрВыданныхНалоговыхДокументов.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДСРеестрПолученныхНалоговыхДокументов.ПолноеИмя());
		РегистрыКонтроля.Добавить(Метаданные.РегистрыНакопления.НДССоставПоставкиДляРегистрацииВходящихНалоговыхДокументов.ПолноеИмя());
	КонецЕсли;
...
Функция ДействияПриРасхождениях() Экспорт
	...
	СписокВозврата.Добавить("Продолжить", НСтр("ru='Продолжить тестирование';uk='Продовжити тестування'"));
	СписокВозврата.Добавить("Остановить", НСтр("ru='Остановить тестирование';uk='Зупинити тестування'"));
	СписокВозврата.Добавить("ОстановитьУсловие", НСтр("ru='Остановить тестирование при достижении лимита расхождений';uk='Зупинити тестування при досягненні ліміту розбіжностей'"));
	СписокВозврата.Добавить("РазбитьФайл", НСтр("ru='При достижении лимита расхождений разбивать файл расхождений';uk='При досягненні ліміту розбіжностей розбивати файл розбіжностей'"));
...
Процедура СообщитьОбОшибке()
	ЗаписьЖурналаРегистрации(НСтр("ru='Тестирование.Ошибка тестирования';uk='Тестування.Помилка тестування'",ОбщегоНазначения.КодОсновногоЯзыка()),
