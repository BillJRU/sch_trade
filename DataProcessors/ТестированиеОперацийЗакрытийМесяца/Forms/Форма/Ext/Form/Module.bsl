...
Процедура ПредставлениеЭтаповДляТестированияНажатие(Элемент, СтандартнаяОбработка)
	...
	СписокЭтаповДляТестирования.ПоказатьОтметкуЭлементов(ЭтапыДляТестированияВыбор, НСтр("ru='Выберите этапы для тестирования';uk='Виберіть етапи для тестування'"));
КонецПроцедуры
...
Процедура ПоказатьРасхожденияЭтапов(Команда)
	Если СписокРасхожденийЭтапов.Количество() Тогда
		...
		СписокРасхожденийЭтапов.ПоказатьВыборЭлемента(ОповещениеОВыбореИзСписка, НСтр("ru='Выберите этап для просмотра';uk='Виберіть етап для перегляду'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПоказатьРасхожденияЭтаповНовыхРасчетов(Команда)
	Если СписокРасхожденийЭтаповНовыеРасчеты.Количество() Тогда
		ОповещениеОВыбореИзСписка = Новый ОписаниеОповещения("ПоказатьОтчетРасхожденийПоЭтапу", ЭтотОбъект);
		СписокРасхожденийЭтаповНовыеРасчеты.ПоказатьВыборЭлемента(ОповещениеОВыбореИзСписка, НСтр("ru='Выберите этап для просмотра';uk='Виберіть етап для перегляду'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура Тестирование(ТестированиеНовыхРасчетов = Ложь)
	...
	Иначе
		...
		КонецЕсли;	
		СписокРасхожденийЭтапов.Добавить(ПутьКФайлу, НСтр("ru='Все расхождения';uk='Всі розбіжності'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПродолжитьТестирование()
	...
	Если РезультатРасчета <> Неопределено Тогда
		...
		Если РезультатРасчета.Отказ Тогда
			СообщениеОбОшибке.ДобавитьСтроку(НСтр("ru='Ошибка тестирования!';uk='Помилка тестування!'"));
		...
		Иначе
			...
			ИнформацияОДействии = НСтр("ru='Выполнен расчет';uk='Виконано розрахунок'");
...
Процедура ВывестиРезультатРасчета(Период, ДлительностьМс, ТекущееКоличествоРасхождений, ИнформацияОДействии)
	...
	СтрокаЛогФайла = "";
	СтрокаЛогФайла = СтрокаЛогФайла + НСтр("ru='%ИнформацияОДействии% за месяц %Период% за %Длительность%';uk='%ИнформацияОДействии% за місяць %Период% за %Длительность%'");
...
Функция ПеревестиМиллисекундыВДеньЧасМинутаСекунда(Знач Миллисекунды)
	...
	Если Секунды >= 60 Тогда
		...
		Если Часы > 0 Тогда
			 Результат = "" + Часы + " "+ НСтр("ru='час.';uk='год.'") + " " + Результат;
		КонецЕсли;
...
Функция ЧислоРасхожденийПрописью(ТекущееКоличествоРасхождений)
	...
	Поз = Найти(КоличествоПрописью, НСтр("ru='расхождени';uk= 'розбіжн'"));
	Если Поз <> 0 Тогда
...
Функция ПредставлениеЭтаповДляТестирования(Список)
	...
	СтрокаВозврата = СтрокаВозврата + ?(СтрокаВозврата = "", НСтр("ru='< нет >';uk='<немає>'"), ".");
...
Процедура ЗаписатьДанныеРасхождений(ТекущиеРасхождения, НовыеРасчеты = Ложь)
	...
	Для каждого Расхождение из ТекущиеРасхождения Цикл
		...
		Если ЗаписьРасхождений.Получить(ИмяЭтапа) = Неопределено Тогда
			...
			ПредставлениеЭтапа = ПредставлениеЭтапа + ?(НовыеРасчеты, " (" + НСтр("ru='отличия при переходе на новые взаиморасчеты';uk='відмінності при переході на нові взаєморозрахунки'") + ")", "");
...
Процедура ПродолжитьЗаполнениеНовыхРасчетов()
	...
	ДлительностьМс = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЗамера;
	ИнформацияОДействии = НСтр("ru='Выполнено заполнение по новым расчетам';uk='Виконано заповнення за новими розрахунками'");
	
	СтрокаЛогФайла = "";
	СтрокаЛогФайла = СтрокаЛогФайла + НСтр("ru='%ИнформацияОДействии% на месяц %Период% за %Длительность%';uk='%ИнформацияОДействии% на місяць %Период% за %Длительность%'");
	СтрокаЛогФайла = СтрЗаменить(СтрокаЛогФайла, "%ИнформацияОДействии%", ИнформацияОДействии);
...
Функция ЗаполнитьНовыеРасчетыВФоне(НачалоРасчета)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		Возврат НСтр("ru='Функциональная опция новых расчетов уже установлена';uk='Функціональна опція нових розрахунків вже встановлена'");
	Иначе
	...
	Задание = ФоновыеЗадания.Выполнить(
		...
		НСтр("ru='Заполнение регистров новой архитектуры взаиморасчетов';uk='Заповнення регістрів нової архітектури взаєморозрахунків'"));
