...
Процедура ОформитьАвансовыйОтчетПоРаспоряжению(Команда)
	...
	Если Не МассивКОформлению.Количество() Тогда
		ОчиститьСообщения();
		ТекстПредупреждения = НСтр("ru='В выделенных строках нет данных для формирования Авансового отчета.';uk='У виділених рядках немає даних для формування Авансового звіту.'");
...
Функция ФормированиеДокументаВозможно(МассивАвансов)
	...
	ТекстОшибки = НСтр("ru='Ввод одного документа Авансовый отчет с несколькими %1 невозможен.';uk='Введення одного документа Авансовий звіт з декількома %1 неможливий.'");
	Если МассивОрганизаций.Количество() > 1 Тогда
		Результат = Ложь;
		ТекстОшибки = СтрШаблон(ТекстОшибки, НСтр("ru='организациями';uk='організаціями'"));
	ИначеЕсли МассивПодотчетныхЛиц.Количество() > 1 Тогда
		Результат = Ложь;
		ТекстОшибки = СтрШаблон(ТекстОшибки, НСтр("ru='подотчетными лицами';uk='підзвітними особами'"));
	ИначеЕсли МассивПодразделений.Количество() > 1 Тогда
		Результат = Ложь;
		ТекстОшибки = СтрШаблон(ТекстОшибки, НСтр("ru='подразделениями';uk='підрозділами'"));
	КонецЕсли;
