...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Выборка.Следующий();
	ЗаголовокВес   = СтрШаблон(НСтр("ru='Вес (%1)';uk='Вага (%1)'"), Выборка.ЕдиницаИзмеренияВеса);
	ЗаголовокОбъем = СтрШаблон(НСтр("ru='Объем (%1)';uk='Об''єм(%1)'"), Выборка.ЕдиницаИзмеренияОбъема);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.ВариантОформления) Тогда
		...
		ТекстСообщения = НСтр("ru='Оформление документов недоступно.';uk='Оформлення документів недоступно.'");
		Возврат;
		Отказ = Истина;
	...
	Если Режим = Элементы.Режим.СписокВыбора[0].Значение Тогда // это распределение остатков
		...
		Если Не ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Склад-отправитель"" не заполнено';uk='Поле ""Склад-відправник"" не заповнено'");
		...
		Если Не ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Дата отгрузки"" не заполнено';uk='Поле ""Дата відвантаження"" не заповнено'");
		...
		ИначеЕсли Объект.ДатаОтгрузки < НачалоДня(ТекущаяДатаСеанса()) Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Дата отгрузки"" не может быть заполнено прошедшей датой';uk='Поле ""Дата відвантаження"" не може бути заповнене минулою датою'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Неопределено, "ДатаОтгрузки", "Объект.ДатаОтгрузки", Отказ);
		...
		ИначеЕсли Объект.ДатаОтгрузки > НачалоДня(ТекущаяДатаСеанса())
			...
			ТекстСообщения = НСтр("ru='Для варианта оформления ""Перемещение товаров"" поле ""Дата отгрузки"" не может быть заполнено будущей датой';uk='Для варіанту оформлення ""Переміщення товарів"" поле ""Дата відвантаження"" не може бути заповнене майбутньою датою'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Неопределено, "ДатаОтгрузки", "Объект.ДатаОтгрузки", Отказ);
			
		ИначеЕсли ЗначениеЗаполнено(Объект.ДатаСледующейОтгрузки) И Объект.ДатаСледующейОтгрузки < Объект.ДатаОтгрузки Тогда
			
			ТекстСообщения = НСтр("ru='Поле ""Дата следующей отгрузки"" не может быть заполнено датой меньшей чем поле ""Дата отгрузки""';uk='Поле ""Дата наступного відвантаження"" не може бути заповнене датою меншою ніж поле ""Дата відвантаження""'");
	...
	ИначеЕсли Режим = Элементы.Режим.СписокВыбора[1].Значение Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.СкладПолучатель) Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Склад-получатель"" не заполнено';uk='Поле ""Склад-одержувач"" не заповнене'");
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.ПотребностиДоДаты) И Объект.ПотребностиДоДаты < Объект.ДатаОтгрузки Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Потребности до даты"" не может быть заполнено датой меньшей чем поле ""Дата отгрузки""';uk='Поле ""Потреби до дати"" не може бути заповнене датою меншою ніж поле ""Дата відвантаження""'");
...
Процедура ДалееКШагуСформированныеДокументы(Команда)
	...
	Если Не ДалееКШагуСформированныеДокументыНаСервере() Тогда
		...
		ТекстСообщения = НСтр("ru='Не указаны перемещаемые товары.';uk='Не вказані переміщувані товари.'");
...
Процедура ДалееКШагуПеремещаемыеТоварыНаСервере()
	...
	НачатьВыполнениеВФоне(Элементы.ДалееКШагуПеремещаемыеТовары.Имя, НСтр("ru='Получение данных о перемещаемых товарах';uk='Отримання даних про переміщувані товари'"));
...
Функция ДалееКШагуСформированныеДокументыНаСервере()
	...
	ИменаДокументов.Добавить(НСтр("ru='Перемещение товаров:';uk='Переміщення товарів:'"));
	...
	ИменаДокументов.Добавить(НСтр("ru='Заказ на перемещение:';uk='Замовлення на переміщення:'"));
...
Процедура ПоказатьВопросУдалитьДокументы()
	...
	Если ОформленоДокументов > 0 Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Пометить на удаление сформированные документы';uk='Помітити на вилучення сформовані документи'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		
		ТекстВопроса = НСтр("ru='При переходе назад сформированные документы будут помечены на удаление.';uk='При переході назад сформовані документи будуть позначені на вилучення.'");
		Ответ = Неопределено;
...
Процедура ПриИзмененииРеквизитов(ИмяЭлемента = Неопределено)
	...
	Если ИмяЭлемента = Неопределено Или ИмяЭлемента = Элементы.Режим.Имя Тогда
		...
		Если Режим = Элементы.Режим.СписокВыбора[0].Значение Тогда
			...
			Заголовок = НСтр("ru='Управление перемещением обособленных товаров: Распределение остатков';uk='Управління переміщенням відокремлених товарів: Розподіл залишків'");
			...
			Элементы.ТоварыДатаПотребности.Заголовок = НСтр("ru='Дата потребности';uk='Дата потреби'");
			Элементы.КорзинаСкладовСклад.Заголовок = НСтр("ru='Склад-получатель';uk='Склад-одержувач'");
			
		ИначеЕсли Режим = Элементы.Режим.СписокВыбора[1].Значение Тогда
			
			Заголовок = НСтр("ru='Управление перемещением обособленных товаров: Обеспечение потребностей';uk='Управління переміщенням відокремлених товарів: Забезпечення потреб'");
			...
			Элементы.ТоварыДатаПотребности.Заголовок = НСтр("ru='Дата потребности (минимальная)';uk='Дата потреби (мінімальна)'");
			Элементы.КорзинаСкладовСклад.Заголовок = НСтр("ru='Склад-отправитель';uk='Склад-відправник'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<несколько дат>';uk='<декілька дат>'"));
