...
Функция ОтправитьPUTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		Если ПотокЗаписи = Неопределено Тогда	
			...
			Ответ.УстановитьТелоИзСтроки(СтрШаблон(НСтр("ru='Не удалось открыть поток записи файла. Идентификатор запроса %1';uk='Не вдалося відкрити потік запису файлу. Ідентифікатор запиту %1'"), 
				ПараметрыЗапроса.ID));
			Возврат Ответ;
		...
		Если ЗначениеЗаполнено(ПолученныйДиапазон) 
			...
			Ответ = Новый HTTPСервисОтвет(500);
			Ответ.УстановитьТелоИзСтроки(СтрШаблон(НСтр("ru='Размер файла %1 не соответствует ожидаемому %2. Идентификатор запроса %3';uk='Розмір файлу %1 не відповідає очікуваному %2. Ідентифікатор запиту %3'"), 
