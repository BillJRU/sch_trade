...
Функция ПолучитьЗначениеПоказателя1С(Данные, ИмяТаблицы,ИмяПоказателя)
	...
	// Если показатель предприятия не задан, он может быть только вычислен
	...
	Исключение
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			...
			ПротоколСообщить("FREDO Звіт. Помилка отримання показника (табличне поле """ + ИмяТаблицы + """, показник """ + ИмяПоказателя + """). Назва табличного поля чи показника вказано не вірно!");
		...
		Если СоответствиеОтчета = Неопределено Тогда 
			...
			Если СоответствиеОтчета = Неопределено Тогда 
				...
				Если ЗначениеЗаполнено(СоответствиеОтчета) Тогда
					...
					КонецЕсли;
				Иначе	
					// Заполнение не предусмотрено в предприятии
