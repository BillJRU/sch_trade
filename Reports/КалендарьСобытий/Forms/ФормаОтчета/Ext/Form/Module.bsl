...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПериодФормирования = Элементы.ПериодФормирования.СписокВыбора[0].Значение;
...
Процедура ЗаполнитьДеревоСобытий()
	...
	Если ЕстьПравоУстановкаЦен Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "УстановкаЦен", НСтр("ru='Установки цен номенклатуры';uk='Встановлення цін номенклатури'"));
	КонецЕсли;
	...
	Если ЕстьПравоОРасхожденияхПослеОтгрузки И ИспользоватьАктыРасхожденийПослеОтгрузки Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "АктыПриемкиКлиента", НСтр("ru='Акты о расхождениях после реализации';uk='Акти про розбіжності після реалізації'"));
	КонецЕсли;
	CRMЛокализация.ДополнитьДеревоСобытийОтчетКалендарьСобытий(УзелДерева);
	УдалитьРодительскийУзелДереваЕслиПустой(УзелДерева);
	...
	Если ЕстьПраваНаВзаимодействия И ИспользоватьПочтовыйКлиент Тогда
		...
		УзелДерева = ДобавленноеВДеревоВидСобытия(ВидыСобытий,"Взаимодействия", НСтр("ru='Взаимодействия';uk='Взаємодії'"));
	...
	Если ЕстьПравоДоверенностьПолучениеТоваров Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ДоверенностьПолучениеТоваров", НСтр("ru='Доверенности на получение товаров';uk='Довіреності на отримання товарів'"));
	КонецЕсли;
	CRMЛокализация.ДополнитьДеревоСобытийОтчетКалендарьСобытий(УзелДерева);
	УдалитьРодительскийУзелДереваЕслиПустой(УзелДерева);
	...
	Если ЕстьПраваНаДоговора Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ДоговораКонтрагентов", НСтр("ru='Договоры контрагентов';uk='Договори контрагентів'"));
	КонецЕсли;
	Если ЕстьПраваНаПроекты Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "Проекты", НСтр("ru='Проекты';uk='Проєкти'"));
	КонецЕсли;
