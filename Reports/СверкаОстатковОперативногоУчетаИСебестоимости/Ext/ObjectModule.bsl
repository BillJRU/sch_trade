...
Процедура ДобавитьПредупреждениеОбОсобенностяхФормированияОтчета(ДокументРезультат)
	...
	Если НЕ РасчетСебестоимостиПовтИсп.ВозможенРасчетСебестоимости(Период) Тогда
		...
		ТекстПредупреждения = 
			НСтр("ru='Отчет используется только при включенном учете себестоимости.';uk='Звіт використовується тільки при включеному обліку собівартості.'");
		
	ИначеЕсли НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22(Период) Тогда
		
		ТекстПредупреждения = 
			НСтр("ru='Отчет используется только при включенном партионном учете версии 2.5.';uk='Звіт використовується тільки при включеному партіонному обліку версії 2.5.'");
