...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Для каждого ПоказательШапки Из ДанныеШапки Цикл
		...
		Если СтруктураДанныхПервичныйДокумент.Свойство(ИмяПоказателя) Тогда
			...
			Попытка
				Если ИмяПоказателя = "ПредставительПоставщика" Тогда
					СтруктураДанныхПервичныйДокумент[ИмяПоказателя] = ДанныеШапки.ДолжностьПредставителя + " " + ПоказательШапки.Значение;
				КонецЕсли;
			Исключение
			КонецПопытки;
		КонецЕсли; 
