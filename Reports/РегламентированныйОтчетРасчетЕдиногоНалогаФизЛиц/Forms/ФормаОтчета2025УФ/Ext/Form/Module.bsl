...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025УФ";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ВыбранныйОтчет", Неопределено);
...
Процедура ПоказатьПериод()
	...
	Если (ЭтаФорма.СтруктураРеквизитовФормы.мПериодичность = ЭтаФорма.ПеречислениеПериодичностьМесяц)  Тогда    
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	Если НЕ Форма.СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
		...
		Если НЕ ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.ВыбранныйОтчет) Тогда
	...
	Если (Форма.СтруктураРеквизитовФормы.мПериодичность = Форма.ПеречислениеПериодичностьМесяц)  Тогда    
		...
		ЗаполнитьПоказательНаСервере("HZM",  Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), Форма, "ДекларацияЕН");
		ЗаполнитьПоказательНаСервере("HZM",  Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), Форма, "Приложение");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("ВыбранныйОтчет", СтруктураРеквизитовФормы.ВыбранныйОтчет) Тогда
		СтруктураРеквизитовФормы.ВыбранныйОтчет = Неопределено;	
	КонецЕсли;	
	//2025
 	СписокСохранения.Свойство("МинЗП", МинЗП);
	СтруктураРеквизитовФормы.МинЗП = МинЗП;
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("ВыбранныйОтчет", СтруктураРеквизитовФормы.ВыбранныйОтчет);
	СписокСохранения.Вставить("МинЗП", СтруктураРеквизитовФормы.МинЗП);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для каждого Строка Из ТаблицаБлока Цикл
		//МПЗ = НГОд  х К х М / 12 (графа 7 х графа 9 х графа 10 / 12), грн
    	Строка.D1T1RXXXXG11 = Строка.D1T1RXXXXG7 * Строка.D1T1RXXXXG9 * Строка.D1T1RXXXXG10 / 12;
		ИмяЯчейкиПрописью = "D1T1RXXXXG11_"+Формат(ТаблицаБлока.Индекс(Строка) + 1, "ЧГ=;");
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG11, Форма, "Приложение2");
		
		//МПЗ = НГО х S х К х М / 12 графа 8  х (графа 3 + графа 5) х графа 9 х графа 10 / 12), грн
    	Строка.D1T1RXXXXG12 = Строка.D1T1RXXXXG8 * (Строка.D1T1RXXXXG3 + Строка.D1T1RXXXXG5) * Строка.D1T1RXXXXG9 * Строка.D1T1RXXXXG10 /12;
		ИмяЯчейкиПрописью = "D1T1RXXXXG12_"+Формат(ТаблицаБлока.Индекс(Строка) + 1, "ЧГ=;");
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG12, Форма, "Приложение2");

		R002G9 = Строка.D1T1RXXXXG9;
		
	КонецЦикла;
	R002G3 = ТаблицаБлока.Итог("D1T1RXXXXG3");
	R002G5 = ТаблицаБлока.Итог("D1T1RXXXXG5");
	R002G11 = ТаблицаБлока.Итог("D1T1RXXXXG11");
	R002G12 = ТаблицаБлока.Итог("D1T1RXXXXG12");
	R002G14 = ТаблицаБлока.Итог("D1T1RXXXXG14");
	ЗаполнитьПоказательНаСервере("R002G5", R002G5, Форма, "Приложение2");
	...
	ЗаполнитьПоказательНаСервере("R002G14", R002G14, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R03G3", R002G14, Форма, "Приложение2");
    //(ЗМПЗ) (ЗМПЗ = сума рядків 1.n граф  11 та/або 12), грн   
	R002G13 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G11", Форма, "Приложение2"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G12", Форма, "Приложение2"));
	ЗаполнитьПоказательНаСервере("R002G13", R002G13, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R01G3", R002G13, Форма, "Приложение2");
	...
	//2025 військовий збір 
	Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.МинЗП) Тогда
		КолвоМесяцев = 0;
		Для КодСтроки = 1 По 12 Цикл
			КолвоМесяцев = КолвоМесяцев + ?(ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R08G" + КодСтроки, Форма, "ДекларацияЕН")),1,0);
		КонецЦикла; 
		//мінімальна заробітна плата на 01 січня податкового (звітного) року × 10 % × кількість місяців
		R022G3 = Форма.СтруктураРеквизитовФормы.МинЗП * КолвоМесяцев * 0.1;
		ЗаполнитьПоказательНаСервере("R022G3", R022G3, Форма, "ДекларацияЕН");    
	КонецЕсли;   
	
	//2025    
	R023G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R005G3", Форма, "ДекларацияЕН"))
	         + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R006G3", Форма, "ДекларацияЕН"))
			 + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R007G3", Форма, "ДекларацияЕН"));			 
	R023G3 = R023G3 * 0.01;      
	ЗаполнитьПоказательНаСервере("R023G3", R023G3, Форма, "ДекларацияЕН");    
	
	R025G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R023G3", Форма, "ДекларацияЕН"))
	         - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R024G3", Форма, "ДекларацияЕН"));			 
	ЗаполнитьПоказательНаСервере("R025G3", R025G3, Форма, "ДекларацияЕН");    
	
	R028G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R027G3", Форма, "ДекларацияЕН"))
			 - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R026G3", Форма, "ДекларацияЕН"));
	Если R028G3 > 0 Тогда
		ЗаполнитьПоказательНаСервере("R028G3", R028G3, Форма, "ДекларацияЕН");   
		ЗаполнитьПоказательНаСервере("R029G3", 0, Форма, "ДекларацияЕН");
	Иначе
		ЗаполнитьПоказательНаСервере("R028G3", 0, Форма, "ДекларацияЕН");   
		ЗаполнитьПоказательНаСервере("R029G3", R028G3, Форма, "ДекларацияЕН");
	КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	//2025 
	ИначеЕсли  Лев(ИмяЯчейки, 4) = "R08G" И НаименованиеТекЛиста = "ДекларацияЕН" Тогда
...
Процедура Обновить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ЭтаФорма.СтруктураРеквизитовФормы.ВыбранныйОтчет) Тогда
		//очистим таблицу КВЕДов
		ОчиститьКВЕДНаСервере()
	КонецЕсли;	
	ВывестиМакетВТаблДокНаСервере(ЭтаФорма.СтруктураРеквизитовФормы.НаимТекущегоРаздела, 1); 
	ЗаполнитьСведенияОбОрганизацииНаСервере();
...
&НаСервере
Процедура ОчиститьНаСервере(Заполнение = Ложь) Экспорт
	
	Перем ТаблицаКВЕД, ИсходноеКоличествоСтрокГруппы;
	...
	// сохраняем таблицу КВЕД
	Если Заполнение Тогда
		ДанныеФормы = ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы1];
		ТаблицаКВЕД = ДанныеФормыВЗначение(ДанныеФормы, Тип("ТаблицаЗначений"));
	КонецЕсли;
	...
	Если Заполнение Тогда
		...
		Для каждого Строка Из ТаблицаКВЕД Цикл
			Если НЕ ЗначениеЗаполнено(Строка.T1RXXXXG1S) Тогда
				Прервать;
			КонецЕсли;
			Если ТаблицаКВЕД.Индекс(Строка)+1 <= ИсходноеКоличествоСтрокГруппы Тогда
				ДобавленнаяСтрока = СтруктураГруппы[ТаблицаКВЕД.Индекс(Строка)];
			Иначе
				ДобавленнаяСтрока = СтруктураГруппы.Добавить();	
			КонецЕсли;
			ДобавленнаяСтрока.T1RXXXXG1S = Строка.T1RXXXXG1S;
			
			ДобавленнаяСтрока.T1RXXXXG2S = Строка.T1RXXXXG2S;
		КонецЦикла;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
		...
		МинЗП = СтруктураРеквизитовФормы.МинЗП;
		
		Если ЗначениеЗаполнено(МинЗП) Тогда
		...
		Если НЕ РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "BASУТ" Тогда
			Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
				Если РегламентированнаяОтчетностьПереопределяемый.ИДПодсистемыЗарплаты() = "ЕРПЗИК"  Тогда
					ТаблЧисленностиССЧ = Новый ТаблицаЗначений;
					ВЫПОЛНИТЬ("ТаблЧисленностиССЧ = КадровыйУчет.СреднесписочнаяЧисленностьРаботающих(СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);");
					...
					СтруктураДанныхДекларацияЕН["HNACTL"] = ВсегоССЧ;
				Иначе	
					...
					ТаблСреднеспЧисленности = Новый ТаблицаЗначений;
					ВЫПОЛНИТЬ("ТаблСреднеспЧисленности = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность);");
					СреднеспКво = ТаблСреднеспЧисленности[0].Средняя;
					СтруктураДанныхДекларацияЕН["HNACTL"] = СреднеспКво;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	ЗаполнитьТаблицуМесяцев1_2групп();
...
Процедура ЗаполнитьРазделПоГруппеЕРП(Группа)
	...
	Запрос.УстановитьПараметр("Группа", Группа);
		...
		Запрос.Текст = "ВЫБРАТЬ
	               |	НАЧАЛОПЕРИОДА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Период, ДЕНЬ) КАК Период,
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Статья,
				   |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Валюта КАК Валюта,
	               |	СУММА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.СуммаОборот) КАК Сумма
	               |ПОМЕСТИТЬ Продажи
	               |ИЗ
	               |	РегистрНакопления.КнигаДоходовРасходовПоЕдиномуНалогу.Обороты(&ДатаНач, &ДатаКон, ДЕНЬ, Организация = &Организация) КАК КнигаДоходовРасходовПоЕдиномуНалогуОбороты
				   ...
				   |ИЗ
				   |	РегистрСведений.КурсыВалют.СрезПоследних(ДОБАВИТЬКДАТЕ(&ДатаНач, ДЕНЬ, -1), ) КАК КурсыВалютСрезПоследних
				   |;
				   |
				   |////////////////////////////////////////////////////////////////////////////////
				   |ВЫБРАТЬ
				   ...
				   |	ЕСТЬNULL(ГраницыПериода.ПериодКонец, &ДатаКон) КАК ПериодКонец
	               ...
	               |	Продажи.Период КАК Период,
	               |	Продажи.Статья как Статья,
				   |	Продажи.Сумма * ТаблицаКурсовВалютПериоды.Курс / ТаблицаКурсовВалютПериоды.Кратность КАК Сумма
	               |ИЗ
	               |	Продажи КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ПериодыУчетнойПолитики КАК ПериодыУчетнойПолитики
	               |		ПО Продажи.Период = ПериодыУчетнойПолитики.ПериодПродаж
				   |		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаКурсовВалютПериоды КАК ТаблицаКурсовВалютПериоды
				   |		ПО Продажи.Валюта = ТаблицаКурсовВалютПериоды.Валюта
				   |			И Продажи.Период >= ТаблицаКурсовВалютПериоды.ПериодНачало
				   |			И Продажи.Период < ТаблицаКурсовВалютПериоды.ПериодКонец
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПлательщиковЕдиногоНалога КАК СтатусыПлательщиковЕдиногоНалога
	               |		ПО (СтатусыПлательщиковЕдиногоНалога.Организация = &Организация)
	               |			И (СтатусыПлательщиковЕдиногоНалога.Период = ПериодыУчетнойПолитики.ПериодУчетнойПолитики)
	               |
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Если ПолучитьданныеЗаПрошлыйПериодИз1С Тогда	
			...
			ПоказателиПрошлойДекларации = ПоказателиПредыдущейДекларации(ДатПрошлогоПериода1,
																		 ...
																		 "ДекларацияЕН",
																		 "РасчетЕдиногоНалога",
																		 НСтр("ru='строк 13 и 24';uk='рядків 13 та 24'"));
			...
			Если НЕ ПоказателиПрошлойДекларации = Неопределено Тогда
				...
				Если Год(ДатПрошлогоПериода1) >= 2025 Тогда
					Попытка
						R024G3  = ПоказателиПрошлойДекларации.R023G3;
						СтруктураДанныхДекларацияЕН.R024G3 = R024G3;
					Исключение КонецПопытки;
				КонецЕсли;
...
Функция ПоказателиПредыдущейДекларации(ДатаНачалаПериода,ДатаОкончанияПериода,ИсточникОтчета,ПредставлениеОтчета,ИмяПоля,
	...
	Если  РезультатЗапроса.Количество() = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сохраненный отчет %3 за %1 не обнаружен! Автоматическое заполнения показателей %2  невозможно!';uk='Звіт %3 за %1 не виявлений! Автоматичне заповнення показників %2 неможливо!'"), ПредставлениеПериода( НачалоМесяца(ДатаНачалаПериода), КонецМесяца(ДатаОкончанияПериода), "Л = ""uk_UA""; ФП = Истина" ), ПредставлениеПоказателя, ПредставлениеОтчета); 
	...
	ИначеЕсли  РезультатЗапроса.Количество() > 1 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='За %1 обнаружено более одного сохраненного отчета %3! Корректное заполнение показателей %2 невозможно!';uk='За %1 виявлено більше одного збереженого звіту %3! Коректне заповнення показників %2 неможливе!'"), ПредставлениеПериода(НачалоМесяца(ДатаНачалаПериода), КонецМесяца(ДатаОкончанияПериода), "Л = ""uk_UA""; ФП = Истина" ), ПредставлениеПоказателя, ПредставлениеОтчета);
		Если СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Ложь Тогда
			Сообщить(ТекстСообщения);
		Иначе
			мСписокСообщенийЗвит1С.Добавить(4,ТекстСообщения)
	...
	Если ТипЗнч(ПараметрыЗаполненияЗвит1С) = Тип("Структура") Тогда
		...
		Попытка
			...
			Иначе
				...
				//2025
				R024G3  = 0;
				Если ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ГодПериода >= 2025 Тогда
					Попытка
						R024G3  = Данные.GetValue("MAIN", "A23");
					Исключение
					КонецПопытки;
				КонецЕсли;
			...
			ЗаполнитьПоказательНаСервере("R024G3", R024G3, ЭтаФорма, "ДекларацияЕН");
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("ВыбранныйОтчет", 	СтруктураРеквизитовФормы.ВыбранныйОтчет);
...
&НаСервере
Процедура ОчиститьКВЕДНаСервере()
...
&НаСервере
Процедура ЗаполнитьКВЕДНаСервере()
	...
	ДанныеОтчета = ЭтаФорма.СтруктураРеквизитовФормы.ВыбранныйОтчет.ДанныеОтчета.Получить();
	ДанныеОтчета.Свойство("ПоказателиОтчета",ПоказателиОтчета);
	...
	Если ПоказателиОтчета.Свойство("ПолеТабличногоДокументаДекларацияЕН", ПоказателиДекларации)  Тогда
		...
		Для НСтр = 1 По 15 Цикл
			
			Попытка 
				Если НЕ ЗначениеЗаполнено(ПоказателиДекларации["T1RXXXXG1S_"+НСтр]) Тогда
					Прервать;
				КонецЕсли;
			Исключение
				Прервать;
			...
			Если НСтр <= ИсходноеКоличествоСтрокГруппы Тогда
				ДобавленнаяСтрока = СтруктураГруппы[НСтр-1];
			Иначе
				ДобавленнаяСтрока = СтруктураГруппы.Добавить();	
			КонецЕсли;
			ДобавленнаяСтрока.T1RXXXXG1S = ПоказателиДекларации["T1RXXXXG1S_"+НСтр];
			ДобавленнаяСтрока.T1RXXXXG2S = ПоказателиДекларации["T1RXXXXG2S_"+НСтр];
...
&НаКлиенте
Процедура ЗаполнитьКВЕД(Команда)  
	
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВыбранныйОтчет) Тогда
		
		ПоказатьПредупреждение(,НСтр("ru='Не указан отчет для заполнения КВЕД. Таблица КВЕДов заполнена не будет!'
                              |;uk= 'Не зазначено звіт для заповнення КВЕД. Таблиця КВЕДів заповнена не буде!'"));
							  
	Иначе
		
		ЗаполнитьКВЕДНаСервере();
		ВывестиМакетВТаблДокНаСервере(СтруктураРеквизитовФормы.НаимТекущегоРаздела, 1); 
		Если Не ЗначениеЗаполнено(ЭтаФорма.СтруктураДанныхДекларацияЕН.HNAME) Тогда
			ЗаполнитьСведенияОбОрганизацииНаСервере();
		КонецЕсли; 
...
&НаСервере     
Функция  ЗаполнитьТаблицуМесяцев1_2групп() 
	...
	ДатаОбрабПериода = СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета;
	ДатаКон = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета; 
	флПервойСтроки = Истина;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если флПервойСтроки Тогда   
			ПлательщикЕН = Выборка.ПлательщикЕдиногоНалога; 
			ГруппаЕН = Выборка.ГруппаПлательщикаЕдиногоНалога;
			флПервойСтроки = Ложь;
		Иначе	
			Если  ДатаПериода > ДатаОбрабПериода И ДатаПериода <= ДатаКон Тогда
				//изменение параметров
				Если ПлательщикЕН И (ГруппаЕН = Перечисления.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа 
					ИЛИ ГруппаЕН = Перечисления.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа) Тогда 
					
					МесяцНач = Месяц(ДатаОбрабПериода);
					МесяцКон = Месяц(ДатаПериода);
					Для КодСтроки = МесяцНач По МесяцКон Цикл
						ЭтаФорма["СтруктураДанныхДекларацияЕН"].Вставить("R08G" + КодСтроки, "X");
					КонецЦикла; 
				
				КонецЕсли; 
				//новые значения
				ДатаОбрабПериода = ДатаПериода;
				ПлательщикЕН = Выборка.ПлательщикЕдиногоНалога; 
				ГруппаЕН = Выборка.ГруппаПлательщикаЕдиногоНалога;
				
			ИначеЕсли  ДатаПериода <= ДатаОбрабПериода Тогда
				
				//новые значения
				ПлательщикЕН = Выборка.ПлательщикЕдиногоНалога; 
				ГруппаЕН = Выборка.ГруппаПлательщикаЕдиногоНалога;
				
			ИначеЕсли  ДатаПериода > ДатаКон Тогда   
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла; 
