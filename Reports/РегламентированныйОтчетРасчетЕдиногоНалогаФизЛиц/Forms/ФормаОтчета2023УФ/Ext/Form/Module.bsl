...
Процедура ОбновитьПользовательскиеНастройки(СписокСохранения, ВыполнялосьОбновлениеДанных)
	...
	Если ВерсияФормы = "15/12/2022" Тогда
		
		ПоказателиОтчета = Неопределено;
		Если СписокСохранения.Свойство( "ПоказателиОтчета", ПоказателиОтчета) Тогда
			Если (ЭтаФорма.СтруктураРеквизитовФормы.мПериодичность = ЭтаФорма.ПеречислениеПериодичностьМесяц)  Тогда    
				ПоказателиОтчета.ПолеТабличногоДокументаДекларацияЕН.Вставить( "HZM",  Месяц(ЭтаФорма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
				ПоказателиОтчета.ПолеТабличногоДокументаПриложение.Вставить( "HZM",  Месяц(ЭтаФорма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
			КонецЕсли;
		КонецЕсли;
		СписокСохранения.ВерсияФормы = "01/06/2023";
