...
Процедура ДобавитьПредупреждениеОбОсобенностяхФормированияОтчета(ДокументРезультат)
	...
	Если РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22(НачалоПериода) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДатаПереходаНаПартионныйУчетВерсии22) Тогда
			ТекстПредупреждения = 
				НСтр("ru='Отчет не используется при включенном партионном учете версии 2.5.';uk='Звіт не використовується при включеному партіонному обліку версії 2.5.'");
		Иначе
			ТекстПредупреждения = 
				НСтр("ru='Отчет может быть сформирован только по тем периодам, в которых не использовался партионный учет версии 2.5.
|Дата перехода на партионный учет версии 2.5: %1'
|;uk='Звіт може бути сформований тільки по тих періодах, в яких не використовувався партіонний облік версії 2.5. 
...
|	%2'
|;uk='За вказаний період немає оборотів по організаціях, у яких на %1 використовувався метод оцінки вартості ""ФІФО (змінна оцінка)"": 
|%2'") + Символы.ПС;
		Иначе
			ТекстПредупреждения = 
				НСтр("ru='Нет организаций, у которых на %1 использовался метод оценки стоимости ""ФИФО (скользящая оценка)"":';uk='Немає організацій, у яких на %1 використовувався метод оцінки вартості ""ФІФО (змінна оцінка)"":'") + Символы.ПС;
		КонецЕсли;
