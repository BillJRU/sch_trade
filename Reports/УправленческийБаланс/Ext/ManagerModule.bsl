...
|;uk='Для роботи звіту після відновлення конфігурації на нову редакцію потрібно виконати початкове заповнення даних по статтях активів і пасивів. 
|Для цього необхідно виконати однойменну команду з розділу НДІ і адміністрування - Фінансовий результат і контролінг.'");
	ИначеЕсли СостояниеРасчета = Перечисления.СостоянияОперацийЗакрытияМесяца.ВыполненоСОшибками
	 ...
	 ИЛИ НЕ ВыполненоРаспределениеВзаиморасчетов Тогда
		ТекстПредупреждения = НСтр("ru='Данные отчета могут быть не актуальны, так как выполнены не все этапы закрытия месяца, влияющие на финансовый результат в управленческом учете.';uk='Дані звіту можуть бути не актуальні, тому що виконані не всі етапи закриття місяця, що впливають на фінансовий результат в управлінському обліку.'");
	КонецЕсли;
