...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сегмент,"ЭтоГруппа") Тогда
		ВызватьИсключение НСтр("ru='Отчет не может быть сформирован для группы сегментов.';uk='Звіт не може бути сформований для групи сегментів.'");
	КонецЕсли;
...
Процедура СформироватьСегмент(Команда)
	...
	ПоказатьВопрос(ОбработчикЗавершения,
	               НСтр("ru='Состав сегмента будет переформирован в соответствии с настройками схемы компоновки. Продолжить?';uk='Склад сегмента буде переформовано відповідно до настройок схеми компонування. Продовжити?'"),
...
Процедура ДобавитьЭлемент(Элемент, СкомпоноватьРезультат = Ложь, ВыводитьСообщения = Ложь)
	...
	Если Запись.Выбран() Тогда
		Если ВыводитьСообщения Тогда
			Если Партнеры Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Элемент.Наименование + " " + НСтр("ru='уже включен в сегмент.';uk='вже включений в сегмент.'"))
			Иначе	
				Если ХарактеристикиНоменклатуры
					И Не Элемент.Характеристика.Пустая() Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1/%2 уже включен в сегмент';uk='%1/%2 вже включений в сегмент'"), 
					                                                                         Строка(Элемент.Номенклатура),
					                                                                         Строка(Элемент.Характеристика)); 
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 уже включен в сегмент';uk='%1 вже включений в сегмент'"), 
					                                                                         Строка(Элемент.Номенклатура)); 
				КонецЕсли;
