...
Функция ПредставлениеОпросаДляЗаголовка(РеквизитыОпрос)
	...
	Если РеквизитыОпрос.ДатаНачала <> Дата(1, 1, 1) И РеквизитыОпрос.ДатаОкончания <> Дата(1, 1, 1) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Опрос проводился в период с %1 по %2 на основании документа %3.';uk='Опитування проводилося в період з %1 по %2 на підставі документа %3.'"),
	...
	ИначеЕсли РеквизитыОпрос.ДатаНачала <> Дата(1, 1, 1) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Опрос проводился в период с %1 на основании документа %2.';uk='Опитування проводилося в період з %1 на підставі документа %2.'"),
			Формат(РеквизитыОпрос.ДатаНачала, "ДЛФ=ДД"), 
			РеквизитыОпрос.Представление);
	ИначеЕсли РеквизитыОпрос.ДатаОкончания <> Дата(1, 1, 1) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Опрос проводился в период по %1 на основании документа %2.';uk='Опитування проводилося в період з %1 на підставі документа %2.'"),
			Формат(РеквизитыОпрос.ДатаОкончания, "ДЛФ=ДД"), 
			РеквизитыОпрос.Представление);
	Иначе
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Опрос на основании документа %1.';uk='Опитування на підставі документа %1.'"),
			РеквизитыОпрос.Представление);
	КонецЕсли; 
	Возврат Результат;
