...
Функция ВыполнитьПакетЗапросовПоПартнеру(Партнер,СоответствиеЗапросыДанные)
	...
	Запрос.УстановитьПараметр("СтрокаПроект",НСтр("ru='Проекты';uk='Проєкти'"));
...
Процедура ВывестиДанныеОбороты(РезультатЗапросаОбороты, ТаблицаОтчета, Макет);
	...
	ВывестиОбластьБезПараметров("ПустаяСтрока",Макет,ТаблицаОтчета);
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Общий объем выручки от продаж клиенту - %1 %2.';uk='Загальний обсяг виторгу від продажів клієнту - %1 %2.'"),
	...
	Пока ВыборкаВалюты.Следующий() Цикл
		...
		Иначе
			...
			Если ВыборкаВалюты.Ссылка > 1 И ВыборкаВалюты.Ссылка < 5 Тогда
				ТекстЗамены = СтрЗаменить(ТекстЗамены, НСтр("ru='заявка';uk='заявка'"), НСтр("ru='заявки';uk='заявки'"));
				ТекстЗамены = СтрЗаменить(ТекстЗамены, НСтр("ru='заказ';uk='замовлення'"), НСтр("ru='заказа';uk='замовлення'"));
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Иначе
			ТекстПоВалютам =  ТекстПоВалютам + ?(ПерваяВалюта,", " + НСтр("ru='в том числе:';uk='у тому числі:'"),"") + Символы.ПС
			                                     + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" " + НСтр("ru='%1 (%2)- излишки на сумму %3 , недостачи на сумму %4';uk='%1 (%2)- надлишки на суму %3 , нестачі на суму %4'") +", ",
			                                     ВыборкаИтоги.Ссылка,
			                                     ВыборкаИтоги.Валюта,
			                                     Формат(ВыборкаИтоги.Излишки,   "ЧДЦ=2"),
			                                     Формат(ВыборкаИтоги.Недостачи, "ЧДЦ=2"));
			...
			Если Не ЗначениеЗаполнено(ВариантКлассификацииЗадолженности)
				...
				Область.Параметры.НаименованиеИнтервала = Строка(ВыборкаДетали.НаименованиеИнтервала) + " " 
					+ ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
						...
						НСтр("ru='день';uk='день'"), НСтр("ru='дня';uk='дні'"), НСтр("ru='дней';uk='днів'"), "м");
			КонецЕсли; 
