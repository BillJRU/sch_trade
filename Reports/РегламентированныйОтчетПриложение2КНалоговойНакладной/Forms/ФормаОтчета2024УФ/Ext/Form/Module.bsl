...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы                                    
	СтруктураРеквизитовФормы.мВерсияФормы = "01/10/2023";
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Иначе
	    ...
	    ТабличныйДокумент.Области.ПредупреждениеОПечати.Текст = НСтр("ru='ВНИМАНИЕ! Печать отчета не предусмотрена. Необходимо воспользоваться процедурой печати налогового документа!';uk= 'Увага! Друк звіту не передбачений. Необхідно використати процедуру друку податкового документа!'");
	КонецЕсли;
...
Процедура ВыгрузитьОтчет() Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьОтчетПослеПроверки", ЭтотОбъект);
	РезультатПроверки = ПроверитьОтчетПередВыгрузкой();
	Если РезультатПроверки.ЕстьОшибки Тогда
		ПоказатьВопрос(ОписаниеОповещения, РезультатПроверки.ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
...
&НаКлиенте
Процедура ВыгрузитьОтчетПослеПроверки(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Функция ПроверитьОтчетПередВыгрузкой()
	
	РезультатПроверки = Новый Структура("ЕстьОшибки, ТекстВопроса, МассивСообщений", Ложь, "", Новый Массив);
	
	Если РезультатПроверки.МассивСообщений.Количество() Тогда
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ДанныеШапки.Свойство("R001G1D", СтруктураДанныхНалоговыйДокумент.R001G1D);	
	ДанныеШапки.Свойство("R001G2S", СтруктураДанныхНалоговыйДокумент.R001G2S);
	ДанныеШапки.Свойство("R001G3", СтруктураДанныхНалоговыйДокумент.R001G3);
	ДанныеШапки.Свойство("R002G1D", СтруктураДанныхНалоговыйДокумент.R002G1D);
	ДанныеШапки.Свойство("R002G2S", СтруктураДанныхНалоговыйДокумент.R002G2S);
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	ЭтоФормаРЕЗ = Ложь;
	Если СтруктураДанныхНалоговыйДокумент.Свойство("HTYPR") И ЗначениеЗаполнено(СтруктураДанныхНалоговыйДокумент.HTYPR) Тогда
		ЭтоФормаРЕЗ = СтруктураДанныхНалоговыйДокумент.HTYPR = 22;
	ИначеЕсли ЗначениеЗаполнено(мНалоговыйДокумент) Тогда
		ЭтоФормаРЕЗ = мНалоговыйДокумент.ТипПричиныНевыдачиПокупателю = 22;
	КонецЕсли;

	Если ЭтоФормаРЕЗ Тогда
