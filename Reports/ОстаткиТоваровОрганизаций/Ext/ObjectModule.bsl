...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	|	ТаблицаОстатков.ВидЗапасов.НалоговоеНазначение КАК НалоговоеНазначение,
	...
	ПараметрДоступно = КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Доступно");
	Если ПараметрДоступно <> Неопределено Тогда
		ПараметрПользовательскойНастройки = КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ПараметрДоступно.ИдентификаторПользовательскойНастройки);
		Если ПараметрПользовательскойНастройки <> Неопределено Тогда
			ПараметрПользовательскойНастройки.Значение = "%ТекстОстаток%";
		Иначе
			ПараметрДоступно.Значение = "%ТекстОстаток%";
		КонецЕсли;
	КонецЕсли;      
...
Функция ПолучитьЗаголовкиПолей(ДатаКонтроля)
	...
	СтруктураЗаголовков.Вставить("ТекстОстаток", СтрШаблон(НСтр("ru='Остаток на %1 + Списано документом';uk='Залишок на %1 + Списано документом'"), Формат(ДатаАктуальности, "ДЛФ=Д")));  
	СтруктураЗаголовков.Вставить("ТекстМинимум", СтрШаблон(НСтр("ru='Минимум(Остаток на %1, Остаток на %2) + Списано документом';uk='Мінімум(Залишок на %1, Залишок на %2) + Списано документом'"), Формат(ДатаКонтроля, "ДЛФ=Д"), Формат(ДатаАктуальности, "ДЛФ=Д")));  
