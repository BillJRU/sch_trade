...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, Настройки, ПользовательскиеНастройки) Экспорт
	...
	Если КлючСхемы <> "ИзмененнаяСхема" Тогда
		...
		ДоступныеЗначения.Добавить("ПередачаМатериаловВПроизводство", НСтр("ru='Передача материалов в производство';uk='Передача матеріалів у виробництво'"));
	...
	Если КлючВарианта = "ОборачиваемостьЗапасов" 
	   И (Не Настройки.ДополнительныеСвойства.Свойство("КлючПредыдущегоВарианта")
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(НастройкаПериод.Значение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено обязательное поле ""Период"".';uk='Не заповнене обов''язкове поле ""Період"".'"), , , , Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НастройкаПериодичность.Значение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено обязательное поле ""Периодичность"".';uk='Не заповнене обов''язкове поле ""Періодичність"".'"), , , , Отказ);
	КонецЕсли;
...
Функция ТаблицаДанныхОтчета(Настройки, СхемаКомпоновки)
	...
	НаборДанныхЗапрос.Запрос = СтрЗаменить(НаборДанныхЗапрос.Запрос, "Запись", ОбщегоНазначения.ИмяЗначенияПеречисления(Периодичность));	
