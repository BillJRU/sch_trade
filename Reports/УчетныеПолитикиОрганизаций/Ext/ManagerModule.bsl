...
Процедура СформироватьОтчетУчетныеПолитикиОрганизаций(ОсновнаяОрганизация, ОрганизацииСравнения, ТаблицаОтчета, ПоказатьОтличия) Экспорт
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаНастройкиУчетаЕН(),					"ТаблицаНастройкиУчетаЕН");
...
Функция ДобавитьКомандуОтчета(КомандыОтчетов) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.УчетныеПолитикиОрганизаций) Тогда
		...
		КомандаОтчет.Представление = НСтр("ru='Учетная политика организаций';uk='Облікова політика організацій'");
...
Функция ТекстЗапросаНастройкиСистемыНалогообложения()
    ...
    |	НастройкиСистемыНалогообложенияСрезПоследних.ИнформацияОСтатусеПлательщикаНалогов КАК ИнформацияОСтатусеПлательщикаНалогов,
	|	НастройкиСистемыНалогообложенияСрезПоследних.РаздельныйУчетТоваровПоНалогообложениюНДС КАК РаздельныйУчетТоваровПоНалогообложениюНДС,
	|	НастройкиСистемыНалогообложенияСрезПоследних.РаздельныйУчетВНАПоНалогообложениюНДС КАК РаздельныйУчетВНАПоНалогообложениюНДС,
	|	ВЫБОР
...
Функция ТекстЗапросаНастройкиУчетаЕН()    
    
	ТекстЗапроса = "
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НастройкиУчетаЕНСрезПоследних.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА НастройкиУчетаЕНСрезПоследних.Организация = &ОсновнаяОрганизация
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОсновнаяОрганизация,
	|	НастройкиУчетаЕНСрезПоследних.ГруппаПлательщикаЕдиногоНалога КАК ГруппаПлательщикаЕдиногоНалога
	|ИЗ
	|	РегистрСведений.НастройкиУчетаЕН.СрезПоследних(&Период, Организация В (ВЫБРАТЬ Организация ИЗ ВтГоловныеОрганизации)) КАК
	|		НастройкиУчетаЕНСрезПоследних
	|УПОРЯДОЧИТЬ ПО
	|	ОсновнаяОрганизация
	|;
	|";
...
Функция ТекстЗапросаНастройкиУчетаНДС()
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОсновнаяОрганизация,
	|	НастройкиУчетаНДССрезПоследних.ИННПлательщикаНДС
	|ИЗ
...
Процедура ВывестиТаблицуНастройки(ТаблицаОтчета, Макет, ОсновнаяОрганизация, ОрганизацииСравнения, ТаблицаРезультата, ПоказатьОтличия)
	...
	ИмяРегистра 		= СтрЗаменить(ТаблицаРезультата.Ключ, "Таблица", "");
	МенеджерРегистра 	= Метаданные.РегистрыСведений[ИмяРегистра];
	РесурсыРегистра 	= МенеджерРегистра.Ресурсы;
	
	ИмяОбласти = "ОбластьЗаголовокТаблицы";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Область.Параметры.ЗаголовокТаблицы = МенеджерРегистра.Синоним;
	Область.Параметры.ЗаголовокТаблицыРасшифровка = ИмяРегистра;
	ТаблицаОтчета.Вывести(Область);
	...
	Для Каждого Колонка Из ТаблицаРезультата.Значение.Колонки Цикл
		Если Колонка.Имя <> "Организация" 
			...
			Область.Параметры.Настройка = РесурсыРегистра[Колонка.Имя].Синоним;
...
Процедура УстановитьНеЗадано(Область)
	...
	Область.Параметры.ЗначениеНастройки = НСтр("ru='<не задано>';uk='<не зазначено>'",ОбщегоНазначения.КодОсновногоЯзыка());
