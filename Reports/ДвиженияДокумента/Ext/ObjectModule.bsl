...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, Настройки, ПользовательскиеНастройки) Экспорт
	...
	Если ПараметрыОтчета.ДокументВладелец = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Отчет о движениях документа не предназначен для рассылки.';uk='Звіт про рух документа не призначений для розсилки.'");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.Параметры.ПредставлениеВарианта = "Расшифровка" Тогда
		ВызватьИсключение НСтр("ru='Выбранное действие в данном отчете не доступно.';uk='Вибрана дія в даному звіті не доступна.'");
	КонецЕсли;
	...
	ИначеЕсли Не Форма.Параметры.Свойство("ПараметрКоманды") Тогда
		ВызватьИсключение НСтр("ru='Для начала работы с отчетом необходимо воспользоваться соответствующей командой в форме интересующего документа.';uk='Для початку роботи зі звітом необхідно скористатися відповідною командою у формі документа, що цікавить.'");
	КонецЕсли;
...
Функция НаборДанныхДляРегистраБухгалтерии(СвойстваРегистра, МетаданныеРегистра)
	...
	ЛокализацияДебет    = НСтр("ru='Дт';uk='Дт'");
	ЛокализацияКредит   = НСтр("ru='Кт';uk='Кт'");
	ЛокализацияСубконто = НСтр("ru='Субконто';uk='Субконто'");
	ЛокализацияСчет     = НСтр("ru='Счет';uk='Рахунок'");
...
Процедура ПодготовитьГоризонтальныйВариантРегистровНакопленияИСведений_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	Если СтруктураСтандартныхРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru='Стандартные реквизиты';uk='Стандартні реквізити'"));
	...
	Если СтруктураИзмерений.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru='Измерения';uk='Виміри'"));
	...
	Если СтруктураРесурсов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru='Ресурсы';uk='Ресурси'"));
	...
	Если СтруктураРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru='Реквизиты';uk='Реквізити'"));
...
Процедура ПодготовитьГоризонтальныйВариантРегистровРасчета(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПользовательскийЗаголовок = НСтр("ru='Регистр расчета';uk='Регістр розрахунку'") + " " + ПредставлениеРегистра;
...
Процедура ПодготовитьВертикальныйВариантРегистровРасчета(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПользовательскийЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Регистр расчета %1';uk='Регістр розрахунку %1'"),
...
Процедура ПодготовитьГоризонтальныйВариантРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПользовательскийЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Регистр бухгалтерии %1';uk='Регістр бухгалтерії %1'"),
		ПредставлениеРегистра);
	...
	Для НомерСтроки = 1 По ДокументРезультат.ВысотаТаблицы Цикл
		...
		Если ГраницаПримечаний = ЭтажностьСтроки Тогда
			...
			ТекстПримечания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В этой строке: %1';uk='У цьому рядку: %1'"), СвойстваПримечания.Примечание + ".");
...
Процедура ПолучитьСвойстваРегистров(Контекст, ДокументВладелец = Неопределено)
	...
	Для Каждого Движение Из ДвиженияДокумента Цикл
		...
		Если ОбщегоНазначения.ЭтоРегистрНакопления(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru='Регистр накопления';uk='Регістр накопичення'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru='Регистр сведений';uk='Регістр відомостей'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрБухгалтерии(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru='Регистр бухгалтерии';uk='Регістр бухгалтерії'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрРасчета(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru='Регистр расчета';uk='Регістр розрахунку'");
	...
	Для Каждого СвойстваРегистра Из СвойстваРегистров Цикл 
		...
		Для Индекс = 0 По ЭтажностьСтроки - 1 Цикл
			...
			Для Каждого ВыбраннаяГруппа Из ВыбранныеГруппыПолей Цикл
				...
				Заголовок = "";
				Если ВыбраннаяГруппа.Заголовок = НСтр("ru='Стандартные реквизиты';uk='Стандартні реквізити'") Тогда
					Заголовок = НСтр("ru='Стандартный реквизит';uk='Стандартний реквізит'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("ru='Измерения';uk='Виміри'") Тогда
					Заголовок = НСтр("ru='Измерение';uk='Вимірювання'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("ru='Ресурсы';uk='Ресурси'") Тогда
					Заголовок = НСтр("ru='Ресурс';uk='Ресурс'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("ru='Реквизиты';uk='Реквізити'") Тогда
					Заголовок = НСтр("ru='Реквизит';uk='Реквізит'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("ru='Данные расчета';uk='Дані розрахунку'") Тогда
					Заголовок = НСтр("ru='Данные расчета';uk='Дані розрахунку'");
				КонецЕсли;
...
Процедура ВосстановитьОтборПоГруппировкамРегистров(Настройки)
	...
	Для Каждого ЭлементСтруктуры Из СтруктураОтчета Цикл
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			...
			НСтр("ru='Служебный отбор';uk='Службовий відбір'"),
			Истина,
