...
Процедура ПодсказкаОМобильномПриложенииНажатие(Элемент)
	...
	ПараметрыФормы = Новый Структура("ИмяМакетаОписания, НазваниеПриложения", 
		"ОписаниеМобильногоПриложенияМониторERP", НазваниеПриложения);
...
Процедура СоздатьБазовуюСтруктуруДереваПоказателей(ДеревоПоказателейЗначение, ПолученныйСоставМонитора)
	...
	Если СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоСостоянию" Тогда
		Если СтатистикаПоСтрокам.НеприемлемоеСостояние > 0 Тогда
			...
			КраткоеНаименование = НСтр("ru='Неприемлемое состояние';uk='Неприйнятний стан'");
		...
		Если СтатистикаПоСтрокам.ПриемлемоеСостояние > 0 Тогда
			...
			КраткоеНаименование = НСтр("ru='Приемлемое состояние';uk='Прийнятний стан'");
			СтруктураЗначений = Новый Структура;
			СтруктураЗначений.Вставить("КраткоеНаименование", КраткоеНаименование);
		...
		Если СтатистикаПоСтрокам.ЦельДостигнута > 0 Тогда
			...
			КраткоеНаименование = НСтр("ru='Цель достигнута';uk='Ціль досягнута'");
			СтруктураЗначений = Новый Структура;
			СтруктураЗначений.Вставить("КраткоеНаименование", КраткоеНаименование);
		...
		Если СтатистикаПоСтрокам.СостояниеНеОпределено > 0 Тогда
			...
			КраткоеНаименование = НСтр("ru='Состояние не определено';uk='Стан не визначено'");
			СтруктураЗначений = Новый Структура;
			СтруктураЗначений.Вставить("КраткоеНаименование", КраткоеНаименование);
		...
		Если СтатистикаПоСтрокам.РассчитаноСОшибками > 0 Тогда
			...
			КраткоеНаименование = НСтр("ru='Рассчитано с ошибками';uk='Розраховано з помилками'");
			СтруктураЗначений = Новый Структура;
			СтруктураЗначений.Вставить("КраткоеНаименование", КраткоеНаименование);
...
Функция ЗаполнитьОтчетНаСервере(ПараметрыСоставаМонитора)
	...
	Исключение
		ИмяСобытияЖР = НСтр("ru='Заполнение отчета ""Монитор целевых показателей""';uk='Заповнення звіту ""Монітор цільових показників""'",ОбщегоНазначения.КодОсновногоЯзыка());
