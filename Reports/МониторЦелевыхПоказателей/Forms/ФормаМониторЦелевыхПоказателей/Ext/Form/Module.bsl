...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.ИдентификаторВариантаАнализа) Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Анализируемый показатель ""%1""';uk='Аналізований показник ""%1""'"),
...
Процедура ПодсказкаОМобильномПриложенииНажатие(Элемент)
	НазваниеПриложения = НСтр("ru='BAS Монитор ERP';uk='BAS монітор ERP'");
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИмяМакетаОписания", "ОписаниеМобильногоПриложенияМониторERP");
	ПараметрыФормы.Вставить("НазваниеПриложения", НазваниеПриложения);
		
	ОткрытьФорму("ОбщаяФорма.ОписаниеМобильногоПриложения",
...
// 
// Параметры:
// 	РезультатЗакрытия - Неопределено, КодВозвратаДиалога - 
...
Процедура Печать(Команда)
	Если СоставМонитораНеопределен Тогда
		ТекстПредупреждения = НСтр("ru='Сформировать сводный отчет невозможно, т.к. монитор показателей не настроен.';uk='Сформувати зведений звіт неможливо, тому що монітор показників не налаштований.'");
		ТекстЗаголовка = НСтр("ru='Ошибка формирования сводного отчета';uk='Помилка формування зведеного звіту'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения,, ТекстЗаголовка);
	ИначеЕсли СоставМонитораПустойПоОтбору Тогда
		ТекстПредупреждения = НСтр("ru='Сформировать сводный отчет невозможно, т.к. в текущий отбор не входит ни один показатель.';uk='Сформувати зведений звіт неможливо, тому що в поточний відбір не входить жоден показник.'");
		ТекстЗаголовка = НСтр("ru='Ошибка формирования сводного отчета';uk='Помилка формування зведеного звіту'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения,, ТекстЗаголовка);
	Иначе 
	...
	Если ВариантОтображенияПоказателейСтроки = ВариантыОтображенияВариантовАнализа.Кратко Тогда
		Для НомерКолонки = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл
			ЗаголовокЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Колонка №%1';uk='Колонка № %1'"),
				НомерКолонки);
			ГруппаКолонкаЭлементов = ВертикальнаяГруппа(
				"ГруппаКолонкаЭлементовКратко" + КлючУникальностиГруппы + "__" + НомерКолонки,
				ЗаголовокЭлемента,
				РодительскаяГруппа);
		...
		Если Элементы.Найти("ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы) = Неопределено Тогда
			Для НомерКолонки = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл
				ЗаголовокЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					НомерКолонки);
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа(
					"ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы + "__" + НомерКолонки,
					ЗаголовокЭлемента,
					РодительскаяГруппа);
				ЗаполнитьЗначенияСвойств(ГруппаКолонкаЭлементов, СтильГруппыКолонкиПоказателей());
			КонецЦикла;
...
Процедура ДобавитьКомандыВариантовАнализа(ВариантАнализа, ГруппаЭлементов, ИдентификаторЭлементаФормы, ВариантОтображения)
	...
	КоманднаяПанельКомандПоказателя = Элементы.Добавить(
		...
		Тип("ГруппаФормы"),
		Элементы["Заголовок__" + ИдентификаторЭлементаФормы]);
...
Процедура ИзменитьДинамическийПараметрДляДиаграммы(ВариантАнализа, ПараметрыИсточникаДанных, ИдентификаторЭлементаФормы)
	...
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны И НЕ ОшибкаРасчета Тогда
		...
		Диаграмма = МониторингЦелевыхПоказателей.ЗаполнитьДиаграммуДляМонитора(
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы],
			ВариантАнализа,
			ИсточникДанных);
		ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = Диаграмма;
	Иначе
	...
	Если Не ИсточникДанныхВариантаАнализа.Пустой Тогда
		...
		Если ВыводитьИтогВТаблице Тогда 
			...
			ПредставлениеЧисла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ПредставлениеЧисла,
				Размерность);
			ЭтаФорма["ИтогТаблицаЗначений__" + ИдентификаторЭлементаФормы] = ПредставлениеЧисла;
			
		КонецЕсли;
		
		МониторингЦелевыхПоказателей.ЗаполнитьТаблицуДляМонитора(
			ЭтаФорма["ТаблицаЗначений__" + ИдентификаторЭлементаФормы],
			ИдентификаторЭлементаФормы,
			ВариантАнализа,
			ИсточникДанныхВариантаАнализа);
	Иначе
		Если Не ИсточникДанныхВариантаАнализа.РасчетыАктуальны Тогда
			ТекстОшибки = НСтр("ru='Данные неактуальны.';uk='Дані неактуальні.'");
		...
		ИначеЕсли ИсточникДанныхВариантаАнализа.ОшибкаРасчета Тогда
			ТекстОшибки = НСтр("ru='Ошибка при расчете значений показателя.';uk='Помилка при розрахунку значень показника.'");
			Диаграмма = МониторингЦелевыхПоказателей.ПолучитьДиаграммуПриОшибкахРасчета(
				ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы],
				ТекстОшибки);
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = Диаграмма;
		Иначе
			ТекстОшибки = НСтр("ru='Недостаточно данных для расчета показателя.';uk='Недостатньо даних для розрахунку показника.'");
			Диаграмма = МониторингЦелевыхПоказателей.ПолучитьДиаграммуПриОшибкахРасчета(
				ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы],
				ТекстОшибки);
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = Диаграмма;
...
Процедура НарисоватьДиаграмму(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	ЗаголовокРеквизита = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Диаграмма для %1';uk='Діаграма для %1'"),
		ВариантАнализа);
	...
	ЗаголовокРеквизита = НСтр("ru='Наименование показателя';uk='Найменування показника'");
	...
	// Добавим поля формы
	ЗаголовокЭлемента = НСтр("ru='Статусы';uk='Статуси'");
	...
	ПолеНаименования = Элементы.Вставить("НаименованиеПоказателя__" + ИдентификаторЭлементаФормы,
		Тип("ПолеФормы"), ГруппаЗаголовокПоказателя);
...
Процедура НарисоватьТаблицу(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны Тогда
		...
		ЗаголовокРеквизита = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Таблица для %1';uk='Таблиця для %1'"),
			ВариантАнализа);
	...
	// Добавим поля формы
	ЗаголовокЭлемента = НСтр("ru='Заголовок показателя';uk='Заголовок показника'");
	ГруппаЗаголовокПоказателя = ГоризонтальнаяГруппа("Заголовок__" + ИдентификаторЭлементаФормы,
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны И Не ОшибкаРасчета Тогда
		...
		Если ВыводитьИтогВТаблице Тогда 
			...
			ЭтаФорма["ИтогТаблицаЗначений__" + ИдентификаторЭлементаФормы] = ЗначениеЧисла;
		КонецЕсли;
...
Процедура НарисоватьКратко(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны Тогда
		ЗаголовокРеквизита = НСтр("ru='Значение показателя';uk='Значення показника'");
...
Процедура НарисоватьПоляСправкиОПоказателе(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	ЗаголовокРеквизита = НСтр("ru='Цель';uk='Ціль'");
	...
	ЗаголовокРеквизита = НСтр("ru='Целевой тренд';uk='Цільовий тренд'");
	...
	ЗаголовокРеквизита = НСтр("ru='Целевые значения';uk='Цільові значення'");
	...
	ЗаголовокРеквизита = НСтр("ru='Состояние';uk='Стан'");
	...
	ЗаголовокРеквизита = НСтр("ru='Тренд';uk='Тренд'");
	...
	ЗаголовокРеквизита = НСтр("ru='Значение';uk='Значення'");
	...
	ЗаголовокРеквизита = НСтр("ru='Ответственный';uk='Відповідальний'");
	...
	ЗаголовокРеквизита = НСтр("ru='Формула';uk='Формула'");
	...
	ЭтаФорма["ЦелевыеЗначенияЗначение__" + ИдентификаторЭлементаФормы] = СвойстваВариантаАнализа.ЦелевыеЗначения;
	...
	ЭтаФорма["ФактическоеСостояниеЗначение__" + ИдентификаторЭлементаФормы] = СписокСвойств.ПояснениеКСтатусу;
	ЭтаФорма["ФактическийТрендЗначение__" + ИдентификаторЭлементаФормы] = СписокСвойств.ПояснениеКТренду;
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны И Не ОшибкаРасчета Тогда
		...
		ЭтаФорма[ИмяРеквизитаФормы] = СокрЛП(ПредставлениеЧисла + СтрокаРазмерности);
	Иначе
		ЭтаФорма[ИмяРеквизитаФормы] = ИсточникДанных.ОписаниеОшибки;
	КонецЕсли;
...
Процедура НарисоватьЭлементыМонитораСУчетомВидаАнализаВзаимосвязаных(КоллекцияВариантовАнализа, ВидПоискаВзаимосвязанных)
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ГруппаСправка.Заголовок = НСтр("ru='Справка о показателе';uk='Довідка про показник'");
		...
		Если КоличествоНенарисованныхПоказателей = 0
		   И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
			...
			Для СчВыравнивающихЭлементов = 1 По НужноДобавитьЭлементов Цикл
				...
				ГруппаЭлементовПоказателя = Элементы.Добавить(ИмяЭлемента, Тип("ГруппаФормы"), ГруппаКолонкаЭлементов);
				ЗаполнитьЗначенияСвойств(ГруппаЭлементовПоказателя, СтильГруппыЭлементовПоказателя());
				
				ГруппаЭлементовПоказателя.Заголовок = НСтр("ru='Контейнер выравнивающего элемента';uk='Контейнер вирівнюючого елемента'");
...
Процедура СоздатьБазовуюСтруктуруЭлементовФормы(СоставМонитора, СтруктураВывода)
	...
	ЗаголовокЭлемента = НСтр("ru='Показатели';uk='Показники'");
	...
	Если РежимАнализаВзаимосвязанных Тогда
		...
		Для Сч = 1 По 2 Цикл
			ИмяЭлемента = "ГруппаКолонкаЭлементовАнализируемыйПоказатель" + "__" + Сч;
			ЗаголовокЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Колонка №%1';uk='Колонка № %1'"),
				Сч);
		...
		Если Зависимые.Количество() > 0 Тогда
			...
			ЗаголовокЭлемента = НСтр("ru='Зависимые показатели';uk='Залежні показники'");
		...
		Если Влияющие.Количество() > 0 Тогда
			...
			ЗаголовокЭлемента = НСтр("ru='Влияющие показатели';uk='Впливаючі показники'");
		...
		КонецЕсли;
	Иначе
		ЗаголовокЭлемента = НСтр("ru='Показатели (кратко)';uk='Показники (коротко)'");
		ГруппаПоказателиКратко = ВертикальнаяГруппа("ГруппаПоказателиКратко",
				ЗаголовокЭлемента,
				ГруппаПоказатели);
		
		ЗаголовокЭлемента = НСтр("ru='Показатели (подробно)';uk='Показники (детально)'");
...
Функция ПолучитьСвойстваВариантаОтчета(ВариантОтчета)
	...
	УстановитьПривилегированныйРежим(Истина);
	СвойстваВариантаОтчета = Новый Структура;
	СвойстваВариантаОтчета.Вставить("КлючВарианта", ВариантОтчета.КлючВарианта);
	СвойстваВариантаОтчета.Вставить("КлючОбъекта",  ВариантОтчета.Отчет.ПолноеИмя);
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура УстановитьВидимостьПредложенияДобавитьПоказателиВСоставМонитора(Видимость)
	...
	Если ПравоДоступа("Изменение", Метаданные.Справочники.ВариантыАнализаЦелевыхПоказателей) Тогда
		Элементы.ПояснениеЕслиМониторНеНастроен.Заголовок = ПостояннаяЧасть + Символы.ПС
			+ НСтр("ru='Для настройки используйте рабочее место ""Финансовый результат и контроллинг"" - ""Настройка доступности вариантов анализа"".';uk='Для настройки використовуйте робоче місце ""Фінансовий результат і контролінг"" - ""Настройка доступності варіантів аналізу"".'");
	Иначе
		Элементы.ПояснениеЕслиМониторНеНастроен.Заголовок = ПостояннаяЧасть + Символы.ПС
			+ НСтр("ru='Обратитесь к админитратору системы для настройки доступности вариантов анализа.';uk='Зверніться до админитратора системи для настройки доступності варіантів аналізу.'");
	КонецЕсли;
...
Функция ЗаполнитьОтчетНаСервере(ПараметрыСоставаМонитора)
	...
	Исключение  
		ЗаписьЖурналаРегистрации(НСтр("ru='Заполнение отчета ""Монитор целевых показателей""';uk='Заповнення звіту ""Монітор цільових показників""'",ОбщегоНазначения.КодОсновногоЯзыка()),
